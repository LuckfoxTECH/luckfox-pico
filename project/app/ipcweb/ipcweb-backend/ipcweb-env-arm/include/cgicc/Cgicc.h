...
public:
   ...
   */
  inline Cgicc(const Cgicc &cgi) : fEnvironment(cgi.fEnvironment) {
    operator=(cgi);
  }
   ...
   */
  inline bool operator==(const Cgicc &cgi) const {
   ...
   */
  inline bool operator!=(const Cgicc &cgi) const { return !operator==(cgi); }

#ifdef WIN32
  /* Dummy operator for MSVC++ */
  inline bool operator<(const Cgicc &cgi) const { return false; }
#endif
   ...
   */
  inline form_iterator operator[](const std::string &name) {
   ...
   * \return An iterator referring to the desired element, if found.
   */
  inline const_form_iterator operator[](const std::string &name) const {
    return getElement(name);
  }
   ...
   */
  bool getElementByValue(const std::string &value,
                         std::vector<FormEntry> &result) const;
   ...
   */
  inline const std::vector<FormEntry> &operator*() const { return fFormData; }
   ...
   * \return A vector containing all the submitted elements.
   */
  inline const std::vector<FormEntry> &getElements() const { return fFormData; }
  //@}
   ...
   */
  inline const std::vector<FormFile> &getFiles() const { return fFormFiles; }
  //@}
