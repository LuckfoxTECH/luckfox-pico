...
public:
   ...
   */
  inline bool operator!=(const HTMLElement &element) const {
    return !operator==(element);
  }

#ifdef WIN32
  /* Dummy operator for MSVC++ */
  inline bool operator<(const HTMLElement &element) const { return false; }
#endif
   ...
   */
  inline EElementType getType() const { return fType; }
  //@}
   ...
   */
  inline void setData(const std::string &data) { fData = data; }
  //@}
   ...
   */
  inline const HTMLElementList *getEmbedded() const { return fEmbedded; }
...
protected:
   ...
   */
  HTMLElement(const HTMLAttributeList *attributes, const HTMLElement *embedded,
              const std::string *data, EElementType type);

  /*!
   * \brief For subclasses only
   *
   * Returns \c true if data was specified in the constructor.
   * \return \c true if data was specified in the constructor, \c false
   * otherwise
   */
  inline bool dataSpecified() const { return fDataSpecified; }
