...
struct rsa_exception : public std::runtime_error {
  ...
  explicit rsa_exception(const char *msg) : std::runtime_error(msg) {}
};
struct ecdsa_exception : public std::runtime_error {
  explicit ecdsa_exception(const std::string &msg) : std::runtime_error(msg) {}
  explicit ecdsa_exception(const char *msg) : std::runtime_error(msg) {}
};
...
namespace helper {
inline std::string extract_pubkey_from_cert(const std::string &certstr,
                                            const std::string &pw = "") {
  // TODO: Cannot find the exact version this change happended
#if OPENSSL_VERSION_NUMBER <= 0x1000114fL
  std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(
      BIO_new_mem_buf(const_cast<char *>(certstr.data()), certstr.size()),
      BIO_free_all);
#else
  std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(
      BIO_new_mem_buf(certstr.data(), certstr.size()), BIO_free_all);
#endif
  std::unique_ptr<BIO, decltype(&BIO_free_all)> keybio(BIO_new(BIO_s_mem()),
                                                       BIO_free_all);

  std::unique_ptr<X509, decltype(&X509_free)> cert(
      PEM_read_bio_X509(certbio.get(), nullptr, nullptr,
  ...
  if (!key)
    throw rsa_exception("Error getting public key from certificate");
  ...
  if (len <= 0 || ptr == nullptr)
    throw rsa_exception("Failed to convert pubkey to pem");
  std::string res(ptr, len);
  return res;
}

inline std::shared_ptr<EVP_PKEY>
load_public_key_from_string(const std::string &key,
                            const std::string &password = "") {
  std::unique_ptr<BIO, decltype(&BIO_free_all)> pubkey_bio(BIO_new(BIO_s_mem()),
                                                           BIO_free_all);
  if (key.substr(0, 27) == "-----BEGIN CERTIFICATE-----") {
    auto epkey = helper::extract_pubkey_from_cert(key, password);
    if ((size_t)BIO_write(pubkey_bio.get(), epkey.data(), epkey.size()) !=
        epkey.size())
      throw rsa_exception("failed to load public key: bio_write failed");
  } else {
    if ((size_t)BIO_write(pubkey_bio.get(), key.data(), key.size()) !=
        key.size())
      throw rsa_exception("failed to load public key: bio_write failed");
  }

  std::shared_ptr<EVP_PKEY> pkey(PEM_read_bio_PUBKEY(pubkey_bio.get(), nullptr,
                                                     nullptr,
                                                     (void *)password.c_str()),
                                 EVP_PKEY_free);
  if (!pkey)
...
inline std::shared_ptr<EVP_PKEY>
load_private_key_from_string(const std::string &key,
                             const std::string &password = "") {
  std::unique_ptr<BIO, decltype(&BIO_free_all)> privkey_bio(
      BIO_new(BIO_s_mem()), BIO_free_all);
  if ((size_t)BIO_write(privkey_bio.get(), key.data(), key.size()) !=
      key.size())
    throw rsa_exception("failed to load private key: bio_write failed");
  std::shared_ptr<EVP_PKEY> pkey(
      PEM_read_bio_PrivateKey(privkey_bio.get(), nullptr, nullptr,
                              const_cast<char *>(password.c_str())),
      EVP_PKEY_free);
  if (!pkey)
    throw rsa_exception(
        "failed to load private key: PEM_read_bio_PrivateKey failed");
  return pkey;
}
} // namespace helper
...
struct none {
  ...
  std::string sign(const std::string &) const { return ""; }
  /// Check if the given signature is empty. JWT's with "none" algorithm should
...
struct hmacsha {
  ...
  std::string sign(const std::string &data) const {
    ...
    if (HMAC(md(), secret.data(), secret.size(),
   ...
   * \throws signature_verification_exception If the provided signature does not
   * match
   */
  void verify(const std::string &data, const std::string &signature) const {
    try {
      auto res = sign(data);
      bool matched = true;
      for (size_t i = 0; i < std::min<size_t>(res.size(), signature.size());
           i++)
...
struct rsa {
   ...
   * \param private_key RSA private key or empty string if not available. If
   * empty, signing will always fail. \param public_key_password Password to
   * decrypt public key pem. \param privat_key_password Password to decrypt
   * private key pem. \param md Pointer to hash function \param name Name of the
   * algorithm
   */
  rsa(const std::string &public_key, const std::string &private_key,
      const std::string &public_key_password,
      const std::string &private_key_password, const EVP_MD *(*md)(),
      const std::string &name)
      : md(md), alg_name(name) {
    if (!private_key.empty()) {
      pkey = helper::load_private_key_from_string(private_key,
                                                  private_key_password);
    } else if (!public_key.empty()) {
      pkey =
          helper::load_public_key_from_string(public_key, public_key_password);
    } else
      throw rsa_exception(
          "at least one of public or private key need to be present");
  }
  /**
   * Sign jwt data
   * \param data The data to sign
  ...
  std::string sign(const std::string &data) const {
#ifdef OPENSSL10
    std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_destroy)> ctx(
    ...
    if (!ctx)
      throw signature_generation_exception(
          "failed to create signature: could not create context");
    if (!EVP_SignInit(ctx.get(), md()))
      throw signature_generation_exception(
          "failed to create signature: SignInit failed");
    ...
    if (!EVP_SignUpdate(ctx.get(), data.data(), data.size()))
      throw signature_generation_exception();
    if (!EVP_SignFinal(ctx.get(), (unsigned char *)res.data(), &len,
                       pkey.get()))
      throw signature_generation_exception();
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    if (!ctx)
      throw signature_verification_exception(
          "failed to verify signature: could not create context");
    if (!EVP_VerifyInit(ctx.get(), md()))
      throw signature_verification_exception(
          "failed to verify signature: VerifyInit failed");
    if (!EVP_VerifyUpdate(ctx.get(), data.data(), data.size()))
      throw signature_verification_exception(
          "failed to verify signature: VerifyUpdate failed");
    auto res =
        EVP_VerifyFinal(ctx.get(), (const unsigned char *)signature.data(),
                        signature.size(), pkey.get());
    if (res != 1)
      throw signature_verification_exception(
          "evp verify final failed: " + std::to_string(res) + " " +
...
struct ecdsa {
   ...
   * \param private_key ECDSA private key or empty string if not available. If
   * empty, signing will always fail. \param public_key_password Password to
   * decrypt public key pem. \param privat_key_password Password to decrypt
   * private key pem. \param md Pointer to hash function \param name Name of the
   * algorithm
  ...
  ecdsa(const std::string &public_key, const std::string &private_key,
    ...
    if (!public_key.empty()) {
      ...
      if (public_key.substr(0, 27) == "-----BEGIN CERTIFICATE-----") {
        auto epkey =
            helper::extract_pubkey_from_cert(public_key, public_key_password);
        if ((size_t)BIO_write(pubkey_bio.get(), epkey.data(), epkey.size()) !=
            epkey.size())
      ...
      pkey.reset(PEM_read_bio_EC_PUBKEY(pubkey_bio.get(), nullptr, nullptr,
                 ...
                 EC_KEY_free);
      if (!pkey)
        throw ecdsa_exception(
            "failed to load public key: PEM_read_bio_EC_PUBKEY failed");
    }

    if (!private_key.empty()) {
      std::unique_ptr<BIO, decltype(&BIO_free_all)> privkey_bio(
          BIO_new(BIO_s_mem()), BIO_free_all);
      if ((size_t)BIO_write(privkey_bio.get(), private_key.data(),
                            private_key.size()) != private_key.size())
        throw rsa_exception("failed to load private key: bio_write failed");
      pkey.reset(PEM_read_bio_ECPrivateKey(
                     privkey_bio.get(), nullptr, nullptr,
                     const_cast<char *>(private_key_password.c_str())),
                 EC_KEY_free);
      if (!pkey)
        throw rsa_exception(
            "failed to load private key: PEM_read_bio_ECPrivateKey failed");
    }
    if (!pkey)
      throw rsa_exception(
          "at least one of public or private key need to be present");
  ...
  std::string sign(const std::string &data) const {
    ...
    std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)> sig(
        ECDSA_do_sign((const unsigned char *)hash.data(), hash.size(),
                      pkey.get()),
        ECDSA_SIG_free);
    if (!sig)
      throw signature_generation_exception();
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    if (ECDSA_do_verify((const unsigned char *)hash.data(), hash.size(), &sig,
                        pkey.get()) != 1)
...
private:
   ...
   */
  static std::unique_ptr<BIGNUM, decltype(&BN_free)>
  raw2bn(const std::string &raw) {
    if (static_cast<uint8_t>(raw[0]) >= 0x80) {
      std::string str(1, 0x00);
      str += raw;
      return std::unique_ptr<BIGNUM, decltype(&BN_free)>(
          BN_bin2bn((const unsigned char *)str.data(), str.size(), nullptr),
          BN_free);
    }
    return std::unique_ptr<BIGNUM, decltype(&BN_free)>(
        BN_bin2bn((const unsigned char *)raw.data(), raw.size(), nullptr),
        BN_free);
  }
...
struct pss {
  ...
  std::string sign(const std::string &data) const {
    ...
    std::unique_ptr<RSA, decltype(&RSA_free)> key(EVP_PKEY_get1_RSA(pkey.get()),
                                                  RSA_free);
    const int size = RSA_size(key.get());

    std::string padded(size, 0x00);
    if (!RSA_padding_add_PKCS1_PSS_mgf1(
            key.get(), (unsigned char *)padded.data(),
    ...
    if (RSA_private_encrypt(size, (const unsigned char *)padded.data(),
                            (unsigned char *)res.data(), key.get(),
                            RSA_NO_PADDING) < 0)
      throw signature_generation_exception(
          "failed to create signature: RSA_private_encrypt failed");
    return res;
  }
  /**
   * Check if signature is valid
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    if (!RSA_public_decrypt(
            signature.size(), (const unsigned char *)signature.data(),
            (unsigned char *)sig.data(), key.get(), RSA_NO_PADDING))
      throw signature_verification_exception("Invalid signature");

    if (!RSA_verify_PKCS1_PSS_mgf1(key.get(),
                                   (const unsigned char *)hash.data(), md(),
                                   md(), (const unsigned char *)sig.data(), -1))
      throw signature_verification_exception("Invalid signature");
  }
  /**
   * Returns the algorithm name provided to the constructor
...
struct rs256 : public rsa {
  ...
  rs256(const std::string &public_key, const std::string &private_key = "",
      ...
      : rsa(public_key, private_key, public_key_password, private_key_password,
            EVP_sha256, "RS256") {}
};
/**
 * RS384 algorithm
...
struct rs384 : public rsa {
   ...
   * private key pem.
   */
  rs384(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : rsa(public_key, private_key, public_key_password, private_key_password,
            EVP_sha384, "RS384") {}
...
struct rs512 : public rsa {
   ...
   * private key pem.
   */
  rs512(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : rsa(public_key, private_key, public_key_password, private_key_password,
            EVP_sha512, "RS512") {}
...
struct es256 : public ecdsa {
   ...
   * private key pem.
   */
  es256(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha256, "ES256") {}
};
/**
 * ES384 algorithm
 */
struct es384 : public ecdsa {
   ...
   * private key pem.
   */
  es384(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha384, "ES384") {}
};
/**
 * ES512 algorithm
 */
struct es512 : public ecdsa {
   ...
   * private key pem.
   */
  es512(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha512, "ES512") {}
};
...
struct ps256 : public pss {
   ...
   * private key pem.
   */
  ps256(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : pss(public_key, private_key, public_key_password, private_key_password,
            EVP_sha256, "PS256") {}
};
/**
 * PS384 algorithm
...
struct ps384 : public pss {
   ...
   * private key pem.
   */
  ps384(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : pss(public_key, private_key, public_key_password, private_key_password,
            EVP_sha384, "PS384") {}
...
struct ps512 : public pss {
   ...
   * private key pem.
   */
  ps512(const std::string &public_key, const std::string &private_key = "",
        const std::string &public_key_password = "",
        const std::string &private_key_password = "")
      : pss(public_key, private_key, public_key_password, private_key_password,
            EVP_sha512, "PS512") {}
...
public:
  enum class type { null, boolean, number, string, array, object, int64 };

  claim() : val() {}
#if JWT_CLAIM_EXPLICIT
  explicit claim(std::string s) : val(std::move(s)) {}
  ...
  explicit claim(const picojson::value &val) : val(val) {}
#else
  claim(std::string s) : val(std::move(s)) {}
  claim(const date &s)
      : val(int64_t(std::chrono::system_clock::to_time_t(s))) {}
  claim(const std::set<std::string> &s)
      : val(picojson::array(s.cbegin(), s.cend())) {}
  claim(const picojson::value &val) : val(val) {}
#endif
   ...
   */
  picojson::value to_json() const { return val; }
...
public:
   ...
   * \throws std::bad_cast Claim was present but not a string (Should not happen
   * in a valid token)
   */
   ...
   * \throws std::runtime_error If claim was not present
   * \throws std::bad_cast Claim was present but not a set (Should not happen in
   * a valid token)
   */
  std::set<std::string> get_audience() const {
    auto aud = get_payload_claim("aud");
    if (aud.get_type() == jwt::claim::type::string)
   ...
   * \throws std::runtime_error If claim was not present
   * \throws std::bad_cast Claim was present but not a date (Should not happen
   * in a valid token)
   */
  const date get_expires_at() const {
    return get_payload_claim("exp").as_date();
  }
  /**
   ...
   * in a valid token)
   */
  const date get_not_before() const {
    return get_payload_claim("nbf").as_date();
  }
  /**
   * Get issued at claim
   ...
   * in a valid token)
   */
  const date get_issued_at() const {
    return get_payload_claim("iat").as_date();
  }
  /**
   * Get id claim
   * \return id as string
...
public:
   ...
   */
  explicit decoded_jwt(const std::string &token) : token(token) {
    ...
    payload = payload_base64 =
        token.substr(hdr_end + 1, payload_end - hdr_end - 1);
...
public:
  ...
  /**
   * Set algorithm claim
   * You normally don't need to do this, as the algorithm is automatically set
   * if you don't change it. \param str Name of algorithm \return *this to allow
   * for method chaining
   */
   ...
   * \return *this to allow for method chaining
   */
  builder &set_audience(const std::string &aud) {
    return set_payload_claim("aud", claim(aud));
  }
  /**
   ...
   */
  template <typename T> std::string sign(const T &algo) {
...
/**
 * Verifier class used to check if a decoded token contains all claims required
 * by your application and has a valid signature.
 */
template <typename Clock> class verifier {
  struct algo_base {
    virtual ~algo_base() {}
...
public:
  ...
  verifier &expires_at_leeway(size_t leeway) {
    return with_claim("exp",
                      claim(std::chrono::system_clock::from_time_t(leeway)));
  }
  /**
   ...
   * If any of the specified audiences is not present in the token the check
   * fails. \param aud Audience to check for. \return *this to allow chaining
   */
  ...
  void verify(const decoded_jwt &jwt) const {
    ...
    auto assert_claim_eq = [](const decoded_jwt &jwt, const std::string &key,
                              const claim &c) {
      if (!jwt.has_payload_claim(key))
        throw token_verification_exception("decoded_jwt is missing " + key +
                                           " claim");
      ...
      if (c.get_type() == claim::type::int64) {
        if (c.as_date() != jc.as_date())
          throw token_verification_exception("claim " + key +
        ...
        throw token_verification_exception("internal error");
    };
      ...
      if (c.first == "exp" || c.first == "iat" || c.first == "nbf") {
        ...
        if (!jwt.has_audience())
          throw token_verification_exception(
              "token doesn't contain the required audience");
 ...
 */
template <typename Clock> verifier<Clock> verify(Clock c) {
 ...
 * \return verifier instance
 */
inline verifier<default_clock> verify() { return verify<default_clock>({}); }
