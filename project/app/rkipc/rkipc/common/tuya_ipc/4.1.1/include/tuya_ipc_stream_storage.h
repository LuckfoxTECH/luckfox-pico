...
OPERATE_RET tuya_ipc_ss_init(IN CHAR_T *base_path, IN IPC_MEDIA_INFO_S *media_setting,
                             IN UINT_T max_event_per_day, AES_HW_CBC_FUNC *aes_func);
 ...
 * \brief trigger a fixed-duration event. NOT supported now!!
 * \return OPERATE_RET
 */
 ...
 * \return OPERATE_RET
 */
OPERATE_RET tuya_ipc_ss_donwload_pre(IN UINT_T pb_idx, IN SS_FILE_TIME_TS_S *pb_file_info);
