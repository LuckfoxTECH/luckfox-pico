...
typedef enum { MBEDTLS_KW_MODE_KW = 0, MBEDTLS_KW_MODE_KWP = 1 } mbedtls_nist_kw_mode_t;
 ...
 *                  <ul><li>For KW mode: a multiple of 8 bytes between 16 and 2^57-8 inclusive.
 * </li> <li>For KWP mode: any length between 1 and 2^32-1 inclusive.</li></ul> \param[out] output
 * The buffer holding the output data. <ul><li>For KW mode: Must be at least 8 bytes larger than \p
 * in_len.</li> <li>For KWP mode: Must be at least 8 bytes larger rounded up to a multiple of 8
 * bytes for KWP (15 bytes at most).</li></ul> \param[out] out_len The number of bytes written to
 * the output buffer. \c 0 on failure. \param[in] out_size The capacity of the output buffer.
 *
 * \return          \c 0 on success.
