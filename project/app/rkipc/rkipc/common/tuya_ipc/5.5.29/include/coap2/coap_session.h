...
#define COAP_SESSION_TYPE_HELLO                                                                    \
	3 /**< server-side ephemeral session for responding to a client hello */
...
typedef struct coap_session_t {
	...
	size_t partial_write; /**< if > 0 indicates number of bytes already written from the pdu at the
	                         head of sendqueue */
	uint8_t read_header[8]; /**< storage space for header of incoming message header */
	size_t
	    partial_read; /**< if > 0 indicates number of bytes already read for an incoming message */
 ...
 * @param local_if Address of local interface. It is recommended to use NULL to let the operating
 * system choose a suitable local interface. If an address is specified, the port number should be
 * zero, which means that a free port is automatically selected.
 * @param server The server's address. If the port number is zero, the default port for the protocol
 * will be used.
 * @param proto Protocol.
 *
 * @return A new CoAP session or NULL if failed. Call coap_session_release to free.
 */
coap_session_t *coap_new_client_session(struct coap_context_t *ctx, const coap_address_t *local_if,
...
coap_session_t *coap_new_client_session_psk(struct coap_context_t *ctx,
                                            ...
                                            const coap_address_t *server, coap_proto_t proto,
                                            const char *identity, const uint8_t *key,
                                            unsigned key_len);
 ...
 * @param listen_addr    Address the endpoint will listen for incoming requests on or originate
 * outgoing requests from. Use NULL to specify that no incoming request will be accepted and use a
 * random endpoint.
 * @param proto          Protocol used on this endpoint
 */

coap_endpoint_t *coap_new_endpoint(struct coap_context_t *context,
                                   const coap_address_t *listen_addr, coap_proto_t proto);
