...
/**
 * \fn OPERATE_RET tuya_ipc_cloud_storage_event_stop_async
 * \brief report the end of event
 * \return OPERATE_RET
 */
...
/**
 * \fn OPERATE_RET tuya_ipc_cloud_storage_event_stop_immediately
 * \brief report the end of event immediately
 * \return OPERATE_RET
 */
OPERATE_RET tuya_ipc_cloud_storage_event_stop_immediately(VOID);
...
/**
 * \fn BOOL_T tuya_ipc_cloud_storage_get_status
 * \brief 获取当前云存储状态。
 * \return BOOL_T：云存储在上报过程中，返回TRUE
 */
BOOL_T tuya_ipc_cloud_storage_get_status();

/**
 * \fn BOOL_T tuya_ipc_cloud_storage_get_ready
 * \brief 当前云存储是否可用。
 * \return BOOL_T：云存储可用，返回TRUE
 */
...
/**
 * \fn VOID tuya_ipc_cloud_storage_set_audio_stat(IN CONST BOOL_T isAudioOpen)
 * \brief set audio open/close
 * \param[in] isAudioOpen:true/false
 * \return EVENT_ID unique evnet id
 */
VOID tuya_ipc_cloud_storage_set_audio_stat(IN CONST BOOL_T isAudioOpen);
...
EVENT_ID tuya_ipc_cloud_storage_event_add_record_only(ClOUD_STORAGE_EVENT_TYPE_E type,
                                                      UINT_T max_duration);
