...
static struct transform ps_trans[PS_TYPE_MAX] = {{"NO_SLEEP", PS_TYPE_NO_SLEEP},
                                                 {"MODEM_SLEEP", PS_TYPE_MODEM_SLEEP},
                                                 {"LIGHT_SLEEP", PS_TYPE_LIGHT_SLEEP},
                                                 {"DEEP_SLEEP", PS_TYPE_DEEP_SLEEP}};

static struct transform key_mgmt_trans[KEY_MGMT_MAX] = {{"NONE", KEY_MGMT_NONE},
                                                        {"WEP", KEY_MGMT_WEP},
...
static struct transform country_trans[COUNTRY_MAX] = {
    {"CHINESE", COUNTRY_CHINESE}, {"USA", COUNTRY_USA},       {"EUROPE", COUNTRY_EUROPE},
    {"JAPAN", COUNTRY_JAPAN},     {"CANADA", COUNTRY_CANADA}, {"AUSTRALIA", COUNTRY_AUSTRALIA},
    {"ISRAEL", COUNTRY_ISRAEL},   {"MEXICO", COUNTRY_MEXICO}, {"FRANCE", COUNTRY_FRANCE}};

static struct transform rate_trans[RATE_INDEX_MAX] = {
    {"1M", RATE_INDEX_B_1M},   {"2M", RATE_INDEX_B_2M},       {"5.5M", RATE_INDEX_B_5_5M},
    {"11M", RATE_INDEX_B_11M}, {"22M", RATE_INDEX_PBCC_22M},  {"33M", RATE_INDEX_PBCC_33M},
    {"6M", RATE_INDEX_A_6M},   {"9M", RATE_INDEX_A_9M},       {"12M", RATE_INDEX_A_12M},
    {"18M", RATE_INDEX_A_18M}, {"24M", RATE_INDEX_A_24M},     {"36M", RATE_INDEX_A_36M},
    {"48M", RATE_INDEX_A_48M}, {"54M", RATE_INDEX_A_54M},     {"6.5M", RATE_INDEX_N_6_5M},
    {"13M", RATE_INDEX_N_13M}, {"19.5M", RATE_INDEX_N_19_5M}, {"26M", RATE_INDEX_N_26M},
    {"39M", RATE_INDEX_N_39M}, {"52M", RATE_INDEX_N_52M},     {"58.5M", RATE_INDEX_N_58_5M},
    {"65M", RATE_INDEX_N_65M}, {"6M", RATE_INDEX_N_MCS32_6M}};
		...
		if ((!is_ap && key_mgmt_set) || (is_ap && ap_key_mgmt_set)) {
			if ((info->keyMgmt == KEY_MGMT_WPA || info->keyMgmt == KEY_MGMT_WPA2) &&
				...
				goto err;
			           ...
			           (info->keyLength != 5 && info->keyLength != 13 && info->keyLength != 10 &&
			            info->keyLength != 26)) {
		...
		if ((!is_ap && key_id_set) || (is_ap && ap_key_id_set)) {
			if ((info->keyMgmt == KEY_MGMT_WEP || info->keyMgmt == KEY_MGMT_WEP_SHARED) &&
				...
				printf("Line:%d key id %d not match key mgmt %s\n", __LINE__, info->keyId,
				       key_mgmt_transform2str(info->keyMgmt));
				goto err;
			}
		}
...
int set_ps_cmd(int fp, int argc, char *argv[]) {
	...
	if (argc < 4 || argc % 2) {
		printf("Invalid SET_POWERSAVE command: needs at least 4 arguments (ps_level<A1><A2>ps_type "
...
int smart_cfg_stop_cmd(int fp, int argc, char *argv[]) { return ioctl(fp, ATBM_SMART_STOP, NULL); }
...
int etf_start_tx_cmd(int fp, int argc, char *argv[]) {
	printf("Current channel:%d  rate:%d  len:%d  is40M:%d  greenfield:%d\n", etf_cfg_set.channel,
	       etf_cfg_set.rate, etf_cfg_set.len, etf_cfg_set.is40M, etf_cfg_set.greenfield);

	return ioctl(fp, ATBM_ETF_START_TX, (unsigned int)(&etf_cfg_set));
}

int etf_stop_tx_cmd(int fp, int argc, char *argv[]) { return ioctl(fp, ATBM_ETF_STOP_TX, NULL); }
...
int etf_stop_rx_cmd(int fp, int argc, char *argv[]) { return ioctl(fp, ATBM_ETF_STOP_RX, NULL); }

int etf_reset_rx_cmd(int fp, int argc, char *argv[]) { return ioctl(fp, ATBM_ETF_RESET_RX, NULL); }
...
int get_time_cmd(int fp, int argc, char *argv[]) {
	...
	static const char day_name[7][4] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
	static const char mon_name[12][4] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
	...
	printf("Current time:\n%s %s %02d %02d:%02d:%02d %02d\n", day_name[time.time.tm_wday],
	       mon_name[time.time.tm_mon], time.time.tm_mday, time.time.tm_hour, time.time.tm_min,
...
int force_reboot_cmd(int fp, int argc, char *argv[]) { return ioctl(fp, ATBM_FORCE_REBOOT, 0); }
...
struct COMMAND_STR command[] = {
    ...
    {"set_network", set_network_cmd, "<variable> <value> = set network variables"},
    ...
    {"set_ip", set_ip_cmd, "= set ip got from enable_network command. auto set ip default"},
    {"set_ip_auto", set_ip_auto_cmd, "<is_auto> = set ip_autoset mode"},
    {"list_filters", list_filters_cmd, "= list enabled filter"},
    {"set_filter", set_filter_cmd, "<variable> <tcp ports ...> = set tcp filter ports"},
    ...
    {"switch_channel", switch_channel_cmd, "<channel id> = switch channel id 1~14"},
    ...
    {"etf_start_rx_no_drop", etf_start_rx_no_drop_cmd, "= start etf rx without drop"},
    ...
    {"listen_itvl", listen_itvl_cmd, "<itvl> = set beacon listen interval unit:100ms"},
    {"fw_cmd", fw_at_cmd, "<at_cmd> = send fw at cmd"},
    {"fw_print", fw_print_cmd, "<enable> = set if enable fw debug print"},
    {"set_rmmod_auto", set_rmmod_auto_cmd, "<auto> = set rmmod auto mode 1 or 0"},
    {"get_rate", get_rate_cmd, "= get transmit rate"},
    {"send_ipc_data", send_ipc_data_cmd,
     "= send ipc data to server.\nserver=xxx.xxx.xxx.xxx port=xx [optional], data=xxxxxxxxxx "
    ...
    {"del_netpattern", del_netpattern_cmd, "<index> = del netpattern info [index]"},
    ...
    {"conn_switch", conn_switch_cmd, "<index> ON/OFF = switch conn[index] status"},
    {"add_wk_ssid", add_wk_ssid_cmd, "<ssid> = add wkup ssid used to wakeup wifi"},
    {"del_wk_ssid", del_wk_ssid_cmd, "<ssid> = del wkup ssid set before"},
    {"wkup_event", wkup_event_cmd,
     "TCP_PATTERN/UDP_PATTERN/WK_SSID/MAGIC_PKT/NONE = set wakeup wifi event support, NONE clear "
     "all set"},
    {"auto_reconn", auto_reconn_cmd,
     "<enable> = set wifi auto reconnect when connect lost, refer:0/1"},
    {"customer_cmd", customer_cmd_cmd, "<cmd_id> <cmd_buff> = send customer command"},
    {"customer_cert", customer_cert_save_cmd, "<type> <file_path> = save cert info to 6441"},
    ...
    {"fast_cfg_recv", fast_cfg_recv_cmd, "<variable> <value>= fast config mode recv"},
    {"fast_cfg_send", fast_cfg_send_cmd, "<variable> <value>= fast config mode send"},
...
int insmod_driver(int test_insmod) {
	...
	while (g_fp < 0) {
		printf("open /dev/atbm_ioctl fail, maybe prefetch bit is not cleared\n");
		g_fp = open("/dev/atbm_ioctl", O_RDWR);
		usleep(10 * 1000);
	}
	sem_post(&sem_status);
...
// 	pthread_create(&fd_tid, NULL, ioctl_msg_func, NULL);
