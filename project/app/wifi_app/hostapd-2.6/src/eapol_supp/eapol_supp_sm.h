...
struct eapol_ctx {
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*set_wep_key)(void *ctx, int unicast, int keyidx, const u8 *key,
                     size_t keylen);
...
static inline const u8 *eapol_sm_get_session_id(struct eapol_sm *sm,
                                                size_t *len) {
  return NULL;
}
...
static inline void eapol_sm_set_ext_pw_ctx(struct eapol_sm *sm,
                                           struct ext_password_data *ext) {}
static inline int eapol_sm_failed(struct eapol_sm *sm) { return 0; }
