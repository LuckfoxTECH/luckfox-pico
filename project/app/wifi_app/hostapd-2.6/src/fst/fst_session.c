...
#define fst_printf_session(s, level, format, ...)                              \
  fst_printf((level), "%u (0x%08x): [" MACSTR "," MACSTR "] :" format,         \
             (s)->id, (s)->data.fsts_id, MAC2STR((s)->data.old_peer_addr),     \
             MAC2STR((s)->data.new_peer_addr), ##__VA_ARGS__)

#define fst_printf_siface(s, iface, level, format, ...)                        \
  fst_printf_session((s), (level), "%s: " format, fst_iface_get_name(iface),   \
                     ##__VA_ARGS__)

#define fst_printf_sframe(s, is_old, level, format, ...)                       \
  fst_printf_siface((s), (is_old) ? (s)->data.old_iface : (s)->data.new_iface, \
...
#define foreach_fst_session(s)                                                 \
  dl_list_for_each((s), &global_sessions_list, struct fst_session,             \
                   global_sessions_lentry)
...
static void fst_session_set_state(struct fst_session *s,
  ...
  if (s->state != state) {
    ...
    fst_session_notify_ctrl(s, EVENT_FST_SESSION_STATE_CHANGED, &evext);
...
static struct fst_session *fst_find_session_in_progress(const u8 *peer_addr,
                                                        struct fst_group *g) {
  struct fst_session *s;

  foreach_fst_session(s) {
    if (s->group == g &&
        (os_memcmp(s->data.old_peer_addr, peer_addr, ETH_ALEN) == 0 ||
         os_memcmp(s->data.new_peer_addr, peer_addr, ETH_ALEN) == 0) &&
...
static void fst_session_reset_ex(struct fst_session *s,
...
static int fst_session_send_action(struct fst_session *s, Boolean old_iface,
  ...
  struct wpabuf *buf;
  u8 action;
  struct fst_iface *iface = old_iface ? s->data.old_iface : s->data.new_iface;
  ...
  if (!iface) {
    fst_printf_session(s, MSG_ERROR,
                       ...
                       old_iface ? "old" : "new", fst_action_names[action]);
    return -1;
  }
  ...
  if (!buf) {
    fst_printf_session(
        s, MSG_ERROR,
        "cannot allocate buffer of %zu bytes for FST Action '%s' sending", len,
  ...
  res = fst_iface_send_action(
      iface, old_iface ? s->data.old_peer_addr : s->data.new_peer_addr, buf);
  if (res < 0)
    fst_printf_siface(s, iface, MSG_ERROR, "failed to send FST Action '%s'",
                      fst_action_names[action]);
  else
...
static int fst_session_send_tear_down(struct fst_session *s) {
  ...
  if (!res)
    ...
    fst_printf_sframe(s, TRUE, MSG_ERROR, "failed to send FST TearDown");
...
static void fst_session_handle_setup_request(struct fst_iface *iface,
  ...
  if (frame_len < IEEE80211_HDRLEN + 1 + sizeof(*req)) {
    fst_printf_iface(iface, MSG_WARNING,
                     "FST Request dropped: too short (%zu < %zu)", frame_len,
                     IEEE80211_HDRLEN + 1 + sizeof(*req));
    return;
  }
  plen = frame_len - IEEE80211_HDRLEN - 1;
  req =
      (const struct fst_setup_req *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
  if (req->stie.element_id != WLAN_EID_SESSION_TRANSITION ||
      req->stie.length < 11) {
    fst_printf_iface(iface, MSG_WARNING, "FST Request dropped: invalid STIE");
    return;
  }
  ...
  if (plen > sizeof(*req)) {
    fst_iface_update_mb_ie(iface, mgmt->sa, (const u8 *)(req + 1),
                           plen - sizeof(*req));
    fst_printf_iface(iface, MSG_INFO, "FST Request: MB IEs updated for " MACSTR,
                     MAC2STR(mgmt->sa));
  }

  new_iface = fst_group_get_peer_other_connection(
      iface, mgmt->sa, req->stie.new_band_id, new_iface_peer_addr);
  if (!new_iface) {
    fst_printf_iface(iface, MSG_WARNING,
  ...
  fst_printf_iface(iface, MSG_INFO,
                   ...
                   fst_iface_get_name(new_iface), MAC2STR(new_iface_peer_addr));
  ...
  if (s) {
    ...
    if (fst_session_is_ready_pending(s) &&
      ...
      fst_printf_session(
          s, MSG_WARNING,
          "FST Request dropped due to MAC comparison (our MAC is " MACSTR ")",
          MAC2STR(mgmt->da));
      return;
    }
    ...
    fst_printf_session(s, MSG_WARNING, "resetting due to FST request");
    ...
    if (le_to_host32(req->stie.fsts_id) != s->data.fsts_id)
      ...
      fst_printf_session(s, MSG_WARNING,
                         "Skipping TearDown as the FST request has the same "
                         "FSTS ID as initiated");
    fst_session_set_state(s, FST_SESSION_STATE_INITIAL, &evext);
    fst_session_stt_disarm(s);
  }
  ...
  if (!s) {
    fst_printf(MSG_WARNING,
               ...
               fst_iface_get_name(iface), fst_iface_get_name(new_iface));
    return;
  }
...
static void fst_session_handle_setup_response(struct fst_session *s,
  ...
  if (plen < sizeof(*res)) {
    fst_printf_session(s, MSG_WARNING, "Too short FST Response dropped");
    return;
  }
  res =
      (const struct fst_setup_res *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
  if (res->stie.element_id != WLAN_EID_SESSION_TRANSITION ||
      res->stie.length < 11) {
    fst_printf_iface(iface, MSG_WARNING, "FST Response dropped: invalid STIE");
    return;
  }
  ...
  if (res->dialog_token != s->data.pending_setup_req_dlgt) {
    fst_printf_session(
        ...
        s->data.pending_setup_req_dlgt, res->dialog_token);
    return;
  }
  ...
  if (fst_hw_mode_to_band(hw_mode) != res->stie.new_band_id) {
    ...
    fst_session_set_state(s, FST_SESSION_STATE_INITIAL, &evext);
    fst_printf_session(s, MSG_WARNING, "invalid FST Setup parameters");
    fst_session_tear_down_setup(s);
    return;
  }

  fst_printf_session(s, MSG_INFO, "%s: FST Setup established for %s (llt=%u)",
...
static void fst_session_handle_tear_down(struct fst_session *s,
  ...
  if (plen < sizeof(*td)) {
    fst_printf_session(s, MSG_WARNING, "Too short FST Tear Down dropped");
    return;
  }
  td =
      (const struct fst_tear_down *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
...
static void fst_session_handle_ack_request(struct fst_session *s,
  ...
  if (iface != s->data.new_iface) {
    fst_printf_siface(s, iface, MSG_ERROR, "Ack received on wrong interface");
    return;
  }

  if (plen < sizeof(*req)) {
    fst_printf_session(s, MSG_WARNING, "Too short FST Ack Request dropped");
    return;
  }
  req = (const struct fst_ack_req *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);

  if (le_to_host32(req->fsts_id) != s->data.fsts_id) {
    fst_printf_siface(s, iface, MSG_WARNING, "Ack for wrong FST Setup ID (%u)",
  ...
  if (!fst_session_send_action(s, FALSE, &res, sizeof(res), NULL)) {
    ...
    fst_session_stt_disarm(s);
    fst_session_set_state(s, FST_SESSION_STATE_TRANSITION_DONE, NULL);
    fst_session_set_state(s, FST_SESSION_STATE_TRANSITION_CONFIRMED, NULL);
    fst_session_set_state(s, FST_SESSION_STATE_INITIAL, &evext);
  }
}

static void fst_session_handle_ack_response(struct fst_session *s,
                                            struct fst_iface *iface,
                                            const struct ieee80211_mgmt *mgmt,
                                            size_t frame_len) {
  ...
  if (plen < sizeof(*res)) {
    fst_printf_session(s, MSG_WARNING, "Too short FST Ack Response dropped");
    return;
  }
  res = (const struct fst_ack_res *)(((const u8 *)mgmt) + IEEE80211_HDRLEN + 1);
...
int fst_session_initiate_setup(struct fst_session *s) {
  ...
  if (s->data.new_iface == s->data.old_iface) {
    fst_printf_session(s, MSG_ERROR, "Same interface set as old and new");
    return -EINVAL;
  }
  ...
  if (_s) {
    fst_printf_session(
        s, MSG_ERROR, "There is another session in progress (old): %u", _s->id);
    return -EINVAL;
  }
  ...
  if (_s) {
    fst_printf_session(
        s, MSG_ERROR, "There is another session in progress (new): %u", _s->id);
    return -EINVAL;
  }
...
int fst_session_respond(struct fst_session *s, u8 status_code) {
  ...
  if (status_code == WLAN_STATUS_SUCCESS) {
    ...
    fst_printf_session(s, MSG_WARNING,
                       ...
                       fst_iface_get_name(s->data.old_iface), status_code);
  }
...
int fst_session_initiate_switch(struct fst_session *s) {
  ...
  if (!res) {
    ...
    fst_printf_sframe(s, FALSE, MSG_ERROR, "Cannot send FST Ack Request");
  }
...
void fst_session_handle_action(struct fst_session *s, struct fst_iface *iface,
  ...
  default:
    fst_printf_sframe(s, FALSE, MSG_ERROR, "Unsupported FST Action frame");
    break;
  }
}
...
u32 fst_session_get_id(struct fst_session *s) { return s->id; }
...
u32 fst_session_get_llt(struct fst_session *s) { return s->data.llt_ms; }
...
void fst_session_enum(struct fst_group *g, fst_session_enum_clb clb,
                      void *ctx) {
  struct fst_session *s;
...
void fst_session_on_action_rx(struct fst_iface *iface,
  ...
  if (len < IEEE80211_HDRLEN + 2 ||
    ...
    fst_printf_iface(iface, MSG_ERROR, "invalid Action frame received");
    return;
  }

  if (mgmt->u.action.u.fst_action.action <= FST_ACTION_MAX_SUPPORTED) {
    fst_printf_iface(iface, MSG_DEBUG, "FST Action '%s' received!",
                     fst_action_names[mgmt->u.action.u.fst_action.action]);
  } else {
    fst_printf_iface(iface, MSG_WARNING, "unknown FST Action (%u) received!",
...
void fst_session_global_on_iface_detached(struct fst_iface *iface) {
  ...
  foreach_fst_session(s) {
    if (fst_session_is_in_progress(s) &&
        (s->data.new_iface == iface || s->data.old_iface == iface))
...
int fst_test_req_send_fst_response(const char *params) {
  ...
  if (!fst_read_next_text_param(endp, response, sizeof(response), &endp)) {
   ...
   */
  _s = fst_find_session_in_progress(fst_session_get_peer_addr(&s, TRUE), g);
  res.dialog_token = (_s && fst_session_is_ready_pending(_s))
                         ? _s->data.pending_setup_req_dlgt
                         : g->dialog_token;
  res.status_code = status_code;
