...
static void fst_dump_mb_ies(const char *group_id, const char *ifname,
  ...
  while (s >= 2) {
    const struct multi_band_ie *mbie = (const struct multi_band_ie *)p;
    WPA_ASSERT(mbie->eid == WLAN_EID_MULTI_BAND);
    WPA_ASSERT(2 + mbie->len >= sizeof(*mbie));

    fst_printf(MSG_WARNING,
               "%s: %s: mb_ctrl=%u band_id=%u op_class=%u chan=%u bssid=" MACSTR
               " beacon_int=%u tsf_offs=[%u %u %u %u %u %u %u %u] mb_cc=0x%02x "
               "tmout=%u",
               group_id, ifname, mbie->mb_ctrl, mbie->band_id, mbie->op_class,
               mbie->chan, MAC2STR(mbie->bssid), mbie->beacon_int,
               mbie->tsf_offs[0], mbie->tsf_offs[1], mbie->tsf_offs[2],
               mbie->tsf_offs[3], mbie->tsf_offs[4], mbie->tsf_offs[5],
               mbie->tsf_offs[6], mbie->tsf_offs[7],
               mbie->mb_connection_capability, mbie->fst_session_tmout);
...
static void fst_fill_mb_ie(struct wpabuf *buf, const u8 *bssid,
                           const u8 *own_addr, enum mb_band_id band,
                           u8 channel) {
...
static unsigned fst_fill_iface_mb_ies(struct fst_iface *f, struct wpabuf *buf) {
  ...
  if (bssid) {
    ...
    while (num_modes--) {
      ...
      ret++;
      if (buf)
        fst_fill_mb_ie(buf, NULL, fst_iface_get_addr(f), b, MB_STA_CHANNEL_ALL);
    }
    return ret;
  }
}
...
static struct wpabuf *fst_group_create_mb_ie(struct fst_group *g,
  ...
  buf = wpabuf_alloc(nof_mbies * (sizeof(struct multi_band_ie) + ETH_ALEN));
  if (!buf) {
    fst_printf_iface(i, MSG_ERROR, "cannot allocate mem for %u MB IEs",
                     nof_mbies);
    return NULL;
  }
  ...
  if (!nof_ifaces_added) {
    wpabuf_free(buf);
    buf = NULL;
    fst_printf_iface(i, MSG_INFO, "cannot add MB IE: no backup ifaces");
  } else {
    fst_dump_mb_ies(fst_group_get_id(g), fst_iface_get_name(i), buf);
  }
...
static const u8 *fst_mbie_get_peer_addr_for_band(const struct wpabuf *mbies,
  ...
  while (s >= 2) {
    ...
    if (mbie->len < sizeof(*mbie) - 2 || mbie->len > s - 2) {
      fst_printf(MSG_INFO, "invalid mbie len %d", mbie->len);
      return NULL;
    }
...
fst_group_get_peer_other_connection_1(struct fst_iface *iface,
  ...
  if (!tmp_peer_addr) {
    fst_printf(MSG_INFO, "couldn't extract other peer addr from mbies");
    return NULL;
  }
  ...
  foreach_fst_group_iface(fst_iface_get_group(iface), other_iface) {
    if (other_iface == iface || band_id != fst_iface_get_band_id(other_iface))
      continue;
...
fst_group_get_peer_other_connection_2(struct fst_iface *iface,
  ...
  foreach_fst_group_iface(fst_iface_get_group(iface), other_iface) {
    if (other_iface == iface || band_id != fst_iface_get_band_id(other_iface))
      continue;
    cur_peer_addr = fst_iface_get_peer_first(other_iface, &ctx, TRUE);
    for (; cur_peer_addr;
         cur_peer_addr = fst_iface_get_peer_next(other_iface, &ctx, TRUE)) {
      cur_mbie = fst_iface_get_peer_mb_ie(other_iface, cur_peer_addr);
      if (!cur_mbie)
        continue;
      this_peer_addr = fst_mbie_get_peer_addr_for_band(cur_mbie, this_band_id);
      if (!this_peer_addr)
        continue;
      if (os_memcmp(this_peer_addr, peer_addr, ETH_ALEN) == 0) {
        os_memcpy(other_peer_addr, cur_peer_addr, ETH_ALEN);
        return other_iface;
      }
    }
  }
 ...
 */
struct fst_iface *fst_group_get_peer_other_connection(struct fst_iface *iface,
                                                      const u8 *peer_addr,
                                                      u8 band_id,
                                                      u8 *other_peer_addr) {
  struct fst_iface *other_iface;
  ...
  other_iface = fst_group_get_peer_other_connection_1(iface, peer_addr, band_id,
                                                      other_peer_addr);
  if (other_iface) {
    fst_printf(MSG_DEBUG, "found by method #1. %s:" MACSTR,
               fst_iface_get_name(other_iface), MAC2STR(other_peer_addr));
    return other_iface;
  }

  other_iface = fst_group_get_peer_other_connection_2(iface, peer_addr, band_id,
                                                      other_peer_addr);
  if (other_iface) {
