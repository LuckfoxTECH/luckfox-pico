...
static Boolean format_session_state_extra(const union fst_event_extra *extra,
  ...
  len = os_snprintf(buffer, size,
                    FST_CES_PNAME_REASON "=%s " FST_CES_PNAME_REJECT_CODE
                                         "=%s " FST_CES_PNAME_INITIATOR "=%s",
                    fst_reason_name(ss->extra.to_initial.reason), reject_str,
                    initiator);
...
static void fst_ctrl_iface_notify(struct fst_iface *f, u32 session_id,
  ...
  case EVENT_FST_IFACE_STATE_CHANGED:
    ...
    wpa_msg_global_only(f->iface_obj.ctx, MSG_INFO,
                        FST_CTRL_EVENT_IFACE " %s " FST_CEI_PNAME_IFNAME
                                             "=%s " FST_CEI_PNAME_GROUP "=%s",
  ...
  case EVENT_PEER_STATE_CHANGED:
    ...
    ps = &extra->peer_state;
    wpa_msg_global_only(
        fst_iface_get_wpa_obj_ctx(f), MSG_INFO,
        FST_CTRL_EVENT_PEER " %s " FST_CEP_PNAME_IFNAME
                            "=%s " FST_CEP_PNAME_ADDR "=" MACSTR,
        ps->connected ? FST_CEP_PNAME_CONNECTED : FST_CEP_PNAME_DISCONNECTED,
        ps->ifname, MAC2STR(ps->addr));
    break;
  case EVENT_FST_SESSION_STATE_CHANGED:
    if (!extra)
      return;
    if (!format_session_state_extra(extra, extra_str, sizeof(extra_str))) {
      fst_printf(MSG_ERROR, "CTRL: Cannot format STATE_CHANGE extra");
      extra_str[0] = 0;
    }
    ss = &extra->session_state;
    wpa_msg_global_only(
        ...
        " " FST_CES_PNAME_SESSION_ID "=%u " FST_CES_PNAME_EVT_TYPE
        "=%s " FST_CES_PNAME_OLD_STATE "=%s " FST_CES_PNAME_NEW_STATE "=%s %s",
        session_id, fst_session_event_type_name(event_type),
        fst_session_state_name(ss->old_state),
        fst_session_state_name(ss->new_state), extra_str);
    break;
  case EVENT_FST_ESTABLISHED:
  case EVENT_FST_SETUP:
    wpa_msg_global_only(fst_iface_get_wpa_obj_ctx(f), MSG_INFO,
                        FST_CTRL_EVENT_SESSION " " FST_CES_PNAME_SESSION_ID
                                               "=%u " FST_CES_PNAME_EVT_TYPE
                                               "=%s",
                        session_id, fst_session_event_type_name(event_type));
    break;
  }
...
static int session_get(const char *session_id, char *buf, size_t buflen) {
  ...
  return os_snprintf(
      buf, buflen,
      FST_CSG_PNAME_OLD_PEER_ADDR
      "=" MACSTR "\n" FST_CSG_PNAME_NEW_PEER_ADDR "=" MACSTR
      "\n" FST_CSG_PNAME_NEW_IFNAME "=%s\n" FST_CSG_PNAME_OLD_IFNAME
      "=%s\n" FST_CSG_PNAME_LLT "=%u\n" FST_CSG_PNAME_STATE "=%s\n",
      MAC2STR(old_peer_addr), MAC2STR(new_peer_addr),
      new_iface ? fst_iface_get_name(new_iface) : FST_CTRL_PVAL_NONE,
      old_iface ? fst_iface_get_name(old_iface) : FST_CTRL_PVAL_NONE,
      fst_session_get_llt(s), fst_session_state_name(fst_session_get_state(s)));
}
...
static int session_add(const char *group_id, char *buf, size_t buflen) {
  ...
  if (!g) {
    fst_printf(MSG_WARNING, "CTRL: Cannot find group '%s'", group_id);
    return os_snprintf(buf, buflen, "FAIL\n");
  }

  s = fst_session_create(g);
  if (!s) {
    fst_printf(MSG_ERROR, "CTRL: Cannot create session for group '%s'",
               group_id);
    return os_snprintf(buf, buflen, "FAIL\n");
  }
...
static int session_respond(const char *session_id, char *buf, size_t buflen) {
  ...
  if (!os_strcasecmp(p, FST_CS_PVAL_RESPONSE_ACCEPT)) {
    ...
    fst_printf(MSG_WARNING, "CTRL: session %u: unknown response status: %s", id,
               p);
    return os_snprintf(buf, buflen, "FAIL\n");
  }

  if (fst_session_respond(s, status_code)) {
    fst_printf(MSG_WARNING, "CTRL: Cannot respond to session %u", id);
    return os_snprintf(buf, buflen, "FAIL\n");
  }
...
static int session_transfer(const char *session_id, char *buf, size_t buflen) {
  ...
  if (fst_session_initiate_switch(s)) {
    fst_printf(MSG_WARNING, "CTRL: Cannot initiate ST for session %u", id);
    return os_snprintf(buf, buflen, "FAIL\n");
  }
...
static int session_teardown(const char *session_id, char *buf, size_t buflen) {
  ...
  if (fst_session_tear_down_setup(s)) {
    fst_printf(MSG_WARNING, "CTRL: Cannot tear down session %u", id);
    return os_snprintf(buf, buflen, "FAIL\n");
  }
...
static int test_request(const char *request, char *buf, size_t buflen) {
  ...
  if (!os_strncasecmp(p, FST_CTR_SEND_SETUP_REQUEST,
    ...
    ret = fst_test_req_send_fst_request(p +
                                        os_strlen(FST_CTR_SEND_SETUP_REQUEST));
...
static int get_peer_mbies(const char *params, char *buf, size_t buflen) {
  ...
  return wpa_snprintf_hex(buf, buflen, wpabuf_head(mbies), wpabuf_len(mbies));
...
static int list_ifaces(const char *group_id, char *buf, size_t buflen) {
  ...
  foreach_fst_group_iface(g, f) {
    ...
    res = os_snprintf(buf + ret, buflen - ret, "%s|" MACSTR "|%u|%u\n",
                      fst_iface_get_name(f), MAC2STR(iface_addr),
                      fst_iface_get_priority(f), fst_iface_get_llt(f));
    if (os_snprintf_error(buflen - ret, res))
      break;
    ret += res;
  }
  ...
  foreach_fst_group(g) {
    foreach_fst_group_iface(g, f) {
      if (fst_iface_is_connected(f, addr, TRUE)) {
        ret += print_band(num++, f, addr, buf + ret, buflen - ret);
      }
    }
  }
...
int fst_ctrl_iface_receive(const char *cmd, char *reply, size_t reply_size) {
  ...
  } commands[] = {{FST_CMD_LIST_GROUPS, 0, list_groups},
...
int fst_read_peer_addr(const char *mac, u8 *peer_addr) {
  if (hwaddr_aton(mac, peer_addr)) {
    fst_printf(MSG_WARNING, "Bad peer_mac %s: invalid addr string", mac);
    return -1;
  }

  if (is_zero_ether_addr(peer_addr) || is_multicast_ether_addr(peer_addr)) {
    fst_printf(MSG_WARNING, "Bad peer_mac %s: not a unicast addr", mac);
    return -1;
  }
...
int fst_parse_attach_command(const char *cmd, char *ifname, size_t ifname_size,
  ...
  if (pos) {
    ...
    if (*pos == '=') {
      val = fst_read_next_int_param(pos + 1, &is_valid, &endp);
      if (is_valid)
        cfg->llt = val;
    }
  }
...
int fst_parse_detach_command(const char *cmd, char *ifname,
                             size_t ifname_size) {
  char *endp;
