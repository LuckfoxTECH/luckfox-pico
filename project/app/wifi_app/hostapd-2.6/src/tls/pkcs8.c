...
struct crypto_private_key *pkcs8_key_import(const u8 *buf, size_t len) {
  ...
  return (struct crypto_private_key *)crypto_rsa_import_private_key(hdr.payload,
                                                                    hdr.length);
}

struct crypto_private_key *pkcs8_enc_key_import(const u8 *buf, size_t len,
                                                const char *passwd) {
  struct asn1_hdr hdr;
