...
int tls_derive_keys(struct tlsv1_client *conn, const u8 *pre_master_secret,
 ...
 */
u8 *tlsv1_client_handshake(struct tlsv1_client *conn, const u8 *in_data,
                           size_t in_len, size_t *out_len, u8 **appl_data,
  ...
  while (pos < end) {
    ...
    while (in_pos < in_end) {
      ...
      if (tlsv1_client_process_handshake(conn, ct, in_pos, &in_msg_len,
                                         appl_data, appl_data_len) < 0)
        goto failed;
      in_pos += in_msg_len;
    }
 ...
 */
int tlsv1_client_encrypt(struct tlsv1_client *conn, const u8 *in_data,
...
struct wpabuf *tlsv1_client_decrypt(struct tlsv1_client *conn,
  ...
  while (pos < in_end) {
    ...
    if (ct == TLS_CONTENT_TYPE_ALERT) {
      ...
      wpa_printf(MSG_DEBUG, "TLSv1: Received alert %d:%d", out_pos[0],
                 out_pos[1]);
 ...
 */
void tlsv1_client_global_deinit(void) { crypto_global_deinit(); }
