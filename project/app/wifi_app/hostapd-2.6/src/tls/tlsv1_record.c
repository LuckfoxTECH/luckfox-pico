...
int tlsv1_record_set_cipher_suite(struct tlsv1_record_layer *rl,
  ...
  wpa_printf(MSG_DEBUG, "TLSv1: Selected cipher suite: 0x%04x", cipher_suite);
...
int tlsv1_record_change_write_cipher(struct tlsv1_record_layer *rl) {
  ...
  if (rl->cipher_alg != CRYPTO_CIPHER_NULL) {
    rl->write_cbc = crypto_cipher_init(rl->cipher_alg, rl->write_iv,
                                       rl->write_key, rl->key_material_len);
...
int tlsv1_record_change_read_cipher(struct tlsv1_record_layer *rl) {
  ...
  if (rl->cipher_alg != CRYPTO_CIPHER_NULL) {
    rl->read_cbc = crypto_cipher_init(rl->cipher_alg, rl->read_iv, rl->read_key,
                                      rl->key_material_len);
    if (rl->read_cbc == NULL) {
      wpa_printf(MSG_DEBUG, "TLSv1: Failed to initialize "
                            "cipher");
...
int tlsv1_record_send(struct tlsv1_record_layer *rl, u8 content_type, u8 *buf,
  ...
  *pos++ = content_type;
  ...
  if (rl->write_cipher_suite != TLS_NULL_WITH_NULL_NULL) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "TLSv1: Record Layer - Write HMAC", pos, clen);
    pos += clen;
    if (rl->iv_size) {
    ...
    if (crypto_cipher_encrypt(rl->write_cbc, cpayload, cpayload,
                              pos - cpayload) < 0)
      return -1;
  }
 ...
 * layer header.
 */
int tlsv1_record_receive(struct tlsv1_record_layer *rl, const u8 *in_data,
  ...
  if (in_len < TLS_RECORD_HEADER_LEN) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "TLSv1: Record Layer - Received", in_data, in_len);
    return 0;
  }
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "length %d",
             ct, in_data[1], in_data[2], (int)rlen);
  ...
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: Record Layer - Received", in_data, rlen);

  if (ct != TLS_CONTENT_TYPE_HANDSHAKE &&
      ct != TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC &&
      ct != TLS_CONTENT_TYPE_ALERT && ct != TLS_CONTENT_TYPE_APPLICATION_DATA) {
    wpa_printf(MSG_DEBUG,
               "TLSv1: Ignore record with unknown "
               "content type 0x%x",
               ct);
  ...
  if (rl->read_cipher_suite != TLS_NULL_WITH_NULL_NULL) {
    size_t plen;
    if (crypto_cipher_decrypt(rl->read_cbc, in_data, out_data, in_len) < 0) {
    ...
    if (rl->iv_size) {
      ...
      if (rl->tls_version >= TLS_VERSION_1_1) {
        ...
        os_memmove(out_data, out_data + rl->iv_size, plen - rl->iv_size);
        plen -= rl->iv_size;
      }
      ...
      for (i = plen - padlen - 1; i < plen - 1; i++) {
        if (out_data[i] != padlen) {
          wpa_hexdump(MSG_DEBUG,
                      ...
                      "received record",
                      out_data + plen - padlen - 1, padlen + 1);
          force_mac_error = 1;
          goto check_mac;
        }
      }
  ...
  check_mac:
    ...
    if (hlen != rl->hash_size ||
        os_memcmp_const(hash, out_data + plen, hlen) != 0 || force_mac_error) {
      wpa_printf(MSG_DEBUG,
