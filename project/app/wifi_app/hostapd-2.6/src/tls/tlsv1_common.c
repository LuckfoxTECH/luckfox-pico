...
static const struct tls_cipher_suite tls_cipher_suites[] = {
    {TLS_NULL_WITH_NULL_NULL, TLS_KEY_X_NULL, TLS_CIPHER_NULL, TLS_HASH_NULL},
    {TLS_RSA_WITH_RC4_128_MD5, TLS_KEY_X_RSA, TLS_CIPHER_RC4_128, TLS_HASH_MD5},
    {TLS_RSA_WITH_RC4_128_SHA, TLS_KEY_X_RSA, TLS_CIPHER_RC4_128, TLS_HASH_SHA},
    {TLS_RSA_WITH_DES_CBC_SHA, TLS_KEY_X_RSA, TLS_CIPHER_DES_CBC, TLS_HASH_SHA},
    {TLS_RSA_WITH_3DES_EDE_CBC_SHA, TLS_KEY_X_RSA, TLS_CIPHER_3DES_EDE_CBC,
     TLS_HASH_SHA},
    ...
    {TLS_DH_anon_WITH_RC4_128_MD5, TLS_KEY_X_DH_anon, TLS_CIPHER_RC4_128,
     TLS_HASH_MD5},
    {TLS_DH_anon_WITH_DES_CBC_SHA, TLS_KEY_X_DH_anon, TLS_CIPHER_DES_CBC,
     TLS_HASH_SHA},
    ...
    {TLS_DH_anon_WITH_AES_256_CBC_SHA, TLS_KEY_X_DH_anon,
     TLS_CIPHER_AES_256_CBC, TLS_HASH_SHA},
    {TLS_RSA_WITH_AES_128_CBC_SHA256, TLS_KEY_X_RSA, TLS_CIPHER_AES_128_CBC,
     TLS_HASH_SHA256},
    {TLS_RSA_WITH_AES_256_CBC_SHA256, TLS_KEY_X_RSA, TLS_CIPHER_AES_256_CBC,
     TLS_HASH_SHA256},
...
static const struct tls_cipher_data tls_ciphers[] = {
    {TLS_CIPHER_NULL, TLS_CIPHER_STREAM, 0, 0, 0, CRYPTO_CIPHER_NULL},
    {TLS_CIPHER_IDEA_CBC, TLS_CIPHER_BLOCK, 16, 16, 8, CRYPTO_CIPHER_NULL},
    {TLS_CIPHER_RC2_CBC_40, TLS_CIPHER_BLOCK, 5, 16, 0, CRYPTO_CIPHER_ALG_RC2},
    {TLS_CIPHER_RC4_40, TLS_CIPHER_STREAM, 5, 16, 0, CRYPTO_CIPHER_ALG_RC4},
    {TLS_CIPHER_RC4_128, TLS_CIPHER_STREAM, 16, 16, 0, CRYPTO_CIPHER_ALG_RC4},
    {TLS_CIPHER_DES40_CBC, TLS_CIPHER_BLOCK, 5, 8, 8, CRYPTO_CIPHER_ALG_DES},
    {TLS_CIPHER_DES_CBC, TLS_CIPHER_BLOCK, 8, 8, 8, CRYPTO_CIPHER_ALG_DES},
...
int tls_parse_cert(const u8 *buf, size_t len, struct crypto_public_key **pk) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: Parse ASN.1 DER certificate", buf, len);
...
#ifdef CONFIG_TLSV12
  if (ver >= TLS_VERSION_1_2) {
    tls_prf_sha256(secret, secret_len, label, seed, seed_len, out, outlen);
    return 0;
  }
#endif /* CONFIG_TLSV12 */
...
int tls_verify_signature(u16 tls_version, struct crypto_public_key *pk,
                         const u8 *data, size_t data_len, const u8 *pos,
                         size_t len, u8 *alert) {
  u8 *buf;
  ...
  if (end - pos > slen) {
    wpa_hexdump(MSG_MSGDUMP, "Additional data after Signature", pos + slen,
                end - pos - slen);
    end = pos + slen;
  }
  ...
  if (crypto_public_key_decrypt_pkcs1(pk, pos, end - pos, buf, &buflen) < 0) {
  ...
  wpa_hexdump_key(MSG_MSGDUMP, "TLSv1: Decrypted Signature", decrypted, buflen);
  ...
  if (buflen != data_len || os_memcmp_const(decrypted, data, data_len) != 0) {
    wpa_printf(MSG_DEBUG, "TLSv1: Invalid Signature in CertificateVerify - did "
                          "not match calculated hash");
    os_free(buf);
    *alert = TLS_ALERT_DECRYPT_ERROR;
    return -1;
  }
