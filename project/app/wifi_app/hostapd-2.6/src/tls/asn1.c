...
struct asn1_oid asn1_sha1_oid = {.oid = {1, 3, 14, 3, 2, 26}, .len = 6};

struct asn1_oid asn1_sha256_oid = {.oid = {2, 16, 840, 1, 101, 3, 4, 2, 1},
                                   .len = 9};
...
void asn1_oid_to_str(const struct asn1_oid *oid, char *buf, size_t len) {
  ...
  for (i = 0; i < oid->len; i++) {
    ret = os_snprintf(pos, buf + len - pos, "%s%lu", i == 0 ? "" : ".",
                      oid->oid[i]);
