...
static int pkcs5_is_oid(struct asn1_oid *oid, unsigned long alg) {
  return oid->len == 7 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ &&
         oid->oid[5] == 5 /* pkcs-5 */ && oid->oid[6] == alg;
}

static int enc_alg_is_oid(struct asn1_oid *oid, unsigned long alg) {
  return oid->len == 6 && oid_is_rsadsi(oid) &&
         oid->oid[4] == 3 /* encryptionAlgorithm */ && oid->oid[5] == alg;
}

static int pkcs12_is_pbe_oid(struct asn1_oid *oid, unsigned long alg) {
  return oid->len == 8 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ &&
         oid->oid[5] == 12 /* pkcs-12 */ &&
         oid->oid[6] == 1 /* pkcs-12PbeIds */ && oid->oid[7] == alg;
}
...
static int pkcs5_get_params_pbes2(struct pkcs5_params *params, const u8 *pos,
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG,
               "PKCS #5: Expected SEQUENCE (keyDerivationFunc) - found class "
               "%d tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG, "PKCS #5: PBES2 keyDerivationFunc algorithm %s", obuf);
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG,
               "PKCS #5: Expected OCTET STRING (salt.specified) - found class "
               "%d tag 0x%x size %d",
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG, "PKCS #5: Expected INTEGER - found class %d tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  if (hdr.length == 1) {
  ...
  wpa_printf(MSG_DEBUG, "PKCS #5: iterationCount=0x%x", params->iter_count);
  ...
  if (asn1_get_next(pos, enc_alg_end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #5: Expected SEQUENCE (encryptionScheme) - found class %d "
               "tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  asn1_oid_to_str(&oid, obuf, sizeof(obuf));
  wpa_printf(MSG_DEBUG, "PKCS #5: PBES2 encryptionScheme algorithm %s", obuf);
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      ...
      hdr.length != 8) {
    wpa_printf(MSG_DEBUG,
               "PKCS #5: Expected OCTET STRING (SIZE(8)) (IV) - found class %d "
               "tag 0x%x size %d",
               hdr.class, hdr.tag, hdr.length);
    return -1;
  }
...
pkcs5_crypto_init_pbes2(struct pkcs5_params *params, const char *passwd) {
  ...
  if (params->enc_alg != PBES2_ENC_ALG_DES_EDE3_CBC || params->iv_len != 8)
    return NULL;

  wpa_hexdump_ascii_key(MSG_DEBUG, "PKCS #5: PBES2 password for PBKDF2", passwd,
                        os_strlen(passwd));
  wpa_hexdump(MSG_DEBUG, "PKCS #5: PBES2 salt for PBKDF2", params->salt,
              params->salt_len);
  wpa_printf(MSG_DEBUG, "PKCS #5: PBES2 PBKDF2 iterations: %u",
             params->iter_count);
  if (pbkdf2_sha1(passwd, params->salt, params->salt_len, params->iter_count,
                  key, sizeof(key)) < 0)
    return NULL;
  wpa_hexdump_key(MSG_DEBUG, "PKCS #5: DES EDE3 key", key, sizeof(key));
  wpa_hexdump(MSG_DEBUG, "PKCS #5: DES IV", params->iv, params->iv_len);

  return crypto_cipher_init(CRYPTO_CIPHER_ALG_3DES, params->iv, key,
                            sizeof(key));
}
...
pkcs12_crypto_init_sha1(struct pkcs5_params *params, const char *passwd) {
  ...
  if (pkcs12_key_gen(pw, pw_len, params->salt, params->salt_len, PKCS12_ID_ENC,
                     params->iter_count, sizeof(key), key) < 0 ||
      pkcs12_key_gen(pw, pw_len, params->salt, params->salt_len, PKCS12_ID_IV,
                     params->iter_count, sizeof(iv), iv) < 0) {
    os_free(pw);
    return NULL;
  }
...
u8 *pkcs5_decrypt(const u8 *enc_alg, size_t enc_alg_len, const u8 *enc_data,
                  size_t enc_data_len, const char *passwd, size_t *data_len) {
  ...
  for (i = enc_data_len - pad; i < enc_data_len; i++) {
    if (eb[i] != pad) {
      wpa_hexdump(MSG_INFO, "PKCS #5: Invalid PS", eb + enc_data_len - pad,
                  pad);
      os_free(eb);
      return NULL;
    }
  ...
  wpa_hexdump_key(MSG_MSGDUMP, "PKCS #5: message M (encrypted key)", eb,
                  enc_data_len - pad);
