...
void p2p_copy_filter_devname(char *dst, size_t dst_len, const void *src,
                             size_t src_len) {
  size_t i;
...
static int p2p_parse_attribute(u8 id, const u8 *data, u16 len,
  ...
  case P2P_ATTR_DEVICE_ID:
    ...
    msg->device_id = data;
    wpa_printf(MSG_DEBUG, "P2P: * Device ID " MACSTR, MAC2STR(msg->device_id));
    break;
  ...
  case P2P_ATTR_CHANNEL_LIST:
    ...
    wpa_hexdump(MSG_MSGDUMP, "P2P: Channel List", msg->channel_list,
  ...
  case P2P_ATTR_DEVICE_INFO:
    ...
    if (nlen > data + len - pos || nlen > WPS_DEV_NAME_MAX_LEN) {
      wpa_printf(MSG_DEBUG,
                 "P2P: Invalid Device Name "
                 "length %u (buf len %d)",
                 nlen, (int)(data + len - pos));
      return -1;
    }
    p2p_copy_filter_devname(msg->device_name, sizeof(msg->device_name), pos,
                            nlen);
    wpa_printf(
        MSG_DEBUG,
        ...
        wps_dev_type_bin2str(msg->pri_dev_type, devtype, sizeof(devtype)),
  ...
  case P2P_ATTR_GROUP_ID:
    ...
    wpa_printf(MSG_DEBUG, "P2P: * P2P Group ID: Device Address " MACSTR,
               MAC2STR(msg->group_id));
  ...
  case P2P_ATTR_INVITATION_FLAGS:
    ...
    wpa_printf(MSG_DEBUG, "P2P: * Invitation Flags: bitmap 0x%x", data[0]);
    break;
  case P2P_ATTR_MANAGEABILITY:
    ...
    wpa_printf(MSG_DEBUG, "P2P: * Manageability: bitmap 0x%x", data[0]);
    break;
  ...
  case P2P_ATTR_SERVICE_HASH:
    if (len < P2PS_HASH_LEN) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Service Hash (length %u)", len);
      return -1;
    }
  ...
  case P2P_ATTR_SESSION_INFORMATION_DATA:
    ...
    wpa_printf(MSG_DEBUG, "P2P: * Service Instance: %u bytes - %p", len, data);
    break;
  case P2P_ATTR_CONNECTION_CAPABILITY:
    if (len < 1) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Connection Capability (length %u)",
                 len);
      return -1;
    }
    msg->conn_cap = data;
    wpa_printf(MSG_DEBUG, "P2P: * Connection Capability: 0x%x", *msg->conn_cap);
    break;
  case P2P_ATTR_ADVERTISEMENT_ID:
    if (len < 10) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Advertisement ID (length %u)", len);
      return -1;
    }
    msg->adv_id = data;
    msg->adv_mac = &data[sizeof(u32)];
    wpa_printf(MSG_DEBUG, "P2P: * Advertisement ID %x", WPA_GET_LE32(data));
    break;
  case P2P_ATTR_ADVERTISED_SERVICE:
    if (len < 8) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Service Instance (length %u)", len);
      return -1;
    }
    ...
    if (len <= 255 + 8) {
      ...
      wpa_printf(MSG_DEBUG, "P2P: * Service Instance: %p", data);
    }
    break;
  case P2P_ATTR_SESSION_ID:
    if (len < sizeof(u32) + ETH_ALEN) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Session ID Info (length %u)", len);
      return -1;
    }
    msg->session_id = data;
    msg->session_mac = &data[sizeof(u32)];
    wpa_printf(MSG_DEBUG, "P2P: * Session ID: %x " MACSTR, WPA_GET_LE32(data),
               MAC2STR(msg->session_mac));
    break;
  case P2P_ATTR_FEATURE_CAPABILITY:
    if (!len) {
      wpa_printf(MSG_DEBUG, "P2P: Too short Feature Capability (length %u)",
                 len);
      return -1;
    }
  ...
  case P2P_ATTR_PERSISTENT_GROUP: {
    if (len < ETH_ALEN || len > ETH_ALEN + SSID_MAX_LEN) {
      wpa_printf(MSG_DEBUG, "P2P: Invalid Persistent Group Info (length %u)",
                 len);
      return -1;
    }
    ...
    wpa_printf(MSG_DEBUG, "P2P: * Persistent Group: " MACSTR " %s",
               ...
               wpa_ssid_txt(msg->persistent_ssid, msg->persistent_ssid_len));
    break;
  }
  default:
    wpa_printf(MSG_DEBUG,
...
int p2p_parse_p2p_ie(const struct wpabuf *buf, struct p2p_message *msg) {
  ...
  while (pos < end) {
    ...
    pos += 2;
    wpa_printf(MSG_DEBUG, "P2P: Attribute %d length %u", id, attr_len);
    if (attr_len > end - pos) {
      wpa_printf(MSG_DEBUG,
...
static int p2p_parse_wps_ie(const struct wpabuf *buf, struct p2p_message *msg) {
  ...
  if (attr.dev_password_id) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: Device Password ID: %d", msg->dev_password_id);
  ...
  if (attr.primary_dev_type) {
    ...
    wpa_printf(
        MSG_DEBUG, "P2P: Primary Device Type (WPS): %s",
        wps_dev_type_bin2str(msg->wps_pri_dev_type, devtype, sizeof(devtype)));
  }
...
int p2p_parse_ies(const u8 *data, size_t len, struct p2p_message *msg) {
  ...
  msg->wps_attributes = ieee802_11_vendor_ie_concat(data, len, WPS_DEV_OUI_WFA);
  if (msg->wps_attributes && p2p_parse_wps_ie(msg->wps_attributes, msg)) {
    p2p_parse_free(msg);
    return -1;
  }

  msg->p2p_attributes =
      ieee802_11_vendor_ie_concat(data, len, P2P_IE_VENDOR_TYPE);
  if (msg->p2p_attributes && p2p_parse_p2p_ie(msg->p2p_attributes, msg)) {
    wpa_printf(MSG_DEBUG, "P2P: Failed to parse P2P IE data");
    if (msg->p2p_attributes)
      wpa_hexdump_buf(MSG_MSGDUMP, "P2P: P2P IE data", msg->p2p_attributes);
    p2p_parse_free(msg);
    return -1;
  }
  ...
  if (elems.wfd) {
    msg->wfd_subelems =
        ieee802_11_vendor_ie_concat(data, len, WFD_IE_VENDOR_TYPE);
  }
...
static int p2p_group_info_text(const u8 *gi, size_t gi_len, char *buf,
  ...
  for (i = 0; i < info.num_clients; i++) {
    ...
    ret = os_snprintf(
        ...
        wps_dev_type_bin2str(cli->pri_dev_type, devtype, sizeof(devtype)));
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
    ...
    for (s = 0; s < cli->num_sec_dev_types; s++) {
      ret = os_snprintf(pos, end - pos, " dev_type=%s",
                        wps_dev_type_bin2str(&cli->sec_dev_types[s * 8],
...
int p2p_attr_text(struct wpabuf *data, char *buf, char *end) {
  ...
  if (msg.pri_dev_type) {
    char devtype[WPS_DEV_TYPE_BUFSIZE];
    ret = os_snprintf(
        pos, end - pos, "p2p_primary_device_type=%s\n",
        wps_dev_type_bin2str(msg.pri_dev_type, devtype, sizeof(devtype)));
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  ...
  ret = p2p_group_info_text(msg.group_info, msg.group_info_len, pos, end);
  if (ret < 0)
    return pos - buf;
  pos += ret;
