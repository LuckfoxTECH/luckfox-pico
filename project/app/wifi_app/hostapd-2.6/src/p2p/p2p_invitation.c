...
static struct wpabuf *p2p_build_invitation_req(struct p2p_data *p2p,
  ...
  if (wfd_ie && p2p->inv_role == P2P_INVITE_ROLE_ACTIVE_GO) {
    ...
    for (i = 0; i < p2p->num_groups; i++) {
      ...
      struct wpabuf *ie;
      if (os_memcmp(p2p_group_get_interface_addr(g), p2p->inv_bssid,
                    ETH_ALEN) != 0)
        continue;
  ...
  if (peer->dialog_token == 0)
    peer->dialog_token = 1;
  p2p_buf_add_public_action_hdr(buf, P2P_INVITATION_REQ, peer->dialog_token);
  ...
  p2p_buf_add_invitation_flags(
      buf, p2p->inv_persistent ? P2P_INVITATION_FLAGS_TYPE : 0);
...
static struct wpabuf *p2p_build_invitation_resp(struct p2p_data *p2p,
  ...
  if (wfd_ie && group_bssid) {
    ...
    for (i = 0; i < p2p->num_groups; i++) {
      struct p2p_group *g = p2p->groups[i];
      struct wpabuf *ie;
      if (os_memcmp(p2p_group_get_interface_addr(g), group_bssid, ETH_ALEN) !=
          0)
        continue;
      ie = p2p_group_get_wfd_ie(g);
      if (ie) {
  ...
  p2p_buf_add_public_action_hdr(buf, P2P_INVITATION_RESP, dialog_token);

  len = p2p_buf_add_ie_hdr(buf);
  p2p_buf_add_status(buf, status);
  p2p_buf_add_config_timeout(buf, 0, 0); /* FIX */
  if (reg_class && channel)
    p2p_buf_add_operating_channel(buf, p2p->cfg->country, reg_class, channel);
  if (group_bssid)
...
void p2p_process_invitation_req(struct p2p_data *p2p, const u8 *sa,
  ...
  if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
    p2p_dbg(p2p, "Invitation Request from unknown peer " MACSTR, MAC2STR(sa));

    if (p2p_add_device(p2p, sa, rx_freq, NULL, 0, data + 1, len - 1, 0)) {
      p2p_dbg(p2p, "Invitation Request add device failed " MACSTR, MAC2STR(sa));
    ...
    dev = p2p_get_device(p2p, sa);
    if (dev == NULL) {
      p2p_dbg(p2p, "Reject Invitation Request from unknown peer " MACSTR,
              MAC2STR(sa));
      status = P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE;
      goto fail;
    }
  ...
  if (!msg.group_id || !msg.channel_list) {
    p2p_dbg(p2p,
            "Mandatory attribute missing in Invitation Request from " MACSTR,
            MAC2STR(sa));
  ...
  if (p2p_peer_channels_check(p2p, &all_channels, dev, msg.channel_list,
  ...
  p2p_channels_intersect(&all_channels, &dev->channels, &intersection);
  ...
  if (p2p->cfg->invitation_process) {
    status = p2p->cfg->invitation_process(
        ...
        msg.group_id + ETH_ALEN, msg.group_id_len - ETH_ALEN, &go, group_bssid,
        &op_freq, persistent, &intersection,
  ...
  if (op_freq) {
    p2p_dbg(p2p, "Invitation processing forced frequency %d MHz", op_freq);
    ...
    if (!p2p_channels_includes(&intersection, reg_class, channel)) {
      p2p_dbg(p2p,
              "forced freq %d MHz not in the supported channels intersection",
              op_freq);
      status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
      goto fail;
    }
    ...
    p2p_dbg(p2p, "No forced channel from invitation processing - figure out "
                 "best one to use");

    /* Default to own configuration as a starting point */
    p2p->op_reg_class = p2p->cfg->op_reg_class;
    p2p->op_channel = p2p->cfg->op_channel;
    p2p_dbg(p2p, "Own default op_class %d channel %d", p2p->op_reg_class,
            p2p->op_channel);

    /* Use peer preference if specified and compatible */
    if (msg.operating_channel) {
      int req_freq;
      req_freq = p2p_channel_to_freq(msg.operating_channel[3],
                                     msg.operating_channel[4]);
      p2p_dbg(p2p, "Peer operating channel preference: %d MHz", req_freq);
      if (req_freq > 0 &&
          p2p_channels_includes(&intersection, msg.operating_channel[3],
                                msg.operating_channel[4])) {
    ...
    if (go && !p2p_channels_includes(&intersection, p2p->op_reg_class,
                                     p2p->op_channel)) {
      p2p_dbg(p2p,
              "Initially selected channel (op_class %d channel %d) not in "
              "channel intersection - try to reselect",
              p2p->op_reg_class, p2p->op_channel);
      ...
      if (!p2p_channels_includes(&intersection, p2p->op_reg_class,
                                 p2p->op_channel)) {
        p2p_dbg(p2p,
                "Peer does not support selected operating channel "
      ...
      p2p_dbg(p2p,
              "Try to reselect channel selection with peer information "
    ...
    op_freq = p2p_channel_to_freq(p2p->op_reg_class, p2p->op_channel);
    if (op_freq < 0) {
      p2p_dbg(
          p2p,
          "Unknown operational channel (country=%c%c reg_class=%u channel=%u)",
          p2p->cfg->country[0], p2p->cfg->country[1], p2p->op_reg_class,
          p2p->op_channel);
      status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
      goto fail;
    }
...
fail:
  if (go && status == P2P_SC_SUCCESS && !is_zero_ether_addr(group_bssid))
    ...
    bssid = NULL;
  resp = p2p_build_invitation_resp(p2p, dev, msg.dialog_token, status, bssid,
                                   reg_class, channel, channels);
...
void p2p_process_invitation_resp(struct p2p_data *p2p, const u8 *sa,
  ...
  p2p_dbg(p2p, "Received Invitation Response from " MACSTR, MAC2STR(sa));

  dev = p2p_get_device(p2p, sa);
  if (dev == NULL) {
    p2p_dbg(p2p, "Ignore Invitation Response from unknown peer " MACSTR,
            MAC2STR(sa));
  ...
  if (dev != p2p->invite_peer) {
    p2p_dbg(p2p, "Ignore unexpected Invitation Response from peer " MACSTR,
            MAC2STR(sa));
    p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
    return;
  }
  ...
  if (!msg.status) {
    p2p_dbg(p2p,
            "Mandatory Status attribute missing in Invitation Response "
            "from " MACSTR,
            MAC2STR(sa));
    p2p_parse_free(&msg);
    p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
  ...
  if (*msg.status == P2P_SC_FAIL_NO_COMMON_CHANNELS &&
      p2p->retry_invite_req_sent && msg.dialog_token != dev->dialog_token) {
  ...
  if (*msg.status == P2P_SC_FAIL_NO_COMMON_CHANNELS && p2p->retry_invite_req &&
    ...
    p2p_dbg(p2p,
            "Resend Invitation Request setting op_class %u channel %u as "
            "operating channel",
            p2p->op_reg_class, p2p->op_channel);
  ...
  if (!msg.channel_list && *msg.status == P2P_SC_SUCCESS) {
    p2p_dbg(p2p,
            "Mandatory Channel List attribute missing in Invitation Response "
            "from " MACSTR,
            MAC2STR(sa));
    ...
    channels = &p2p->channels;
  } else if (p2p_peer_channels_check(p2p, &p2p->channels, dev, msg.channel_list,
                                     msg.channel_list_len) < 0) {
    p2p_dbg(p2p, "No common channels found");
    p2p_parse_free(&msg);
    return;
  ...
  if (p2p->cfg->invitation_result) {
    ...
    int freq = p2p_channel_to_freq(p2p->op_reg_class, p2p->op_channel);
    if (freq < 0)
      freq = 0;

    if (msg.operating_channel) {
      peer_oper_freq = p2p_channel_to_freq(msg.operating_channel[3],
                                           msg.operating_channel[4]);
      if (peer_oper_freq < 0)
        peer_oper_freq = 0;
    }
    ...
    p2p->cfg->invitation_result(p2p->cfg->cb_ctx, *msg.status, msg.group_bssid,
                                channels, sa, freq, peer_oper_freq);
  }
...
void p2p_invitation_resp_cb(struct p2p_data *p2p, int success) {
  ...
  if (!success)
    p2p_dbg(p2p, "Assume Invitation Response was actually received by the peer "
                 "even though Ack was not reported");

  if (p2p->cfg->invitation_received) {
    p2p->cfg->invitation_received(p2p->cfg->cb_ctx, p2p->inv_sa,
                                  p2p->inv_group_bssid_ptr, p2p->inv_ssid,
                                  p2p->inv_ssid_len, p2p->inv_go_dev_addr,
                                  p2p->inv_status, p2p->inv_op_freq);
  }
}
...
int p2p_invite(struct p2p_data *p2p, const u8 *peer, enum p2p_invite_role role,
  ...
  p2p_dbg(p2p,
          "Request to invite peer " MACSTR " role=%d persistent=%d "
          "force_freq=%u",
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "Invitation for SSID", ssid, ssid_len);
  if (dev_pw_id >= 0) {
    p2p_dbg(p2p, "Invitation to use Device Password ID %d", dev_pw_id);
  }
  p2p->invite_dev_pw_id = dev_pw_id;
  p2p->retry_invite_req =
      role == P2P_INVITE_ROLE_GO && persistent_group && !force_freq;
  p2p->retry_invite_req_sent = 0;

  dev = p2p_get_device(p2p, peer);
  if (dev == NULL || (dev->listen_freq <= 0 && dev->oper_freq <= 0 &&
                      dev->oob_go_neg_freq <= 0)) {
    p2p_dbg(p2p, "Cannot invite unknown P2P Device " MACSTR, MAC2STR(peer));
    return -1;
  }
  ...
  if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
    if (!(dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
      p2p_dbg(p2p,
              "Cannot invite a P2P Device " MACSTR
