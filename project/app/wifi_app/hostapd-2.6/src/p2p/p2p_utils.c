...
void p2p_channels_union_inplace(struct p2p_channels *res,
  ...
  for (j = 0; j < b->reg_classes; j++) {
    ...
    if (i == res->reg_classes) {
      if (res->reg_classes == P2P_MAX_REG_CLASSES)
        return;
      os_memcpy(&res->reg_class[res->reg_classes++], b_cl,
...
void p2p_channels_remove_freqs(struct p2p_channels *chan,
  ...
  while (o < chan->reg_classes) {
    ...
    c = 0;
    while (c < op->channels) {
      int freq = p2p_channel_to_freq(op->reg_class, op->channel[c]);
    ...
    if (op->channels == 0) {
      ...
      os_memmove(&chan->reg_class[o], &chan->reg_class[o + 1],
                 (chan->reg_classes - o) * sizeof(struct p2p_reg_class));
    } else
      o++;
  }
}
...
void p2p_channels_dump(struct p2p_data *p2p, const char *title,
  ...
  for (i = 0; i < chan->reg_classes; i++) {
    ...
    for (j = 0; j < c->channels; j++) {
      ret =
          os_snprintf(pos, end - pos, "%s%u", j == 0 ? "" : ",", c->channel[j]);
      if (os_snprintf_error(end - pos, ret))
        break;
      pos += ret;
    }
...
static u8 p2p_channel_pick_random(const u8 *channels,
