...
static void p2ps_add_new_group_info(struct p2p_data *p2p,
  ...
  found = p2p->cfg->get_go_info(p2p->cfg->cb_ctx, intended_addr, ssid,
  ...
  if (found) {
    ...
    p2p_buf_add_group_id(buf, p2p->cfg->dev_addr, ssid, ssid_len);
    ...
    p2p_buf_add_group_id(buf, p2p->cfg->dev_addr, p2p->ssid, p2p->ssid_len);

    if (group_iface)
      p2p_buf_add_intended_addr(buf, p2p->intended_addr);
    else
      p2p_buf_add_intended_addr(buf, p2p->cfg->dev_addr);
  }
}
...
static void p2ps_add_pd_req_attrs(struct p2p_data *p2p, struct p2p_device *dev,
  ...
  u8 intended_addr[ETH_ALEN];
  int follow_on_req_fail =
      prov->status >= 0 && prov->status != P2P_SC_SUCCESS_DEFERRED;
  ...
  if (!follow_on_req_fail) {
    if (p2p->cfg->get_persistent_group) {
      shared_group = p2p->cfg->get_persistent_group(
          p2p->cfg->cb_ctx, dev->info.p2p_device_addr, NULL, 0, go_dev_addr,
          ssid, &ssid_len, intended_addr);
    }

    if (shared_group || (prov->conncap & (P2PS_SETUP_CLIENT | P2PS_SETUP_NEW)))
  ...
  if (!follow_on_req_fail) {
    if (shared_group || prov->conncap == P2PS_SETUP_NEW ||
        prov->conncap == (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_NEW) ||
        prov->conncap == (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_CLIENT)) {
      /* Add Config Timeout */
  ...
  if (shared_group) {
    p2p_buf_add_persistent_group_info(buf, go_dev_addr, ssid, ssid_len);
...
static struct wpabuf *p2p_build_prov_disc_req(struct p2p_data *p2p,
  ...
  if (p2p->p2ps_prov)
    extra +=
        os_strlen(p2p->p2ps_prov->info) + 1 + sizeof(struct p2ps_provision);
  ...
  p2p_buf_add_capability(
      buf, p2p->dev_capab & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY, group_capab);
...
static struct wpabuf *p2p_build_prov_disc_resp(
    struct p2p_data *p2p, struct p2p_device *dev, u8 dialog_token,
    enum p2p_status_code status, u16 config_methods, u32 adv_id,
    const u8 *group_id, size_t group_id_len, const u8 *persist_ssid,
    size_t persist_ssid_len, const u8 *fcap, u16 fcap_len) {
  struct wpabuf *buf;
  size_t extra = 0;
  int persist = 0;
  ...
  if (wfd_ie && group_id) {
    ...
    for (i = 0; i < p2p->num_groups; i++) {
      ...
      struct wpabuf *ie;
      if (!p2p_group_is_group_id_match(g, group_id, group_id_len))
        continue;
  ...
  if (p2p->p2ps_prov && p2p->p2ps_prov->adv_id == adv_id) {
    ...
    group_capab =
        P2P_GROUP_CAPAB_PERSISTENT_GROUP | P2P_GROUP_CAPAB_PERSISTENT_RECONN;
    if (p2p->cross_connect)
      group_capab |= P2P_GROUP_CAPAB_CROSS_CONN;
    if (p2p->cfg->p2p_intra_bss)
      group_capab |= P2P_GROUP_CAPAB_INTRA_BSS_DIST;
    p2p_buf_add_capability(
        buf, p2p->dev_capab & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY,
        group_capab);
    p2p_buf_add_device_info(buf, p2p, NULL);
    ...
    if (persist_ssid && p2p->cfg->get_persistent_group && dev &&
      ...
      persist = p2p->cfg->get_persistent_group(
          p2p->cfg->cb_ctx, dev->info.p2p_device_addr, persist_ssid,
          persist_ssid_len, go_dev_addr, ssid, &ssid_len, intended_addr);
      if (persist) {
        p2p_buf_add_persistent_group_info(buf, go_dev_addr, ssid, ssid_len);
    ...
    if (persist || (conncap & (P2PS_SETUP_CLIENT | P2PS_SETUP_GROUP_OWNER)))
      p2p_buf_add_channel_list(buf, p2p->cfg->country, &p2p->channels);
    ...
    if (p2p->p2ps_prov)
      p2p_buf_add_advertisement_id(buf, adv_id, p2p->p2ps_prov->adv_mac);
...
void p2p_process_prov_disc_req(struct p2p_data *p2p, const u8 *sa,
  ...
  if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
    p2p_dbg(p2p, "Provision Discovery Request from unknown peer " MACSTR,
            MAC2STR(sa));

    if (p2p_add_device(p2p, sa, rx_freq, NULL, 0, data + 1, len - 1, 0)) {
      p2p_dbg(p2p, "Provision Discovery Request add device failed " MACSTR,
              MAC2STR(sa));
      goto out;
    }
    ...
    if (!dev) {
      dev = p2p_get_device(p2p, sa);
      if (!dev) {
        p2p_dbg(p2p, "Provision Discovery device not found " MACSTR,
                MAC2STR(sa));
        goto out;
      }
    }
  ...
  if (!msg.adv_id) {
    ...
    if (msg.group_id) {
      ...
      for (i = 0; i < p2p->num_groups; i++) {
        if (p2p_group_is_group_id_match(p2p->groups[i], msg.group_id,
                                        msg.group_id_len))
          break;
      }
      if (i == p2p->num_groups) {
  ...
  if (msg.wps_config_methods & WPS_CONFIG_DISPLAY) {
    p2p_dbg(p2p, "Peer " MACSTR " requested us to show a PIN on display",
            MAC2STR(sa));
    dev->flags |= P2P_DEV_PD_PEER_KEYPAD;
    passwd_id = DEV_PW_USER_SPECIFIED;
  } else if (msg.wps_config_methods & WPS_CONFIG_KEYPAD) {
    p2p_dbg(p2p, "Peer " MACSTR " requested us to write its PIN using keypad",
            MAC2STR(sa));
  ...
  if (p2p->cfg->remove_stale_groups) {
    p2p->cfg->remove_stale_groups(p2p->cfg->cb_ctx, dev->info.p2p_device_addr,
                                  msg.persistent_dev, msg.persistent_ssid,
  ...
  if (!msg.status) {
    ...
    if (os_memcmp(p2p->cfg->dev_addr, msg.adv_mac, ETH_ALEN)) {
      p2p_dbg(p2p, "P2PS PD adv mac does not match the local one");
    ...
    conncap = p2p->cfg->p2ps_group_capability(
        p2p->cfg->cb_ctx, conncap, auto_accept, &forced_freq, &pref_freq);

    p2p_dbg(p2p, "Conncap: local:%d remote:%d result:%d", auto_accept,
            remote_conncap, conncap);

    p2p_prepare_channel(p2p, dev, forced_freq, pref_freq, 0);

    resp_fcap.cpt =
        p2ps_own_preferred_cpt(p2ps_adv->cpt_priority, req_fcap->cpt);

    p2p_dbg(p2p, "cpt: service:0x%x remote:0x%x result:0x%x",
            p2ps_adv->cpt_mask, req_fcap->cpt, resp_fcap.cpt);

    if (!resp_fcap.cpt) {
      p2p_dbg(p2p, "Incompatible P2PS feature capability CPT bitmask");
      reject = P2P_SC_FAIL_INCOMPATIBLE_PARAMS;
    } else if (p2ps_adv->config_methods &&
               !(msg.wps_config_methods & p2ps_adv->config_methods)) {
      p2p_dbg(p2p,
              "Unsupported config methods in Provision Discovery Request "
              "(own=0x%x peer=0x%x)",
              p2ps_adv->config_methods, msg.wps_config_methods);
      reject = P2P_SC_FAIL_INCOMPATIBLE_PARAMS;
    ...
    if ((remote_conncap & (P2PS_SETUP_NEW | P2PS_SETUP_CLIENT) ||
         msg.persistent_dev) &&
        conncap != P2PS_SETUP_NEW && msg.channel_list && msg.channel_list_len &&
        p2p_peer_channels_check(p2p, &p2p->channels, dev, msg.channel_list,
                                msg.channel_list_len) < 0) {
      p2p_dbg(p2p, "No common channels - force deferred flow");
      auto_accept = 0;
    }

    if (((remote_conncap & P2PS_SETUP_GROUP_OWNER) || msg.persistent_dev) &&
      ...
      if (dev->channels.reg_classes == 0 ||
          !p2p_channels_includes(&dev->channels, msg.operating_channel[3],
                                 msg.operating_channel[4])) {
        struct p2p_channels *ch = &dev->channels;

        os_memset(ch, 0, sizeof(*ch));
        ch->reg_class[0].reg_class = msg.operating_channel[3];
        ch->reg_class[0].channel[0] = msg.operating_channel[4];
        ch->reg_class[0].channels = 1;
        ch->reg_classes = 1;
      }

      p2p_channels_intersect(&p2p->channels, &dev->channels, &intersect);
    ...
    if (auto_accept || reject != P2P_SC_SUCCESS) {
      ...
      if (p2ps_setup_p2ps_prov(p2p, adv_id, session_id, msg.wps_config_methods,
  ...
  if (p2p->p2ps_prov->adv_id != adv_id ||
    ...
    p2p_dbg(p2p, "P2PS Follow-on PD with mismatch Advertisement ID/MAC");
    goto out;
  }
  ...
  conncap = p2p->cfg->p2ps_group_capability(
      p2p->cfg->cb_ctx, remote_conncap, p2p->p2ps_prov->conncap,
      &p2p->p2ps_prov->force_freq, &p2p->p2ps_prov->pref_freq);

  resp_fcap.cpt =
      p2ps_own_preferred_cpt(p2p->p2ps_prov->cpt_priority, req_fcap->cpt);
  ...
  if (!conncap || !(msg.wps_config_methods & method)) {
    ...
    reject = P2P_SC_FAIL_INCOMPATIBLE_PARAMS;
              ...
              msg.persistent_dev) &&
             conncap != P2PS_SETUP_NEW && msg.channel_list &&
             msg.channel_list_len &&
             p2p_peer_channels_check(p2p, &p2p->channels, dev, msg.channel_list,
                                     msg.channel_list_len) < 0) {
    p2p_dbg(p2p, "No common channels in Follow-On Provision Discovery Request");
...
out:
   ...
   */
  if (!msg.status || *msg.status == P2P_SC_SUCCESS_DEFERRED) {
    resp = p2p_build_prov_disc_resp(
        p2p, dev, msg.dialog_token, reject, config_methods, adv_id,
        msg.group_id, msg.group_id_len, msg.persistent_ssid,
        msg.persistent_ssid_len, (const u8 *)&resp_fcap, sizeof(resp_fcap));
    if (!resp) {
      p2p_parse_free(&msg);
      return;
    }
    ...
    if (p2p_send_action(p2p, freq, sa, p2p->cfg->dev_addr, p2p->cfg->dev_addr,
  ...
  if (!p2p->cfg->p2ps_prov_complete) {
    ...
    reject = *msg.status;
    p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, reject, sa, adv_mac,
                                 session_mac, NULL, adv_id, session_id, 0, 0,
                                 msg.persistent_ssid, msg.persistent_ssid_len,
                                 0, 0, NULL, NULL, 0, freq, NULL, 0);
    ...
    if (reject != P2P_SC_SUCCESS)
      p2p->cfg->p2ps_prov_complete(
          p2p->cfg->cb_ctx, reject, sa, adv_mac, session_mac, NULL, adv_id,
          session_id, conncap, 0, msg.persistent_ssid, msg.persistent_ssid_len,
          0, 0, NULL, NULL, 0, freq, NULL, 0);
    else
      p2p->cfg->p2ps_prov_complete(
          p2p->cfg->cb_ctx, *msg.status, sa, adv_mac, session_mac, group_mac,
          adv_id, session_id, conncap, passwd_id, msg.persistent_ssid,
          msg.persistent_ssid_len, 0, 0, NULL, (const u8 *)&resp_fcap,
    ...
    p2p->cfg->p2ps_prov_complete(
        p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac,
        adv_id, session_id, conncap, passwd_id, msg.persistent_ssid,
        msg.persistent_ssid_len, 0, 0, NULL, (const u8 *)&resp_fcap,
        sizeof(resp_fcap), freq, msg.group_id ? msg.group_id + ETH_ALEN : NULL,
    ...
    p2p->cfg->p2ps_prov_complete(
        p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac,
        adv_id, session_id, conncap, passwd_id, msg.persistent_ssid,
        msg.persistent_ssid_len, 0, 1, NULL, (const u8 *)&resp_fcap,
        sizeof(resp_fcap), freq, NULL, 0);
  } else if (reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
    ...
    if (buf) {
      ...
      p2p->cfg->p2ps_prov_complete(
          p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac,
          adv_id, session_id, conncap, passwd_id, msg.persistent_ssid,
          msg.persistent_ssid_len, 0, 1, buf, (const u8 *)&resp_fcap,
          sizeof(resp_fcap), freq, NULL, 0);
  ...
  if (p2p->cfg->prov_disc_req &&
      ((reject == P2P_SC_SUCCESS && !msg.adv_id) ||
       (!msg.status &&
        (reject == P2P_SC_SUCCESS ||
         reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) &&
        passwd_id == DEV_PW_USER_SPECIFIED) ||
       (!msg.status && reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE &&
        passwd_id == DEV_PW_REGISTRAR_SPECIFIED) ||
       (reject == P2P_SC_SUCCESS && msg.status &&
        *msg.status == P2P_SC_SUCCESS_DEFERRED &&
        passwd_id == DEV_PW_REGISTRAR_SPECIFIED))) {
    ...
    p2p->cfg->prov_disc_req(
        p2p->cfg->cb_ctx, sa, msg.wps_config_methods, dev_addr,
        msg.pri_dev_type, msg.device_name, msg.config_methods,
        msg.capability ? msg.capability[0] : 0,
        msg.capability ? msg.capability[1] : 0, msg.group_id, msg.group_id_len);
  }
...
#define P2PS_PD_RESP_CHECK(_val, _attr)                                        \
  ...
  if (p2p->p2ps_prov->session_id != session_id) {
    p2p_dbg(p2p, "Ignore PD Response with unexpected Session ID");
    return -1;
  }

  if (os_memcmp(p2p->p2ps_prov->session_mac, msg->session_mac, ETH_ALEN)) {
    p2p_dbg(p2p, "Ignore PD Response with unexpected Session MAC");
    return -1;
  }

  if (p2p->p2ps_prov->adv_id != adv_id) {
    p2p_dbg(p2p, "Ignore PD Response with unexpected Advertisement ID");
    return -1;
  }

  if (os_memcmp(p2p->p2ps_prov->adv_mac, msg->adv_mac, ETH_ALEN) != 0) {
    p2p_dbg(p2p, "Ignore PD Response with unexpected Advertisement MAC");
    return -1;
  }

  if (msg->listen_channel) {
    p2p_dbg(p2p, "Ignore malformed PD Response - unexpected Listen Channel");
    return -1;
  }

  if (*msg->status == P2P_SC_SUCCESS &&
      !(!!msg->conn_cap ^ !!msg->persistent_dev)) {
    p2p_dbg(p2p, "Ignore malformed PD Response - either conn_cap or persistent "
                 "group should be present");
    return -1;
  }

  if (msg->persistent_dev && *msg->status != P2P_SC_SUCCESS) {
    p2p_dbg(p2p, "Ignore malformed PD Response - persistent group is present, "
                 "but the status isn't success");
    return -1;
  }
...
void p2p_process_prov_disc_resp(struct p2p_data *p2p, const u8 *sa,
  ...
  if (dev->dialog_token != msg.dialog_token) {
    p2p_dbg(p2p,
            "Ignore Provision Discovery Response with unexpected Dialog Token "
            "%u (expected %u)",
  ...
  if (msg.wps_config_methods != req_config_methods) {
    p2p_dbg(p2p,
            "Peer rejected our Provision Discovery Request (received "
  ...
  if (req_config_methods & WPS_CONFIG_DISPLAY) {
    p2p_dbg(p2p, "Peer " MACSTR " accepted to show a PIN on display",
            MAC2STR(sa));
    dev->flags |= P2P_DEV_PD_PEER_DISPLAY;
    passwd_id = DEV_PW_REGISTRAR_SPECIFIED;
  } else if (msg.wps_config_methods & WPS_CONFIG_KEYPAD) {
    p2p_dbg(p2p, "Peer " MACSTR " accepted to write our PIN using keypad",
            MAC2STR(sa));
    dev->flags |= P2P_DEV_PD_PEER_KEYPAD;
    passwd_id = DEV_PW_USER_SPECIFIED;
  } else if (msg.wps_config_methods & WPS_CONFIG_P2PS) {
  ...
  if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) &&
    ...
    if (msg.operating_channel) {
      if (p2p_channels_includes(&p2p->channels, msg.operating_channel[3],
                                msg.operating_channel[4]) &&
          p2p_channels_includes(&dev->channels, msg.operating_channel[3],
                                msg.operating_channel[4])) {
        dev->oper_freq = p2p_channel_to_freq(msg.operating_channel[3],
        ...
        p2p_dbg(p2p, "P2PS PD Response - invalid operating channel");
      }
    }
    ...
    if (p2p->cfg->p2ps_prov_complete) {
      ...
      if (conncap == P2PS_SETUP_GROUP_OWNER) {
        ...
        if (p2p_go_select_channel(p2p, dev, &tmp) < 0)
          p2p_dbg(p2p, "P2PS PD channel selection failed");
      ...
      p2p->cfg->p2ps_prov_complete(
          p2p->cfg->cb_ctx, status, sa, adv_mac, p2p->p2ps_prov->session_mac,
          group_mac, adv_id, p2p->p2ps_prov->session_id, conncap, passwd_id,
          msg.persistent_ssid, msg.persistent_ssid_len, 1, 0, NULL,
    ...
    if (p2p->cfg->p2ps_prov_complete)
      p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, status, sa, adv_mac,
                                   p2p->p2ps_prov->session_mac, group_mac,
                                   adv_id, p2p->p2ps_prov->session_id, 0, 0,
                                   NULL, 0, 1, 0, NULL, NULL, 0, 0, NULL, 0);
    p2ps_prov_free(p2p);
  }
  ...
  if (status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
    ...
    if (msg.session_info && msg.session_info_len) {
      ...
      utf8_escape((char *)msg.session_info, info_len, deferred_sess_resp,
                  2 * info_len + 1);

      if (p2p->cfg->prov_disc_fail)
        p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, sa,
                                 P2P_PROV_DISC_INFO_UNAVAILABLE, adv_id,
                                 adv_mac, deferred_sess_resp);
      os_free(deferred_sess_resp);
    } else if (p2p->cfg->prov_disc_fail)
      p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, sa,
                               P2P_PROV_DISC_INFO_UNAVAILABLE, adv_id, adv_mac,
                               NULL);
  } else if (status != P2P_SC_SUCCESS) {
...
out:
  ...
  if (dev->flags & P2P_DEV_PD_BEFORE_GO_NEG) {
    p2p_dbg(p2p,
            "Start GO Neg after the PD-before-GO-Neg workaround with " MACSTR,
  ...
  if (p2p->cfg->prov_disc_resp &&
      ((status == P2P_SC_SUCCESS && !adv_id) ||
       (p2ps_seeker && status == P2P_SC_SUCCESS &&
        passwd_id == DEV_PW_REGISTRAR_SPECIFIED) ||
       (p2ps_seeker && status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE &&
        passwd_id == DEV_PW_USER_SPECIFIED)))
...
int p2p_send_prov_disc_req(struct p2p_data *p2p, struct p2p_device *dev,
  ...
  if (freq <= 0) {
    p2p_dbg(p2p,
            "No Listen/Operating frequency known for the peer " MACSTR
  ...
  if (p2p->p2ps_prov) {
    if (p2p->p2ps_prov->status == P2P_SC_SUCCESS_DEFERRED) {
      ...
      if (p2p->p2ps_prov->method)
        dev->req_config_methods = p2p->p2ps_prov->method;
    ...
    p2p_dbg(p2p,
            "Building PD Request based on P2PS config method 0x%x status %d "
  ...
  if (p2p_send_action(p2p, freq, dev->info.p2p_device_addr, p2p->cfg->dev_addr,
                      dev->info.p2p_device_addr, wpabuf_head(req),
                      wpabuf_len(req), 200) < 0) {
    p2p_dbg(p2p, "Failed to send Action frame");
    wpabuf_free(req);
    return -1;
  }
...
int p2p_prov_disc_req(struct p2p_data *p2p, const u8 *peer_addr,
  ...
  if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
    p2p_dbg(p2p,
            "Provision Discovery Request destination " MACSTR " not yet known",
            MAC2STR(peer_addr));
    os_free(p2ps_prov);
    return -1;
  }
  ...
  p2p_dbg(p2p,
          "Provision Discovery Request with " MACSTR " (config methods 0x%x)",
  ...
  if (p2p->state != P2P_IDLE && p2p->state != P2P_SEARCH &&
    ...
    p2p_dbg(p2p,
            "Busy with other operations; postpone Provision Discovery Request "
...
void p2p_reset_pending_pd(struct p2p_data *p2p) {
  ...
  dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
    if (os_memcmp(p2p->pending_pd_devaddr, dev->info.p2p_device_addr, ETH_ALEN))
      continue;
