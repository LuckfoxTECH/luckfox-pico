...
struct p2p_sd_query *p2p_pending_sd_req(struct p2p_data *p2p,
  ...
  for (q = p2p->sd_queries; q; q = q->next) {
    ...
    if (!q->for_all_peers &&
        os_memcmp(q->peer, dev->info.p2p_device_addr, ETH_ALEN) == 0)
      goto found;
  }
...
found:
  if (dev->sd_reqs > 100) {
    p2p_dbg(p2p,
            "Too many SD request attempts to " MACSTR
...
static void p2p_send_gas_comeback_req(struct p2p_data *p2p, const u8 *dst,
  ...
  if (p2p_send_action(p2p, freq, dst, p2p->cfg->dev_addr, dst, wpabuf_head(req),
...
static struct wpabuf *p2p_build_sd_response(u8 dialog_token, u16 status_code,
  ...
  buf = gas_anqp_build_initial_resp(dialog_token, status_code, comeback_delay,
...
p2p_build_gas_comeback_resp(u8 dialog_token, u16 status_code, u16 update_indic,
                            const u8 *data, size_t len, u8 frag_id, u8 more,
                            u16 total_len) {
  struct wpabuf *buf;

  buf = gas_anqp_build_comeback_resp(dialog_token, status_code, frag_id, more,
                                     0, 100 + len);
  if (buf == NULL)
    return NULL;
...
int p2p_start_sd(struct p2p_data *p2p, struct p2p_device *dev) {
  ...
  if (p2p->state == P2P_SEARCH &&
      os_memcmp(p2p->sd_query_no_ack, dev->info.p2p_device_addr, ETH_ALEN) ==
          0) {
    p2p_dbg(p2p,
            "Do not start Service Discovery with " MACSTR
...
void p2p_rx_gas_initial_req(struct p2p_data *p2p, const u8 *sa, const u8 *data,
  ...
  dialog_token = *pos++;
  p2p_dbg(p2p, "GAS Initial Request from " MACSTR " (dialog token %u, freq %d)",
  ...
  if (*pos != ACCESS_NETWORK_QUERY_PROTOCOL) {
    p2p_dbg(p2p, "Unsupported GAS advertisement protocol id %u", *pos);
    return;
  }
  ...
  if (WPA_GET_BE32(pos) != P2P_IE_VENDOR_TYPE) {
    p2p_dbg(p2p, "Unsupported ANQP vendor OUI-type %08x", WPA_GET_BE32(pos));
    return;
  }
  pos += 4;
  ...
  p2p->cfg->sd_request(p2p->cfg->cb_ctx, freq, sa, dialog_token, update_indic,
                       pos, end - pos);
...
void p2p_sd_response(struct p2p_data *p2p, int freq, const u8 *dst,
  ...
  if (wpabuf_len(resp_tlvs) > max_len) {
    ...
    p2p->sd_resp_pos = 0;
    p2p->sd_frag_id = 0;
    resp = p2p_build_sd_response(dialog_token, WLAN_STATUS_SUCCESS, 1,
                                 p2p->srv_update_indic, NULL);
  } else {
    p2p_dbg(p2p, "SD response fits in initial response");
    resp = p2p_build_sd_response(dialog_token, WLAN_STATUS_SUCCESS, 0,
  ...
  p2p->pending_action_state = P2P_NO_PENDING_ACTION;
  if (p2p_send_action(p2p, freq, dst, p2p->cfg->dev_addr, p2p->cfg->dev_addr,
...
void p2p_rx_gas_initial_resp(struct p2p_data *p2p, const u8 *sa, const u8 *data,
                             size_t len, int rx_freq) {
  const u8 *pos = data;
  const u8 *end = data + len;
  const u8 *next;
  ...
  if (p2p->state != P2P_SD_DURING_FIND || p2p->sd_peer == NULL ||
    ...
    p2p_dbg(p2p, "Ignore unexpected GAS Initial Response from " MACSTR,
            MAC2STR(sa));
    return;
  }
  ...
  pos += 2;
  p2p_dbg(p2p, "dialog_token=%u status_code=%u comeback_delay=%u", dialog_token,
          status_code, comeback_delay);
  if (status_code) {
    p2p_dbg(p2p, "Service Discovery failed: status code %u", status_code);
    return;
  }
  ...
  if (p2p->sd_query) {
    if (!p2p->sd_query->for_all_peers) {
      struct p2p_sd_query *q;
      p2p_dbg(p2p, "Remove completed SD query %p", p2p->sd_query);
      q = p2p->sd_query;
  ...
  if (p2p->cfg->sd_response)
    p2p->cfg->sd_response(p2p->cfg->cb_ctx, sa, update_indic, pos, end - pos);
  p2p_continue_find(p2p);
}

void p2p_rx_gas_comeback_req(struct p2p_data *p2p, const u8 *sa, const u8 *data,
                             size_t len, int rx_freq) {
  struct wpabuf *resp;
  u8 dialog_token;
  ...
  if (os_memcmp(sa, p2p->sd_resp_addr, ETH_ALEN) != 0) {
    p2p_dbg(p2p, "No pending SD response fragment for " MACSTR, MAC2STR(sa));
    return;
  }
  ...
  resp = p2p_build_gas_comeback_resp(
      dialog_token, WLAN_STATUS_SUCCESS, p2p->srv_update_indic,
      wpabuf_head_u8(p2p->sd_resp) + p2p->sd_resp_pos, frag_len,
      p2p->sd_frag_id, more, wpabuf_len(p2p->sd_resp));
  if (resp == NULL)
    return;
  ...
  p2p->pending_action_state = P2P_NO_PENDING_ACTION;
  if (p2p_send_action(p2p, rx_freq, sa, p2p->cfg->dev_addr, p2p->cfg->dev_addr,
                      wpabuf_head(resp), wpabuf_len(resp), 200) < 0)
    p2p_dbg(p2p, "Failed to send Action frame");
...
void p2p_rx_gas_comeback_resp(struct p2p_data *p2p, const u8 *sa,
  ...
  if (p2p->state != P2P_SD_DURING_FIND || p2p->sd_peer == NULL ||
      os_memcmp(sa, p2p->sd_peer->info.p2p_device_addr, ETH_ALEN) != 0) {
    p2p_dbg(p2p, "Ignore unexpected GAS Comeback Response from " MACSTR,
            MAC2STR(sa));
    return;
  }
  p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
  ...
  pos += 2;
  p2p_dbg(p2p,
          "dialog_token=%u status_code=%u frag_id=%d more_frags=%d "
          "comeback_delay=%u",
          dialog_token, status_code, frag_id, more_frags, comeback_delay);
  ...
  if (*pos != WLAN_EID_ADV_PROTO) {
    p2p_dbg(p2p, "Unexpected IE in GAS Comeback Response: %u", *pos);
    return;
  }
  pos++;
...
skip_nqp_header:
  ...
  if (p2p->cfg->sd_response)
    p2p->cfg->sd_response(p2p->cfg->cb_ctx, sa, p2p->sd_rx_update_indic,
...
void p2p_sd_service_update(struct p2p_data *p2p) { p2p->srv_update_indic++; }
