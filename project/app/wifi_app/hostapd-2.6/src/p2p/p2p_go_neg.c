...
int p2p_peer_channels_check(struct p2p_data *p2p, struct p2p_channels *own,
                            struct p2p_device *dev, const u8 *channel_list,
  ...
  if (pos[2] != 0x04 && os_memcmp(pos, p2p->cfg->country, 2) != 0) {
    p2p_info(p2p, "Mismatching country (ours=%c%c peer's=%c%c)",
             p2p->cfg->country[0], p2p->cfg->country[1], pos[0], pos[1]);
    return -1;
  }
  pos += 3;
  ...
  p2p_dbg(p2p,
          "Own reg_classes %d peer reg_classes %d intersection reg_classes %d",
          (int)own->reg_classes, (int)dev->channels.reg_classes,
...
static int p2p_peer_channels(struct p2p_data *p2p, struct p2p_device *dev,
  ...
  return p2p_peer_channels_check(p2p, &p2p->channels, dev, channel_list,
                                 channel_list_len);
}
...
static struct wpabuf *p2p_build_go_neg_req(struct p2p_data *p2p,
  ...
  p2p_buf_add_pref_channel_list(buf, p2p->pref_freq_list, p2p->num_pref_freq);
...
int p2p_connect_send(struct p2p_data *p2p, struct p2p_device *dev) {
  ...
  if (dev->flags & P2P_DEV_PD_BEFORE_GO_NEG) {
    ...
    if (dev->wps_method == WPS_PIN_DISPLAY)
      ...
      return -1;
    return p2p_prov_disc_req(p2p, dev->info.p2p_device_addr, NULL,
                             config_method, 0, 0, 1);
  }
...
void p2p_reselect_channel(struct p2p_data *p2p,
  ...
  if (p2p->own_freq_preference > 0 &&
      p2p_freq_to_channel(p2p->own_freq_preference, &op_reg_class,
                          &op_channel) == 0 &&
      p2p_channels_includes(intersection, op_reg_class, op_channel)) {
    p2p_dbg(p2p,
            "Pick own channel preference (reg_class %u channel %u) from "
            "intersection",
  ...
  if (p2p->best_freq_overall > 0 &&
      p2p_freq_to_channel(p2p->best_freq_overall, &op_reg_class, &op_channel) ==
          0 &&
      p2p_channels_includes(intersection, op_reg_class, op_channel)) {
    p2p_dbg(
        p2p,
        "Pick best overall channel (reg_class %u channel %u) from intersection",
        op_reg_class, op_channel);
    p2p->op_reg_class = op_reg_class;
    p2p->op_channel = op_channel;
    return;
  ...
  if (freq >= 2400 && freq < 2500 && p2p->best_freq_5 > 0 &&
      !p2p_channels_includes(intersection, p2p->op_reg_class,
                             p2p->op_channel) &&
      p2p_freq_to_channel(p2p->best_freq_5, &op_reg_class, &op_channel) == 0 &&
      p2p_channels_includes(intersection, op_reg_class, op_channel)) {
    p2p_dbg(
        p2p,
        "Pick best 5 GHz channel (reg_class %u channel %u) from intersection",
        op_reg_class, op_channel);
    p2p->op_reg_class = op_reg_class;
    p2p->op_channel = op_channel;
    return;
  ...
  if (freq >= 4900 && freq < 6000 && p2p->best_freq_24 > 0 &&
      !p2p_channels_includes(intersection, p2p->op_reg_class,
                             p2p->op_channel) &&
      p2p_freq_to_channel(p2p->best_freq_24, &op_reg_class, &op_channel) == 0 &&
      p2p_channels_includes(intersection, op_reg_class, op_channel)) {
    p2p_dbg(
        p2p,
        "Pick best 2.4 GHz channel (reg_class %u channel %u) from intersection",
        op_reg_class, op_channel);
    p2p->op_reg_class = op_reg_class;
    p2p->op_channel = op_channel;
    return;
  ...
  for (i = 0; p2p->cfg->pref_chan && i < p2p->cfg->num_pref_chan; i++) {
    if (p2p_channels_includes(intersection, p2p->cfg->pref_chan[i].op_class,
                              p2p->cfg->pref_chan[i].chan)) {
      p2p->op_reg_class = p2p->cfg->pref_chan[i].op_class;
      p2p->op_channel = p2p->cfg->pref_chan[i].chan;
      p2p_dbg(p2p,
              "Pick highest preferred channel (op_class %u channel %u) from "
              "intersection",
  ...
  if (p2p_channel_select(intersection, op_classes_vht, &p2p->op_reg_class,
                         &p2p->op_channel) == 0) {
    p2p_dbg(
        p2p,
        "Pick possible VHT channel (op_class %u channel %u) from intersection",
        p2p->op_reg_class, p2p->op_channel);
    return;
  }

  /* Try a channel where we might be able to use HT40 */
  if (p2p_channel_select(intersection, op_classes_ht40, &p2p->op_reg_class,
                         &p2p->op_channel) == 0) {
    p2p_dbg(
        p2p,
        "Pick possible HT40 channel (op_class %u channel %u) from intersection",
        p2p->op_reg_class, p2p->op_channel);
    return;
  }

  /* Prefer a 5 GHz channel */
  if (p2p_channel_select(intersection, op_classes_5ghz, &p2p->op_reg_class,
                         &p2p->op_channel) == 0) {
    p2p_dbg(p2p,
            "Pick possible 5 GHz channel (op_class %u channel %u) from "
            "intersection",
            p2p->op_reg_class, p2p->op_channel);
    return;
  }
   ...
   * randomness.
   */
  if (p2p_channels_includes(intersection, p2p->op_reg_class, p2p->op_channel)) {
    p2p_dbg(p2p,
            "Using original operating class and channel (op_class %u channel "
            "%u) from intersection",
            p2p->op_reg_class, p2p->op_channel);
    return;
  }
  ...
  p2p_dbg(p2p,
          "Pick another channel (reg_class %u channel %u) from intersection",
...
int p2p_go_select_channel(struct p2p_data *p2p, struct p2p_device *dev,
  ...
  p2p_channels_dump(p2p, "intersection with local channel list", &intersection);
  if (intersection.reg_classes == 0 ||
  ...
  if (!p2p_channels_includes(&intersection, p2p->op_reg_class,
    ...
    p2p_dbg(p2p,
            "Selected operating channel (op_class %u channel %u) not "
            "acceptable to the peer",
            p2p->op_reg_class, p2p->op_channel);
    p2p_reselect_channel(p2p, &intersection);
  } else if (!(dev->flags & P2P_DEV_FORCE_FREQ) && !p2p->cfg->cfg_op_channel) {
    p2p_dbg(p2p,
            "Try to optimize channel selection with peer information received; "
...
static void p2p_check_pref_chan_no_recv(struct p2p_data *p2p, int go,
  ...
  p2p_dbg(p2p, "Peer didn't provide a preferred frequency list, see if any of "
               "our preferred channels are supported by peer device");
  ...
  for (i = 0; i < size && !found; i++) {
    ...
    if (p2p_freq_to_channel(oper_freq, &op_class, &op_channel) < 0) {
    ...
    if (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) &&
        (go || !p2p_channels_includes(&p2p->cfg->cli_channels, op_class,
                                      op_channel))) {
      p2p_dbg(p2p, "Freq %u MHz (oper_class %u channel %u) not allowed for P2P",
  ...
  if (found) {
    p2p_dbg(p2p,
            "Freq %d MHz is a preferred channel and is also supported by peer, "
            "use it as the operating channel",
            oper_freq);
  } else {
    p2p_dbg(p2p,
            "None of our preferred channels are supported by peer!. Use: %d "
            "MHz for oper_channel",
            dev->oper_freq);
  }
}
...
static void p2p_check_pref_chan_recv(struct p2p_data *p2p, int go,
  ...
  for (i = 0; i < size && !found; i++) {
    for (j = 2; j < (msg->pref_freq_list_len / 2); j++) {
      oper_freq = p2p_channel_to_freq(msg->pref_freq_list[2 * j],
  ...
  if (found) {
    p2p_dbg(p2p,
            "Freq %d MHz is a common preferred channel for both peer and "
            "local, use it as operating channel",
            oper_freq);
  } else {
    p2p_dbg(p2p,
...
void p2p_check_pref_chan(struct p2p_data *p2p, int go, struct p2p_device *dev,
                         struct p2p_message *msg) {
  ...
  if (p2p->cfg->get_pref_freq_list(p2p->cfg->cb_ctx, go, &size, freq_list))
    return;
  ...
  if (i != size) {
    ...
    if (p2p_freq_to_channel(freq_list[i], &op_class, &op_channel) < 0) {
      p2p_dbg(
          p2p,
  ...
  p2p_dbg(p2p,
          "Peer operating channel preference: %d MHz is not in our preferred "
          "channel list",
          dev->oper_freq);
...
void p2p_process_go_neg_req(struct p2p_data *p2p, const u8 *sa, const u8 *data,
  ...
  if (!msg.capability) {
    p2p_dbg(
        p2p,
        "Mandatory Capability attribute missing from GO Negotiation Request");
  ...
  if (msg.go_intent)
    ...
    p2p_dbg(
        p2p,
        "Mandatory GO Intent attribute missing from GO Negotiation Request");
#ifdef CONFIG_P2P_STRICT
    goto fail;
#endif /* CONFIG_P2P_STRICT */
  }
  ...
  if (!msg.config_timeout) {
    p2p_dbg(p2p, "Mandatory Configuration Timeout attribute missing from GO "
                 "Negotiation Request");
#ifdef CONFIG_P2P_STRICT
    goto fail;
#endif /* CONFIG_P2P_STRICT */
  ...
  if (os_memcmp(msg.p2p_device_addr, sa, ETH_ALEN) != 0) {
    p2p_dbg(p2p,
            "Unexpected GO Negotiation Request SA=" MACSTR
            " != dev_addr=" MACSTR,
  ...
  if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
    ...
    p2p_dbg(p2p, "Not ready for GO negotiation with " MACSTR, MAC2STR(sa));
    status = P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE;
    p2p->cfg->go_neg_req_rx(p2p->cfg->cb_ctx, sa, msg.dev_password_id,
                            msg.go_intent ? (*msg.go_intent >> 1) : 0);
    ...
    if (dev->go_neg_req_sent &&
      ...
      p2p_dbg(p2p, "Do not reply since peer has higher address and GO Neg "
                   "Request already sent");
      p2p_parse_free(&msg);
      return;
    }

    if (dev->go_neg_req_sent && (dev->flags & P2P_DEV_PEER_WAITING_RESPONSE)) {
      p2p_dbg(p2p, "Do not reply since peer is waiting for us to start a new "
                   "GO Negotiation and GO Neg Request already sent");
      p2p_parse_free(&msg);
      return;
    }
    ...
    if (p2p_peer_channels(p2p, dev, msg.channel_list, msg.channel_list_len) <
        0) {
      p2p_dbg(p2p, "No common channels found");
      status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
      goto fail;
    ...
    case DEV_PW_P2PS_DEFAULT:
      ...
      break;
    default:
      if (msg.dev_password_id && msg.dev_password_id == dev->oob_pw_id) {
      ...
      if (p2p->authorized_oob_dev_pw_id &&
          msg.dev_password_id == p2p->authorized_oob_dev_pw_id) {
        p2p_dbg(p2p,
                "Using static handover with our device password from NFC Tag");
        dev->wps_method = WPS_NFC;
        dev->oob_pw_id = p2p->authorized_oob_dev_pw_id;
        break;
      }
#endif /* CONFIG_WPS_NFC */
      p2p_dbg(p2p, "Unsupported Device Password ID %d", msg.dev_password_id);
      status = P2P_SC_FAIL_INCOMPATIBLE_PROV_METHOD;
      goto fail;
    }
    ...
    dev->oper_freq =
        p2p_channel_to_freq(msg.operating_channel[3], msg.operating_channel[4]);
    p2p_dbg(p2p, "Peer operating channel preference: %d MHz", dev->oper_freq);
...
fail:
  if (dev)
    dev->status = status;
  resp =
      p2p_build_go_neg_resp(p2p, dev, msg.dialog_token, status, !tie_breaker);
  p2p_parse_free(&msg);
  if (resp == NULL)
    return;
  ...
  if (status == P2P_SC_SUCCESS) {
    ...
    p2p->pending_action_state = P2P_PENDING_GO_NEG_RESPONSE_FAILURE;
...
static struct wpabuf *p2p_build_go_neg_conf(struct p2p_data *p2p,
  ...
  if (go || resp_chan == NULL)
    ...
    p2p_buf_add_operating_channel(buf, (const char *)resp_chan, resp_chan[3],
                                  resp_chan[4]);
  p2p_channels_intersect(&p2p->channels, &peer->channels, &res);
  p2p_buf_add_channel_list(buf, p2p->cfg->country, &res);
  if (go) {
...
void p2p_process_go_neg_resp(struct p2p_data *p2p, const u8 *sa, const u8 *data,
                             size_t len, int rx_freq) {
  struct p2p_device *dev;
  int go = -1;
  struct p2p_message msg;
  ...
  p2p_dbg(p2p, "Received GO Negotiation Response from " MACSTR " (freq=%d)",
          MAC2STR(sa), rx_freq);
  dev = p2p_get_device(p2p, sa);
  ...
  if (msg.dialog_token != dev->dialog_token) {
    p2p_dbg(p2p, "Unexpected Dialog Token %u (expected %u)", msg.dialog_token,
            dev->dialog_token);
    p2p_parse_free(&msg);
    return;
  }
  ...
  if (*msg.status) {
    ...
    if (*msg.status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
      ...
      dev->flags |= P2P_DEV_NOT_YET_READY;
      eloop_cancel_timeout(p2p_go_neg_wait_timeout, p2p, NULL);
      eloop_register_timeout(120, 0, p2p_go_neg_wait_timeout, p2p, NULL);
  ...
  if (!msg.capability) {
    p2p_dbg(
        p2p,
        "Mandatory Capability attribute missing from GO Negotiation Response");
#ifdef CONFIG_P2P_STRICT
    status = P2P_SC_FAIL_INVALID_PARAMS;
    goto fail;
#endif /* CONFIG_P2P_STRICT */
  ...
  if (!msg.p2p_device_info) {
    p2p_dbg(p2p, "Mandatory P2P Device Info attribute missing from GO "
  ...
  if ((*msg.go_intent >> 1) > P2P_MAX_GO_INTENT) {
    p2p_dbg(p2p, "Invalid GO Intent value (%u) received", *msg.go_intent >> 1);
    status = P2P_SC_FAIL_INVALID_PARAMS;
    goto fail;
  }
  ...
  if (!go && msg.group_id) {
    ...
    p2p_dbg(p2p, "Mandatory P2P Group ID attribute missing from GO Negotiation "
                 "Response");
    p2p->ssid_len = 0;
    status = P2P_SC_FAIL_INVALID_PARAMS;
    goto fail;
  ...
  if (p2p_peer_channels(p2p, dev, msg.channel_list, msg.channel_list_len) < 0) {
    p2p_dbg(p2p, "No common channels found");
    status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
    goto fail;
  }
...
fail:
  ...
  dev->go_neg_conf = p2p_build_go_neg_conf(p2p, dev, msg.dialog_token, status,
...
void p2p_process_go_neg_conf(struct p2p_data *p2p, const u8 *sa, const u8 *data,
                             size_t len) {
  struct p2p_device *dev;
  struct p2p_message msg;

  p2p_dbg(p2p, "Received GO Negotiation Confirm from " MACSTR, MAC2STR(sa));
  dev = p2p_get_device(p2p, sa);
  if (dev == NULL || dev->wps_method == WPS_NOT_READY ||
      dev != p2p->go_neg_peer) {
    p2p_dbg(p2p, "Not ready for GO negotiation with " MACSTR, MAC2STR(sa));
  ...
  if (p2p->pending_action_state == P2P_PENDING_GO_NEG_RESPONSE) {
    p2p_dbg(p2p, "Stopped waiting for TX status on GO Negotiation Response "
                 "since we already received Confirmation");
    p2p->pending_action_state = P2P_NO_PENDING_ACTION;
  }
  ...
  if (!msg.operating_channel) {
    p2p_dbg(p2p, "Mandatory Operating Channel attribute missing from GO "
    ...
    if (oper_freq != dev->oper_freq) {
      p2p_dbg(p2p,
              "Updated peer (GO) operating channel preference from %d MHz to "
              "%d MHz",
