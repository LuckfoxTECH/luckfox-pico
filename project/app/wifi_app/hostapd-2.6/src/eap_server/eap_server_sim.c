...
struct eap_sim_data {
  ...
  int num_chal;
  enum { START, CHALLENGE, REAUTH, NOTIFICATION, SUCCESS, FAILURE } state;
  char *next_pseudonym;
  char *next_reauth_id;
  u16 counter;
...
static struct wpabuf *eap_sim_build_start(struct eap_sm *sm,
  ...
  eap_sim_msg_add(msg, EAP_SIM_AT_VERSION_LIST, sizeof(ver), ver, sizeof(ver));
...
static int eap_sim_build_encr(struct eap_sm *sm, struct eap_sim_data *data,
  ...
  if (nonce_s == NULL) {
    data->next_pseudonym =
        eap_sim_db_get_next_pseudonym(sm->eap_sim_db_priv, EAP_SIM_DB_SIM);
  } else {
  ...
  if (data->counter <= EAP_SIM_MAX_FAST_REAUTHS) {
    data->next_reauth_id =
        eap_sim_db_get_next_reauth_id(sm->eap_sim_db_priv, EAP_SIM_DB_SIM);
  } else {
...
eap_sim_build_challenge(struct eap_sm *sm, struct eap_sim_data *data, u8 id) {
  struct eap_sim_msg *msg;
  ...
  return eap_sim_msg_finish(msg, EAP_TYPE_SIM, data->k_aut, data->nonce_mt,
...
static struct wpabuf *eap_sim_build_reauth(struct eap_sm *sm,
  ...
  if (random_get_bytes(data->nonce_s, EAP_SIM_NONCE_S_LEN))
    return NULL;
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-SIM: NONCE_S", data->nonce_s,
                  EAP_SIM_NONCE_S_LEN);
...
static void eap_sim_process_start(struct eap_sm *sm, struct eap_sim_data *data,
                                  struct wpabuf *respData,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-SIM: Identity", sm->identity,
                    sm->identity_len);
  ...
  if (username[0] == EAP_SIM_REAUTH_ID_PREFIX) {
    wpa_printf(MSG_DEBUG, "EAP-SIM: Reauth username '%s'", username);
  ...
  if (username[0] == EAP_SIM_PSEUDONYM_PREFIX) {
    ...
    wpa_printf(MSG_DEBUG, "EAP-SIM: Pseudonym username '%s'", username);
    ...
    wpa_printf(MSG_DEBUG, "EAP-SIM: Permanent username '%s'", username);
    os_strlcpy(data->permanent, username, sizeof(data->permanent));
    os_free(username);
  } else {
    wpa_printf(MSG_DEBUG, "EAP-SIM: Unrecognized username '%s'", username);
    os_free(username);
    goto failed;
  }
...
skip_id_update:
  ...
  data->num_chal = eap_sim_db_get_gsm_triplets(
      sm->eap_sim_db_priv, data->permanent, EAP_SIM_MAX_CHAL, (u8 *)data->rand,
      (u8 *)data->kc, (u8 *)data->sres, sm);
...
static void eap_sim_process_challenge(struct eap_sm *sm,
                                      ...
                                      struct eap_sim_attrs *attr) {
  if (attr->mac == NULL ||
      eap_sim_verify_mac(data->k_aut, respData, attr->mac, (u8 *)data->sres,
...
static void eap_sim_process_reauth(struct eap_sm *sm, struct eap_sim_data *data,
                                   struct wpabuf *respData,
                                   struct eap_sim_attrs *attr) {
