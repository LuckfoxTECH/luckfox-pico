...
static struct wpabuf *eap_sake_build_msg(struct eap_sake_data *data, u8 id,
  ...
  msg =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_SAKE, plen, EAP_CODE_REQUEST, id);
  if (msg == NULL) {
...
eap_sake_build_identity(struct eap_sm *sm, struct eap_sake_data *data, u8 id) {
  struct wpabuf *msg;
  size_t plen;
  ...
  eap_sake_add_attr(msg, EAP_SAKE_AT_SERVERID, sm->server_id,
                    sm->server_id_len);
...
static struct wpabuf *
eap_sake_build_challenge(struct eap_sm *sm, struct eap_sake_data *data, u8 id) {
  struct wpabuf *msg;
  size_t plen;
  ...
  eap_sake_add_attr(msg, EAP_SAKE_AT_RAND_S, data->rand_s, EAP_SAKE_RAND_LEN);
...
static struct wpabuf *
eap_sake_build_confirm(struct eap_sm *sm, struct eap_sake_data *data, u8 id) {
  struct wpabuf *msg;
  u8 *mic;
  ...
  if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
                           sm->server_id, sm->server_id_len, data->peerid,
                           data->peerid_len, 0, wpabuf_head(msg),
                           wpabuf_len(msg), mic, mic)) {
...
static Boolean eap_sake_check(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_INFO, "EAP-SAKE: Unexpected subtype=%d in state=%d", subtype,
             data->state);
...
static void eap_sake_process_challenge(struct eap_sm *sm,
  ...
  eap_sake_derive_keys(
      sm->user->password, sm->user->password + EAP_SAKE_ROOT_SECRET_LEN,
      data->rand_s, data->rand_p, (u8 *)&data->tek, data->msk, data->emsk);

  eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
                       sm->server_id, sm->server_id_len, data->peerid,
                       data->peerid_len, 1, wpabuf_head(respData),
                       wpabuf_len(respData), attr.mic_p, mic_p);
...
static void eap_sake_process(struct eap_sm *sm, void *priv,
  ...
  case EAP_SAKE_SUBTYPE_AUTH_REJECT:
    eap_sake_process_auth_reject(sm, data, respData, pos, end - pos);
    break;
  }
}
