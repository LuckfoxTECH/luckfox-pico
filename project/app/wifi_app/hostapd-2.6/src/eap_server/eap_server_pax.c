...
static struct wpabuf *eap_pax_build_std_1(struct eap_sm *sm,
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PAX,
                      sizeof(*pax) + 2 + EAP_PAX_RAND_LEN + EAP_PAX_ICV_LEN,
                      EAP_CODE_REQUEST, id);
  if (req == NULL) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: A = X (server rand)", data->rand.r.x,
              EAP_PAX_RAND_LEN);

  pos = wpabuf_put(req, EAP_PAX_MAC_LEN);
  eap_pax_mac(data->mac_id, (u8 *)"", 0, wpabuf_mhead(req),
              wpabuf_len(req) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, pos);
...
static struct wpabuf *eap_pax_build_std_3(struct eap_sm *sm,
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PAX,
                      sizeof(*pax) + 2 + EAP_PAX_MAC_LEN + EAP_PAX_ICV_LEN,
                      EAP_CODE_REQUEST, id);
  if (req == NULL) {
    wpa_printf(MSG_ERROR, "EAP-PAX: Failed to allocate memory "
                          "request");
  ...
  pos = wpabuf_put(req, EAP_PAX_MAC_LEN);
  eap_pax_mac(data->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.y,
              EAP_PAX_RAND_LEN, (u8 *)data->cid, data->cid_len, NULL, 0, pos);
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: MAC_CK(B, CID)", pos, EAP_PAX_MAC_LEN);

  /* Optional ADE could be added here, if needed */

  pos = wpabuf_put(req, EAP_PAX_MAC_LEN);
  eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_mhead(req),
              wpabuf_len(req) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, pos);
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: ICV", pos, EAP_PAX_ICV_LEN);
...
static struct wpabuf *eap_pax_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  default:
    wpa_printf(MSG_DEBUG, "EAP-PAX: Unknown state %d in buildReq", data->state);
    break;
  }
  return NULL;
}
...
static Boolean eap_pax_check(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             resp->public_key_id);
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: received payload", (u8 *)(resp + 1),
              len - sizeof(*resp) - EAP_PAX_ICV_LEN);

  if (data->state == PAX_STD_1 && resp->op_code != EAP_PAX_OP_STD_2) {
    wpa_printf(MSG_DEBUG,
  ...
  if (data->state == PAX_STD_3 && resp->op_code != EAP_PAX_OP_ACK) {
    wpa_printf(MSG_DEBUG,
  ...
  if (resp->op_code != EAP_PAX_OP_STD_2 && resp->op_code != EAP_PAX_OP_ACK) {
    wpa_printf(MSG_DEBUG, "EAP-PAX: Unknown op_code 0x%x", resp->op_code);
  }
  ...
  if (resp->dh_group_id != EAP_PAX_DH_GROUP_NONE) {
    wpa_printf(MSG_INFO,
               ...
               "received 0x%x",
               EAP_PAX_DH_GROUP_NONE, resp->dh_group_id);
    return TRUE;
  }
  ...
  if (resp->public_key_id != EAP_PAX_PUBLIC_KEY_NONE) {
    wpa_printf(MSG_INFO,
               ...
               "received 0x%x",
               EAP_PAX_PUBLIC_KEY_NONE, resp->public_key_id);
    return TRUE;
  }
  ...
  if (data->keys_set) {
    ...
    eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN,
                wpabuf_mhead(respData), wpabuf_len(respData) - EAP_PAX_ICV_LEN,
                NULL, 0, NULL, 0, icvbuf);
    if (os_memcmp_const(icvbuf, icv, EAP_PAX_ICV_LEN) != 0) {
      wpa_printf(MSG_INFO, "EAP-PAX: Invalid ICV");
      wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: Expected ICV", icvbuf,
                  EAP_PAX_ICV_LEN);
      return TRUE;
    }
  }
...
static void eap_pax_process_std_2(struct eap_sm *sm, struct eap_pax_data *data,
                                  struct wpabuf *respData) {
  struct eap_pax_hdr *resp;
  ...
  if (left < 2 + EAP_PAX_RAND_LEN || WPA_GET_BE16(pos) != EAP_PAX_RAND_LEN) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: Y (client rand)", data->rand.r.y,
              EAP_PAX_RAND_LEN);
  ...
  pos += 2 + data->cid_len;
  left -= 2 + data->cid_len;
  wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-PAX: CID", (u8 *)data->cid,
                    data->cid_len);

  if (left < 2 + EAP_PAX_MAC_LEN || WPA_GET_BE16(pos) != EAP_PAX_MAC_LEN) {
    wpa_printf(MSG_INFO, "EAP-PAX: Too short PAX_STD-2 (MAC_CK)");
    return;
  }
  pos += 2;
  left -= 2;
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: MAC_CK(A, B, CID)", pos, EAP_PAX_MAC_LEN);

  if (eap_user_get(sm, (u8 *)data->cid, data->cid_len, 0) < 0) {
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-PAX: unknown CID", (u8 *)data->cid,
                      data->cid_len);
    data->state = FAILURE;
    return;
  }
  ...
  for (i = 0;
       i < EAP_MAX_METHODS && (sm->user->methods[i].vendor != EAP_VENDOR_IETF ||
  ...
  if (i >= EAP_MAX_METHODS || sm->user->methods[i].vendor != EAP_VENDOR_IETF ||
      sm->user->methods[i].method != EAP_TYPE_PAX) {
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-PAX: EAP-PAX not enabled for CID",
                      (u8 *)data->cid, data->cid_len);
    data->state = FAILURE;
    return;
  }

  if (sm->user->password == NULL || sm->user->password_len != EAP_PAX_AK_LEN) {
  ...
  if (eap_pax_initial_key_derivation(data->mac_id, data->ak, data->rand.e,
  ...
  eap_pax_mac(data->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.x,
              EAP_PAX_RAND_LEN, data->rand.r.y, EAP_PAX_RAND_LEN,
  ...
  if (os_memcmp_const(mac, pos, EAP_PAX_MAC_LEN) != 0) {
    wpa_printf(MSG_INFO, "EAP-PAX: Invalid MAC_CK(A, B, CID) in "
                         "PAX_STD-2");
    wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: Expected MAC_CK(A, B, CID)", mac,
                EAP_PAX_MAC_LEN);
    data->state = FAILURE;
    return;
  }
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: ICV", pos, EAP_PAX_ICV_LEN);
  eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(respData),
              wpabuf_len(respData) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, icvbuf);
...
static void eap_pax_process_ack(struct eap_sm *sm, struct eap_pax_data *data,
                                struct wpabuf *respData) {
...
static u8 *eap_pax_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  *len = EAP_MSK_LEN;
  eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Master Session Key",
              data->rand.e, 2 * EAP_PAX_RAND_LEN, EAP_MSK_LEN, key);
...
static u8 *eap_pax_get_emsk(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  *len = EAP_EMSK_LEN;
  eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN,
              "Extended Master Session Key", data->rand.e, 2 * EAP_PAX_RAND_LEN,
              EAP_EMSK_LEN, key);
