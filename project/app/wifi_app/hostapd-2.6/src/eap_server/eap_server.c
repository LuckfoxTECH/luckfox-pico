...
static int eap_copy_data(u8 **dst, size_t *dst_len, const u8 *src,
                         size_t src_len) {
  if (src == NULL)
    return -1;
...
int eap_user_get(struct eap_sm *sm, const u8 *identity, size_t identity_len,
  ...
  if (sm == NULL || sm->eapol_cb == NULL || sm->eapol_cb->get_eap_user == NULL)
    return -1;
  ...
  if (sm->eapol_cb->get_eap_user(sm->eapol_ctx, identity, identity_len, phase2,
                                 user) != 0) {
    eap_user_free(user);
    return -1;
  }
...
SM_STATE(EAP, INITIALIZE) {
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_STARTED MACSTR,
...
SM_STATE(EAP, PICK_UP_METHOD) {
  ...
  if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
    ...
    sm->m = eap_server_get_eap_method(EAP_VENDOR_IETF, sm->currentMethod);
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_PROPOSED_METHOD "method=%u",
          sm->currentMethod);
}
...
SM_STATE(EAP, IDLE) {
  ...
  sm->eap_if.retransWhile =
      eap_sm_calculateTimeout(sm, sm->retransCount, sm->eap_if.eapSRTT,
                              sm->eap_if.eapRTTVAR, sm->methodTimeout);
}
...
SM_STATE(EAP, INTEGRITY_CHECK) {
  ...
  if (sm->m->check) {
    sm->ignore = sm->m->check(sm, sm->eap_method_priv, sm->eap_if.eapRespData);
  }
}
...
SM_STATE(EAP, METHOD_REQUEST) {
  ...
  sm->eap_if.eapReqData =
      sm->m->buildReq(sm, sm->eap_method_priv, sm->currentId);
  if (sm->m->getTimeout)
...
SM_STATE(EAP, METHOD_RESPONSE) {
  ...
  if (sm->m->isDone(sm, sm->eap_method_priv)) {
    ...
    bin_clear_free(sm->eap_if.eapKeyData, sm->eap_if.eapKeyDataLen);
    if (sm->m->getKey) {
      sm->eap_if.eapKeyData =
          sm->m->getKey(sm, sm->eap_method_priv, &sm->eap_if.eapKeyDataLen);
    } else {
      sm->eap_if.eapKeyData = NULL;
      sm->eap_if.eapKeyDataLen = 0;
    }
    ...
    if (sm->m->getSessionId) {
      sm->eap_if.eapSessionId = sm->m->getSessionId(
          sm, sm->eap_method_priv, &sm->eap_if.eapSessionIdLen);
      wpa_hexdump(MSG_DEBUG, "EAP: Session-Id", sm->eap_if.eapSessionId,
...
try_another_method:
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u", vendor,
          sm->currentMethod);
  eap_log_msg(sm, "Propose EAP method vendor=%u method=%u", vendor,
              sm->currentMethod);
}
...
SM_STATE(EAP, FAILURE) {
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_FAILURE MACSTR,
          MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, SUCCESS) {
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_SUCCESS MACSTR,
          MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, INITIATE_REAUTH_START) {
  ...
  sm->lastId = sm->currentId;
  wpabuf_free(sm->eap_if.eapReqData);
  sm->eap_if.eapReqData = eap_sm_buildInitiateReauthStart(sm, sm->currentId);
  wpabuf_free(sm->lastReqData);
  sm->lastReqData = NULL;
}
...
static void erp_send_finish_reauth(struct eap_sm *sm,
  ...
  if (hash_len)
    plen += 1 + hash_len;
  msg = eap_msg_alloc(EAP_VENDOR_IETF, (EapType)EAP_ERP_TYPE_REAUTH, plen,
                      EAP_CODE_FINISH, id);
  if (msg == NULL)
    return;
  ...
  if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
                      ...
                      sizeof(seed), sm->eap_if.eapKeyData, erp->rRK_len) < 0) {
  ...
  sm->eap_if.eapKeyAvailable = TRUE;
  wpa_hexdump_key(MSG_DEBUG, "EAP: ERP rMSK", sm->eap_if.eapKeyData,
...
SM_STATE(EAP, INITIATE_RECEIVED) {
  ...
  if (!parse.keyname) {
    wpa_printf(MSG_DEBUG, "EAP: No keyName-NAI in EAP-Initiate/Re-auth Packet");
    goto fail;
  }
  ...
  if (parse.keyname_len > 253) {
    wpa_printf(MSG_DEBUG, "EAP: Too long keyName-NAI in EAP-Initiate/Re-auth");
    goto fail;
  }
  ...
  if (!sm->eap_server) {
    ...
    eap_server_clear_identity(sm);
    sm->identity = (u8 *)dup_binstr(parse.keyname, parse.keyname_len);
    if (!sm->identity)
      goto fail;
  ...
  if (erp->recv_seq != (u32)-1 && erp->recv_seq >= seq) {
    wpa_printf(MSG_DEBUG, "EAP: SEQ=%u replayed (already received SEQ=%u)", seq,
               erp->recv_seq);
    goto fail;
  }
  ...
  if (hash_len) {
    if (hmac_sha256(erp->rIK, erp->rIK_len, hdr, end - hdr - hash_len, hash) <
        0)
      goto fail;
  ...
  if (!hash_len && max_len >= 1 + 32 &&
    ...
    if (hmac_sha256(erp->rIK, erp->rIK_len, hdr, end - hdr - 32, hash) < 0)
      goto fail;
  ...
  if (!hash_len && end[-17] == EAP_ERP_CS_HMAC_SHA256_128) {
    if (hmac_sha256(erp->rIK, erp->rIK_len, hdr, end - hdr - 16, hash) < 0)
      goto fail;
   ...
   * that contains them.
   */
  wpa_hexdump(MSG_DEBUG, "EAP: EAP-Initiate/Re-Auth TVs/TLVs", tlvs,
              end - tlvs);
  if (erp_parse_tlvs(tlvs, end, &parse, 0) < 0)
    goto fail;

  wpa_printf(MSG_DEBUG, "EAP: ERP key %s SEQ updated to %u", erp->keyname_nai,
             seq);
  erp->recv_seq = seq;
  ...
  if (sm->eap_if.eapRestart && sm->eap_if.portEnabled)
    ...
    case EAP_INITIALIZE:
      if (sm->backend_auth) {
        ...
        else if (sm->rxResp && (sm->respMethod == EAP_TYPE_NAK ||
    ...
    case EAP_RECEIVED:
      if (sm->rxResp && (sm->respId == sm->currentId) &&
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   sm->rxResp, sm->respId, sm->currentId, sm->respMethod,
                   sm->currentMethod);
    ...
    case EAP_METHOD_REQUEST:
      ...
      if (sm->eap_if.eapNoReq && !sm->eap_if.eapReq) {
         ...
         * buildReq fails.
         */
        wpa_printf(MSG_DEBUG, "EAP: Method did not return a request");
        SM_ENTER(EAP, FAILURE);
        break;
      }
      break;
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp) {
  ...
  if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
    wpa_printf(MSG_DEBUG,
               ...
               "len=%lu",
               resp, resp ? (unsigned long)wpabuf_len(resp) : 0);
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "EAP: parseEapResp: rxResp=%d rxInitiate=%d respId=%d "
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP: configured methods", (u8 *)sm->user->methods,
  ...
  not_found:
    ...
    if (i + 1 < EAP_MAX_METHODS) {
      os_memmove(&sm->user->methods[i], &sm->user->methods[i + 1],
                 (EAP_MAX_METHODS - i - 1) * sizeof(sm->user->methods[0]));
    }
    sm->user->methods[EAP_MAX_METHODS - 1].vendor = EAP_VENDOR_IETF;
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor) {
  ...
  wpa_printf(MSG_DEBUG, "EAP: getNextMethod: vendor %d type %d", *vendor, next);
  return next;
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm) {
  ...
  if ((sm->user == NULL || sm->update_user) && sm->identity &&
    ...
    if (id_req && sm->user && sm->user->methods[0].vendor == EAP_VENDOR_IETF &&
        sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
  ...
  if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
      (sm->user->methods[sm->user_eap_method_index].vendor != EAP_VENDOR_IETF ||
  ...
  if (!sm->identity && eap_get_erp_send_reauth_start(sm) &&
    ...
    wpa_printf(MSG_DEBUG, "EAP: getDecision: send EAP-Initiate/Re-auth-Start");
...
struct eap_sm *eap_server_sm_init(void *eapol_ctx,
  ...
  if (conf->pac_opaque_encr_key) {
    ...
    if (sm->pac_opaque_encr_key) {
      os_memcpy(sm->pac_opaque_encr_key, conf->pac_opaque_encr_key, 16);
    }
  }
...
void eap_server_mschap_rx_callback(struct eap_sm *sm, const char *source,
  ...
  if (username)
    ...
    user[0] = '\0';
  wpa_snprintf_hex_sep(hex_challenge, sizeof(hex_challenge), challenge,
                       sizeof(challenge), ':');
  wpa_snprintf_hex_sep(hex_response, sizeof(hex_response), response, 24, ':');
  wpa_printf(MSG_DEBUG, "[%s/user=%s] asleap -C %s -R %s", source, user,
             hex_challenge, hex_response);
}
#endif /* CONFIG_TESTING_OPTIONS */
