...
static struct wpabuf *eap_psk_build_1(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PSK: RAND_S (server rand)", data->rand_s,
              EAP_PSK_RAND_LEN);

  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PSK,
                      sizeof(*psk) + sm->server_id_len, EAP_CODE_REQUEST, id);
  if (req == NULL) {
...
static struct wpabuf *eap_psk_build_3(struct eap_sm *sm,
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PSK, sizeof(*psk) + 4 + 16 + 1,
                      EAP_CODE_REQUEST, id);
  if (req == NULL) {
    wpa_printf(MSG_ERROR, "EAP-PSK: Failed to allocate memory "
                          "request");
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-PSK: PCHANNEL (plaintext)", pchannel, 4 + 16 + 1);
  if (aes_128_eax_encrypt(data->tek, nonce, sizeof(nonce), wpabuf_head(req), 22,
                          pchannel + 4 + 16, 1, pchannel + 4))
    goto fail;
  wpa_hexdump(MSG_DEBUG, "EAP-PSK: PCHANNEL (encrypted)", pchannel, 4 + 16 + 1);
...
static struct wpabuf *eap_psk_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  default:
    wpa_printf(MSG_DEBUG, "EAP-PSK: Unknown state %d in buildReq", data->state);
    break;
  }
  return NULL;
}
...
static void eap_psk_process_2(struct eap_sm *sm, struct eap_psk_data *data,
                              struct wpabuf *respData) {
  ...
  cpos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_PSK, respData, &left);
  ...
  data->id_p_len = left;
  wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-PSK: ID_P", data->id_p, data->id_p_len);

  if (eap_user_get(sm, data->id_p, data->id_p_len, 0) < 0) {
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-PSK: unknown ID_P", data->id_p,
                      data->id_p_len);
    data->state = FAILURE;
    return;
  }
  ...
  if (i >= EAP_MAX_METHODS || sm->user->methods[i].vendor != EAP_VENDOR_IETF ||
      sm->user->methods[i].method != EAP_TYPE_PSK) {
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-PSK: EAP-PSK not enabled for ID_P",
                      data->id_p, data->id_p_len);
    data->state = FAILURE;
    return;
  }

  if (sm->user->password == NULL || sm->user->password_len != EAP_PSK_PSK_LEN) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PSK: RAND_P (client rand)", resp->rand_p,
              EAP_PSK_RAND_LEN);
  ...
  if (os_memcmp_const(mac, resp->mac_p, EAP_PSK_MAC_LEN) != 0) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "EAP-PSK: Expected MAC_P", mac, EAP_PSK_MAC_LEN);
    data->state = FAILURE;
    return;
  }
...
static void eap_psk_process_4(struct eap_sm *sm, struct eap_psk_data *data,
                              struct wpabuf *respData) {
  ...
  if (aes_128_eax_decrypt(data->tek, nonce, sizeof(nonce),
                          wpabuf_head(respData), 22, decrypted, left, tag)) {
    wpa_printf(MSG_WARNING, "EAP-PSK: PCHANNEL decryption failed");
    os_free(decrypted);
    data->state = FAILURE;
    return;
  }
  wpa_hexdump(MSG_DEBUG, "EAP-PSK: Decrypted PCHANNEL message", decrypted,
              left);
