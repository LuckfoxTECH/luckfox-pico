...
static void eap_ttls_state(struct eap_ttls_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-TTLS: %s -> %s", eap_ttls_state_txt(data->state),
...
static u8 *eap_ttls_avp_hdr(u8 *avphdr, u32 avp_code, u32 vendor_id,
  ...
  avp->avp_length = host_to_be32(((u32)flags << 24) | ((u32)(hdrlen + len)));
...
static int eap_ttls_avp_parse(struct wpabuf *buf, struct eap_ttls_avp *parse) {
  ...
  while (left > 0) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "length=%d",
               (int)avp_code, avp_flags, (int)avp_length);
    ...
    if (avp_flags & AVP_FLAGS_VENDOR) {
      ...
      wpa_printf(MSG_DEBUG, "EAP-TTLS: AVP vendor_id %d", (int)vendor_id);
      dpos += 4;
      dlen -= 4;
    }
    ...
    if (vendor_id == 0 && avp_code == RADIUS_ATTR_EAP_MESSAGE) {
      ...
      if (parse->eap == NULL) {
        ...
        u8 *neweap = os_realloc(parse->eap, parse->eap_len + dlen);
        if (neweap == NULL) {
          wpa_printf(MSG_WARNING, "EAP-TTLS: "
                                  "failed to allocate memory "
                                  "for Phase 2 EAP data");
    ...
    } else if (vendor_id == 0 && avp_code == RADIUS_ATTR_USER_NAME) {
      wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: User-Name", dpos, dlen);
      parse->user_name = dpos;
      parse->user_name_len = dlen;
    } else if (vendor_id == 0 && avp_code == RADIUS_ATTR_USER_PASSWORD) {
      u8 *password = dpos;
      size_t password_len = dlen;
      while (password_len > 0 && password[password_len - 1] == '\0') {
        password_len--;
      }
    ...
    } else if (vendor_id == 0 && avp_code == RADIUS_ATTR_CHAP_CHALLENGE) {
      wpa_hexdump(MSG_DEBUG, "EAP-TTLS: CHAP-Challenge (CHAP)", dpos, dlen);
      parse->chap_challenge = dpos;
      parse->chap_challenge_len = dlen;
    } else if (vendor_id == 0 && avp_code == RADIUS_ATTR_CHAP_PASSWORD) {
      wpa_hexdump(MSG_DEBUG, "EAP-TTLS: CHAP-Password (CHAP)", dpos, dlen);
      parse->chap_password = dpos;
      parse->chap_password_len = dlen;
    } else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
               avp_code == RADIUS_ATTR_MS_CHAP_CHALLENGE) {
      wpa_hexdump(MSG_DEBUG, "EAP-TTLS: MS-CHAP-Challenge", dpos, dlen);
      parse->mschap_challenge = dpos;
      parse->mschap_challenge_len = dlen;
    } else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
               avp_code == RADIUS_ATTR_MS_CHAP_RESPONSE) {
      wpa_hexdump(MSG_DEBUG, "EAP-TTLS: MS-CHAP-Response (MSCHAP)", dpos, dlen);
      parse->mschap_response = dpos;
      parse->mschap_response_len = dlen;
    } else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
               avp_code == RADIUS_ATTR_MS_CHAP2_RESPONSE) {
      wpa_hexdump(MSG_DEBUG, "EAP-TTLS: MS-CHAP2-Response (MSCHAPV2)", dpos,
                  dlen);
...
static u8 *eap_ttls_implicit_challenge(struct eap_sm *sm,
  ...
  return eap_server_tls_derive_key(sm, &data->ssl, "ttls challenge", len);
}
...
static struct wpabuf *eap_ttls_build_phase2_eap_req(struct eap_sm *sm,
  ...
  wpa_hexdump_buf_key(MSG_DEBUG,
                      "EAP-TTLS/EAP: Encrypt encapsulated "
                      "Phase 2 data",
                      buf);
...
eap_ttls_build_phase2_mschapv2(struct eap_sm *sm, struct eap_ttls_data *data) {
  ...
  if (data->mschapv2_resp_ok) {
    ...
    *pos++ = data->mschapv2_ident;
    ...
    if (!os_snprintf_error(end - pos, ret))
      pos += ret;
    pos += wpa_snprintf_hex_uppercase((char *)pos, end - pos,
...
static struct wpabuf *eap_ttls_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  if (data->ssl.state == FRAG_ACK) {
    return eap_server_tls_build_ack(id, EAP_TYPE_TTLS, data->ttls_version);
  }
  ...
  case PHASE2_MSCHAPV2_RESP:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-TTLS: %s - unexpected state %d", __func__,
               data->state);
    return NULL;
  }

  return eap_server_tls_build_msg(&data->ssl, EAP_TYPE_TTLS, data->ttls_version,
                                  id);
}
...
static void eap_ttls_process_phase2_pap(struct eap_sm *sm,
  ...
  if (sm->user->password_len != user_password_len ||
      os_memcmp_const(sm->user->password, user_password, user_password_len) !=
          0) {
...
static void
eap_ttls_process_phase2_chap(struct eap_sm *sm, struct eap_ttls_data *data,
  ...
  if (challenge == NULL || password == NULL ||
    ...
    wpa_printf(MSG_DEBUG,
               ...
               (unsigned long)challenge_len, (unsigned long)password_len);
    eap_ttls_state(data, FAILURE);
    return;
  }
  ...
  chal = eap_ttls_implicit_challenge(sm, data, EAP_TTLS_CHAP_CHALLENGE_LEN + 1);
  if (chal == NULL) {
  ...
  if (os_memcmp_const(challenge, chal, EAP_TTLS_CHAP_CHALLENGE_LEN) != 0 ||
  ...
  chap_md5(password[0], sm->user->password, sm->user->password_len, challenge,
           challenge_len, hash);

  if (os_memcmp_const(hash, password + 1, EAP_TTLS_CHAP_PASSWORD_LEN) == 0) {
...
static void eap_ttls_process_phase2_mschap(struct eap_sm *sm,
  ...
  if (challenge == NULL || response == NULL ||
    ...
    wpa_printf(MSG_DEBUG,
               ...
               (unsigned long)challenge_len, (unsigned long)response_len);
    eap_ttls_state(data, FAILURE);
    return;
  }
  ...
  chal =
      eap_ttls_implicit_challenge(sm, data, EAP_TTLS_MSCHAP_CHALLENGE_LEN + 1);
  if (chal == NULL) {
  ...
  eap_server_mschap_rx_callback(sm, "TTLS-MSCHAP", sm->identity,
                                sm->identity_len, challenge, response + 2 + 24);
#endif /* CONFIG_TESTING_OPTIONS */

  if (os_memcmp_const(challenge, chal, EAP_TTLS_MSCHAP_CHALLENGE_LEN) != 0 ||
  ...
  if (sm->user->password_hash)
    ...
    nt_challenge_response(challenge, sm->user->password, sm->user->password_len,
                          nt_response);
  ...
  if (os_memcmp_const(nt_response, response + 2 + 24, 24) == 0) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAP: Received", response + 2 + 24,
                24);
    wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAP: Expected", nt_response, 24);
    eap_ttls_state(data, FAILURE);
  }
}
...
static void eap_ttls_process_phase2_mschapv2(struct eap_sm *sm,
  ...
  if (os_memcmp_const(challenge, chal, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN) != 0 ||
  ...
  wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-TTLS/MSCHAPV2: User", username,
                    username_len);
  wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAPV2: auth_challenge", auth_challenge,
              EAP_TTLS_MSCHAPV2_CHALLENGE_LEN);
  wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAPV2: peer_challenge", peer_challenge,
              EAP_TTLS_MSCHAPV2_CHALLENGE_LEN);

  if (sm->user->password_hash) {
    generate_nt_response_pwhash(auth_challenge, peer_challenge, username,
                                username_len, sm->user->password, nt_response);
  } else {
    generate_nt_response(auth_challenge, peer_challenge, username, username_len,
...
#ifdef CONFIG_TESTING_OPTIONS
  {
    ...
    if (challenge_hash(peer_challenge, auth_challenge, username, username_len,
                       challenge2) == 0) {
      eap_server_mschap_rx_callback(sm, "TTLS-MSCHAPV2", username, username_len,
                                    challenge2, rx_resp);
    }
  }
#endif /* CONFIG_TESTING_OPTIONS */
  if (os_memcmp_const(nt_response, rx_resp, 24) == 0) {
    ...
    if (sm->user->password_hash) {
      generate_authenticator_response_pwhash(
          sm->user->password, peer_challenge, auth_challenge, username,
          username_len, nt_response, data->mschapv2_auth_response);
    } else {
      generate_authenticator_response(
          sm->user->password, sm->user->password_len, peer_challenge,
          auth_challenge, username, username_len, nt_response,
          data->mschapv2_auth_response);
    }
  } else {
    wpa_printf(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Invalid "
                          "NT-Response");
    wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAPV2: Received", rx_resp, 24);
    wpa_hexdump(MSG_MSGDUMP, "EAP-TTLS/MSCHAPV2: Expected", nt_response, 24);
...
static void eap_ttls_process_phase2_eap_response(struct eap_sm *sm,
  ...
  if (in_len > sizeof(*hdr) && *pos == EAP_TYPE_NAK) {
    ...
    if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
        sm->user->methods[sm->user_eap_method_index].method != EAP_TYPE_NONE) {
      next_type = sm->user->methods[sm->user_eap_method_index++].method;
...
static void eap_ttls_process_phase2_eap(struct eap_sm *sm,
  ...
  case EAP_CODE_RESPONSE:
    eap_ttls_process_phase2_eap_response(sm, data, (u8 *)hdr, len);
    break;
  default:
    wpa_printf(MSG_INFO,
...
static void eap_ttls_process_phase2(struct eap_sm *sm,
  ...
  if (data->pending_phase2_eap_resp) {
    ...
    eap_ttls_process_phase2_eap(sm, data,
                                wpabuf_head(data->pending_phase2_eap_resp),
  ...
  in_decrypted = tls_connection_decrypt(sm->ssl_ctx, data->ssl.conn, in_buf);
  ...
  if (parse.user_name) {
    ...
    if (nbuf) {
      printf_encode(nbuf, parse.user_name_len * 4 + 1, parse.user_name,
                    parse.user_name_len);
    ...
    if (eap_user_get(sm, parse.user_name, parse.user_name_len, 1) != 0) {
  ...
  if (parse.eap) {
    eap_ttls_process_phase2_eap(sm, data, parse.eap, parse.eap_len);
    ...
    eap_ttls_process_phase2_chap(sm, data, parse.chap_challenge,
                                 parse.chap_challenge_len, parse.chap_password,
                                 parse.chap_password_len);
  } else if (parse.mschap_response) {
    eap_ttls_process_phase2_mschap(
        sm, data, parse.mschap_challenge, parse.mschap_challenge_len,
        parse.mschap_response, parse.mschap_response_len);
  } else if (parse.mschap2_response) {
    eap_ttls_process_phase2_mschapv2(
        sm, data, parse.mschap_challenge, parse.mschap_challenge_len,
        parse.mschap2_response, parse.mschap2_response_len);
  }
...
static void eap_ttls_process_msg(struct eap_sm *sm, void *priv,
  ...
  case PHASE2_METHOD:
    ...
    break;
  case PHASE2_MSCHAPV2_RESP:
    if (data->mschapv2_resp_ok && wpabuf_len(data->ssl.tls_in) == 0) {
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 (unsigned long)wpabuf_len(data->ssl.tls_in));
      eap_ttls_state(data, FAILURE);
    }
    eap_ttls_start_tnc(sm, data);
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-TTLS: Unexpected state %d in %s", data->state,
               __func__);
    break;
  }
}
...
static void eap_ttls_process(struct eap_sm *sm, void *priv,
  ...
  if (eap_server_tls_process(sm, &data->ssl, respData, data, EAP_TYPE_TTLS,
                             eap_ttls_process_version,
  ...
  pos++;
  id_len = *pos++;
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: Identity from cached session", pos,
                    id_len);
  os_free(sm->identity);
  ...
  wpa_printf(MSG_DEBUG, "EAP-TTLS: Resuming previous session - skip Phase2");
  eap_ttls_state(data, SUCCESS);
...
static u8 *eap_ttls_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  eapKeyData = eap_server_tls_derive_key(sm, &data->ssl, "ttls keying material",
                                         EAP_TLS_KEY_LEN);
  if (eapKeyData) {
    *len = EAP_TLS_KEY_LEN;
    wpa_hexdump_key(MSG_DEBUG, "EAP-TTLS: Derived key", eapKeyData,
                    EAP_TLS_KEY_LEN);
  } else {
...
static u8 *eap_ttls_get_session_id(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  return eap_server_tls_derive_session_id(sm, &data->ssl, EAP_TYPE_TTLS, len);
}
...
static u8 *eap_ttls_get_emsk(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  if (eapKeyData) {
    ...
    if (emsk)
      os_memcpy(emsk, eapKeyData + EAP_TLS_KEY_LEN, EAP_EMSK_LEN);
  ...
  if (emsk) {
    *len = EAP_EMSK_LEN;
    wpa_hexdump(MSG_DEBUG, "EAP-TTLS: Derived EMSK", emsk, EAP_EMSK_LEN);
  } else {
