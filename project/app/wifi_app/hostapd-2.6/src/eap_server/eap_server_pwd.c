...
struct eap_pwd_data {
  enum { PWD_ID_Req, PWD_Commit_Req, PWD_Confirm_Req, SUCCESS, FAILURE } state;
  u8 *id_peer;
  size_t id_peer_len;
  u8 *id_server;
...
static void *eap_pwd_init(struct eap_sm *sm) {
  ...
  wpa_printf(MSG_DEBUG, "EAP-pwd: Selected group number %d", data->group_num);
...
static void eap_pwd_build_id_req(struct eap_sm *sm, struct eap_pwd_data *data,
  ...
  data->outbuf = wpabuf_alloc(sizeof(struct eap_pwd_id) + data->id_server_len);
  if (data->outbuf == NULL) {
...
static void eap_pwd_build_commit_req(struct eap_sm *sm,
  ...
  if (BN_rand_range(data->private_value, data->grp->order) != 1 ||
      ...
      BN_mod(data->my_scalar, data->my_scalar, data->grp->order, data->bnctx) !=
          1) {
    wpa_printf(MSG_INFO, "EAP-pwd (server): unable to get randomness");
    goto fin;
  }
...
static void eap_pwd_build_confirm_req(struct eap_sm *sm,
  ...
  /* peer element: x, y */
  if (!EC_POINT_get_affine_coordinates_GFp(data->grp->group, data->peer_element,
                                           x, y, data->bnctx)) {
    wpa_printf(MSG_INFO, "EAP-PWD (server): confirm point "
                         "assignment fail");
    goto fin;
  ...
  /* peer scalar */
  os_memset(cruft, 0, BN_num_bytes(data->grp->prime));
  offset = BN_num_bytes(data->grp->order) - BN_num_bytes(data->peer_scalar);
...
static struct wpabuf *eap_pwd_build_req(struct eap_sm *sm, void *priv, u8 id) {
  struct eap_pwd_data *data = priv;
  struct wpabuf *req;
  u8 lm_exch;
  const u8 *buf;
  ...
  if (data->in_frag_pos) {
    ...
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PWD, EAP_PWD_HDR_SIZE,
                        EAP_CODE_REQUEST, id);
    if (req == NULL) {
      eap_pwd_state(data, FAILURE);
      return NULL;
  ...
  case PWD_Confirm_Req:
    ...
    break;
  default:
    wpa_printf(MSG_INFO, "EAP-pwd: Unknown state %d in build_req", data->state);
    eap_pwd_state(data, FAILURE);
  ...
  if ((len + EAP_PWD_HDR_SIZE) > data->mtu) {
    ...
    if (data->out_frag_pos == 0) {
      ...
      totlen = wpabuf_len(data->outbuf) + EAP_PWD_HDR_SIZE + sizeof(u16);
      len -= sizeof(u16);
    ...
    wpa_printf(MSG_DEBUG, "EAP-pwd: Send a %d byte fragment", (int)len);
  }
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PWD,
                      EAP_PWD_HDR_SIZE + len + (totlen ? sizeof(u16) : 0),
                      EAP_CODE_REQUEST, id);
  if (req == NULL) {
    eap_pwd_state(data, FAILURE);
    return NULL;
...
static Boolean eap_pwd_check(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_INFO, "EAP-pwd: Unexpected opcode=%d in state=%d", *pos,
             data->state);
...
static void eap_pwd_process_id_resp(struct eap_sm *sm,
  ...
  res = compute_password_element(data->grp, data->group_num, password,
                                 password_len, data->id_server,
                                 data->id_server_len, data->id_peer,
                                 data->id_peer_len, (u8 *)&data->token);
...
static void eap_pwd_process_commit_resp(struct eap_sm *sm,
                                        struct eap_pwd_data *data,
                                        const u8 *payload, size_t payload_len) {
  u8 *ptr;
  ...
  if (((data->peer_scalar = BN_new()) == NULL) ||
      ((data->k = BN_new()) == NULL) || ((cofactor = BN_new()) == NULL) ||
      ((x = BN_new()) == NULL) || ((y = BN_new()) == NULL) ||
  ...
  if (!EC_POINT_set_affine_coordinates_GFp(data->grp->group, data->peer_element,
                                           x, y, data->bnctx)) {
  ...
  if (BN_cmp(cofactor, BN_value_one())) {
    if (!EC_POINT_mul(data->grp->group, point, NULL, data->peer_element,
                      cofactor, NULL)) {
...
static void eap_pwd_process_confirm_resp(struct eap_sm *sm,
                                         struct eap_pwd_data *data,
                                         const u8 *payload,
                                         size_t payload_len) {
  BIGNUM *x = NULL, *y = NULL;
  ...
  *ptr = EAP_PWD_DEFAULT_PRF;
  ...
  if (compute_keys(data->grp, data->bnctx, data->k, data->peer_scalar,
                   data->my_scalar, conf, data->my_confirm, &cs, data->msk,
...
static void eap_pwd_process(struct eap_sm *sm, void *priv,
  ...
  if ((pos == NULL) || (len < 1)) {
    wpa_printf(MSG_INFO, "Bad EAP header! pos %s and len = %d",
               (pos == NULL) ? "is NULL" : "is not NULL", (int)len);
    return;
  }
  ...
  if (EAP_PWD_GET_MORE_BIT(lm_exch)) {
    wpa_printf(MSG_DEBUG, "EAP-pwd: Got a %d byte fragment", (int)len);
    return;
  }
  /*
