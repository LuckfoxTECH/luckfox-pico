...
static void eap_peap_state(struct eap_peap_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-PEAP: %s -> %s", eap_peap_state_txt(data->state),
...
static void *eap_peap_init(struct eap_sm *sm) {
  ...
  if (sm->user && sm->user->force_version >= 0) {
    ...
    wpa_printf(MSG_DEBUG, "EAP-PEAP: forcing version %d", data->force_version);
...
static struct wpabuf *eap_peap_build_start(struct eap_sm *sm,
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PEAP, 1, EAP_CODE_REQUEST, id);
  if (req == NULL) {
...
static struct wpabuf *eap_peap_build_phase2_req(struct eap_sm *sm,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 data", req, req_len);
...
static struct wpabuf *eap_peap_build_phase2_soh(struct eap_sm *sm,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 SOH data", req,
                  req_len);
...
static void eap_peap_get_isk(struct eap_peap_data *data, u8 *isk,
                             size_t isk_len) {
  size_t key_len;
...
static struct wpabuf *eap_peap_build_phase2_tlv(struct eap_sm *sm,
  ...
  buf =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TLV, mlen, EAP_CODE_REQUEST, id);
  if (buf == NULL)
    return NULL;
  ...
  wpa_hexdump_buf_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 TLV data", buf);
...
static struct wpabuf *eap_peap_build_phase2_term(struct eap_sm *sm,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 data", (u8 *)hdr,
                  req_len);
...
static struct wpabuf *eap_peap_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  if (data->ssl.state == FRAG_ACK) {
    return eap_server_tls_build_ack(id, EAP_TYPE_PEAP, data->peap_version);
  }
  ...
  case FAILURE_REQ:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-PEAP: %s - unexpected state %d", __func__,
               data->state);
    return NULL;
  }

  return eap_server_tls_build_msg(&data->ssl, EAP_TYPE_PEAP, data->peap_version,
                                  id);
}
...
static int eap_tlv_validate_cryptobinding(struct eap_sm *sm,
  ...
  if (os_memcmp_const(mac, pos, SHA1_MAC_LEN) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Cryptobinding seed data", buf, 61);
    return -1;
  }
...
static void eap_peap_process_phase2_tlv(struct eap_sm *sm,
  ...
  if (result_tlv) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Result TLV", result_tlv, result_tlv_len);
    if (result_tlv_len < 2) {
      wpa_printf(MSG_INFO,
    ...
    requested = data->tlv_request == TLV_REQ_SUCCESS ? "Success" : "Failure";
...
static void eap_peap_process_phase2_soh(struct eap_sm *sm,
  ...
  if (soh_tlv) {
    int failure = 0;
    wpabuf_free(data->soh_response);
    data->soh_response = tncs_process_soh(soh_tlv, soh_tlv_len, &failure);
    if (failure) {
      eap_peap_state(data, FAILURE);
      return;
    }
...
static void eap_peap_process_phase2_response(struct eap_sm *sm,
  ...
  if (data->phase2_method->getKey) {
    data->phase2_key = data->phase2_method->getKey(sm, data->phase2_priv,
                                                   &data->phase2_key_len);
    if (data->phase2_key == NULL) {
  ...
  case PHASE2_SOH:
...
#ifdef EAP_SERVER_TNC
    if (data->state != PHASE2_SOH && sm->tnc && data->peap_version == 0) {
    ...
    next_type = sm->user->methods[0].method;
    sm->user_eap_method_index = 1;
    wpa_printf(MSG_DEBUG, "EAP-PEAP: try EAP vendor %d type 0x%x", next_vendor,
               next_type);
    break;
  case PHASE2_METHOD:
...
static void eap_peap_process_phase2(struct eap_sm *sm,
  ...
  if (data->pending_phase2_resp) {
    ...
    eap_peap_process_phase2_response(sm, data, data->pending_phase2_resp);
    wpabuf_free(data->pending_phase2_resp);
...
static void eap_peap_process_msg(struct eap_sm *sm, void *priv,
  ...
  case FAILURE_REQ:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-PEAP: Unexpected state %d in %s", data->state,
               __func__);
    break;
  }
}
...
static void eap_peap_process(struct eap_sm *sm, void *priv,
  ...
  if (eap_server_tls_process(sm, &data->ssl, respData, data, EAP_TYPE_PEAP,
  ...
  pos++;
  id_len = *pos++;
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-PEAP: Identity from cached session", pos,
                    id_len);
  os_free(sm->identity);
  ...
  wpa_printf(MSG_DEBUG, "EAP-PEAP: Resuming previous session - skip Phase2");
  eap_peap_req_success(sm, data);
  if (data->state == SUCCESS_REQ)
...
static u8 *eap_peap_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  if (data->crypto_binding_used) {
    ...
    if (eapKeyData) {
      ...
      *len = EAP_TLS_KEY_LEN;
      wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived key", eapKeyData,
                  EAP_TLS_KEY_LEN);
    } else {
  ...
  eapKeyData = eap_server_tls_derive_key(
      sm, &data->ssl, "client EAP encryption", EAP_TLS_KEY_LEN);
  if (eapKeyData) {
    *len = EAP_TLS_KEY_LEN;
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived key", eapKeyData,
                EAP_TLS_KEY_LEN);
...
static u8 *eap_peap_get_session_id(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  return eap_server_tls_derive_session_id(sm, &data->ssl, EAP_TYPE_PEAP, len);
}
