...
static struct wpabuf *eap_gtc_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GTC, msg_len, EAP_CODE_REQUEST,
                      id);
  if (req == NULL) {
...
static void eap_gtc_process(struct eap_sm *sm, void *priv,
  ...
  if (data->prefix) {
    ...
    wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-GTC: Response user", pos, pos2 - pos);
    ...
    pos = pos2 + 1;
    rlen = end - pos;
    wpa_hexdump_ascii_key(MSG_MSGDUMP, "EAP-GTC: Response password", pos, rlen);
  }
#endif /* EAP_SERVER_FAST */
