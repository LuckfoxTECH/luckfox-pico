...
struct eap_aka_data {
  ...
  size_t res_len;
  enum { IDENTITY, CHALLENGE, REAUTH, NOTIFICATION, SUCCESS, FAILURE } state;
  char *next_pseudonym;
  char *next_reauth_id;
  u16 counter;
...
static int eap_aka_check_identity_reauth(struct eap_sm *sm,
  ...
  data->reauth = eap_sim_db_get_reauth_entry(sm->eap_sim_db_priv, username);
  if (data->reauth == NULL) {
  ...
  os_strlcpy(data->permanent, data->reauth->permanent, sizeof(data->permanent));
  data->counter = data->reauth->counter;
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    os_memcpy(data->k_encr, data->reauth->k_encr, EAP_SIM_K_ENCR_LEN);
    os_memcpy(data->k_aut, data->reauth->k_aut, EAP_AKA_PRIME_K_AUT_LEN);
    os_memcpy(data->k_re, data->reauth->k_re, EAP_AKA_PRIME_K_RE_LEN);
  } else {
...
static void eap_aka_check_identity(struct eap_sm *sm,
  ...
  if ((data->eap_method == EAP_TYPE_AKA_PRIME &&
    ...
    wpa_printf(MSG_DEBUG, "EAP-AKA: Pseudonym username '%s'", username);
    permanent = eap_sim_db_get_permanent(sm->eap_sim_db_priv, username);
    if (permanent == NULL) {
      os_free(username);
    ...
    os_strlcpy(data->permanent, permanent, sizeof(data->permanent));
    /*
...
static int eap_aka_build_encr(struct eap_sm *sm, struct eap_aka_data *data,
  ...
  os_free(data->next_pseudonym);
  if (nonce_s == NULL) {
    data->next_pseudonym = eap_sim_db_get_next_pseudonym(
        sm->eap_sim_db_priv, data->eap_method == EAP_TYPE_AKA_PRIME
  ...
  if (data->counter <= EAP_AKA_MAX_FAST_REAUTHS) {
    data->next_reauth_id = eap_sim_db_get_next_reauth_id(
        sm->eap_sim_db_priv, data->eap_method == EAP_TYPE_AKA_PRIME
                                 ? EAP_SIM_DB_AKA_PRIME
                                 : EAP_SIM_DB_AKA);
  } else {
  ...
  if (nonce_s) {
    ...
    eap_sim_msg_add(msg, EAP_SIM_AT_NONCE_S, 0, nonce_s, EAP_SIM_NONCE_S_LEN);
  }

  if (data->next_pseudonym) {
    wpa_printf(MSG_DEBUG, "   *AT_NEXT_PSEUDONYM (%s)", data->next_pseudonym);
    eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_PSEUDONYM,
                    os_strlen(data->next_pseudonym), (u8 *)data->next_pseudonym,
                    os_strlen(data->next_pseudonym));
  }

  if (data->next_reauth_id) {
    wpa_printf(MSG_DEBUG, "   *AT_NEXT_REAUTH_ID (%s)", data->next_reauth_id);
    eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_REAUTH_ID,
                    os_strlen(data->next_reauth_id), (u8 *)data->next_reauth_id,
...
eap_aka_build_challenge(struct eap_sm *sm, struct eap_aka_data *data, u8 id) {
  struct eap_sim_msg *msg;
  ...
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    if (data->kdf) {
      ...
      eap_sim_msg_add(msg, EAP_SIM_AT_KDF, data->kdf, NULL, 0);
    }
    wpa_printf(MSG_DEBUG, "   AT_KDF");
    eap_sim_msg_add(msg, EAP_SIM_AT_KDF, EAP_AKA_PRIME_KDF, NULL, 0);
    wpa_printf(MSG_DEBUG, "   AT_KDF_INPUT");
    eap_sim_msg_add(msg, EAP_SIM_AT_KDF_INPUT, data->network_name_len,
  ...
  if (data->eap_method == EAP_TYPE_AKA) {
    ...
    while (sm->user && i < EAP_MAX_METHODS &&
      ...
      if (sm->user->methods[i].vendor == EAP_VENDOR_IETF) {
        if (sm->user->methods[i].method == EAP_TYPE_AKA)
          break;
        if (sm->user->methods[i].method == EAP_TYPE_AKA_PRIME) {
...
static struct wpabuf *eap_aka_build_reauth(struct eap_sm *sm,
  ...
  if (random_get_bytes(data->nonce_s, EAP_SIM_NONCE_S_LEN))
    return NULL;
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-AKA: NONCE_S", data->nonce_s,
                  EAP_SIM_NONCE_S_LEN);

  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    eap_aka_prime_derive_keys_reauth(data->k_re, data->counter, sm->identity,
                                     sm->identity_len, data->nonce_s, data->msk,
                                     data->emsk);
  } else {
    eap_sim_derive_keys(data->mk, data->k_encr, data->k_aut, data->msk,
                        data->emsk);
    eap_sim_derive_keys_reauth(data->counter, sm->identity, sm->identity_len,
                               data->nonce_s, data->mk, data->msk, data->emsk);
  }
...
static struct wpabuf *eap_aka_build_notification(struct eap_sm *sm,
  ...
  eap_sim_msg_add(msg, EAP_SIM_AT_NOTIFICATION, data->notification, NULL, 0);
...
static Boolean eap_aka_check(struct eap_sm *sm, void *priv,
  ...
  pos = eap_hdr_validate(EAP_VENDOR_IETF, data->eap_method, respData, &len);
  if (pos == NULL || len < 3) {
...
static void eap_aka_determine_identity(struct eap_sm *sm,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-AKA: Identity", sm->identity,
                    sm->identity_len);
  ...
  if ((data->eap_method == EAP_TYPE_AKA_PRIME &&
    ...
    wpa_printf(MSG_DEBUG, "EAP-AKA: Permanent username '%s'", username);
    os_strlcpy(data->permanent, username, sizeof(data->permanent));
    os_free(username);
  } else {
    wpa_printf(MSG_DEBUG, "EAP-AKA: Unrecognized username '%s'", username);
    os_free(username);
    data->notification = EAP_SIM_GENERAL_FAILURE_BEFORE_AUTH;
    eap_aka_state(data, NOTIFICATION);
    return;
...
static void eap_aka_fullauth(struct eap_sm *sm, struct eap_aka_data *data) {
  ...
  res = eap_sim_db_get_aka_auth(sm->eap_sim_db_priv, data->permanent,
                                data->rand, data->autn, data->ik, data->ck,
                                data->res, &data->res_len, sm);
  ...
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    eap_aka_prime_derive_keys(sm->identity, identity_len, data->ik, data->ck,
                              data->k_encr, data->k_aut, data->k_re, data->msk,
                              data->emsk);
  } else {
    eap_aka_derive_mk(sm->identity, identity_len, data->ik, data->ck, data->mk);
    eap_sim_derive_keys(data->mk, data->k_encr, data->k_aut, data->msk,
                        data->emsk);
  }
...
static void eap_aka_process_challenge(struct eap_sm *sm,
  ...
  if (attr->res == NULL || attr->res_len < data->res_len ||
    ...
    wpa_printf(MSG_WARNING,
               ...
               (unsigned long)attr->res_len, (unsigned long)attr->res_len_bits,
  ...
  if (data->next_reauth_id) {
    if (data->eap_method == EAP_TYPE_AKA_PRIME) {
#ifdef EAP_SERVER_AKA_PRIME
      eap_sim_db_add_reauth_prime(sm->eap_sim_db_priv, data->permanent,
                                  data->next_reauth_id, data->counter + 1,
                                  data->k_encr, data->k_aut, data->k_re);
#endif /* EAP_SERVER_AKA_PRIME */
    } else {
      eap_sim_db_add_reauth(sm->eap_sim_db_priv, data->permanent,
                            data->next_reauth_id, data->counter + 1, data->mk);
    }
    data->next_reauth_id = NULL;
  }
}
...
static void eap_aka_process_sync_failure(struct eap_sm *sm,
  ...
  if (!data->auts_reported &&
      eap_sim_db_resynchronize(sm->eap_sim_db_priv, data->permanent, attr->auts,
                               data->rand)) {
...
static void eap_aka_process_reauth(struct eap_sm *sm, struct eap_aka_data *data,
                                   struct wpabuf *respData,
                                   struct eap_sim_attrs *attr) {
...
static void eap_aka_process_authentication_reject(struct eap_sm *sm,
                                                  struct eap_aka_data *data,
                                                  struct wpabuf *respData,
                                                  struct eap_sim_attrs *attr) {
...
static void eap_aka_process(struct eap_sm *sm, void *priv,
  ...
  if (subtype == EAP_AKA_SUBTYPE_AUTHENTICATION_REJECT) {
    eap_aka_process_authentication_reject(sm, data, respData, &attr);
    return;
  }
  ...
  case CHALLENGE:
    if (subtype == EAP_AKA_SUBTYPE_SYNCHRONIZATION_FAILURE) {
      eap_aka_process_sync_failure(sm, data, respData, &attr);
    } else {
