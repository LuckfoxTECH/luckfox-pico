...
struct wpabuf *eap_tls_msg_alloc(EapType type, size_t payload_len, u8 code,
                                 u8 identifier) {
...
int eap_server_tls_ssl_init(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (tls_connection_set_verify(sm->ssl_ctx, data->conn, verify_peer, flags,
                                session_ctx, sizeof(session_ctx))) {
...
u8 *eap_server_tls_derive_key(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (tls_connection_export_key(sm->ssl_ctx, data->conn, label, out, len)) {
    os_free(out);
    return NULL;
  }
...
struct wpabuf *eap_server_tls_build_msg(struct eap_ssl_data *data, int eap_type,
                                        int version, u8 id) {
  struct wpabuf *req;
  u8 flags;
  ...
  if (data->tls_out_pos == wpabuf_len(data->tls_out)) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(%lu more to send)",
               (unsigned long)send_len,
               (unsigned long)wpabuf_len(data->tls_out) - data->tls_out_pos);
...
static int eap_server_tls_process_cont(struct eap_ssl_data *data, const u8 *buf,
                                       size_t len) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "bytes more",
             (unsigned long)len, (unsigned long)wpabuf_tailroom(data->tls_in));

  return 0;
}

static int eap_server_tls_process_fragment(struct eap_ssl_data *data, u8 flags,
                                           u32 message_length, const u8 *buf,
                                           size_t len) {
...
int eap_server_tls_phase1(struct eap_sm *sm, struct eap_ssl_data *data) {
  ...
  data->tls_out = tls_connection_server_handshake(sm->ssl_ctx, data->conn,
                                                  data->tls_in, NULL);
  if (data->tls_out == NULL) {
...
static int eap_server_tls_reassemble(struct eap_ssl_data *data, u8 flags,
  ...
  if (data->tls_in && eap_server_tls_process_cont(data, *pos, end - *pos) < 0)
    return -1;

  if (flags & EAP_TLS_FLAGS_MORE_FRAGMENTS) {
    if (eap_server_tls_process_fragment(data, flags, tls_msg_len, *pos,
                                        end - *pos) < 0)
      return -1;
...
struct wpabuf *eap_server_tls_encrypt(struct eap_sm *sm,
  ...
  buf = tls_connection_encrypt(sm->ssl_ctx, data->conn, plain);
  if (buf == NULL) {
...
int eap_server_tls_process(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (proc_version && proc_version(sm, priv, flags & EAP_TLS_VERSION_MASK) < 0)
    return -1;
