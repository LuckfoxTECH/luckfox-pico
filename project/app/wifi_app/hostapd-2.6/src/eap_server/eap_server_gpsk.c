...
static struct wpabuf *eap_gpsk_build_gpsk_1(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-GPSK: RAND_Server", data->rand_server,
              EAP_GPSK_RAND_LEN);

  len = 1 + 2 + sm->server_id_len + EAP_GPSK_RAND_LEN + 2 +
        data->csuite_count * sizeof(struct eap_gpsk_csuite);
  req =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GPSK, len, EAP_CODE_REQUEST, id);
  if (req == NULL) {
  ...
  wpabuf_put_be16(req, data->csuite_count * sizeof(struct eap_gpsk_csuite));
...
static Boolean eap_gpsk_check(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_INFO, "EAP-GPSK: Unexpected opcode=%d in state=%d", *pos,
             data->state);
...
static void eap_gpsk_process_gpsk_2(struct eap_sm *sm,
  ...
  if (alen != sm->server_id_len || os_memcmp(pos, sm->server_id, alen) != 0) {
  ...
  if (os_memcmp(data->rand_server, pos, EAP_GPSK_RAND_LEN) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-1", data->rand_server,
                EAP_GPSK_RAND_LEN);
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-2", pos,
                EAP_GPSK_RAND_LEN);
    eap_gpsk_state(data, FAILURE);
    return;
  }
  ...
  for (i = 0; i < data->csuite_count; i++) {
    if (os_memcmp(csuite, &data->csuite_list[i], sizeof(*csuite)) == 0)
      break;
  }
  if (i == data->csuite_count) {
    wpa_printf(MSG_DEBUG,
               ...
               "ciphersuite %d:%d",
               WPA_GET_BE32(csuite->vendor), WPA_GET_BE16(csuite->specifier));
    eap_gpsk_state(data, FAILURE);
    return;
  }
  data->vendor = WPA_GET_BE32(csuite->vendor);
  data->specifier = WPA_GET_BE16(csuite->specifier);
  wpa_printf(MSG_DEBUG, "EAP-GPSK: CSuite_Sel %d:%d", data->vendor,
             data->specifier);
  pos += sizeof(*csuite);
  ...
  if (eap_gpsk_derive_keys(
          sm->user->password, sm->user->password_len, data->vendor,
          data->specifier, data->rand_peer, data->rand_server, data->id_peer,
          data->id_peer_len, sm->server_id, sm->server_id_len, data->msk,
          data->emsk, data->sk, &data->sk_len, data->pk, &data->pk_len) < 0) {
  ...
  if (eap_gpsk_derive_session_id(
          sm->user->password, sm->user->password_len, data->vendor,
          data->specifier, data->rand_peer, data->rand_server, data->id_peer,
          data->id_peer_len, sm->server_id, sm->server_id_len, EAP_TYPE_GPSK,
