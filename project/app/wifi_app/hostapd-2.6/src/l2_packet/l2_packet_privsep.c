...
static int wpa_priv_cmd(struct l2_packet_data *l2, int cmd, const void *data,
                        size_t data_len) {
  struct msghdr msg;
  struct iovec io[2];
...
int l2_packet_send(struct l2_packet_data *l2, const u8 *dst_addr, u16 proto,
  ...
  if (sendmsg(l2->fd, &msg, 0) < 0) {
    wpa_printf(MSG_ERROR, "L2: sendmsg(packet_send): %s", strerror(errno));
    return -1;
  }
...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  if (res < 0) {
    wpa_printf(MSG_ERROR, "l2_packet_receive - recvfrom: %s", strerror(errno));
    return;
  }
  if (res < ETH_ALEN) {
  ...
  l2->rx_callback(l2->rx_callback_ctx, buf, buf + ETH_ALEN, res - ETH_ALEN);
}
...
l2_packet_init(const char *ifname, const u8 *own_addr, unsigned short protocol,
  ...
  os_snprintf(l2->own_socket_path, len, "%s/wpa_privsep-l2-%d-%d", own_dir,
              getpid(), counter++);

  l2->priv_addr.sun_family = AF_UNIX;
  os_snprintf(l2->priv_addr.sun_path, sizeof(l2->priv_addr.sun_path), "%s/%s",
              priv_dir, ifname);
  ...
  if (bind(l2->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    wpa_printf(MSG_ERROR, "l2-pkt-privsep: bind(PF_UNIX): %s", strerror(errno));
    goto fail;
  }

  reg_cmd[0] = protocol;
  reg_cmd[1] = l2_hdr;
  if (wpa_priv_cmd(l2, PRIVSEP_CMD_L2_REGISTER, reg_cmd, sizeof(reg_cmd)) < 0) {
