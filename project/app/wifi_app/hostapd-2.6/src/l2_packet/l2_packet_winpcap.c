...
static DWORD WINAPI l2_packet_receive_thread(LPVOID arg) {
  ...
  while (l2->running) {
    pcap_dispatch(l2->pcap, 1, l2_packet_receive_cb, (u_char *)l2);
    if (l2->rx_no_wait > 0)
      l2->rx_no_wait--;
    if (WaitForSingleObject(l2->rx_notify, l2->rx_no_wait ? 0 : 50) ==
        WAIT_OBJECT_0) {
      l2->rx_no_wait = no_wait_count;
...
static void l2_packet_rx_event(void *eloop_data, void *user_data) {
  ...
  l2->rx_callback(l2->rx_callback_ctx, l2->rx_src, l2->rx_buf, l2->rx_len);
...
l2_packet_init(const char *ifname, const u8 *own_addr, unsigned short protocol,
  ...
  if (os_strncmp(ifname, "\\Device\\NPF_", 12) == 0)
    ...
    os_snprintf(l2->ifname, sizeof(l2->ifname), "\\Device\\NPF_%s", ifname);
  ...
  if (l2->rx_avail == NULL || l2->rx_done == NULL || l2->rx_notify == NULL) {
  ...
  l2->running = 1;
  l2->rx_thread =
      CreateThread(NULL, 0, l2_packet_receive_thread, l2, 0, &thread_id);
