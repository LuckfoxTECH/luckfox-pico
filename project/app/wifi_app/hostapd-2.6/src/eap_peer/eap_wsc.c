...
static void *eap_wsc_init(struct eap_sm *sm) {
  ...
  if (identity && identity_len == WSC_ID_REGISTRAR_LEN &&
    ...
    wpa_hexdump_ascii(MSG_INFO, "EAP-WSC: Unexpected identity", identity,
                      identity_len);
    return NULL;
  }
  ...
  if (registrar && cfg.pin) {
    wps_registrar_add_pin(data->wps_ctx->registrar, NULL, NULL, cfg.pin,
                          cfg.pin_len, 0);
  }
...
static struct wpabuf *eap_wsc_build_msg(struct eap_wsc_data *data,
  ...
  if (data->out_used == wpabuf_len(data->out_buf)) {
    ...
    if ((data->state == FAIL && data->out_op_code == WSC_ACK) ||
        data->out_op_code == WSC_NACK || data->out_op_code == WSC_Done) {
...
int eap_peer_wsc_register(void) {
  ...
  eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION, EAP_VENDOR_WFA,
                              EAP_VENDOR_TYPE_WSC, "WSC");
  if (eap == NULL)
    return -1;
