...
static void eap_notify_status(struct eap_sm *sm, const char *status,
  ...
  wpa_printf(MSG_DEBUG, "EAP: Status notification: %s (param=%s)", status,
             parameter);
...
int eap_allowed_method(struct eap_sm *sm, int vendor, u32 method) {
  ...
  for (i = 0; m[i].vendor != EAP_VENDOR_IETF || m[i].method != EAP_TYPE_NONE;
       i++) {
...
SM_STATE(EAP, INITIALIZE) {
  ...
  if (sm->fast_reauth && sm->m && sm->m->has_reauth_data &&
      sm->m->has_reauth_data(sm, sm->eap_method_priv) && !sm->prev_failure &&
 ...
 */
SM_STATE(EAP, IDLE) { SM_ENTRY(EAP, IDLE); }
...
SM_STATE(EAP, GET_METHOD) {
  ...
  if (!eap_sm_allowMethod(sm, sm->reqVendor, method)) {
    wpa_printf(MSG_DEBUG, "EAP: vendor %u method %u not allowed", sm->reqVendor,
               method);
    wpa_msg(sm->msg_ctx, MSG_INFO,
            WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u -> NAK",
            sm->reqVendor, method);
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u", sm->reqVendor,
          method);
   ...
   * is allocated here.
   */
  if (sm->fast_reauth && sm->m && sm->m->vendor == sm->reqVendor &&
      sm->m->method == method && sm->m->has_reauth_data &&
  ...
  if (reinit) {
    sm->eap_method_priv = sm->m->init_for_reauth(sm, sm->eap_method_priv);
  } else {
  ...
  if (sm->eap_method_priv == NULL) {
    ...
    if (sm->reqMethod == EAP_TYPE_TLS && config &&
        (config->pending_req_pin || config->pending_req_passphrase)) {
      /*
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_METHOD "EAP vendor %u method %u (%s) selected",
          sm->reqVendor, method, sm->m->name);
  return;
...
static char *eap_home_realm(struct eap_sm *sm) {
  ...
  if (config->anonymous_identity) {
    ...
    if (i < config->anonymous_identity_len) {
      ...
      if (realm == NULL)
        return NULL;
      os_memcpy(realm, &config->anonymous_identity[i + 1], realm_len);
      realm[realm_len] = '\0';
      return realm;
    }
  }
...
static struct eap_erp_key *eap_erp_get_key(struct eap_sm *sm,
                                           const char *realm) {
...
static struct eap_erp_key *eap_erp_get_key_nai(struct eap_sm *sm,
                                               const char *nai) {
  struct eap_erp_key *erp;
...
static void eap_erp_remove_keys_realm(struct eap_sm *sm, const char *realm) {
  ...
  while ((erp = eap_erp_get_key(sm, realm)) != NULL) {
    wpa_printf(MSG_DEBUG, "EAP: Delete old ERP key %s", erp->keyname_nai);
...
static void eap_peer_erp_init(struct eap_sm *sm) {
  ...
  if (!emsk || emsk_len == 0 || emsk_len > ERP_MAX_KEY_LEN) {
    wpa_printf(MSG_DEBUG, "EAP: No suitable EMSK available for ERP");
    goto fail;
  }
  ...
  WPA_PUT_BE16(len, 8);
  if (hmac_sha256_kdf(sm->eapSessionId, sm->eapSessionIdLen, "EMSK", len,
  ...
  pos = wpa_snprintf_hex(erp->keyname_nai, nai_buf_len, EMSKname,
                         EAP_EMSK_NAME_LEN);
  erp->keyname_nai[pos] = '@';
  os_memcpy(&erp->keyname_nai[pos + 1], realm, realm_len);

  WPA_PUT_BE16(len, emsk_len);
  if (hmac_sha256_kdf(emsk, emsk_len, "EAP Re-authentication Root Key@ietf.org",
  ...
  if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
                      "EAP Re-authentication Integrity Key@ietf.org", len,
                      sizeof(len), erp->rIK, erp->rRK_len) < 0) {
...
static int eap_peer_erp_reauth_start(struct eap_sm *sm,
  ...
  if (hmac_sha256(erp->rIK, erp->rIK_len, wpabuf_head(msg), wpabuf_len(msg),
                  hash) < 0) {
    wpabuf_free(msg);
    return -1;
  }
...
SM_STATE(EAP, METHOD) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             eap_sm_decision_txt(ret.decision), sm->eapRespData);
  ...
  if (sm->m->isKeyAvailable && sm->m->getKey &&
    ...
    if (sm->m->getSessionId) {
      sm->eapSessionId =
          sm->m->getSessionId(sm, sm->eap_method_priv, &sm->eapSessionIdLen);
      wpa_hexdump(MSG_DEBUG, "EAP: Session-Id", sm->eapSessionId,
                  sm->eapSessionIdLen);
    }
...
SM_STATE(EAP, FAILURE) {
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_FAILURE "EAP authentication failed");
...
static int eap_success_workaround(struct eap_sm *sm, int reqId, int lastId) {
   ...
   */
  if (sm->workaround &&
      (reqId == ((lastId + 1) & 0xff) || reqId == ((lastId + 2) & 0xff))) {
    wpa_printf(MSG_DEBUG,
  ...
  if (eapol_get_bool(sm, EAPOL_eapReq))
    ...
    SM_ENTER(EAP, SUCCESS);
           ...
           (eapol_get_bool(sm, EAPOL_altAccept) &&
            sm->methodState != METHOD_CONT && sm->decision == DECISION_FAIL))
    SM_ENTER(EAP, FAILURE);
  else if (sm->selectedMethod == EAP_TYPE_LEAP && sm->leap_done &&
           sm->decision != DECISION_FAIL && sm->methodState == METHOD_DONE)
    SM_ENTER(EAP, SUCCESS);
  else if (sm->selectedMethod == EAP_TYPE_PEAP && sm->peap_done &&
           sm->decision != DECISION_FAIL && sm->methodState == METHOD_DONE)
    SM_ENTER(EAP, SUCCESS);
}
...
static void eap_peer_sm_step_received(struct eap_sm *sm) {
  ...
  else if (sm->workaround && sm->lastId == -1 && sm->rxFailure && !sm->rxReq &&
           sm->methodState != METHOD_CONT && eap_peer_sm_allow_canned(sm))
    SM_ENTER(EAP, FAILURE); /* EAP-Failure prior any EAP method */
  else if (sm->workaround && sm->rxSuccess && !sm->rxFailure && !sm->rxReq &&
           sm->methodState != METHOD_CONT && eap_peer_sm_allow_canned(sm))
    SM_ENTER(EAP, SUCCESS); /* EAP-Success after Identity */
  else if (sm->methodState != METHOD_CONT &&
           ((sm->rxFailure && sm->decision != DECISION_UNCOND_SUCC) ||
  ...
  else if (sm->rxReq && !duplicate && sm->reqMethod == EAP_TYPE_NOTIFICATION &&
           sm->allowNotifications)
    SM_ENTER(EAP, NOTIFICATION);
  else if (sm->rxReq && !duplicate && sm->selectedMethod == EAP_TYPE_NONE &&
  ...
  else if (sm->rxReq && !duplicate && sm->reqMethod == sm->selectedMethod &&
           sm->methodState != METHOD_DONE)
    SM_ENTER(EAP, METHOD);
  else if (sm->selectedMethod == EAP_TYPE_LEAP && (sm->rxSuccess || sm->rxResp))
    SM_ENTER(EAP, METHOD);
...
static void eap_peer_sm_step_local(struct eap_sm *sm) {
  ...
  case EAP_INITIALIZE:
    ...
    break;
  case EAP_DISABLED:
    if (eapol_get_bool(sm, EAPOL_portEnabled) && !sm->force_disabled)
  ...
  case EAP_METHOD:
    ...
    if (sm->ignore)
      SM_ENTER(EAP, DISCARD);
    else if (sm->methodState == METHOD_DONE && sm->decision == DECISION_FAIL &&
             !sm->eapRespData)
      SM_ENTER(EAP, FAILURE);
    else
      SM_ENTER(EAP, SEND_RESPONSE);
...
eap_sm_build_expanded_nak(struct eap_sm *sm, int id,
                          const struct eap_method *methods, size_t count) {
  struct wpabuf *resp;
  int found = 0;
  ...
  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_EXPANDED, 8 + 8 * (count + 1),
  ...
  for (m = methods; m; m = m->next) {
    if (sm->reqVendor == m->vendor && sm->reqVendorMethod == m->method)
...
static struct wpabuf *eap_sm_buildNak(struct eap_sm *sm, int id) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "vendor=%u method=%u not allowed)",
             sm->reqMethod, sm->reqVendor, sm->reqVendorMethod);
...
static void eap_sm_processIdentity(struct eap_sm *sm,
  ...
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_STARTED "EAP authentication started");
  eap_notify_status(sm, "started", "");

  pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY, req, &msg_len);
  if (pos == NULL)
    return;
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP: EAP-Request Identity data", pos, msg_len);
}
...
static int eap_sm_append_3gpp_realm(struct eap_sm *sm, char *imsi,
  ...
  pos += os_snprintf(pos, imsi + max_len - pos,
                     "@wlan.mnc%s.mcc%c%c%c.3gppnetwork.org", mnc, imsi[0],
                     imsi[1], imsi[2]);
  *imsi_len = pos - imsi;
...
static int eap_sm_imsi_identity(struct eap_sm *sm,
  ...
  for (i = 0;
       m && (m[i].vendor != EAP_VENDOR_IETF || m[i].method != EAP_TYPE_NONE);
       i++) {
    if (m[i].vendor == EAP_VENDOR_IETF && m[i].method == EAP_TYPE_AKA_PRIME) {
      method = EAP_SM_AKA_PRIME;
      break;
    }

    if (m[i].vendor == EAP_VENDOR_IETF && m[i].method == EAP_TYPE_AKA) {
      method = EAP_SM_AKA;
      break;
    }
  }
...
struct wpabuf *eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted) {
  ...
  if (sm->m && sm->m->get_identity &&
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP: using anonymous identity", identity,
                      identity_len);
  } else {
    identity = config->identity;
    identity_len = config->identity_len;
    wpa_hexdump_ascii(MSG_DEBUG, "EAP: using real identity", identity,
                      identity_len);
  }
  ...
  if (config->pcsc) {
    ...
    if (!identity) {
      ...
      identity = config->identity;
      identity_len = config->identity_len;
      wpa_hexdump_ascii(MSG_DEBUG, "permanent identity from IMSI", identity,
                        identity_len);
...
static void eap_sm_processNotify(struct eap_sm *sm, const struct wpabuf *req) {
  ...
  pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_NOTIFICATION, req, &msg_len);
  if (pos == NULL)
    return;
  wpa_hexdump_ascii(MSG_DEBUG, "EAP: EAP-Request Notification data", pos,
                    msg_len);
  ...
  msg[msg_len] = '\0';
  wpa_msg(sm->msg_ctx, MSG_INFO, "%s%s", WPA_EVENT_EAP_NOTIFICATION, msg);
  os_free(msg);
}
...
static void eap_peer_initiate(struct eap_sm *sm, const struct eap_hdr *hdr,
  ...
  if (*pos != EAP_ERP_TYPE_REAUTH_START) {
    wpa_printf(MSG_DEBUG, "EAP: Ignored unexpected EAP-Initiate Type=%u", *pos);
    return;
  }

  pos++;
  if (pos >= end) {
    wpa_printf(MSG_DEBUG, "EAP: Too short EAP-Initiate/Re-auth-Start");
    return;
  }
  pos++; /* Reserved */
  wpa_hexdump(MSG_DEBUG, "EAP: EAP-Initiate/Re-auth-Start TVs/TLVs", pos,
              end - pos);
...
#endif /* CONFIG_ERP */
  wpa_printf(MSG_DEBUG,
             "EAP: EAP-Initiate/Re-auth-Start - No suitable ERP keys available "
             "- try to start full EAP authentication");
...
static void eap_peer_finish(struct eap_sm *sm, const struct eap_hdr *hdr,
  ...
  if (*pos != EAP_ERP_TYPE_REAUTH) {
    wpa_printf(MSG_DEBUG, "EAP: Ignored unexpected EAP-Finish Type=%u", *pos);
    return;
  }

  if (len < sizeof(*hdr) + 4) {
    wpa_printf(MSG_DEBUG, "EAP: Ignored too short EAP-Finish/Re-auth");
    return;
  }
  ...
  if (seq != sm->erp_seq) {
    wpa_printf(MSG_DEBUG, "EAP: Unexpected EAP-Finish/Re-auth SEQ=%u", seq);
    return;
  }
  ...
  if (!parse.keyname) {
    wpa_printf(MSG_DEBUG, "EAP: No keyName-NAI in EAP-Finish/Re-auth Packet");
    return;
  }
  ...
  if (parse.keyname_len > 253) {
    wpa_printf(MSG_DEBUG, "EAP: Too long keyName-NAI in EAP-Finish/Re-auth");
    return;
  }
  ...
  if (!erp) {
    wpa_printf(MSG_DEBUG, "EAP: No matching ERP key found for %s", nai);
    return;
  }
  ...
  if (max_len < 1 + (int)hash_len) {
    wpa_printf(MSG_DEBUG, "EAP: Not enough room for Authentication Tag");
    if (flags & 0x80)
      goto no_auth_tag;
    return;
  }
  ...
  if (os_memcmp(end - hash_len, hash, hash_len) != 0) {
    wpa_printf(MSG_DEBUG, "EAP: Authentication Tag mismatch");
    return;
  }
  auth_tag_ok = 1;
  end -= 1 + hash_len;
...
no_auth_tag:
   ...
   * that contains them.
   */
  wpa_hexdump(MSG_DEBUG, "EAP: EAP-Finish/Re-Auth TVs/TLVs", pos, end - pos);
  if (erp_parse_tlvs(pos, end, &parse, 0) < 0)
    return;

  if (flags & 0x80 || !auth_tag_ok) {
    wpa_printf(MSG_DEBUG, "EAP: EAP-Finish/Re-auth indicated failure");
    eapol_set_bool(sm, EAPOL_eapFail, TRUE);
    eapol_set_bool(sm, EAPOL_eapReq, FALSE);
    eapol_set_bool(sm, EAPOL_eapNoResp, TRUE);
    wpa_msg(sm->msg_ctx, MSG_INFO,
  ...
  if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
                      "Re-authentication Master Session Key@ietf.org", seed,
                      sizeof(seed), sm->eapKeyData, erp->rRK_len) < 0) {
    wpa_printf(MSG_DEBUG, "EAP: Could not derive rMSK for ERP");
    eap_sm_free_key(sm);
    return;
  }
  wpa_hexdump_key(MSG_DEBUG, "EAP: ERP rMSK", sm->eapKeyData,
                  sm->eapKeyDataLen);
  sm->eapKeyAvailable = TRUE;
  eapol_set_bool(sm, EAPOL_eapSuccess, TRUE);
  eapol_set_bool(sm, EAPOL_eapReq, FALSE);
  eapol_set_bool(sm, EAPOL_eapNoResp, TRUE);
  wpa_msg(sm->msg_ctx, MSG_INFO,
          WPA_EVENT_EAP_SUCCESS "EAP re-authentication completed successfully");
#endif /* CONFIG_ERP */
}
...
static void eap_sm_parseEapReq(struct eap_sm *sm, const struct wpabuf *req) {
  ...
  if (plen > wpabuf_len(req)) {
    wpa_printf(MSG_DEBUG,
               ...
               "(len=%lu plen=%lu)",
               (unsigned long)wpabuf_len(req), (unsigned long)plen);
    return;
  }
  ...
  case EAP_CODE_REQUEST:
    ...
    wpa_printf(MSG_DEBUG,
               ...
               sm->reqId, sm->reqMethod, sm->reqVendor, sm->reqVendorMethod);
    break;
...
static void eap_peer_sm_tls_event(void *ctx, enum tls_event ev,
  ...
  case TLS_CERT_CHAIN_SUCCESS:
    eap_notify_status(sm, "remote certificate verification", "success");
    if (sm->ext_cert_check) {
      sm->waiting_ext_cert_check = 1;
      eap_sm_request(sm, WPA_CTRL_REQ_EXT_CERT_CHECK, NULL, 0);
    }
    break;
  case TLS_CERT_CHAIN_FAILURE:
    wpa_msg(sm->msg_ctx, MSG_INFO,
            ...
            data->cert_fail.reason, data->cert_fail.depth,
            data->cert_fail.subject, data->cert_fail.reason_txt);
  ...
  case TLS_PEER_CERTIFICATE:
    ...
    if (data->peer_cert.hash) {
      ...
      if (hash_hex) {
        wpa_snprintf_hex(hash_hex, len, data->peer_cert.hash,
                         data->peer_cert.hash_len);
      }
    }

    sm->eapol_cb->notify_cert(
        sm->eapol_ctx, data->peer_cert.depth, data->peer_cert.subject,
        data->peer_cert.altsubject, data->peer_cert.num_altsubject, hash_hex,
        data->peer_cert.cert);
    break;
  case TLS_ALERT:
    if (data->alert.is_local)
      eap_notify_status(sm, "local TLS alert", data->alert.description);
    else
      eap_notify_status(sm, "remote TLS alert", data->alert.description);
    break;
  }
...
int eap_sm_get_status(struct eap_sm *sm, char *buf, size_t buflen,
  ...
  if (sm->selectedMethod != EAP_TYPE_NONE) {
    ...
    if (sm->m) {
      ...
      const struct eap_method *m =
          eap_peer_get_eap_method(EAP_VENDOR_IETF, sm->selectedMethod);
      if (m)
        name = m->name;
      else
        name = "?";
    }
    ret = os_snprintf(buf + len, buflen - len, "selectedMethod=%d (EAP-%s)\n",
    ...
    if (sm->m && sm->m->get_status) {
      len += sm->m->get_status(sm, sm->eap_method_priv, buf + len, buflen - len,
                               verbose);
    }
  }
  ...
  if (verbose) {
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      "decision=%s\n"
                      "ClientTimeout=%d\n",
                      sm->reqMethod, eap_sm_method_state_txt(sm->methodState),
                      eap_sm_decision_txt(sm->decision), sm->ClientTimeout);
    if (os_snprintf_error(buflen - len, ret))
      return len;
    len += ret;
  }
  ...
  *count = 0;
  ...
  for (m = methods; m; m = m->next) {
    ...
    if (eap_allowed_phase2_type(vendor, method)) {
      if (vendor == EAP_VENDOR_IETF && method == EAP_TYPE_TLS && config &&
...
void eap_notify_lower_layer_success(struct eap_sm *sm) {
  ...
  if (eapol_get_bool(sm, EAPOL_eapSuccess) || sm->decision == DECISION_FAIL ||
      (sm->methodState != METHOD_MAY_CONT && sm->methodState != METHOD_DONE))
    return;
