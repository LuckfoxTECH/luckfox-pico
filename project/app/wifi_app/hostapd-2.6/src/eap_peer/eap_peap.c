...
static struct wpabuf *eap_tlv_build_nak(int id, u16 nak_type) {
  ...
  msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TLV, 10, EAP_CODE_RESPONSE, id);
  if (msg == NULL)
    return NULL;
...
static int eap_peap_derive_cmk(struct eap_sm *sm, struct eap_peap_data *data) {
  ...
  wpa_printf(MSG_DEBUG,
             "EAP-PEAP: CMK derivation - reauth=%d resumed=%d "
             "phase2_eap_started=%d phase2_success=%d",
  ...
  if (data->reauth && !data->phase2_eap_started && resumed) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: IPMK from TK", data->ipmk, 40);
    os_memcpy(data->cmk, tk + 40, 20);
    wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: CMK from TK", data->cmk, 20);
    return 0;
  }
  ...
  if (peap_prfplus(data->peap_version, tk, 40, "Inner Methods Compound Keys",
                   isk, sizeof(isk), imck, sizeof(imck)) < 0)
    return -1;
  wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: IMCK (IPMKj)", imck, sizeof(imck));
...
static int eap_tlv_add_cryptobinding(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PEAP: Compound_MAC data 1", addr[0], len[0]);
  wpa_hexdump(MSG_MSGDUMP, "EAP-PEAP: Compound_MAC data 2", addr[1], len[1]);
...
static struct wpabuf *eap_tlv_build_result(struct eap_sm *sm,
  ...
  msg =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TLV, len, EAP_CODE_RESPONSE, id);
  if (msg == NULL)
    return NULL;
...
static int eap_tlv_validate_cryptobinding(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Compound_MAC data", buf, sizeof(buf));
  ...
  if (os_memcmp_const(mac, pos, SHA1_MAC_LEN) != 0) {
    wpa_printf(MSG_DEBUG, "EAP-PEAP: Invalid Compound_MAC in "
                          "cryptobinding TLV");
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Received MAC", pos, SHA1_MAC_LEN);
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Expected MAC", mac, SHA1_MAC_LEN);
    return -1;
  }
...
static int eap_tlv_process(struct eap_sm *sm, struct eap_peap_data *data,
                           struct eap_method_ret *ret, const struct wpabuf *req,
                           struct wpabuf **resp, int force_failure) {
  size_t left, tlv_len;
  const u8 *pos;
  ...
  while (left >= 4) {
    ...
    if (tlv_len > left) {
      wpa_printf(MSG_DEBUG,
                 ...
                 "(tlv_len=%lu left=%lu)",
                 (unsigned long)tlv_len, (unsigned long)left);
      return -1;
    }
    switch (tlv_type) {
    ...
    default:
      wpa_printf(MSG_DEBUG,
                 ...
                 "%d%s",
                 tlv_type, mandatory ? " (mandatory)" : "");
      if (mandatory) {
        /* NAK TLV and ignore all TLVs in this packet.
         */
        *resp = eap_tlv_build_nak(eap_get_id(req), tlv_type);
  ...
  if (crypto_tlv && data->crypto_binding != NO_BINDING) {
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Cryptobinding TLV", crypto_tlv,
                crypto_tlv_len);
  ...
  if (result_tlv) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-TLV: Result TLV", result_tlv, result_tlv_len);
    if (result_tlv_len < 2) {
      wpa_printf(MSG_INFO,
    ...
    *resp = eap_tlv_build_result(sm, data, crypto_tlv != NULL, eap_get_id(req),
                                 resp_status);
  }
...
static int eap_peap_phase2_request(struct eap_sm *sm,
                                   ...
                                   struct wpabuf *req, struct wpabuf **resp) {
  ...
  case EAP_TYPE_EXPANDED:
    ...
    if (data->soh) {
      ...
      epos = eap_hdr_validate(EAP_VENDOR_MICROSOFT, 0x21, req, &eleft);
      if (epos) {
        struct wpabuf *buf;
        wpa_printf(MSG_DEBUG, "EAP-PEAP: SoH EAP Extensions");
        buf = tncc_process_soh_request(data->soh, epos, eleft);
        if (buf) {
          *resp = eap_msg_alloc(EAP_VENDOR_MICROSOFT, 0x21, wpabuf_len(buf),
  ...
  default:
    ...
    if (*pos != data->phase2_type.method || *pos == EAP_TYPE_NONE) {
      if (eap_peer_tls_phase2_nak(data->phase2_types, data->num_phase2_types,
                                  hdr, resp))
        return -1;
      return 0;
    }
    ...
    os_memset(&iret, 0, sizeof(iret));
    *resp = data->phase2_method->process(sm, data->phase2_priv, &iret, req);
    if ((iret.methodState == METHOD_DONE ||
         iret.methodState == METHOD_MAY_CONT) &&
...
static int eap_peap_decrypt(struct eap_sm *sm, struct eap_peap_data *data,
  ...
  if (wpabuf_len(in_data) == 0 && sm->workaround && data->phase2_success) {
    /*
...
continue_req:
  wpa_hexdump_buf(MSG_DEBUG, "EAP-PEAP: Decrypted Phase 2 EAP", in_decrypted);
  ...
  if (data->peap_version == 0 && !skip_change) {
    ...
    nhdr->length =
        host_to_be16(sizeof(struct eap_hdr) + wpabuf_len(in_decrypted));
  ...
  if (len > wpabuf_len(in_decrypted)) {
    wpa_printf(MSG_INFO,
               ...
               (unsigned long)wpabuf_len(in_decrypted), (unsigned long)len);
    wpabuf_free(in_decrypted);
    return 0;
  }
  if (len < wpabuf_len(in_decrypted)) {
    wpa_printf(MSG_INFO,
               ...
               "(%lu < %lu)",
               (unsigned long)len, (unsigned long)wpabuf_len(in_decrypted));
  }
  wpa_printf(MSG_DEBUG,
  ...
  case EAP_CODE_REQUEST:
    if (eap_peap_phase2_request(sm, data, ret, in_decrypted, &resp)) {
      wpabuf_free(in_decrypted);
  ...
  case EAP_CODE_SUCCESS:
    ...
    if (data->peap_version == 1) {
       ...
       * been completed. */
      if (data->phase2_eap_started && !data->phase2_eap_success) {
  ...
  if (resp) {
    ...
    wpa_hexdump_buf_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 data", resp);
    /* PEAP version changes */
    if (wpabuf_len(resp) >= 5 && wpabuf_head_u8(resp)[0] == EAP_CODE_RESPONSE &&
        eap_get_type(resp) == EAP_TYPE_TLV)
      skip_change2 = 1;
    rmsg = resp;
    if (data->peap_version == 0 && !skip_change2) {
      wpabuf_set(&buf, wpabuf_head_u8(resp) + sizeof(struct eap_hdr),
                 wpabuf_len(resp) - sizeof(struct eap_hdr));
      rmsg = &buf;
    }

    if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_PEAP, data->peap_version,
                             req->identifier, rmsg, out_data)) {
...
static struct wpabuf *eap_peap_process(struct eap_sm *sm, void *priv,
  ...
  pos = eap_peer_tls_process_init(sm, &data->ssl, EAP_TYPE_PEAP, ret, reqData,
                                  &left, &flags);
  if (pos == NULL)
    return NULL;
  ...
  if (flags & EAP_TLS_FLAGS_START) {
    wpa_printf(MSG_DEBUG,
               ...
               "ver=%d)",
               flags & EAP_TLS_VERSION_MASK, data->peap_version);
  ...
  if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
    ...
    if (sm->waiting_ext_cert_check && data->pending_resp) {
      ...
      if (config->pending_ext_cert_check == EXT_CERT_CHECK_GOOD) {
        wpa_printf(MSG_DEBUG, "EAP-PEAP: External certificate check succeeded "
                              "- continue handshake");
      ...
      if (config->pending_ext_cert_check == EXT_CERT_CHECK_BAD) {
        wpa_printf(MSG_DEBUG, "EAP-PEAP: External certificate check failed - "
      ...
      wpa_printf(MSG_DEBUG, "EAP-PEAP: Continuing to wait external server "
                            "certificate validation");
      return NULL;
    }

    res = eap_peer_tls_process_helper(sm, &data->ssl, EAP_TYPE_PEAP,
                                      data->peap_version, id, &msg, &resp);

    if (res < 0) {
      wpa_printf(MSG_DEBUG, "EAP-PEAP: TLS processing failed");
      ret->methodState = METHOD_DONE;
      ret->decision = DECISION_FAIL;
      return resp;
    }
    ...
    if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 label);
      data->key_data =
          eap_peer_tls_derive_key(sm, &data->ssl, label, EAP_TLS_KEY_LEN);
      if (data->key_data) {
        wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Derived key", data->key_data,
                        EAP_TLS_KEY_LEN);
      } else {
      ...
      data->session_id = eap_peer_tls_derive_session_id(
          sm, &data->ssl, EAP_TYPE_PEAP, &data->id_len);
      if (data->session_id) {
        wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived Session-Id", data->session_id,
                    data->id_len);
      } else {
  ...
  if (res == 1) {
    wpabuf_free(resp);
    return eap_peer_tls_build_ack(id, EAP_TYPE_PEAP, data->peap_version);
  }
...
static int eap_peap_get_status(struct eap_sm *sm, void *priv, char *buf,
  ...
  if (data->phase2_method) {
    ret = os_snprintf(buf + len, buflen - len, "EAP-PEAPv%d Phase2 method=%s\n",
  ...
  *len = EAP_TLS_KEY_LEN;
  ...
  if (data->crypto_binding_used) {
    ...
    if (peap_prfplus(data->peap_version, data->ipmk, 40,
                     "Session Key Generating Function", (u8 *)"\00", 1, csk,
                     sizeof(csk)) < 0) {
      os_free(key);
      return NULL;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived key", key, EAP_TLS_KEY_LEN);
  } else
