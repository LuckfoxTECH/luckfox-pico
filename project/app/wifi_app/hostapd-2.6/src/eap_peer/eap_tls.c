...
static void *eap_tls_init(struct eap_sm *sm) {
  ...
  if (config == NULL ||
      ((sm->init_phase2 ? config->private_key2 : config->private_key) == NULL &&
...
static void eap_tls_success(struct eap_sm *sm, struct eap_tls_data *data,
  ...
  if (data->key_data) {
    wpa_hexdump_key(MSG_DEBUG, "EAP-TLS: Derived key", data->key_data,
                    EAP_TLS_KEY_LEN);
  ...
  data->session_id = eap_peer_tls_derive_session_id(
      sm, &data->ssl, EAP_TYPE_TLS, &data->id_len);
  if (data->session_id) {
    wpa_hexdump(MSG_DEBUG, "EAP-TLS: Derived Session-Id", data->session_id,
                data->id_len);
  } else {
...
static struct wpabuf *eap_tls_process(struct eap_sm *sm, void *priv,
  ...
  if (sm->waiting_ext_cert_check && data->pending_resp) {
    ...
    if (config->pending_ext_cert_check == EXT_CERT_CHECK_BAD) {
      wpa_printf(MSG_DEBUG, "EAP-TLS: External certificate check failed - "
  ...
  pos = eap_peer_tls_process_init(sm, &data->ssl, data->eap_type, ret, reqData,
                                  &left, &flags);
  if (pos == NULL)
    return NULL;
  ...
  res = eap_peer_tls_process_helper(sm, &data->ssl, data->eap_type, 0, id, &msg,
                                    &resp);
...
int eap_peer_unauth_tls_register(void) {
  ...
  eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION,
                              EAP_VENDOR_UNAUTH_TLS, EAP_VENDOR_TYPE_UNAUTH_TLS,
                              "UNAUTH-TLS");
  if (eap == NULL)
    return -1;
