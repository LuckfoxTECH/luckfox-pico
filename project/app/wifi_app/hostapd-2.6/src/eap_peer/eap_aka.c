...
static void eap_aka_state(struct eap_aka_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-AKA: %s -> %s", eap_aka_state_txt(data->state),
...
static int eap_aka_ext_sim_result(struct eap_sm *sm, struct eap_aka_data *data,
  ...
  wpa_printf(MSG_DEBUG, "EAP-AKA: Use result from external USIM processing");
  ...
  if (os_strncmp(resp, "UMTS-AUTS:", 10) == 0) {
    ...
    if (hexstr2bin(pos, data->auts, EAP_AKA_AUTS_LEN) < 0)
      goto invalid;
    wpa_hexdump_key(MSG_DEBUG, "EAP-AKA: AUTS", data->auts, EAP_AKA_AUTS_LEN);
    os_free(resp);
    return -2;
  }
  ...
  if (os_strncmp(resp, "UMTS-AUTH:", 10) != 0) {
    wpa_printf(MSG_DEBUG,
               "EAP-AKA: Unrecognized external USIM processing response");
    os_free(resp);
    return -1;
  }
...
invalid:
  wpa_printf(MSG_DEBUG,
             "EAP-AKA: Invalid external USIM processing UMTS-AUTH response");
  os_free(resp);
  return -1;
}
...
static int eap_aka_umts_auth(struct eap_sm *sm, struct eap_aka_data *data) {
  ...
  if (conf->pcsc) {
    return scard_umts_auth(sm->scard_ctx, data->rand, data->autn, data->res,
                           &data->res_len, data->ik, data->ck, data->auts);
  }
  ...
  if (conf->password) {
    ...
    return milenage_check(opc, k, sqn, data->rand, data->autn, data->ik,
                          data->ck, data->res, &data->res_len, data->auts);
  }
...
static int eap_aka_learn_ids(struct eap_sm *sm, struct eap_aka_data *data,
  ...
  if (attr->next_pseudonym) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-AKA: (encr) AT_NEXT_PSEUDONYM",
  ...
  if (attr->next_reauth_id) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-AKA: (encr) AT_NEXT_REAUTH_ID",
...
static struct wpabuf *eap_aka_client_error(struct eap_aka_data *data, u8 id,
  ...
  wpa_printf(MSG_DEBUG, "EAP-AKA: Send Client-Error (error code %d)", err);
...
static struct wpabuf *eap_aka_synchronization_failure(struct eap_aka_data *data,
  ...
  eap_sim_msg_add_full(msg, EAP_SIM_AT_AUTS, data->auts, EAP_AKA_AUTS_LEN);
  return eap_sim_msg_finish(msg, data->eap_method, NULL, NULL, 0);
}
...
static struct wpabuf *eap_aka_response_identity(struct eap_sm *sm,
  ...
  if (id_req == ANY_ID && data->reauth_id) {
    ...
    identity = eap_get_config_identity(sm, &identity_len);
    if (identity) {
      eap_aka_clear_identities(sm, data, CLEAR_PSEUDONYM | CLEAR_REAUTH_ID);
    }
  }
  if (id_req != NO_ID_REQ)
...
static struct wpabuf *eap_aka_response_challenge(struct eap_aka_data *data,
  ...
  eap_sim_msg_add(msg, EAP_SIM_AT_RES, data->res_len * 8, data->res,
                  data->res_len);
  ...
  return eap_sim_msg_finish(msg, data->eap_method, data->k_aut, (u8 *)"", 0);
}

static struct wpabuf *eap_aka_response_reauth(struct eap_aka_data *data, u8 id,
                                              int counter_too_small,
                                              const u8 *nonce_s) {
  struct eap_sim_msg *msg;
  unsigned int counter;

  wpa_printf(MSG_DEBUG, "Generating EAP-AKA Reauthentication (id=%d)", id);
  msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
...
static int eap_aka_verify_mac(struct eap_aka_data *data,
                              const struct wpabuf *req, const u8 *mac,
                              const u8 *extra, size_t extra_len) {
  if (data->eap_method == EAP_TYPE_AKA_PRIME)
    return eap_sim_verify_mac_sha256(data->k_aut, req, mac, extra, extra_len);
  return eap_sim_verify_mac(data->k_aut, req, mac, extra, extra_len);
}

#ifdef EAP_AKA_PRIME
static struct wpabuf *eap_aka_prime_kdf_select(struct eap_aka_data *data, u8 id,
                                               u16 kdf) {
  struct eap_sim_msg *msg;
...
static struct wpabuf *eap_aka_prime_kdf_neg(struct eap_aka_data *data, u8 id,
                                            struct eap_sim_attrs *attr) {
  size_t i;

  for (i = 0; i < attr->kdf_count; i++) {
    if (attr->kdf[i] == EAP_AKA_PRIME_KDF)
      return eap_aka_prime_kdf_select(data, id, EAP_AKA_PRIME_KDF);
  }
...
static int eap_aka_prime_kdf_valid(struct eap_aka_data *data,
  ...
  if (data->kdf_negotiation) {
    ...
    if (i == attr->kdf_count && attr->kdf_count < EAP_AKA_PRIME_KDF_MAX) {
      wpa_printf(MSG_WARNING, "EAP-AKA': The server did not "
...
static struct wpabuf *eap_aka_process_challenge(struct eap_sm *sm,
  ...
  if (attr->checkcode &&
      eap_aka_verify_checkcode(data, attr->checkcode, attr->checkcode_len)) {
  ...
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    ...
    os_memcpy(data->network_name, attr->kdf_input, attr->kdf_input_len);
  ...
  if (data->eap_method == EAP_TYPE_AKA && attr->bidding) {
    ...
    if ((flags & EAP_AKA_BIDDING_FLAG_D) &&
        eap_allowed_method(sm, EAP_VENDOR_IETF, EAP_TYPE_AKA_PRIME)) {
  ...
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    ...
    eap_aka_prime_derive_ck_ik_prime(data->ck, data->ik, data->autn,
                                     data->network_name,
  ...
  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    eap_aka_prime_derive_keys(identity, identity_len, data->ik, data->ck,
                              data->k_encr, data->k_aut, data->k_re, data->msk,
                              data->emsk);
  } else {
    eap_aka_derive_mk(identity, identity_len, data->ik, data->ck, data->mk);
  ...
  if (data->state != FAILURE) {
    eap_aka_state(data, data->use_result_ind ? RESULT_SUCCESS : SUCCESS);
  }
...
static int eap_aka_process_notification_auth(struct eap_aka_data *data,
  ...
  if (data->reauth && eap_aka_process_notification_reauth(data, attr)) {
...
eap_aka_process_reauthentication(struct eap_sm *sm, struct eap_aka_data *data,
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-AKA: (encr) AT_NONCE_S", data->nonce_s,
              EAP_SIM_NONCE_S_LEN);

  if (data->eap_method == EAP_TYPE_AKA_PRIME) {
    eap_aka_prime_derive_keys_reauth(data->k_re, data->counter, data->reauth_id,
                                     data->reauth_id_len, data->nonce_s,
                                     data->msk, data->emsk);
  } else {
...
static struct wpabuf *eap_aka_process(struct eap_sm *sm, void *priv,
  ...
  pos = eap_hdr_validate(EAP_VENDOR_IETF, data->eap_method, reqData, &len);
  if (pos == NULL || len < 3) {
    ret->ignore = TRUE;
    return NULL;
  }
  ...
  if (eap_sim_parse_attr(pos, wpabuf_head_u8(reqData) + len, &attr,
                         data->eap_method == EAP_TYPE_AKA_PRIME ? 2 : 1, 0)) {
    res = eap_aka_client_error(data, id, EAP_AKA_UNABLE_TO_PROCESS_PACKET);
    goto done;
  }
  ...
  case EAP_AKA_SUBTYPE_NOTIFICATION:
    res = eap_aka_process_notification(sm, data, id, reqData, &attr);
    break;
  case EAP_AKA_SUBTYPE_REAUTHENTICATION:
    res = eap_aka_process_reauthentication(sm, data, id, reqData, &attr);
    break;
...
int eap_peer_aka_prime_register(void) {
  ...
  eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION,
                              EAP_VENDOR_IETF, EAP_TYPE_AKA_PRIME, "AKA'");
  if (eap == NULL)
    return -1;
