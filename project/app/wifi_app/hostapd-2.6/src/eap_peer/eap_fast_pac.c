...
static int eap_fast_copy_buf(u8 **dst, size_t *dst_len, const u8 *src,
                             size_t src_len) {
  if (src) {
...
int eap_fast_add_pac(struct eap_fast_pac **pac_root,
  ...
  eap_fast_remove_pac(pac_root, pac_current, entry->a_id, entry->a_id_len,
                      entry->pac_type);
  ...
  if (eap_fast_copy_buf(&pac->pac_opaque, &pac->pac_opaque_len,
      ...
      eap_fast_copy_buf(&pac->pac_info, &pac->pac_info_len, entry->pac_info,
                        entry->pac_info_len) < 0 ||
      eap_fast_copy_buf(&pac->a_id, &pac->a_id_len, entry->a_id,
                        entry->a_id_len) < 0 ||
      eap_fast_copy_buf(&pac->i_id, &pac->i_id_len, entry->i_id,
                        entry->i_id_len) < 0 ||
      eap_fast_copy_buf(&pac->a_id_info, &pac->a_id_info_len, entry->a_id_info,
                        entry->a_id_info_len) < 0) {
    eap_fast_free_pac(pac);
    return -1;
  }
...
int eap_fast_load_pac(struct eap_sm *sm, struct eap_fast_pac **pac_root,
  ...
  wpa_printf(MSG_DEBUG, "EAP-FAST: Read %d PAC entries from '%s'", count,
             pac_file);
...
static void eap_fast_write(char **buf, char **pos, size_t *buf_len,
                           const char *field, const u8 *data, size_t len,
                           int txt) {
  size_t i, need;
  int ret;
  char *end;

  if (data == NULL || buf == NULL || *buf == NULL || pos == NULL ||
      *pos == NULL || *pos < *buf)
    return;
...
static int eap_fast_add_pac_data(struct eap_fast_pac *pac, char **buf,
  ...
  *pos += ret;
  ...
  eap_fast_write(buf, pos, buf_len, "PAC-Opaque", pac->pac_opaque,
  ...
  eap_fast_write(buf, pos, buf_len, "A-ID-Info", pac->a_id_info,
                 pac->a_id_info_len, 1);
  if (*buf == NULL) {
...
int eap_fast_save_pac(struct eap_sm *sm, struct eap_fast_pac *pac_root,
  ...
  wpa_printf(MSG_DEBUG, "EAP-FAST: Wrote %d PAC entries into '%s'", count,
             pac_file);
...
int eap_fast_save_pac_bin(struct eap_sm *sm, struct eap_fast_pac *pac_root,
  ...
  while (pac) {
    if (pac->pac_opaque_len > 65535 || pac->pac_info_len > 65535)
      return -1;
    len += 2 + EAP_FAST_PAC_KEY_LEN + 2 + pac->pac_opaque_len + 2 +
           pac->pac_info_len;
    pac = pac->next;
  }
