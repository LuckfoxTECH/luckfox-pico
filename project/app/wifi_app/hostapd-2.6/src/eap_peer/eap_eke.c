...
struct eap_eke_data {
  enum { IDENTITY, COMMIT, CONFIRM, SUCCESS, FAILURE } state;
  u8 msk[EAP_MSK_LEN];
...
static void eap_eke_state(struct eap_eke_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-EKE: %s -> %s", eap_eke_state_txt(data->state),
...
static void *eap_eke_init(struct eap_sm *sm) {
  ...
  if (phase1) {
    ...
    if (pos) {
      ...
      wpa_printf(MSG_DEBUG, "EAP-EKE: Forced dhgroup %u", data->dhgroup);
    }

    pos = os_strstr(phase1, "encr=");
    if (pos) {
      data->encr = atoi(pos + 5);
      wpa_printf(MSG_DEBUG, "EAP-EKE: Forced encr %u", data->encr);
    }

    pos = os_strstr(phase1, "prf=");
    if (pos) {
      data->prf = atoi(pos + 4);
      wpa_printf(MSG_DEBUG, "EAP-EKE: Forced prf %u", data->prf);
    }

    pos = os_strstr(phase1, "mac=");
    if (pos) {
      data->mac = atoi(pos + 4);
      wpa_printf(MSG_DEBUG, "EAP-EKE: Forced mac %u", data->mac);
    }
  }
...
static struct wpabuf *eap_eke_build_msg(struct eap_eke_data *data, int id,
  ...
  msg =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_EKE, plen, EAP_CODE_RESPONSE, id);
  if (msg == NULL) {
...
static int eap_eke_supp_dhgroup(u8 dhgroup) {
  return dhgroup == EAP_EKE_DHGROUP_EKE_2 || dhgroup == EAP_EKE_DHGROUP_EKE_5 ||
         dhgroup == EAP_EKE_DHGROUP_EKE_14 ||
         dhgroup == EAP_EKE_DHGROUP_EKE_15 || dhgroup == EAP_EKE_DHGROUP_EKE_16;
}
...
static int eap_eke_supp_prf(u8 prf) {
  return prf == EAP_EKE_PRF_HMAC_SHA1 || prf == EAP_EKE_PRF_HMAC_SHA2_256;
}

static int eap_eke_supp_mac(u8 mac) {
  return mac == EAP_EKE_MAC_HMAC_SHA1 || mac == EAP_EKE_MAC_HMAC_SHA2_256;
}
...
static struct wpabuf *eap_eke_process_id(struct eap_eke_data *data,
  ...
  if (data->state != IDENTITY) {
    return eap_eke_build_fail(data, ret, id, EAP_EKE_FAIL_PROTO_ERROR);
  }
  ...
  for (i = 0; i < num_prop; i++) {
    ...
    if ((data->dhgroup && data->dhgroup != *tmp) || !eap_eke_supp_dhgroup(*tmp))
      continue;
    tmp++;
    if ((data->encr && data->encr != *tmp) || !eap_eke_supp_encr(*tmp))
      continue;
    tmp++;
    if ((data->prf && data->prf != *tmp) || !eap_eke_supp_prf(*tmp))
      continue;
    tmp++;
    if ((data->mac && data->mac != *tmp) || !eap_eke_supp_mac(*tmp))
      continue;

    prop = tmp - 3;
    if (eap_eke_session_init(&data->sess, prop[0], prop[1], prop[2], prop[3]) <
        0) {
      prop = NULL;
      continue;
    }
  ...
  if (prop == NULL) {
    ...
    return eap_eke_build_fail(data, ret, id, EAP_EKE_FAIL_NO_PROPOSAL_CHOSEN);
  }
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-EKE: Server Identity", pos, end - pos);
  os_free(data->serverid);
  ...
  resp = eap_eke_build_msg(data, id, 2 + 4 + 1 + data->peerid_len, EAP_EKE_ID);
  if (resp == NULL) {
    return eap_eke_build_fail(data, ret, id,
                              EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
  }
...
eap_eke_process_commit(struct eap_sm *sm, struct eap_eke_data *data,
  ...
  if (data->state != COMMIT) {
    wpa_printf(
        MSG_DEBUG,
        "EAP-EKE: EAP-EKE-Commit/Request received in unexpected state (%d)",
        data->state);
    return eap_eke_build_fail(data, ret, id, EAP_EKE_FAIL_PROTO_ERROR);
  }
  ...
  if (password == NULL) {
    wpa_printf(MSG_INFO, "EAP-EKE: No password configured!");
    return eap_eke_build_fail(data, ret, id, EAP_EKE_FAIL_PASSWD_NOT_FOUND);
  }
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent_S", pos, data->sess.dhcomp_len);
  dhcomp = pos;
   ...
   */
  if (eap_eke_derive_key(&data->sess, password, password_len, data->serverid,
                         data->serverid_len, data->peerid, data->peerid_len,
                         key) < 0) {
  ...
  if (eap_eke_derive_ke_ki(&data->sess, data->serverid, data->serverid_len,
  ...
  resp = eap_eke_build_msg(
      data, id, data->sess.dhcomp_len + data->sess.pnonce_len, EAP_EKE_COMMIT);
  if (resp == NULL) {
    os_memset(key, 0, sizeof(key));
    return eap_eke_build_fail(data, ret, id,
                              EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent_P", rpos, data->sess.dhcomp_len);
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Nonce_P", data->nonce_p,
                  data->sess.nonce_len);
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: PNonce_P", wpabuf_put(resp, 0), prot_len);
  wpabuf_put(resp, prot_len);

  /* TODO: CBValue */

  if (wpabuf_resize(&data->msgs, wpabuf_len(reqData) + wpabuf_len(resp)) < 0) {
    wpabuf_free(resp);
    return eap_eke_build_fail(data, ret, id,
                              EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
  }
...
static struct wpabuf *eap_eke_process_confirm(struct eap_eke_data *data,
  ...
  if (eap_eke_decrypt_prot(&data->sess, pos, data->sess.pnonce_ps_len, nonces,
                           &decrypt_len) < 0) {
  ...
  if (decrypt_len != (size_t)2 * data->sess.nonce_len) {
    wpa_printf(MSG_INFO, "EAP-EKE: PNonce_PS protected data length does not "
                         "match length of Nonce_P and Nonce_S");
    return eap_eke_build_fail(data, ret, id, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
  }
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Received Nonce_P | Nonce_S", nonces,
                  2 * data->sess.nonce_len);
  ...
  os_memcpy(data->nonce_s, nonces + data->sess.nonce_len, data->sess.nonce_len);
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Nonce_S", data->nonce_s,
                  data->sess.nonce_len);

  if (eap_eke_derive_ka(&data->sess, data->serverid, data->serverid_len,
                        data->peerid, data->peerid_len, data->nonce_p,
                        data->nonce_s) < 0) {
    return eap_eke_build_fail(data, ret, id,
                              EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
  }
  ...
  resp = eap_eke_build_msg(data, id, data->sess.pnonce_len + data->sess.prf_len,
                           EAP_EKE_CONFIRM);
  if (resp == NULL) {
    return eap_eke_build_fail(data, ret, id,
                              EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
  ...
  if (eap_eke_derive_msk(&data->sess, data->serverid, data->serverid_len,
                         data->peerid, data->peerid_len, data->nonce_s,
                         data->nonce_p, data->msk, data->emsk) < 0) {
...
static struct wpabuf *eap_eke_process(struct eap_sm *sm, void *priv,
  ...
  case EAP_EKE_ID:
    ...
    break;
  case EAP_EKE_COMMIT:
    resp = eap_eke_process_commit(sm, data, ret, reqData, pos, end - pos);
    break;
  case EAP_EKE_CONFIRM:
    resp = eap_eke_process_confirm(data, ret, reqData, pos, end - pos);
    break;
  case EAP_EKE_FAILURE:
    resp = eap_eke_process_failure(data, ret, reqData, pos, end - pos);
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-EKE: Ignoring message with unknown EKE-Exch %d",
               eke_exch);
    ret->ignore = TRUE;
    return NULL;
  }
