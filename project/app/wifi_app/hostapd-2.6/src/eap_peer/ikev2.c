...
static int ikev2_derive_keys(struct ikev2_responder_data *data) {
  ...
  shared = dh_derive_shared(data->i_dh_public, data->r_dh_private, data->dh);
  if (shared == NULL)
    return -1;
...
static int ikev2_parse_proposal(struct ikev2_proposal_data *prop, const u8 *pos,
                                const u8 *end) {
  const u8 *pend, *ppos;
  int proposal_len, i;
...
static int ikev2_process_sai1(struct ikev2_responder_data *data, const u8 *sai1,
                              size_t sai1_len) {
...
static int ikev2_process_kei(struct ikev2_responder_data *data, const u8 *kei,
                             size_t kei_len) {
  u16 group;
...
static int ikev2_process_ni(struct ikev2_responder_data *data, const u8 *ni,
                            size_t ni_len) {
  if (ni == NULL) {
  ...
  if (ni_len < IKEV2_NONCE_MIN_LEN || ni_len > IKEV2_NONCE_MAX_LEN) {
    wpa_printf(MSG_INFO, "IKEV2: Invalid Ni length %ld", (long)ni_len);
    return -1;
  }

  data->i_nonce_len = ni_len;
  os_memcpy(data->i_nonce, ni, ni_len);
  wpa_hexdump(MSG_MSGDUMP, "IKEV2: Ni", data->i_nonce, data->i_nonce_len);
...
static int ikev2_process_idi(struct ikev2_responder_data *data, const u8 *idi,
                             size_t idi_len) {
  u8 id_type;
...
static int ikev2_process_cert(struct ikev2_responder_data *data, const u8 *cert,
                              size_t cert_len) {
  u8 cert_encoding;
...
static int ikev2_process_auth_cert(struct ikev2_responder_data *data, u8 method,
...
static int ikev2_process_auth_secret(struct ikev2_responder_data *data,
  ...
  if (ikev2_derive_auth_data(data->proposal.prf, data->i_sign_msg, data->IDi,
                             data->IDi_len, data->IDi_type, &data->keys, 1,
...
static int ikev2_process_auth(struct ikev2_responder_data *data, const u8 *auth,
                              size_t auth_len) {
  u8 auth_method;
...
int ikev2_responder_process(struct ikev2_responder_data *data,
  ...
  wpa_hexdump(MSG_DEBUG, "IKEV2:   IKE_SA Initiator's SPI", hdr->i_spi,
              IKEV2_SPI_LEN);
  wpa_hexdump(MSG_DEBUG, "IKEV2:   IKE_SA Responder's SPI", hdr->r_spi,
              IKEV2_SPI_LEN);
  wpa_printf(MSG_DEBUG,
...
static int ikev2_build_nr(struct ikev2_responder_data *data, struct wpabuf *msg,
                          u8 next_payload) {
  struct ikev2_payload_hdr *phdr;
  size_t plen;
...
static int ikev2_build_auth(struct ikev2_responder_data *data,
  ...
  if (ikev2_derive_auth_data(data->proposal.prf, data->r_sign_msg, data->IDr,
                             data->IDr_len, ID_KEY_ID, &data->keys, 0,
                             data->shared_secret, data->shared_secret_len,
...
static struct wpabuf *ikev2_build_sa_init(struct ikev2_responder_data *data) {
  ...
  if (os_get_random(data->r_spi, IKEV2_SPI_LEN))
    return NULL;
  wpa_hexdump(MSG_DEBUG, "IKEV2: IKE_SA Responder's SPI", data->r_spi,
              IKEV2_SPI_LEN);
  ...
  if (ikev2_build_sar1(data, msg, IKEV2_PAYLOAD_KEY_EXCHANGE) ||
      ...
      ikev2_build_nr(data, msg,
                     data->peer_auth == PEER_AUTH_SECRET
  ...
  if (data->peer_auth == PEER_AUTH_SECRET) {
    ...
    if (ikev2_build_idr(data, plain, IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) ||
        ikev2_build_encrypted(data->proposal.encr, data->proposal.integ,
                              &data->keys, 0, msg, plain, IKEV2_PAYLOAD_IDr)) {
      wpabuf_free(plain);
      wpabuf_free(msg);
      return NULL;
    }
...
static struct wpabuf *ikev2_build_notify(struct ikev2_responder_data *data) {
  ...
  if (data->last_msg == LAST_MSG_SA_AUTH) {
    ...
    ikev2_build_hdr(data, msg, IKE_SA_AUTH, IKEV2_PAYLOAD_ENCRYPTED, 1);
    if (ikev2_build_notification(data, plain, IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) ||
        ikev2_build_encrypted(data->proposal.encr, data->proposal.integ,
    ...
    /* HDR, N */
    ikev2_build_hdr(data, msg, IKE_SA_INIT, IKEV2_PAYLOAD_NOTIFICATION, 0);
    if (ikev2_build_notification(data, msg, IKEV2_PAYLOAD_NO_NEXT_PAYLOAD)) {
      wpabuf_free(msg);
      return NULL;
    }
    data->state = SA_INIT;
  ...
  wpa_hexdump_buf(MSG_MSGDUMP, "IKEV2: Sending message (Notification)", msg);
