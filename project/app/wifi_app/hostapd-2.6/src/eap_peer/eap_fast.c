...
static int eap_fast_session_ticket_cb(void *ctx, const u8 *ticket, size_t len,
  ...
  if (client_random == NULL || server_random == NULL || master_secret == NULL) {
  ...
  eap_fast_derive_master_secret(data->current_pac->pac_key, server_random,
...
static void *eap_fast_init(struct eap_sm *sm) {
  ...
  if (eap_peer_select_phase2_methods(config, "auth=", &data->phase2_types,
  ...
  if (tls_connection_set_session_ticket_cb(
          sm->ssl_ctx, data->ssl.conn, eap_fast_session_ticket_cb, data) < 0) {
...
static int eap_fast_derive_key_auth(struct eap_sm *sm,
  ...
  sks = eap_fast_derive_key(sm->ssl_ctx, data->ssl.conn, EAP_FAST_SKS_LEN);
  if (sks == NULL) {
   ...
   */
  wpa_hexdump_key(MSG_DEBUG, "EAP-FAST: session_key_seed (SKS = S-IMCK[0])",
...
static int eap_fast_derive_key_provisioning(struct eap_sm *sm,
  ...
  os_free(data->key_block_p);
  data->key_block_p =
      (struct eap_fast_key_block_provisioning *)eap_fast_derive_key(
          sm->ssl_ctx, data->ssl.conn, sizeof(*data->key_block_p));
...
static int eap_fast_init_phase2_method(struct eap_sm *sm,
                                       struct eap_fast_data *data) {
  data->phase2_method = eap_peer_get_eap_method(data->phase2_type.vendor,
...
static int eap_fast_select_phase2_method(struct eap_fast_data *data, u8 type) {
  ...
  if (type == EAP_TYPE_TNC) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               data->phase2_type.vendor, data->phase2_type.method);
    return 0;
  }
#endif /* EAP_TNC */
...
static int eap_fast_phase2_request(struct eap_sm *sm,
  ...
  if (data->phase2_type.vendor == EAP_VENDOR_IETF &&
    ...
    if (eap_peer_tls_phase2_nak(data->phase2_types, data->num_phase2_types, hdr,
                                resp))
      return -1;
    return 0;
  }
  ...
  *resp = data->phase2_method->process(sm, data->phase2_priv, &iret, &msg);
  if (*resp == NULL ||
      (iret.methodState == METHOD_DONE && iret.decision == DECISION_FAIL)) {
    ret->methodState = METHOD_DONE;
    ret->decision = DECISION_FAIL;
...
static struct wpabuf *eap_fast_tlv_pac_ack(void) {
  ...
  ack->tlv_type = host_to_be16(EAP_TLV_PAC_TLV | EAP_TLV_TYPE_MANDATORY);
...
static struct wpabuf *eap_fast_process_eap_payload_tlv(
    struct eap_sm *sm, struct eap_fast_data *data, struct eap_method_ret *ret,
...
static int
eap_fast_validate_crypto_binding(struct eap_tlv_crypto_binding_tlv *_bind) {
  wpa_printf(MSG_DEBUG,
             "EAP-FAST: Crypto-Binding TLV: Version %d "
             "Received Version %d SubType %d",
             _bind->version, _bind->received_version, _bind->subtype);
  wpa_hexdump(MSG_MSGDUMP, "EAP-FAST: NONCE", _bind->nonce,
              sizeof(_bind->nonce));
  wpa_hexdump(MSG_MSGDUMP, "EAP-FAST: Compound MAC", _bind->compound_mac,
              sizeof(_bind->compound_mac));
...
static void
eap_fast_write_crypto_binding(struct eap_tlv_crypto_binding_tlv *rbind,
                              struct eap_tlv_crypto_binding_tlv *_bind,
                              const u8 *cmk) {
  rbind->tlv_type =
      host_to_be16(EAP_TLV_TYPE_MANDATORY | EAP_TLV_CRYPTO_BINDING_TLV);
  rbind->length = host_to_be16(sizeof(*rbind) - sizeof(struct eap_tlv_hdr));
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-FAST: NONCE", rbind->nonce,
              sizeof(rbind->nonce));
  wpa_hexdump(MSG_MSGDUMP, "EAP-FAST: Compound MAC", rbind->compound_mac,
              sizeof(rbind->compound_mac));
}

static int eap_fast_get_phase2_key(struct eap_sm *sm,
                                   struct eap_fast_data *data, u8 *isk,
                                   size_t isk_len) {
  u8 *key;
  size_t key_len;
  ...
  if (!data->phase2_method->isKeyAvailable(sm, data->phase2_priv) ||
      (key = data->phase2_method->getKey(sm, data->phase2_priv, &key_len)) ==
          NULL) {
  ...
  if (key_len > isk_len)
    key_len = isk_len;
  if (key_len == 32 && data->phase2_method->vendor == EAP_VENDOR_IETF &&
...
static int eap_fast_get_cmk(struct eap_sm *sm, struct eap_fast_data *data,
  ...
  if (sha1_t_prf(data->simck, EAP_FAST_SIMCK_LEN, "Inner Methods Compound Keys",
                 isk, sizeof(isk), imck, sizeof(imck)) < 0)
    return -1;
  data->simck_idx++;
  os_memcpy(data->simck, imck, EAP_FAST_SIMCK_LEN);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-FAST: S-IMCK[j]", data->simck,
                  EAP_FAST_SIMCK_LEN);
  os_memcpy(cmk, imck + EAP_FAST_SIMCK_LEN, EAP_FAST_CMK_LEN);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-FAST: CMK[j]", cmk, EAP_FAST_CMK_LEN);
...
static struct wpabuf *eap_fast_process_crypto_binding(
  ...
  wpa_hexdump(MSG_MSGDUMP,
              ...
              "MAC calculation",
              (u8 *)_bind, bind_len);
  hmac_sha1(cmk, EAP_FAST_CMK_LEN, (u8 *)_bind, bind_len, _bind->compound_mac);
  res = os_memcmp_const(cmac, _bind->compound_mac, sizeof(cmac));
  wpa_hexdump(MSG_MSGDUMP, "EAP-FAST: Received Compound MAC", cmac,
              sizeof(cmac));
  ...
  if (!data->anon_provisioning && data->phase2_success) {
    ...
    if (data->session_id) {
      wpa_hexdump(MSG_DEBUG, "EAP-FAST: Derived Session-Id", data->session_id,
                  data->id_len);
    } else {
  ...
  eap_fast_write_crypto_binding((struct eap_tlv_crypto_binding_tlv *)pos, _bind,
                                cmk);
...
static void eap_fast_parse_pac_tlv(struct eap_fast_pac *entry, int type,
  ...
  case PAC_TYPE_PAC_INFO:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-FAST: Ignored unknown PAC type %d", type);
    break;
  }
}

static int eap_fast_process_pac_tlv(struct eap_fast_pac *entry, u8 *pac,
                                    size_t pac_len) {
  ...
  while (left > sizeof(*hdr)) {
    ...
    if (len > left) {
      wpa_printf(MSG_DEBUG,
                 ...
                 type, (unsigned long)len, (unsigned long)left);
      return -1;
    }
...
static int eap_fast_parse_pac_info(struct eap_fast_pac *entry, int type,
  ...
  case PAC_TYPE_A_ID:
    ...
    break;
  case PAC_TYPE_I_ID:
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-FAST: PAC-Info - I-ID", pos, len);
    entry->i_id = pos;
    entry->i_id_len = len;
    break;
  case PAC_TYPE_A_ID_INFO:
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-FAST: PAC-Info - A-ID-Info", pos, len);
    entry->a_id_info = pos;
  ...
  case PAC_TYPE_PAC_TYPE:
    ...
    if (len != 2) {
      wpa_printf(MSG_INFO,
                 ...
                 (unsigned long)len);
      wpa_hexdump_ascii(MSG_DEBUG, "EAP-FAST: PAC-Info - PAC-Type", pos, len);
      return -1;
    }
    ...
    wpa_printf(MSG_DEBUG, "EAP-FAST: PAC-Info - PAC-Type %d", pac_type);
...
static struct wpabuf *eap_fast_process_pac(struct eap_sm *sm,
                                           struct eap_fast_data *data,
                                           struct eap_method_ret *ret, u8 *pac,
                                           size_t pac_len) {
  struct eap_peer_config *config = eap_get_config(sm);
...
static int eap_fast_parse_decrypted(struct wpabuf *decrypted,
  ...
  while (end - pos > 4) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               tlv_type, (unsigned int)len, mandatory ? " (mandatory)" : "");
...
static int eap_fast_encrypt_response(struct eap_sm *sm,
  ...
  wpa_hexdump_buf(MSG_DEBUG, "EAP-FAST: Encrypting Phase 2 data", resp);
  if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_FAST, data->fast_version,
...
static int eap_fast_process_decrypted(struct eap_sm *sm,
  ...
  if (eap_fast_parse_decrypted(decrypted, &tlv, &resp) < 0)
    return 0;
  if (resp)
    return eap_fast_encrypt_response(sm, data, resp, identifier, out_data);
  ...
  if (tlv.crypto_binding) {
    tmp = eap_fast_process_crypto_binding(sm, data, ret, tlv.crypto_binding,
  ...
  if (tlv.iresult == EAP_TLV_RESULT_SUCCESS) {
    tmp = eap_fast_tlv_result(
        failed ? EAP_TLV_RESULT_FAILURE : EAP_TLV_RESULT_SUCCESS, 1);
    resp = wpabuf_concat(resp, tmp);
  }

  if (tlv.eap_payload_tlv) {
    tmp = eap_fast_process_eap_payload_tlv(sm, data, ret, tlv.eap_payload_tlv,
  ...
  if (tlv.pac && tlv.result != EAP_TLV_RESULT_SUCCESS) {
    ...
    tmp = eap_fast_process_pac(sm, data, ret, tlv.pac, tlv.pac_len);
    resp = wpabuf_concat(resp, tmp);
  }
...
static int eap_fast_decrypt(struct eap_sm *sm, struct eap_fast_data *data,
  ...
  if (wpabuf_len(in_data) == 0) {
    ...
    return eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_FAST,
                                data->fast_version, identifier, NULL, out_data);
  }
...
continue_req:
  ...
  res = eap_fast_process_decrypted(sm, data, ret, identifier, in_decrypted,
                                   out_data);
...
static void eap_fast_select_pac(struct eap_fast_data *data, const u8 *a_id,
                                size_t a_id_len) {
  data->current_pac =
      eap_fast_get_pac(data->pac, a_id, a_id_len, PAC_TYPE_TUNNEL_PAC);
  if (data->current_pac == NULL) {
    /*
...
static int eap_fast_set_provisioning_ciphers(struct eap_sm *sm,
  ...
  if (tls_connection_set_cipher_list(sm->ssl_ctx, data->ssl.conn, ciphers)) {
...
static int eap_fast_process_start(struct eap_sm *sm, struct eap_fast_data *data,
                                  u8 flags, const u8 *pos, size_t left) {
  const u8 *a_id;
  size_t a_id_len;
  ...
  wpa_printf(MSG_DEBUG, "EAP-FAST: Using FAST version %d", data->fast_version);
...
static struct wpabuf *eap_fast_process(struct eap_sm *sm, void *priv,
  ...
  pos = eap_peer_tls_process_init(sm, &data->ssl, EAP_TYPE_FAST, ret, reqData,
                                  &left, &flags);
  if (pos == NULL)
    return NULL;
  ...
  if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
    ...
    if (sm->waiting_ext_cert_check && data->pending_resp) {
      ...
      if (config->pending_ext_cert_check == EXT_CERT_CHECK_GOOD) {
        wpa_printf(MSG_DEBUG, "EAP-FAST: External certificate check succeeded "
                              "- continue handshake");
      ...
      if (config->pending_ext_cert_check == EXT_CERT_CHECK_BAD) {
        wpa_printf(MSG_DEBUG, "EAP-FAST: External certificate check failed - "
      ...
      wpa_printf(MSG_DEBUG, "EAP-FAST: Continuing to wait external server "
                            "certificate validation");
      return NULL;
    }

    /* Continue processing TLS handshake (phase 1). */
    res = eap_peer_tls_process_helper(sm, &data->ssl, EAP_TYPE_FAST,
                                      data->fast_version, id, &msg, &resp);
    if (res < 0) {
      wpa_printf(MSG_DEBUG, "EAP-FAST: TLS processing failed");
      ret->methodState = METHOD_DONE;
      ret->decision = DECISION_FAIL;
      return resp;
    }
    ...
    if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
      char cipher[80];
      wpa_printf(MSG_DEBUG, "EAP-FAST: TLS done, proceed to Phase 2");
      if (data->provisioning &&
          (!(data->provisioning_allowed & EAP_FAST_PROV_AUTH) ||
           tls_get_cipher(sm->ssl_ctx, data->ssl.conn, cipher, sizeof(cipher)) <
               0 ||
           os_strstr(cipher, "ADH-") || os_strstr(cipher, "anon"))) {
      ...
      if (eap_fast_derive_keys(sm, data) < 0) {
        wpa_printf(MSG_DEBUG, "EAP-FAST: Could not derive keys");
        ret->methodState = METHOD_DONE;
        ret->decision = DECISION_FAIL;
        wpabuf_free(resp);
        return NULL;
  ...
  if (res == 1) {
    wpabuf_free(resp);
    return eap_peer_tls_build_ack(id, EAP_TYPE_FAST, data->fast_version);
  }
...
static int eap_fast_get_status(struct eap_sm *sm, void *priv, char *buf,
  ...
  if (data->phase2_method) {
    ret = os_snprintf(buf + len, buflen - len, "EAP-FAST Phase2 method=%s\n",
