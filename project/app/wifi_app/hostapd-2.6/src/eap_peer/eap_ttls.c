...
static void *eap_ttls_init(struct eap_sm *sm) {
  ...
  if (config && config->phase2) {
    ...
    while ((token = cstr_token(config->phase2, " \t", &last))) {
      ...
      if (last - token == 8 && os_strncmp(token, "MSCHAPV2", 8) == 0) {
        selected = "MSCHAPV2";
        data->phase2_type = EAP_TTLS_PHASE2_MSCHAPV2;
      } else if (last - token == 6 && os_strncmp(token, "MSCHAP", 6) == 0) {
        selected = "MSCHAP";
        data->phase2_type = EAP_TTLS_PHASE2_MSCHAP;
      } else if (last - token == 3 && os_strncmp(token, "PAP", 3) == 0) {
        selected = "PAP";
        data->phase2_type = EAP_TTLS_PHASE2_PAP;
      } else if (last - token == 4 && os_strncmp(token, "CHAP", 4) == 0) {
        selected = "CHAP";
        data->phase2_type = EAP_TTLS_PHASE2_CHAP;
      } else {
        wpa_printf(MSG_ERROR, "EAP-TTLS: Unsupported Phase2 type '%s'", token);
  ...
  if (data->phase2_type == EAP_TTLS_PHASE2_EAP) {
    if (eap_peer_select_phase2_methods(config,
                                       "autheap=", &data->phase2_eap_types,
                                       &data->num_phase2_eap_types) < 0) {
      eap_ttls_deinit(sm, data);
      return NULL;
    }
...
static u8 *eap_ttls_avp_add(u8 *start, u8 *avphdr, u32 avp_code, u32 vendor_id,
                            int mandatory, const u8 *data, size_t len) {
  u8 *pos;
...
static int eap_ttls_v0_derive_key(struct eap_sm *sm,
                                  struct eap_ttls_data *data) {
  eap_ttls_free_key(data);
  data->key_data = eap_peer_tls_derive_key(
      sm, &data->ssl, "ttls keying material", EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
  if (!data->key_data) {
    wpa_printf(MSG_INFO, "EAP-TTLS: Failed to derive key");
    return -1;
  }

  wpa_hexdump_key(MSG_DEBUG, "EAP-TTLS: Derived key", data->key_data,
                  EAP_TLS_KEY_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-TTLS: Derived EMSK",
                  data->key_data + EAP_TLS_KEY_LEN, EAP_EMSK_LEN);

  os_free(data->session_id);
  data->session_id = eap_peer_tls_derive_session_id(
      sm, &data->ssl, EAP_TYPE_TTLS, &data->id_len);
  if (data->session_id) {
    wpa_hexdump(MSG_DEBUG, "EAP-TTLS: Derived Session-Id", data->session_id,
                data->id_len);
  } else {
...
static void eap_ttls_phase2_select_eap_method(struct eap_ttls_data *data,
  ...
  for (i = 0; i < data->num_phase2_eap_types; i++) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               data->phase2_eap_type.vendor, data->phase2_eap_type.method);
    break;
  }
}
  ...
  *resp = data->phase2_method->process(sm, data->phase2_priv, &iret, &msg);
  if ((iret.methodState == METHOD_DONE ||
      ...
      (iret.decision == DECISION_UNCOND_SUCC ||
       iret.decision == DECISION_COND_SUCC || iret.decision == DECISION_FAIL)) {
...
static int eap_ttls_phase2_request_eap_method(struct eap_sm *sm,
...
#ifdef EAP_TNC
  if (data->tnc_started && data->phase2_method && data->phase2_priv &&
      method == EAP_TYPE_TNC && data->phase2_eap_type.method == EAP_TYPE_TNC)
    return eap_ttls_phase2_eap_process(sm, data, ret, hdr, len, resp);

  if (data->ready_for_tnc && !data->tnc_started && method == EAP_TYPE_TNC) {
  ...
  if (method != data->phase2_eap_type.method || method == EAP_TYPE_NONE) {
    if (eap_peer_tls_phase2_nak(data->phase2_eap_types,
                                data->num_phase2_eap_types, hdr, resp))
      return -1;
    return 0;
  }

  if (data->phase2_priv == NULL) {
    data->phase2_method = eap_peer_get_eap_method(EAP_VENDOR_IETF, method);
...
static int eap_ttls_phase2_request_eap(struct eap_sm *sm,
  ...
  case EAP_TYPE_IDENTITY:
    *resp = eap_sm_buildIdentity(sm, hdr->identifier, 1);
    break;
  default:
    if (eap_ttls_phase2_request_eap_method(sm, data, ret, hdr, len, *pos,
                                           resp) < 0)
      return -1;
    break;
  }
  ...
  wpa_hexdump_buf(MSG_DEBUG, "EAP-TTLS: AVP encapsulate EAP Response", *resp);
...
static int eap_ttls_phase2_request_mschapv2(struct eap_sm *sm,
  ...
  /* User-Name */
  pos = eap_ttls_avp_add(buf, pos, RADIUS_ATTR_USER_NAME, 0, 1, identity,
                         identity_len);

  /* MS-CHAP-Challenge */
  challenge = eap_ttls_implicit_challenge(sm, data,
  ...
  pos = eap_ttls_avp_add(buf, pos, RADIUS_ATTR_MS_CHAP_CHALLENGE,
                         RADIUS_VENDOR_ID_MICROSOFT, 1, challenge,
  ...
  *pos++ = 0; /* Flags */
  ...
  pos += 8;
  if (mschapv2_derive_response(identity, identity_len, password, password_len,
                               pwhash, challenge, peer_challenge, pos,
...
static int eap_ttls_phase2_request_mschap(struct eap_sm *sm,
  ...
  *pos++ = 1;            /* Flags: Use NT style passwords */
  ...
  if (pwhash) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "EAP-TTLS: MSCHAP password hash", password, 16);
  } else {
    nt_challenge_response(challenge, password, password_len,
                          pos); /* NT-Response */
    wpa_hexdump_ascii_key(MSG_DEBUG, "EAP-TTLS: MSCHAP password", password,
                          password_len);
  }
  wpa_hexdump(MSG_DEBUG, "EAP-TTLS: MSCHAP implicit challenge", challenge,
              EAP_TTLS_MSCHAP_CHALLENGE_LEN);
...
static int eap_ttls_phase2_request_chap(struct eap_sm *sm,
  ...
  /* CHAP-Challenge */
  challenge =
      eap_ttls_implicit_challenge(sm, data, EAP_TTLS_CHAP_CHALLENGE_LEN + 1);
  if (challenge == NULL) {
    wpabuf_free(msg);
  ...
  pos = eap_ttls_avp_add(buf, pos, RADIUS_ATTR_CHAP_CHALLENGE, 0, 1, challenge,
  ...
  *pos++ = data->ident;
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: CHAP username", identity,
                    identity_len);
  wpa_hexdump_ascii_key(MSG_DEBUG, "EAP-TTLS: CHAP password", password,
                        password_len);
  wpa_hexdump(MSG_DEBUG, "EAP-TTLS: CHAP implicit challenge", challenge,
              EAP_TTLS_CHAP_CHALLENGE_LEN);
  wpa_hexdump(MSG_DEBUG, "EAP-TTLS: CHAP password", pos,
...
static int eap_ttls_phase2_request(struct eap_sm *sm,
  ...
  if (phase2_type == EAP_TTLS_PHASE2_MSCHAPV2 ||
    ...
    if (eap_get_config_identity(sm, &len) == NULL) {
      wpa_printf(MSG_INFO, "EAP-TTLS: Identity not configured");
    ...
    if (eap_get_config_password(sm, &len) == NULL) {
      wpa_printf(MSG_INFO, "EAP-TTLS: Password not configured");
      eap_sm_request_password(sm);
      return 0;
    }
  }
...
static int eap_ttls_parse_avp(u8 *pos, size_t left,
  ...
  if (avp_length < sizeof(*avp)) {
    wpa_printf(MSG_WARNING, "EAP-TTLS: Invalid AVP length %d", avp_length);
    return -1;
  }
  ...
  if (vendor_id == 0 && avp_code == RADIUS_ATTR_EAP_MESSAGE) {
     ...
     * the user. */
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: AVP - Reply-Message", dpos, dlen);
  } else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
             avp_code == RADIUS_ATTR_MS_CHAP2_SUCCESS) {
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: MS-CHAP2-Success", dpos, dlen);
    if (dlen != 43) {
      wpa_printf(MSG_WARNING,
                 "EAP-TTLS: Unexpected "
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: MS-CHAP-Error", dpos, dlen);
    parse->mschapv2_error = 1;
  } else if (avp_flags & AVP_FLAGS_MANDATORY) {
    wpa_printf(MSG_WARNING,
               "EAP-TTLS: Unsupported mandatory AVP "
               "code %d vendor_id %d - dropped",
               (int)avp_code, (int)vendor_id);
    return -1;
  } else {
    wpa_printf(MSG_DEBUG,
               "EAP-TTLS: Ignoring unsupported AVP "
               "code %d vendor_id %d",
               (int)avp_code, (int)vendor_id);
  }
...
static int eap_ttls_parse_avps(struct wpabuf *in_decrypted,
  ...
  if (left < sizeof(struct ttls_avp)) {
    wpa_printf(MSG_WARNING,
               ...
               (unsigned long)left, (unsigned long)sizeof(struct ttls_avp));
    return -1;
  }
...
static int eap_ttls_encrypt_response(struct eap_sm *sm,
  ...
  wpa_hexdump_buf_key(MSG_DEBUG, "EAP-TTLS: Encrypting Phase 2 data", resp);
  if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_TTLS, data->ttls_version,
                           identifier, resp, out_data)) {
...
static int eap_ttls_process_phase2_eap(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-TTLS: Phase 2 EAP", parse->eapdata,
              parse->eap_len);
  hdr = (struct eap_hdr *)parse->eapdata;

  if (parse->eap_len < sizeof(*hdr)) {
    wpa_printf(MSG_WARNING,
               "EAP-TTLS: Too short Phase 2 EAP "
               "frame (len=%lu, expected %lu or more) - dropped",
               (unsigned long)parse->eap_len, (unsigned long)sizeof(*hdr));
    return -1;
  }
  len = be_to_host16(hdr->length);
  if (len > parse->eap_len) {
    wpa_printf(MSG_INFO,
               "EAP-TTLS: Length mismatch in Phase 2 "
               "EAP frame (EAP hdr len=%lu, EAP data len in "
               "AVP=%lu)",
               (unsigned long)len, (unsigned long)parse->eap_len);
    return -1;
  }
  wpa_printf(MSG_DEBUG,
...
static int eap_ttls_process_phase2_mschapv2(struct eap_sm *sm,
  ...
  if (parse->mschapv2[0] != data->ident) {
    wpa_printf(MSG_WARNING,
               "EAP-TTLS: Ident mismatch for Phase 2 "
               "MSCHAPV2 (received Ident 0x%02x, expected 0x%02x)",
               parse->mschapv2[0], data->ident);
    return -1;
  }
  if (!data->auth_response_valid ||
      mschapv2_verify_auth_response(data->auth_response, parse->mschapv2 + 1,
                                    42)) {
...
static int eap_ttls_process_decrypted(struct eap_sm *sm,
  ...
  case EAP_TTLS_PHASE2_MSCHAPV2:
    ...
    if (res == 1 && parse->eapdata && data->phase2_success) {
      ...
      if (eap_ttls_process_tnc_start(sm, data, ret, parse, &resp) == 0)
        break;
    }
#endif /* EAP_TNC */
    return res;
  ...
  if (resp) {
    if (eap_ttls_encrypt_response(sm, data, resp, identifier, out_data) < 0)
      return -1;
  } else if (config->pending_req_identity || config->pending_req_password ||
...
static int eap_ttls_implicit_identity_request(struct eap_sm *sm,
  ...
  if (eap_ttls_phase2_request(sm, data, ret, hdr, &resp)) {
    ...
    retval = eap_ttls_encrypt_response(sm, data, resp, identifier, out_data);
  }
...
static int eap_ttls_phase2_start(struct eap_sm *sm, struct eap_ttls_data *data,
   ...
   * needed.
   */
  if (data->reauth && tls_connection_resumed(sm->ssl_ctx, data->ssl.conn)) {
    wpa_printf(MSG_DEBUG, "EAP-TTLS: Session resumption - "
                          "skip phase 2");
    *out_data =
        eap_peer_tls_build_ack(identifier, EAP_TYPE_TTLS, data->ttls_version);
    ret->methodState = METHOD_DONE;
    ret->decision = DECISION_UNCOND_SUCC;
    data->phase2_success = 1;
    return 0;
...
static int eap_ttls_decrypt(struct eap_sm *sm, struct eap_ttls_data *data,
  ...
  if ((in_data == NULL || wpabuf_len(in_data) == 0) && data->phase2_start) {
    return eap_ttls_phase2_start(sm, data, ret, identifier, out_data);
  }

  if (in_data == NULL || wpabuf_len(in_data) == 0) {
    /* Received TLS ACK - requesting more fragments */
    return eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_TTLS,
                                data->ttls_version, identifier, NULL, out_data);
  }
...
continue_req:
  ...
  retval = eap_ttls_process_decrypted(sm, data, ret, identifier, &parse,
...
static int eap_ttls_process_handshake(struct eap_sm *sm,
  ...
  if (sm->waiting_ext_cert_check && data->pending_resp) {
    ...
    if (config->pending_ext_cert_check == EXT_CERT_CHECK_GOOD) {
      wpa_printf(MSG_DEBUG, "EAP-TTLS: External certificate check succeeded - "
                            "continue handshake");
    ...
    if (config->pending_ext_cert_check == EXT_CERT_CHECK_BAD) {
      wpa_printf(MSG_DEBUG, "EAP-TTLS: External certificate check failed - "
  ...
  res = eap_peer_tls_process_helper(sm, &data->ssl, EAP_TYPE_TTLS,
                                    data->ttls_version, identifier, in_data,
                                    out_data);
  if (res < 0) {
  ...
  if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
    ...
    if (*out_data == NULL || wpabuf_len(*out_data) == 0) {
      if (eap_ttls_decrypt(sm, data, ret, identifier, NULL, out_data)) {
  ...
  if (res == 2) {
    ...
    *out_data = NULL;
    res = eap_ttls_decrypt(sm, data, ret, identifier, in_data, out_data);
  }
...
static void eap_ttls_check_auth_status(struct eap_sm *sm,
  ...
  if (ret->methodState == METHOD_DONE) {
    ...
    data->phase2_success = 1;
    data->decision_succ = ret->decision;
  } else if (data->decision_succ != DECISION_FAIL && data->phase2_success &&
             !data->ssl.tls_out) {
    /*
     ...
     */
    wpa_printf(MSG_DEBUG, "EAP-TTLS: Restore success decision after fragmented "
...
static struct wpabuf *eap_ttls_process(struct eap_sm *sm, void *priv,
  ...
  pos = eap_peer_tls_process_init(sm, &data->ssl, EAP_TYPE_TTLS, ret, reqData,
                                  &left, &flags);
  if (pos == NULL)
    return NULL;
  ...
  if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
    ...
    res = eap_ttls_process_handshake(sm, data, ret, id, &msg, &resp);
  }
  ...
  if (res == 1) {
    wpabuf_free(resp);
    return eap_peer_tls_build_ack(id, EAP_TYPE_TTLS, data->ttls_version);
  }
  return resp;
}
...
static int eap_ttls_get_status(struct eap_sm *sm, void *priv, char *buf,
  ...
  ret = os_snprintf(buf + len, buflen - len,
                    "EAP-TTLSv%d Phase2 method=", data->ttls_version);
