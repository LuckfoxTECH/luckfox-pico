...
static struct wpabuf *eap_tls_msg_alloc(EapType type, size_t payload_len,
  ...
  if (type == EAP_UNAUTH_TLS_TYPE)
    return eap_msg_alloc(EAP_VENDOR_UNAUTH_TLS, EAP_VENDOR_TYPE_UNAUTH_TLS,
                         payload_len, code, identifier);
  if (type == EAP_WFA_UNAUTH_TLS_TYPE)
    return eap_msg_alloc(EAP_VENDOR_WFA_NEW, EAP_VENDOR_WFA_UNAUTH_TLS,
                         payload_len, code, identifier);
  return eap_msg_alloc(EAP_VENDOR_IETF, type, payload_len, code, identifier);
}
...
static int eap_tls_params_from_conf(struct eap_sm *sm,
  ...
  if (eap_tls_check_blob(sm, &params->ca_cert, &params->ca_cert_blob,
      ...
      eap_tls_check_blob(sm, &params->client_cert, &params->client_cert_blob,
                         &params->client_cert_blob_len) ||
      eap_tls_check_blob(sm, &params->private_key, &params->private_key_blob,
...
static int eap_tls_init_connection(struct eap_sm *sm, struct eap_ssl_data *data,
                                   struct eap_peer_config *config,
                                   struct tls_connection_params *params) {
  int res;
  ...
  if (res == TLS_SET_PARAMS_ENGINE_PRV_BAD_PIN) {
     ...
     */
    wpa_printf(MSG_INFO, "TLS: Bad PIN provided, requesting a new one");
    os_free(config->pin);
    config->pin = NULL;
...
int eap_peer_tls_ssl_init(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (eap_tls_params_from_conf(sm, data, &params, config, data->phase2) < 0)
    return -1;
  ...
  if (config->phase1 && os_strstr(config->phase1, "include_tls_length=1")) {
...
u8 *eap_peer_tls_derive_key(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (tls_connection_export_key(data->ssl_ctx, data->conn, label, out, len)) {
    os_free(out);
    return NULL;
  }
 ...
 * The caller is responsible for freeing the returned buffer.
 */
u8 *eap_peer_tls_derive_session_id(struct eap_sm *sm, struct eap_ssl_data *data,
                                   u8 eap_type, size_t *len) {
...
static int eap_peer_tls_reassemble_fragment(struct eap_ssl_data *data,
  ...
  if (tls_in_len + in_len == 0) {
    ...
    wpa_printf(MSG_WARNING,
               ...
               (unsigned long)data->tls_in_left, (unsigned long)tls_in_len,
...
eap_peer_tls_data_reassemble(struct eap_ssl_data *data,
                             const struct wpabuf *in_data,
                             int *need_more_input) {
  *need_more_input = 0;
...
static int eap_tls_process_input(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  appl_data = NULL;
  data->tls_out =
      tls_connection_handshake(data->ssl_ctx, data->conn, msg, &appl_data);

  eap_peer_tls_reset_input(data);

  if (appl_data && tls_connection_established(data->ssl_ctx, data->conn) &&
      !tls_connection_get_failed(data->ssl_ctx, data->conn)) {
    wpa_hexdump_buf_key(MSG_MSGDUMP, "SSL: Application data", appl_data);
    *out_data = appl_data;
    return 2;
  }
...
static int eap_tls_process_output(struct eap_ssl_data *data, EapType eap_type,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "%lu bytes)",
             (unsigned long)len, (unsigned long)wpabuf_len(data->tls_out));
  ...
  if (!length_included && eap_type == EAP_TYPE_PEAP && peap_version == 0 &&
  ...
  *flags = peap_version;
  ...
  wpabuf_put_data(*out_data, wpabuf_head_u8(data->tls_out) + data->tls_out_pos,
                  len);
 ...
 */
int eap_peer_tls_status(struct eap_sm *sm, struct eap_ssl_data *data, char *buf,
                        size_t buflen, int verbose) {
  char version[20], name[128];
  int len = 0, ret;

  if (tls_get_version(data->ssl_ctx, data->conn, version, sizeof(version)) < 0)
    version[0] = '\0';
...
const u8 *eap_peer_tls_process_init(struct eap_sm *sm,
                                    struct eap_ssl_data *data, EapType eap_type,
                                    struct eap_method_ret *ret,
                                    const struct wpabuf *reqData, size_t *len,
                                    u8 *flags) {
  const u8 *pos;
  size_t left;
  ...
  if (eap_type == EAP_UNAUTH_TLS_TYPE)
    pos = eap_hdr_validate(EAP_VENDOR_UNAUTH_TLS, EAP_VENDOR_TYPE_UNAUTH_TLS,
                           reqData, &left);
  else if (eap_type == EAP_WFA_UNAUTH_TLS_TYPE)
    pos = eap_hdr_validate(EAP_VENDOR_WFA_NEW, EAP_VENDOR_WFA_UNAUTH_TLS,
                           reqData, &left);
  else
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "Flags 0x%02x",
             (unsigned long)wpabuf_len(reqData), *flags);
  if (*flags & EAP_TLS_FLAGS_LENGTH_INCLUDED) {
    ...
    wpa_printf(MSG_DEBUG, "SSL: TLS Message Length: %d", tls_msg_len);
    if (data->tls_in_left == 0) {
...
int eap_peer_tls_encrypt(struct eap_sm *sm, struct eap_ssl_data *data,
  ...
  if (in_data) {
    eap_peer_tls_reset_output(data);
    data->tls_out = tls_connection_encrypt(data->ssl_ctx, data->conn, in_data);
    if (data->tls_out == NULL) {
      wpa_printf(MSG_INFO,
  ...
  return eap_tls_process_output(data, eap_type, peap_version, id, 0, out_data);
}
...
int eap_peer_select_phase2_methods(struct eap_peer_config *config,
  ...
  while (start && *start != '\0') {
    ...
    if (vendor == EAP_VENDOR_IETF && method == EAP_TYPE_NONE) {
      ...
      num_methods++;
      _methods = os_realloc_array(methods, num_methods, sizeof(*methods));
      if (_methods == NULL) {
        os_free(methods);
        os_free(buf);
        return -1;
...
get_defaults:
  ...
  wpa_hexdump(MSG_DEBUG, "TLS: Phase2 EAP types", (u8 *)methods,
...
int eap_peer_tls_phase2_nak(struct eap_method_type *types, size_t num_types,
  ...
  wpa_hexdump(MSG_DEBUG, "TLS: Allowed Phase2 EAP types", (u8 *)types,
              num_types * sizeof(struct eap_method_type));
  *resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_NAK, num_types,
  ...
  for (i = 0; i < num_types; i++) {
    if (types[i].vendor == EAP_VENDOR_IETF && types[i].method < 256)
