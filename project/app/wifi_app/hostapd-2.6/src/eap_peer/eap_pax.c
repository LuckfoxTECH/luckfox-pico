...
static struct wpabuf *eap_pax_alloc_resp(const struct eap_pax_hdr *req, u8 id,
                                         u8 op_code, size_t plen) {
  struct wpabuf *resp;
  struct eap_pax_hdr *pax;

  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PAX, sizeof(*pax) + plen,
...
static struct wpabuf *eap_pax_process_std_1(struct eap_pax_data *data,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: X (server rand)", data->rand.r.x,
              EAP_PAX_RAND_LEN);
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: B = Y (client rand)", data->rand.r.y,
              EAP_PAX_RAND_LEN);
  ...
  eap_pax_mac(req->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.x,
  ...
  eap_pax_mac(req->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(resp),
              wpabuf_len(resp) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0, rpos);
...
static struct wpabuf *eap_pax_process_std_3(struct eap_pax_data *data,
  ...
  if (eap_pax_mac(data->mac_id, data->ck, EAP_PAX_CK_LEN, data->rand.r.y,
                  EAP_PAX_RAND_LEN, (u8 *)data->cid, data->cid_len, NULL, 0,
                  mac) < 0) {
    wpa_printf(MSG_INFO, "EAP-PAX: Could not derive MAC_CK(B, CID)");
  ...
  if (os_memcmp_const(pos, mac, EAP_PAX_MAC_LEN) != 0) {
    wpa_printf(MSG_INFO, "EAP-PAX: Invalid MAC_CK(B, CID) "
                         "received");
    wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: expected MAC_CK(B, CID)", mac,
                EAP_PAX_MAC_LEN);
    ret->methodState = METHOD_DONE;
    ret->decision = DECISION_FAIL;
    return NULL;
  ...
  rpos = wpabuf_put(resp, EAP_PAX_ICV_LEN);
  if (eap_pax_mac(data->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(resp),
                  wpabuf_len(resp) - EAP_PAX_ICV_LEN, NULL, 0, NULL, 0,
                  rpos) < 0) {
    wpabuf_free(resp);
    return NULL;
  }
...
static struct wpabuf *eap_pax_process(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             req->public_key_id);
  wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: received payload", pos,
              len - EAP_PAX_ICV_LEN);
  ...
  if (req->op_code == EAP_PAX_OP_STD_1) {
    eap_pax_mac(req->mac_id, (u8 *)"", 0, wpabuf_head(reqData), mlen, NULL, 0,
                NULL, 0, icvbuf);
  } else {
    eap_pax_mac(req->mac_id, data->ick, EAP_PAX_ICK_LEN, wpabuf_head(reqData),
                mlen, NULL, 0, NULL, 0, icvbuf);
  }
...
static u8 *eap_pax_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  *len = EAP_MSK_LEN;
  if (eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN, "Master Session Key",
                  data->rand.e, 2 * EAP_PAX_RAND_LEN, EAP_MSK_LEN, key) < 0) {
    os_free(key);
    return NULL;
  }
...
static u8 *eap_pax_get_emsk(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  *len = EAP_EMSK_LEN;
  if (eap_pax_kdf(data->mac_id, data->mk, EAP_PAX_MK_LEN,
                  "Extended Master Session Key", data->rand.e,
                  2 * EAP_PAX_RAND_LEN, EAP_EMSK_LEN, key) < 0) {
    os_free(key);
    return NULL;
  }
