...
static struct wpabuf *eap_tnc_build_msg(struct eap_tnc_data *data,
  ...
  if (flags & EAP_TNC_FLAGS_LENGTH_INCLUDED)
    plen += 4;
  resp =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TNC, plen, EAP_CODE_RESPONSE, id);
  if (resp == NULL)
    return NULL;
...
static struct wpabuf *eap_tnc_process(struct eap_sm *sm, void *priv,
  ...
  if (pos == NULL) {
    wpa_printf(MSG_INFO, "EAP-TNC: Invalid frame (pos=%p len=%lu)", pos,
               (unsigned long)len);
    ret->ignore = TRUE;
    return NULL;
  }
  ...
  if (flags & EAP_TNC_FLAGS_MORE_FRAGMENTS) {
    return eap_tnc_process_fragment(data, ret, id, flags, message_length, pos,
                                    end - pos);
  }
  ...
  if (data->state == WAIT_START) {
    ...
    res = tncc_process_if_tnccs(data->tncc, wpabuf_head(data->in_buf),
    ...
    case TNCCS_RECOMMENDATION_ALLOW:
      wpa_msg(sm->msg_ctx, MSG_INFO, "TNC: Recommendation = allow");
      tncs_done = 1;
      break;
    case TNCCS_RECOMMENDATION_NONE:
      wpa_msg(sm->msg_ctx, MSG_INFO, "TNC: Recommendation = none");
      tncs_done = 1;
      break;
    case TNCCS_RECOMMENDATION_ISOLATE:
      wpa_msg(sm->msg_ctx, MSG_INFO, "TNC: Recommendation = isolate");
      tncs_done = 1;
      break;
    }
  }
  ...
  if (tncs_done) {
    resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TNC, 1, EAP_CODE_RESPONSE,
                         eap_get_id(reqData));
    if (resp == NULL)
      return NULL;
  ...
  wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-TNC: Response", wpabuf_head(resp),
                    wpabuf_len(resp));
