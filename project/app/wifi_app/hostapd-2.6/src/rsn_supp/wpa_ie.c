...
static int wpa_gen_wpa_ie_wpa(u8 *wpa_ie, size_t wpa_ie_len,
  ...
  if (wpa_ie_len < sizeof(*hdr) + WPA_SELECTOR_LEN + 2 + WPA_SELECTOR_LEN + 2 +
                       WPA_SELECTOR_LEN)
    return -1;
  ...
  *pos++ = 0;
  ...
  if (suite == 0 || (!wpa_cipher_valid_pairwise(pairwise_cipher) &&
                     pairwise_cipher != WPA_CIPHER_NONE)) {
    wpa_printf(MSG_WARNING, "Invalid pairwise cipher (%d).", pairwise_cipher);
    return -1;
  }
  RSN_SELECTOR_PUT(pos, suite);
  pos += WPA_SELECTOR_LEN;
...
static int wpa_gen_wpa_ie_rsn(u8 *rsn_ie, size_t rsn_ie_len,
  ...
  if (rsn_ie_len < sizeof(*hdr) + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN + 2 +
                       RSN_SELECTOR_LEN + 2 +
  ...
  *pos++ = 0;
  ...
  if (wpa_cipher_valid_mgmt_group(mgmt_group_cipher)) {
    ...
    RSN_SELECTOR_PUT(pos,
                     wpa_cipher_to_suite(WPA_PROTO_RSN, mgmt_group_cipher));
    pos += RSN_SELECTOR_LEN;
  }
#endif /* CONFIG_IEEE80211W */
...
static int wpa_gen_wpa_ie_osen(u8 *wpa_ie, size_t wpa_ie_len,
  ...
  if (wpa_ie_len <
      2 + 4 + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN)
    return -1;
...
int wpa_gen_wpa_ie(struct wpa_sm *sm, u8 *wpa_ie, size_t wpa_ie_len) {
  if (sm->proto == WPA_PROTO_RSN)
    return wpa_gen_wpa_ie_rsn(wpa_ie, wpa_ie_len, sm->pairwise_cipher,
                              sm->group_cipher, sm->key_mgmt,
                              sm->mgmt_group_cipher, sm);
#ifdef CONFIG_HS20
  else if (sm->proto == WPA_PROTO_OSEN)
    return wpa_gen_wpa_ie_osen(wpa_ie, wpa_ie_len, sm->pairwise_cipher,
                               sm->group_cipher, sm->key_mgmt);
#endif /* CONFIG_HS20 */
  else
    return wpa_gen_wpa_ie_wpa(wpa_ie, wpa_ie_len, sm->pairwise_cipher,
                              sm->group_cipher, sm->key_mgmt);
}
...
static int wpa_parse_vendor_specific(const u8 *pos, const u8 *end,
  ...
  if (oui == OUI_MICROSOFT && pos[5] == WMM_OUI_TYPE && pos[1] > 4) {
    if (pos[6] == WMM_OUI_SUBTYPE_INFORMATION_ELEMENT) {
      ...
      ie->wmm = &pos[2];
      ie->wmm_len = pos[1];
      wpa_hexdump(MSG_DEBUG, "WPA: WMM Parameter Element", ie->wmm,
                  ie->wmm_len);
    }
  }
  return 0;
...
static int wpa_parse_generic(const u8 *pos, const u8 *end,
  ...
  if (pos[1] >= 6 && RSN_SELECTOR_GET(pos + 2) == WPA_OUI_TYPE &&
    ...
    wpa_hexdump(MSG_DEBUG, "WPA: WPA IE in EAPOL-Key", ie->wpa_ie,
                ie->wpa_ie_len);
    return 0;
  }
  ...
  if (pos[1] > RSN_SELECTOR_LEN + 2 &&
    ...
    wpa_hexdump(MSG_DEBUG, "WPA: MAC Address in EAPOL-Key", pos, pos[1] + 2);
    return 0;
  }
...
int wpa_supplicant_parse_ies(const u8 *buf, size_t len,
  ...
  for (pos = buf, end = pos + len; end - pos > 1; pos += 2 + pos[1]) {
    ...
    if (*pos == WLAN_EID_RSN) {
      ...
      wpa_hexdump(MSG_DEBUG, "WPA: RSN IE in EAPOL-Key", ie->rsn_ie,
                  ie->rsn_ie_len);
    } else if (*pos == WLAN_EID_MOBILITY_DOMAIN &&
               pos[1] >= sizeof(struct rsn_mdie)) {
      ie->mdie = pos;
      ie->mdie_len = pos[1] + 2;
      wpa_hexdump(MSG_DEBUG, "WPA: MDIE in EAPOL-Key", ie->mdie, ie->mdie_len);
    } else if (*pos == WLAN_EID_FAST_BSS_TRANSITION &&
               pos[1] >= sizeof(struct rsn_ftie)) {
      ie->ftie = pos;
      ie->ftie_len = pos[1] + 2;
      wpa_hexdump(MSG_DEBUG, "WPA: FTIE in EAPOL-Key", ie->ftie, ie->ftie_len);
