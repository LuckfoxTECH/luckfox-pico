...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa) {
  ...
  if (entry) {
    ...
    if (sec < 0)
      sec = 0;
    eloop_register_timeout(sec, 0, pmksa_cache_reauth, pmksa, NULL);
  }
}
...
pmksa_cache_add(struct rsn_pmksa_cache *pmksa, const u8 *pmk, size_t pmk_len,
                const u8 *pmkid, const u8 *kck, size_t kck_len, const u8 *aa,
                const u8 *spa, void *network_ctx, int akmp) {
  ...
  while (pos) {
    if (os_memcmp(aa, pos->aa, ETH_ALEN) == 0) {
      if (pos->pmk_len == pmk_len &&
          ...
          os_memcmp_const(pos->pmkid, entry->pmkid, PMKID_LEN) == 0) {
      ...
      if (!pos->opportunistic)
        pmksa_cache_flush(pmksa, network_ctx, pos->pmk, pos->pmk_len);
  ...
  pmksa->pmksa_count++;
  wpa_printf(MSG_DEBUG,
             "RSN: Added PMKSA cache entry for " MACSTR " network_ctx=%p",
             MAC2STR(entry->aa), network_ctx);
...
void pmksa_cache_flush(struct rsn_pmksa_cache *pmksa, void *network_ctx,
  ...
  while (entry) {
    if ((entry->network_ctx == network_ctx || network_ctx == NULL) &&
        (pmk == NULL || (pmk_len == entry->pmk_len &&
...
struct rsn_pmksa_cache_entry *pmksa_cache_get(struct rsn_pmksa_cache *pmksa,
  ...
  while (entry) {
    if ((aa == NULL || os_memcmp(entry->aa, aa, ETH_ALEN) == 0) &&
        (pmkid == NULL || os_memcmp(entry->pmkid, pmkid, PMKID_LEN) == 0) &&
...
pmksa_cache_clone_entry(struct rsn_pmksa_cache *pmksa,
  ...
  new_entry = pmksa_cache_add(pmksa, old_entry->pmk, old_entry->pmk_len, NULL,
                              NULL, 0, aa, pmksa->sm->own_addr,
...
int pmksa_cache_set_current(struct wpa_sm *sm, const u8 *pmkid, const u8 *bssid,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "try_opportunistic=%d",
             network_ctx, try_opportunistic);
  if (pmkid)
    wpa_hexdump(MSG_DEBUG, "RSN: Search for PMKID", pmkid, PMKID_LEN);
  if (bssid)
    wpa_printf(MSG_DEBUG, "RSN: Search for BSSID " MACSTR, MAC2STR(bssid));

  sm->cur_pmksa = NULL;
  if (pmkid)
    sm->cur_pmksa = pmksa_cache_get(pmksa, NULL, pmkid, network_ctx);
  if (sm->cur_pmksa == NULL && bssid)
    sm->cur_pmksa = pmksa_cache_get(pmksa, bssid, NULL, network_ctx);
  if (sm->cur_pmksa == NULL && try_opportunistic && bssid)
    sm->cur_pmksa = pmksa_cache_get_opportunistic(pmksa, network_ctx, bssid);
  if (sm->cur_pmksa) {
 ...
 */
struct rsn_pmksa_cache *
pmksa_cache_init(void (*free_cb)(struct rsn_pmksa_cache_entry *entry, void *ctx,
                                 enum pmksa_free_reason reason),
