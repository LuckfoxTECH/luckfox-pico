...
static void rsn_preauth_receive(void *ctx, const u8 *src_addr, const u8 *buf,
                                size_t len) {
  ...
  if (sm->preauth_eapol == NULL || is_zero_ether_addr(sm->preauth_bssid) ||
...
static void rsn_preauth_eapol_cb(struct eapol_sm *eapol,
  ...
  if (result == EAPOL_SUPP_RESULT_SUCCESS) {
    ...
    if (res == 0) {
      ...
      sm->pmk_len = pmk_len;
      pmksa_cache_add(sm->pmksa, pmk, pmk_len, NULL, NULL, 0, sm->preauth_bssid,
                      sm->own_addr, sm->network_ctx, WPA_KEY_MGMT_IEEE8021X);
    } else {
  ...
  wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
          "RSN: pre-authentication with " MACSTR " %s",
          MAC2STR(sm->preauth_bssid),
...
static void rsn_preauth_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
          "RSN: pre-authentication with " MACSTR " timed out",
          MAC2STR(sm->preauth_bssid));
  rsn_preauth_deinit(sm);
  rsn_preauth_candidate_process(sm);
}
...
static int rsn_preauth_eapol_send(void *ctx, int type, const u8 *buf,
  ...
  res = l2_packet_send(sm->l2_preauth, sm->preauth_bssid, ETH_P_RSN_PREAUTH,
                       msg, msglen);
  os_free(msg);
  return res;
}
...
int rsn_preauth_init(struct wpa_sm *sm, const u8 *dst,
  ...
  sm->l2_preauth = l2_packet_init(sm->ifname, sm->own_addr, ETH_P_RSN_PREAUTH,
  ...
  if (sm->bridge_ifname) {
    sm->l2_preauth_br =
        l2_packet_init(sm->bridge_ifname, sm->own_addr, ETH_P_RSN_PREAUTH,
                       rsn_preauth_receive, sm, 0);
  ...
  eloop_register_timeout(sm->dot11RSNAConfigSATimeout, 0, rsn_preauth_timeout,
                         sm, NULL);
...
void rsn_preauth_candidate_process(struct wpa_sm *sm) {
  ...
  wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG,
          ...
          "list");
  if (sm->preauth_eapol || sm->proto != WPA_PROTO_RSN ||
  ...
  dl_list_for_each_safe(candidate, n, &sm->pmksa_candidates,
    ...
    if (os_memcmp(sm->bssid, candidate->bssid, ETH_ALEN) != 0 &&
      ...
      wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG,
              "RSN: PMKSA "
              "candidate " MACSTR " selected for pre-authentication",
              MAC2STR(candidate->bssid));
      dl_list_del(&candidate->list);
      rsn_preauth_init(sm, candidate->bssid, sm->eap_conf_ctx);
      os_free(candidate);
      return;
    }
    wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG,
            "RSN: PMKSA candidate " MACSTR
 ...
 */
void pmksa_candidate_add(struct wpa_sm *sm, const u8 *bssid, int prio,
                         int preauth) {
  struct rsn_pmksa_candidate *cand, *pos;

  if (sm->network_ctx && sm->proactive_key_caching)
    pmksa_cache_get_opportunistic(sm->pmksa, sm->network_ctx, bssid);
  ...
  cand = NULL;
  dl_list_for_each(pos, &sm->pmksa_candidates, struct rsn_pmksa_candidate,
                   list) {
 ...
 * candidate list.
 */
void rsn_preauth_scan_result(struct wpa_sm *sm, const u8 *bssid, const u8 *ssid,
                             const u8 *rsn) {
  ...
  if (pmksa &&
      (!pmksa->opportunistic || !(ie.capabilities & WPA_CAPABILITY_PREAUTH)))
    return;
...
int rsn_preauth_get_status(struct wpa_sm *sm, char *buf, size_t buflen,
  ...
  if (sm->preauth_eapol) {
    ...
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
    res = eapol_sm_get_status(sm->preauth_eapol, pos, end - pos, verbose);
    if (res >= 0)
      pos += res;
  }
