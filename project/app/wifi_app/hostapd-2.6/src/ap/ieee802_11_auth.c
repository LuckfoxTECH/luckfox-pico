...
static void
hostapd_acl_cache_free(struct hostapd_cached_radius_acl *acl_cache) {
...
static int hostapd_radius_acl_query(struct hostapd_data *hapd, const u8 *addr,
  ...
  if (add_common_radius_attr(hapd, hapd->conf->radius_auth_req_attr, NULL,
                             msg) < 0)
    goto fail;
  ...
  if (!radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO, (u8 *)buf,
                           os_strlen(buf))) {
...
int hostapd_check_acl(struct hostapd_data *hapd, const u8 *addr,
                      struct vlan_description *vlan_id) {
  if (hostapd_maclist_found(hapd->conf->accept_mac, hapd->conf->num_accept_mac,
                            addr, vlan_id))
    return HOSTAPD_ACL_ACCEPT;

  if (hostapd_maclist_found(hapd->conf->deny_mac, hapd->conf->num_deny_mac,
                            addr, vlan_id))
...
int hostapd_allowed_address(struct hostapd_data *hapd, const u8 *addr,
  ...
  if (hapd->conf->macaddr_acl == USE_EXTERNAL_RADIUS_AUTH) {
    ...
    res = hostapd_acl_cache_get(hapd, addr, session_timeout,
                                acct_interim_interval, vlan_id, psk, identity,
                                radius_cui);
...
static void hostapd_acl_expire_cache(struct hostapd_data *hapd,
  ...
  while (entry) {
    if (os_reltime_expired(now, &entry->timestamp, RADIUS_ACL_TIMEOUT)) {
      wpa_printf(MSG_DEBUG, "Cached ACL entry for " MACSTR " has expired.",
                 MAC2STR(entry->addr));
      if (prev)
...
static void hostapd_acl_expire_queries(struct hostapd_data *hapd,
  ...
  while (entry) {
    if (os_reltime_expired(now, &entry->timestamp, RADIUS_ACL_TIMEOUT)) {
      wpa_printf(MSG_DEBUG, "ACL query for " MACSTR " has expired.",
                 MAC2STR(entry->addr));
      if (prev)
        prev->next = entry->next;
      else
...
static void decode_tunnel_passwords(struct hostapd_data *hapd,
  ...
  for (i = 0;; i++) {
    passphrase = radius_msg_get_tunnel_password(
        msg, &passphraselen, shared_secret, shared_secret_len, req, i);
    /*
    ...
    if (psk) {
      if ((passphraselen == MAX_PASSPHRASE_LEN + 1) &&
        ...
        hostapd_logger(
            hapd, cache->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_WARNING,
            "invalid hex string (%d chars) in Tunnel-Password", passphraselen);
        goto skip;
...
static RadiusRxResult hostapd_acl_recv_radius(struct radius_msg *msg,
  ...
  if (hdr->code == RADIUS_CODE_ACCESS_ACCEPT) {
    ...
    if (radius_msg_get_attr_int32(msg, RADIUS_ATTR_ACCT_INTERIM_INTERVAL,
                                  &cache->acct_interim_interval) == 0 &&
        cache->acct_interim_interval < 60) {
      wpa_printf(MSG_DEBUG,
                 "Ignored too small "
                 "Acct-Interim-Interval %d for STA " MACSTR,
                 cache->acct_interim_interval, MAC2STR(query->addr));
      cache->acct_interim_interval = 0;
    }
    ...
    *notempty =
    ...
    decode_tunnel_passwords(hapd, shared_secret, shared_secret_len, msg, req,
                            cache);
    ...
    if (cache->vlan_id.notempty &&
      ...
      hostapd_logger(
          hapd, query->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
          "Invalid VLAN %d%s received from RADIUS server",
          cache->vlan_id.untagged, cache->vlan_id.tagged[0] ? "+" : "");
...
#ifndef CONFIG_NO_RADIUS
  if (radius_client_register(hapd->radius, RADIUS_AUTH, hostapd_acl_recv_radius,
                             hapd))
    return -1;
#endif /* CONFIG_NO_RADIUS */
