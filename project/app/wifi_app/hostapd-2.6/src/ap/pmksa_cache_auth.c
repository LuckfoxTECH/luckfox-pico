...
void pmksa_cache_auth_flush(struct rsn_pmksa_cache *pmksa) {
  while (pmksa->pmksa) {
    wpa_printf(MSG_DEBUG, "RSN: Flush PMKSA cache entry for " MACSTR,
...
static void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx) {
  ...
  while (pmksa->pmksa && pmksa->pmksa->expiration <= now.sec) {
    wpa_printf(MSG_DEBUG, "RSN: expired PMKSA cache entry for " MACSTR,
               MAC2STR(pmksa->pmksa->spa));
    pmksa_cache_free_entry(pmksa, pmksa->pmksa);
  }
...
static void pmksa_cache_from_eapol_data(struct rsn_pmksa_cache_entry *entry,
  ...
  if (eapol->identity) {
    ...
    if (entry->identity) {
      ...
      os_memcpy(entry->identity, eapol->identity, eapol->identity_len);
    }
  }
...
void pmksa_cache_to_eapol_data(struct hostapd_data *hapd,
  ...
  if (entry->identity) {
    ...
    if (eapol->identity) {
      ...
      os_memcpy(eapol->identity, entry->identity, entry->identity_len);
    }
    wpa_hexdump_ascii(MSG_DEBUG, "STA identity from PMKSA", eapol->identity,
                      eapol->identity_len);
  }
  ...
  if (eapol->radius_class.attr) {
    wpa_printf(MSG_DEBUG,
               ...
               "PMKSA",
               (unsigned long)eapol->radius_class.count);
  }
 ...
 * based on the PMK.
 */
struct rsn_pmksa_cache_entry *pmksa_cache_auth_add(
    struct rsn_pmksa_cache *pmksa, const u8 *pmk, size_t pmk_len,
    const u8 *pmkid, const u8 *kck, size_t kck_len, const u8 *aa, const u8 *spa,
    int session_timeout, struct eapol_state_machine *eapol, int akmp) {
  ...
  if (pmkid)
    ...
    rsn_pmkid(pmk, pmk_len, aa, spa, entry->pmkid, wpa_key_mgmt_sha256(akmp));
  os_get_reltime(&now);
...
pmksa_cache_add_okc(struct rsn_pmksa_cache *pmksa,
                    const struct rsn_pmksa_cache_entry *old_entry, const u8 *aa,
                    const u8 *pmkid) {
  struct rsn_pmksa_cache_entry *entry;
  ...
  if (old_entry->identity) {
    ...
    if (entry->identity) {
      ...
      os_memcpy(entry->identity, old_entry->identity, old_entry->identity_len);
    }
  }
  if (old_entry->cui)
 ...
 */
struct rsn_pmksa_cache_entry *
pmksa_cache_auth_get(struct rsn_pmksa_cache *pmksa, const u8 *spa,
                     const u8 *pmkid) {
  struct rsn_pmksa_cache_entry *entry;

  if (pmkid) {
    for (entry = pmksa->pmkid[PMKID_HASH(pmkid)]; entry; entry = entry->hnext) {
      if ((spa == NULL || os_memcmp(entry->spa, spa, ETH_ALEN) == 0) &&
 ...
 */
struct rsn_pmksa_cache *pmksa_cache_auth_init(
    void (*free_cb)(struct rsn_pmksa_cache_entry *entry, void *ctx),
    void *ctx) {
...
static int das_attr_match(struct rsn_pmksa_cache_entry *entry,
  ...
  if (attr->cui) {
    if (!entry->cui || attr->cui_len != wpabuf_len(entry->cui) ||
        os_memcmp(attr->cui, wpabuf_head(entry->cui), attr->cui_len) != 0)
      return 0;
    match++;
  }

  if (attr->user_name) {
    if (!entry->identity || attr->user_name_len != entry->identity_len ||
        os_memcmp(attr->user_name, entry->identity, attr->user_name_len) != 0)
      return 0;
    match++;
  }
 ...
 */
int pmksa_cache_auth_list(struct rsn_pmksa_cache *pmksa, char *buf,
                          size_t len) {
  int i, ret;
  char *pos = buf;
  struct rsn_pmksa_cache_entry *entry;
  ...
  entry = pmksa->pmksa;
  while (entry) {
    ret = os_snprintf(pos, buf + len - pos, "%d " MACSTR " ", i,
                      MAC2STR(entry->spa));
    if (os_snprintf_error(buf + len - pos, ret))
      return pos - buf;
    pos += ret;
    pos += wpa_snprintf_hex(pos, buf + len - pos, entry->pmkid, PMKID_LEN);
    ret = os_snprintf(pos, buf + len - pos, " %d %d\n",
                      (int)(entry->expiration - now.sec), entry->opportunistic);
    if (os_snprintf_error(buf + len - pos, ret))
      return pos - buf;
    pos += ret;
    entry = entry->next;
