...
int vlan_add(const char *if_name, int vid, const char *vlan_if_name) {
  ...
  if (err < 0) {
    wpa_printf(MSG_ERROR,
               "VLAN: failed to create link %s for "
               "vlan %d on %s (%d): %s",
               vlan_if_name, vid, if_name, if_idx, nl_geterror(err));
    goto vlan_add_error;
  }
...
int vlan_rem(const char *if_name) {
  ...
  if (err < 0) {
    wpa_printf(MSG_ERROR, "VLAN: failed to remove link %s: %s", if_name,
               nl_geterror(err));
    goto vlan_rem_error;
  }
...
int vlan_set_name_type(unsigned int name_type) { return 0; }
