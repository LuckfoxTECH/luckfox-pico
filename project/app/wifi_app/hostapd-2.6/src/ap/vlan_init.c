...
static int vlan_if_add(struct hostapd_data *hapd, struct hostapd_vlan *vlan,
  ...
  for (i = 0; i < NUM_WEP_KEYS; i++) {
    if (!hapd->conf->ssid.wep.key[i])
      continue;
    wpa_printf(MSG_ERROR, "VLAN: Refusing to set up VLAN iface %s with WEP",
               vlan->ifname);
    return -1;
  }
...
int vlan_if_remove(struct hostapd_data *hapd, struct hostapd_vlan *vlan) {
  ...
  if (ret)
    wpa_printf(MSG_ERROR, "WPA deinitialization for VLAN %d failed (%d)",
               vlan->vlan_id, ret);
  ...
  while (vlan) {
    if (vlan->vlan_id != VLAN_ID_WILDCARD) {
      if (vlan_if_add(hapd, vlan, 1)) {
        wpa_printf(MSG_ERROR, "VLAN: Could not add VLAN %s: %s", vlan->ifname,
                   strerror(errno));
        return -1;
      }
...
static void vlan_dynamic_remove(struct hostapd_data *hapd,
  ...
  while (vlan) {
    ...
    if (vlan->vlan_id != VLAN_ID_WILDCARD && vlan_if_remove(hapd, vlan)) {
...
int vlan_init(struct hostapd_data *hapd) {
  ...
  if ((hapd->conf->ssid.dynamic_vlan != DYNAMIC_VLAN_DISABLED ||
    ...
    os_snprintf(vlan->ifname, sizeof(vlan->ifname), "%s.#", hapd->conf->iface);
...
struct hostapd_vlan *vlan_add_dynamic(struct hostapd_data *hapd,
                                      struct hostapd_vlan *vlan, int vlan_id,
  ...
  wpa_printf(MSG_DEBUG, "VLAN: %s(vlan_id=%d ifname=%s)", __func__, vlan_id,
             vlan->ifname);
  ...
  *pos++ = '\0';
  ...
  os_snprintf(n->ifname, sizeof(n->ifname), "%s%d%s", ifname, vlan_id, pos);
...
int vlan_remove_dynamic(struct hostapd_data *hapd, int vlan_id) {
  ...
  wpa_printf(MSG_DEBUG, "VLAN: %s(ifname=%s vlan_id=%d)", __func__,
             hapd->conf->iface, vlan_id);
