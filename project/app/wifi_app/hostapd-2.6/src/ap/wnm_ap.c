...
#define MAX_IGTK_SUBELEM_LEN 26
  ...
  if (!wpa_auth_uses_mfp(sta->wpa_sm) || action_type != WNM_SLEEP_MODE_EXIT) {
    mgmt->u.action.u.wnm_sleep_resp.keydata_len = 0;
  } else {
    gtk_elem_len = wpa_wnmsleep_gtk_subelem(sta->wpa_sm, pos);
    pos += gtk_elem_len;
    wpa_printf(MSG_DEBUG, "Pass 4, gtk_len = %d", (int)gtk_elem_len);
    ...
    igtk_elem_len = res;
    pos += igtk_elem_len;
    wpa_printf(MSG_DEBUG, "Pass 4 igtk_len = %d", (int)igtk_elem_len);
#endif /* CONFIG_IEEE80211W */

    WPA_PUT_LE16((u8 *)&mgmt->u.action.u.wnm_sleep_resp.keydata_len,
   ...
   * PS mode */
  res = hostapd_drv_send_action(hapd, hapd->iface->freq, 0, mgmt->da,
                                &mgmt->u.action.category, len);
  ...
  if (!res) {
    ...
    if (wnmsleep_ie.status == WNM_STATUS_SLEEP_ACCEPT &&
      ...
      hostapd_drv_wnm_oper(hapd, WNM_SLEEP_ENTER_CONFIRM, addr, NULL, NULL);
    ...
    if ((wnmsleep_ie.status == WNM_STATUS_SLEEP_ACCEPT ||
      ...
      hostapd_drv_wnm_oper(hapd, WNM_SLEEP_EXIT_CONFIRM, addr, NULL, NULL);
...
static void ieee802_11_rx_wnmsleep_req(struct hostapd_data *hapd,
  ...
  while (pos + 1 < frm + len) {
    ...
    if (*pos == WLAN_EID_WNMSLEEP)
      ...
      wpa_printf(MSG_DEBUG, "WNM: EID %d not recognized", *pos);
    pos += ie_len + 2;
  }
  ...
  if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_ENTER && tfsreq_ie_start &&
      tfsreq_ie_end && tfsreq_ie_end - tfsreq_ie_start >= 0) {
    tfsreq_ie_len = (tfsreq_ie_end + tfsreq_ie_end[1] + 2) - tfsreq_ie_start;
    wpa_printf(MSG_DEBUG, "TFS Req IE(s) found");
    /* pass the TFS Req IE(s) to driver for processing */
    if (ieee80211_11_set_tfs_ie(hapd, addr, tfsreq_ie_start, &tfsreq_ie_len,
  ...
  if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_EXIT) {
    ...
    ieee80211_11_set_tfs_ie(hapd, addr, tfsreq_ie_start, &tfsreq_ie_len,
...
static int ieee802_11_send_bss_trans_mgmt_request(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG,
             "WNM: Send BSS Transition Management Request to " MACSTR
             " dialog_token=%u req_mode=0x%x disassoc_timer=%u "
             "validity_interval=%u",
             MAC2STR(addr), dialog_token, mgmt->u.action.u.bss_tm_req.req_mode,
...
static void ieee802_11_rx_bss_trans_mgmt_query(struct hostapd_data *hapd,
  ...
  if (len < 2) {
    wpa_printf(
        MSG_DEBUG,
        "WNM: Ignore too short BSS Transition Management Query from " MACSTR,
        MAC2STR(addr));
    return;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "WNM: BSS Transition Candidate List Entries", pos,
              end - pos);
...
static void ieee802_11_rx_bss_trans_mgmt_resp(struct hostapd_data *hapd,
  ...
  if (len < 3) {
    wpa_printf(
        MSG_DEBUG,
        "WNM: Ignore too short BSS Transition Management Response from " MACSTR,
        MAC2STR(addr));
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "WNM: BSS Transition Management Response from " MACSTR
  ...
  if (status_code == WNM_BSS_TM_ACCEPT) {
    ...
    wpa_printf(MSG_DEBUG, "WNM: Target BSSID: " MACSTR, MAC2STR(pos));
    wpa_msg(hapd->msg_ctx, MSG_INFO,
            BSS_TM_RESP MACSTR
            " status_code=%u bss_termination_delay=%u target_bssid=" MACSTR,
            MAC2STR(addr), status_code, bss_termination_delay, MAC2STR(pos));
    pos += ETH_ALEN;
  } else {
    wpa_msg(hapd->msg_ctx, MSG_INFO,
            BSS_TM_RESP MACSTR " status_code=%u bss_termination_delay=%u",
...
static void ieee802_11_rx_wnm_notification_req(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG,
             "WNM: Received WNM Notification Request frame from " MACSTR
             " (dialog_token=%u type=%u)",
             MAC2STR(addr), dialog_token, type);
  wpa_hexdump(MSG_MSGDUMP, "WNM: Notification Request subelements", buf, len);
...
int ieee802_11_rx_wnm_action_ap(struct hostapd_data *hapd,
  ...
  case WNM_BSS_TRANS_MGMT_QUERY:
    ieee802_11_rx_bss_trans_mgmt_query(hapd, mgmt->sa, payload, plen);
    return 0;
  case WNM_BSS_TRANS_MGMT_RESP:
    ieee802_11_rx_bss_trans_mgmt_resp(hapd, mgmt->sa, payload, plen);
    return 0;
  case WNM_SLEEP_MODE_REQ:
    ieee802_11_rx_wnmsleep_req(hapd, mgmt->sa, payload, plen);
    return 0;
  case WNM_NOTIFICATION_REQ:
    ieee802_11_rx_wnm_notification_req(hapd, mgmt->sa, payload, plen);
    return 0;
  }

  wpa_printf(MSG_DEBUG, "WNM: Unsupported WNM Action %u from " MACSTR, action,
             MAC2STR(mgmt->sa));
  return -1;
}

int wnm_send_disassoc_imminent(struct hostapd_data *hapd, struct sta_info *sta,
                               int disassoc_timer) {
  u8 buf[1000], *pos;
  struct ieee80211_mgmt *mgmt;
  ...
  mgmt->u.action.u.bss_tm_req.req_mode = WNM_BSS_TM_REQ_DISASSOC_IMMINENT;
  mgmt->u.action.u.bss_tm_req.disassoc_timer = host_to_le16(disassoc_timer);
  mgmt->u.action.u.bss_tm_req.validity_interval = 0;

  pos = mgmt->u.action.u.bss_tm_req.variable;

  wpa_printf(MSG_DEBUG,
             "WNM: Send BSS Transition Management Request frame to indicate "
             "imminent disassociation (disassoc_timer=%d) to " MACSTR,
             disassoc_timer, MAC2STR(sta->addr));
...
static void set_disassoc_timer(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  wpa_printf(MSG_DEBUG, "Disassociation timer for " MACSTR " set to %d ms",
             MAC2STR(sta->addr), timeout);

  sta->timeout_next = STA_DISASSOC_FROM_CLI;
  eloop_cancel_timeout(ap_handle_timer, hapd, sta);
  eloop_register_timeout(timeout / 1000, timeout % 1000 * 1000, ap_handle_timer,
                         hapd, sta);
}
...
int wnm_send_ess_disassoc_imminent(struct hostapd_data *hapd,
  ...
  mgmt->u.action.u.bss_tm_req.req_mode =
      WNM_BSS_TM_REQ_DISASSOC_IMMINENT | WNM_BSS_TM_REQ_ESS_DISASSOC_IMMINENT;
  mgmt->u.action.u.bss_tm_req.disassoc_timer = host_to_le16(disassoc_timer);
...
int wnm_send_bss_tm_req(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (mbo_len > 0) {
    pos += mbo_add_ie(pos, buf + sizeof(buf) - pos, mbo_attrs, mbo_len);
  }
