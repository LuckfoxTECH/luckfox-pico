...
void sta_ip6addr_del(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  dl_list_for_each_safe(ip6addr, prev, &sta->ip6addr, struct ip6addr, list) {
...
static void ucast_to_stas(struct hostapd_data *hapd, const u8 *buf,
                          size_t len) {
  struct sta_info *sta;
...
static void handle_ndisc(void *ctx, const u8 *src_addr, const u8 *buf,
  ...
  case NEIGHBOR_SOLICITATION:
    ...
    if (!(saddr.s6_addr32[0] == 0 && saddr.s6_addr32[1] == 0 &&
      ...
      if (inet_ntop(AF_INET6, &saddr, addrtxt, sizeof(addrtxt)) == NULL)
        addrtxt[0] = '\0';
      wpa_printf(MSG_DEBUG,
                 "ndisc_snoop: Learned new IPv6 address %s for " MACSTR,
                 addrtxt, MAC2STR(sta->addr));
      hostapd_drv_br_delete_ip_neigh(hapd, 6, (u8 *)&saddr);
      res = hostapd_drv_br_add_ip_neigh(hapd, 6, (u8 *)&saddr, 128, sta->addr);
      if (res) {
        wpa_printf(MSG_ERROR, "ndisc_snoop: Adding ip neigh failed: %d", res);
        return;
      }
...
int ndisc_snoop_init(struct hostapd_data *hapd) {
  hapd->sock_ndisc =
      x_snoop_get_l2_packet(hapd, handle_ndisc, L2_PACKET_FILTER_NDISC);
  if (hapd->sock_ndisc == NULL) {
    wpa_printf(MSG_DEBUG,
               "ndisc_snoop: Failed to initialize L2 packet processing for "
               "NDISC packets: %s",
               strerror(errno));
    return -1;
  }
