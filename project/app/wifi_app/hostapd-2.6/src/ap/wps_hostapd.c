...
static int hostapd_wps_for_each(struct hostapd_data *hapd,
  ...
  return iface->interfaces->for_each_interface(iface->interfaces, wps_for_each,
                                               &data);
}
...
static int hostapd_wps_new_psk_cb(void *ctx, const u8 *mac_addr,
  ...
  if (psk_len != PMK_LEN) {
    wpa_printf(MSG_DEBUG, "Unexpected PSK length %lu", (unsigned long)psk_len);
    return -1;
  }
  ...
  if (hapd->new_psk_cb) {
    hapd->new_psk_cb(hapd->new_psk_cb_ctx, mac_addr, p2p_dev_addr, psk,
                     psk_len);
  }
...
static void hostapd_wps_pin_needed_cb(void *ctx, const u8 *uuid_e,
  ...
  len = os_snprintf(
      txt, sizeof(txt),
      WPS_EVENT_PIN_NEEDED "%s " MACSTR " [%s|%s|%s|%s|%s|%s]", uuid,
...
static int wps_stop_registrar(struct hostapd_data *hapd, void *ctx) {
  ...
  if (hapd != data->current_hapd && hapd->wps != NULL)
    wps_registrar_complete(hapd->wps->registrar, data->uuid_e, data->dev_pw,
                           data->dev_pw_len);
  return 0;
}
...
static void hostapd_wps_reg_success_cb(void *ctx, const u8 *mac_addr,
  ...
  if (hapd->wps_reg_success_cb)
    hapd->wps_reg_success_cb(hapd->wps_reg_success_cb_ctx, mac_addr, uuid_e);
...
static void hostapd_wps_enrollee_seen_cb(void *ctx, const u8 *addr,
  ...
  wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO,
               WPS_EVENT_ENROLLEE_SEEN MACSTR " %s %s 0x%x %u %u [%s]",
               MAC2STR(addr), uuid,
...
void hostapd_wps_eap_completed(struct hostapd_data *hapd) {
   ...
   */
  if (eloop_deplete_timeout(0, 0, wps_reload_config, hapd->iface, NULL) == 1)
...
static void hapd_new_ap_event(struct hostapd_data *hapd, const u8 *attr,
  ...
  if (buf) {
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_NEW_AP_SETTINGS "%s", buf);
    os_free(buf);
  }
}
  ...
  if (bss->wpa) {
    ...
    if (cred->key_len >= 8 && cred->key_len < 64) {
      ...
      if (bss->ssid.wpa_passphrase)
        os_memcpy(bss->ssid.wpa_passphrase, cred->key, cred->key_len);
      hostapd_config_clear_wpa_psk(&bss->ssid.wpa_psk);
    } else if (cred->key_len == 64) {
      hostapd_config_clear_wpa_psk(&bss->ssid.wpa_psk);
      bss->ssid.wpa_psk = os_zalloc(sizeof(struct hostapd_wpa_psk));
      if (bss->ssid.wpa_psk &&
          hexstr2bin((const char *)cred->key, bss->ssid.wpa_psk->psk,
                     PMK_LEN) == 0) {
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx) {
  ...
  if ((hapd->conf->wps_cred_processing == 1 ||
    ...
    if (attr && wps_build_credential_wrap(attr, cred) == 0)
      hapd_new_ap_event(hapd, wpabuf_head_u8(attr), wpabuf_len(attr));
    wpabuf_free(attr);
  } else
  ...
  while (fgets(buf, sizeof(buf), oconf)) {
    if (os_strncmp(buf, "bss=", 4) == 0)
      multi_bss = 1;
    if (!multi_bss &&
        (str_starts(buf, "ssid=") || str_starts(buf, "ssid2=") ||
         str_starts(buf, "auth_algs=") || str_starts(buf, "wep_default_key=") ||
         str_starts(buf, "wep_key") || str_starts(buf, "wps_state=") ||
         str_starts(buf, "wpa=") || str_starts(buf, "wpa_psk=") ||
         str_starts(buf, "wpa_pairwise=") || str_starts(buf, "rsn_pairwise=") ||
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx) {
  ...
  if (!hapd->conf->ap_setup_locked && hapd->ap_pin_failures_consecutive >= 10) {
    /*
    ...
    eloop_register_timeout(hapd->ap_pin_lockout_time, 0,
                           hostapd_wps_reenable_ap_pin, hapd, NULL);
  }
...
static int wps_ap_pin_success(struct hostapd_data *hapd, void *ctx) {
  ...
  wpa_printf(MSG_DEBUG,
             "WPS: Clear consecutive AP PIN failure counter "
...
static void hostapd_wps_event_fail(struct hostapd_data *hapd,
  ...
  if (fail->error_indication > 0 &&
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d",
...
static int hostapd_wps_rf_band_cb(void *ctx) {
  ...
  return hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A ? WPS_RF_50GHZ
         : hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211AD
             ? WPS_RF_60GHZ
             : WPS_RF_24GHZ; /* FIX: dualband AP */
}
...
static const u8 *get_own_uuid(struct hostapd_iface *iface) {
  ...
  if (iface->interfaces == NULL ||
    ...
    return NULL;
  uuid = NULL;
  iface->interfaces->for_each_interface(iface->interfaces, get_uuid_cb, &uuid);
  return uuid;
}
...
static int interface_count(struct hostapd_iface *iface) {
  ...
  if (iface->interfaces == NULL ||
      iface->interfaces->for_each_interface == NULL)
    return 0;
  iface->interfaces->for_each_interface(iface->interfaces, count_interface_cb,
                                        &count);
  return count;
}
...
static int hostapd_wps_set_vendor_ext(struct hostapd_data *hapd,
  ...
  for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++) {
    ...
    wps->dev.vendor_ext[i] = wpabuf_dup(hapd->conf->wps_vendor_ext[i]);
...
int hostapd_init_wps(struct hostapd_data *hapd,
  ...
  if (is_nil_uuid(hapd->conf->uuid)) {
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: Use configured UUID", wps->uuid, UUID_LEN);
  }
  ...
  wps->dev.device_name =
      hapd->conf->device_name ? os_strdup(hapd->conf->device_name) : NULL;
  wps->dev.manufacturer =
      hapd->conf->manufacturer ? os_strdup(hapd->conf->manufacturer) : NULL;
  wps->dev.model_name =
      hapd->conf->model_name ? os_strdup(hapd->conf->model_name) : NULL;
  wps->dev.model_number =
      hapd->conf->model_number ? os_strdup(hapd->conf->model_number) : NULL;
  wps->dev.serial_number =
      hapd->conf->serial_number ? os_strdup(hapd->conf->serial_number) : NULL;
  wps->config_methods = wps_config_methods_str2bin(hapd->conf->config_methods);
  if ((wps->config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_VIRT_DISPLAY |
  ...
  os_memcpy(wps->dev.pri_dev_type, hapd->conf->device_type, WPS_DEV_TYPE_LEN);
  ...
  if (conf->ssid.wpa_psk_file) {
    ...
    if (wps->network_key == NULL)
      goto fail;
    os_memcpy(wps->network_key, conf->ssid.wep.key[0], conf->ssid.wep.len[0]);
  ...
  cfg.disable_auto_conf =
      (hapd->conf->wps_cred_processing == 1) && conf->skip_cred_build;
...
static int wps_add_pin(struct hostapd_data *hapd, void *ctx) {
  ...
  if (hapd->wps == NULL)
    return 0;
  ret = wps_registrar_add_pin(hapd->wps->registrar, data->addr, data->uuid,
                              data->pin, data->pin_len, data->timeout);
  if (ret == 0)
    data->added++;
  return ret;
}
...
static int hostapd_wps_probe_req_rx(void *ctx, const u8 *addr, const u8 *da,
  ...
  if (ieee802_11_parse_elems(ie, ie_len, &elems, 0) == ParseFailed) {
    wpa_printf(MSG_DEBUG, "WPS: Could not parse ProbeReq from " MACSTR,
               MAC2STR(addr));
    return 0;
  }
...
static int hostapd_rx_req_put_wlan_response(
    void *priv, enum upnp_wps_wlanevent_type ev_type, const u8 *mac_addr,
    const struct wpabuf *msg, enum wps_msg_type msg_type) {
  struct hostapd_data *hapd = priv;
  struct sta_info *sta;
  struct upnp_pending_message *p;

  wpa_printf(MSG_DEBUG, "WPS UPnP: PutWLANResponse ev_type=%d mac_addr=" MACSTR,
             ev_type, MAC2STR(mac_addr));
  wpa_hexdump(MSG_MSGDUMP, "WPS UPnP: PutWLANResponse NewMessage",
              wpabuf_head(msg), wpabuf_len(msg));
  if (ev_type != UPNP_WPS_WLANEVENT_TYPE_EAP) {
    wpa_printf(MSG_DEBUG,
               "WPS UPnP: Ignored unexpected "
               "PutWLANResponse WLANEventType %d",
               ev_type);
    return -1;
  }
  ...
  if (!sta->eapol_sm) {
    ...
    wpa_printf(
        MSG_DEBUG,
        "WPS UPnP: Matching STA did not have EAPOL state machine initialized");
    return 0;
  }
...
static int hostapd_wps_upnp_init(struct hostapd_data *hapd,
  ...
  hapd->wps_upnp = upnp_wps_device_init(ctx, wps, hapd, hapd->conf->upnp_iface);
...
static void hostapd_wps_ap_pin_enable(struct hostapd_data *hapd, int timeout) {
  ...
  eloop_cancel_timeout(hostapd_wps_ap_pin_timeout, hapd, NULL);
  if (timeout > 0)
    eloop_register_timeout(timeout, 0, hostapd_wps_ap_pin_timeout, hapd, NULL);
}
...
static int wps_add_nfc_password_token(struct hostapd_data *hapd, void *ctx) {
  ...
  if (hapd->wps == NULL)
    return 0;
  ret = wps_registrar_add_nfc_password_token(
      hapd->wps->registrar, data->oob_dev_pw, data->oob_dev_pw_len);
  if (ret == 0)
    data->added++;
  return ret;
}
...
struct wpabuf *hostapd_wps_nfc_hs_cr(struct hostapd_data *hapd, int ndef) {
  ...
  ret = wps_build_nfc_handover_sel(hapd->wps, hapd->conf->wps_nfc_dh_pubkey,
...
int hostapd_wps_nfc_report_handover(struct hostapd_data *hapd,
  ...
  ret = wps_registrar_add_nfc_pw_token(
      hapd->wps->registrar, attr.oob_dev_password,
...
struct wpabuf *hostapd_wps_nfc_token_gen(struct hostapd_data *hapd, int ndef) {
  if (hapd->conf->wps_nfc_pw_from_config) {
    return wps_nfc_token_build(ndef, hapd->conf->wps_nfc_dev_pw_id,
                               hapd->conf->wps_nfc_dh_pubkey,
                               hapd->conf->wps_nfc_dev_pw);
  }

  return wps_nfc_token_gen(
      ndef, &hapd->conf->wps_nfc_dev_pw_id, &hapd->conf->wps_nfc_dh_pubkey,
      &hapd->conf->wps_nfc_dh_privkey, &hapd->conf->wps_nfc_dev_pw);
}
...
int hostapd_wps_nfc_token_enable(struct hostapd_data *hapd) {
  ...
  if (!hapd->conf->wps_nfc_dh_pubkey || !hapd->conf->wps_nfc_dh_privkey ||
      !hapd->conf->wps_nfc_dev_pw || !hapd->conf->wps_nfc_dev_pw_id)
    return -1;
  ...
  wps->ap_nfc_dev_pw = wpabuf_alloc(wpabuf_len(pw) * 2 + 1);
  if (wps->ap_nfc_dev_pw) {
    wpa_snprintf_hex_uppercase(
        (char *)wpabuf_put(wps->ap_nfc_dev_pw, wpabuf_len(pw) * 2),
        wpabuf_len(pw) * 2 + 1, wpabuf_head(pw), wpabuf_len(pw));
  }
