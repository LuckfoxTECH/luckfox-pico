...
static void iapp_process_add_notify(struct iapp_data *iapp,
  ...
  hostapd_logger(iapp->hapd, add->mac_addr, HOSTAPD_MODULE_IAPP,
                 ...
                 be_to_host16(add->seq_num), inet_ntoa(from->sin_addr),
                 ntohs(from->sin_port), sta ? "" : " (STA not found)");
  ...
  hostapd_logger(iapp->hapd, add->mac_addr, HOSTAPD_MODULE_IAPP,
                 HOSTAPD_LEVEL_DEBUG, "Removing STA due to IAPP ADD-notify");
...
static void iapp_receive_udp(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  fromlen = sizeof(from);
  len = recvfrom(iapp->udp_sock, buf, sizeof(buf), 0, (struct sockaddr *)&from,
                 &fromlen);
  if (len < 0) {
  ...
  hostapd_logger(iapp->hapd, NULL, HOSTAPD_MODULE_IAPP, HOSTAPD_LEVEL_DEBUG,
  ...
  hostapd_logger(iapp->hapd, NULL, HOSTAPD_MODULE_IAPP, HOSTAPD_LEVEL_DEBUG,
                 ...
                 hdr->command, be_to_host16(hdr->identifier), hlen);
...
void iapp_deinit(struct iapp_data *iapp) {
  ...
  if (iapp->udp_sock >= 0) {
    ...
    mreq.imr_address.s_addr = INADDR_ANY;
    mreq.imr_ifindex = 0;
    if (setsockopt(iapp->udp_sock, SOL_IP, IP_DROP_MEMBERSHIP, &mreq,
                   sizeof(mreq)) < 0) {
      wpa_printf(MSG_INFO,
