...
static int wpa_ft_rrb_send(struct wpa_authenticator *wpa_auth, const u8 *dst,
  ...
  return wpa_auth->cb.send_ether(wpa_auth->cb.ctx, dst, ETH_P_RRB, data,
                                 data_len);
}

static int wpa_ft_action_send(struct wpa_authenticator *wpa_auth, const u8 *dst,
                              const u8 *data, size_t data_len) {
  if (wpa_auth->cb.send_ft_action == NULL)
    return -1;
  return wpa_auth->cb.send_ft_action(wpa_auth->cb.ctx, dst, data, data_len);
}
...
int wpa_write_ftie(struct wpa_auth_config *conf, const u8 *r0kh_id,
                   size_t r0kh_id_len, const u8 *anonce, const u8 *snonce,
                   u8 *buf, size_t len, const u8 *subelem, size_t subelem_len) {
  u8 *pos = buf, *ielen;
  struct rsn_ftie *hdr;

  if (len <
      2 + sizeof(*hdr) + 2 + FT_R1KH_ID_LEN + 2 + r0kh_id_len + subelem_len)
    return -1;
...
static int wpa_ft_fetch_pmk_r0(struct wpa_authenticator *wpa_auth,
                               const u8 *spa, const u8 *pmk_r0_name, u8 *pmk_r0,
                               int *pairwise) {
  struct wpa_ft_pmk_cache *cache = wpa_auth->ft_pmk_cache;
  struct wpa_ft_pmk_r0_sa *r0;
  ...
  while (r0) {
    if (os_memcmp(r0->spa, spa, ETH_ALEN) == 0 &&
        os_memcmp_const(r0->pmk_r0_name, pmk_r0_name, WPA_PMK_NAME_LEN) == 0) {
...
static int wpa_ft_fetch_pmk_r1(struct wpa_authenticator *wpa_auth,
                               const u8 *spa, const u8 *pmk_r1_name, u8 *pmk_r1,
                               int *pairwise) {
  struct wpa_ft_pmk_cache *cache = wpa_auth->ft_pmk_cache;
  struct wpa_ft_pmk_r1_sa *r1;
  ...
  while (r1) {
    if (os_memcmp(r1->spa, spa, ETH_ALEN) == 0 &&
        os_memcmp_const(r1->pmk_r1_name, pmk_r1_name, WPA_PMK_NAME_LEN) == 0) {
...
static int wpa_ft_pull_pmk_r1(struct wpa_state_machine *sm, const u8 *ies,
                              size_t ies_len, const u8 *pmk_r0_name) {
  ...
  while (r0kh) {
    if (r0kh->id_len == sm->r0kh_id_len &&
        os_memcmp_const(r0kh->id, sm->r0kh_id, sm->r0kh_id_len) == 0)
      break;
    r0kh = r0kh->next;
  }
  if (r0kh == NULL) {
    wpa_hexdump(MSG_DEBUG, "FT: Did not find R0KH-ID", sm->r0kh_id,
                sm->r0kh_id_len);
    return -1;
  }
  ...
  os_memcpy(sm->ft_pending_pull_nonce, f.nonce, FT_R0KH_R1KH_PULL_NONCE_LEN);
  ...
  if (aes_wrap(r0kh->key, sizeof(r0kh->key),
               (FT_R0KH_R1KH_PULL_DATA_LEN + 7) / 8, f.nonce, frame.nonce) < 0)
    return -1;
...
int wpa_auth_derive_ptk_ft(struct wpa_state_machine *sm, const u8 *pmk,
  ...
  wpa_derive_pmk_r0(sm->xxkey, sm->xxkey_len, ssid, ssid_len, mdid, r0kh,
  ...
  wpa_derive_pmk_r1(pmk_r0, pmk_r0_name, r1kh, sm->addr, pmk_r1,
                    sm->pmk_r1_name);
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1", pmk_r1, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name", sm->pmk_r1_name, WPA_PMK_NAME_LEN);
  wpa_ft_store_pmk_r1(sm->wpa_auth, sm->addr, pmk_r1, sm->pmk_r1_name,
                      sm->pairwise);

  return wpa_pmk_r1_to_ptk(pmk_r1, sm->SNonce, sm->ANonce, sm->addr,
                           sm->wpa_auth->addr, sm->pmk_r1_name, ptk, ptk_name,
...
static u8 *wpa_ft_gtk_subelem(struct wpa_state_machine *sm, size_t *len) {
  ...
  if (aes_wrap(sm->PTK.kek, sm->PTK.kek_len, key_len / 8, key, subelem + 13)) {
    os_free(subelem);
    return NULL;
  }
...
static u8 *wpa_ft_process_rdie(struct wpa_state_machine *sm, u8 *pos, u8 *end,
                               u8 id, u8 descr_count, const u8 *ies,
                               size_t ies_len) {
  struct ieee802_11_elems parse;
  struct rsn_rdie *rdie;

  wpa_printf(MSG_DEBUG, "FT: Resource Request: id=%d descr_count=%d", id,
             descr_count);
  wpa_hexdump(MSG_MSGDUMP, "FT: Resource descriptor IE(s)", ies, ies_len);
  ...
  *pos++ = sizeof(*rdie);
  ...
  if (ieee802_11_parse_elems((u8 *)ies, ies_len, &parse, 1) == ParseFailed) {
  ...
  if (parse.wmm_tspec && sm->wpa_auth->conf.ap_mlme) {
    ...
    if (res == WMM_ADDTS_STATUS_INVALID_PARAMETERS)
      rdie->status_code = host_to_le16(WLAN_STATUS_INVALID_PARAMETERS);
...
static u8 *wpa_ft_process_ric(struct wpa_state_machine *sm, u8 *pos, u8 *end,
  ...
  while (rpos + sizeof(*rdie) < ric + ric_len) {
    ...
    while (rpos + 2 <= ric + ric_len && rpos + 2 + rpos[1] <= ric + ric_len) {
      if (rpos[0] == WLAN_EID_RIC_DATA)
        break;
      rpos += 2 + rpos[1];
    }
    pos = wpa_ft_process_rdie(sm, pos, end, rdie->id, rdie->descr_count, start,
                              rpos - start);
  }
...
u8 *wpa_sm_write_assoc_resp_ies(struct wpa_state_machine *sm, u8 *pos,
  ...
  if (wpa_ft_parse_ies(req_ies, req_ies_len, &parse) == 0 && parse.ric) {
    pos = wpa_ft_process_ric(sm, pos, end, parse.ric, parse.ric_len);
    if (auth_alg == WLAN_AUTH_FT)
      _ftie->mic_control[1] += ieee802_11_ie_count(ric_start, pos - ric_start);
  }
  if (ric_start == pos)
    ric_start = NULL;

  if (auth_alg == WLAN_AUTH_FT &&
      wpa_ft_mic(sm->PTK.kck, sm->PTK.kck_len, sm->addr, sm->wpa_auth->addr, 6,
                 mdie, mdie_len, ftie, ftie_len, rsnie, rsnie_len, ric_start,
                 ric_start ? pos - ric_start : 0, _ftie->mic) < 0)
...
void wpa_ft_install_ptk(struct wpa_state_machine *sm) {
  ...
  if (sm->tk_already_set) {
     ...
     * PN in the driver */
    wpa_printf(MSG_DEBUG, "FT: Do not re-install same PTK to the driver");
    return;
  }
...
static int wpa_ft_process_auth_req(struct wpa_state_machine *sm, const u8 *ies,
                                   size_t ies_len, u8 **resp_ies,
                                   size_t *resp_ies_len) {
  ...
  *resp_ies_len = 0;
  ...
  wpa_hexdump(MSG_DEBUG, "FT: Received authentication frame IEs", ies, ies_len);
  ...
  if (mdie == NULL || parse.mdie_len < sizeof(*mdie) ||
      os_memcmp(mdie->mobility_domain, sm->wpa_auth->conf.mobility_domain,
  ...
  wpa_hexdump(MSG_DEBUG, "FT: STA R0KH-ID", parse.r0kh_id, parse.r0kh_id_len);
  ...
  wpa_hexdump(MSG_DEBUG, "FT: Requested PMKR0Name", parse.rsn_pmkid,
              WPA_PMK_NAME_LEN);
  wpa_derive_pmk_r1_name(parse.rsn_pmkid, sm->wpa_auth->conf.r1_key_holder,
                         sm->addr, pmk_r1_name);
  wpa_hexdump(MSG_DEBUG, "FT: Derived requested PMKR1Name", pmk_r1_name,
              WPA_PMK_NAME_LEN);
  ...
  wpa_hexdump(MSG_DEBUG, "FT: Received SNonce", sm->SNonce, WPA_NONCE_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: Generated ANonce", sm->ANonce, WPA_NONCE_LEN);

  if (wpa_pmk_r1_to_ptk(pmk_r1, sm->SNonce, sm->ANonce, sm->addr,
                        sm->wpa_auth->addr, pmk_r1_name, &sm->PTK, ptk_name,
                        sm->wpa_key_mgmt, pairwise) < 0)
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  ...
  buflen = 2 + sizeof(struct rsn_mdie) + 2 + sizeof(struct rsn_ftie) + 2 +
           FT_R1KH_ID_LEN + 200;
  *resp_ies = os_zalloc(buflen);
  ...
  ret = wpa_write_ftie(conf, parse.r0kh_id, parse.r0kh_id_len, sm->ANonce,
                       sm->SNonce, pos, end - pos, NULL, 0);
  if (ret < 0) {
    os_free(*resp_ies);
    *resp_ies = NULL;
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
...
void wpa_ft_process_auth(struct wpa_state_machine *sm, const u8 *bssid,
  ...
  wpa_printf(MSG_DEBUG,
             "FT: Received authentication frame: STA=" MACSTR " BSSID=" MACSTR
             " transaction=%d",
             MAC2STR(sm->addr), MAC2STR(bssid), auth_transaction);
  sm->ft_pending_cb = cb;
  sm->ft_pending_cb_ctx = ctx;
  sm->ft_pending_auth_transaction = auth_transaction;
  res = wpa_ft_process_auth_req(sm, ies, ies_len, &resp_ies, &resp_ies_len);
  if (res < 0) {
  ...
  cb(ctx, sm->addr, bssid, auth_transaction + 1, status, resp_ies,
     resp_ies_len);
  os_free(resp_ies);
}
...
u16 wpa_ft_validate_reassoc(struct wpa_state_machine *sm, const u8 *ies,
  ...
  if (os_memcmp(ftie->snonce, sm->SNonce, WPA_NONCE_LEN) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "FT: Received SNonce", ftie->snonce, WPA_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "FT: Expected SNonce", sm->SNonce, WPA_NONCE_LEN);
    return -1;
  }

  if (os_memcmp(ftie->anonce, sm->ANonce, WPA_NONCE_LEN) != 0) {
    wpa_printf(MSG_DEBUG, "FT: ANonce mismatch in FTIE");
    wpa_hexdump(MSG_DEBUG, "FT: Received ANonce", ftie->anonce, WPA_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "FT: Expected ANonce", sm->ANonce, WPA_NONCE_LEN);
    return -1;
  }
  ...
  if (parse.r0kh_id_len != sm->r0kh_id_len ||
    ...
    wpa_printf(MSG_DEBUG, "FT: R0KH-ID in FTIE did not match with "
                          "the current R0KH-ID");
    wpa_hexdump(MSG_DEBUG, "FT: R0KH-ID in FTIE", parse.r0kh_id,
                parse.r0kh_id_len);
    wpa_hexdump(MSG_DEBUG, "FT: The current R0KH-ID", sm->r0kh_id,
                sm->r0kh_id_len);
    return -1;
  }
  ...
  if (os_memcmp_const(parse.r1kh_id, sm->wpa_auth->conf.r1_key_holder,
    ...
    wpa_printf(MSG_DEBUG, "FT: Unknown R1KH-ID used in "
                          "ReassocReq");
    wpa_hexdump(MSG_DEBUG, "FT: R1KH-ID in FTIE", parse.r1kh_id,
                FT_R1KH_ID_LEN);
  ...
  if (wpa_ft_mic(sm->PTK.kck, sm->PTK.kck_len, sm->addr, sm->wpa_auth->addr, 5,
                 parse.mdie - 2, parse.mdie_len + 2, parse.ftie - 2,
                 parse.ftie_len + 2, parse.rsn - 2, parse.rsn_len + 2,
                 parse.ric, parse.ric_len, mic) < 0) {
    wpa_printf(MSG_DEBUG, "FT: Failed to calculate MIC");
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  }
  ...
  if (os_memcmp_const(mic, ftie->mic, mic_len) != 0) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "FT: Received MIC", ftie->mic, mic_len);
    wpa_hexdump(MSG_MSGDUMP, "FT: Calculated MIC", mic, mic_len);
    wpa_hexdump(MSG_MSGDUMP, "FT: MDIE", parse.mdie - 2, parse.mdie_len + 2);
    wpa_hexdump(MSG_MSGDUMP, "FT: FTIE", parse.ftie - 2, parse.ftie_len + 2);
    wpa_hexdump(MSG_MSGDUMP, "FT: RSN", parse.rsn - 2, parse.rsn_len + 2);
    return WLAN_STATUS_INVALID_FTIE;
  }
...
int wpa_ft_action_rx(struct wpa_state_machine *sm, const u8 *data, size_t len) {
  ...
  wpa_printf(MSG_DEBUG,
             "FT: Received FT Action frame (STA=" MACSTR " Target AP=" MACSTR
             " Action=%d)",
  ...
  wpa_ft_rrb_send(sm->wpa_auth, target_ap, (u8 *)frame, sizeof(*frame) + len);
  os_free(frame);
...
static int wpa_ft_rrb_rx_request(struct wpa_authenticator *wpa_auth,
  ...
  res = wpa_ft_process_auth_req(sm, body, len, &resp_ies, &resp_ies_len);
  if (res < 0) {
    wpa_printf(MSG_DEBUG,
  ...
  res = wpa_ft_send_rrb_auth_resp(sm, current_ap, sta_addr, status, resp_ies,
                                  resp_ies_len);
  os_free(resp_ies);
  return res;
}
...
static int wpa_ft_send_rrb_auth_resp(struct wpa_state_machine *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "FT: RRB authentication response: STA=" MACSTR " CurrentAP=" MACSTR
             " status=%d",
  ...
  *pos++ = 2; /* Action: Response */
  ...
  wpa_ft_rrb_send(wpa_auth, current_ap, (u8 *)frame, sizeof(*frame) + rlen);
  os_free(frame);
...
static int wpa_ft_rrb_rx_pull(struct wpa_authenticator *wpa_auth,
                              const u8 *src_addr, const u8 *data,
                              size_t data_len) {
  ...
  if (aes_unwrap(r1kh->key, sizeof(r1kh->key),
                 (FT_R0KH_R1KH_PULL_DATA_LEN + 7) / 8, crypt, plain) < 0) {
    wpa_printf(MSG_DEBUG,
  ...
  wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 pull - nonce", f.nonce, sizeof(f.nonce));
  wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 pull - PMKR0Name", f.pmk_r0_name,
              WPA_PMK_NAME_LEN);
  wpa_printf(MSG_DEBUG, "FT: PMK-R1 pull - R1KH-ID=" MACSTR " S1KH-ID=" MACSTR,
  ...
  wpa_derive_pmk_r1(pmk_r0, f.pmk_r0_name, f.r1kh_id, f.s1kh_id, r.pmk_r1,
                    r.pmk_r1_name);
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1", r.pmk_r1, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name", r.pmk_r1_name, WPA_PMK_NAME_LEN);
  r.pairwise = host_to_le16(pairwise);
  os_memset(r.pad, 0, sizeof(r.pad));

  if (aes_wrap(r1kh->key, sizeof(r1kh->key),
               (FT_R0KH_R1KH_RESP_DATA_LEN + 7) / 8, r.nonce, resp.nonce) < 0) {
...
static void ft_pull_resp_cb_finish(void *eloop_ctx, void *timeout_ctx) {
  ...
  res = wpa_ft_process_auth_req(sm, wpabuf_head(sm->ft_pending_req_ies),
                                wpabuf_len(sm->ft_pending_req_ies), &resp_ies,
                                &resp_ies_len);
  wpabuf_free(sm->ft_pending_req_ies);
  ...
  status = res;
  wpa_printf(MSG_DEBUG,
             "FT: Postponed auth callback result for " MACSTR " - status %u",
             MAC2STR(sm->addr), status);

  sm->ft_pending_cb(sm->ft_pending_cb_ctx, sm->addr, sm->wpa_auth->addr,
                    sm->ft_pending_auth_transaction + 1, status, resp_ies,
                    resp_ies_len);
  os_free(resp_ies);
}
...
static int wpa_ft_rrb_rx_resp(struct wpa_authenticator *wpa_auth,
  ...
  if (aes_unwrap(r0kh->key, sizeof(r0kh->key),
                 (FT_R0KH_R1KH_RESP_DATA_LEN + 7) / 8, crypt, plain) < 0) {
    wpa_printf(MSG_DEBUG,
               "FT: Failed to decrypt PMK-R1 pull "
  ...
  wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 pull - nonce", f.nonce, sizeof(f.nonce));
  wpa_printf(MSG_DEBUG,
             "FT: PMK-R1 pull - R1KH-ID=" MACSTR " S1KH-ID=" MACSTR
             " pairwise=0x%x",
             MAC2STR(f.r1kh_id), MAC2STR(f.s1kh_id), pairwise);
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1 pull - PMK-R1", f.pmk_r1, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 pull - PMKR1Name", f.pmk_r1_name,
              WPA_PMK_NAME_LEN);
...
static int wpa_ft_rrb_rx_push(struct wpa_authenticator *wpa_auth,
  ...
  os_memset(&f, 0, sizeof(f));
  plain = ((u8 *)&f) + offsetof(struct ft_r0kh_r1kh_push_frame, timestamp);
  /* aes_unwrap() does not support inplace decryption, so use a temporary
   * buffer for the data. */
  if (aes_unwrap(r0kh->key, sizeof(r0kh->key),
                 (FT_R0KH_R1KH_PUSH_DATA_LEN + 7) / 8, crypt, plain) < 0) {
    wpa_printf(MSG_DEBUG, "FT: Failed to decrypt PMK-R1 push from " MACSTR,
               MAC2STR(src_addr));
    return -1;
  }
  ...
  if (os_memcmp_const(f.r1kh_id, wpa_auth->conf.r1_key_holder,
    ...
    wpa_printf(MSG_DEBUG,
               ...
               MAC2STR(f.r1kh_id), MAC2STR(wpa_auth->conf.r1_key_holder));
    return -1;
  }

  pairwise = le_to_host16(f.pairwise);
  wpa_printf(MSG_DEBUG,
             "FT: PMK-R1 push - R1KH-ID=" MACSTR " S1KH-ID=" MACSTR
             " pairwise=0x%x",
             MAC2STR(f.r1kh_id), MAC2STR(f.s1kh_id), pairwise);
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1 push - PMK-R1", f.pmk_r1, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMK-R1 push - PMKR1Name", f.pmk_r1_name,
              WPA_PMK_NAME_LEN);

  wpa_ft_store_pmk_r1(wpa_auth, f.s1kh_id, f.pmk_r1, f.pmk_r1_name, pairwise);
  os_memset(f.pmk_r1, 0, PMK_LEN);
...
int wpa_ft_rrb_rx(struct wpa_authenticator *wpa_auth, const u8 *src_addr,
  ...
  pos += ETH_ALEN;
  wpa_printf(MSG_DEBUG,
             "FT: RRB Action Frame: action=%d sta_addr=" MACSTR
  ...
  if (frame->packet_type == FT_PACKET_REQUEST) {
    ...
    if (wpa_ft_rrb_rx_request(wpa_auth, frame->ap_address, sta_addr, pos,
                              end - pos) < 0)
      return -1;
  ...
  if (end > pos) {
    wpa_hexdump(MSG_DEBUG, "FT: Ignore extra data in end", pos, end - pos);
  }
...
static void wpa_ft_generate_pmk_r1(struct wpa_authenticator *wpa_auth,
  ...
  wpa_derive_pmk_r1(pmk_r0->pmk_r0, pmk_r0->pmk_r0_name, r1kh->id, s1kh_id,
                    f.pmk_r1, f.pmk_r1_name);
  wpa_printf(MSG_DEBUG, "FT: R1KH-ID " MACSTR, MAC2STR(r1kh->id));
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1", f.pmk_r1, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name", f.pmk_r1_name, WPA_PMK_NAME_LEN);
  os_get_time(&now);
  WPA_PUT_LE32(f.timestamp, now.sec);
  f.pairwise = host_to_le16(pairwise);
  os_memset(f.pad, 0, sizeof(f.pad));
  plain =
      ((const u8 *)&f) + offsetof(struct ft_r0kh_r1kh_push_frame, timestamp);
  crypt = ((u8 *)&frame) + offsetof(struct ft_r0kh_r1kh_push_frame, timestamp);
  if (aes_wrap(r1kh->key, sizeof(r1kh->key),
               (FT_R0KH_R1KH_PUSH_DATA_LEN + 7) / 8, plain, crypt) < 0)
    return;
