...
static void mbo_ap_parse_non_pref_chan(struct sta_info *sta, const u8 *buf,
                                       size_t len) {
  ...
  *pos = '\0';
  for (i = 0; i < num_chan; i++) {
    ret = os_snprintf(pos, end - pos, "%s%u", i == 0 ? "" : " ", buf[i]);
  ...
  wpa_printf(MSG_DEBUG,
             "MBO: STA " MACSTR " non-preferred channel list (op class %u, "
             "pref %u, reason code %u, channels %s)",
             MAC2STR(sta->addr), info->op_class, info->pref, info->reason_code,
             channels);
}
...
int mbo_ap_get_info(struct sta_info *sta, char *buf, size_t buflen) {
  ...
  for (info = sta->non_pref_chan; info; info = info->next) {
    ...
    ret = os_snprintf(pos2, end - pos2, "non_pref_chan[%u]=%u:%u:%u:", count,
    ...
    for (i = 0; i < info->num_channels; i++) {
      ret = os_snprintf(pos2, end - pos2, "%u%s", info->channels[i],
...
static void mbo_ap_wnm_notif_req_cell_capa(struct sta_info *sta, const u8 *buf,
                                           size_t len) {
  if (len < 1)
    return;
  wpa_printf(MSG_DEBUG,
             "MBO: STA " MACSTR " updated cellular data capability: %u",
...
static void mbo_ap_wnm_notif_req_elem(struct sta_info *sta, u8 type,
  ...
  default:
    wpa_printf(MSG_DEBUG,
               "MBO: Ignore unknown WNM Notification WFA subelement %u", type);
    break;
  }
}
...
void mbo_ap_wnm_notification_req(struct hostapd_data *hapd, const u8 *addr,
  ...
  while (end - pos > 1) {
    ...
    if (pos[0] == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 4 &&
        WPA_GET_BE24(pos + 2) == OUI_WFA)
      mbo_ap_wnm_notif_req_elem(sta, pos[5], pos + 6, ie_len - 4,
                                &first_non_pref_chan);
    else
      wpa_printf(MSG_DEBUG,
