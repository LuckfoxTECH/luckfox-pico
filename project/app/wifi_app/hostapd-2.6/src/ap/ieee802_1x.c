...
#endif /* CONFIG_HS20 */
static void ieee802_1x_finished(struct hostapd_data *hapd, struct sta_info *sta,
                                int success, int remediation);
...
static void ieee802_1x_send(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (hapd->ext_eapol_frame_io) {
    ...
    if (sta->flags & WLAN_STA_PREAUTH) {
      ...
      hostapd_drv_hapd_send_eapol(hapd, sta->addr, buf, len, encrypt,
...
static void ieee802_1x_tx_key_one(struct hostapd_data *hapd,
                                  struct sta_info *sta, int idx, int broadcast,
  ...
  wpa_printf(MSG_DEBUG,
             "IEEE 802.1X: Sending EAPOL-Key to " MACSTR " (%s index=%d)",
             MAC2STR(sm->addr), broadcast ? "broadcast" : "unicast", idx);
...
static void ieee802_1x_tx_key(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  if (hapd->conf->individual_wep_key_len > 0) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "Individual WEP key", ikey,
    ...
    if (hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP, sta->addr, 0,
                            1, NULL, 0, ikey,
                            hapd->conf->individual_wep_key_len)) {
...
static void ieee802_1x_learn_identity(struct hostapd_data *hapd,
  ...
  if (len <= sizeof(struct eap_hdr) ||
      ...
      (hdr->code != EAP_CODE_RESPONSE && hdr->code != EAP_CODE_INITIATE))
    return;
  ...
  hostapd_logger(hapd, sm->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                 "STA identity '%s'", sm->identity);
...
static int add_common_radius_sta_attr_rsn(struct hostapd_data *hapd,
  ...
  suite = wpa_cipher_to_suite(((hapd->conf->wpa & 0x2) || hapd->conf->osen)
                                  ? WPA_PROTO_RSN
                                  : WPA_PROTO_WPA,
                              hapd->conf->wpa_group);
  if (!hostapd_config_get_radius_attr(req_attr,
                                      RADIUS_ATTR_WLAN_GROUP_CIPHER) &&
      !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_GROUP_CIPHER, suite)) {
  ...
  if (val != -1 &&
      !hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_AKM_SUITE) &&
      !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_AKM_SUITE, suite)) {
  ...
  if (hapd->conf->ieee80211w != NO_MGMT_FRAME_PROTECTION) {
    ...
    if (!hostapd_config_get_radius_attr(req_attr,
        ...
        !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_GROUP_MGMT_CIPHER,
                                   suite)) {
...
static int add_common_radius_sta_attr(struct hostapd_data *hapd,
  ...
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_SERVICE_TYPE) &&
  ...
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_PORT) &&
      sta->aid > 0 &&
  ...
  buf[sizeof(buf) - 1] = '\0';
  if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID, (u8 *)buf,
                           os_strlen(buf))) {
  ...
  if (sta->flags & WLAN_STA_PREAUTH) {
    ...
    os_snprintf(
        buf, sizeof(buf), "CONNECT %d%sMbps %s", radius_sta_rate(hapd, sta) / 2,
        (radius_sta_rate(hapd, sta) & 1) ? ".5" : "", radius_mode_txt(hapd));
    buf[sizeof(buf) - 1] = '\0';
  }
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_CONNECT_INFO) &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO, (u8 *)buf,
                           os_strlen(buf))) {
  ...
  if ((hapd->conf->wpa & 2) && !hapd->conf->disable_pmksa_caching &&
      sta->eapol_sm && sta->eapol_sm->acct_multi_session_id) {
    os_snprintf(buf, sizeof(buf), "%016llX",
                (unsigned long long)sta->eapol_sm->acct_multi_session_id);
    if (!radius_msg_add_attr(msg, RADIUS_ATTR_ACCT_MULTI_SESSION_ID, (u8 *)buf,
                             os_strlen(buf))) {
      wpa_printf(MSG_INFO, "Could not add Acct-Multi-Session-Id");
      return -1;
    }
  }
  ...
  if (hapd->conf->wpa && wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) &&
      ...
      !radius_msg_add_attr_int32(msg, RADIUS_ATTR_MOBILITY_DOMAIN_ID,
                                 WPA_GET_BE16(hapd->conf->mobility_domain))) {
...
int add_common_radius_attr(struct hostapd_data *hapd,
  ...
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IP_ADDRESS) &&
...
#ifdef CONFIG_IPV6
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IPV6_ADDRESS) &&
  ...
  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IDENTIFIER) &&
  ...
  os_memcpy(&buf[len], hapd->conf->ssid.ssid, hapd->conf->ssid.ssid_len);
  len += hapd->conf->ssid.ssid_len;
  if (!hostapd_config_get_radius_attr(req_attr,
                                      RADIUS_ATTR_CALLED_STATION_ID) &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_CALLED_STATION_ID, (u8 *)buf,
                           len)) {
    wpa_printf(MSG_ERROR, "Could not add Called-Station-Id");
    return -1;
  }

  if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_PORT_TYPE) &&
  ...
  if (hapd->conf->interworking && !is_zero_ether_addr(hapd->conf->hessid)) {
    ...
    buf[sizeof(buf) - 1] = '\0';
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_HESSID) &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_WLAN_HESSID, (u8 *)buf,
                             os_strlen(buf))) {
...
static void ieee802_1x_encapsulate_radius(struct hostapd_data *hapd,
                                          struct sta_info *sta, const u8 *eap,
                                          size_t len) {
  struct radius_msg *msg;
  struct eapol_state_machine *sm = sta->eapol_sm;
  ...
  if (sm->identity && !radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME,
                                           sm->identity, sm->identity_len)) {
    wpa_printf(MSG_INFO, "Could not add User-Name");
    goto fail;
  }

  if (add_common_radius_attr(hapd, hapd->conf->radius_auth_req_attr, sta, msg) <
      0)
    goto fail;
  ...
  if (sm->last_recv_radius && radius_msg_get_hdr(sm->last_recv_radius)->code ==
                                  RADIUS_CODE_ACCESS_CHALLENGE) {
    int res =
        radius_msg_copy_attr(msg, sm->last_recv_radius, RADIUS_ATTR_STATE);
    if (res < 0) {
      wpa_printf(
          MSG_INFO,
  ...
  if (hapd->conf->hs20) {
    u8 ver = 1; /* Release 2 */
    if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_AP_VERSION, &ver,
                            1)) {
    ...
    if (sta->hs20_ie && wpabuf_len(sta->hs20_ie) > 0) {
      ...
      if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_STA_VERSION, buf,
                              sizeof(buf))) {
...
static void handle_eap_response(struct hostapd_data *hapd, struct sta_info *sta,
                                struct eap_hdr *eap, size_t len) {
  u8 type, *data;
  struct eapol_state_machine *sm = sta->eapol_sm;
  if (sm == NULL)
    return;
...
static void handle_eap_initiate(struct hostapd_data *hapd, struct sta_info *sta,
                                struct eap_hdr *eap, size_t len) {
#ifdef CONFIG_ERP
  u8 type, *data;
  struct eapol_state_machine *sm = sta->eapol_sm;
  ...
  hostapd_logger(hapd, sm->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                 ...
                 eap->code, eap->identifier, be_to_host16(eap->length), type);
...
static void handle_eap(struct hostapd_data *hapd, struct sta_info *sta, u8 *buf,
                       size_t len) {
  struct eap_hdr *eap;
  u16 eap_len;
  ...
  wpa_printf(MSG_DEBUG, "EAP: code=%d identifier=%d length=%d", eap->code,
             eap->identifier, eap_len);
...
ieee802_1x_alloc_eapol_sm(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  return eapol_auth_alloc(hapd->eapol_auth, sta->addr, flags, sta->wps_ie,
                          sta->p2p_ie, sta, sta->identity, sta->radius_cui);
}
...
void ieee802_1x_receive(struct hostapd_data *hapd, const u8 *sa, const u8 *buf,
  ...
  if (!sta || (!(sta->flags & (WLAN_STA_ASSOC | WLAN_STA_PREAUTH)) &&
    ...
    if (sta && (sta->flags & WLAN_STA_AUTH)) {
      wpa_printf(MSG_DEBUG, "Saving EAPOL frame from " MACSTR " for later use",
                 MAC2STR(sta->addr));
  ...
  if (!sta->eapol_sm) {
    ...
    if (!hapd->conf->ieee802_1x && hapd->conf->wps_state) {
      u32 wflags =
          sta->flags & (WLAN_STA_WPS | WLAN_STA_WPS2 | WLAN_STA_MAYBE_WPS);
...
void ieee802_1x_new_station(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  if (sta->eapol_sm == NULL) {
    ...
    if (sta->eapol_sm == NULL) {
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
                     HOSTAPD_LEVEL_INFO, "failed to allocate state machine");
      return;
    }
    reassoc = 0;
  }
  ...
  if (sta->auth_alg == WLAN_AUTH_FT) {
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
                   HOSTAPD_LEVEL_DEBUG, "PMK from FT - skip IEEE 802.1X/EAP");
...
static void ieee802_1x_decapsulate_radius(struct hostapd_data *hapd,
  ...
  buf[sizeof(buf) - 1] = '\0';
  hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                 "decapsulated EAP packet (code=%d "
                 "id=%d len=%d) from RADIUS server: %s",
                 hdr->code, hdr->identifier, be_to_host16(hdr->length), buf);
...
static void ieee802_1x_get_keys(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (keys && keys->send && keys->recv) {
    ...
    if (sm->eap_if->aaaEapKeyData) {
      ...
      os_memcpy(sm->eap_if->aaaEapKeyData + keys->recv_len, keys->send,
                keys->send_len);
...
static void ieee802_1x_store_radius_class(struct hostapd_data *hapd,
  ...
  for (i = 0; i < count; i++) {
    do {
      if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_CLASS, &attr_class,
                                  &class_len, attr_class) < 0) {
        i = count;
        break;
      }
    } while (class_len < 1);
  ...
  wpa_printf(MSG_DEBUG,
             ...
             (unsigned long)sm->radius_class.count, MAC2STR(sta->addr));
}
...
static void ieee802_1x_update_sta_identity(struct hostapd_data *hapd,
  ...
  hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                 ...
                 sm->identity ? (char *)sm->identity : "N/A", (char *)identity);
...
static void ieee802_1x_update_sta_cui(struct hostapd_data *hapd,
  ...
  if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_CHARGEABLE_USER_IDENTITY, &buf,
                              &len, NULL) < 0)
    return;
...
static RadiusRxResult ieee802_1x_receive_auth(struct radius_msg *msg,
  ...
  if (hdr->code == RADIUS_CODE_ACCESS_REJECT &&
      radius_msg_get_attr(msg, RADIUS_ATTR_MESSAGE_AUTHENTICATOR, NULL, 0) <
          0 &&
    ...
    wpa_printf(MSG_INFO, "Incoming RADIUS packet did not have correct "
  ...
  session_timeout_set = !radius_msg_get_attr_int32(
      msg, RADIUS_ATTR_SESSION_TIMEOUT, &session_timeout);
  ...
  case RADIUS_CODE_ACCESS_ACCEPT:
    ...
    if (hapd->conf->ssid.dynamic_vlan != DYNAMIC_VLAN_DISABLED) {
      ...
      *notempty =
          !!radius_msg_get_vlanid(msg, untagged, MAX_NUM_TAGGED_VLAN, tagged);
    }
    ...
    if (vlan_desc.notempty &&
      ...
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
                     ...
                     vlan_desc.untagged, vlan_desc.tagged[0] ? "+" : "");
      os_memset(&vlan_desc, 0, sizeof(vlan_desc));
    ...
    if (session_timeout_set &&
        termination_action == RADIUS_TERMINATION_ACTION_RADIUS_REQUEST) {
    ...
    ieee802_1x_check_hs20(hapd, sta, msg,
                          session_timeout_set ? (int)session_timeout : -1);
    break;
...
static void ieee802_1x_rekey(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (ieee802_1x_rekey_broadcast(hapd)) {
    hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_WARNING,
  ...
  if (hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP,
                          broadcast_ether_addr, eapol->default_wep_key_idx, 1,
                          NULL, 0, eapol->default_wep_key,
                          hapd->conf->default_wep_key_len)) {
    hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_WARNING,
  ...
  if (hapd->conf->wep_rekeying_period > 0) {
    eloop_register_timeout(hapd->conf->wep_rekeying_period, 0, ieee802_1x_rekey,
                           hapd, NULL);
  }
}
...
static void ieee802_1x_aaa_send(void *ctx, void *sta_ctx, const u8 *data,
                                size_t datalen) {
#ifndef CONFIG_NO_RADIUS
...
static struct eap_server_erp_key *ieee802_1x_erp_get_key(void *ctx,
                                                         const char *keyname) {
  struct hostapd_data *hapd = ctx;
...
int ieee802_1x_init(struct hostapd_data *hapd) {
...
#ifndef CONFIG_NO_RADIUS
  if (radius_client_register(hapd->radius, RADIUS_AUTH, ieee802_1x_receive_auth,
                             hapd))
    return -1;
#endif /* CONFIG_NO_RADIUS */
  ...
  if (hapd->conf->default_wep_key_len) {
    for (i = 0; i < 4; i++)
      hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_NONE, NULL, i, 0,
                          NULL, 0, NULL, 0);
...
int ieee802_1x_tx_status(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  u8 *pos;
  const unsigned char rfc1042_hdr[ETH_ALEN] = {0xaa, 0xaa, 0x03,
                                               0x00, 0x00, 0x00};
...
int ieee802_1x_eapol_tx_status(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (len < (int)sizeof(*xhdr))
    return 0;
  wpa_printf(MSG_DEBUG,
             "IEEE 802.1X: " MACSTR " TX status - version=%d "
  ...
  if (!ack && pos + sizeof(*key) <= buf + len) {
    ...
    hostapd_logger(
        hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
        "did not Ack EAPOL-Key "
        "frame (%scast index=%d)",
        key->key_index & BIT(7) ? "uni" : "broad", key->key_index & ~BIT(7));
...
static const char *bool_txt(Boolean val) { return val ? "TRUE" : "FALSE"; }
...
int ieee802_1x_get_mib_sta(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  ret = os_snprintf(buf + len, buflen - len,
                    ...
                    sm->auth_pae_state + 1, sm->be_auth_state + 1,
                    sm->adminControlledDirections, sm->operControlledDirections,
                    sm->authPortStatus, sm->portControl, sm->quietPeriod,
                    sm->serverTimeout, sm->reAuthPeriod,
                    bool_txt(sm->reAuthEnabled), bool_txt(sm->keyTxEnabled));
  if (os_snprintf_error(buflen - len, ret))
    return len;
  len += ret;
  ...
  ret = os_snprintf(
      ...
      sm->dot1xAuthEapolFramesRx, sm->dot1xAuthEapolFramesTx,
      sm->dot1xAuthEapolStartFramesRx, sm->dot1xAuthEapolLogoffFramesRx,
      sm->dot1xAuthEapolRespIdFramesRx, sm->dot1xAuthEapolRespFramesRx,
      sm->dot1xAuthEapolReqIdFramesTx, sm->dot1xAuthEapolReqFramesTx,
      sm->dot1xAuthInvalidEapolFramesRx, sm->dot1xAuthEapLengthErrorFramesRx,
      sm->dot1xAuthLastEapolFrameVersion, MAC2STR(sm->addr));
  if (os_snprintf_error(buflen - len, ret))
    return len;
  len += ret;
  ...
  ret = os_snprintf(
      ...
      sm->authEntersConnecting, sm->authEapLogoffsWhileConnecting,
      sm->authEntersAuthenticating, sm->authAuthSuccessesWhileAuthenticating,
      ...
      sm->authAuthEapLogoffWhileAuthenticated, sm->backendResponses,
      sm->backendAccessChallenges, sm->backendOtherRequestsToSupplicant,
  ...
  ret = os_snprintf(
      ...
      (unsigned long long)sta->acct_session_id,
      (wpa_key_mgmt_wpa_ieee8021x(wpa_auth_sta_key_mgmt(sta->wpa_sm))) ? 1 : 2,
      (unsigned int)diff.sec, sm->identity);
  if (os_snprintf_error(buflen - len, ret))
    return len;
  len += ret;

  if (sm->acct_multi_session_id) {
    ret = os_snprintf(buf + len, buflen - len, "authMultiSessionId=%016llX\n",
                      (unsigned long long)sm->acct_multi_session_id);
    if (os_snprintf_error(buflen - len, ret))
      return len;
    len += ret;
  }
...
static void ieee802_1x_wnm_notif_send(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (sta->remediation) {
    wpa_printf(MSG_DEBUG,
               ...
               MAC2STR(sta->addr));
    hs20_send_wnm_notification(hapd, sta->addr, sta->remediation_method,
                               sta->remediation_url);
    os_free(sta->remediation_url);
    sta->remediation_url = NULL;
  }
...
static void ieee802_1x_finished(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (success && (sta->remediation || sta->hs20_deauth_req)) {
    wpa_printf(MSG_DEBUG,
               "HS 2.0: Schedule WNM-Notification to " MACSTR " in 100 ms",
               MAC2STR(sta->addr));
    eloop_cancel_timeout(ieee802_1x_wnm_notif_send, hapd, sta);
    eloop_register_timeout(0, 100000, ieee802_1x_wnm_notif_send, hapd, sta);
  }
#endif /* CONFIG_HS20 */
