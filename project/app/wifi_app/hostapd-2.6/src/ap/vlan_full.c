...
static int br_delif(const char *br_name, const char *if_name) {
  ...
  if (ioctl(fd, SIOCDEVPRIVATE, &ifr) < 0 && errno != EINVAL) {
    ...
    wpa_printf(MSG_ERROR,
               ...
               "%s",
               __func__, br_name, if_name, strerror(errno));
    close(fd);
    return -1;
  }
...
static void vlan_newlink_tagged(int vlan_naming, const char *tagged_interface,
  ...
  if (vlan_naming == DYNAMIC_VLAN_NAMING_WITH_DEVICE)
    os_snprintf(vlan_ifname, sizeof(vlan_ifname), "%s.%d", tagged_interface,
                vid);
  else
...
static void vlan_bridge_name(char *br_name, struct hostapd_data *hapd,
  ...
  if (hapd->conf->vlan_bridge[0]) {
    os_snprintf(br_name, IFNAMSIZ, "%s%d", hapd->conf->vlan_bridge, vid);
  } else if (tagged_interface) {
    os_snprintf(br_name, IFNAMSIZ, "br%s.%d", tagged_interface, vid);
  } else {
...
static void vlan_get_bridge(const char *br_name, struct hostapd_data *hapd,
  ...
  if (tagged_interface)
    vlan_newlink_tagged(vlan_naming, tagged_interface, br_name, vid, hapd);
}
...
void vlan_newlink(const char *ifname, struct hostapd_data *hapd) {
  ...
  for (vlan = hapd->conf->vlan; vlan; vlan = vlan->next) {
    if (vlan->configured || os_strcmp(ifname, vlan->ifname) != 0)
      continue;
    break;
  }
  if (!vlan)
  ...
  if (!notempty) {
    /* Non-VLAN STA */
    if (hapd->conf->bridge[0] && !br_addif(hapd->conf->bridge, ifname))
  ...
  for (i = 0; i < MAX_NUM_TAGGED_VLAN && tagged[i]; i++) {
    if (tagged[i] == untagged || tagged[i] <= 0 || tagged[i] > MAX_VLAN_ID ||
        (i > 0 && tagged[i] == tagged[i - 1]))
      continue;
    vlan_bridge_name(br_name, hapd, tagged[i]);
    vlan_get_bridge(br_name, hapd, tagged[i]);
    vlan_newlink_tagged(DYNAMIC_VLAN_NAMING_WITH_DEVICE, ifname, br_name,
                        tagged[i], hapd);
  }
...
static void vlan_put_bridge(const char *br_name, struct hostapd_data *hapd,
  ...
  if (tagged_interface)
    vlan_dellink_tagged(vlan_naming, tagged_interface, br_name, vid, hapd);
...
void vlan_dellink(const char *ifname, struct hostapd_data *hapd) {
  ...
  if (vlan->configured) {
    ...
    for (i = 0; i < MAX_NUM_TAGGED_VLAN && tagged[i]; i++) {
      if (tagged[i] == untagged || tagged[i] <= 0 || tagged[i] > MAX_VLAN_ID ||
        ...
        continue;
      vlan_bridge_name(br_name, hapd, tagged[i]);
      vlan_dellink_tagged(DYNAMIC_VLAN_NAMING_WITH_DEVICE, ifname, br_name,
                          tagged[i], hapd);
  ...
  if (if_nametoindex(vlan->ifname) && vlan_if_remove(hapd, vlan))
    wpa_printf(MSG_ERROR, "VLAN: Could not remove VLAN iface: %s: %s",
...
static void vlan_read_ifnames(struct nlmsghdr *h, size_t len, int del,
                              struct hostapd_data *hapd) {
  ...
  wpa_printf(MSG_DEBUG,
             "VLAN: RTM_%sLINK: ifi_index=%d ifname=%s ifi_family=%d "
             "ifi_flags=0x%x (%s%s%s%s)",
             del ? "DEL" : "NEW", ifi->ifi_index, ifname, ifi->ifi_family,
             ifi->ifi_flags, (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
...
static void vlan_event_receive(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  if (left < 0) {
    if (errno != EINTR && errno != EAGAIN)
      wpa_printf(MSG_ERROR, "VLAN: %s: recvfrom failed: %s", __func__,
                 strerror(errno));
    return;
  }
...
struct full_dynamic_vlan *full_dynamic_vlan_init(struct hostapd_data *hapd) {
  ...
  if (bind(priv->s, (struct sockaddr *)&local, sizeof(local)) < 0) {
    wpa_printf(MSG_ERROR, "VLAN: %s: bind(netlink) failed: %s", __func__,
               strerror(errno));
    close(priv->s);
    os_free(priv);
    return NULL;
