...
int hostapd_get_hw_features(struct hostapd_iface *iface) {
  ...
  if (modes == NULL) {
    hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
  ...
  for (i = 0; i < num_modes; i++) {
    ...
    int dfs_enabled =
        hapd->iconf->ieee80211h && (iface->drv_flags & WPA_DRIVER_FLAGS_RADAR);
    ...
    for (j = 0; j < feature->num_channels; j++) {
      ...
      if ((feature->channels[j].flag & HOSTAPD_CHAN_RADAR) && dfs_enabled) {
        dfs = 1;
                 ...
                 (feature->channels[j].flag & HOSTAPD_CHAN_NO_IR)) {
      ...
      wpa_printf(MSG_MSGDUMP,
                 ...
                 feature->mode, feature->channels[j].chan,
                 feature->channels[j].freq, feature->channels[j].max_tx_power,
...
int hostapd_prepare_rates(struct hostapd_iface *iface,
  ...
  for (i = 0; i < mode->num_rates; i++) {
    ...
    if (iface->conf->supported_rates &&
        !hostapd_rate_found(iface->conf->supported_rates, mode->rates[i]))
      continue;
    ...
    wpa_printf(MSG_DEBUG, "RATE[%d] rate=%d flags=0x%x", iface->num_rates,
               rate->rate, rate->flags);
    iface->num_rates++;
  }
...
static void
ieee80211n_scan_channels_2g4(struct hostapd_iface *iface,
...
#define HT2040_COEX_SCAN_RETRY 15
  ...
  if (ret == -EBUSY && iface->num_ht40_scan_tries < HT2040_COEX_SCAN_RETRY) {
    wpa_printf(MSG_ERROR,
               "Failed to request a scan of neighboring BSSes ret=%d (%s) - "
               "try to scan again (attempt %d)",
...
static int ieee80211n_check_40mhz(struct hostapd_iface *iface) {
  ...
  if (ret < 0) {
    wpa_printf(MSG_ERROR,
               "Failed to request a scan of neighboring BSSes ret=%d (%s)", ret,
               strerror(-ret));
    return -1;
  }
...
static int ieee80211n_supported_ht_capab(struct hostapd_iface *iface) {
  ...
  if ((conf & HT_CAP_INFO_GREEN_FIELD) && !(hw & HT_CAP_INFO_GREEN_FIELD)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
                          "HT capability [GF]");
    return 0;
  }

  if ((conf & HT_CAP_INFO_SHORT_GI20MHZ) && !(hw & HT_CAP_INFO_SHORT_GI20MHZ)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
                          "HT capability [SHORT-GI-20]");
    return 0;
  }

  if ((conf & HT_CAP_INFO_SHORT_GI40MHZ) && !(hw & HT_CAP_INFO_SHORT_GI40MHZ)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
  ...
  if ((conf & HT_CAP_INFO_RX_STBC_MASK) > (hw & HT_CAP_INFO_RX_STBC_MASK)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
                          "HT capability [RX-STBC*]");
    return 0;
  }

  if ((conf & HT_CAP_INFO_DELAYED_BA) && !(hw & HT_CAP_INFO_DELAYED_BA)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
  ...
  if ((conf & HT_CAP_INFO_DSSS_CCK40MHZ) && !(hw & HT_CAP_INFO_DSSS_CCK40MHZ)) {
    wpa_printf(MSG_ERROR, "Driver does not support configured "
...
static int ieee80211ac_cap_check(u32 hw, u32 conf, u32 cap, const char *name) {
  ...
  if ((hw & req_cap) != req_cap) {
    wpa_printf(MSG_ERROR,
               "Driver does not support configured VHT capability [%s]", name);
    return 0;
  }
  return 1;
}
...
static int ieee80211ac_cap_check_max(u32 hw, u32 conf, u32 mask,
                                     unsigned int shift, const char *name) {
  u32 hw_max = hw & mask;
  u32 conf_val = conf & mask;

  if (conf_val > hw_max) {
    wpa_printf(MSG_ERROR,
               "Configured VHT capability [%s] exceeds max value supported by "
               "the driver (%d > %d)",
...
static int ieee80211ac_supported_vht_capab(struct hostapd_iface *iface) {
  ...
  wpa_printf(MSG_DEBUG, "hw vht capab: 0x%x, conf vht capab: 0x%x", hw, conf);

  if (mode->mode == HOSTAPD_MODE_IEEE80211G &&
      iface->conf->bss[0]->vendor_vht && mode->vht_capab == 0 &&
      iface->hw_features) {
    int i;

    for (i = 0; i < iface->num_hw_features; i++) {
      if (iface->hw_features[i].mode == HOSTAPD_MODE_IEEE80211A) {
        mode = &iface->hw_features[i];
        hw = mode->vht_capab;
        wpa_printf(MSG_DEBUG, "update hw vht capab based on 5 GHz band: 0x%x",
                   hw);
        break;
      }
    }
...
#define VHT_CAP_CHECK_MAX(cap)                                                 \
  do {                                                                         \
    if (!ieee80211ac_cap_check_max(hw, conf, cap, cap##_SHIFT, #cap))          \
      return 0;                                                                \
  } while (0)
...
static int hostapd_is_usable_chan(struct hostapd_iface *iface, int channel,
                                  int primary) {
  int i;
  ...
  for (i = 0; i < iface->current_mode->num_channels; i++) {
    ...
    wpa_printf(
        ...
        primary ? "" : "Configured HT40 secondary ", i, chan->chan, chan->flag,
...
static int hostapd_is_usable_chans(struct hostapd_iface *iface) {
  ...
  return hostapd_is_usable_chan(
      iface, iface->conf->channel + iface->conf->secondary_channel * 4, 0);
}
...
static void hostapd_notify_bad_chans(struct hostapd_iface *iface) {
  ...
  hostapd_logger(iface->bss[0], NULL, HOSTAPD_MODULE_IEEE80211,
                 ...
                 iface->conf->channel, iface->current_mode->mode,
...
int hostapd_select_hw_mode(struct hostapd_iface *iface) {
  ...
  if (iface->current_mode == NULL) {
    if (!(iface->drv_flags & WPA_DRIVER_FLAGS_ACS_OFFLOAD) ||
      ...
      hostapd_logger(iface->bss[0], NULL, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_WARNING,
                     "Hardware does not support configured mode (%d) (hw_mode "
                     "in hostapd.conf)",
