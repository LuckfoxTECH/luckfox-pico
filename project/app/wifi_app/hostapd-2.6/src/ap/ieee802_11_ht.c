  ...
  *pos++ = sizeof(*cap);
  ...
  if (hapd->iconf->obss_interval) {
    ...
    *pos++ = sizeof(*scan_params);
    ...
    scan_params->scan_passive_dwell = host_to_le16(20);
    scan_params->scan_active_dwell = host_to_le16(10);
    scan_params->scan_passive_total_per_channel = host_to_le16(200);
    scan_params->scan_active_total_per_channel = host_to_le16(20);
    scan_params->channel_transition_delay_factor = host_to_le16(5);
    scan_params->scan_activity_threshold = host_to_le16(25);
...
u8 *hostapd_eid_ht_operation(struct hostapd_data *hapd, u8 *eid) {
  ...
  *pos++ = sizeof(*oper);
  ...
  if (hapd->iconf->secondary_channel == 1)
    oper->ht_param |=
        HT_INFO_HT_PARAM_SECONDARY_CHNL_ABOVE | HT_INFO_HT_PARAM_STA_CHNL_WIDTH;
  if (hapd->iconf->secondary_channel == -1)
    oper->ht_param |=
        HT_INFO_HT_PARAM_SECONDARY_CHNL_BELOW | HT_INFO_HT_PARAM_STA_CHNL_WIDTH;
...
u8 *hostapd_eid_secondary_channel(struct hostapd_data *hapd, u8 *eid) {
  ...
  if (!hapd->cs_freq_params.channel || !hapd->cs_freq_params.sec_channel_offset)
    return eid;
...
int hostapd_ht_operation_update(struct hostapd_iface *iface) {
  ...
  wpa_printf(MSG_DEBUG, "%s current operation mode=0x%X", __func__,
             iface->ht_op_mode);
  ...
  if (!(iface->ht_op_mode & HT_OPER_OP_MODE_OBSS_NON_HT_STAS_PRESENT) &&
    ...
    op_mode_changes++;
  } else if ((iface->ht_op_mode & HT_OPER_OP_MODE_OBSS_NON_HT_STAS_PRESENT) &&
  ...
  wpa_printf(MSG_DEBUG, "%s new operation mode=0x%X changes=%d", __func__,
             iface->ht_op_mode, op_mode_changes);
...
void hostapd_2040_coex_action(struct hostapd_data *hapd,
  ...
  hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                 "hostapd_public_action - action=%d",
  ...
  if (bc_ie->coex_param & WLAN_20_40_BSS_COEX_20MHZ_WIDTH_REQ) {
    hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG,
                   "20 MHz BSS width request bit is set in BSS coexistence "
                   "information field");
    is_ht40_allowed = 0;
  }
  ...
  if (start + len - data >= 3 && data[0] == WLAN_EID_20_40_BSS_INTOLERANT &&
    ...
    for (i = 0; i < ielen - 1; i++) {
      ...
      hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_DEBUG,
                     "20_40_INTOLERANT channel %d reported", chan);
      is_ht40_allowed = 0;
    }
  }
  ...
  if (!is_ht40_allowed && (iface->drv_flags & WPA_DRIVER_FLAGS_HT_2040_COEX)) {
    if (iface->conf->secondary_channel) {
      hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_INFO, "Switching to 20 MHz operation");
      iface->conf->secondary_channel = 0;
      ieee802_11_set_beacons(iface);
    }
    if (!iface->num_sta_ht40_intolerant && iface->conf->obss_interval) {
      unsigned int delay_time;
      delay_time =
          OVERLAPPING_BSS_TRANS_DELAY_FACTOR * iface->conf->obss_interval;
      eloop_cancel_timeout(ap_ht2040_timeout, hapd->iface, NULL);
      eloop_register_timeout(delay_time, 0, ap_ht2040_timeout, hapd->iface,
                             NULL);
      wpa_printf(MSG_DEBUG,
...
u16 copy_sta_ht_capab(struct hostapd_data *hapd, struct sta_info *sta,
   ...
   * frame.
   */
  if (!ht_capab || !(sta->flags & WLAN_STA_WMM) || hapd->conf->disable_11n) {
...
void ht40_intolerant_add(struct hostapd_iface *iface, struct sta_info *sta) {
  ...
  wpa_printf(MSG_INFO,
             "HT: Forty MHz Intolerant is set by STA " MACSTR
...
void ht40_intolerant_remove(struct hostapd_iface *iface, struct sta_info *sta) {
  ...
  if (iface->num_sta_ht40_intolerant == 0 &&
    ...
    unsigned int delay_time =
        OVERLAPPING_BSS_TRANS_DELAY_FACTOR * iface->conf->obss_interval;
    wpa_printf(MSG_DEBUG, "HT: Start 20->40 MHz transition timer (%d seconds)",
               delay_time);
    eloop_cancel_timeout(ap_ht2040_timeout, iface, NULL);
...
static void update_sta_ht(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  wpa_printf(MSG_DEBUG,
             "HT: STA " MACSTR " HT Capabilities Info: "
             "0x%04x",
             MAC2STR(sta->addr), ht_capab);
  if ((ht_capab & HT_CAP_INFO_GREEN_FIELD) == 0) {
    ...
    wpa_printf(MSG_DEBUG,
               "%s STA " MACSTR " - no greenfield, num "
               "of non-gf stations %d",
               __func__, MAC2STR(sta->addr), hapd->iface->num_sta_ht_no_gf);
  }
  if ((ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) == 0) {
    ...
    wpa_printf(MSG_DEBUG,
               "%s STA " MACSTR " - 20 MHz HT, num of "
               "20MHz HT STAs %d",
               __func__, MAC2STR(sta->addr), hapd->iface->num_sta_ht_20mhz);
  }
...
static void update_sta_no_ht(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  if (hapd->iconf->ieee80211n) {
    wpa_printf(MSG_DEBUG,
               "%s STA " MACSTR " - no HT, num of "
               "non-HT stations %d",
               __func__, MAC2STR(sta->addr), hapd->iface->num_sta_no_ht);
  }
}
