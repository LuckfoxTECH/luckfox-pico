...
int vlan_add(const char *if_name, int vid, const char *vlan_if_name) {
  ...
  wpa_printf(MSG_DEBUG, "VLAN: vlan_add(if_name=%s, vid=%d)", if_name, vid);
  ifconfig_up(if_name);
  ...
  os_snprintf(if_request.device1, sizeof(if_request.device1), "vlan%d", vid);

  if_request.cmd = GET_VLAN_VID_CMD;

  if (ioctl(fd, SIOCSIFVLAN, &if_request) == 0 && if_request.u.VID == vid) {
    ...
    if (ioctl(fd, SIOCSIFVLAN, &if_request) == 0 &&
      ...
      close(fd);
      wpa_printf(MSG_DEBUG, "VLAN: vlan_add: if_name %s exists already",
                 if_request.device1);
      return 1;
    }
  }
  ...
  if (ioctl(fd, SIOCSIFVLAN, &if_request) < 0) {
    wpa_printf(MSG_ERROR, "VLAN: %s: ADD_VLAN_CMD failed for %s: %s", __func__,
               if_request.device1, strerror(errno));
    close(fd);
    return -1;
  }
...
int vlan_set_name_type(unsigned int name_type) {
  ...
  wpa_printf(MSG_DEBUG, "VLAN: vlan_set_name_type(name_type=%u)", name_type);
  if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
    wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) failed: %s",
               __func__, strerror(errno));
    return -1;
  }
