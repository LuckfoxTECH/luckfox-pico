...
int allowed_ht40_channel_pair(struct hostapd_hw_modes *mode, int pri_chan,
                              int sec_chan) {
  int ok, j, first;
  int allowed[] = {36,  44,  52,  60,  100, 108, 116,
                   124, 132, 140, 149, 157, 184, 192};
  size_t k;

  if (pri_chan == sec_chan || !sec_chan)
    return 1; /* HT40 not used */

  wpa_printf(MSG_DEBUG, "HT40: control channel: %d  secondary channel: %d",
             pri_chan, sec_chan);
  ...
  if (!ok) {
    wpa_printf(MSG_ERROR, "HT40 channel pair (%d, %d) not allowed", pri_chan,
               sec_chan);
    return 0;
  }
...
int check_40mhz_5g(struct hostapd_hw_modes *mode,
  ...
  if (sec_bss && !pri_bss) {
    wpa_printf(MSG_INFO, "Switch own primary and secondary channel to get "
                         "secondary channel with no Beacons from other BSSes");
    return 2;
  }
...
static int check_20mhz_bss(struct wpa_scan_res *bss, int pri_freq, int start,
  ...
  if (!elems.ht_capabilities) {
    wpa_printf(MSG_DEBUG, "Found overlapping legacy BSS: " MACSTR " freq=%d",
  ...
  if (elems.ht_operation) {
    ...
    wpa_printf(MSG_DEBUG, "Found overlapping 20 MHz HT BSS: " MACSTR " freq=%d",
               MAC2STR(bss->bssid), bss->freq);
    return 1;
  }
  return 0;
...
int check_40mhz_2g4(struct hostapd_hw_modes *mode,
  ...
  for (i = 0; i < scan_res->num; i++) {
    ...
    wpa_printf(MSG_DEBUG, "Neighboring BSS: " MACSTR " freq=%d pri=%d sec=%d",
               MAC2STR(bss->bssid), bss->freq, pri_chan, sec_chan);

    if (sec_chan) {
      if (pri_freq != pri || sec_freq != sec) {
        wpa_printf(MSG_DEBUG,
                   "40 MHz pri/sec mismatch with BSS " MACSTR
                   " <%d,%d> (chan=%d%c) vs. <%d,%d>",
                   MAC2STR(bss->bssid), pri, sec, pri_chan,
                   sec > pri ? '+' : '-', pri_freq, sec_freq);
        return 0;
      }
    }
    ...
    if (elems.ht_capabilities) {
      ...
      if (le_to_host16(ht_cap->ht_capabilities_info) &
        ...
        wpa_printf(MSG_DEBUG,
                   "40 MHz Intolerant is set on channel %d in BSS " MACSTR, pri,
                   MAC2STR(bss->bssid));
        return 0;
      }
    }
  ...
  if (data->vht_enabled)
    ...
    case VHT_CHANWIDTH_80MHZ:
      ...
      if ((vht_oper_chwidth == 1 && center_segment1) ||
          (vht_oper_chwidth == 3 && !center_segment1) || !sec_channel_offset)
        return -1;
      if (!center_segment0) {
        if (channel <= 48)
          center_segment0 = 42;
         ...
         * configuration.
         */
        if (center_segment0 == channel + 6 || center_segment0 == channel + 2 ||
            center_segment0 == channel - 2 || center_segment0 == channel - 6)
          data->center_freq1 = 5000 + center_segment0 * 5;
        else
          return -1;
      }
    ...
    case VHT_CHANWIDTH_160MHZ:
       ...
       */
      if (center_segment0 == channel + 14 || center_segment0 == channel + 10 ||
          center_segment0 == channel + 6 || center_segment0 == channel + 2 ||
          center_segment0 == channel - 2 || center_segment0 == channel - 6 ||
          center_segment0 == channel - 10 || center_segment0 == channel - 14)
        data->center_freq1 = 5000 + center_segment0 * 5;
      else
        return -1;
      break;
