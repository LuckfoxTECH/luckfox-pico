...
static const struct ieee802_11_parse_test_data parse_tests[] = {
    ...
    {(u8 *)"\xdd\x04\x00\x90\x4c\xff\xdd\x04\x00\x90\x4c\x33", 12, ParseUnknown,
     2},
    ...
    {(u8 *)"\x65\x12\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d"
           "\x0e\x0f\x10\x11",
     20, ParseOK, 1},
    {(u8 *)"\x6e\x00", 2, ParseOK, 1},
    {(u8 *)"\xc7\x00", 2, ParseOK, 1},
    {(u8 *)"\xc7\x01\x00", 3, ParseOK, 1},
    {(u8 *)"\x03\x00\x2a\x00\x36\x00\x37\x00\x38\x00\x2d\x00\x3d\x00\xbf\x00"
           "\xc0\x00",
     18, ParseOK, 9},
...
static int ieee802_11_parse_tests(void) {
  ...
  for (i = 0; parse_tests[i].data; i++) {
    ...
    if (res != test->result ||
      ...
      wpa_printf(MSG_ERROR, "ieee802_11_parse test %d failed", i);
      ret = -1;
    }
  }

  if (ieee802_11_vendor_ie_concat((const u8 *)"\x00\x01", 2, 0) != NULL) {
    wpa_printf(MSG_ERROR, "ieee802_11_vendor_ie_concat test failed");
    ret = -1;
  }
...
static const struct rsn_ie_parse_test_data rsn_parse_tests[] = {
    ...
    {(u8 *)"\x30\x0c\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04", 14, 0},
    {(u8 *)"\x30\x0c\x01\x00\x00\x0f\xac\x04\x00\x01\x00\x0f\xac\x04", 14, -4},
    {(u8 *)"\x30\x0c\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x06", 14, -1},
    {(u8 *)"\x30\x10\x01\x00\x00\x0f\xac\x04\x02\x00\x00\x0f\xac\x04\x00\x0f"
           "\xac\x08",
     18, 0},
    {(u8 *)"\x30\x0d\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x00", 15,
     -7},
    ...
    {(u8 *)"\x30\x0e\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x00\x01",
     16, -6},
    {(u8 *)"\x30\x12\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01",
     20, 0},
    {(u8 *)"\x30\x16\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x02\x00"
           "\x00\x0f\xac\x01\x00\x0f\xac\x02",
     24, 0},
    {(u8 *)"\x30\x13\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00",
     21, 0},
    {(u8 *)"\x30\x14\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00",
     22, 0},
    {(u8 *)"\x30\x16\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00\x00\x00",
     24, 0},
    {(u8 *)"\x30\x16\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00\x00\x01",
     24, -9},
    {(u8 *)"\x30\x1a\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00\x00\x00\x00\x00\x00\x00",
     28, -10},
    {(u8 *)"\x30\x1a\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00\x00\x00\x00\x0f\xac\x06",
     28, 0},
    {(u8 *)"\x30\x1c\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00"
           "\x00\x0f\xac\x01\x00\x00\x00\x00\x00\x0f\xac\x06\x01\x02",
     30, 0},
    {NULL, 0, 0}};
...
static int rsn_ie_parse_tests(void) {
  ...
  for (i = 0; rsn_parse_tests[i].data; i++) {
    ...
    if (wpa_parse_wpa_ie_rsn(test->data, test->len, &data) != test->result) {
...
int common_module_tests(void) {
  ...
  if (ieee802_11_parse_tests() < 0 || gas_tests() < 0 ||
