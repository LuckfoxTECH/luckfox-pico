...
static int ieee802_11_parse_vendor_specific(const u8 *pos, size_t elen,
  ...
  case OUI_BROADCOM:
    ...
    case VENDOR_VHT_TYPE:
      if (elen > 4 &&
          (pos[4] == VENDOR_VHT_SUBTYPE || pos[4] == VENDOR_VHT_SUBTYPE2)) {
...
enum hostapd_hw_mode ieee80211_freq_to_chan(int freq, u8 *channel) {
  ...
  return ieee80211_freq_to_channel_ext(freq, 0, VHT_CHANWIDTH_USE_HT, &op_class,
                                       channel);
}
...
static const char *const us_op_class_cc[] = {"US", "CA", NULL};
...
static const char *const jp_op_class_cc[] = {"JP", NULL};

static const char *const cn_op_class_cc[] = {"CN", NULL};
...
int supp_rates_11b_only(struct ieee802_11_elems *elems) {
  ...
  for (i = 0; elems->ext_supp_rates && i < elems->ext_supp_rates_len; i++) {
...
int mb_ies_info_by_ies(struct mb_ies_info *info, const u8 *ies_buf,
  ...
  while (ies_buf && ies_len >= 2 && info->nof_ies < MAX_NOF_MB_IES_SUPPORTED) {
  ...
  if (info->nof_ies) {
    ...
    if (mb_ies) {
      for (i = 0; i < info->nof_ies; i++) {
        ...
        wpabuf_put_data(mb_ies, info->ies[i].ie, info->ies[i].ie_len);
      }
    }
  }
...
size_t mbo_add_ie(u8 *buf, size_t len, const u8 *attr, size_t attr_len) {
  ...
  if (len < 6 + attr_len) {
    wpa_printf(MSG_DEBUG,
               "MBO: Not enough room in buffer for MBO IE: buf len = %zu, "
               "attr_len = %zu",
               len, attr_len);
    return 0;
  }
