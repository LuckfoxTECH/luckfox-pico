...
try_again:
  if (cli_path && cli_path[0] == '/') {
    ret = os_snprintf(ctrl->local.sun_path, sizeof(ctrl->local.sun_path),
                      "%s/" CONFIG_CTRL_IFACE_CLIENT_PREFIX "%d-%d", cli_path,
                      (int)getpid(), counter);
  } else {
    ret = os_snprintf(ctrl->local.sun_path, sizeof(ctrl->local.sun_path),
  ...
  if (os_strncmp(ctrl_path, "@android:", 9) == 0) {
    if (socket_local_client_connect(ctrl->s, ctrl_path + 9,
  ...
  if (*ctrl_path != '/') {
    ...
    if (socket_local_client_connect(
            ctrl->s, buf, ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_DGRAM) < 0) {
      close(ctrl->s);
      unlink(ctrl->local.sun_path);
      os_free(ctrl);
      return NULL;
  ...
  if (os_strncmp(ctrl_path, "@abstract:", 10) == 0) {
    ...
    res =
        os_strlcpy(ctrl->dest.sun_path, ctrl_path, sizeof(ctrl->dest.sun_path));
  ...
  if (connect(ctrl->s, (struct sockaddr *)&ctrl->dest, sizeof(ctrl->dest)) <
      0) {
    close(ctrl->s);
    unlink(ctrl->local.sun_path);
    os_free(ctrl);
...
struct wpa_ctrl *wpa_ctrl_open(const char *ctrl_path) {
  ...
  if (connect(ctrl->s, (struct sockaddr *)&ctrl->dest, sizeof(ctrl->dest)) <
    ...
    wpa_printf(
        MSG_ERROR, "connect(%s:%d) failed: %s",
        inet_ntop(AF_INET6, &ctrl->dest.sin6_addr, addr, sizeof(ctrl->dest)),
        ntohs(ctrl->dest.sin6_port), strerror(errno));
#else  /* CONFIG_CTRL_IFACE_UDP_IPV6 */
    wpa_printf(MSG_ERROR, "connect(%s:%d) failed: %s",
               inet_ntoa(ctrl->dest.sin_addr), ntohs(ctrl->dest.sin_port),
               strerror(errno));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
    close(ctrl->s);
    os_free(ctrl->remote_ip);
...
#define WPA_CTRL_MAX_PS_NAME 100
  ...
  os_snprintf(ps, WPA_CTRL_MAX_PS_NAME, "%s/%s", ctrl->remote_ip,
              ctrl->remote_ifname);
  return ps;
}
...
static int wpa_ctrl_attach_helper(struct wpa_ctrl *ctrl, int attach) {
  ...
  ret =
      wpa_ctrl_request(ctrl, attach ? "ATTACH" : "DETACH", 6, buf, &len, NULL);
  if (ret < 0)
    return ret;
...
int wpa_ctrl_get_fd(struct wpa_ctrl *ctrl) { return ctrl->s; }
...
struct wpa_ctrl *wpa_ctrl_open(const char *ctrl_path) {
  ...
  for (i = 0; i < 10; i++) {
    ctrl->pipe = CreateFile(name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
int wpa_ctrl_get_fd(struct wpa_ctrl *ctrl) { return -1; }
