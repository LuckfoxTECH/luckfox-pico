...
int wpa_pmk_to_ptk(const u8 *pmk, size_t pmk_len, const char *label,
                   ...
                   const u8 *nonce2, struct wpa_ptk *ptk, int akmp,
                   int cipher) {
  ...
  if (wpa_key_mgmt_sha384(akmp))
    sha384_prf(pmk, pmk_len, label, data, sizeof(data), tmp, ptk_len);
  else
#endif /* CONFIG_SUITEB192 */
...
int wpa_ft_mic(const u8 *kck, size_t kck_len, const u8 *sta_addr,
               const u8 *ap_addr, u8 transaction_seqnum, const u8 *mdie,
               size_t mdie_len, const u8 *ftie, size_t ftie_len,
               const u8 *rsnie, size_t rsnie_len, const u8 *ric, size_t ric_len,
               u8 *mic) {
  const u8 *addr[9];
  size_t len[9];
  size_t i, num_elem = 0;
  ...
  while (end - pos >= 2) {
    ...
    case FTIE_SUBELEM_R1KH_ID:
      if (len != FT_R1KH_ID_LEN) {
        wpa_printf(MSG_DEBUG, "FT: Invalid R1KH-ID length in FTIE: %d", len);
        return -1;
      }
      parse->r1kh_id = pos;
      break;
    ...
    case FTIE_SUBELEM_R0KH_ID:
      if (len < 1 || len > FT_R0KH_ID_MAX_LEN) {
        wpa_printf(MSG_DEBUG, "FT: Invalid R0KH-ID length in FTIE: %d", len);
        return -1;
      }
      parse->r0kh_id = pos;
...
int wpa_ft_parse_ies(const u8 *ies, size_t ies_len, struct wpa_ft_ies *parse) {
  ...
  while (end - pos >= 2) {
    ...
    case WLAN_EID_RSN:
      parse->rsn = pos;
      parse->rsn_len = len;
      ret = wpa_parse_wpa_ie_rsn(parse->rsn - 2, parse->rsn_len + 2, &data);
      if (ret < 0) {
  ...
  if (parse->ric) {
    pos = parse->ric;
    while (end - pos >= 2 && 2 + pos[1] <= end - pos && prot_ie_count) {
      prot_ie_count--;
      pos += 2 + pos[1];
    }
...
int wpa_cipher_valid_group(int cipher) {
  return wpa_cipher_valid_pairwise(cipher) || cipher == WPA_CIPHER_GTK_NOT_USED;
}
...
int wpa_cipher_valid_mgmt_group(int cipher) {
  return cipher == WPA_CIPHER_AES_128_CMAC ||
         ...
         cipher == WPA_CIPHER_BIP_GMAC_256 || cipher == WPA_CIPHER_BIP_CMAC_256;
}
#endif /* CONFIG_IEEE80211W */
...
int wpa_parse_wpa_ie_rsn(const u8 *rsn_ie, size_t rsn_ie_len,
  ...
  if (left >= RSN_SELECTOR_LEN) {
    ...
    if (!wpa_cipher_valid_group(data->group_cipher)) {
      wpa_printf(MSG_DEBUG, "%s: invalid group cipher 0x%x (%08x)", __func__,
                 data->group_cipher, WPA_GET_BE32(pos));
      return -1;
    }
    pos += RSN_SELECTOR_LEN;
    left -= RSN_SELECTOR_LEN;
  } else if (left > 0) {
    wpa_printf(MSG_DEBUG, "%s: ie length mismatch, %u too much", __func__,
               left);
    return -3;
  }
  ...
  if (left >= 4) {
    ...
    if (!wpa_cipher_valid_mgmt_group(data->mgmt_group_cipher)) {
      wpa_printf(MSG_DEBUG,
                 ...
                 __func__, data->mgmt_group_cipher, WPA_GET_BE32(pos));
      return -10;
    }
    pos += RSN_SELECTOR_LEN;
    left -= RSN_SELECTOR_LEN;
  ...
  if (left > 0) {
    wpa_hexdump(MSG_DEBUG, "wpa_parse_wpa_ie_rsn: ignore trailing bytes", pos,
                left);
  }
...
int wpa_parse_wpa_ie_wpa(const u8 *wpa_ie, size_t wpa_ie_len,
  ...
  if (hdr->elem_id != WLAN_EID_VENDOR_SPECIFIC || hdr->len != wpa_ie_len - 2 ||
  ...
  if (left > 0) {
    wpa_hexdump(MSG_DEBUG, "wpa_parse_wpa_ie_wpa: ignore trailing bytes", pos,
                left);
  }
 ...
 */
void wpa_derive_pmk_r0(const u8 *xxkey, size_t xxkey_len, const u8 *ssid,
                       size_t ssid_len, const u8 *mdid, const u8 *r0kh_id,
                       size_t r0kh_id_len, const u8 *s0kh_id, u8 *pmk_r0,
                       u8 *pmk_r0_name) {
  u8 buf[1 + SSID_MAX_LEN + MOBILITY_DOMAIN_ID_LEN + 1 + FT_R0KH_ID_MAX_LEN +
         ETH_ALEN];
  ...
  *pos++ = r0kh_id_len;
  ...
  sha256_prf(xxkey, xxkey_len, "FT-R0", buf, pos - buf, r0_key_data,
             sizeof(r0_key_data));
...
void wpa_derive_pmk_r1(const u8 *pmk_r0, const u8 *pmk_r0_name,
                       const u8 *r1kh_id, const u8 *s1kh_id, u8 *pmk_r1,
                       u8 *pmk_r1_name) {
...
int wpa_pmk_r1_to_ptk(const u8 *pmk_r1, const u8 *snonce, const u8 *anonce,
                      ...
                      const u8 *pmk_r1_name, struct wpa_ptk *ptk, u8 *ptk_name,
                      int akmp, int cipher) {
...
int wpa_insert_pmkid(u8 *ies, size_t *ies_len, const u8 *pmkid) {
  ...
  wpa_hexdump(MSG_DEBUG, "FT: RSN IE before modification", start, 2 + start[1]);
  ...
  if (rpos == rend) {
    ...
    if (num_pmkid != 0) {
       ...
       */
      wpa_printf(MSG_DEBUG, "FT: Remove %u old PMKID(s) from RSN IE",
                 num_pmkid);
...
int wpa_write_ciphers(char *start, char *end, int ciphers, const char *delim) {
  ...
  if (ciphers & WPA_CIPHER_CCMP_256) {
    ret = os_snprintf(pos, end - pos, "%sCCMP-256", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
  if (ciphers & WPA_CIPHER_GCMP_256) {
    ret = os_snprintf(pos, end - pos, "%sGCMP-256", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
  if (ciphers & WPA_CIPHER_CCMP) {
    ret = os_snprintf(pos, end - pos, "%sCCMP", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
  if (ciphers & WPA_CIPHER_GCMP) {
    ret = os_snprintf(pos, end - pos, "%sGCMP", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
  if (ciphers & WPA_CIPHER_TKIP) {
    ret = os_snprintf(pos, end - pos, "%sTKIP", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
  if (ciphers & WPA_CIPHER_NONE) {
    ret = os_snprintf(pos, end - pos, "%sNONE", pos == start ? "" : delim);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
...
int wpa_select_ap_group_cipher(int wpa, int wpa_pairwise, int rsn_pairwise) {
  ...
  if ((pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) == WPA_CIPHER_GCMP)
    return WPA_CIPHER_GCMP;
  if ((pairwise & (WPA_CIPHER_GCMP_256 | WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) ==
      WPA_CIPHER_GCMP_256)
    return WPA_CIPHER_GCMP_256;
  if ((pairwise & (WPA_CIPHER_CCMP_256 | WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) ==
      WPA_CIPHER_CCMP_256)
    return WPA_CIPHER_CCMP_256;
  return WPA_CIPHER_CCMP;
}
