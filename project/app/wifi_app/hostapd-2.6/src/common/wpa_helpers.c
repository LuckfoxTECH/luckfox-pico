...
int wpa_command_resp(const char *ifname, const char *cmd, char *resp,
                     size_t resp_size) {
  struct wpa_ctrl *ctrl;
  size_t len;
...
int get_wpa_cli_event2(struct wpa_ctrl *mon, const char *event,
                       const char *event2, char *buf, size_t buf_size) {
  int fd, ret;
  fd_set rfd;
  char *pos;
  struct timeval tv;
  ...
  while (1) {
    ...
    pos = strchr(buf, '>');
    if (pos && (strncmp(pos + 1, event, strlen(event)) == 0 ||
                (event2 && strncmp(pos + 1, event2, strlen(event2)) == 0)))
...
int get_wpa_cli_event(struct wpa_ctrl *mon, const char *event, char *buf,
                      size_t buf_size) {
...
int get_wpa_status(const char *ifname, const char *field, char *obuf,
  ...
  if (ctrl == NULL)
    return -1;
  len = sizeof(buf);
  if (wpa_ctrl_request(ctrl, "STATUS-NO_EVENTS", 16, buf, &len, NULL) < 0) {
    wpa_ctrl_close(ctrl);
    return -1;
  }
  wpa_ctrl_close(ctrl);
