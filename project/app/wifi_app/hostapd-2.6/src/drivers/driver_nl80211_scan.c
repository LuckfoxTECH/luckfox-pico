...
static int
nl80211_get_noise_for_scan_results(struct wpa_driver_nl80211_data *drv,
                                   struct wpa_scan_results *scan_res) {
  struct nl_msg *msg;

  msg = nl80211_drv_msg(drv, NLM_F_DUMP, NL80211_CMD_GET_SURVEY);
  return send_and_recv_msgs(drv, msg, get_noise_for_scan_results, scan_res);
}
...
nl80211_scan_common(struct i802_bss *bss, u8 cmd,
  ...
  if (params->num_ssids) {
    ...
    for (i = 0; i < params->num_ssids; i++) {
      ...
      if (nla_put(msg, i + 1, params->ssids[i].ssid_len, params->ssids[i].ssid))
        goto fail;
    }
  ...
  if (params->extra_ies) {
    wpa_hexdump(MSG_MSGDUMP, "nl80211: Scan extra IEs", params->extra_ies,
                params->extra_ies_len);
    if (nla_put(msg, NL80211_ATTR_IE, params->extra_ies_len, params->extra_ies))
      goto fail;
  }
  ...
  if (params->mac_addr_rand) {
    wpa_printf(MSG_DEBUG, "nl80211: Add NL80211_SCAN_FLAG_RANDOM_ADDR");
    scan_flags |= NL80211_SCAN_FLAG_RANDOM_ADDR;

    if (params->mac_addr) {
      wpa_printf(MSG_DEBUG, "nl80211: MAC address: " MACSTR,
                 MAC2STR(params->mac_addr));
      if (nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, params->mac_addr))
        goto fail;
    }

    if (params->mac_addr_mask) {
      wpa_printf(MSG_DEBUG, "nl80211: MAC address mask: " MACSTR,
                 MAC2STR(params->mac_addr_mask));
      if (nla_put(msg, NL80211_ATTR_MAC_MASK, ETH_ALEN, params->mac_addr_mask))
        goto fail;
    }
  }

  if (scan_flags && nla_put_u32(msg, NL80211_ATTR_SCAN_FLAGS, scan_flags))
    goto fail;
...
int wpa_driver_nl80211_scan(struct i802_bss *bss,
  ...
  if (params->p2p_probe) {
     ...
     */
    if (nla_put(msg, NL80211_BAND_2GHZ, 8, "\x0c\x12\x18\x24\x30\x48\x60\x6c"))
      goto fail;
  ...
  if (params->bssid) {
    wpa_printf(MSG_DEBUG, "nl80211: Scan for a specific BSSID: " MACSTR,
  ...
  if (ret) {
    ...
    if (drv->hostapd && is_ap_interface(drv->nlmode)) {
       ...
       */
      if (wpa_driver_nl80211_set_mode(bss, NL80211_IFTYPE_STATION))
        goto fail;
...
nl80211_sched_scan_add_scan_plans(struct wpa_driver_nl80211_data *drv,
  ...
  for (i = 0; i < params->sched_scan_plans_num; i++) {
    ...
    if (!scan_plans[i].interval ||
        scan_plans[i].interval > drv->capa.max_sched_scan_plan_interval) {
      wpa_printf(MSG_DEBUG,
                 "nl80211: sched scan plan no. %u: Invalid interval: %u", i,
    ...
    if (scan_plans[i].iterations > drv->capa.max_sched_scan_plan_iterations) {
      wpa_printf(
          MSG_DEBUG,
  ...
  if (i != params->sched_scan_plans_num - 1) {
    wpa_printf(MSG_DEBUG, "nl80211: All sched scan plans but the last must "
...
int wpa_driver_nl80211_sched_scan(void *priv,
  ...
  if (!params->sched_scan_plans_num ||
    ...
    wpa_printf(MSG_ERROR, "nl80211: Invalid number of sched scan plans: %u",
  ...
  if ((drv->num_filter_ssids &&
    ...
    for (i = 0; i < drv->num_filter_ssids; i++) {
      ...
      wpa_hexdump_ascii(MSG_MSGDUMP, "nl80211: Sched scan filter SSID",
      ...
      if (match_set_ssid == NULL ||
          nla_put(msg, NL80211_ATTR_SCHED_SCAN_MATCH_SSID,
                  drv->filter_ssids[i].ssid_len, drv->filter_ssids[i].ssid) ||
          (params->filter_rssi &&
    ...
    if (params->filter_rssi) {
      ...
      if (match_set_rssi == NULL ||
          nla_put_u32(msg, NL80211_SCHED_SCAN_MATCH_ATTR_RSSI,
                      params->filter_rssi))
        goto fail;
      wpa_printf(MSG_MSGDUMP, "nl80211: Sched scan RSSI filter %d dBm",
                 params->filter_rssi);
...
int wpa_driver_nl80211_stop_sched_scan(void *priv) {
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Sched scan stop failed: ret=%d (%s)", ret,
               strerror(-ret));
  } else {
    wpa_printf(MSG_DEBUG, "nl80211: Sched scan stop sent");
  }
...
static int nl80211_scan_filtered(struct wpa_driver_nl80211_data *drv,
  ...
  for (i = 0; i < drv->num_filter_ssids; i++) {
    if (ssid[1] == drv->filter_ssids[i].ssid_len &&
        os_memcmp(ssid + 2, drv->filter_ssids[i].ssid, ssid[1]) == 0)
      return 0;
  }
...
int bss_info_handler(struct nl_msg *msg, void *arg) {
  ...
  if (bss[NL80211_BSS_STATUS]) {
    ...
    if (status == NL80211_BSS_STATUS_ASSOCIATED && bss[NL80211_BSS_FREQUENCY]) {
      _arg->assoc_freq = nla_get_u32(bss[NL80211_BSS_FREQUENCY]);
      wpa_printf(MSG_DEBUG, "nl80211: Associated on %u MHz", _arg->assoc_freq);
    }
    if (status == NL80211_BSS_STATUS_IBSS_JOINED &&
        bss[NL80211_BSS_FREQUENCY]) {
      _arg->ibss_freq = nla_get_u32(bss[NL80211_BSS_FREQUENCY]);
      wpa_printf(MSG_DEBUG, "nl80211: IBSS-joined on %u MHz", _arg->ibss_freq);
    }
    if (status == NL80211_BSS_STATUS_ASSOCIATED && bss[NL80211_BSS_BSSID]) {
      os_memcpy(_arg->assoc_bssid, nla_data(bss[NL80211_BSS_BSSID]), ETH_ALEN);
      wpa_printf(MSG_DEBUG, "nl80211: Associated with " MACSTR,
  ...
  if (bss[NL80211_BSS_BSSID])
    os_memcpy(r->bssid, nla_data(bss[NL80211_BSS_BSSID]), ETH_ALEN);
  ...
  for (i = 0; i < res->num; i++) {
    ...
    s1 = get_ie((u8 *)(res->res[i] + 1), res->res[i]->ie_len, WLAN_EID_SSID);
...
static void clear_state_mismatch(struct wpa_driver_nl80211_data *drv,
  ...
  if (drv->capa.flags & WPA_DRIVER_FLAGS_SME) {
    wpa_printf(MSG_DEBUG,
               ...
               "mismatch (" MACSTR ")",
               MAC2STR(addr));
    wpa_driver_nl80211_mlme(drv, addr, NL80211_CMD_DEAUTHENTICATE,
                            WLAN_REASON_PREV_AUTH_NOT_VALID, 1);
  }
}

static void
wpa_driver_nl80211_check_bss_status(struct wpa_driver_nl80211_data *drv,
  ...
  for (i = 0; i < res->num; i++) {
    ...
    if (r->flags & WPA_SCAN_ASSOCIATED) {
      wpa_printf(MSG_DEBUG,
                 ...
                 "indicate BSS status with " MACSTR " as associated",
                 MAC2STR(r->bssid));
...
void nl80211_dump_scan(struct wpa_driver_nl80211_data *drv) {
  ...
  for (i = 0; i < res->num; i++) {
    struct wpa_scan_res *r = res->res[i];
    wpa_printf(MSG_DEBUG, "nl80211: %d/%d " MACSTR "%s", (int)i, (int)res->num,
               MAC2STR(r->bssid),
...
int wpa_driver_nl80211_abort_scan(void *priv) {
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Abort scan failed: ret=%d (%s)", ret,
               strerror(-ret));
  }
...
static int scan_cookie_handler(struct nl_msg *msg, void *arg) {
  ...
  if (tb[NL80211_ATTR_VENDOR_DATA]) {
    ...
    nla_parse(tb_vendor, QCA_WLAN_VENDOR_ATTR_SCAN_MAX, nla_data(nl_vendor),
              nla_len(nl_vendor), NULL);

    if (tb_vendor[QCA_WLAN_VENDOR_ATTR_SCAN_COOKIE])
      *cookie = nla_get_u64(tb_vendor[QCA_WLAN_VENDOR_ATTR_SCAN_COOKIE]);
  }
...
int wpa_driver_nl80211_vendor_scan(struct i802_bss *bss,
  ...
  if (params->extra_ies) {
    wpa_hexdump(MSG_MSGDUMP, "nl80211: Scan extra IEs", params->extra_ies,
                params->extra_ies_len);
    if (nla_put(msg, QCA_WLAN_VENDOR_ATTR_SCAN_IE, params->extra_ies_len,
                params->extra_ies))
      goto fail;
  }
  ...
  if (params->freqs) {
    ...
    freqs = nla_nest_start(msg, QCA_WLAN_VENDOR_ATTR_SCAN_FREQUENCIES);
    if (freqs == NULL)
      goto fail;
    for (i = 0; params->freqs[i]; i++) {
      wpa_printf(MSG_MSGDUMP, "nl80211: Scan frequency %u MHz",
                 params->freqs[i]);
      if (nla_put_u32(msg, i + 1, params->freqs[i]))
        goto fail;
    }
  ...
  if (params->mac_addr_rand) {
    ...
    if (params->mac_addr) {
      wpa_printf(MSG_DEBUG, "nl80211: MAC address: " MACSTR,
                 MAC2STR(params->mac_addr));
      if (nla_put(msg, QCA_WLAN_VENDOR_ATTR_SCAN_MAC, ETH_ALEN,
                  params->mac_addr))
        goto fail;
    }
    ...
    if (params->mac_addr_mask) {
      wpa_printf(MSG_DEBUG, "nl80211: MAC address mask: " MACSTR,
                 MAC2STR(params->mac_addr_mask));
      if (nla_put(msg, QCA_WLAN_VENDOR_ATTR_SCAN_MAC_MASK, ETH_ALEN,
  ...
  if (params->p2p_probe) {
    ...
    rates = nla_nest_start(msg, QCA_WLAN_VENDOR_ATTR_SCAN_SUPP_RATES);
    if (rates == NULL)
      goto fail;
  ...
  msg = NULL;
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Vendor scan trigger failed: ret=%d (%s)",
               ret, strerror(-ret));
    goto fail;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Vendor scan requested (ret=%d) - scan timeout 30 "
             "seconds, scan cookie:0x%llx",
             ret, (long long unsigned int)cookie);
  eloop_cancel_timeout(wpa_driver_nl80211_scan_timeout, drv, drv->ctx);
  eloop_register_timeout(30, 0, wpa_driver_nl80211_scan_timeout, drv, drv->ctx);
...
int nl80211_set_default_scan_ies(void *priv, const u8 *ies, size_t ies_len) {
  ...
  if (nla_put(msg, QCA_WLAN_VENDOR_ATTR_CONFIG_SCAN_DEFAULT_IES, ies_len, ies))
    goto fail;
  ...
  msg = NULL;
  if (ret) {
    wpa_printf(MSG_ERROR, "nl80211: Set scan default IEs failed: ret=%d (%s)",
               ret, strerror(-ret));
    goto fail;
  }
