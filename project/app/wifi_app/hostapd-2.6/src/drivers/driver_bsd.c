...
static struct bsd_driver_data *bsd_get_drvindex(void *priv,
...
#ifndef HOSTAPD
static struct bsd_driver_data *bsd_get_drvname(void *priv, const char *ifname) {
  struct bsd_driver_global *global = priv;
  struct bsd_driver_data *drv;
...
static int bsd_set80211(void *priv, int op, int val, const void *arg,
                        int arg_len) {
  ...
  if (ioctl(drv->global->sock, SIOCS80211, &ireq) < 0) {
    wpa_printf(MSG_ERROR,
               ...
               "arg_len=%u]: %s",
               op, val, arg_len, strerror(errno));
    return -1;
  }
  return 0;
}

static int bsd_get80211(void *priv, struct ieee80211req *ireq, int op,
...
static int get80211var(struct bsd_driver_data *drv, int op, void *arg,
                       int arg_len) {
  struct ieee80211req ireq;
...
static int set80211var(struct bsd_driver_data *drv, int op, const void *arg,
                       int arg_len) {
...
static int bsd_get_if_media(void *priv) {
  ...
  if (ioctl(drv->global->sock, SIOCGIFMEDIA, &ifmr) < 0) {
    wpa_printf(MSG_ERROR, "%s: SIOCGIFMEDIA %s", __func__, strerror(errno));
    return -1;
  }
...
static int bsd_set_if_media(void *priv, int media) {
  ...
  if (ioctl(drv->global->sock, SIOCSIFMEDIA, &ifr) < 0) {
    wpa_printf(MSG_ERROR, "%s: SIOCSIFMEDIA %s", __func__, strerror(errno));
    return -1;
  }
...
static int bsd_send_mlme_param(void *priv, const u8 op, const u16 reason,
                               const u8 *addr) {
...
static int bsd_ctrl_iface(void *priv, int enable) {
  ...
  if (ioctl(drv->global->sock, SIOCGIFFLAGS, &ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGIFFLAGS]: %s", strerror(errno));
    return -1;
  }
  ...
  if (ioctl(drv->global->sock, SIOCSIFFLAGS, &ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIFFLAGS]: %s", strerror(errno));
    return -1;
  }
...
static int bsd_set_key(const char *ifname, void *priv, enum wpa_alg alg,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             __func__, alg, addr, key_idx, set_tx, seq_len, key_len);
   ...
   */
  if (drv->opmode == IEEE80211_M_IBSS || drv->opmode == IEEE80211_M_AHDEMO)
...
static int bsd_configure_wpa(void *priv, struct wpa_bss_params *params) {
  ...
  static const char *ciphernames[] = {"WEP",     "TKIP", "AES-OCB",
                                      "AES-CCM", "CKIP", "NONE"};
  int v;
  ...
  case WPA_CIPHER_NONE:
    ...
    break;
  default:
    wpa_printf(MSG_INFO, "Unknown group key cipher %u", params->wpa_group);
    return -1;
  }
  wpa_printf(MSG_DEBUG, "%s: group key cipher=%s (%u)", __func__,
             ciphernames[v], v);
  if (set80211param(priv, IEEE80211_IOC_MCASTCIPHER, v)) {
    wpa_printf(MSG_INFO, "Unable to set group key cipher to %u (%s)", v,
               ciphernames[v]);
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG, "%s: key management algorithms=0x%x", __func__,
             params->wpa_key_mgmt);
  if (set80211param(priv, IEEE80211_IOC_KEYMGTALGS, params->wpa_key_mgmt)) {
...
static int bsd_set_ieee8021x(void *priv, struct wpa_bss_params *params) {
  ...
  if (!params->enabled) {
    /* XXX restore state */
    return set80211param(priv, IEEE80211_IOC_AUTHMODE, IEEE80211_AUTH_AUTO);
  }
  if (!params->wpa && !params->ieee802_1x) {
    wpa_printf(MSG_ERROR, "%s: No 802.1X or WPA enabled", __func__);
    return -1;
  }
  if (params->wpa && bsd_configure_wpa(priv, params) != 0) {
    wpa_printf(MSG_ERROR, "%s: Failed to configure WPA state", __func__);
    return -1;
  }
  if (set80211param(
          priv, IEEE80211_IOC_AUTHMODE,
          (params->wpa ? IEEE80211_AUTH_WPA : IEEE80211_AUTH_8021X))) {
    wpa_printf(MSG_ERROR, "%s: Failed to enable WPA/802.1X", __func__);
    return -1;
  }
...
static void bsd_new_sta(void *priv, void *ctx, u8 addr[IEEE80211_ADDR_LEN]) {
  ...
  if (get80211var(priv, IEEE80211_IOC_WPAIE, &ie, sizeof(ie)) < 0) {
    wpa_printf(MSG_INFO, "Failed to get WPA/RSN information element");
    goto no_ie;
  }
  iebuf = ie.wpa_ie;
  ielen = ie.wpa_ie[1];
...
static int bsd_send_eapol(void *priv, const u8 *addr, const u8 *data,
  ...
  return l2_packet_send(drv->sock_xmit, addr, ETH_P_EAPOL, data, data_len);
}
...
static int bsd_set_freq(void *priv, struct hostapd_freq_params *freq) {
  ...
  if (bsd_set_mediaopt(drv, IFM_MMASK, mode) < 0) {
    wpa_printf(MSG_ERROR, "%s: failed to set modulation mode", __func__);
    return -1;
  }
...
static int bsd_set_opt_ie(void *priv, const u8 *ie, size_t ie_len) {
  ...
  return bsd_set80211(priv, IEEE80211_IOC_APPIE, IEEE80211_APPIE_WPA, ie,
                      ie_len);
#endif /* IEEE80211_IOC_APPIE */
  return 0;
}

static size_t rtbuf_len(void) {
  size_t len;

  int mib[6] = {CTL_NET, AF_ROUTE, 0, AF_INET, NET_RT_DUMP, 0};

  if (sysctl(mib, 6, NULL, &len, NULL, 0) < 0) {
    wpa_printf(MSG_WARNING, "%s failed: %s", __func__, strerror(errno));
    len = 2048;
  }
...
static int bsd_set_privacy(void *priv, int enabled) {
  wpa_printf(MSG_DEBUG, "%s: enabled=%d", __func__, enabled);

  return set80211param(priv, IEEE80211_IOC_PRIVACY, enabled);
}

static int bsd_get_seqnum(const char *ifname, void *priv, const u8 *addr,
                          int idx, u8 *seq) {
  struct ieee80211req_key wk;
...
static int bsd_flush(void *priv) {
...
static int bsd_read_sta_driver_data(void *priv,
                                    struct hostap_sta_driver_data *data,
                                    const u8 *addr) {
  struct ieee80211req_sta_stats stats;

  memcpy(stats.is_u.macaddr, addr, IEEE80211_ADDR_LEN);
  if (get80211var(priv, IEEE80211_IOC_STA_STATS, &stats, sizeof(stats)) > 0) {
...
static int bsd_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr,
                          int reason_code) {
  return bsd_send_mlme_param(priv, IEEE80211_MLME_DEAUTH, reason_code, addr);
}

static int bsd_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr,
                            int reason_code) {
  return bsd_send_mlme_param(priv, IEEE80211_MLME_DISASSOC, reason_code, addr);
}

static void bsd_wireless_event_receive(int sock, void *ctx, void *sock_ctx) {
  ...
  if (n < 0) {
    if (errno != EINTR && errno != EAGAIN)
      wpa_printf(MSG_ERROR, "%s read() failed: %s", __func__, strerror(errno));
    return;
  }
  ...
  case RTM_IEEE80211:
    ...
    case RTM_IEEE80211_MICHAEL:
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 "keyix=%u src_addr=" MACSTR,
                 mic->iev_keyix, MAC2STR(mic->iev_src));
...
static void *bsd_init(struct hostapd_data *hapd,
  ...
  if (drv->ifindex == 0) {
    wpa_printf(MSG_DEBUG, "%s: interface %s does not exist", __func__,
               params->ifname);
    goto bad;
  }
  ...
  drv->sock_xmit =
      l2_packet_init(drv->ifname, NULL, ETH_P_EAPOL, handle_read, drv, 0);
  ...
  if (bsd_set_mediaopt(drv, IFM_OMASK, IFM_IEEE80211_HOSTAP) < 0) {
    wpa_printf(MSG_ERROR, "%s: failed to set operation mode", __func__);
    goto bad;
  }
...
static void bsd_deinit(void *priv) {
  struct bsd_driver_data *drv = priv;
...
static int bsd_set_sta_authorized(void *priv, const u8 *addr,
  ...
  return bsd_send_mlme_param(
      priv, authorized ? IEEE80211_MLME_AUTHORIZE : IEEE80211_MLME_UNAUTHORIZE,
      0, addr);
}
#else /* HOSTAPD */
...
static int wpa_driver_bsd_get_bssid(void *priv, u8 *bssid) {
  ...
  return 0;
#else
  return get80211var(drv, IEEE80211_IOC_BSSID, bssid, IEEE80211_ADDR_LEN) < 0
             ? -1
             : 0;
#endif
}
...
static int wpa_driver_bsd_set_wpa_internal(void *priv, int wpa, int privacy) {
  ...
  wpa_printf(MSG_DEBUG, "%s: wpa=%d privacy=%d", __func__, wpa, privacy);
...
static int wpa_driver_bsd_deauthenticate(void *priv, const u8 *addr,
                                         int reason_code) {
  return bsd_send_mlme_param(priv, IEEE80211_MLME_DEAUTH, reason_code, addr);
}
...
static int
wpa_driver_bsd_associate(void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             __func__, (unsigned int)params->ssid_len, params->ssid,
             (unsigned int)params->wpa_ie_len, params->pairwise_suite,
  ...
  if (wpa_driver_bsd_set_drop_unencrypted(drv, params->drop_unencrypted) < 0)
    ret = -1;
  ...
  privacy =
      !(params->pairwise_suite == WPA_CIPHER_NONE &&
        ...
        params->key_mgmt_suite == WPA_KEY_MGMT_NONE && params->wpa_ie_len == 0);
...
static int wpa_driver_bsd_scan(void *priv,
  ...
  if (set80211param(drv, IEEE80211_IOC_ROAMING, IEEE80211_ROAMING_MANUAL) < 0) {
    wpa_printf(MSG_ERROR,
               "%s: failed to set "
               "wpa_supplicant-based roaming: %s",
  ...
  for (i = 0; i < sr.sr_nssid; i++) {
    ...
    os_memcpy(sr.sr_ssid[i].ssid, params->ssids[i].ssid, sr.sr_ssid[i].len);
  }
  ...
  if (bsd_set_ssid(drv, params->ssids[0].ssid, params->ssids[0].ssid_len) < 0)
    return -1;
...
static void wpa_driver_bsd_event_receive(int sock, void *ctx, void *sock_ctx) {
  ...
  case RTM_IFANNOUNCE:
    ...
    wpa_printf(MSG_DEBUG, "RTM_IFANNOUNCE: Interface '%s' %s", ifan->ifan_name,
               ifan->ifan_what == IFAN_DEPARTURE ? "removed" : "added");
  ...
  case RTM_IEEE80211:
    ...
    case RTM_IEEE80211_MICHAEL:
      ...
      os_memset(&event, 0, sizeof(event));
      event.michael_mic_failure.unicast = !IEEE80211_IS_MULTICAST(mic->iev_dst);
  ...
  case RTM_IFINFO:
    ...
    if (drv == NULL)
      return;
    if ((ifm->ifm_flags & IFF_UP) == 0 && (drv->flags & IFF_UP) != 0) {
      wpa_printf(MSG_DEBUG, "RTM_IFINFO: Interface '%s' DOWN", drv->ifname);
      wpa_supplicant_event(drv->ctx, EVENT_INTERFACE_DISABLED, NULL);
    } else if ((ifm->ifm_flags & IFF_UP) != 0 && (drv->flags & IFF_UP) == 0) {
      wpa_printf(MSG_DEBUG, "RTM_IFINFO: Interface '%s' UP", drv->ifname);
...
struct wpa_scan_results *wpa_driver_bsd_get_scan_results2(void *priv) {
  ...
  wpa_printf(MSG_DEBUG, "Received %d bytes of scan results (%lu BSSes)", len,
...
#define IEEE80211_C_WPA2 0x01000000   /* CAPABILITY: WPA2 avail */
  ...
  wpa_printf(MSG_DEBUG, "%s: drivercaps=0x%08x,cryptocaps=0x%08x", __func__,
             devcaps.dc_drivercaps, devcaps.dc_cryptocaps);
...
static enum ieee80211_opmode get80211opmode(struct bsd_driver_data *drv) {
  struct ifmediareq ifmr;
...
static void *wpa_driver_bsd_init(void *ctx, const char *ifname, void *priv) {
#define GETPARAM(drv, param, v) (((v) = get80211param(drv, param)) != -1)
  ...
  if (drv->ifindex == 0) {
    wpa_printf(MSG_DEBUG, "%s: interface %s does not exist", __func__, ifname);
    goto fail;
  }
  ...
  if (!GETPARAM(drv, IEEE80211_IOC_ROAMING, drv->prev_roaming)) {
    wpa_printf(MSG_DEBUG, "%s: failed to get roaming state: %s", __func__,
               strerror(errno));
    goto fail;
  }
  if (!GETPARAM(drv, IEEE80211_IOC_PRIVACY, drv->prev_privacy)) {
    wpa_printf(MSG_DEBUG, "%s: failed to get privacy state: %s", __func__,
               strerror(errno));
    goto fail;
  }
  if (!GETPARAM(drv, IEEE80211_IOC_WPA, drv->prev_wpa)) {
    wpa_printf(MSG_DEBUG, "%s: failed to get wpa state: %s", __func__,
               strerror(errno));
    goto fail;
  }
...
static void wpa_driver_bsd_deinit(void *priv) {
  ...
  if (drv->ifindex != 0 && !drv->if_removed) {
    ...
    if (set80211param(drv, IEEE80211_IOC_ROAMING, drv->prev_roaming) < 0)
      wpa_printf(MSG_DEBUG, "%s: failed to restore roaming state", __func__);
  }
...
static void *bsd_global_init(void *ctx) {
  struct bsd_driver_global *global;
  ...
  if (global->route < 0) {
    wpa_printf(MSG_ERROR, "socket[PF_ROUTE,SOCK_RAW]: %s", strerror(errno));
    goto fail;
  }
...
#ifdef HOSTAPD
  eloop_register_read_sock(global->route, bsd_wireless_event_receive, NULL,
                           global);

#else  /* HOSTAPD */
  eloop_register_read_sock(global->route, wpa_driver_bsd_event_receive, NULL,
                           global);
#endif /* HOSTAPD */
...
static void bsd_global_deinit(void *priv) {
  struct bsd_driver_global *global = priv;
