...
static int wired_multicast_membership(int sock, int ifindex, const u8 *addr,
                                      int add) {
#ifdef __linux__
...
static void handle_data(void *ctx, unsigned char *buf, size_t len) {
   ...
   * 2 byte ethertype */
  if (len < 14) {
    wpa_printf(MSG_MSGDUMP, "handle_data: too short (%lu)", (unsigned long)len);
    return;
  }
...
static int wired_init_sockets(struct wpa_driver_wired_data *drv, u8 *own_addr) {
  ...
  if (drv->sock < 0) {
    wpa_printf(MSG_ERROR, "socket[PF_PACKET,SOCK_RAW]: %s", strerror(errno));
    return -1;
  }
  ...
  if (wired_multicast_membership(drv->sock, ifr.ifr_ifindex, pae_group_addr,
                                 1) < 0) {
  ...
  if (ioctl(drv->sock, SIOCGIFHWADDR, &ifr) != 0) {
    wpa_printf(MSG_ERROR, "ioctl(SIOCGIFHWADDR): %s", strerror(errno));
    return -1;
  }
  ...
  if ((drv->dhcp_sock = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
    wpa_printf(MSG_ERROR, "socket call failed for dhcp: %s", strerror(errno));
    return -1;
  }

  if (eloop_register_read_sock(drv->dhcp_sock, handle_dhcp, drv->ctx, NULL)) {
    wpa_printf(MSG_INFO, "Could not register read socket");
    return -1;
  }
  ...
  if (setsockopt(drv->dhcp_sock, SOL_SOCKET, SO_BINDTODEVICE, (char *)&ifr,
                 sizeof(ifr)) < 0) {
    wpa_printf(MSG_ERROR, "setsockopt[SOL_SOCKET,SO_BINDTODEVICE]: %s",
               strerror(errno));
    return -1;
  }
...
static int wired_send_eapol(void *priv, const u8 *addr, const u8 *data,
  ...
  hdr = os_zalloc(len);
  if (hdr == NULL) {
    wpa_printf(MSG_INFO, "malloc() failed for wired_send_eapol(len=%lu)",
               (unsigned long)len);
    return -1;
  }
...
static void *wired_driver_hapd_init(struct hostapd_data *hapd,
  ...
  if (drv == NULL) {
    wpa_printf(MSG_INFO, "Could not allocate memory for wired driver data");
    return NULL;
  }
...
static void *wpa_driver_wired_init(void *ctx, const char *ifname) {
  ...
  if (wpa_driver_wired_get_ifflags(ifname, &flags) == 0 && !(flags & IFF_UP) &&
      wpa_driver_wired_set_ifflags(ifname, flags | IFF_UP) == 0) {
    drv->iff_up = 1;
  }

  if (wired_multicast_membership(drv->pf_sock, if_nametoindex(drv->ifname),
                                 pae_group_addr, 1) == 0) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "for multicast",
               __func__);
  } else if (wpa_driver_wired_set_ifflags(ifname, flags | IFF_ALLMULTI) < 0) {
    ...
    defined(__FreeBSD_kernel__)
  {
    ...
    while (wpa_driver_wired_get_ifstatus(ifname, &status) == 0 && status == 0)
      sleep(1);
  }
#endif /* defined(__FreeBSD__) || defined(__DragonFly__) ||                    \
          defined(FreeBSD_kernel__) */
...
static void wpa_driver_wired_deinit(void *priv) {
  ...
  if (drv->membership &&
      wired_multicast_membership(drv->pf_sock, if_nametoindex(drv->ifname),
                                 pae_group_addr, 0) < 0) {
    wpa_printf(MSG_DEBUG,
  ...
  if (drv->iff_allmulti &&
      (wpa_driver_wired_get_ifflags(drv->ifname, &flags) < 0 ||
       wpa_driver_wired_set_ifflags(drv->ifname, flags & ~IFF_ALLMULTI) < 0)) {
    wpa_printf(MSG_DEBUG, "%s: Failed to disable allmulti mode", __func__);
  }

  if (drv->iff_up && wpa_driver_wired_get_ifflags(drv->ifname, &flags) == 0 &&
      (flags & IFF_UP) &&
