...
struct rfkill_data *rfkill_init(struct rfkill_config *cfg) {
  ...
  os_snprintf(buf, sizeof(buf), "/sys/class/net/%s/phy80211", cfg->ifname);
  ...
  if (fcntl(rfkill->fd, F_SETFL, O_NONBLOCK) < 0) {
    wpa_printf(MSG_ERROR,
               "rfkill: Cannot set non-blocking mode: "
               "%s",
               strerror(errno));
    goto fail2;
  }
  ...
  for (;;) {
    ...
    os_snprintf(buf2, sizeof(buf2), "/sys/class/rfkill/rfkill%d/device",
                event.idx);
    ...
    wpa_printf(MSG_DEBUG,
               "rfkill: initial event: idx=%u type=%d "
               "op=%u soft=%u hard=%u",
               event.idx, event.type, event.op, event.soft, event.hard);
