...
static int wiphy_info_iface_comb_process(struct wiphy_info_data *info,
  ...
  static struct nla_policy iface_combination_policy[NUM_NL80211_IFACE_COMB] =
      {
          ...
          [NL80211_IFACE_COMB_RADAR_DETECT_WIDTHS] = {.type = NLA_U32},
                           ...
                           iface_limit_policy[NUM_NL80211_IFACE_LIMIT] = {
                               [NL80211_IFACE_LIMIT_TYPES] = {.type =
                                                                  NLA_NESTED},
                               [NL80211_IFACE_LIMIT_MAX] = {.type = NLA_U32},
                           };

  err = nla_parse_nested(tb_comb, MAX_NL80211_IFACE_COMB, nl_combi,
  ...
  nla_for_each_nested(nl_limit, tb_comb[NL80211_IFACE_COMB_LIMITS], rem_limit) {
    err = nla_parse_nested(tb_limit, MAX_NL80211_IFACE_LIMIT, nl_limit,
                           iface_limit_policy);
    if (err || !tb_limit[NL80211_IFACE_LIMIT_TYPES])
      return 0; /* broken combination */

    nla_for_each_nested(nl_mode, tb_limit[NL80211_IFACE_LIMIT_TYPES],
                        rem_mode) {
      int ift = nla_type(nl_mode);
      if (ift == NL80211_IFTYPE_P2P_GO || ift == NL80211_IFTYPE_P2P_CLIENT)
...
static void wiphy_info_wowlan_triggers(struct wpa_driver_capa *capa,
  ...
  if (nla_parse_nested(triggers, MAX_NL80211_WOWLAN_TRIG, tb, NULL))
    return;
...
static void wiphy_info_extended_capab(struct wpa_driver_nl80211_data *drv,
  ...
  nla_for_each_nested(attr, tb, rem) {
    ...
    nla_parse(tb1, NL80211_ATTR_MAX, nla_data(attr), nla_len(attr), NULL);

    if (!tb1[NL80211_ATTR_IFTYPE] || !tb1[NL80211_ATTR_EXT_CAPA] ||
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: Driver-advertised extended capabilities for interface "
               "type %s",
    ...
    os_memcpy(capa->ext_capa, nla_data(tb1[NL80211_ATTR_EXT_CAPA]), len);
    capa->ext_capa_len = len;
    wpa_hexdump(MSG_DEBUG, "nl80211: Extended capabilities", capa->ext_capa,
                capa->ext_capa_len);
    ...
    os_memcpy(capa->ext_capa_mask, nla_data(tb1[NL80211_ATTR_EXT_CAPA_MASK]),
              len);
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg) {
  ...
  if (tb[NL80211_ATTR_MAX_NUM_SCAN_SSIDS])
    capa->max_scan_ssids = nla_get_u8(tb[NL80211_ATTR_MAX_NUM_SCAN_SSIDS]);
  ...
  if (tb[NL80211_ATTR_MAX_MATCH_SETS])
    capa->max_match_sets = nla_get_u8(tb[NL80211_ATTR_MAX_MATCH_SETS]);

  if (tb[NL80211_ATTR_MAC_ACL_MAX])
    capa->max_acl_mac_addrs = nla_get_u8(tb[NL80211_ATTR_MAC_ACL_MAX]);
  ...
  wiphy_info_probe_resp_offload(capa, tb[NL80211_ATTR_PROBE_RESP_OFFLOAD]);

  if (tb[NL80211_ATTR_EXT_CAPA] && tb[NL80211_ATTR_EXT_CAPA_MASK] &&
      drv->extended_capa == NULL) {
    drv->extended_capa = os_malloc(nla_len(tb[NL80211_ATTR_EXT_CAPA]));
    if (drv->extended_capa) {
      os_memcpy(drv->extended_capa, nla_data(tb[NL80211_ATTR_EXT_CAPA]),
    ...
    if (drv->extended_capa_mask) {
      ...
      wpa_hexdump(
          ...
          drv->extended_capa_mask, drv->extended_capa_len);
    } else {
...
static int dfs_info_handler(struct nl_msg *msg, void *arg) {
  ...
  if (tb[NL80211_ATTR_VENDOR_DATA]) {
    ...
    if (tb_vendor[QCA_WLAN_VENDOR_ATTR_DFS]) {
      ...
      wpa_printf(MSG_DEBUG, "nl80211: DFS offload capability: %u", val);
...
static int features_info_handler(struct nl_msg *msg, void *arg) {
  ...
  if (nl_vend) {
    ...
    attr = tb_vendor[QCA_WLAN_VENDOR_ATTR_MAX_CONCURRENT_CHANNELS_2_4_BAND];
    if (attr)
      info->capa->max_conc_chan_2_4 = nla_get_u32(attr);

    attr = tb_vendor[QCA_WLAN_VENDOR_ATTR_MAX_CONCURRENT_CHANNELS_5_0_BAND];
    if (attr)
...
static int check_feature(enum qca_wlan_vendor_features feature,
  ...
  return (idx < info->flags_len) && (info->flags[idx] & BIT(feature % 8));
}
...
int wpa_driver_nl80211_capa(struct wpa_driver_nl80211_data *drv) {
  ...
  drv->capa.key_mgmt =
      ...
      WPA_DRIVER_CAPA_KEY_MGMT_SUITE_B | WPA_DRIVER_CAPA_KEY_MGMT_SUITE_B_192;
  drv->capa.auth =
...
static void phy_info_vht_capa(struct hostapd_hw_modes *mode,
                              struct nlattr *capa, struct nlattr *mcs_set) {
  if (capa)
...
static int phy_info_freqs(struct phy_info_arg *phy_info,
  ...
  channel = os_realloc_array(mode->channels, mode->num_channels + new_channels,
...
static int phy_info_band(struct phy_info_arg *phy_info,
  ...
  if (phy_info->last_mode != nl_band->nla_type) {
    mode = os_realloc_array(phy_info->modes, *phy_info->num_modes + 1,
                            sizeof(*mode));
    if (!mode) {
      phy_info->failed = 1;
      return NL_STOP;
    ...
    mode->flags =
        HOSTAPD_MODE_FLAG_HT_INFO_KNOWN | HOSTAPD_MODE_FLAG_VHT_INFO_KNOWN;
  ...
  nla_parse(tb_band, NL80211_BAND_ATTR_MAX, nla_data(nl_band), nla_len(nl_band),
            NULL);
...
wpa_driver_nl80211_postprocess_modes(struct hostapd_hw_modes *modes,
  ...
  mode->channels =
      os_malloc(mode11g->num_channels * sizeof(struct hostapd_channel_data));
...
static void nl80211_reg_rule_ht40(u32 start, u32 end,
  ...
  for (m = 0; m < *results->num_modes; m++) {
    if (!(results->modes[m].ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET))
      continue;
...
static int nl80211_get_reg(struct nl_msg *msg, void *arg) {
  ...
  nla_parse(tb_msg, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
            genlmsg_attrlen(gnlh, 0), NULL);
  if (!tb_msg[NL80211_ATTR_REG_ALPHA2] || !tb_msg[NL80211_ATTR_REG_RULES]) {
  ...
  nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule) {
    ...
    nla_parse(tb_rule, NL80211_FREQUENCY_ATTR_MAX, nla_data(nl_rule),
              nla_len(nl_rule), reg_policy);
    ...
    if (tb_rule[NL80211_ATTR_POWER_RULE_MAX_EIRP])
      nl80211_reg_rule_max_eirp(start, end, max_eirp, results);
  }
...
struct hostapd_hw_modes *nl80211_get_hw_feature_data(void *priv, u16 *num_modes,
                                                     u16 *flags) {
  u32 feat;
  ...
  *flags = 0;
  ...
  if (send_and_recv_msgs(drv, msg, phy_info_handler, &result) == 0) {
    ...
    return wpa_driver_nl80211_postprocess_modes(result.modes, num_modes);
  }
