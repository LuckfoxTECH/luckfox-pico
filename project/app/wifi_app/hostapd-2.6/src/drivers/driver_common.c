...
#define CHECK_TRIGGER(trigger)                                                 \
  wpa_check_wowlan_trigger(start, #trigger, capa->wowlan_triggers.trigger,     \
                           &triggers->trigger)
  ...
  while (*start != '\0') {
    ...
    *end = '\0';
    ...
    if (!CHECK_TRIGGER(any) && !CHECK_TRIGGER(disconnect) &&
        !CHECK_TRIGGER(magic_pkt) && !CHECK_TRIGGER(gtk_rekey_failure) &&
        !CHECK_TRIGGER(eap_identity_req) &&
        !CHECK_TRIGGER(four_way_handshake) && !CHECK_TRIGGER(rfkill_release)) {
      wpa_printf(MSG_DEBUG, "Unknown/unsupported wowlan trigger '%s'", start);
      os_free(triggers);
      triggers = NULL;
      goto out;
    }
