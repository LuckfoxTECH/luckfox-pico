...
static void mlme_event_auth(struct wpa_driver_nl80211_data *drv,
  ...
  if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME) && drv->force_connect_cmd) {
    /*
     * Avoid reporting two association events that would confuse
     * the core code.
     */
    wpa_printf(MSG_DEBUG, "nl80211: Ignore auth event when using driver SME");
    return;
  }
...
static void nl80211_parse_wmm_params(struct nlattr *wmm_attr,
  ...
  if (!wmm_attr ||
      nla_parse_nested(wmm_info, NL80211_STA_WME_MAX, wmm_attr, wme_policy) ||
      !wmm_info[NL80211_STA_WME_UAPSD_QUEUES])
    return;

  wmm_params->uapsd_queues = nla_get_u8(wmm_info[NL80211_STA_WME_UAPSD_QUEUES]);
...
static void mlme_event_assoc(struct wpa_driver_nl80211_data *drv,
  ...
  if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME) && drv->force_connect_cmd) {
     ...
     * the core code.
     */
    wpa_printf(MSG_DEBUG, "nl80211: Ignore assoc event when using driver SME");
    return;
  }
  ...
  if (status != WLAN_STATUS_SUCCESS) {
    ...
    if (len > 24 + sizeof(mgmt->u.assoc_resp)) {
      event.assoc_reject.resp_ies = (u8 *)mgmt->u.assoc_resp.variable;
      event.assoc_reject.resp_ies_len = len - 24 - sizeof(mgmt->u.assoc_resp);
    }
...
static void mlme_event_connect(struct wpa_driver_nl80211_data *drv,
                               ...
                               struct nlattr *resp_ie, struct nlattr *timed_out,
                               struct nlattr *authorized,
                               struct nlattr *key_replay_ctr,
                               struct nlattr *ptk_kck, struct nlattr *ptk_kek,
  ...
  if (cmd == NL80211_CMD_CONNECT && status_code != WLAN_STATUS_SUCCESS) {
    ...
    if (drv->ignore_next_local_disconnect) {
      ...
      if (!event.assoc_reject.bssid ||
          (os_memcmp(event.assoc_reject.bssid, drv->auth_attempt_bssid,
                     ETH_ALEN) != 0)) {
        /*
         ...
         */
        wpa_printf(MSG_DEBUG, "nl80211: Ignore connection failure event "
                              "triggered during reassociation");
        return;
      }
    }
    if (resp_ie) {
  ...
  if (req_ie) {
    ...
    if (cmd == NL80211_CMD_ROAM) {
      ssid = get_ie(event.assoc_info.req_ies, event.assoc_info.req_ies_len,
                    WLAN_EID_SSID);
...
static void mlme_event_ch_switch(struct wpa_driver_nl80211_data *drv,
  ...
  if (type) {
    ...
    chan_offset =
        calculate_chan_offset(nla_get_u32(bw), nla_get_u32(freq),
                              nla_get_u32(cf1), cf2 ? nla_get_u32(cf2) : 0);
  } else {
    wpa_printf(MSG_WARNING,
               "nl80211: Unknown secondary channel information - following "
               "channel definition calculations may fail");
  }
...
static void mlme_timeout_event(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: MLME event %d; timeout with " MACSTR, cmd,
...
static void mlme_event_mgmt(struct i802_bss *bss, struct nlattr *freq,
  ...
  wpa_printf(
      ...
      MAC2STR(mgmt->da), MAC2STR(mgmt->sa), MAC2STR(mgmt->bssid), rx_freq,
      ssi_signal, fc, le_to_host16(mgmt->seq_ctrl), stype, fc2str(fc),
      (unsigned int)len);
...
static void mlme_event_mgmt_tx_status(struct wpa_driver_nl80211_data *drv,
  ...
  if (!is_ap_interface(drv->nlmode)) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               cookie_val == drv->send_action_cookie ? " (match)"
                                                     : " (unknown)",
               ack != NULL);
...
static void mlme_event_deauth_disassoc(struct wpa_driver_nl80211_data *drv,
  ...
  if (len >= 24) {
    ...
    if ((drv->capa.flags & WPA_DRIVER_FLAGS_SME) && !drv->associated &&
       ...
       */
      if (!os_memcmp(mgmt->sa, drv->first_bss->addr, ETH_ALEN)) {
        wpa_printf(MSG_DEBUG, "nl80211: Received a locally generated deauth "
                              "event. Clear ignore_next_local_deauth flag");
        drv->ignore_next_local_deauth = 0;
      } else {
        wpa_printf(MSG_DEBUG,
                   "nl80211: Ignore deauth/disassoc event from old AP " MACSTR
                   " when already authenticating with " MACSTR,
                   MAC2STR(bssid), MAC2STR(drv->auth_attempt_bssid));
      }
      return;
    }

    if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME) && drv->connect_reassoc &&
        drv->associated && os_memcmp(bssid, drv->prev_bssid, ETH_ALEN) == 0 &&
        os_memcmp(bssid, drv->auth_attempt_bssid, ETH_ALEN) != 0) {
      /*
       * Avoid issues with some roaming cases where
       * disconnection event for the old AP may show up after
    ...
    if (drv->associated != 0 && os_memcmp(bssid, drv->bssid, ETH_ALEN) != 0 &&
        os_memcmp(bssid, drv->auth_bssid, ETH_ALEN) != 0) {
      /*
  ...
  if (type == EVENT_DISASSOC) {
    ...
    if (frame + len > mgmt->u.disassoc.variable) {
      ...
      event.disassoc_info.ie_len = frame + len - mgmt->u.disassoc.variable;
    }
  } else {
    event.deauth_info.locally_generated =
        !os_memcmp(mgmt->sa, drv->first_bss->addr, ETH_ALEN);
    if (drv->ignore_deauth_event) {
      wpa_printf(MSG_DEBUG, "nl80211: Ignore deauth event due to previous "
                            "forced deauth-during-auth");
    ...
    if (drv->ignore_next_local_deauth) {
      ...
      wpa_printf(MSG_WARNING, "nl80211: Was expecting local deauth but got "
    ...
    if (frame + len > mgmt->u.deauth.variable) {
      ...
      event.deauth_info.ie_len = frame + len - mgmt->u.deauth.variable;
    }
  }
...
static void mlme_event(struct i802_bss *bss, enum nl80211_commands cmd,
  ...
  if (frame == NULL) {
    wpa_printf(MSG_DEBUG, "nl80211: MLME event %d (%s) without frame data", cmd,
  ...
  if (len < 4 + 2 * ETH_ALEN) {
    wpa_printf(MSG_MSGDUMP,
               "nl80211: MLME event %d (%s) on %s(" MACSTR ") - too short", cmd,
               nl80211_command_to_string(cmd), bss->ifname, MAC2STR(bss->addr));
    return;
  }
  wpa_printf(
      MSG_MSGDUMP,
      "nl80211: MLME event %d (%s) on %s(" MACSTR ") A1=" MACSTR " A2=" MACSTR,
      cmd, nl80211_command_to_string(cmd), bss->ifname, MAC2STR(bss->addr),
  ...
  wpa_hexdump(MSG_MSGDUMP, "nl80211: MLME event frame", nla_data(frame),
              nla_len(frame));
  ...
  case NL80211_CMD_DEAUTHENTICATE:
    mlme_event_deauth_disassoc(drv, EVENT_DEAUTH, nla_data(frame),
                               nla_len(frame));
    break;
  case NL80211_CMD_DISASSOCIATE:
    mlme_event_deauth_disassoc(drv, EVENT_DISASSOC, nla_data(frame),
                               nla_len(frame));
    break;
  ...
  case NL80211_CMD_FRAME_TX_STATUS:
    mlme_event_mgmt_tx_status(drv, cookie, nla_data(frame), nla_len(frame),
                              ack);
    break;
  case NL80211_CMD_UNPROT_DEAUTHENTICATE:
    mlme_event_unprot_disconnect(drv, EVENT_UNPROT_DEAUTH, nla_data(frame),
                                 nla_len(frame));
    break;
  case NL80211_CMD_UNPROT_DISASSOCIATE:
    mlme_event_unprot_disconnect(drv, EVENT_UNPROT_DISASSOC, nla_data(frame),
                                 nla_len(frame));
    break;
  default:
    break;
...
static void mlme_event_michael_mic_failure(struct i802_bss *bss,
  ...
  if (tb[NL80211_ATTR_MAC]) {
    wpa_hexdump(MSG_DEBUG, "nl80211: Source MAC address",
                nla_data(tb[NL80211_ATTR_MAC]), nla_len(tb[NL80211_ATTR_MAC]));
    data.michael_mic_failure.src = nla_data(tb[NL80211_ATTR_MAC]);
  }
  if (tb[NL80211_ATTR_KEY_SEQ]) {
    wpa_hexdump(MSG_DEBUG, "nl80211: TSC", nla_data(tb[NL80211_ATTR_KEY_SEQ]),
                nla_len(tb[NL80211_ATTR_KEY_SEQ]));
  }
  if (tb[NL80211_ATTR_KEY_TYPE]) {
    enum nl80211_key_type key_type = nla_get_u32(tb[NL80211_ATTR_KEY_TYPE]);
...
static void mlme_event_join_ibss(struct wpa_driver_nl80211_data *drv,
  ...
  drv->associated = 1;
  wpa_printf(MSG_DEBUG, "nl80211: IBSS " MACSTR " joined", MAC2STR(drv->bssid));

  freq = nl80211_get_assoc_freq(drv);
  if (freq) {
    wpa_printf(MSG_DEBUG, "nl80211: IBSS on frequency %u MHz", freq);
...
static void mlme_event_remain_on_channel(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_supplicant_event(drv->ctx,
                       cancel_event ? EVENT_CANCEL_REMAIN_ON_CHANNEL
...
static void mlme_event_ft_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (tb[NL80211_ATTR_MAC])
    os_memcpy(data.ft_ies.target_ap, nla_data(tb[NL80211_ATTR_MAC]), ETH_ALEN);
  ...
  if (tb[NL80211_ATTR_SCAN_FREQUENCIES]) {
    ...
    *pos = '\0';
    ...
    nla_for_each_nested(nl, tb[NL80211_ATTR_SCAN_FREQUENCIES], rem) {
      ...
      res = os_snprintf(pos, end - pos, " %d", freqs[num_freqs]);
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Scan included frequencies:%s", msg);
  }
...
static void nl80211_cqm_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (cqm[NL80211_ATTR_CQM_PKT_LOSS_EVENT]) {
    if (!tb[NL80211_ATTR_MAC])
      return;
    os_memcpy(ed.low_ack.addr, nla_data(tb[NL80211_ATTR_MAC]), ETH_ALEN);
...
static void nl80211_new_peer_candidate(struct wpa_driver_nl80211_data *drv,
  ...
  if (drv->nlmode != NL80211_IFTYPE_MESH_POINT || !tb[NL80211_ATTR_MAC] ||
      !tb[NL80211_ATTR_IE])
    return;
...
static void nl80211_del_station_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (tb[NL80211_ATTR_MAC] == NULL)
    return;
  addr = nla_data(tb[NL80211_ATTR_MAC]);
  wpa_printf(MSG_DEBUG, "nl80211: Delete station " MACSTR, MAC2STR(addr));
...
static void nl80211_rekey_offload_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (!tb[NL80211_ATTR_MAC] || !tb[NL80211_ATTR_REKEY_DATA] ||
...
static void nl80211_pmksa_candidate_event(struct wpa_driver_nl80211_data *drv,
  ...
  data.pmkid_candidate.index = nla_get_u32(cand[NL80211_PMKSA_CANDIDATE_INDEX]);
  data.pmkid_candidate.preauth = cand[NL80211_PMKSA_CANDIDATE_PREAUTH] != NULL;
...
static void nl80211_client_probe_event(struct wpa_driver_nl80211_data *drv,
  ...
  os_memset(&data, 0, sizeof(data));
  os_memcpy(data.client_poll.addr, nla_data(tb[NL80211_ATTR_MAC]), ETH_ALEN);
...
static void nl80211_tdls_oper_event(struct wpa_driver_nl80211_data *drv,
  ...
  case NL80211_TDLS_SETUP:
    wpa_printf(MSG_DEBUG, "nl80211: TDLS setup request for peer " MACSTR,
               MAC2STR(data.tdls.peer));
    data.tdls.oper = TDLS_REQUEST_SETUP;
    break;
  case NL80211_TDLS_TEARDOWN:
    wpa_printf(MSG_DEBUG, "nl80211: TDLS teardown request for peer " MACSTR,
               MAC2STR(data.tdls.peer));
    data.tdls.oper = TDLS_REQUEST_TEARDOWN;
    break;
  case NL80211_TDLS_DISCOVERY_REQ:
    wpa_printf(MSG_DEBUG, "nl80211: TDLS discovery request for peer " MACSTR,
               MAC2STR(data.tdls.peer));
  ...
  if (tb[NL80211_ATTR_REASON_CODE]) {
    data.tdls.reason_code = nla_get_u16(tb[NL80211_ATTR_REASON_CODE]);
  }
...
static void nl80211_connect_failed_event(struct wpa_driver_nl80211_data *drv,
  ...
  os_memset(&data, 0, sizeof(data));
  os_memcpy(data.connect_failed_reason.addr, nla_data(tb[NL80211_ATTR_MAC]),
            ETH_ALEN);
  ...
  case NL80211_CONN_FAIL_BLOCKED_CLIENT:
    wpa_printf(MSG_DEBUG, "nl80211: Blocked client " MACSTR " tried to connect",
...
static void nl80211_radar_event(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: DFS event on freq %d MHz, ht: %d, offset: %d, width: "
             "%d, cf1: %dMHz, cf2: %dMHz",
...
static void qca_nl80211_avoid_freq(struct wpa_driver_nl80211_data *drv,
  ...
  if (len <
      sizeof(freq_range->count) + count * sizeof(struct qca_avoid_freq_range)) {
    wpa_printf(MSG_DEBUG,
...
static void qca_nl80211_acs_select_ch(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: ACS channel selection vendor event received");

  if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_ACS_MAX, (struct nlattr *)data, len,
                NULL) ||
  ...
  if (tb[QCA_WLAN_VENDOR_ATTR_ACS_HW_MODE]) {
    ...
    if (event.acs_selected_channels.hw_mode == NUM_HOSTAPD_MODES ||
        event.acs_selected_channels.hw_mode == HOSTAPD_MODE_IEEE80211ANY) {
      wpa_printf(MSG_DEBUG,
                 "nl80211: Invalid hw_mode %d in ACS selection event", hw_mode);
      return;
    }
  }
...
static void qca_nl80211_key_mgmt_auth(struct wpa_driver_nl80211_data *drv,
  ...
  if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_MAX, (struct nlattr *)data,
                len, NULL) ||
  ...
  mlme_event_connect(drv, NL80211_CMD_ROAM, NULL,
                     tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_BSSID],
                     tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_REQ_IE],
                     tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_RESP_IE], NULL,
                     tb[QCA_WLAN_VENDOR_ATTR_ROAM_AUTH_AUTHORIZED],
...
static void
qca_nl80211_dfs_offload_radar_event(struct wpa_driver_nl80211_data *drv,
                                    u32 subcmd, u8 *msg, int length) {
  union wpa_event_data data;
  struct nlattr *tb[NL80211_ATTR_MAX + 1];

  wpa_printf(MSG_DEBUG, "nl80211: DFS offload radar vendor event received");

  if (nla_parse(tb, NL80211_ATTR_MAX, (struct nlattr *)msg, length, NULL))
    return;
  ...
  default:
    wpa_printf(MSG_DEBUG,
               "nl80211: Unknown DFS offload radar event %d received", subcmd);
    break;
  }
}
...
static void qca_nl80211_scan_trigger_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (nla_parse(tb, QCA_WLAN_VENDOR_ATTR_SCAN_MAX, (struct nlattr *)data, len,
                NULL) ||
...
static void send_vendor_scan_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (tb[QCA_WLAN_VENDOR_ATTR_SCAN_SSIDS]) {
    nla_for_each_nested(nl, tb[QCA_WLAN_VENDOR_ATTR_SCAN_SSIDS], rem) {
      struct wpa_driver_scan_ssid *s = &info->ssids[info->num_ssids];
      s->ssid = nla_data(nl);
      s->ssid_len = nla_len(nl);
      wpa_printf(MSG_DEBUG, "nl80211: Scan probed for SSID '%s'",
  ...
  if (tb[QCA_WLAN_VENDOR_ATTR_SCAN_FREQUENCIES]) {
    ...
    *pos = '\0';
    ...
    nla_for_each_nested(nl, tb[QCA_WLAN_VENDOR_ATTR_SCAN_FREQUENCIES], rem) {
      freqs[num_freqs] = nla_get_u32(nl);
      res = os_snprintf(pos, end - pos, " %d", freqs[num_freqs]);
      if (!os_snprintf_error(end - pos, res))
        pos += res;
...
static void qca_nl80211_p2p_lo_stop_event(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: P2P Listen offload stop reason: %d",
             event.p2p_lo_stop.reason_code);
...
#endif /* CONFIG_DRIVER_NL80211_QCA */
  default:
    wpa_printf(MSG_DEBUG, "nl80211: Ignore unsupported QCA vendor event %u",
               subcmd);
    break;
  }
}
...
static void nl80211_vendor_event(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Vendor event: wiphy=%u vendor_id=0x%x subcmd=%u", wiphy,
             vendor_id, subcmd);
...
static void nl80211_reg_change_event(struct wpa_driver_nl80211_data *drv,
  ...
  if (tb[NL80211_ATTR_REG_TYPE]) {
    ...
    case NL80211_REGDOM_TYPE_CUSTOM_WORLD:
      data.channel_list_changed.type = REGDOM_TYPE_CUSTOM_WORLD;
      break;
    case NL80211_REGDOM_TYPE_INTERSECTION:
      data.channel_list_changed.type = REGDOM_TYPE_INTERSECTION;
      break;
    }
  }
  ...
  if (tb[NL80211_ATTR_REG_ALPHA2]) {
    ...
    wpa_printf(MSG_DEBUG, " * alpha2=%s", data.channel_list_changed.alpha2);
  }
...
static void do_process_drv_event(struct i802_bss *bss, int cmd,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Drv Event %d (%s) received for %s", cmd,
  ...
  if (cmd == NL80211_CMD_ROAM &&
     ...
     */
    wpa_printf(MSG_DEBUG,
               "nl80211: Ignore roam event (cmd=%d), device will use vendor "
               "event roam+auth",
               cmd);
    return;
  }
  ...
  if (drv->ap_scan_as_station != NL80211_IFTYPE_UNSPECIFIED &&
    ...
    wpa_driver_nl80211_set_mode(drv->first_bss, drv->ap_scan_as_station);
  ...
  case NL80211_CMD_TRIGGER_SCAN:
    ...
    if (drv->scan_for_auth) {
       ...
       * scanning state.
       */
      wpa_printf(MSG_DEBUG, "nl80211: Do not indicate scan-start event due to "
                            "internal scan_for_auth");
      break;
    }
  ...
  case NL80211_CMD_SCHED_SCAN_RESULTS:
    wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: New sched scan results available");
  ...
  case NL80211_CMD_UNPROT_DEAUTHENTICATE:
  case NL80211_CMD_UNPROT_DISASSOCIATE:
    mlme_event(bss, cmd, tb[NL80211_ATTR_FRAME], tb[NL80211_ATTR_MAC],
               tb[NL80211_ATTR_TIMED_OUT], tb[NL80211_ATTR_WIPHY_FREQ],
               tb[NL80211_ATTR_ACK], tb[NL80211_ATTR_COOKIE],
               tb[NL80211_ATTR_RX_SIGNAL_DBM], tb[NL80211_ATTR_STA_WME]);
    break;
  case NL80211_CMD_CONNECT:
  case NL80211_CMD_ROAM:
    mlme_event_connect(drv, cmd, tb[NL80211_ATTR_STATUS_CODE],
                       tb[NL80211_ATTR_MAC], tb[NL80211_ATTR_REQ_IE],
                       tb[NL80211_ATTR_RESP_IE], tb[NL80211_ATTR_TIMED_OUT],
                       NULL, NULL, NULL, NULL, NULL);
    break;
  case NL80211_CMD_CH_SWITCH_NOTIFY:
    mlme_event_ch_switch(
        drv, tb[NL80211_ATTR_IFINDEX], tb[NL80211_ATTR_WIPHY_FREQ],
        tb[NL80211_ATTR_WIPHY_CHANNEL_TYPE], tb[NL80211_ATTR_CHANNEL_WIDTH],
...
int process_global_event(struct nl_msg *msg, void *arg) {
  ...
  dl_list_for_each_safe(drv, tmp, &global->interfaces,
    ...
    for (bss = drv->first_bss; bss; bss = bss->next) {
      if ((ifidx == -1 && !wdev_id_set) || ifidx == bss->ifindex ||
          (wdev_id_set && bss->wdev_id_set && wdev_id == bss->wdev_id)) {
        do_process_drv_event(bss, gnlh->cmd, tb);
        return NL_SKIP;
      }
    }
    wpa_printf(MSG_DEBUG,
               "nl80211: Ignored event (cmd=%d) for foreign interface (ifindex "
               "%d wdev 0x%llx)",
...
int process_bss_event(struct nl_msg *msg, void *arg) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: BSS Event %d (%s) received for %s", gnlh->cmd,
             nl80211_command_to_string(gnlh->cmd), bss->ifname);

  switch (gnlh->cmd) {
  case NL80211_CMD_FRAME:
  case NL80211_CMD_FRAME_TX_STATUS:
    mlme_event(bss, gnlh->cmd, tb[NL80211_ATTR_FRAME], tb[NL80211_ATTR_MAC],
               tb[NL80211_ATTR_TIMED_OUT], tb[NL80211_ATTR_WIPHY_FREQ],
               tb[NL80211_ATTR_ACK], tb[NL80211_ATTR_COOKIE],
               tb[NL80211_ATTR_RX_SIGNAL_DBM], tb[NL80211_ATTR_STA_WME]);
    break;
