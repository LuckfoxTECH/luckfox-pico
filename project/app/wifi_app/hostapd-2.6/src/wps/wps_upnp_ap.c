...
int upnp_er_set_selected_registrar(struct wps_registrar *reg,
  ...
  wpa_hexdump_buf(MSG_MSGDUMP, "WPS: SetSelectedRegistrar attributes", msg);
  ...
  if (attr.selected_registrar == NULL || *attr.selected_registrar == 0) {
    ...
    if (attr.authorized_macs) {
      ...
      os_memcpy(s->authorized_macs, attr.authorized_macs, count * ETH_ALEN);
    ...
    eloop_register_timeout(WPS_PBC_WALK_TIME, 0, upnp_er_set_selected_timeout,
                           s, reg);
  }
