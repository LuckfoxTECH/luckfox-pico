  ...
  *islast = 0;
  ...
  case ADVERTISE_DOWN:
    ...
    wpabuf_printf(msg, "NTS: %s\r\n",
                  (a->type == ADVERTISE_UP ? "ssdp:alive" : "ssdp:byebye"));
    break;
  case MSEARCH_REPLY:
    NTString = "ST";
  ...
  if (a->type != ADVERTISE_DOWN) {
    ...
    wpabuf_printf(msg, "LOCATION: http://%s:%d/%s\r\n", sm->ip_addr_text,
                  sm->web_port, UPNP_WPS_DEVICE_XML_FILE);
  }
  ...
  if (sendto(sm->multicast_sd, wpabuf_head(msg), wpabuf_len(msg), 0,
    ...
    if (a->type == ADVERTISE_DOWN) {
      ...
      if (os_get_random((void *)&r, sizeof(r)) < 0)
        r = 32768;
      next_timeout_sec =
          UPNP_CACHE_SEC / 4 + (((UPNP_CACHE_SEC / 4) * r) >> 16);
      sm->advertise_count++;
...
static void msearchreply_state_machine_handler(void *eloop_data,
  ...
  wpa_printf(MSG_MSGDUMP, "WPS UPnP: M-SEARCH reply state=%d (%s:%d)", a->state,
             inet_ntoa(a->client.sin_addr), ntohs(a->client.sin_port));
  msg = next_advertisement(sm, a, &islast);
  if (msg == NULL)
    return;
  ...
  if (sendto(sm->multicast_sd, wpabuf_head(msg), wpabuf_len(msg), 0,
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "errno %d (%s) for %s:%d",
               errno, strerror(errno), inet_ntoa(a->client.sin_addr),
...
static void ssdp_parse_msearch(struct upnp_wps_device_sm *sm,
  ...
  for (; *data != '\0'; data += line_length(data)) {
    if (token_eq(data, "host")) {
      ...
      if (str_starts(data, "uuid:")) {
        char uuid_string[80];
        struct upnp_wps_device_interface *iface;
        iface = dl_list_first(&sm->interfaces, struct upnp_wps_device_interface,
                              list);
        if (!iface)
          continue;
        data += os_strlen("uuid:");
  ...
  if (!got_host || !got_st || !got_man || !got_mx || mx < 0) {
    wpa_printf(MSG_DEBUG,
               ...
               "%d mx=%d",
               got_host, got_st, got_man, got_mx, mx);
    goto bad;
  }
  if (!st_match) {
...
void ssdp_listener_stop(struct upnp_wps_device_sm *sm) {
  ...
  eloop_cancel_timeout(msearchreply_state_machine_handler, sm, ELOOP_ALL_CTX);
}
...
int ssdp_listener_open(void) {
  ...
  if (sd < 0 || fcntl(sd, F_SETFL, O_NONBLOCK) != 0 ||
  ...
  if (setsockopt(sd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&mcast_addr,
                 sizeof(mcast_addr)) ||
...
int ssdp_listener_start(struct upnp_wps_device_sm *sm) {
  ...
  if (eloop_register_sock(sm->ssdp_sd, EVENT_TYPE_READ, ssdp_listener_handler,
                          NULL, sm))
    goto fail;
...
int ssdp_open_multicast_sock(u32 ip_addr, const char *forced_ifname) {
  ...
  if (forced_ifname) {
    ...
    if (setsockopt(sd, SOL_SOCKET, SO_BINDTODEVICE, &req, sizeof(req)) < 0) {
      wpa_printf(MSG_INFO,
                 "WPS UPnP: Failed to bind "
  ...
  if (setsockopt(sd, IPPROTO_IP, IP_MULTICAST_IF, &ip_addr, sizeof(ip_addr))) {
    wpa_printf(MSG_DEBUG,
               "WPS: setsockopt(IP_MULTICAST_IF) %x: "
               "%d (%s)",
  ...
  if (setsockopt(sd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl))) {
    wpa_printf(MSG_DEBUG,
               "WPS: setsockopt(IP_MULTICAST_TTL): "
               "%d (%s)",
               errno, strerror(errno));
    close(sd);
    return -1;
