...
static struct wpabuf *ndef_parse_records(const struct wpabuf *buf,
  ...
  while (len > 0) {
    ...
    if (filter == NULL || filter(&record))
      return wpabuf_alloc_copy(record.payload, record.payload_length);
...
static struct wpabuf *ndef_build_record(u8 flags, const void *type,
                                        u8 type_length, void *id, u8 id_length,
...
struct wpabuf *ndef_build_wifi(const struct wpabuf *buf) {
  return ndef_build_record(
      FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046,
      wifi_handover_type, os_strlen(wifi_handover_type), NULL, 0, buf);
}
...
struct wpabuf *ndef_build_p2p(const struct wpabuf *buf) {
  return ndef_build_record(
      FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046,
      p2p_handover_type, os_strlen(p2p_handover_type), NULL, 0, buf);
}
