...
static void wps_free_nfc_pw_tokens(struct dl_list *tokens, u16 pw_id) {
  ...
  dl_list_for_each_safe(token, prev, tokens, struct wps_nfc_pw_token, list) {
...
#define wps_free_nfc_pw_tokens(t, p)                                           \
  do {                                                                         \
  } while (0)
...
static void wps_registrar_add_authorized_mac(struct wps_registrar *reg,
                                             const u8 *addr) {
  int i;
  wpa_printf(MSG_DEBUG, "WPS: Add authorized MAC " MACSTR, MAC2STR(addr));
  ...
  for (i = WPS_MAX_AUTHORIZED_MACS - 1; i > 0; i--)
    os_memcpy(reg->authorized_macs[i], reg->authorized_macs[i - 1], ETH_ALEN);
  os_memcpy(reg->authorized_macs[0], addr, ETH_ALEN);
  wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs", (u8 *)reg->authorized_macs,
              sizeof(reg->authorized_macs));
}

static void wps_registrar_remove_authorized_mac(struct wps_registrar *reg,
                                                const u8 *addr) {
  int i;
  wpa_printf(MSG_DEBUG, "WPS: Remove authorized MAC " MACSTR, MAC2STR(addr));
  for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++) {
  ...
  for (; i + 1 < WPS_MAX_AUTHORIZED_MACS; i++)
    os_memcpy(reg->authorized_macs[i], reg->authorized_macs[i + 1], ETH_ALEN);
  os_memset(reg->authorized_macs[WPS_MAX_AUTHORIZED_MACS - 1], 0, ETH_ALEN);
  wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs", (u8 *)reg->authorized_macs,
              sizeof(reg->authorized_macs));
}
...
int wps_device_store(struct wps_registrar *reg, struct wps_device_data *dev,
                     const u8 *uuid) {
...
static void wps_registrar_add_pbc_session(struct wps_registrar *reg,
  ...
  while (pbc) {
    if (os_reltime_expired(&now, &pbc->timestamp, WPS_PBC_WALK_TIME)) {
      prev->next = NULL;
  ...
  while (pbc) {
    if (os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN) == 0 ||
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 "addr=" MACSTR,
                 MAC2STR(tmp->addr));
      wpa_hexdump(MSG_DEBUG, "WPS: Removed UUID-E", tmp->uuid_e, WPS_UUID_LEN);
      os_free(tmp);
      continue;
    }
    prev = pbc;
...
int wps_registrar_pbc_overlap(struct wps_registrar *reg, const u8 *addr,
                              const u8 *uuid_e) {
  int count = 0;
  ...
  if (uuid_e) {
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: Requested UUID", uuid_e, WPS_UUID_LEN);
    count++;
  }
  ...
  for (pbc = reg->pbc_sessions; pbc; pbc = pbc->next) {
    wpa_printf(MSG_DEBUG, "WPS: Consider PBC session with " MACSTR,
               MAC2STR(pbc->addr));
    wpa_hexdump(MSG_DEBUG, "WPS: UUID-E", pbc->uuid_e, WPS_UUID_LEN);
    if (os_reltime_expired(&now, &pbc->timestamp, WPS_PBC_WALK_TIME)) {
      wpa_printf(MSG_DEBUG, "WPS: PBC walk time has expired");
      break;
    }
    if (first && os_memcmp(pbc->uuid_e, first->uuid_e, WPS_UUID_LEN) == 0) {
      wpa_printf(MSG_DEBUG, "WPS: Same Enrollee");
      continue; /* same Enrollee */
    }
    if (uuid_e == NULL || os_memcmp(uuid_e, pbc->uuid_e, WPS_UUID_LEN)) {
...
static void wps_set_pushbutton(u16 *methods, u16 conf_methods) {
  *methods |= WPS_CONFIG_PUSHBUTTON;
  if ((conf_methods & WPS_CONFIG_VIRT_PUSHBUTTON) == WPS_CONFIG_VIRT_PUSHBUTTON)
    *methods |= WPS_CONFIG_VIRT_PUSHBUTTON;
  if ((conf_methods & WPS_CONFIG_PHY_PUSHBUTTON) == WPS_CONFIG_PHY_PUSHBUTTON)
    *methods |= WPS_CONFIG_PHY_PUSHBUTTON;
  if ((*methods & WPS_CONFIG_VIRT_PUSHBUTTON) != WPS_CONFIG_VIRT_PUSHBUTTON &&
      (*methods & WPS_CONFIG_PHY_PUSHBUTTON) != WPS_CONFIG_PHY_PUSHBUTTON) {
    /*
...
static int wps_build_sel_reg_config_methods(struct wps_registrar *reg,
  ...
  methods &= ~WPS_CONFIG_PUSHBUTTON;
  methods &= ~(WPS_CONFIG_VIRT_PUSHBUTTON | WPS_CONFIG_PHY_PUSHBUTTON);
  if (reg->pbc)
...
wps_registrar_init(struct wps_context *wps,
  ...
  if (cfg->extra_cred) {
    reg->extra_cred = wpabuf_alloc_copy(cfg->extra_cred, cfg->extra_cred_len);
...
int wps_registrar_add_pin(struct wps_registrar *reg, const u8 *addr,
  ...
  wpa_printf(MSG_DEBUG, "WPS: A new PIN configured (timeout=%d)", timeout);
  ...
  if (addr)
    ...
    wps_registrar_add_authorized_mac(reg, (u8 *)"\xff\xff\xff\xff\xff\xff");
...
static void wps_registrar_expire_pins(struct wps_registrar *reg) {
  ...
  dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
    if ((pin->flags & PIN_EXPIRES) &&
      ...
      wpa_hexdump(MSG_DEBUG, "WPS: Expired PIN for UUID", pin->uuid,
                  WPS_UUID_LEN);
      wps_registrar_remove_pin(reg, pin);
    }
  }
...
static int wps_registrar_invalidate_wildcard_pin(struct wps_registrar *reg,
  ...
  dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
    ...
    if (pin->wildcard_uuid) {
      wpa_hexdump(MSG_DEBUG, "WPS: Invalidated PIN for UUID", pin->uuid,
                  WPS_UUID_LEN);
      wps_registrar_remove_pin(reg, pin);
      return 0;
    }
...
static const u8 *wps_registrar_get_pin(struct wps_registrar *reg,
  ...
  dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
    if (!pin->wildcard_uuid && os_memcmp(pin->uuid, uuid, WPS_UUID_LEN) == 0) {
      found = pin;
      break;
    }
  }
  ...
  if (!found) {
     ...
     * PINs matched */
    dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
      if (pin->wildcard_uuid == 1 || pin->wildcard_uuid == 2) {
...
int wps_registrar_button_pushed(struct wps_registrar *reg,
                                const u8 *p2p_dev_addr) {
  if (p2p_dev_addr == NULL && wps_registrar_pbc_overlap(reg, NULL, NULL)) {
  ...
  eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
  eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
  eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_registrar_pbc_timeout, reg,
                         NULL);
  return 0;
}
...
void wps_registrar_complete(struct wps_registrar *registrar, const u8 *uuid_e,
                            const u8 *dev_pw, size_t dev_pw_len) {
  if (registrar->pbc) {
    wps_registrar_remove_pbc_session(registrar, uuid_e, NULL);
  ...
  if (dev_pw && wps_registrar_invalidate_wildcard_pin(registrar, dev_pw,
                                                      dev_pw_len) == 0) {
    wpa_hexdump_key(MSG_DEBUG, "WPS: Invalidated wildcard PIN", dev_pw,
                    dev_pw_len);
  }
}
...
void wps_registrar_probe_req_rx(struct wps_registrar *reg, const u8 *addr,
  ...
  wpa_hexdump_buf(MSG_MSGDUMP, "WPS: Probe Request with WPS data received",
                  wps_data);
  ...
  if (reg->enrollee_seen_cb && attr.uuid_e && attr.primary_dev_type &&
      attr.request_type && !p2p_wildcard) {
    char *dev_name = NULL;
    if (attr.dev_name) {
      dev_name = os_zalloc(attr.dev_name_len + 1);
      if (dev_name) {
        os_memcpy(dev_name, attr.dev_name, attr.dev_name_len);
      }
    }
    reg->enrollee_seen_cb(reg->cb_ctx, addr, attr.uuid_e, attr.primary_dev_type,
  ...
  wpa_printf(MSG_DEBUG, "WPS: Probe Request for PBC received from " MACSTR,
             MAC2STR(addr));
  if (attr.uuid_e == NULL) {
...
static void wps_cb_set_sel_reg(struct wps_registrar *reg) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             reg->selected_registrar, reg->wps->config_methods, reg->pbc,
             methods);

  reg->set_sel_reg_cb(reg->cb_ctx, reg->selected_registrar,
                      reg->pbc ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, methods);
}
...
static int wps_set_ie(struct wps_registrar *reg) {
  ...
  if (wps_build_version(beacon) || wps_build_wps_state(reg->wps, beacon) ||
  ...
  if (wps_build_version(probe) || wps_build_wps_state(reg->wps, probe) ||
      ...
      wps_build_resp_type(probe,
                          reg->wps->ap ? WPS_RESP_AP : WPS_RESP_REGISTRAR) ||
  ...
  if (reg->static_wep_only) {
    ...
    const u8 ms_wps[7] = {
        ...
        0x00, 0x50, 0xf2, 5, 0x00 /* no legacy 802.1X or MS WPS */
    };
...
static int wps_get_dev_password(struct wps_data *wps) {
  ...
  if (wps->pbc) {
    ...
    wpa_printf(MSG_DEBUG,
               "WPS: Use OOB Device Password from own NFC Password Token");
  ...
  if (pin == NULL) {
    ...
    wps_cb_pin_needed(wps->wps->registrar, wps->uuid_e, &wps->peer_dev);
    return -1;
  }
...
static int wps_build_r_hash(struct wps_data *wps, struct wpabuf *msg) {
  ...
  wpa_hexdump(MSG_DEBUG, "WPS: R-S2", wps->snonce + WPS_SECRET_NONCE_LEN,
...
static int wps_build_cred_ssid(struct wpabuf *msg,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID for Credential", cred->ssid,
                    cred->ssid_len);
...
static int wps_build_cred_network_key(struct wpabuf *msg,
                                      const struct wps_credential *cred) {
  wpa_printf(MSG_DEBUG, "WPS:  * Network Key (len=%d)", (int)cred->key_len);
  wpa_hexdump_key(MSG_DEBUG, "WPS: Network Key", cred->key, cred->key_len);
...
static int wps_build_credential(struct wpabuf *msg,
                                const struct wps_credential *cred) {
  if (wps_build_cred_network_idx(msg, cred) || wps_build_cred_ssid(msg, cred) ||
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg) {
  ...
  wpa_printf(MSG_DEBUG,
             "WPS: Own encr types 0x%x (rsn: 0x%x, wpa: 0x%x) - masked "
             "Enrollee encr types 0x%x",
  ...
  if (wps->auth_type == WPS_AUTH_WPA2PSK || wps->auth_type == WPS_AUTH_WPAPSK) {
  ...
  if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
    ...
    if (random_pool_ready() != 1 || random_get_bytes(r, sizeof(r)) < 0) {
      wpa_printf(MSG_INFO, "WPS: Could not generate random PSK");
      return -1;
    }
    os_free(wps->new_psk);
    ...
    while (wps->new_psk_len && wps->new_psk[wps->new_psk_len - 1] == '=')
      wps->new_psk_len--;
    wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated passphrase", wps->new_psk,
                          wps->new_psk_len);
    os_memcpy(wps->cred.key, wps->new_psk, wps->new_psk_len);
    wps->cred.key_len = wps->new_psk_len;
  } else if (!wps->wps->registrar->force_per_enrollee_psk && wps->use_psk_key &&
             wps->wps->psk_set) {
    char hex[65];
    ...
    wps->cred.key_len = 32 * 2;
             ...
             wps->wps->network_key) {
    os_memcpy(wps->cred.key, wps->wps->network_key, wps->wps->network_key_len);
    ...
    wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK", wps->new_psk,
                    wps->new_psk_len);
    wpa_snprintf_hex(hex, sizeof(hex), wps->new_psk, wps->new_psk_len);
...
static struct wpabuf *wps_build_m2(struct wps_data *wps) {
  ...
  if (random_get_bytes(wps->nonce_r, WPS_NONCE_LEN) < 0)
    return NULL;
  wpa_hexdump(MSG_DEBUG, "WPS: Registrar Nonce", wps->nonce_r, WPS_NONCE_LEN);
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M2) ||
      wps_build_enrollee_nonce(wps, msg) ||
      wps_build_registrar_nonce(wps, msg) || wps_build_uuid_r(wps, msg) ||
      wps_build_public_key(wps, msg) || wps_derive_keys(wps) ||
  ...
  if (wps->nfc_pw_token && wps->nfc_pw_token->pk_hash_provided_oob &&
    ...
    if (plain == NULL || wps_build_cred(wps, plain) ||
...
static struct wpabuf *wps_build_m2d(struct wps_data *wps) {
  ...
  if (wps->wps->ap && wps->wps->ap_setup_locked && err == WPS_CFG_NO_ERROR)
    err = WPS_CFG_SETUP_LOCKED;

  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M2D) ||
      wps_build_enrollee_nonce(wps, msg) ||
      wps_build_registrar_nonce(wps, msg) || wps_build_uuid_r(wps, msg) ||
      wps_build_auth_type_flags(wps, msg) ||
      wps_build_encr_type_flags(wps, msg) ||
      ...
      wps_build_rf_bands(&wps->wps->dev, msg,
                         wps->wps->rf_band_cb(wps->wps->cb_ctx)) ||
      wps_build_assoc_state(wps, msg) || wps_build_config_error(msg, err) ||
      wps_build_os_version(&wps->wps->dev, msg) ||
      wps_build_wfa_ext(msg, 0, NULL, 0)) {
    wpabuf_free(msg);
    return NULL;
...
static struct wpabuf *wps_build_m4(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M4) ||
      wps_build_enrollee_nonce(wps, msg) || wps_build_r_hash(wps, msg) ||
      wps_build_r_snonce1(wps, plain) || wps_build_key_wrap_auth(wps, plain) ||
      wps_build_encr_settings(wps, msg, plain) ||
      wps_build_wfa_ext(msg, 0, NULL, 0) || wps_build_authenticator(wps, msg)) {
    wpabuf_clear_free(plain);
    wpabuf_free(msg);
    return NULL;
  }
...
static struct wpabuf *wps_build_m6(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M6) ||
      wps_build_enrollee_nonce(wps, msg) || wps_build_r_snonce2(wps, plain) ||
      wps_build_key_wrap_auth(wps, plain) ||
      wps_build_encr_settings(wps, msg, plain) ||
      wps_build_wfa_ext(msg, 0, NULL, 0) || wps_build_authenticator(wps, msg)) {
    wpabuf_clear_free(plain);
...
static struct wpabuf *wps_build_m8(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M8) ||
      wps_build_enrollee_nonce(wps, msg) ||
...
static int wps_process_enrollee_nonce(struct wps_data *wps, const u8 *e_nonce) {
  ...
  wpa_hexdump(MSG_DEBUG, "WPS: Enrollee Nonce", wps->nonce_e, WPS_NONCE_LEN);
...
static int wps_process_e_snonce1(struct wps_data *wps, const u8 *e_snonce1) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce1", e_snonce1, WPS_SECRET_NONCE_LEN);
...
static int wps_process_e_snonce2(struct wps_data *wps, const u8 *e_snonce2) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce2", e_snonce2, WPS_SECRET_NONCE_LEN);
...
static int wps_process_mac_addr(struct wps_data *wps, const u8 *mac_addr) {
  ...
  wpa_printf(MSG_DEBUG, "WPS: Enrollee MAC Address " MACSTR, MAC2STR(mac_addr));
...
static int wps_process_auth_type_flags(struct wps_data *wps, const u8 *auth) {
   ...
   */
  if ((auth_types & (WPS_AUTH_WPA2 | WPS_AUTH_WPA2PSK | WPS_AUTH_WPAPSK)) ==
      (WPS_AUTH_WPA2 | WPS_AUTH_WPAPSK)) {
    wpa_printf(MSG_DEBUG, "WPS: Workaround - assume Enrollee supports WPA2PSK "
                          "based on claimed WPA2 support");
...
static int wps_process_encr_type_flags(struct wps_data *wps, const u8 *encr) {
  ...
  wpa_printf(MSG_DEBUG, "WPS: Enrollee Encryption Type flags 0x%x", encr_types);
...
static int wps_process_conn_type_flags(struct wps_data *wps, const u8 *conn) {
  ...
  wpa_printf(MSG_DEBUG, "WPS: Enrollee Connection Type flags 0x%x", *conn);
...
static int wps_process_wps_state(struct wps_data *wps, const u8 *state) {
  ...
  wpa_printf(MSG_DEBUG, "WPS: Enrollee Wi-Fi Protected Setup State %d", *state);
...
static int wps_registrar_p2p_dev_addr_match(struct wps_data *wps) {
  ...
  if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               ...
               "filtering for PBC: expected " MACSTR " was " MACSTR
               " - indicate PBC session overlap",
               MAC2STR(reg->p2p_dev_addr), MAC2STR(wps->p2p_dev_addr));
    return 0;
  }
#endif /* CONFIG_P2P */
  return 1;
...
static enum wps_process_res wps_process_m1(struct wps_data *wps,
  ...
  if (wps->dev_pw_id < 0x10 && wps->dev_pw_id != DEV_PW_DEFAULT &&
...
#endif /* CONFIG_WPS_NFC */
      (wps->dev_pw_id != DEV_PW_PUSHBUTTON || !wps->wps->registrar->pbc)) {
  ...
  if (wps->dev_pw_id >= 0x10 ||
    ...
    wpa_printf(
        MSG_DEBUG,
        "WPS: Searching for NFC token match for id=%d (ctx %p registrar %p)",
        wps->dev_pw_id, wps->wps, wps->wps->registrar);
    token = wps_get_nfc_pw_token(&wps->wps->registrar->nfc_pw_tokens,
                                 wps->dev_pw_id);
    if (token && token->peer_pk_hash_known) {
      ...
      if (os_memcmp_const(hash, wps->nfc_pw_token->pubkey_hash,
  ...
  if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
    if ((wps->wps->registrar->force_pbc_overlap ||
         wps_registrar_pbc_overlap(wps->wps->registrar, wps->mac_addr_e,
                                   wps->uuid_e) ||
      ...
      wps_fail_event(wps->wps, WPS_M1, WPS_CFG_MULTIPLE_PBC_DETECTED,
                     WPS_EI_NO_ERROR, wps->mac_addr_e);
      wps->wps->registrar->force_pbc_overlap = 1;
      return WPS_CONTINUE;
    }
    wps_registrar_add_pbc_session(wps->wps->registrar, wps->mac_addr_e,
                                  wps->uuid_e);
    wps->pbc = 1;
  }
   ...
   * used.
   */
  if (!wps->use_psk_key && wps->peer_dev.manufacturer &&
...
static int wps_process_ap_settings_r(struct wps_data *wps,
  ...
  if (wps->new_ap_settings) {
    ...
    if (wps->ap_settings_cb) {
      wps->ap_settings_cb(wps->ap_settings_cb_ctx, &wps->cred);
      wpabuf_free(msg);
      return 1;
    }
    wps_sta_cred_cb(wps);
...
static enum wps_process_res wps_process_wsc_msg(struct wps_data *wps,
  ...
  case WPS_M1:
    ...
    if (wps->wps->wps_upnp && attr.mac_addr) {
      ...
      upnp_wps_device_send_wlan_event(wps->wps->wps_upnp, attr.mac_addr,
  ...
  case WPS_M3:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M3, wps->config_error, wps->error_indication,
                     wps->mac_addr_e);
    break;
  case WPS_M5:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M5, wps->config_error, wps->error_indication,
                     wps->mac_addr_e);
    break;
  case WPS_M7:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M7, wps->config_error, wps->error_indication,
                     wps->mac_addr_e);
    break;
  default:
...
static enum wps_process_res wps_process_wsc_ack(struct wps_data *wps,
  ...
  if (*attr.msg_type != WPS_WSC_ACK) {
    wpa_printf(MSG_DEBUG, "WPS: Invalid Message Type %d", *attr.msg_type);
    return WPS_FAILURE;
  }
...
#ifdef CONFIG_WPS_UPNP
    if (wps->wps->wps_upnp && upnp_wps_subscribers(wps->wps->wps_upnp)) {
      if (wps->wps->upnp_msgs)
        return WPS_CONTINUE;
      if (wps->ext_reg == 0)
        wps->ext_reg = 1;
...
static enum wps_process_res wps_process_wsc_nack(struct wps_data *wps,
  ...
  switch (old_state) {
  case RECV_M3:
    wps_fail_event(wps->wps, WPS_M2, config_error, wps->error_indication,
                   wps->mac_addr_e);
    break;
  case RECV_M5:
    wps_fail_event(wps->wps, WPS_M4, config_error, wps->error_indication,
                   wps->mac_addr_e);
    break;
  case RECV_M7:
    wps_fail_event(wps->wps, WPS_M6, config_error, wps->error_indication,
                   wps->mac_addr_e);
    break;
  case RECV_DONE:
    wps_fail_event(wps->wps, WPS_M8, config_error, wps->error_indication,
                   wps->mac_addr_e);
    break;
  default:
    break;
...
static enum wps_process_res wps_process_wsc_done(struct wps_data *wps,
  ...
  if (wps->wps->wps_upnp && wps->ext_reg) {
    ...
    wps_device_store(wps->wps->registrar, &wps->peer_dev, wps->uuid_e);
    return WPS_DONE;
  }
#endif /* CONFIG_WPS_UPNP */
  ...
  if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated random passphrase",
  ...
  if (wps->new_psk) {
    if (wps_cb_new_psk(wps->wps->registrar, wps->mac_addr_e, wps->p2p_dev_addr,
                       wps->new_psk, wps->new_psk_len)) {
  ...
  if (wps->pbc) {
    wps_registrar_remove_pbc_session(wps->wps->registrar, wps->uuid_e,
                                     wps->p2p_dev_addr);
    wps_registrar_pbc_completed(wps->wps->registrar);
#ifdef WPS_WORKAROUNDS
    os_get_reltime(&wps->wps->registrar->pbc_ignore_start);
#endif /* WPS_WORKAROUNDS */
    os_memcpy(wps->wps->registrar->pbc_ignore_uuid, wps->uuid_e, WPS_UUID_LEN);
  } else {
    wps_registrar_pin_completed(wps->wps->registrar);
  }
...
enum wps_process_res wps_registrar_process_msg(struct wps_data *wps,
  ...
  if (wps->ext_reg > 1)
    wps_registrar_free_pending_m2(wps->wps);
  if (wps->wps->wps_upnp && wps->ext_reg && wps->wps->upnp_msgs == NULL &&
    ...
    wpa_printf(MSG_DEBUG,
               ...
               type);
    upnp_wps_device_send_wlan_event(wps->wps->wps_upnp, wps->mac_addr_e,
                                    UPNP_WPS_WLANEVENT_TYPE_EAP, msg);
    if (op_code == WSC_MSG)
      return WPS_PENDING;
  ...
  case WSC_Done:
    ...
    if (ret == WPS_FAILURE) {
      wps->state = SEND_WSC_NACK;
      wps_fail_event(wps->wps, WPS_WSC_DONE, wps->config_error,
...
static void wps_registrar_sel_reg_add(struct wps_registrar *reg,
  ...
  for (j = 0; i < WPS_MAX_AUTHORIZED_MACS && j < WPS_MAX_AUTHORIZED_MACS; j++) {
    if (is_zero_ether_addr(s->authorized_macs[j]))
      break;
    wpa_printf(MSG_DEBUG, "WPS: Add authorized MAC into union: " MACSTR,
               MAC2STR(s->authorized_macs[j]));
    os_memcpy(reg->authorized_macs_union[i], s->authorized_macs[j], ETH_ALEN);
    i++;
  }
...
static void wps_registrar_sel_reg_union(struct wps_registrar *reg) {
  ...
  dl_list_for_each(s, &reg->wps->wps_upnp->subscriptions, struct subscription,
                   list) {
    struct subscr_addr *sa;
    sa = dl_list_first(&s->addr_list, struct subscr_addr, list);
    if (sa) {
      wpa_printf(MSG_DEBUG, "WPS: External Registrar %s:%d",
                 inet_ntoa(sa->saddr.sin_addr), ntohs(sa->saddr.sin_port));
    }
...
int wps_registrar_get_info(struct wps_registrar *reg, const u8 *addr, char *buf,
                           size_t buflen) {
  struct wps_registrar_device *d;
  int len = 0, ret;
  char uuid[40];
  ...
  ret = os_snprintf(
      ...
      "wpsModelNumber=%s\n"
      "wpsSerialNumber=%s\n",
      uuid, wps_dev_type_bin2str(d->dev.pri_dev_type, devtype, sizeof(devtype)),
...
int wps_registrar_add_nfc_pw_token(struct wps_registrar *reg,
  ...
  if (dev_pw) {
    wpa_snprintf_hex_uppercase((char *)token->dev_pw, sizeof(token->dev_pw),
                               dev_pw, dev_pw_len);
...
int wps_registrar_add_nfc_password_token(struct wps_registrar *reg,
  ...
  if (oob_dev_pw_len < WPS_OOB_PUBKEY_HASH_LEN + 2 ||
      oob_dev_pw_len >
          WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN)
    return -1;
  ...
  wpa_printf(MSG_DEBUG, "WPS: Add NFC Password Token for Password ID %u", id);

  wpa_hexdump(MSG_DEBUG, "WPS: Public Key Hash", hash, WPS_OOB_PUBKEY_HASH_LEN);
  wpa_hexdump_key(MSG_DEBUG, "WPS: Device Password", dev_pw, dev_pw_len);

  return wps_registrar_add_nfc_pw_token(reg, hash, id, dev_pw, dev_pw_len, 0);
}
