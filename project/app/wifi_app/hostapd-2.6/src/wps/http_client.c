...
static void http_client_got_response(struct httpread *handle, void *cookie,
  ...
  wpa_printf(MSG_DEBUG,
             "HTTP: httpread callback: handle=%p cookie=%p "
             "e=%d",
             handle, cookie, e);
    ...
    if (httpread_hdr_type_get(c->hread) == HTTPREAD_HDR_TYPE_REPLY) {
      ...
      if (reply_code == 200 /* OK */) {
        ...
        wpa_printf(MSG_DEBUG,
                   "HTTP: Error %d from "
                   "%s:%d",
                   reply_code, inet_ntoa(c->dst.sin_addr),
...
static void http_client_tx_ready(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "bytes remaining)",
             inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port),
             (unsigned long)wpabuf_len(c->req), (unsigned long)send_len);

  res = send(c->sd, wpabuf_head_u8(c->req) + c->req_pos, send_len, 0);
  if (res < 0) {
    wpa_printf(MSG_DEBUG, "HTTP: Failed to send buffer: %s", strerror(errno));
...
struct http_client *http_client_addr(
    struct sockaddr_in *dst, struct wpabuf *req, size_t max_response,
    void (*cb)(void *ctx, struct http_client *c, enum http_client_event event),
    void *cb_ctx) {
  ...
  if (connect(c->sd, (struct sockaddr *)dst, sizeof(*dst))) {
    if (errno != EINPROGRESS) {
      wpa_printf(MSG_DEBUG, "HTTP: Failed to connect: %s", strerror(errno));
      goto fail;
    }
  ...
  if (eloop_register_sock(c->sd, EVENT_TYPE_WRITE, http_client_tx_ready, c,
                          NULL) ||
      eloop_register_timeout(HTTP_CLIENT_TIMEOUT_SEC, 0, http_client_timeout, c,
                             NULL))
    goto fail;
...
struct http_client *http_client_url(const char *url, struct wpabuf *req,
                                    size_t max_response,
...
char *http_link_update(char *url, const char *base) {
  ...
  if (url[0] == '/') {
    ...
    if (pos == NULL) {
      ...
      os_memcpy(n + (pos - base) + 1, url, os_strlen(url) + 1);
    }
  }
