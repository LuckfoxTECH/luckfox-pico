...
static struct wps_er_sta *wps_er_sta_get(struct wps_er_ap *ap, const u8 *addr,
  ...
  dl_list_for_each(sta, &ap->sta, struct wps_er_sta, list) {
    if ((addr == NULL || os_memcmp(sta->addr, addr, ETH_ALEN) == 0) &&
        (uuid == NULL || os_memcmp(uuid, sta->uuid, WPS_UUID_LEN) == 0))
      return sta;
  }
  return NULL;
}
...
static struct wps_er_ap *wps_er_ap_get(struct wps_er *er, struct in_addr *addr,
                                       const u8 *uuid, const u8 *mac_addr) {
  struct wps_er_ap *ap;
  dl_list_for_each(ap, &er->ap, struct wps_er_ap, list) {
    if ((addr == NULL || ap->addr.s_addr == addr->s_addr) &&
        (uuid == NULL || os_memcmp(uuid, ap->uuid, WPS_UUID_LEN) == 0) &&
        (mac_addr == NULL || os_memcmp(mac_addr, ap->mac_addr, ETH_ALEN) == 0))
      return ap;
  }
  return NULL;
}
...
static void wps_er_ap_unsubscribe(struct wps_er *er, struct wps_er_ap *ap) {
  ...
  ap->http = http_client_addr(&dst, req, 1000, wps_er_http_unsubscribe_cb, ap);
  if (ap->http == NULL) {
    wpabuf_free(req);
    goto fail;
  }
...
static struct wps_er_ap_settings *wps_er_ap_get_settings(struct wps_er *er,
  ...
  dl_list_for_each(s, &er->ap_settings, struct wps_er_ap_settings,
                   list) if (os_memcmp(uuid, s->uuid, WPS_UUID_LEN) ==
                             0) return s;
  return NULL;
}
...
static int wps_er_get_sid(struct wps_er_ap *ap, char *sid) {
  ...
  if (!pos) {
    wpa_printf(MSG_DEBUG,
               ...
               "%s (%s): '%s'",
               inet_ntoa(ap->addr), ap->location, sid);
    return -1;
  }
...
static void wps_er_subscribe(struct wps_er_ap *ap) {
  ...
  wpabuf_printf(req,
                ...
                ap->er->ip_addr_text, ap->er->http_port, ap->er->event_id,
                ap->id, 1800);
  os_free(url);
  wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Subscription request",
                    wpabuf_head(req), wpabuf_len(req));

  ap->http = http_client_addr(&dst, req, 1000, wps_er_http_subscribe_cb, ap);
  if (ap->http == NULL)
    wpabuf_free(req);
}
  ...
  while (!found) {
    ...
    for (;;) {
      if (xml_next_tag(data, &tag, &tagname, &end))
        return NULL;
      data = end;
      if (!os_strncasecmp(tagname, "device", 6) && *tag != '/' &&
...
static void wps_er_parse_device_description(struct wps_er_ap *ap,
  ...
  wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Device info", wpabuf_head(reply),
                    wpabuf_len(reply));
  ...
  wpa_printf(MSG_DEBUG, "WPS ER: manufacturerURL='%s'", ap->manufacturer_url);

  ap->model_description = xml_get_first_item(data, "modelDescription");
  wpa_printf(MSG_DEBUG, "WPS ER: modelDescription='%s'", ap->model_description);
  ...
  ap->scpd_url =
      http_link_update(xml_get_first_item(data, "SCPDURL"), ap->location);
  wpa_printf(MSG_DEBUG, "WPS ER: SCPDURL='%s'", ap->scpd_url);

  ap->control_url =
      http_link_update(xml_get_first_item(data, "controlURL"), ap->location);
  wpa_printf(MSG_DEBUG, "WPS ER: controlURL='%s'", ap->control_url);

  ap->event_sub_url =
      http_link_update(xml_get_first_item(data, "eventSubURL"), ap->location);
...
void wps_er_ap_add(struct wps_er *er, const u8 *uuid, struct in_addr *addr,
  ...
  ap->http =
      http_client_url(ap->location, NULL, 10000, wps_er_http_dev_desc_cb, ap);
}
...
static void wps_er_ap_remove_all(struct wps_er *er) {
  ...
  dl_list_for_each_safe(ap, prev, &er->ap, struct wps_er_ap, list)
      wps_er_ap_remove_entry(er, ap);
  dl_list_for_each_safe(s, prev_s, &er->ap_settings, struct wps_er_ap_settings,
                        list) os_free(s);
}
...
static void wps_er_http_resp_not_found(struct http_request *req) {
  ...
  wpabuf_put_str(buf, "HTTP/1.1 404 Not Found\r\n"
...
static void wps_er_http_resp_ok(struct http_request *req) {
  ...
  wpabuf_put_str(buf, "HTTP/1.1 200 OK\r\n"
                      "Server: unspecified, UPnP/1.0, unspecified\r\n"
                      "Connection: close\r\n"
                      "Content-Length: 0\r\n");
  http_put_date(buf);
...
static struct wps_er_sta *wps_er_add_sta_data(struct wps_er_ap *ap,
  ...
  if (attr->manufacturer) {
    os_free(sta->manufacturer);
    sta->manufacturer = dup_binstr(attr->manufacturer, attr->manufacturer_len);
  }

  if (attr->model_name) {
    os_free(sta->model_name);
    sta->model_name = dup_binstr(attr->model_name, attr->model_name_len);
  }

  if (attr->model_number) {
    os_free(sta->model_number);
    sta->model_number = dup_binstr(attr->model_number, attr->model_number_len);
  }

  if (attr->serial_number) {
    os_free(sta->serial_number);
    sta->serial_number =
        dup_binstr(attr->serial_number, attr->serial_number_len);
  }
...
static void wps_er_process_wlanevent_probe_req(struct wps_er_ap *ap,
  ...
  wpa_printf(MSG_DEBUG, "WPS ER: WLANEvent - Probe Request - from " MACSTR,
             MAC2STR(addr));
  ...
  if (wps_validate_probe_req(msg, addr) < 0) {
    wpa_printf(MSG_INFO,
               ...
               "Probe Request frame from " MACSTR,
               MAC2STR(addr));
    return;
  }
...
static struct wpabuf *wps_er_soap_hdr(const struct wpabuf *msg,
  ...
  if (msg) {
    encoded = base64_encode(wpabuf_head(msg), wpabuf_len(msg), &encoded_len);
    if (encoded == NULL)
      return NULL;
  } else {
    encoded = NULL;
...
static void wps_er_soap_end(struct wpabuf *buf, const char *name, char *len_ptr,
                            char *body_ptr) {
  char len_buf[10];
...
static void wps_er_sta_send_msg(struct wps_er_sta *sta, struct wpabuf *msg) {
  ...
  sta->http =
      http_client_addr(&dst, buf, 1000, wps_er_http_put_wlan_response_cb, sta);
  if (sta->http == NULL)
    wpabuf_free(buf);
}
...
static void wps_er_sta_process(struct wps_er_sta *sta, struct wpabuf *msg,
  ...
  if (res == WPS_CONTINUE) {
    ...
    if (res == WPS_DONE) {
      ...
      eloop_cancel_timeout(wps_er_sta_timeout, sta, NULL);
      eloop_register_timeout(10, 0, wps_er_sta_timeout, sta, NULL);
    }
  }
}
...
static void wps_er_sta_start(struct wps_er_sta *sta, struct wpabuf *msg) {
  ...
  if (sta->ap->ap_settings) {
    ...
    if (sta->cred) {
      os_memcpy(sta->cred, sta->ap->ap_settings, sizeof(*sta->cred));
...
static void wps_er_process_wlanevent(struct wps_er_ap *ap,
  ...
  wpa_hexdump(MSG_MSGDUMP, "WPS ER: Received WLANEvent", wpabuf_head(event),
              wpabuf_len(event));
  ...
  case 2:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "WPS ER: Unknown WLANEventType %d", wlan_event_type);
    break;
  }
}
...
static void wps_er_http_event(struct wps_er *er, struct http_request *req,
  ...
  wpa_printf(MSG_MSGDUMP, "WPS ER: HTTP event from AP id %u: %s", ap_id,
             http_request_get_data(req));

  event = xml_get_base64_item(http_request_get_data(req), "WLANEvent", &ret);
  if (event == NULL) {
...
static void wps_er_http_notify(struct wps_er *er, struct http_request *req) {
  ...
  if (os_strncmp(uri, "/event/", 7) == 0) {
    ...
    wpa_printf(MSG_DEBUG, "WPS ER: Unknown HTTP NOTIFY for '%s'", uri);
    wps_er_http_resp_not_found(req);
  }
}
...
static void wps_er_http_req(void *ctx, struct http_request *req) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "%s:%d",
             http_request_get_uri(req), type, inet_ntoa(cli->sin_addr),
             ntohs(cli->sin_port));
  ...
  default:
    ...
    wpabuf_put_str(buf, "HTTP/1.1 501 Unimplemented\r\n"
                        "Connection: close\r\n");
    http_put_date(buf);
    wpabuf_put_str(buf, "\r\n");
    http_request_send_and_deinit(req, buf);
...
struct wps_er *wps_er_init(struct wps_context *wps, const char *ifname,
  ...
  er->wps = wps;
  if (os_get_random((unsigned char *)&er->event_id, sizeof(er->event_id)) < 0) {
  ...
  if (filter && os_strncmp(filter, "ifname=", 7) == 0) {
    ...
    if (end) {
      size_t len = end - pos;
      os_strlcpy(er->ifname, pos,
                 len < sizeof(er->ifname) ? len + 1 : sizeof(er->ifname));
      filter = end + 1;
    } else {
  ...
  wpa_printf(MSG_DEBUG, "WPS ER: Start (ifname=%s ip_addr=%s)", er->ifname,
             er->ip_addr_text);
...
static void wps_er_deinit_finish(void *eloop_data, void *user_ctx) {
  ...
  dl_list_for_each_safe(ap, tmp, &er->ap_unsubscribing, struct wps_er_ap,
    ...
    wpa_printf(
        MSG_DEBUG,
        "WPS ER: AP entry for %s (%s) still in ap_unsubscribing list - free it",
        inet_ntoa(ap->addr), ap->location);
    dl_list_del(&ap->list);
    wps_er_ap_free(ap);
  }
...
static void wps_er_http_set_sel_reg_cb(void *ctx, struct http_client *c,
  ...
  if (data.set_sel_reg.uuid)
    ap->er->wps->event_cb(ap->er->wps->cb_ctx, WPS_EV_ER_SET_SELECTED_REGISTRAR,
                          &data);
}
...
static void wps_er_send_set_sel_reg(struct wps_er_ap *ap, struct wpabuf *msg) {
  ...
  buf = wps_er_soap_hdr(msg, "SetSelectedRegistrar", "NewMessage", path, &dst,
                        &len_ptr, &body_ptr);
  os_free(url);
  if (buf == NULL)
    return;
  ...
  ap->http = http_client_addr(&dst, buf, 1000, wps_er_http_set_sel_reg_cb, ap);
  if (ap->http == NULL)
    wpabuf_free(buf);
}
...
void wps_er_set_sel_reg(struct wps_er *er, int sel_reg, u16 dev_passwd_id,
  ...
  if (wps_build_version(msg) || wps_er_build_selected_registrar(msg, sel_reg) ||
  ...
  dl_list_for_each(ap, &er->ap, struct wps_er_ap, list) {
    if (er->set_sel_reg_uuid_filter &&
        os_memcmp(ap->uuid, er->set_sel_reg_uuid_filter, WPS_UUID_LEN) != 0)
      continue;
    data.set_sel_reg.uuid = ap->uuid;
    er->wps->event_cb(er->wps->cb_ctx, WPS_EV_ER_SET_SELECTED_REGISTRAR, &data);
...
static void wps_er_ap_settings_cb(void *ctx,
  ...
  ap->er->wps->event_cb(ap->er->wps->cb_ctx, WPS_EV_ER_AP_SETTINGS, &data);
}
...
static void wps_er_ap_put_message(struct wps_er_ap *ap,
  ...
  buf = wps_er_soap_hdr(msg, "PutMessage", "NewInMessage", path, &dst, &len_ptr,
                        &body_ptr);
  os_free(url);
  if (buf == NULL)
    goto fail;
  ...
  ap->http = http_client_addr(&dst, buf, 10000, wps_er_http_put_message_cb, ap);
  if (ap->http == NULL) {
    wpabuf_free(buf);
    goto fail;
  }
...
static int wps_er_send_get_device_info(struct wps_er_ap *ap,
  ...
  buf = wps_er_soap_hdr(NULL, "GetDeviceInfo", NULL, path, &dst, &len_ptr,
                        &body_ptr);
  os_free(url);
  if (buf == NULL)
    return -1;
  ...
  ap->http =
      http_client_addr(&dst, buf, 10000, wps_er_http_get_dev_info_cb, ap);
  if (ap->http == NULL) {
    wpabuf_free(buf);
    return -1;
  }
...
struct wpabuf *wps_er_config_token_from_cred(struct wps_context *wps,
  ...
  data.wps = wps;
  data.use_cred = cred;
  if (wps_build_cred(&data, ret) || wps_build_wfa_ext(ret, 0, NULL, 0)) {
    wpabuf_free(ret);
    return NULL;
  }
