...
static int httpread_hdr_option_analyze(
    struct httpread *h, char *hbp /* pointer to current line in header buffer */
) {
  if (word_eq(hbp, "CONTENT-LENGTH:")) {
    ...
    if (h->content_length < 0 || h->content_length > h->max_bytes) {
      wpa_printf(MSG_DEBUG, "httpread: Unacceptable Content-Length %d",
                 h->content_length);
      return -1;
    }
...
static void httpread_read_handler(int sd, void *eloop_ctx, void *sock_ctx) {
  ...
  if (nread == 0) {
    ...
    if (h->chunked || h->got_content_length) {
      ...
      wpa_printf(MSG_DEBUG, "httpread premature eof(%p) %d/%d", h,
  ...
  if (!os_strncasecmp(h->hdr, "SUBSCRIBE", 9) ||
      ...
      !os_strncasecmp(h->hdr, "HEAD", 4) || !os_strncasecmp(h->hdr, "GET", 3)) {
    if (!h->got_body) {
  ...
  if (!h->got_body) {
    ...
    if (h->body_alloc_nbytes < (h->body_nbytes + nread + 1)) {
      ...
      if (h->body_nbytes >= h->max_bytes) {
        wpa_printf(MSG_DEBUG, "httpread: body_nbytes=%d >= max_bytes=%d",
      ...
      if (new_alloc_nbytes < h->body_alloc_nbytes ||
          new_alloc_nbytes > h->max_bytes + HTTPREAD_BODYBUF_DELTA) {
        wpa_printf(MSG_DEBUG,
                   "httpread: Unacceptable body length %d "
                   "(body_alloc_nbytes=%u max_bytes=%u)",
                   new_alloc_nbytes, h->body_alloc_nbytes, h->max_bytes);
        goto bad;
      }
      if ((new_body = os_realloc(h->body, new_alloc_nbytes)) == NULL) {
        wpa_printf(MSG_DEBUG, "httpread: Failed to reallocate buffer (len=%d)",
                   new_alloc_nbytes);
        goto bad;
      }
      ...
      if (h->chunked && h->in_chunk_data == 0) {
        ...
        if (bbp - cbp >= 2 && bbp[-2] == '\r' && bbp[-1] == '\n') {
          ...
          if (h->chunk_size < 0 || h->chunk_size > h->max_bytes) {
            wpa_printf(MSG_DEBUG, "httpread: Invalid chunk size %d",
                       h->chunk_size);
            goto bad;
          }
...
got_file:
  wpa_printf(MSG_DEBUG, "httpread got file %d bytes type %d", h->body_nbytes,
             h->hdr_type);
 ...
 */
struct httpread *
httpread_create(int sd, /* descriptor of TCP socket to read from */
                void (*cb)(struct httpread *handle, void *cookie,
                           enum httpread_event e), /* call on event */
                void *cookie,                      /* pass to callback */
  ...
  if (timeout_seconds > 0 &&
      eloop_register_timeout(timeout_seconds, 0, httpread_timeout_handler, NULL,
                             h)) {
    /* No way to recover (from malloc failure) */
    goto fail;
  }
  if (eloop_register_sock(sd, EVENT_TYPE_READ, httpread_read_handler, NULL,
                          h)) {
    /* No way to recover (from malloc failure) */
    goto fail;
  }
