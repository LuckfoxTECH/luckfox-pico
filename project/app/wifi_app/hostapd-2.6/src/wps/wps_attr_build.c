    ...
    pubkey = wpabuf_dup(wps->wps->dh_pubkey);
             ...
             (wps->wps->ap ||
              (wps->wps->ap_nfc_dh_pubkey &&
               wps->wps->ap_nfc_dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER &&
               wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)) &&
...
int wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg) {
  ...
  if (wps_force_auth_types_in_use) {
    wpa_printf(MSG_DEBUG, "WPS: Testing - replace auth type 0x%x with 0x%x",
               auth_types, wps_force_auth_types);
    auth_types = wps_force_auth_types;
  }
#endif /* CONFIG_WPS_TESTING */
  wpa_printf(MSG_DEBUG, "WPS:  * Authentication Type Flags (0x%x)", auth_types);
...
int wps_build_encr_type_flags(struct wps_data *wps, struct wpabuf *msg) {
  ...
  if (wps_force_encr_types_in_use) {
    wpa_printf(MSG_DEBUG, "WPS: Testing - replace encr type 0x%x with 0x%x",
               encr_types, wps_force_encr_types);
    encr_types = wps_force_encr_types;
  }
#endif /* CONFIG_WPS_TESTING */
  wpa_printf(MSG_DEBUG, "WPS:  * Encryption Type Flags (0x%x)", encr_types);
...
int wps_build_key_wrap_auth(struct wps_data *wps, struct wpabuf *msg) {
  ...
  hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, wpabuf_head(msg), wpabuf_len(msg),
              hash);
...
int wps_build_oob_dev_pw(struct wpabuf *msg, u16 dev_pw_id,
  ...
  if (wps_corrupt_pkhash) {
    wpa_hexdump(MSG_DEBUG, "WPS: Real Public Key Hash", pubkey_hash,
  ...
  wpa_hexdump(MSG_DEBUG, "WPS: Public Key Hash", pubkey_hash,
              WPS_OOB_PUBKEY_HASH_LEN);
