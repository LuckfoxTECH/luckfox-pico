...
static xml_node_t *get_node_uri_iter(struct xml_node_ctx *ctx, xml_node_t *root,
                                     char *uri) {
  char *end;
  xml_node_t *node;
  const char *name;
  ...
  if (end) {
    return get_node_uri_iter(ctx, xml_node_first_child(ctx, node), end);
  }
...
static xml_node_t *get_node_iter(struct xml_node_ctx *ctx, xml_node_t *root,
                                 const char *path) {
  char *end;
  xml_node_t *node;
  const char *name;
...
int node_to_file(struct xml_node_ctx *ctx, const char *fname,
                 xml_node_t *node) {
  FILE *f;
  char *str;
...
xml_node_t *mo_to_tnds(struct xml_node_ctx *ctx, xml_node_t *mo, int use_path,
                       const char *urn, const char *ns_uri) {
  xml_node_t *root;
  xml_node_t *node;
  const char *name;
...
static xml_node_t *get_first_child_node(struct xml_node_ctx *ctx,
                                        xml_node_t *node, const char *name) {
  const char *lname;
  xml_node_t *child;
...
static xml_node_t *add_mo_node(struct xml_node_ctx *ctx, xml_node_t *root,
  ...
  if (root == NULL) {
    root = xml_node_create_root(ctx, NULL, NULL, NULL, nodename);
    if (root && value)
    ...
    if (value)
      xml_node_create_text(ctx, parent, NULL, nodename, value);
    else
...
xml_node_t *soap_build_envelope(struct xml_node_ctx *ctx, xml_node_t *node) {
  ...
  envelope =
      xml_node_create_root(ctx, "http://www.w3.org/2003/05/soap-envelope",
                           "soap12", &ns, "Envelope");
  if (envelope == NULL)
    return NULL;
