...
static void history_prev(void) {
  ...
  history_curr =
      dl_list_entry(history_curr->list.next, struct edit_history, list);
  history_use();
}

static void history_next(void) {
  if (history_curr == NULL ||
      history_curr == dl_list_first(&history_list, struct edit_history, list)) {
    if (currbuf_valid) {
      currbuf_valid = 0;
      edit_clear_line();
  ...
  history_curr =
      dl_list_entry(history_curr->list.prev, struct edit_history, list);
  history_use();
}
...
int edit_init(void (*cmd_cb)(void *ctx, char *cmd), void (*eof_cb)(void *ctx),
