...
static struct dl_list active_references = {&active_references,
                                           &active_references};
...
static void find_addr_sect(bfd *abfd, asection *section, void *obj) {
  ...
  data->found =
      bfd_find_nearest_line(abfd, section, syms, data->pc - vma,
...
static void wpa_trace_bfd_addr(void *pc) {
  ...
  do {
    ...
    data.found =
        bfd_find_inliner_info(abfd, &data.filename, &data.function, &data.line);
  } while (data.found);
}
...
size_t wpa_trace_calling_func(const char *buf[], size_t len) {
  ...
  for (i = 0; i < btrace_num; i++) {
    ...
    while (data.found) {
      if (data.function &&
          (pos > 0 || os_strcmp(data.function, __func__) != 0)) {
        buf[pos++] = data.function;
        if (pos == len)
          return pos;
      }

      data.found = bfd_find_inliner_info(abfd, &data.filename, &data.function,
                                         &data.line);
    }
  }
...
#define wpa_trace_bfd_init()                                                   \
  do {                                                                         \
  } while (0)
#define wpa_trace_bfd_addr(pc)                                                 \
  do {                                                                         \
  } while (0)
