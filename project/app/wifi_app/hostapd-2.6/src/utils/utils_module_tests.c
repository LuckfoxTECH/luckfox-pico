...
static const struct printf_test_data printf_tests[] = {
    ...
    {(u8 *)"\xe5\xe4\xf6\xc5\xc4\xd6", 6, "\\xe5\\xe4\\xf6\\xc5\\xc4\\xd6"},
    {NULL, 0, NULL}};
...
static int base64_tests(void) {
  ...
  if (!res || res_len != 5 || res[0] != 'P' || res[1] != 'Q' || res[2] != '=' ||
      res[3] != '=' || res[4] != '\n')
    errors++;
  os_free(res);

  res = base64_encode((const unsigned char *)"=", 1, NULL);
  if (!res || res[0] != 'P' || res[1] != 'Q' || res[2] != '=' ||
      res[3] != '=' || res[4] != '\n')
    errors++;
  os_free(res);
...
static int ip_addr_tests(void) {
  ...
  if (hostapd_parse_ip_addr("1.2.3.4", &addr) != 0 || addr.af != AF_INET ||
  ...
  if (hostapd_parse_ip_addr("::", &addr) != 0 || addr.af != AF_INET6 ||
      hostapd_ip_txt(&addr, buf, 1) != buf || buf[0] != '\0' ||
      hostapd_ip_txt(&addr, buf, sizeof(buf)) != buf)
    errors++;
...
static void eloop_test_read_2(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  if (t->magic != 0x12345678) {
    wpa_printf(MSG_INFO, "%s: unexpected magic 0x%x", __func__, t->magic);
  }

  if (t->pipefd2[0] != sock) {
    wpa_printf(MSG_INFO, "%s: unexpected sock %d != %d", __func__, sock,
               t->pipefd2[0]);
  }

  res = read(sock, buf, sizeof(buf));
  wpa_printf(MSG_INFO, "%s: sock=%d --> res=%d", __func__, sock, (int)res);
}
...
static void reopen_pipefd2(struct test_eloop *t) {
  if (t->pipefd2[0] < 0) {
    ...
    wpa_printf(MSG_INFO, "re-register pipefd2 with new sockets %d,%d",
               t->pipefd2[0], t->pipefd2[1]);
    eloop_register_read_sock(t->pipefd2[0], eloop_test_read_2_wrong, t, NULL);
  }
}
...
static void eloop_tests_start(int close_in_timeout) {
  ...
  wpa_printf(MSG_INFO, "starting eloop tests (%p) (close_in_timeout=%d)", t,
             close_in_timeout);
  ...
  wpa_printf(MSG_INFO, "pipe fds: %d,%d %d,%d", t->pipefd1[0], t->pipefd1[1],
             t->pipefd2[0], t->pipefd2[1]);
...
int utils_module_tests(void) {
  ...
  if (printf_encode_decode_tests() < 0 || ext_password_tests() < 0 ||
      trace_tests() < 0 || bitfield_tests() < 0 || base64_tests() < 0 ||
      common_tests() < 0 || os_tests() < 0 || wpabuf_tests() < 0 ||
      ip_addr_tests() < 0 || eloop_tests() < 0 || int_array_tests() < 0)
    ret = -1;
