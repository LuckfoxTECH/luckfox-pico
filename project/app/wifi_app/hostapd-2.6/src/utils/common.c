...
/**
 * hwaddr_masked_aton - Convert ASCII string with optional mask to MAC address
 * (colon-delimited format)
 * @txt: MAC address with optional mask as a string (e.g.,
 * "00:11:22:33:44:55/ff:ff:ff:ff:00:00")
 * @addr: Buffer for the MAC address (ETH_ALEN = 6 bytes)
...
/**
 * hwaddr_compact_aton - Convert ASCII string to MAC address (no colon
 * delimitors format)
...
int hwaddr_mask_txt(char *buf, size_t len, const u8 *addr, const u8 *mask) {
  ...
  if (print_mask)
    res =
        os_snprintf(buf, len, MACSTR "/" MACSTR, MAC2STR(addr), MAC2STR(mask));
  else
...
char *freq_range_list_str(const struct wpa_freq_range_list *list) {
  ...
  for (i = 0; i < list->num; i++) {
    ...
    if (range->min == range->max)
      ...
      res = os_snprintf(pos, end - pos, "%s%u-%u", i == 0 ? "" : ",",
                        range->min, range->max);
...
size_t utf8_unescape(const char *inp, size_t in_size, char *outp,
                     size_t out_size) {
  size_t res_size = 0;
...
size_t utf8_escape(const char *inp, size_t in_size, char *outp,
                   size_t out_size) {
  size_t res_size = 0;
...
int is_ctrl_char(char c) { return c > 0 && c < 32; }
