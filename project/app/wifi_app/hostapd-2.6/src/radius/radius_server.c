...
radius_server_get_new_session(struct radius_server_data *data,
  ...
  sess->eap = eap_server_sm_init(sess, &radius_server_eapol_cb, &eap_conf);
  if (sess->eap == NULL) {
...
send_reply:
  ...
  if (is_complete) {
    RADIUS_DEBUG("Removing completed session 0x%x after timeout",
                 sess->sess_id);
    eloop_cancel_timeout(radius_server_session_remove_timeout, data, sess);
    eloop_register_timeout(10, 0, radius_server_session_remove_timeout, data,
                           sess);
  }
...
struct radius_server_data *radius_server_init(struct radius_server_conf *conf) {
  ...
  if (conf->pac_opaque_encr_key) {
    ...
    if (data->pac_opaque_encr_key) {
      os_memcpy(data->pac_opaque_encr_key, conf->pac_opaque_encr_key, 16);
    }
  }
  ...
  if (conf->acct_port) {
    ...
    if (eloop_register_read_sock(data->acct_sock, radius_server_receive_acct,
                                 data, NULL)) {
      radius_server_deinit(data);
      return NULL;
    }
...
static int radius_server_get_eap_user(void *ctx, const u8 *identity,
  ...
  ret =
      data->get_eap_user(data->conf_ctx, identity, identity_len, phase2, user);
  if (ret == 0 && user) {
