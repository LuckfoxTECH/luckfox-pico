...
static int tls_add_ca_from_keystore_encoded(X509_STORE *ctx,
  ...
  if (decoded_alias) {
    if (!hexstr2bin(encoded_key_alias, decoded_alias, len / 2)) {
      ...
      rc = tls_add_ca_from_keystore(ctx, (const char *)decoded_alias);
    }
...
static void tls_show_errors(int level, const char *func, const char *txt) {
  ...
  wpa_printf(level, "OpenSSL: %s - %s %s", func, txt,
...
static void cryptoapi_error(const char *msg) {
  wpa_printf(MSG_INFO, "CryptoAPI: %s; err=%u", msg,
...
static int cryptoapi_rsa_priv_enc(int flen, const unsigned char *from,
  ...
  if (flen != 16 /* MD5 */ + 20 /* SHA-1 */) {
    wpa_printf(MSG_INFO, "%s - only MD5-SHA1 hash supported", __func__);
...
static const CERT_CONTEXT *cryptoapi_find_cert(const char *name, DWORD store) {
  ...
  cs = CertOpenStore(
      (LPCSTR)CERT_STORE_PROV_SYSTEM, 0, 0,
      store | CERT_STORE_OPEN_EXISTING_FLAG | CERT_STORE_READONLY_FLAG, L"MY");
  if (cs == NULL) {
  ...
  if (strncmp(name, "cert://", 7) == 0) {
    ...
    ret =
        CertFindCertificateInStore(cs, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
...
static int tls_cryptoapi_cert(SSL *ssl, const char *name) {
  ...
  if (name == NULL ||
      (strncmp(name, "cert://", 7) != 0 && strncmp(name, "hash://", 7) != 0))
    return -1;
  ...
  if (priv->cert == NULL) {
    priv->cert = cryptoapi_find_cert(name, CERT_SYSTEM_STORE_LOCAL_MACHINE);
  }
  if (priv->cert == NULL) {
    wpa_printf(MSG_INFO,
  ...
  cert = d2i_X509(NULL, (const unsigned char **)&priv->cert->pbCertEncoded,
  ...
  if (!CryptAcquireCertificatePrivateKey(
          priv->cert, CRYPT_ACQUIRE_COMPARE_KEY_FLAG, NULL, &priv->crypt_prov,
          &priv->key_spec, &priv->free_crypt_prov)) {
...
static int tls_cryptoapi_ca_cert(SSL_CTX *ssl_ctx, SSL *ssl, const char *name) {
  ...
  if (cs == NULL) {
    wpa_printf(MSG_DEBUG,
               ...
               "'%s': error=%d",
               __func__, store, (int)GetLastError());
    return -1;
  }
  ...
  while ((ctx = CertEnumCertificatesInStore(cs, ctx))) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "system certificate store: subject='%s'",
               buf);
  ...
  if (!CertCloseStore(cs, 0)) {
    wpa_printf(MSG_DEBUG,
               ...
               "'%s': error=%d",
               __func__, name + 13, (int)GetLastError());
  }
...
static int tls_cryptoapi_cert(SSL *ssl, const char *name) { return -1; }
...
static void ssl_info_cb(const SSL *ssl, int where, int ret) {
  ...
  if (where & SSL_CB_LOOP) {
    wpa_printf(MSG_DEBUG, "SSL: %s:%s", str, SSL_state_string_long(ssl));
  } else if (where & SSL_CB_ALERT) {
    struct tls_connection *conn = SSL_get_app_data((SSL *)ssl);
    wpa_printf(MSG_INFO, "SSL: SSL3 alert: %s:%s:%s",
               where & SSL_CB_READ ? "read (remote end reported an error)"
    ...
    wpa_printf(MSG_DEBUG, "SSL: %s:%s in %s", str,
               ret == 0 ? "failed" : "error", SSL_state_string_long(ssl));
  }
}
...
static int tls_engine_load_dynamic_generic(const char *pre[],
  ...
  if (engine == NULL) {
    wpa_printf(MSG_INFO, "ENGINE: Can't find engine %s [%s]", dynamic_id,
               ERR_error_string(ERR_get_error(), NULL));
    return -1;
  }
  ...
  while (pre && pre[0]) {
    ...
    if (ENGINE_ctrl_cmd_string(engine, pre[0], pre[1], 0) == 0) {
      wpa_printf(MSG_INFO,
                 ...
                 "%s %s [%s]",
                 pre[0], pre[1], ERR_error_string(ERR_get_error(), NULL));
      ENGINE_free(engine);
      return -1;
    }
    pre += 2;
  ...
  engine = ENGINE_by_id(id);
  if (engine == NULL) {
    wpa_printf(MSG_INFO, "ENGINE: Can't find engine %s [%s]", id,
               ERR_error_string(ERR_get_error(), NULL));
    return -1;
  }
found:
  while (post && post[0]) {
    ...
    if (ENGINE_ctrl_cmd_string(engine, post[0], post[1], 0) == 0) {
      wpa_printf(MSG_DEBUG,
                 "ENGINE: ctrl cmd_string failed:"
                 " %s %s [%s]",
                 post[0], post[1], ERR_error_string(ERR_get_error(), NULL));
...
static int tls_engine_load_dynamic_pkcs11(const char *pkcs11_so_path,
  ...
  const char *pre_cmd[] = {"SO_PATH", NULL /* pkcs11_so_path */, "ID",
                           NULL /* engine_id */, "LIST_ADD", "1",
                           /* "NO_VCHECK", "1", */
                           "LOAD", NULL, NULL, NULL};
  const char *post_cmd[] = {"MODULE_PATH", NULL /* pkcs11_module_path */, NULL,
                            NULL};
...
static int tls_engine_load_dynamic_opensc(const char *opensc_so_path) {
  ...
  const char *pre_cmd[] = {"SO_PATH",  NULL /* opensc_so_path */,
                           "ID",       NULL /* engine_id */,
                           "LIST_ADD", "1",
                           "LOAD",     NULL,
                           NULL,       NULL};
...
static void remove_session_cb(SSL_CTX *ctx, SSL_SESSION *sess) {
  ...
  if (!buf)
    return;
  wpa_printf(MSG_DEBUG, "OpenSSL: Free application session data %p (sess %p)",
             buf, sess);
  wpabuf_free(buf);
...
void *tls_init(const struct tls_config *conf) {
  ...
  if (conf && (conf->opensc_engine_path || conf->pkcs11_engine_path ||
  ...
  if (SSL_CTX_set_cipher_list(ssl, ciphers) != 1) {
    wpa_printf(MSG_ERROR, "OpenSSL: Failed to set cipher string '%s'", ciphers);
    tls_deinit(data);
    return NULL;
  }
...
#error "This code depends on OPENSSL_NO_ENGINE being defined by BoringSSL."
  ...
  if (!conn->engine) {
    wpa_printf(MSG_ERROR, "ENGINE: engine %s not available [%s]", engine_id,
               ERR_error_string(ERR_get_error(), NULL));
    goto err;
  }
  ...
  if (key_id) {
    ...
    conn->private_key =
        ENGINE_load_private_key(conn->engine, key_id, NULL, &key_cb);
    if (!conn->private_key) {
      unsigned long err = ERR_get_error();

      wpa_printf(MSG_ERROR, "ENGINE: cannot load private key with id '%s' [%s]",
                 key_id, ERR_error_string(err, NULL));
  ...
  if (cert_id || ca_cert_id) {
    ...
    if (!ENGINE_ctrl(conn->engine, ENGINE_CTRL_GET_CMD_FROM_NAME, 0,
                     (void *)cmd_name, NULL)) {
...
int tls_get_errors(void *ssl_ctx) {
  ...
  while ((err = ERR_get_error())) {
    wpa_printf(MSG_INFO, "TLS - SSL error: %s", ERR_error_string(err, NULL));
    count++;
  }
...
static void tls_msg_cb(int write_p, int version, int content_type,
  ...
  if (write_p == 2) {
    wpa_printf(MSG_DEBUG, "OpenSSL: session ver=0x%x content_type=%d", version,
               content_type);
...
static int domain_suffix_match(const u8 *val, size_t len, const char *match,
  ...
  if (os_strncasecmp((const char *)val + len - match_len, match, match_len) !=
      0)
    return 0; /* no match */
...
static int tls_match_suffix(X509 *cert, const char *match, int full) {
  ...
  wpa_printf(MSG_DEBUG, "TLS: Match domain against %s%s", full ? "" : "suffix ",
             match);
  ...
  for (j = 0; ext && j < sk_GENERAL_NAME_num(ext); j++) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "TLS: Certificate dNSName",
                      gen->d.dNSName->data, gen->d.dNSName->length);
    if (domain_suffix_match(gen->d.dNSName->data, gen->d.dNSName->length, match,
                            full) == 1) {
  ...
  for (;;) {
    ...
    if (cn == NULL)
      continue;
    wpa_hexdump_ascii(MSG_DEBUG, "TLS: Certificate commonName", cn->data,
                      cn->length);
...
static void openssl_tls_fail_event(struct tls_connection *conn, X509 *err_cert,
                                   int err, int depth, const char *subject,
                                   const char *err_str,
  ...
  os_memset(&ev, 0, sizeof(ev));
  ev.cert_fail.reason =
      reason != TLS_FAIL_UNSPECIFIED ? reason : openssl_tls_fail_reason(err);
...
static void openssl_tls_cert_event(struct tls_connection *conn, X509 *err_cert,
  ...
  for (i = 0; ext && i < sk_GENERAL_NAME_num(ext); i++) {
    ...
    if (num_altsubject == TLS_MAX_ALT_SUBJECT)
      break;
    gen = sk_GENERAL_NAME_value(ext, i);
    if (gen->type != GEN_EMAIL && gen->type != GEN_DNS && gen->type != GEN_URI)
      continue;
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx) {
  ...
  if (depth == 0 && conn->server_cert_only) {
    ...
    if (!cert) {
      ...
      if (sha256_vector(1, addr, len, hash) < 0 ||
         ...
         */
        wpa_printf(MSG_DEBUG, "OpenSSL: Ignore validation issues for a pinned "
                              "server certificate");
        preverify_ok = 1;
      }
      wpabuf_free(cert);
  ...
  if (!preverify_ok) {
    ...
    openssl_tls_fail_event(conn, err_cert, err, depth, buf, err_str,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             preverify_ok, err, err_str, conn->ca_cert_verify, depth, buf);
  if (depth == 0 && match && os_strstr(buf, match) == NULL) {
    wpa_printf(MSG_WARNING,
               ...
               buf, match);
    preverify_ok = 0;
    openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Subject mismatch",
    ...
    wpa_printf(MSG_WARNING, "TLS: Domain match '%s' not found", domain_match);
    preverify_ok = 0;
    openssl_tls_fail_event(conn, err_cert, err, depth, buf, "Domain mismatch",
  ...
  if (depth == 0 && (conn->flags & TLS_CONN_REQUEST_OCSP) && preverify_ok) {
    enum ocsp_result res;

    res = check_ocsp_resp(conn->ssl_ctx, conn->ssl, err_cert, conn->peer_issuer,
                          conn->peer_issuer_issuer);
    if (res == OCSP_REVOKED) {
      preverify_ok = 0;
      openssl_tls_fail_event(conn, err_cert, err, depth, buf,
                             "certificate revoked", TLS_FAIL_REVOKED);
      if (err == X509_V_OK)
        X509_STORE_CTX_set_error(x509_ctx, X509_V_ERR_CERT_REVOKED);
    } else if (res != OCSP_GOOD && (conn->flags & TLS_CONN_REQUIRE_OCSP)) {
      preverify_ok = 0;
      openssl_tls_fail_event(conn, err_cert, err, depth, buf,
  ...
  if (depth == 0 && preverify_ok && context->event_cb != NULL)
    context->event_cb(context->cb_ctx, TLS_CERT_CHAIN_SUCCESS, NULL);
...
static int tls_load_ca_der(struct tls_data *data, const char *ca_cert) {
  ...
  lookup = X509_STORE_add_lookup(SSL_CTX_get_cert_store(ssl_ctx),
                                 X509_LOOKUP_file());
  if (lookup == NULL) {
    tls_show_errors(MSG_WARNING, __func__, "Failed add lookup for X509 store");
    return -1;
  }

  if (!X509_LOOKUP_load_file(lookup, ca_cert, X509_FILETYPE_ASN1)) {
    unsigned long err = ERR_peek_error();
    tls_show_errors(MSG_WARNING, __func__, "Failed load CA in DER format");
...
static int tls_connection_ca_cert(struct tls_data *data,
  ...
  if (ca_cert_blob) {
    X509 *cert =
        d2i_X509(NULL, (const unsigned char **)&ca_cert_blob, ca_cert_blob_len);
    if (cert == NULL) {
      tls_show_errors(MSG_WARNING, __func__, "Failed to parse ca_cert_blob");
      return -1;
    }
  ...
  if (ca_cert && os_strncmp("keystores://", ca_cert, 12) == 0) {
    ...
    for (; alias; alias = strtok_r(NULL, delim, &savedptr)) {
      if (tls_add_ca_from_keystore_encoded(SSL_CTX_get_cert_store(ssl_ctx),
                                           alias)) {
        wpa_printf(MSG_WARNING,
...
#ifdef CONFIG_NATIVE_WINDOWS
  if (ca_cert && tls_cryptoapi_ca_cert(ssl_ctx, conn->ssl, ca_cert) == 0) {
  ...
  if (ca_cert || ca_path) {
#ifndef OPENSSL_NO_STDIO
    if (SSL_CTX_load_verify_locations(ssl_ctx, ca_cert, ca_path) != 1) {
      tls_show_errors(MSG_WARNING, __func__,
                      "Failed to load root certificates");
      if (ca_cert && tls_load_ca_der(data, ca_cert) == 0) {
        wpa_printf(MSG_DEBUG,
                   "OpenSSL: %s - loaded "
...
static int tls_global_ca_cert(struct tls_data *data, const char *ca_cert) {
  ...
  if (ca_cert) {
    ...
    SSL_CTX_set_client_CA_list(ssl_ctx, SSL_load_client_CA_file(ca_cert));
#endif /* OPENSSL_NO_STDIO */
  }
...
int tls_connection_set_verify(void *ssl_ctx, struct tls_connection *conn,
  ...
  if (data->tls_session_lifetime == 0) {
     ...
     */
    counter++;
    SSL_set_session_id_context(conn->ssl, (const unsigned char *)&counter,
                               sizeof(counter));
  } else if (session_ctx) {
    SSL_set_session_id_context(conn->ssl, session_ctx, session_ctx_len);
  }
...
static int tls_connection_client_cert(struct tls_connection *conn,
  ...
  if (client_cert_blob &&
    ...
    tls_show_errors(MSG_DEBUG, __func__, "SSL_use_certificate_ASN1 failed");
  }
...
#ifndef OPENSSL_NO_STDIO
  if (SSL_use_certificate_file(conn->ssl, client_cert, SSL_FILETYPE_ASN1) ==
      1) {
  ...
  if (SSL_use_certificate_file(conn->ssl, client_cert, SSL_FILETYPE_PEM) == 1) {
    ERR_clear_error();
  ...
  tls_show_errors(MSG_DEBUG, __func__, "SSL_use_certificate_file failed");
#else  /* OPENSSL_NO_STDIO */
  wpa_printf(MSG_DEBUG, "OpenSSL: %s - OPENSSL_NO_STDIO", __func__);
#endif /* OPENSSL_NO_STDIO */
...
static int tls_global_client_cert(struct tls_data *data,
  ...
  if (SSL_CTX_use_certificate_file(ssl_ctx, client_cert, SSL_FILETYPE_ASN1) !=
          1 &&
      SSL_CTX_use_certificate_chain_file(ssl_ctx, client_cert) != 1 &&
      SSL_CTX_use_certificate_file(ssl_ctx, client_cert, SSL_FILETYPE_PEM) !=
          1) {
    tls_show_errors(MSG_INFO, __func__, "Failed to load client certificate");
    return -1;
  }
  return 0;
#else  /* OPENSSL_NO_STDIO */
...
static int tls_parse_pkcs12(struct tls_data *data, SSL *ssl, PKCS12 *p12,
  ...
  if (!PKCS12_parse(p12, passwd, &pkey, &cert, &certs)) {
    tls_show_errors(MSG_DEBUG, __func__, "Failed to parse PKCS12 file");
    PKCS12_free(p12);
    return -1;
  }
  ...
  if (certs) {
    ...
    while ((cert = sk_X509_pop(certs)) != NULL) {
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 " from PKCS12: subject='%s'",
                 buf);
      if ((ssl && SSL_add1_chain_cert(ssl, cert) != 1) ||
          (!ssl && SSL_CTX_add1_chain_cert(data->ssl, cert) != 1)) {
    ...
    if (ssl)
      res = SSL_build_cert_chain(ssl, SSL_BUILD_CHAIN_FLAG_CHECK |
      ...
      res = SSL_CTX_build_cert_chain(data->ssl,
                                     SSL_BUILD_CHAIN_FLAG_CHECK |
                                         SSL_BUILD_CHAIN_FLAG_IGNORE_ERROR);
    if (!res) {
      tls_show_errors(MSG_DEBUG, __func__, "Failed to build certificate chain");
    } else if (res == 2) {
      wpa_printf(MSG_DEBUG,
                 "TLS: Ignore certificate chain verification error when "
                 "building chain with PKCS#12 extra certificates");
    }
#endif /* OPENSSL_IS_BORINGSSL */
    /*
...
static int tls_read_pkcs12(struct tls_data *data, SSL *ssl,
  ...
  if (p12 == NULL) {
    tls_show_errors(MSG_INFO, __func__, "Failed to use PKCS#12 file");
    return -1;
  }
...
static int tls_read_pkcs12_blob(struct tls_data *data, SSL *ssl, const u8 *blob,
                                size_t len, const char *passwd) {
#ifdef PKCS12_FUNCS
  PKCS12 *p12;

  p12 = d2i_PKCS12(NULL, (const unsigned char **)&blob, len);
  if (p12 == NULL) {
    tls_show_errors(MSG_INFO, __func__, "Failed to use PKCS#12 blob");
    return -1;
  }
...
#ifndef OPENSSL_NO_ENGINE
static int tls_engine_get_cert(struct tls_connection *conn, const char *cert_id,
                               X509 **cert) {
  ...
  if (!ENGINE_ctrl_cmd(conn->engine, "LOAD_CERT_CTRL", 0, &params, NULL, 1)) {
    unsigned long err = ERR_get_error();

    wpa_printf(MSG_ERROR,
               "ENGINE: cannot load client cert with id"
               " '%s' [%s]",
               cert_id, ERR_error_string(err, NULL));
    if (tls_is_pin_error(err))
...
static int tls_connection_engine_client_cert(struct tls_connection *conn,
  ...
  if (!SSL_use_certificate(conn->ssl, cert)) {
    tls_show_errors(MSG_ERROR, __func__, "SSL_use_certificate failed");
    X509_free(cert);
    return -1;
  }
  X509_free(cert);
...
static int tls_connection_private_key(struct tls_data *data,
  ...
  while (private_key_blob) {
    if (SSL_use_PrivateKey_ASN1(EVP_PKEY_RSA, conn->ssl, (u8 *)private_key_blob,
    ...
    if (SSL_use_PrivateKey_ASN1(EVP_PKEY_DSA, conn->ssl, (u8 *)private_key_blob,
                                private_key_blob_len) == 1) {
      wpa_printf(MSG_DEBUG, "OpenSSL: SSL_use_PrivateKey_"
    ...
    if (SSL_use_RSAPrivateKey_ASN1(conn->ssl, (u8 *)private_key_blob,
                                   private_key_blob_len) == 1) {
  ...
  while (!ok && private_key) {
#ifndef OPENSSL_NO_STDIO
    if (SSL_use_PrivateKey_file(conn->ssl, private_key, SSL_FILETYPE_ASN1) ==
        1) {
      wpa_printf(MSG_DEBUG, "OpenSSL: "
    ...
    if (SSL_use_PrivateKey_file(conn->ssl, private_key, SSL_FILETYPE_PEM) ==
        1) {
      wpa_printf(MSG_DEBUG, "OpenSSL: "
    ...
    if (tls_read_pkcs12(data, conn->ssl, private_key, passwd) == 0) {
...
static int tls_global_private_key(struct tls_data *data,
  ...
  if (
#ifndef OPENSSL_NO_STDIO
      SSL_CTX_use_PrivateKey_file(ssl_ctx, private_key, SSL_FILETYPE_ASN1) !=
          1 &&
...
static int tls_connection_dh(struct tls_connection *conn, const char *dh_file) {
  ...
  if (bio == NULL) {
    wpa_printf(MSG_INFO, "TLS: Failed to open DH file '%s': %s", dh_file,
               ERR_error_string(ERR_get_error(), NULL));
    return -1;
  }
...
int tls_connection_get_random(void *ssl_ctx, struct tls_connection *conn,
  ...
  keys->client_random_len = SSL_get_client_random(ssl, conn->client_random,
                                                  sizeof(conn->client_random));
  keys->server_random = conn->server_random;
  keys->server_random_len = SSL_get_server_random(ssl, conn->server_random,
                                                  sizeof(conn->server_random));
...
static int openssl_get_keyblock_size(SSL *ssl) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "IV_len=%d",
             EVP_CIPHER_key_length(c), md_size, EVP_CIPHER_iv_length(c));
  return 2 * (EVP_CIPHER_key_length(c) + md_size + EVP_CIPHER_iv_length(c));
#else
  ...
  wpa_printf(MSG_DEBUG, "OpenSSL: cipher nid %d digest nid %d", cipher, digest);
  ...
  wpa_printf(MSG_DEBUG,
             ...
             EVP_CIPHER_key_length(c), EVP_MD_size(h), EVP_CIPHER_iv_length(c));
  return 2 *
         (EVP_CIPHER_key_length(c) + EVP_MD_size(h) + EVP_CIPHER_iv_length(c));
#endif
}
...
int tls_connection_export_key(void *tls_ctx, struct tls_connection *conn,
  ...
  if (!conn || SSL_export_keying_material(conn->ssl, out, out_len, label,
...
int tls_connection_get_eap_fast_key(void *tls_ctx, struct tls_connection *conn,
  ...
  master_key_len =
      SSL_SESSION_get_master_key(sess, master_key, sizeof(master_key));
  ...
  if (os_strcmp(ver, "TLSv1.2") == 0) {
    tls_prf_sha256(master_key, master_key_len, "key expansion", rnd,
                   2 * SSL3_RANDOM_SIZE, _out, skip + out_len);
    ret = 0;
  } else if (tls_prf_sha1_md5(master_key, master_key_len, "key expansion", rnd,
                              2 * SSL3_RANDOM_SIZE, _out,
                              skip + out_len) == 0) {
    ret = 0;
  }
...
static struct wpabuf *openssl_handshake(struct tls_connection *conn,
  ...
  if (in_data && wpabuf_len(in_data) > 0 &&
      BIO_write(conn->ssl_in, wpabuf_head(in_data), wpabuf_len(in_data)) < 0) {
    tls_show_errors(MSG_INFO, __func__, "Handshake failed - BIO_write");
    return NULL;
  }
  ...
  res = res == 0 ? 0 : BIO_read(conn->ssl_out, wpabuf_mhead(out_data), res);
  if (res < 0) {
    tls_show_errors(MSG_INFO, __func__, "Handshake failed - BIO_read");
    if (BIO_reset(conn->ssl_out) < 0) {
      tls_show_errors(MSG_INFO, __func__, "BIO_reset failed");
    }
...
static struct wpabuf *openssl_get_appl_data(struct tls_connection *conn,
  ...
  res = SSL_read(conn->ssl, wpabuf_mhead(appl_data), wpabuf_size(appl_data));
  if (res < 0) {
    int err = SSL_get_error(conn->ssl, res);
...
static struct wpabuf *openssl_connection_handshake(struct tls_connection *conn,
  ...
  if (SSL_is_init_finished(conn->ssl)) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Handshake finished - resumed=%d",
...
struct wpabuf *tls_connection_handshake(void *ssl_ctx,
                                        struct tls_connection *conn,
                                        const struct wpabuf *in_data,
                                        struct wpabuf **appl_data) {
...
struct wpabuf *tls_connection_encrypt(void *tls_ctx,
  ...
  if (res < 0) {
    tls_show_errors(MSG_INFO, __func__, "Encryption failed - SSL_write");
    return NULL;
  }
  ...
  if (res < 0) {
    tls_show_errors(MSG_INFO, __func__, "Encryption failed - BIO_read");
    wpabuf_free(buf);
    return NULL;
  }
  wpabuf_put(buf, res);
...
struct wpabuf *tls_connection_decrypt(void *tls_ctx,
  ...
  res = BIO_write(conn->ssl_in, wpabuf_head(in_data), wpabuf_len(in_data));
  if (res < 0) {
    tls_show_errors(MSG_INFO, __func__, "Decryption failed - BIO_write");
    return NULL;
  }
  if (BIO_reset(conn->ssl_out) < 0) {
    tls_show_errors(MSG_INFO, __func__, "BIO_reset failed");
  ...
  if (res < 0) {
    tls_show_errors(MSG_INFO, __func__, "Decryption failed - SSL_read");
    wpabuf_free(buf);
    return NULL;
  }
  wpabuf_put(buf, res);
...
int tls_connection_set_cipher_list(void *tls_ctx, struct tls_connection *conn,
  ...
  if (SSL_set_cipher_list(conn->ssl, buf + 1) != 1) {
    tls_show_errors(MSG_INFO, __func__, "Cipher suite configuration failed");
    return -1;
  }
...
int tls_get_version(void *ssl_ctx, struct tls_connection *conn, char *buf,
                    size_t buflen) {
  const char *name;
...
int tls_get_cipher(void *ssl_ctx, struct tls_connection *conn, char *buf,
                   size_t buflen) {
  const char *name;
  if (conn == NULL || conn->ssl == NULL)
    return -1;
...
int tls_connection_client_hello_ext(void *ssl_ctx, struct tls_connection *conn,
  ...
  if (SSL_set_session_ticket_ext(conn->ssl, (void *)data, data_len) != 1)
    return -1;
...
int tls_connection_get_write_alerts(void *ssl_ctx,
                                    struct tls_connection *conn) {
  if (conn == NULL)
    return -1;
...
static int ocsp_resp_cb(SSL *s, void *arg) {
  ...
  if (status != OCSP_RESPONSE_STATUS_SUCCESSFUL) {
    wpa_printf(MSG_INFO, "OpenSSL: OCSP responder error %d (%s)", status,
  ...
  if (conn->peer_issuer) {
    ...
    if (certs) {
      ...
      if (certs && conn->peer_issuer_issuer) {
        ...
        if (cert && !sk_X509_push(certs, cert)) {
          tls_show_errors(MSG_INFO, __func__,
                          "OpenSSL: Could not add issuer's issuer to OCSP "
                          "responder trust store");
          X509_free(cert);
        }
      }
    }
  ...
  if (!OCSP_resp_find_status(basic, id, &status, &reason, &produced_at,
    ...
    wpa_printf(MSG_INFO,
               "OpenSSL: Could not find current server certificate from OCSP "
               "response%s",
  ...
  if (!OCSP_check_validity(this_update, next_update, 5 * 60, -1)) {
    tls_show_errors(MSG_INFO, __func__, "OpenSSL: OCSP status times invalid");
    OCSP_BASICRESP_free(basic);
    OCSP_RESPONSE_free(rsp);
    return 0;
  }
  ...
  wpa_printf(MSG_DEBUG, "OpenSSL: OCSP status unknown, but OCSP was not "
                        "required, so allow connection to continue");
  return 1;
}
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn,
  ...
  if (params->flags & TLS_CONN_REQUIRE_OCSP_ALL) {
    wpa_printf(MSG_INFO, "OpenSSL: ocsp=3 not supported");
    return -1;
  }
  ...
  if (params->flags & TLS_CONN_EAP_FAST) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Use TLSv1_method() for EAP-FAST");
  ...
  while ((err = ERR_get_error())) {
    wpa_printf(MSG_INFO, "%s: Clearing pending SSL error: %s", __func__,
               ERR_error_string(err, NULL));
  }

  if (engine_id) {
    wpa_printf(MSG_DEBUG, "SSL: Initializing TLS engine");
    ret = tls_engine_init(conn, engine_id, params->pin, key_id, cert_id,
                          ca_cert_id);
    if (ret)
      return ret;
  }
  if (tls_connection_set_subject_match(
          conn, params->subject_match, params->altsubject_match,
          params->suffix_match, params->domain_match))
    return -1;
  ...
  if (engine_id && key_id) {
    ...
    if (tls_connection_engine_private_key(conn))
      return TLS_SET_PARAMS_ENGINE_PRV_VERIFY_FAILED;
                 ...
                 data, conn, params->private_key, params->private_key_passwd,
  ...
  if (tls_connection_dh(conn, params->dh_file)) {
    wpa_printf(MSG_INFO, "TLS: Failed to load DH file '%s'", params->dh_file);
    return -1;
  }

  if (params->openssl_ciphers &&
      SSL_set_cipher_list(conn->ssl, params->openssl_ciphers) != 1) {
    wpa_printf(MSG_INFO, "OpenSSL: Failed to set cipher string '%s'",
  ...
  if (params->flags & TLS_CONN_REQUEST_OCSP) {
    wpa_printf(MSG_DEBUG, "OpenSSL: No OCSP support included - allow optional "
                          "OCSP case to continue");
  }
#endif /* HAVE_OCSP */
#endif /* OPENSSL_IS_BORINGSSL */
...
#endif /* OPENSSL_IS_BORINGSSL */
{
  ...
  ret =
      conn->session_ticket_cb(conn->session_ticket_cb_ctx, conn->session_ticket,
                              conn->session_ticket_len, s->s3->client_random,
...
static int tls_session_ticket_ext_cb(SSL *s, const unsigned char *data, int len,
                                     void *arg) {
  struct tls_connection *conn = arg;
...
int tls_connection_set_session_ticket_cb(void *tls_ctx,
  ...
  if (cb) {
    if (SSL_set_session_secret_cb(conn->ssl, tls_sess_sec_cb, conn) != 1)
      return -1;
    SSL_set_session_ticket_ext_cb(conn->ssl, tls_session_ticket_ext_cb, conn);
  } else {
...
void tls_connection_set_success_data(struct tls_connection *conn,
  ...
  if (old) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Replacing old success data %p", old);
    wpabuf_free(old);
  }
...
void tls_connection_set_success_data_resumed(struct tls_connection *conn) {
  wpa_printf(MSG_DEBUG, "OpenSSL: Success data accepted for resumed session");
  conn->success_data = 1;
}
...
tls_connection_get_success_data(struct tls_connection *conn) {
  ...
  if (tls_ex_idx_session < 0 || !(sess = SSL_get_session(conn->ssl)))
    return NULL;
...
void tls_connection_remove_session(struct tls_connection *conn) {
  ...
  if (SSL_CTX_remove_session(conn->ssl_ctx, sess) != 1)
    wpa_printf(MSG_DEBUG, "OpenSSL: Session was not cached");
  else
    wpa_printf(MSG_DEBUG,
