 ...
 * given key.
 */
int sha256_prf(const u8 *key, size_t key_len, const char *label, const u8 *data,
...
int sha256_prf_bits(const u8 *key, size_t key_len, const char *label,
  ...
  while (pos < buf_len) {
    ...
    if (plen >= SHA256_MAC_LEN) {
      if (hmac_sha256_vector(key, key_len, 4, addr, len, &buf[pos]) < 0)
        return -1;
      pos += SHA256_MAC_LEN;
    } else {
      if (hmac_sha256_vector(key, key_len, 4, addr, len, hash) < 0)
        return -1;
