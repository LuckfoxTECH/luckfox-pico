...
int generate_nt_response_pwhash(const u8 *auth_challenge,
                                const u8 *peer_challenge, const u8 *username,
                                size_t username_len, const u8 *password_hash,
                                u8 *response) {
  u8 challenge[8];
...
/**
 * generate_authenticator_response_pwhash - GenerateAuthenticatorResponse() -
 * RFC 2759, Sect. 8.7
 * @password_hash: 16-octet PasswordHash (IN)
...
int generate_authenticator_response_pwhash(
    const u8 *password_hash, const u8 *peer_challenge, const u8 *auth_challenge,
    const u8 *username, size_t username_len, const u8 *nt_response,
    u8 *response) {
  static const u8 magic1[39] = {0x4D, 0x61, 0x67, 0x69, 0x63, 0x20, 0x73, 0x65,
...
/**
 * generate_authenticator_response - GenerateAuthenticatorResponse() - RFC 2759,
 * Sect. 8.7
 * @password: 0-to-256-unicode-char Password (IN; UTF-8)
 * @password_len: Length of password
 * @nt_response: 24-octet NT-Response (IN)
...
int generate_authenticator_response(const u8 *password, size_t password_len,
  ...
  return generate_authenticator_response_pwhash(
      password_hash, peer_challenge, auth_challenge, username, username_len,
...
int get_master_key(const u8 *password_hash_hash, const u8 *nt_response,
                   u8 *master_key) {
  static const u8 magic1[27] = {0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73,
...
/**
 * encrypt_pw_block_with_password_hash - EncryptPwBlockWithPasswordHash() - RFC
 * 2759, Sect. 8.10
 * @password: 0-to-256-unicode-char Password (IN; UTF-8)
 * @password_len: Length of password
 * @password_hash: 16-octet PasswordHash (IN)
 ...
 * Returns: 0 on success, -1 on failure
 */
int encrypt_pw_block_with_password_hash(const u8 *password, size_t password_len,
...
/**
 * nt_password_hash_encrypted_with_block - NtPasswordHashEncryptedWithBlock() -
 * RFC 2759, Sect 8.13
...
int old_nt_password_hash_encrypted_with_new_nt_password_hash(
    const u8 *new_password, size_t new_password_len, const u8 *old_password,
    size_t old_password_len, u8 *encrypted_password_hash) {
  u8 old_password_hash[16], new_password_hash[16];

  if (nt_password_hash(old_password, old_password_len, old_password_hash) ||
      nt_password_hash(new_password, new_password_len, new_password_hash))
    return -1;
  nt_password_hash_encrypted_with_block(old_password_hash, new_password_hash,
