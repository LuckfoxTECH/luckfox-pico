...
struct crypto_hash *crypto_hash_init(enum crypto_hash_alg alg, const u8 *key,
  ...
  case CRYPTO_HASH_ALG_HMAC_MD5:
    if (hmac_init(&ctx->u.hmac, find_hash("md5"), key, key_len) != CRYPT_OK)
      goto fail;
    break;
  case CRYPTO_HASH_ALG_HMAC_SHA1:
    if (hmac_init(&ctx->u.hmac, find_hash("sha1"), key, key_len) != CRYPT_OK)
      goto fail;
    break;
  default:
    goto fail;
...
int crypto_cipher_encrypt(struct crypto_cipher *ctx, const u8 *plain, u8 *crypt,
  ...
  if (ctx->rc4) {
    ...
    rc4_skip(ctx->u.rc4.key, ctx->u.rc4.keylen, ctx->u.rc4.used_bytes, crypt,
             len);
...
int crypto_cipher_decrypt(struct crypto_cipher *ctx, const u8 *crypt, u8 *plain,
  ...
  if (ctx->rc4) {
    ...
    rc4_skip(ctx->u.rc4.key, ctx->u.rc4.keylen, ctx->u.rc4.used_bytes, plain,
             len);
    ctx->u.rc4.used_bytes += len;
    return 0;
  }
...
static int pkcs1_generate_encryption_block(u8 block_type, size_t modlen,
  ...
  if (modlen < 12 || modlen > *outlen || inlen > modlen - 11) {
    wpa_printf(MSG_DEBUG,
               ...
               __func__, (unsigned long)modlen, (unsigned long)*outlen,
               (unsigned long)inlen);
    return -1;
  }
...
static int crypto_rsa_encrypt_pkcs1(int block_type, rsa_key *key, int key_type,
  ...
  if (pkcs1_generate_encryption_block(block_type, modlen, in, inlen, out,
                                      outlen) < 0)
    return -1;
...
int crypto_public_key_encrypt_pkcs1_v15(struct crypto_public_key *key,
                                        const u8 *in, size_t inlen, u8 *out,
                                        size_t *outlen) {
  return crypto_rsa_encrypt_pkcs1(2, &key->rsa, PK_PUBLIC, in, inlen, out,
                                  outlen);
}

int crypto_private_key_sign_pkcs1(struct crypto_private_key *key, const u8 *in,
                                  size_t inlen, u8 *out, size_t *outlen) {
  return crypto_rsa_encrypt_pkcs1(1, &key->rsa, PK_PRIVATE, in, inlen, out,
                                  outlen);
}
...
int crypto_public_key_decrypt_pkcs1(struct crypto_public_key *key,
  ...
  len = *plain_len;
  res = rsa_exptmod(crypt, crypt_len, plain, &len, PK_PUBLIC, &key->rsa);
  if (res != CRYPT_OK) {
    wpa_printf(MSG_DEBUG, "LibTomCrypt: rsa_exptmod failed: %s",
               error_to_string(res));
    return -1;
  ...
  if (len < 3 + 8 + 16 /* min hash len */ || plain[0] != 0x00 ||
      plain[1] != 0x01 || plain[2] != 0xff) {
...
int crypto_global_init(void) {
  ...
  if (register_hash(&md4_desc) < 0 || register_hash(&md5_desc) < 0 ||
      register_hash(&sha1_desc) < 0 || register_cipher(&aes_desc) < 0 ||
      register_cipher(&des_desc) < 0 || register_cipher(&des3_desc) < 0) {
