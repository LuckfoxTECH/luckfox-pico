...
static void tls_log_func(int level, const char *msg) {
  ...
  wpa_printf(level > 3 ? MSG_MSGDUMP : MSG_DEBUG, "gnutls<%d> %s", level, s);
  os_free(s);
}
...
void *tls_init(const struct tls_config *conf) {
  ...
  if (tls_gnutls_ref_count == 0) {
    wpa_printf(MSG_DEBUG, "GnuTLS: Library version %s (runtime) - %s (build)",
...
int tls_get_errors(void *ssl_ctx) { return 0; }
...
static int tls_gnutls_init_session(struct tls_global *global,
  ...
  ret = gnutls_priority_set_direct(conn->session, "NORMAL:-VERS-SSL3.0", &err);
  if (ret < 0) {
...
int tls_connection_shutdown(void *ssl_ctx, struct tls_connection *conn) {
  ...
  ret = gnutls_credentials_set(conn->session, GNUTLS_CRD_CERTIFICATE,
                               conn->params_set ? conn->xcred : global->xcred);
  if (ret < 0) {
    wpa_printf(MSG_INFO,
               "GnuTLS: Failed to configure credentials "
               "for session resumption: %s",
               gnutls_strerror(ret));
    return -1;
  }

  if (global->session_data) {
    ret = gnutls_session_set_data(conn->session, global->session_data,
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn,
  ...
  if (params->flags & TLS_CONN_REQUIRE_OCSP_ALL) {
    wpa_printf(MSG_INFO, "GnuTLS: ocsp=3 not supported");
    return -1;
  }

  if (params->flags & TLS_CONN_EXT_CERT_CHECK) {
    wpa_printf(MSG_INFO, "GnuTLS: tls_ext_cert_check=1 not supported");
    return -1;
  }
  ...
  if (params->ca_cert) {
    wpa_printf(MSG_DEBUG, "GnuTLS: Try to parse %s in DER format",
               params->ca_cert);
    ret = gnutls_certificate_set_x509_trust_file(conn->xcred, params->ca_cert,
                                                 GNUTLS_X509_FMT_DER);
    if (ret < 0) {
      wpa_printf(MSG_DEBUG,
                 "GnuTLS: Failed to read CA cert '%s' in DER format (%s) - try "
                 "in PEM format",
                 params->ca_cert, gnutls_strerror(ret));
      ret = gnutls_certificate_set_x509_trust_file(conn->xcred, params->ca_cert,
                                                   GNUTLS_X509_FMT_PEM);
      if (ret < 0) {
        wpa_printf(MSG_DEBUG, "Failed to read CA cert '%s' in PEM format: %s",
                   params->ca_cert, gnutls_strerror(ret));
        return -1;
      }
    }
    ...
    ret = gnutls_certificate_set_x509_trust_mem(conn->xcred, &ca,
                                                GNUTLS_X509_FMT_DER);
    if (ret < 0) {
      wpa_printf(MSG_DEBUG, "Failed to parse CA cert in DER format: %s",
                 gnutls_strerror(ret));
      ret = gnutls_certificate_set_x509_trust_mem(conn->xcred, &ca,
                                                  GNUTLS_X509_FMT_PEM);
      if (ret < 0) {
        wpa_printf(MSG_DEBUG, "Failed to parse CA cert in PEM format: %s",
                   gnutls_strerror(ret));
        return -1;
      }
    }
  ...
  if (params->ca_cert || params->ca_cert_blob) {
    conn->verify_peer = 1;
    gnutls_certificate_set_verify_function(conn->xcred,
                                           tls_connection_verify_peer);

    if (params->flags & TLS_CONN_ALLOW_SIGN_RSA_MD5) {
      gnutls_certificate_set_verify_flags(conn->xcred,
  ...
  if (params->client_cert && params->private_key) {
    ...
    ret = gnutls_certificate_set_x509_key_file(conn->xcred, params->client_cert,
                                               params->private_key,
                                               GNUTLS_X509_FMT_DER);
#endif
    if (ret < 0) {
...
#if GNUTLS_VERSION_NUMBER >= 0x03010b
    ret = gnutls_certificate_set_x509_key_mem2(conn->xcred, &cert, &key,
                                               GNUTLS_X509_FMT_DER,
                                               params->private_key_passwd, 0);
#else
    /* private_key_passwd not (easily) supported here */
    ret = gnutls_certificate_set_x509_key_mem(conn->xcred, &cert, &key,
                                              GNUTLS_X509_FMT_DER);
#endif
    if (ret < 0) {
      wpa_printf(MSG_DEBUG,
    ...
    ret = gnutls_certificate_set_x509_simple_pkcs12_mem(
        conn->xcred, &key, GNUTLS_X509_FMT_DER, params->private_key_passwd);
    if (ret != 0) {
      wpa_printf(MSG_DEBUG,
                 "Failed to load private_key in "
                 "PKCS#12 format: %s",
  ...
  if (params->flags & (TLS_CONN_REQUEST_OCSP | TLS_CONN_REQUIRE_OCSP)) {
    ret =
        gnutls_ocsp_status_request_enable_client(conn->session, NULL, 0, NULL);
    if (ret != GNUTLS_E_SUCCESS) {
      wpa_printf(MSG_INFO, "GnuTLS: Failed to enable OCSP client");
      return -1;
    }
  }
#else  /* 3.1.3 */
...
static int server_ocsp_status_req(gnutls_session_t session, void *ptr,
  ...
  wpa_printf(MSG_DEBUG, "GnuTLS: OCSP status callback - send cached response");
...
int tls_global_set_params(void *tls_ctx,
  ...
  if (params->ca_cert) {
    ret = gnutls_certificate_set_x509_trust_file(global->xcred, params->ca_cert,
                                                 GNUTLS_X509_FMT_DER);
    if (ret < 0) {
      wpa_printf(MSG_DEBUG,
    ...
    if (params->flags & TLS_CONN_ALLOW_SIGN_RSA_MD5) {
      gnutls_certificate_set_verify_flags(global->xcred,
                                          GNUTLS_VERIFY_ALLOW_SIGN_RSA_MD5);
    }
  ...
  if (params->client_cert && params->private_key) {
    ...
    /* Try to load in PKCS#12 format */
    ret = gnutls_certificate_set_x509_simple_pkcs12_file(
        global->xcred, params->private_key, GNUTLS_X509_FMT_DER,
        params->private_key_passwd);
    if (ret != 0) {
      wpa_printf(MSG_DEBUG,
                 "Failed to load private_key in "
  ...
  if (params->ocsp_stapling_response)
    global->ocsp_stapling_response = os_strdup(params->ocsp_stapling_response);
  else
...
int tls_connection_set_verify(void *ssl_ctx, struct tls_connection *conn,
  ...
  gnutls_certificate_server_set_request(
      conn->session, verify_peer ? GNUTLS_CERT_REQUIRE : GNUTLS_CERT_REQUEST);
...
int tls_connection_export_key(void *tls_ctx, struct tls_connection *conn,
  ...
  return gnutls_prf(conn->session, os_strlen(label), label,
                    0 /* client_random first */, 0, NULL, out_len, (char *)out);
}
...
static int server_eku_purpose(gnutls_x509_crt_t cert) {
  ...
  for (i = 0;; i++) {
    ...
    res = gnutls_x509_crt_get_key_purpose_oid(cert, i, oid, &oid_size, NULL);
...
static int check_ocsp(struct tls_connection *conn, gnutls_session_t session,
  ...
  if (!gnutls_ocsp_status_request_is_checked(session, 0)) {
    if (conn->flags & TLS_CONN_REQUIRE_OCSP) {
      wpa_printf(MSG_INFO, "GnuTLS: No valid OCSP response received");
      goto ocsp_error;
    }

    wpa_printf(MSG_DEBUG, "GnuTLS: Valid OCSP response was not received - "
                          "continue since OCSP was not required");
    return 0;
  }
  ...
  if (res != GNUTLS_E_SUCCESS) {
    wpa_printf(MSG_INFO, "GnuTLS: Could not parse received OCSP response: %s",
               gnutls_strerror(res));
  ...
  res = gnutls_ocsp_resp_get_single(resp, 0, NULL, NULL, NULL, NULL,
                                    &cert_status, NULL, NULL, NULL, NULL);
  gnutls_ocsp_resp_deinit(resp);
  if (res != GNUTLS_E_SUCCESS) {
    wpa_printf(MSG_INFO, "GnuTLS: Failed to extract OCSP information: %s",
               gnutls_strerror(res));
    goto ocsp_error;
  }
  ...
  if (cert_status == GNUTLS_OCSP_CERT_GOOD) {
    ...
    wpa_printf(MSG_DEBUG, "GnuTLS: OCSP cert status: revoked");
    goto ocsp_error;
  } else {
    wpa_printf(MSG_DEBUG, "GnuTLS: OCSP cert status: unknown");
    if (conn->flags & TLS_CONN_REQUIRE_OCSP)
      goto ocsp_error;
    wpa_printf(
        MSG_DEBUG,
...
ocsp_error:
  gnutls_tls_fail_event(conn, NULL, 0, NULL, "bad certificate status response",
                        TLS_FAIL_REVOKED);
...
static int tls_connection_verify_peer(gnutls_session_t session) {
  ...
  if (!conn->verify_peer) {
    wpa_printf(MSG_DEBUG, "GnuTLS: No peer certificate verification enabled");
    return 0;
  }
...
#if GNUTLS_VERSION_NUMBER >= 0x030300
  {
    ...
    res = gnutls_certificate_verify_peers(session, data, 1, &status);
  }
#else /* < 3.3.0 */
...
#if GNUTLS_VERSION_NUMBER >= 0x030104
  {
    ...
    ret = gnutls_certificate_verification_status_print(status, type, &info, 0);
    if (ret < 0) {
      wpa_printf(MSG_DEBUG, "GnuTLS: Failed to print verification status");
      err = GNUTLS_A_INTERNAL_ERROR;
      goto out;
    }
  ...
  if (conn->verify_peer && (status & GNUTLS_CERT_INVALID)) {
    ...
    if (status & GNUTLS_CERT_NOT_ACTIVATED) {
      wpa_printf(MSG_INFO, "TLS: Certificate not yet "
                           "activated");
      gnutls_tls_fail_event(conn, NULL, 0, NULL, "certificate not yet valid",
    ...
    if (status & GNUTLS_CERT_EXPIRED) {
      ...
      gnutls_tls_fail_event(conn, NULL, 0, NULL, "certificate has expired",
                            TLS_FAIL_EXPIRED);
      err = GNUTLS_A_CERTIFICATE_EXPIRED;
      goto out;
    }
    gnutls_tls_fail_event(conn, NULL, 0, NULL, "untrusted certificate",
                          TLS_FAIL_UNTRUSTED);
    err = GNUTLS_A_INTERNAL_ERROR;
    goto out;
  }
  ...
  if (status & GNUTLS_CERT_REVOKED) {
    ...
    gnutls_tls_fail_event(conn, NULL, 0, NULL, "certificate revoked",
                          TLS_FAIL_REVOKED);
    err = GNUTLS_A_CERTIFICATE_REVOKED;
    goto out;
  }
  ...
  if (status != 0) {
    wpa_printf(MSG_INFO, "TLS: Unknown verification status: %d", status);
    err = GNUTLS_A_INTERNAL_ERROR;
    goto out;
  }
  ...
  for (i = 0; i < num_certs; i++) {
    ...
    wpa_printf(MSG_DEBUG, "TLS: Peer cert chain %d/%d: %s", i + 1, num_certs,
               buf);
    ...
    if (conn->global->event_cb) {
      ...
      if (conn->global->cert_in_cb) {
        cert_buf = wpabuf_alloc_copy(certs[i].data, certs[i].size);
      ...
      conn->global->event_cb(conn->global->cb_ctx, TLS_PEER_CERTIFICATE, &ev);
      wpabuf_free(cert_buf);
    }

    if (i == 0) {
      if (conn->suffix_match &&
          !gnutls_x509_crt_check_hostname(cert, conn->suffix_match)) {
        wpa_printf(MSG_WARNING, "TLS: Domain suffix match '%s' not found",
                   conn->suffix_match);
        gnutls_tls_fail_event(conn, &certs[i], i, buf, "Domain suffix mismatch",
      ...
      if (conn->domain_match &&
          !gnutls_x509_crt_check_hostname2(
              cert, conn->domain_match, GNUTLS_VERIFY_DO_NOT_ALLOW_WILDCARDS)) {
        wpa_printf(MSG_WARNING, "TLS: Domain match '%s' not found",
                   conn->domain_match);
        gnutls_tls_fail_event(conn, &certs[i], i, buf, "Domain mismatch",
       ...
       */
      if (!conn->global->server && !server_eku_purpose(cert)) {
        wpa_printf(MSG_WARNING, "GnuTLS: No server EKU");
        gnutls_tls_fail_event(conn, &certs[i], i, buf, "No server EKU",
                              TLS_FAIL_BAD_CERTIFICATE);
        err = GNUTLS_A_BAD_CERTIFICATE;
        gnutls_x509_crt_deinit(cert);
        os_free(buf);
    ...
    if (!conn->disable_time_checks &&
      ...
      wpa_printf(MSG_INFO,
                 ...
                 i + 1, num_certs);
      gnutls_tls_fail_event(conn, &certs[i], i, buf,
  ...
  if (conn->global->event_cb != NULL)
    conn->global->event_cb(conn->global->cb_ctx, TLS_CERT_CHAIN_SUCCESS, NULL);
...
static struct wpabuf *gnutls_get_appl_data(struct tls_connection *conn) {
  ...
  res = gnutls_record_recv(conn->session, wpabuf_mhead(ad), wpabuf_size(ad));
  wpa_printf(MSG_DEBUG, "GnuTLS: gnutls_record_recv: %d", res);
  if (res < 0) {
    wpa_printf(MSG_DEBUG,
               "%s - gnutls_record_recv failed: %d "
               "(%s)",
               __func__, (int)res, gnutls_strerror(res));
    wpabuf_free(ad);
    return NULL;
  }

  wpabuf_put(ad, res);
  wpa_printf(MSG_DEBUG, "GnuTLS: Received %d bytes of Application Data", res);
  return ad;
}
...
struct wpabuf *tls_connection_handshake(void *tls_ctx,
  ...
  if (in_data && wpabuf_len(in_data) > 0) {
    if (conn->pull_buf) {
      wpa_printf(MSG_DEBUG,
                 ...
                 "pull_buf",
                 __func__, (unsigned long)wpabuf_len(conn->pull_buf));
      wpabuf_free(conn->pull_buf);
    }
  ...
  if (ret < 0) {
    ...
    switch (ret) {
    case GNUTLS_E_AGAIN:
      if (global->server && conn->established && conn->push_buf == NULL) {
    ...
    case GNUTLS_E_FATAL_ALERT_RECEIVED:
      ...
      wpa_printf(MSG_DEBUG, "%s - received fatal '%s' alert", __func__,
      ...
      if (conn->global->event_cb != NULL) {
        ...
        conn->global->event_cb(conn->global->cb_ctx, TLS_ALERT, &ev);
      }
      /* continue */
    default:
      wpa_printf(MSG_DEBUG,
    ...
    if (global->session_data == NULL || global->session_data_size < size) {
      os_free(global->session_data);
      global->session_data = os_malloc(size);
    }
    if (global->session_data) {
      global->session_data_size = size;
      gnutls_session_get_data(conn->session, global->session_data,
                              &global->session_data_size);
    }
...
struct wpabuf *tls_connection_encrypt(void *tls_ctx,
  ...
  res = gnutls_record_send(conn->session, wpabuf_head(in_data),
                           wpabuf_len(in_data));
  if (res < 0) {
    wpa_printf(MSG_INFO, "%s: Encryption failed: %s", __func__,
               gnutls_strerror(res));
    return NULL;
  }
...
struct wpabuf *tls_connection_decrypt(void *tls_ctx,
  ...
  res = gnutls_record_recv(conn->session, wpabuf_mhead(out), wpabuf_size(out));
  if (res < 0) {
    wpa_printf(MSG_DEBUG,
               "%s - gnutls_record_recv failed: %d "
               "(%s)",
...
int tls_get_library_version(char *buf, size_t buf_len) {
  return os_snprintf(buf, buf_len, "GnuTLS build=%s run=%s", GNUTLS_VERSION,
