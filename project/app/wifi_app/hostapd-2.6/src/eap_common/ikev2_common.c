...
static const struct ikev2_prf_alg ikev2_prf_algs[] = {{PRF_HMAC_SHA1, 20, 20},
                                                      {PRF_HMAC_MD5, 16, 16}};
...
int ikev2_prf_hash(int alg, const u8 *key, size_t key_len, size_t num_elem,
                   const u8 *addr[], const size_t *len, u8 *hash) {
  switch (alg) {
  case PRF_HMAC_SHA1:
    return hmac_sha1_vector(key, key_len, num_elem, addr, len, hash);
  case PRF_HMAC_MD5:
...
int ikev2_prf_plus(int alg, const u8 *key, size_t key_len, const u8 *data,
                   size_t data_len, u8 *out, size_t out_len) {
  ...
  while (pos < end) {
    size_t clen;
    if (iter == 1)
      res = ikev2_prf_hash(alg, key, key_len, 2, &addr[1], &len[1], hash);
    else
      res = ikev2_prf_hash(alg, key, key_len, 3, addr, len, hash);
    if (res < 0)
      return -1;
    clen = hash_len;
    if ((int)clen > end - pos)
...
int ikev2_parse_payloads(struct ikev2_payloads *payloads, u8 next_payload,
  ...
  while (next_payload != IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) {
    ...
    const u8 *pdata;
    wpa_printf(MSG_DEBUG, "IKEV2: Processing payload %u", next_payload);
    if (end < pos)
      return -1;
    left = end - pos;
    if (left < sizeof(*phdr)) {
    ...
    if (next_payload == IKEV2_PAYLOAD_ENCRYPTED && pos + plen == end) {
      /*
...
int ikev2_derive_auth_data(int prf_alg, const struct wpabuf *sign_msg,
                           ...
                           const u8 *nonce, size_t nonce_len, const u8 *key_pad,
                           size_t key_pad_len, u8 *auth_data) {
  ...
  if (sign_msg == NULL || ID == NULL || SK_p == NULL || shared_secret == NULL ||
      nonce == NULL || prf == NULL)
    return -1;
  ...
  if (ikev2_prf_hash(prf->id, SK_p, keys->SK_prf_len, 1, (const u8 **)&buf,
                     &buf_len, hash) < 0) {
    os_free(buf);
    return -1;
  }
  ...
  if (ikev2_prf_hash(prf->id, shared_secret, shared_secret_len, 1, &key_pad,
                     &key_pad_len, hash) < 0 ||
      ikev2_prf_hash(prf->id, hash, prf->hash_len, 1, (const u8 **)&sign_data,
                     &sign_len, auth_data) < 0) {
    os_free(sign_data);
    return -1;
  }
  os_free(sign_data);
...
u8 *ikev2_decrypt_payload(int encr_id, int integ_id, struct ikev2_keys *keys,
  ...
  if (ikev2_integ_hash(integ_id, SK_a, keys->SK_integ_len, (const u8 *)hdr,
...
int ikev2_build_encrypted(int encr_id, int integ_id, struct ikev2_keys *keys,
  ...
  return ikev2_integ_hash(integ_id, SK_a, keys->SK_integ_len, wpabuf_head(msg),
...
int ikev2_derive_sk_keys(const struct ikev2_prf_alg *prf,
                         ...
                         const struct ikev2_encr_alg *encr, const u8 *skeyseed,
  ...
  keybuf_len = keys->SK_d_len + 2 * keys->SK_integ_len + 2 * keys->SK_encr_len +
               2 * keys->SK_prf_len;
  keybuf = os_malloc(keybuf_len);
  if (keybuf == NULL)
    return -1;

  if (ikev2_prf_plus(prf->id, skeyseed, prf->hash_len, data, data_len, keybuf,
                     keybuf_len)) {
    os_free(keybuf);
    return -1;
  }
  ...
  if (keys->SK_ai) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "IKEV2: SK_ai", keys->SK_ai, keys->SK_integ_len);
  }
  ...
  if (keys->SK_ar) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "IKEV2: SK_ar", keys->SK_ar, keys->SK_integ_len);
  }
  pos += keys->SK_integ_len;
  ...
  if (keys->SK_pi) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "IKEV2: SK_pi", keys->SK_pi, keys->SK_prf_len);
  }
  pos += keys->SK_prf_len;
  ...
  if (keys->SK_pr) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "IKEV2: SK_pr", keys->SK_pr, keys->SK_prf_len);
  }
