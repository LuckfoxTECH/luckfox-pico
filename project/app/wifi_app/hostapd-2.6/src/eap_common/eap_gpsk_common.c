...
int eap_gpsk_supported_ciphersuite(int vendor, int specifier) {
  if (vendor == EAP_GPSK_VENDOR_IETF && specifier == EAP_GPSK_CIPHER_AES)
    return 1;
#ifdef EAP_GPSK_SHA256
  if (vendor == EAP_GPSK_VENDOR_IETF && specifier == EAP_GPSK_CIPHER_SHA256)
    return 1;
#endif /* EAP_GPSK_SHA256 */
  return 0;
}

static int eap_gpsk_gkdf_cmac(const u8 *psk /* Y */, const u8 *data /* Z */,
                              size_t data_len, u8 *buf, size_t len /* X */) {
  u8 *opos;
...
#ifdef EAP_GPSK_SHA256
static int eap_gpsk_gkdf_sha256(const u8 *psk /* Y */, const u8 *data /* Z */,
                                size_t data_len, u8 *buf, size_t len /* X */) {
  u8 *opos;
  size_t i, n, hashlen, left, clen;
...
static int eap_gpsk_derive_keys_helper(u32 csuite_specifier, u8 *kdf_out,
                                       size_t kdf_out_len, const u8 *psk,
                                       size_t psk_len, const u8 *seed,
                                       size_t seed_len, u8 *msk, u8 *emsk,
                                       u8 *sk, size_t sk_len, u8 *pk,
                                       size_t pk_len) {
  u8 mk[32], *pos, *data;
  size_t data_len, mk_len;
  int (*gkdf)(const u8 *_psk, const u8 *_data, size_t _data_len, u8 *buf,
              size_t len);
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-GPSK: Data to MK derivation", data, data_len);
...
static int eap_gpsk_derive_keys_aes(const u8 *psk, size_t psk_len,
                                    const u8 *seed, size_t seed_len, u8 *msk,
                                    u8 *emsk, u8 *sk, size_t *sk_len, u8 *pk,
                                    size_t *pk_len) {
...
#define EAP_GPSK_PK_LEN_AES 16
  ...
  *pk_len = EAP_GPSK_PK_LEN_AES;
  ...
  return eap_gpsk_derive_keys_helper(
      EAP_GPSK_CIPHER_AES, kdf_out, sizeof(kdf_out), psk, psk_len, seed,
      seed_len, msk, emsk, sk, *sk_len, pk, *pk_len);
}

#ifdef EAP_GPSK_SHA256
static int eap_gpsk_derive_keys_sha256(const u8 *psk, size_t psk_len,
                                       const u8 *seed, size_t seed_len, u8 *msk,
                                       u8 *emsk, u8 *sk, size_t *sk_len) {
...
#define EAP_GPSK_PK_LEN_SHA256 SHA256_MAC_LEN
  ...
  *sk_len = EAP_GPSK_SK_LEN_SHA256;
  ...
  return eap_gpsk_derive_keys_helper(EAP_GPSK_CIPHER_SHA256, kdf_out,
                                     sizeof(kdf_out), psk, psk_len, seed,
                                     seed_len, msk, emsk, sk, *sk_len, NULL, 0);
}
#endif /* EAP_GPSK_SHA256 */
...
int eap_gpsk_derive_keys(const u8 *psk, size_t psk_len, int vendor,
                         int specifier, const u8 *rand_peer,
                         const u8 *rand_server, const u8 *id_peer,
                         size_t id_peer_len, const u8 *id_server,
                         size_t id_server_len, u8 *msk, u8 *emsk, u8 *sk,
                         size_t *sk_len, u8 *pk, size_t *pk_len) {
  u8 *seed, *pos;
  int ret;

  wpa_printf(MSG_DEBUG, "EAP-GPSK: Deriving keys (%d:%d)", vendor, specifier);
  ...
  switch (specifier) {
  case EAP_GPSK_CIPHER_AES:
    ret = eap_gpsk_derive_keys_aes(psk, psk_len, seed, pos - seed, msk, emsk,
                                   sk, sk_len, pk, pk_len);
    break;
#ifdef EAP_GPSK_SHA256
  case EAP_GPSK_CIPHER_SHA256:
    ret = eap_gpsk_derive_keys_sha256(psk, psk_len, seed, pos - seed, msk, emsk,
                                      sk, sk_len);
    break;
#endif /* EAP_GPSK_SHA256 */
  default:
...
static int eap_gpsk_derive_mid_helper(u32 csuite_specifier, u8 *kdf_out,
                                      size_t kdf_out_len, const u8 *psk,
  ...
  default:
    wpa_printf(MSG_DEBUG,
               "EAP-GPSK: Unknown cipher %d used in "
...
#undef SID_LABEL
  ...
  pos += 2;
  os_memcpy(pos, seed, seed_len); /* inputString */
  wpa_hexdump(MSG_DEBUG, "EAP-GPSK: Data to Method ID derivation", data,
              data_len);
...
int eap_gpsk_derive_session_id(const u8 *psk, size_t psk_len, int vendor,
                               ...
                               const u8 *rand_server, const u8 *id_peer,
                               size_t id_peer_len, const u8 *id_server,
                               size_t id_server_len, u8 method_type, u8 *sid,
                               size_t *sid_len) {
  u8 *seed, *pos;
  u8 kdf_out[16];
  int ret;

  wpa_printf(MSG_DEBUG, "EAP-GPSK: Deriving Session ID(%d:%d)", vendor,
             specifier);
  ...
  ret = eap_gpsk_derive_mid_helper(specifier, kdf_out, sizeof(kdf_out), psk,
                                   seed, pos - seed, method_type);
...
static int eap_gpsk_compute_mic_aes(const u8 *sk, size_t sk_len, const u8 *data,
                                    size_t len, u8 *mic) {
  if (sk_len != 16) {
    wpa_printf(MSG_DEBUG,
               "EAP-GPSK: Invalid SK length %lu for "
               "AES-CMAC MIC",
 ...
 * Returns: 0 on success, -1 on failure
 */
int eap_gpsk_compute_mic(const u8 *sk, size_t sk_len, int vendor, int specifier,
