...
static void usage(void) {
  ...
  fprintf(stderr,
          ...
          "/var/run/hostapd)\n"
          "   -s<dir_path> dir path to open client sockets "
          "(default: " CONFIG_CTRL_IFACE_DIR ")\n"
...
static void register_event_handler(struct wpa_ctrl *ctrl) {
  if (!ctrl_conn)
    return;
  if (interactive) {
    event_handler_registered = !eloop_register_read_sock(
        wpa_ctrl_get_fd(ctrl), hostapd_cli_receive, NULL, NULL);
  }
}
...
static void hostapd_cli_msg_cb(char *msg, size_t len) { printf("%s\n", msg); }
...
static int hostapd_cli_cmd(struct wpa_ctrl *ctrl, const char *cmd, int min_args,
                           int argc, char *argv[]) {
  char buf[4096];
...
static int hostapd_cli_cmd_relog(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  return wpa_ctrl_command(ctrl, "RELOG");
}

static int hostapd_cli_cmd_status(struct wpa_ctrl *ctrl, int argc,
                                  char *argv[]) {
...
static int hostapd_cli_cmd_signature(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc != 1) {
    printf("Invalid 'signature' command - exactly one argument, STA address, "
           "is required.\n");
    return -1;
  }
...
static int hostapd_cli_cmd_wps_pin(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc > 3)
    snprintf(buf, sizeof(buf), "WPS_PIN %s %s %s %s", argv[0], argv[1], argv[2],
             argv[3]);
  else if (argc > 2)
    snprintf(buf, sizeof(buf), "WPS_PIN %s %s %s", argv[0], argv[1], argv[2]);
  else
...
static int hostapd_cli_cmd_wps_check_pin(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc == 2)
    res =
        os_snprintf(cmd, sizeof(cmd), "WPS_CHECK_PIN %s %s", argv[0], argv[1]);
  else
    res = os_snprintf(cmd, sizeof(cmd), "WPS_CHECK_PIN %s", argv[0]);
...
static int hostapd_cli_cmd_wps_nfc_config_token(struct wpa_ctrl *ctrl, int argc,
                                                char *argv[]) {
  char cmd[64];
  int res;
  ...
  res = os_snprintf(cmd, sizeof(cmd), "WPS_NFC_CONFIG_TOKEN %s", argv[0]);
  if (os_snprintf_error(sizeof(cmd), res)) {
...
static int hostapd_cli_cmd_wps_nfc_token(struct wpa_ctrl *ctrl, int argc,
                                         char *argv[]) {
  char cmd[64];
  int res;
...
static int hostapd_cli_cmd_nfc_get_handover_sel(struct wpa_ctrl *ctrl, int argc,
                                                char *argv[]) {
  char cmd[64];
  int res;
  ...
  res = os_snprintf(cmd, sizeof(cmd), "NFC_GET_HANDOVER_SEL %s %s", argv[0],
                    argv[1]);
  if (os_snprintf_error(sizeof(cmd), res)) {
...
static int hostapd_cli_cmd_wps_ap_pin(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc > 2)
    snprintf(buf, sizeof(buf), "WPS_AP_PIN %s %s %s", argv[0], argv[1],
             argv[2]);
  else if (argc > 1)
    snprintf(buf, sizeof(buf), "WPS_AP_PIN %s %s", argv[0], argv[1]);
  else
...
static int hostapd_cli_cmd_wps_config(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc > 3) {
    for (i = 0; i < 64; i++) {
      if (argv[3][i] == '\0')
        break;
      os_snprintf(&key_hex[i * 2], 3, "%02x", argv[3][i]);
    }
  }

  if (argc > 3)
    snprintf(buf, sizeof(buf), "WPS_CONFIG %s %s %s %s", ssid_hex, argv[1],
             argv[2], key_hex);
  else if (argc > 2)
    snprintf(buf, sizeof(buf), "WPS_CONFIG %s %s %s", ssid_hex, argv[1],
             argv[2]);
  else
    snprintf(buf, sizeof(buf), "WPS_CONFIG %s %s", ssid_hex, argv[1]);
  return wpa_ctrl_command(ctrl, buf);
}
#endif /* CONFIG_WPS */
...
static int hostapd_cli_cmd_disassoc_imminent(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(buf, sizeof(buf), "DISASSOC_IMMINENT %s %s", argv[0],
                    argv[1]);
...
static int hostapd_cli_cmd_ess_disassoc(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(buf, sizeof(buf), "ESS_DISASSOC %s %s %s", argv[0], argv[1],
                    argv[2]);
  if (os_snprintf_error(sizeof(buf), res))
    return -1;
  return wpa_ctrl_command(ctrl, buf);
...
static int hostapd_cli_cmd_bss_tm_req(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc < 1) {
    printf("Invalid 'bss_tm_req' command - at least one argument (STA addr) is "
           "needed\n");
    return -1;
  }
...
static int hostapd_cli_cmd_set_qos_map_set(struct wpa_ctrl *ctrl, int argc,
                                           char *argv[]) {
  char buf[200];
  int res;
...
static int hostapd_cli_cmd_send_qos_map_conf(struct wpa_ctrl *ctrl, int argc,
                                             char *argv[]) {
  char buf[50];
  int res;
...
static int hostapd_cli_cmd_hs20_wnm_notif(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(buf, sizeof(buf), "HS20_WNM_NOTIF %s %s", argv[0], argv[1]);
  if (os_snprintf_error(sizeof(buf), res))
    return -1;
  return wpa_ctrl_command(ctrl, buf);
}
...
static int hostapd_cli_cmd_hs20_deauth_req(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc < 3) {
    printf("Invalid 'hs20_deauth_req' command - at least three arguments (STA "
           "addr, Code, Re-auth Delay) are needed\n");
    return -1;
  }

  if (argc > 3)
    res = os_snprintf(buf, sizeof(buf), "HS20_DEAUTH_REQ %s %s %s %s", argv[0],
                      argv[1], argv[2], argv[3]);
  else
    res = os_snprintf(buf, sizeof(buf), "HS20_DEAUTH_REQ %s %s %s", argv[0],
                      argv[1], argv[2]);
  if (os_snprintf_error(sizeof(buf), res))
    return -1;
  return wpa_ctrl_command(ctrl, buf);
...
static int hostapd_cli_cmd_level(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  char cmd[256];
  if (argc != 1) {
...
static int hostapd_cli_cmd_chan_switch(struct wpa_ctrl *ctrl, int argc,
                                       char *argv[]) {
  char cmd[256];
  int res;
  int i;
  ...
  res = os_snprintf(cmd, sizeof(cmd), "CHAN_SWITCH %s %s", argv[0], argv[1]);
  if (os_snprintf_error(sizeof(cmd), res)) {
...
static int hostapd_cli_cmd_vendor(struct wpa_ctrl *ctrl, int argc,
                                  char *argv[]) {
  char cmd[256];
  int res;
...
static int hostapd_cli_cmd_log_level(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(cmd, sizeof(cmd), "LOG_LEVEL%s%s%s%s", argc >= 1 ? " " : "",
                    argc >= 1 ? argv[0] : "", argc == 2 ? " " : "",
                    argc == 2 ? argv[1] : "");
  if (os_snprintf_error(sizeof(cmd), res)) {
...
static int hostapd_cli_cmd_pmksa(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
...
static int hostapd_cli_cmd_set_neighbor(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(cmd, sizeof(cmd), "SET_NEIGHBOR %s %s %s %s %s", argv[0],
...
static int hostapd_cli_cmd_remove_neighbor(struct wpa_ctrl *ctrl, int argc,
  ...
  res =
      os_snprintf(cmd, sizeof(cmd), "REMOVE_NEIGHBOR %s %s", argv[0], argv[1]);
  if (os_snprintf_error(sizeof(cmd), res)) {
...
static int hostapd_cli_cmd_req_range(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc < 4) {
    printf(
        "Invalid req_range command: needs at least 4 arguments - dest address, "
        "randomization interval, min AP count, and 1 to 16 AP addresses\n");
    return -1;
  }
...
static const struct hostapd_cli_cmd hostapd_cli_commands[] = {
    {"ping", hostapd_cli_cmd_ping, NULL, "= pings hostapd"},
    ...
    {"new_sta", hostapd_cli_cmd_new_sta, NULL, "<addr> = add a new station"},
    {"deauthenticate", hostapd_cli_cmd_deauthenticate,
     hostapd_complete_deauthenticate, "<addr> = deauthenticate a station"},
    {"disassociate", hostapd_cli_cmd_disassociate,
     hostapd_complete_disassociate, "<addr> = disassociate a station"},
#ifdef CONFIG_TAXONOMY
    ...
    {"nfc_get_handover_sel", hostapd_cli_cmd_nfc_get_handover_sel, NULL, NULL},
#endif /* CONFIG_WPS_NFC */
    ...
    {"quit", hostapd_cli_cmd_quit, NULL, "= exit hostapd_cli"},
...
static void hostapd_cli_edit_eof_cb(void *ctx) { eloop_terminate(); }
