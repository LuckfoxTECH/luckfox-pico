...
#ifdef EAP_SERVER
static struct hostapd_radius_attr *hostapd_parse_radius_attr(const char *value);
#endif /* EAP_SERVER */
...
static int hostapd_config_read_vlan_file(struct hostapd_bss_config *bss,
  ...
  while (fgets(buf, sizeof(buf), f)) {
    ...
    if (buf[0] == '*') {
      ...
      if (buf == pos || vlan_id < 1 || vlan_id > MAX_VLAN_ID) {
...
static int hostapd_config_read_eap_user(const char *fname,
  ...
  if (os_strncmp(fname, "sqlite:", 7) == 0) {
    ...
    return 0;
#else  /* CONFIG_SQLITE */
    wpa_printf(MSG_ERROR, "EAP user file in SQLite DB, but CONFIG_SQLITE was "
  ...
  while (fgets(buf, sizeof(buf), f)) {
    ...
    if (user && os_strncmp(buf, "radius_accept_attr=", 19) == 0) {
      ...
      if (attr == NULL) {
        wpa_printf(MSG_ERROR, "Invalid radius_auth_req_attr: %s", buf + 19);
    ...
    while (*start) {
      ...
      user->methods[num_methods].method =
          eap_server_get_type(start, &user->methods[num_methods].vendor);
    ...
    if (*pos == '"') {
      ...
      if (hexstr2bin(pos, user->password, (pos2 - pos) / 2) < 0) {
        wpa_printf(MSG_ERROR,
                   "Invalid hex password "
                   "on line %d in '%s'",
                   line, fname);
  ...
  while (*start != '\0') {
    ...
    *end = '\0';
    if (os_strcmp(start, "WPA-PSK") == 0)
      ...
      wpa_printf(MSG_ERROR, "Line %d: invalid key_mgmt '%s'", line, start);
      os_free(buf);
      return -1;
    }
...
static int hostapd_config_parse_cipher(int line, const char *value) {
  ...
  if (val < 0) {
    wpa_printf(MSG_ERROR, "Line %d: invalid cipher '%s'.", line, value);
    return -1;
  }
  if (val == 0) {
    wpa_printf(MSG_ERROR, "Line %d: no cipher values configured.", line);
    return -1;
  }
  return val;
}
...
static int valid_cw(int cw) {
  return (cw == 1 || cw == 3 || cw == 7 || cw == 15 || cw == 31 || cw == 63 ||
          cw == 127 || cw == 255 || cw == 511 || cw == 1023 || cw == 2047 ||
          cw == 4095 || cw == 8191 || cw == 16383 || cw == 32767);
}
...
static int hostapd_config_tx_queue(struct hostapd_config *conf,
  ...
  pos = name + 9;
  if (os_strncmp(pos, "data", 4) == 0 && pos[4] >= '0' && pos[4] <= '9' &&
      pos[5] == '_') {
    num = pos[4] - '0';
    pos += 6;
  ...
  if (os_strcmp(pos, "aifs") == 0) {
    ...
    if (queue->aifs < 0 || queue->aifs > 255) {
      wpa_printf(MSG_ERROR, "Invalid AIFS value %d", queue->aifs);
      return -1;
    }
  } else if (os_strcmp(pos, "cwmin") == 0) {
    queue->cwmin = atoi(val);
    if (!valid_cw(queue->cwmin)) {
      wpa_printf(MSG_ERROR, "Invalid cwMin value %d", queue->cwmin);
      return -1;
    }
  } else if (os_strcmp(pos, "cwmax") == 0) {
    queue->cwmax = atoi(val);
    if (!valid_cw(queue->cwmax)) {
      wpa_printf(MSG_ERROR, "Invalid cwMax value %d", queue->cwmax);
      return -1;
    }
...
static int parse_lang_string(struct hostapd_lang_string **array,
  ...
  *sep++ = '\0';
  ...
  ls = os_realloc_array(*array, *count + 1, sizeof(struct hostapd_lang_string));
  if (ls == NULL)
    goto fail;
...
static int parse_venue_name(struct hostapd_bss_config *bss, char *pos,
  ...
  if (parse_lang_string(&bss->venue_name, &bss->venue_name_count, pos)) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid venue_name '%s'", line, pos);
    return -1;
  }
  return 0;
}
...
static int parse_3gpp_cell_net(struct hostapd_bss_config *bss, char *buf,
  ...
  *ipos++ = count; /* Number of PLMNs */
  ...
  while (pos && *pos) {
    ...
    *ipos++ =
        (((mnc_len == 2) ? 0xf0 : ((mnc[2] - '0') << 4))) | (mcc[2] - '0');
...
fail:
  wpa_printf(MSG_ERROR, "Line %d: Invalid anqp_3gpp_cell_net: %s", line, buf);
  os_free(info);
  return -1;
}

static int parse_nai_realm(struct hostapd_bss_config *bss, char *buf,
                           int line) {
  ...
  offsets = os_calloc(bss->nai_realm_count * MAX_NAI_REALMS, sizeof(int));
  if (offsets == NULL)
    return -1;
  ...
  for (i = 0; i < bss->nai_realm_count; i++) {
    ...
    for (j = 0; j < MAX_NAI_REALMS; j++) {
      offsets[i * MAX_NAI_REALMS + j] =
          realm->realm[j] ? realm->realm[j] - realm->realm_buf : -1;
    }
  }
...
static int parse_anqp_elem(struct hostapd_bss_config *bss, char *buf,
                           int line) {
  char *delim;
  u16 infoid;
  size_t len;
...
static int parse_qos_map_set(struct hostapd_bss_config *bss, char *buf,
                             int line) {
  ...
  if (count < 16 || count & 1) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid qos_map_set '%s'", line, buf);
    return -1;
  }
...
fail:
  wpa_printf(MSG_ERROR, "Line %d: Invalid hs20_wan_metrics '%s'", line, buf);
  os_free(wan_metrics);
  return -1;
}
...
static int hs20_parse_osu_ssid(struct hostapd_bss_config *bss, char *pos,
                               int line) {
  size_t slen;
  char *str;
...
static int hs20_parse_osu_server_uri(struct hostapd_bss_config *bss, char *pos,
                                     int line) {
...
static int hs20_parse_osu_friendly_name(struct hostapd_bss_config *bss,
  ...
  if (parse_lang_string(&bss->last_osu->friendly_name,
    ...
    wpa_printf(MSG_ERROR, "Line %d: Invalid osu_friendly_name '%s'", line, pos);
    return -1;
  }
...
static int hs20_parse_osu_nai(struct hostapd_bss_config *bss, char *pos,
                              int line) {
  if (bss->last_osu == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: Unexpected OSU field", line);
    return -1;
...
static int hs20_parse_osu_service_desc(struct hostapd_bss_config *bss,
  ...
  if (parse_lang_string(&bss->last_osu->service_desc,
    ...
    wpa_printf(MSG_ERROR, "Line %d: Invalid osu_service_desc '%s'", line, pos);
    return -1;
  }
...
static int parse_wpabuf_hex(int line, const char *name, struct wpabuf **buf,
  ...
  if (!elems) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid %s '%s'", line, name, val);
    return -1;
  }
...
static int hostapd_config_fill(struct hostapd_config *conf,
                               struct hostapd_bss_config *bss, const char *buf,
                               char *pos, int line) {
  if (os_strcmp(buf, "interface") == 0) {
    os_strlcpy(conf->bss[0]->iface, pos, sizeof(conf->bss[0]->iface));
    ...
    if (conf->driver == NULL) {
      wpa_printf(MSG_ERROR, "Line %d: invalid/unknown driver '%s'", line, pos);
      return 1;
    }
    ...
    wpa_printf(MSG_DEBUG,
               "Line %d: DEPRECATED: 'debug' configuration variable is not "
               "used anymore",
               line);
    ...
    wpa_printf(MSG_INFO,
               "Line %d: DEPRECATED: 'dump_file' configuration variable is not "
               "used anymore",
               line);
    ...
    if (bss->macaddr_acl != ACCEPT_UNLESS_DENIED &&
      ...
      wpa_printf(MSG_ERROR, "Line %d: unknown macaddr_acl %d", line,
                 bss->macaddr_acl);
    }
    ...
    if (hostapd_config_read_maclist(pos, &bss->deny_mac, &bss->num_deny_mac)) {
      wpa_printf(MSG_ERROR, "Line %d: Failed to read deny_mac_file '%s'", line,
                 pos);
      return 1;
    }
    ...
    if (bss->eapol_version < 1 || bss->eapol_version > 2) {
      wpa_printf(MSG_ERROR, "Line %d: invalid EAPOL version (%d): '%s'.", line,
                 bss->eapol_version, pos);
      return 1;
    }
    ...
    wpa_printf(MSG_ERROR,
               "Line %d: obsolete eap_authenticator used; this has been "
               "renamed to eap_server",
               line);
    ...
    if (bss->pac_opaque_encr_key == NULL) {
      wpa_printf(MSG_ERROR, "Line %d: No memory for pac_opaque_encr_key", line);
      return 1;
    } else if (hexstr2bin(pos, bss->pac_opaque_encr_key, 16)) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid pac_opaque_encr_key", line);
      return 1;
    }
  } else if (os_strcmp(buf, "eap_fast_a_id") == 0) {
    size_t idlen = os_strlen(pos);
    if (idlen & 1) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid eap_fast_a_id", line);
      return 1;
    }
    ...
    if (bss->eap_fast_a_id == NULL ||
      ...
      wpa_printf(MSG_ERROR, "Line %d: Failed to parse eap_fast_a_id", line);
      os_free(bss->eap_fast_a_id);
    ...
    if (bss->eap_req_id_text == NULL) {
      wpa_printf(MSG_ERROR,
                 "Line %d: Failed to allocate memory for eap_req_id_text",
                 line);
      return 1;
    }
    ...
    if (term) {
      *term++ = '\0';
      os_memmove(term, term + 1,
                 bss->eap_req_id_text_len - (term - bss->eap_req_id_text) - 1);
    ...
    if (bss->default_wep_key_len > 13) {
      wpa_printf(MSG_ERROR, "Line %d: invalid WEP key len %lu (= %lu bits)",
                 line, (unsigned long)bss->default_wep_key_len,
                 (unsigned long)bss->default_wep_key_len * 8);
      return 1;
    }
  } else if (os_strcmp(buf, "wep_key_len_unicast") == 0) {
    bss->individual_wep_key_len = atoi(pos);
    if (bss->individual_wep_key_len < 0 || bss->individual_wep_key_len > 13) {
      wpa_printf(MSG_ERROR, "Line %d: invalid WEP key len %d (= %d bits)", line,
                 bss->individual_wep_key_len, bss->individual_wep_key_len * 8);
      return 1;
    }
  } else if (os_strcmp(buf, "wep_rekey_period") == 0) {
    bss->wep_rekeying_period = atoi(pos);
    if (bss->wep_rekeying_period < 0) {
      wpa_printf(MSG_ERROR, "Line %d: invalid period %d", line,
                 bss->wep_rekeying_period);
      return 1;
    }
    ...
    if (hostapd_parse_ip_addr(pos, &bss->own_ip_addr)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid IP address '%s'", line, pos);
      return 1;
    }
    ...
    if (hostapd_config_read_radius_addr(&bss->radius->auth_servers,
                                        &bss->radius->num_auth_servers, pos,
                                        1812, &bss->radius->auth_server)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid IP address '%s'", line, pos);
      return 1;
    }
  } else if (bss->radius->auth_server &&
             os_strcmp(buf, "auth_server_addr_replace") == 0) {
    if (hostapd_parse_ip_addr(pos, &bss->radius->auth_server->addr)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid IP address '%s'", line, pos);
      return 1;
    }
  } else if (bss->radius->auth_server &&
    ...
    if (hostapd_config_read_radius_addr(&bss->radius->acct_servers,
                                        &bss->radius->num_acct_servers, pos,
                                        1813, &bss->radius->acct_server)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid IP address '%s'", line, pos);
      return 1;
    }
  } else if (bss->radius->acct_server &&
             os_strcmp(buf, "acct_server_addr_replace") == 0) {
    if (hostapd_parse_ip_addr(pos, &bss->radius->acct_server->addr)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid IP address '%s'", line, pos);
      return 1;
    }
  } else if (bss->radius->acct_server &&
    ...
    if (attr == NULL) {
      wpa_printf(MSG_ERROR, "Line %d: invalid radius_auth_req_attr", line);
      return 1;
    ...
    attr = hostapd_parse_radius_attr(pos);
    if (attr == NULL) {
      wpa_printf(MSG_ERROR, "Line %d: invalid radius_acct_req_attr", line);
      return 1;
    ...
    if (hostapd_parse_das_client(bss, pos) < 0) {
      wpa_printf(MSG_ERROR, "Line %d: invalid DAS client", line);
      return 1;
    }
    ...
    if (bss->max_num_sta < 0 || bss->max_num_sta > MAX_STA_COUNT) {
      wpa_printf(MSG_ERROR,
                 "Line %d: Invalid max_num_sta=%d; allowed range 0..%d", line,
    ...
    if (hexstr2bin(pos, bss->ssid.wpa_psk->psk, PMK_LEN) ||
        pos[PMK_LEN * 2] != '\0') {
      wpa_printf(MSG_ERROR, "Line %d: Invalid PSK '%s'.", line, pos);
      hostapd_config_clear_wpa_psk(&bss->ssid.wpa_psk);
      return 1;
    }
    ...
    if (bss->wpa_psk_radius != PSK_RADIUS_IGNORED &&
      ...
      wpa_printf(MSG_ERROR, "Line %d: unknown wpa_psk_radius %d", line,
                 bss->wpa_psk_radius);
      return 1;
    }
    ...
    if (os_strlen(pos) != 2 * MOBILITY_DOMAIN_ID_LEN ||
        hexstr2bin(pos, bss->mobility_domain, MOBILITY_DOMAIN_ID_LEN) != 0) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid mobility_domain '%s'", line, pos);
      return 1;
    }
  } else if (os_strcmp(buf, "r1_key_holder") == 0) {
    if (os_strlen(pos) != 2 * FT_R1KH_ID_LEN ||
        hexstr2bin(pos, bss->r1_key_holder, FT_R1KH_ID_LEN) != 0) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid r1_key_holder '%s'", line, pos);
      return 1;
    }
    ...
    if (add_r0kh(bss, pos) < 0) {
      wpa_printf(MSG_DEBUG, "Line %d: Invalid r0kh '%s'", line, pos);
      return 1;
    }
  } else if (os_strcmp(buf, "r1kh") == 0) {
    if (add_r1kh(bss, pos) < 0) {
      wpa_printf(MSG_DEBUG, "Line %d: Invalid r1kh '%s'", line, pos);
      return 1;
    }
    ...
    if (*group == '\0' || *endp != '\0') {
      wpa_printf(MSG_DEBUG, "Line %d: Invalid group '%s'", line, group);
      return 1;
    }
    bss->ctrl_interface_gid_set = 1;
    wpa_printf(MSG_DEBUG, "ctrl_interface_group=%d", bss->ctrl_interface_gid);
#endif /* CONFIG_NATIVE_WINDOWS */
    ...
    if (os_strcmp(pos, "a") == 0)
      ...
      wpa_printf(MSG_ERROR, "Line %d: unknown hw_mode '%s'", line, pos);
      return 1;
    }
    ...
    if (os_strcmp(pos, "ad") == 0)
      ...
      wpa_printf(MSG_ERROR, "Line %d: unknown wps_rf_band '%s'", line, pos);
      return 1;
    }
...
#ifndef CONFIG_ACS
      wpa_printf(MSG_ERROR,
                 "Line %d: tries to enable ACS but CONFIG_ACS disabled", line);
      return 1;
#else  /* CONFIG_ACS */
      conf->acs = 1;
      conf->channel = 0;
    ...
    if (hostapd_parse_chanlist(conf, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid channel list", line);
      return 1;
    }
    ...
    if (val < 15 || val > 65535) {
      wpa_printf(MSG_ERROR,
                 "Line %d: invalid beacon_int %d (expected 15..65535)", line,
                 val);
      return 1;
    }
    conf->beacon_int = val;
    ...
    if (val <= 0 || val > 100) {
      wpa_printf(MSG_ERROR,
                 "Line %d: invalid acs_num_scans %d (expected 1..100)", line,
                 val);
      return 1;
    }
    ...
    if (hostapd_config_parse_acs_chan_bias(conf, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid acs_chan_bias", line);
      return -1;
    }
#endif /* CONFIG_ACS */
    ...
    if (bss->dtim_period < 1 || bss->dtim_period > 255) {
      wpa_printf(MSG_ERROR, "Line %d: invalid dtim_period %d", line,
                 bss->dtim_period);
      return 1;
    }
  } else if (os_strcmp(buf, "bss_load_update_period") == 0) {
    bss->bss_load_update_period = atoi(pos);
    if (bss->bss_load_update_period < 0 || bss->bss_load_update_period > 100) {
      wpa_printf(MSG_ERROR, "Line %d: invalid bss_load_update_period %d", line,
    ...
    if (conf->rts_threshold < -1 || conf->rts_threshold > 65535) {
      wpa_printf(MSG_ERROR, "Line %d: invalid rts_threshold %d", line,
                 conf->rts_threshold);
      return 1;
    }
    ...
    if (conf->fragm_threshold == -1) {
      /* allow a value of -1 */
    } else if (conf->fragm_threshold < 256 || conf->fragm_threshold > 2346) {
      wpa_printf(MSG_ERROR, "Line %d: invalid fragm_threshold %d", line,
    ...
    if (val != 0 && val != 1) {
      wpa_printf(MSG_ERROR,
                 "Line %d: invalid send_probe_response %d (expected 0 or 1)",
                 line, val);
      return 1;
    }
    ...
    if (hostapd_parse_intlist(&conf->supported_rates, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid rate list", line);
      return 1;
    }
    ...
    if (bss->ssid.wep.idx > 3) {
      wpa_printf(MSG_ERROR, "Invalid wep_default_key index %d",
                 bss->ssid.wep.idx);
      return 1;
    }
    ...
    if (hostapd_config_read_wep(&bss->ssid.wep, buf[7] - '0', pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid WEP key '%s'", line, buf);
      return 1;
    }
#ifndef CONFIG_NO_VLAN
    ...
    if (hostapd_config_read_vlan_file(bss, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: failed to read VLAN file '%s'", line,
                 pos);
      return 1;
    }
    ...
    if (bss->ssid.vlan_naming >= DYNAMIC_VLAN_NAMING_END ||
      ...
      wpa_printf(MSG_ERROR, "Line %d: invalid naming scheme %d", line,
    ...
    if (hostapd_config_tx_queue(conf, buf, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid TX queue item", line);
      return 1;
    }
    ...
    if (hostapd_config_bss(conf, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid bss item", line);
      return 1;
    }
  } else if (os_strcmp(buf, "bssid") == 0) {
    if (hwaddr_aton(pos, bss->bssid)) {
      wpa_printf(MSG_ERROR, "Line %d: invalid bssid item", line);
      return 1;
    }
    ...
    if (os_strcmp(pos, "AES-128-CMAC") == 0) {
      ...
      wpa_printf(MSG_ERROR, "Line %d: invalid group_mgmt_cipher: %s", line,
                 pos);
      return 1;
    }
    ...
    if (hostapd_config_ht_capab(conf, pos) < 0) {
      wpa_printf(MSG_ERROR, "Line %d: invalid ht_capab", line);
      return 1;
    }
    ...
    if (hostapd_config_vht_capab(conf, pos) < 0) {
      wpa_printf(MSG_ERROR, "Line %d: invalid vht_capab", line);
      return 1;
    }
    ...
    if (bss->wps_state < 0 || bss->wps_state > 2) {
      wpa_printf(MSG_ERROR, "Line %d: invalid wps_state", line);
      return 1;
    }
    ...
    if (os_strlen(pos) > 64) {
      wpa_printf(MSG_ERROR, "Line %d: Too long manufacturer", line);
      return 1;
    }
    ...
    if (os_strlen(pos) > 32) {
      wpa_printf(MSG_ERROR, "Line %d: Too long model_name", line);
      return 1;
    }
    ...
    if (os_strlen(pos) > 32) {
      wpa_printf(MSG_ERROR, "Line %d: Too long model_number", line);
      return 1;
    }
    ...
    if (os_strlen(pos) > 32) {
      wpa_printf(MSG_ERROR, "Line %d: Too long serial_number", line);
      return 1;
    }
    ...
    bss->ap_settings = (u8 *)os_readfile(pos, &bss->ap_settings_len);
    ...
    if (bss->wps_nfc_dev_pw_id < 0x10 || bss->wps_nfc_dev_pw_id > 0xffff) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid wps_nfc_dev_pw_id value", line);
      return 1;
    }
    ...
    if (tz_len < 4 || tz_len > 255) {
      wpa_printf(MSG_DEBUG, "Line %d: invalid time_zone", line);
      return 1;
    }
    ...
    if (bss->access_network_type < 0 || bss->access_network_type > 15) {
      wpa_printf(MSG_ERROR, "Line %d: invalid access_network_type", line);
      return 1;
    }
    ...
    if (hexstr2bin(pos, &auth_type, 1)) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid network_auth_type '%s'", line,
                 pos);
      return 1;
    }
    ...
    *bss->network_auth_type = auth_type;
    ...
    if (redirect_url_len)
      os_memcpy(bss->network_auth_type + 3, pos + 2, redirect_url_len);
    ...
    if (oper_class_len < 2 || (oper_class_len & 0x01)) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid hs20_operating_class '%s'", line,
                 pos);
      return 1;
    }
    oper_class_len /= 2;
    ...
    if (hs20_parse_icon(bss, pos) < 0) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid hs20_icon '%s'", line, pos);
      return 1;
    }
...
#define PARSE_TEST_PROBABILITY(_val)                                           \
    ...
    if (*end || conf->_val < 0.0 || conf->_val > 1.0) {                        \
      wpa_printf(MSG_ERROR, "Line %d: Invalid value '%s'", line, pos);         \
      return 1;                                                                \
    }
    ...
    pos = os_strchr(pos, ':');
    if (pos == NULL) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid bss_load_test", line);
      return 1;
    }
    pos++;
    ...
    if (hexstr2bin(pos, wpabuf_put(tmp, len), len)) {
      wpabuf_free(tmp);
      wpa_printf(MSG_ERROR, "Line %d: Invalid own_ie_override '%s'", line, pos);
      return 1;
    }
    ...
    if (hostapd_parse_intlist(&bss->sae_groups, pos)) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid sae_groups value '%s'", line,
                 pos);
      return 1;
    }
    ...
    if (val < 0 || val > 255) {
      wpa_printf(MSG_ERROR,
                 "Line %d: Invalid local_pwr_constraint %d (expected 0..255)",
                 line, val);
      return 1;
    }
    ...
    if (!len || len >= sizeof(conf->fst_cfg.group_id)) {
      wpa_printf(MSG_ERROR, "Line %d: Invalid fst_group_id value '%s'", line,
                 pos);
      return 1;
    }

    if (conf->fst_cfg.group_id[0]) {
      wpa_printf(MSG_ERROR, "Line %d: Duplicate fst_group value '%s'", line,
                 pos);
      return 1;
    }

    os_strlcpy(conf->fst_cfg.group_id, pos, sizeof(conf->fst_cfg.group_id));
    ...
    if (!*pos) {
      wpa_printf(MSG_ERROR,
                 "Line %d: fst_llt value not supplied (expected 1..%u)", line,
                 FST_MAX_LLT_MS);
      return -1;
    }
    val = strtol(pos, &endp, 0);
    if (*endp || val < 1 || (unsigned long int)val > FST_MAX_LLT_MS) {
      wpa_printf(MSG_ERROR,
                 "Line %d: Invalid fst_llt %ld (%s) (expected 1..%u)", line,
                 val, pos, FST_MAX_LLT_MS);
      return 1;
    }
    ...
    wpa_printf(MSG_ERROR, "Line %d: unknown configuration item '%s'", line,
               buf);
    return 1;
  }
