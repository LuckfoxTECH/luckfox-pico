...
static void hostapd_ctrl_iface_send(struct hostapd_data *hapd, int level,
                                    enum wpa_msg_type type, const char *buf,
                                    size_t len);
...
static int hostapd_ctrl_iface_new_sta(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG,
             "Add new STA " MACSTR " based on ctrl_iface "
             "notification",
             MAC2STR(addr));
...
static int hostapd_ctrl_iface_wps_check_pin(struct hostapd_data *hapd,
                                            char *cmd, char *buf,
                                            size_t buflen) {
  char pin[9];
  size_t len;
...
static int hostapd_ctrl_iface_wps_get_status(struct hostapd_data *hapd,
  ...
  ret = os_snprintf(
      pos, end - pos, "Last WPS result: %s\n",
      (hapd->wps_stats.status == WPS_STATUS_SUCCESS
           ? "Success"
           : (hapd->wps_stats.status == WPS_STATUS_FAILURE ? "Failed"
                                                           : "None")));
...
static int hostapd_ctrl_iface_send_qos_map_conf(struct hostapd_data *hapd,
  ...
  if (!sta->qos_map_enabled) {
    wpa_printf(MSG_DEBUG,
               "Station " MACSTR " did not indicate "
               "support for QoS Map",
               MAC2STR(addr));
    return -1;
  }
...
static int hostapd_ctrl_iface_bss_tm_req(struct hostapd_data *hapd,
  ...
  if (sta == NULL) {
    wpa_printf(MSG_DEBUG,
               "Station " MACSTR " not found for BSS TM Request message",
               MAC2STR(addr));
    return -1;
  }
  ...
  while (pos) {
    ...
    if (nei_pos + 15 > nei_rep + sizeof(nei_rep)) {
      wpa_printf(MSG_DEBUG, "Not enough room for additional neighbor");
      return -1;
    }
    pos += 10;
    ...
    *nei_pos++ = atoi(pos); /* PHY Type */
    ...
    if (tmp && (!end || tmp < end)) {
      ...
      if (nei_pos + len / 2 > nei_rep + sizeof(nei_rep)) {
        wpa_printf(MSG_DEBUG, "Not enough room for neighbor subelements");
        return -1;
      }
      if (len & 0x01 || hexstr2bin(pos, nei_pos, len / 2) < 0) {
        wpa_printf(MSG_DEBUG, "Invalid neighbor subelement info");
        return -1;
      }
      nei_pos += len / 2;
      pos = end;
  ...
  if (pos) {
    ...
    if (ret != 3) {
      wpa_printf(MSG_DEBUG, "MBO requires three arguments: "
                            "mbo=<reason>:<reassoc_delay>:<cell_pref>");
      return -1;
    }

    if (mbo_reason > MBO_TRANSITION_REASON_PREMIUM_AP) {
      wpa_printf(MSG_DEBUG, "Invalid MBO transition reason code %u",
                 mbo_reason);
      return -1;
    }
    ...
    if (reassoc_delay > 65535 ||
        (reassoc_delay && !(req_mode & WNM_BSS_TM_REQ_DISASSOC_IMMINENT))) {
      wpa_printf(
          MSG_DEBUG,
  ...
  ret =
      wnm_send_bss_tm_req(hapd, sta, req_mode, disassoc_timer, valid_int,
                          bss_term_dur, url, nei_pos > nei_rep ? nei_rep : NULL,
                          nei_pos - nei_rep, mbo_len ? mbo : NULL, mbo_len);
  os_free(url);
  return ret;
}
...
static int hostapd_ctrl_iface_get_key_mgmt(struct hostapd_data *hapd, char *buf,
                                           size_t buflen) {
  int ret = 0;
  char *pos, *end;
...
static int hostapd_ctrl_iface_get_config(struct hostapd_data *hapd, char *buf,
                                         size_t buflen) {
  int ret;
  char *pos, *end;
  ...
  ret = os_snprintf(
      ...
      hapd->conf->wps_state == 0
          ? "disabled"
          : (hapd->conf->wps_state == 1 ? "not configured" : "configured"));
  if (os_snprintf_error(end - pos, ret))
    return pos - buf;
  pos += ret;
  ...
  if (hapd->conf->wps_state && hapd->conf->wpa && hapd->conf->ssid.wpa_psk &&
    ...
    if (os_strcasecmp(cmd, "deny_mac_file") == 0) {
      ...
      for (sta = hapd->sta_list; sta; sta = sta->next) {
        if (!hostapd_maclist_found(hapd->conf->accept_mac,
                                   hapd->conf->num_accept_mac, sta->addr,
                                   &vlan_id) ||
            (vlan_id.notempty && vlan_compare(&vlan_id, sta->vlan_desc)))
          ap_sta_disconnect(hapd, sta, sta->addr, WLAN_REASON_UNSPECIFIED);
      }
...
static int hostapd_ctrl_iface_data_test_config(struct hostapd_data *hapd,
  ...
  hapd->l2_test = l2_packet_init(ifname, hapd->own_addr, ETHERTYPE_IP,
...
static int hostapd_ctrl_iface_data_test_tx(struct hostapd_data *hapd,
                                           char *cmd) {
...
static int hostapd_ctrl_iface_data_test_frame(struct hostapd_data *hapd,
  ...
  l2 = l2_packet_init(ifname, hapd->own_addr, ethertype, hostapd_data_test_rx,
                      hapd, 1);
  if (l2 == NULL)
    goto done;
...
static int hostapd_ctrl_get_alloc_fail(struct hostapd_data *hapd, char *buf,
                                       size_t buflen) {
#ifdef WPA_TRACE_BFD
...
static int hostapd_ctrl_get_fail(struct hostapd_data *hapd, char *buf,
                                 size_t buflen) {
#ifdef WPA_TRACE_BFD
...
static int hostapd_ctrl_iface_vendor(struct hostapd_data *hapd, char *cmd,
  ...
  ret = hostapd_drv_vendor_cmd(hapd, vendor_id, subcmd, data, data_len, reply);
...
static int hostapd_ctrl_iface_track_sta_list(struct hostapd_data *hapd,
  ...
  os_get_reltime(&now);
  dl_list_for_each_reverse(info, &iface->sta_seen, struct hostapd_sta_info,
                           list) {
...
static int hostapd_ctrl_iface_req_range(struct hostapd_data *hapd, char *cmd) {
  ...
  return hostapd_send_range_req(hapd, addr, random_interval, min_ap, responders,
                                n_responders);
}

static int hostapd_ctrl_iface_set_neighbor(struct hostapd_data *hapd,
                                           char *buf) {
...
static int hostapd_ctrl_iface_remove_neighbor(struct hostapd_data *hapd,
  ...
  tmp = os_strstr(buf, "ssid=");
  if (!tmp || ssid_parse(tmp + 5, &ssid)) {
    wpa_printf(MSG_ERROR, "CTRL: REMOVE_NEIGHBORr: Bad or missing SSID");
    return -1;
  }
...
static int hostapd_ctrl_iface_receive_process(struct hostapd_data *hapd,
  ...
  if (os_strcmp(buf, "PING") == 0) {
    ...
    if (reply_len >= 0) {
      res = radius_client_get_mib(hapd->radius, reply + reply_len,
                                  reply_size - reply_len);
      if (res < 0)
        reply_len = -1;
      else
    ...
    reply_len =
        hostapd_ctrl_iface_wps_check_pin(hapd, buf + 14, reply, reply_size);
    ...
    reply_len =
        hostapd_ctrl_iface_wps_ap_pin(hapd, buf + 11, reply, reply_size);
    ...
    reply_len = hostapd_ctrl_iface_wps_nfc_config_token(hapd, buf + 21, reply,
                                                        reply_size);
  } else if (os_strncmp(buf, "WPS_NFC_TOKEN ", 14) == 0) {
    reply_len =
        hostapd_ctrl_iface_wps_nfc_token(hapd, buf + 14, reply, reply_size);
  } else if (os_strncmp(buf, "NFC_GET_HANDOVER_SEL ", 21) == 0) {
    reply_len = hostapd_ctrl_iface_nfc_get_handover_sel(hapd, buf + 21, reply,
                                                        reply_size);
...
static void hostapd_ctrl_iface_receive(int sock, void *eloop_ctx,
  ...
  if (os_strcmp(buf, "GET_COOKIE") == 0) {
    ...
    wpa_snprintf_hex(reply + 7, 2 * COOKIE_LEN + 1, cookie, COOKIE_LEN);
  ...
  reply_len = hostapd_ctrl_iface_receive_process(hapd, pos, reply, reply_size,
                                                 &from, fromlen);

#ifdef CONFIG_CTRL_IFACE_UDP
done:
#endif /* CONFIG_CTRL_IFACE_UDP */
  if (sendto(sock, reply, reply_len, 0, (struct sockaddr *)&from, fromlen) <
      0) {
    wpa_printf(MSG_DEBUG, "CTRL: sendto failed: %s", strerror(errno));
  }
  os_free(reply);
...
static char *hostapd_ctrl_iface_path(struct hostapd_data *hapd) {
  ...
  len =
      os_strlen(hapd->conf->ctrl_interface) + os_strlen(hapd->conf->iface) + 2;
  buf = os_malloc(len);
  if (buf == NULL)
    return NULL;
...
try_again:
  ...
  if (eloop_register_read_sock(hapd->ctrl_sock, hostapd_ctrl_iface_receive,
                               hapd, NULL) < 0) {
...
fail:
  ...
  if (hapd->conf->ctrl_interface_gid_set &&
      chown(hapd->conf->ctrl_interface, -1, hapd->conf->ctrl_interface_gid) <
          0) {
  ...
  if (bind(s, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    ...
    if (connect(s, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
      ...
      if (bind(s, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        wpa_printf(MSG_ERROR, "hostapd-ctrl-iface: bind(PF_UNIX): %s",
                   strerror(errno));
        goto fail;
      }
      wpa_printf(MSG_DEBUG,
  ...
  if (hapd->conf->ctrl_interface_gid_set &&
    ...
    wpa_printf(MSG_ERROR, "chown[ctrl_interface/ifname]: %s", strerror(errno));
    goto fail;
  }
  ...
  if (hapd->ctrl_sock > -1) {
    ...
    if (hapd->conf->ctrl_interface && rmdir(hapd->conf->ctrl_interface) < 0) {
      if (errno == ENOTEMPTY) {
        ...
        wpa_printf(MSG_ERROR, "rmdir[ctrl_interface=%s]: %s",
                   hapd->conf->ctrl_interface, strerror(errno));
      }
    }
#endif /* !CONFIG_CTRL_IFACE_UDP */
  }
...
static int hostapd_ctrl_iface_dup_param(struct hostapd_data *src_hapd,
  ...
  if (os_strcmp(param, "wpa") == 0) {
    ...
    res = hostapd_ctrl_iface_get_key_mgmt(src_hapd, value,
    ...
    res = wpa_write_ciphers(value, value + HOSTAPD_CLI_DUP_VALUE_MAX_LEN,
                            src_hapd->conf->wpa_pairwise, " ");
    if (res < 0)
      goto error_stringify;
...
static int hostapd_global_ctrl_iface_ifname(struct hapd_interfaces *interfaces,
  ...
  return hostapd_ctrl_iface_receive_process(hapd, buf, reply, reply_size, from,
                                            fromlen);
}
...
static void hostapd_global_ctrl_iface_receive(int sock, void *eloop_ctx,
  ...
  res = recvfrom(sock, buffer, sizeof(buffer) - 1, 0, (struct sockaddr *)&from,
                 &fromlen);
  if (res < 0) {
    wpa_printf(MSG_ERROR, "recvfrom(ctrl_iface): %s", strerror(errno));
    return;
  ...
  if (os_strcmp(buf, "GET_COOKIE") == 0) {
    os_memcpy(reply, "COOKIE=", 7);
    wpa_snprintf_hex(reply + 7, 2 * COOKIE_LEN + 1, gcookie, COOKIE_LEN);
    reply_len = 7 + 2 * COOKIE_LEN;
    goto send_reply;
  }
    ...
    if (pos) {
      *pos++ = '\0';
      reply_len = hostapd_global_ctrl_iface_ifname(
          interfaces, buf + 7, pos, reply, reply_size, &from, fromlen);
      goto send_reply;
    }
  }
  ...
  if (os_strcmp(buf, "PING") == 0) {
    ...
    if (hostapd_global_ctrl_iface_attach(interfaces, &from, fromlen))
      reply_len = -1;
  } else if (os_strcmp(buf, "DETACH") == 0) {
    ...
    reply_len = hostapd_global_ctrl_iface_interfaces(interfaces, buf + 10,
                                                     reply, sizeof(buffer));
...
try_again:
  ...
  if (eloop_register_read_sock(interface->global_ctrl_sock,
                               hostapd_global_ctrl_iface_receive, interface,
                               NULL) < 0) {
...
fail:
  ...
  if (os_strlen(interface->global_iface_path) + 1 +
          os_strlen(interface->global_iface_name) >=
      sizeof(addr.sun_path))
    goto fail;
  ...
  eloop_register_read_sock(s, hostapd_global_ctrl_iface_receive, interface,
                           NULL);
...
void hostapd_global_ctrl_iface_deinit(struct hapd_interfaces *interfaces) {
  ...
  if (interfaces->global_ctrl_sock > -1) {
    ...
    if (interfaces->global_iface_path &&
      ...
      if (errno == ENOTEMPTY) {
        ...
        wpa_printf(MSG_ERROR, "rmdir[ctrl_interface=%s]: %s",
                   interfaces->global_iface_path, strerror(errno));
      }
    }
#endif /* CONFIG_CTRL_IFACE_UDP */
  }
  ...
  dl_list_for_each_safe(dst, prev, &interfaces->global_ctrl_dst,
                        struct wpa_ctrl_dst, list) os_free(dst);
}
...
static void hostapd_ctrl_iface_send(struct hostapd_data *hapd, int level,
  ...
  dl_list_for_each_safe(dst, next, ctrl_dst, struct wpa_ctrl_dst, list) {
    if (level >= dst->debug_level) {
      sockaddr_print(MSG_DEBUG, "CTRL_IFACE monitor send", &dst->addr,
                     dst->addrlen);
      ...
      if (sendmsg(s, &msg, 0) < 0) {
        ...
        if (dst->errors > 10 || _errno == ENOENT) {
          if (type != WPA_MSG_ONLY_GLOBAL)
            hostapd_ctrl_iface_detach(hapd, &dst->addr, dst->addrlen);
          else
            hostapd_global_ctrl_iface_detach(hapd->iface->interfaces,
