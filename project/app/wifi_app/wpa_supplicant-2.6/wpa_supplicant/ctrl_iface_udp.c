...
static void wpa_supplicant_ctrl_iface_msg_cb(void *ctx, int level,
  ...
  wpa_supplicant_ctrl_iface_send(wpa_s, NULL, wpa_s->ctrl_iface->sock,
                                 &wpa_s->ctrl_iface->ctrl_dst, level, txt, len);
}
...
static void wpa_supplicant_ctrl_iface_send(struct wpa_supplicant *wpa_s,
  ...
  if (ifname)
    os_snprintf(levelstr, sizeof(levelstr), "IFACE=%s <%d>", ifname, level);
  else
...
#ifdef CONFIG_CTRL_IFACE_UDP_IPV6
      wpa_printf(
          MSG_DEBUG, "CTRL_IFACE monitor send %s:%d",
...
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
      if (sendto(sock, sbuf, llen + len, 0, (struct sockaddr *)&dst->addr,
                 sizeof(dst->addr)) < 0) {
        wpa_printf(MSG_ERROR, "sendto(CTRL_IFACE monitor): %s",
                   strerror(errno));
        dst->errors++;
        if (dst->errors > 10) {
          wpa_supplicant_ctrl_iface_detach(head, &dst->addr, dst->addrlen);
        }
      } else
        dst->errors = 0;
    }
