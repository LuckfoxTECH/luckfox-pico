...
static int p2p_sd_dns_uncompress_label(char **upos, char *uend, u8 *start,
  ...
  while (*spos < end) {
    ...
    if (val == 3) {
      ...
      return p2p_sd_dns_uncompress_label(upos, uend, start, &spos_tmp,
                                         *spos - 2);
    }
...
wpas_p2p_service_get_bonjour(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(bsrv, &wpa_s->global->p2p_srv_bonjour,
    ...
    if (len == wpabuf_len(bsrv->query) &&
        os_memcmp(wpabuf_head(query), wpabuf_head(bsrv->query), len) == 0)
      return bsrv;
  }
  return NULL;
}
...
wpas_p2p_service_get_upnp(struct wpa_supplicant *wpa_s, u8 version,
  ...
  dl_list_for_each(usrv, &wpa_s->global->p2p_srv_upnp, struct p2p_srv_upnp,
                   list) {
    if (version == usrv->version && os_strcmp(service, usrv->service) == 0)
      return usrv;
  }
  return NULL;
}
...
static void wpas_sd_add_proto_not_avail(struct wpabuf *resp, u8 srv_proto,
                                        u8 srv_trans_id) {
  wpas_sd_add_empty(resp, srv_proto, srv_trans_id, P2P_SD_PROTO_NOT_AVAILABLE);
}
...
static int match_bonjour_query(struct p2p_srv_bonjour *bsrv, const u8 *query,
  ...
  if (p2p_sd_dns_uncompress(str_rx, sizeof(str_rx), query, query_len - 3, 0))
    return 0; /* Failed to uncompress query */
  if (p2p_sd_dns_uncompress(str_srv, sizeof(str_srv), wpabuf_head(bsrv->query),
...
static void wpas_sd_req_bonjour(struct wpa_supplicant *wpa_s,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "P2P: SD Request for Bonjour", query, query_len);
  if (dl_list_empty(&wpa_s->global->p2p_srv_bonjour)) {
    wpa_printf(MSG_DEBUG, "P2P: Bonjour protocol not available");
    wpas_sd_add_proto_not_avail(resp, P2P_SERV_BONJOUR, srv_trans_id);
    return;
  }
  ...
  dl_list_for_each(bsrv, &wpa_s->global->p2p_srv_bonjour,
    ...
    if (wpabuf_tailroom(resp) < 5 + query_len + wpabuf_len(bsrv->resp))
      return;
...
static void wpas_sd_all_upnp(struct wpa_supplicant *wpa_s, struct wpabuf *resp,
                             u8 srv_trans_id) {
  struct p2p_srv_upnp *usrv;
  u8 *len_pos;
  ...
  dl_list_for_each(usrv, &wpa_s->global->p2p_srv_upnp, struct p2p_srv_upnp,
    ...
    wpa_printf(MSG_DEBUG, "P2P: Matching UPnP Service: %s", usrv->service);
    wpabuf_put_str(resp, usrv->service);
    WPA_PUT_LE16(len_pos, (u8 *)wpabuf_put(resp, 0) - len_pos - 2);
  }
}

static void wpas_sd_req_upnp(struct wpa_supplicant *wpa_s, struct wpabuf *resp,
                             u8 srv_trans_id, const u8 *query,
                             size_t query_len) {
  struct p2p_srv_upnp *usrv;
  u8 *len_pos;
  u8 version;
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "P2P: SD Request for UPnP", query, query_len);
...
static void wpas_sd_req_wfd(struct wpa_supplicant *wpa_s, struct wpabuf *resp,
                            u8 srv_trans_id, const u8 *query,
                            size_t query_len) {
  const u8 *pos;
  u8 role;
  ...
  if (!wpa_s->global->wifi_display) {
    ...
    wpas_sd_add_proto_not_avail(resp, P2P_SERV_WIFI_DISPLAY, srv_trans_id);
    return;
  }
  ...
  while (pos < query + query_len) {
    if (*pos < MAX_WFD_SUBELEMS && wpa_s->global->wfd_subelem[*pos] &&
        wpabuf_tailroom(resp) >= wpabuf_len(wpa_s->global->wfd_subelem[*pos])) {
      wpa_printf(MSG_DEBUG,
...
static void wpas_sd_req_asp(struct wpa_supplicant *wpa_s, struct wpabuf *resp,
                            u8 srv_trans_id, const u8 *query,
                            size_t query_len) {
  ...
  for (adv_data = p2p_get_p2ps_adv_list(wpa_s->global->p2p); adv_data;
    ...
    if (os_memcmp(adv_data->svc_name, svc, svc_len) == 0 &&
      ...
      if (!len_pos && !count_pos) {
        if (wpabuf_tailroom(resp) < len + svc_info_len + 16)
          return;
        ...
        *count_pos = 0;
      } else if (wpabuf_tailroom(resp) < len + svc_info_len + 10)
        return;

      if (svc_info_len) {
        wpa_printf(MSG_DEBUG, "P2P: Add Svc: %s info: %s", adv_data->svc_name,
                   adv_data->svc_info);
      } else {
        wpa_printf(MSG_DEBUG, "P2P: Add Svc: %s", adv_data->svc_name);
      }
...
static void wpas_sd_all_asp(struct wpa_supplicant *wpa_s, struct wpabuf *resp,
                            u8 srv_trans_id) {
...
void wpas_sd_request(void *ctx, int freq, const u8 *sa, u8 dialog_token,
  ...
  wpa_hexdump(MSG_MSGDUMP, "P2P: Service Discovery Request TLVs", tlvs,
              tlvs_len);
  ...
  if (buf) {
    ...
    wpa_msg_ctrl(wpa_s, MSG_INFO,
                 P2P_EVENT_SERV_DISC_REQ "%d " MACSTR " %u %u %s", freq,
                 MAC2STR(sa), dialog_token, update_indic, buf);
    os_free(buf);
  }

  if (wpa_s->p2p_sd_over_ctrl_iface) {
    wpas_notify_p2p_sd_request(wpa_s, freq, sa, dialog_token, update_indic,
                               tlvs, tlvs_len);
  ...
  while (end - pos > 1) {
    ...
    srv_proto = *pos++;
    wpa_printf(MSG_DEBUG, "P2P: Service Protocol Type %u", srv_proto);
    srv_trans_id = *pos++;
    wpa_printf(MSG_DEBUG, "P2P: Service Transaction ID %u", srv_trans_id);

    wpa_hexdump(MSG_MSGDUMP, "P2P: Query Data", pos, tlv_end - pos);
    ...
    case P2P_SERV_ALL_SERVICES:
      ...
      if (dl_list_empty(&wpa_s->global->p2p_srv_upnp) &&
        ...
        wpas_sd_add_proto_not_avail(resp, P2P_SERV_ALL_SERVICES, srv_trans_id);
        break;
      }
      wpas_sd_all_bonjour(wpa_s, resp, srv_trans_id);
      wpas_sd_all_upnp(wpa_s, resp, srv_trans_id);
    ...
    case P2P_SERV_BONJOUR:
      wpas_sd_req_bonjour(wpa_s, resp, srv_trans_id, pos, tlv_end - pos);
      break;
    case P2P_SERV_UPNP:
      wpas_sd_req_upnp(wpa_s, resp, srv_trans_id, pos, tlv_end - pos);
      break;
#ifdef CONFIG_WIFI_DISPLAY
    case P2P_SERV_WIFI_DISPLAY:
      wpas_sd_req_wfd(wpa_s, resp, srv_trans_id, pos, tlv_end - pos);
      break;
#endif /* CONFIG_WIFI_DISPLAY */
    case P2P_SERV_P2PS:
      wpas_sd_req_asp(wpa_s, resp, srv_trans_id, pos, tlv_end - pos);
      break;
    default:
      wpa_printf(MSG_DEBUG,
                 ...
                 "protocol %u",
                 srv_proto);
      wpas_sd_add_proto_not_avail(resp, srv_proto, srv_trans_id);
      break;
    }
  ...
  wpas_notify_p2p_sd_request(wpa_s, freq, sa, dialog_token, update_indic, tlvs,
                             tlvs_len);
...
static void wpas_sd_p2ps_serv_response(struct wpa_supplicant *wpa_s,
  ...
  while (left-- && pos < tlv_end) {
    ...
    if (buf_len) {
      ...
      if (buf) {
        utf8_escape((const char *)pos, buf_len, buf, 2 * buf_len + 1);
      }
    }
    ...
    if (buf) {
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_SERV_ASP_RESP MACSTR " %x %x %x %x %s '%s'",
                     MAC2STR(sa), srv_trans_id, adv_id, svc_status,
                     config_methods, svc_str, buf);
      os_free(buf);
    } else {
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_SERV_ASP_RESP MACSTR " %x %x %x %x %s",
                     MAC2STR(sa), srv_trans_id, adv_id, svc_status,
                     config_methods, svc_str);
    }
  }
}

void wpas_sd_response(void *ctx, const u8 *sa, u16 update_indic, const u8 *tlvs,
                      size_t tlvs_len) {
  struct wpa_supplicant *wpa_s = ctx;
  const u8 *pos = tlvs;
  const u8 *end = tlvs + tlvs_len;
  ...
  wpa_hexdump(MSG_MSGDUMP, "P2P: Service Discovery Response TLVs", tlvs,
              tlvs_len);
  if (tlvs_len > 1500) {
    ...
    wpa_msg_ctrl(wpa_s, MSG_INFO,
                 ...
                 MAC2STR(sa), update_indic, (unsigned int)tlvs_len);
  } else {
    buf_len = 2 * tlvs_len + 1;
    buf = os_malloc(buf_len);
    if (buf) {
      wpa_snprintf_hex(buf, buf_len, tlvs, tlvs_len);
      wpa_msg_ctrl(wpa_s, MSG_INFO, P2P_EVENT_SERV_DISC_RESP MACSTR " %u %s",
  ...
  while (end - pos >= 2) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: Service Transaction ID %u", srv_trans_id);
    status = *pos++;
    wpa_printf(MSG_DEBUG, "P2P: Status Code ID %u", status);

    wpa_hexdump(MSG_MSGDUMP, "P2P: Response Data", pos, tlv_end - pos);

    if (srv_proto == P2P_SERV_P2PS && pos < tlv_end) {
      wpas_sd_p2ps_serv_response(wpa_s, sa, srv_trans_id, pos, tlv_end);
    }
...
int wpas_p2p_sd_cancel_request(struct wpa_supplicant *wpa_s, u64 req) {
  if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
    return -1;
  return p2p_sd_cancel_request(wpa_s->global->p2p, (void *)(uintptr_t)req);
}

void wpas_p2p_sd_response(struct wpa_supplicant *wpa_s, int freq, const u8 *dst,
                          u8 dialog_token, const struct wpabuf *resp_tlvs) {
  if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
    return;
  p2p_sd_response(wpa_s->global->p2p, freq, dst, dialog_token, resp_tlvs);
}
...
void wpas_p2p_service_flush(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each_safe(usrv, un, &wpa_s->global->p2p_srv_upnp,
                        struct p2p_srv_upnp, list) wpas_p2p_srv_upnp_free(usrv);
...
int wpas_p2p_service_add_asp(struct wpa_supplicant *wpa_s, int auto_accept,
  ...
  ret = p2p_service_add_asp(wpa_s->global->p2p, auto_accept, adv_id, adv_str,
                            svc_state, config_methods, svc_info, cpt_priority);
  if (ret == 0)
    wpas_p2p_sd_service_update(wpa_s);
  return ret;
}
