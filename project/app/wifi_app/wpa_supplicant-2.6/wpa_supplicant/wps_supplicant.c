...
static void wpas_wps_assoc_with_cred(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (!use_fast_assoc || wpa_supplicant_fast_associate(wpa_s) != 1)
...
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s) {
  ...
  if (!wpa_s->wps_success && wpa_s->current_ssid &&
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid &&
    ...
    if (bss) {
      ...
      if (wps && wps_parse_msg(wps, &attr) == 0 && attr.wps_state &&
    ...
    eloop_register_timeout(0, 10000, wpas_wps_assoc_with_cred, wpa_s,
...
static void wpas_wps_security_workaround(struct wpa_supplicant *wpa_s,
  ...
  if (wpa2 && !(ssid->proto & WPA_PROTO_RSN) && (ssid->proto & WPA_PROTO_WPA) &&
  ...
  for (ssid = wpa_s->conf->ssid, next = ssid ? ssid->next : NULL; ssid;
    ...
    if (ssid->auth_alg != new_ssid->auth_alg ||
        ...
        (ssid->group_cipher != new_ssid->group_cipher &&
         !(ssid->group_cipher & new_ssid->group_cipher & WPA_CIPHER_CCMP)))
      continue;
    ...
    if (ssid->wps_run && ssid->wps_run == new_ssid->wps_run &&
      ...
      if (new_ssid->passphrase && ssid->passphrase &&
          os_strcmp(new_ssid->passphrase, ssid->passphrase) != 0) {
        wpa_printf(MSG_DEBUG,
      ...
      if (new_ssid->psk_set &&
          (!ssid->psk_set || os_memcmp(new_ssid->psk, ssid->psk, 32) != 0)) {
        wpa_printf(MSG_DEBUG,
      ...
      wpa_printf(MSG_DEBUG, "WPS: Workaround - merge likely WPA/WPA2-mixed "
                            "mode creds in same M8 message");
...
static int wpa_supplicant_wps_cred(void *ctx,
  ...
  if ((wpa_s->conf->wps_cred_processing == 1 ||
    ...
    if (buf) {
      ...
      wpa_msg(wpa_s, MSG_INFO, "%s%s", WPS_EVENT_CRED_RECEIVED, buf);
      os_free(buf);
    }
  ...
  wpa_printf(MSG_DEBUG, "WPS: MAC Address " MACSTR, MAC2STR(cred->mac_addr));
  ...
  if (auth_type != WPS_AUTH_OPEN && auth_type != WPS_AUTH_WPAPSK &&
      auth_type != WPS_AUTH_WPA2PSK) {
    wpa_printf(MSG_DEBUG,
...
#ifdef CONFIG_WPS_REG_DISABLE_OPEN
    if (ssid->eap.identity && ssid->eap.identity_len == WSC_ID_REGISTRAR_LEN &&
        os_memcmp(ssid->eap.identity, WSC_ID_REGISTRAR, WSC_ID_REGISTRAR_LEN) ==
            0)
      registrar = 1;
#endif /* CONFIG_WPS_REG_DISABLE_OPEN */
  ...
  case WPS_ENCR_AES:
    ssid->pairwise_cipher = WPA_CIPHER_CCMP;
    if (wpa_s->drv_capa_known && (wpa_s->drv_enc & WPA_DRIVER_CAPA_ENC_GCMP)) {
  ...
  case WPS_AUTH_OPEN:
    ...
    if (registrar) {
      wpa_msg(wpa_s, MSG_INFO,
              WPS_EVENT_OPEN_NETWORK "id=%d - Credentials for an open "
...
static void wpa_supplicant_wps_event_m2d(struct wpa_supplicant *wpa_s,
  ...
  wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_M2D "dev_password_id=%d config_error=%d",
          m2d->dev_password_id, m2d->config_error);
  wpas_notify_wps_event_m2d(wpa_s, m2d);
#ifdef CONFIG_P2P
  if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s) {
    wpa_msg(wpa_s->p2pdev, MSG_INFO,
            WPS_EVENT_M2D "dev_password_id=%d config_error=%d",
            m2d->dev_password_id, m2d->config_error);
  }
...
static void wpa_supplicant_wps_event_fail(struct wpa_supplicant *wpa_s,
  ...
  if (fail->error_indication > 0 &&
    ...
    wpa_msg(wpa_s, MSG_INFO,
            WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%s)", fail->msg,
    ...
    wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d", fail->msg,
            fail->config_error);
    if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s)
      wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d",
...
int wpas_wps_reenable_networks_pending(struct wpa_supplicant *wpa_s) {
  return eloop_is_timeout_registered(wpas_wps_reenable_networks_cb, wpa_s,
                                     NULL);
}
...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s) {
   ...
   */
  eloop_register_timeout(10, 0, wpas_wps_reenable_networks_cb, wpa_s, NULL);
...
static void wpa_supplicant_wps_event_er_ap_add(struct wpa_supplicant *wpa_s,
  ...
  if (ap->pri_dev_type)
    wps_dev_type_bin2str(ap->pri_dev_type, dev_type, sizeof(dev_type));
  else
    dev_type[0] = '\0';

  wpa_msg(wpa_s, MSG_INFO,
          WPS_EVENT_ER_AP_ADD
          "%s " MACSTR " pri_dev_type=%s wps_state=%d |%s|%s|%s|%s|%s|%s|",
...
static void wpa_supplicant_wps_event_er_enrollee_add(
  ...
  if (enrollee->pri_dev_type)
    wps_dev_type_bin2str(enrollee->pri_dev_type, dev_type, sizeof(dev_type));
  else
    dev_type[0] = '\0';
...
static void wpa_supplicant_wps_event_er_enrollee_remove(
  ...
  char uuid_str[100];
  uuid_bin2str(enrollee->uuid, uuid_str, sizeof(uuid_str));
  wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_ER_ENROLLEE_REMOVE "%s " MACSTR, uuid_str,
          MAC2STR(enrollee->mac_addr));
}
...
static void wpa_supplicant_wps_event_er_set_sel_reg(
  ...
  case WPS_ER_SET_SEL_REG_DONE:
    wpa_msg(wpa_s, MSG_DEBUG, WPS_EVENT_ER_SET_SEL_REG "uuid=%s state=DONE",
            uuid_str);
    break;
  case WPS_ER_SET_SEL_REG_FAILED:
    wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_ER_SET_SEL_REG "uuid=%s state=FAILED",
            uuid_str);
    break;
  }
}
...
static int wpa_supplicant_wps_rf_band(void *ctx) {
  ...
  return (wpa_s->assoc_freq > 50000)  ? WPS_RF_60GHZ
         : (wpa_s->assoc_freq > 2484) ? WPS_RF_50GHZ
                                      : WPS_RF_24GHZ;
}
...
static void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_msg(wpa_s, MSG_INFO,
          WPS_EVENT_TIMEOUT "Requested operation timed "
                            "out");
...
static struct wpa_ssid *wpas_wps_add_network(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_config_set(ssid, "key_mgmt", "WPS", 0) < 0 ||
      ...
      wpa_config_set(ssid, "identity",
                     registrar ? "\"" WSC_ID_REGISTRAR "\""
                               : "\"" WSC_ID_ENROLLEE "\"",
                     0) < 0) {
    wpas_notify_network_removed(wpa_s, ssid);
    wpa_config_remove_network(wpa_s->conf, ssid->id);
...
int wpas_wps_start_pbc(struct wpa_supplicant *wpa_s, const u8 *bssid,
...
#ifdef CONFIG_AP
  if (wpa_s->ap_iface) {
    wpa_printf(MSG_DEBUG, "WPS: Reject request to start Registrar(as station) "
                          "operation while AP mode is enabled");
    return -1;
  }
#endif /* CONFIG_AP */
  wpas_clear_wps(wpa_s);
  ...
  if (p2p_group && wpa_s->go_params && wpa_s->go_params->ssid_len) {
    ...
    if (ssid->ssid) {
      ...
      os_memcpy(ssid->ssid, wpa_s->go_params->ssid, ssid->ssid_len);
...
static int wpas_wps_start_dev_pw(struct wpa_supplicant *wpa_s,
                                 ...
                                 const u8 *peer_pubkey_hash, const u8 *ssid_val,
                                 size_t ssid_len, int freq) {
  struct wpa_ssid *ssid;
  ...
  if (peer_pubkey_hash) {
    ...
    wpa_snprintf_hex_uppercase(hash + 8, sizeof(hash) - 8, peer_pubkey_hash,
...
#endif /* CONFIG_P2P */
  if (pin)
    os_snprintf(val, sizeof(val), "\"pin=%s dev_pw_id=%u%s\"", pin, dev_pw_id,
                hash);
    ...
    os_snprintf(val, sizeof(val), "\"pin=%08d dev_pw_id=%u%s\"", rpin,
                dev_pw_id, hash);
  }
...
int wpas_wps_start_pin(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  return wpas_wps_start_dev_pw(wpa_s, NULL, bssid, pin, p2p_group, dev_pw_id,
                               NULL, NULL, 0, 0);
}
...
int wpas_wps_cancel(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->wpa_state == WPA_SCANNING ||
    ...
    wpas_wps_reenable_networks(wpa_s);
    wpas_wps_clear_ap_info(wpa_s);
    if (eloop_cancel_timeout(wpas_wps_clear_timeout, wpa_s, NULL) > 0)
      wpas_clear_wps(wpa_s);
  }
...
int wpas_wps_start_reg(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (settings) {
    res = os_snprintf(pos, end - pos,
                      ...
                      settings->ssid_hex, settings->auth, settings->encr,
                      settings->key_hex);
    if (os_snprintf_error(end - pos, res))
      return -1;
    pos += res;
...
static int wpas_wps_new_psk_cb(void *ctx, const u8 *mac_addr,
  ...
  if (is_zero_ether_addr(p2p_dev_addr)) {
    wpa_printf(MSG_DEBUG, "Received new WPA/WPA2-PSK from WPS for STA " MACSTR,
               MAC2STR(mac_addr));
  } else {
    wpa_printf(MSG_DEBUG,
...
static void wpas_wps_pin_needed_cb(void *ctx, const u8 *uuid_e,
  ...
  len = os_snprintf(
      txt, sizeof(txt),
      "WPS-EVENT-PIN-NEEDED %s " MACSTR " [%s|%s|%s|%s|%s|%s]", uuid,
      MAC2STR(dev->mac_addr), dev->device_name, dev->manufacturer,
      dev->model_name, dev->model_number, dev->serial_number,
      wps_dev_type_bin2str(dev->pri_dev_type, devtype, sizeof(devtype)));
...
static u16 wps_fix_config_methods(u16 config_methods) {
  if ((config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_VIRT_DISPLAY |
                         WPS_CONFIG_PHY_DISPLAY)) == WPS_CONFIG_DISPLAY) {
    wpa_printf(MSG_INFO, "WPS: Converting display to "
                         "virtual_display for WPS 2.0 compliance");
    config_methods |= WPS_CONFIG_VIRT_DISPLAY;
  }
  if ((config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_VIRT_PUSHBUTTON |
  ...
  if (is_nil_uuid(wpa_s->conf->uuid)) {
    ...
    if (first && first != wpa_s) {
      if (wps != wpa_s->global->ifaces->wps)
        os_memcpy(wps->uuid, wpa_s->global->ifaces->wps->uuid, WPS_UUID_LEN);
...
static void wpas_wps_set_vendor_ext_m1(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->conf->wps_vendor_ext_m1) {
    wps->dev.vendor_ext_m1 = wpabuf_dup(wpa_s->conf->wps_vendor_ext_m1);
  ...
  if (eap_is_wps_pin_enrollee(&ssid->eap)) {
    ...
    if (!wps_is_addr_authorized(wps_ie, wpa_s->own_addr, 1)) {
      ...
      if (wpa_s->scan_runs < WPS_PIN_SCAN_IGNORE_SEL_REG ||
        ...
        wpa_printf(MSG_DEBUG,
                   "   skip - WPS AP without active PIN Registrar "
                   "(scan_runs=%d age=%d)",
...
int wpas_wps_ssid_wildcard_ok(struct wpa_supplicant *wpa_s,
  ...
  if (wps_ie) {
    if (wps_validate_beacon_probe_resp(wps_ie, bss->beacon_ie_len > 0,
                                       bss->bssid) < 0)
      ret = 0;
    if (bss->beacon_ie_len) {
      struct wpabuf *bcn_wps;
      bcn_wps = wpa_bss_get_vendor_ie_multi_beacon(bss, WPS_IE_VENDOR_TYPE);
      if (bcn_wps == NULL) {
...
int wpas_wps_scan_pbc_overlap(struct wpa_supplicant *wpa_s,
  ...
  if (wps_ie) {
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: UUID of the selected BSS", sel_uuid, UUID_LEN);
  } else {
  ...
  for (i = 0; i < wpa_s->num_wps_ap; i++) {
    ...
    if (!ap->pbc_active || os_memcmp(selected->bssid, ap->bssid, ETH_ALEN) == 0)
      continue;

    wpa_printf(MSG_DEBUG, "WPS: Another BSS in active PBC mode: " MACSTR,
               MAC2STR(ap->bssid));
    wpa_hexdump(MSG_DEBUG, "WPS: UUID of the other BSS", ap->uuid, UUID_LEN);
    if (sel_uuid == NULL || os_memcmp(sel_uuid, ap->uuid, UUID_LEN) != 0) {
      ret = 1; /* PBC overlap */
      wpa_msg(wpa_s, MSG_INFO,
              "WPS: PBC overlap detected: " MACSTR " and " MACSTR,
              MAC2STR(selected->bssid), MAC2STR(ap->bssid));
      break;
    }
...
int wpas_wps_er_add_pin(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  if (os_strcmp(uuid, "any") == 0) {
    ...
    return -1;
  return wps_registrar_add_pin(wpa_s->wps->registrar, addr, use_uuid,
...
static int wpas_wps_network_to_cred(struct wpa_ssid *ssid,
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_PSK) {
    cred->auth_type =
        (ssid->proto & WPA_PROTO_RSN) ? WPS_AUTH_WPA2PSK : WPS_AUTH_WPAPSK;
...
int wpas_wps_er_config(struct wpa_supplicant *wpa_s, const char *uuid,
  ...
  return wps_er_config(wpa_s->wps_er, use_uuid, use_addr, (const u8 *)pin,
                       os_strlen(pin), &cred);
}
...
struct wpabuf *wpas_wps_nfc_config_token(struct wpa_supplicant *wpa_s, int ndef,
                                         const char *id_str) {
#ifdef CONFIG_WPS_ER
  if (id_str) {
    int id;
...
struct wpabuf *wpas_wps_nfc_token(struct wpa_supplicant *wpa_s, int ndef) {
  if (wpa_s->conf->wps_nfc_pw_from_config) {
    return wps_nfc_token_build(ndef, wpa_s->conf->wps_nfc_dev_pw_id,
                               wpa_s->conf->wps_nfc_dh_pubkey,
                               wpa_s->conf->wps_nfc_dev_pw);
  }

  return wps_nfc_token_gen(
      ndef, &wpa_s->conf->wps_nfc_dev_pw_id, &wpa_s->conf->wps_nfc_dh_pubkey,
      &wpa_s->conf->wps_nfc_dh_privkey, &wpa_s->conf->wps_nfc_dev_pw);
}

int wpas_wps_start_nfc(struct wpa_supplicant *wpa_s, const u8 *go_dev_addr,
                       const u8 *bssid, const struct wpabuf *dev_pw,
                       u16 dev_pw_id, int p2p_group, const u8 *peer_pubkey_hash,
  ...
  if (dev_pw) {
    wpa_snprintf_hex_uppercase(pw, sizeof(pw), wpabuf_head(dev_pw),
                               wpabuf_len(dev_pw));
  }
  return wpas_wps_start_dev_pw(wpa_s, go_dev_addr, bssid, dev_pw ? pw : NULL,
                               p2p_group, dev_pw_id, peer_pubkey_hash, ssid,
                               ssid_len, freq);
}
...
static int wpas_wps_add_nfc_password_token(struct wpa_supplicant *wpa_s,
                                           struct wps_parse_attr *attr) {
  return wps_registrar_add_nfc_password_token(wpa_s->wps->registrar,
...
int wpas_wps_nfc_tag_read(struct wpa_supplicant *wpa_s,
  ...
  if (*wpabuf_head_u8(data) != 0x10) {
    ...
    if (tmp == NULL) {
      ...
      if (tmp) {
        ret = wpas_p2p_nfc_tag_process(wpa_s, tmp, forced_freq);
        wpabuf_free(tmp);
        return ret;
      }
#endif /* CONFIG_P2P */
...
struct wpabuf *wpas_wps_nfc_handover_req(struct wpa_supplicant *wpa_s,
  ...
  ret = wps_build_nfc_handover_req(wpa_s->wps, wpa_s->conf->wps_nfc_dh_pubkey);
...
static struct wpabuf *wpas_wps_er_nfc_handover_sel(struct wpa_supplicant *wpa_s,
                                                   int ndef, const char *uuid) {
#ifdef CONFIG_WPS_ER
  struct wpabuf *ret;
  u8 u[UUID_LEN], *use_uuid = NULL;
  ...
  ret = wps_er_nfc_handover_sel(wpa_s->wps_er, wpa_s->wps, use_uuid, use_addr,
                                wpa_s->conf->wps_nfc_dh_pubkey);
  if (ndef && ret) {
    struct wpabuf *tmp;
    tmp = ndef_build_wifi(ret);
...
struct wpabuf *wpas_wps_nfc_handover_sel(struct wpa_supplicant *wpa_s, int ndef,
                                         int cr, const char *uuid) {
  struct wpabuf *ret;
  if (!cr)
    return NULL;
...
static int wpas_wps_nfc_rx_handover_sel(struct wpa_supplicant *wpa_s,
  ...
  if (wsc_len > wpabuf_len(wps) - 2) {
    wpa_printf(MSG_DEBUG,
               ...
               "in Wi-Fi Handover Select Message",
               wsc_len);
    goto out;
  }
  pos += 2;

  wpa_hexdump(MSG_DEBUG, "WPS: WSC attributes in Wi-Fi Handover Select Message",
              pos, wsc_len);
  if (wsc_len < wpabuf_len(wps) - 2) {
    wpa_hexdump(MSG_DEBUG, "WPS: Ignore extra data after WSC attributes",
  ...
  if (attr.mac_addr) {
    bssid = attr.mac_addr;
    wpa_printf(MSG_DEBUG, "WPS: MAC Address (BSSID): " MACSTR, MAC2STR(bssid));
  }
  ...
  if (attr.ap_channel) {
    ...
    if (chan >= 1 && chan <= 13 &&
      ...
      freq = 2484;
             ...
             (attr.rf_bands == NULL || *attr.rf_bands & WPS_RF_50GHZ))
      freq = 5000 + 5 * chan;
    else if (chan >= 1 && chan <= 4 &&
             (attr.rf_bands == NULL || *attr.rf_bands & WPS_RF_60GHZ))
      freq = 56160 + 2160 * chan;

    if (freq) {
      wpa_printf(MSG_DEBUG, "WPS: AP indicated channel %u -> %u MHz", chan,
                 freq);
    }
  }
  ...
  dev_pw_id = WPA_GET_BE16(attr.oob_dev_password + WPS_OOB_PUBKEY_HASH_LEN);
  if (dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER) {
    wpa_printf(MSG_DEBUG,
               "WPS: Unexpected OOB Device Password ID "
               "%u in Wi-Fi Handover Select Message",
               dev_pw_id);
    ret = -1;
    goto out;
  }
  wpa_hexdump(MSG_DEBUG, "WPS: AP Public Key hash", attr.oob_dev_password,
              WPS_OOB_PUBKEY_HASH_LEN);

  ret =
      wpas_wps_start_nfc(wpa_s, NULL, bssid, NULL, dev_pw_id, 0,
                         attr.oob_dev_password, attr.ssid, attr.ssid_len, freq);
...
int wpas_er_wps_nfc_report_handover(struct wpa_supplicant *wpa_s,
  ...
  if (wsc_len > wpabuf_len(wps) - 2) {
    wpa_printf(MSG_DEBUG,
               "WPS: Invalid WSC attribute length (%u) "
               "in rt Wi-Fi Handover Request Message",
               wsc_len);
    goto out;
  }
  pos += 2;
  ...
  wpa_hexdump(MSG_DEBUG,
              "WPS: WSC attributes in Wi-Fi Handover Request Message", pos,
              wsc_len);
  if (wsc_len < wpabuf_len(wps) - 2) {
    wpa_hexdump(MSG_DEBUG, "WPS: Ignore extra data after WSC attributes",
                pos + wsc_len, wpabuf_len(wps) - 2 - wsc_len);
  ...
  if (dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER) {
    wpa_printf(MSG_DEBUG,
               "WPS: Unexpected OOB Device Password ID "
               "%u in Wi-Fi Handover Request Message",
               dev_pw_id);
    ret = -1;
    goto out;
  }
  wpa_hexdump(MSG_DEBUG, "WPS: Enrollee Public Key hash", attr.oob_dev_password,
              WPS_OOB_PUBKEY_HASH_LEN);

  ret = wps_registrar_add_nfc_pw_token(
      wpa_s->wps->registrar, attr.oob_dev_password,
      DEV_PW_NFC_CONNECTION_HANDOVER, NULL, 0, 1);
...
static void wpas_wps_update_ap_info_bss(struct wpa_supplicant *wpa_s,
  ...
  if (ap) {
    if (ap->type != type) {
      wpa_printf(MSG_DEBUG, "WPS: AP " MACSTR " changed type %d -> %d",
  ...
  if (uuid)
    os_memcpy(ap->uuid, uuid, WPS_UUID_LEN);
  wpa_printf(MSG_DEBUG, "WPS: AP " MACSTR " type %d added", MAC2STR(ap->bssid),
             ap->type);
