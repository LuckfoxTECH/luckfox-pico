...
void mesh_auth_timer(void *eloop_ctx, void *user_data) {
  ...
  if (sta->sae->state != SAE_ACCEPTED) {
    wpa_printf(MSG_DEBUG, "AUTH: Re-authenticate with " MACSTR " (attempt %d) ",
    ...
    if (sta->sae_auth_retry < MESH_AUTH_RETRY) {
      ...
      wpa_msg(wpa_s, MSG_INFO,
              MESH_SAE_AUTH_BLOCKED "addr=" MACSTR " duration=%d",
              MAC2STR(sta->addr), hapd->conf->ap_max_inactivity);
    }
    sta->sae_auth_retry++;
  }
}
...
static void auth_logger(void *ctx, const u8 *addr, logger_level level,
                        const char *txt) {
  if (addr)
    wpa_printf(MSG_DEBUG, "AUTH: " MACSTR " - %s", MAC2STR(addr), txt);
  else
...
static const u8 *auth_get_psk(void *ctx, const u8 *addr, const u8 *p2p_dev_addr,
  ...
  wpa_printf(MSG_DEBUG, "AUTH: %s (addr=" MACSTR " prev_psk=%p)", __func__,
...
static int auth_set_key(void *ctx, int vlan_id, enum wpa_alg alg,
  ...
  if (addr) {
    wpa_printf(MSG_DEBUG, "AUTH: %s(alg=%d addr=" MACSTR " key_idx=%d)",
               __func__, alg, MAC2STR(addr), idx);
  } else {
    wpa_printf(MSG_DEBUG, "AUTH: %s(alg=%d key_idx=%d)", __func__, alg, idx);
  }
  wpa_hexdump_key(MSG_DEBUG, "AUTH: set_key - key", key, key_len);

  return wpa_drv_set_key(mesh_rsn->wpa_s, alg, addr, idx, 1, seq, 6, key,
                         key_len);
}
...
static int __mesh_rsn_auth_init(struct mesh_rsn *rsn, const u8 *addr,
  ...
  if (ieee80211w != NO_MGMT_FRAME_PROTECTION) {
    ...
    /* group mgmt */
    wpa_hexdump_key(MSG_DEBUG, "mesh: Own TX IGTK", rsn->igtk, rsn->igtk_len);
    wpa_drv_set_key(rsn->wpa_s, wpa_cipher_to_alg(rsn->mgmt_group_cipher), NULL,
                    rsn->igtk_key_id, 1, seq, sizeof(seq), rsn->igtk,
                    rsn->igtk_len);
  }
#endif /* CONFIG_IEEE80211W */

  /* group privacy / data frames */
  wpa_hexdump_key(MSG_DEBUG, "mesh: Own TX MGTK", rsn->mgtk, rsn->mgtk_len);
  wpa_drv_set_key(rsn->wpa_s, wpa_cipher_to_alg(rsn->group_cipher), NULL,
                  rsn->mgtk_key_id, 1, seq, sizeof(seq), rsn->mgtk,
                  rsn->mgtk_len);
...
struct mesh_rsn *mesh_rsn_auth_init(struct wpa_supplicant *wpa_s,
  ...
  if (__mesh_rsn_auth_init(mesh_rsn, wpa_s->own_addr, conf->ieee80211w) < 0) {
...
static int mesh_rsn_sae_group(struct wpa_supplicant *wpa_s,
  ...
  for (;;) {
    ...
    if (sae_set_group(sae, group) == 0) {
      wpa_dbg(wpa_s, MSG_DEBUG, "SME: Selected SAE group %d", sae->group);
      return 0;
    }
...
static int mesh_rsn_build_sae_commit(struct wpa_supplicant *wpa_s,
  ...
  return sae_prepare_commit(wpa_s->own_addr, sta->addr, (u8 *)ssid->passphrase,
...
int mesh_rsn_auth_sae_sta(struct wpa_supplicant *wpa_s, struct sta_info *sta) {
  ...
  if (pmksa) {
    ...
    if (!sta->wpa_sm) {
      wpa_printf(MSG_ERROR, "mesh: Failed to initialize RSN state machine");
      return -1;
    }
  ...
  eloop_register_timeout(MESH_AUTH_TIMEOUT + rnd, 0, mesh_auth_timer, wpa_s,
                         sta);
  return 0;
}
...
static void mesh_rsn_derive_aek(struct mesh_rsn *rsn, struct sta_info *sta) {
  ...
  sha256_prf(sta->sae->pmk, sizeof(sta->sae->pmk), "AEK Derivation", context,
             sizeof(context), sta->aek, sizeof(sta->aek));
}
...
int mesh_rsn_derive_mtk(struct wpa_supplicant *wpa_s, struct sta_info *sta) {
  ...
  sha256_prf(sta->sae->pmk, SAE_PMK_LEN, "Temporal Key Derivation", context,
             sizeof(context), sta->mtk, sta->mtk_len);
  return 0;
}
...
int mesh_rsn_protect_frame(struct mesh_rsn *rsn, struct sta_info *sta,
  ...
  RSN_SELECTOR_PUT(ampe->selected_pairwise_suite, RSN_CIPHER_SUITE_CCMP);
...
skip_keys:
  wpa_hexdump_key(MSG_DEBUG, "mesh: Plaintext AMPE element", ampe_ie, 2 + len);
  ...
  if (aes_siv_encrypt(sta->aek, ampe_ie, 2 + len, 3, aad, aad_len,
                      mic_payload)) {
...
int mesh_rsn_process_ampe(struct wpa_supplicant *wpa_s, struct sta_info *sta,
                          ...
                          const u8 *chosen_pmk, const u8 *start,
                          size_t elems_len) {
  int ret = 0;
  struct ieee80211_ampe_ie *ampe;
  ...
  if (!sta->sae) {
    ...
    if (!wpa_auth_pmksa_get(hapd->wpa_auth, sta->addr)) {
      wpa_printf(MSG_INFO, "Mesh RSN: SAE is not prepared yet");
      return -1;
    }
    mesh_rsn_auth_sae_sta(wpa_s, sta);
  }
  ...
  if (aes_siv_decrypt(sta->aek, crypt, crypt_len, 3, aad, aad_len, ampe_buf)) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "mesh: Decrypted AMPE element", ampe_buf,
                  crypt_len);

  ampe_eid = *ampe_buf++;
  ampe_ie_len = *ampe_buf++;

  if (ampe_eid != WLAN_EID_AMPE || (size_t)2 + ampe_ie_len > crypt_len ||
  ...
  os_memcpy(sta->peer_nonce, ampe->local_nonce, sizeof(ampe->local_nonce));
  ...
  if (cat[1] != PLINK_OPEN) {
    if (end > pos) {
      wpa_hexdump_key(MSG_DEBUG,
                      "mesh: Ignore unexpected GTKdata(etc.) fields in the end "
                      "of AMPE element in Mesh Peering Confirm/Close",
                      pos, end - pos);
    }
    goto free;
  }
  ...
  wpa_hexdump_key(MSG_DEBUG, "mesh: GTKdata - MGTK", sta->mgtk, sta->mgtk_len);
  pos += sta->mgtk_len;
  wpa_hexdump(MSG_DEBUG, "mesh: GTKdata - MGTK - Key RSC", pos,
              WPA_KEY_RSC_LEN);
  os_memcpy(sta->mgtk_rsc, pos, sizeof(sta->mgtk_rsc));
  pos += WPA_KEY_RSC_LEN;
  wpa_printf(MSG_DEBUG, "mesh: GTKdata - MGTK - GTKExpirationTime: %u seconds",
             WPA_GET_LE32(pos));
  pos += 4;
  ...
  if (end - pos >= (int)(2 + 6 + key_len)) {
    ...
    wpa_printf(MSG_DEBUG, "mesh: IGTKdata - Key ID %u", sta->igtk_key_id);
    pos += 2;
    os_memcpy(sta->igtk_rsc, pos, sizeof(sta->igtk_rsc));
    wpa_hexdump(MSG_DEBUG, "mesh: IGTKdata - IPN", sta->igtk_rsc,
                sizeof(sta->igtk_rsc));
    pos += 6;
    os_memcpy(sta->igtk, pos, key_len);
    sta->igtk_len = key_len;
    wpa_hexdump_key(MSG_DEBUG, "mesh: IGTKdata - IGTK", sta->igtk,
                    sta->igtk_len);
  }
#endif /* CONFIG_IEEE80211W */
