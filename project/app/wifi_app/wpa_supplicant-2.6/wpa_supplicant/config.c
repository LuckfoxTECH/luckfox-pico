...
struct parse_data {
   ...
   * -1. */
  int (*parser)(const struct parse_data *data, struct wpa_ssid *ssid, int line,
                const char *value);
...
static int wpa_config_parse_str(const struct parse_data *data,
  ...
  if (os_strcmp(value, "NULL") == 0) {
    wpa_printf(MSG_DEBUG, "Unset configuration string '%s'", data->name);
    tmp = NULL;
    res_len = 0;
    goto set;
  }
  ...
  if (tmp == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: failed to parse %s '%s'.", line, data->name,
               data->key_data ? "[KEY DATA REMOVED]" : value);
    return -1;
  }

  if (data->key_data) {
    wpa_hexdump_ascii_key(MSG_MSGDUMP, data->name, (u8 *)tmp, res_len);
  } else {
  ...
  if (data->param3 && res_len < (size_t)data->param3) {
    wpa_printf(MSG_ERROR,
               ...
               "min_len=%ld)",
               line, data->name, (unsigned long)res_len, (long)data->param3);
    os_free(tmp);
    return -1;
  }
  ...
  if (data->param4 && res_len > (size_t)data->param4) {
    wpa_printf(MSG_ERROR,
               ...
               "max_len=%ld)",
               line, data->name, (unsigned long)res_len, (long)data->param4);
    os_free(tmp);
    return -1;
  }
...
set:
  ...
  if (data->param2)
    ...
    prev_len = 0;
  if ((*dst == NULL && tmp == NULL) || (*dst && tmp && prev_len == res_len &&
...
static int wpa_config_parse_int(const struct parse_data *data,
  ...
  if (*end) {
    wpa_printf(MSG_ERROR, "Line %d: invalid number \"%s\"", line, value);
    return -1;
  }
  ...
  *dst = val;
  ...
  if (data->param3 && *dst < (long)data->param3) {
    wpa_printf(MSG_ERROR,
               ...
               "min_value=%ld)",
               line, data->name, *dst, (long)data->param3);
  ...
  if (data->param4 && *dst > (long)data->param4) {
    wpa_printf(MSG_ERROR,
               ...
               "max_value=%ld)",
               line, data->name, *dst, (long)data->param4);
  ...
  while (pos && *pos) {
    ...
    if (hwaddr_masked_aton(pos, addr, &addr[ETH_ALEN], masked)) {
      if (abort_on_error || count == 0) {
        wpa_printf(MSG_ERROR, "Line %d: Invalid %s address '%s'", line, name,
                   value);
        os_free(buf);
        return -1;
      }
      ...
      wpa_printf(MSG_INFO, "Line %d: Ignore likely truncated %s address '%s'",
                 line, name, pos);
    } else {
      ...
      count++;
      wpa_printf(MSG_MSGDUMP, "%s: addr=" MACSTR " mask=" MACSTR, name,
                 MAC2STR(addr), MAC2STR(&addr[ETH_ALEN]));
    }
...
static int wpa_config_parse_bssid_blacklist(const struct parse_data *data,
                                            struct wpa_ssid *ssid, int line,
                                            const char *value) {
  return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_blacklist,
...
static int wpa_config_parse_bssid_whitelist(const struct parse_data *data,
                                            struct wpa_ssid *ssid, int line,
                                            const char *value) {
  return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_whitelist,
...
static int wpa_config_parse_psk(const struct parse_data *data,
  ...
  if (os_strncmp(value, "ext:", 4) == 0) {
    ...
    if (ssid->ext_psk == NULL)
      return -1;
    wpa_printf(MSG_DEBUG, "PSK: External password '%s'", ssid->ext_psk);
    return 0;
  }
  ...
  if (*value == '"') {
    ...
    wpa_hexdump_ascii_key(MSG_MSGDUMP, "PSK (ASCII passphrase)", (u8 *)value,
                          len);
  ...
  if (hexstr2bin(value, ssid->psk, PMK_LEN) || value[PMK_LEN * 2] != '\0') {
    wpa_printf(MSG_ERROR, "Line %d: Invalid PSK '%s'.", line, value);
    return -1;
  }
...
static char *wpa_config_write_psk(const struct parse_data *data,
  ...
  if (ssid->passphrase)
    return wpa_config_write_string_ascii((const u8 *)ssid->passphrase,
...
static int wpa_config_parse_proto(const struct parse_data *data,
  ...
  while (*start != '\0') {
    ...
    *end = '\0';
    if (os_strcmp(start, "WPA") == 0)
      val |= WPA_PROTO_WPA;
    else if (os_strcmp(start, "RSN") == 0 || os_strcmp(start, "WPA2") == 0)
      val |= WPA_PROTO_RSN;
    else if (os_strcmp(start, "OSEN") == 0)
      val |= WPA_PROTO_OSEN;
    else {
      wpa_printf(MSG_ERROR, "Line %d: invalid proto '%s'", line, start);
      errors++;
    }
  ...
  if (val == 0) {
    wpa_printf(MSG_ERROR, "Line %d: no proto values configured.", line);
    errors++;
  }
...
static char *wpa_config_write_proto(const struct parse_data *data,
  ...
  if (ssid->proto & WPA_PROTO_OSEN) {
    ret = os_snprintf(pos, end - pos, "%sOSEN", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return buf;
    pos += ret;
  }
...
static char *wpa_config_write_key_mgmt(const struct parse_data *data,
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_PSK) {
    ret = os_snprintf(pos, end - pos, "%sWPA-PSK", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X) {
    ret = os_snprintf(pos, end - pos, "%sWPA-EAP", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_NO_WPA) {
    ret = os_snprintf(pos, end - pos, "%sIEEE8021X", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_NONE) {
    ret = os_snprintf(pos, end - pos, "%sNONE", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_WPA_NONE) {
    ret = os_snprintf(pos, end - pos, "%sWPA-NONE", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_FT_PSK) {
    ret = os_snprintf(pos, end - pos, "%sFT-PSK", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_FT_IEEE8021X) {
    ret = os_snprintf(pos, end - pos, "%sFT-EAP", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_PSK_SHA256) {
    ret =
        os_snprintf(pos, end - pos, "%sWPA-PSK-SHA256", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_SHA256) {
    ret =
        os_snprintf(pos, end - pos, "%sWPA-EAP-SHA256", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_WPS) {
    ret = os_snprintf(pos, end - pos, "%sWPS", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_FT_SAE) {
    ret = os_snprintf(pos, end - pos, "%sFT-SAE", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_SUITE_B) {
    ret =
        os_snprintf(pos, end - pos, "%sWPA-EAP-SUITE-B", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
...
static int wpa_config_parse_auth_alg(const struct parse_data *data,
  ...
  while (*start != '\0') {
    ...
    *end = '\0';
    if (os_strcmp(start, "OPEN") == 0)
      ...
      wpa_printf(MSG_ERROR, "Line %d: invalid auth_alg '%s'", line, start);
      errors++;
    }
  ...
  if (val == 0) {
    wpa_printf(MSG_ERROR, "Line %d: no auth_alg values configured.", line);
    errors++;
  }
...
static char *wpa_config_write_freqs(const struct parse_data *data,
  ...
  for (i = 0; freqs[i]; i++) {
    ret = os_snprintf(pos, end - pos, "%s%u", i == 0 ? "" : " ", freqs[i]);
    if (os_snprintf_error(end - pos, ret)) {
      end[-1] = '\0';
      return buf;
    }
...
static int wpa_config_parse_eap(const struct parse_data *data,
  ...
  while (*start != '\0') {
    ...
    *end = '\0';
    ...
    methods[num_methods].method =
        eap_peer_get_type(start, &methods[num_methods].vendor);
  ...
  wpa_hexdump(MSG_MSGDUMP, "eap methods", (u8 *)methods,
...
static char *wpa_config_write_eap(const struct parse_data *data,
  ...
  for (i = 0; eap_methods[i].vendor != EAP_VENDOR_IETF ||
              eap_methods[i].method != EAP_TYPE_NONE;
       i++) {
    name = eap_get_name(eap_methods[i].vendor, eap_methods[i].method);
    if (name) {
      ret = os_snprintf(pos, end - pos, "%s%s", pos == buf ? "" : " ", name);
      if (os_snprintf_error(end - pos, ret))
        break;
      pos += ret;
    }
...
static char *wpa_config_write_password(const struct parse_data *data,
  ...
  if (!(ssid->eap.flags & EAP_CONFIG_FLAGS_PASSWORD_NTHASH)) {
    return wpa_config_write_string(ssid->eap.password, ssid->eap.password_len);
  }
...
static int wpa_config_parse_wep_key(u8 *key, size_t *len, int line,
  ...
  if (buf == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid WEP key %d '%s'.", line, idx,
               value);
    return -1;
  }
  if (*len > MAX_WEP_KEY_LEN) {
    wpa_printf(MSG_ERROR, "Line %d: Too long WEP key %d '%s'.", line, idx,
               value);
    os_free(buf);
    return -1;
  }
...
static int wpa_config_parse_wep_key0(const struct parse_data *data,
                                     struct wpa_ssid *ssid, int line,
                                     const char *value) {
  return wpa_config_parse_wep_key(ssid->wep_key[0], &ssid->wep_key_len[0], line,
                                  value, 0);
}
...
static int wpa_config_parse_wep_key1(const struct parse_data *data,
                                     struct wpa_ssid *ssid, int line,
                                     const char *value) {
  return wpa_config_parse_wep_key(ssid->wep_key[1], &ssid->wep_key_len[1], line,
                                  value, 1);
}
...
static int wpa_config_parse_wep_key2(const struct parse_data *data,
                                     struct wpa_ssid *ssid, int line,
                                     const char *value) {
  return wpa_config_parse_wep_key(ssid->wep_key[2], &ssid->wep_key_len[2], line,
                                  value, 2);
}
...
static int wpa_config_parse_wep_key3(const struct parse_data *data,
                                     struct wpa_ssid *ssid, int line,
                                     const char *value) {
  return wpa_config_parse_wep_key(ssid->wep_key[3], &ssid->wep_key_len[3], line,
                                  value, 3);
}
...
static char *wpa_config_write_wep_key(struct wpa_ssid *ssid, int idx) {
  if (ssid->wep_key_len[idx] == 0)
    return NULL;
  return wpa_config_write_string(ssid->wep_key[idx], ssid->wep_key_len[idx]);
}
...
static int wpa_config_parse_go_p2p_dev_addr(const struct parse_data *data,
  ...
  if (hwaddr_aton(value, ssid->go_p2p_dev_addr)) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid GO P2P Device Address '%s'.", line,
               value);
    return -1;
  }
  ssid->bssid_set = 1;
...
static int wpa_config_parse_p2p_client_list(const struct parse_data *data,
                                            struct wpa_ssid *ssid, int line,
                                            const char *value) {
  return wpa_config_parse_addr_list(data, line, value, &ssid->p2p_client_list,
                                    &ssid->num_p2p_clients, "p2p_client_list",
                                    0, 0);
}
...
static int wpa_config_parse_psk_list(const struct parse_data *data,
  ...
  if (hwaddr_aton(pos, p->addr)) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid psk_list address '%s'", line, pos);
    os_free(p);
    return -1;
  }
  pos += 17;
  if (*pos != '-') {
    wpa_printf(MSG_ERROR, "Line %d: Invalid psk_list '%s'", line, pos);
    os_free(p);
    return -1;
  }
  pos++;
  ...
  if (hexstr2bin(pos, p->psk, PMK_LEN) || pos[PMK_LEN * 2] != '\0') {
    wpa_printf(MSG_ERROR, "Line %d: Invalid psk_list PSK '%s'", line, pos);
    os_free(p);
    return -1;
  }
...
static int wpa_config_parse_mesh_basic_rates(const struct parse_data *data,
  ...
  if (rates == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid mesh_basic_rates '%s'", line,
               value);
    return -1;
  }
  if (rates[0] == 0) {
...
#else /* NO_CONFIG_WRITE */
#define _INT(f)                                                                \
#f, wpa_config_parse_int, wpa_config_write_int, OFFSET(f), (void *)0
#define _INTe(f)                                                               \
#f, wpa_config_parse_int, wpa_config_write_int, OFFSET(eap.f), (void *)0
#endif /* NO_CONFIG_WRITE */
...
#else /* NO_CONFIG_WRITE */
#define _FUNC(f)                                                               \
#f, wpa_config_parse_##f, wpa_config_write_##f, NULL, NULL, NULL, NULL
#endif /* NO_CONFIG_WRITE */
...
void wpa_config_free_ssid(struct wpa_ssid *ssid) {
  ...
  os_free(ssid->mesh_basic_rates);
#endif /* CONFIG_MESH */
  while ((psk = dl_list_first(&ssid->psk_list, struct psk_list_entry, list))) {
  ...
  for (i = 0; i < NUM_SSID_FIELDS; i++) {
    ...
    if (os_strcmp(var, field->name) == 0) {
      ...
      if (ret && has_newline(ret)) {
        wpa_printf(MSG_ERROR, "Found newline in value for %s; not returning it",
                   var);
        os_free(ret);
        ret = NULL;
      }
...
#ifndef CONFIG_NO_PBKDF2
  pbkdf2_sha1(ssid->passphrase, ssid->ssid, ssid->ssid_len, 4096, ssid->psk,
              PMK_LEN);
...
int wpa_config_set_cred(struct wpa_cred *cred, const char *var,
  ...
  if (os_strcmp(var, "eap") == 0) {
    ...
    if (method.vendor == EAP_VENDOR_IETF && method.method == EAP_TYPE_NONE) {
      wpa_printf(MSG_ERROR,
  ...
  if (val == NULL || (os_strcmp(var, "excluded_ssid") != 0 &&
  ...
  if (os_strcmp(var, "domain") == 0) {
    char **new_domain;
    new_domain =
        os_realloc_array(cred->domain, cred->num_domain + 1, sizeof(char *));
    if (new_domain == NULL) {
      os_free(val);
      return -1;
    }
  ...
  if (os_strcmp(var, "excluded_ssid") == 0) {
    ...
    if (len > SSID_MAX_LEN) {
      wpa_printf(MSG_ERROR,
                 "Line %d: invalid "
                 "excluded_ssid length %d",
                 line, (int)len);
      os_free(val);
      return -1;
    }

    e = os_realloc_array(cred->excluded_ssid, cred->num_excluded_ssid + 1,
  ...
  if (os_strcmp(var, "roaming_partner") == 0) {
    ...
    p = os_realloc_array(cred->roaming_partner, cred->num_roaming_partner + 1,
  ...
  if (line) {
    wpa_printf(MSG_ERROR, "Line %d: unknown cred field '%s'.", line, var);
  }
...
char *wpa_config_get_cred_no_key(struct wpa_cred *cred, const char *var) {
  ...
  if (os_strcmp(var, "eap") == 0) {
    if (!cred->eap_method)
      return NULL;
    return alloc_strdup(
        eap_get_name(cred->eap_method[0].vendor, cred->eap_method[0].method));
  }
  ...
  if (os_strcmp(var, "req_conn_capab") == 0) {
    ...
    for (i = 0; i < cred->num_req_conn_capab; i++) {
      ...
      ret = os_snprintf(pos, end - pos, "%s%u", i > 0 ? "\n" : "",
      ...
      if (ports) {
        ...
        for (j = 0; ports[j] != -1; j++) {
          ret =
              os_snprintf(pos, end - pos, "%s%d", j > 0 ? "," : ":", ports[j]);
          if (os_snprintf_error(end - pos, ret))
            return buf;
          pos += ret;
        }
  ...
  if (os_strcmp(var, "domain") == 0) {
    ...
    for (i = 0; i < cred->num_domain; i++) {
      ret = os_snprintf(pos, end - pos, "%s%s", i > 0 ? "\n" : "",
                        cred->domain[i]);
      if (os_snprintf_error(end - pos, ret))
        return buf;
      pos += ret;
  ...
  if (os_strcmp(var, "roaming_partner") == 0) {
    ...
    for (i = 0; i < cred->num_roaming_partner; i++) {
      ...
      ret = os_snprintf(pos, end - pos, "%s%s,%d,%u,%s", i > 0 ? "\n" : "",
                        p->fqdn, p->exact_match, p->priority, p->country);
      if (os_snprintf_error(end - pos, ret))
        return buf;
      pos += ret;
    }
...
void wpa_config_debug_dump_networks(struct wpa_config *config) {
  ...
  for (prio = 0; prio < config->num_prio; prio++) {
    ...
    while (ssid) {
      wpa_printf(MSG_DEBUG, "   id=%d ssid='%s'", ssid->id,
...
struct global_parse_data {
  char *name;
  int (*parser)(const struct global_parse_data *data, struct wpa_config *config,
  ...
  *dst = val;
  ...
  if (data->param2 && *dst < (long)data->param2) {
    wpa_printf(MSG_ERROR,
               "Line %d: too small %s (value=%d "
               "min_value=%ld)",
               line, data->name, *dst, (long)data->param2);
...
static int wpa_global_config_parse_str(const struct global_parse_data *data,
  ...
  if (data->param2 && len < (size_t)data->param2) {
    wpa_printf(MSG_ERROR,
               "Line %d: too short %s (len=%lu "
               "min_len=%ld)",
               line, data->name, (unsigned long)len, (long)data->param2);
    return -1;
  }
  ...
  if (data->param3 && len > (size_t)data->param3) {
    wpa_printf(MSG_ERROR,
               "Line %d: too long %s (len=%lu "
               "max_len=%ld)",
               line, data->name, (unsigned long)len, (long)data->param3);
    return -1;
  }

  if (has_newline(pos)) {
    wpa_printf(MSG_ERROR, "Line %d: invalid %s value with newline", line,
               data->name);
    return -1;
  }
...
static int wpa_global_config_parse_ipv4(const struct global_parse_data *data,
  ...
  wpa_printf(MSG_DEBUG, "%s = 0x%x", data->name, WPA_GET_BE32((u8 *)dst));
...
static int wpa_config_process_country(const struct global_parse_data *data,
  ...
  wpa_printf(MSG_DEBUG, "country='%c%c'", config->country[0],
             config->country[1]);
  return 0;
}

static int
wpa_config_process_load_dynamic_eap(const struct global_parse_data *data,
                                    struct wpa_config *config, int line,
                                    const char *so) {
  int ret;
...
static int wpa_config_process_os_version(const struct global_parse_data *data,
  ...
  wpa_printf(MSG_DEBUG, "os_version=%08x", WPA_GET_BE32(config->os_version));
  return 0;
}

static int
wpa_config_process_wps_vendor_ext_m1(const struct global_parse_data *data,
                                     struct wpa_config *config, int line,
                                     const char *pos) {
  struct wpabuf *tmp;
...
#ifdef CONFIG_P2P
static int
wpa_config_process_sec_device_type(const struct global_parse_data *data,
                                   struct wpa_config *config, int line,
                                   const char *pos) {
  int idx;
...
static int
wpa_config_process_p2p_pref_chan(const struct global_parse_data *data,
                                 struct wpa_config *config, int line,
                                 const char *pos) {
  ...
  while (*pos) {
    ...
    n = os_realloc_array(pref, num + 1, sizeof(struct p2p_channel));
    if (n == NULL)
      goto fail;
    pref = n;
...
static int
wpa_config_process_p2p_no_go_freq(const struct global_parse_data *data,
                                  struct wpa_config *config, int line,
                                  const char *pos) {
  int ret;
...
static int wpa_config_process_hessid(const struct global_parse_data *data,
                                     struct wpa_config *config, int line,
                                     const char *pos) {
  if (hwaddr_aton2(pos, config->hessid) < 0) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid hessid '%s'", line, pos);
    return -1;
  }
...
static int wpa_config_process_sae_groups(const struct global_parse_data *data,
                                         struct wpa_config *config, int line,
                                         const char *pos) {
  int *groups = wpa_config_parse_int_array(pos);
  if (groups == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: Invalid sae_groups '%s'", line, pos);
    return -1;
  }
...
static int
wpa_config_process_ap_vendor_elements(const struct global_parse_data *data,
                                      struct wpa_config *config, int line,
                                      const char *pos) {
  struct wpabuf *tmp;
  int len = os_strlen(pos) / 2;
  ...
  if (!len) {
    wpa_printf(MSG_ERROR, "Line %d: invalid ap_vendor_elements", line);
    return -1;
  }
...
static int
wpa_config_process_no_ctrl_interface(const struct global_parse_data *data,
                                     struct wpa_config *config, int line,
                                     const char *pos) {
...
#define IPV4(f)                                                                \
#f, wpa_global_config_parse_ipv4, wpa_config_get_ipv4, OFFSET(f), NULL, NULL
...
static const struct global_parse_data global_fields[] = {
    ...
    {STR_RANGE(device_name, 0, WPS_DEV_NAME_MAX_LEN), CFG_CHANGED_DEVICE_NAME},
    ...
    {INT_RANGE(p2p_passphrase_len, 8, 63), CFG_CHANGED_P2P_PASSPHRASE_LEN},
...
int wpa_config_dump_values(struct wpa_config *config, char *buf,
  ...
  for (i = 0; i < NUM_GLOBAL_FIELDS; i++) {
    ...
    tmp = field->get(field->name, config, (long)field->param1, buf, buflen, 1);
    if (tmp < 0)
      return -1;
    buf += tmp;
    buflen -= tmp;
...
int wpa_config_get_value(const char *name, struct wpa_config *config, char *buf,
                         size_t buflen) {
  size_t i;
...
int wpa_config_get_num_global_field_names(void) { return NUM_GLOBAL_FIELDS; }
...
int wpa_config_process_global(struct wpa_config *config, char *pos, int line) {
  ...
  if (i == NUM_GLOBAL_FIELDS) {
    ...
    if (os_strncmp(pos, "wmm_ac_", 7) == 0) {
      ...
      *tmp++ = '\0';
      if (hostapd_config_wmm_ac(config->wmm_ac_params, pos, tmp)) {
        wpa_printf(MSG_ERROR,
                   "Line %d: invalid WMM "
                   "AC item",
                   line);
