...
void wpas_notify_state_changed(struct wpa_supplicant *wpa_s,
  ...
  wpa_supplicant_dbus_notify_state_change(wpa_s, new_state, old_state);
  ...
  wpa_msg_ctrl(wpa_s, MSG_INFO,
               WPA_EVENT_STATE_CHANGE "id=%d state=%d BSSID=" MACSTR " SSID=%s",
               wpa_s->current_ssid ? wpa_s->current_ssid->id : -1, new_state,
               MAC2STR(wpa_s->bssid),
...
void wpas_notify_bss_added(struct wpa_supplicant *wpa_s, u8 bssid[],
  ...
  wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_BSS_ADDED "%u " MACSTR, id,
               MAC2STR(bssid));
}
...
void wpas_notify_bss_removed(struct wpa_supplicant *wpa_s, u8 bssid[],
  ...
  wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_BSS_REMOVED "%u " MACSTR, id,
               MAC2STR(bssid));
}
...
void wpas_notify_bss_signal_changed(struct wpa_supplicant *wpa_s,
  ...
  wpas_dbus_bss_signal_prop_changed(wpa_s, WPAS_DBUS_BSS_PROP_SIGNAL, id);
}
...
void wpas_notify_bss_privacy_changed(struct wpa_supplicant *wpa_s,
  ...
  wpas_dbus_bss_signal_prop_changed(wpa_s, WPAS_DBUS_BSS_PROP_PRIVACY, id);
}
...
void wpas_notify_p2p_go_neg_req(struct wpa_supplicant *wpa_s, const u8 *src,
...
void wpas_notify_p2p_invitation_result(struct wpa_supplicant *wpa_s, int status,
                                       const u8 *bssid) {
  wpas_dbus_signal_p2p_invitation_result(wpa_s, status, bssid);
}

void wpas_notify_p2p_sd_request(struct wpa_supplicant *wpa_s, int freq,
                                const u8 *sa, u8 dialog_token, u16 update_indic,
                                const u8 *tlvs, size_t tlvs_len) {
  wpas_dbus_signal_p2p_sd_request(wpa_s, freq, sa, dialog_token, update_indic,
                                  tlvs, tlvs_len);
}

void wpas_notify_p2p_sd_response(struct wpa_supplicant *wpa_s, const u8 *sa,
                                 u16 update_indic, const u8 *tlvs,
                                 size_t tlvs_len) {
  wpas_dbus_signal_p2p_sd_response(wpa_s, sa, update_indic, tlvs, tlvs_len);
}
...
void wpas_notify_p2p_provision_discovery(struct wpa_supplicant *wpa_s,
  ...
  wpas_dbus_signal_p2p_provision_discovery(wpa_s, dev_addr, request, status,
...
void wpas_notify_p2p_invitation_received(struct wpa_supplicant *wpa_s,
  ...
  wpas_dbus_signal_p2p_invitation_received(wpa_s, sa, go_dev_addr, bssid, id,
                                           op_freq);
}
...
void wpas_notify_certification(struct wpa_supplicant *wpa_s, int depth,
  ...
  wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_CERT "depth=%d subject='%s'%s%s",
  ...
  if (altsubject) {
    ...
    for (i = 0; i < num_altsubject; i++)
      wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_ALT "depth=%d %s", depth,
              altsubject[i]);
  }

  /* notify the old DBus API */
  wpa_supplicant_dbus_notify_certification(wpa_s, depth, subject, cert_hash,
                                           cert);
  /* notify the new DBus API */
...
void wpas_notify_preq(struct wpa_supplicant *wpa_s, const u8 *addr,
...
void wpas_notify_eap_status(struct wpa_supplicant *wpa_s, const char *status,
  ...
  wpa_msg_ctrl(wpa_s, MSG_INFO,
               WPA_EVENT_EAP_STATUS "status='%s' parameter='%s'", status,
               parameter);
}
...
void wpas_notify_network_bssid_set_changed(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Network bssid config changed for the current network - within-ESS "
          "roaming %s",
