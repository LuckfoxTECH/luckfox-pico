...
static void ctrl_flush_broken_pipes(struct ctrl_iface_priv *priv) {
  ...
  while (dst) {
    ...
    if (ctrl_broken_pipe(dst->pipe, dst->used)) {
      wpa_printf(MSG_DEBUG, "CTRL: closing broken pipe %p", dst);
      ctrl_close_pipe(dst);
    }
    dst = next;
  }
...
static int ctrl_open_pipe(struct ctrl_iface_priv *priv) {
  ...
  if (dst->overlap.hEvent == NULL) {
    wpa_printf(MSG_ERROR, "CTRL: CreateEvent failed: %d", (int)GetLastError());
    goto fail;
  }

  eloop_register_event(dst->overlap.hEvent, sizeof(dst->overlap.hEvent),
                       wpa_supplicant_ctrl_iface_receive, dst, NULL);

#ifdef UNICODE
  _snwprintf(name, 256, NAMED_PIPE_PREFIX TEXT("-%S"), priv->wpa_s->ifname);
#else  /* UNICODE */
  os_snprintf(name, 256, NAMED_PIPE_PREFIX "-%s", priv->wpa_s->ifname);
#endif /* UNICODE */

  /* TODO: add support for configuring access list for the pipe */
  dst->pipe = CreateNamedPipe(
      name, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
      PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 15, REPLY_BUFSIZE,
      REQUEST_BUFSIZE, 1000, priv->sec_attr_set ? &priv->attr : NULL);
  ...
  case ERROR_PIPE_CONNECTED:
    ...
    /* fall through */
  default:
    wpa_printf(MSG_DEBUG, "CTRL: ConnectNamedPipe error: %d", (int)err);
    CloseHandle(dst->pipe);
    os_free(dst);
    return -1;
  }
...
static void ctrl_close_pipe(struct wpa_ctrl_dst *dst) {
  ...
  if (dst->overlap.hEvent) {
    eloop_unregister_event(dst->overlap.hEvent, sizeof(dst->overlap.hEvent));
...
static VOID WINAPI ctrl_iface_write_completed(DWORD err, DWORD bytes,
  ...
  if (!ReadFileEx(dst->pipe, dst->req_buf, sizeof(dst->req_buf), &dst->overlap,
                  ctrl_iface_read_completed)) {
    wpa_printf(MSG_DEBUG, "CTRL: ReadFileEx failed: %d", (int)GetLastError());
    ctrl_close_pipe(dst);
    return;
  }
...
static void wpa_supplicant_ctrl_iface_rx(struct wpa_ctrl_dst *dst, size_t len) {
  ...
  if (!WriteFileEx(dst->pipe, dst->rsp_buf, send_len, &dst->overlap,
    ...
    wpa_printf(MSG_DEBUG, "CTRL: WriteFileEx failed: %d", (int)GetLastError());
    ctrl_close_pipe(dst);
  } else {
...
static int ctrl_iface_parse(struct ctrl_iface_priv *priv, const char *params) {
  ...
  if (!ConvertStringSecurityDescriptorToSecurityDescriptor(
          ...
          (PSECURITY_DESCRIPTOR *)(void *)&priv->attr.lpSecurityDescriptor,
          NULL)) {
    os_free(t_sddl);
...
static void wpa_supplicant_ctrl_iface_send(struct ctrl_iface_priv *priv,
  ...
  while (dst) {
    ...
    if (dst->attached && level >= dst->debug_level) {
      wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor send %p", dst);
      if (!WriteFile(dst->pipe, sbuf, llen + len, &written, NULL)) {
        wpa_printf(MSG_DEBUG,
...
static int global_open_pipe(struct ctrl_iface_global_priv *priv) {
  ...
  /* TODO: add support for configuring access list for the pipe */
  dst->pipe = CreateNamedPipe(
      NAMED_PIPE_PREFIX, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
      PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 10, REPLY_BUFSIZE,
      REQUEST_BUFSIZE, 1000, NULL);
  if (dst->pipe == INVALID_HANDLE_VALUE) {
    wpa_printf(MSG_ERROR, "CTRL: CreateNamedPipe failed: %d",
               (int)GetLastError());
    goto fail;
