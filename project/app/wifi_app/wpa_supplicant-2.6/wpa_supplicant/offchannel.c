...
#ifdef CONFIG_P2P
    if (wpa_s->p2p_mgmt && wpa_s != wpa_s->parent && wpa_s->parent->ap_iface &&
        os_memcmp(wpa_s->parent->own_addr, wpa_s->own_addr, ETH_ALEN) == 0 &&
        wpabuf_len(wpa_s->pending_action_tx) >= 2 &&
        *wpabuf_head_u8(wpa_s->pending_action_tx) != WLAN_ACTION_PUBLIC) {
      /*
...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(MSG_DEBUG,
             "Off-channel: Send Action callback (without_roc=%d "
             "pending_action_tx=%p pending_action_tx_done=%d)",
  ...
  if (wpa_s->off_channel_freq != wpa_s->pending_action_freq &&
    ...
    wpa_printf(MSG_DEBUG,
               "Off-channel: Pending Action frame TX "
               "waiting for another freq=%u (off_channel_freq=%u "
               "assoc_freq=%u)",
               wpa_s->pending_action_freq, wpa_s->off_channel_freq,
               iface->assoc_freq);
    if (without_roc && wpa_s->off_channel_freq == 0) {
      ...
      if (wpa_drv_remain_on_channel(wpa_s, wpa_s->pending_action_freq,
                                    duration) < 0) {
        wpa_printf(MSG_DEBUG,
                   "Off-channel: Failed to "
        ...
        wpa_s->roc_waiting_drv_freq = wpa_s->pending_action_freq;
      }
    }
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "Off-channel: Sending pending Action frame to " MACSTR
             " using interface %s (pending_action_tx=%p)",
             MAC2STR(wpa_s->pending_action_dst), iface->ifname,
             wpa_s->pending_action_tx);
  res = wpa_drv_send_action(
      iface, wpa_s->pending_action_freq, 0, wpa_s->pending_action_dst,
      wpa_s->pending_action_src, wpa_s->pending_action_bssid,
      wpabuf_head(wpa_s->pending_action_tx),
      wpabuf_len(wpa_s->pending_action_tx), wpa_s->pending_action_no_cck);
  if (res) {
    ...
    offchannel_send_action_tx_status(
        wpa_s, wpa_s->pending_action_dst, wpabuf_head(wpa_s->pending_action_tx),
        wpabuf_len(wpa_s->pending_action_tx), OFFCHANNEL_SEND_ACTION_FAILED);
  }
}
...
void offchannel_send_action_tx_status(
  ...
  if (data_len - wpabuf_len(wpa_s->pending_action_tx) != 24 ||
    ...
    wpa_hexdump(MSG_MSGDUMP, "TX status frame data", data, data_len);
    wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame", wpa_s->pending_action_tx);
    return;
  }

  wpa_printf(MSG_DEBUG,
             "Off-channel: Delete matching pending action frame (dst=" MACSTR
             " pending_action_tx=%p)",
  ...
  wpa_printf(MSG_DEBUG, "Off-channel: TX status result=%d cb=%p", result,
             wpa_s->pending_action_tx_status_cb);

  if (wpa_s->pending_action_tx_status_cb) {
    wpa_s->pending_action_tx_status_cb(
        wpa_s, wpa_s->pending_action_freq, wpa_s->pending_action_dst,
        wpa_s->pending_action_src, wpa_s->pending_action_bssid, data, data_len,
        result);
  }
 ...
 */
int offchannel_send_action(
    struct wpa_supplicant *wpa_s, unsigned int freq, const u8 *dst,
    const u8 *src, const u8 *bssid, const u8 *buf, size_t len,
    unsigned int wait_time,
    void (*tx_cb)(struct wpa_supplicant *wpa_s, unsigned int freq,
                  const u8 *dst, const u8 *src, const u8 *bssid, const u8 *data,
                  size_t data_len, enum offchannel_send_action_result result),
    int no_cck) {
  wpa_printf(MSG_DEBUG,
             "Off-channel: Send action frame: freq=%d dst=" MACSTR
             " src=" MACSTR " bssid=" MACSTR " len=%d",
             freq, MAC2STR(dst), MAC2STR(src), MAC2STR(bssid), (int)len);

  wpa_s->pending_action_tx_status_cb = tx_cb;

  if (wpa_s->pending_action_tx) {
    wpa_printf(MSG_DEBUG,
               "Off-channel: Dropped pending Action "
               "frame TX to " MACSTR " (pending_action_tx=%p)",
               MAC2STR(wpa_s->pending_action_dst), wpa_s->pending_action_tx);
    wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame", wpa_s->pending_action_tx);
    wpabuf_free(wpa_s->pending_action_tx);
  }
  ...
  if (wpa_s->pending_action_tx == NULL) {
    wpa_printf(MSG_DEBUG,
               "Off-channel: Failed to allocate Action "
  ...
  if (freq != 0 && wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) {
    ...
    ret = wpa_drv_send_action(
        iface, wpa_s->pending_action_freq, wait_time, wpa_s->pending_action_dst,
        wpa_s->pending_action_src, wpa_s->pending_action_bssid,
        wpabuf_head(wpa_s->pending_action_tx),
        wpabuf_len(wpa_s->pending_action_tx), wpa_s->pending_action_no_cck);
    if (ret == 0)
  ...
  if (wpa_s->roc_waiting_drv_freq == freq) {
    wpa_printf(MSG_DEBUG,
               "Off-channel: Already waiting for "
...
#ifdef CONFIG_TESTING_OPTIONS
  if (wpa_s->extra_roc_dur) {
    wpa_printf(MSG_DEBUG, "TESTING: Increase ROC duration %u -> %u", wait_time,
  ...
  if (wpa_drv_remain_on_channel(wpa_s, freq, wait_time) < 0) {
    wpa_printf(MSG_DEBUG,
               "Off-channel: Failed to request driver "
...
void offchannel_send_action_done(struct wpa_supplicant *wpa_s) {
  wpa_printf(MSG_DEBUG,
             ...
             "pending_action_tx=%p drv_offchan_tx=%d action_tx_wait_time=%d "
             "off_channel_freq=%d roc_waiting_drv_freq=%d",
             wpa_s->pending_action_tx,
