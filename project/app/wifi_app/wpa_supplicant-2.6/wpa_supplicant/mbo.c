...
static void wpas_mbo_non_pref_chan_attrs(struct wpa_supplicant *wpa_s,
  ...
  for (i = 1; i <= wpa_s->non_pref_chan_num; i++) {
    ...
    if (!non_pref || non_pref->oper_class != start_pref->oper_class ||
...
static void wpas_mbo_send_wnm_notification(struct wpa_supplicant *wpa_s,
  ...
  res = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                            ...
                            wpabuf_len(buf), 0);
  if (res < 0)
    wpa_printf(MSG_DEBUG, "Failed to send WNM-Notification Request frame with "
...
int wpas_mbo_update_non_pref_chan(struct wpa_supplicant *wpa_s,
  ...
  while ((token = str_token(cmd, " ", &context))) {
    ...
    ret = sscanf(token, "%u:%u:%u:%u", &_oper_class, &_chan, &_preference,
                 &_reason);
    if (ret != 4 || _oper_class > 255 || _chan > 255 || _preference > 255 ||
        _reason > 65535) {
    ...
    if (wpas_mbo_validate_non_pref_chan(chan->oper_class, chan->chan,
                                        chan->reason)) {
    ...
    for (i = 0; i < num; i++)
      if (wpa_non_pref_chan_is_eq(chan, &chans[i]))
        break;
    if (i != num) {
      wpa_printf(MSG_ERROR, "oper class %d chan %d is duplicated",
...
enum chan_allowed { NOT_ALLOWED, ALLOWED };
...
static int wpas_op_class_supported(struct wpa_supplicant *wpa_s,
  ...
  if (op_class->op_class == 128) {
    ...
    for (i = 0; i < ARRAY_SIZE(channels); i++) {
      if (verify_channel(mode, channels[i], op_class->bw) == ALLOWED)
        return 1;
    }
...
int wpas_mbo_supp_op_class_ie(struct wpa_supplicant *wpa_s, int freq, u8 *pos,
   ...
   */
  if (ieee80211_freq_to_channel_ext(freq, 0, VHT_CHANWIDTH_USE_HT, &current,
  ...
  *ie_len = wpabuf_len(buf) - 2;
  if (*ie_len < 2 || wpabuf_len(buf) > len) {
    wpa_printf(MSG_ERROR, "Failed to add supported operating classes IE");
    res = 0;
  } else {
    os_memcpy(pos, wpabuf_head(buf), wpabuf_len(buf));
    res = wpabuf_len(buf);
    wpa_hexdump_buf(MSG_DEBUG, "MBO: Added supported operating classes IE",
                    buf);
  }
...
void wpas_mbo_ie_trans_req(struct wpa_supplicant *wpa_s, const u8 *mbo_ie,
  ...
  while (len >= 2) {
    ...
    case MBO_ATTR_ID_ASSOC_RETRY_DELAY:
      ...
      if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_BSS_TERMINATION_INCLUDED) {
        ...
        wpa_printf(MSG_DEBUG, "MBO: Association retry delay attribute not in "
                              "disassoc imminent mode");
      }
    ...
    case MBO_ATTR_ID_TRANSITION_REJECT_REASON:
      wpa_printf(
          MSG_DEBUG,
          "MBO: Attribute %d should not be included in BTM Request frame", id);
      break;
    default:
  ...
  if (reason)
    wpa_msg(wpa_s, MSG_INFO, MBO_TRANSITION_REASON "reason=%u", *reason);

  if (disallowed_sec && wpa_s->current_bss)
    wpa_bss_tmp_disallow(wpa_s, wpa_s->current_bss->bssid, disallowed_sec);

  return;
fail:
  wpa_printf(MSG_DEBUG, "MBO IE parsing failed (id=%u len=%u left=%zu)", id,
             elen, len);
}
...
void wpas_mbo_update_cell_capa(struct wpa_supplicant *wpa_s, u8 mbo_cell_capa) {
  ...
  if (wpa_s->conf->mbo_cell_capa == mbo_cell_capa) {
    wpa_printf(MSG_DEBUG, "MBO: Cellular capability already set to %u",
               mbo_cell_capa);
    return;
  }
