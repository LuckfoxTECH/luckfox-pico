...
static int wpas_p2p_join(struct wpa_supplicant *wpa_s, const u8 *iface_addr,
                         ...
                         int auto_join, int freq, const u8 *ssid,
                         size_t ssid_len);
...
static int wpas_p2p_num_unused_channels(struct wpa_supplicant *wpa_s) {
  ...
  num = get_shared_radio_freqs(wpa_s, freqs, wpa_s->num_multichan_concurrent);
  os_free(freqs);
...
static void wpas_p2p_set_own_freq_preference(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->conf->p2p_ignore_shared_freq && freq > 0 &&
      wpa_s->num_multichan_concurrent > 1 &&
      wpas_p2p_num_unused_channels(wpa_s) > 0) {
    wpa_printf(MSG_DEBUG,
               "P2P: Ignore own channel preference %d MHz due to "
               "p2p_ignore_shared_freq=1 configuration",
               freq);
    freq = 0;
  }
...
static void wpas_p2p_scan_res_handler(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < scan_res->num; i++) {
    ...
    if (bss->beacon_ie_len > 0 &&
      ...
      wpa_printf(MSG_DEBUG,
                 "P2P: Use P2P IE(s) from Beacon frame since no P2P IE(s) in "
                 "Probe Response frames received for " MACSTR,
                 MAC2STR(bss->bssid));
      ies = ies + ies_len;
      ies_len = bss->beacon_ie_len;
    }

    if (p2p_scan_res_handler(wpa_s->global->p2p, bss->bssid, bss->freq,
                             &entry_ts, bss->level, ies, ies_len) > 0)
      break;
  }
...
static int wpas_p2p_scan(void *ctx, enum p2p_scan_type type, int freq,
  ...
  wpa_s->wps->dev.p2p = 1;
  wps_ie = wps_build_probe_req_ie(pw_id, &wpa_s->wps->dev, wpa_s->wps->uuid,
  ...
  case P2P_SCAN_SOCIAL:
    params->freqs =
        os_calloc(ARRAY_SIZE(social_channels_freq) + 1, sizeof(int));
    if (params->freqs == NULL)
      goto fail;
    for (i = 0; i < ARRAY_SIZE(social_channels_freq); i++) {
      if (wpas_p2p_search_social_channel(wpa_s, social_channels_freq[i]))
        params->freqs[num_channels++] = social_channels_freq[i];
    }
  ...
  case P2P_SCAN_SOCIAL_PLUS_ONE:
    params->freqs =
        os_calloc(ARRAY_SIZE(social_channels_freq) + 2, sizeof(int));
    if (params->freqs == NULL)
      goto fail;
    for (i = 0; i < ARRAY_SIZE(social_channels_freq); i++) {
      if (wpas_p2p_search_social_channel(wpa_s, social_channels_freq[i]))
...
static struct wpa_supplicant *wpas_get_p2p_group(struct wpa_supplicant *wpa_s,
  ...
  for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    for (s = wpa_s->conf->ssid; s; s = s->next) {
      if (s->disabled != 0 || !s->p2p_group || s->ssid_len != ssid_len ||
          os_memcmp(ssid, s->ssid, ssid_len) != 0)
        continue;
      if (s->mode == WPAS_MODE_P2P_GO && s != wpa_s->current_ssid)
        continue;
      if (go)
...
static void run_wpas_p2p_disconnect(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: Complete previously requested removal of %s",
             wpa_s->ifname);
...
static int wpas_p2p_disconnect_safely(struct wpa_supplicant *wpa_s,
  ...
  if (calling_wpa_s == wpa_s && wpa_s &&
     ...
     */
    if (eloop_register_timeout(0, 0, run_wpas_p2p_disconnect, wpa_s, NULL) < 0)
      return -1;
    return 0;
  }
...
static unsigned int p2p_group_go_member_count(struct wpa_supplicant *wpa_s) {
  ...
  for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    for (s = wpa_s->conf->ssid; s; s = s->next) {
      wpa_printf(MSG_DEBUG, "P2P: sup:%p ssid:%p disabled:%d p2p:%d mode:%d",
                 wpa_s, s, s->disabled, s->p2p_group, s->mode);
      if (!s->disabled && s->p2p_group && s->mode == WPAS_MODE_P2P_GO) {
        count += p2p_get_group_num_members(wpa_s->p2p_group);
      }
    }
  }
...
static unsigned int
p2p_is_active_persistent_group(struct wpa_supplicant *wpa_s) {
  return !wpa_s->p2p_mgmt && wpa_s->current_ssid &&
         !wpa_s->current_ssid->disabled && wpa_s->current_ssid->p2p_group &&
...
static u8 p2ps_group_capability(void *ctx, u8 incoming, u8 role,
  ...
  if (force_freq && pref_freq &&
      !wpas_p2p_setup_freqs(wpa_s, 0, (int *)force_freq, (int *)pref_freq, 0,
                            pref_freq_list, &size))
  ...
  if (p2p_no_group_iface) {
    ...
    if (go_wpa_s) {
      if (role == P2PS_SETUP_CLIENT || incoming == P2PS_SETUP_GROUP_OWNER ||
    ...
    if (persistent_go) {
      if (role == P2PS_SETUP_NONE || role == P2PS_SETUP_NEW) {
        if (!incoming)
          return P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_CLIENT;
        if (incoming == P2PS_SETUP_NEW) {
          u8 r;

          if (os_get_random(&r, sizeof(r)) < 0 || (r & 1))
...
grp_owner:
  if ((conncap & P2PS_SETUP_GROUP_OWNER) ||
    ...
    if (!s && !go_wpa_s && p2p_no_group_iface) {
      p2p_set_intended_addr(wpa_s->global->p2p, wpa_s->p2p_mgmt
      ...
      p2p_set_intended_addr(wpa_s->global->p2p, wpa_s->pending_interface_addr);
    }
  }
...
static int wpas_p2p_group_delete(struct wpa_supplicant *wpa_s,
  ...
  if (os_strcmp(gtype, "client") == 0) {
    ...
    if (eloop_is_timeout_registered(wpas_p2p_psk_failure_removal, wpa_s,
                                    NULL)) {
      wpa_printf(MSG_DEBUG, "P2P: PSK failure removal was scheduled, so use "
                            "PSK failure as reason for group removal");
  ...
  if (wpa_s->cross_connect_in_use) {
    ...
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                   P2P_EVENT_CROSS_CONNECT_DISABLE "%s %s", wpa_s->ifname,
                   wpa_s->cross_connect_uplink);
  }
  ...
  if (removal_reason != P2P_GROUP_REMOVAL_SILENT) {
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_REMOVED "%s %s%s",
  ...
  if (eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev,
                           NULL) > 0) {
  ...
  if (wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE) {
    ...
    char *ifname;
    enum wpa_driver_if_type type;
    wpa_printf(MSG_DEBUG, "P2P: Remove group interface %s", wpa_s->ifname);
    global = wpa_s->global;
  ...
  if (ssid && (ssid->p2p_group || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION ||
...
static int wpas_p2p_persistent_group(struct wpa_supplicant *wpa_s,
  ...
  if (bss == NULL && wpa_s->go_params &&
    ...
    bss = wpa_bss_get_p2p_dev_addr(wpa_s, wpa_s->go_params->peer_device_addr);
  if (bss == NULL) {
    u8 iface_addr[ETH_ALEN];
    if (p2p_get_interface_addr(wpa_s->global->p2p, bssid, iface_addr) == 0)
  ...
  if (p2p == NULL)
    p2p = wpa_bss_get_vendor_ie_multi_beacon(bss, P2P_IE_VENDOR_TYPE);
  if (p2p == NULL) {
    wpa_printf(MSG_DEBUG,
               "P2P: Could not figure out whether "
               "group is persistent - BSS " MACSTR " did not include P2P IE",
               MAC2STR(bssid));
    wpa_hexdump(MSG_DEBUG, "P2P: Probe Response IEs", (u8 *)(bss + 1),
                bss->ie_len);
    wpa_hexdump(MSG_DEBUG, "P2P: Beacon IEs", ((u8 *)bss + 1) + bss->ie_len,
                bss->beacon_ie_len);
    return 0;
  }
  ...
  if (addr) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: GO Device Address " MACSTR, MAC2STR(addr));
  } else
    os_memset(go_dev_addr, 0, ETH_ALEN);
  wpabuf_free(p2p);

  wpa_printf(MSG_DEBUG,
             "P2P: BSS " MACSTR " group_capab=0x%x "
             "go_dev_addr=" MACSTR,
...
static void wpas_p2p_add_persistent_group_client(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; s->p2p_client_list && i < s->num_p2p_clients; i++) {
    if (os_memcmp(s->p2p_client_list + i * 2 * ETH_ALEN, addr, ETH_ALEN) != 0)
      continue;
    ...
    os_memcpy(s->p2p_client_list + (s->num_p2p_clients - 1) * 2 * ETH_ALEN,
              addr, ETH_ALEN);
    os_memset(s->p2p_client_list + (s->num_p2p_clients - 1) * 2 * ETH_ALEN +
                  ETH_ALEN,
              0xff, ETH_ALEN);
    found = 1;
    break;
  ...
  if (!found && s->num_p2p_clients < P2P_MAX_STORED_CLIENTS) {
    ...
    if (n == NULL)
      return;
    os_memcpy(n + s->num_p2p_clients * 2 * ETH_ALEN, addr, ETH_ALEN);
    os_memset(n + s->num_p2p_clients * 2 * ETH_ALEN + ETH_ALEN, 0xff, ETH_ALEN);
    s->p2p_client_list = n;
    s->num_p2p_clients++;
  } else if (!found && s->p2p_client_list) {
    /* Not enough room for an additional entry - drop the oldest
     * entry */
    os_memmove(s->p2p_client_list, s->p2p_client_list + 2 * ETH_ALEN,
...
static void wpas_p2p_group_started(struct wpa_supplicant *wpa_s, int go,
                                   struct wpa_ssid *ssid, int freq,
   ...
   */
  wpa_msg_global_ctrl(
      wpa_s->p2pdev, MSG_INFO,
      P2P_EVENT_GROUP_STARTED
      "%s %s ssid=\"%s\" freq=%d%s%s%s%s%s go_dev_addr=" MACSTR "%s%s",
      wpa_s->ifname, go ? "GO" : "client", ssid_txt, freq, psk ? " psk=" : "",
      psk_txt, passphrase ? " passphrase=\"" : "", passphrase ? passphrase : "",
      passphrase ? "\"" : "", MAC2STR(go_dev_addr),
      persistent ? " [PERSISTENT]" : "", extra);
  wpa_printf(MSG_INFO,
             P2P_EVENT_GROUP_STARTED
             "%s %s ssid=\"%s\" freq=%d go_dev_addr=" MACSTR "%s%s",
             wpa_s->ifname, go ? "GO" : "client", ssid_txt, freq,
             MAC2STR(go_dev_addr), persistent ? " [PERSISTENT]" : "", extra);
}
...
static void wpas_group_formation_completed(struct wpa_supplicant *wpa_s,
  ...
  if (!success) {
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_FORMATION_FAILURE);
    wpas_notify_p2p_group_formation_failure(wpa_s, "");
    if (already_deleted)
      return;
    wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_FORMATION_FAILED);
    return;
  }

  wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_FORMATION_SUCCESS);
  ...
  if (ssid) {
    ...
    if (ssid->mode == WPAS_MODE_P2P_GO) {
      ...
      os_memcpy(go_dev_addr, wpa_s->global->p2p_dev_addr, ETH_ALEN);
    } else
      persistent = wpas_p2p_persistent_group(wpa_s, go_dev_addr, ssid->ssid,
                                             ssid->ssid_len);
  } else {
    client = wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT;
    os_memset(go_dev_addr, 0, ETH_ALEN);
  }
  ...
  if (client) {
    ...
    wpas_p2p_group_started(
        wpa_s, 1, ssid, ssid ? ssid->frequency : 0,
        ssid && ssid->passphrase == NULL && ssid->psk_set ? ssid->psk : NULL,
        ssid ? ssid->passphrase : NULL, go_dev_addr, persistent, "");
    wpas_p2p_cross_connect_setup(wpa_s);
    wpas_p2p_set_group_idle_timeout(wpa_s);
  }

  if (persistent)
    wpas_p2p_store_persistent_group(wpa_s->p2pdev, ssid, go_dev_addr);
  else {
...
static void wpas_p2p_free_send_action_work(struct wpa_supplicant *wpa_s) {
  ...
  wpa_printf(MSG_DEBUG,
             "P2P: Free Action frame radio work @%p (freq=%u dst=" MACSTR
             " src=" MACSTR " bssid=" MACSTR " wait_time=%u)",
             wpa_s->p2p_send_action_work, awork->freq, MAC2STR(awork->dst),
             MAC2STR(awork->src), MAC2STR(awork->bssid), awork->wait_time);
  wpa_hexdump(MSG_DEBUG, "P2P: Freeing pending Action frame", awork->buf,
              awork->len);
  os_free(awork);
...
static void wpas_p2p_action_tx_clear(struct wpa_supplicant *wpa_s) {
  if (wpa_s->p2p_send_action_work) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: Clear Action TX work @%p (wait_time=%u)",
    ...
    if (awork->wait_time == 0) {
      ...
      eloop_register_timeout(0, awork->wait_time * 1000,
                             wpas_p2p_send_action_work_timeout, wpa_s, NULL);
    }
  }
}

static void
wpas_p2p_send_action_tx_status(struct wpa_supplicant *wpa_s, unsigned int freq,
  ...
  if (result != OFFCHANNEL_SEND_ACTION_SUCCESS &&
    ...
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                   P2P_EVENT_FALLBACK_TO_GO_NEG "reason=no-ACK-to-PD-Req");
...
static void wpas_send_action_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (offchannel_send_action(wpa_s, awork->freq, awork->dst, awork->src,
                             ...
                             awork->wait_time, wpas_p2p_send_action_tx_status,
                             1) < 0) {
    os_free(awork);
    radio_work_done(work);
    return;
...
static int wpas_send_action_work(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->p2p_send_action_work) {
    wpa_printf(MSG_DEBUG, "P2P: Cannot schedule new p2p-send-action work since "
  ...
  if (radio_add_work(wpa_s, freq, "p2p-send-action", 0, wpas_send_action_cb,
                     awork) < 0) {
    os_free(awork);
    return -1;
  }
...
static int wpas_send_action(void *ctx, unsigned int freq, const u8 *dst,
  ...
  if (listen_freq != (int)freq && send_freq != (int)freq) {
    wpa_printf(MSG_DEBUG,
               "P2P: Schedule new radio work for Action frame TX "
               "(listen_freq=%d send_freq=%d)",
               listen_freq, send_freq);
    return wpas_send_action_work(wpa_s, freq, dst, src, bssid, buf, len,
                                 wait_time);
  }
  ...
  return offchannel_send_action(wpa_s, freq, dst, src, bssid, buf, len,
                                wait_time, wpas_p2p_send_action_tx_status, 1);
}
...
static void wpas_start_wps_enrollee(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             "P2P: Start WPS Enrollee for peer " MACSTR " dev_addr " MACSTR
             " wps_method %d",
             MAC2STR(res->peer_interface_addr), MAC2STR(res->peer_device_addr),
             res->wps_method);
  wpa_hexdump_ascii(MSG_DEBUG, "P2P: Start WPS Enrollee for SSID", res->ssid,
                    res->ssid_len);
  wpa_supplicant_ap_deinit(wpa_s);
  ...
  if (res->wps_method == WPS_PBC) {
    ...
    wpas_wps_start_nfc(
        wpa_s, res->peer_device_addr, res->peer_interface_addr,
        wpa_s->p2pdev->p2p_oob_dev_pw, wpa_s->p2pdev->p2p_oob_dev_pw_id, 1,
        wpa_s->p2pdev->p2p_oob_dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER
    ...
    wpas_wps_start_pin(wpa_s, res->peer_interface_addr, wpa_s->p2p_pin, 1,
                       dev_pw_id);
  }
}
...
static void wpas_p2p_add_psk_list(struct wpa_supplicant *wpa_s,
  ...
  persistent =
      wpas_p2p_get_persistent(wpa_s->p2pdev, NULL, ssid->ssid, ssid->ssid_len);
  if (persistent == NULL)
    return;

  hapd = wpa_s->ap_iface->bss[0];

  dl_list_for_each(psk, &persistent->psk_list, struct psk_list_entry, list) {
    struct hostapd_wpa_psk *hpsk;

    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Add persistent group PSK entry for " MACSTR " psk=%d",
...
static void p2p_go_dump_common_freqs(struct wpa_supplicant *wpa_s) {
  ...
  for (i = 0; i < wpa_s->p2p_group_common_freqs_num; i++)
    wpa_dbg(wpa_s, MSG_DEBUG, "freq[%u]: %d", i,
...
static void p2p_go_save_group_common_freqs(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < len; i++) {
    if (!wpa_s->go_params->freq_list[i])
      break;
    wpa_s->p2p_group_common_freqs[i] = wpa_s->go_params->freq_list[i];
  }
...
static void p2p_go_configured(void *ctx, void *data) {
  ...
  if (ssid && ssid->mode == WPAS_MODE_P2P_GO) {
    ...
    wpas_p2p_group_started(wpa_s, 1, ssid, ssid->frequency,
                           params->passphrase[0] == '\0' ? params->psk : NULL,
                           params->passphrase, wpa_s->global->p2p_dev_addr,
                           params->persistent_group, "");
    wpa_s->group_formation_reported = 1;

    if (wpa_s->p2pdev->p2ps_method_config_any) {
      if (is_zero_ether_addr(wpa_s->p2pdev->p2ps_join_addr)) {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2PS: Setting default PIN for ANY");
        wpa_supplicant_ap_wps_pin(wpa_s, NULL, "12345670", NULL, 0, 0);
      } else {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2PS: Setting default PIN for " MACSTR,
    ...
    if (params->persistent_group) {
      wpas_p2p_store_persistent_group(wpa_s->p2pdev, ssid,
                                      wpa_s->global->p2p_dev_addr);
      wpas_p2p_add_psk_list(wpa_s, ssid);
    }

    wpas_notify_p2p_group_started(wpa_s, ssid, params->persistent_group, 0);
    wpas_p2p_cross_connect_setup(wpa_s);
    wpas_p2p_set_group_idle_timeout(wpa_s);

    if (wpa_s->p2p_first_connection_timeout) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: Start group formation timeout of %d seconds until first "
              "data connection on GO",
              wpa_s->p2p_first_connection_timeout);
      wpa_s->p2p_go_group_formation_completed = 0;
      wpa_s->global->p2p_group_formation = wpa_s;
      eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev,
                           NULL);
      eloop_register_timeout(wpa_s->p2p_first_connection_timeout, 0,
                             wpas_p2p_group_formation_timeout, wpa_s->p2pdev,
                             NULL);
    }
  ...
  if (wpa_supplicant_ap_mac_addr_filter(wpa_s, params->peer_interface_addr)) {
  ...
  if (params->wps_method == WPS_PBC) {
    ...
    if (wpa_s->p2pdev->p2p_oob_dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER &&
        !wpa_s->p2pdev->p2p_oob_dev_pw) {
      wpa_printf(MSG_DEBUG, "P2P: No NFC Dev Pw known");
      return;
    }
    wpas_ap_wps_add_nfc_pw(wpa_s, wpa_s->p2pdev->p2p_oob_dev_pw_id,
                           wpa_s->p2pdev->p2p_oob_dev_pw,
...
static void wpas_start_wps_go(struct wpa_supplicant *wpa_s,
  ...
  ssid->mode =
      group_formation ? WPAS_MODE_P2P_GROUP_FORMATION : WPAS_MODE_P2P_GO;
  ...
  if (os_strlen(params->passphrase) > 0) {
    ...
    if (ssid->passphrase == NULL) {
      wpa_msg_global(wpa_s, MSG_ERROR, "P2P: Failed to copy passphrase for GO");
...
#undef C
  ...
  os_memcpy(d->sec_device_type, s->sec_device_type, sizeof(d->sec_device_type));
...
static void wpas_p2p_get_group_ifname(struct wpa_supplicant *wpa_s,
  ...
  if (os_strlen(ifname) >= IFNAMSIZ && os_strlen(wpa_s->ifname) < IFNAMSIZ) {
    int res;
...
static int wpas_p2p_add_group_interface(struct wpa_supplicant *wpa_s,
  ...
  if (force_ifname[0]) {
    wpa_printf(MSG_DEBUG, "P2P: Driver forced interface name %s", force_ifname);
    ...
    os_strlcpy(wpa_s->pending_interface_name, ifname,
               sizeof(wpa_s->pending_interface_name));
  wpa_printf(
      MSG_DEBUG, "P2P: Created pending virtual interface %s addr " MACSTR,
      wpa_s->pending_interface_name, MAC2STR(wpa_s->pending_interface_addr));

  return 0;
}

static void
wpas_p2p_remove_pending_group_interface(struct wpa_supplicant *wpa_s) {
  if (!wpa_s->pending_interface_name[0] ||
...
wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go) {
  ...
  if (wpa_s->conf->ctrl_interface == NULL && wpa_s->parent != wpa_s &&
      wpa_s->p2p_mgmt &&
  ...
  wpa_s->pending_interface_name[0] = '\0';
  group_wpa_s->p2p_group_interface =
      go ? P2P_GROUP_INTERFACE_GO : P2P_GROUP_INTERFACE_CLIENT;
...
static void wpas_p2p_grpform_fail_after_wps(struct wpa_supplicant *wpa_s) {
  ...
  eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
  eloop_register_timeout(0, 0, wpas_p2p_group_formation_timeout, wpa_s->p2pdev,
                         NULL);
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res) {
  ...
  if (res->status) {
    wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_FAILURE "status=%d",
                   res->status);
  ...
  wpa_msg_global(
      wpa_s, MSG_INFO,
      P2P_EVENT_GO_NEG_SUCCESS "role=%s "
                               "freq=%d ht40=%d peer_dev=" MACSTR
                               " peer_iface=" MACSTR " wps_method=%s",
      res->role_go ? "GO" : "client", res->freq, res->ht40,
      MAC2STR(res->peer_device_addr), MAC2STR(res->peer_interface_addr),
      p2p_wps_method_text(res->wps_method));
  wpas_notify_p2p_go_neg_completed(wpa_s, res);

  if (res->role_go && wpa_s->p2p_persistent_id >= 0) {
    struct wpa_ssid *ssid;
    ssid = wpa_config_get_network(wpa_s->conf, wpa_s->p2p_persistent_id);
    if (ssid && ssid->disabled == 2 && ssid->mode == WPAS_MODE_P2P_GO &&
        ssid->passphrase) {
  ...
  if (wpa_s->create_p2p_iface) {
    group_wpa_s = wpas_p2p_init_group_interface(wpa_s, res->role_go);
    if (group_wpa_s == NULL) {
      wpas_p2p_remove_pending_group_interface(wpa_s);
...
static void wpas_dev_found(void *ctx, const u8 *addr,
  ...
  wfd_dev_info_hex =
      wifi_display_subelem_hex(info->wfd_subelems, WFD_SUBELEM_DEVICE_INFO);
  ...
  if (info->p2ps_instance) {
    ...
    while (len) {
      ...
      wpa_msg_global(
          wpa_s, MSG_INFO,
          P2P_EVENT_DEVICE_FOUND MACSTR " p2p_dev_addr=" MACSTR
          ...
          info->device_name, methods, info->dev_capab, info->group_capab, id,
          str, info->vendor_elems ? " vendor_elems=1" : "");
    }
    goto done;
  }
  ...
  wpa_msg_global(
      ...
      " p2p_dev_addr=" MACSTR " pri_dev_type=%s name='%s' config_methods=0x%x "
      "dev_capab=0x%x group_capab=0x%x%s%s%s new=%d",
      MAC2STR(addr), MAC2STR(info->p2p_device_addr),
      wps_dev_type_bin2str(info->pri_dev_type, devtype, sizeof(devtype)),
      info->device_name, info->config_methods, info->dev_capab,
      info->group_capab, wfd_dev_info_hex ? " wfd_dev_info=0x" : "",
      wfd_dev_info_hex ? wfd_dev_info_hex : "",
      info->vendor_elems ? " vendor_elems=1" : "", new_device);
...
static void wpas_dev_lost(void *ctx, const u8 *dev_addr) {
  ...
  wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_LOST "p2p_dev_addr=" MACSTR,
                 MAC2STR(dev_addr));
...
static void wpas_start_listen_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (wpa_s->extra_roc_dur) {
    wpa_printf(MSG_DEBUG, "TESTING: Increase ROC duration %u -> %u", duration,
...
static int wpas_send_probe_resp(void *ctx, const struct wpabuf *buf,
                                unsigned int freq) {
  struct wpa_supplicant *wpa_s = ctx;
  return wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, freq);
}

static void wpas_prov_disc_local_display(struct wpa_supplicant *wpa_s,
                                         const u8 *peer, const char *params,
                                         unsigned int generated_pin) {
  wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_SHOW_PIN MACSTR " %08d%s",
                 MAC2STR(peer), generated_pin, params);
}

static void wpas_prov_disc_local_keypad(struct wpa_supplicant *wpa_s,
                                        const u8 *peer, const char *params) {
  wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_ENTER_PIN MACSTR "%s",
                 MAC2STR(peer), params);
}
...
static void wpas_prov_disc_req(void *ctx, const u8 *peer, u16 config_methods,
  ...
  if (group_id) {
    for (group = wpa_s->global->ifaces; group; group = group->next) {
      ...
      if (s != NULL && s->mode == WPAS_MODE_P2P_GO &&
          ...
          os_memcmp(group_id + ETH_ALEN, s->ssid, s->ssid_len) == 0)
        break;
    }
  }
  ...
  res = os_snprintf(
      ...
      MAC2STR(dev_addr),
      wps_dev_type_bin2str(pri_dev_type, devtype, sizeof(devtype)), dev_name,
      supp_config_methods, dev_capab, group_capab, group ? " group=" : "",
      group ? group->ifname : "");
  ...
  if (config_methods & WPS_CONFIG_DISPLAY) {
    ...
    wpas_prov_disc_local_display(wpa_s, peer, params, generated_pin);
  ...
  wpas_notify_p2p_provision_discovery(wpa_s, peer, 1 /* request */,
                                      P2P_PROV_DISC_SUCCESS, config_methods,
                                      generated_pin);
}
...
static void wpas_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods) {
  ...
  if (config_methods & WPS_CONFIG_DISPLAY)
    ...
    wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_PBC_RESP MACSTR "%s",
                   MAC2STR(peer), params);
...
static void wpas_prov_disc_fail(void *ctx, const u8 *peer,
  ...
  if (wpa_s->p2p_fallback_to_go_neg) {
    ...
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                   P2P_EVENT_FALLBACK_TO_GO_NEG "reason=PD-failed");
    wpas_p2p_fallback_to_go_neg(wpa_s, 0);
    return;
  }
  ...
  if (adv_id && adv_mac && deferred_session_resp) {
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR
                                               " status=%d adv_id=%x"
                                               " deferred_session_resp='%s'",
                   MAC2STR(peer), status, adv_id, deferred_session_resp);
  ...
  wpas_notify_p2p_provision_discovery(wpa_s, peer, 0 /* response */, status, 0,
                                      0);
}
...
static void wpas_p2p_go_update_common_freqs(struct wpa_supplicant *wpa_s) {
  ...
  if (ret < 0) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Failed to get group common freqs");
...
static int wpas_sta_check_ecsa(struct hostapd_data *hapd, struct sta_info *sta,
                               void *ctx) {
...
static int wpas_p2p_go_clients_support_ecsa(struct wpa_supplicant *wpa_s) {
  ...
  ap_for_each_sta(wpa_s->ap_iface->bss[0], wpas_sta_check_ecsa, &ecsa_support);
...
static u8 wpas_invitation_process(void *ctx, const u8 *sa, const u8 *bssid,
  ...
  if (!persistent_group) {
    ...
    if (!is_zero_ether_addr(wpa_s->p2p_auth_invite) &&
        (os_memcmp(go_dev_addr, wpa_s->p2p_auth_invite, ETH_ALEN) == 0 ||
    ...
    if (dev_pw_id >= 0 && wpa_s->p2p_nfc_tag_enabled &&
      ...
      os_memcpy(wpa_s->pending_join_dev_addr, go_dev_addr, ETH_ALEN);
      os_memcpy(wpa_s->pending_join_iface_addr, bssid, ETH_ALEN);
      goto accept_inv;
    }
#endif /* CONFIG_WPS_NFC */
  ...
  for (s = wpa_s->conf->ssid; s; s = s->next) {
    if (s->disabled == 2 && os_memcmp(s->bssid, go_dev_addr, ETH_ALEN) == 0 &&
        s->ssid_len == ssid_len && os_memcmp(ssid, s->ssid, ssid_len) == 0)
      break;
  }
  ...
  if (s->mode == WPAS_MODE_P2P_GO && !wpas_p2p_create_iface(wpa_s)) {
    *go = 1;
    ...
    if (wpa_s->p2p_mgmt)
      os_memcpy(group_bssid, wpa_s->parent->own_addr, ETH_ALEN);
    else
    ...
    *go = 1;
    ...
    os_memcpy(group_bssid, wpa_s->pending_interface_addr, ETH_ALEN);
  }
...
accept_inv:
  ...
  if (best_freq > 0) {
    wpa_printf(MSG_DEBUG, "P2P: Trying to prefer a channel already used by one "
                          "of the interfaces");
    ...
    if (wpa_s->num_multichan_concurrent < 2 ||
      ...
      wpa_printf(MSG_DEBUG,
                 "P2P: No extra channels available - trying to force channel "
                 "to match a channel already used by one of the interfaces");
...
static void wpas_invitation_received(void *ctx, const u8 *sa, const u8 *bssid,
  ...
  for (s = wpa_s->conf->ssid; s; s = s->next) {
    if (s->disabled == 2 && s->ssid_len == ssid_len &&
        os_memcmp(ssid, s->ssid, ssid_len) == 0)
      break;
  }
  ...
  if (status == P2P_SC_SUCCESS) {
    ...
    if (s) {
      ...
      if (go) {
        wpa_msg_global(wpa_s, MSG_INFO,
                       P2P_EVENT_INVITATION_ACCEPTED "sa=" MACSTR
                                                     " persistent=%d freq=%d",
      ...
      wpas_p2p_group_add_persistent(
          ...
          go ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE : 0, 1);
    } else if (bssid) {
      wpa_s->user_initiated_pd = 0;
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_INVITATION_ACCEPTED
                     "sa=" MACSTR " go_dev_addr=" MACSTR " bssid=" MACSTR
                     " unknown-network",
                     MAC2STR(sa), MAC2STR(go_dev_addr), MAC2STR(bssid));
      wpas_p2p_join(wpa_s, bssid, go_dev_addr, wpa_s->p2p_wps_method, 0,
  ...
  if (status != P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
    wpa_printf(MSG_DEBUG,
               "P2P: Invitation from peer " MACSTR " was rejected (status %u)",
               MAC2STR(sa), status);
    return;
  }
  ...
  if (!s) {
    if (bssid) {
      ...
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_INVITATION_RECEIVED
                     "sa=" MACSTR " go_dev_addr=" MACSTR " unknown-network",
                     MAC2STR(sa), MAC2STR(go_dev_addr));
    }
    wpas_notify_p2p_invitation_received(wpa_s, sa, go_dev_addr, bssid, 0,
                                        op_freq);
    return;
  }
  ...
  if (s->mode == WPAS_MODE_P2P_GO && op_freq) {
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR
                                                 " persistent=%d freq=%d",
                   MAC2STR(sa), s->id, op_freq);
  } else {
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR " persistent=%d",
                   MAC2STR(sa), s->id);
  }
  wpas_notify_p2p_invitation_received(wpa_s, sa, go_dev_addr, bssid, s->id,
                                      op_freq);
}

static void wpas_remove_persistent_peer(struct wpa_supplicant *wpa_s,
                                        struct wpa_ssid *ssid, const u8 *peer,
                                        int inv) {
  size_t i;
  struct wpa_supplicant *p2p_wpa_s = wpa_s->global->p2p_init_wpa_s;
  ...
  for (i = 0; ssid->p2p_client_list && i < ssid->num_p2p_clients; i++) {
    if (os_memcmp(ssid->p2p_client_list + i * 2 * ETH_ALEN, peer, ETH_ALEN) ==
        0)
      break;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "P2P: Remove peer " MACSTR " from persistent "
             "group %d client list%s",
             MAC2STR(peer), ssid->id, inv ? " due to invitation result" : "");
...
static void wpas_remove_persistent_client(struct wpa_supplicant *wpa_s,
  ...
  ssid =
      wpas_p2p_get_persistent(wpa_s->p2pdev, peer, ssid->ssid, ssid->ssid_len);
...
static void wpas_invitation_result(void *ctx, int status, const u8 *bssid,
  ...
  if (bssid) {
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_INVITATION_RESULT "status=%d " MACSTR, status,
                   MAC2STR(bssid));
  } else {
    wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RESULT "status=%d ",
                   status);
  }
  ...
  if (wpa_s->pending_invite_ssid_id == -1) {
    struct wpa_supplicant *group_if = wpa_s->global->p2p_invite_group;
     ...
     * more quickly.
     */
    if (status == P2P_SC_SUCCESS && group_if && group_if->current_ssid &&
        group_if->current_ssid->mode == WPAS_MODE_P2P_GO) {
      os_get_reltime(&wpa_s->global->p2p_go_wait_client);
#ifdef CONFIG_TESTING_OPTIONS
      if (group_if->p2p_go_csa_on_inv) {
        wpa_printf(MSG_DEBUG, "Testing: force P2P GO CSA after invitation");
        eloop_cancel_timeout(wpas_p2p_reconsider_moving_go, wpa_s, NULL);
        eloop_register_timeout(0, 50000, wpas_p2p_reconsider_moving_go, wpa_s,
                               NULL);
      }
#endif /* CONFIG_TESTING_OPTIONS */
    }
  ...
  if (status != P2P_SC_SUCCESS) {
    if (status == P2P_SC_FAIL_UNKNOWN_GROUP) {
      ssid = wpa_config_get_network(wpa_s->conf, wpa_s->pending_invite_ssid_id);
      wpas_remove_persistent_peer(wpa_s, ssid, peer, 1);
    }
    wpas_p2p_remove_pending_group_interface(wpa_s);
    return;
  ...
  wpa_printf(
      MSG_DEBUG,
      "P2P: Persistent group invitation success - op_freq=%d MHz SSID=%s", freq,
      wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
  wpas_p2p_group_add_persistent(
      wpa_s, ssid, ssid->mode == WPAS_MODE_P2P_GO,
      wpa_s->p2p_persistent_go_freq, freq, wpa_s->p2p_go_vht_center_freq2,
      wpa_s->p2p_go_ht40, wpa_s->p2p_go_vht, wpa_s->p2p_go_max_oper_chwidth,
      channels,
      ssid->mode == WPAS_MODE_P2P_GO ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE
                                     : 0,
      1);
}
...
static int has_channel(struct wpa_global *global, struct hostapd_hw_modes *mode,
                       u8 chan, int *flags) {
  int i;
  unsigned int freq;

  freq = (mode->mode == HOSTAPD_MODE_IEEE80211A ? 5000 : 2407) + chan * 5;
  ...
  for (i = 0; i < mode->num_channels; i++) {
    if (mode->channels[i].chan == chan) {
      ...
      if (mode->channels[i].flag & (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_RADAR))
        return NOT_ALLOWED;
...
static int wpas_p2p_get_center_80mhz(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < ARRAY_SIZE(center_channels); i++)
     ...
     */
    if (channel >= center_channels[i] - 6 && channel <= center_channels[i] + 6)
...
static int wpas_p2p_setup_channels(struct wpa_supplicant *wpa_s,
  ...
  for (op = 0; global_op_class[op].op_class; op++) {
    ...
    for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
      ...
      if (res == ALLOWED) {
        if (reg == NULL) {
          wpa_printf(MSG_DEBUG, "P2P: Add operating class %u", o->op_class);
        ...
        reg->channels++;
      } else if (res == NO_IR && wpa_s->conf->p2p_add_cli_chan) {
        if (cli_reg == NULL) {
    ...
    if (reg) {
      wpa_hexdump(MSG_DEBUG, "P2P: Channels", reg->channel, reg->channels);
    }
    if (cli_reg) {
      wpa_hexdump(MSG_DEBUG, "P2P: Channels (client only)", cli_reg->channel,
                  cli_reg->channels);
    }
  }
  ...
  for (op = 0; global_op_class[op].op_class; op++) {
    ...
    for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
      if (o->mode != HOSTAPD_MODE_IEEE80211A ||
          (o->bw != BW40PLUS && o->bw != BW40MINUS) || ch != channel)
        continue;
...
struct wpa_supplicant *wpas_get_p2p_go_iface(struct wpa_supplicant *wpa_s,
  ...
  for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    ...
    if (s == NULL)
      continue;
    if (s->mode != WPAS_MODE_P2P_GO && s->mode != WPAS_MODE_AP &&
        s->mode != WPAS_MODE_P2P_GROUP_FORMATION)
      continue;
    if (s->ssid_len != ssid_len || os_memcmp(ssid, s->ssid, ssid_len) != 0)
      continue;
    return wpa_s;
  }
...
int wpas_p2p_add_p2pdev_interface(struct wpa_supplicant *wpa_s,
  ...
  ret = wpa_drv_if_add(wpa_s, WPA_IF_P2P_DEVICE, ifname, NULL, NULL, force_name,
...
static void wpas_presence_resp(void *ctx, const u8 *src, u8 status,
  ...
  wpa_msg(wpa_s, MSG_INFO,
          P2P_EVENT_PRESENCE_RESPONSE "src=" MACSTR " status=%u noa=%s",
          MAC2STR(src), status, hex);
}
  ...
  if (s) {
    ...
    *ret_ssid_len = s->ssid_len;
    ...
    if (s->mode != WPAS_MODE_P2P_GO) {
      ...
      os_memcpy(intended_iface_addr, wpa_s->pending_interface_addr, ETH_ALEN);
    } else {
      os_memcpy(intended_iface_addr, wpa_s->own_addr, ETH_ALEN);
    }
    return 1;
  }
...
static int wpas_get_go_info(void *ctx, u8 *intended_addr, u8 *ssid,
                            size_t *ssid_len, int *group_iface,
                            unsigned int *freq) {
  struct wpa_supplicant *wpa_s = ctx;
...
static int wpas_remove_stale_groups(void *ctx, const u8 *peer, const u8 *go,
  ...
  while ((s = wpas_p2p_get_persistent(wpa_s, peer, NULL, 0))) {
    if (go && ssid && ssid_len && s->ssid_len == ssid_len &&
    ...
    for (i = 0; i < s->num_p2p_clients; i++) {
      if (os_memcmp(s->p2p_client_list + i * 2 * ETH_ALEN, peer, ETH_ALEN) != 0)
        continue;
...
static void wpas_p2ps_get_feat_cap_str(char *buf, size_t buf_len,
  ...
  ret = wpa_snprintf_hex(&buf[sizeof(pref) - 1], buf_len - sizeof(pref) + 1,
...
static void wpas_p2ps_prov_complete(
    void *ctx, u8 status, const u8 *dev, const u8 *adv_mac, const u8 *ses_mac,
    const u8 *grp_mac, u32 adv_id, u32 ses_id, u8 conncap, int passwd_id,
    const u8 *persist_ssid, size_t persist_ssid_size, int response_done,
    int prov_start, const char *session_info, const u8 *feat_cap,
    size_t feat_cap_len, unsigned int freq, const u8 *group_ssid,
  ...
  wpas_p2ps_get_feat_cap_str(feat_cap_str, sizeof(feat_cap_str), feat_cap,
                             feat_cap_len);
  ...
  if (prov_start) {
    if (session_info == NULL) {
      wpa_msg_global(wpa_s, MSG_INFO,
                     ...
                     " adv_id=%x conncap=%x"
                     " adv_mac=" MACSTR " session=%x mac=" MACSTR
                     " dev_passwd_id=%d%s",
                     MAC2STR(dev), adv_id, conncap, MAC2STR(adv_mac), ses_id,
                     MAC2STR(ses_mac), passwd_id, feat_cap_str);
    } else {
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_P2PS_PROVISION_START MACSTR
                     " adv_id=%x conncap=%x"
                     ...
                     MAC2STR(dev), adv_id, conncap, MAC2STR(adv_mac), ses_id,
                     MAC2STR(ses_mac), passwd_id, session_info, feat_cap_str);
    }
    return;
  }
  ...
  if (status && status != P2P_SC_SUCCESS_DEFERRED) {
    ...
    wpa_msg_global(wpa_s, MSG_INFO,
                   ...
                   " status=%d"
                   " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR "%s",
                   MAC2STR(dev), status, adv_id, MAC2STR(adv_mac), ses_id,
                   MAC2STR(ses_mac), feat_cap_str);
    return;
  }

  /* Clean up stale persistent groups with this device */
  if (persist_ssid && persist_ssid_size)
    s = wpas_p2p_get_persistent(wpa_s, dev, persist_ssid, persist_ssid_size);

  if (persist_ssid && s && s->mode != WPAS_MODE_P2P_GO &&
      is_zero_ether_addr(grp_mac)) {
    wpa_dbg(wpa_s, MSG_ERROR,
            "P2P: Peer device is a GO in a persistent group, but it did not "
            "provide the intended MAC address");
    return;
  }
  ...
  for (;;) {
    ...
    /* Remove stale persistent group */
    if (stale->mode != WPAS_MODE_P2P_GO || stale->num_p2p_clients <= 1) {
      wpa_config_remove_network(wpa_s->conf, stale->id);
    } else {
      size_t i;

      for (i = 0; i < stale->num_p2p_clients; i++) {
        if (os_memcmp(stale->p2p_client_list + i * ETH_ALEN, dev, ETH_ALEN) ==
            0) {
          os_memmove(stale->p2p_client_list + i * ETH_ALEN,
                     stale->p2p_client_list + (i + 1) * ETH_ALEN,
                     (stale->num_p2p_clients - i - 1) * ETH_ALEN);
          break;
        }
      }
  ...
  if (s) {
    ...
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                   " status=%d"
                   " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR
                   " persist=%d%s",
                   MAC2STR(dev), status, adv_id, MAC2STR(adv_mac), ses_id,
                   MAC2STR(ses_mac), s->id, feat_cap_str);
    return;
  }
  ...
  if (conncap == P2PS_SETUP_GROUP_OWNER) {
    ...
    if (!go_wpa_s) {
      ...
      if (!wpas_p2p_create_iface(wpa_s))
        os_memcpy(go_ifname, wpa_s->ifname, sizeof(go_ifname));
      else if (wpa_s->pending_interface_name[0])
        os_memcpy(go_ifname, wpa_s->pending_interface_name, sizeof(go_ifname));

      if (!go_ifname[0]) {
        wpas_p2ps_prov_complete(wpa_s, P2P_SC_FAIL_UNKNOWN_GROUP, dev, adv_mac,
                                ses_mac, grp_mac, adv_id, ses_id, 0, 0, NULL, 0,
      ...
      if (response_done && persistent_go) {
        wpas_p2p_group_add_persistent(
            wpa_s, persistent_go, 0, 0, freq, 0, 0, 0, 0, NULL,
      ...
      os_memcpy(go_ifname, go_wpa_s->ifname, sizeof(go_ifname));

      if (is_zero_ether_addr(grp_mac)) {
        wpa_dbg(go_wpa_s, MSG_DEBUG, "P2P: Setting PIN-1 for ANY");
        wpa_supplicant_ap_wps_pin(go_wpa_s, NULL, "12345670", NULL, 0, 0);
      } else {
        wpa_dbg(go_wpa_s, MSG_DEBUG, "P2P: Setting PIN-1 for " MACSTR,
                MAC2STR(grp_mac));
        wpa_supplicant_ap_wps_pin(go_wpa_s, grp_mac, "12345670", NULL, 0, 0);
      }
    ...
    wpa_msg_global(wpa_s, MSG_INFO,
                   ...
                   MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac),
                   ses_id, MAC2STR(ses_mac), passwd_id, go_ifname,
                   feat_cap_str);
    return;
  }
  ...
  if (conncap == P2PS_SETUP_CLIENT) {
    ...
    if (group_ssid)
      wpa_snprintf_hex(ssid_hex, sizeof(ssid_hex), group_ssid, group_ssid_len);
    else
      ssid_hex[0] = '\0';
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                   ...
                   MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac),
                   ses_id, MAC2STR(ses_mac), passwd_id, MAC2STR(grp_mac),
                   feat_cap_str, group_ssid ? " group_ssid=" : "", ssid_hex);
  } else {
    wpa_msg_global(wpa_s, MSG_INFO,
                   P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                   " status=%d conncap=%x"
                   ...
                   " dev_passwd_id=%d%s",
                   MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac),
                   ses_id, MAC2STR(ses_mac), passwd_id, feat_cap_str);
  }
}
...
static int wpas_prov_disc_resp_cb(void *ctx) {
  ...
  if (persistent_go) {
    wpas_p2p_group_add_persistent(wpa_s, persistent_go, 0, 0, 0, 0, 0, 0, 0,
                                  NULL,
                                  persistent_go->mode == WPAS_MODE_P2P_GO
                                      ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE
                                      : 0,
...
static int wpas_p2p_get_pref_freq_list(void *ctx, int go, unsigned int *len,
                                       unsigned int *freq_list) {
  struct wpa_supplicant *wpa_s = ctx;

  return wpa_drv_get_pref_freq_list(
      wpa_s, go ? WPA_IF_P2P_GO : WPA_IF_P2P_CLIENT, len, freq_list);
}
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s) {
  ...
  if (wpas_p2p_setup_channels(wpa_s, &p2p.channels, &p2p.cli_channels)) {
    wpa_printf(MSG_ERROR, "P2P: Failed to configure supported channel list");
    return -1;
  }

  if (wpa_s->conf->p2p_listen_reg_class && wpa_s->conf->p2p_listen_channel) {
     ...
     * channel.
     */
    if (p2p_config_get_random_social(&p2p, &p2p.reg_class, &p2p.channel) != 0) {
      wpa_printf(MSG_INFO, "P2P: No social channels supported by the driver - "
                           "do not enable P2P");
      return 0;
    }
    p2p.channel_forced = 0;
  }
  wpa_printf(MSG_DEBUG, "P2P: Own listen channel: %d:%d", p2p.reg_class,
             p2p.channel);
  ...
  p2p.concurrent_operations =
      !!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_CONCURRENT);

  p2p.max_peers = 100;

  if (wpa_s->conf->p2p_ssid_postfix) {
    p2p.ssid_postfix_len = os_strlen(wpa_s->conf->p2p_ssid_postfix);
...
void wpas_p2p_deinit(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->go_params) {
    ...
    p2p_clear_provisioning_info(wpa_s->global->p2p,
...
static int wpas_p2p_start_go_neg(struct wpa_supplicant *wpa_s,
   ...
   */
  p2p_set_config_timeout(wpa_s->global->p2p, wpa_s->p2p_go_ht40 ? 255 : 100,
                         20);

  return p2p_connect(wpa_s->global->p2p, peer_addr, wps_method, go_intent,
                     own_interface_addr, force_freq, persistent_group,
                     ssid ? ssid->ssid : NULL, ssid ? ssid->ssid_len : 0,
                     wpa_s->p2p_pd_before_go_neg, pref_freq,
                     wps_method == WPS_NFC ? wpa_s->p2p_oob_dev_pw_id : 0);
}
...
static int wpas_p2p_auth_go_neg(struct wpa_supplicant *wpa_s,
  ...
  return p2p_authorize(wpa_s->global->p2p, peer_addr, wps_method, go_intent,
                       own_interface_addr, force_freq, persistent_group,
                       ssid ? ssid->ssid : NULL, ssid ? ssid->ssid_len : 0,
                       pref_freq,
                       wps_method == WPS_NFC ? wpa_s->p2p_oob_dev_pw_id : 0);
...
static int wpas_check_freq_conflict(struct wpa_supplicant *wpa_s, int freq) {
  ...
  num =
      wpas_p2p_valid_oper_freqs(wpa_s, freqs, wpa_s->num_multichan_concurrent);

  for (i = 0; i < num; i++) {
    if (freqs[i].freq == freq) {
      wpa_printf(MSG_DEBUG,
                 "P2P: Frequency %d MHz in use by another virtual interface "
                 "and can be used",
                 freq);
      res = 0;
      goto exit_free;
...
static void wpas_p2p_scan_res_join(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->p2p_auto_pd) {
    int join = wpas_p2p_peer_go(wpa_s, wpa_s->pending_join_dev_addr);
    if (join == 0 && wpa_s->auto_pd_scan_retry < P2P_AUTO_PD_SCAN_ATTEMPTS) {
      wpa_s->auto_pd_scan_retry++;
      bss = wpa_bss_get_bssid_latest(wpa_s, wpa_s->pending_join_dev_addr);
      if (bss) {
        freq = bss->freq;
        wpa_printf(MSG_DEBUG,
    ...
    if (p2p_prov_disc_req(wpa_s->global->p2p, wpa_s->pending_join_dev_addr,
                          NULL, wpa_s->pending_pd_config_methods, join, 0,
                          wpa_s->user_initiated_pd) < 0) {
      wpa_s->p2p_auto_pd = 0;
      wpa_msg_global(wpa_s, MSG_INFO,
                     P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR
  ...
  if (wpa_s->p2p_auto_join) {
    ...
    if (join < 0) {
      ...
      wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                     P2P_EVENT_FALLBACK_TO_GO_NEG "reason=peer-not-running-GO");
      wpas_p2p_connect(wpa_s, wpa_s->pending_join_dev_addr, wpa_s->p2p_pin,
                       wpa_s->p2p_wps_method, wpa_s->p2p_persistent_group, 0, 0,
                       0, wpa_s->p2p_go_intent, wpa_s->p2p_connect_freq,
                       wpa_s->p2p_go_vht_center_freq2, wpa_s->p2p_persistent_id,
                       wpa_s->p2p_pd_before_go_neg, wpa_s->p2p_go_ht40,
                       wpa_s->p2p_go_vht, wpa_s->p2p_go_max_oper_chwidth, NULL,
                       0);
      return;
    }
  ...
  freq = p2p_get_oper_freq(wpa_s->global->p2p, wpa_s->pending_join_iface_addr);
  if (freq < 0 &&
      p2p_get_interface_addr(wpa_s->global->p2p, wpa_s->pending_join_dev_addr,
                             iface_addr) == 0 &&
    ...
    wpa_printf(MSG_DEBUG,
               ...
               MAC2STR(wpa_s->pending_join_iface_addr), MAC2STR(iface_addr));
    os_memcpy(wpa_s->pending_join_iface_addr, iface_addr, ETH_ALEN);

    freq =
        p2p_get_oper_freq(wpa_s->global->p2p, wpa_s->pending_join_iface_addr);
  }
  if (freq >= 0) {
    wpa_printf(MSG_DEBUG,
  ...
  if (wpa_s->p2p_join_ssid_len) {
    wpa_printf(MSG_DEBUG,
               "P2P: Trying to find target GO BSS entry based on BSSID " MACSTR
               " and SSID %s",
               MAC2STR(wpa_s->pending_join_iface_addr),
               wpa_ssid_txt(wpa_s->p2p_join_ssid, wpa_s->p2p_join_ssid_len));
    bss = wpa_bss_get(wpa_s, wpa_s->pending_join_iface_addr,
                      wpa_s->p2p_join_ssid, wpa_s->p2p_join_ssid_len);
  } else if (!bss) {
    wpa_printf(MSG_DEBUG,
               "P2P: Trying to find target GO BSS entry based on BSSID " MACSTR,
               MAC2STR(wpa_s->pending_join_iface_addr));
  ...
  if (bss) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               freq, wpa_ssid_txt(bss->ssid, bss->ssid_len));
    if (p2p_parse_dev_addr((const u8 *)(bss + 1), bss->ie_len, dev_addr) == 0 &&
        os_memcmp(wpa_s->pending_join_dev_addr, wpa_s->pending_join_iface_addr,
                  ETH_ALEN) == 0 &&
        os_memcmp(dev_addr, wpa_s->pending_join_dev_addr, ETH_ALEN) != 0) {
      wpa_printf(
          MSG_DEBUG,
          "P2P: Update target GO device address based on BSS entry: " MACSTR
          " (was " MACSTR ")",
          MAC2STR(dev_addr), MAC2STR(wpa_s->pending_join_dev_addr));
  ...
  if (freq > 0) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "prior to joining an existing group (GO " MACSTR " freq=%u MHz)",
               MAC2STR(wpa_s->pending_join_dev_addr), freq);
...
start:
  ...
  wpas_p2p_join_start(wpa_s, 0, wpa_s->p2p_join_ssid, wpa_s->p2p_join_ssid_len);
}
...
static void wpas_p2p_join_scan_req(struct wpa_supplicant *wpa_s, int freq,
  ...
  wps_ie = wps_build_probe_req_ie(DEV_PW_DEFAULT, &wpa_s->wps->dev,
                                  wpa_s->wps->uuid, WPS_REQ_ENROLLEE, 0, NULL);
  if (wps_ie == NULL) {
...
static int wpas_p2p_join(struct wpa_supplicant *wpa_s, const u8 *iface_addr,
                         const u8 *dev_addr, enum p2p_wps_method wps_method,
                         int auto_join, int op_freq, const u8 *ssid,
                         size_t ssid_len) {
  wpa_printf(MSG_DEBUG,
             "P2P: Request to join existing group (iface " MACSTR " dev " MACSTR
             " op_freq=%d)%s",
...
static int wpas_p2p_join_start(struct wpa_supplicant *wpa_s, int freq,
  ...
  if (group == NULL)
    return -1;
  if (group != wpa_s) {
    os_memcpy(group->p2p_pin, wpa_s->p2p_pin, sizeof(group->p2p_pin));
  ...
  os_memcpy(res.peer_interface_addr, wpa_s->pending_join_iface_addr, ETH_ALEN);
  ...
  if (freq && ssid && ssid_len) {
    ...
    if (ssid && ssid_len) {
      bss = wpa_bss_get(wpa_s, wpa_s->pending_join_iface_addr, ssid, ssid_len);
    } else {
      bss = wpa_bss_get_bssid_latest(wpa_s, wpa_s->pending_join_iface_addr);
    }
    if (bss) {
      ...
      wpa_printf(MSG_DEBUG,
                 "P2P: Join target GO operating frequency from BSS table: %d "
                 "MHz (SSID %s)",
                 bss->freq, wpa_ssid_txt(bss->ssid, bss->ssid_len));
   ...
   */
  eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s, NULL);
  eloop_register_timeout(60, 0, wpas_p2p_group_formation_timeout, wpa_s, NULL);
...
static int wpas_p2p_setup_freqs(struct wpa_supplicant *wpa_s, int freq,
  ...
  *num_pref_freq = 0;
  ...
  if (freq > 0) {
    ...
    if (!ret) {
      if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
         ...
         */
        wpa_printf(MSG_DEBUG,
                   "P2P: The forced channel for GO (%u MHz) is DFS, and DFS is "
    ...
    if (num_unused <= 0 && !freq_in_use) {
      wpa_printf(MSG_DEBUG,
                 "P2P: Cannot start P2P group on %u MHz as there are no "
                 "available channels",
                 freq);
      res = -2;
      goto exit_free;
  ...
  if (!wpa_s->conf->num_p2p_pref_chan && *pref_freq == 0) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: best_freq=%d, go=%d", best_freq, go);
    ...
    if (!res && max_pref_freq > 0) {
      *num_pref_freq = max_pref_freq;
      i = 0;
      while (wpas_p2p_disallowed_freq(wpa_s->global, pref_freq_list[i]) &&
             i < *num_pref_freq) {
        wpa_printf(MSG_DEBUG, "P2P: preferred_freq_list[%d]=%d is disallowed",
                   i, pref_freq_list[i]);
        i++;
      }
      if (i != *num_pref_freq) {
        best_freq = pref_freq_list[i];
        wpa_printf(MSG_DEBUG, "P2P: Using preferred_freq_list[%d]=%d", i,
                   best_freq);
      } else {
        wpa_printf(
            MSG_DEBUG,
      ...
      wpa_printf(MSG_DEBUG, "P2P: No preferred frequency list available");
    }
  }
  ...
  if (best_freq > 0) {
    if (*pref_freq == 0 && num_unused > 0) {
      ...
      *pref_freq = best_freq;
      ...
      wpa_printf(MSG_DEBUG,
                 "P2P: Try to force us to use frequency (%u MHz) which is "
                 "already in use",
                 best_freq);
      *force_freq = best_freq;
    }
  } else if (num_unused > 0) {
    wpa_printf(MSG_DEBUG, "P2P: Current operating channels are not available "
                          "for P2P. Try to use another channel");
    *force_freq = 0;
  } else {
    wpa_printf(MSG_DEBUG, "P2P: All channels are in use and none of them are "
                          "P2P enabled. Cannot start P2P group");
    res = -2;
    goto exit_free;
  }
...
int wpas_p2p_connect(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
  ...
  if (pin)
    ...
    if (wps_generate_pin((unsigned int *)&ret) < 0)
      return -1;
    res = os_snprintf(wpa_s->p2p_pin, sizeof(wpa_s->p2p_pin), "%08d", ret);
    if (os_snprintf_error(sizeof(wpa_s->p2p_pin), res))
      wpa_s->p2p_pin[sizeof(wpa_s->p2p_pin) - 1] = '\0';
    wpa_printf(MSG_DEBUG, "P2P: Randomly generated PIN: %s", wpa_s->p2p_pin);
  ...
  if (join || auto_join) {
    ...
    if (p2p_get_interface_addr(wpa_s->global->p2p, peer_addr, iface_addr) < 0) {
      os_memcpy(iface_addr, peer_addr, ETH_ALEN);
      p2p_get_dev_addr(wpa_s->global->p2p, peer_addr, dev_addr);
    }
    if (auto_join) {
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 "%ld.%06ld",
                 wpa_s->p2p_auto_started.sec, wpa_s->p2p_auto_started.usec);
    }
    wpa_s->user_initiated_pd = 1;
    if (wpas_p2p_join(wpa_s, iface_addr, dev_addr, wps_method, auto_join, freq,
  ...
  if (res)
    return res;
  wpas_p2p_set_own_freq_preference(wpa_s, force_freq ? force_freq : pref_freq);
  ...
  if (auth) {
    if (wpas_p2p_auth_go_neg(wpa_s, peer_addr, wps_method, go_intent, if_addr,
                             force_freq, persistent_group, ssid, pref_freq) < 0)
      return -1;
    return ret;
  }

  if (wpas_p2p_start_go_neg(wpa_s, peer_addr, wps_method, go_intent, if_addr,
...
void wpas_p2p_remain_on_channel_cb(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(
      MSG_DEBUG,
      "P2P: remain-on-channel callback (off_channel_freq=%u "
      "pending_listen_freq=%d roc_waiting_drv_freq=%d freq=%u duration=%u)",
  ...
  if (wpa_s->off_channel_freq &&
    ...
    wpa_printf(MSG_DEBUG,
               "P2P: Ignore remain-on-channel callback (off_channel_freq=%u "
               "pending_listen_freq=%d freq=%u duration=%u)",
               wpa_s->off_channel_freq, wpa_s->pending_listen_freq, freq,
               duration);
  }
}
...
int wpas_p2p_group_remove(struct wpa_supplicant *wpa_s, const char *ifname) {
  ...
  if (os_strcmp(ifname, "*") == 0) {
    ...
    while (wpa_s) {
      prev = wpa_s;
      wpa_s = wpa_s->next;
      if (prev->p2p_group_interface != NOT_P2P_GROUP_INTERFACE ||
          (prev->current_ssid && prev->current_ssid->p2p_group))
...
static int wpas_p2p_select_go_freq(struct wpa_supplicant *wpa_s, int freq) {
  ...
  if (!wpa_s->conf->num_p2p_pref_chan && !freq) {
    ...
    res =
        wpa_drv_get_pref_freq_list(wpa_s, WPA_IF_P2P_GO, &size, pref_freq_list);
    if (!res && size > 0) {
      i = 0;
      while (wpas_p2p_disallowed_freq(wpa_s->global, pref_freq_list[i]) &&
             i < size) {
      ...
      if (i != size) {
        ...
        wpa_printf(MSG_DEBUG, "P2P: Using preferred_freq_list[%d]=%d", i, freq);
      } else {
        wpa_printf(
            MSG_DEBUG,
            "P2P: All driver preferred frequencies are disallowed for P2P use");
  ...
  if (freq == 2) {
    ...
    if (wpa_s->best_24_freq > 0 &&
        p2p_supported_freq_go(wpa_s->global->p2p, wpa_s->best_24_freq)) {
  ...
  if (freq == 5) {
    ...
    if (wpa_s->best_5_freq > 0 &&
        p2p_supported_freq_go(wpa_s->global->p2p, wpa_s->best_5_freq)) {
...
static void
wpas_p2p_select_go_freq_no_pref(struct wpa_supplicant *wpa_s,
                                struct p2p_go_neg_results *params,
                                const struct p2p_channels *channels) {
  unsigned int i, r;
  ...
  for (i = 0; i < 11; i++) {
    ...
    if (params->freq == 2412 || params->freq == 2437 || params->freq == 2462)
      continue;
...
static int wpas_p2p_init_go_params(struct wpa_supplicant *wpa_s,
                                   struct p2p_go_neg_results *params, int freq,
  ...
  if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_P2P_GO &&
      wpa_s->wpa_state == WPA_COMPLETED) {
    wpa_printf(MSG_DEBUG, "P2P: %s called for an active GO", __func__);
  ...
  if (freq) {
    if (!wpas_p2p_supported_freq_go(wpa_s, channels, freq)) {
      wpa_printf(MSG_DEBUG, "P2P: Forced GO freq %d MHz not accepted", freq);
      goto fail;
    }

    for (i = 0; i < num; i++) {
      if (freqs[i].freq == freq) {
        wpa_printf(MSG_DEBUG, "P2P: forced freq (%d MHz) is also shared", freq);
        params->freq = freq;
        goto success;
      }
    }
    ...
    if (!ignore_no_freqs && !unused_channels) {
      wpa_printf(MSG_DEBUG,
                 "P2P: Cannot force GO on freq (%d MHz) as all the channels "
                 "are in use",
                 freq);
      goto fail;
    }

    wpa_printf(MSG_DEBUG, "P2P: force GO freq (%d MHz) on a free channel",
               freq);
    params->freq = freq;
    goto success;
  }
  ...
  if (num && (!wpa_s->conf->p2p_ignore_shared_freq || !unused_channels)) {
    cand = wpas_p2p_pick_best_used_freq(wpa_s, freqs, num);
    if (wpas_p2p_supported_freq_go(wpa_s, channels, cand)) {
      wpa_printf(MSG_DEBUG, "P2P: Use shared freq (%d MHz) for GO", cand);
      params->freq = cand;
      goto success;
    }
    ...
    for (i = 0; i < num; i++) {
      if (wpas_p2p_supported_freq_go(wpa_s, channels, freqs[i].freq)) {
        wpa_printf(MSG_DEBUG, "P2P: Use shared freq (%d MHz) for GO",
                   freqs[i].freq);
  ...
  if (wpa_s->conf->p2p_oper_channel == 0 && wpa_s->best_overall_freq > 0 &&
      wpas_p2p_supported_freq_go(wpa_s, channels, wpa_s->best_overall_freq)) {
  ...
  if (wpa_s->conf->p2p_oper_channel == 0 && wpa_s->best_24_freq > 0 &&
      wpas_p2p_supported_freq_go(wpa_s, channels, wpa_s->best_24_freq)) {
  ...
  if (wpa_s->conf->p2p_oper_channel == 0 && wpa_s->best_5_freq > 0 &&
      wpas_p2p_supported_freq_go(wpa_s, channels, wpa_s->best_5_freq)) {
  ...
  if (wpa_s->p2p_group_common_freqs) {
    for (i = 0; i < wpa_s->p2p_group_common_freqs_num; i++) {
      ...
      if (wpas_p2p_supported_freq_go(wpa_s, channels, cand)) {
        params->freq = cand;
        wpa_printf(MSG_DEBUG, "P2P: Use freq %d MHz common with the peer",
                   params->freq);
        goto success;
      }
    }
...
wpas_p2p_get_group_iface(struct wpa_supplicant *wpa_s, int addr_allocated,
  ...
  if (wpas_p2p_add_group_interface(wpa_s, go ? WPA_IF_P2P_GO
                                             : WPA_IF_P2P_CLIENT) < 0) {
    wpa_msg_global(wpa_s, MSG_ERROR, "P2P: Failed to add group interface");
    return NULL;
  }
...
int wpas_p2p_group_add(struct wpa_supplicant *wpa_s, int persistent_group,
  ...
  if (wpas_p2p_init_go_params(wpa_s, &params, freq, vht_center_freq2, ht40, vht,
                              max_oper_chwidth, NULL))
    return -1;
  if (params.freq && !p2p_supported_freq_go(wpa_s->global->p2p, params.freq)) {
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
       ...
       */
      wpa_printf(MSG_DEBUG,
                 "P2P: %s: The forced channel for GO (%u MHz) is DFS, and DFS "
                 "is offloaded to driver",
                 __func__, params.freq);
    } else {
      wpa_printf(MSG_DEBUG,
                 "P2P: The selected channel for GO (%u MHz) is not supported "
                 "for P2P uses",
                 params.freq);
      return -1;
    }
...
static int wpas_start_p2p_client(struct wpa_supplicant *wpa_s,
  ...
  ssid->pairwise_cipher = params->pbss ? WPA_CIPHER_GCMP : WPA_CIPHER_CCMP;
  ...
  eloop_register_timeout(P2P_MAX_INITIAL_CONN_WAIT, 0,
                         wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
...
int wpas_p2p_group_add_persistent(struct wpa_supplicant *wpa_s,
  ...
  if (wpas_get_p2p_group(wpa_s, ssid->ssid, ssid->ssid_len, &go) &&
    ...
    wpa_printf(MSG_DEBUG, "P2P: Requested persistent group is "
                          "already running");
    if (go == 0 && eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
                                        wpa_s->p2pdev, NULL)) {
      /*
       ...
       */
      wpa_printf(MSG_DEBUG, "P2P: Reschedule group formation timeout since "
                            "peer is still trying to invite us");
      eloop_register_timeout(P2P_MAX_INITIAL_CONN_WAIT, 0,
                             wpas_p2p_group_formation_timeout, wpa_s->p2pdev,
                             NULL);
    }
  ...
  if (ssid->mode == WPAS_MODE_P2P_GO) {
    if (force_freq > 0) {
      ...
      if (freq < 0 || (freq > 0 && !freq_included(wpa_s, channels, freq)))
        freq = 0;
    }
    ...
    if (freq <= 0 || !freq_included(wpa_s, channels, freq)) {
      ...
      struct wpa_bss *bss = wpa_bss_get_p2p_dev_addr(wpa_s, ssid->bssid);

      os_get_reltime(&now);
      if (bss && !os_reltime_expired(&now, &bss->last_update, 5) &&
    ...
    return wpas_start_p2p_client(wpa_s, ssid, addr_allocated, freq, force_scan);
  } else {
    return -1;
  }

  if (wpas_p2p_init_go_params(wpa_s, &params, freq, vht_center_freq2, ht40, vht,
                              max_oper_chwidth, channels))
    return -1;
  ...
  if (ssid->passphrase) {
    ...
    os_strlcpy(params.passphrase, ssid->passphrase, sizeof(params.passphrase));
  }
...
static void wpas_p2p_idle_update(void *ctx, int idle) {
  ...
  if (idle) {
    if (wpa_s->global->p2p_fail_on_wps_complete && wpa_s->p2p_in_provisioning) {
...
struct p2p_group *wpas_p2p_group_init(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->max_stations && wpa_s->max_stations < wpa_s->conf->max_num_sta)
...
void wpas_p2p_wps_success(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
  ...
  if (ssid && ssid->mode == WPAS_MODE_INFRA) {
    ...
    wpas_p2p_persistent_group(wpa_s, go_dev_addr, ssid->ssid, ssid->ssid_len);
    /* Clear any stored provisioning info */
  ...
  if (ssid && ssid->mode == WPAS_MODE_INFRA) {
     ...
     */
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Re-start group formation timeout (%d seconds) as client for "
            "initial connection",
     ...
     */
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Re-start group formation timeout (%d seconds) as GO for "
            "initial connection",
...
void wpas_p2p_wps_failed(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s == wpa_s->global->p2p_group_formation) {
     ...
     */
    wpa_printf(MSG_DEBUG, "P2P: WPS step failed during group formation - "
                          "reject connection from timeout");
    wpa_s->global->p2p_fail_on_wps_complete = 1;
    eloop_deplete_timeout(0, 50000, wpas_p2p_group_formation_timeout,
                          wpa_s->p2pdev, NULL);
  }
}
...
int wpas_p2p_prov_disc(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
  ...
  if (p2ps_prov && use == WPAS_P2P_PD_FOR_ASP) {
    p2ps_prov->conncap =
        p2ps_group_capability(wpa_s, P2PS_SETUP_NONE, p2ps_prov->role,
                              &p2ps_prov->force_freq, &p2ps_prov->pref_freq);

    wpa_printf(MSG_DEBUG, "P2P: %s conncap: %d - ASP parsed: %x %x %d %s",
               __func__, p2ps_prov->conncap, p2ps_prov->adv_id,
               p2ps_prov->conncap, p2ps_prov->status, p2ps_prov->info);
  ...
  return p2p_prov_disc_req(wpa_s->global->p2p, peer_addr, p2ps_prov,
                           config_methods, use == WPAS_P2P_PD_FOR_JOIN, 0, 1);
}
...
int wpas_p2p_find(struct wpa_supplicant *wpa_s, unsigned int timeout,
                  ...
                  unsigned int search_delay, u8 seek_cnt,
                  const char **seek_string, int freq) {
  ...
  return p2p_find(wpa_s->global->p2p, timeout, type, num_req_dev_types,
                  req_dev_types, dev_id, search_delay, seek_cnt, seek_string,
                  freq);
}
...
int wpas_p2p_listen(struct wpa_supplicant *wpa_s, unsigned int timeout) {
  ...
  if (wpa_s->p2p_lo_started) {
    wpa_printf(MSG_DEBUG, "P2P: Cannot start P2P listen, it is offloaded");
    return -1;
  }
  ...
  if (res == 0 && timeout * 1000 > wpa_s->max_remain_on_chan) {
    ...
    eloop_register_timeout(timeout, 0, wpas_p2p_long_listen_timeout, wpa_s,
                           NULL);
  }
...
int wpas_p2p_assoc_req_ie(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  ret = p2p_assoc_req_ie(wpa_s->global->p2p, bss->bssid, buf, len, p2p_group,
                         p2p_ie);
  wpabuf_free(p2p_ie);
...
int wpas_p2p_probe_req_rx(struct wpa_supplicant *wpa_s, const u8 *addr,
                          ...
                          size_t ie_len, unsigned int rx_freq, int ssi_signal) {
  if (wpa_s->global->p2p_disabled)
    return 0;
  if (wpa_s->global->p2p == NULL)
    return 0;

  switch (p2p_probe_req_rx(wpa_s->global->p2p, addr, dst, bssid, ie, ie_len,
                           rx_freq, wpa_s->p2p_lo_started)) {
  case P2P_PREQ_NOT_P2P:
    wpas_notify_preq(wpa_s, addr, dst, bssid, ie, ie_len, ssi_signal);
    /* fall through */
...
void wpas_p2p_rx_action(struct wpa_supplicant *wpa_s, const u8 *da,
                        const u8 *sa, const u8 *bssid, u8 category,
  ...
  p2p_rx_action(wpa_s->global->p2p, da, sa, bssid, category, data, len, freq);
}
...
int wpas_p2p_invite(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
  ...
  size = P2P_MAX_PREF_CHANNELS;
  res = wpas_p2p_setup_freqs(wpa_s, freq, &force_freq, &pref_freq,
                             role == P2P_INVITE_ROLE_GO, pref_freq_list, &size);
  if (res)
    return res;
  ...
  if (wpa_s->parent->conf->p2p_ignore_shared_freq && no_pref_freq_given &&
      pref_freq > 0 && wpa_s->num_multichan_concurrent > 1 &&
      wpas_p2p_num_unused_channels(wpa_s) > 0) {
    wpa_printf(MSG_DEBUG,
               "P2P: Ignore own channel preference %d MHz for invitation due "
               "to p2p_ignore_shared_freq=1 configuration",
               pref_freq);
    pref_freq = 0;
  }
  ...
  return p2p_invite(wpa_s->global->p2p, peer_addr, role, bssid, ssid->ssid,
                    ssid->ssid_len, force_freq, go_dev_addr, 1, pref_freq, -1);
}
...
int wpas_p2p_invite_group(struct wpa_supplicant *wpa_s, const char *ifname,
  ...
  persistent = ssid->p2p_persistent_group &&
               wpas_p2p_get_persistent(wpa_s->p2pdev, peer_addr, ssid->ssid,
                                       ssid->ssid_len);
  ...
  if (ssid->mode == WPAS_MODE_P2P_GO) {
    ...
    bssid = wpa_s->bssid;
    if (go_dev_addr == NULL && !is_zero_ether_addr(wpa_s->go_dev_addr))
      go_dev_addr = wpa_s->go_dev_addr;
    freq =
        wpa_s->current_bss ? wpa_s->current_bss->freq : (int)wpa_s->assoc_freq;
  }
  ...
  size = P2P_MAX_PREF_CHANNELS;
  res = wpas_p2p_setup_freqs(wpa_s, freq, &force_freq, &pref_freq,
                             role == P2P_INVITE_ROLE_ACTIVE_GO, pref_freq_list,
                             &size);
  if (res)
    return res;
  ...
  return p2p_invite(wpa_s->global->p2p, peer_addr, role, bssid, ssid->ssid,
                    ssid->ssid_len, force_freq, go_dev_addr, persistent,
                    pref_freq, -1);
}
...
void wpas_p2p_completed(struct wpa_supplicant *wpa_s) {
  ...
  freq = wpa_s->current_bss ? wpa_s->current_bss->freq : (int)wpa_s->assoc_freq;
  ...
  wpas_p2p_group_started(wpa_s, 0, ssid, freq,
                         ssid->passphrase == NULL && ssid->psk_set ? ssid->psk
                                                                   : NULL,
                         ssid->passphrase, go_dev_addr, persistent, ip_addr);
...
int wpas_p2p_presence_req(struct wpa_supplicant *wpa_s, u32 duration1,
  ...
  ret = p2p_presence_req(wpa_s->global->p2p, wpa_s->bssid, wpa_s->own_addr,
                         wpa_s->assoc_freq, duration1, interval1, duration2,
                         interval2);
  if (ret == 0)
...
static void wpas_p2p_set_group_idle_timeout(struct wpa_supplicant *wpa_s) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: Set P2P group idle timeout to %u seconds",
             timeout);
  eloop_register_timeout(timeout, 0, wpas_p2p_group_idle_timeout, wpa_s, NULL);
}
...
int wpas_p2p_deauth_notif(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (reason_code == WLAN_REASON_DEAUTH_LEAVING && !locally_generated &&
    ...
    wpa_printf(MSG_DEBUG, "P2P: GO indicated that the P2P Group "
                          "session is ending");
    if (wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_GO_ENDING_SESSION) > 0)
      return 1;
  }
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->conf->changed_parameters & CFG_CHANGED_SEC_DEVICE_TYPE)
    p2p_set_sec_dev_types(p2p, (void *)wpa_s->conf->sec_device_type,
  ...
  if (wpa_s->conf->changed_parameters & CFG_CHANGED_VENDOR_EXTENSION) {
    ...
    for (i = 0; i < MAX_WPS_VENDOR_EXT; i++) {
      if (wpa_s->conf->wps_vendor_ext[i] == NULL)
        continue;
      p2p_add_wps_vendor_extension(p2p, wpa_s->conf->wps_vendor_ext[i]);
    }
  }
  ...
  if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_LISTEN_CHANNEL) {
    ...
    ret = p2p_set_listen_channel(p2p, reg_class, channel, channel_forced);
    if (ret)
      wpa_printf(MSG_ERROR,
  ...
  if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_OPER_CHANNEL) {
    ...
    ret = p2p_set_oper_channel(p2p, op_reg_class, op_channel, cfg_op_channel);
    if (ret)
      wpa_printf(MSG_ERROR,
...
int wpas_p2p_set_noa(struct wpa_supplicant *wpa_s, u8 count, int start,
  ...
  if (!wpa_s->ap_iface)
    return -1;
  return hostapd_p2p_set_noa(wpa_s->ap_iface->bss[0], count, start, duration);
}
  ...
  if (!enabled) {
    ...
    for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
      ...
      wpa_msg_global(iface->p2pdev, MSG_INFO,
                     P2P_EVENT_CROSS_CONNECT_DISABLE "%s %s", iface->ifname,
...
static void wpas_p2p_enable_cross_connect(struct wpa_supplicant *uplink) {
  ...
  for (iface = uplink->global->ifaces; iface; iface = iface->next) {
    if (!iface->cross_connect_enabled)
      continue;
    if (os_strcmp(uplink->ifname, iface->cross_connect_uplink) != 0)
      continue;
    ...
    wpa_msg_global(iface->p2pdev, MSG_INFO,
                   P2P_EVENT_CROSS_CONNECT_ENABLE "%s %s", iface->ifname,
                   iface->cross_connect_uplink);
  }
}
...
void wpas_p2p_notif_disconnected(struct wpa_supplicant *wpa_s) {
  wpas_p2p_disable_cross_connect(wpa_s);
  if (!wpa_s->ap_iface &&
      !eloop_is_timeout_registered(wpas_p2p_group_idle_timeout, wpa_s, NULL))
    wpas_p2p_set_group_idle_timeout(wpa_s);
}
...
static void wpas_p2p_cross_connect_setup(struct wpa_supplicant *wpa_s) {
  ...
  for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
    if (iface == wpa_s)
      continue;
    if (iface->drv_flags & WPA_DRIVER_FLAGS_P2P_DEDICATED_INTERFACE)
      continue;
    ...
    if (iface->ap_iface || iface->current_ssid == NULL ||
        ...
        iface->cross_connect_disallowed || iface->wpa_state != WPA_COMPLETED)
      break;

    wpa_s->cross_connect_in_use = 1;
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                   P2P_EVENT_CROSS_CONNECT_ENABLE "%s %s", wpa_s->ifname,
                   wpa_s->cross_connect_uplink);
    break;
  }
}
...
int wpas_p2p_cancel(struct wpa_supplicant *wpa_s) {
  ...
  if (peer) {
    wpa_printf(MSG_DEBUG, "P2P: Unauthorize pending GO Neg peer " MACSTR,
               MAC2STR(peer));
  ...
  for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    if (wpa_s == global->p2p_group_formation &&
        (wpa_s->p2p_in_provisioning ||
         wpa_s->parent->pending_interface_type == WPA_IF_P2P_CLIENT)) {
      wpa_printf(MSG_DEBUG,
      ...
      wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_REQUESTED);
      break;
...
void wpas_p2p_update_best_channels(struct wpa_supplicant *wpa_s, int freq_24,
                                   int freq_5, int freq_overall) {
  struct p2p_data *p2p = wpa_s->global->p2p;
  if (p2p == NULL)
    return;
...
int wpas_p2p_disconnect(struct wpa_supplicant *wpa_s) {
  ...
  return wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_REQUESTED) < 0 ? -1 : 0;
}
...
struct wpa_ssid *wpas_p2p_get_persistent(struct wpa_supplicant *wpa_s,
  ...
  for (s = wpa_s->conf->ssid; s; s = s->next) {
    if (s->disabled != 2)
      continue;
    if (ssid &&
        (ssid_len != s->ssid_len || os_memcmp(ssid, s->ssid, ssid_len) != 0))
      continue;
    if (addr == NULL) {
      if (s->mode == WPAS_MODE_P2P_GO)
        return s;
...
void wpas_p2p_notify_ap_sta_authorized(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->p2p_go_group_formation_completed) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Marking group formation completed on GO on first data "
            "connection");
    wpa_s->p2p_go_group_formation_completed = 1;
    wpa_s->global->p2p_group_formation = NULL;
    wpa_s->p2p_in_provisioning = 0;
...
static int wpas_p2p_fallback_to_go_neg(struct wpa_supplicant *wpa_s,
  ...
  wpas_p2p_connect(wpa_s, wpa_s->pending_join_dev_addr, wpa_s->p2p_pin,
                   wpa_s->p2p_wps_method, wpa_s->p2p_persistent_group, 0, 0, 0,
                   wpa_s->p2p_go_intent, wpa_s->p2p_connect_freq,
                   wpa_s->p2p_go_vht_center_freq2, wpa_s->p2p_persistent_id,
                   wpa_s->p2p_pd_before_go_neg, wpa_s->p2p_go_ht40,
                   wpa_s->p2p_go_vht, wpa_s->p2p_go_max_oper_chwidth, NULL, 0);
  return ret;
}
...
int wpas_p2p_scan_no_go_seen(struct wpa_supplicant *wpa_s) {
  ...
  if (!wpa_s->p2p_fallback_to_go_neg || wpa_s->p2p_in_provisioning <= 5)
    return 0;
  ...
  wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
                 P2P_EVENT_FALLBACK_TO_GO_NEG "reason=GO-not-found");
  ...
  dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant,
    ...
    if (ifs != wpa_s && ifs->wpa_state > WPA_SCANNING) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              ...
              "interface %s",
              wpa_s->conf->p2p_search_delay, ifs->ifname);
      return wpa_s->conf->p2p_search_delay;
    }
  }
...
static int wpas_p2p_remove_psk_entry(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each_safe(psk, tmp, &s->psk_list, struct psk_list_entry, list) {
    if ((iface_addr && !psk->p2p &&
         os_memcmp(addr, psk->addr, ETH_ALEN) == 0) ||
        (!iface_addr && psk->p2p &&
         os_memcmp(addr, psk->addr, ETH_ALEN) == 0)) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: Remove persistent group PSK list entry for " MACSTR
              " p2p=%u",
              MAC2STR(psk->addr), psk->p2p);
...
void wpas_p2p_new_psk_cb(struct wpa_supplicant *wpa_s, const u8 *mac_addr,
  ...
  if (p2p_dev_addr) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: New PSK for addr=" MACSTR " p2p_dev_addr=" MACSTR,
  ...
  if (!persistent) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Could not find persistent group information to store the new "
            "PSK");
    return;
  }
  ...
  if (dl_list_len(&persistent->psk_list) > P2P_MAX_STORED_CLIENTS &&
      (last =
           dl_list_last(&persistent->psk_list, struct psk_list_entry, list))) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Remove oldest PSK entry for " MACSTR
  ...
  wpas_p2p_remove_psk_entry(wpa_s->p2pdev, persistent,
                            ...
                            p2p_dev_addr == NULL);
  if (p2p_dev_addr) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Add new PSK for p2p_dev_addr=" MACSTR,
...
static void wpas_p2p_remove_psk(struct wpa_supplicant *wpa_s,
  ...
  if (res > 0 && wpa_s->conf->update_config &&
      wpa_config_write(wpa_s->confname, wpa_s->conf))
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Failed to update configuration");
}
...
static void wpas_p2p_remove_client_go(struct wpa_supplicant *wpa_s,
  ...
  while (psk) {
    if ((iface_addr && os_memcmp(peer, psk->addr, ETH_ALEN) == 0) ||
        (!iface_addr && os_memcmp(peer, psk->p2p_dev_addr, ETH_ALEN) == 0)) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: Remove operating group PSK entry for " MACSTR
              " iface_addr=%d",
  ...
  if (sta) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Disconnect peer " MACSTR " (iface_addr=%d) from group",
            MAC2STR(peer), iface_addr);
...
int wpas_p2p_handle_frequency_conflicts(struct wpa_supplicant *wpa_s, int freq,
  ...
  for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
    if (!iface->current_ssid || iface->current_ssid->frequency == freq ||
    ...
    if (!wpas_is_p2p_prioritized(iface)) {
      ...
      wpa_printf(MSG_DEBUG, "P2P: Removing P2P connection due to single "
                            "channel concurrent mode frequency conflict");
      eloop_register_timeout(0, 0, wpas_p2p_group_freq_conflict, iface, NULL);
      ...
      wpa_msg(wpa_s->global->ifaces, MSG_INFO, WPA_EVENT_FREQ_CONFLICT " id=%d",
              ssid->id);
...
int wpas_p2p_4way_hs_failed(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->p2p_last_4way_hs_fail && wpa_s->p2p_last_4way_hs_fail == ssid) {
    u8 go_dev_addr[ETH_ALEN];
    struct wpa_ssid *persistent;

    if (wpas_p2p_persistent_group(wpa_s, go_dev_addr, ssid->ssid,
                                  ssid->ssid_len) <= 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: Could not determine whether 4-way handshake failures were "
              "for a persistent group");
      goto disconnect;
    }

    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Two 4-way handshake failures for a P2P group - "
            "go_dev_addr=" MACSTR,
            MAC2STR(go_dev_addr));
    persistent = wpas_p2p_get_persistent(wpa_s->p2pdev, go_dev_addr, ssid->ssid,
                                         ssid->ssid_len);
    if (persistent == NULL || persistent->mode != WPAS_MODE_INFRA) {
      wpa_dbg(wpa_s, MSG_DEBUG, "P2P: No matching persistent group stored");
      goto disconnect;
    }
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_PERSISTENT_PSK_FAIL "%d",
                   persistent->id);
  disconnect:
    wpa_s->p2p_last_4way_hs_fail = NULL;
    /*
     ...
     * interface.
     */
    eloop_register_timeout(0, 0, wpas_p2p_psk_failure_removal, wpa_s, NULL);
    return 1;
  }
...
static struct wpabuf *wpas_p2p_nfc_handover(int ndef, struct wpabuf *wsc,
  ...
  wpabuf_free(wsc);
  wpabuf_free(p2p);
  wpa_hexdump_buf(MSG_DEBUG, "P2P: Generated NFC connection handover message",
                  ret);
...
static int wpas_p2p_cli_freq(struct wpa_supplicant *wpa_s,
  ...
  if (ssid)
    *ssid = NULL;
  for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
    if (iface->wpa_state < WPA_ASSOCIATING || iface->current_ssid == NULL ||
...
struct wpabuf *wpas_p2p_nfc_handover_req(struct wpa_supplicant *wpa_s,
  ...
  if (cli_freq == 0) {
    ...
    wsc = NULL;
  p2p = p2p_build_nfc_handover_req(wpa_s->global->p2p, cli_freq, go_dev_addr,
                                   ssid ? ssid->ssid : NULL,
...
struct wpabuf *wpas_p2p_nfc_handover_sel(struct wpa_supplicant *wpa_s, int ndef,
                                         int tag) {
  struct wpabuf *wsc, *p2p;
  struct wpa_ssid *ssid;
  u8 go_dev_addr[ETH_ALEN];
  ...
  if (cli_freq == 0) {
    wsc = wps_build_nfc_handover_sel_p2p(
        wpa_s->parent->wps,
        tag ? wpa_s->conf->wps_nfc_dev_pw_id : DEV_PW_NFC_CONNECTION_HANDOVER,
        wpa_s->conf->wps_nfc_dh_pubkey,
        tag ? wpa_s->conf->wps_nfc_dev_pw : NULL);
  } else
    wsc = NULL;
  p2p = p2p_build_nfc_handover_sel(wpa_s->global->p2p, cli_freq, go_dev_addr,
                                   ssid ? ssid->ssid : NULL,
                                   ssid ? ssid->ssid_len : 0);
...
static int wpas_p2p_nfc_join_group(struct wpa_supplicant *wpa_s,
  ...
  if (params->go_freq && params->go_ssid_len) {
    ...
    os_memcpy(wpa_s->pending_join_dev_addr, params->go_dev_addr, ETH_ALEN);
    return wpas_p2p_join_start(wpa_s, params->go_freq, params->go_ssid,
                               params->go_ssid_len);
  }

  return wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC,
                          0, 0, 1, 0, wpa_s->conf->p2p_go_intent,
                          params->go_freq, wpa_s->p2p_go_vht_center_freq2, -1,
                          0, 1, 1, wpa_s->p2p_go_max_oper_chwidth,
...
static int wpas_p2p_nfc_auth_join(struct wpa_supplicant *wpa_s,
  ...
  res = wpas_ap_wps_add_nfc_pw(wpa_s, wpa_s->p2pdev->p2p_oob_dev_pw_id,
                               wpa_s->p2pdev->p2p_oob_dev_pw,
                               wpa_s->p2pdev->p2p_peer_oob_pk_hash_known
                                   ? wpa_s->p2pdev->p2p_peer_oob_pubkey_hash
                                   : NULL);
  ...
  wpa_printf(MSG_DEBUG, "P2P: Static handover - invite peer " MACSTR " to join",
             MAC2STR(params->peer->p2p_device_addr));

  wpa_s->global->p2p_invite_group = wpa_s;
  persistent =
      ssid->p2p_persistent_group &&
      wpas_p2p_get_persistent(wpa_s->p2pdev, params->peer->p2p_device_addr,
                              ssid->ssid, ssid->ssid_len);
  wpa_s->p2pdev->pending_invite_ssid_id = -1;

  return p2p_invite(wpa_s->global->p2p, params->peer->p2p_device_addr,
                    P2P_INVITE_ROLE_ACTIVE_GO, wpa_s->own_addr, ssid->ssid,
                    ssid->ssid_len, ssid->frequency,
...
static int wpas_p2p_nfc_init_go_neg(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "P2P: Initiate GO Negotiation based on NFC "
                        "connection handover");
  return wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC,
                          0, 0, 0, 0, wpa_s->conf->p2p_go_intent, forced_freq,
                          wpa_s->p2p_go_vht_center_freq2, -1, 0, 1, 1,
...
static int wpas_p2p_nfc_resp_go_neg(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "P2P: Authorize GO Negotiation based on NFC "
                        "connection handover");
  res = wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC, 0,
                         0, 0, 1, wpa_s->conf->p2p_go_intent, forced_freq,
                         wpa_s->p2p_go_vht_center_freq2, -1, 0, 1, 1,
                         wpa_s->p2p_go_max_oper_chwidth, NULL, 0);
  if (res)
    return res;
  ...
  if (res) {
    p2p_unauthorize(wpa_s->global->p2p, params->peer->p2p_device_addr);
  }
...
static int wpas_p2p_nfc_connection_handover(struct wpa_supplicant *wpa_s,
                                            const struct wpabuf *data, int sel,
                                            int tag, int forced_freq) {
  const u8 *pos, *end;
  u16 len, id;
  struct p2p_nfc_params params;
  int res;
  ...
  wpa_hexdump(MSG_DEBUG, "P2P: WSC attributes", params.wsc_attr,
              params.wsc_len);
  wpa_hexdump(MSG_DEBUG, "P2P: P2P attributes", params.p2p_attr,
              params.p2p_len);
  if (pos < end) {
    wpa_hexdump(MSG_DEBUG, "P2P: Ignored extra data after P2P attributes", pos,
                end - pos);
  }
  ...
  if (params.next_step == PEER_CLIENT) {
    if (!is_zero_ether_addr(params.go_dev_addr)) {
      wpa_msg(wpa_s, MSG_INFO,
              ...
              "peer=" MACSTR " freq=%d go_dev_addr=" MACSTR " ssid=\"%s\"",
              MAC2STR(params.peer->p2p_device_addr), params.go_freq,
              MAC2STR(params.go_dev_addr),
              wpa_ssid_txt(params.go_ssid, params.go_ssid_len));
    } else {
      wpa_msg(wpa_s, MSG_INFO,
              P2P_EVENT_NFC_PEER_CLIENT "peer=" MACSTR " freq=%d",
              MAC2STR(params.peer->p2p_device_addr), params.go_freq);
    }
    return 0;
  }
  ...
  if (wpas_p2p_cli_freq(wpa_s, NULL, NULL)) {
    wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_NFC_WHILE_CLIENT "peer=" MACSTR,
            MAC2STR(params.peer->p2p_device_addr));
    return 0;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "P2P: Peer OOB Public Key hash", params.oob_dev_pw,
              WPS_OOB_PUBKEY_HASH_LEN);
  os_memcpy(wpa_s->p2p_peer_oob_pubkey_hash, params.oob_dev_pw,
            WPS_OOB_PUBKEY_HASH_LEN);
  ...
  if (tag) {
    if (id < 0x10) {
      wpa_printf(MSG_DEBUG,
                 ...
                 "peer OOB Device Password Id %u",
                 id);
      return -1;
    }
    wpa_printf(MSG_DEBUG,
               "P2P: Static handover - use peer OOB "
               "Device Password Id %u",
               id);
    wpa_hexdump_key(MSG_DEBUG, "P2P: Peer OOB Device Password",
                    params.oob_dev_pw + WPS_OOB_PUBKEY_HASH_LEN + 2,
                    params.oob_dev_pw_len - WPS_OOB_PUBKEY_HASH_LEN - 2);
    wpa_s->p2p_oob_dev_pw_id = id;
    wpa_s->p2p_oob_dev_pw =
        wpabuf_alloc_copy(params.oob_dev_pw + WPS_OOB_PUBKEY_HASH_LEN + 2,
                          params.oob_dev_pw_len - WPS_OOB_PUBKEY_HASH_LEN - 2);
...
int wpas_p2p_nfc_report_handover(struct wpa_supplicant *wpa_s, int init,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "NFC: Req", wpabuf_head(req), wpabuf_len(req));
  wpa_hexdump_ascii(MSG_DEBUG, "NFC: Sel", wpabuf_head(sel), wpabuf_len(sel));
  if (forced_freq)
  ...
  ret = wpas_p2p_nfc_connection_handover(wpa_s, tmp, init, 0, forced_freq);
  wpabuf_free(tmp);
...
int wpas_p2p_nfc_tag_enabled(struct wpa_supplicant *wpa_s, int enabled) {
  ...
  if (!enabled) {
    ...
    p2p_set_authorized_oob_dev_pw_id(wpa_s->global->p2p, 0, 0, NULL);
  ...
  for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
    ...
    hapd->conf->wps_nfc_dh_pubkey = wpabuf_dup(wpa_s->conf->wps_nfc_dh_pubkey);
    ...
    hapd->conf->wps_nfc_dev_pw = wpabuf_dup(wpa_s->conf->wps_nfc_dev_pw);
    hapd->conf->wps_nfc_dev_pw_id = wpa_s->conf->wps_nfc_dev_pw_id;

    if (hostapd_wps_nfc_token_enable(iface->ap_iface->bss[0]) < 0) {
      wpa_dbg(iface, MSG_DEBUG, "P2P: Failed to enable NFC Tag for GO");
    }
  }
  p2p_set_authorized_oob_dev_pw_id(
      wpa_s->global->p2p, wpa_s->conf->wps_nfc_dev_pw_id, go_intent, if_addr);
...
static int wpas_p2p_move_go_csa(struct wpa_supplicant *wpa_s) {
  ...
  if (wpas_p2p_init_go_params(wpa_s, &params, 0, 0, 0, 0, 0, NULL)) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: Failed to select new frequency for GO");
    return -1;
  }
  ...
  if (!conf) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: Failed to allocate default config");
    return -1;
  }

  current_ssid->frequency = params.freq;
  if (wpa_supplicant_conf_ap_ht(wpa_s, current_ssid, conf)) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: Failed to create new GO config");
    ret = -1;
    goto out;
  }
  ...
  if (conf->ieee80211ac) {
    ...
    if (freq1 < 0 || freq2 < 0) {
      wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: Selected invalid VHT center freqs");
      ret = -1;
      goto out;
    }
...
static void wpas_p2p_move_go_no_csa(struct wpa_supplicant *wpa_s) {
  ...
  if (wpas_p2p_init_go_params(wpa_s, &params, 0, 0, 0, 0, 0, NULL)) {
    ...
    wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_GO_LEAVE_CHANNEL);
    return;
  }
  ...
  if (params.freq && !p2p_supported_freq_go(wpa_s->global->p2p, params.freq)) {
    wpa_printf(MSG_DEBUG, "P2P: Selected freq (%u MHz) is not valid for P2P",
               params.freq);
    wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_GO_LEAVE_CHANNEL);
    return;
  }
...
static void wpas_p2p_move_go(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (hostapd_csa_in_progress(wpa_s->ap_iface)) {
    wpa_printf(MSG_DEBUG, "P2P: CSA is in progress - not moving GO");
    return;
  }
...
static void wpas_p2p_reconsider_moving_go(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpas_p2p_consider_moving_gos(wpa_s, freqs, num, WPAS_P2P_CHANNEL_UPDATE_ANY);
...
static void wpas_p2p_consider_moving_one_go(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0, invalid_freq = 0; i < num; i++) {
    if (freqs[i].freq == freq) {
      ...
      if (!p2p_supported_freq_go(wpa_s->global->p2p, freq)) {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Freq=%d MHz no longer valid for GO",
                freq);
        invalid_freq = 1;
      }
      ...
      continue;
    } else if (!p2p_supported_freq(wpa_s->global->p2p, freqs[i].freq)) {
      /* Freq is not valid for P2P use cases */
      continue;
    } else if (wpa_s->conf->p2p_go_freq_change_policy == P2P_GO_FREQ_MOVE_SCM) {
      policy_move = 1;
      ...
      if (!p2p_get_group_num_members(wpa_s->p2p_group)) {
        policy_move = 1;
      } else if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_AP_CSA) &&
  ...
  if (!invalid_freq || policy_move) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Cancel a GO move from freq=%d MHz", freq);
    eloop_cancel_timeout(wpas_p2p_move_go, wpa_s, NULL);

    if (wpas_p2p_in_progress(wpa_s)) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: GO move: policy CS is not allowed - setting timeout to "
              "re-consider GO move");
      eloop_cancel_timeout(wpas_p2p_reconsider_moving_go, wpa_s, NULL);
      eloop_register_timeout(P2P_RECONSIDER_GO_MOVE_DELAY, 0,
                             wpas_p2p_reconsider_moving_go, wpa_s, NULL);
      return;
    }
  }

  if (!invalid_freq && (!policy_move || flags != 0)) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Not initiating a GO frequency change");
    return;
  }
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Move GO from freq=%d MHz in %d secs", freq,
          timeout);
  eloop_cancel_timeout(wpas_p2p_move_go, wpa_s, NULL);
...
wpas_p2p_consider_moving_gos(struct wpa_supplicant *wpa_s,
  ...
  eloop_cancel_timeout(wpas_p2p_reconsider_moving_go, ELOOP_ALL_CTX, NULL);
  ...
  dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant,
     ...
     * to move it.
     */
    if (wpa_s == ifs && (trig == WPAS_P2P_CHANNEL_UPDATE_STATE_CHANGE ||
                         trig == WPAS_P2P_CHANNEL_UPDATE_CS)) {
      wpa_dbg(wpa_s, MSG_DEBUG, "P2P: GO move - schedule re-consideration");
      eloop_register_timeout(P2P_RECONSIDER_GO_MOVE_DELAY, 0,
                             wpas_p2p_reconsider_moving_go, wpa_s, NULL);
      continue;
    }
...
void wpas_p2p_indicate_state_change(struct wpa_supplicant *wpa_s) {
  ...
  wpas_p2p_update_channel_list(wpa_s, WPAS_P2P_CHANNEL_UPDATE_STATE_CHANGE);
}
...
int wpas_p2p_lo_start(struct wpa_supplicant *wpa_s, unsigned int freq,
  ...
  if (wpa_s->p2p_lo_started) {
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P Listen offload is already started");
    return 0;
  }
  ...
  if (!p2p_supported_freq(wpa_s->global->p2p, freq)) {
    wpa_printf(MSG_ERROR, "P2P: Input channel not supported: %u", freq);
    return -1;
  }

  /* Get device type */
  dev_types_len = (wpa_s->conf->num_sec_device_types + 1) * WPS_DEV_TYPE_LEN;
  ...
  ret = wpa_drv_p2p_lo_start(wpa_s, freq, period, interval, count, device_types,
                             dev_types_len, wpabuf_mhead_u8(buf),
                             wpabuf_len(buf));
  if (ret < 0)
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Failed to start P2P listen offload");
...
int wpas_p2p_lo_stop(struct wpa_supplicant *wpa_s) {
  ...
  if (ret < 0)
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Failed to stop P2P listen offload");
