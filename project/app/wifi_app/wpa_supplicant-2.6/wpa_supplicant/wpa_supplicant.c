...
int wpa_set_wep_keys(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  ...
  for (i = 0; i < NUM_WEP_KEYS; i++) {
    ...
    set = 1;
    wpa_drv_set_key(wpa_s, WPA_ALG_WEP, NULL, i, i == ssid->wep_tx_keyidx, NULL,
                    0, ssid->wep_key[i], ssid->wep_key_len[i]);
  }
...
void wpa_supplicant_initiate_eapol(struct wpa_supplicant *wpa_s) {
  ...
  if (ssid->mode == WPAS_MODE_IBSS && wpa_s->key_mgmt != WPA_KEY_MGMT_NONE &&
  ...
  eapol_conf.eap_disabled = !wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt) &&
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS) {
    ...
    if (wpa_s->current_bss) {
      struct wpabuf *ie;
      ie = wpa_bss_get_vendor_ie_multi(wpa_s->current_bss, WPS_IE_VENDOR_TYPE);
      if (ie) {
        if (wps_is_20(ie))
          eapol_conf.wps |= EAPOL_PEER_IS_WPS20_AP;
        wpabuf_free(ie);
      }
    }
  }
...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s) {
  ...
  eloop_cancel_timeout(wpa_supplicant_delayed_mic_error_report, wpa_s, NULL);
...
void wpa_clear_keys(struct wpa_supplicant *wpa_s, const u8 *addr) {
  ...
  for (i = 0; i < max; i++) {
    if (wpa_s->keys_cleared & BIT(i))
      continue;
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, i, 0, NULL, 0, NULL, 0);
  }
  if (!(wpa_s->keys_cleared & BIT(0)) && addr && !is_zero_ether_addr(addr)) {
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, addr, 0, 0, NULL, 0, NULL, 0);
    /* MLME-SETPROTECTION.request(None) */
    wpa_drv_mlme_setprotection(wpa_s, addr,
...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->conf->p2p_cli_probe && wpa_s->current_ssid &&
    ...
    if (state == WPA_COMPLETED && !wpa_s->p2p_cli_probe) {
      wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Enable CLI Probe Request RX reporting");
      wpa_s->p2p_cli_probe = wpa_drv_probe_req_report(wpa_s, 1) >= 0;
    } else if (state != WPA_COMPLETED && wpa_s->p2p_cli_probe) {
      wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Disable CLI Probe Request RX reporting");
  ...
  if (state == WPA_COMPLETED && wpa_s->new_connection) {
    ...
    wpa_msg(wpa_s, MSG_INFO,
            ...
            MAC2STR(wpa_s->bssid), ssid ? ssid->id : -1,
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s) {
  ...
  reconf_ctrl =
      ...
      (conf->ctrl_interface && wpa_s->conf->ctrl_interface &&
       os_strcmp(conf->ctrl_interface, wpa_s->conf->ctrl_interface) != 0);
...
static void wpa_supplicant_reconfig(int sig, void *signal_ctx) {
  ...
  for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Signal %d received - reconfiguring", sig);
...
int wpa_supplicant_set_suites(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "WPA: ssid proto=0x%x pairwise_cipher=0x%x group_cipher=0x%x "
            "key_mgmt=0x%x",
    ...
    if (wpa_supplicant_suites_from_ai(wpa_s, ssid, &ie) < 0) {
      ...
      ie.key_mgmt = ssid->key_mgmt;
#ifdef CONFIG_IEEE80211W
      ie.mgmt_group_cipher = ssid->ieee80211w != NO_MGMT_FRAME_PROTECTION
  ...
  if (0) {
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT 802.1X with Suite B");
#endif /* CONFIG_SUITEB */
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT 802.1X with SHA256");
  } else if (sel & WPA_KEY_MGMT_PSK_SHA256) {
    wpa_s->key_mgmt = WPA_KEY_MGMT_PSK_SHA256;
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT PSK with SHA256");
#endif /* CONFIG_IEEE80211W */
  ...
  if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt)) {
    ...
    if (bss && ssid->bssid_set && ssid->ssid_len == 0 && ssid->passphrase) {
      u8 psk[PMK_LEN];
      pbkdf2_sha1(ssid->passphrase, bss->ssid, bss->ssid_len, 4096, psk,
                  PMK_LEN);
      wpa_hexdump_key(MSG_MSGDUMP, "PSK (from passphrase)", psk, PMK_LEN);
    ...
    if (ssid->ext_psk) {
      ...
      if (wpabuf_len(pw) >= 8 && wpabuf_len(pw) < 64 && bss) {
        pbkdf2_sha1(pw_str, bss->ssid, bss->ssid_len, 4096, psk, PMK_LEN);
    ...
    if (!psk_set) {
      wpa_msg(wpa_s, MSG_INFO, "No PSK available for association");
      return -1;
    }
  } else
...
int wpas_build_ext_capab(struct wpa_supplicant *wpa_s, u8 *buf, size_t buflen) {
  ...
  if (len < 9 && (wpa_s->conf->ftm_initiator || wpa_s->conf->ftm_responder))
    len = 9;
...
int wpas_update_random_addr(struct wpa_supplicant *wpa_s, int style) {
  ...
  if (wpa_drv_set_mac_addr(wpa_s, addr) < 0) {
    wpa_msg(wpa_s, MSG_INFO, "Failed to set random MAC address");
    return -1;
  }
  ...
  if (wpa_supplicant_update_mac_addr(wpa_s) < 0) {
    wpa_msg(wpa_s, MSG_INFO, "Could not update MAC address information");
    return -1;
  }

  wpa_msg(wpa_s, MSG_DEBUG, "Using random MAC address " MACSTR, MAC2STR(addr));

  return 0;
}

int wpas_update_random_addr_disassoc(struct wpa_supplicant *wpa_s) {
  if (wpa_s->wpa_state >= WPA_AUTHENTICATING || !wpa_s->conf->preassoc_mac_addr)
    return 0;
...
void wpa_supplicant_associate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (wpa_s->last_ssid == ssid) {
    ...
    if (wpa_drv_set_mac_addr(wpa_s, NULL) < 0) {
      wpa_msg(wpa_s, MSG_INFO, "Could not restore permanent MAC address");
      return;
    }
  ...
  if (ssid->mode == WPAS_MODE_MESH) {
    ...
    if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MESH)) {
      wpa_msg(wpa_s, MSG_INFO, "Driver does not support mesh mode");
      return;
    }
    if (bss)
...
#else  /* CONFIG_MESH */
    wpa_msg(wpa_s, MSG_ERROR, "mesh mode support not included in the build");
#endif /* CONFIG_MESH */
    return;
  }
...
void ibss_mesh_setup_freq(struct wpa_supplicant *wpa_s,
  ...
  enum hostapd_hw_mode hw_mode;
  struct hostapd_hw_modes *mode = NULL;
  int ht40plus[] = {36,  44,  52,  60,  100, 108, 116,
  ...
  if (wpa_s->group_cipher == WPA_CIPHER_WEP40 ||
    ...
    wpa_printf(MSG_DEBUG, "IBSS: WEP/TKIP detected, do not try to enable HT");
    return;
  }
  ...
  for (j = 0; j < ARRAY_SIZE(vht80); j++) {
    if (freq->channel >= vht80[j] && freq->channel < vht80[j] + 16)
      break;
  }
  ...
  if (ssid->max_oper_chwidth == VHT_CHANWIDTH_80P80MHZ) {
    ...
    for (k = 0; k < ARRAY_SIZE(vht80); k++) {
      ...
      for (i = vht80[k]; i < vht80[k] + 16; i += 4) {
        ...
        if (chan->flag &
            (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_NO_IR | HOSTAPD_CHAN_RADAR))
          continue;

        /* Found a suitable second segment for 80+80 */
        chwidth = VHT_CHANWIDTH_80P80MHZ;
        vht_caps |= VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ;
        seg1 = vht80[k] + 6;
      }
  ...
  if (hostapd_set_freq_params(&vht_freq, mode->mode, freq->freq, freq->channel,
                              freq->ht_enabled, vht_freq.vht_enabled,
                              freq->sec_channel_offset, chwidth, seg0, seg1,
                              vht_caps) != 0)
    return;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (cwork->bss_removed || !wpas_valid_bss_ssid(wpa_s, bss, ssid) ||
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "BSS/SSID entry for association not valid anymore - drop "
            "connection attempt");
    wpas_connect_work_done(wpa_s);
    return;
  }
  ...
  if (bss && (!wpas_driver_bss_selection(wpa_s) || wpas_wps_searching(wpa_s))) {
#ifdef CONFIG_IEEE80211R
    const u8 *ie, *md = NULL;
#endif /* CONFIG_IEEE80211R */
    wpa_msg(wpa_s, MSG_INFO,
            "Trying to associate with " MACSTR " (SSID='%s' freq=%d MHz)",
            MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len),
            bss->freq);
    ...
    if (md) {
      ...
      wpa_ft_prepare_auth_request(wpa_s->wpa, ie);
             ...
             wpa_s->conf->ap_scan == 2 && (ssid->key_mgmt & WPA_KEY_MGMT_WPS)) {
  ...
  if (bss &&
    ...
    if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, ssid,
                                try_opportunistic) == 0)
    ...
    wpa_ie_len = sizeof(wpa_ie);
    if (wpa_supplicant_set_suites(wpa_s, NULL, ssid, wpa_ie, &wpa_ie_len)) {
      wpa_msg(wpa_s, MSG_WARNING,
              "WPA: Failed to set WPA "
  ...
  if (bss) {
    ...
    if (p2p) {
      wpa_s->cross_connect_disallowed = p2p_get_cross_connect_disallowed(p2p);
      wpabuf_free(p2p);
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: WLAN AP %s cross "
              "connection",
              wpa_s->cross_connect_disallowed ? "disallows" : "allows");
    }
  }
  ...
  if (bss) {
    ...
    if (mbo) {
      ...
      len = wpas_mbo_supp_op_class_ie(wpa_s, bss->freq, wpa_ie + wpa_ie_len,
  ...
  if (!bss || wpa_bss_get_ie(bss, WLAN_EID_EXT_CAPAB)) {
    ...
    if (ext_capab_len > 0) {
      ...
      if (wpa_ie_len > 0 && pos[0] == WLAN_EID_RSN)
        pos += 2 + pos[1];
      os_memmove(pos + ext_capab_len, pos, wpa_ie_len - (pos - wpa_ie));
  ...
  if (is_hs20_network(wpa_s, ssid, bss)) {
    ...
    if (hs20) {
      ...
      if (wpabuf_len(hs20) <= len) {
        os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(hs20), wpabuf_len(hs20));
  ...
  if (wpa_s->vendor_elem[VENDOR_ELEM_ASSOC_REQ]) {
    ...
    if (wpabuf_len(buf) <= len) {
      os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(buf), wpabuf_len(buf));
  ...
  if (wpa_s->fst_ies) {
    ...
    if (wpa_ie_len + fst_ies_len <= sizeof(wpa_ie)) {
      os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(wpa_s->fst_ies), fst_ies_len);
  ...
  if (mbo) {
    ...
    len = wpas_mbo_ie(wpa_s, wpa_ie + wpa_ie_len, sizeof(wpa_ie) - wpa_ie_len);
    if (len >= 0)
      wpa_ie_len += len;
  }
#endif /* CONFIG_MBO */
  ...
  if (bss) {
    ...
    if (!wpas_driver_bss_selection(wpa_s) || ssid->bssid_set) {
      wpa_printf(MSG_DEBUG,
                 "Limit connection to BSSID " MACSTR
                 " freq=%u MHz based on scan results "
                 "(bssid_set=%d)",
                 MAC2STR(bss->bssid), bss->freq, ssid->bssid_set);
  ...
  if (wpa_s->conf->key_mgmt_offload) {
    if (params.key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X ||
      ...
      params.req_key_mgmt_offload = ssid->proactive_key_caching < 0
                                        ? wpa_s->conf->okc
                                        : ssid->proactive_key_caching;
    else
  ...
  if (wpa_s->num_multichan_concurrent < 2) {
    ...
    if (num > 0 && freq > 0 && freq != params.freq.freq) {
      wpa_printf(MSG_DEBUG, "Assoc conflicting freq found (%d != %d)", freq,
                 params.freq.freq);
      if (wpas_p2p_handle_frequency_conflicts(wpa_s, params.freq.freq, ssid) <
          0) {
        wpas_connect_work_done(wpa_s);
        return;
      }
...
void wpa_supplicant_deauthenticate(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Request to deauthenticate - bssid=" MACSTR " pending_bssid=" MACSTR
          " reason=%d state=%s",
          MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid), reason_code,
          wpa_supplicant_state_txt(wpa_s->wpa_state));
...
#ifdef CONFIG_MESH
  if (wpa_s->ifmsh) {
    wpa_msg(wpa_s, MSG_INFO, MESH_GROUP_REMOVED "%s", wpa_s->ifname);
...
void wpa_supplicant_enable_network(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->reassociate && !wpa_s->disconnected &&
      (!wpa_s->current_ssid || wpa_s->wpa_state == WPA_DISCONNECTED ||
       wpa_s->wpa_state == WPA_SCANNING)) {
...
int wpas_set_pkcs11_engine_and_module_path(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_supplicant_init_eapol(wpa_s)) {
    ...
    if (pkcs11_engine_path != NULL && pkcs11_module_path != NULL)
      wpas_set_pkcs11_engine_and_module_path(wpa_s, NULL, NULL);
...
int wpa_supplicant_set_ap_scan(struct wpa_supplicant *wpa_s, int ap_scan) {
  ...
  if (ap_scan == 2 && os_strcmp(wpa_s->driver->name, "nl80211") == 0) {
    wpa_printf(MSG_INFO,
               "Note: nl80211 driver interface is not designed to be used with "
               "ap_scan=2; this can result in connection failures");
  }

#ifdef ANDROID
  if (ap_scan == 2 && ap_scan != wpa_s->conf->ap_scan &&
      wpa_s->wpa_state >= WPA_ASSOCIATING && wpa_s->wpa_state < WPA_COMPLETED) {
...
int wpa_supplicant_set_bss_expiration_age(struct wpa_supplicant *wpa_s,
  ...
  if (bss_expire_age < 10) {
    wpa_msg(wpa_s, MSG_ERROR, "Invalid bss expiration age %u", bss_expire_age);
    return -1;
  }
...
int wpa_supplicant_set_scan_interval(struct wpa_supplicant *wpa_s,
                                     int scan_interval) {
  if (scan_interval < 0) {
    wpa_msg(wpa_s, MSG_ERROR, "Invalid scan interval %d", scan_interval);
    return -1;
  }
  wpa_msg(wpa_s, MSG_DEBUG, "Setting scan interval: %d sec", scan_interval);
...
int wpa_supplicant_set_debug_params(struct wpa_global *global, int debug_level,
  ...
  if (debug_level != MSG_EXCESSIVE && debug_level != MSG_MSGDUMP &&
      debug_level != MSG_DEBUG && debug_level != MSG_INFO &&
      debug_level != MSG_WARNING && debug_level != MSG_ERROR)
    return -1;
...
struct wpa_ssid *wpa_supplicant_get_ssid(struct wpa_supplicant *wpa_s) {
  ...
  wired =
      wpa_s->conf->ap_scan == 0 && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_WIRED);
  ...
  while (entry) {
    if (!wpas_network_disabled(wpa_s, entry) &&
        ((ssid_len == entry->ssid_len &&
         ...
         wired) &&
        (!entry->bssid_set || os_memcmp(bssid, entry->bssid, ETH_ALEN) == 0))
      return entry;
#ifdef CONFIG_WPS
    if (!wpas_network_disabled(wpa_s, entry) &&
    ...
    if (!wpas_network_disabled(wpa_s, entry) && entry->bssid_set &&
        entry->ssid_len == 0 && os_memcmp(bssid, entry->bssid, ETH_ALEN) == 0)
      return entry;
...
void wpa_supplicant_rx_eapol(void *ctx, const u8 *src_addr, const u8 *buf,
  ...
  if (wpa_s->wpa_state < WPA_ASSOCIATED ||
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            ...
            wpa_supplicant_state_txt(wpa_s->wpa_state), MAC2STR(wpa_s->bssid));
    wpabuf_free(wpa_s->pending_eapol_rx);
  ...
  if (wpa_s->eapol_received == 0 &&
    ...
    if (wpa_s->current_ssid && wpa_s->current_bss &&
      ...
      wps_ie =
          wpa_bss_get_vendor_ie_multi(wpa_s->current_bss, WPS_IE_VENDOR_TYPE);
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s) {
  if ((!wpa_s->p2p_mgmt ||
    ...
    l2_packet_deinit(wpa_s->l2);
    wpa_s->l2 = l2_packet_init(wpa_s->ifname, wpa_drv_get_mac_addr(wpa_s),
                               ETH_P_EAPOL, wpa_supplicant_rx_eapol, wpa_s, 0);
    if (wpa_s->l2 == NULL)
      return -1;
  } else {
...
static void wpa_supplicant_rx_eapol_bridge(void *ctx, const u8 *src_addr,
  ...
  if (os_memcmp(eth->h_dest, wpa_s->own_addr, ETH_ALEN) != 0 &&
      !(eth->h_dest[0] & 0x01)) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "RX EAPOL from " MACSTR " to " MACSTR
            " (bridge - not for this interface - ignore)",
            MAC2STR(src_addr), MAC2STR(eth->h_dest));
    return;
  }

  wpa_dbg(wpa_s, MSG_DEBUG, "RX EAPOL from " MACSTR " to " MACSTR " (bridge)",
          MAC2STR(src_addr), MAC2STR(eth->h_dest));
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->bridge_ifname[0]) {
    ...
    wpa_s->l2_br = l2_packet_init_bridge(
        wpa_s->bridge_ifname, wpa_s->ifname, wpa_s->own_addr, ETH_P_EAPOL,
        wpa_supplicant_rx_eapol_bridge, wpa_s, 1);
    if (wpa_s->l2_br == NULL) {
      wpa_msg(wpa_s, MSG_ERROR,
...
static int wpa_set_htcap_mcs(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < IEEE80211_HT_MCS_MASK_LEN; i++) {
    ...
    if (errno == 0) {
      wpa_msg(wpa_s, MSG_DEBUG, "htcap value[%i]: %ld end: %p  tmp: %p", i, v,
              end, tmp);
      if (end == tmp)
        break;
      ...
      wpa_msg(wpa_s, MSG_ERROR, "Failed to parse ht-mcs: %s, error: %s\n",
...
static int wpa_set_ampdu_density(struct wpa_supplicant *wpa_s,
  ...
  if (density < 0 || density > 7) {
    wpa_msg(wpa_s, MSG_ERROR,
            "ampdu_density: %d out of range. Must be 0-7 or -1.", density);
    return -EINVAL;
  }
...
static int wpa_set_disable_sgi(struct wpa_supplicant *wpa_s,
  ...
  le16 msk =
      host_to_le16(HT_CAP_INFO_SHORT_GI20MHZ | HT_CAP_INFO_SHORT_GI40MHZ);
...
void wpa_supplicant_apply_vht_overrides(
  ...
  if (ssid->vht_capa_mask & VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX) {
    ...
    max_ampdu = (ssid->vht_capa & VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX) >>
                VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX_SHIFT;

    max_ampdu = max_ampdu < 3 ? max_ampdu : 3;
    wpa_set_ampdu_factor(wpa_s, (void *)params->htcaps,
                         (void *)params->htcaps_mask, max_ampdu);
  }
#endif /* CONFIG_HT_OVERRIDES */
...
#define OVERRIDE_MCS(i)                                                        \
  if (ssid->vht_tx_mcs_nss_##i >= 0) {                                         \
    ...
    vhtcaps->vht_supported_mcs_set.tx_map |=                                   \
        host_to_le16(ssid->vht_tx_mcs_nss_##i << 2 * (i - 1));                 \
  }                                                                            \
  if (ssid->vht_rx_mcs_nss_##i >= 0) {                                         \
    vhtcaps_mask->vht_supported_mcs_set.rx_map |=                              \
        host_to_le16(3 << 2 * (i - 1));                                        \
    vhtcaps->vht_supported_mcs_set.rx_map |=                                   \
        host_to_le16(ssid->vht_rx_mcs_nss_##i << 2 * (i - 1));                 \
  }
...
static int pcsc_reader_init(struct wpa_supplicant *wpa_s) {
  ...
  wpa_printf(MSG_DEBUG, "SCARD: IMSI %s (MNC length %d)", wpa_s->imsi,
             wpa_s->mnc_len);
...
static const u8 *wpas_fst_get_bssid_cb(void *ctx) {
  ...
  return (is_zero_ether_addr(wpa_s->bssid) || wpa_s->wpa_state != WPA_COMPLETED)
             ? NULL
             : wpa_s->bssid;
}
...
static int wpas_fst_send_action_cb(void *ctx, const u8 *da,
                                   struct wpabuf *data) {
  struct wpa_supplicant *wpa_s = ctx;

  WPA_ASSERT(os_memcmp(wpa_s->bssid, da, ETH_ALEN) == 0);
  return wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                             wpa_s->own_addr, wpa_s->bssid, wpabuf_head(data),
                             wpabuf_len(data), 0);
}
...
static void wpas_fst_update_mb_ie_cb(void *ctx, const u8 *addr, const u8 *buf,
                                     size_t size) {
  struct wpa_supplicant *wpa_s = ctx;
...
static const u8 *wpas_fst_get_peer_first(void *ctx,
  ...
  *get_ctx = NULL;
  if (!is_zero_ether_addr(wpa_s->bssid))
    return (wpa_s->received_mb_ies || !mb_only) ? wpa_s->bssid : NULL;
  return NULL;
}
  ...
  if (freqs) {
    ...
    for (i = 0; i < wpa_s->hw.num_modes; i++) {
      if (wpa_s->hw.modes[i].num_channels != 0) {
        if (wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211B ||
            wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211G)
          band |= BAND_2_4_GHZ;
        else if (wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211A)
          band |= BAND_5_GHZ;
        else if (wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211AD)
          band |= BAND_60_GHZ;
        else if (wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211ANY)
          band = BAND_2_4_GHZ | BAND_5_GHZ | BAND_60_GHZ;
      }
    }
  }
...
static struct wpa_radio *radio_add_interface(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "Add interface %s to a new radio %s", wpa_s->ifname,
             rn ? rn : "N/A");
...
static void radio_work_free(struct wpa_radio_work *work) {
  ...
  if (work->wpa_s->p2p_scan_work == work) {
    /* This should not really happen. */
    wpa_dbg(work->wpa_s, MSG_INFO,
            "Freeing radio work '%s'@%p (started=%d) that is marked as "
            "p2p_scan_work",
            work->type, work, work->started);
  ...
  if (work->started) {
    ...
    wpa_dbg(work->wpa_s, MSG_DEBUG,
            "radio_work_free('%s'@%p: num_active_works --> %u", work->type,
            work, work->wpa_s->radio->num_active_works);
  }
...
static struct wpa_radio_work *
radio_work_get_next_work(struct wpa_radio *radio) {
  ...
  if (!active_work) {
    ...
    dl_list_for_each(tmp, &radio->work, struct wpa_radio_work, list) {
      if (os_strcmp(tmp->type, "scan") == 0 && radio->external_scan_running &&
          (((struct wpa_driver_scan_params *)tmp->ctx)->only_new_results ||
  ...
  if (os_strcmp(active_work->type, "sme-connect") == 0 ||
    ...
    wpa_dbg(active_work->wpa_s, MSG_DEBUG,
            "Do not parallelize radio work with %s", active_work->type);
    return NULL;
  }
  ...
  dl_list_for_each(tmp, &radio->work, struct wpa_radio_work, list) {
    ...
    if (os_strcmp(active_work->type, tmp->type) != 0 &&
      ...
      wpa_dbg(active_work->wpa_s, MSG_DEBUG, "active_work:%s new_work:%s",
...
static void radio_start_next_work(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_s = dl_list_first(&radio->ifaces, struct wpa_supplicant, radio_list);

  if (!(wpa_s && wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_SIMULTANEOUS)) {
    if (work->started)
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Starting radio work '%s'@%p after %ld.%06ld second wait", work->type,
          work, diff.sec, diff.usec);
  work->started = 1;
  work->time = now;
...
void radio_remove_works(struct wpa_supplicant *wpa_s, const char *type,
  ...
  dl_list_for_each_safe(work, tmp, &radio->work, struct wpa_radio_work, list) {
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "Remove radio work '%s'@%p%s", work->type, work,
...
int radio_add_work(struct wpa_supplicant *wpa_s, unsigned int freq,
  ...
  if (freq)
    ...
    work->bands =
        wpas_get_bands(wpa_s, ((struct wpa_driver_scan_params *)ctx)->freqs);
  else
...
void radio_work_done(struct wpa_radio_work *work) {
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "Radio work '%s'@%p %s in %ld.%06ld seconds",
          work->type, work, started ? "done" : "canceled", diff.sec, diff.usec);
  radio_work_free(work);
  if (started)
    radio_work_check_next(wpa_s);
}
...
static int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             iface->ifname, iface->confname ? iface->confname : "N/A",
  ...
  if (iface->confname) {
    ...
    wpa_printf(MSG_DEBUG, "Configuration file '%s' -> '%s'", iface->confname,
               wpa_s->confname);
  ...
  if (wpa_s->hw.modes) {
    ...
    for (i = 0; i < wpa_s->hw.num_modes; i++) {
      ...
      if (wpa_s->hw.modes[i].ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET)
  ...
  if (capa_res == 0) {
    ...
    if (wpa_s->sched_scan_supported && capa.mac_addr_rand_sched_scan_supported)
  ...
  if (wpa_s->conf->fst_group_id) {
    ...
    os_strlcpy(cfg.group_id, wpa_s->conf->fst_group_id, sizeof(cfg.group_id));
    cfg.priority = wpa_s->conf->fst_priority;
    cfg.llt = wpa_s->conf->fst_llt;

    wpa_s->fst = fst_attach(wpa_s->ifname, wpa_s->own_addr, &iface_obj, &cfg);
    if (!wpa_s->fst) {
      wpa_msg(wpa_s, MSG_ERROR, "FST: Cannot attach iface %s to group %s",
...
#ifdef CONFIG_EAP_PROXY
  {
    size_t len;
    wpa_s->mnc_len =
        eapol_sm_get_eap_proxy_imsi(wpa_s->eapol, wpa_s->imsi, &len);
    if (wpa_s->mnc_len > 0) {
      wpa_s->imsi[len] = '\0';
      wpa_printf(MSG_DEBUG, "eap_proxy: IMSI %s (MNC length %d)", wpa_s->imsi,
                 wpa_s->mnc_len);
    } else {
...
static void wpa_supplicant_deinit_iface(struct wpa_supplicant *wpa_s,
  ...
  while (iface) {
    ...
    wpa_printf(MSG_DEBUG, "Remove remaining child interface %s from parent %s",
...
struct wpa_interface *wpa_supplicant_match_iface(struct wpa_global *global,
  ...
  for (i = 0; i < global->params.match_iface_count; i++) {
    ...
    if (!miface->ifname || fnmatch(miface->ifname, ifname, 0) == 0) {
...
struct wpa_supplicant *wpa_supplicant_add_iface(struct wpa_global *global,
  ...
  if (global->params.override_ctrl_interface) {
    wpa_printf(MSG_DEBUG,
               ...
               "ctrl_interface ('%s' -> '%s')",
               iface->ctrl_interface, global->params.override_ctrl_interface);
...
int wpa_supplicant_remove_iface(struct wpa_global *global,
  ...
  if (mesh_if_created) {
    ...
    if (ifname == NULL) {
      wpa_dbg(wpa_s, MSG_ERROR, "mesh: Failed to malloc ifname");
      return -1;
    }
  }
#endif /* CONFIG_MESH */
...
static void wpas_periodic(void *eloop_ctx, void *timeout_ctx) {
  ...
  eloop_register_timeout(WPA_SUPPLICANT_CLEANUP_INTERVAL, 0, wpas_periodic,
                         global, NULL);
...
struct wpa_global *wpa_supplicant_init(struct wpa_params *params) {
  ...
  if (params->match_iface_count) {
    global->params.match_ifaces =
        os_calloc(params->match_iface_count, sizeof(struct wpa_interface));
    os_memcpy(global->params.match_ifaces, params->match_ifaces,
              params->match_iface_count * sizeof(struct wpa_interface));
  }
#endif /* CONFIG_MATCH_IFACE */
#ifdef CONFIG_P2P
...
int wpa_supplicant_run(struct wpa_global *global) {
  ...
  if (global->params.daemonize &&
      (wpa_supplicant_daemon(global->params.pid_file) || eloop_sock_requeue()))
    return -1;
...
int wpas_get_ssid_pmf(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  ...
  if (ssid == NULL || ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT) {
    ...
    if (ssid && (ssid->key_mgmt & ~(WPA_KEY_MGMT_NONE | WPA_KEY_MGMT_WPS |
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, char *reason) {
  ...
  if (ssid->auth_failures > 1 && wpa_key_mgmt_wpa_ieee8021x(ssid->key_mgmt))
...
void wpas_clear_temp_disabled(struct wpa_supplicant *wpa_s,
  ...
  if (ssid->disabled_until.sec) {
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_REENABLED "id=%d ssid=\"%s\"", ssid->id,
            wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
  }
...
int disallowed_bssid(struct wpa_supplicant *wpa_s, const u8 *bssid) {
  ...
  for (i = 0; i < wpa_s->disallow_aps_bssid_count; i++) {
    if (os_memcmp(wpa_s->disallow_aps_bssid + i * ETH_ALEN, bssid, ETH_ALEN) ==
        0)
      return 1;
  }
...
void dump_freq_data(struct wpa_supplicant *wpa_s, const char *title,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "Shared frequencies (len=%u): %s", len, title);
  for (i = 0; i < len; i++) {
    struct wpa_used_freq_data *cur = &freqs_data[i];
    wpa_dbg(wpa_s, MSG_DEBUG, "freq[%u]: %d, flags=0x%X", i, cur->freq,
            cur->flags);
  }
}
...
int get_shared_radio_freqs_data(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "Determining shared radio frequencies (max len %u)",
          len);
...
static void wpas_rrm_neighbor_rep_timeout_handler(void *data, void *user_ctx) {
  ...
  if (!rrm->notify_neighbor_rep) {
    wpa_printf(MSG_ERROR, "RRM: Unexpected neighbor report timeout");
    return;
  }
...
void wpas_rrm_process_neighbor_rep(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "RRM: Notifying neighbor report (token = %d)",
             report[0]);
  wpa_s->rrm.notify_neighbor_rep(wpa_s->rrm.neighbor_rep_cb_ctx, neighbor_rep);
...
int wpas_rrm_send_neighbor_rep_request(
    struct wpa_supplicant *wpa_s, const struct wpa_ssid_value *ssid, int lci,
    int civic, void (*cb)(void *ctx, struct wpabuf *neighbor_rep),
    void *cb_ctx) {
  struct wpabuf *buf;
  const u8 *rrm_ie;
  ...
  rrm_ie =
      wpa_bss_get_ie(wpa_s->current_bss, WLAN_EID_RRM_ENABLED_CAPABILITIES);
  if (!rrm_ie || !(wpa_s->current_bss->caps & IEEE80211_CAP_RRM) ||
      !(rrm_ie[2] & WLAN_RRM_CAPS_NEIGHBOR_REPORT)) {
    wpa_printf(MSG_DEBUG, "RRM: No network support for Neighbor Report.");
    return -EOPNOTSUPP;
  }
  ...
  if (wpa_s->rrm.notify_neighbor_rep) {
    wpa_printf(MSG_DEBUG, "RRM: Currently handling previous Neighbor Report.");
    return -EBUSY;
  }
  ...
  if (buf == NULL) {
    wpa_printf(MSG_DEBUG, "RRM: Failed to allocate Neighbor Report Request");
    return -ENOMEM;
  }
  ...
  if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                          wpa_s->own_addr, wpa_s->bssid, wpabuf_head(buf),
                          wpabuf_len(buf), 0) < 0) {
    wpa_printf(MSG_DEBUG, "RRM: Failed to send Neighbor Report Request");
    wpabuf_free(buf);
    return -ECANCELED;
  }
  ...
  eloop_register_timeout(RRM_NEIGHBOR_REPORT_TIMEOUT, 0,
                         wpas_rrm_neighbor_rep_timeout_handler, &wpa_s->rrm,
                         NULL);
...
static struct wpabuf *wpas_rrm_build_lci_report(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             "Measurement request token %u type %u location subject %u", token,
             type, subject);
...
void wpas_rrm_handle_radio_measurement_request(struct wpa_supplicant *wpa_s,
  ...
  if (len < 3) {
    wpa_printf(MSG_INFO, "RRM: Ignoring too short radio measurement request");
    return;
  }
  ...
  while ((ie = get_ie(frame, end - frame, WLAN_EID_MEASURE_REQUEST)) &&
    ...
    case MEASURE_TYPE_LCI:
      report = wpas_rrm_build_lci_report(wpa_s, ie + 2, ie[1], report);
      break;
    default:
      wpa_printf(MSG_INFO, "RRM: Unsupported radio measurement request %d",
                 msmt_type);
      break;
    }
  ...
  if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, src, wpa_s->own_addr,
                          wpa_s->bssid, wpabuf_head(buf), wpabuf_len(buf), 0)) {
    wpa_printf(
        MSG_ERROR,
...
void wpas_rrm_handle_link_measurement_request(struct wpa_supplicant *wpa_s,
  ...
  wpa_hexdump(MSG_DEBUG, "RRM: Link measurement report:", wpabuf_head(buf),
              wpabuf_len(buf));
...
#ifdef CONFIG_P2P
  if (wpa_s->parent == wpa_s && wpa_s->global->p2p &&
...
struct hostapd_hw_modes *get_mode(struct hostapd_hw_modes *modes, u16 num_modes,
...
static struct wpa_bss_tmp_disallowed *
wpas_get_disallowed_bss(struct wpa_supplicant *wpa_s, const u8 *bssid) {
...
void wpa_bss_tmp_disallow(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (!bss) {
    wpa_printf(MSG_DEBUG, "Failed to allocate memory for temp disallow BSS");
    return;
  }
...
int wpa_is_bss_tmp_disallowed(struct wpa_supplicant *wpa_s, const u8 *bssid) {
  ...
  wpa_printf(MSG_DEBUG, "BSS " MACSTR " disabled for %ld.%0ld seconds",
