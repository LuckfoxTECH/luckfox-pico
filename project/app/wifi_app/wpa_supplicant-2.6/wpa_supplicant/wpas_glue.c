...
static int wpa_ether_send(struct wpa_supplicant *wpa_s, const u8 *dest,
  ...
  if (wpa_s->ext_eapol_frame_io && proto == ETH_P_EAPOL) {
    ...
    wpa_msg(wpa_s, MSG_INFO, "EAPOL-TX " MACSTR " %s", MAC2STR(dest), hex);
    os_free(hex);
    return 0;
  }
...
static int wpa_supplicant_eapol_send(void *ctx, int type, const u8 *buf,
  ...
  if (pmksa_cache_get_current(wpa_s->wpa) &&
     ...
     * EAP authentication.
     */
    if (!wpa_s->current_ssid || wpa_s->current_ssid->eap_workaround) {
      wpa_printf(MSG_DEBUG,
                 "RSN: Timeout on waiting for the AP to initiate 4-way "
                 "handshake for PMKSA caching or EAP authentication - try to "
                 "force it to start EAP authentication");
    } else {
...
static int wpa_eapol_set_wep_key(void *ctx, int unicast, int keyidx,
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_IEEE8021X_NO_WPA) {
    int cipher = (keylen == 5) ? WPA_CIPHER_WEP40 : WPA_CIPHER_WEP104;
    if (unicast)
      wpa_s->pairwise_cipher = cipher;
    else
      wpa_s->group_cipher = cipher;
  }
  return wpa_drv_set_key(wpa_s, WPA_ALG_WEP, unicast ? wpa_s->bssid : NULL,
...
static void wpa_supplicant_eapol_cb(struct eapol_sm *eapol,
  ...
  wpa_s->eap_expected_failure = result == EAPOL_SUPP_RESULT_EXPECTED_FAILURE;
...
static u8 *_wpa_alloc_eapol(void *wpa_s, u8 type, const void *data,
...
static int wpa_supplicant_set_key(void *_wpa_s, enum wpa_alg alg,
  ...
  return wpa_drv_set_key(wpa_s, alg, addr, key_idx, set_tx, seq, seq_len, key,
                         key_len);
}
...
static int wpa_supplicant_mlme_setprotection(void *wpa_s, const u8 *addr,
                                             int protection_type,
                                             int key_type) {
  return wpa_drv_mlme_setprotection(wpa_s, addr, protection_type, key_type);
}

static int wpa_supplicant_add_pmkid(void *wpa_s, const u8 *bssid,
                                    const u8 *pmkid) {
  return wpa_drv_add_pmkid(wpa_s, bssid, pmkid);
}

static int wpa_supplicant_remove_pmkid(void *wpa_s, const u8 *bssid,
                                       const u8 *pmkid) {
  return wpa_drv_remove_pmkid(wpa_s, bssid, pmkid);
}

#ifdef CONFIG_IEEE80211R
static int wpa_supplicant_update_ft_ies(void *ctx, const u8 *md, const u8 *ies,
                                        size_t ies_len) {
  struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_send_ft_action(void *ctx, u8 action,
                                         const u8 *target_ap, const u8 *ies,
                                         size_t ies_len) {
  struct wpa_supplicant *wpa_s = ctx;
  int ret;
  u8 *data, *pos;
  ...
  if (action != 1) {
    wpa_printf(MSG_ERROR, "Unsupported send_ft_action action %d", action);
    return -1;
  }
  ...
  *pos++ = action;
  ...
  ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                            wpa_s->own_addr, wpa_s->bssid, data, data_len, 0);
  os_free(data);
...
static int wpa_supplicant_send_tdls_mgmt(void *ctx, const u8 *dst,
  ...
  return wpa_drv_send_tdls_mgmt(wpa_s, dst, action_code, dialog_token,
                                status_code, peer_capab, initiator, buf, len);
}
...
static int wpa_supplicant_tdls_peer_addset(
    ...
    const struct ieee80211_vht_capabilities *vht_capab, u8 qosinfo, int wmm,
    const u8 *ext_capab, size_t ext_capab_len, const u8 *supp_channels,
...
static int wpa_supplicant_tdls_enable_channel_switch(
  ...
  return wpa_drv_tdls_enable_channel_switch(wpa_s, addr, oper_class, params);
}

static int wpa_supplicant_tdls_disable_channel_switch(void *ctx,
                                                      const u8 *addr) {
  struct wpa_supplicant *wpa_s = ctx;
...
void wpas_send_ctrl_req(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid,
  ...
  if (buf == NULL)
    return;
  len = os_snprintf(buf, buflen, "%s-%d:%s needed for SSID ", field_name,
                    ssid->id, txt);
...
static void wpa_supplicant_eap_proxy_cb(void *ctx) {
  ...
  wpa_s->mnc_len = eapol_sm_get_eap_proxy_imsi(wpa_s->eapol, wpa_s->imsi, &len);
  if (wpa_s->mnc_len > 0) {
    wpa_s->imsi[len] = '\0';
...
static void wpa_supplicant_cert_cb(void *ctx, int depth, const char *subject,
  ...
  wpas_notify_certification(wpa_s, depth, subject, altsubject, num_altsubject,
                            cert_hash, cert);
}
...
static int wpa_supplicant_key_mgmt_set_pmk(void *ctx, const u8 *pmk,
  ...
  if (wpa_s->conf->key_mgmt_offload &&
    ...
    return wpa_drv_set_key(wpa_s, WPA_ALG_PMK, NULL, 0, 0, NULL, 0, pmk,
                           pmk_len);
  else
    return 0;
}
...
void wpa_supplicant_rsn_supp_set_config(struct wpa_supplicant *wpa_s,
  ...
  if (ssid) {
    ...
    if (ssid->p2p_group && wpa_s->current_bss &&
      ...
      struct wpabuf *p2p;
      p2p = wpa_bss_get_vendor_ie_multi(wpa_s->current_bss, P2P_IE_VENDOR_TYPE);
      if (p2p) {
        u8 group_capab;
        group_capab = p2p_get_group_capab(p2p);
        if (group_capab & P2P_GROUP_CAPAB_IP_ADDR_ALLOCATION)
          conf.p2p = 1;
        wpabuf_free(p2p);
      }
    }
