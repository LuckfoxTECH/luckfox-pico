...
static int ieee80211_11_set_tfs_ie(struct wpa_supplicant *wpa_s, const u8 *addr,
...
int ieee802_11_send_wnmsleep_req(struct wpa_supplicant *wpa_s, u8 action,
                                 u16 intval, struct wpabuf *tfs_req) {
  ...
  enum wnm_oper tfs_oper =
      action == 0 ? WNM_SLEEP_TFS_REQ_IE_ADD : WNM_SLEEP_TFS_REQ_IE_NONE;

  wpa_printf(MSG_DEBUG,
             "WNM: Request to send WNM-Sleep Mode Request "
             "action=%s to " MACSTR,
             action == 0 ? "enter" : "exit", MAC2STR(wpa_s->bssid));
  ...
  wpa_hexdump(MSG_DEBUG, "WNM: WNM-Sleep Mode element", (u8 *)wnmsleep_ie,
              wnmsleep_ie_len);
  ...
  if (tfs_req) {
    ...
    if (ieee80211_11_get_tfs_ie(wpa_s, wnmtfs_ie, &wnmtfs_ie_len, tfs_oper)) {
      wnmtfs_ie_len = 0;
      os_free(wnmtfs_ie);
      wnmtfs_ie = NULL;
    }
  }
  wpa_hexdump(MSG_DEBUG, "WNM: TFS Request element", (u8 *)wnmtfs_ie,
              wnmtfs_ie_len);
  ...
  /* copy TFS IE here */
  if (wnmtfs_ie_len > 0) {
    os_memcpy(mgmt->u.action.u.wnm_sleep_req.variable + wnmsleep_ie_len,
...
static void wnm_sleep_mode_enter_success(struct wpa_supplicant *wpa_s,
  ...
  if (tfsresp_ie_start && tfsresp_ie_end &&
    ...
    u16 tfsresp_ie_len;
    tfsresp_ie_len =
        (tfsresp_ie_end + tfsresp_ie_end[1] + 2) - tfsresp_ie_start;
    wpa_printf(MSG_DEBUG, "TFS Resp IE(s) found");
    /* pass the TFS Resp IE(s) to driver for processing */
    if (ieee80211_11_set_tfs_ie(wpa_s, wpa_s->bssid, tfsresp_ie_start,
                                tfsresp_ie_len, WNM_SLEEP_TFS_RESP_IE_SET))
...
static void wnm_sleep_mode_exit_success(struct wpa_supplicant *wpa_s,
  ...
  wpa_drv_wnm_oper(wpa_s, WNM_SLEEP_EXIT_CONFIRM, wpa_s->bssid, NULL, NULL);
  ...
  while (end - ptr > 1) {
    ...
    if (*ptr == WNM_SLEEP_SUBELEM_GTK) {
      ...
      gtk_len = *(ptr + 4);
      if (ptr[1] < 11 + gtk_len || gtk_len < 5 || gtk_len > 32) {
        wpa_printf(MSG_DEBUG, "WNM: Invalid GTK "
                              "subelem");
        break;
      }
      wpa_wnmsleep_install_key(wpa_s->wpa, WNM_SLEEP_SUBELEM_GTK, ptr);
      ptr += 13 + gtk_len;
      ...
      wpa_wnmsleep_install_key(wpa_s->wpa, WNM_SLEEP_SUBELEM_IGTK, ptr);
      ptr += 10 + WPA_IGTK_LEN;
...
static void ieee802_11_rx_wnmsleep_resp(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->wnmsleep_used) {
    wpa_printf(MSG_DEBUG, "WNM: Ignore WNM-Sleep Mode Response frame since "
                          "WNM-Sleep Mode operation has not been requested");
    return;
  }
  ...
  if (wnmsleep_ie->status == WNM_STATUS_SLEEP_ACCEPT ||
    ...
    wpa_printf(MSG_DEBUG,
               ...
               wnmsleep_ie->action_type, wnmsleep_ie->intval);
    if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_ENTER)
      wpa_drv_wnm_oper(wpa_s, WNM_SLEEP_ENTER_FAIL, wpa_s->bssid, NULL, NULL);
    else if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_EXIT)
      wpa_drv_wnm_oper(wpa_s, WNM_SLEEP_EXIT_FAIL, wpa_s->bssid, NULL, NULL);
  }
}
...
static void wnm_parse_neighbor_report_elem(struct neighbor_report *rep, u8 id,
                                           u8 elen, const u8 *pos) {
  switch (id) {
...
static int wnm_nei_get_chan(struct wpa_supplicant *wpa_s, u8 op_class,
                            u8 chan) {
...
static void wnm_parse_neighbor_report(struct wpa_supplicant *wpa_s,
  ...
  while (left >= 2) {
    ...
    left -= 2;
    if (elen > left) {
      wpa_printf(MSG_DEBUG, "WNM: Truncated neighbor report subelement");
      break;
    }
  ...
  rep->freq =
      wnm_nei_get_chan(wpa_s, rep->regulatory_class, rep->channel_number);
}
...
compare_scan_neighbor_results(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
    ...
    if (nei->preference_present && nei->preference == 0) {
      wpa_printf(MSG_DEBUG, "Skip excluded BSS " MACSTR, MAC2STR(nei->bssid));
      continue;
    }
    ...
    if (!target) {
      wpa_printf(MSG_DEBUG,
                 "Candidate BSS " MACSTR " (pref %d) not found in scan results",
                 MAC2STR(nei->bssid),
    ...
    if (age_secs) {
      ...
      if (os_get_reltime(&now) == 0 &&
          os_reltime_expired(&now, &target->last_update, age_secs)) {
        wpa_printf(MSG_DEBUG, "Candidate BSS is more than %ld seconds old",
                   age_secs);
        continue;
      }
    }
    ...
    if (bss->ssid_len != target->ssid_len ||
       ...
       */
      wpa_printf(
          MSG_DEBUG, "Candidate BSS " MACSTR " (pref %d) in different ESS",
          MAC2STR(nei->bssid), nei->preference_present ? nei->preference : -1);
      continue;
    }

    if (wpa_s->current_ssid &&
        !wpa_scan_res_match(wpa_s, 0, target, wpa_s->current_ssid, 1)) {
      wpa_printf(MSG_DEBUG,
    ...
    if (wpa_is_bss_tmp_disallowed(wpa_s, target->bssid)) {
      wpa_printf(MSG_DEBUG,
                 "MBO: Candidate BSS " MACSTR " retry delay is not over yet",
                 MAC2STR(nei->bssid));
      continue;
    }
    ...
    if (target->level < bss->level && target->level < -80) {
      wpa_printf(MSG_DEBUG,
                 ...
                 MAC2STR(nei->bssid),
                 nei->preference_present ? nei->preference : -1, target->level);
      continue;
    }

    wpa_printf(
        MSG_DEBUG,
        "WNM: Found an acceptable preferred transition candidate BSS " MACSTR
        " (RSSI %d)",
...
static int wnm_add_nei_rep(u8 *buf, size_t len, const u8 *bssid, u32 bss_info,
  ...
  if (len < 18) {
    wpa_printf(MSG_DEBUG, "WNM: Not enough room for Neighbor Report element");
    return -1;
  }
...
static int wnm_nei_rep_add_bss(struct wpa_supplicant *wpa_s,
  ...
  if (ieee80211_freq_to_channel_ext(bss->freq, sec_chan, vht, &op_class,
    ...
    wpa_printf(MSG_DEBUG, "WNM: Cannot determine operating class and channel");
    return -2;
  }

  phy_type =
      ieee80211_get_phy_type(bss->freq, (ht_oper != NULL), (vht_oper != NULL));
  ...
  return wnm_add_nei_rep(buf, len, bss->bssid, info, op_class, chan, phy_type,
                         pref);
}

static int wnm_add_cand_list(struct wpa_supplicant *wpa_s, u8 *buf,
                             size_t len) {
  u8 *pos = buf;
  ...
  wpa_hexdump(MSG_DEBUG,
              "WNM: BSS Transition Management Response candidate list", buf,
              pos - buf);
...
static void
wnm_send_bss_transition_mgmt_resp(struct wpa_supplicant *wpa_s, u8 dialog_token,
  ...
  if (!wpa_s->current_bss) {
    wpa_printf(MSG_DEBUG, "WNM: Current BSS not known - drop response");
    return;
  }
  ...
  if (status != WNM_BSS_TM_ACCEPT) {
    pos +=
        wpas_mbo_ie_bss_trans_reject(wpa_s, pos, buf + sizeof(buf) - pos,
...
static void wnm_bss_tm_connect(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          ...
          " based on BSS Transition Management Request (old BSSID " MACSTR
          " after_new_scan=%d)",
...
int wnm_scan_process(struct wpa_supplicant *wpa_s, int reply_on_fail) {
  ...
  if (os_reltime_before(&wpa_s->wnm_cand_valid_until,
    ...
    wpa_printf(MSG_DEBUG, "WNM: Previously stored BSS transition candidate "
                          "list is not valid anymore - drop it");
    wnm_deallocate_memory(wpa_s);
    return 0;
  }
  ...
  if (!wpa_s->current_bss ||
      os_memcmp(wpa_s->wnm_cand_from_bss, wpa_s->current_bss->bssid,
                ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG, "WNM: Stored BSS transition candidate list not from "
                          "the current BSS - ignore it");
    return 0;
  }
...
send_bss_resp_fail:
  ...
  if (wpa_s->wnm_reply) {
    wpa_s->wnm_reply = 0;
    wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, status, 0,
                                      NULL);
  }
  wnm_deallocate_memory(wpa_s);
...
static void wnm_sort_cand_list(struct wpa_supplicant *wpa_s) {
  if (!wpa_s->wnm_neighbor_report_elements)
    return;
  qsort(wpa_s->wnm_neighbor_report_elements, wpa_s->wnm_num_neighbor_report,
        sizeof(struct neighbor_report), cand_pref_compar);
}
...
static void wnm_dump_cand_list(struct wpa_supplicant *wpa_s) {
  ...
  for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               i, MAC2STR(nei->bssid), nei->bssid_info, nei->regulatory_class,
               nei->channel_number, nei->phy_type,
               nei->preference_present ? nei->preference : -1, nei->freq);
  }
}
...
static int chan_supported(struct wpa_supplicant *wpa_s, int freq) {
  ...
  for (i = 0; i < wpa_s->hw.num_modes; i++) {
    ...
    for (j = 0; j < mode->num_channels; j++) {
      ...
      if (chan->freq == freq && !(chan->flag & HOSTAPD_CHAN_DISABLED))
        return 1;
    }
  }
...
static void wnm_set_scan_freqs(struct wpa_supplicant *wpa_s) {
  ...
  for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
    ...
    if (nei->freq <= 0) {
      wpa_printf(MSG_DEBUG,
                 "WNM: Unknown neighbor operating frequency for " MACSTR
                 " - scan all channels",
                 MAC2STR(nei->bssid));
      os_free(freqs);
      return;
...
static int wnm_fetch_scan_results(struct wpa_supplicant *wpa_s) {
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "WNM: Fetch current scan results from the driver for checking "
    ...
    for (j = 0; j < scan_res->num; j++) {
      ...
      if (bss && ssid_ie &&
          (bss->ssid_len != ssid_ie[1] ||
           os_memcmp(bss->ssid, ssid_ie + 2, bss->ssid_len) != 0))
        continue;
      ...
      wpa_bss_update_scan_res(wpa_s, res, &scan_res->fetch_time);
    }
  }
  ...
  if (!bss) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "WNM: Comparison of scan results against transition candidates did "
            "not find matches");
    return 0;
  }
...
static void ieee802_11_rx_bss_trans_mgmt_req(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             wpa_s->wnm_dialog_token, wpa_s->wnm_mode, wpa_s->wnm_dissoc_timer,
             valid_int);

#if defined(CONFIG_MBO) && defined(CONFIG_TESTING_OPTIONS)
  if (wpa_s->reject_btm_req_reason) {
    wpa_printf(MSG_INFO,
               "WNM: Testing - reject BSS Transition Management Request: "
               "reject_btm_req_reason=%d",
               wpa_s->reject_btm_req_reason);
    wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token,
                                      wpa_s->reject_btm_req_reason, 0, NULL);
    return;
  }
  ...
  if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_PREF_CAND_LIST_INCLUDED) {
    ...
    wpa_s->wnm_neighbor_report_elements =
        os_calloc(WNM_MAX_NEIGHBOR_REPORT, sizeof(struct neighbor_report));
    ...
    while (end - pos >= 2 &&
      ...
      if (tag == WLAN_EID_NEIGHBOR_REPORT) {
        ...
        rep =
            &wpa_s
                 ->wnm_neighbor_report_elements[wpa_s->wnm_num_neighbor_report];
    ...
    if (!wpa_s->wnm_num_neighbor_report) {
      ...
      wnm_send_bss_transition_mgmt_resp(
          ...
          WNM_BSS_TM_REJECT_NO_SUITABLE_CANDIDATES, 0, NULL);
      return;
    }
    ...
    wpa_printf(MSG_DEBUG, "WNM: Candidate list valid for %u ms", valid_ms);
    ...
    if (wpa_s->wnm_num_neighbor_report == 1) {
      os_memcpy(wpa_s->next_scan_bssid,
                wpa_s->wnm_neighbor_report_elements[0].bssid, ETH_ALEN);
      wpa_printf(MSG_DEBUG,
                 "WNM: Scan only for a specific BSSID since there is only a "
                 "single candidate " MACSTR,
                 MAC2STR(wpa_s->next_scan_bssid));
    }
    wpa_supplicant_req_scan(wpa_s, 0, 0);
  } else if (reply) {
    enum bss_trans_mgmt_status_code status;
...
int wnm_send_bss_transition_mgmt_query(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             "WNM: Send BSS Transition Management Query to " MACSTR
             " query_reason=%u%s",
...
static void ieee802_11_rx_wnm_notif_req_wfa(struct wpa_supplicant *wpa_s,
  ...
  while (end - pos > 1) {
    ie = *pos++;
    ie_len = *pos++;
    wpa_printf(MSG_DEBUG, "WNM: WFA subelement %u len %u", ie, ie_len);
    if (ie_len > end - pos) {
    ...
    wpa_printf(MSG_DEBUG, "WNM: Subelement OUI %06x type %u", WPA_GET_BE24(pos),
               pos[3]);

#ifdef CONFIG_HS20
    if (ie == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 5 &&
        WPA_GET_BE24(pos) == OUI_WFA && pos[3] == HS20_WNM_SUB_REM_NEEDED) {
      ...
      if (url_len == 0) {
        ...
        if (url_len + 1 > ie_end - pos) {
          wpa_printf(MSG_DEBUG,
                     "WNM: Not enough room for Server URL (len=%u) and Server "
                     "Method (left %d)",
                     url_len, (int)(ie_end - pos));
          break;
        }
      ...
      hs20_rx_subscription_remediation(wpa_s, url, osu_method);
      os_free(url);
      pos = next;
      continue;
    }
    ...
    if (ie == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 8 &&
      ...
      os_memcpy(url, pos, url_len);
      url[url_len] = '\0';
      hs20_rx_deauth_imminent_notice(wpa_s, code, reauth_delay, url);
      os_free(url);
      pos = next;
      continue;
    }
...
static void ieee802_11_rx_wnm_notif_req(struct wpa_supplicant *wpa_s,
  ...
  wpa_hexdump(MSG_DEBUG, "WNM-Notification Request subelements", pos,
              end - pos);
...
void ieee802_11_rx_wnm_action(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "WNM: RX action %u from " MACSTR, act,
             MAC2STR(mgmt->sa));
  ...
  case WNM_BSS_TRANS_MGMT_REQ:
    ieee802_11_rx_bss_trans_mgmt_req(wpa_s, pos, end, !(mgmt->da[0] & 0x01));
    break;
