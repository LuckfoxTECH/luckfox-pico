...
#define TNAMELEN 255
    ...
    namelen = TNAMELEN;
    datalen = sizeof(data);
    ret = RegEnumValue(bhk, i, name, &namelen, NULL, &type, (LPBYTE)data,
                       &datalen);
    ...
    if (ret != ERROR_SUCCESS) {
      wpa_printf(MSG_DEBUG, "RegEnumValue failed: 0x%x", (unsigned int)ret);
      break;
    }
    ...
    wpa_printf(MSG_MSGDUMP, "blob %d: field='%s' len %d", (int)i, name,
               (int)datalen);
...
static int wpa_config_read_global(struct wpa_config *config, HKEY hk) {
  ...
  wpa_config_read_reg_dword(hk, TEXT("fast_reauth"), &config->fast_reauth);
  wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigPMKLifetime"),
                            (int *)&config->dot11RSNAConfigPMKLifetime);
  wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigPMKReauthThreshold"),
                            (int *)&config->dot11RSNAConfigPMKReauthThreshold);
  wpa_config_read_reg_dword(hk, TEXT("dot11RSNAConfigSATimeout"),
                            (int *)&config->dot11RSNAConfigSATimeout);
  wpa_config_read_reg_dword(hk, TEXT("update_config"), &config->update_config);
  ...
  config->ctrl_interface =
      wpa_config_read_reg_string(hk, TEXT("ctrl_interface"));

#ifdef CONFIG_WPS
  if (wpa_config_read_global_uuid(config, hk))
    errors++;
  config->device_name = wpa_config_read_reg_string(hk, TEXT("device_name"));
  config->manufacturer = wpa_config_read_reg_string(hk, TEXT("manufacturer"));
  config->model_name = wpa_config_read_reg_string(hk, TEXT("model_name"));
  config->serial_number = wpa_config_read_reg_string(hk, TEXT("serial_number"));
  {
    char *t = wpa_config_read_reg_string(hk, TEXT("device_type"));
    if (t && wps_dev_type_str2bin(t, config->device_type))
      errors++;
    os_free(t);
  }
  config->config_methods =
      wpa_config_read_reg_string(hk, TEXT("config_methods"));
  ...
  config->p2p_ssid_postfix =
      wpa_config_read_reg_string(hk, TEXT("p2p_ssid_postfix"));
  ...
  wpa_config_read_reg_dword(hk, TEXT("filter_ssids"), &config->filter_ssids);
...
static struct wpa_ssid *wpa_config_read_network(HKEY hk, const TCHAR *netw,
  ...
  for (i = 0;; i++) {
    ...
    namelen = 255;
    datalen = sizeof(data);
    ret = RegEnumValue(nhk, i, name, &namelen, NULL, &type, (LPBYTE)data,
                       &datalen);
    ...
    if (ret != ERROR_SUCCESS) {
      wpa_printf(MSG_ERROR, "RegEnumValue failed: 0x%x", (unsigned int)ret);
      break;
    }
...
static int wpa_config_read_networks(struct wpa_config *config, HKEY hk) {
  ...
  ret = RegOpenKeyEx(hk, TEXT("networks"), 0, KEY_ENUMERATE_SUB_KEYS, &nhk);
  if (ret != ERROR_SUCCESS) {
  ...
  for (i = 0;; i++) {
    ...
    namelen = 255;
    ret = RegEnumKeyEx(nhk, i, name, &namelen, NULL, NULL, NULL, NULL);

    if (ret == ERROR_NO_MORE_ITEMS)
      break;

    if (ret != ERROR_SUCCESS) {
      wpa_printf(MSG_DEBUG, "RegEnumKeyEx failed: 0x%x", (unsigned int)ret);
      break;
    }
...
struct wpa_config *wpa_config_read(const char *name, struct wpa_config *cfgp) {
  ...
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_ERROR,
               ...
               "configuration registry HKLM\\" TSTR,
               buf);
    os_free(config);
    return NULL;
  }
...
static int wpa_config_write_reg_dword(HKEY hk, const TCHAR *name, int val,
  ...
  ret = RegSetValueEx(hk, name, 0, REG_DWORD, (LPBYTE)&_val, sizeof(_val));
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_ERROR, "WINREG: Failed to set %s=%d: error %d", name, val,
               (int)GetLastError());
    return -1;
  }
...
static int wpa_config_write_global(struct wpa_config *config, HKEY hk) {
  ...
  wpa_config_write_reg_string(hk, "ctrl_interface", config->ctrl_interface);
#endif /* CONFIG_CTRL_IFACE */

  wpa_config_write_reg_dword(hk, TEXT("eapol_version"), config->eapol_version,
                             DEFAULT_EAPOL_VERSION);
  wpa_config_write_reg_dword(hk, TEXT("ap_scan"), config->ap_scan,
                             DEFAULT_AP_SCAN);
  wpa_config_write_reg_dword(hk, TEXT("fast_reauth"), config->fast_reauth,
                             DEFAULT_FAST_REAUTH);
  wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigPMKLifetime"),
                             config->dot11RSNAConfigPMKLifetime, 0);
  wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigPMKReauthThreshold"),
                             config->dot11RSNAConfigPMKReauthThreshold, 0);
  wpa_config_write_reg_dword(hk, TEXT("dot11RSNAConfigSATimeout"),
                             config->dot11RSNAConfigSATimeout, 0);
  wpa_config_write_reg_dword(hk, TEXT("update_config"), config->update_config,
                             0);
#ifdef CONFIG_WPS
  ...
  wpa_config_write_reg_string(hk, "serial_number", config->serial_number);
  {
    char _buf[WPS_DEV_TYPE_BUFSIZE], *buf;
    buf = wps_dev_type_bin2str(config->device_type, _buf, sizeof(_buf));
    wpa_config_write_reg_string(hk, "device_type", buf);
  }
  wpa_config_write_reg_string(hk, "config_methods", config->config_methods);
  if (WPA_GET_BE32(config->os_version)) {
    char vbuf[10];
    os_snprintf(vbuf, sizeof(vbuf), "%08x", WPA_GET_BE32(config->os_version));
...
#endif /* CONFIG_WPS */
#ifdef CONFIG_P2P
  wpa_config_write_reg_string(hk, "p2p_ssid_postfix", config->p2p_ssid_postfix);
  wpa_config_write_reg_dword(hk, TEXT("p2p_group_idle"), config->p2p_group_idle,
                             0);
#endif /* CONFIG_P2P */

  wpa_config_write_reg_dword(hk, TEXT("bss_max_count"), config->bss_max_count,
                             DEFAULT_BSS_MAX_COUNT);
  wpa_config_write_reg_dword(hk, TEXT("filter_ssids"), config->filter_ssids, 0);
  wpa_config_write_reg_dword(hk, TEXT("max_num_sta"), config->max_num_sta,
                             DEFAULT_MAX_NUM_STA);
  ...
  wpa_config_write_reg_dword(hk, TEXT("external_sim"), config->external_sim, 0);
...
static int wpa_config_delete_subkeys(HKEY hk, const TCHAR *key) {
  ...
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_DEBUG,
               ...
               "' for subkey deletion: error 0x%x (%d)",
               key, (unsigned int)ret, (int)GetLastError());
    return 0;
  }
  ...
  for (i = 0;; i++) {
    ...
    if (ret != ERROR_SUCCESS) {
      wpa_printf(MSG_DEBUG, "RegEnumKeyEx failed: 0x%x (%d)", (unsigned int)ret,
                 (int)GetLastError());
      break;
    }
    ...
    if (ret != ERROR_SUCCESS) {
      wpa_printf(MSG_DEBUG, "RegDeleteKey failed: 0x%x (%d)", (unsigned int)ret,
                 (int)GetLastError());
      errors++;
    }
  }
...
#endif /* UNICODE */
  ret = RegCreateKeyEx(nhk, name, 0, NULL, 0, KEY_WRITE, NULL, &netw, NULL);
  RegCloseKey(nhk);
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_DEBUG,
...
#endif /* IEEE8021X_EAPOL */
  INT(mode);
  write_int(netw, "proactive_key_caching", ssid->proactive_key_caching, -1);
  INT(disabled);
  INT(peerkey);
...
static int wpa_config_write_blob(HKEY hk, struct wpa_config_blob *blob) {
  ...
  ret = RegCreateKeyEx(hk, TEXT("blobs"), 0, NULL, 0, KEY_WRITE, NULL, &bhk,
                       NULL);
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_DEBUG,
  ...
  ret = RegSetValueEx(bhk, name, 0, REG_BINARY, blob->data, blob->len);
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_ERROR,
               "WINREG: Failed to set blob %s': "
               "error 0x%x (%d)",
               blob->name, (unsigned int)ret, (int)GetLastError());
    RegCloseKey(bhk);
    os_free(name);
    return -1;
  }
...
int wpa_config_write(const char *name, struct wpa_config *config) {
  ...
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_ERROR,
               "Could not open wpa_supplicant "
               "configuration registry %s: error %d",
               buf, (int)GetLastError());
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG, "Configuration '%s' written %ssuccessfully", name,
             errors ? "un" : "");
  return errors ? -1 : 0;
}
