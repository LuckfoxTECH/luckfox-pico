...
static struct wpa_cred *
interworking_credentials_available_realm(struct wpa_supplicant *wpa_s,
                                         struct wpa_bss *bss, int ignore_bw,
                                         int *excluded);
static struct wpa_cred *
interworking_credentials_available_3gpp(struct wpa_supplicant *wpa_s,
                                        struct wpa_bss *bss, int ignore_bw,
                                        int *excluded);
...
static void interworking_anqp_resp_cb(void *ctx, const u8 *dst, u8 dialog_token,
  ...
  wpa_printf(MSG_DEBUG,
             "ANQP: Response callback dst=" MACSTR
             " dialog_token=%u result=%d status_code=%u",
             MAC2STR(dst), dialog_token, result, status_code);
  anqp_resp_cb(wpa_s, dst, dialog_token, result, adv_proto, resp, status_code);
...
static int cred_with_domain(struct wpa_supplicant *wpa_s) {
  ...
  for (cred = wpa_s->conf->cred; cred; cred = cred->next) {
    if (cred->domain || cred->pcsc || cred->imsi || cred->roaming_partner)
      return 1;
  }
  return 0;
}
...
static int cred_with_min_backhaul(struct wpa_supplicant *wpa_s) {
  ...
  for (cred = wpa_s->conf->cred; cred; cred = cred->next) {
    if (cred->min_dl_bandwidth_home || cred->min_ul_bandwidth_home ||
        cred->min_dl_bandwidth_roaming || cred->min_ul_bandwidth_roaming)
      return 1;
  }
  return 0;
}
...
static int interworking_anqp_send_req(struct wpa_supplicant *wpa_s,
  ...
  wpa_hexdump(MSG_DEBUG, "Interworking: ANQP Query info", (u8 *)info_ids,
              num_info_ids * 2);
  ...
  if (res < 0) {
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "ANQP: Query started with dialog token %u", res);
...
static const u8 *nai_realm_parse_eap(struct nai_realm_eap *e, const u8 *pos,
  ...
  e->method = *pos++;
  auth_count = *pos++;
  wpa_printf(MSG_DEBUG, "EAP Method: len=%u method=%u auth_count=%u", elen,
             e->method, auth_count);
  ...
  for (a = 0; a < auth_count; a++) {
    ...
    len = *pos++;
    if (len > end - pos) {
      wpa_printf(MSG_DEBUG, "No room for Authentication Parameter subfield");
      return NULL;
    }
    ...
    case NAI_REALM_EAP_AUTH_INNER_AUTH_EAP_METHOD:
      if (len < 1)
        break;
      e->inner_method = *pos;
      wpa_printf(MSG_DEBUG, "Inner EAP method: %u", e->inner_method);
      break;
    case NAI_REALM_EAP_AUTH_CRED_TYPE:
      if (len < 1)
        break;
      e->cred_type = *pos;
      wpa_printf(MSG_DEBUG, "Credential Type: %u", e->cred_type);
      break;
    case NAI_REALM_EAP_AUTH_TUNNELED_CRED_TYPE:
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 "Type: %u",
                 e->tunneled_cred_type);
      break;
    default:
      wpa_printf(MSG_DEBUG,
                 "Unsupported Authentication "
                 "Parameter: id=%u len=%u",
                 id, len);
...
static int nai_realm_cred_username(struct wpa_supplicant *wpa_s,
  ...
  if (eap->inner_method && eap->inner_method != EAP_TYPE_GTC &&
...
static int nai_realm_cred_cert(struct wpa_supplicant *wpa_s,
                               struct nai_realm_eap *eap) {
  if (eap_get_name(EAP_VENDOR_IETF, eap->method) == NULL) {
    wpa_msg(wpa_s, MSG_DEBUG, "nai-realm-cred-cert: Method not supported: %d",
            eap->method);
    return 0; /* method not supported */
  }
  ...
  if (eap->method != EAP_TYPE_TLS) {
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "nai-realm-cred-cert: Method not TLS: %d",
            eap->method);
    return 0;
  }
...
static struct nai_realm_eap *nai_realm_find_eap(struct wpa_supplicant *wpa_s,
  ...
  if (cred->username == NULL || cred->username[0] == '\0' ||
      ((cred->password == NULL || cred->password[0] == '\0') &&
       (cred->private_key == NULL || cred->private_key[0] == '\0'))) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "nai-realm-find-eap: incomplete cred info: username: %s  password: "
            "%s private_key: %s",
...
static int plmn_id_match(struct wpabuf *anqp, const char *imsi, int mnc_len) {
  ...
  wpa_printf(MSG_DEBUG,
             "Interworking: Matching against MCC/MNC alternatives: "
             "%02x:%02x:%02x or %02x:%02x:%02x (IMSI %s, MNC length %d)",
             plmn[0], plmn[1], plmn[2], plmn2[0], plmn2[1], plmn2[2], imsi,
             mnc_len);
  ...
  while (end - pos >= 2) {
    ...
    if (iei == 0 && len > 0) {
      /* PLMN List */
      u8 num, i;
      wpa_hexdump(MSG_DEBUG, "Interworking: PLMN List information element", pos,
                  len);
      num = *pos++;
      for (i = 0; i < num; i++) {
        if (l_end - pos < 3)
          break;
        if (os_memcmp(pos, plmn, 3) == 0 || os_memcmp(pos, plmn2, 3) == 0)
          return 1; /* Found matching PLMN */
        pos += 3;
      }
    } else {
      wpa_hexdump(MSG_DEBUG,
                  "Interworking: Unrecognized 3GPP information element", pos,
                  len);
    }
...
static int build_root_nai(char *nai, size_t nai_len, const char *imsi,
  ...
  os_snprintf(pos, end - pos, ".mcc%c%c%c.3gppnetwork.org", imsi[0], imsi[1],
              imsi[2]);
...
static int interworking_connect_3gpp(struct wpa_supplicant *wpa_s,
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "Interworking: Connect with " MACSTR " (3GPP)",
          MAC2STR(bss->bssid));
  ...
  if (res < 0) {
    wpa_msg(wpa_s, MSG_DEBUG, "Selected EAP method (%d) not supported",
            eap_type);
    goto fail;
  }
  ...
  if (cred->milenage && cred->milenage[0]) {
    if (wpa_config_set_quoted(ssid, "password", cred->milenage) < 0)
      goto fail;
  } else if (cred->pcsc) {
    if (wpa_config_set_quoted(ssid, "pcsc", "") < 0)
      goto fail;
    if (wpa_s->conf->pcsc_pin &&
        wpa_config_set_quoted(ssid, "pin", wpa_s->conf->pcsc_pin) < 0)
      goto fail;
  }
...
static int cred_no_required_oi_match(struct wpa_cred *cred,
                                     struct wpa_bss *bss) {
  const u8 *ie;
  ...
  return !roaming_consortium_match(
      ie, bss->anqp ? bss->anqp->roaming_consortium : NULL,
      cred->required_roaming_consortium, cred->required_roaming_consortium_len);
}
...
static int cred_below_min_backhaul(struct wpa_supplicant *wpa_s,
  ...
  res = interworking_home_sp_cred(wpa_s, cred,
                                  bss->anqp ? bss->anqp->domain_name : NULL);
  if (res > 0) {
...
static int cred_conn_capab_missing(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < cred->num_req_conn_capab; i++) {
    ...
    if (!ports) {
      if (!has_proto_match(capab, end, cred->req_conn_capab_proto[i]))
        return 1;
    } else {
      for (j = 0; ports[j] > -1; j++) {
        if (!has_proto_port_match(capab, end, cred->req_conn_capab_proto[i],
                                  ports[j]))
          return 1;
      }
    }
...
static struct wpa_cred *interworking_credentials_available_roaming_consortium(
  ...
  for (cred = wpa_s->conf->cred; cred; cred = cred->next) {
    ...
    if (!roaming_consortium_match(
            ie, bss->anqp ? bss->anqp->roaming_consortium : NULL,
            cred->roaming_consortium, cred->roaming_consortium_len))
      continue;
...
static int interworking_set_eap_params(struct wpa_ssid *ssid,
  ...
  if (cred->eap_method) {
    ...
    ssid->eap.eap_methods = os_malloc(sizeof(struct eap_method_type) * 2);
  ...
  if (ttls && cred->username && cred->username[0]) {
    ...
    if (wpa_config_set_quoted(ssid, "anonymous_identity", anon) < 0) {
      os_free(anon);
      return -1;
    }
    os_free(anon);
  ...
  if (!ttls && cred->username && cred->username[0] && cred->realm &&
    ...
    buflen = os_strlen(cred->username) + 1 + os_strlen(cred->realm) + 1;
  ...
  if (cred->private_key &&
    ...
    if (wpa_config_set_quoted(ssid, "engine_id", "keystore") < 0 ||
        wpa_config_set_quoted(ssid, "key_id", cred->private_key + 11) < 0 ||
...
static int interworking_connect_roaming_consortium(struct wpa_supplicant *wpa_s,
                                                   struct wpa_cred *cred,
                                                   struct wpa_bss *bss,
                                                   int only_add) {
  struct wpa_ssid *ssid;
  ...
  wpa_msg(wpa_s, MSG_DEBUG,
          ...
          " based on roaming consortium match",
          MAC2STR(bss->bssid));
  ...
  if (cred->eap_method == NULL) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: No EAP method set for credential using roaming "
            "consortium");
    goto fail;
  }
...
int interworking_connect(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (disallowed_bssid(wpa_s, bss->bssid) ||
    ...
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Reject connection to disallowed BSS " MACSTR,
            MAC2STR(bss->bssid));
    return -1;
  }

  wpa_printf(MSG_DEBUG,
             "Interworking: Considering BSS " MACSTR " for connection",
             MAC2STR(bss->bssid));
  ...
  if (!wpa_bss_get_ie(bss, WLAN_EID_RSN)) {
     ...
     */
    wpa_msg(wpa_s, MSG_DEBUG, "Interworking: Network does not use RSN");
    return -1;
  }

  cred_rc = interworking_credentials_available_roaming_consortium(wpa_s, bss, 0,
                                                                  excl);
  if (cred_rc) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Highest roaming consortium matching credential "
            "priority %d sp_priority %d",
  ...
  if (cred) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Highest NAI Realm list matching credential priority "
            "%d sp_priority %d",
  ...
  cred_3gpp = interworking_credentials_available_3gpp(wpa_s, bss, 0, excl);
  if (cred_3gpp) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Highest 3GPP matching credential priority %d "
            "sp_priority %d",
  ...
  if (!cred_rc && !cred && !cred_3gpp) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: No full credential matches - consider options "
            "without BW(etc.) limits");
    cred_rc = interworking_credentials_available_roaming_consortium(wpa_s, bss,
                                                                    1, excl);
    if (cred_rc) {
      wpa_msg(wpa_s, MSG_DEBUG,
              "Interworking: Highest roaming consortium matching credential "
              "priority %d sp_priority %d (ignore BW)",
              cred_rc->priority, cred_rc->sp_priority);
      if (excl && !(*excl))
        excl = NULL;
    }

    cred = interworking_credentials_available_realm(wpa_s, bss, 1, excl);
    if (cred) {
      wpa_msg(wpa_s, MSG_DEBUG,
              "Interworking: Highest NAI Realm list matching credential "
              "priority %d sp_priority %d (ignore BW)",
              cred->priority, cred->sp_priority);
      if (excl && !(*excl))
        excl = NULL;
    ...
    cred_3gpp = interworking_credentials_available_3gpp(wpa_s, bss, 1, excl);
    if (cred_3gpp) {
      wpa_msg(wpa_s, MSG_DEBUG,
              "Interworking: Highest 3GPP matching credential priority %d "
  ...
  if (cred_rc && (cred == NULL || cred_prio_cmp(cred_rc, cred) >= 0) &&
    ...
    return interworking_connect_roaming_consortium(wpa_s, cred_rc, bss,
                                                   only_add);

  if (cred_3gpp && (cred == NULL || cred_prio_cmp(cred_3gpp, cred) >= 0)) {
    return interworking_connect_3gpp(wpa_s, cred_3gpp, bss, only_add);
  }

  if (cred == NULL) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: No matching credentials found for " MACSTR,
            MAC2STR(bss->bssid));
    return -1;
  }

  realm = nai_realm_parse(bss->anqp ? bss->anqp->nai_realm : NULL, &count);
  if (realm == NULL) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Could not parse NAI Realm list from " MACSTR,
            MAC2STR(bss->bssid));
    return -1;
  }
  ...
  if (!eap) {
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: No matching credentials and EAP method found "
            "for " MACSTR,
            MAC2STR(bss->bssid));
  ...
  if (wpa_config_set(ssid, "eap", eap_get_name(EAP_VENDOR_IETF, eap->method),
                     0) < 0)
    goto fail;
  ...
  case EAP_TYPE_TTLS:
    ...
    case NAI_REALM_INNER_NON_EAP_PAP:
      if (wpa_config_set(ssid, "phase2", "\"auth=PAP\"", 0) < 0)
        goto fail;
      break;
    case NAI_REALM_INNER_NON_EAP_CHAP:
      if (wpa_config_set(ssid, "phase2", "\"auth=CHAP\"", 0) < 0)
        goto fail;
      break;
    case NAI_REALM_INNER_NON_EAP_MSCHAP:
      if (wpa_config_set(ssid, "phase2", "\"auth=MSCHAP\"", 0) < 0)
        goto fail;
      break;
    case NAI_REALM_INNER_NON_EAP_MSCHAPV2:
      if (wpa_config_set(ssid, "phase2", "\"auth=MSCHAPV2\"", 0) < 0)
        goto fail;
      break;
    default:
  ...
  case EAP_TYPE_PEAP:
  case EAP_TYPE_FAST:
    if (wpa_config_set(ssid, "phase1", "\"fast_provisioning=2\"", 0) < 0)
      goto fail;
    if (wpa_config_set(ssid, "pac_file", "\"blob://pac_interworking\"", 0) < 0)
      goto fail;
    name = eap_get_name(EAP_VENDOR_IETF, eap->inner_method ? eap->inner_method
                                                           : EAP_TYPE_MSCHAPV2);
    if (name == NULL)
      goto fail;
  ...
  if (interworking_set_eap_params(ssid, cred, eap->method == EAP_TYPE_TTLS) < 0)
    goto fail;
...
interworking_credentials_available_3gpp(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->imsi[0]) {
    size_t len;
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: IMSI not available - try to read again through "
            "eap_proxy");
    wpa_s->mnc_len =
  ...
  for (cred = wpa_s->conf->cred; cred; cred = cred->next) {
    ...
    if (sep == NULL || (sep - cred->imsi != 5 && sep - cred->imsi != 6))
      continue;
  ...
  compare:
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "Interworking: Parsing 3GPP info from " MACSTR,
            MAC2STR(bss->bssid));
    ret = plmn_id_match(bss->anqp->anqp_3gpp, imsi, mnc_len);
    wpa_msg(wpa_s, MSG_DEBUG, "PLMN match %sfound", ret ? "" : "not ");
    if (ret) {
      if (cred_no_required_oi_match(cred, bss))
        continue;
      if (!ignore_bw && cred_below_min_backhaul(wpa_s, cred, bss))
...
interworking_credentials_available_realm(struct wpa_supplicant *wpa_s,
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "Interworking: Parsing NAI Realm list from " MACSTR,
          MAC2STR(bss->bssid));
...
static struct wpa_cred *
interworking_credentials_available_helper(struct wpa_supplicant *wpa_s,
                                          struct wpa_bss *bss, int ignore_bw,
                                          int *excluded) {
  ...
  if (disallowed_bssid(wpa_s, bss->bssid) ||
      disallowed_ssid(wpa_s, bss->ssid, bss->ssid_len)) {
    wpa_printf(MSG_DEBUG, "Interworking: Ignore disallowed BSS " MACSTR,
               MAC2STR(bss->bssid));
    return NULL;
  }
...
static struct wpa_cred *
interworking_credentials_available(struct wpa_supplicant *wpa_s,
                                   struct wpa_bss *bss, int *excluded) {
  struct wpa_cred *cred;

  if (excluded)
    *excluded = 0;
  cred = interworking_credentials_available_helper(wpa_s, bss, 0, excluded);
  if (cred)
    return cred;
  return interworking_credentials_available_helper(wpa_s, bss, 1, excluded);
}

int domain_name_list_contains(struct wpabuf *domain_names, const char *domain,
                              int exact_match) {
  const u8 *pos, *end;
  size_t len;
  ...
  while (end - pos > 1) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "Interworking: AP domain name", pos, elen);
    if (elen == len && os_strncasecmp(domain, (const char *)pos, len) == 0)
      return 1;
...
#endif /* CONFIG_EAP_PROXY */
  if (domain_names && imsi &&
      build_root_nai(nai, sizeof(nai), imsi, mnc_len, 0) == 0) {
    realm = os_strchr(nai, '@');
    if (realm)
      realm++;
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Search for match with SIM/USIM domain %s", realm);
    if (realm && domain_name_list_contains(domain_names, realm, 1))
      return 1;
    if (realm)
      ret = 0;
  }
...
static int interworking_find_network_match(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
      if (wpas_network_disabled(wpa_s, ssid) || ssid->mode != WPAS_MODE_INFRA)
        continue;
      if (ssid->ssid_len != bss->ssid_len ||
...
static int roaming_partner_match(struct wpa_supplicant *wpa_s,
                                 ...
                                 struct wpabuf *domain_names) {
  wpa_printf(MSG_DEBUG,
             "Interworking: Comparing roaming_partner info fqdn='%s' "
             "exact_match=%d priority=%u country='%s'",
             partner->fqdn, partner->exact_match, partner->priority,
             partner->country);
  wpa_hexdump_ascii(MSG_DEBUG, "Interworking: Domain names",
                    wpabuf_head(domain_names), wpabuf_len(domain_names));
...
static u8 roaming_prio(struct wpa_supplicant *wpa_s, struct wpa_cred *cred,
  ...
  if (bss->anqp == NULL || bss->anqp->domain_name == NULL) {
    wpa_printf(MSG_DEBUG, "Interworking: No ANQP domain name info -> use "
                          "default roaming partner priority 128");
    return 128; /* cannot check preference with domain name */
  }

  if (interworking_home_sp_cred(wpa_s, cred, bss->anqp->domain_name) > 0) {
    wpa_printf(MSG_DEBUG, "Interworking: Determined to be home SP -> use "
                          "maximum preference 0 as roaming partner priority");
  ...
  for (i = 0; i < cred->num_roaming_partner; i++) {
    if (roaming_partner_match(wpa_s, &cred->roaming_partner[i],
      ...
      wpa_printf(MSG_DEBUG,
                 "Interworking: Roaming partner preference match - priority %u",
                 cred->roaming_partner[i].priority);
      return cred->roaming_partner[i].priority;
    }
  }

  wpa_printf(MSG_DEBUG, "Interworking: No roaming partner preference match - "
                        "use default roaming partner priority 128");
  return 128;
}
...
static struct wpa_bss *pick_best_roaming_partner(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             "Interworking: roaming_prio=%u for selected BSS " MACSTR
             " (cred=%d)",
             best_prio, MAC2STR(selected->bssid), cred->id);
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    ...
    wpa_printf(MSG_DEBUG,
               "Interworking: roaming_prio=%u for BSS " MACSTR " (cred=%d)",
               prio, MAC2STR(bss->bssid), cred2->id);
    if (prio < best_prio) {
      ...
      wpa_printf(MSG_DEBUG, "Interworking: old: %d %d %d  new: %d %d %d", bh1,
                 load1, conn1, bh2, load2, conn2);
      if (bh1 || load1 || conn1 || !(bh2 || load2 || conn2)) {
        wpa_printf(MSG_DEBUG,
                   "Interworking: Better roaming partner " MACSTR " selected",
                   MAC2STR(bss->bssid));
        best_prio = prio;
        selected = bss;
      }
...
static void interworking_select_network(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    ...
    cred = interworking_credentials_available(wpa_s, bss, &excluded);
    if (!cred)
      continue;
    ...
    if (!excluded)
      count++;
    res =
        interworking_home_sp(wpa_s, bss->anqp ? bss->anqp->domain_name : NULL);
    if (res > 0)
      type = "home";
    else if (res == 0)
      type = "roaming";
    ...
    wpa_msg(wpa_s, MSG_INFO,
            "%s" MACSTR " type=%s%s%s%s id=%d priority=%d sp_priority=%d",
            excluded ? INTERWORKING_BLACKLISTED : INTERWORKING_AP,
            MAC2STR(bss->bssid), type, bh ? " below_min_backhaul=1" : "",
            bss_load ? " over_max_bss_load=1" : "",
            conn_capab ? " conn_capab_missing=1" : "", cred->id, cred->priority,
            cred->sp_priority);
    if (excluded)
      continue;
    if (wpa_s->auto_select ||
        (wpa_s->conf->auto_interworking && wpa_s->auto_network_select)) {
      if (bh || bss_load || conn_capab) {
        if (selected2_cred == NULL || cred_prio_cmp(cred, selected2_cred) > 0) {
          wpa_printf(MSG_DEBUG, "Interworking: Mark as selected2");
          selected2 = bss;
          selected2_cred = cred;
        }
        if (res > 0 && (selected2_home_cred == NULL ||
                        cred_prio_cmp(cred, selected2_home_cred) > 0)) {
          wpa_printf(MSG_DEBUG, "Interworking: Mark as selected2_home");
          selected2_home = bss;
          selected2_home_cred = cred;
        }
      } else {
        if (selected_cred == NULL || cred_prio_cmp(cred, selected_cred) > 0) {
          wpa_printf(MSG_DEBUG, "Interworking: Mark as selected");
          selected = bss;
          selected_cred = cred;
        }
        if (res > 0 && (selected_home_cred == NULL ||
                        cred_prio_cmp(cred, selected_home_cred) > 0)) {
  ...
  if (selected_home && selected_home != selected && selected_home_cred &&
      (selected_cred == NULL ||
       cred_prio_cmp(selected_home_cred, selected_cred) >= 0)) {
    /* Prefer network operated by the Home SP */
    wpa_printf(MSG_DEBUG,
               "Interworking: Overrided selected with selected_home");
  ...
  if (!selected) {
    if (selected2_home) {
      wpa_printf(MSG_DEBUG, "Interworking: Use home BSS with BW limit mismatch "
                            "since no other network could be selected");
      selected = selected2_home;
      selected_cred = selected2_home_cred;
    } else if (selected2) {
      wpa_printf(MSG_DEBUG,
                 "Interworking: Use visited BSS with BW limit mismatch since "
                 "no other network could be selected");
      selected = selected2;
  ...
  if (count == 0) {
    ...
    if (interworking_find_network_match(wpa_s)) {
      wpa_msg(wpa_s, MSG_DEBUG,
              "Interworking: Possible BSS match for enabled network "
              "configurations");
      if (wpa_s->auto_select) {
        interworking_reconnect(wpa_s);
        return;
    ...
    wpa_msg(wpa_s, MSG_INFO,
            INTERWORKING_NO_MATCH "No network "
...
interworking_match_anqp_info(struct wpa_supplicant *wpa_s,
                             struct wpa_bss *bss) {
  ...
  dl_list_for_each(other, &wpa_s->bss, struct wpa_bss, list) {
    ...
    if (other->anqp->roaming_consortium == NULL &&
        other->anqp->nai_realm == NULL && other->anqp->anqp_3gpp == NULL &&
    ...
    wpa_msg(wpa_s, MSG_DEBUG,
            "Interworking: Share ANQP data with already fetched BSSID " MACSTR
            " and " MACSTR,
...
static void interworking_next_anqp_fetch(struct wpa_supplicant *wpa_s) {
  ...
  wpa_printf(MSG_DEBUG,
             "Interworking: next_anqp_fetch - "
             "fetch_anqp_in_progress=%d fetch_osu_icon_in_progress=%d",
             wpa_s->fetch_anqp_in_progress, wpa_s->fetch_osu_icon_in_progress);
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    ...
    if (!(bss->flags & WPA_BSS_ANQP_FETCH_TRIED)) {
      if (bss->anqp == NULL) {
        bss->anqp = interworking_match_anqp_info(wpa_s, bss);
        if (bss->anqp) {
      ...
      wpa_msg(wpa_s, MSG_INFO, "Starting ANQP fetch for " MACSTR,
              MAC2STR(bss->bssid));
  ...
  if (found == 0) {
    ...
    if (wpa_s->fetch_osu_info) {
      if (wpa_s->num_prov_found == 0 && wpa_s->fetch_osu_waiting_scan &&
          wpa_s->num_osu_scans < 3) {
        wpa_printf(MSG_DEBUG,
                   "HS 2.0: No OSU providers seen - try to scan again");
...
void interworking_start_fetch_anqp(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) bss->flags &=
...
int anqp_send_req(struct wpa_supplicant *wpa_s, const u8 *dst, u16 info_ids[],
                  size_t num_ids, u32 subtypes, int get_cell_pref) {
  struct wpabuf *buf;
  ...
  if (!bss) {
    wpa_printf(MSG_WARNING, "ANQP: Cannot send query to unknown BSS " MACSTR,
               MAC2STR(dst));
    return -1;
  }
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "ANQP: Query Request to " MACSTR " for %u id(s)",
...
static void anqp_add_extra(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(tmp, &anqp->anqp_elems, struct wpa_bss_anqp_elem, list) {
...
static void interworking_parse_rx_anqp_resp(struct wpa_supplicant *wpa_s,
  ...
  case ANQP_CAPABILITY_LIST:
    wpa_msg(wpa_s, MSG_INFO, RX_ANQP MACSTR " ANQP Capability list",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "ANQP: Capability list", pos, slen);
    if (anqp) {
  ...
  case ANQP_VENUE_NAME:
    wpa_msg(wpa_s, MSG_INFO, RX_ANQP MACSTR " Venue Name", MAC2STR(sa));
  ...
  case ANQP_NETWORK_AUTH_TYPE:
    wpa_msg(wpa_s, MSG_INFO,
            RX_ANQP MACSTR " Network Authentication Type information",
            MAC2STR(sa));
  ...
  case ANQP_ROAMING_CONSORTIUM:
    wpa_msg(wpa_s, MSG_INFO, RX_ANQP MACSTR " Roaming Consortium list",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "ANQP: Roaming Consortium", pos, slen);
    if (anqp) {
  ...
  case ANQP_IP_ADDR_TYPE_AVAILABILITY:
    wpa_msg(wpa_s, MSG_INFO,
            RX_ANQP MACSTR " IP Address Type Availability information",
            MAC2STR(sa));
    wpa_hexdump(MSG_MSGDUMP, "ANQP: IP Address Availability", pos, slen);
    if (anqp) {
      wpabuf_free(anqp->ip_addr_type_availability);
      anqp->ip_addr_type_availability = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case ANQP_NAI_REALM:
    wpa_msg(wpa_s, MSG_INFO, RX_ANQP MACSTR " NAI Realm list", MAC2STR(sa));
  ...
  case ANQP_3GPP_CELLULAR_NETWORK:
    wpa_msg(wpa_s, MSG_INFO,
            RX_ANQP MACSTR " 3GPP Cellular Network information", MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "ANQP: 3GPP Cellular Network", pos, slen);
    if (anqp) {
  ...
  case ANQP_DOMAIN_NAME:
    wpa_msg(wpa_s, MSG_INFO, RX_ANQP MACSTR " Domain Name list", MAC2STR(sa));
  ...
  case ANQP_VENDOR_SPECIFIC:
    ...
    case OUI_WFA:
      ...
      case HS20_ANQP_OUI_TYPE:
        hs20_parse_rx_hs20_anqp_resp(wpa_s, bss, sa, pos, slen, dialog_token);
        break;
      default:
        wpa_msg(wpa_s, MSG_DEBUG, "HS20: Unsupported ANQP vendor type %u",
                type);
        break;
      }
      break;
    ...
    break;
  default:
    wpa_msg(wpa_s, MSG_DEBUG, "Interworking: Unsupported ANQP Info ID %u",
            info_id);
...
void anqp_resp_cb(void *ctx, const u8 *dst, u8 dialog_token,
  ...
  wpa_printf(MSG_DEBUG,
             "Interworking: anqp_resp_cb dst=" MACSTR
             " dialog_token=%u result=%d status_code=%u",
             MAC2STR(dst), dialog_token, result, status_code);
  ...
  if (wpabuf_len(adv_proto) < 4 || pos[0] != WLAN_EID_ADV_PROTO || pos[1] < 2 ||
      pos[3] != ACCESS_NETWORK_QUERY_PROTOCOL) {
    wpa_msg(wpa_s, MSG_DEBUG,
  ...
  while (pos < end) {
    ...
    left -= 4;
    if (left < slen) {
      wpa_msg(wpa_s, MSG_DEBUG, "ANQP: Invalid element length for Info ID %u",
              info_id);
      anqp_result = "INVALID_FRAME";
      goto out_parse_done;
    }
    interworking_parse_rx_anqp_resp(wpa_s, bss, dst, info_id, pos, slen,
                                    dialog_token);
    pos += slen;
  }
...
int gas_send_request(struct wpa_supplicant *wpa_s, const u8 *dst,
  ...
  if (res < 0) {
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "GAS: Query started with dialog token %u", res);
