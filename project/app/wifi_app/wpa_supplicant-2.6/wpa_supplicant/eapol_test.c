...
static void ieee802_1x_encapsulate_radius(struct eapol_test_data *e,
  ...
  msg = radius_msg_new(RADIUS_CODE_ACCESS_REQUEST, e->radius_identifier);
  if (msg == NULL) {
  ...
  if (e->eap_identity &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME, e->eap_identity,
                           e->eap_identity_len)) {
  ...
  if (e->req_eap_key_name &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_EAP_KEY_NAME, (u8 *)"\0", 1)) {
  ...
  if (!find_extra_attr(e->extra_attrs, RADIUS_ATTR_CALLING_STATION_ID) &&
      !radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID, (u8 *)buf,
                           os_strlen(buf))) {
  ...
  if (e->last_recv_radius && radius_msg_get_hdr(e->last_recv_radius)->code ==
                                 RADIUS_CODE_ACCESS_CHALLENGE) {
    int res = radius_msg_copy_attr(msg, e->last_recv_radius, RADIUS_ATTR_STATE);
    if (res < 0) {
  ...
  if (radius_client_send(e->radius, msg, RADIUS_AUTH, e->wpa_s->own_addr) < 0)
    goto fail;
  return;
...
static int eapol_test_compare_pmk(struct eapol_test_data *e) {
  ...
  if (eapol_sm_get_key(e->wpa_s->eapol, pmk, PMK_LEN) == 0) {
    ...
    if (os_memcmp(pmk, e->authenticator_pmk, PMK_LEN) != 0) {
      ...
      wpa_hexdump(MSG_DEBUG, "PMK from AS", e->authenticator_pmk, PMK_LEN);
    ...
    if (os_memcmp(pmk, e->authenticator_pmk, 16) != 0) {
      printf("WARNING: PMK mismatch\n");
      wpa_hexdump(MSG_DEBUG, "PMK from AS", e->authenticator_pmk, 16);
    } else if (e->radius_access_accept_received)
      ret = 0;
  ...
  if (e->authenticator_eap_key_name_len != sess_id_len ||
    ...
    wpa_printf(MSG_INFO, "Locally derived EAP Session-Id does not match "
                         "EAP-Key-Name from server");
...
static void eapol_test_eap_param_needed(void *ctx, enum wpa_ctrl_req_type field,
  ...
  field_name = wpa_supplicant_ctrl_req_to_string(field, default_txt, &txt);
  if (field_name == NULL) {
    wpa_printf(MSG_WARNING, "Unhandled EAP param %d needed", field);
    return;
  }
  ...
  if (buf == NULL)
    return;
  len = os_snprintf(buf, buflen, WPA_CTRL_REQ "%s-%d:%s needed for SSID ",
...
static void eapol_test_cert_cb(void *ctx, int depth, const char *subject,
  ...
  wpa_msg(e->wpa_s, MSG_INFO,
          WPA_EVENT_EAP_PEER_CERT "depth=%d subject='%s'%s%s", depth, subject,
          cert_hash ? " hash=" : "", cert_hash ? cert_hash : "");
  ...
  if (cert) {
    ...
    if (cert_hex) {
      ...
      wpa_msg_ctrl(e->wpa_s, MSG_INFO,
                   WPA_EVENT_EAP_PEER_CERT "depth=%d subject='%s' cert=%s",
                   depth, subject, cert_hex);
      os_free(cert_hex);
    }

    if (e->server_cert_file)
      eapol_test_write_cert(e->server_cert_file, subject, cert);
  }
  ...
  if (altsubject) {
    ...
    for (i = 0; i < num_altsubject; i++)
      wpa_msg(e->wpa_s, MSG_INFO, WPA_EVENT_EAP_PEER_ALT "depth=%d %s", depth,
              altsubject[i]);
  }
}
...
static void eapol_test_set_anon_id(void *ctx, const u8 *id, size_t len) {
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP method updated anonymous_identity", id,
                    len);
  ...
  if (id == NULL) {
    if (wpa_config_set(wpa_s->current_ssid, "anonymous_identity", "NULL", 0) <
        0)
      return;
  } else {
    ...
    res = wpa_config_set(wpa_s->current_ssid, "anonymous_identity", str, 0);
    os_free(str);
    if (res < 0)
      return;
  }
...
static void send_eap_request_identity(void *eloop_ctx, void *timeout_ctx) {
  ...
  *pos = EAP_TYPE_IDENTITY;
  ...
  eapol_sm_rx_eapol(wpa_s->eapol, wpa_s->bssid, buf, sizeof(*hdr) + 5);
}
...
static void ieee802_1x_decapsulate_radius(struct eapol_test_data *e) {
  ...
  case EAP_CODE_REQUEST:
    os_snprintf(buf, sizeof(buf), "EAP-Request-%s (%d)",
                eap_type >= 0 ? eap_type_text(eap_type) : "??", eap_type);
    break;
...

  {
    ...
    os_memcpy((u8 *)(dot1x + 1), wpabuf_head(eap), wpabuf_len(eap));
    eapol_sm_rx_eapol(e->wpa_s->eapol, e->wpa_s->bssid, (u8 *)dot1x,
...
static void ieee802_1x_get_keys(struct eapol_test_data *e,
  ...
  if (keys && keys->send == NULL && keys->recv == NULL) {
    os_free(keys);
    keys =
        radius_msg_get_cisco_keys(msg, req, shared_secret, shared_secret_len);
  }

  if (keys) {
    if (keys->send) {
      wpa_hexdump(MSG_DEBUG, "MS-MPPE-Send-Key (sign)", keys->send,
                  keys->send_len);
    }
    if (keys->recv) {
      wpa_hexdump(MSG_DEBUG, "MS-MPPE-Recv-Key (crypt)", keys->recv,
                  keys->recv_len);
      e->authenticator_pmk_len =
          keys->recv_len > PMK_LEN ? PMK_LEN : keys->recv_len;
      os_memcpy(e->authenticator_pmk, keys->recv, e->authenticator_pmk_len);
      if (e->authenticator_pmk_len == 16 && keys->send &&
        ...
        os_memcpy(e->authenticator_pmk + e->authenticator_pmk_len, keys->send,
                  keys->send_len);
...
static RadiusRxResult ieee802_1x_receive_auth(struct radius_msg *msg,
  ...
  case RADIUS_CODE_ACCESS_ACCEPT:
    ...
    ieee802_1x_get_keys(e, msg, req, shared_secret, shared_secret_len);
    break;
...
static int driver_get_bssid(void *priv, u8 *bssid) {
  ...
  if (e->ctrl_iface && !e->id_req_sent) {
    eloop_register_timeout(0, 0, send_eap_request_identity, e->wpa_s, NULL);
    e->id_req_sent = 1;
  }
...
static void wpa_init_conf(struct eapol_test_data *e,
                          ...
                          int port, const char *secret, const char *cli_addr,
                          const char *ifname) {
  ...
  if (hostapd_parse_ip_addr(authsrv, &as->addr) < 0) {
    wpa_printf(MSG_ERROR, "Invalid IP address '%s'", authsrv);
    assert(0);
  }
  ...
  if (cli_addr) {
    if (hostapd_parse_ip_addr(cli_addr, &e->radius_conf->client_addr) == 0)
      e->radius_conf->force_client_addr = 1;
    else {
      wpa_printf(MSG_ERROR, "Invalid IP address '%s'", cli_addr);
      assert(0);
    }
  }
  ...
  res = radius_client_register(e->radius, RADIUS_AUTH, ieee802_1x_receive_auth,
                               e);
  assert(res == 0);
}
...
#define CK_LEN 16
  ...
  wpa_printf(MSG_DEBUG, "SCARD: MNC length %d", scard_get_mnc_len(scard));
  ...
  os_memcpy(aka_autn,
            ...
            "\xa3\xb3\x97\x9d\x07\x32\xa2\x12",
            16);
...
static int scard_get_triplets(struct eapol_test_data *e, int argc,
                              char *argv[]) {
  struct scard_data *scard;
  size_t len;
  char imsi[20];
...
int main(int argc, char *argv[]) {
  ...
  wpa_init_conf(&eapol_test, &wpa_s, as_addr, as_port, as_secret, cli_addr,
                ifname);
  ...
  if (wpa_s.conf->ssid && wpa_supplicant_scard_init(&wpa_s, wpa_s.conf->ssid))
    return -1;
  ...
  if (!ctrl_iface) {
    eloop_register_timeout(timeout, 0, eapol_test_timeout, &eapol_test, NULL);
  ...
  if (eapol_test_compare_pmk(&eapol_test) == 0 || eapol_test.no_mppe_keys)
    ret = 0;
