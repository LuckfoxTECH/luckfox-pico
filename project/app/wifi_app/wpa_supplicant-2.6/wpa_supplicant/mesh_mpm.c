...
static const char *const mplstate[] = {
    [0] = "UNINITIALIZED",         [PLINK_IDLE] = "IDLE",
    [PLINK_OPN_SNT] = "OPN_SNT",   [PLINK_OPN_RCVD] = "OPN_RCVD",
    [PLINK_CNF_RCVD] = "CNF_RCVD", [PLINK_ESTAB] = "ESTAB",
    [PLINK_HOLDING] = "HOLDING",   [PLINK_BLOCKED] = "BLOCKED"};

static const char *const mplevent[] = {
    [PLINK_UNDEFINED] = "UNDEFINED", [OPN_ACPT] = "OPN_ACPT",
    [OPN_RJCT] = "OPN_RJCT",         [CNF_ACPT] = "CNF_ACPT",
    [CNF_RJCT] = "CNF_RJCT",         [CLS_ACPT] = "CLS_ACPT",
    [REQ_RJCT] = "REQ_RJCT",
};

static int mesh_mpm_parse_peer_mgmt(struct wpa_supplicant *wpa_s,
                                    u8 action_field, const u8 *ie, size_t len,
...
static u16 copy_supp_rates(struct wpa_supplicant *wpa_s, struct sta_info *sta,
  ...
  if (elems->supp_rates_len + elems->ext_supp_rates_len >
    ...
    wpa_msg(wpa_s, MSG_ERROR,
            "Invalid supported rates element length " MACSTR " %d+%d",
            MAC2STR(sta->addr), elems->supp_rates_len,
...
static Boolean matches_local(struct wpa_supplicant *wpa_s,
  ...
  return (mconf->meshid_len == elems->mesh_id_len &&
          os_memcmp(mconf->meshid, elems->mesh_id, elems->mesh_id_len) == 0 &&
...
static void mesh_mpm_send_plink_action(struct wpa_supplicant *wpa_s,
  ...
  if (type != PLINK_CLOSE) {
    ...
    wpabuf_put_u8(buf, MESH_CAP_ACCEPT_ADDITIONAL_PEER | MESH_CAP_FORWARDING);
  ...
  if (ampe) {
    ...
    mesh_rsn_get_pmkid(wpa_s->mesh_rsn, sta, wpabuf_put(buf, PMKID_LEN));
  }
  ...
  if (ampe && mesh_rsn_protect_frame(wpa_s->mesh_rsn, sta, cat, buf)) {
    wpa_msg(wpa_s, MSG_INFO, "Mesh MPM: failed to add AMPE and MIC IE");
    goto fail;
  }

  wpa_msg(wpa_s, MSG_DEBUG,
          "Mesh MPM: Sending peering frame type %d to " MACSTR
          " (my_lid=0x%x peer_lid=0x%x)",
          type, MAC2STR(sta->addr), sta->my_lid, sta->peer_lid);
  ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, sta->addr,
                            wpa_s->own_addr, wpa_s->own_addr, wpabuf_head(buf),
                            wpabuf_len(buf), 0);
  if (ret < 0)
...
void wpa_mesh_set_plink_state(struct wpa_supplicant *wpa_s,
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "MPM set " MACSTR " from %s into %s",
          MAC2STR(sta->addr), mplstate[sta->plink_state], mplstate[state]);
  ...
  case PLINK_HOLDING:
    ...
    if (sta->mesh_sae_pmksa_caching) {
      wpa_printf(MSG_DEBUG,
                 "MPM: Peer " MACSTR
                 " looks like it does not support mesh SAE PMKSA caching, so "
                 "remove the cached entry for it",
                 MAC2STR(sta->addr));
...
static void mesh_mpm_plink_open(struct wpa_supplicant *wpa_s,
                                struct sta_info *sta,
...
static int mesh_mpm_plink_close(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (sta) {
    wpa_mesh_set_plink_state(wpa_s, sta, PLINK_HOLDING);
    mesh_mpm_send_plink_action(wpa_s, sta, PLINK_CLOSE, reason);
    wpa_printf(MSG_DEBUG, "MPM closing plink sta=" MACSTR, MAC2STR(sta->addr));
    eloop_cancel_timeout(plink_timer, wpa_s, sta);
    return 0;
  }
...
int mesh_mpm_connect_peer(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  if ((PLINK_OPN_SNT <= sta->plink_state && sta->plink_state <= PLINK_ESTAB) ||
      (sta->sae && sta->sae->state > SAE_NOTHING)) {
    wpa_msg(wpa_s, MSG_INFO, "Specified peer is connecting/connected");
    return -1;
  }
  ...
  if (conf->security == MESH_CONF_SEC_NONE) {
    ...
    eloop_register_timeout(duration == -1 ? 10 : duration, 0, peer_add_timer,
                           wpa_s, NULL);
  }
...
void mesh_mpm_auth_peer(struct wpa_supplicant *wpa_s, const u8 *addr) {
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "MPM authenticating " MACSTR, MAC2STR(sta->addr));
  ret = wpa_drv_sta_add(wpa_s, &params);
  if (ret) {
    wpa_msg(wpa_s, MSG_ERROR, "Driver failed to set " MACSTR ": %d",
            MAC2STR(sta->addr), ret);
...
static struct sta_info *mesh_mpm_add_peer(struct wpa_supplicant *wpa_s,
  ...
  if (elems->mesh_config_len >= 7 &&
    ...
    wpa_msg(wpa_s, MSG_DEBUG, "mesh: Ignore a crowded peer " MACSTR,
            MAC2STR(addr));
    return NULL;
  }
...
void wpa_mesh_new_mesh_peer(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  if (ssid && ssid->no_auto_peer &&
    ...
    wpa_msg(wpa_s, MSG_INFO,
            "will not initiate new peer link with " MACSTR
            " because of no_auto_peer",
            MAC2STR(addr));
    if (data->mesh_pending_auth) {
      ...
      if (age.sec < 2 && os_memcmp(mgmt->sa, addr, ETH_ALEN) == 0) {
        wpa_printf(MSG_DEBUG,
                   "mesh: Process pending Authentication frame from %u.%06u "
                   "seconds ago",
                   (unsigned int)age.sec, (unsigned int)age.usec);
        os_memset(&fi, 0, sizeof(fi));
        ieee802_11_mgmt(data, wpabuf_head(data->mesh_pending_auth),
                        wpabuf_len(data->mesh_pending_auth), &fi);
      }
...
void mesh_mpm_mgmt_rx(struct wpa_supplicant *wpa_s, struct rx_mgmt *rx_mgmt) {
  ...
  ieee802_11_mgmt(wpa_s->ifmsh->bss[0], rx_mgmt->frame, rx_mgmt->frame_len,
                  &fi);
}
...
static void mesh_mpm_plink_estab(struct wpa_supplicant *wpa_s,
  ...
  if (conf->security & MESH_CONF_SEC_AMPE) {
    ...
    wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->pairwise_cipher), sta->addr,
                    0, 0, seq, sizeof(seq), sta->mtk, sta->mtk_len);

    wpa_hexdump_key(MSG_DEBUG, "mesh: RX MGTK Key RSC", sta->mgtk_rsc,
                    sizeof(sta->mgtk_rsc));
    wpa_hexdump_key(MSG_DEBUG, "mesh: RX MGTK", sta->mgtk, sta->mgtk_len);
    wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->group_cipher), sta->addr,
                    sta->mgtk_key_id, 0, sta->mgtk_rsc, sizeof(sta->mgtk_rsc),
                    sta->mgtk, sta->mgtk_len);

    if (sta->igtk_len) {
      wpa_hexdump_key(MSG_DEBUG, "mesh: RX IGTK Key RSC", sta->igtk_rsc,
                      sizeof(sta->igtk_rsc));
      wpa_hexdump_key(MSG_DEBUG, "mesh: RX IGTK", sta->igtk, sta->igtk_len);
      wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->mgmt_group_cipher),
                      sta->addr, sta->igtk_key_id, 0, sta->igtk_rsc,
                      sizeof(sta->igtk_rsc), sta->igtk, sta->igtk_len);
    }
  }
  ...
  /* Send ctrl event */
  wpa_msg(wpa_s, MSG_INFO, MESH_PEER_CONNECTED MACSTR, MAC2STR(sta->addr));
}
...
static void mesh_mpm_fsm(struct wpa_supplicant *wpa_s, struct sta_info *sta,
  ...
  wpa_msg(wpa_s, MSG_DEBUG, "MPM " MACSTR " state %s event %s",
          MAC2STR(sta->addr), mplstate[sta->plink_state], mplevent[event]);
  ...
  case PLINK_OPN_SNT:
    ...
    case CNF_ACPT:
      ...
      eloop_cancel_timeout(plink_timer, wpa_s, sta);
      eloop_register_timeout(conf->dot11MeshConfirmTimeout / 1000,
  ...
  case PLINK_ESTAB:
    ...
    case CLS_ACPT:
      ...
      wpa_msg(wpa_s, MSG_INFO,
              "mesh plink with " MACSTR " closed with reason %d",
  ...
  case PLINK_HOLDING:
    ...
    break;
  default:
    wpa_msg(wpa_s, MSG_DEBUG, "Unsupported MPM event %s for state %s",
...
void mesh_mpm_action_rx(struct wpa_supplicant *wpa_s,
  ...
  if (ie_len < 2 + 2) {
    wpa_printf(MSG_DEBUG, "MPM: Ignore too short action frame %u ie_len %u",
  ...
  if (action_field == PLINK_OPEN || action_field == PLINK_CONFIRM) {
    wpa_printf(MSG_DEBUG, "MPM: Capability 0x%x", WPA_GET_LE16(ies));
    ies += 2; /* capability */
    ie_len -= 2;
  }
  ...
  if (!elems.peer_mgmt) {
    wpa_printf(MSG_DEBUG, "MPM: No Mesh Peering Management element");
    return;
  }
  if (action_field != PLINK_CLOSE) {
    if (!elems.mesh_id || !elems.mesh_config) {
      wpa_printf(MSG_DEBUG, "MPM: No Mesh ID or Mesh Configuration element");
      return;
    }
  ...
  ret = mesh_mpm_parse_peer_mgmt(wpa_s, action_field, elems.peer_mgmt,
                                 elems.peer_mgmt_len, &peer_mgmt_ie);
  if (ret) {
  ...
  if (mconf->security & MESH_CONF_SEC_AMPE) {
    ...
    res = mesh_rsn_process_ampe(wpa_s, sta, &elems, &mgmt->u.action.category,
                                peer_mgmt_ie.chosen_pmk, ies, ie_len);
    if (res) {
      wpa_printf(MSG_DEBUG, "MPM: RSN process rejected frame (res=%d)", res);
      if (action_field == PLINK_OPEN && res == -2) {
        /* AES-SIV decryption failed */
        mesh_mpm_fsm(wpa_s, sta, OPN_RJCT, WLAN_REASON_MESH_INVALID_GTK);
      }
      return;
    }
  }
  ...
  case PLINK_OPEN:
    if (plink_free_count(hapd) == 0) {
      ...
      wpa_printf(MSG_INFO, "MPM: Peer link num over quota(%d)",
                 hapd->max_plinks);
    } else if (sta->peer_lid && sta->peer_lid != plid) {
      wpa_printf(MSG_DEBUG, "MPM: peer_lid mismatch: 0x%x != 0x%x",
                 sta->peer_lid, plid);
      return; /* no FSM event */
    } else {
      sta->peer_lid = plid;
  ...
  case PLINK_CLOSE:
    if (sta->plink_state == PLINK_ESTAB)
      ...
      wpa_printf(MSG_DEBUG, "MPM: my_lid mismatch: 0x%x != 0x%x", sta->my_lid,
                 llid);
      return; /* no FSM event */
    } else {
      event = CLS_ACPT;
