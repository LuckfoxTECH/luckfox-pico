...
static void wpa_bss_anqp_free(struct wpa_bss_anqp *anqp) {
  ...
  while ((elem = dl_list_first(&anqp->anqp_elems, struct wpa_bss_anqp_elem,
                               list))) {
...
void wpa_bss_remove(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (wpa_s->last_scan_res) {
    ...
    for (i = 0; i < wpa_s->last_scan_res_used; i++) {
      if (wpa_s->last_scan_res[i] == bss) {
        os_memmove(&wpa_s->last_scan_res[i], &wpa_s->last_scan_res[i + 1],
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "BSS: Remove id %u BSSID " MACSTR " SSID '%s' due to %s", bss->id,
          MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len), reason);
...
struct wpa_bss *wpa_bss_get(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    if (os_memcmp(bss->bssid, bssid, ETH_ALEN) == 0 &&
        bss->ssid_len == ssid_len && os_memcmp(bss->ssid, ssid, ssid_len) == 0)
      return bss;
  }
  return NULL;
}
...
static int wpa_bss_in_use(struct wpa_supplicant *wpa_s, struct wpa_bss *bss) {
  ...
  if (wpa_s->current_bss &&
      (bss->ssid_len != wpa_s->current_bss->ssid_len ||
       os_memcmp(bss->ssid, wpa_s->current_bss->ssid, bss->ssid_len) != 0))
...
static int wpa_bss_remove_oldest_unknown(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    if (!wpa_bss_known(wpa_s, bss) && !wpa_bss_is_wps_candidate(wpa_s, bss)) {
...
static struct wpa_bss *wpa_bss_add(struct wpa_supplicant *wpa_s, const u8 *ssid,
                                   size_t ssid_len, struct wpa_scan_res *res,
                                   struct os_reltime *fetch_time) {
  struct wpa_bss *bss;
  ...
  wpa_s->num_bss++;
  wpa_dbg(wpa_s, MSG_DEBUG,
          "BSS: Add new id %u BSSID " MACSTR " SSID '%s' freq %d", bss->id,
...
static struct wpa_bss *wpa_bss_update(struct wpa_supplicant *wpa_s,
                                      struct wpa_bss *bss,
  ...
  if (wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) &&
     ...
     */
    wpa_dbg(wpa_s, MSG_DEBUG,
            "BSS: Do not update scan IEs for " MACSTR
            " since that would remove P2P IE information",
            MAC2STR(bss->bssid));
  } else
#endif /* CONFIG_P2P */
    if (bss->ie_len + bss->beacon_ie_len >= res->ie_len + res->beacon_ie_len) {
      os_memcpy(bss + 1, res + 1, res->ie_len + res->beacon_ie_len);
      bss->ie_len = res->ie_len;
      bss->beacon_ie_len = res->beacon_ie_len;
    } else {
      ...
      dl_list_del(&bss->list_id);
      nbss = os_realloc(bss, sizeof(*bss) + res->ie_len + res->beacon_ie_len);
      if (nbss) {
        unsigned int i;
        for (i = 0; i < wpa_s->last_scan_res_used; i++) {
          if (wpa_s->last_scan_res[i] == bss) {
...
void wpa_bss_update_scan_res(struct wpa_supplicant *wpa_s,
  ...
  if (ssid == NULL) {
    wpa_dbg(wpa_s, MSG_DEBUG, "BSS: No SSID IE included for " MACSTR,
            MAC2STR(res->bssid));
    return;
  }
  if (ssid[1] > SSID_MAX_LEN) {
    wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Too long SSID IE included for " MACSTR,
            MAC2STR(res->bssid));
    return;
  }
...
#ifdef CONFIG_P2P
  if (p2p == NULL && wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE) {
    /*
    ...
    for (i = 0; i < info->num_ssids; i++) {
      ...
      if ((s->ssid == NULL || s->ssid_len == 0) ||
          (s->ssid_len == bss->ssid_len &&
           os_memcmp(s->ssid, bss->ssid, bss->ssid_len) == 0)) {
        found = 1;
        break;
      }
    }
...
void wpa_bss_update_end(struct wpa_supplicant *wpa_s, struct scan_info *info,
  ...
  dl_list_for_each_safe(bss, n, &wpa_s->bss, struct wpa_bss, list) {
    ...
    if (bss->last_update_idx < wpa_s->bss_update_idx)
      bss->scan_miss_count++;
    if (bss->scan_miss_count >= wpa_s->conf->bss_expiration_scan_count) {
 ...
 * @wpa_s: Pointer to wpa_supplicant data
 */
void wpa_bss_deinit(struct wpa_supplicant *wpa_s) { wpa_bss_flush(wpa_s); }
...
struct wpa_bss *wpa_bss_get_p2p_dev_addr(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each_reverse(bss, &wpa_s->bss, struct wpa_bss, list) {
    u8 addr[ETH_ALEN];
    if (p2p_parse_dev_addr((const u8 *)(bss + 1), bss->ie_len, addr) == 0 &&
