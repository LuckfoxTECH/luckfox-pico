...
static void gas_query_done(struct gas_query *gas,
  ...
  wpa_msg(gas->wpa_s, MSG_INFO,
          GAS_QUERY_DONE "addr=" MACSTR
...
void gas_query_deinit(struct gas_query *gas) {
  ...
  dl_list_for_each_safe(query, next, &gas->pending, struct gas_query_pending,
                        list)
...
static void gas_query_tx_status(struct wpa_supplicant *wpa_s, unsigned int freq,
  ...
  if (gas->current == NULL) {
    wpa_printf(MSG_DEBUG,
               "GAS: Unexpected TX status: freq=%u dst=" MACSTR
               " result=%d - no query in progress",
  ...
  if (result == OFFCHANNEL_SEND_ACTION_SUCCESS) {
    eloop_cancel_timeout(gas_query_timeout, gas, query);
    eloop_register_timeout(GAS_QUERY_TIMEOUT_PERIOD, 0, gas_query_timeout, gas,
                           query);
    if (query->wait_comeback && !query->retry) {
      eloop_cancel_timeout(gas_query_rx_comeback_timeout, gas, query);
      eloop_register_timeout(0, (GAS_QUERY_WAIT_TIME_COMEBACK + 10) * 1000,
...
static int pmf_in_use(struct wpa_supplicant *wpa_s, const u8 *addr) {
  if (wpa_s->current_ssid == NULL || wpa_s->wpa_state < WPA_4WAY_HANDSHAKE ||
...
static int gas_query_tx(struct gas_query *gas, struct gas_query_pending *query,
  ...
  wpa_printf(MSG_DEBUG,
             "GAS: Send action frame to " MACSTR " len=%u "
             "freq=%d prot=%d",
             MAC2STR(query->addr), (unsigned int)wpabuf_len(req), query->freq,
             prot);
  if (prot) {
  ...
  if (!gas->wpa_s->conf->gas_address3 ||
      (gas->wpa_s->current_ssid && gas->wpa_s->wpa_state >= WPA_ASSOCIATED &&
       os_memcmp(query->addr, gas->wpa_s->bssid, ETH_ALEN) == 0))
    bssid = query->addr;
  else
    bssid = wildcard_bssid;
  res = offchannel_send_action(
      gas->wpa_s, query->freq, query->addr, gas->wpa_s->own_addr, bssid,
      wpabuf_head(req), wpabuf_len(req), wait_time, gas_query_tx_status, 0);
  if (res == 0)
...
static void gas_query_tx_comeback_req(struct gas_query *gas,
  ...
  wait_time = (query->retry || !query->offchannel_tx_started)
                  ? GAS_QUERY_WAIT_TIME_INITIAL
                  : GAS_QUERY_WAIT_TIME_COMEBACK;

  if (gas_query_tx(gas, query, req, wait_time) < 0) {
    wpa_printf(MSG_DEBUG, "GAS: Failed to send Action frame to " MACSTR,
               MAC2STR(query->addr));
    gas_query_done(gas, query, GAS_QUERY_INTERNAL_ERROR);
  }
...
static void gas_query_tx_comeback_req_delay(struct gas_query *gas,
  ...
  wpa_printf(MSG_DEBUG,
             "GAS: Send comeback request to " MACSTR " in %u secs %u usecs",
             MAC2STR(query->addr), secs, usecs);
  eloop_cancel_timeout(gas_query_tx_comeback_timeout, gas, query);
  eloop_register_timeout(secs, usecs, gas_query_tx_comeback_timeout, gas,
                         query);
}
...
static void gas_query_rx_initial(struct gas_query *gas,
  ...
  wpa_printf(MSG_DEBUG,
             "GAS: Received initial response from " MACSTR
...
static void gas_query_rx_comeback(struct gas_query *gas,
                                  ...
                                  u16 comeback_delay) {
  wpa_printf(MSG_DEBUG,
             "GAS: Received comeback response from " MACSTR
             " (dialog_token=%u frag_id=%u more_frags=%u "
             "comeback_delay=%u)",
             MAC2STR(query->addr), query->dialog_token, frag_id, more_frags,
             comeback_delay);
  eloop_cancel_timeout(gas_query_rx_comeback_timeout, gas, query);
  ...
  if ((size_t)2 + adv_proto[1] != wpabuf_len(query->adv_proto) ||
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "between initial and comeback response from " MACSTR,
               MAC2STR(query->addr));
...
int gas_query_rx(struct gas_query *gas, const u8 *da, const u8 *sa,
  ...
  if (action != WLAN_PA_GAS_INITIAL_RESP && action != WLAN_PA_GAS_COMEBACK_RESP)
  ...
  if (query == NULL) {
    wpa_printf(MSG_DEBUG,
               "GAS: No pending query found for " MACSTR " dialog token %u",
  ...
  if (query->wait_comeback && action == WLAN_PA_GAS_INITIAL_RESP) {
    wpa_printf(MSG_DEBUG,
               "GAS: Unexpected initial response from " MACSTR
  ...
  if (!query->wait_comeback && action == WLAN_PA_GAS_COMEBACK_RESP) {
    wpa_printf(MSG_DEBUG,
               "GAS: Unexpected comeback response from " MACSTR
  ...
  if (query->status_code == WLAN_STATUS_QUERY_RESP_OUTSTANDING &&
    ...
    wpa_printf(MSG_DEBUG,
               "GAS: Query to " MACSTR " dialog token "
  ...
  if (action == WLAN_PA_GAS_COMEBACK_RESP)
    gas_query_rx_comeback(gas, query, adv_proto, pos, resp_len, frag_id,
                          more_frags, comeback_delay);
  else
    gas_query_rx_initial(gas, query, adv_proto, pos, resp_len, comeback_delay);
...
static void gas_query_start_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (wpas_update_random_addr_disassoc(wpa_s) < 0) {
    wpa_msg(wpa_s, MSG_INFO, "Failed to assign random MAC address for GAS");
    gas_query_free(query, 1);
...
static void gas_query_tx_initial_req(struct gas_query *gas,
                                     struct gas_query_pending *query) {
  if (gas_query_tx(gas, query, query->req, GAS_QUERY_WAIT_TIME_INITIAL) < 0) {
    wpa_printf(MSG_DEBUG, "GAS: Failed to send Action frame to " MACSTR,
               MAC2STR(query->addr));
    gas_query_done(gas, query, GAS_QUERY_INTERNAL_ERROR);
    return;
...
static int gas_query_new_dialog_token(struct gas_query *gas, const u8 *dst) {
  ...
  for (dialog_token = 0; dialog_token < 256; dialog_token++) {
    if (gas_query_dialog_token_available(gas, dst,
                                         (next_start + dialog_token) % 256))
      break;
  }
  if (dialog_token == 256)
...
int gas_query_req(struct gas_query *gas, const u8 *dst, int freq,
  ...
  *(wpabuf_mhead_u8(req) + 2) = dialog_token;
  ...
  wpa_msg(gas->wpa_s, MSG_INFO,
          GAS_QUERY_START "addr=" MACSTR " dialog_token=%u freq=%d",
          MAC2STR(query->addr), query->dialog_token, query->freq);
