...
no_vht:
  conf->vht_oper_centr_freq_seg0_idx = channel + conf->secondary_channel * 2;
#else  /* CONFIG_P2P */
...
int wpa_supplicant_conf_ap_ht(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->hw.modes) {
    ...
    if (!no_ht && mode && mode->ht_capab) {
      ...
      if (conf->hw_mode == HOSTAPD_MODE_IEEE80211A &&
          (mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) && ssid->ht40)
        conf->secondary_channel =
            wpas_p2p_get_ht40_mode(wpa_s, mode, conf->channel);
      if (conf->secondary_channel)
      ...
      conf->ht_capab |= mode->ht_capab &
                        (HT_CAP_INFO_GREEN_FIELD | HT_CAP_INFO_SHORT_GI20MHZ |
                         HT_CAP_INFO_SHORT_GI40MHZ | HT_CAP_INFO_RX_STBC_MASK |
                         HT_CAP_INFO_TX_STBC | HT_CAP_INFO_MAX_AMSDU_SIZE);
  ...
  if (conf->secondary_channel) {
    ...
    for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
      if (iface == wpa_s || iface->wpa_state < WPA_AUTHENTICATING ||
...
static int wpa_supplicant_conf_ap(struct wpa_supplicant *wpa_s,
  ...
  if (ssid->pbss > 1) {
    wpa_printf(MSG_ERROR, "Invalid pbss value(%d) for AP mode", ssid->pbss);
    return -1;
  }
  bss->pbss = ssid->pbss;
  ...
  if (ssid->p2p_group) {
    ...
    os_memcpy(bss->ip_addr_mask, wpa_s->p2pdev->conf->ip_addr_mask, 4);
    os_memcpy(bss->ip_addr_start, wpa_s->p2pdev->conf->ip_addr_start, 4);
    os_memcpy(bss->ip_addr_end, wpa_s->p2pdev->conf->ip_addr_end, 4);
  }
#endif /* CONFIG_P2P */
  ...
  if (ssid->psk_set) {
    ...
    for (i = 0; i < NUM_WEP_KEYS; i++) {
      ...
      if (wep->key[i] == NULL)
        return -1;
      os_memcpy(wep->key[i], ssid->wep_key[i], ssid->wep_key_len[i]);
  ...
  if (ssid->mode == WPAS_MODE_P2P_GO ||
    ...
    if (wpa_s->conf->p2p_go_ctwindow > conf->beacon_int) {
      wpa_printf(MSG_INFO,
                 "CTWindow (%d) is bigger than beacon interval (%d) - avoid "
                 "configuring it",
  ...
  if (bss->wpa_group_rekey < 86400 && (bss->wpa & 2) &&
      (bss->wpa_group == WPA_CIPHER_CCMP || bss->wpa_group == WPA_CIPHER_GCMP ||
  ...
  os_memcpy(bss->device_type, wpa_s->conf->device_type, WPS_DEV_TYPE_LEN);
...
no_wps:
  ...
  if (wpa_s->conf->ap_vendor_elements) {
    bss->vendor_elements = wpabuf_dup(wpa_s->conf->ap_vendor_elements);
  }
...
static void ap_public_action_rx(void *ctx, const u8 *buf, size_t len,
                                int freq) {
#ifdef CONFIG_P2P
  ...
  wpas_p2p_rx_action(wpa_s, mgmt->da, mgmt->sa, mgmt->bssid,
                     mgmt->u.action.category, buf + IEEE80211_HDRLEN + 1,
...
static void ap_sta_authorized_cb(void *ctx, const u8 *mac_addr, int authorized,
...
static int ap_probe_req_rx(void *ctx, const u8 *sa, const u8 *da,
  ...
  return wpas_p2p_probe_req_rx(wpa_s, sa, da, bssid, ie, ie_len, freq,
                               ssi_signal);
}
...
int wpa_supplicant_create_ap(struct wpa_supplicant *wpa_s,
  ...
  wpa_s->pairwise_cipher = wpa_pick_pairwise_cipher(ssid->pairwise_cipher, 1);
  ...
  os_memcpy(wpa_s->ap_iface->conf->wmm_ac_params, wpa_s->conf->wmm_ac_params,
  ...
  if (ssid->mode == WPAS_MODE_P2P_GO)
    ...
    conf->bss[0]->p2p = P2P_ENABLED | P2P_GROUP_OWNER | P2P_GROUP_FORMATION;
#endif /* CONFIG_P2P */
  ...
  for (i = 0; i < conf->num_bss; i++) {
    ...
    hostapd_register_probereq_cb(hapd_iface->bss[i], ap_probe_req_rx, wpa_s);
    ...
    hapd_iface->bss[i]->p2p_group = wpas_p2p_group_init(wpa_s, ssid);
#endif /* CONFIG_P2P */
    hapd_iface->bss[i]->setup_complete_cb = wpas_ap_configured_cb;
    hapd_iface->bss[i]->setup_complete_cb_ctx = wpa_s;
#ifdef CONFIG_TESTING_OPTIONS
    hapd_iface->bss[i]->ext_eapol_frame_io = wpa_s->ext_eapol_frame_io;
...
void wpa_supplicant_ap_deinit(struct wpa_supplicant *wpa_s) {
  ...
  wpa_msg(wpa_s, MSG_INFO,
          WPA_EVENT_DISCONNECTED "bssid=" MACSTR
                                 " reason=%d locally_generated=1",
          MAC2STR(wpa_s->own_addr), WLAN_REASON_DEAUTH_LEAVING);
}

void ap_tx_status(void *ctx, const u8 *addr, const u8 *buf, size_t len,
                  int ack) {
#ifdef NEED_AP_MLME
  struct wpa_supplicant *wpa_s = ctx;
...
void ap_eapol_tx_status(void *ctx, const u8 *dst, const u8 *data, size_t len,
                        int ack) {
#ifdef NEED_AP_MLME
  struct wpa_supplicant *wpa_s = ctx;
  if (!wpa_s->ap_iface)
...
void ap_mgmt_rx(void *ctx, struct rx_mgmt *rx_mgmt) {
  ...
  ieee802_11_mgmt(wpa_s->ap_iface->bss[0], rx_mgmt->frame, rx_mgmt->frame_len,
                  &fi);
#endif /* NEED_AP_MLME */
}
...
int wpa_supplicant_ap_wps_pbc(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (!wpa_s->ap_iface)
    return -1;
  return hostapd_wps_button_pushed(wpa_s->ap_iface->bss[0], p2p_dev_addr);
}
...
int wpa_supplicant_ap_wps_cancel(struct wpa_supplicant *wpa_s) {
  ...
  wps_sta = ap_for_each_sta(wpa_s->ap_iface->bss[0], ap_sta_wps_cancel, NULL);
...
int wpa_supplicant_ap_wps_pin(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  ret =
      hostapd_wps_add_pin(wpa_s->ap_iface->bss[0], bssid, "any", pin, timeout);
  if (ret)
    return -1;
  return ret_len;
}
...
static void wpas_wps_ap_pin_enable(struct wpa_supplicant *wpa_s, int timeout) {
  ...
  eloop_cancel_timeout(wpas_wps_ap_pin_timeout, wpa_s, NULL);
  if (timeout > 0)
    eloop_register_timeout(timeout, 0, wpas_wps_ap_pin_timeout, wpa_s, NULL);
}
...
int ap_ctrl_iface_sta_first(struct wpa_supplicant *wpa_s, char *buf,
                            size_t buflen) {
  struct hostapd_data *hapd;
...
int ap_ctrl_iface_sta_disassociate(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->ap_iface == NULL)
    return -1;
  return hostapd_ctrl_iface_disassociate(wpa_s->ap_iface->bss[0], txtaddr);
}
...
int ap_ctrl_iface_sta_deauthenticate(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->ap_iface == NULL)
    return -1;
  return hostapd_ctrl_iface_deauthenticate(wpa_s->ap_iface->bss[0], txtaddr);
}
...
int ap_ctrl_iface_wpa_get_status(struct wpa_supplicant *wpa_s, char *buf,
  ...
  ret = os_snprintf(pos, end - pos,
                    ...
                    wpa_key_mgmt_txt(conf->wpa_key_mgmt, conf->wpa));
...
int wpa_supplicant_ap_update_beacon(struct wpa_supplicant *wpa_s) {
  ...
  if (ssid == NULL || wpa_s->ap_iface == NULL ||
      ssid->mode == WPAS_MODE_INFRA || ssid->mode == WPAS_MODE_IBSS)
    return -1;
  ...
  if (ssid->mode == WPAS_MODE_P2P_GO)
    ...
    iface->conf->bss[0]->p2p =
        P2P_ENABLED | P2P_GROUP_OWNER | P2P_GROUP_FORMATION;
#endif /* CONFIG_P2P */
...
void wpas_ap_ch_switch(struct wpa_supplicant *wpa_s, int freq, int ht,
  ...
  hostapd_event_ch_switch(wpa_s->ap_iface->bss[0], freq, ht, offset, width, cf1,
                          cf2);
}
...
int wpa_supplicant_ap_mac_addr_filter(struct wpa_supplicant *wpa_s,
  ...
  if (addr)
    wpa_printf(MSG_DEBUG, "AP: Set MAC address filter: " MACSTR, MAC2STR(addr));
  else
...
int wpas_ap_wps_add_nfc_pw(struct wpa_supplicant *wpa_s, u16 pw_id,
  ...
  wps->dh_privkey = wpabuf_dup(wpa_s->p2pdev->conf->wps_nfc_dh_privkey);
  wps->dh_pubkey = wpabuf_dup(wpa_s->p2pdev->conf->wps_nfc_dh_pubkey);
...
int wpas_ap_pmksa_cache_list(struct wpa_supplicant *wpa_s, char *buf,
  ...
  if (wpa_s->ap_iface) {
    ...
    for (i = 0; i < wpa_s->ap_iface->num_bss; i++) {
      reply_len += hostapd_ctrl_iface_pmksa_list(
          wpa_s->ap_iface->bss[i], &buf[reply_len], len - reply_len);
    }
  }
  ...
  if (wpa_s->ifmsh) {
    ...
    reply_len += hostapd_ctrl_iface_pmksa_list(
        wpa_s->ifmsh->bss[0], &buf[reply_len], len - reply_len);
  }
...
void wpas_event_dfs_radar_detected(struct wpa_supplicant *wpa_s,
  ...
  hostapd_dfs_radar_detected(wpa_s->ap_iface, radar->freq, radar->ht_enabled,
...
void wpas_event_dfs_cac_started(struct wpa_supplicant *wpa_s,
  ...
  hostapd_dfs_start_cac(wpa_s->ap_iface, radar->freq, radar->ht_enabled,
                        radar->chan_offset, radar->chan_width, radar->cf1,
                        radar->cf2);
}
...
void wpas_event_dfs_cac_finished(struct wpa_supplicant *wpa_s,
  ...
  hostapd_dfs_complete_cac(wpa_s->ap_iface, 1, radar->freq, radar->ht_enabled,
                           radar->chan_offset, radar->chan_width, radar->cf1,
                           radar->cf2);
}
...
void wpas_event_dfs_cac_aborted(struct wpa_supplicant *wpa_s,
  ...
  hostapd_dfs_complete_cac(wpa_s->ap_iface, 0, radar->freq, radar->ht_enabled,
                           radar->chan_offset, radar->chan_width, radar->cf1,
                           radar->cf2);
}
...
void wpas_event_dfs_cac_nop_finished(struct wpa_supplicant *wpa_s,
  ...
  hostapd_dfs_nop_finished(wpa_s->ap_iface, radar->freq, radar->ht_enabled,
                           radar->chan_offset, radar->chan_width, radar->cf1,
                           radar->cf2);
}
#endif /* NEED_AP_MLME */
