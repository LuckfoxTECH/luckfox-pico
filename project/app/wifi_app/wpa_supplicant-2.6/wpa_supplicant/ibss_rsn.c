...
static int supp_ether_send(void *ctx, const u8 *dest, u16 proto, const u8 *buf,
  ...
  wpa_printf(MSG_DEBUG,
             "SUPP: %s(dest=" MACSTR " proto=0x%04x "
             "len=%lu)",
...
static u8 *supp_alloc_eapol(void *ctx, u8 type, const void *data, u16 data_len,
                            size_t *msg_len, void **data_pos) {
  struct ieee802_1x_hdr *hdr;

  wpa_printf(MSG_DEBUG, "SUPP: %s(type=%d data_len=%d)", __func__, type,
             data_len);
...
static void ibss_check_rsn_completed(struct ibss_rsn_peer *peer) {
  ...
  wpa_msg(wpa_s, MSG_INFO, IBSS_RSN_COMPLETED MACSTR, MAC2STR(peer->addr));
}

static int supp_set_key(void *ctx, enum wpa_alg alg, const u8 *addr,
                        int key_idx, int set_tx, const u8 *seq, size_t seq_len,
                        const u8 *key, size_t key_len) {
  struct ibss_rsn_peer *peer = ctx;

  wpa_printf(MSG_DEBUG,
             "SUPP: %s(alg=%d addr=" MACSTR " key_idx=%d "
             "set_tx=%d)",
  ...
  if (key_idx == 0) {
     ...
     */
    if (os_memcmp(peer->ibss_rsn->wpa_s->own_addr, peer->addr, ETH_ALEN) > 0) {
  ...
  if (is_broadcast_ether_addr(addr))
    addr = peer->addr;
  return wpa_drv_set_key(peer->ibss_rsn->wpa_s, alg, addr, key_idx, set_tx, seq,
                         seq_len, key, key_len);
}
...
static int supp_mlme_setprotection(void *ctx, const u8 *addr,
  ...
  wpa_printf(MSG_DEBUG,
             "SUPP: %s(addr=" MACSTR " protection_type=%d "
             "key_type=%d)",
...
static int auth_send_eapol(void *ctx, const u8 *addr, const u8 *data,
  ...
  wpa_printf(MSG_DEBUG,
             "AUTH: %s(addr=" MACSTR " data_len=%lu "
             "encrypt=%d)",
             __func__, MAC2STR(addr), (unsigned long)data_len, encrypt);

  if (wpa_s->l2)
    return l2_packet_send(wpa_s->l2, addr, ETH_P_EAPOL, data, data_len);
...
static int auth_set_key(void *ctx, int vlan_id, enum wpa_alg alg,
  ...
  if (idx == 0) {
    if (addr) {
      ...
      if (peer) {
        peer->authentication_status |= IBSS_RSN_SET_PTK_AUTH;
        ibss_check_rsn_completed(peer);
      }
    }
    /*
  ...
  return wpa_drv_set_key(ibss_rsn->wpa_s, alg, addr, idx, 1, seq, 6, key,
                         key_len);
}
...
static int auth_for_each_sta(void *ctx,
...
static void ibss_set_sta_authorized(struct ibss_rsn *ibss_rsn,
  ...
  if (authorized) {
    res = wpa_drv_sta_set_flags(ibss_rsn->wpa_s, peer->addr, WPA_STA_AUTHORIZED,
                                WPA_STA_AUTHORIZED, ~0);
    wpa_printf(MSG_DEBUG, "AUTH: " MACSTR " authorizing port",
               MAC2STR(peer->addr));
  } else {
    res = wpa_drv_sta_set_flags(ibss_rsn->wpa_s, peer->addr, 0, 0,
                                ~WPA_STA_AUTHORIZED);
  ...
  if (res && errno != ENOENT) {
    wpa_printf(MSG_DEBUG,
               "Could not set station " MACSTR " flags "
               "for kernel driver (errno=%d)",
               MAC2STR(peer->addr), errno);
  }
}

static void auth_set_eapol(void *ctx, const u8 *addr, wpa_eapol_variable var,
                           int value) {
  struct ibss_rsn *ibss_rsn = ctx;
...
static int ibss_rsn_send_auth(struct ibss_rsn *ibss_rsn, const u8 *da,
                              int seq) {
  ...
  auth.frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT, WLAN_FC_STYPE_AUTH);
  ...
  wpa_printf(MSG_DEBUG, "RSN: IBSS TX Auth frame (SEQ %d) to " MACSTR, seq,
             MAC2STR(da));

  return wpa_s->driver->send_frame(wpa_s->drv_priv, (u8 *)&auth, auth_length,
                                   0);
}
...
static struct ibss_rsn_peer *ibss_rsn_peer_init(struct ibss_rsn *ibss_rsn,
  ...
  if (peer) {
    wpa_printf(MSG_DEBUG,
               "RSN: IBSS Supplicant for peer " MACSTR " already running",
               MAC2STR(addr));
    return peer;
  }
  ...
  if (ibss_rsn_supp_init(peer, ibss_rsn->wpa_s->own_addr, ibss_rsn->psk) < 0) {
    ibss_rsn_free(peer);
    return NULL;
  }
...
static void ibss_rsn_auth_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(
      MSG_DEBUG,
      "RSN: Timeout on waiting Authentication frame response from " MACSTR
      " - start authenticator",
      MAC2STR(peer->addr));
...
static int ibss_rsn_peer_authenticated(struct ibss_rsn *ibss_rsn,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "for now-authenticated peer " MACSTR,
             MAC2STR(peer->addr));
...
void ibss_rsn_stop(struct ibss_rsn *ibss_rsn, const u8 *peermac) {
  ...
  if (peermac == NULL) {
    ...
    wpa_printf(MSG_DEBUG, "%s: Remove specific peer " MACSTR, __func__,
               MAC2STR(peermac));
...
static int ibss_rsn_process_rx_eapol(struct ibss_rsn *ibss_rsn,
                                     struct ibss_rsn_peer *peer, const u8 *buf,
                                     size_t len) {
  int supp;
  u8 *tmp;
  ...
  if (supp) {
    ...
    wpa_printf(MSG_DEBUG, "RSN: IBSS RX EAPOL for Supplicant from " MACSTR,
               MAC2STR(peer->addr));
...
int ibss_rsn_rx_eapol(struct ibss_rsn *ibss_rsn, const u8 *src_addr,
  ...
  if (ibss_rsn_eapol_dst_supp(buf, len) > 0) {
    ...
    wpa_printf(MSG_DEBUG, "RSN: IBSS Not using IBSS Auth for peer " MACSTR,
               MAC2STR(src_addr));
    ibss_rsn_peer_authenticated(ibss_rsn, peer, IBSS_RSN_AUTH_EAPOL_BY_US);

    return ibss_rsn_process_rx_eapol(ibss_rsn, ibss_rsn->peers, buf, len);
  }
...
static void ibss_rsn_handle_auth_1_of_2(struct ibss_rsn *ibss_rsn,
  ...
  if (peer && peer->authentication_status & IBSS_RSN_AUTH_EAPOL_BY_PEER) {
...
void ibss_rsn_handle_auth(struct ibss_rsn *ibss_rsn, const u8 *auth_frame,
  ...
  case 1:
    ...
    break;
  case 2:
    wpa_printf(MSG_DEBUG, "RSN: IBSS RX Auth frame (SEQ 2) from " MACSTR,
               MAC2STR(header->sa));
    if (!peer) {
      wpa_printf(MSG_DEBUG,
    ...
    wpa_printf(MSG_DEBUG, "RSN: IBSS Auth completed with " MACSTR,
               MAC2STR(header->sa));
    ibss_rsn_peer_authenticated(ibss_rsn, peer, IBSS_RSN_AUTH_BY_US);
    break;
  }
}
