...
static struct mesh_conf *mesh_config_create(struct wpa_supplicant *wpa_s,
  ...
  if (ssid->key_mgmt & WPA_KEY_MGMT_SAE)
    conf->security |= MESH_CONF_SEC_AUTH | MESH_CONF_SEC_AMPE;
  else
...
static void wpas_mesh_copy_groups(struct hostapd_data *bss,
  ...
  for (num_groups = 0; wpa_s->conf->sae_groups[num_groups] > 0; num_groups++)
    ;
...
static int wpa_supplicant_mesh_init(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->conf->user_mpm) {
    ...
    wpa_msg(wpa_s, MSG_WARNING, "user_mpm is not enabled in configuration");
    return 0;
  }
  ...
  ifmsh->num_bss = 1;
  ifmsh->bss = os_calloc(wpa_s->ifmsh->num_bss, sizeof(struct hostapd_data *));
  if (!ifmsh->bss)
    goto out_free;
  ...
  if (conf->hw_mode == HOSTAPD_MODE_IEEE80211A && ssid->vht) {
    ...
    case VHT_CHANWIDTH_80P80MHZ:
      ieee80211_freq_to_chan(ssid->frequency,
  ...
  if (ssid->mesh_basic_rates == NULL) {
    ...
    if (conf->hw_mode == HOSTAPD_MODE_IEEE80211G) {
      ...
      if (!conf->basic_rates)
        goto out_free;
      os_memcpy(conf->basic_rates, basic_rates_erp, sizeof(basic_rates_erp));
    }
  } else {
    rate_len = 0;
    while (1) {
  ...
  if (hostapd_setup_interface(ifmsh)) {
    wpa_printf(MSG_ERROR, "Failed to initialize hostapd interface for mesh");
    return -1;
  }
  ...
  if (mconf->security != MESH_CONF_SEC_NONE) {
    if (ssid->passphrase == NULL) {
      wpa_printf(MSG_ERROR, "mesh: Passphrase for SAE not configured");
      goto out_free;
    }
    ...
    if (wpa_s->conf->sae_groups && wpa_s->conf->sae_groups[0] > 0) {
      ...
      bss->conf->sae_groups = os_malloc(sizeof(default_groups));
      if (!bss->conf->sae_groups)
        goto out_free;
      os_memcpy(bss->conf->sae_groups, default_groups, sizeof(default_groups));
    }
...
void wpa_mesh_notify_peer(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  wpa_msg(wpa_s, MSG_INFO, "new peer notification for " MACSTR, MAC2STR(addr));
  ...
  if (wpa_s->mesh_vht_enabled) {
    ...
    case 80:
      if (params.freq.center_freq2) {
        ...
        ssid->vht_center_freq2 = params.freq.center_freq2;
      } else {
...
static int mesh_attr_text(const u8 *ies, size_t ies_len, char *buf, char *end) {
  ...
  bss_basic_rate_set =
      os_malloc(elems.supp_rates_len + elems.ext_supp_rates_len);
  ...
  if (bss_basic_rate_set_len > 0) {
    ...
    for (i = 1; i < bss_basic_rate_set_len; i++) {
      ret = os_snprintf(pos, end - pos, " %d", bss_basic_rate_set[i]);
      if (os_snprintf_error(end - pos, ret))
        goto fail;
      pos += ret;
    }
...
static int wpas_mesh_get_ifname(struct wpa_supplicant *wpa_s, char *ifname,
  ...
  res = os_snprintf(ifname, len, "mesh-%s-%d", ifname_ptr, wpa_s->mesh_if_idx);
  if (os_snprintf_error(len, res) ||
      (os_strlen(ifname) >= IFNAMSIZ && os_strlen(wpa_s->ifname) < IFNAMSIZ)) {
...
int wpas_mesh_add_interface(struct wpa_supplicant *wpa_s, char *ifname,
  ...
  if (wpa_drv_if_add(wpa_s, WPA_IF_MESH, ifname, NULL, NULL, NULL, addr, NULL) <
      0) {
    wpa_printf(MSG_ERROR, "mesh: Failed to create new mesh interface");
    return -1;
  }
  wpa_printf(MSG_INFO, "mesh: Created virtual interface %s addr " MACSTR,
             ifname, MAC2STR(addr));
