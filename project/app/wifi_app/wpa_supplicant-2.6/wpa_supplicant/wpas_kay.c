...
static int wpas_get_receive_lowest_pn(void *wpa_s, u32 channel, u8 an,
                                      u32 *lowest_pn) {
  return wpa_drv_get_receive_lowest_pn(wpa_s, channel, an, lowest_pn);
}

static int wpas_get_transmit_next_pn(void *wpa_s, u32 channel, u8 an,
                                     u32 *next_pn) {
  return wpa_drv_get_transmit_next_pn(wpa_s, channel, an, next_pn);
}

static int wpas_set_transmit_next_pn(void *wpa_s, u32 channel, u8 an,
                                     u32 next_pn) {
...
static int wpas_create_receive_sc(void *wpa_s, u32 channel,
  ...
  return wpa_drv_create_receive_sc(wpa_s, channel, sci->addr,
                                   be_to_host16(sci->port), conf_offset_val(co),
                                   vf);
}
...
static int wpas_create_transmit_sc(void *wpa_s, u32 channel,
                                   const struct ieee802_1x_mka_sci *sci,
                                   enum confidentiality_offset co) {
  return wpa_drv_create_transmit_sc(
      wpa_s, channel, sci->addr, be_to_host16(sci->port), conf_offset_val(co));
}
...
static int wpas_create_transmit_sa(void *wpa_s, u32 channel, u8 an, u32 next_pn,
...
void *ieee802_1x_notify_create_actor(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             "IEEE 802.1X: External notification - Create MKA for " MACSTR,
             MAC2STR(peer_addr));
  ...
  if (ieee802_1x_cak_128bits_aes_cmac(msk->key, wpa_s->own_addr, peer_addr,
                                      cak->key)) {
  ...
  if (ieee802_1x_ckn_128bits_aes_cmac(msk->key, wpa_s->own_addr, peer_addr, sid,
                                      sid_len, ckn->name)) {
