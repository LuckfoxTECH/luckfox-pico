...
static int wpa_ether_send(void *wpa_s, const u8 *dest, u16 proto, const u8 *buf,
                          size_t len) {
...
static int wpa_supplicant_set_key(void *wpa_s, enum wpa_alg alg, const u8 *addr,
...
static void eapol_test_poll(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (!rsn_preauth_in_progress(wpa_s->wpa))
    ...
    eloop_register_timeout(0, 100000, eapol_test_poll, eloop_ctx, timeout_ctx);
  }
}
...
static void wpa_init_conf(struct wpa_supplicant *wpa_s, const char *ifname) {
  ...
  l2 = l2_packet_init(wpa_s->ifname, NULL, ETH_P_RSN_PREAUTH, NULL, NULL, 0);
  assert(l2 != NULL);
...
int main(int argc, char *argv[]) {
  ...
  if (preauth_test.auth_timed_out)
    ...
    ret = pmksa_cache_set_current(wpa_s.wpa, NULL, bssid, NULL, 0) ? 0 : -3;
  }
