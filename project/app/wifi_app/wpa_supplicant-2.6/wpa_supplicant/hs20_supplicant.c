...
void hs20_configure_frame_filters(struct wpa_supplicant *wpa_s) {
  ...
  if (ext_capa[3] & BIT(4))
    filter |= WPA_DATA_FRAME_FILTER_FLAG_ARP | WPA_DATA_FRAME_FILTER_FLAG_NA;
...
int hs20_anqp_send_req(struct wpa_supplicant *wpa_s, const u8 *dst, u32 stypes,
  ...
  wpa_printf(MSG_DEBUG,
             "HS20: ANQP Query Request to " MACSTR " for "
             "subtypes 0x%x",
             MAC2STR(dst), stypes);
...
static void hs20_free_icon_entry(struct icon_entry *icon) {
  wpa_printf(MSG_DEBUG,
             "HS20: Free stored icon from " MACSTR
...
int hs20_del_icon(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (!bssid)
    ...
    wpa_printf(MSG_DEBUG, "HS20: Delete all stored icons for " MACSTR,
               MAC2STR(bssid));
  else
    wpa_printf(MSG_DEBUG,
               "HS20: Delete stored icons for " MACSTR " file name %s",
               MAC2STR(bssid), file_name);

  dl_list_for_each_safe(icon, tmp, &wpa_s->icon_head, struct icon_entry, list) {
    if ((!bssid || os_memcmp(icon->bssid, bssid, ETH_ALEN) == 0) &&
        (!file_name || os_strcmp(icon->file_name, file_name) == 0)) {
...
static void hs20_set_osu_access_permission(const char *osu_dir,
  ...
  if (stat(osu_dir, &statbuf) < 0) {
    wpa_printf(MSG_WARNING, "Cannot stat the OSU directory %s", osu_dir);
    return;
  }
...
static int hs20_process_icon_binary_file(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(icon, &wpa_s->icon_head, struct icon_entry, list) {
    if (icon->dialog_token == dialog_token && !icon->image &&
      ...
      wpa_msg(wpa_s, MSG_INFO, RX_HS20_ICON MACSTR " %s %u", MAC2STR(sa),
              icon->file_name, (unsigned int)icon->image_len);
      return 0;
    }
  }
  ...
  if (slen < 4) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "HS 2.0: Too short Icon Binary File "
            "value from " MACSTR,
            MAC2STR(sa));
    return -1;
  }
  ...
  if (wpa_s->osu_icon_id == 0)
    wpa_s->osu_icon_id++;
  snprintf(fname, sizeof(fname), "%s/osu-icon-%u.%s", wpa_s->conf->osu_dir,
           wpa_s->osu_icon_id, png ? "png" : "icon");
  f = fopen(fname, "wb");
  if (f == NULL)
    return -1;
...
static void hs20_osu_icon_fetch_result(struct wpa_supplicant *wpa_s, int res) {
  ...
  wpa_printf(MSG_DEBUG, "HS 2.0: Icon fetch dur=%d ms res=%d", dur, res);
...
void hs20_parse_rx_hs20_anqp_resp(struct wpa_supplicant *wpa_s,
  ...
  case HS20_STYPE_CAPABILITY_LIST:
    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " HS Capability List",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "HS Capability List", pos, slen);
    if (anqp) {
      wpabuf_free(anqp->hs20_capability_list);
      anqp->hs20_capability_list = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case HS20_STYPE_OPERATOR_FRIENDLY_NAME:
    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Operator Friendly Name",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "oper friendly name", pos, slen);
    if (anqp) {
      wpabuf_free(anqp->hs20_operator_friendly_name);
      anqp->hs20_operator_friendly_name = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case HS20_STYPE_WAN_METRICS:
    ...
    if (slen < 13) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              ...
              "Metrics value from " MACSTR,
              MAC2STR(sa));
      break;
    }
    wpa_msg(wpa_s, MSG_INFO,
            RX_HS20_ANQP MACSTR " WAN Metrics %02x:%u:%u:%u:%u:%u", MAC2STR(sa),
            pos[0], WPA_GET_LE32(pos + 1), WPA_GET_LE32(pos + 5), pos[9],
            pos[10], WPA_GET_LE16(pos + 11));
    if (anqp) {
  ...
  case HS20_STYPE_CONNECTION_CAPABILITY:
    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Connection Capability",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "conn capability", pos, slen);
    if (anqp) {
      wpabuf_free(anqp->hs20_connection_capability);
      anqp->hs20_connection_capability = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case HS20_STYPE_OPERATING_CLASS:
    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " Operating Class",
            MAC2STR(sa));
    wpa_hexdump_ascii(MSG_DEBUG, "Operating Class", pos, slen);
    if (anqp) {
      wpabuf_free(anqp->hs20_operating_class);
      anqp->hs20_operating_class = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case HS20_STYPE_OSU_PROVIDERS_LIST:
    wpa_msg(wpa_s, MSG_INFO, RX_HS20_ANQP MACSTR " OSU Providers list",
            MAC2STR(sa));
    wpa_s->num_prov_found++;
    if (anqp) {
      wpabuf_free(anqp->hs20_osu_providers_list);
      anqp->hs20_osu_providers_list = wpabuf_alloc_copy(pos, slen);
    }
    break;
  case HS20_STYPE_ICON_BINARY_FILE:
    ret = hs20_process_icon_binary_file(wpa_s, sa, pos, slen, dialog_token);
    if (wpa_s->fetch_osu_icon_in_progress) {
...
static void hs20_osu_fetch_done(struct wpa_supplicant *wpa_s) {
  ...
  snprintf(fname, sizeof(fname), "%s/osu-providers.txt", wpa_s->conf->osu_dir);
  f = fopen(fname, "w");
  if (f == NULL) {
    wpa_msg(wpa_s, MSG_INFO, "Could not write OSU provider information");
    hs20_free_osu_prov(wpa_s);
    wpa_s->fetch_anqp_in_progress = 0;
    return;
  }
  ...
  for (i = 0; i < wpa_s->osu_prov_count; i++) {
    ...
    if (osu->osu_ssid_len) {
      fprintf(f, "osu_ssid=%s\n",
              wpa_ssid_txt(osu->osu_ssid, osu->osu_ssid_len));
    }
    if (osu->osu_nai[0])
      fprintf(f, "osu_nai=%s\n", osu->osu_nai);
    ...
    for (j = 0; j < osu->icon_count; j++) {
      ...
      fprintf(f, "icon=%u:%u:%u:%s:%s:%s\n", icon->id, icon->width,
              icon->height, icon->lang, icon->icon_type, icon->filename);
    }
  }
  fclose(f);
  hs20_free_osu_prov(wpa_s);
...
void hs20_next_osu_icon(struct wpa_supplicant *wpa_s) {
  ...
  for (i = 0; i < wpa_s->osu_prov_count; i++) {
    ...
    for (j = 0; j < osu->icon_count; j++) {
      ...
      wpa_printf(MSG_DEBUG,
                 "HS 2.0: Try to fetch icon '%s' "
                 "from " MACSTR,
                 icon->filename, MAC2STR(osu->bssid));
      os_get_reltime(&wpa_s->osu_icon_fetch_start);
      if (hs20_anqp_send_req(wpa_s, osu->bssid, BIT(HS20_STYPE_ICON_REQUEST),
                             (u8 *)icon->filename, os_strlen(icon->filename),
                             0) < 0) {
        icon->failed = 1;
        continue;
      }
...
static void hs20_osu_add_prov(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  prov = os_realloc_array(wpa_s->osu_prov, wpa_s->osu_prov_count + 1,
                          sizeof(*prov));
  if (prov == NULL)
    return;
  wpa_s->osu_prov = prov;
  ...
  /* OSU Server URI */
  if (end - pos < 1) {
    wpa_printf(MSG_DEBUG, "HS 2.0: Not enough room for OSU Server URI length");
    return;
  }
  uri_len = *pos++;
  if (uri_len > end - pos) {
...
void hs20_osu_icon_fetch(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    ...
    if (prov_anqp == NULL)
      continue;
    wpa_printf(MSG_DEBUG, "HS 2.0: Parsing OSU Providers list from " MACSTR,
               MAC2STR(bss->bssid));
    wpa_hexdump_buf(MSG_DEBUG, "HS 2.0: OSU Providers list", prov_anqp);
    ...
    num_providers = *pos++;
    wpa_printf(MSG_DEBUG, "HS 2.0: Number of OSU Providers: %u", num_providers);
    ...
    while (end - pos > 2 && num_providers > 0) {
      ...
      if (len > (unsigned int)(end - pos))
        break;
      hs20_osu_add_prov(wpa_s, bss, osu_ssid, osu_ssid_len, pos, len);
      pos += len;
    }

    if (pos != end) {
      wpa_printf(MSG_DEBUG,
                 "HS 2.0: Ignored %d bytes of "
...
int hs20_fetch_osu(struct wpa_supplicant *wpa_s, int skip_scan) {
  ...
  if (wpa_s->fetch_anqp_in_progress || wpa_s->network_select) {
    wpa_printf(MSG_DEBUG,
               "HS 2.0: Cannot start fetch_osu - "
               "fetch in progress (%d, %d)",
               wpa_s->fetch_anqp_in_progress, wpa_s->network_select);
    return -1;
  }
...
void hs20_rx_subscription_remediation(struct wpa_supplicant *wpa_s,
  ...
  if (url)
    wpa_msg(wpa_s, MSG_INFO, HS20_SUBSCRIPTION_REMEDIATION "%u %s", osu_method,
            url);
  else
...
void hs20_rx_deauth_imminent_notice(struct wpa_supplicant *wpa_s, u8 code,
  ...
  if (!wpa_sm_pmf_enabled(wpa_s->wpa)) {
    wpa_printf(MSG_DEBUG, "HS 2.0: Ignore deauthentication imminent notice "
                          "since PMF was not enabled");
    return;
  }

  wpa_msg(wpa_s, MSG_INFO, HS20_DEAUTH_IMMINENT_NOTICE "%u %u %s", code,
          reauth_delay, url);
  ...
  if (code == HS20_DEAUTH_REASON_CODE_BSS) {
    ...
    if (wpa_s->current_ssid) {
      ...
      os_get_reltime(&now);
      if (now.sec + reauth_delay <= wpa_s->current_ssid->disabled_until.sec)
        return;
      wpa_printf(MSG_DEBUG, "HS 2.0: Disable network for %u seconds (BSS)",
                 reauth_delay);
      wpa_s->current_ssid->disabled_until.sec = now.sec + reauth_delay;
    }
  }
