...
static inline int
wpa_drv_associate(struct wpa_supplicant *wpa_s,
...
static inline int
wpa_drv_join_mesh(struct wpa_supplicant *wpa_s,
...
static inline int wpa_drv_get_seqnum(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->get_seqnum)
    return wpa_s->driver->get_seqnum(wpa_s->ifname, wpa_s->drv_priv, addr, idx,
                                     seq);
  return -1;
}
...
static inline int wpa_drv_sta_deauth(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->sta_deauth) {
    return wpa_s->driver->sta_deauth(wpa_s->drv_priv, wpa_s->own_addr, addr,
                                     reason_code);
  }
  return -1;
}
...
wpa_drv_get_hw_feature_data(struct wpa_supplicant *wpa_s, u16 *num_modes,
  ...
  if (wpa_s->driver->get_hw_feature_data)
    return wpa_s->driver->get_hw_feature_data(wpa_s->drv_priv, num_modes,
                                              flags);
  return NULL;
}
...
static inline int wpa_drv_send_mlme(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->send_mlme)
    return wpa_s->driver->send_mlme(wpa_s->drv_priv, data, data_len, noack,
                                    freq, NULL, 0);
  return -1;
}

static inline int wpa_drv_update_ft_ies(struct wpa_supplicant *wpa_s,
                                        const u8 *md, const u8 *ies,
                                        size_t ies_len) {
...
static inline int wpa_drv_hapd_send_eapol(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->hapd_send_eapol)
    return wpa_s->driver->hapd_send_eapol(wpa_s->drv_priv, addr, data, data_len,
...
static inline int wpa_drv_sta_set_flags(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->sta_set_flags)
    return wpa_s->driver->sta_set_flags(wpa_s->drv_priv, addr, total_flags,
                                        flags_or, flags_and);
  return -1;
}
...
static inline void
wpa_drv_send_action_cancel_wait(struct wpa_supplicant *wpa_s) {
...
static inline int wpa_drv_if_add(struct wpa_supplicant *wpa_s,
                                 ...
                                 void *bss_ctx, char *force_ifname, u8 *if_addr,
                                 const char *bridge) {
  if (wpa_s->driver->if_add)
    return wpa_s->driver->if_add(wpa_s->drv_priv, type, ifname, addr, bss_ctx,
                                 NULL, force_ifname, if_addr, bridge, 0, 0);
  return -1;
}
...
static inline int
wpa_drv_cancel_remain_on_channel(struct wpa_supplicant *wpa_s) {
...
static inline int wpa_drv_signal_monitor(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->signal_monitor)
    return wpa_s->driver->signal_monitor(wpa_s->drv_priv, threshold,
                                         hysteresis);
  return -1;
}
...
static inline int wpa_drv_set_ap_wps_ie(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->set_ap_wps_ie)
    return -1;
  return wpa_s->driver->set_ap_wps_ie(wpa_s->drv_priv, beacon, proberesp,
                                      assocresp);
}

static inline int wpa_drv_get_noa(struct wpa_supplicant *wpa_s, u8 *buf,
                                  size_t buf_len) {
...
static inline int wpa_drv_set_p2p_powersave(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->set_p2p_powersave)
    return -1;
  return wpa_s->driver->set_p2p_powersave(wpa_s->drv_priv, legacy_ps, opp_ps,
                                          ctwindow);
}
...
static inline int wpa_drv_send_tdls_mgmt(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->driver->send_tdls_mgmt) {
    return wpa_s->driver->send_tdls_mgmt(wpa_s->drv_priv, dst, action_code,
                                         dialog_token, status_code, peer_capab,
                                         initiator, buf, len);
  }
  return -1;
}
...
#ifdef ANDROID
static inline int wpa_drv_driver_cmd(struct wpa_supplicant *wpa_s, char *cmd,
...
static inline void wpa_drv_set_rekey_info(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->set_rekey_info)
    return;
  wpa_s->driver->set_rekey_info(wpa_s->drv_priv, kek, kek_len, kck, kck_len,
                                replay_ctr);
}
...
static inline int wpa_drv_add_ts(struct wpa_supplicant *wpa_s, u8 tsid,
  ...
  if (!wpa_s->driver->add_tx_ts)
    return -1;
  return wpa_s->driver->add_tx_ts(wpa_s->drv_priv, tsid, address, user_priority,
                                  admitted_time);
}
...
static inline int wpa_drv_wnm_oper(struct wpa_supplicant *wpa_s,
                                   enum wnm_oper oper, const u8 *peer, u8 *buf,
                                   u16 *buf_len) {
...
static inline int wpa_drv_status(struct wpa_supplicant *wpa_s, char *buf,
                                 size_t buflen) {
...
static inline int wpa_drv_vendor_cmd(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->vendor_cmd)
    return -1;
  return wpa_s->driver->vendor_cmd(wpa_s->drv_priv, vendor_id, subcmd, data,
                                   data_len, buf);
}
...
static inline int wpa_drv_get_receive_lowest_pn(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->get_receive_lowest_pn)
    return -1;
  return wpa_s->driver->get_receive_lowest_pn(wpa_s->drv_priv, channel, an,
                                              lowest_pn);
}
...
static inline int wpa_drv_get_transmit_next_pn(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->get_transmit_next_pn)
    return -1;
  return wpa_s->driver->get_transmit_next_pn(wpa_s->drv_priv, channel, an,
                                             next_pn);
}
...
static inline int wpa_drv_set_transmit_next_pn(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->set_transmit_next_pn)
    return -1;
  return wpa_s->driver->set_transmit_next_pn(wpa_s->drv_priv, channel, an,
                                             next_pn);
}
...
static inline int wpa_drv_create_receive_sc(struct wpa_supplicant *wpa_s,
                                            u32 channel, const u8 *sci_addr,
                                            u16 sci_port,
                                            unsigned int conf_offset,
                                            int validation) {
  if (!wpa_s->driver->create_receive_sc)
    return -1;
  return wpa_s->driver->create_receive_sc(wpa_s->drv_priv, channel, sci_addr,
                                          sci_port, conf_offset, validation);
}
...
static inline int wpa_drv_create_receive_sa(struct wpa_supplicant *wpa_s,
                                            u32 channel, u8 an, u32 lowest_pn,
                                            const u8 *sak) {
...
static inline int wpa_drv_create_transmit_sc(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->create_transmit_sc)
    return -1;
  return wpa_s->driver->create_transmit_sc(wpa_s->drv_priv, channel, sci_addr,
                                           sci_port, conf_offset);
}
...
static inline int wpa_drv_get_pref_freq_list(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->get_pref_freq_list)
    return -1;
  return wpa_s->driver->get_pref_freq_list(wpa_s->drv_priv, if_type, num,
                                           freq_list);
}
...
static inline int wpa_drv_get_ext_capa(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->get_ext_capab)
    return -1;
  return wpa_s->driver->get_ext_capab(
      wpa_s->drv_priv, type, &wpa_s->extended_capa, &wpa_s->extended_capa_mask,
      &wpa_s->extended_capa_len);
}

static inline int
wpa_drv_p2p_lo_start(struct wpa_supplicant *wpa_s, unsigned int channel,
                     unsigned int period, unsigned int interval,
                     unsigned int count, const u8 *device_types,
                     size_t dev_types_len, const u8 *ies, size_t ies_len) {
  if (!wpa_s->driver->p2p_lo_start)
    return -1;
  return wpa_s->driver->p2p_lo_start(wpa_s->drv_priv, channel, period, interval,
                                     count, device_types, dev_types_len, ies,
                                     ies_len);
}
