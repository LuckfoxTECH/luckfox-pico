...
static const enum wmm_ac up_to_ac[8] = {WMM_AC_BK, WMM_AC_BE, WMM_AC_BE,
                                        WMM_AC_BK, WMM_AC_VI, WMM_AC_VI,
                                        WMM_AC_VO, WMM_AC_VO};
...
static int wmm_ac_add_ts(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  if (wpa_s->tspecs[ac][idx]) {
    wpa_printf(MSG_ERROR, "WMM AC: tspec (ac=%d, dir=%d) already exists!", ac,
               dir);
    return -1;
  }
  ...
  wpa_msg(wpa_s, MSG_INFO,
          WMM_AC_EVENT_TSPEC_ADDED "tsid=%d addr=" MACSTR " admitted_time=%d",
...
static void wmm_ac_del_ts_idx(struct wpa_supplicant *wpa_s, u8 ac,
  ...
  wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_REMOVED "tsid=%d addr=" MACSTR,
...
static void wmm_ac_addts_req_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(MSG_DEBUG, "Timeout getting ADDTS response (tsid=%d up=%d)",
...
static int wmm_ac_send_addts_request(struct wpa_supplicant *wpa_s,
  ...
  ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, req->address,
                            ...
                            wpabuf_len(buf), 0);
  if (ret) {
    wpa_printf(MSG_WARNING, "WMM AC: Failed to send ADDTS Request");
  }
...
static int wmm_ac_send_delts(struct wpa_supplicant *wpa_s,
  ...
  ret =
      wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, address, wpa_s->own_addr,
...
wmm_ac_build_addts_req(struct wpa_supplicant *wpa_s,
  ...
  if (params->fixed_nominal_msdu)
    tspec->nominal_msdu_size |= host_to_le16(WMM_AC_FIXED_MSDU_SIZE);
...
static int param_in_range(const char *name, long value, long min_val,
                          long max_val) {
  if (value < min_val || (max_val >= 0 && value > max_val)) {
    wpa_printf(MSG_DEBUG, "WMM AC: param %s (%ld) is out of range (%ld-%ld)",
...
static int wmm_ac_should_replace_ts(struct wpa_supplicant *wpa_s, u8 tsid,
                                    u8 ac, u8 dir) {
  enum ts_dir_idx idx;
  ...
  if (cur_ac >= 0) {
    if (cur_ac != ac) {
      wpa_printf(MSG_DEBUG,
                 "WMM AC: TSID %i already exists on different ac (%d)", tsid,
                 cur_ac);
      return -1;
    }
  ...
  case WMM_AC_DIR_UPLINK:
    ...
    replace_ts |= existing_ts & (BIT(TS_DIR_IDX_UPLINK) | BIT(TS_DIR_IDX_BIDI));
    break;
  case WMM_AC_DIR_DOWNLINK:
    /* replace existing downlink/bidi tspecs */
    replace_ts |=
        existing_ts & (BIT(TS_DIR_IDX_DOWNLINK) | BIT(TS_DIR_IDX_BIDI));
    break;
  case WMM_AC_DIR_BIDIRECTIONAL:
...
#define PARAM_IN_RANGE(field, min_value, max_value)                            \
  ...
  if (!PARAM_IN_RANGE(tsid, 0, WMM_AC_MAX_TID) ||
      ...
      !PARAM_IN_RANGE(surplus_bandwidth_allowance, WMM_AC_MIN_SBA_UNITY, -1))
    return 0;
...
wmm_ac_process_param_elem(struct wpa_supplicant *wpa_s, const u8 *ies,
  ...
  for (i = 0; i < WMM_AC_NUM; i++)
    assoc_data->ac_params[i].acm = !!(wmm_params->ac[i].aci_aifsn & WMM_AC_ACM);
...
static int wmm_ac_init(struct wpa_supplicant *wpa_s, const u8 *ies,
  ...
  wpa_printf(MSG_DEBUG, "WMM AC: U-APSD queues=0x%x", wmm_params->uapsd_queues);

  for (ac = 0; ac < WMM_AC_NUM; ac++) {
    assoc_data->ac_params[ac].uapsd = !!(wmm_params->uapsd_queues & BIT(ac));
  }
...
void wmm_ac_notify_assoc(struct wpa_supplicant *wpa_s, const u8 *ies,
  ...
  wpa_printf(MSG_DEBUG, "WMM AC: Valid WMM association, WMM AC is enabled");
}
...
int wpas_wmm_ac_delts(struct wpa_supplicant *wpa_s, u8 tsid) {
  ...
  if (!wpa_s->wmm_ac_assoc_info) {
    wpa_printf(MSG_DEBUG, "WMM AC: Failed to delete TS, WMM AC is disabled");
    return -1;
  }
...
int wpas_wmm_ac_addts(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->wmm_ac_assoc_info) {
    wpa_printf(MSG_DEBUG, "WMM AC: Cannot add TS - missing assoc data");
    return -1;
  }
   ...
   */
  if (params->direction != WMM_AC_DIR_DOWNLINK && !wpa_s->wmm_ac_supported) {
    wpa_printf(MSG_DEBUG, "Cannot set uplink/bidi TS without driver support");
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG,
             ...
             MAC2STR(wpa_s->bssid), params->tsid, params->user_priority,
             params->direction);
  ...
  eloop_register_timeout(1, 0, wmm_ac_addts_req_timeout, wpa_s, addts_req);
  return 0;
err:
  os_free(addts_req);
  return -1;
...
static void wmm_ac_handle_delts(struct wpa_supplicant *wpa_s, const u8 *sa,
  ...
  wpa_printf(MSG_DEBUG,
             "WMM AC: DELTS frame has been received TSID=%u addr=" MACSTR, tsid,
             MAC2STR(sa));

  ac = wmm_ac_find_tsid(wpa_s, tsid, &idx);
  if (ac < 0) {
    wpa_printf(MSG_DEBUG, "WMM AC: Ignoring DELTS frame - TSID does not exist");
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "TS was deleted successfully (tsid=%u address=" MACSTR ")", tsid,
             MAC2STR(sa));
}

static void wmm_ac_handle_addts_resp(struct wpa_supplicant *wpa_s, const u8 *sa,
                                     const u8 resp_dialog_token,
  ...
  if (!req || req->dialog_token != resp_dialog_token) {
    wpa_printf(MSG_DEBUG, "WMM AC: no req with dialog=%u, ignoring frame",
               resp_dialog_token);
    return;
  }
  ...
  if (os_memcmp(req->address, sa, ETH_ALEN) != 0 ||
    ...
    wpa_printf(MSG_DEBUG, "WMM AC: ADDTS params do not match, ignoring frame");
    return;
  }
  ...
  wpa_printf(MSG_DEBUG, "ADDTS response status=%d tsid=%u up=%u direction=%u",
  ...
  wpa_printf(MSG_DEBUG,
             ...
             tsid, MAC2STR(sa), le_to_host16(tspec->medium_time), ac, dir);
...
err_msg:
  wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_REQ_FAILED "tsid=%u", tsid);
}

void wmm_ac_rx_action(struct wpa_supplicant *wpa_s, const u8 *da, const u8 *sa,
  ...
  if (wpa_s->wmm_ac_assoc_info == NULL) {
    wpa_printf(MSG_DEBUG, "WMM AC: WMM AC is disabled, ignoring action frame");
    return;
  }

  action = data[0];

  if (action != WMM_ACTION_CODE_ADDTS_RESP && action != WMM_ACTION_CODE_DELTS) {
    wpa_printf(MSG_DEBUG, "WMM AC: Unknown action (%d), ignoring action frame",
               action);
    return;
  }
  ...
  if (os_memcmp(da, wpa_s->own_addr, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               ...
               " is other than ours, ignoring frame",
               MAC2STR(da));
    return;
  }
  ...
  if (os_memcmp(sa, wpa_s->bssid, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               ...
               " different other than our bssid",
               MAC2STR(da));
    return;
  }

  if (len < 2 + sizeof(struct wmm_tspec_element)) {
    wpa_printf(MSG_DEBUG, "WMM AC: Short ADDTS response ignored (len=%lu)",
               (unsigned long)len);
    return;
  }
  ...
  if (ieee802_11_parse_elems(data + 2, len - 2, &elems, 1) != ParseOK) {
    wpa_printf(MSG_DEBUG, "WMM AC: Could not parse WMM AC action from " MACSTR,
               MAC2STR(sa));
    return;
  }
  ...
  wpa_printf(MSG_DEBUG, "WMM AC: RX WMM AC Action from " MACSTR, MAC2STR(sa));
  wpa_hexdump(MSG_MSGDUMP, "WMM AC: WMM AC Action content", data, len);

  switch (action) {
  case WMM_ACTION_CODE_ADDTS_RESP:
    wmm_ac_handle_addts_resp(wpa_s, sa, dialog_token, status_code, tspec);
    break;
...
int wpas_wmm_ac_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen) {
  ...
  for (ac = 0; ac < WMM_AC_NUM; ac++) {
    ...
    pos += wpa_scnprintf(buf + pos, buflen - pos, "%s: acm=%d uapsd=%d\n",
                         get_ac_str(ac), assoc_info->ac_params[ac].acm,
    ...
    for (idx = 0; idx < TS_DIR_IDX_COUNT; idx++) {
      ...
      pos += wpa_scnprintf(buf + pos, buflen - pos,
                           ...
                           tsid, up, MAC2STR(wpa_s->bssid), dir_str,
                           le_to_host16(tspec->medium_time));
    }

    if (!ts_count) {
      pos +=
          wpa_scnprintf(buf + pos, buflen - pos, "\t(No Traffic Stream)\n\n");
    }
  }
...
void wmm_ac_save_tspecs(struct wpa_supplicant *wpa_s) {
  ...
  wpa_s->last_tspecs = os_calloc(tspecs_count, sizeof(*wpa_s->last_tspecs));
  ...
  for (ac = 0; ac < WMM_AC_NUM; ac++) {
    for (dir = 0; dir < TS_DIR_IDX_COUNT; dir++) {
      ...
      wpa_s->last_tspecs[wpa_s->last_tspecs_count++] = *wpa_s->tspecs[ac][dir];
    }
  }
