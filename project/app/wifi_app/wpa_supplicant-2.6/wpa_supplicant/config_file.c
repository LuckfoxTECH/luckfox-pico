...
static int wpa_config_validate_network(struct wpa_ssid *ssid, int line) {
  ...
  if (ssid->mode == WPAS_MODE_MESH && (ssid->key_mgmt != WPA_KEY_MGMT_NONE &&
...
static struct wpa_config_blob *wpa_config_read_blob(FILE *f, int *line,
  ...
  wpa_printf(MSG_MSGDUMP, "Line: %d - start of a new named blob '%s'", *line,
             name);
...
static int wpa_config_process_blob(struct wpa_config *config, FILE *f,
  ...
  if (name_end == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: no blob name terminator", *line);
    return -1;
  }
  *name_end = '\0';
  ...
  if (blob == NULL) {
    wpa_printf(MSG_ERROR, "Line %d: failed to read blob %s", *line, bname);
    return -1;
  }
...
struct wpa_config *wpa_config_read(const char *name, struct wpa_config *cfgp) {
  ...
  while (wpa_config_get_line(buf, sizeof(buf), f, &line, &pos)) {
    if (os_strcmp(pos, "network={") == 0) {
      ...
      if (wpa_config_process_blob(config, f, &line, pos + 12) < 0) {
        wpa_printf(MSG_ERROR,
                   "Line %d: failed to "
                   "process blob.",
                   line);
...
#define INT_DEFe(t, def) write_int(f, #t, ssid->eap.t, def)
  ...
  write_int(f, "ieee80211w", ssid->ieee80211w, MGMT_FRAME_PROTECTION_DEFAULT);
...
static void wpa_config_write_cred(FILE *f, struct wpa_cred *cred) {
  ...
  if (cred->eap_method) {
    const char *name;
    name = eap_get_name(cred->eap_method[0].vendor, cred->eap_method[0].method);
    if (name)
      fprintf(f, "\teap=%s\n", name);
  }
  if (cred->phase1)
  ...
  if (cred->roaming_partner) {
    for (i = 0; i < cred->num_roaming_partner; i++) {
      ...
      fprintf(f, "\troaming_partner=\"%s,%d,%u,%s\"\n", p->fqdn, p->exact_match,
              p->priority, p->country);
    }
  }
  ...
  if (cred->num_req_conn_capab) {
    for (i = 0; i < cred->num_req_conn_capab; i++) {
      ...
      fprintf(f, "\treq_conn_capab=%u", cred->req_conn_capab_proto[i]);
      ports = cred->req_conn_capab_port[i];
      if (ports) {
        int j;
        for (j = 0; ports[j] != -1; j++) {
          fprintf(f, "%s%d", j > 0 ? "," : ":", ports[j]);
        }
      }
      fprintf(f, "\n");
    }
  ...
  if (cred->required_roaming_consortium_len) {
    ...
    for (i = 0; i < cred->required_roaming_consortium_len; i++)
      fprintf(f, "%02x", cred->required_roaming_consortium[i]);
    fprintf(f, "\n");
  }
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config) {
  ...
  if (config->p2p_pref_chan) {
    ...
    for (i = 0; i < config->num_p2p_pref_chan; i++) {
      fprintf(f, "%s%u:%u", i > 0 ? "," : "", config->p2p_pref_chan[i].op_class,
  ...
  if (config->p2p_optimize_listen_chan != DEFAULT_P2P_OPTIMIZE_LISTEN_CHAN)
  ...
  if (WPA_GET_BE32(config->ip_addr_go))
    fprintf(f, "ip_addr_go=%u.%u.%u.%u\n", config->ip_addr_go[0],
            config->ip_addr_go[1], config->ip_addr_go[2],
            config->ip_addr_go[3]);
  if (WPA_GET_BE32(config->ip_addr_mask))
    fprintf(f, "ip_addr_mask=%u.%u.%u.%u\n", config->ip_addr_mask[0],
            config->ip_addr_mask[1], config->ip_addr_mask[2],
            config->ip_addr_mask[3]);
  if (WPA_GET_BE32(config->ip_addr_start))
    fprintf(f, "ip_addr_start=%u.%u.%u.%u\n", config->ip_addr_start[0],
            config->ip_addr_start[1], config->ip_addr_start[2],
            config->ip_addr_start[3]);
  if (WPA_GET_BE32(config->ip_addr_end))
    fprintf(f, "ip_addr_end=%u.%u.%u.%u\n", config->ip_addr_end[0],
            config->ip_addr_end[1], config->ip_addr_end[2],
            config->ip_addr_end[3]);
#endif /* CONFIG_P2P */
  ...
  if (config->bss_expiration_scan_count != DEFAULT_BSS_EXPIRATION_SCAN_COUNT)
  ...
  if (config->sae_groups) {
    ...
    for (i = 0; config->sae_groups[i] >= 0; i++) {
      fprintf(f, "%s%d", i > 0 ? " " : "", config->sae_groups[i]);
    }
    fprintf(f, "\n");
  }
  ...
  if (config->freq_list && config->freq_list[0]) {
    ...
    for (i = 0; config->freq_list[i]; i++) {
      fprintf(f, "%s%d", i > 0 ? " " : "", config->freq_list[i]);
    }
    fprintf(f, "\n");
  }
