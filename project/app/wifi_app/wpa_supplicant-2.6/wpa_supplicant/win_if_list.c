...
static HANDLE ndisuio_open(void) {
  ...
  h = CreateFile(TEXT("\\\\.\\\\Ndisuio"), GENERIC_READ | GENERIC_WRITE, 0,
...
#ifndef _WIN32_WCE
  if (!DeviceIoControl(h, IOCTL_NDISUIO_BIND_WAIT, NULL, 0, NULL, 0, &written,
                       NULL)) {
    printf("IOCTL_NDISUIO_BIND_WAIT failed: %d", (int)GetLastError());
    CloseHandle(h);
...
static void ndisuio_query_bindings(HANDLE ndisuio) {
  ...
  for (i = 0;; i++) {
    ...
    if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b,
                         sizeof(NDISUIO_QUERY_BINDING), b, (DWORD)blen,
                         &written, NULL)) {
