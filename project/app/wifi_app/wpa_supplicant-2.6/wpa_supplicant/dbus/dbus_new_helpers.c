...
static dbus_bool_t fill_dict_with_properties(
  ...
  for (dsc = props; dsc && dsc->dbus_property; dsc++) {
    ...
    if (os_strncmp(dsc->dbus_interface, interface, WPAS_DBUS_INTERFACE_MAX) !=
        0)
      continue;
    ...
    if (!dbus_message_iter_open_container(dict_iter, DBUS_TYPE_DICT_ENTRY, NULL,
                                          &entry_iter) ||
    ...
    if (!dsc->getter(dsc, &entry_iter, error, user_data)) {
      wpa_printf(MSG_INFO,
                 ...
                 __func__, dsc->dbus_interface, dsc->dbus_property);
      return FALSE;
    }
...
static DBusMessage *get_all_properties(DBusMessage *message, char *interface,
  ...
  if (!fill_dict_with_properties(&dict_iter, obj_dsc->properties, interface,
                                 obj_dsc->user_data, &error)) {
    dbus_message_unref(reply);
...
static DBusMessage *properties_get_all(DBusMessage *message, char *interface,
  ...
  if (os_strcmp(dbus_message_get_signature(message), "s") != 0)
    return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS, NULL);
...
static DBusMessage *properties_get(DBusMessage *message,
  ...
  if (dsc->getter(dsc, &iter, &error, user_data) == FALSE) {
    dbus_message_unref(reply);
    reply = wpas_dbus_reply_new_from_error(message, &error, DBUS_ERROR_FAILED,
...
properties_get_or_set(DBusMessage *message, DBusMessageIter *iter,
                      char *interface, struct wpa_dbus_object_desc *obj_dsc) {
  ...
  if (property_dsc == NULL || property_dsc->dbus_property == NULL) {
    wpa_printf(MSG_DEBUG, "no property handler for %s.%s on %s", interface,
               property, dbus_message_get_path(message));
    return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS,
                                  "No such property");
  }
  ...
  if (os_strncmp(WPA_DBUS_PROPERTIES_GET, method,
    ...
    return properties_get(message, property_dsc, obj_dsc->user_data);
  }
...
static DBusMessage *msg_method_handler(DBusMessage *message,
  ...
  if (method_dsc == NULL || method_dsc->dbus_method == NULL) {
    wpa_printf(MSG_DEBUG, "no method handler for %s.%s on %s", msg_interface,
               method, dbus_message_get_path(message));
    return dbus_message_new_error(message, DBUS_ERROR_UNKNOWN_METHOD, NULL);
  }
...
static DBusHandlerResult message_handler(DBusConnection *connection,
  ...
  wpa_printf(MSG_MSGDUMP, "dbus: %s.%s (%s) [%s]", msg_interface, method, path,
...
int wpa_dbus_ctrl_iface_init(struct wpas_dbus_priv *iface, char *dbus_path,
  ...
  DBusObjectPathVTable wpa_vtable = {
      &free_dbus_object_desc_cb, &message_handler, NULL, NULL, NULL, NULL};

  obj_desc->connection = iface->con;
  obj_desc->path = os_strdup(dbus_path);

  /* Register the message handler for the global dbus interface */
  if (!dbus_connection_register_object_path(iface->con, dbus_path, &wpa_vtable,
                                            obj_desc)) {
  ...
  case DBUS_REQUEST_NAME_REPLY_ALREADY_OWNER:
    ...
    break;
  default:
    wpa_printf(MSG_ERROR, "dbus: Could not request service name: %s %s",
...
int wpa_dbus_register_object_per_iface(struct wpas_dbus_priv *ctrl_iface,
  ...
  if (!dbus_connection_try_register_object_path(con, path, &vtable, obj_desc,
                                                &error)) {
...
int wpa_dbus_unregister_object_per_iface(struct wpas_dbus_priv *ctrl_iface,
  ...
  if (!obj_desc) {
    wpa_printf(MSG_ERROR,
               "dbus: %s: Could not obtain object's private data: %s", __func__,
               path);
    return 0;
  }
...
static dbus_bool_t
put_changed_properties(const struct wpa_dbus_object_desc *obj_dsc,
                       const char *interface, DBusMessageIter *dict_iter,
                       int clear_changed) {
  DBusMessageIter entry_iter;
  const struct wpa_dbus_property_desc *dsc;
  int i;
  ...
  for (dsc = obj_dsc->properties, i = 0; dsc && dsc->dbus_property;
       dsc++, i++) {
    if (obj_dsc->prop_changed_flags == NULL || !obj_dsc->prop_changed_flags[i])
      continue;
    ...
    if (!dsc->getter(dsc, &entry_iter, &error, obj_dsc->user_data)) {
      if (dbus_error_is_set(&error)) {
        wpa_printf(MSG_ERROR,
                   ...
                   __func__, dsc->dbus_property, error.name, error.message);
      } else {
        wpa_printf(MSG_ERROR, "dbus: %s: Cannot get new value of property %s",
...
static void
do_send_prop_changed_signal(DBusConnection *con, const char *path,
  ...
  if (!dbus_message_iter_append_basic(&signal_iter, DBUS_TYPE_STRING,
      ...
      !dbus_message_iter_open_container(&signal_iter, DBUS_TYPE_ARRAY, "{sv}",
                                        &dict_iter) ||
      ...
      !dbus_message_iter_open_container(&signal_iter, DBUS_TYPE_ARRAY, "s",
                                        &dict_iter) ||
      !dbus_message_iter_close_container(&signal_iter, &dict_iter)) {
...
static void do_send_deprecated_prop_changed_signal(
  ...
  if (!dbus_message_iter_open_container(&signal_iter, DBUS_TYPE_ARRAY, "{sv}",
                                        &dict_iter) ||
...
static void
send_prop_changed_signal(DBusConnection *con, const char *path,
                         const char *interface,
                         const struct wpa_dbus_object_desc *obj_dsc) {
  /*
...
static void recursive_flush_changed_properties(DBusConnection *con,
  ...
  for (i = 0; objects[i]; i++) {
    os_snprintf(subobj_path, WPAS_DBUS_OBJECT_PATH_MAX, "%s/%s", path,
                objects[i]);
...
void wpa_dbus_flush_object_changed_properties(DBusConnection *con,
  ...
  for (dsc = obj_desc->properties, i = 0; dsc && dsc->dbus_property;
    ...
    if (obj_desc->prop_changed_flags == NULL ||
      ...
      continue;
    send_prop_changed_signal(con, path, dsc->dbus_interface, obj_desc);
  }
}
...
void wpa_dbus_mark_property_changed(struct wpas_dbus_priv *iface,
  ...
  dbus_connection_get_object_path_data(iface->con, path, (void **)&obj_desc);
  if (!obj_desc) {
    wpa_printf(MSG_ERROR,
               "dbus: wpa_dbus_property_changed: could not obtain object's "
               "private data: %s",
               path);
    return;
  }
  ...
  if (!eloop_is_timeout_registered(flush_object_timeout_handler, iface->con,
                                   obj_desc)) {
...
dbus_bool_t wpa_dbus_get_object_properties(struct wpas_dbus_priv *iface,
  ...
  if (!wpa_dbus_dict_open_write(iter, &dict_iter)) {
    wpa_printf(MSG_ERROR, "dbus: %s: failed to open message dict", __func__);
    return FALSE;
  }

  dbus_error_init(&error);
  if (!fill_dict_with_properties(&dict_iter, obj_desc->properties, interface,
                                 obj_desc->user_data, &error)) {
    wpa_printf(MSG_ERROR, "dbus: %s: failed to get object properties: (%s) %s",
               __func__, dbus_error_is_set(&error) ? error.name : "none",
...
/**
 * wpas_dbus_new_decompose_object_path - Decompose an interface object path into
 * parts
...
DBusMessage *wpas_dbus_reply_new_from_error(DBusMessage *message,
  ...
  if (fallback_name && fallback_string) {
    return dbus_message_new_error(message, fallback_name, fallback_string);
  }
  return NULL;
}
