...
/**
 * wpas_dbus_error_persistent_group_unknown - Return a new
 * PersistentGroupUnknown error message
...
DBusMessage *wpas_dbus_handler_p2p_find(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "Timeout") == 0 && entry.type == DBUS_TYPE_INT32) {
      ...
      req_dev_types = os_malloc(WPS_DEV_TYPE_LEN * entry.array_len);
      if (!req_dev_types)
        goto error_clear;
      ...
      for (i = 0; i < entry.array_len; i++) {
        ...
        os_memcpy(req_dev_types + i * WPS_DEV_TYPE_LEN,
                  wpabuf_head(entry.binarray_value[i]), WPS_DEV_TYPE_LEN);
      }
  ...
  wpas_p2p_find(wpa_s, timeout, type, num_req_dev_types, req_dev_types, NULL, 0,
                0, NULL, 0);
  os_free(req_dev_types);
  return reply;
...
DBusMessage *wpas_dbus_handler_p2p_rejectpeer(DBusMessage *message,
  ...
  if (wpas_p2p_reject(wpa_s, peer_addr) < 0)
    return wpas_dbus_error_unknown_error(
        message, "Failed to call wpas_p2p_reject method.");
...
DBusMessage *
wpas_dbus_handler_p2p_extendedlisten(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "period") == 0 && entry.type == DBUS_TYPE_INT32)
...
wpas_dbus_handler_p2p_presence_request(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "duration1") == 0 && entry.type == DBUS_TYPE_INT32)
...
DBusMessage *wpas_dbus_handler_p2p_group_add(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "persistent") == 0 &&
      ...
      if (freq <= 0)
        goto inv_args_clear;
    } else if (os_strcmp(entry.key, "persistent_group_object") == 0 &&
  ...
  if (pg_object_path != NULL) {
    ...
    iface = wpas_dbus_new_decompose_object_path(
        pg_object_path, WPAS_DBUS_NEW_PERSISTENT_GROUPS_PART, &net_id_str);
    if (iface == NULL || net_id_str == NULL || !wpa_s->parent->dbus_new_path ||
        os_strcmp(iface, wpa_s->parent->dbus_new_path) != 0) {
      reply = wpas_dbus_error_invalid_args(message, pg_object_path);
      goto out;
    }
    ...
    if (wpas_p2p_group_add_persistent(wpa_s, ssid, 0, freq, 0, 0, 0, 0, 0, NULL,
                                      0, 0)) {
      reply = wpas_dbus_error_unknown_error(
          message, "Failed to reinvoke a persistent group");
      goto out;
    }
  } else if (wpas_p2p_group_add(wpa_s, persistent_group, freq, 0, 0, 0, 0))
    goto inv_args;
...
DBusMessage *wpas_dbus_handler_p2p_disconnect(DBusMessage *message,
  ...
  if (wpas_p2p_disconnect(wpa_s))
    return wpas_dbus_error_unknown_error(message, "failed to disconnect");
...
static dbus_bool_t wpa_dbus_p2p_check_enabled(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL) {
    if (out_reply) {
      *out_reply =
          dbus_message_new_error(message, DBUS_ERROR_FAILED,
...
DBusMessage *wpas_dbus_handler_p2p_connect(DBusMessage *message,
  ...
  new_pin = wpas_p2p_connect(wpa_s, addr, pin, wps_method, persistent_group, 0,
                             join, authorize_only, go_intent, freq, 0, -1, 0, 0,
                             0, 0, NULL, 0);
  ...
  if (new_pin >= 0) {
    ...
    dbus_message_append_args(reply, DBUS_TYPE_STRING, &generated_pin,
                             DBUS_TYPE_INVALID);
  } else {
    switch (new_pin) {
    case -2:
      err_msg = "connect failed due to channel unavailability.";
...
DBusMessage *wpas_dbus_handler_p2p_cancel(DBusMessage *message,
  ...
  if (wpas_p2p_cancel(wpa_s))
    return wpas_dbus_error_unknown_error(message, "P2P cancel failed");
...
DBusMessage *wpas_dbus_handler_p2p_invite(DBusMessage *message,
  ...
  if (persistent) {
    ...
    if (wpas_p2p_invite(wpa_s, peer_addr, ssid, NULL, 0, 0, 0, 0, 0, 0) < 0) {
      reply = wpas_dbus_error_unknown_error(
          message, "Failed to reinvoke a persistent group");
      goto out;
    }
     ...
     */
    if (wpas_p2p_invite_group(wpa_s, wpa_s->ifname, peer_addr, NULL)) {
      reply = wpas_dbus_error_unknown_error(
...
DBusMessage *wpas_dbus_handler_p2p_prov_disc_req(DBusMessage *message,
  ...
  if (os_strcmp(config_method, "display") &&
      os_strcmp(config_method, "keypad") && os_strcmp(config_method, "pbc") &&
  ...
  if (wpas_p2p_prov_disc(wpa_s, peer_addr, config_method,
    ...
    return wpas_dbus_error_unknown_error(
        message, "Failed to send provision discovery request");
...
dbus_bool_t wpas_dbus_getter_p2p_device_config(
  ...
  if (wpa_s->conf->num_sec_device_types) {
    if (!wpa_dbus_dict_begin_array(
            &dict_iter, "SecondaryDeviceTypes",
            DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING,
            &iter_secdev_dict_entry, &iter_secdev_dict_val,
            &iter_secdev_dict_array))
      goto err_no_mem;

    for (i = 0; i < wpa_s->conf->num_sec_device_types; i++)
      wpa_dbus_dict_bin_array_add_element(&iter_secdev_dict_array,
                                          wpa_s->conf->sec_device_type[i],
                                          WPS_DEV_TYPE_LEN);

    if (!wpa_dbus_dict_end_array(&dict_iter, &iter_secdev_dict_entry,
  ...
  for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
    if (wpa_s->conf->wps_vendor_ext[i] == NULL)
      continue;
    vendor_ext[num_vendor_extensions++] = wpa_s->conf->wps_vendor_ext[i];
  }

  if ((num_vendor_extensions &&
       !wpa_dbus_dict_append_wpabuf_array(&dict_iter, "VendorExtension",
...
dbus_bool_t wpas_dbus_setter_p2p_device_config(
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "DeviceName") == 0) {
      ...
      os_memcpy(wpa_s->conf->device_type, entry.bytearray_value,
                WPS_DEV_TYPE_LEN);
      ...
      wpa_s->conf->changed_parameters |= CFG_CHANGED_SEC_DEVICE_TYPE;
      ...
      wpa_s->conf->changed_parameters |= CFG_CHANGED_VENDOR_EXTENSION;

      for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
        wpabuf_free(wpa_s->conf->wps_vendor_ext[i]);
        if (i < entry.array_len) {
          wpa_s->conf->wps_vendor_ext[i] = entry.binarray_value[i];
          entry.binarray_value[i] = NULL;
        } else
          wpa_s->conf->wps_vendor_ext[i] = NULL;
      }
    } else if (os_strcmp(entry.key, "GOIntent") == 0 &&
               entry.type == DBUS_TYPE_UINT32 && (entry.uint32_value <= 15))
      ...
      wpa_s->conf->changed_parameters |= CFG_CHANGED_P2P_OPER_CHANNEL;
      ...
      wpa_s->conf->changed_parameters |= CFG_CHANGED_P2P_INTRA_BSS;
...
dbus_bool_t
wpas_dbus_getter_p2p_peers(const struct wpa_dbus_property_desc *property_desc,
  ...
  while (peer_info != NULL) {
    ...
    os_snprintf(node->path, WPAS_DBUS_OBJECT_PATH_MAX,
                ...
                wpa_s->parent->parent->dbus_new_path, MAC2STR(addr));
...
dbus_bool_t
wpas_dbus_getter_p2p_role(const struct wpa_dbus_property_desc *property_desc,
                          DBusMessageIter *iter, DBusError *error,
                          void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  char *str;
...
dbus_bool_t
wpas_dbus_getter_p2p_group(const struct wpa_dbus_property_desc *property_desc,
                           DBusMessageIter *iter, DBusError *error,
                           void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  ...
  if (wpa_s->dbus_groupobj_path == NULL)
    ...
    os_snprintf(dbus_groupobj_path, WPAS_DBUS_OBJECT_PATH_MAX, "%s",
...
dbus_bool_t
wpas_dbus_getter_p2p_peergo(const struct wpa_dbus_property_desc *property_desc,
                            DBusMessageIter *iter, DBusError *error,
                            void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_OBJECT_PATH, &path,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_p2p_peer_device_name(
  ...
  if (!wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &tmp, error)) {
    dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, "no memory");
    os_free(tmp);
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_getter_p2p_peer_level(
  ...
  if (!wpas_dbus_simple_property_getter(iter, DBUS_TYPE_INT32, &info->level,
                                        error)) {
    dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, "no memory");
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_getter_p2p_peer_device_capability(
  ...
  if (!wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BYTE, &info->dev_capab,
                                        error)) {
    dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, "no memory");
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_getter_p2p_peer_secondary_device_types(
  ...
  if (!dbus_message_iter_open_container(
      ...
      !dbus_message_iter_open_container(
          &variant_iter, DBUS_TYPE_ARRAY,
          DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING, &array_iter)) {
  ...
  if (info->wps_sec_dev_type_list_len) {
    ...
    for (i = 0; i < num_sec_device_types; i++) {
      if (!dbus_message_iter_open_container(&array_iter, DBUS_TYPE_ARRAY,
                                            DBUS_TYPE_BYTE_AS_STRING,
                                            &inner_array_iter) ||
          !dbus_message_iter_append_fixed_array(
              &inner_array_iter, DBUS_TYPE_BYTE, &sec_dev_type_list,
              WPS_DEV_TYPE_LEN) ||
          !dbus_message_iter_close_container(&array_iter, &inner_array_iter)) {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "%s: failed to construct message 2 (%d)", __func__, i);
        return FALSE;
      }
...
dbus_bool_t wpas_dbus_getter_p2p_peer_vendor_extension(
  ...
  if (!wpas_dbus_simple_array_array_property_getter(
          iter, DBUS_TYPE_BYTE, vendor_extension, num, error))
    return FALSE;
...
dbus_bool_t wpas_dbus_getter_p2p_peer_ies(
  ...
  if (info->wfd_subelems == NULL)
    return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, NULL, 0,
                                                  error);
...
dbus_bool_t wpas_dbus_getter_p2p_peer_device_address(
  ...
  return wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_BYTE, (char *)info->p2p_device_addr, ETH_ALEN, error);
}
...
static int match_group_where_peer_is_client(struct p2p_group *group,
  ...
  paths = os_realloc_array(data->paths, data->nb_paths + 1, sizeof(char *));
  if (paths == NULL)
    goto out_of_memory;
...
dbus_bool_t wpas_dbus_getter_p2p_peer_groups(
  ...
  if (data.paths == NULL) {
    return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_OBJECT_PATH,
                                                  NULL, 0, error);
  }

  success = wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_OBJECT_PATH, data.paths, data.nb_paths, error);
  goto out;
...
wpas_dbus_handler_add_persistent_group(DBusMessage *message,
  ...
  if (ssid == NULL) {
    wpa_printf(MSG_ERROR, "dbus: %s: Cannot add new persistent group",
               __func__);
    reply = wpas_dbus_error_unknown_error(
        message,
  ...
  if (!set_network_properties(wpa_s, ssid, &iter, &error)) {
    wpa_printf(
        ...
        __func__);
    reply =
        wpas_dbus_reply_new_from_error(message, &error, DBUS_ERROR_INVALID_ARGS,
...
wpas_dbus_handler_remove_persistent_group(DBusMessage *message,
   ...
   */
  iface = wpas_dbus_new_decompose_object_path(
      op, WPAS_DBUS_NEW_PERSISTENT_GROUPS_PART, &persistent_group_id);
...
dbus_bool_t wpas_dbus_getter_p2p_group_bssid(
  ...
  return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, p_bssid,
                                                ETH_ALEN, error);
}
...
dbus_bool_t wpas_dbus_getter_p2p_group_frequency(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT16, &op_freq,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_p2p_group_passphrase(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &p_pass,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_p2p_group_psk(
  ...
  return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, p_psk,
                                                psk_len, error);
}
...
dbus_bool_t wpas_dbus_getter_p2p_group_vendor_ext(
  ...
  return wpas_dbus_simple_array_array_property_getter(
...
dbus_bool_t wpas_dbus_setter_p2p_group_vendor_ext(
  ...
  if (wpas_get_p2p_role(wpa_s) == WPAS_P2P_ROLE_GO && wpa_s->ap_iface != NULL)
    hapd = wpa_s->ap_iface->bss[0];
  else
    return FALSE;
   ...
   */
  if (dbus_message_iter_get_element_type(&variant_iter) == DBUS_TYPE_ARRAY) {
    ...
    if (dbus_message_iter_get_arg_type(&array_iter) != DBUS_TYPE_ARRAY ||
      ...
      wpa_printf(MSG_DEBUG, "dbus: Not an array of array of bytes");
      return FALSE;
    }
    ...
    while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_ARRAY) {
      ...
      if (i == MAX_WPS_VENDOR_EXTENSIONS) {
        wpa_printf(MSG_DEBUG, "dbus: Too many WPSVendorExtensions values");
      ...
      wpa_hexdump(MSG_DEBUG, "dbus: WPSVendorExtentions[]", val, len);
  ...
  if (dbus_message_iter_get_element_type(&variant_iter) != DBUS_TYPE_DICT_ENTRY)
    return FALSE;

  wpa_printf(MSG_DEBUG, "dbus: Try to use backwards compatibility version of "
                        "WPSVendorExtensions setter");
  if (!wpa_dbus_dict_open_read(&variant_iter, &iter_dict, error))
    return FALSE;
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "WPSVendorExtensions") == 0) {
      ...
      for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++) {
        wpabuf_free(hapd->conf->wps_vendor_ext[i]);
        if (i < entry.array_len) {
          hapd->conf->wps_vendor_ext[i] = entry.binarray_value[i];
          entry.binarray_value[i] = NULL;
        } else
...
DBusMessage *wpas_dbus_handler_p2p_add_service(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "service_type") == 0 &&
      ...
      if (entry.type != DBUS_TYPE_ARRAY || entry.array_type != DBUS_TYPE_BYTE)
        goto error_clear;
      resp = wpabuf_alloc_copy(entry.bytearray_value, entry.array_len);
    }
    wpa_dbus_dict_entry_clear(&entry);
  }
...
DBusMessage *
wpas_dbus_handler_p2p_delete_service(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
  DBusMessageIter iter_dict;
  DBusMessage *reply = NULL;
  DBusMessageIter iter;
  ...
  if (upnp == 1) {
    while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
      if (!wpa_dbus_dict_get_entry(&iter_dict, &entry))
        goto error;
      if (os_strcmp(entry.key, "version") == 0 && entry.type == DBUS_TYPE_INT32)
        version = entry.uint32_value;
      else if (os_strcmp(entry.key, "service") == 0 &&
               entry.type == DBUS_TYPE_STRING) {
        os_free(service);
...
DBusMessage *
wpas_dbus_handler_p2p_service_sd_req(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
  DBusMessageIter iter_dict;
  DBusMessage *reply = NULL;
  DBusMessageIter iter;
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (os_strcmp(entry.key, "peer_object") == 0 &&
      ...
      if (entry.type != DBUS_TYPE_ARRAY || entry.array_type != DBUS_TYPE_BYTE)
        goto error_clear;
      tlv = wpabuf_alloc_copy(entry.bytearray_value, entry.array_len);
    } else
      goto error_clear;
  ...
  if (ref != 0) {
    reply = dbus_message_new_method_return(message);
    dbus_message_append_args(reply, DBUS_TYPE_UINT64, &ref, DBUS_TYPE_INVALID);
  } else {
    reply = wpas_dbus_error_unknown_error(message, "Unable to send SD request");
  }
out:
  os_free(service);
  os_free(peer_object_path);
...
DBusMessage *
wpas_dbus_handler_p2p_service_sd_res(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
  DBusMessageIter iter_dict;
  DBusMessage *reply = NULL;
  DBusMessageIter iter;
...
DBusMessage *
wpas_dbus_handler_p2p_service_update(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
...
dbus_bool_t wpas_dbus_getter_global_wfd_ies(
  ...
  ret = wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_BYTE, wpabuf_head(ie), wpabuf_len(ie), error);
  wpabuf_free(ie);
