...
static DBusHandlerResult noc_filter(DBusConnection *conn, DBusMessage *message,
                                    void *data) {
  ...
  if (dbus_message_is_signal(message, DBUS_INTERFACE_DBUS,
    ...
    if (!dbus_message_get_args(message, &derr, DBUS_TYPE_STRING, &name,
                               DBUS_TYPE_STRING, &prev_owner, DBUS_TYPE_STRING,
...
void wpas_dbus_signal_scan_done(struct wpa_supplicant *wpa_s, int success) {
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_INTERFACE, "ScanDone");
  if (msg == NULL)
    return;
...
static void wpas_dbus_signal_bss(struct wpa_supplicant *wpa_s,
                                 const char *bss_obj_path, const char *sig_name,
                                 int properties) {
  struct wpas_dbus_priv *iface;
  DBusMessage *msg;
  DBusMessageIter iter;
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH,
      ...
      (properties && !wpa_dbus_get_object_properties(
                         iface, bss_obj_path, WPAS_DBUS_NEW_IFACE_BSS, &iter)))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
  dbus_message_unref(msg);
 ...
 */
static void wpas_dbus_signal_network(struct wpa_supplicant *wpa_s, int id,
                                     const char *sig_name, int properties) {
  struct wpas_dbus_priv *iface;
  DBusMessage *msg;
  DBusMessageIter iter;
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path) ||
      (properties &&
       !wpa_dbus_get_object_properties(iface, net_obj_path,
                                       WPAS_DBUS_NEW_IFACE_NETWORK, &iter)))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
  dbus_message_unref(msg);
...
void wpas_dbus_signal_network_request(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(
      wpa_s->dbus_new_path, WPAS_DBUS_NEW_IFACE_INTERFACE, "NetworkRequest");
  if (msg == NULL)
    return;
  ...
  dbus_message_iter_init_append(msg, &iter);
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &net_ptr) ||
...
void wpas_dbus_signal_wps_event_pbc_overlap(struct wpa_supplicant *wpa_s) {
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path, WPAS_DBUS_NEW_IFACE_WPS,
                                "Event");
  if (msg == NULL)
    return;
...
void wpas_dbus_signal_wps_event_m2d(struct wpa_supplicant *wpa_s,
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &key) ||
      ...
      !wpa_dbus_dict_append_byte_array(&dict_iter, "primary_dev_type",
                                       (const char *)m2d->primary_dev_type,
                                       8) ||
...
void wpas_dbus_signal_wps_cred(struct wpa_supplicant *wpa_s,
  ...
  if ((wpa_s->current_ssid &&
       !wpa_dbus_dict_append_byte_array(
           &dict_iter, "BSSID", (const char *)wpa_s->current_ssid->bssid,
           ETH_ALEN)) ||
      !wpa_dbus_dict_append_byte_array(
          &dict_iter, "SSID", (const char *)cred->ssid, cred->ssid_len) ||
      !wpa_dbus_dict_append_string_array(&dict_iter, "AuthType",
                                         (const char **)auth_type, at_num) ||
      !wpa_dbus_dict_append_string_array(&dict_iter, "EncrType",
                                         (const char **)encr_type, et_num) ||
      !wpa_dbus_dict_append_byte_array(
          &dict_iter, "Key", (const char *)cred->key, cred->key_len) ||
      !wpa_dbus_dict_append_uint32(&dict_iter, "KeyIndex", cred->key_idx) ||
      !wpa_dbus_dict_close_write(&iter, &dict_iter))
    goto nomem;
...
void wpas_dbus_signal_certification(struct wpa_supplicant *wpa_s, int depth,
                                    const char *subject,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_INTERFACE, "Certification");
  if (msg == NULL)
    return;
  ...
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      ...
      (altsubject && num_altsubject &&
       !wpa_dbus_dict_append_string_array(&dict_iter, "altsubject", altsubject,
                                          num_altsubject)) ||
      (cert_hash &&
       !wpa_dbus_dict_append_string(&dict_iter, "cert_hash", cert_hash)) ||
      (cert && !wpa_dbus_dict_append_byte_array(
                   &dict_iter, "cert", wpabuf_head(cert), wpabuf_len(cert))) ||
      !wpa_dbus_dict_close_write(&iter, &dict_iter))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
...
void wpas_dbus_signal_eap_status(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_INTERFACE, "EAP");
  if (msg == NULL)
    return;
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &status) ||
      !dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &parameter))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
  dbus_message_unref(msg);
 ...
 */
static void wpas_dbus_signal_sta(struct wpa_supplicant *wpa_s, const u8 *sta,
                                 const char *sig_name) {
  struct wpas_dbus_priv *iface;
  DBusMessage *msg;
  ...
  wpa_printf(MSG_DEBUG, "dbus: Station MAC address '%s' '%s'", sta_mac,
             sig_name);
}
...
void wpas_dbus_signal_p2p_group_removed(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(parent->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_P2PDEVICE, "GroupFinished");
  if (msg == NULL)
    return;

  dbus_message_iter_init_append(msg, &iter);
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      !wpa_dbus_dict_append_object_path(&dict_iter, "interface_object",
                                        wpa_s->dbus_new_path) ||
...
void wpas_dbus_signal_p2p_provision_discovery(struct wpa_supplicant *wpa_s,
  ...
  if (request || !status) {
    if (config_methods & WPS_CONFIG_DISPLAY)
      _signal = request ? "ProvisionDiscoveryRequestDisplayPin"
                        : "ProvisionDiscoveryResponseEnterPin";
    else if (config_methods & WPS_CONFIG_KEYPAD)
      _signal = request ? "ProvisionDiscoveryRequestEnterPin"
                        : "ProvisionDiscoveryResponseDisplayPin";
  ...
  add_pin = ((request && (config_methods & WPS_CONFIG_DISPLAY)) ||
             (!request && !status && (config_methods & WPS_CONFIG_KEYPAD)));
  ...
  if (!request && status)
    ...
    error_ret = (add_pin && !dbus_message_iter_append_basic(
                                &iter, DBUS_TYPE_STRING, &p_pin));
...
void wpas_dbus_signal_p2p_go_neg_req(struct wpa_supplicant *wpa_s,
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path) ||
      !dbus_message_iter_append_basic(&iter, DBUS_TYPE_UINT16,
                                      &dev_passwd_id) ||
      !dbus_message_iter_append_basic(&iter, DBUS_TYPE_BYTE, &go_intent))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
...
static int wpas_dbus_get_group_obj_path(struct wpa_supplicant *wpa_s,
  ...
  os_snprintf(group_obj_path, WPAS_DBUS_OBJECT_PATH_MAX,
              "%s/" WPAS_DBUS_NEW_P2P_GROUPS_PART "/%s", wpa_s->dbus_new_path,
              group_name);
...
static void signal_peer_groups_changed(struct p2p_peer_info *info,
  ...
  wpa_s_go = wpas_get_p2p_client_iface(data->wpa_s, info->p2p_device_addr);
  if (wpa_s_go != NULL && wpa_s_go == data->wpa_s) {
    wpas_dbus_signal_peer_groups_changed(data->wpa_s->p2pdev,
...
static void peer_groups_changed(struct wpa_supplicant *wpa_s) {
  ...
  p2p_loop_on_known_peers(wpa_s->global->p2p, signal_peer_groups_changed,
                          &data);
}
...
void wpas_dbus_signal_p2p_group_started(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(parent->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_P2PDEVICE, "GroupStarted");
  if (msg == NULL)
    return;
...
void wpas_dbus_signal_p2p_go_neg_resp(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(
      wpa_s->dbus_new_path, WPAS_DBUS_NEW_IFACE_P2PDEVICE,
      res->status ? "GONegotiationFailure" : "GONegotiationSuccess");
  if (msg == NULL)
    return;

  dbus_message_iter_init_append(msg, &iter);
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      !wpa_dbus_dict_append_object_path(&dict_iter, "peer_object", path) ||
  ...
  if (!res->status) {
    ...
    if ((res->role_go && !wpa_dbus_dict_append_string(&dict_iter, "passphrase",
                                                      res->passphrase)) ||
        ...
        !wpa_dbus_dict_append_byte_array(
            &dict_iter, "ssid", (const char *)res->ssid, res->ssid_len) ||
    ...
    if (!wpa_dbus_dict_begin_array(&dict_iter, "frequency_list",
                                   DBUS_TYPE_INT32_AS_STRING, &iter_dict_entry,
                                   &iter_dict_val, &iter_dict_array) ||
        !dbus_message_iter_append_fixed_array(&iter_dict_array, DBUS_TYPE_INT32,
                                              &f_array, freq_list_num) ||
        !wpa_dbus_dict_end_array(&dict_iter, &iter_dict_entry, &iter_dict_val,
                                 &iter_dict_array) ||
...
void wpas_dbus_signal_p2p_invitation_result(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(
      wpa_s->dbus_new_path, WPAS_DBUS_NEW_IFACE_P2PDEVICE, "InvitationResult");
...
void wpas_dbus_signal_p2p_peer_joined(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_groupobj_path,
                                WPAS_DBUS_NEW_IFACE_P2P_GROUP, "PeerJoined");
  if (msg == NULL)
    return;
...
void wpas_dbus_signal_p2p_peer_disconnected(struct wpa_supplicant *wpa_s,
  ...
  path = peer_obj_path;
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path)) {
    wpa_printf(MSG_ERROR, "dbus: Failed to construct PeerDisconnected signal");
  } else {
    dbus_connection_send(iface->con, msg, NULL);
    wpas_dbus_signal_peer_groups_changed(parent, peer_addr);
  }
 ...
 */
void wpas_dbus_signal_p2p_sd_request(struct wpa_supplicant *wpa_s, int freq,
  ...
  os_snprintf(peer_obj_path, WPAS_DBUS_OBJECT_PATH_MAX,
              "%s/" WPAS_DBUS_NEW_P2P_PEERS_PART "/" COMPACT_MACSTR,
              wpa_s->dbus_new_path, MAC2STR(sa));
  ...
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      ...
      !wpa_dbus_dict_append_int32(&dict_iter, "dialog_token", dialog_token) ||
      !wpa_dbus_dict_append_uint16(&dict_iter, "update_indicator",
                                   update_indic) ||
      !wpa_dbus_dict_append_byte_array(&dict_iter, "tlvs", (const char *)tlvs,
                                       tlvs_len) ||
      !wpa_dbus_dict_close_write(&iter, &dict_iter))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
...
static void wpas_dbus_signal_persistent_group(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_P2PDEVICE, sig_name);
  if (msg == NULL)
    return;
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path) ||
      (properties &&
       !wpa_dbus_get_object_properties(
           iface, pgrp_obj_path, WPAS_DBUS_NEW_IFACE_PERSISTENT_GROUP, &iter)))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
 ...
 */
static void
wpas_dbus_signal_persistent_group_added(struct wpa_supplicant *wpa_s, int id) {
  wpas_dbus_signal_persistent_group(wpa_s, id, "PersistentGroupAdded", TRUE);
}
 ...
 */
static void
wpas_dbus_signal_persistent_group_removed(struct wpa_supplicant *wpa_s,
                                          int id) {
  wpas_dbus_signal_persistent_group(wpa_s, id, "PersistentGroupRemoved", FALSE);
}
...
void wpas_dbus_signal_p2p_wps_failed(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->dbus_new_path)
    return;
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_P2PDEVICE, "WpsFailed");
  if (msg == NULL)
    return;
...
/**
 * wpas_dbus_signal_p2p_group_formation_failure - Signals GroupFormationFailure
 * event
 * @wpa_s: %wpa_supplicant network interface data
...
void wpas_dbus_signal_p2p_invitation_received(struct wpa_supplicant *wpa_s,
  ...
  dbus_message_iter_init_append(msg, &iter);
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      (sa && !wpa_dbus_dict_append_byte_array(&dict_iter, "sa",
                                              (const char *)sa, ETH_ALEN)) ||
      (dev_addr &&
       !wpa_dbus_dict_append_byte_array(&dict_iter, "go_dev_addr",
                                        (const char *)dev_addr, ETH_ALEN)) ||
      (bssid && !wpa_dbus_dict_append_byte_array(
                    &dict_iter, "bssid", (const char *)bssid, ETH_ALEN)) ||
      (id && !wpa_dbus_dict_append_int32(&dict_iter, "persistent_id", id)) ||
...
void wpas_dbus_signal_prop_changed(struct wpa_supplicant *wpa_s,
  ...
  case WPAS_DBUS_PROP_ASSOC_STATUS_CODE:
    ...
    break;
  default:
    wpa_printf(MSG_ERROR, "dbus: %s: Unknown Property value %d", __func__,
               property);
    return;
  }
...
void wpas_dbus_bss_signal_prop_changed(struct wpa_supplicant *wpa_s,
  ...
  os_snprintf(path, WPAS_DBUS_OBJECT_PATH_MAX,
              "%s/" WPAS_DBUS_NEW_BSSIDS_PART "/%u", wpa_s->dbus_new_path, id);
...
void wpas_dbus_signal_debug_level_changed(struct wpa_global *global) {
  wpa_dbus_mark_property_changed(global->dbus, WPAS_DBUS_NEW_PATH,
                                 WPAS_DBUS_NEW_INTERFACE, "DebugLevel");
}
...
void wpas_dbus_signal_debug_timestamp_changed(struct wpa_global *global) {
  wpa_dbus_mark_property_changed(global->dbus, WPAS_DBUS_NEW_PATH,
                                 WPAS_DBUS_NEW_INTERFACE, "DebugTimestamp");
}
...
void wpas_dbus_signal_debug_show_keys_changed(struct wpa_global *global) {
  wpa_dbus_mark_property_changed(global->dbus, WPAS_DBUS_NEW_PATH,
                                 WPAS_DBUS_NEW_INTERFACE, "DebugShowKeys");
}
...
static void wpas_dbus_register(struct wpa_dbus_object_desc *obj_desc,
  ...
  if (!obj_desc->prop_changed_flags)
    wpa_printf(MSG_DEBUG, "dbus: %s: can't register handlers", __func__);
}
...
static const struct wpa_dbus_property_desc wpas_dbus_global_properties[] = {
    {"DebugLevel", WPAS_DBUS_NEW_INTERFACE, "s", wpas_dbus_getter_debug_level,
     wpas_dbus_setter_debug_level, NULL},
    {"DebugTimestamp", WPAS_DBUS_NEW_INTERFACE, "b",
     wpas_dbus_getter_debug_timestamp, wpas_dbus_setter_debug_timestamp, NULL},
    {"DebugShowKeys", WPAS_DBUS_NEW_INTERFACE, "b",
     wpas_dbus_getter_debug_show_keys, wpas_dbus_setter_debug_show_keys, NULL},
    {"Interfaces", WPAS_DBUS_NEW_INTERFACE, "ao", wpas_dbus_getter_interfaces,
     NULL, NULL},
    {"EapMethods", WPAS_DBUS_NEW_INTERFACE, "as", wpas_dbus_getter_eap_methods,
     NULL, NULL},
    {"Capabilities", WPAS_DBUS_NEW_INTERFACE, "as",
     wpas_dbus_getter_global_capabilities, NULL, NULL},
#ifdef CONFIG_WIFI_DISPLAY
    {"WFDIEs", WPAS_DBUS_NEW_INTERFACE, "ay", wpas_dbus_getter_global_wfd_ies,
     wpas_dbus_setter_global_wfd_ies, NULL},
...
static const struct wpa_dbus_signal_desc wpas_dbus_global_signals[] = {
    {"InterfaceAdded",
     WPAS_DBUS_NEW_INTERFACE,
     {{"path", "o", ARG_OUT}, {"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    {"InterfaceRemoved",
     WPAS_DBUS_NEW_INTERFACE,
     {{"path", "o", ARG_OUT}, END_ARGS}},
    /* Deprecated: use org.freedesktop.DBus.Properties.PropertiesChanged */
    {"PropertiesChanged",
     WPAS_DBUS_NEW_INTERFACE,
     {{"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    {NULL, NULL, {END_ARGS}}};
...
static void wpa_dbus_ctrl_iface_props_deinit(struct wpas_dbus_priv *priv) {
  ...
  while (priv->all_interface_properties[idx].dbus_property)
    os_free((char *)priv->all_interface_properties[idx++].dbus_property);
...
int wpas_dbus_ctrl_iface_init(struct wpas_dbus_priv *priv) {
  ...
  if (!obj_desc) {
    wpa_printf(MSG_ERROR, "Not enough memory to create object description");
    goto error;
  }

  wpas_dbus_register(obj_desc, priv->global, NULL, wpas_dbus_global_methods,
                     wpas_dbus_global_properties, wpas_dbus_global_signals);

  wpa_printf(MSG_DEBUG, "dbus: Register D-Bus object '%s'", WPAS_DBUS_NEW_PATH);
  ret = wpa_dbus_ctrl_iface_init(priv, WPAS_DBUS_NEW_PATH,
                                 WPAS_DBUS_NEW_SERVICE, obj_desc);
  if (ret < 0) {
...
static void wpa_dbus_free(void *ptr) { os_free(ptr); }

static const struct wpa_dbus_property_desc wpas_dbus_network_properties[] = {
    {"Properties", WPAS_DBUS_NEW_IFACE_NETWORK, "a{sv}",
     wpas_dbus_getter_network_properties, wpas_dbus_setter_network_properties,
     NULL},
    {"Enabled", WPAS_DBUS_NEW_IFACE_NETWORK, "b", wpas_dbus_getter_enabled,
     wpas_dbus_setter_enabled, NULL},
    {NULL, NULL, NULL, NULL, NULL, NULL}};
...
int wpas_dbus_register_network(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG, "dbus: Register network object '%s'", net_obj_path);
  obj_desc = os_zalloc(sizeof(struct wpa_dbus_object_desc));
  if (!obj_desc) {
    wpa_printf(MSG_ERROR, "Not enough memory to create object description");
    goto err;
  ...
  if (!arg) {
    wpa_printf(MSG_ERROR, "Not enough memory to create arguments for method");
    goto err;
  }
  ...
  wpas_dbus_register(obj_desc, arg, wpa_dbus_free, NULL,
                     wpas_dbus_network_properties, wpas_dbus_network_signals);
...
int wpas_dbus_unregister_network(struct wpa_supplicant *wpa_s, int nid) {
  ...
  wpa_printf(MSG_DEBUG, "dbus: Unregister network object '%s'", net_obj_path);
...
static const struct wpa_dbus_property_desc wpas_dbus_bss_properties[] = {
    {"SSID", WPAS_DBUS_NEW_IFACE_BSS, "ay", wpas_dbus_getter_bss_ssid, NULL,
     NULL},
    {"BSSID", WPAS_DBUS_NEW_IFACE_BSS, "ay", wpas_dbus_getter_bss_bssid, NULL,
     NULL},
    {"Privacy", WPAS_DBUS_NEW_IFACE_BSS, "b", wpas_dbus_getter_bss_privacy,
     NULL, NULL},
    {"Mode", WPAS_DBUS_NEW_IFACE_BSS, "s", wpas_dbus_getter_bss_mode, NULL,
     NULL},
    {"Signal", WPAS_DBUS_NEW_IFACE_BSS, "n", wpas_dbus_getter_bss_signal, NULL,
     NULL},
    {"Frequency", WPAS_DBUS_NEW_IFACE_BSS, "q", wpas_dbus_getter_bss_frequency,
     NULL, NULL},
    {"Rates", WPAS_DBUS_NEW_IFACE_BSS, "au", wpas_dbus_getter_bss_rates, NULL,
     NULL},
    {"WPA", WPAS_DBUS_NEW_IFACE_BSS, "a{sv}", wpas_dbus_getter_bss_wpa, NULL,
     NULL},
    {"RSN", WPAS_DBUS_NEW_IFACE_BSS, "a{sv}", wpas_dbus_getter_bss_rsn, NULL,
     NULL},
    {"WPS", WPAS_DBUS_NEW_IFACE_BSS, "a{sv}", wpas_dbus_getter_bss_wps, NULL,
     NULL},
    {"IEs", WPAS_DBUS_NEW_IFACE_BSS, "ay", wpas_dbus_getter_bss_ies, NULL,
     NULL},
    {"Age", WPAS_DBUS_NEW_IFACE_BSS, "u", wpas_dbus_getter_bss_age, NULL, NULL},
    {NULL, NULL, NULL, NULL, NULL, NULL}};
 ...
 */
int wpas_dbus_unregister_bss(struct wpa_supplicant *wpa_s, u8 bssid[ETH_ALEN],
                             unsigned int id) {
  struct wpas_dbus_priv *ctrl_iface;
  ...
  wpa_printf(MSG_DEBUG, "dbus: Unregister BSS object '%s'", bss_obj_path);
 ...
 */
int wpas_dbus_register_bss(struct wpa_supplicant *wpa_s, u8 bssid[ETH_ALEN],
                           unsigned int id) {
  struct wpas_dbus_priv *ctrl_iface;
  struct wpa_dbus_object_desc *obj_desc;
  char bss_obj_path[WPAS_DBUS_OBJECT_PATH_MAX];
  ...
  if (!arg) {
    wpa_printf(MSG_ERROR, "Not enough memory to create arguments for handler");
    goto err;
  }
  arg->wpa_s = wpa_s;
  arg->id = id;
  ...
  wpas_dbus_register(obj_desc, arg, wpa_dbus_free, NULL,
                     wpas_dbus_bss_properties, wpas_dbus_bss_signals);

  wpa_printf(MSG_DEBUG, "dbus: Register BSS object '%s'", bss_obj_path);
  if (wpa_dbus_register_object_per_iface(ctrl_iface, bss_obj_path,
                                         wpa_s->ifname, obj_desc)) {
    wpa_printf(MSG_ERROR, "Cannot register BSSID dbus object %s.",
               bss_obj_path);
    goto err;
  }
...
static const struct wpa_dbus_method_desc wpas_dbus_interface_methods[] = {
    {"Scan",
     ...
     {{"args", "a{sv}", ARG_IN}, END_ARGS}},
    {"SignalPoll",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_signal_poll,
     {{"args", "a{sv}", ARG_OUT}, END_ARGS}},
    {"Disconnect",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
    ...
    {"AddBlob",
     ...
     {{"name", "s", ARG_IN}, {"data", "ay", ARG_IN}, END_ARGS}},
    {"GetBlob",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_get_blob,
     {{"name", "s", ARG_IN}, {"data", "ay", ARG_OUT}, END_ARGS}},
    {"RemoveBlob",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_remove_blob,
     {{"name", "s", ARG_IN}, END_ARGS}},
#endif /* CONFIG_NO_CONFIG_BLOBS */
    {"SetPKCS11EngineAndModulePath",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_set_pkcs11_engine_and_module_path,
    ...
    {"Start",
     ...
     {{"args", "a{sv}", ARG_IN}, {"output", "a{sv}", ARG_OUT}, END_ARGS}},
    {"Cancel",
     WPAS_DBUS_NEW_IFACE_WPS,
    ...
    {"ProvisionDiscoveryRequest",
     ...
     {{"peer", "o", ARG_IN}, {"config_method", "s", ARG_IN}, END_ARGS}},
    {"Connect",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     (WPADBusMethodHandler)wpas_dbus_handler_p2p_connect,
     {{"args", "a{sv}", ARG_IN}, {"generated_pin", "s", ARG_OUT}, END_ARGS}},
    {"GroupAdd",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
    ...
    {"ServiceDiscoveryRequest",
     ...
     {{"args", "a{sv}", ARG_IN}, {"ref", "t", ARG_OUT}, END_ARGS}},
    {"ServiceDiscoveryResponse",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
    ...
    {"RemoveAllPersistentGroups",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     (WPADBusMethodHandler)wpas_dbus_handler_remove_all_persistent_groups,
     {END_ARGS}},
#endif /* CONFIG_P2P */
    {"FlushBSS",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_flush_bss,
     {{"age", "u", ARG_IN}, END_ARGS}},
#ifdef CONFIG_AP
    {"SubscribeProbeReq",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
    ...
    {"TDLSStatus",
     ...
     {{"peer_address", "s", ARG_IN}, {"status", "s", ARG_OUT}, END_ARGS}},
    {"TDLSTeardown",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
    ...
    {"VendorElemAdd",
     ...
     {{"frame_id", "i", ARG_IN}, {"ielems", "ay", ARG_IN}, END_ARGS}},
    {"VendorElemGet",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     (WPADBusMethodHandler)wpas_dbus_handler_vendor_elem_get,
     {{"frame_id", "i", ARG_IN}, {"ielems", "ay", ARG_OUT}, END_ARGS}},
    {"VendorElemRem",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
...
static const struct wpa_dbus_property_desc wpas_dbus_interface_properties[] = {
    {"Capabilities", WPAS_DBUS_NEW_IFACE_INTERFACE, "a{sv}",
     wpas_dbus_getter_capabilities, NULL, NULL},
    {"State", WPAS_DBUS_NEW_IFACE_INTERFACE, "s", wpas_dbus_getter_state, NULL,
     NULL},
    {"Scanning", WPAS_DBUS_NEW_IFACE_INTERFACE, "b", wpas_dbus_getter_scanning,
     NULL, NULL},
    {"ApScan", WPAS_DBUS_NEW_IFACE_INTERFACE, "u", wpas_dbus_getter_ap_scan,
     wpas_dbus_setter_ap_scan, NULL},
    {"BSSExpireAge", WPAS_DBUS_NEW_IFACE_INTERFACE, "u",
     wpas_dbus_getter_bss_expire_age, wpas_dbus_setter_bss_expire_age, NULL},
    {"BSSExpireCount", WPAS_DBUS_NEW_IFACE_INTERFACE, "u",
     wpas_dbus_getter_bss_expire_count, wpas_dbus_setter_bss_expire_count,
     NULL},
    {"Country", WPAS_DBUS_NEW_IFACE_INTERFACE, "s", wpas_dbus_getter_country,
     wpas_dbus_setter_country, NULL},
    {"Ifname", WPAS_DBUS_NEW_IFACE_INTERFACE, "s", wpas_dbus_getter_ifname,
     NULL, NULL},
    {"Driver", WPAS_DBUS_NEW_IFACE_INTERFACE, "s", wpas_dbus_getter_driver,
     NULL, NULL},
    {"BridgeIfname", WPAS_DBUS_NEW_IFACE_INTERFACE, "s",
     wpas_dbus_getter_bridge_ifname, NULL, NULL},
    {"ConfigFile", WPAS_DBUS_NEW_IFACE_INTERFACE, "s",
     wpas_dbus_getter_config_file, NULL, NULL},
    {"CurrentBSS", WPAS_DBUS_NEW_IFACE_INTERFACE, "o",
     wpas_dbus_getter_current_bss, NULL, NULL},
    {"CurrentNetwork", WPAS_DBUS_NEW_IFACE_INTERFACE, "o",
     wpas_dbus_getter_current_network, NULL, NULL},
    {"CurrentAuthMode", WPAS_DBUS_NEW_IFACE_INTERFACE, "s",
     wpas_dbus_getter_current_auth_mode, NULL, NULL},
    {"Blobs", WPAS_DBUS_NEW_IFACE_INTERFACE, "a{say}", wpas_dbus_getter_blobs,
     NULL, NULL},
    {"BSSs", WPAS_DBUS_NEW_IFACE_INTERFACE, "ao", wpas_dbus_getter_bsss, NULL,
     NULL},
    {"Networks", WPAS_DBUS_NEW_IFACE_INTERFACE, "ao", wpas_dbus_getter_networks,
     NULL, NULL},
    {"FastReauth", WPAS_DBUS_NEW_IFACE_INTERFACE, "b",
     wpas_dbus_getter_fast_reauth, wpas_dbus_setter_fast_reauth, NULL},
    {"ScanInterval", WPAS_DBUS_NEW_IFACE_INTERFACE, "i",
     wpas_dbus_getter_scan_interval, wpas_dbus_setter_scan_interval, NULL},
    {"PKCS11EnginePath", WPAS_DBUS_NEW_IFACE_INTERFACE, "s",
     wpas_dbus_getter_pkcs11_engine_path, NULL, NULL},
    {"PKCS11ModulePath", WPAS_DBUS_NEW_IFACE_INTERFACE, "s",
     wpas_dbus_getter_pkcs11_module_path, NULL, NULL},
#ifdef CONFIG_WPS
    {"ProcessCredentials", WPAS_DBUS_NEW_IFACE_WPS, "b",
     wpas_dbus_getter_process_credentials, wpas_dbus_setter_process_credentials,
     NULL},
    {"ConfigMethods", WPAS_DBUS_NEW_IFACE_WPS, "s",
     wpas_dbus_getter_config_methods, wpas_dbus_setter_config_methods, NULL},
#endif /* CONFIG_WPS */
#ifdef CONFIG_P2P
    {"P2PDeviceConfig", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "a{sv}",
     wpas_dbus_getter_p2p_device_config, wpas_dbus_setter_p2p_device_config,
     NULL},
    {"Peers", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "ao", wpas_dbus_getter_p2p_peers,
     NULL, NULL},
    {"Role", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "s", wpas_dbus_getter_p2p_role,
     NULL, NULL},
    {"Group", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "o", wpas_dbus_getter_p2p_group,
     NULL, NULL},
    {"PeerGO", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "o", wpas_dbus_getter_p2p_peergo,
     NULL, NULL},
    {"PersistentGroups", WPAS_DBUS_NEW_IFACE_P2PDEVICE, "ao",
     wpas_dbus_getter_persistent_groups, NULL, NULL},
#endif /* CONFIG_P2P */
    {"DisconnectReason", WPAS_DBUS_NEW_IFACE_INTERFACE, "i",
     wpas_dbus_getter_disconnect_reason, NULL, NULL},
    {"AssocStatusCode", WPAS_DBUS_NEW_IFACE_INTERFACE, "i",
     wpas_dbus_getter_assoc_status_code, NULL, NULL},
    {NULL, NULL, NULL, NULL, NULL, NULL}};

static const struct wpa_dbus_signal_desc wpas_dbus_interface_signals[] = {
    {"ScanDone",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     {{"success", "b", ARG_OUT}, END_ARGS}},
    {"BSSAdded",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     {{"path", "o", ARG_OUT}, {"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    {"BSSRemoved",
    ...
    {"Event",
     WPAS_DBUS_NEW_IFACE_WPS,
     {{"name", "s", ARG_OUT}, {"args", "a{sv}", ARG_OUT}, END_ARGS}},
    {"Credentials",
     WPAS_DBUS_NEW_IFACE_WPS,
     {{"credentials", "a{sv}", ARG_OUT}, END_ARGS}},
    /* Deprecated: use org.freedesktop.DBus.Properties.PropertiesChanged */
    {"PropertiesChanged",
     WPAS_DBUS_NEW_IFACE_WPS,
     {{"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    ...
    {"DeviceLost",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"path", "o", ARG_OUT}, END_ARGS}},
    {"FindStopped", WPAS_DBUS_NEW_IFACE_P2PDEVICE, {END_ARGS}},
    {"ProvisionDiscoveryRequestDisplayPin",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"peer_object", "o", ARG_OUT}, {"pin", "s", ARG_OUT}, END_ARGS}},
    {"ProvisionDiscoveryResponseDisplayPin",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"peer_object", "o", ARG_OUT}, {"pin", "s", ARG_OUT}, END_ARGS}},
    {"ProvisionDiscoveryRequestEnterPin",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"peer_object", "o", ARG_OUT}, END_ARGS}},
    {"ProvisionDiscoveryResponseEnterPin",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"peer_object", "o", ARG_OUT}, END_ARGS}},
    {"ProvisionDiscoveryPBCRequest",
    ...
    {"ProvisionDiscoveryFailure",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"peer_object", "o", ARG_OUT}, {"status", "i", ARG_OUT}, END_ARGS}},
    {"GroupStarted",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    {"GroupFormationFailure",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"reason", "s", ARG_OUT}, END_ARGS}},
    ...
    {"InvitationResult",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"invite_result", "a{sv}", ARG_OUT}, END_ARGS}},
    {"GroupFinished",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"properties", "a{sv}", ARG_OUT}, END_ARGS}},
    {"ServiceDiscoveryRequest",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"sd_request", "a{sv}", ARG_OUT}, END_ARGS}},
    {"ServiceDiscoveryResponse",
     WPAS_DBUS_NEW_IFACE_P2PDEVICE,
     {{"sd_response", "a{sv}", ARG_OUT}, END_ARGS}},
    ...
    {"Certification",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     {{"certification", "a{sv}", ARG_OUT}, END_ARGS}},
    {"EAP",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     {{"status", "s", ARG_OUT}, {"parameter", "s", ARG_OUT}, END_ARGS}},
    {"StaAuthorized",
     WPAS_DBUS_NEW_IFACE_INTERFACE,
     {{"name", "s", ARG_OUT}, END_ARGS}},
    {"StaDeauthorized",
...
static int wpa_dbus_ctrl_iface_props_init(struct wpas_dbus_priv *priv) {
  ...
  static const char *const ignored_globals[] = {"bss_expiration_age",
  ...
  all_size =
      (num_globals + num_const + 1) * sizeof(wpas_dbus_interface_properties[0]);
  priv->all_interface_properties = os_zalloc(all_size);
  if (!priv->all_interface_properties) {
    wpa_printf(MSG_ERROR, "dbus: Not enough memory for interface properties");
    return -1;
  }

  /* Copy constant interface properties to the start of the array */
  os_memcpy(priv->all_interface_properties, wpas_dbus_interface_properties,
  ...
  for (i = 0, count = num_const; i < num_globals; i++) {
    ...
    if (!desc->dbus_property) {
      wpa_printf(MSG_ERROR, "dbus: Not enough memory for D-Bus property name");
      goto error;
    }
...
int wpas_dbus_register_interface(struct wpa_supplicant *wpa_s) {
  ...
  os_snprintf(wpa_s->dbus_new_path, WPAS_DBUS_OBJECT_PATH_MAX,
              WPAS_DBUS_NEW_PATH_INTERFACES "/%u", next);
  ...
  wpa_printf(MSG_DEBUG, "dbus: Register interface object '%s'",
             wpa_s->dbus_new_path);
  if (wpa_dbus_register_object_per_iface(ctrl_iface, wpa_s->dbus_new_path,
                                         wpa_s->ifname, obj_desc))
    goto err;
...
int wpas_dbus_unregister_interface(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_dbus_unregister_object_per_iface(ctrl_iface, wpa_s->dbus_new_path))
    return -1;
...
static const struct wpa_dbus_property_desc wpas_dbus_p2p_peer_properties[] = {
    {"DeviceName", WPAS_DBUS_NEW_IFACE_P2P_PEER, "s",
     wpas_dbus_getter_p2p_peer_device_name, NULL, NULL},
    {"Manufacturer", WPAS_DBUS_NEW_IFACE_P2P_PEER, "s",
     wpas_dbus_getter_p2p_peer_manufacturer, NULL, NULL},
    {"ModelName", WPAS_DBUS_NEW_IFACE_P2P_PEER, "s",
     wpas_dbus_getter_p2p_peer_modelname, NULL, NULL},
    {"ModelNumber", WPAS_DBUS_NEW_IFACE_P2P_PEER, "s",
     wpas_dbus_getter_p2p_peer_modelnumber, NULL, NULL},
    {"SerialNumber", WPAS_DBUS_NEW_IFACE_P2P_PEER, "s",
     wpas_dbus_getter_p2p_peer_serialnumber, NULL, NULL},
    {"PrimaryDeviceType", WPAS_DBUS_NEW_IFACE_P2P_PEER, "ay",
     wpas_dbus_getter_p2p_peer_primary_device_type, NULL, NULL},
    {"config_method", WPAS_DBUS_NEW_IFACE_P2P_PEER, "q",
     wpas_dbus_getter_p2p_peer_config_method, NULL, NULL},
    {"level", WPAS_DBUS_NEW_IFACE_P2P_PEER, "i",
     wpas_dbus_getter_p2p_peer_level, NULL, NULL},
    {"devicecapability", WPAS_DBUS_NEW_IFACE_P2P_PEER, "y",
     wpas_dbus_getter_p2p_peer_device_capability, NULL, NULL},
    {"groupcapability", WPAS_DBUS_NEW_IFACE_P2P_PEER, "y",
     wpas_dbus_getter_p2p_peer_group_capability, NULL, NULL},
    {"SecondaryDeviceTypes", WPAS_DBUS_NEW_IFACE_P2P_PEER, "aay",
     wpas_dbus_getter_p2p_peer_secondary_device_types, NULL, NULL},
    {"VendorExtension", WPAS_DBUS_NEW_IFACE_P2P_PEER, "aay",
     wpas_dbus_getter_p2p_peer_vendor_extension, NULL, NULL},
    {"IEs", WPAS_DBUS_NEW_IFACE_P2P_PEER, "ay", wpas_dbus_getter_p2p_peer_ies,
     NULL, NULL},
    {"DeviceAddress", WPAS_DBUS_NEW_IFACE_P2P_PEER, "ay",
     wpas_dbus_getter_p2p_peer_device_address, NULL, NULL},
    {"Groups", WPAS_DBUS_NEW_IFACE_P2P_PEER, "ao",
     wpas_dbus_getter_p2p_peer_groups, NULL, NULL},
    {NULL, NULL, NULL, NULL, NULL, NULL}};
...
static void wpas_dbus_signal_peer(struct wpa_supplicant *wpa_s,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path, interface, sig_name);
  if (msg == NULL)
    return;
  ...
  if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_OBJECT_PATH, &path) ||
      (properties &&
       !wpa_dbus_get_object_properties(iface, peer_obj_path,
                                       WPAS_DBUS_NEW_IFACE_P2P_PEER, &iter)))
    wpa_printf(MSG_ERROR, "dbus: Failed to construct signal");
  else
    dbus_connection_send(iface->con, msg, NULL);
...
void wpas_dbus_signal_peer_device_found(struct wpa_supplicant *wpa_s,
                                        const u8 *dev_addr) {
  wpas_dbus_signal_peer(wpa_s, dev_addr, WPAS_DBUS_NEW_IFACE_P2PDEVICE,
                        "DeviceFound", FALSE);
...
int wpas_dbus_register_peer(struct wpa_supplicant *wpa_s, const u8 *dev_addr) {
  ...
  wpa_printf(MSG_INFO, "dbus: Register peer object '%s'", peer_obj_path);
  obj_desc = os_zalloc(sizeof(struct wpa_dbus_object_desc));
  if (!obj_desc) {
    wpa_printf(MSG_ERROR, "Not enough memory to create object description");
    goto err;
  ...
  wpas_dbus_register(obj_desc, arg, wpa_dbus_free, NULL,
                     wpas_dbus_p2p_peer_properties, wpas_dbus_p2p_peer_signals);
...
int wpas_dbus_unregister_peer(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_INFO, "dbus: Unregister peer object '%s'", peer_obj_path);
...
void wpas_dbus_signal_p2p_find_stopped(struct wpa_supplicant *wpa_s) {
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_P2PDEVICE, "FindStopped");
  if (msg == NULL)
    return;
...
static const struct wpa_dbus_property_desc wpas_dbus_p2p_group_properties[] = {
    {"Members", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "ao",
     wpas_dbus_getter_p2p_group_members, NULL, NULL},
    {"Group", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "o", wpas_dbus_getter_p2p_group,
     NULL, NULL},
    {"Role", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "s", wpas_dbus_getter_p2p_role,
     NULL, NULL},
    {"SSID", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "ay",
     wpas_dbus_getter_p2p_group_ssid, NULL, NULL},
    {"BSSID", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "ay",
     wpas_dbus_getter_p2p_group_bssid, NULL, NULL},
    {"Frequency", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "q",
     wpas_dbus_getter_p2p_group_frequency, NULL, NULL},
    {"Passphrase", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "s",
     wpas_dbus_getter_p2p_group_passphrase, NULL, NULL},
    {"PSK", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "ay", wpas_dbus_getter_p2p_group_psk,
     NULL, NULL},
    {"WPSVendorExtensions", WPAS_DBUS_NEW_IFACE_P2P_GROUP, "aay",
     wpas_dbus_getter_p2p_group_vendor_ext,
     wpas_dbus_setter_p2p_group_vendor_ext, NULL},
    {NULL, NULL, NULL, NULL, NULL, NULL}};

static const struct wpa_dbus_signal_desc wpas_dbus_p2p_group_signals[] = {
    {"PeerJoined",
     WPAS_DBUS_NEW_IFACE_P2P_GROUP,
     {{"peer", "o", ARG_OUT}, END_ARGS}},
    {"PeerDisconnected",
     WPAS_DBUS_NEW_IFACE_P2P_GROUP,
     {{"peer", "o", ARG_OUT}, END_ARGS}},
    {NULL, NULL, {END_ARGS}}};
...
void wpas_dbus_register_p2p_group(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->dbus_groupobj_path) {
    wpa_printf(MSG_INFO, "%s: Group object '%s' already exists", __func__,
               wpa_s->dbus_groupobj_path);
    return;
  }
  ...
  wpa_printf(MSG_INFO, "dbus: Register group object '%s'", group_obj_path);
  obj_desc = os_zalloc(sizeof(struct wpa_dbus_object_desc));
  if (!obj_desc) {
    wpa_printf(MSG_ERROR, "Not enough memory to create object description");
    goto err;
...
void wpas_dbus_unregister_p2p_group(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->dbus_groupobj_path) {
    wpa_printf(MSG_DEBUG, "%s: Group object '%s' already unregistered",
  ...
  wpa_dbus_unregister_object_per_iface(ctrl_iface, wpa_s->dbus_groupobj_path);
...
static const struct wpa_dbus_property_desc
    wpas_dbus_persistent_group_properties[] = {
        {"Properties", WPAS_DBUS_NEW_IFACE_PERSISTENT_GROUP, "a{sv}",
         ...
         wpas_dbus_setter_persistent_group_properties, NULL},
        {NULL, NULL, NULL, NULL, NULL, NULL}};
...
int wpas_dbus_register_persistent_group(struct wpa_supplicant *wpa_s,
  ...
  wpas_dbus_register(obj_desc, arg, wpa_dbus_free, NULL,
                     wpas_dbus_persistent_group_properties, NULL);
