...
static int wpas_dbus_handler_wps_role(DBusMessage *message,
  ...
  if (dbus_message_iter_get_arg_type(&variant_iter) != DBUS_TYPE_STRING) {
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong Role type, string required");
    *reply = wpas_dbus_error_invalid_args(message, "Role must be a string");
    return -1;
  }
...
static int wpas_dbus_handler_wps_type(DBusMessage *message,
  ...
  dbus_message_iter_recurse(entry_iter, &variant_iter);
  if (dbus_message_iter_get_arg_type(&variant_iter) != DBUS_TYPE_STRING) {
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong Type type, string required");
    *reply = wpas_dbus_error_invalid_args(message, "Type must be a string");
    return -1;
  }
  dbus_message_iter_get_basic(&variant_iter, &val);
  if (os_strcmp(val, "pin") == 0)
    ...
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Unknown type %s", val);
    *reply = wpas_dbus_error_invalid_args(message, val);
    return -1;
  }
  return 0;
...
static int wpas_dbus_handler_wps_bssid(DBusMessage *message,
  ...
  if (dbus_message_iter_get_arg_type(&variant_iter) != DBUS_TYPE_ARRAY ||
    ...
    *reply =
        wpas_dbus_error_invalid_args(message, "Bssid must be a byte array");
    return -1;
  }
  dbus_message_iter_recurse(&variant_iter, &array_iter);
  dbus_message_iter_get_fixed_array(&array_iter, &params->bssid, &len);
  if (len != ETH_ALEN) {
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong Bssid length %d", len);
    *reply = wpas_dbus_error_invalid_args(message, "Bssid is wrong length");
    return -1;
  }
  return 0;
}
...
static int wpas_dbus_handler_wps_pin(DBusMessage *message,
  ...
  dbus_message_iter_recurse(entry_iter, &variant_iter);
  if (dbus_message_iter_get_arg_type(&variant_iter) != DBUS_TYPE_STRING) {
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong Pin type, string required");
    *reply = wpas_dbus_error_invalid_args(message, "Pin must be a string");
    return -1;
  }
...
static int wpas_dbus_handler_wps_p2p_dev_addr(DBusMessage *message,
  ...
  dbus_message_iter_recurse(&variant_iter, &array_iter);
  dbus_message_iter_get_fixed_array(&array_iter, &params->p2p_dev_addr, &len);
  if (len != ETH_ALEN) {
    wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Wrong P2PDeviceAddress length %d",
               len);
    *reply = wpas_dbus_error_invalid_args(message,
                                          "P2PDeviceAddress has wrong length");
    return -1;
  }
  return 0;
}
...
static int wpas_dbus_handler_wps_start_entry(DBusMessage *message, char *key,
  ...
  if (os_strcmp(key, "Role") == 0)
    return wpas_dbus_handler_wps_role(message, entry_iter, params, reply);
  else if (os_strcmp(key, "Type") == 0)
    return wpas_dbus_handler_wps_type(message, entry_iter, params, reply);
  else if (os_strcmp(key, "Bssid") == 0)
    return wpas_dbus_handler_wps_bssid(message, entry_iter, params, reply);
  else if (os_strcmp(key, "Pin") == 0)
    return wpas_dbus_handler_wps_pin(message, entry_iter, params, reply);
#ifdef CONFIG_P2P
  else if (os_strcmp(key, "P2PDeviceAddress") == 0)
    return wpas_dbus_handler_wps_p2p_dev_addr(message, entry_iter, params,
                                              reply);
#endif /* CONFIG_P2P */
...
DBusMessage *wpas_dbus_handler_wps_start(DBusMessage *message,
  ...
  while (dbus_message_iter_get_arg_type(&dict_iter) == DBUS_TYPE_DICT_ENTRY) {
    ...
    if (wpas_dbus_handler_wps_start_entry(message, key, &entry_iter, &params,
                                          &reply))
      return reply;
  ...
  if (wpa_s->ap_iface && params.type == 1) {
    ...
    ret = wpa_supplicant_ap_wps_pin(wpa_s, params.bssid, params.pin, npin,
                                    sizeof(npin), 0);
  } else if (wpa_s->ap_iface) {
    ret = wpa_supplicant_ap_wps_pbc(wpa_s, params.bssid, params.p2p_dev_addr);
  } else
#endif /* CONFIG_AP */
    if (params.role == 0) {
      wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Role not specified");
      return wpas_dbus_error_invalid_args(message, "Role not specified");
      ...
      ret = wpas_wps_start_reg(wpa_s, params.bssid, params.pin, NULL);
    } else if (params.type == 0) {
      wpa_printf(MSG_DEBUG, "dbus: WPS.Start - Type not specified");
      return wpas_dbus_error_invalid_args(message, "Type not specified");
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG,
               ...
               (params.type == 0 ? "" : (params.type == 1 ? "pin" : "pbc")));
    return wpas_dbus_error_unknown_error(message, "WPS start failed");
  }
...
DBusMessage *wpas_dbus_handler_wps_cancel(DBusMessage *message,
  ...
  if (wpas_wps_cancel(wpa_s))
    return wpas_dbus_error_unknown_error(message, "WPS cancel failed");
...
dbus_bool_t wpas_dbus_getter_process_credentials(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &process,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_config_methods(
  ...
  if (methods == NULL)
    methods = "";
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &methods,
                                          error);
}
