...
static void dispatch_data(DBusConnection *con) {
  while (dbus_connection_get_dispatch_status(con) == DBUS_DISPATCH_DATA_REMAINS)
...
static void process_watch(struct wpas_dbus_priv *priv, DBusWatch *watch,
...
static dbus_bool_t add_watch(DBusWatch *watch, void *data) {
  ...
  eloop_register_sock(fd, EVENT_TYPE_EXCEPTION, process_watch_exception, priv,
                      watch);

  if (flags & DBUS_WATCH_READABLE) {
    eloop_register_sock(fd, EVENT_TYPE_READ, process_watch_read, priv, watch);
  }
  if (flags & DBUS_WATCH_WRITABLE) {
    eloop_register_sock(fd, EVENT_TYPE_WRITE, process_watch_write, priv, watch);
  }
...
static int integrate_with_eloop(struct wpas_dbus_priv *priv) {
  if (!dbus_connection_set_watch_functions(priv->con, add_watch, remove_watch,
                                           watch_toggled, priv, NULL) ||
  ...
  if (eloop_register_signal(SIGPOLL, process_wakeup_main, priv))
    return -1;
  dbus_connection_set_wakeup_main_function(priv->con, wakeup_main, priv, NULL);
...
static int wpas_dbus_init_common(struct wpas_dbus_priv *priv) {
  ...
  if (priv->con) {
    dbus_connection_add_filter(priv->con, disconnect_filter, priv, NULL);
  } else {
    wpa_printf(MSG_ERROR, "dbus: Could not acquire the system bus: %s - %s",
...
static int wpas_dbus_init_common_finish(struct wpas_dbus_priv *priv) {
   ...
   */
  eloop_register_timeout(0, 50, dispatch_initial_dbus_messages, priv->con,
                         NULL);
...
static void wpas_dbus_deinit_common(struct wpas_dbus_priv *priv) {
  if (priv->con) {
    eloop_cancel_timeout(dispatch_initial_dbus_messages, priv->con, NULL);
    eloop_cancel_timeout(process_timeout, priv, ELOOP_ALL_CTX);

    dbus_connection_set_watch_functions(priv->con, NULL, NULL, NULL, NULL,
                                        NULL);
    dbus_connection_set_timeout_functions(priv->con, NULL, NULL, NULL, NULL,
                                          NULL);
    dbus_connection_remove_filter(priv->con, disconnect_filter, priv);
