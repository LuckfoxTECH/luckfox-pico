...
static dbus_bool_t
_wpa_dbus_add_dict_entry_start(DBusMessageIter *iter_dict,
                               DBusMessageIter *iter_dict_entry,
                               const char *key, const int value_type) {
  if (!dbus_message_iter_open_container(iter_dict, DBUS_TYPE_DICT_ENTRY, NULL,
                                        iter_dict_entry))
    return FALSE;
...
static dbus_bool_t
_wpa_dbus_add_dict_entry_end(DBusMessageIter *iter_dict,
                             DBusMessageIter *iter_dict_entry,
...
static dbus_bool_t _wpa_dbus_add_dict_entry_basic(DBusMessageIter *iter_dict,
  ...
  if (!_wpa_dbus_add_dict_entry_start(iter_dict, &iter_dict_entry, key,
                                      value_type) ||
      !dbus_message_iter_open_container(&iter_dict_entry, DBUS_TYPE_VARIANT,
...
static dbus_bool_t
_wpa_dbus_add_dict_entry_byte_array(DBusMessageIter *iter_dict, const char *key,
                                    const char *value,
  ...
  if (!_wpa_dbus_add_dict_entry_start(iter_dict, &iter_dict_entry, key,
      ...
      !dbus_message_iter_open_container(
          &iter_dict_entry, DBUS_TYPE_VARIANT,
          DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING, &iter_dict_val) ||
...
dbus_bool_t wpa_dbus_dict_append_bool(DBusMessageIter *iter_dict,
  ...
  return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_BOOLEAN,
                                        &value);
}
...
dbus_bool_t wpa_dbus_dict_append_object_path(DBusMessageIter *iter_dict,
  ...
  if (!value)
    return FALSE;
  return _wpa_dbus_add_dict_entry_basic(iter_dict, key, DBUS_TYPE_OBJECT_PATH,
                                        &value);
}
...
dbus_bool_t wpa_dbus_dict_append_byte_array(DBusMessageIter *iter_dict,
  ...
  if (!key || (!value && value_len != 0))
    return FALSE;
  return _wpa_dbus_add_dict_entry_byte_array(iter_dict, key, value, value_len);
}
...
dbus_bool_t wpa_dbus_dict_begin_array(DBusMessageIter *iter_dict,
  ...
  err = os_snprintf(array_type, sizeof(array_type),
                    DBUS_TYPE_ARRAY_AS_STRING "%s", type);
  if (os_snprintf_error(sizeof(array_type), err))
    return FALSE;

  if (!iter_dict || !iter_dict_entry || !iter_dict_val || !iter_array ||
      !_wpa_dbus_add_dict_entry_start(iter_dict, iter_dict_entry, key,
                                      DBUS_TYPE_ARRAY) ||
      !dbus_message_iter_open_container(iter_dict_entry, DBUS_TYPE_VARIANT,
                                        array_type, iter_dict_val))
    return FALSE;

  return dbus_message_iter_open_container(iter_dict_val, DBUS_TYPE_ARRAY, type,
                                          iter_array);
}
...
dbus_bool_t wpa_dbus_dict_begin_string_array(DBusMessageIter *iter_dict,
                                             ...
                                             DBusMessageIter *iter_dict_val,
                                             DBusMessageIter *iter_array) {
  return wpa_dbus_dict_begin_array(iter_dict, key, DBUS_TYPE_STRING_AS_STRING,
...
dbus_bool_t wpa_dbus_dict_bin_array_add_element(DBusMessageIter *iter_array,
  ...
  if (!iter_array || !value ||
      !dbus_message_iter_open_container(iter_array, DBUS_TYPE_ARRAY,
                                        DBUS_TYPE_BYTE_AS_STRING, &iter_bytes))
    return FALSE;

  for (i = 0; i < value_len; i++) {
    if (!dbus_message_iter_append_basic(&iter_bytes, DBUS_TYPE_BYTE,
                                        &(value[i])))
      return FALSE;
  }
...
dbus_bool_t wpa_dbus_dict_append_string_array(DBusMessageIter *iter_dict,
  ...
  if (!key || (!items && num_items != 0) ||
      !wpa_dbus_dict_begin_string_array(iter_dict, key, &iter_dict_entry,
                                        &iter_dict_val, &iter_array))
    return FALSE;

  for (i = 0; i < num_items; i++) {
    if (!wpa_dbus_dict_string_array_add_element(&iter_array, items[i]))
      return FALSE;
  }
...
dbus_bool_t wpa_dbus_dict_append_wpabuf_array(DBusMessageIter *iter_dict,
  ...
  if (!key || (!items && num_items != 0) ||
      !wpa_dbus_dict_begin_array(
          iter_dict, key, DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING,
          &iter_dict_entry, &iter_dict_val, &iter_array))
    return FALSE;

  for (i = 0; i < num_items; i++) {
    if (!wpa_dbus_dict_bin_array_add_element(&iter_array, wpabuf_head(items[i]),
                                             wpabuf_len(items[i])))
      return FALSE;
  }

  return wpa_dbus_dict_end_array(iter_dict, &iter_dict_entry, &iter_dict_val,
                                 &iter_array);
}
...
dbus_bool_t wpa_dbus_dict_open_read(DBusMessageIter *iter,
  ...
  if (type != DBUS_TYPE_ARRAY ||
    ...
    wpa_printf(
        MSG_DEBUG, "%s: unexpected message argument types (arg=%c element=%c)",
        __func__, type,
...
static dbus_bool_t
_wpa_dbus_dict_entry_get_byte_array(DBusMessageIter *iter,
  ...
  while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_BYTE) {
    ...
    if ((count % BYTE_ARRAY_CHUNK_SIZE) == 0 && count != 0) {
      nbuffer = os_realloc_array(buffer, count + BYTE_ARRAY_CHUNK_SIZE,
...
static dbus_bool_t
_wpa_dbus_dict_entry_get_string_array(DBusMessageIter *iter, int array_type,
                                      struct wpa_dbus_dict_entry *entry) {
  dbus_uint32_t count = 0;
  char **buffer, **nbuffer;
  ...
  while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_STRING) {
    ...
    if ((count % STR_ARRAY_CHUNK_SIZE) == 0 && count != 0) {
      nbuffer = os_realloc_array(buffer, count + STR_ARRAY_CHUNK_SIZE,
                                 STR_ARRAY_ITEM_SIZE);
      if (nbuffer == NULL) {
        wpa_printf(MSG_ERROR,
                   "dbus: %s out of memory trying to retrieve the string array",
    ...
    wpa_printf(MSG_MSGDUMP, "%s: string_array value: %s", __func__,
               wpa_debug_show_keys ? value : "[omitted]");
    str = os_strdup(value);
    if (str == NULL) {
      wpa_printf(MSG_ERROR,
  ...
  entry->array_len = count;
  wpa_printf(MSG_MSGDUMP, "%s: string_array length %u", __func__,
             entry->array_len);
...
static dbus_bool_t
_wpa_dbus_dict_entry_get_binarray(DBusMessageIter *iter,
                                  struct wpa_dbus_dict_entry *entry) {
  ...
  if (type != DBUS_TYPE_ARRAY) {
    wpa_printf(MSG_DEBUG, "%s: not an array type: %c", __func__, type);
    return FALSE;
  }

  type = dbus_message_iter_get_element_type(iter);
  if (type != DBUS_TYPE_BYTE) {
    wpa_printf(MSG_DEBUG, "%s: unexpected element type %c", __func__, type);
    return FALSE;
  }
  ...
  while (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_ARRAY) {
    ...
    if (entry->array_len == buflen) {
      ...
      newbuf =
          os_realloc_array(entry->binarray_value, buflen, BIN_ARRAY_ITEM_SIZE);
      if (!newbuf)
        goto cleanup;
    ...
    if (_wpa_dbus_dict_entry_get_byte_array(&iter_array, &tmpentry) == FALSE)
      goto cleanup;

    entry->binarray_value[entry->array_len] = wpabuf_alloc_ext_data(
        (u8 *)tmpentry.bytearray_value, tmpentry.array_len);
  ...
  wpa_printf(MSG_MSGDUMP, "%s: binarray length %u", __func__, entry->array_len);
...
static dbus_bool_t
_wpa_dbus_dict_entry_get_array(DBusMessageIter *iter_dict_val,
                               struct wpa_dbus_dict_entry *entry) {
  ...
  switch (array_type) {
  case DBUS_TYPE_BYTE:
    success = _wpa_dbus_dict_entry_get_byte_array(&iter_array, entry);
    break;
  case DBUS_TYPE_STRING:
    success =
        _wpa_dbus_dict_entry_get_string_array(&iter_array, array_type, entry);
    break;
  case DBUS_TYPE_ARRAY:
    success = _wpa_dbus_dict_entry_get_binarray(&iter_array, entry);
    break;
  default:
    wpa_printf(MSG_MSGDUMP, "%s: unsupported array type %c", __func__,
               array_type);
    break;
  }
...
static dbus_bool_t
_wpa_dbus_dict_fill_value_from_variant(struct wpa_dbus_dict_entry *entry,
  ...
  case DBUS_TYPE_OBJECT_PATH:
    ...
    wpa_printf(MSG_MSGDUMP, "%s: object path value: %s", __func__, v);
  ...
  case DBUS_TYPE_STRING:
    dbus_message_iter_get_basic(iter, &v);
    wpa_printf(MSG_MSGDUMP, "%s: string value: %s", __func__,
               wpa_debug_show_keys ? v : "[omitted]");
    entry->str_value = os_strdup(v);
    if (entry->str_value == NULL)
      return FALSE;
    break;
  case DBUS_TYPE_BOOLEAN:
    ...
    wpa_printf(MSG_MSGDUMP, "%s: boolean value: %d", __func__,
               entry->bool_value);
    break;
  case DBUS_TYPE_BYTE:
    dbus_message_iter_get_basic(iter, &entry->byte_value);
    wpa_printf(MSG_MSGDUMP, "%s: byte value: %d", __func__, entry->byte_value);
    break;
  case DBUS_TYPE_INT16:
    dbus_message_iter_get_basic(iter, &entry->int16_value);
    wpa_printf(MSG_MSGDUMP, "%s: int16 value: %d", __func__,
               entry->int16_value);
    break;
  case DBUS_TYPE_UINT16:
    dbus_message_iter_get_basic(iter, &entry->uint16_value);
    wpa_printf(MSG_MSGDUMP, "%s: uint16 value: %d", __func__,
               entry->uint16_value);
    break;
  case DBUS_TYPE_INT32:
    dbus_message_iter_get_basic(iter, &entry->int32_value);
    wpa_printf(MSG_MSGDUMP, "%s: int32 value: %d", __func__,
               entry->int32_value);
    break;
  case DBUS_TYPE_UINT32:
    dbus_message_iter_get_basic(iter, &entry->uint32_value);
    wpa_printf(MSG_MSGDUMP, "%s: uint32 value: %d", __func__,
               entry->uint32_value);
    break;
  case DBUS_TYPE_INT64:
    dbus_message_iter_get_basic(iter, &entry->int64_value);
    wpa_printf(MSG_MSGDUMP, "%s: int64 value: %lld", __func__,
               (long long int)entry->int64_value);
    break;
  case DBUS_TYPE_UINT64:
    dbus_message_iter_get_basic(iter, &entry->uint64_value);
    wpa_printf(MSG_MSGDUMP, "%s: uint64 value: %llu", __func__,
               (unsigned long long int)entry->uint64_value);
    break;
  case DBUS_TYPE_DOUBLE:
    dbus_message_iter_get_basic(iter, &entry->double_value);
    wpa_printf(MSG_MSGDUMP, "%s: double value: %f", __func__,
               entry->double_value);
    break;
  case DBUS_TYPE_ARRAY:
    return _wpa_dbus_dict_entry_get_array(iter, entry);
  default:
    wpa_printf(MSG_MSGDUMP, "%s: unsupported type %c", __func__, entry->type);
    return FALSE;
  }
...
dbus_bool_t wpa_dbus_dict_get_entry(DBusMessageIter *iter_dict,
  ...
  if (type != DBUS_TYPE_VARIANT) {
    wpa_printf(MSG_DEBUG, "%s: unexpected dict entry variant type: %c",
               __func__, type);
    goto error;
  }
  ...
  wpa_printf(MSG_MSGDUMP, "%s: dict entry variant content type: %c", __func__,
             entry->type);
  entry->array_type = DBUS_TYPE_INVALID;
  if (!_wpa_dbus_dict_fill_value_from_variant(entry, &iter_dict_val)) {
    wpa_printf(MSG_DEBUG, "%s: failed to fetch dict values from variant",
               __func__);
    goto error;
  }
