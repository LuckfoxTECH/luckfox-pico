  ...
  if (dbus_message_iter_next(&iter)) {
    ...
    while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
      if (!wpa_dbus_dict_get_entry(&iter_dict, &entry))
        goto error;
      if (!strcmp(entry.key, "driver") && entry.type == DBUS_TYPE_STRING) {
        os_free(driver);
...
DBusMessage *wpas_dbus_global_get_interface(DBusMessage *message,
  ...
  if (!dbus_message_get_args(message, NULL, DBUS_TYPE_STRING, &ifname,
                             DBUS_TYPE_INVALID)) {
    reply = wpas_dbus_new_invalid_opts_error(message, NULL);
    goto out;
  }
...
DBusMessage *wpas_dbus_global_set_debugparams(DBusMessage *message,
  ...
  if (!dbus_message_get_args(message, NULL, DBUS_TYPE_INT32, &debug_level,
...
DBusMessage *wpas_dbus_iface_scan_results(DBusMessage *message,
  ...
  if (!dbus_message_iter_open_container(
          &iter, DBUS_TYPE_ARRAY, DBUS_TYPE_OBJECT_PATH_AS_STRING, &sub_iter))
    goto error;
  ...
  dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
    ...
    os_snprintf(path, WPAS_DBUS_OBJECT_PATH_MAX,
                "%s/" WPAS_DBUS_BSSIDS_PART "/" WPAS_DBUS_BSSID_FORMAT,
                wpa_s->dbus_path, MAC2STR(bss->bssid));
    if (!dbus_message_iter_append_basic(&sub_iter, DBUS_TYPE_OBJECT_PATH,
                                        &path))
      goto error;
  }
...
DBusMessage *wpas_dbus_iface_capabilities(DBusMessage *message,
  ...
  if (eap_methods) {
    ...
    success = wpa_dbus_dict_append_string_array(
        &iter_dict, "eap", (const char **)eap_methods, num_items);
  ...
  if (res < 0) {
    if (!strict) {
      const char *args[] = {"WPA-PSK", "WPA-EAP", "IEEE8021X", "WPA-NONE",
                            "NONE"};
      if (!wpa_dbus_dict_append_string_array(&iter_dict, "key_mgmt", args,
                                             ARRAY_SIZE(args)))
        goto error;
    }
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "key_mgmt",
        ...
        ((capa.key_mgmt &
         ...
         !wpa_dbus_dict_string_array_add_element(&iter_array, "WPA-EAP")) ||
        ((capa.key_mgmt & (WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK |
                           WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK)) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "WPA-PSK")) ||
        ((capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_WPA_NONE) &&
  ...
  if (res < 0) {
    if (!strict) {
      ...
      if (!wpa_dbus_dict_append_string_array(&iter_dict, "proto", args,
                                             ARRAY_SIZE(args)))
        goto error;
    }
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "proto", &iter_dict_entry,
                                          &iter_dict_val, &iter_array) ||
  ...
  if (res < 0) {
    if (!strict) {
      ...
      if (!wpa_dbus_dict_append_string_array(&iter_dict, "auth_alg", args,
                                             ARRAY_SIZE(args)))
        goto error;
    }
  } else {
...
DBusMessage *wpas_dbus_iface_add_network(DBusMessage *message,
  ...
  os_snprintf(path, WPAS_DBUS_OBJECT_PATH_MAX,
              "%s/" WPAS_DBUS_NETWORKS_PART "/%d", wpa_s->dbus_path, ssid->id);
...
DBusMessage *wpas_dbus_iface_select_network(DBusMessage *message,
  ...
  if (os_strlen(dbus_message_get_signature(message)) == 0) {
    ...
    iface_obj_path = wpas_dbus_decompose_object_path(op, &network, NULL);
 ...
 */
DBusMessage *
wpas_dbus_iface_set_smartcard_modules(DBusMessage *message,
                                      struct wpa_supplicant *wpa_s) {
  DBusMessageIter iter, iter_dict;
...
DBusMessage *wpas_dbus_iface_get_scanning(DBusMessage *message,
  ...
  if (reply != NULL) {
    ...
    wpa_printf(MSG_ERROR, "dbus: Not enough memory to return scanning state");
  }
...
DBusMessage *wpas_dbus_iface_set_blobs(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    if (entry.type != DBUS_TYPE_ARRAY || entry.array_type != DBUS_TYPE_BYTE) {
      reply = wpas_dbus_new_invalid_opts_error(message, "Byte array expected.");
      break;
    }

    if ((entry.array_len <= 0) || (entry.array_len > 65536) ||
        !strlen(entry.key)) {
      reply = wpas_dbus_new_invalid_opts_error(message, "Invalid array size.");
      break;
    }

    blob = os_zalloc(sizeof(*blob));
    if (blob == NULL) {
      reply = dbus_message_new_error(message, WPAS_ERROR_ADD_ERROR,
    ...
    os_memcpy(blob->data, (u8 *)entry.bytearray_value, entry.array_len);
    if (blob->name == NULL) {
...
DBusMessage *wpas_dbus_iface_remove_blobs(DBusMessage *message,
  ...
  if (err_msg)
    return dbus_message_new_error(message, WPAS_ERROR_REMOVE_ERROR, err_msg);
...
DBusMessage *wpas_dbus_iface_flush(DBusMessage *message,
  ...
  if (os_strlen(dbus_message_get_signature(message)) != 0 &&
      !dbus_message_get_args(message, NULL, DBUS_TYPE_INT32, &flush_age,
                             DBUS_TYPE_INVALID)) {
    return wpas_dbus_new_invalid_opts_error(message, NULL);
  }
