...
static void add_entry(struct wpabuf *xml, const char *type, const char *name,
  ...
  for (arg = args; arg && arg->name; arg++) {
    add_arg(xml, arg->name, arg->type,
            include_dir ? (arg->dir == ARG_IN ? "in" : "out") : NULL);
  }
...
static void add_property(struct wpabuf *xml,
  ...
  wpabuf_printf(xml,
                ...
                "access=\"%s%s\"/>",
                dsc->dbus_property, dsc->type, dsc->getter ? "read" : "",
                dsc->setter ? "write" : "");
}

static void
extract_interfaces_methods(struct dl_list *list,
  ...
  for (dsc = methods; dsc && dsc->dbus_method; dsc++) {
    ...
    if (iface)
      add_entry(iface->xml, "method", dsc->dbus_method, dsc->args, 1);
  }
}

static void
extract_interfaces_signals(struct dl_list *list,
  ...
  for (dsc = signals; dsc && dsc->dbus_signal; dsc++) {
    iface = add_interface(list, dsc->dbus_interface);
    if (iface)
      add_entry(iface->xml, "signal", dsc->dbus_signal, dsc->args, 0);
  }
}

static void
extract_interfaces_properties(struct dl_list *list,
  ...
  dl_list_for_each_safe(iface, n, list, struct interfaces, list) {
    if (wpabuf_len(iface->xml) + 20 < wpabuf_tailroom(xml)) {
      ...
      wpa_printf(MSG_DEBUG,
                 "dbus: Not enough room for add_interfaces inspect data: "
                 "tailroom %u, add %u",
...
static void add_properties_interface(struct wpabuf *xml) {
  wpabuf_printf(xml, "<interface name=\"%s\">", WPA_DBUS_PROPERTIES_INTERFACE);
...
DBusMessage *wpa_dbus_introspect(DBusMessage *message,
  ...
  add_child_nodes(xml, obj_dsc->connection, dbus_message_get_path(message));
