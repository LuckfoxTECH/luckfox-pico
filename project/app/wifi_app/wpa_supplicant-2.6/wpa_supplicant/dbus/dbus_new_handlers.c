...
DBusMessage *wpas_dbus_error_unknown_error(DBusMessage *message,
                                           const char *arg) {
  return dbus_message_new_error(message, WPAS_DBUS_ERROR_UNKNOWN_ERROR, arg);
}
...
static DBusMessage *wpas_dbus_error_scan_error(DBusMessage *message,
                                               const char *error) {
  return dbus_message_new_error(message, WPAS_DBUS_ERROR_IFACE_SCAN_ERROR,
                                error);
}
...
static const char *const dont_quote[] = {"key_mgmt",
                                         "proto",
                                         "pairwise",
                                         "auth_alg",
                                         "group",
...
static struct wpa_supplicant *get_iface_by_dbus_path(struct wpa_global *global,
  ...
  for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    if (wpa_s->dbus_new_path && os_strcmp(wpa_s->dbus_new_path, path) == 0)
      return wpa_s;
  }
  return NULL;
}
...
dbus_bool_t set_network_properties(struct wpa_supplicant *wpa_s,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    ...
    value = NULL;
    if (entry.type == DBUS_TYPE_ARRAY && entry.array_type == DBUS_TYPE_BYTE) {
      ...
      if (should_quote_opt(entry.key)) {
        ...
        ret = os_snprintf(value, size, "\"%s\"", entry.str_value);
    ...
    if ((os_strcmp(entry.key, "psk") == 0 && value[0] == '"' &&
         ssid->ssid_len) ||
...
dbus_bool_t wpas_dbus_simple_property_getter(DBusMessageIter *iter,
  ...
  if (!dbus_type_is_basic(type)) {
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: given type is not basic",
                   __func__);
    return FALSE;
  }
  ...
  if (!dbus_message_iter_open_container(iter, DBUS_TYPE_VARIANT,
    ...
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: error constructing reply",
                   __func__);
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_simple_property_setter(DBusMessageIter *iter,
                                             DBusError *error, const int type,
                                             void *val) {
  DBusMessageIter variant_iter;
  ...
  if (dbus_message_iter_get_arg_type(&variant_iter) != type) {
    dbus_set_error_const(error, DBUS_ERROR_FAILED, "wrong property type");
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_simple_array_property_getter(DBusMessageIter *iter,
  ...
  if (!dbus_message_iter_open_container(iter, DBUS_TYPE_VARIANT, type_str,
                                        &variant_iter) ||
      !dbus_message_iter_open_container(&variant_iter, DBUS_TYPE_ARRAY,
                                        sub_type_str, &array_iter)) {
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: failed to construct message",
                   __func__);
    return FALSE;
  }
  ...
  case DBUS_TYPE_OBJECT_PATH:
    ...
    break;
  default:
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: unknown element type %d",
                   __func__, type);
    return FALSE;
  }
  ...
  for (i = 0; i < array_len; i++) {
    if (!dbus_message_iter_append_basic(
            &array_iter, type, (const char *)array + i * element_size)) {
      dbus_set_error(error, DBUS_ERROR_FAILED,
                     "%s: failed to construct message 2.5", __func__);
      return FALSE;
    }
  }
...
dbus_bool_t wpas_dbus_simple_array_array_property_getter(DBusMessageIter *iter,
  ...
  for (i = 0; i < array_len && array[i]; i++) {
    wpa_dbus_dict_bin_array_add_element(&array_iter, wpabuf_head(array[i]),
                                        wpabuf_len(array[i]));
  }

  if (!dbus_message_iter_close_container(&variant_iter, &array_iter) ||
      !dbus_message_iter_close_container(iter, &variant_iter)) {
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: failed to close message",
                   __func__);
    return FALSE;
  }
...
DBusMessage *wpas_dbus_handler_create_interface(DBusMessage *message,
  ...
  while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
    if (!wpa_dbus_dict_get_entry(&iter_dict, &entry))
      goto error;
    if (os_strcmp(entry.key, "Driver") == 0 && entry.type == DBUS_TYPE_STRING) {
      os_free(driver);
  ...
  if (wpa_supplicant_get_iface(global, ifname) != NULL) {
    ...
    if (wpa_s && wpa_s->dbus_new_path) {
      ...
      reply = wpas_dbus_error_unknown_error(
          message, "wpa_supplicant couldn't grab this interface.");
    }
  }
...
DBusMessage *wpas_dbus_handler_remove_interface(DBusMessage *message,
  ...
  if (wpa_s == NULL)
    ...
    reply = wpas_dbus_error_unknown_error(
        message, "wpa_supplicant couldn't remove this interface.");
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_debug_level(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  const char *str;
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &str, error);
}
...
dbus_bool_t wpas_dbus_getter_debug_timestamp(
    const struct wpa_dbus_property_desc *property_desc, DBusMessageIter *iter,
 ...
 */
dbus_bool_t
wpas_dbus_setter_debug_level(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  ...
  if (!wpas_dbus_simple_property_setter(iter, error, DBUS_TYPE_STRING, &str))
    return FALSE;
  ...
  if (val < 0 || wpa_supplicant_set_debug_params(
                     global, val, wpa_debug_timestamp, wpa_debug_show_keys)) {
    dbus_set_error_const(error, DBUS_ERROR_FAILED, "wrong debug level value");
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_setter_debug_timestamp(
  ...
  if (!wpas_dbus_simple_property_setter(iter, error, DBUS_TYPE_BOOLEAN, &val))
    return FALSE;
...
dbus_bool_t wpas_dbus_setter_debug_show_keys(
  ...
  wpa_supplicant_set_debug_params(global, wpa_debug_level, wpa_debug_timestamp,
                                  val ? 1 : 0);
  return TRUE;
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_interfaces(const struct wpa_dbus_property_desc *property_desc,
                            DBusMessageIter *iter, DBusError *error,
                            void *user_data) {
  struct wpa_global *global = user_data;
  struct wpa_supplicant *wpa_s;
  ...
  success = wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_OBJECT_PATH,
                                                   paths, num, error);
 ...
 */
dbus_bool_t
wpas_dbus_getter_eap_methods(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  char **eap_methods;
  size_t num_items = 0;
  ...
  success = wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_STRING, eap_methods, num_items, error);
...
dbus_bool_t wpas_dbus_getter_global_capabilities(
  ...
  return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_STRING,
                                                capabilities, num_items, error);
}

static int wpas_dbus_get_scan_type(DBusMessage *message, DBusMessageIter *var,
                                   char **type, DBusMessage **reply) {
  if (dbus_message_iter_get_arg_type(var) != DBUS_TYPE_STRING) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: Type must be a string", __func__);
...
static int wpas_dbus_get_scan_ssids(DBusMessage *message, DBusMessageIter *var,
  ...
  if (dbus_message_iter_get_arg_type(var) != DBUS_TYPE_ARRAY) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: ssids must be an array of arrays of bytes",
               __func__);
    *reply = wpas_dbus_error_invalid_args(
        message, "Wrong SSIDs value type. Array of arrays of bytes required");
    return -1;
  }
  ...
  while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_ARRAY) {
    if (ssids_num >= WPAS_MAX_SCAN_SSIDS) {
      wpa_printf(MSG_DEBUG,
                 ...
                 __func__);
      *reply = wpas_dbus_error_invalid_args(
          message, "Too many ssids specified. Specify at most four");
      return -1;
    }
    ...
    if (len > SSID_MAX_LEN) {
      wpa_printf(MSG_DEBUG, "%s[dbus]: SSID too long (len=%d max_len=%d)",
                 __func__, len, SSID_MAX_LEN);
      *reply = wpas_dbus_error_invalid_args(message, "Invalid SSID: too long");
      return -1;
    }
...
static int wpas_dbus_get_scan_ies(DBusMessage *message, DBusMessageIter *var,
  ...
  if (dbus_message_iter_get_arg_type(var) != DBUS_TYPE_ARRAY) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: ies must be an array of arrays of bytes",
               __func__);
    *reply = wpas_dbus_error_invalid_args(
        message, "Wrong IEs value type. Array of arrays of bytes required");
    return -1;
  }
...
static int wpas_dbus_get_scan_channels(DBusMessage *message,
  ...
  if (dbus_message_iter_get_arg_type(var) != DBUS_TYPE_ARRAY) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: Channels must be an array of structs",
               __func__);
    *reply = wpas_dbus_error_invalid_args(
        message, "Wrong Channels value type. Array of structs required");
    return -1;
  }
  ...
  while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_STRUCT) {
    ...
    if (dbus_message_iter_get_arg_type(&sub_array_iter) != DBUS_TYPE_UINT32) {
      wpa_printf(
          MSG_DEBUG,
          "%s[dbus]: Channel must by specified by struct of two UINT32s %c",
          __func__, dbus_message_iter_get_arg_type(&sub_array_iter));
      *reply = wpas_dbus_error_invalid_args(
          message, "Wrong Channel struct. Two UINT32s required");
      os_free(freqs);
      return -1;
    }
    ...
    if (!dbus_message_iter_next(&sub_array_iter) ||
        dbus_message_iter_get_arg_type(&sub_array_iter) != DBUS_TYPE_UINT32) {
      wpa_printf(MSG_DEBUG,
...
#define FREQS_ALLOC_CHUNK 32
    if (freqs_num % FREQS_ALLOC_CHUNK == 0) {
      nfreqs =
          os_realloc_array(freqs, freqs_num + FREQS_ALLOC_CHUNK, sizeof(int));
      if (nfreqs == NULL)
        os_free(freqs);
      freqs = nfreqs;
    }
...
static int wpas_dbus_get_scan_allow_roam(DBusMessage *message,
  ...
  if (dbus_message_iter_get_arg_type(var) != DBUS_TYPE_BOOLEAN) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: Type must be a boolean", __func__);
    *reply = wpas_dbus_error_invalid_args(
...
DBusMessage *wpas_dbus_handler_scan(DBusMessage *message,
  ...
  while (dbus_message_iter_get_arg_type(&dict_iter) == DBUS_TYPE_DICT_ENTRY) {
    ...
    if (os_strcmp(key, "Type") == 0) {
      if (wpas_dbus_get_scan_type(message, &variant_iter, &type, &reply) < 0)
        goto out;
    } else if (os_strcmp(key, "SSIDs") == 0) {
      if (wpas_dbus_get_scan_ssids(message, &variant_iter, &params, &reply) < 0)
        goto out;
    } else if (os_strcmp(key, "IEs") == 0) {
      if (wpas_dbus_get_scan_ies(message, &variant_iter, &params, &reply) < 0)
        goto out;
    } else if (os_strcmp(key, "Channels") == 0) {
      if (wpas_dbus_get_scan_channels(message, &variant_iter, &params, &reply) <
          0)
        goto out;
    } else if (os_strcmp(key, "AllowRoam") == 0) {
      if (wpas_dbus_get_scan_allow_roam(message, &variant_iter, &allow_roam,
                                        &reply) < 0)
        goto out;
    } else {
      wpa_printf(MSG_DEBUG, "%s[dbus]: Unknown argument %s", __func__, key);
  ...
  if (!type) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: Scan type not specified", __func__);
    reply = wpas_dbus_error_invalid_args(message, key);
    goto out;
  }
  ...
  if (os_strcmp(type, "passive") == 0) {
    if (params.num_ssids || params.extra_ies_len) {
      wpa_printf(MSG_DEBUG,
                 ...
                 __func__);
      reply = wpas_dbus_error_invalid_args(
          message, "You can specify only Channels in passive scan");
      goto out;
    } else {
      if (wpa_s->sched_scanning) {
        wpa_printf(MSG_DEBUG,
                   "%s[dbus]: Stop ongoing sched_scan to allow requested scan "
                   "to proceed",
                   __func__);
      ...
      if (params.freqs && params.freqs[0]) {
        ...
        if (wpa_supplicant_trigger_scan(wpa_s, &params)) {
          reply = wpas_dbus_error_scan_error(message, "Scan request rejected");
        }
      } else {
    ...
    if (wpa_s->sched_scanning) {
      wpa_printf(MSG_DEBUG,
                 "%s[dbus]: Stop ongoing sched_scan to allow requested scan to "
                 "proceed",
                 __func__);
      wpa_supplicant_cancel_sched_scan(wpa_s);
    }
    ...
    wpa_printf(MSG_DEBUG, "%s[dbus]: Unknown scan type: %s", __func__, type);
    reply = wpas_dbus_error_invalid_args(message, "Wrong scan type");
    goto out;
  }
...
DBusMessage *wpas_dbus_handler_signal_poll(DBusMessage *message,
  ...
  if (!dbus_message_iter_open_container(&iter, DBUS_TYPE_VARIANT, "a{sv}",
      ...
      !wpa_dbus_dict_append_int32(&iter_dict, "rssi", si.current_signal) ||
      !wpa_dbus_dict_append_int32(&iter_dict, "linkspeed",
                                  si.current_txrate / 1000) ||
      !wpa_dbus_dict_append_int32(&iter_dict, "noise", si.current_noise) ||
      !wpa_dbus_dict_append_uint32(&iter_dict, "frequency", si.frequency) ||
      (si.chanwidth != CHAN_WIDTH_UNKNOWN &&
       !wpa_dbus_dict_append_string(&iter_dict, "width",
...
DBusMessage *wpas_dbus_handler_add_network(DBusMessage *message,
  ...
  if (ssid == NULL) {
    wpa_printf(MSG_ERROR, "%s[dbus]: can't add new interface.", __func__);
    reply = wpas_dbus_error_unknown_error(
        message, "wpa_supplicant could not add a network on this interface.");
    goto err;
  }
  ...
  if (!set_network_properties(wpa_s, ssid, &iter, &error)) {
    ...
    reply = wpas_dbus_reply_new_from_error(
        message, &error, DBUS_ERROR_INVALID_ARGS, "Failed to add network");
  ...
  os_snprintf(path, WPAS_DBUS_OBJECT_PATH_MAX,
              "%s/" WPAS_DBUS_NEW_NETWORKS_PART "/%d", wpa_s->dbus_new_path,
              ssid->id);
 ...
 * This will prevent wpa_supplicant from adding blacklists upon next
 * disconnect..
 */
...
DBusMessage *wpas_dbus_handler_remove_network(DBusMessage *message,
  ...
  iface = wpas_dbus_new_decompose_object_path(op, WPAS_DBUS_NEW_NETWORKS_PART,
                                              &net_id);
  ...
  if (result == -2) {
    wpa_printf(MSG_ERROR, "%s[dbus]: error occurred when removing network %d",
               __func__, id);
    reply = wpas_dbus_error_unknown_error(
        message, "error removing the specified network on is interface.");
    goto out;
  }
 ...
 */
DBusMessage *
wpas_dbus_handler_remove_all_networks(DBusMessage *message,
                                      struct wpa_supplicant *wpa_s) {
  if (wpa_s->sched_scanning)
    wpa_supplicant_cancel_sched_scan(wpa_s);
...
DBusMessage *wpas_dbus_handler_network_reply(DBusMessage *message,
  ...
  if (!dbus_message_get_args(message, NULL, DBUS_TYPE_OBJECT_PATH, &op,
                             DBUS_TYPE_STRING, &field, DBUS_TYPE_STRING, &value,
                             DBUS_TYPE_INVALID))
  ...
  if (wpa_supplicant_ctrl_iface_ctrl_rsp_handle(wpa_s, ssid, field, value) < 0)
...
DBusMessage *wpas_dbus_handler_add_blob(DBusMessage *message,
  ...
  if (wpa_config_get_blob(wpa_s->conf, blob_name)) {
    return dbus_message_new_error(message, WPAS_DBUS_ERROR_BLOB_EXISTS, NULL);
  }
...
DBusMessage *wpas_dbus_handler_get_blob(DBusMessage *message,
  ...
  if (!dbus_message_iter_open_container(
          &iter, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE_AS_STRING, &array_iter) ||
...
static int get_peer_hwaddr_helper(DBusMessage *message, const char *func_name,
  ...
  *error = NULL;
  ...
  if (!dbus_message_get_args(message, NULL, DBUS_TYPE_STRING, &peer_string,
                             DBUS_TYPE_INVALID)) {
  ...
  if (hwaddr_aton(peer_string, peer_address)) {
    wpa_printf(MSG_DEBUG, "%s: invalid address '%s'", func_name, peer_string);
    *error = wpas_dbus_error_invalid_args(message,
...
DBusMessage *wpas_dbus_handler_tdls_status(DBusMessage *message,
  ...
  reply = dbus_message_new_method_return(message);
  dbus_message_append_args(reply, DBUS_TYPE_STRING, &tdls_status,
                           DBUS_TYPE_INVALID);
  return reply;
}
...
DBusMessage *wpas_dbus_handler_save_config(DBusMessage *message,
  ...
  if (!wpa_s->conf->update_config) {
    return wpas_dbus_error_unknown_error(
        message, "Not allowed to update configuration (update_config=0)");
  }
...
/**
 * wpas_dbus_handler_set_pkcs11_engine_and_module_path - Set PKCS #11 engine and
 * module path
 * @message: Pointer to incoming dbus message
 * @wpa_s: %wpa_supplicant data structure
 * Returns: A dbus message containing an error on failure or NULL on success
...
DBusMessage *wpas_dbus_handler_set_pkcs11_engine_and_module_path(
  ...
  if (wpa_s->dbus_new_path) {
    wpa_dbus_mark_property_changed(wpa_s->global->dbus, wpa_s->dbus_new_path,
...
dbus_bool_t wpas_dbus_getter_capabilities(
  ...
  if (res < 0) {
    ...
    if (!wpa_dbus_dict_append_string_array(&iter_dict, "Pairwise", args,
                                           ARRAY_SIZE(args)))
      goto nomem;
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "Pairwise",
        ...
        ((capa.enc & WPA_DRIVER_CAPA_ENC_CCMP_256) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "ccmp-256")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_GCMP_256) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "gcmp-256")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_CCMP) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "ccmp")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_GCMP) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "gcmp")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_TKIP) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "tkip")) ||
        ((capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_WPA_NONE) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "none")) ||
        !wpa_dbus_dict_end_string_array(&iter_dict, &iter_dict_entry,
  ...
  /***** group cipher */
  if (res < 0) {
    const char *args[] = {"ccmp", "tkip", "wep104", "wep40"};

    if (!wpa_dbus_dict_append_string_array(&iter_dict, "Group", args,
                                           ARRAY_SIZE(args)))
      goto nomem;
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "Group", &iter_dict_entry,
                                          &iter_dict_val, &iter_array) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_CCMP_256) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "ccmp-256")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_GCMP_256) &&
        ...
        ((capa.enc & WPA_DRIVER_CAPA_ENC_WEP104) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "wep104")) ||
        ((capa.enc & WPA_DRIVER_CAPA_ENC_WEP40) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "wep40")) ||
        !wpa_dbus_dict_end_string_array(&iter_dict, &iter_dict_entry,
                                        &iter_dict_val, &iter_array))
      goto nomem;
  }
  ...
  /***** key management */
  if (res < 0) {
    const char *args[] = {"wpa-psk", "wpa-eap", "ieee8021x", "wpa-none",
#ifdef CONFIG_WPS
                          "wps",
#endif /* CONFIG_WPS */
                          "none"};
    if (!wpa_dbus_dict_append_string_array(&iter_dict, "KeyMgmt", args,
                                           ARRAY_SIZE(args)))
      goto nomem;
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "KeyMgmt",
                                          ...
                                          &iter_array) ||
        !wpa_dbus_dict_string_array_add_element(&iter_array, "none") ||
        !wpa_dbus_dict_string_array_add_element(&iter_array, "ieee8021x"))
      goto nomem;

    if (capa.key_mgmt &
        (WPA_DRIVER_CAPA_KEY_MGMT_WPA | WPA_DRIVER_CAPA_KEY_MGMT_WPA2)) {
      if (!wpa_dbus_dict_string_array_add_element(&iter_array, "wpa-eap") ||
          ((capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_FT) &&
           !wpa_dbus_dict_string_array_add_element(&iter_array, "wpa-ft-eap")))
        goto nomem;
...
/* TODO: Ensure that driver actually supports sha256 encryption. */
    ...
    if (capa.key_mgmt & (WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK |
      ...
      if (!wpa_dbus_dict_string_array_add_element(&iter_array, "wpa-psk") ||
          ((capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_FT_PSK) &&
           !wpa_dbus_dict_string_array_add_element(&iter_array, "wpa-ft-psk")))
        goto nomem;
...
/* TODO: Ensure that driver actually supports sha256 encryption. */
    ...
    if ((capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_WPA_NONE) &&
        !wpa_dbus_dict_string_array_add_element(&iter_array, "wpa-none"))
      goto nomem;
  ...
  if (res < 0) {
    ...
    if (!wpa_dbus_dict_append_string_array(&iter_dict, "Protocol", args,
                                           ARRAY_SIZE(args)))
      goto nomem;
  } else {
    if (!wpa_dbus_dict_begin_string_array(&iter_dict, "Protocol",
        ...
        ((capa.key_mgmt & (WPA_DRIVER_CAPA_KEY_MGMT_WPA2 |
                           WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK)) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "rsn")) ||
        ((capa.key_mgmt &
          (WPA_DRIVER_CAPA_KEY_MGMT_WPA | WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK)) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "wpa")) ||
        !wpa_dbus_dict_end_string_array(&iter_dict, &iter_dict_entry,
                                        &iter_dict_val, &iter_array))
      goto nomem;
  }
  ...
  if (res < 0) {
    ...
    if (!wpa_dbus_dict_append_string_array(&iter_dict, "AuthAlg", args,
                                           ARRAY_SIZE(args)))
      goto nomem;
  } else {
    ...
    if (((capa.auth & WPA_DRIVER_AUTH_OPEN) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "open")) ||
        ((capa.auth & WPA_DRIVER_AUTH_SHARED) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "shared")) ||
        ((capa.auth & WPA_DRIVER_AUTH_LEAP) &&
         !wpa_dbus_dict_string_array_add_element(&iter_array, "leap")) ||
        !wpa_dbus_dict_end_string_array(&iter_dict, &iter_dict_entry,
                                        &iter_dict_val, &iter_array))
      goto nomem;
  }
  ...
  /***** Modes */
  if (!wpa_dbus_dict_begin_string_array(&iter_dict, "Modes", &iter_dict_entry,
                                        &iter_dict_val, &iter_array) ||
      !wpa_dbus_dict_string_array_add_element(&iter_array, "infrastructure") ||
      (res >= 0 && (capa.flags & WPA_DRIVER_FLAGS_IBSS) &&
       !wpa_dbus_dict_string_array_add_element(&iter_array, "ad-hoc")) ||
      (res >= 0 && (capa.flags & WPA_DRIVER_FLAGS_AP) &&
       !wpa_dbus_dict_string_array_add_element(&iter_array, "ap")) ||
      (res >= 0 && (capa.flags & WPA_DRIVER_FLAGS_P2P_CAPABLE) &&
       !wpa_s->conf->p2p_disabled &&
       !wpa_dbus_dict_string_array_add_element(&iter_array, "p2p")) ||
      !wpa_dbus_dict_end_string_array(&iter_dict, &iter_dict_entry,
                                      &iter_dict_val, &iter_array))
    goto nomem;
  /***** Modes end */
  ...
  if (res >= 0) {
    ...
    if (!wpa_dbus_dict_append_int32(&iter_dict, "MaxScanSSID", max_scan_ssid))
      goto nomem;
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_state(const struct wpa_dbus_property_desc *property_desc,
                       DBusMessageIter *iter, DBusError *error,
                       void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  ...
  success = wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &state_ls,
                                             error);
 ...
 */
dbus_bool_t
wpas_dbus_getter_scanning(const struct wpa_dbus_property_desc *property_desc,
                          DBusMessageIter *iter, DBusError *error,
                          void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  dbus_bool_t scanning = wpa_s->scanning ? TRUE : FALSE;

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &scanning,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_ap_scan(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  dbus_uint32_t ap_scan = wpa_s->conf->ap_scan;

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT32, &ap_scan,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_setter_ap_scan(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
 ...
 */
dbus_bool_t
wpas_dbus_getter_fast_reauth(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  dbus_bool_t fast_reauth = wpa_s->conf->fast_reauth ? TRUE : FALSE;

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &fast_reauth,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_setter_fast_reauth(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
...
dbus_bool_t wpas_dbus_getter_disconnect_reason(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_INT32, &reason,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_assoc_status_code(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_INT32, &status_code,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_bss_expire_age(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT32, &expire_age,
                                          error);
}
...
dbus_bool_t wpas_dbus_getter_bss_expire_count(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT32, &expire_count,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_country(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  char country[3];
 ...
 */
dbus_bool_t
wpas_dbus_setter_country(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  const char *country;
  ...
  if (!country[0] || !country[1]) {
    dbus_set_error_const(error, DBUS_ERROR_FAILED, "invalid country code");
    return FALSE;
  }
...
dbus_bool_t wpas_dbus_getter_scan_interval(
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_INT32, &scan_interval,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_ifname(const struct wpa_dbus_property_desc *property_desc,
                        DBusMessageIter *iter, DBusError *error,
                        void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  const char *ifname = wpa_s->ifname;

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &ifname,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_driver(const struct wpa_dbus_property_desc *property_desc,
                        DBusMessageIter *iter, DBusError *error,
                        void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  const char *driver;
  ...
  if (wpa_s->driver == NULL || wpa_s->driver->name == NULL) {
    wpa_printf(MSG_DEBUG, "%s[dbus]: wpa_s has no driver set", __func__);
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: no driver set", __func__);
    return FALSE;
  }

  driver = wpa_s->driver->name;
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &driver,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_current_bss(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  ...
  if (wpa_s->current_bss && wpa_s->dbus_new_path)
    os_snprintf(bss_obj_path, WPAS_DBUS_OBJECT_PATH_MAX,
                "%s/" WPAS_DBUS_NEW_BSSIDS_PART "/%u", wpa_s->dbus_new_path,
                wpa_s->current_bss->id);
  else
    os_snprintf(bss_obj_path, WPAS_DBUS_OBJECT_PATH_MAX, "/");
...
dbus_bool_t wpas_dbus_getter_current_network(
  ...
  if (wpa_s->current_ssid && wpa_s->dbus_new_path)
    os_snprintf(net_obj_path, WPAS_DBUS_OBJECT_PATH_MAX,
                "%s/" WPAS_DBUS_NEW_NETWORKS_PART "/%u", wpa_s->dbus_new_path,
...
dbus_bool_t wpas_dbus_getter_current_auth_mode(
  ...
  if (wpa_s->wpa_state != WPA_COMPLETED) {
    ...
    os_snprintf(eap_mode_buf, WPAS_DBUS_AUTH_MODE_MAX, "EAP-%s", eap_mode);
    auth_mode = eap_mode_buf;

  } else if (wpa_s->current_ssid) {
    auth_mode = wpa_key_mgmt_txt(wpa_s->key_mgmt, wpa_s->current_ssid->proto);
  } else {
    auth_mode = "UNKNOWN";
  }

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &auth_mode,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_config_file(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  char *confname = "";
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &confname,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bsss(const struct wpa_dbus_property_desc *property_desc,
                      DBusMessageIter *iter, DBusError *error,
                      void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  struct wpa_bss *bss;
  ...
  if (!wpa_s->dbus_new_path) {
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: no D-Bus interface",
                   __func__);
    return FALSE;
  }
  ...
  success = wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_OBJECT_PATH, paths, wpa_s->num_bss, error);
 ...
 */
dbus_bool_t
wpas_dbus_getter_networks(const struct wpa_dbus_property_desc *property_desc,
                          DBusMessageIter *iter, DBusError *error,
                          void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  struct wpa_ssid *ssid;
 ...
 */
dbus_bool_t
wpas_dbus_getter_blobs(const struct wpa_dbus_property_desc *property_desc,
                       DBusMessageIter *iter, DBusError *error,
                       void *user_data) {
  struct wpa_supplicant *wpa_s = user_data;
  ...
  if (!dbus_message_iter_open_container(iter, DBUS_TYPE_VARIANT, "a{say}",
                                        &variant_iter) ||
      !dbus_message_iter_open_container(&variant_iter, DBUS_TYPE_ARRAY, "{say}",
                                        &dict_iter)) {
    dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, "no memory");
    return FALSE;
  }
  ...
  while (blob) {
    if (!dbus_message_iter_open_container(&dict_iter, DBUS_TYPE_DICT_ENTRY,
        ...
        !dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING,
                                        &(blob->name)) ||
        !dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_ARRAY,
                                          DBUS_TYPE_BYTE_AS_STRING,
                                          &array_iter) ||
        !dbus_message_iter_append_fixed_array(&array_iter, DBUS_TYPE_BYTE,
                                              &(blob->data), blob->len) ||
        !dbus_message_iter_close_container(&entry_iter, &array_iter) ||
...
dbus_bool_t wpas_dbus_getter_iface_global(
  ...
  ret =
      wpa_config_get_value(property_desc->data, wpa_s->conf, buf, sizeof(buf));
  if (ret < 0)
    *p = '\0';

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &p, error);
}
...
dbus_bool_t wpas_dbus_setter_iface_global(
  ...
  combined_len = os_strlen(property_desc->data) + os_strlen(new_value) + 3;
  ...
  ret = os_snprintf(buf, combined_len, "%s=%s", property_desc->data, new_value);
...
static struct wpa_bss *get_bss_helper(struct bss_handler_args *args,
  ...
  if (!res) {
    wpa_printf(MSG_ERROR, "%s[dbus]: no bss with id %d found", func_name,
               args->id);
    dbus_set_error(error, DBUS_ERROR_FAILED, "%s: BSS %d not found", func_name,
                   args->id);
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_bssid(const struct wpa_dbus_property_desc *property_desc,
                           DBusMessageIter *iter, DBusError *error,
                           void *user_data) {
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_ssid(const struct wpa_dbus_property_desc *property_desc,
                          DBusMessageIter *iter, DBusError *error,
                          void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, res->ssid,
                                                res->ssid_len, error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_privacy(const struct wpa_dbus_property_desc *property_desc,
                             DBusMessageIter *iter, DBusError *error,
                             void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &privacy,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_mode(const struct wpa_dbus_property_desc *property_desc,
                          DBusMessageIter *iter, DBusError *error,
                          void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_STRING, &mode, error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_signal(const struct wpa_dbus_property_desc *property_desc,
                            DBusMessageIter *iter, DBusError *error,
                            void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  level = (s16)res->level;
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_INT16, &level, error);
}
...
dbus_bool_t wpas_dbus_getter_bss_frequency(
  ...
  freq = (u16)res->freq;
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT16, &freq, error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_rates(const struct wpa_dbus_property_desc *property_desc,
                           DBusMessageIter *iter, DBusError *error,
                           void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  success = wpas_dbus_simple_array_property_getter(
      iter, DBUS_TYPE_UINT32, real_rates, rates_num, error);
...
static dbus_bool_t wpas_dbus_get_bss_security_prop(
    const struct wpa_dbus_property_desc *property_desc, DBusMessageIter *iter,
    struct wpa_ie_data *ie_data, DBusError *error) {
  ...
  if (!wpa_dbus_dict_append_string_array(&iter_dict, "KeyMgmt", key_mgmt, n))
    goto nomem;
  ...
  if (!wpa_dbus_dict_append_string_array(&iter_dict, "Pairwise", pairwise, n))
    goto nomem;
  ...
  if (ie_data->proto == WPA_PROTO_RSN) {
    ...
    if (!wpa_dbus_dict_append_string(&iter_dict, "MgmtGroup", group))
      goto nomem;
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_wpa(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  if (ie && wpa_parse_wpa_ie(ie, 2 + ie[1], &wpa_data) < 0) {
    dbus_set_error_const(error, DBUS_ERROR_FAILED, "failed to parse WPA IE");
    return FALSE;
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_rsn(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  if (ie && wpa_parse_wpa_ie(ie, 2 + ie[1], &wpa_data) < 0) {
    dbus_set_error_const(error, DBUS_ERROR_FAILED, "failed to parse RSN IE");
    return FALSE;
  }
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_wps(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_ies(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  return wpas_dbus_simple_array_property_getter(iter, DBUS_TYPE_BYTE, res + 1,
                                                res->ie_len, error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_bss_age(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct bss_handler_args *args = user_data;
  struct wpa_bss *res;
  ...
  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_UINT32, &age, error);
}
 ...
 */
dbus_bool_t
wpas_dbus_getter_enabled(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct network_handler_args *net = user_data;
  dbus_bool_t enabled = net->ssid->disabled ? FALSE : TRUE;

  return wpas_dbus_simple_property_getter(iter, DBUS_TYPE_BOOLEAN, &enabled,
                                          error);
}
 ...
 */
dbus_bool_t
wpas_dbus_setter_enabled(const struct wpa_dbus_property_desc *property_desc,
                         DBusMessageIter *iter, DBusError *error,
                         void *user_data) {
  struct network_handler_args *net = user_data;
  struct wpa_supplicant *wpa_s;
...
dbus_bool_t wpas_dbus_getter_network_properties(
  ...
  iterator = props;
  while (*iterator) {
    if (!wpa_dbus_dict_append_string(&dict_iter, *iterator, *(iterator + 1))) {
      dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, "no memory");
      goto out;
    }
    iterator += 2;
...
DBusMessage *wpas_dbus_handler_subscribe_preq(DBusMessage *message,
                                              struct wpa_supplicant *wpa_s) {
  struct wpas_dbus_priv *priv = wpa_s->global->dbus;
  char *name;

  if (wpa_s->preq_notify_peer != NULL) {
    if (os_strcmp(dbus_message_get_sender(message), wpa_s->preq_notify_peer) ==
        0)
      return NULL;

    return dbus_message_new_error(message, WPAS_DBUS_ERROR_SUBSCRIPTION_IN_USE,
...
DBusMessage *wpas_dbus_handler_unsubscribe_preq(DBusMessage *message,
                                                struct wpa_supplicant *wpa_s) {
  struct wpas_dbus_priv *priv = wpa_s->global->dbus;

  if (!wpa_s->preq_notify_peer)
    return dbus_message_new_error(message, WPAS_DBUS_ERROR_NO_SUBSCRIPTION,
                                  "Not subscribed");

  if (os_strcmp(wpa_s->preq_notify_peer, dbus_message_get_sender(message)))
    return dbus_message_new_error(message, WPAS_DBUS_ERROR_SUBSCRIPTION_EPERM,
...
void wpas_dbus_signal_preq(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  msg = dbus_message_new_signal(wpa_s->dbus_new_path,
                                WPAS_DBUS_NEW_IFACE_INTERFACE, "ProbeRequest");
  if (msg == NULL)
    return;
  ...
  if (!wpa_dbus_dict_open_write(&iter, &dict_iter) ||
      (addr && !wpa_dbus_dict_append_byte_array(
                   &dict_iter, "addr", (const char *)addr, ETH_ALEN)) ||
      (dst && !wpa_dbus_dict_append_byte_array(&dict_iter, "dst",
                                               (const char *)dst, ETH_ALEN)) ||
      (bssid && !wpa_dbus_dict_append_byte_array(
                    &dict_iter, "bssid", (const char *)bssid, ETH_ALEN)) ||
      (ie && ie_len &&
       !wpa_dbus_dict_append_byte_array(&dict_iter, "ies", (const char *)ie,
                                        ie_len)) ||
      (ssi_signal &&
       !wpa_dbus_dict_append_int32(&dict_iter, "signal", ssi_signal)) ||
...
DBusMessage *wpas_dbus_handler_vendor_elem_get(DBusMessage *message,
  ...
  if (!dbus_message_iter_open_container(
          &iter, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE_AS_STRING, &array_iter) ||
      !dbus_message_iter_append_fixed_array(&array_iter, DBUS_TYPE_BYTE, &elem,
                                            elem_len) ||
      !dbus_message_iter_close_container(&iter, &array_iter)) {
    dbus_message_unref(reply);
    reply = wpas_dbus_error_no_memory(message);
...
DBusMessage *
wpas_dbus_handler_vendor_elem_remove(DBusMessage *message,
                                     struct wpa_supplicant *wpa_s) {
  u8 *ielems;
  int len;
  struct ieee802_11_elems elems;
  ...
  return dbus_message_new_error(message, DBUS_ERROR_INVALID_ARGS, "Not found");
}
