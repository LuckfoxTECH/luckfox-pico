...
/**
 * wpas_dbus_decompose_object_path - Decompose an interface object path into
 * parts
...
char *wpas_dbus_decompose_object_path(const char *path, char **network,
  ...
  if (!path ||
      strncmp(path, WPAS_DBUS_PATH_INTERFACES "/", dev_path_prefix_len))
    return NULL;
  ...
  if (next_sep != NULL) {
    const char *net_part = strstr(next_sep, WPAS_DBUS_NETWORKS_PART "/");
    const char *bssid_part = strstr(next_sep, WPAS_DBUS_BSSIDS_PART "/");

    if (network && net_part) {
      /* Deal with a request for a configured network */
      const char *net_name = net_part + strlen(WPAS_DBUS_NETWORKS_PART "/");
      *network = NULL;
      if (strlen(net_name))
        *network = os_strdup(net_name);
    } else if (bssid && bssid_part) {
      /* Deal with a request for a scanned BSSID */
      const char *bssid_name = bssid_part + strlen(WPAS_DBUS_BSSIDS_PART "/");
...
/**
 * wpas_dbus_new_invalid_iface_error - Return a new invalid interface error
 * message
...
/**
 * wpas_dbus_new_invalid_network_error - Return a new invalid network error
 * message
 * @message: Pointer to incoming dbus message this error refers to
 * Returns: a dbus error message
 *
...
static DBusHandlerResult wpas_iface_message_handler(DBusConnection *connection,
  ...
  wpa_printf(MSG_MSGDUMP, "dbus[old/iface]: %s.%s (%s) [%s]", msg_interface,
             method, path, dbus_message_get_signature(message));

  iface_obj_path = wpas_dbus_decompose_object_path(path, &network, &bssid);
   ...
   */
  if (wpa_supplicant_get_iface_by_dbus_path(wpa_s->global, iface_obj_path) !=
      wpa_s) {
    reply = wpas_dbus_new_invalid_iface_error(message);
    goto out;
  }
  ...
  if (network && !strcmp(msg_interface, WPAS_DBUS_IFACE_NETWORK)) {
    ...
    if (!strcmp(method, "scan"))
      ...
      reply = wpas_dbus_iface_set_smartcard_modules(message, wpa_s);
...
static DBusHandlerResult wpas_message_handler(DBusConnection *connection,
  ...
  wpa_printf(MSG_MSGDUMP, "dbus[old]: %s.%s (%s) [%s]", msg_interface, method,
             path, dbus_message_get_signature(message));
  ...
  if (!strcmp(path, WPAS_DBUS_PATH)) {
    ...
    if (!strcmp(method, "addInterface")) {
      reply = wpas_dbus_global_add_interface(message, ctrl_iface->global);
    } else if (!strcmp(method, "removeInterface")) {
      reply = wpas_dbus_global_remove_interface(message, ctrl_iface->global);
    } else if (!strcmp(method, "getInterface")) {
      reply = wpas_dbus_global_get_interface(message, ctrl_iface->global);
    } else if (!strcmp(method, "setDebugParams")) {
      reply = wpas_dbus_global_set_debugparams(message, ctrl_iface->global);
    }
  }
...
void wpa_supplicant_dbus_notify_scan_results(struct wpa_supplicant *wpa_s) {
  ...
  _signal = dbus_message_new_signal(wpa_s->dbus_path, WPAS_DBUS_IFACE_INTERFACE,
...
void wpa_supplicant_dbus_notify_state_change(struct wpa_supplicant *wpa_s,
  ...
  if (!dbus_message_append_args(_signal, DBUS_TYPE_STRING, &new_state_str,
...
void wpa_supplicant_dbus_notify_scanning(struct wpa_supplicant *wpa_s) {
  ...
  if (dbus_message_append_args(_signal, DBUS_TYPE_BOOLEAN, &scanning,
                               DBUS_TYPE_INVALID)) {
    dbus_connection_send(iface->con, _signal, NULL);
  } else {
    wpa_printf(MSG_ERROR, "dbus: Not enough memory to construct signal");
  }
  dbus_message_unref(_signal);
}
...
void wpa_supplicant_dbus_notify_wps_cred(struct wpa_supplicant *wpa_s,
  ...
  if (!dbus_message_append_args(_signal, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE,
                                &cred->cred_attr, cred->cred_attr_len,
                                DBUS_TYPE_INVALID)) {
    wpa_printf(MSG_ERROR, "dbus: %s: Not enough memory to construct signal",
               __func__);
    goto out;
  }
...
void wpa_supplicant_dbus_notify_certification(struct wpa_supplicant *wpa_s,
  ...
  if (!dbus_message_append_args(_signal, DBUS_TYPE_INT32, &depth,
                                DBUS_TYPE_STRING, &subject, DBUS_TYPE_STRING,
                                &hash, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE,
                                &cert_hex, cert_hex_len, DBUS_TYPE_INVALID)) {
    wpa_printf(MSG_ERROR, "dbus: %s: Not enough memory to construct signal",
               __func__);
    goto out;
  }
...
int wpa_supplicant_dbus_ctrl_iface_init(struct wpas_dbus_priv *iface) {
  ...
  if (!dbus_connection_register_object_path(iface->con, WPAS_DBUS_PATH,
                                            &wpas_vtable, iface)) {
  ...
  switch (dbus_bus_request_name(iface->con, WPAS_DBUS_SERVICE, 0, &error)) {
...
int wpas_dbus_register_iface(struct wpa_supplicant *wpa_s) {
  ...
  os_snprintf(wpa_s->dbus_path, WPAS_DBUS_OBJECT_PATH_MAX,
              WPAS_DBUS_PATH_INTERFACES "/%u", next);
...
wpa_supplicant_get_iface_by_dbus_path(struct wpa_global *global,
                                      const char *path) {
  struct wpa_supplicant *wpa_s;
