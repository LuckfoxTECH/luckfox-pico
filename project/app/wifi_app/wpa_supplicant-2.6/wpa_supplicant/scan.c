...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (wpas_update_random_addr_disassoc(wpa_s) < 0) {
    wpa_msg(wpa_s, MSG_INFO, "Failed to assign random MAC address for a scan");
    wpa_scan_free_params(params);
  ...
  if (ret) {
    ...
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_FAILED "ret=%d%s", ret,
            retry ? " retry=1" : "");
    radio_work_done(work);
...
static void wpa_supplicant_delayed_sched_scan_timeout(void *eloop_ctx,
                                                      void *timeout_ctx) {
...
static void wpa_supplicant_sched_scan_timeout(void *eloop_ctx,
                                              void *timeout_ctx) {
  struct wpa_supplicant *wpa_s = eloop_ctx;
...
static void
wpa_supplicant_optimize_freqs(struct wpa_supplicant *wpa_s,
                              struct wpa_driver_scan_params *params) {
#ifdef CONFIG_P2P
  if (params->freqs == NULL && wpa_s->p2p_in_provisioning && wpa_s->go_params) {
    /* Optimize provisioning state scan based on GO information */
    if (wpa_s->p2p_in_provisioning < 5 && wpa_s->go_params->freq > 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "P2P: Scan only GO "
      ...
      wpa_dbg(wpa_s, MSG_DEBUG,
              ...
              "channels");
      int_array_concat(&params->freqs, wpa_s->go_params->freq_list);
      if (params->freqs)
  ...
  if (params->freqs == NULL && wpa_s->p2p_in_invitation) {
     ...
     * group reinvocation
     */
    if (wpa_s->p2p_in_invitation < 5 && wpa_s->p2p_invite_go_freq > 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
...
static void wpas_add_interworking_elements(struct wpa_supplicant *wpa_s,
  ...
  wpabuf_put_u8(buf,
                is_zero_ether_addr(wpa_s->conf->hessid) ? 1 : 1 + ETH_ALEN);
...
void wpa_supplicant_set_default_scan_ies(struct wpa_supplicant *wpa_s) {
  ...
  if (ext_capab_len > 0 && wpabuf_resize(&default_ies, ext_capab_len) == 0)
...
static struct wpabuf *wpa_supplicant_extra_ies(struct wpa_supplicant *wpa_s) {
  ...
  ext_capab_len = wpas_build_ext_capab(wpa_s, ext_capab, sizeof(ext_capab));
  if (ext_capab_len > 0 && wpabuf_resize(&extra_ie, ext_capab_len) == 0)
    wpabuf_put_data(extra_ie, ext_capab, ext_capab_len);

#ifdef CONFIG_INTERWORKING
  if (wpa_s->conf->interworking && wpabuf_resize(&extra_ie, 100) == 0)
  ...
  if (wps) {
    ...
    wps_ie = wps_build_probe_req_ie(
        wps == 2 ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, &wpa_s->wps->dev,
  ...
  for (i = 0; i < wpa_s->scan_id_count; i++) {
    ...
    for (j = 0; j < params->num_ssids; j++) {
      if (params->ssids[j].ssid_len == ssid->ssid_len &&
          params->ssids[j].ssid &&
          os_memcmp(params->ssids[j].ssid, ssid->ssid, ssid->ssid_len) == 0)
        break;
    }
...
static int wpa_set_ssids_from_scan_req(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->ssids_from_scan_req == NULL || wpa_s->num_ssids_from_scan_req == 0)
    return 0;
  ...
  for (i = 0; i < wpa_s->num_ssids_from_scan_req; i++) {
    ...
    params->ssids[i].ssid_len = wpa_s->ssids_from_scan_req[i].ssid_len;
    wpa_hexdump_ascii(MSG_DEBUG, "specific SSID", params->ssids[i].ssid,
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx) {
  ...
  ssid = NULL;
  if (wpa_s->scan_req != MANUAL_SCAN_REQ && wpa_s->connect_without_scan) {
  ...
  if ((wpa_s->p2p_in_provisioning || wpa_s->show_group_started) &&
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(p2p_in_provisioning=%d show_group_started=%d)",
  ...
  if (wpa_s->last_scan_req != MANUAL_SCAN_REQ &&
    ...
    if (wpa_s->scan_id_count && wpa_s->last_scan_req == MANUAL_SCAN_REQ)
  ...
  if (ssid && max_ssids == 1) {
    ...
    if (!wpa_s->prev_scan_wildcard) {
      ...
      wpa_s->prev_scan_ssid = ssid;
      wpa_s->prev_scan_wildcard = 0;
      wpa_dbg(wpa_s, MSG_DEBUG, "Starting AP scan for specific SSID: %s",
              wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
    }
  } else if (ssid) {
    /* max_ssids > 1 */
...
ssid_list_set:
  ...
  if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && wpa_s->manual_scan_only_new) {
    wpa_printf(
        MSG_DEBUG,
  ...
  if (wpa_s->conf->freq_list && !params.freqs) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Optimize scan based on conf->freq_list");
  ...
  params.filter_ssids =
      wpa_supplicant_build_filter_ssids(wpa_s->conf, &params.num_filter_ssids);
  if (extra_ie) {
  ...
  if (!is_zero_ether_addr(wpa_s->next_scan_bssid)) {
    ...
    if (bss && bss->ssid_len && params.num_ssids == 1 &&
      ...
      wpa_dbg(wpa_s, MSG_DEBUG,
              "Scan a previously specified BSSID " MACSTR " and SSID %s",
              MAC2STR(params.bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len));
    } else {
      wpa_dbg(wpa_s, MSG_DEBUG, "Scan a previously specified BSSID " MACSTR,
...
scan:
   ...
   */
  if (wpa_s->scan_for_connection && wpa_s->last_scan_req == NORMAL_SCAN_REQ &&
      !scan_params->freqs && !params.freqs && wpas_is_p2p_prioritized(wpa_s) &&
    ...
    if (params.freqs) {
      ...
      if (num > 0 && num == wpa_s->num_multichan_concurrent) {
        wpa_dbg(wpa_s, MSG_DEBUG,
                "Scan only the current operating channels since all channels "
                "are already used");
      } else {
        os_free(params.freqs);
        params.freqs = NULL;
...
void wpa_supplicant_update_scan_int(struct wpa_supplicant *wpa_s, int sec) {
  ...
  cancelled =
      eloop_cancel_timeout_one(wpa_supplicant_scan, wpa_s, NULL, &remaining);
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec) {
  ...
  if (wpa_s->p2p_mgmt) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Ignore scan request (%d.%06d sec) on p2p_mgmt interface", sec,
            usec);
    return;
  }
  ...
  if (res == 1) {
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Ignore new scan request for %d.%06d sec since an earlier request "
            "is scheduled to trigger sooner",
            sec, usec);
  } else {
 ...
 */
int wpa_supplicant_delayed_sched_scan(struct wpa_supplicant *wpa_s, int sec,
                                      int usec) {
  if (!wpa_s->sched_scan_supported)
    return -1;

  eloop_register_timeout(sec, usec, wpa_supplicant_delayed_sched_scan_timeout,
                         wpa_s, NULL);
...
int wpa_supplicant_req_sched_scan(struct wpa_supplicant *wpa_s) {
  ...
  for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
    if (!wpas_network_disabled(wpa_s, ssid) && !ssid->scan_ssid) {
      ...
      wildcard = 1;
    } else if (!wpas_network_disabled(wpa_s, ssid) && ssid->ssid_len)
      need_ssids++;
  ...
  params.filter_ssids =
      os_calloc(wpa_s->max_match_sets, sizeof(struct wpa_driver_scan_filter));
  ...
  while (ssid) {
    ...
    if (params.num_filter_ssids < wpa_s->max_match_sets &&
      ...
      wpa_dbg(wpa_s, MSG_DEBUG, "add to filter ssid: %s",
              wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
      os_memcpy(params.filter_ssids[params.num_filter_ssids].ssid, ssid->ssid,
                ssid->ssid_len);
      params.filter_ssids[params.num_filter_ssids].ssid_len = ssid->ssid_len;
      ...
      if (params.num_ssids >= max_sched_scan_ssids) {
        ...
        do {
          ssid = ssid->next;
                 ...
                 (wpas_network_disabled(wpa_s, ssid) || !ssid->scan_ssid));
        break;
      }
    }
...
scan:
  ...
  if (wpa_s->sched_scan_plans_num == 1 ||
    ...
    if (scan_plan.interval > wpa_s->max_sched_scan_plan_interval) {
      wpa_printf(MSG_WARNING,
                 ...
                 scan_plan.interval, wpa_s->max_sched_scan_plan_interval);
      scan_plan.interval = wpa_s->max_sched_scan_plan_interval;
    }
  ...
  if (ssid || !wpa_s->first_sched_scan) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Starting sched scan: interval %u timeout %d",
            params.sched_scan_plans[0].interval, wpa_s->sched_scan_timeout);
  } else {
  ...
  if (wpa_s->mac_addr_rand_enable & MAC_ADDR_RAND_SCHED_SCAN) {
    ...
    if (wpa_s->mac_addr_sched_scan) {
      ...
      params.mac_addr_mask = wpa_s->mac_addr_sched_scan + ETH_ALEN;
    }
  }
  ...
  if (ssid || !wpa_s->first_sched_scan) {
    ...
    eloop_register_timeout(wpa_s->sched_scan_timeout, 0,
                           wpa_supplicant_sched_scan_timeout, wpa_s, NULL);
...
void wpa_supplicant_cancel_delayed_sched_scan(struct wpa_supplicant *wpa_s) {
  ...
  eloop_cancel_timeout(wpa_supplicant_delayed_sched_scan_timeout, wpa_s, NULL);
}
  ...
  for (i = 0; i < scan_res->num; i++) {
    ...
    if (r->flags & WPA_SCAN_LEVEL_DBM) {
      ...
      wpa_printf(MSG_EXCESSIVE,
                 MACSTR " freq=%d qual=%d "
                        "noise=%d%s level=%d snr=%d%s flags=0x%x age=%u est=%u",
                 MAC2STR(r->bssid), r->freq, r->qual, r->noise,
                 noise_valid ? "" : "~", r->level, r->snr,
                 r->snr >= GREAT_SNR ? "*" : "", r->flags, r->age,
                 r->est_throughput);
    } else {
      wpa_printf(MSG_EXCESSIVE,
                 MACSTR " freq=%d qual=%d "
                        "noise=%d level=%d flags=0x%x age=%u est=%u",
                 MAC2STR(r->bssid), r->freq, r->qual, r->noise, r->level,
                 r->flags, r->age, r->est_throughput);
    }
    pos = (u8 *)(r + 1);
    if (r->ie_len)
...
int wpa_supplicant_filter_bssid_match(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < wpa_s->bssid_filter_count; i++) {
    if (os_memcmp(wpa_s->bssid_filter + i * ETH_ALEN, bssid, ETH_ALEN) == 0)
      return 1;
  }
...
void filter_scan_res(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0, j = 0; i < res->num; i++) {
    if (wpa_supplicant_filter_bssid_match(wpa_s, res->res[i]->bssid)) {
      res->res[j++] = res->res[i];
    } else {
      os_free(res->res[i]);
      res->res[i] = NULL;
  ...
  if (res->num != j) {
    wpa_printf(MSG_DEBUG, "Filtered out %d scan results", (int)(res->num - j));
    res->num = j;
  }
}
...
void scan_snr(struct wpa_scan_res *res) {
  if (res->flags & WPA_SCAN_NOISE_INVALID) {
    res->noise = IS_5GHZ(res->freq) ? DEFAULT_NOISE_FLOOR_5GHZ
...
void scan_est_throughput(struct wpa_supplicant *wpa_s,
  ...
  if (capab == CAPAB_VHT) {
    ...
    if (ie) {
      ...
      if (ie && ie[1] >= 1 && (ie[2] & VHT_OPMODE_CHANNEL_WIDTH_MASK)) {
...
wpa_scan_clone_params(const struct wpa_driver_scan_params *src) {
  ...
  for (i = 0; i < src->num_ssids; i++) {
    if (src->ssids[i].ssid) {
      ...
      if (n == NULL)
        goto failed;
      os_memcpy(n, src->ssids[i].ssid, src->ssids[i].ssid_len);
  ...
  if (src->filter_ssids) {
    params->filter_ssids =
        os_malloc(sizeof(*params->filter_ssids) * src->num_filter_ssids);
  ...
  if (src->sched_scan_plans_num > 0) {
    params->sched_scan_plans =
        os_malloc(sizeof(*src->sched_scan_plans) * src->sched_scan_plans_num);
    if (!params->sched_scan_plans)
      goto failed;

    os_memcpy(params->sched_scan_plans, src->sched_scan_plans,
              sizeof(*src->sched_scan_plans) * src->sched_scan_plans_num);
  ...
  if (src->mac_addr_rand) {
    ...
    if (src->mac_addr && src->mac_addr_mask) {
      ...
      os_memcpy(mac_addr + ETH_ALEN, src->mac_addr_mask, ETH_ALEN);
...
int wpas_start_pno(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->sched_scan_stop_req) {
    wpa_printf(MSG_DEBUG, "Schedule PNO after previous sched scan has stopped");
    wpa_s->pno_sched_pending = 1;
    return 0;
  }
  ...
  if (num_ssid > max_sched_scan_ssids) {
    wpa_printf(MSG_DEBUG,
               ...
               "%u",
               max_sched_scan_ssids, (unsigned int)num_ssid);
  ...
  params.filter_ssids =
      os_calloc(num_match_ssid, sizeof(struct wpa_driver_scan_filter));
  if (params.filter_ssids == NULL)
    return -1;
  ...
  while (ssid) {
    if (!wpas_network_disabled(wpa_s, ssid)) {
      ...
      os_memcpy(params.filter_ssids[i].ssid, ssid->ssid, ssid->ssid_len);
...
int wpas_mac_addr_rand_scan_set(struct wpa_supplicant *wpa_s, unsigned int type,
  ...
  if (type == MAC_ADDR_RAND_SCAN) {
    ...
    wpa_printf(MSG_INFO, "scan: Invalid MAC randomization type=0x%x", type);
    os_free(tmp);
    return -1;
  }
...
int wpas_sched_scan_plans_set(struct wpa_supplicant *wpa_s, const char *cmd) {
  ...
  while ((token = cstr_token(cmd, " ", &context))) {
    ...
    ret = sscanf(token, "%u:%u", &scan_plan->interval, &scan_plan->iterations);
    if (ret <= 0 || ret > 2 || !scan_plan->interval) {
      wpa_printf(MSG_ERROR, "Invalid sched scan plan input: %s", token);
      goto fail;
    }

    if (scan_plan->interval > wpa_s->max_sched_scan_plan_interval) {
      wpa_printf(MSG_WARNING,
                 "scan plan %u: Scan interval too long(%u), use the maximum "
                 "allowed(%u)",
                 num, scan_plan->interval, wpa_s->max_sched_scan_plan_interval);
      scan_plan->interval = wpa_s->max_sched_scan_plan_interval;
    }
    ...
    if (!scan_plan->iterations) {
      wpa_printf(MSG_ERROR, "scan plan %u: Number of iterations cannot be zero",
                 num);
      goto fail;
    }

    if (scan_plan->iterations > wpa_s->max_sched_scan_plan_iterations) {
      wpa_printf(
          MSG_WARNING,
          "scan plan %u: Too many iterations(%u), use the maximum allowed(%u)",
          num, scan_plan->iterations, wpa_s->max_sched_scan_plan_iterations);
      scan_plan->iterations = wpa_s->max_sched_scan_plan_iterations;
    }

    wpa_printf(MSG_DEBUG, "scan plan %u: interval=%u iterations=%u", num,
  ...
  wpa_printf(MSG_DEBUG, "scan plan %u (last plan): interval=%u", num,
             scan_plans[num - 1].interval);

  if (num > wpa_s->max_sched_scan_plans) {
    wpa_printf(MSG_WARNING, "Too many scheduled scan plans (only %u supported)",
