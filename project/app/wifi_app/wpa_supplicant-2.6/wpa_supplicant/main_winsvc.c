...
#define TBUFLEN 255
  ...
  ret = RegQueryValueEx(hk, TEXT("adapter"), NULL, NULL, (LPBYTE)adapter,
                        &buflen);
  if (ret == ERROR_SUCCESS) {
  ...
  ret =
      RegQueryValueEx(hk, TEXT("config"), NULL, NULL, (LPBYTE)config, &buflen);
  if (ret == ERROR_SUCCESS) {
  ...
  buflen = sizeof(val);
  ret = RegQueryValueEx(hk, TEXT("skip_on_error"), NULL, NULL, (LPBYTE)&val,
                        &buflen);
...
static int wpa_supplicant_thread(void) {
  ...
  buflen = sizeof(val);
  ret = RegQueryValueEx(hk, TEXT("debug_level"), NULL, NULL, (LPBYTE)&val,
                        &buflen);
  if (ret == ERROR_SUCCESS && buflen == sizeof(val)) {
  ...
  buflen = sizeof(val);
  ret = RegQueryValueEx(hk, TEXT("debug_show_keys"), NULL, NULL, (LPBYTE)&val,
                        &buflen);
  if (ret == ERROR_SUCCESS && buflen == sizeof(val)) {
  ...
  buflen = sizeof(val);
  ret = RegQueryValueEx(hk, TEXT("debug_timestamp"), NULL, NULL, (LPBYTE)&val,
                        &buflen);
  if (ret == ERROR_SUCCESS && buflen == sizeof(val)) {
  ...
  buflen = sizeof(val);
  ret = RegQueryValueEx(hk, TEXT("debug_use_file"), NULL, NULL, (LPBYTE)&val,
                        &buflen);
  ...
  ret = RegOpenKeyEx(hk, TEXT("interfaces"), 0, KEY_ENUMERATE_SUB_KEYS, &ihk);
  RegCloseKey(hk);
  if (ret != ERROR_SUCCESS) {
  ...
  for (i = 0;; i++) {
    ...
    namelen = 255;
    ret = RegEnumKeyEx(ihk, i, name, &namelen, NULL, NULL, NULL, NULL);
...
static int register_service(const TCHAR *exe) {
  ...
  svc = CreateService(scm, WPASVC_NAME, WPASVC_DISPLAY_NAME, SERVICE_ALL_ACCESS,
                      SERVICE_WIN32_OWN_PROCESS, SERVICE_DEMAND_START,
                      SERVICE_ERROR_NORMAL, exe, NULL, NULL, NULL, NULL, NULL);
...
static void WINAPI service_start(DWORD argc, LPTSTR *argv) {
  ...
  svc_status_handle =
      RegisterServiceCtrlHandler(WPASVC_NAME, service_ctrl_handler);
  ...
  if (CreateThread(0, 0, (LPTHREAD_START_ROUTINE)svc_thread, 0, 0, &id) == 0) {
...
int main(int argc, char *argv[]) {
  SERVICE_TABLE_ENTRY dt[] = {{WPASVC_NAME, service_start}, {NULL, NULL}};
