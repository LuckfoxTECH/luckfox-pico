...
static struct bgscan_learn_bss *
bgscan_learn_get_bss(struct bgscan_learn_data *data, const u8 *bssid) {
...
static void bgscan_learn_save(struct bgscan_learn_data *data) {
  ...
  wpa_printf(MSG_DEBUG, "bgscan learn: Saving data to %s", data->fname);
...
static void bgscan_learn_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (data->ssid->scan_freq)
    ...
    for (i = 0; freqs && freqs[i]; i++) {
      int ret;
      ret = os_snprintf(pos, msg + sizeof(msg) - pos, " %d", freqs[i]);
      if (os_snprintf_error(msg + sizeof(msg) - pos, ret))
        break;
      pos += ret;
    }
    pos[0] = '\0';
    wpa_printf(MSG_DEBUG, "bgscan learn: Scanning frequencies:%s", msg);
    params.freqs = freqs;
  }
  ...
  if (wpa_supplicant_trigger_scan(wpa_s, &params)) {
    ...
    eloop_register_timeout(data->scan_interval, 0, bgscan_learn_timeout, data,
                           NULL);
  } else
...
static void *bgscan_learn_init(struct wpa_supplicant *wpa_s, const char *params,
                               const struct wpa_ssid *ssid) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             data->signal_threshold, data->short_interval, data->long_interval);
...
static void bgscan_learn_deinit(void *priv) {
  ...
  os_free(data->fname);
  dl_list_for_each_safe(bss, n, &data->bss, struct bgscan_learn_bss, list) {
...
#define MAX_BSS 50
  ...
  for (i = 0; i < scan_res->num; i++) {
    ...
    if (num_bssid < MAX_BSS) {
      os_memcpy(bssid + num_bssid * ETH_ALEN, res->bssid, ETH_ALEN);
      num_bssid++;
    }
  }
  wpa_printf(MSG_DEBUG,
  ...
  for (i = 0; i < scan_res->num; i++) {
    ...
    if (bss && bss->freq != res->freq) {
      wpa_printf(MSG_DEBUG, "bgscan learn: Update BSS " MACSTR " freq %d -> %d",
                 MAC2STR(res->bssid), bss->freq, res->freq);
      bss->freq = res->freq;
    } else if (!bss) {
      wpa_printf(MSG_DEBUG, "bgscan learn: Add BSS " MACSTR " freq=%d",
                 MAC2STR(res->bssid), res->freq);
      bss = os_zalloc(sizeof(*bss));
      if (!bss)
        continue;
