...
static void wpa_supplicant_ctrl_iface_send(struct wpa_supplicant *wpa_s,
                                           ...
                                           struct dl_list *ctrl_dst, int level,
...
static void wpas_ctrl_sock_debug(const char *title, int sock, const char *buf,
  ...
  wpa_printf(level, "CTRL-DEBUG: %s: sock=%d sndbuf=%d outq=%d send_len=%d",
...
static void wpa_supplicant_ctrl_iface_receive(int sock, void *eloop_ctx,
  ...
  if (reply) {
    ...
    if (sendto(sock, reply, reply_len, 0, (struct sockaddr *)&from, fromlen) <
        0) {
      int _errno = errno;
      wpa_dbg(wpa_s, MSG_DEBUG, "ctrl_iface sendto failed: %d - %s", _errno,
              strerror(_errno));
...
static int wpas_ctrl_iface_throttle(int sock) {
  ...
  sndbuf = 0;
  if (getsockopt(sock, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen) < 0 ||
      ioctl(sock, TIOCOUTQ, &outq) < 0 || sndbuf <= 0 || outq < 0)
    return 0;
...
static void wpas_ctrl_msg_send_pending_global(struct wpa_global *global) {
  ...
  while (gpriv && !dl_list_empty(&gpriv->msg_queue) &&
    ...
    msg = dl_list_first(&gpriv->msg_queue, struct ctrl_iface_msg, list);
    if (!msg)
      break;
    dl_list_del(&msg->list);
    wpa_supplicant_ctrl_iface_send(
        msg->wpa_s,
        msg->type != WPA_MSG_PER_INTERFACE ? NULL : msg->wpa_s->ifname,
        gpriv->sock, &gpriv->ctrl_dst, msg->level, msg->txt, msg->len, NULL,
        gpriv);
    os_free(msg);
  }
}
...
static void wpas_ctrl_msg_send_pending_iface(struct wpa_supplicant *wpa_s) {
  ...
  while (priv && !dl_list_empty(&priv->msg_queue) &&
    ...
    msg = dl_list_first(&priv->msg_queue, struct ctrl_iface_msg, list);
    if (!msg)
      break;
    dl_list_del(&msg->list);
    wpa_supplicant_ctrl_iface_send(wpa_s, NULL, priv->sock, &priv->ctrl_dst,
                                   msg->level, msg->txt, msg->len, priv, NULL);
    os_free(msg);
  }
}
...
static void wpas_ctrl_msg_queue_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (sock > -1 || gsock > -1) {
    ...
    wpa_printf(MSG_MSGDUMP,
               "CTRL: Had to throttle pending event message transmission for "
               "(sock %d gsock %d)",
               sock, gsock);
    eloop_register_timeout(0, 20000, wpas_ctrl_msg_queue_timeout, wpa_s, NULL);
  }
}
...
static void wpa_supplicant_ctrl_iface_msg_cb(void *ctx, int level,
  ...
  if (type != WPA_MSG_NO_GLOBAL && gpriv && !dl_list_empty(&gpriv->ctrl_dst)) {
    if (!dl_list_empty(&gpriv->msg_queue) ||
      ...
      wpa_supplicant_ctrl_iface_send(
          wpa_s, type != WPA_MSG_PER_INTERFACE ? NULL : wpa_s->ifname,
          gpriv->sock, &gpriv->ctrl_dst, level, txt, len, NULL, gpriv);
    }
  }
...
static int wpas_ctrl_iface_open_sock(struct wpa_supplicant *wpa_s,
  ...
  if (mkdir(dir, S_IRWXU | S_IRWXG) < 0) {
    if (errno == EEXIST) {
      ...
      wpa_printf(MSG_ERROR, "mkdir[ctrl_interface=%s]: %s", dir,
                 strerror(errno));
      goto fail;
    }
  }
  ...
  if (gid_str) {
    ...
    if (grp) {
      ...
      gid_set = 1;
      wpa_printf(MSG_DEBUG, "ctrl_interface_group=%d", (int)gid);
    }
  }

  if (gid_set && chown(dir, -1, gid) < 0) {
    wpa_printf(MSG_ERROR, "chown[ctrl_interface=%s,gid=%d]: %s", dir, (int)gid,
               strerror(errno));
    goto fail;
  }
  ...
  if (gid_set &&
    ...
    wpa_printf(MSG_ERROR, "CTRL: chmod[ctrl_interface]: %s", strerror(errno));
    goto fail;
  }

  if (os_strlen(dir) + 1 + os_strlen(wpa_s->ifname) >= sizeof(addr.sun_path)) {
  ...
  if (gid_set && chown(fname, -1, gid) < 0) {
    wpa_printf(MSG_ERROR, "chown[ctrl_interface=%s,gid=%d]: %s", fname,
               (int)gid, strerror(errno));
    goto fail;
  }

  if (chmod(fname, S_IRWXU | S_IRWXG) < 0) {
    wpa_printf(MSG_ERROR, "chmod[ctrl_interface=%s]: %s", fname,
               strerror(errno));
    goto fail;
  }
  os_free(fname);
...
havesock:
  ...
  eloop_register_read_sock(priv->sock, wpa_supplicant_ctrl_iface_receive, wpa_s,
                           priv);
...
wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->global->params.ctrl_interface) {
    ...
    if (wpa_s->global->params.ctrl_interface[0] == '/') {
       ...
       * is modified.
       */
      pos = os_strchr(wpa_s->global->params.ctrl_interface, '_');
      if (pos && os_strcmp(pos + 1, wpa_s->conf->ctrl_interface) == 0)
        same = 1;
    }
    ...
    if (same) {
       ...
       */
      wpa_printf(MSG_ERROR,
                 "global ctrl interface %s matches ctrl interface %s - do not "
                 "open per-interface ctrl interface",
  ...
  if (priv->sock > -1) {
    ...
    if (rmdir(dir) < 0) {
      if (errno == ENOTEMPTY) {
        ...
        wpa_printf(MSG_ERROR, "rmdir[ctrl_interface=%s]: %s", dir,
                   strerror(errno));
      }
    }
    os_free(buf);
...
free_dst:
  ...
  dl_list_for_each_safe(msg, prev_msg, &priv->msg_queue, struct ctrl_iface_msg,
                        list) {
    dl_list_del(&msg->list);
    os_free(msg);
  }
...
static void wpa_supplicant_ctrl_iface_send(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each_safe(dst, next, ctrl_dst, struct wpa_ctrl_dst, list) {
    ...
    if (sendmsg(sock, &msg, MSG_DONTWAIT) >= 0) {
      sockaddr_print(MSG_MSGDUMP, "CTRL_IFACE monitor sent successfully to",
    ...
    if (_errno == ENOBUFS || _errno == EAGAIN) {
      ...
      if (priv)
        ...
        sock = wpas_ctrl_iface_global_reinit(wpa_s->global, gp);
      else
        break;
      if (sock < 0) {
        wpa_dbg(wpa_s, MSG_DEBUG, "Failed to reinitialize ctrl_iface socket");
...
static void wpa_supplicant_global_ctrl_iface_receive(int sock, void *eloop_ctx,
  ...
  if (os_strcmp(buf, "ATTACH") == 0) {
    if (wpa_supplicant_ctrl_iface_attach(&priv->ctrl_dst, &from, fromlen, 1))
      reply_len = 1;
    else
      reply_len = 2;
  } else if (os_strcmp(buf, "DETACH") == 0) {
    ...
    reply_buf =
        wpa_supplicant_global_ctrl_iface_process(global, buf, &reply_len);
    reply = reply_buf;
...
static int
wpas_global_ctrl_iface_open_sock(struct wpa_global *global,
                                 struct ctrl_iface_global_priv *priv) {
  struct sockaddr_un addr;
  ...
  if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    ...
    if (connect(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
      ...
      if (unlink(ctrl) < 0) {
        wpa_printf(MSG_ERROR,
                   "Could not unlink existing ctrl_iface socket '%s': %s", ctrl,
                   strerror(errno));
        goto fail;
      }
      if (bind(priv->sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        wpa_printf(MSG_ERROR, "supp-glb-iface-init: bind(PF_UNIX;%s): %s", ctrl,
                   strerror(errno));
        goto fail;
      }
      wpa_printf(MSG_DEBUG,
  ...
  if (global->params.ctrl_interface_group) {
    ...
    if (chown(ctrl, -1, gid) < 0) {
      wpa_printf(MSG_ERROR, "chown[global_ctrl_interface=%s,gid=%d]: %s", ctrl,
                 (int)gid, strerror(errno));
      goto fail;
    }

    if (chmod(ctrl, S_IRWXU | S_IRWXG) < 0) {
      wpa_printf(MSG_ERROR, "chmod[global_ctrl_interface=%s]: %s", ctrl,
                 strerror(errno));
      goto fail;
    }
  } else {
    if (chmod(ctrl, S_IRWXU) < 0) {
      wpa_printf(MSG_DEBUG, "chmod[global_ctrl_interface=%s](S_IRWXU): %s",
                 ctrl, strerror(errno));
