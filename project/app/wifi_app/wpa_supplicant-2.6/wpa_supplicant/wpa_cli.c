...
static int wpa_cli_open_connection(const char *ifname, int attach) {
  ...
  if (mon_conn) {
    if (wpa_ctrl_attach(mon_conn) == 0) {
      wpa_cli_attached = 1;
      if (interactive)
        eloop_register_read_sock(wpa_ctrl_get_fd(mon_conn), wpa_cli_mon_receive,
                                 NULL, NULL);
    } else {
...
static int wpa_cli_cmd_bss_flush(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  char cmd[256];
  int res;
...
static int wpa_cli_cmd_stkstart(struct wpa_ctrl *ctrl, int argc, char *argv[]) {
...
static int wpa_cli_cmd_wps_reg(struct wpa_ctrl *ctrl, int argc, char *argv[]) {
  ...
  if (argc == 2)
    ...
    res = os_snprintf(cmd, sizeof(cmd), "WPS_REG %s %s %s %s %s %s", argv[0],
                      argv[1], ssid_hex, argv[3], argv[4], key_hex);
  } else {
...
static int wpa_cli_cmd_wps_er_config(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc == 5 || argc == 6) {
    ...
    res = os_snprintf(cmd, sizeof(cmd), "WPS_ER_CONFIG %s %s %s %s %s %s",
                      argv[0], argv[1], ssid_hex, argv[3], argv[4], key_hex);
  } else {
...
static int wpa_cli_cmd_identity(struct wpa_ctrl *ctrl, int argc, char *argv[]) {
  ...
  pos = cmd;
  ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "IDENTITY-%s:%s", argv[0],
                    argv[1]);
...
static int wpa_cli_cmd_password(struct wpa_ctrl *ctrl, int argc, char *argv[]) {
  ...
  pos = cmd;
  ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "PASSWORD-%s:%s", argv[0],
                    argv[1]);
  if (os_snprintf_error(end - pos, ret)) {
...
static int wpa_cli_cmd_new_password(struct wpa_ctrl *ctrl, int argc,
  ...
  pos = cmd;
  ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "NEW_PASSWORD-%s:%s", argv[0],
                    argv[1]);
  if (os_snprintf_error(end - pos, ret)) {
...
static int wpa_cli_cmd_passphrase(struct wpa_ctrl *ctrl, int argc,
  ...
  pos = cmd;
  ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "PASSPHRASE-%s:%s", argv[0],
                    argv[1]);
  if (os_snprintf_error(end - pos, ret)) {
...
static int wpa_cli_cmd_blacklist(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  return wpa_cli_cmd(ctrl, "BLACKLIST", 0, argc, argv);
}

static int wpa_cli_cmd_log_level(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
...
static const char *network_fields[] = {"ssid",
                                       "scan_ssid",
                                       "bssid",
                                       "bssid_blacklist",
                                       "bssid_whitelist",
...
static int wpa_cli_cmd_interface(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  if (argc < 1) {
...
static int wpa_cli_cmd_interface_add(struct wpa_ctrl *ctrl, int argc,
   ...
   */
  res = os_snprintf(
      cmd, sizeof(cmd), "INTERFACE_ADD %s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", argv[0],
      argc > 1 ? argv[1] : "", argc > 2 ? argv[2] : "", argc > 3 ? argv[3] : "",
      argc > 4 ? argv[4] : "", argc > 5 ? argv[5] : "", argc > 6 ? argv[6] : "",
      argc > 7 ? argv[7] : "");
  if (os_snprintf_error(sizeof(cmd), res))
    return -1;
  cmd[sizeof(cmd) - 1] = '\0';
...
static int wpa_ctrl_command_sta(struct wpa_ctrl *ctrl, char *cmd, char *addr,
                                size_t addr_len) {
  char buf[4096], *pos;
  size_t len;
  int ret;
...
static int wpa_cli_cmd_p2p_serv_disc_cancel_req(struct wpa_ctrl *ctrl, int argc,
                                                char *argv[]) {
...
static int wpa_cli_cmd_p2p_serv_disc_resp(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(cmd, sizeof(cmd), "P2P_SERV_DISC_RESP %s %s %s %s", argv[0],
...
static int wpa_cli_cmd_p2p_serv_disc_external(struct wpa_ctrl *ctrl, int argc,
                                              char *argv[]) {
...
static int wpa_cli_cmd_p2p_service_del(struct wpa_ctrl *ctrl, int argc,
  ...
  if (argc == 3)
    res = os_snprintf(cmd, sizeof(cmd), "P2P_SERVICE_DEL %s %s %s", argv[0],
                      argv[1], argv[2]);
  else
    res = os_snprintf(cmd, sizeof(cmd), "P2P_SERVICE_DEL %s %s", argv[0],
                      argv[1]);
  if (os_snprintf_error(sizeof(cmd), res))
    return -1;
  cmd[sizeof(cmd) - 1] = '\0';
...
static int wpa_cli_cmd_p2p_reject(struct wpa_ctrl *ctrl, int argc,
                                  char *argv[]) {
  return wpa_cli_cmd(ctrl, "P2P_REJECT", 1, argc, argv);
}

static int wpa_cli_cmd_p2p_invite(struct wpa_ctrl *ctrl, int argc,
                                  char *argv[]) {
...
static int wpa_cli_cmd_p2p_peers(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  char addr[32], cmd[64];
  int discovered;
  ...
  if (wpa_ctrl_command_p2p_peer(ctrl, "P2P_PEER FIRST", addr, sizeof(addr),
                                discovered))
    return -1;
  do {
...
static char **wpa_cli_complete_p2p_set(const char *str, int pos) {
  int arg = get_cmd_arg_num(str, pos);
  const char *fields[] = {
      "discoverability", "managed",       "listen_channel",
      "ssid_postfix",    "noa",           "ps",
      "oppps",           "ctwindow",      "disabled",
      "conc_pref",       "force_long_sd", "peer_filter",
      "cross_connect",   "go_apsd",       "client_apsd",
      "disallow_freq",   "disc_int",      "per_sta_psk",
  };
  int i, num_fields = ARRAY_SIZE(fields);
...
static int wpa_cli_cmd_p2p_flush(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
...
static int wpa_cli_cmd_wfd_subelem_set(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(cmd, sizeof(cmd), "WFD_SUBELEM_SET %s %s", argv[0],
                    argc > 1 ? argv[1] : "");
  if (os_snprintf_error(sizeof(cmd), res))
    return -1;
  cmd[sizeof(cmd) - 1] = '\0';
...
static int wpa_cli_cmd_wfd_subelem_get(struct wpa_ctrl *ctrl, int argc,
  ...
  res = os_snprintf(cmd, sizeof(cmd), "WFD_SUBELEM_GET %s", argv[0]);
  if (os_snprintf_error(sizeof(cmd), res))
    return -1;
  cmd[sizeof(cmd) - 1] = '\0';
  return wpa_ctrl_command(ctrl, cmd);
...
static int wpa_cli_cmd_get_nai_home_realm_list(struct wpa_ctrl *ctrl, int argc,
  ...
  if (write_cmd(cmd, sizeof(cmd), "HS20_GET_NAI_HOME_REALM_LIST", argc, argv) <
      0)
    return -1;
...
static int wpa_cli_cmd_fetch_osu(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
...
static int wpa_cli_cmd_wnm_sleep(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
  return wpa_cli_cmd(ctrl, "WNM_SLEEP", 0, argc, argv);
}

static int wpa_cli_cmd_wnm_bss_query(struct wpa_ctrl *ctrl, int argc,
                                     char *argv[]) {
...
static int wpa_cli_cmd_radio_work(struct wpa_ctrl *ctrl, int argc,
                                  char *argv[]) {
...
static int wpa_cli_cmd_erp_flush(struct wpa_ctrl *ctrl, int argc,
                                 char *argv[]) {
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
    {"status", wpa_cli_cmd_status, NULL, cli_cmd_flag_none,
     "[verbose] = get current WPA/EAPOL/EAP status"},
    {"ifname", wpa_cli_cmd_ifname, NULL, cli_cmd_flag_none,
     "= get current interface name"},
    {"ping", wpa_cli_cmd_ping, NULL, cli_cmd_flag_none,
     "= pings wpa_supplicant"},
    {"relog", wpa_cli_cmd_relog, NULL, cli_cmd_flag_none,
     "= re-open log-file (allow rolling logs)"},
    {"note", wpa_cli_cmd_note, NULL, cli_cmd_flag_none,
     "<text> = add a note to wpa_supplicant debug log"},
    {"mib", wpa_cli_cmd_mib, NULL, cli_cmd_flag_none,
     "= get MIB variables (dot1x, dot11)"},
    {"help", wpa_cli_cmd_help, wpa_cli_complete_help, cli_cmd_flag_none,
     "[command] = show usage help"},
    {"interface", wpa_cli_cmd_interface, NULL, cli_cmd_flag_none,
     "[ifname] = show interfaces/select interface"},
    {"level", wpa_cli_cmd_level, NULL, cli_cmd_flag_none,
     "<debug level> = change debug level"},
    {"license", wpa_cli_cmd_license, NULL, cli_cmd_flag_none,
     "= show full wpa_cli license"},
    {"quit", wpa_cli_cmd_quit, NULL, cli_cmd_flag_none, "= exit wpa_cli"},
    {"set", wpa_cli_cmd_set, wpa_cli_complete_set, cli_cmd_flag_none,
     "= set variables (shows list of variables when run without "
     "arguments)"},
    {"dump", wpa_cli_cmd_dump, NULL, cli_cmd_flag_none,
     "= dump config variables"},
    {"get", wpa_cli_cmd_get, wpa_cli_complete_get, cli_cmd_flag_none,
     "<name> = get information"},
    {"driver_flags", wpa_cli_cmd_driver_flags, NULL, cli_cmd_flag_none,
     "= list driver flags"},
    {"logon", wpa_cli_cmd_logon, NULL, cli_cmd_flag_none,
     "= IEEE 802.1X EAPOL state machine logon"},
    {"logoff", wpa_cli_cmd_logoff, NULL, cli_cmd_flag_none,
     "= IEEE 802.1X EAPOL state machine logoff"},
    {"pmksa", wpa_cli_cmd_pmksa, NULL, cli_cmd_flag_none, "= show PMKSA cache"},
    {"pmksa_flush", wpa_cli_cmd_pmksa_flush, NULL, cli_cmd_flag_none,
     "= flush PMKSA cache entries"},
    {"reassociate", wpa_cli_cmd_reassociate, NULL, cli_cmd_flag_none,
     "= force reassociation"},
    {"reattach", wpa_cli_cmd_reattach, NULL, cli_cmd_flag_none,
     "= force reassociation back to the same BSS"},
    {"preauthenticate", wpa_cli_cmd_preauthenticate, wpa_cli_complete_bss,
     cli_cmd_flag_none, "<BSSID> = force preauthentication"},
    {"identity", wpa_cli_cmd_identity, NULL, cli_cmd_flag_none,
     "<network id> <identity> = configure identity for an SSID"},
    {"password", wpa_cli_cmd_password, NULL, cli_cmd_flag_sensitive,
     "<network id> <password> = configure password for an SSID"},
    {"new_password", wpa_cli_cmd_new_password, NULL, cli_cmd_flag_sensitive,
     "<network id> <password> = change password for an SSID"},
    {"pin", wpa_cli_cmd_pin, NULL, cli_cmd_flag_sensitive,
     "<network id> <pin> = configure pin for an SSID"},
    {"otp", wpa_cli_cmd_otp, NULL, cli_cmd_flag_sensitive,
     "<network id> <password> = configure one-time-password for an SSID"},
    {"passphrase", wpa_cli_cmd_passphrase, NULL, cli_cmd_flag_sensitive,
     "<network id> <passphrase> = configure private key passphrase\n"
     "  for an SSID"},
    {"sim", wpa_cli_cmd_sim, NULL, cli_cmd_flag_sensitive,
     "<network id> <pin> = report SIM operation result"},
    {"bssid", wpa_cli_cmd_bssid, NULL, cli_cmd_flag_none,
    ...
    {"get_network", wpa_cli_cmd_get_network, wpa_cli_complete_network,
     cli_cmd_flag_none, "<network id> <variable> = get network variables"},
    {"dup_network", wpa_cli_cmd_dup_network, wpa_cli_complete_dup_network,
     cli_cmd_flag_none,
     "<src network id> <dst network id> <variable> = duplicate network "
     "variables"},
    {"list_creds", wpa_cli_cmd_list_creds, NULL, cli_cmd_flag_none,
     "= list configured credentials"},
    {"add_cred", wpa_cli_cmd_add_cred, NULL, cli_cmd_flag_none,
     "= add a credential"},
    {"remove_cred", wpa_cli_cmd_remove_cred, NULL, cli_cmd_flag_none,
     "<cred id> = remove a credential"},
    {"set_cred", wpa_cli_cmd_set_cred, NULL, cli_cmd_flag_sensitive,
     "<cred id> <variable> <value> = set credential variables"},
    {"get_cred", wpa_cli_cmd_get_cred, NULL, cli_cmd_flag_none,
     "<cred id> <variable> = get credential variables"},
    {"save_config", wpa_cli_cmd_save_config, NULL, cli_cmd_flag_none,
     "= save the current configuration"},
    {"disconnect", wpa_cli_cmd_disconnect, NULL, cli_cmd_flag_none,
     "= disconnect and wait for reassociate/reconnect command before\n"
     "  connecting"},
    {"reconnect", wpa_cli_cmd_reconnect, NULL, cli_cmd_flag_none,
     "= like reassociate, but only takes effect if already disconnected"},
    {"scan", wpa_cli_cmd_scan, NULL, cli_cmd_flag_none,
     "= request new BSS scan"},
    {"scan_results", wpa_cli_cmd_scan_results, NULL, cli_cmd_flag_none,
     "= get latest scan results"},
    {"abort_scan", wpa_cli_cmd_abort_scan, NULL, cli_cmd_flag_none,
     "= request ongoing scan to be aborted"},
    {"bss", wpa_cli_cmd_bss, wpa_cli_complete_bss, cli_cmd_flag_none,
    ...
    {"get_capability", wpa_cli_cmd_get_capability,
     ...
     "= get capabilities"},
    {"reconfigure", wpa_cli_cmd_reconfigure, NULL, cli_cmd_flag_none,
     "= force wpa_supplicant to re-read its configuration file"},
    {"terminate", wpa_cli_cmd_terminate, NULL, cli_cmd_flag_none,
    ...
    {"ap_scan", wpa_cli_cmd_ap_scan, NULL, cli_cmd_flag_none,
     "<value> = set ap_scan parameter"},
    {"scan_interval", wpa_cli_cmd_scan_interval, NULL, cli_cmd_flag_none,
    ...
    {"bss_flush", wpa_cli_cmd_bss_flush, NULL, cli_cmd_flag_none,
     "<value> = set BSS flush age (0 by default)"},
    {"stkstart", wpa_cli_cmd_stkstart, NULL, cli_cmd_flag_none,
     "<addr> = request STK negotiation with <addr>"},
    {"ft_ds", wpa_cli_cmd_ft_ds, wpa_cli_complete_bss, cli_cmd_flag_none,
    ...
    {"wps_nfc_config_token", wpa_cli_cmd_wps_nfc_config_token, NULL,
     cli_cmd_flag_none, "<WPS|NDEF> = build configuration token"},
    {"wps_nfc_token", wpa_cli_cmd_wps_nfc_token, NULL, cli_cmd_flag_none,
    ...
    {"nfc_get_handover_req", wpa_cli_cmd_nfc_get_handover_req, NULL,
     cli_cmd_flag_none, "<NDEF> <WPS> = create NFC handover request"},
    {"nfc_get_handover_sel", wpa_cli_cmd_nfc_get_handover_sel, NULL,
     cli_cmd_flag_none, "<NDEF> <WPS> = create NFC handover select"},
    ...
    {"wps_ap_pin", wpa_cli_cmd_wps_ap_pin, NULL, cli_cmd_flag_sensitive,
     "[params..] = enable/disable AP PIN"},
    {"wps_er_start", wpa_cli_cmd_wps_er_start, NULL, cli_cmd_flag_none,
     "[IP address] = start Wi-Fi Protected Setup External Registrar"},
    {"wps_er_stop", wpa_cli_cmd_wps_er_stop, NULL, cli_cmd_flag_none,
     "= stop Wi-Fi Protected Setup External Registrar"},
    {"wps_er_pin", wpa_cli_cmd_wps_er_pin, NULL, cli_cmd_flag_sensitive,
     "<UUID> <PIN> = add an Enrollee PIN to External Registrar"},
    {"wps_er_pbc", wpa_cli_cmd_wps_er_pbc, NULL, cli_cmd_flag_none,
    ...
    {"wps_er_nfc_config_token", wpa_cli_cmd_wps_er_nfc_config_token, NULL,
     cli_cmd_flag_none, "<WPS/NDEF> <UUID> = build NFC configuration token"},
#endif /* CONFIG_WPS_NFC */
    {"ibss_rsn", wpa_cli_cmd_ibss_rsn, NULL, cli_cmd_flag_none,
     "<addr> = request RSN authentication with <addr> in IBSS"},
#ifdef CONFIG_AP
    {"sta", wpa_cli_cmd_sta, NULL, cli_cmd_flag_none,
     "<addr> = get information about an associated station (AP)"},
    {"all_sta", wpa_cli_cmd_all_sta, NULL, cli_cmd_flag_none,
     "= get information about all associated stations (AP)"},
    {"deauthenticate", wpa_cli_cmd_deauthenticate, NULL, cli_cmd_flag_none,
     "<addr> = deauthenticate a station"},
    {"disassociate", wpa_cli_cmd_disassociate, NULL, cli_cmd_flag_none,
     "<addr> = disassociate a station"},
    {"chan_switch", wpa_cli_cmd_chanswitch, NULL, cli_cmd_flag_none,
...
#endif /* CONFIG_TESTING_OPTIONS */
    {"roam", wpa_cli_cmd_roam, wpa_cli_complete_bss, cli_cmd_flag_none,
     "<addr> = roam to the specified BSS"},
#ifdef CONFIG_MESH
    {"mesh_interface_add", wpa_cli_cmd_mesh_interface_add, NULL,
     cli_cmd_flag_none, "[ifname] = Create a new mesh interface"},
    ...
    {"mesh_group_remove", wpa_cli_cmd_mesh_group_remove, NULL,
     cli_cmd_flag_none, "<ifname> = Remove mesh group interface"},
    {"mesh_peer_remove", wpa_cli_cmd_mesh_peer_remove, NULL, cli_cmd_flag_none,
     "<addr> = Remove a mesh peer"},
    {"mesh_peer_add", wpa_cli_cmd_mesh_peer_add, NULL, cli_cmd_flag_none,
    ...
    {"p2p_asp_provision", wpa_cli_cmd_p2p_asp_provision, NULL,
     cli_cmd_flag_none,
     "<addr> adv_id=<adv_id> conncap=<conncap> [info=<infodata>] = provision "
     "with a P2P ASP Device"},
    {"p2p_asp_provision_resp", wpa_cli_cmd_p2p_asp_provision_resp, NULL,
     cli_cmd_flag_none,
     "<addr> adv_id=<adv_id> [role<conncap>] [info=<infodata>] = provision "
     "with a P2P ASP Device"},
    ...
    {"p2p_group_member", wpa_cli_cmd_p2p_group_member, NULL, cli_cmd_flag_none,
     "<dev_addr> = Get peer interface address on local GO using peer Device "
     "Address"},
    {"p2p_prov_disc", wpa_cli_cmd_p2p_prov_disc, wpa_cli_complete_p2p_peer,
     cli_cmd_flag_none, "<addr> <method> = request provisioning discovery"},
    {"p2p_get_passphrase", wpa_cli_cmd_p2p_get_passphrase, NULL,
     cli_cmd_flag_none, "= get the passphrase for a group (GO only)"},
    ...
    {"p2p_serv_disc_cancel_req", wpa_cli_cmd_p2p_serv_disc_cancel_req, NULL,
     cli_cmd_flag_none, "<id> = cancel pending service discovery request"},
    ...
    {"p2p_service_update", wpa_cli_cmd_p2p_service_update, NULL,
     cli_cmd_flag_none, "= indicate change in local services"},
    ...
    {"p2p_service_flush", wpa_cli_cmd_p2p_service_flush, NULL,
     cli_cmd_flag_none, "= remove all stored service entries"},
    ...
    {"p2p_invite", wpa_cli_cmd_p2p_invite, NULL, cli_cmd_flag_none,
    ...
    {"p2p_peer", wpa_cli_cmd_p2p_peer, wpa_cli_complete_p2p_peer,
     cli_cmd_flag_none, "<address> = show information about known P2P peer"},
    {"p2p_set", wpa_cli_cmd_p2p_set, wpa_cli_complete_p2p_set,
     cli_cmd_flag_none, "<field> <value> = set a P2P parameter"},
    ...
    {"p2p_unauthorize", wpa_cli_cmd_p2p_unauthorize, wpa_cli_complete_p2p_peer,
     cli_cmd_flag_none, "<address> = unauthorize a peer"},
    ...
    {"vendor_elem_add", wpa_cli_cmd_vendor_elem_add, NULL, cli_cmd_flag_none,
     "<frame id> <hexdump of elem(s)> = add vendor specific IEs to "
     "frame(s)\n" VENDOR_ELEM_FRAME_ID},
    {"vendor_elem_get", wpa_cli_cmd_vendor_elem_get, NULL, cli_cmd_flag_none,
     "<frame id> = get vendor specific IE(s) to "
     "frame(s)\n" VENDOR_ELEM_FRAME_ID},
    {"vendor_elem_remove", wpa_cli_cmd_vendor_elem_remove, NULL,
     cli_cmd_flag_none,
     "<frame id> <hexdump of elem(s)> = remove vendor specific IE(s) in "
     "frame(s)\n" VENDOR_ELEM_FRAME_ID},
#endif /* CONFIG_P2P */
#ifdef CONFIG_WIFI_DISPLAY
    ...
    {"interworking_select", wpa_cli_cmd_interworking_select, NULL,
     cli_cmd_flag_none, "[auto] = perform Interworking network selection"},
    ...
    {"gas_request", wpa_cli_cmd_gas_request, wpa_cli_complete_bss,
     cli_cmd_flag_none, "<addr> <AdvProtoID> [QueryReq] = GAS request"},
    ...
    {"hs20_icon_request", wpa_cli_cmd_hs20_icon_request, wpa_cli_complete_bss,
     cli_cmd_flag_none, "<addr> <icon name> = get Hotspot 2.0 OSU icon"},
    ...
    {"tdls_discover", wpa_cli_cmd_tdls_discover, NULL, cli_cmd_flag_none,
     "<addr> = request TDLS discovery with <addr>"},
    {"tdls_setup", wpa_cli_cmd_tdls_setup, NULL, cli_cmd_flag_none,
     "<addr> = request TDLS setup with <addr>"},
    {"tdls_teardown", wpa_cli_cmd_tdls_teardown, NULL, cli_cmd_flag_none,
     "<addr> = tear down TDLS with <addr>"},
    {"tdls_link_status", wpa_cli_cmd_tdls_link_status, NULL, cli_cmd_flag_none,
     "<addr> = TDLS link status with <addr>"},
    {"wmm_ac_addts", wpa_cli_cmd_wmm_ac_addts, NULL, cli_cmd_flag_none,
    ...
    {"wmm_ac_delts", wpa_cli_cmd_wmm_ac_delts, NULL, cli_cmd_flag_none,
     "<tsid> = delete WMM-AC traffic stream"},
    {"wmm_ac_status", wpa_cli_cmd_wmm_ac_status, NULL, cli_cmd_flag_none,
    ...
    {"signal_poll", wpa_cli_cmd_signal_poll, NULL, cli_cmd_flag_none,
     "= get signal parameters"},
    {"signal_monitor", wpa_cli_cmd_signal_monitor, NULL, cli_cmd_flag_none,
     "= set signal monitor parameters"},
    {"pktcnt_poll", wpa_cli_cmd_pktcnt_poll, NULL, cli_cmd_flag_none,
    ...
    {"vendor", wpa_cli_cmd_vendor, NULL, cli_cmd_flag_none,
     "<vendor id> <command id> [<hex formatted command argument>] = Send "
     "vendor command"},
    {"neighbor_rep_request", wpa_cli_cmd_neighbor_rep_request, NULL,
     cli_cmd_flag_none,
     "[ssid=<SSID>] [lci] [civic] = Trigger request to AP for neighboring AP "
     "report (with optional given SSID in hex or enclosed in double quotes, "
     "default: current SSID; with optional LCI and location civic request)"},
    {"erp_flush", wpa_cli_cmd_erp_flush, NULL, cli_cmd_flag_none,
     "= flush ERP keys"},
    {"mac_rand_scan", wpa_cli_cmd_mac_rand_scan, NULL, cli_cmd_flag_none,
     "<scan|sched|pno|all> enable=<0/1> [addr=mac-address "
     "mask=mac-address-mask] = scan MAC randomization"},
    {"get_pref_freq_list", wpa_cli_cmd_get_pref_freq_list, NULL,
     cli_cmd_flag_none,
     "<interface type> = retrieve preferred freq list for the specified "
     "interface type"},
    {"p2p_lo_start", wpa_cli_cmd_p2p_lo_start, NULL, cli_cmd_flag_none,
     "<freq> <period> <interval> <count> = start P2P listen offload"},
    {"p2p_lo_stop", wpa_cli_cmd_p2p_lo_stop, NULL, cli_cmd_flag_none,
...
static int wpa_cli_edit_filter_history_cb(void *ctx, const char *cmd) {
  ...
  for (n = 0; (c = wpa_cli_commands[n].cmd); n++) {
    if (os_strncasecmp(cmd, c, len) == 0 && len == os_strlen(c))
      return (wpa_cli_commands[n].flags & cli_cmd_flag_sensitive);
  }
  return 0;
}
...
static void wpa_cli_edit_eof_cb(void *ctx) { eloop_terminate(); }
...
static void wpa_cli_action_ping(void *eloop_ctx, void *timeout_ctx) {
  ...
  len = sizeof(buf) - 1;
  if (wpa_ctrl_request(ctrl, "PING", 4, buf, &len, wpa_cli_action_cb) < 0 ||
      len < 4 || os_memcmp(buf, "PONG", 4) != 0) {
    printf("wpa_supplicant did not reply to PING command - exiting\n");
    eloop_terminate();
    return;
  }
  eloop_register_timeout(ping_interval, 0, wpa_cli_action_ping, ctrl, NULL);
}
...
static void wpa_cli_terminate(int sig, void *ctx) { eloop_terminate(); }
