...
static void wpa_priv_cmd_register(struct wpa_priv_interface *iface,
  ...
  if (iface->driver->init2) {
    if (iface->driver->global_init) {
      ...
      if (!iface->drv_global_priv) {
        wpa_printf(MSG_INFO, "Failed to initialize driver global context");
        return;
      }
    } else {
      iface->drv_global_priv = NULL;
    ...
    iface->drv_priv =
        iface->driver->init2(iface, iface->ifname, iface->drv_global_priv);
  ...
  wpa_printf(MSG_DEBUG,
             "Driver wrapper '%s' initialized for interface "
             "'%s'",
             iface->driver_name, iface->ifname);
...
static void wpa_priv_cmd_scan(struct wpa_priv_interface *iface, char *buf,
                              size_t len) {
...
static void wpa_priv_get_scan_results2(struct wpa_priv_interface *iface,
  ...
  sendto(iface->fd, buf, pos - buf, 0, (struct sockaddr *)from, sizeof(*from));
...
static void wpa_priv_cmd_associate(struct wpa_priv_interface *iface, void *buf,
                                   size_t len) {
...
static void wpa_priv_cmd_get_bssid(struct wpa_priv_interface *iface,
  ...
  sendto(iface->fd, bssid, ETH_ALEN, 0, (struct sockaddr *)from, sizeof(*from));
  return;
...
static void wpa_priv_cmd_set_key(struct wpa_priv_interface *iface, void *buf,
                                 size_t len) {
  ...
  res = iface->driver->set_key(
      iface->ifname, iface->drv_priv, params->alg, params->addr,
      params->key_idx, params->set_tx, params->seq_len ? params->seq : NULL,
      params->seq_len, params->key_len ? params->key : NULL, params->key_len);
...
static void wpa_priv_cmd_l2_register(struct wpa_priv_interface *iface,
                                     struct sockaddr_un *from, void *buf,
                                     size_t len) {
  int *reg_cmd = buf;
  ...
  if (len != 2 * sizeof(int)) {
    wpa_printf(MSG_DEBUG, "Invalid l2_register length %lu", (unsigned long)len);
    return;
  }
  ...
  if (proto != ETH_P_EAPOL && proto != ETH_P_RSN_PREAUTH &&
    ...
    wpa_printf(MSG_DEBUG,
               "Refused l2_packet connection for "
               "ethertype 0x%x",
               proto);
    return;
  }
  ...
  iface->l2 = l2_packet_init(iface->ifname, NULL, proto, wpa_priv_l2_rx, iface,
                             reg_cmd[1]);
  if (iface->l2 == NULL) {
    wpa_printf(MSG_DEBUG,
               "Failed to initialize l2_packet "
               "instance for protocol %d",
               proto);
    return;
  }
  ...
  res = sendto(iface->fd, own_addr, ETH_ALEN, 0, (struct sockaddr *)from,
               sizeof(*from));
...
static void wpa_priv_cmd_l2_send(struct wpa_priv_interface *iface,
  ...
  if (proto != ETH_P_EAPOL && proto != ETH_P_RSN_PREAUTH) {
    wpa_printf(MSG_DEBUG,
               "Refused l2_packet send for ethertype "
               "0x%x",
               proto);
    return;
  }
...
static void wpa_priv_receive(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  res = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *)&from, &fromlen);
  if (res < 0) {
  ...
  wpa_printf(MSG_DEBUG, "Command %d for interface %s", cmd, iface->ifname);
...
wpa_priv_interface_init(void *ctx, const char *dir, const char *params) {
  ...
  for (i = 0; wpa_drivers[i]; i++) {
    if (os_strcmp(iface->driver_name, wpa_drivers[i]->name) == 0) {
      iface->driver = wpa_drivers[i];
      break;
    }
  }
  if (iface->driver == NULL) {
    wpa_printf(MSG_ERROR, "Unsupported driver '%s'", iface->driver_name);
    wpa_priv_interface_deinit(iface);
    return NULL;
  }
  ...
  if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    wpa_printf(MSG_DEBUG, "bind(PF_UNIX) failed: %s", strerror(errno));
    if (connect(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
      ...
      if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        wpa_printf(MSG_ERROR, "wpa-priv-iface-init: bind(PF_UNIX): %s",
                   strerror(errno));
        goto fail;
      }
      wpa_printf(MSG_DEBUG,
...
static int wpa_priv_send_event(struct wpa_priv_interface *iface, int event,
  ...
  if (sendmsg(iface->fd, &msg, 0) < 0) {
    wpa_printf(MSG_ERROR, "sendmsg(wpas_socket): %s", strerror(errno));
    return -1;
  }
...
static void wpa_priv_send_assoc(struct wpa_priv_interface *iface, int event,
  ...
  if (data) {
    buflen += data->assoc_info.req_ies_len + data->assoc_info.resp_ies_len +
...
static void wpa_priv_send_interface_status(struct wpa_priv_interface *iface,
  ...
  wpa_priv_send_event(iface, PRIVSEP_EVENT_INTERFACE_STATUS, buf,
                      sizeof(int) + len);
...
void wpa_supplicant_event(void *ctx, enum wpa_event_type event,
  ...
  case EVENT_MICHAEL_MIC_FAILURE:
    ...
    wpa_priv_send_event(iface, PRIVSEP_EVENT_MICHAEL_MIC_FAILURE,
                        &data->michael_mic_failure.unicast, sizeof(int));
    break;
  case EVENT_SCAN_STARTED:
    wpa_priv_send_event(iface, PRIVSEP_EVENT_SCAN_STARTED, NULL, 0);
    break;
  case EVENT_SCAN_RESULTS:
    wpa_priv_send_event(iface, PRIVSEP_EVENT_SCAN_RESULTS, NULL, 0);
    break;
  ...
  case EVENT_PMKID_CANDIDATE:
    ...
    wpa_priv_send_event(iface, PRIVSEP_EVENT_PMKID_CANDIDATE,
                        &data->pmkid_candidate, sizeof(struct pmkid_candidate));
    break;
  case EVENT_STKSTART:
    if (data == NULL)
      return;
    wpa_priv_send_event(iface, PRIVSEP_EVENT_STKSTART, &data->stkstart.peer,
                        ETH_ALEN);
    break;
  ...
  case EVENT_AUTH:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "Unsupported driver event %d (%s) - TODO", event,
...
void wpa_supplicant_event_global(void *ctx, enum wpa_event_type event,
  ...
  if (iface && iface->driver->get_ifindex) {
    ...
    if (ifindex != data->interface_status.ifindex) {
      wpa_printf(MSG_DEBUG, "%s: interface status ifindex %d mismatch (%d)",
                 iface->ifname, ifindex, data->interface_status.ifindex);
      return;
    }
  }
  if (iface)
