...
static struct wpa_bss *wpa_supplicant_get_new_bss(struct wpa_supplicant *wpa_s,
                                                  const u8 *bssid) {
...
static int wpa_supplicant_select_config(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->conf->ap_scan == 1 && wpa_s->current_ssid) {
    ...
    if (res < 0) {
      wpa_msg(wpa_s, MSG_INFO, "Failed to read SSID from driver");
      return 0; /* try to use current profile */
    }
    drv_ssid_len = res;

    if (drv_ssid_len == wpa_s->current_ssid->ssid_len &&
        os_memcmp(drv_ssid, wpa_s->current_ssid->ssid, drv_ssid_len) == 0)
      return 0; /* current profile still in use */
  ...
  if (res > 0) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Selected network is temporarily "
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Network configuration found for the "
          "current AP");
  if (wpa_key_mgmt_wpa_any(ssid->key_mgmt)) {
    u8 wpa_ie[80];
    size_t wpa_ie_len = sizeof(wpa_ie);
    if (wpa_supplicant_set_suites(wpa_s, NULL, ssid, wpa_ie, &wpa_ie_len) < 0)
...
static void wpa_find_assoc_pmkid(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_sm_parse_own_wpa_ie(wpa_s->wpa, &ie) < 0 || ie.pmkid == NULL)
    return;

  for (i = 0; i < ie.num_pmkid; i++) {
    pmksa_set = pmksa_cache_set_current(wpa_s->wpa, ie.pmkid + i * PMKID_LEN,
                                        NULL, NULL, 0);
    if (pmksa_set == 0) {
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "RSN: PMKID from assoc IE %sfound from "
          "PMKSA cache",
          pmksa_set == 0 ? "" : "not ");
}
...
static void wpa_supplicant_event_pmkid_candidate(struct wpa_supplicant *wpa_s,
  ...
  if (data == NULL) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "RSN: No data in PMKID candidate "
            "event");
    return;
  }
  wpa_dbg(wpa_s, MSG_DEBUG,
          "RSN: PMKID candidate event - bssid=" MACSTR " index=%d preauth=%d",
          MAC2STR(data->pmkid_candidate.bssid), data->pmkid_candidate.index,
...
int wpa_supplicant_scard_init(struct wpa_supplicant *wpa_s,
  ...
  if ((ssid != NULL && ssid->eap.pcsc == NULL) || wpa_s->scard != NULL ||
  ...
  if (ssid == NULL || ssid->eap.eap_methods == NULL) {
    ...
    while (eap->vendor != EAP_VENDOR_IETF || eap->method != EAP_TYPE_NONE) {
  ...
  if (eap_peer_get_eap_method(EAP_VENDOR_IETF, EAP_TYPE_AKA) == NULL &&
      eap_peer_get_eap_method(EAP_VENDOR_IETF, EAP_TYPE_AKA_PRIME) == NULL)
    aka = 0;

  if (!sim && !aka) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Selected network is configured to "
...
static int wpa_supplicant_ssid_bss_match(struct wpa_supplicant *wpa_s,
  ...
  while ((ssid->proto & WPA_PROTO_RSN) && rsn_ie) {
    ...
    if (wep_ok && (ie.group_cipher & (WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104))) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "   selected based on TSN "
              "in RSN IE");
      return 1;
    }
    ...
    if ((ie.capabilities & WPA_CAPABILITY_MFPR) &&
      ...
      wpa_dbg(wpa_s, MSG_DEBUG,
              "   skip RSN IE - no mgmt frame protection enabled but AP "
              "requires it");
      break;
    }
#ifdef CONFIG_MBO
  ...
  if ((ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_NO_WPA) && !wpa_ie && !rsn_ie) {
...
static int rate_match(struct wpa_supplicant *wpa_s, struct wpa_bss *bss) {
  ...
  for (i = 0; i < wpa_s->hw.num_modes; i++) {
    for (j = 0; j < modes[i].num_channels; j++) {
      ...
      if (freq == bss->freq) {
        if (mode && mode->mode == HOSTAPD_MODE_IEEE80211G)
  ...
  for (i = 0; i < (int)sizeof(scan_ie); i++) {
    ...
    for (j = 2; j < rate_ie[1] + 2; j++) {
       ...
       */
      if (flagged && ((rate_ie[j] & 0x7f) == BSS_MEMBERSHIP_SELECTOR_HT_PHY)) {
      ...
      if (flagged && ((rate_ie[j] & 0x7f) == BSS_MEMBERSHIP_SELECTOR_VHT_PHY)) {
      ...
      if (k == mode->num_rates) {
         ...
         */
        wpa_dbg(wpa_s, MSG_DEBUG,
                "   hardware does not support required rate %d.%d Mbps "
                "(freq=%d mode==%d num_rates=%d)",
                r / 10, r % 10, bss->freq, mode->mode, mode->num_rates);
        return 0;
      }
    }
  }
...
static int bss_is_ess(struct wpa_bss *bss) {
  if (bss_is_dmg(bss)) {
    return (bss->caps & IEEE80211_CAP_DMG_MASK) == IEEE80211_CAP_DMG_AP;
  }
...
struct wpa_ssid *wpa_scan_res_match(struct wpa_supplicant *wpa_s, int i,
                                    struct wpa_bss *bss, struct wpa_ssid *group,
                                    int only_first_ssid) {
  ...
  for (ssid = group; ssid; ssid = only_first_ssid ? NULL : ssid->pnext) {
    ...
    if (check_ssid && (bss->ssid_len != ssid->ssid_len ||
    ...
    if (!osen && !wpa && !(ssid->key_mgmt & WPA_KEY_MGMT_NONE) &&
        !(ssid->key_mgmt & WPA_KEY_MGMT_WPS) &&
        !(ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_NO_WPA)) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "   skip - non-WPA network "
              "not allowed");
      continue;
    }
    ...
    if (ssid->mode != IEEE80211_MODE_MESH && !bss_is_ess(bss) &&
        !bss_is_pbss(bss)) {
      wpa_dbg(wpa_s, MSG_DEBUG, "   skip - not ESS, PBSS, or MBSS");
      continue;
    }
...
#ifdef CONFIG_P2P
    if (ssid->p2p_group && !wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) &&
        !wpa_bss_get_vendor_ie_beacon(bss, P2P_IE_VENDOR_TYPE)) {
    ...
    if (os_reltime_before(&bss->last_update, &wpa_s->scan_min_time)) {
      ...
      wpa_dbg(
          ...
          (unsigned int)diff.sec, (unsigned int)diff.usec);
      continue;
    }
#ifdef CONFIG_MBO
    ...
    if (wpa_is_bss_tmp_disallowed(wpa_s, bss->bssid)) {
      wpa_dbg(wpa_s, MSG_DEBUG, "   skip - MBO retry delay has not passed yet");
      continue;
    }
#ifdef CONFIG_TESTING_OPTIONS
  skip_assoc_disallow:
...
wpa_supplicant_select_bss(struct wpa_supplicant *wpa_s, struct wpa_ssid *group,
  ...
  if (only_first_ssid)
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Try to find BSS matching pre-selected network id=%d", group->id);
  else
  ...
  for (i = 0; i < wpa_s->last_scan_res_used; i++) {
    ...
    *selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid);
    if (!*selected_ssid)
      continue;
    wpa_dbg(wpa_s, MSG_DEBUG, "   selected BSS " MACSTR " ssid='%s'",
...
struct wpa_bss *wpa_supplicant_pick_network(struct wpa_supplicant *wpa_s,
  ...
  while (selected == NULL) {
    for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
      if (next_ssid &&
          next_ssid->priority == wpa_s->conf->pssid[prio]->priority) {
        selected =
            wpa_supplicant_select_bss(wpa_s, next_ssid, selected_ssid, 1);
        if (selected)
          break;
      }
      selected = wpa_supplicant_select_bss(wpa_s, wpa_s->conf->pssid[prio],
                                           selected_ssid, 0);
      if (selected)
        break;
    }

    if (selected == NULL && wpa_s->blacklist && !wpa_s->countermeasures) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "No APs found - clear "
  ...
  if (selected && ssid && ssid->mem_only_psk && !ssid->psk_set &&
    ...
    wpas_notify_network_request(wpa_s, ssid, WPA_CTRL_REQ_PSK_PASSPHRASE, NULL);

    field_name = wpa_supplicant_ctrl_req_to_string(WPA_CTRL_REQ_PSK_PASSPHRASE,
                                                   NULL, &txt);
    if (field_name == NULL)
      return NULL;
...
static void wpa_supplicant_req_new_scan(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_supplicant_enabled_networks(wpa_s)) {
     ...
     */
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Short-circuit new scan request "
...
int wpa_supplicant_connect(struct wpa_supplicant *wpa_s,
  ...
  if (wpas_wps_scan_pbc_overlap(wpa_s, selected, ssid)) {
    wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_OVERLAP "PBC session overlap");
    wpas_notify_wps_event_pbc_overlap(wpa_s);
#ifdef CONFIG_P2P
    if (wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT ||
        wpa_s->p2p_in_provisioning) {
      eloop_register_timeout(0, 0, wpas_p2p_pbc_overlap_cb, wpa_s, NULL);
      return -1;
    }
#endif /* CONFIG_P2P */
  ...
  wpa_msg(wpa_s, MSG_DEBUG,
          "Considering connect request: reassociate: %d  selected: " MACSTR
          "  bssid: " MACSTR "  pending: " MACSTR
          "  wpa_state: %s  ssid=%p  current_ssid=%p",
          wpa_s->reassociate, MAC2STR(selected->bssid), MAC2STR(wpa_s->bssid),
          MAC2STR(wpa_s->pending_bssid),
          wpa_supplicant_state_txt(wpa_s->wpa_state), ssid,
          wpa_s->current_ssid);
  ...
  if (wpa_s->reassociate ||
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Already associated or trying to "
  ...
  for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
    for (ssid = wpa_s->conf->pssid[prio]; ssid; ssid = ssid->pnext) {
      ...
      if (ssid->mode == IEEE80211_MODE_IBSS ||
          ssid->mode == IEEE80211_MODE_AP || ssid->mode == IEEE80211_MODE_MESH)
        return ssid;
    }
  }
  return NULL;
...
static void
wpa_supplicant_rsn_preauth_scan_results(struct wpa_supplicant *wpa_s) {
  struct wpa_bss *bss;
...
static int wpa_supplicant_need_to_roam(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->current_ssid->ssid)
    current_bss = wpa_bss_get(wpa_s, wpa_s->bssid, wpa_s->current_ssid->ssid,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Current BSS: " MACSTR " level=%d snr=%d est_throughput=%u",
          MAC2STR(current_bss->bssid), current_bss->level, current_bss->snr,
          current_bss->est_throughput);
  wpa_dbg(wpa_s, MSG_DEBUG,
          "Selected BSS: " MACSTR " level=%d snr=%d est_throughput=%u",
          MAC2STR(selected->bssid), selected->level, selected->snr,
          selected->est_throughput);

  if (wpa_s->current_ssid->bssid_set &&
      os_memcmp(selected->bssid, wpa_s->current_ssid->bssid, ETH_ALEN) == 0) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Allow reassociation - selected BSS "
            "has preferred BSSID");
    return 1;
  }
  ...
  if (current_bss->level < 0 &&
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Skip roam - Current BSS has better "
            "signal level");
    return 0;
  }
  ...
  if (abs(current_bss->level - selected->level) < min_diff) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Skip roam - too small difference "
            "in signal level");
    return 0;
  }
...
static int _wpa_supplicant_event_scan_results(struct wpa_supplicant *wpa_s,
  ...
  scan_res =
      wpa_supplicant_get_scan_results(wpa_s, data ? &data->scan_info : NULL, 1);
  if (scan_res == NULL) {
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "New scan results available (own=%u ext=%u)",
          wpa_s->own_scan_running, data ? data->scan_info.external_scan : 0);
  if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && wpa_s->manual_scan_use_id &&
      wpa_s->own_scan_running && own_request &&
      !(data && data->scan_info.external_scan)) {
  ...
  if (data && data->scan_info.external_scan) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Do not use results from externally requested scan operation for "
            "network selection");
...
static int wpas_select_network_from_last_scan(struct wpa_supplicant *wpa_s,
  ...
  if (time_to_reenable > 0) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Postpone network selection by %d seconds since all networks are "
            "disabled",
            time_to_reenable);
    eloop_cancel_timeout(wpas_network_reenabled, wpa_s, NULL);
    eloop_register_timeout(time_to_reenable, 0, wpas_network_reenabled, wpa_s,
                           NULL);
    return 0;
  }
  ...
  if (selected) {
    ...
    if (ssid != wpa_s->current_ssid && wpa_s->wpa_state >= WPA_AUTHENTICATING) {
...
static void wnm_bss_keep_alive(void *eloop_ctx, void *sock_ctx) {
  ...
  if (wpa_s->sme.bss_max_idle_period) {
    ...
    if (msec > 100)
      msec -= 100;
    eloop_register_timeout(msec / 1000, msec % 1000 * 1000, wnm_bss_keep_alive,
                           wpa_s, NULL);
  }
#endif /* CONFIG_SME */
}

static void wnm_process_assoc_resp(struct wpa_supplicant *wpa_s, const u8 *ies,
                                   size_t ies_len) {
  ...
  if (elems.bss_max_idle_period) {
    unsigned int msec;
    wpa_s->sme.bss_max_idle_period = WPA_GET_LE16(elems.bss_max_idle_period);
    wpa_printf(MSG_DEBUG,
               "WNM: BSS Max Idle Period: %u (* 1000 "
               "TU)%s",
...
static int wpas_qos_map_set(struct wpa_supplicant *wpa_s, const u8 *qos_map,
  ...
  if (res) {
    wpa_printf(MSG_DEBUG,
               "Interworking: Failed to configure QoS Map Set to the driver");
  }
...
static void interworking_process_assoc_resp(struct wpa_supplicant *wpa_s,
  ...
  if (elems.qos_map_set) {
    wpas_qos_map_set(wpa_s, elems.qos_map_set, elems.qos_map_set_len);
  }
}
...
#ifdef CONFIG_FST
static int wpas_fst_update_mbie(struct wpa_supplicant *wpa_s, const u8 *ie,
                                size_t ie_len) {
  ...
  while (ie_len >= 2 && mb_ies.nof_ies < MAX_NOF_MB_IES_SUPPORTED) {
    ...
    len = 2 + ie[1];
    if (len > ie_len) {
      wpa_hexdump(MSG_DEBUG, "FST: Truncated IE found", ie, ie_len);
      break;
    }

    if (ie[0] == WLAN_EID_MULTI_BAND) {
      wpa_printf(MSG_DEBUG, "MB IE of %u bytes found", (unsigned int)len);
...
static int wpa_supplicant_event_associnfo(struct wpa_supplicant *wpa_s,
  ...
  if (data->assoc_info.beacon_ies)
    wpa_hexdump(MSG_DEBUG, "beacon_ies", data->assoc_info.beacon_ies,
                data->assoc_info.beacon_ies_len);
  if (data->assoc_info.freq)
    wpa_dbg(wpa_s, MSG_DEBUG, "freq=%u MHz", data->assoc_info.freq);
  ...
  if (wpa_s->sme.auth_alg == WPA_AUTH_ALG_FT) {
    if (wpa_drv_get_bssid(wpa_s, bssid) < 0 ||
        wpa_ft_validate_reassoc_resp(wpa_s->wpa, data->assoc_info.resp_ies,
                                     data->assoc_info.resp_ies_len,
                                     bssid) < 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "FT: Validation of "
  ...
  while (p && l >= 2) {
    ...
    if (p[0] == WLAN_EID_MOBILITY_DOMAIN && p[1] >= MOBILITY_DOMAIN_ID_LEN) {
      wpa_s->sme.ft_used = 1;
      os_memcpy(wpa_s->sme.mobility_domain, p + 2, MOBILITY_DOMAIN_ID_LEN);
      break;
    }
    l -= len;
    p += len;
  ...
  while (p && l >= 2) {
    len = p[1] + 2;
    if (len > l) {
      wpa_hexdump(MSG_DEBUG, "Truncated IE in beacon_ies", p, l);
      break;
    }
    if (!wpa_found && p[0] == WLAN_EID_VENDOR_SPECIFIC && p[1] >= 6 &&
        os_memcmp(&p[2], "\x00\x50\xF2\x01\x01\x00", 6) == 0) {
      wpa_found = 1;
      wpa_sm_set_ap_wpa_ie(wpa_s->wpa, p, len);
    }

    if (!rsn_found && p[0] == WLAN_EID_RSN && p[1] >= 2) {
      rsn_found = 1;
  ...
  if (wpa_s->assoc_freq && data->assoc_info.freq &&
    ...
    wpa_printf(MSG_DEBUG,
               "Operating frequency changed from "
               "%u to %u MHz",
               wpa_s->assoc_freq, data->assoc_info.freq);
    wpa_supplicant_update_scan_results(wpa_s);
  }
...
static void wpas_fst_update_mb_assoc(struct wpa_supplicant *wpa_s,
  ...
  if (ai && !wpas_fst_update_mbie(wpa_s, ai->resp_ies, ai->resp_ies_len)) {
    wpa_printf(MSG_DEBUG,
               "FST: MB IEs updated from Association Response frame");
    return;
  }

  if (ai && !wpas_fst_update_mbie(wpa_s, ai->beacon_ies, ai->beacon_ies_len)) {
    wpa_printf(MSG_DEBUG,
...
static void wpa_supplicant_event_assoc(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->ap_iface) {
    if (!data)
      return;
    hostapd_notif_assoc(wpa_s->ap_iface->bss[0], data->assoc_info.addr,
  ...
  if (os_memcmp(bssid, wpa_s->bssid, ETH_ALEN) != 0) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Associated to a new BSS: BSSID=" MACSTR,
            MAC2STR(bssid));
    new_bss = 1;
  ...
  if (wpa_s->conf->ap_scan == 1 &&
    ...
    if (wpa_supplicant_assoc_update_ie(wpa_s) < 0 && new_bss)
      wpa_msg(wpa_s, MSG_WARNING, "WPA/RSN IEs not updated");
  }
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
    ...
    if (wpa_s->current_ssid && wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE &&
        (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SET_KEYS_AFTER_ASSOC_DONE)) {
      /*
       * Set the key after having received joined-IBSS event
       * from the driver.
       */
      wpa_supplicant_set_wpa_none_key(wpa_s, wpa_s->current_ssid);
    }
  ...
  if (wpa_s->pending_eapol_rx) {
    ...
    if (age.sec == 0 && age.usec < 100000 &&
        os_memcmp(wpa_s->pending_eapol_rx_src, bssid, ETH_ALEN) == 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "Process pending EAPOL "
              "frame that was received just before "
              "association notification");
      wpa_supplicant_rx_eapol(wpa_s, wpa_s->pending_eapol_rx_src,
  ...
  if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_IBSS &&
      ...
      wpa_s->key_mgmt != WPA_KEY_MGMT_WPA_NONE && wpa_s->ibss_rsn == NULL) {
...
static void wpa_supplicant_event_disassoc(struct wpa_supplicant *wpa_s,
  ...
  if (!is_zero_ether_addr(bssid) || wpa_s->wpa_state >= WPA_AUTHENTICATING) {
    wpa_msg(wpa_s, MSG_INFO,
            WPA_EVENT_DISCONNECTED "bssid=" MACSTR " reason=%d%s",
...
static void wpa_supplicant_event_disassoc_finish(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->disconnected &&
      (!wpa_s->auto_reconnect_disabled || wpa_s->key_mgmt == WPA_KEY_MGMT_WPS ||
       wpas_wps_searching(wpa_s) ||
       wpas_wps_reenable_networks_pending(wpa_s))) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Auto connect enabled: try to "
            "reconnect (wps=%d/%d wpa_state=%d)",
            wpa_s->key_mgmt == WPA_KEY_MGMT_WPS, wpas_wps_searching(wpa_s),
            wpa_s->wpa_state);
    if (wpa_s->wpa_state == WPA_COMPLETED && wpa_s->current_ssid &&
        wpa_s->current_ssid->mode == WPAS_MODE_INFRA && !locally_generated &&
  ...
  if (fast_reconnect && !wpas_network_disabled(wpa_s, fast_reconnect_ssid) &&
      ...
      !disallowed_ssid(wpa_s, fast_reconnect->ssid, fast_reconnect->ssid_len) &&
      !wpas_temp_disabled(wpa_s, fast_reconnect_ssid) &&
      !wpa_is_bss_tmp_disallowed(wpa_s, fast_reconnect->bssid)) {
#ifndef CONFIG_NO_SCAN_PROCESSING
    wpa_dbg(wpa_s, MSG_DEBUG, "Try to reconnect to the same BSS");
    if (wpa_supplicant_connect(wpa_s, fast_reconnect, fast_reconnect_ssid) <
        0) {
...
wpa_supplicant_event_michael_mic_failure(struct wpa_supplicant *wpa_s,
  ...
  if ((wpa_s->last_michael_mic_error.sec &&
    ...
    wpa_supplicant_deauthenticate(wpa_s, WLAN_REASON_MICHAEL_MIC_FAILURE);
    eloop_cancel_timeout(wpa_supplicant_stop_countermeasures, wpa_s, NULL);
    eloop_register_timeout(60, 0, wpa_supplicant_stop_countermeasures, wpa_s,
                           NULL);
    ...
    if (wpa_s->mic_errors_seen) {
      ...
      eloop_cancel_timeout(wpa_supplicant_delayed_mic_error_report, wpa_s,
                           NULL);
      eloop_register_timeout(sec, os_random() % 1000000,
                             wpa_supplicant_delayed_mic_error_report, wpa_s,
                             NULL);
    } else {
      wpa_sm_key_request(wpa_s->wpa, 1, pairwise);
    }
...
#ifdef CONFIG_P2P
    if (!wpa_s->global->p2p && !wpa_s->global->p2p_disabled &&
        !wpa_s->conf->p2p_disabled &&
        (wpa_s->drv_flags & WPA_DRIVER_FLAGS_DEDICATED_P2P_DEVICE) &&
        wpas_p2p_add_p2pdev_interface(wpa_s,
...
#ifdef CONFIG_P2P
    if (wpa_s->global->p2p && wpa_s->global->p2p_init_wpa_s->parent == wpa_s &&
        (wpa_s->drv_flags & WPA_DRIVER_FLAGS_DEDICATED_P2P_DEVICE)) {
      wpa_dbg(wpa_s, MSG_DEBUG, "Removing P2P Device interface");
      wpa_supplicant_remove_iface(wpa_s->global, wpa_s->global->p2p_init_wpa_s,
                                  0);
...
static void wpa_supplicant_event_stkstart(struct wpa_supplicant *wpa_s,
                                          union wpa_event_data *data) {
  if (data == NULL)
    return;
...
static void wpa_supplicant_event_tdls(struct wpa_supplicant *wpa_s,
  ...
  case TDLS_REQUEST_TEARDOWN:
    if (wpa_tdls_is_external_setup(wpa_s->wpa))
      ...
      wpa_drv_tdls_oper(wpa_s, TDLS_TEARDOWN, data->tdls.peer);
    break;
  case TDLS_REQUEST_DISCOVER:
    wpa_tdls_send_discovery_request(wpa_s->wpa, data->tdls.peer);
    break;
  }
}
#endif /* CONFIG_TDLS */
...
static void wpa_supplicant_event_wnm(struct wpa_supplicant *wpa_s,
  ...
  case WNM_OPER_SLEEP:
    wpa_printf(MSG_DEBUG,
               "Start sending WNM-Sleep Request "
               "(action=%d, intval=%d)",
...
#ifdef CONFIG_IEEE80211R
static void wpa_supplicant_event_ft_response(struct wpa_supplicant *wpa_s,
                                             union wpa_event_data *data) {
  if (data == NULL)
    return;

  if (wpa_ft_process_response(wpa_s->wpa, data->ft_ies.ies,
                              data->ft_ies.ies_len, data->ft_ies.ft_action,
                              data->ft_ies.target_ap, data->ft_ies.ric_ies,
...
static void ft_rx_action(struct wpa_supplicant *wpa_s, const u8 *data,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "FT: Received FT Action Response: STA " MACSTR " TargetAP " MACSTR
          " status %u",
          MAC2STR(sta_addr), MAC2STR(target_ap_addr), status);

  if (os_memcmp(sta_addr, wpa_s->own_addr, ETH_ALEN) != 0) {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "FT: Foreign STA Address " MACSTR " in FT Action Response",
            MAC2STR(sta_addr));
    return;
  }
  ...
  if (wpa_ft_process_response(wpa_s->wpa, data + 1 + 2 * ETH_ALEN + 2,
                              len - (1 + 2 * ETH_ALEN + 2), 1, target_ap_addr,
                              NULL, 0) < 0)
    return;
...
static void wpa_supplicant_event_unprot_deauth(struct wpa_supplicant *wpa_s,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "dropped: " MACSTR " -> " MACSTR " (reason code %u)",
...
static void wpas_event_disconnect(struct wpa_supplicant *wpa_s, const u8 *addr,
  ...
  wpa_supplicant_event_disassoc_finish(wpa_s, reason_code, locally_generated);
}
...
static void wpas_event_disassoc(struct wpa_supplicant *wpa_s,
  ...
  if (info) {
    ...
    if (addr)
      wpa_dbg(wpa_s, MSG_DEBUG, " * address " MACSTR, MAC2STR(addr));
    wpa_hexdump(MSG_DEBUG, "Disassociation frame IE(s)", ie, ie_len);
  }
...
#ifdef CONFIG_P2P
  if (info) {
    wpas_p2p_disassoc_notif(wpa_s, info->addr, reason_code, info->ie,
                            info->ie_len, locally_generated);
  }
#endif /* CONFIG_P2P */
  ...
  wpas_event_disconnect(wpa_s, addr, reason_code, locally_generated, ie, ie_len,
                        0);
}
...
static void wpas_event_deauth(struct wpa_supplicant *wpa_s,
  ...
  if (info) {
    ...
    wpa_hexdump(MSG_DEBUG, "Deauthentication frame IE(s)", ie, ie_len);
  }
...
static void
wpa_supplicant_update_channel_list(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant,
                   radio_list) {
    wpa_printf(MSG_DEBUG, "%s: Updating hw mode", ifs->ifname);
    free_hw_features(ifs);
    ifs->hw.modes =
        wpa_drv_get_hw_feature_data(ifs, &ifs->hw.num_modes, &ifs->hw.flags);
    ...
    if (ifs->pno) {
      ...
      wpa_dbg(ifs, MSG_DEBUG, "Channel list changed - restart sched_scan");
...
static void wpas_event_rx_mgmt_action(struct wpa_supplicant *wpa_s,
  ...
  if ((mgmt->u.action.category == WLAN_ACTION_PUBLIC ||
      ...
      gas_query_rx(wpa_s->gas, mgmt->da, mgmt->sa, mgmt->bssid,
                   mgmt->u.action.category, payload, plen, freq) == 0)
    return;
#endif /* CONFIG_GAS */
  ...
  if (category == WLAN_ACTION_PUBLIC && plen >= 4 &&
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "TDLS: Received Discovery Response from " MACSTR,
            MAC2STR(mgmt->sa));
    return;
  }
#endif /* CONFIG_TDLS */
  ...
  if (category == WLAN_ACTION_QOS && plen >= 1 &&
    ...
    size_t qlen = plen - 1;
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Interworking: Received QoS Map Configure frame from " MACSTR,
            MAC2STR(mgmt->sa));
    if (os_memcmp(mgmt->sa, wpa_s->bssid, ETH_ALEN) == 0 && qlen > 2 &&
        pos[0] == WLAN_EID_QOS_MAP_SET && pos[1] <= qlen - 2 && pos[1] >= 16)
  ...
  if (category == WLAN_ACTION_RADIO_MEASUREMENT &&
    ...
    wpas_rrm_handle_radio_measurement_request(wpa_s, mgmt->sa, payload + 1,
                                              plen - 1);
    return;
  }
  ...
  if (category == WLAN_ACTION_RADIO_MEASUREMENT &&
    ...
    wpas_rrm_handle_link_measurement_request(wpa_s, mgmt->sa, payload + 1,
                                             plen - 1, rssi);
    return;
  }
  ...
  wpas_p2p_rx_action(wpa_s, mgmt->da, mgmt->sa, mgmt->bssid, category, payload,
                     plen, freq);
  if (wpa_s->ifmsh)
...
static void wpa_supplicant_notify_avoid_freq(struct wpa_supplicant *wpa_s,
  ...
  wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_AVOID_FREQ "ranges=%s", str ? str : "");

#ifdef CONFIG_P2P
  if (freq_range_list_parse(&wpa_s->global->p2p_go_avoid_freq, str)) {
    wpa_dbg(wpa_s, MSG_ERROR, "%s: Failed to parse freq range", __func__);
  } else {
    wpa_dbg(wpa_s, MSG_DEBUG,
            "P2P: Update channel list based on frequency avoid event");

    /*
     * The update channel flow will also take care of moving a GO
     * from the unsafe frequency if needed.
     */
    wpas_p2p_update_channel_list(wpa_s, WPAS_P2P_CHANNEL_UPDATE_AVOID);
  }
#endif /* CONFIG_P2P */
...
static void wpa_supplicant_event_assoc_auth(struct wpa_supplicant *wpa_s,
  ...
  wpa_sm_set_ptk_kck_kek(wpa_s->wpa, data->assoc_info.ptk_kck,
                         data->assoc_info.ptk_kck_len, data->assoc_info.ptk_kek,
...
void wpa_supplicant_event(void *ctx, enum wpa_event_type event,
  ...
  if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED &&
      event != EVENT_INTERFACE_ENABLED && event != EVENT_INTERFACE_STATUS &&
      event != EVENT_SCAN_RESULTS && event != EVENT_SCHED_SCAN_STOPPED) {
    wpa_dbg(wpa_s, MSG_DEBUG,
...
#ifndef CONFIG_NO_STDOUT_DEBUG
  {
    ...
    wpa_dbg(wpa_s, level, "Event %s (%d) received", event_to_string(event),
            event);
  }
  ...
  case EVENT_AUTH:
#ifdef CONFIG_FST
    if (!wpas_fst_update_mbie(wpa_s, data->auth.ies, data->auth.ies_len))
      wpa_printf(MSG_DEBUG, "FST: MB IEs updated from auth IE");
#endif /* CONFIG_FST */
  ...
  case EVENT_ASSOC:
    ...
    if (wpa_s->ignore_auth_resp) {
      wpa_printf(MSG_INFO, "EVENT_ASSOC - ignore_auth_resp active!");
      break;
    }
#endif /* CONFIG_TESTING_OPTIONS */
    ...
    if (data) {
      wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SUBNET_STATUS_UPDATE "status=%u",
...
#ifdef CONFIG_TESTING_OPTIONS
    if (wpa_s->ignore_auth_resp) {
      wpa_printf(MSG_INFO, "EVENT_DEAUTH - ignore_auth_resp active!");
      break;
    }
#endif /* CONFIG_TESTING_OPTIONS */
  ...
  case EVENT_SCAN_STARTED:
    if (wpa_s->own_scan_requested || (data && !data->scan_info.external_scan)) {
      ...
      wpa_dbg(wpa_s, MSG_DEBUG,
              "Own scan request started a scan in %ld.%06ld seconds", diff.sec,
              diff.usec);
  ...
  case EVENT_SCAN_RESULTS:
    ...
    if (!(data && data->scan_info.external_scan) &&
      ...
      wpa_dbg(wpa_s, MSG_DEBUG, "Scan completed in %ld.%06ld seconds", diff.sec,
              diff.usec);
    }
  ...
  case EVENT_ASSOC_REJECT:
    if (data->assoc_reject.bssid)
      wpa_msg(wpa_s, MSG_INFO,
              WPA_EVENT_ASSOC_REJECT "bssid=" MACSTR " status_code=%u%s",
              MAC2STR(data->assoc_reject.bssid), data->assoc_reject.status_code,
              data->assoc_reject.timed_out ? " timeout" : "");
    else
      wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_ASSOC_REJECT "status_code=%u%s",
  ...
  case EVENT_AUTH_TIMED_OUT:
    ...
    if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_MESH) {
      wpa_dbg(wpa_s, MSG_DEBUG, "Ignore AUTH_TIMED_OUT in mesh configuration");
      break;
    }
    if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME)
  ...
  case EVENT_ASSOC_TIMED_OUT:
    /* It is possible to get this event from earlier connection */
    if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_MESH) {
      wpa_dbg(wpa_s, MSG_DEBUG, "Ignore ASSOC_TIMED_OUT in mesh configuration");
      break;
    }
    if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME)
  ...
  case EVENT_TX_STATUS:
    wpa_dbg(wpa_s, MSG_DEBUG, "EVENT_TX_STATUS dst=" MACSTR " type=%d stype=%d",
    ...
    if (wpa_s->ap_iface == NULL) {
      ...
      if (data->tx_status.type == WLAN_FC_TYPE_MGMT &&
        ...
        offchannel_send_action_tx_status(
            wpa_s, data->tx_status.dst, data->tx_status.data,
            data->tx_status.data_len,
            data->tx_status.ack ? OFFCHANNEL_SEND_ACTION_SUCCESS
...
#endif /* CONFIG_AP */
#ifdef CONFIG_OFFCHANNEL
    wpa_dbg(wpa_s, MSG_DEBUG, "EVENT_TX_STATUS pending_dst=" MACSTR,
            MAC2STR(wpa_s->p2pdev->pending_action_dst));
    /*
    ...
    if (data->tx_status.type == WLAN_FC_TYPE_MGMT &&
        data->tx_status.stype == WLAN_FC_STYPE_ACTION &&
        os_memcmp(wpa_s->p2pdev->pending_action_dst, data->tx_status.dst,
                  ETH_ALEN) == 0) {
      offchannel_send_action_tx_status(
          wpa_s->p2pdev, data->tx_status.dst, data->tx_status.data,
          data->tx_status.data_len,
          data->tx_status.ack ? OFFCHANNEL_SEND_ACTION_SUCCESS
                              : OFFCHANNEL_SEND_ACTION_NO_ACK);
      break;
    ...
    case WLAN_FC_TYPE_MGMT:
      ap_mgmt_tx_cb(wpa_s, data->tx_status.data, data->tx_status.data_len,
                    data->tx_status.stype, data->tx_status.ack);
      break;
    case WLAN_FC_TYPE_DATA:
      ap_tx_status(wpa_s, data->tx_status.dst, data->tx_status.data,
                   data->tx_status.data_len, data->tx_status.ack);
      break;
    }
#endif /* CONFIG_AP */
    break;
  ...
  case EVENT_EAPOL_TX_STATUS:
    ap_eapol_tx_status(
        wpa_s, data->eapol_tx_status.dst, data->eapol_tx_status.data,
        data->eapol_tx_status.data_len, data->eapol_tx_status.ack);
    break;
  ...
  case EVENT_CH_SWITCH:
    ...
    wpa_msg(wpa_s, MSG_INFO,
            ...
            data->ch_switch.freq, data->ch_switch.ht_enabled,
    ...
    if (wpa_s->current_ssid->mode == WPAS_MODE_AP ||
      ...
      wpas_ap_ch_switch(wpa_s, data->ch_switch.freq, data->ch_switch.ht_enabled,
                        data->ch_switch.ch_offset, data->ch_switch.ch_width,
                        data->ch_switch.cf1, data->ch_switch.cf2);
    }
  ...
  case EVENT_DFS_NOP_FINISHED:
    if (data)
      wpas_event_dfs_cac_nop_finished(wpa_s, &data->dfs_event);
    break;
#endif /* NEED_AP_MLME */
#endif /* CONFIG_AP */
  case EVENT_RX_MGMT: {
    ...
    if (wpa_s->ext_mgmt_frame_handling) {
      ...
      if (hex) {
        wpa_snprintf_hex(hex, hex_len, rx->frame, rx->frame_len);
        wpa_msg(wpa_s, MSG_INFO, "MGMT-RX freq=%d datarate=%u ssi_signal=%d %s",
                rx->freq, rx->datarate, rx->ssi_signal, hex);
        os_free(hex);
      }
      break;
    }
    ...
    if (wpa_s->ap_iface == NULL) {
      ...
      if (stype == WLAN_FC_STYPE_PROBE_REQ &&
        ...
        wpas_p2p_probe_req_rx(wpa_s, src, mgmt->da, mgmt->bssid, ie, ie_len,
                              data->rx_mgmt.freq, data->rx_mgmt.ssi_signal);
        break;
      }
#endif /* CONFIG_P2P */
#ifdef CONFIG_IBSS_RSN
      if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_IBSS &&
          stype == WLAN_FC_STYPE_AUTH && data->rx_mgmt.frame_len >= 30) {
      ...
      if (stype == WLAN_FC_STYPE_ACTION) {
        wpas_event_rx_mgmt_action(wpa_s, data->rx_mgmt.frame,
                                  data->rx_mgmt.frame_len, data->rx_mgmt.freq,
    ...
    if (stype == WLAN_FC_STYPE_PROBE_REQ &&
      ...
      wpas_notify_preq(wpa_s, mgmt->sa, mgmt->da, mgmt->bssid, ie, ie_len,
                       data->rx_mgmt.ssi_signal);
    }
  ...
  case EVENT_RX_PROBE_REQ:
    if (data->rx_probe_req.sa == NULL || data->rx_probe_req.ie == NULL)
      break;
#ifdef CONFIG_AP
    if (wpa_s->ap_iface) {
      hostapd_probe_req_rx(wpa_s->ap_iface->bss[0], data->rx_probe_req.sa,
                           data->rx_probe_req.da, data->rx_probe_req.bssid,
                           data->rx_probe_req.ie, data->rx_probe_req.ie_len,
                           data->rx_probe_req.ssi_signal);
      break;
    }
#endif /* CONFIG_AP */
    wpas_p2p_probe_req_rx(wpa_s, data->rx_probe_req.sa, data->rx_probe_req.da,
                          data->rx_probe_req.bssid, data->rx_probe_req.ie,
                          data->rx_probe_req.ie_len, 0,
                          data->rx_probe_req.ssi_signal);
    break;
  case EVENT_REMAIN_ON_CHANNEL:
#ifdef CONFIG_OFFCHANNEL
    offchannel_remain_on_channel_cb(wpa_s, data->remain_on_channel.freq,
                                    data->remain_on_channel.duration);
#endif /* CONFIG_OFFCHANNEL */
    wpas_p2p_remain_on_channel_cb(wpa_s, data->remain_on_channel.freq,
                                  data->remain_on_channel.duration);
    break;
  case EVENT_CANCEL_REMAIN_ON_CHANNEL:
#ifdef CONFIG_OFFCHANNEL
    offchannel_cancel_remain_on_channel_cb(wpa_s, data->remain_on_channel.freq);
#endif /* CONFIG_OFFCHANNEL */
    wpas_p2p_cancel_remain_on_channel_cb(wpa_s, data->remain_on_channel.freq);
    break;
  case EVENT_EAPOL_RX:
    wpa_supplicant_rx_eapol(wpa_s, data->eapol_rx.src, data->eapol_rx.data,
                            data->eapol_rx.data_len);
    break;
  case EVENT_SIGNAL_CHANGE:
    wpa_msg(
        wpa_s, MSG_INFO,
        WPA_EVENT_SIGNAL_CHANGE "above=%d signal=%d noise=%d txrate=%d",
        data->signal_change.above_threshold, data->signal_change.current_signal,
        data->signal_change.current_noise, data->signal_change.current_txrate);
    wpa_bss_update_level(wpa_s->current_bss,
                         data->signal_change.current_signal);
    bgscan_notify_signal_change(wpa_s, data->signal_change.above_threshold,
                                data->signal_change.current_signal,
  ...
  case EVENT_INTERFACE_DISABLED:
    ...
    if (wpa_s->wpa_state >= WPA_AUTHENTICATING) {
       ...
       * consistent.
       */
      wpa_supplicant_event_disassoc(wpa_s, WLAN_REASON_DEAUTH_LEAVING, 1);
    }
    wpa_supplicant_mark_disassoc(wpa_s);
    wpa_bss_flush(wpa_s);
  ...
  case EVENT_CHANNEL_LIST_CHANGED:
    wpa_supplicant_update_channel_list(wpa_s, &data->channel_list_changed);
    break;
  ...
  case EVENT_UNPROT_DEAUTH:
    wpa_supplicant_event_unprot_deauth(wpa_s, &data->unprot_deauth);
    break;
  case EVENT_UNPROT_DISASSOC:
    wpa_supplicant_event_unprot_disassoc(wpa_s, &data->unprot_disassoc);
    break;
  case EVENT_STATION_LOW_ACK:
#ifdef CONFIG_AP
    if (wpa_s->ap_iface && data)
      hostapd_event_sta_low_ack(wpa_s->ap_iface->bss[0], data->low_ack.addr);
#endif /* CONFIG_AP */
#ifdef CONFIG_TDLS
    if (data)
      wpa_tdls_disable_unreachable_link(wpa_s->wpa, data->low_ack.addr);
#endif /* CONFIG_TDLS */
    break;
  ...
  case EVENT_DRIVER_GTK_REKEY:
    if (os_memcmp(data->driver_gtk_rekey.bssid, wpa_s->bssid, ETH_ALEN))
      break;
    if (!wpa_s->wpa)
      break;
    wpa_sm_update_replay_ctr(wpa_s->wpa, data->driver_gtk_rekey.replay_ctr);
    break;
  case EVENT_SCHED_SCAN_STOPPED:
     ...
     */
    if (!wpa_s->sched_scan_stop_req && wpa_s->wpa_state == WPA_SCANNING) {
      wpa_dbg(wpa_s, MSG_DEBUG,
  ...
  case EVENT_CONNECT_FAILED_REASON:
    ...
    if (!wpa_s->ap_iface || !data)
      break;
    hostapd_event_connect_failed_reason(wpa_s->ap_iface->bss[0],
  ...
  case EVENT_NEW_PEER_CANDIDATE:
    ...
    if (!wpa_s->ifmsh || !data)
      break;
    wpa_mesh_notify_peer(wpa_s, data->mesh_peer.peer, data->mesh_peer.ies,
  ...
  case EVENT_SURVEY:
    ...
    if (!wpa_s->ap_iface)
      break;
    hostapd_event_get_survey(wpa_s->ap_iface, &data->survey_results);
#endif /* CONFIG_AP */
    break;
  ...
  case EVENT_P2P_LO_STOP:
    ...
    wpa_msg(wpa_s, MSG_INFO,
            P2P_EVENT_LISTEN_OFFLOAD_STOP P2P_LISTEN_OFFLOAD_STOP_REASON
            "reason=%d",
...
void wpa_supplicant_event_global(void *ctx, enum wpa_event_type event,
  ...
  if (wpa_s && wpa_s->driver->get_ifindex) {
    ...
    if (ifindex != data->interface_status.ifindex) {
      wpa_dbg(wpa_s, MSG_DEBUG, "interface status ifindex %d mismatch (%d)",
    ...
    wpa_i = wpa_supplicant_match_iface(ctx, data->interface_status.ifname);
    if (!wpa_i)
      return;
