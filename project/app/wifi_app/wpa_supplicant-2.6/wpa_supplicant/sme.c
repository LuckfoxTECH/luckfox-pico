...
static struct wpabuf *sme_auth_build_sae_commit(struct wpa_supplicant *wpa_s,
  ...
  if (sae_prepare_commit(wpa_s->own_addr, bssid, (u8 *)ssid->passphrase,
                         os_strlen(ssid->passphrase), &wpa_s->sme.sae) < 0) {
...
static void sme_auth_handle_rrm(struct wpa_supplicant *wpa_s,
  ...
  if (!((wpa_s->drv_rrm_flags & WPA_DRIVER_FLAGS_DS_PARAM_SET_IE_IN_PROBES) &&
...
static void sme_send_authentication(struct wpa_supplicant *wpa_s,
  ...
  if ((wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
    ...
    if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid,
                                wpa_s->current_ssid, try_opportunistic) == 0)
      eapol_sm_notify_pmkid_attempt(wpa_s->eapol);
    wpa_s->sme.assoc_req_ie_len = sizeof(wpa_s->sme.assoc_req_ie);
    if (wpa_supplicant_set_suites(wpa_s, bss, ssid, wpa_s->sme.assoc_req_ie,
    ...
    wpa_s->sme.assoc_req_ie_len = sizeof(wpa_s->sme.assoc_req_ie);
    if (wpa_supplicant_set_suites(wpa_s, NULL, ssid, wpa_s->sme.assoc_req_ie,
                                  &wpa_s->sme.assoc_req_ie_len)) {
      wpa_msg(wpa_s, MSG_WARNING,
              "SME: Failed to set WPA "
  ...
  if (mbo) {
    ...
    len = wpas_mbo_supp_op_class_ie(
        wpa_s, bss->freq, wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len,
  ...
  if (ext_capab_len > 0) {
    ...
    os_memmove(pos + ext_capab_len, pos,
               wpa_s->sme.assoc_req_ie_len - (pos - wpa_s->sme.assoc_req_ie));
  ...
  if (mbo) {
    ...
    len = wpas_mbo_ie(
        wpa_s, wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len,
        sizeof(wpa_s->sme.assoc_req_ie) - wpa_s->sme.assoc_req_ie_len);
    if (len >= 0)
      wpa_s->sme.assoc_req_ie_len += len;
  }
  ...
  if (!skip_auth && params.auth_alg == WPA_AUTH_ALG_SAE &&
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "PMKSA cache entry found - try to use PMKSA caching instead of new "
            "SAE authentication");
    params.auth_alg = WPA_AUTH_ALG_OPEN;
  ...
  wpa_msg(wpa_s, MSG_INFO,
          "SME: Trying to authenticate with " MACSTR " (SSID='%s' freq=%d MHz)",
          MAC2STR(params.bssid), wpa_ssid_txt(params.ssid, params.ssid_len),
          params.freq);
  ...
  if (wpa_s->num_multichan_concurrent < 2) {
    ...
    if (num > 0 && freq > 0 && freq != params.freq) {
      wpa_printf(MSG_DEBUG, "Conflicting frequency found (%d != %d)", freq,
                 params.freq);
...
static void sme_auth_start_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (cwork->bss_removed ||
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "SME: BSS/SSID entry for authentication not valid anymore - drop "
            "connection attempt");
    wpas_connect_work_done(wpa_s);
    return;
  }
...
void sme_authenticate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
                      struct wpa_ssid *ssid) {
  ...
  if (radio_add_work(wpa_s, bss->freq, "sme-connect", 1, sme_auth_start_cb,
                     cwork) < 0)
    wpas_connect_work_free(cwork);
}
...
static int sme_sae_auth(struct wpa_supplicant *wpa_s, u16 auth_transaction,
  ...
  if (auth_transaction == 1 &&
      ...
      wpa_s->sme.sae.state == SAE_COMMITTED && wpa_s->current_bss &&
      wpa_s->current_ssid) {
    int default_groups[] = {19, 20, 21, 25, 26, 0};
    u16 group;
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "SME: SAE anti-clogging token requested (group %u)", group);
    if (sae_group_allowed(&wpa_s->sme.sae, groups, group) !=
        WLAN_STATUS_SUCCESS) {
      wpa_dbg(wpa_s, MSG_ERROR,
              "SME: SAE group %u of anti-clogging request is invalid", group);
      return -1;
    }
    wpabuf_free(wpa_s->sme.sae_token);
    wpa_s->sme.sae_token =
        wpabuf_alloc_copy(data + sizeof(le16), len - sizeof(le16));
...
void sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data) {
  ...
  wpa_hexdump(MSG_MSGDUMP, "SME: Authentication response IEs", data->auth.ies,
              data->auth.ies_len);
  ...
  if (data->auth.auth_type == WLAN_AUTH_SAE) {
    int res;
    res =
        sme_sae_auth(wpa_s, data->auth.auth_transaction, data->auth.status_code,
  ...
  if (data->auth.status_code != WLAN_STATUS_SUCCESS) {
    ...
    if (data->auth.ies && data->auth.ies_len) {
      ...
      if (ie_txt) {
        wpa_snprintf_hex(ie_txt, buflen, data->auth.ies, data->auth.ies_len);
      }
    }
    wpa_msg(wpa_s, MSG_INFO,
            ...
            MAC2STR(data->auth.peer), data->auth.auth_type,
            data->auth.auth_transaction, data->auth.status_code, ie_txt);
    os_free(ie_txt);
  ...
  if (data->auth.auth_type == WLAN_AUTH_FT) {
    if (wpa_ft_process_response(wpa_s->wpa, data->auth.ies, data->auth.ies_len,
                                0, data->auth.peer, NULL, 0) < 0) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "SME: FT Authentication response processing failed");
      wpa_msg(wpa_s, MSG_INFO,
              WPA_EVENT_DISCONNECTED "bssid=" MACSTR
                                     " reason=%d locally_generated=1",
              MAC2STR(wpa_s->pending_bssid), WLAN_REASON_DEAUTH_LEAVING);
      wpas_connection_failed(wpa_s, wpa_s->pending_bssid);
      wpa_supplicant_mark_disassoc(wpa_s);
      return;
    }
...
void sme_associate(struct wpa_supplicant *wpa_s, enum wpas_mode mode,
  ...
  params.bg_scan_period =
      wpa_s->current_ssid ? wpa_s->current_ssid->bg_scan_period : -1;
  ...
  if (params.wpa_ie == NULL ||
      ieee802_11_parse_elems(params.wpa_ie, params.wpa_ie_len, &elems, 0) < 0) {
...
int sme_update_ft_ies(struct wpa_supplicant *wpa_s, const u8 *md, const u8 *ies,
                      size_t ies_len) {
  if (md == NULL || ies == NULL) {
...
void sme_event_assoc_reject(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "SME: Association with " MACSTR " failed: "
          "status code %d",
          MAC2STR(wpa_s->pending_bssid), data->assoc_reject.status_code);
  ...
  if (wpa_s->sme.sae_pmksa_caching && wpa_s->current_ssid &&
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "PMKSA caching attempt rejected - drop PMKSA cache entry and fall "
            "back to SAE authentication");
...
static void sme_send_2040_bss_coex(struct wpa_supplicant *wpa_s,
  ...
  wpa_hexdump(MSG_DEBUG, "SME: 20/40 BSS Intolerant Channels", chan_list,
              num_channels);
...
int sme_proc_obss_scan(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
    ...
    wpa_printf(MSG_DEBUG,
               "SME OBSS scan BSS " MACSTR " freq=%u chan=%u ht_cap=0x%x",
...
static void wpa_obss_scan_freqs_list(struct wpa_supplicant *wpa_s,
  ...
  mode =
      get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, HOSTAPD_MODE_IEEE80211G);
  if (mode == NULL) {
  ...
  wpa_printf(MSG_DEBUG, "OBSS: assoc_freq %d possible affected range %d-%d",
...
static void sme_obss_scan_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  eloop_register_timeout(wpa_s->sme.obss_scan_int, 0, sme_obss_scan_timeout,
                         wpa_s, NULL);
}
...
static void sme_send_sa_query_req(struct wpa_supplicant *wpa_s,
  ...
  wpa_dbg(wpa_s, MSG_DEBUG, "SME: Sending SA Query Request to " MACSTR,
          MAC2STR(wpa_s->bssid));
  wpa_hexdump(MSG_DEBUG, "SME: SA Query Transaction ID", trans_id,
              WLAN_SA_QUERY_TR_ID_LEN);
  req[0] = WLAN_ACTION_SA_QUERY;
  req[1] = WLAN_SA_QUERY_REQUEST;
  os_memcpy(req + 2, trans_id, WLAN_SA_QUERY_TR_ID_LEN);
  if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                          wpa_s->own_addr, wpa_s->bssid, req, sizeof(req),
                          0) < 0)
    wpa_msg(wpa_s, MSG_INFO,
...
void sme_sa_query_rx(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *data,
                     size_t len) {
  int i;

  if (wpa_s->sme.sa_query_trans_id == NULL ||
      len < 1 + WLAN_SA_QUERY_TR_ID_LEN || data[0] != WLAN_SA_QUERY_RESPONSE)
    return;
  wpa_dbg(wpa_s, MSG_DEBUG,
          "SME: Received SA Query response from " MACSTR " (trans_id %02x%02x)",
  ...
  for (i = 0; i < wpa_s->sme.sa_query_count; i++) {
    if (os_memcmp(wpa_s->sme.sa_query_trans_id + i * WLAN_SA_QUERY_TR_ID_LEN,
