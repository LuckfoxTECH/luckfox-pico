...
static int set_disallow_aps(struct wpa_supplicant *wpa_s, char *val) {
  ...
  while (pos) {
    ...
    if (os_strncmp(pos, "bssid ", 6) == 0) {
      ...
      ns =
          os_realloc_array(ssid, ssid_count + 1, sizeof(struct wpa_ssid_value));
      if (ns == NULL) {
        os_free(ssid);
        os_free(bssid);
        return -1;
      ...
      if ((end - pos) & 0x01 || end - pos > 2 * SSID_MAX_LEN ||
          hexstr2bin(pos, ssid[ssid_count].ssid, (end - pos) / 2) < 0) {
        os_free(ssid);
        os_free(bssid);
        wpa_printf(MSG_DEBUG,
                   "Invalid disallow_aps "
      ...
      wpa_hexdump_ascii(MSG_DEBUG, "disallow_aps SSID", ssid[ssid_count].ssid,
...
static int wpa_supplicant_ctrl_iface_set(struct wpa_supplicant *wpa_s,
  ...
  *value++ = '\0';
  ...
  if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
    eapol_sm_configure(wpa_s->eapol, atoi(value), -1, -1, -1);
  } else if (os_strcasecmp(cmd, "EAPOL::authPeriod") == 0) {
    eapol_sm_configure(wpa_s->eapol, -1, atoi(value), -1, -1);
  } else if (os_strcasecmp(cmd, "EAPOL::startPeriod") == 0) {
    eapol_sm_configure(wpa_s->eapol, -1, -1, atoi(value), -1);
  } else if (os_strcasecmp(cmd, "EAPOL::maxStart") == 0) {
    eapol_sm_configure(wpa_s->eapol, -1, -1, -1, atoi(value));
  } else if (os_strcasecmp(cmd, "dot11RSNAConfigPMKLifetime") == 0) {
    if (wpa_sm_set_param(wpa_s->wpa, RSNA_PMK_LIFETIME, atoi(value)))
      ret = -1;
  } else if (os_strcasecmp(cmd, "dot11RSNAConfigPMKReauthThreshold") == 0) {
    if (wpa_sm_set_param(wpa_s->wpa, RSNA_PMK_REAUTH_THRESHOLD, atoi(value)))
      ret = -1;
...
#ifdef CONFIG_AP
    if (wpa_s->ap_iface) {
      wpa_s->ap_iface->bss[0]->ext_eapol_frame_io = wpa_s->ext_eapol_frame_io;
    }
#endif /* CONFIG_AP */
...
static int wpa_supplicant_ctrl_iface_get(struct wpa_supplicant *wpa_s,
  ...
  if (os_strcmp(cmd, "version") == 0) {
    ...
    if (wpa_s->conf->country[0] && wpa_s->conf->country[1])
      res = os_snprintf(buf, buflen, "%c%c", wpa_s->conf->country[0],
    ...
    if (wpa_s->last_gtk_len == 0)
      return -1;
    res = wpa_snprintf_hex(buf, buflen, wpa_s->last_gtk, wpa_s->last_gtk_len);
    return res;
...
static int wpa_supplicant_ctrl_iface_stkstart(struct wpa_supplicant *wpa_s,
                                              char *addr) {
  u8 peer[ETH_ALEN];
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE STKSTART " MACSTR, MAC2STR(peer));
...
static int wpa_supplicant_ctrl_iface_tdls_discover(struct wpa_supplicant *wpa_s,
                                                   char *addr) {
  u8 peer[ETH_ALEN];
  int ret;
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_DISCOVER " MACSTR, MAC2STR(peer));
...
static int wpa_supplicant_ctrl_iface_tdls_setup(struct wpa_supplicant *wpa_s,
                                                char *addr) {
  u8 peer[ETH_ALEN];
  int ret;
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_SETUP " MACSTR, MAC2STR(peer));
...
static int wpa_supplicant_ctrl_iface_tdls_teardown(struct wpa_supplicant *wpa_s,
                                                   char *addr) {
  u8 peer[ETH_ALEN];
  int ret;
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_TEARDOWN " MACSTR, MAC2STR(peer));
...
static int ctrl_iface_get_capability_tdls(struct wpa_supplicant *wpa_s,
                                          char *buf, size_t buflen) {
  int ret;

  ret =
      os_snprintf(buf, buflen, "%s\n",
...
static int
wpa_supplicant_ctrl_iface_tdls_chan_switch(struct wpa_supplicant *wpa_s,
                                           char *cmd) {
  u8 peer[ETH_ALEN];
  ...
  if (!wpa_tdls_is_external_setup(wpa_s->wpa)) {
    wpa_printf(MSG_INFO, "tdls_chanswitch: Only supported with external setup");
    return -1;
  }
  ...
  *pos++ = '\0';
  ...
  if (pos == end) {
    wpa_printf(MSG_INFO, "tdls_chanswitch: Invalid op class provided");
    return -1;
  }
...
#undef SET_FREQ_SETTING
  ...
  if (hwaddr_aton(cmd, peer)) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_CHAN_SWITCH: Invalid address '%s'",
               cmd);
    return -1;
  }
  ...
  wpa_printf(
      MSG_DEBUG,
      "CTRL_IFACE TDLS_CHAN_SWITCH " MACSTR
      " OP CLASS %d FREQ %d CENTER1 %d CENTER2 %d BW %d SEC_OFFSET %d%s%s",
      MAC2STR(peer), oper_class, freq_params.freq, freq_params.center_freq1,
      freq_params.center_freq2, freq_params.bandwidth,
      freq_params.sec_channel_offset, freq_params.ht_enabled ? " HT" : "",
...
static int
wpa_supplicant_ctrl_iface_tdls_cancel_chan_switch(struct wpa_supplicant *wpa_s,
                                                  char *cmd) {
  u8 peer[ETH_ALEN];
  ...
  if (hwaddr_aton(cmd, peer)) {
    wpa_printf(MSG_DEBUG,
               "CTRL_IFACE TDLS_CANCEL_CHAN_SWITCH: Invalid address '%s'", cmd);
    return -1;
  }
...
static int wpa_supplicant_ctrl_iface_tdls_link_status(
    struct wpa_supplicant *wpa_s, const char *addr, char *buf, size_t buflen) {
  u8 peer[ETH_ALEN];
  const char *tdls_status;
  int ret;

  if (hwaddr_aton(addr, peer)) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_LINK_STATUS: Invalid address '%s'",
               addr);
    return -1;
  }
  wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_LINK_STATUS " MACSTR, MAC2STR(peer));
...
static int wmm_ac_ctrl_addts(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  while ((token = str_token(cmd, " ", &context))) {
    if (sscanf(token, "tsid=%i", &params.tsid) == 1 ||
        ...
        sscanf(token, "nominal_msdu_size=%i", &params.nominal_msdu_size) == 1 ||
        sscanf(token, "mean_data_rate=%i", &params.mean_data_rate) == 1 ||
        sscanf(token, "min_phy_rate=%i", &params.minimum_phy_rate) == 1 ||
        sscanf(token, "sba=%i", &params.surplus_bandwidth_allowance) == 1)
      continue;
    ...
    if (os_strcasecmp(token, "downlink") == 0) {
      ...
      wpa_printf(MSG_DEBUG, "CTRL: Invalid WMM_AC_ADDTS parameter: '%s'",
                 token);
      return -1;
    }
  }
...
static int wpa_supplicant_ctrl_iface_ft_ds(struct wpa_supplicant *wpa_s,
                                           char *addr) {
...
static int wpa_supplicant_ctrl_iface_wps_pbc(struct wpa_supplicant *wpa_s,
  ...
  if (cmd == NULL || os_strcmp(cmd, "any") == 0) {
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE WPS_PBC: invalid BSSID '%s'", cmd);
    return -1;
  }
...
static int wpa_supplicant_ctrl_iface_wps_pin(struct wpa_supplicant *wpa_s,
  ...
  if (os_strcmp(cmd, "any") == 0)
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE WPS_PIN: invalid BSSID '%s'", cmd);
    return -1;
  }
  ...
  if (wpa_s->ap_iface) {
    ...
    return wpa_supplicant_ap_wps_pin(wpa_s, _bssid, pin, buf, buflen, timeout);
  }
#endif /* CONFIG_AP */
...
static int wpa_supplicant_ctrl_iface_wps_check_pin(struct wpa_supplicant *wpa_s,
                                                   char *cmd, char *buf,
                                                   size_t buflen) {
  char pin[9];
  size_t len;
...
static int wpa_supplicant_ctrl_iface_wps_nfc(struct wpa_supplicant *wpa_s,
  ...
  return wpas_wps_start_nfc(wpa_s, NULL, _bssid, NULL, 0, 0, NULL, NULL, 0, 0);
}
...
static int wpa_supplicant_ctrl_iface_wps_nfc_token(struct wpa_supplicant *wpa_s,
                                                   char *cmd, char *reply,
                                                   size_t max_len) {
  int ndef;
  struct wpabuf *buf;
...
static int
wpa_supplicant_ctrl_iface_wps_nfc_tag_read(struct wpa_supplicant *wpa_s,
                                           char *pos) {
  size_t len;
  struct wpabuf *buf;
  int ret;
...
static int wpas_ctrl_nfc_get_handover_req(struct wpa_supplicant *wpa_s,
  ...
  *pos++ = '\0';
  ...
  if (os_strcmp(pos, "WPS") == 0 || os_strcmp(pos, "WPS-CR") == 0) {
    if (!ndef)
      return -1;
    return wpas_ctrl_nfc_get_handover_req_wps(wpa_s, reply, max_len, ndef);
  }
  ...
  *pos++ = '\0';
  ...
  if (os_strcmp(pos, "WPS") == 0 || os_strcmp(pos, "WPS-CR") == 0) {
    ...
    return wpas_ctrl_nfc_get_handover_sel_wps(
        wpa_s, reply, max_len, ndef, os_strcmp(pos, "WPS-CR") == 0, pos2);
  }

#ifdef CONFIG_P2P
  if (os_strcmp(pos, "P2P-CR") == 0) {
    return wpas_ctrl_nfc_get_handover_sel_p2p(wpa_s, reply, max_len, ndef, 0);
  }

  if (os_strcmp(pos, "P2P-CR-TAG") == 0) {
    return wpas_ctrl_nfc_get_handover_sel_p2p(wpa_s, reply, max_len, ndef, 1);
  }
#endif /* CONFIG_P2P */
...
static int wpas_ctrl_nfc_report_handover(struct wpa_supplicant *wpa_s,
  ...
  *pos2++ = '\0';
  ...
  if (len & 0x01) {
    wpa_printf(MSG_DEBUG,
               "NFC: Invalid request message length in handover report");
    return -1;
  }
  len /= 2;
  ...
  if (hexstr2bin(pos, wpabuf_put(req, len), len) < 0) {
    wpa_printf(MSG_DEBUG,
               "NFC: Invalid request message hexdump in handover report");
    wpabuf_free(req);
    return -1;
  }
  ...
  if (len & 0x01) {
    wpa_printf(MSG_DEBUG,
               "NFC: Invalid select message length in handover report");
    wpabuf_free(req);
    return -1;
  }
  len /= 2;
  ...
  if (hexstr2bin(pos2, wpabuf_put(sel, len), len) < 0) {
    wpa_printf(MSG_DEBUG,
               "NFC: Invalid select message hexdump in handover report");
    wpabuf_free(req);
    wpabuf_free(sel);
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "NFC: Connection handover reported - role=%s type=%s req_len=%d "
             "sel_len=%d",
...
static int wpa_supplicant_ctrl_iface_wps_reg(struct wpa_supplicant *wpa_s,
  ...
  *pin++ = '\0';
  ...
  if (hwaddr_aton(cmd, bssid)) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE WPS_REG: invalid BSSID '%s'", cmd);
    return -1;
  }
...
static int
wpa_supplicant_ctrl_iface_wps_er_set_config(struct wpa_supplicant *wpa_s,
                                            char *cmd) {
  char *uuid = cmd, *id;
  id = os_strchr(uuid, ' ');
  if (id == NULL)
...
static int wpa_supplicant_ctrl_iface_wps_er_config(struct wpa_supplicant *wpa_s,
                                                   char *cmd) {
  char *pin;
  char *new_ssid;
  char *new_auth;
...
static int wpa_supplicant_ctrl_iface_ibss_rsn(struct wpa_supplicant *wpa_s,
                                              char *addr) {
  u8 peer[ETH_ALEN];
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE IBSS_RSN " MACSTR, MAC2STR(peer));
...
static int wpa_supplicant_ctrl_iface_ctrl_rsp(struct wpa_supplicant *wpa_s,
  ...
  *pos++ = '\0';
  ...
  wpa_hexdump_ascii_key(MSG_DEBUG, "CTRL_IFACE: value", (u8 *)pos,
                        os_strlen(pos));
  ...
  return wpa_supplicant_ctrl_iface_ctrl_rsp_handle(wpa_s, ssid, rsp, pos);
...
static int wpa_supplicant_ctrl_iface_status(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->current_bss &&
      (hs20 = wpa_bss_get_vendor_ie(wpa_s->current_bss, HS20_IE_VENDOR_TYPE)) &&
  ...
  if (wpa_s->current_ssid) {
    ...
    for (cred = wpa_s->conf->cred; cred; cred = cred->next) {
      ...
      if (cred->provisioning_sp) {
        ret = os_snprintf(pos, end - pos, "provisioning_sp=%s\n",
      ...
      if (wpa_s->current_bss && wpa_s->current_bss->anqp) {
        struct wpabuf *names = wpa_s->current_bss->anqp->domain_name;
        for (i = 0; names && i < cred->num_domain; i++) {
          if (domain_name_list_contains(names, cred->domain[i], 1))
            break;
        }
    ...
    no_domain:
      if (wpa_s->current_bss == NULL || wpa_s->current_bss->anqp == NULL)
        res = -1;
      else
        res = interworking_home_sp_cred(wpa_s, cred,
  ...
  if (os_strcmp(params, "-NO_EVENTS")) {
    wpa_msg_ctrl(wpa_s, MSG_INFO,
                 ...
                 wpa_s->wpa_state, MAC2STR(wpa_s->bssid),
    ...
    if (wpa_s->wpa_state == WPA_COMPLETED) {
      struct wpa_ssid *ssid = wpa_s->current_ssid;
      wpa_msg_ctrl(wpa_s, MSG_INFO,
                   WPA_EVENT_CONNECTED "- connection to " MACSTR
                                       " completed %s [id=%d id_str=%s]",
                   MAC2STR(wpa_s->bssid), "(auth)", ssid ? ssid->id : -1,
...
static int wpa_supplicant_ctrl_iface_list_networks(struct wpa_supplicant *wpa_s,
  ...
  while (ssid) {
    prev = pos;
    ret = os_snprintf(pos, end - pos, "%d\t%s", ssid->id,
...
static char *wpa_supplicant_ie_txt(char *pos, char *end, const char *proto,
  ...
  if (data.key_mgmt & WPA_KEY_MGMT_IEEE8021X) {
    ret = os_snprintf(pos, end - pos, "%sEAP", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_PSK) {
    ret = os_snprintf(pos, end - pos, "%sPSK", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_WPA_NONE) {
    ret = os_snprintf(pos, end - pos, "%sNone", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_SAE) {
    ret = os_snprintf(pos, end - pos, "%sSAE", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  ...
  if (data.key_mgmt & WPA_KEY_MGMT_FT_IEEE8021X) {
    ret = os_snprintf(pos, end - pos, "%sFT/EAP", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_FT_PSK) {
    ret = os_snprintf(pos, end - pos, "%sFT/PSK", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_FT_SAE) {
    ret = os_snprintf(pos, end - pos, "%sFT/SAE", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  ...
  if (data.key_mgmt & WPA_KEY_MGMT_IEEE8021X_SHA256) {
    ret = os_snprintf(pos, end - pos, "%sEAP-SHA256", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  if (data.key_mgmt & WPA_KEY_MGMT_PSK_SHA256) {
    ret = os_snprintf(pos, end - pos, "%sPSK-SHA256", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  ...
  if (data.key_mgmt & WPA_KEY_MGMT_IEEE8021X_SUITE_B) {
    ret = os_snprintf(pos, end - pos, "%sEAP-SUITE-B", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
  ...
  if (data.key_mgmt & WPA_KEY_MGMT_OSEN) {
    ret = os_snprintf(pos, end - pos, "%sOSEN", pos == start ? "" : "+");
    if (os_snprintf_error(end - pos, ret))
      return pos;
    pos += ret;
  }
...
static char *wpa_supplicant_wps_ie_txt(struct wpa_supplicant *wpa_s, char *pos,
                                       char *end, const struct wpa_bss *bss) {
#ifdef CONFIG_WPS
  struct wpabuf *wps_ie;
...
static int wpa_supplicant_ctrl_iface_scan_result(struct wpa_supplicant *wpa_s,
  ...
  ret = os_snprintf(pos, end - pos, MACSTR "\t%d\t%d\t", MAC2STR(bss->bssid),
                    bss->freq, bss->level);
  if (os_snprintf_error(end - pos, ret))
    return -1;
  pos += ret;
  ...
  if (ie2) {
    pos =
        wpa_supplicant_ie_txt(pos, end, mesh ? "RSN" : "WPA2", ie2, 2 + ie2[1]);
  }
...
static int wpa_supplicant_ctrl_iface_scan_results(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
    ret = wpa_supplicant_ctrl_iface_scan_result(wpa_s, bss, pos, end - pos);
...
static int
wpa_supplicant_ctrl_iface_mesh_group_add(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
  int id;
  struct wpa_ssid *ssid;
  ...
  ssid = wpa_config_get_network(wpa_s->conf, id);
  if (ssid == NULL) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: Could not find network id=%d", id);
    return -1;
  }
...
static int
wpa_supplicant_ctrl_iface_mesh_group_remove(struct wpa_supplicant *wpa_s,
                                            char *cmd) {
  ...
  if (!found) {
    wpa_printf(MSG_ERROR, "CTRL_IFACE: MESH_GROUP_REMOVE ifname=%s not found",
               cmd);
    return -1;
  }
  if (wpa_s->mesh_if_created && wpa_s == orig) {
    wpa_printf(MSG_ERROR, "CTRL_IFACE: MESH_GROUP_REMOVE can't remove itself");
    return -1;
  }
...
static int
wpa_supplicant_ctrl_iface_mesh_peer_remove(struct wpa_supplicant *wpa_s,
                                           char *cmd) {
  u8 addr[ETH_ALEN];
...
static int wpa_supplicant_ctrl_iface_mesh_peer_add(struct wpa_supplicant *wpa_s,
                                                   char *cmd) {
  u8 addr[ETH_ALEN];
  int duration;
  char *pos;
...
static int
wpa_supplicant_ctrl_iface_select_network(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
  int id;
  struct wpa_ssid *ssid;
  char *pos;
...
static int
wpa_supplicant_ctrl_iface_enable_network(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
  int id;
  struct wpa_ssid *ssid;
...
static int
wpa_supplicant_ctrl_iface_disable_network(struct wpa_supplicant *wpa_s,
                                          char *cmd) {
  int id;
  struct wpa_ssid *ssid;
...
static int
wpa_supplicant_ctrl_iface_remove_network(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
  int id;
  struct wpa_ssid *ssid;
  int result;
...
static int
wpa_supplicant_ctrl_iface_update_network(struct wpa_supplicant *wpa_s,
                                         struct wpa_ssid *ssid, char *name,
                                         char *value) {
  int ret;
  ...
  if (os_strcmp(name, "bssid") != 0 && os_strcmp(name, "priority") != 0) {
...
static int wpa_supplicant_ctrl_iface_set_network(struct wpa_supplicant *wpa_s,
  ...
  *value++ = '\0';
  ...
  wpa_hexdump_ascii_key(MSG_DEBUG, "CTRL_IFACE: value", (u8 *)value,
                        os_strlen(value));
  ...
  ret = wpa_supplicant_ctrl_iface_update_network(wpa_s, ssid, name, value);
  if (ret == 0 && (ssid->bssid_set != prev_bssid_set ||
...
static int wpa_supplicant_ctrl_iface_get_network(struct wpa_supplicant *wpa_s,
  ...
  *name++ = '\0';
  ...
  id = atoi(cmd);
  wpa_printf(MSG_EXCESSIVE, "CTRL_IFACE: GET_NETWORK id=%d name='%s'", id,
             name);
...
static int
wpa_supplicant_ctrl_iface_dup_network(struct wpa_supplicant *wpa_s, char *cmd,
  ...
  *name++ = '\0';
  ...
  ret =
      wpa_supplicant_ctrl_iface_update_network(dst_wpa_s, ssid_d, name, value);
...
static int wpa_supplicant_ctrl_iface_list_creds(struct wpa_supplicant *wpa_s,
  ...
  while (cred) {
    ret = os_snprintf(
        pos, end - pos, "%d\t%s\t%s\t%s\t%s\n", cred->id,
        cred->realm ? cred->realm : "", cred->username ? cred->username : "",
        cred->domain ? cred->domain[0] : "", cred->imsi ? cred->imsi : "");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
...
static int wpa_supplicant_ctrl_iface_remove_cred(struct wpa_supplicant *wpa_s,
  ...
  if (os_strncmp(cmd, "sp_fqdn=", 8) == 0) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: REMOVE_CRED SP FQDN '%s'", cmd + 8);
    cred = wpa_s->conf->cred;
    while (cred) {
      prev = cred;
      cred = cred->next;
      if (prev->domain) {
        ...
        for (i = 0; i < prev->num_domain; i++) {
          if (os_strcmp(prev->domain[i], cmd + 8) != 0)
            continue;
          wpas_ctrl_remove_cred(wpa_s, prev);
          break;
        }
...
static int wpa_supplicant_ctrl_iface_set_cred(struct wpa_supplicant *wpa_s,
  ...
  *value++ = '\0';
  ...
  cred = wpa_config_get_cred(wpa_s->conf, id);
  if (cred == NULL) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: Could not find cred id=%d", id);
    return -1;
  }
...
static int wpa_supplicant_ctrl_iface_get_cred(struct wpa_supplicant *wpa_s,
  ...
  *name++ = '\0';
  ...
  id = atoi(cmd);
  wpa_printf(MSG_DEBUG, "CTRL_IFACE: GET_CRED id=%d name='%s'", id, name);
  ...
  if (value == NULL) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: Failed to get cred variable '%s'", name);
    return -1;
  }
...
static int ctrl_iface_get_capability_pairwise(int res, char *strict,
  ...
  for (i = 0; i < ARRAY_SIZE(ciphers); i++) {
    if (!ciphers[i].group_only && capa->enc & ciphers[i].capa) {
      ret = os_snprintf(pos, end - pos, "%s%s", pos == buf ? "" : " ",
                        ciphers[i].name);
      if (os_snprintf_error(end - pos, ret))
        return pos - buf;
      pos += ret;
...
static int ctrl_iface_get_capability_proto(int res, char *strict,
  ...
  if (capa->key_mgmt &
    ...
    ret = os_snprintf(pos, end - pos, "%sRSN", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (capa->key_mgmt &
    ...
    ret = os_snprintf(pos, end - pos, "%sWPA", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
...
static int ctrl_iface_get_capability_auth_alg(struct wpa_supplicant *wpa_s,
  ...
  if (capa->auth & (WPA_DRIVER_AUTH_OPEN)) {
    ret = os_snprintf(pos, end - pos, "%sOPEN", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (capa->auth & (WPA_DRIVER_AUTH_SHARED)) {
    ret = os_snprintf(pos, end - pos, "%sSHARED", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (capa->auth & (WPA_DRIVER_AUTH_LEAP)) {
    ret = os_snprintf(pos, end - pos, "%sLEAP", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SAE) {
    ret = os_snprintf(pos, end - pos, "%sSAE", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
...
static int ctrl_iface_get_capability_modes(int res, char *strict,
  ...
  if (capa->flags & WPA_DRIVER_FLAGS_IBSS) {
    ret = os_snprintf(pos, end - pos, "%sIBSS", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (capa->flags & WPA_DRIVER_FLAGS_AP) {
    ret = os_snprintf(pos, end - pos, "%sAP", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (capa->flags & WPA_DRIVER_FLAGS_MESH) {
    ret = os_snprintf(pos, end - pos, "%sMESH", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  for (j = 0; j < wpa_s->hw.num_modes; j++) {
    ...
    for (i = 0; i < wpa_s->hw.modes[j].num_channels; i++) {
      ...
      ret = os_snprintf(pos, end - pos, " %d = %d MHz%s%s\n", chnl[i].chan,
                        chnl[i].freq,
                        chnl[i].flag & HOSTAPD_CHAN_NO_IR ? " (NO_IR)" : "",
                        chnl[i].flag & HOSTAPD_CHAN_RADAR ? " (DFS)" : "");
...
static int
wpa_supplicant_ctrl_iface_get_capability(struct wpa_supplicant *wpa_s,
                                         const char *_field, char *buf,
                                         size_t buflen) {
  struct wpa_driver_capa capa;
  int res;
  char *strict;
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE: GET_CAPABILITY '%s' %s", field,
             strict ? strict : "");
  ...
  if (os_strcmp(field, "pairwise") == 0)
    return ctrl_iface_get_capability_pairwise(res, strict, &capa, buf, buflen);

  if (os_strcmp(field, "group") == 0)
    return ctrl_iface_get_capability_group(res, strict, &capa, buf, buflen);

  if (os_strcmp(field, "group_mgmt") == 0)
    return ctrl_iface_get_capability_group_mgmt(res, strict, &capa, buf,
                                                buflen);

  if (os_strcmp(field, "key_mgmt") == 0)
    return ctrl_iface_get_capability_key_mgmt(res, strict, &capa, buf, buflen);

  if (os_strcmp(field, "proto") == 0)
    return ctrl_iface_get_capability_proto(res, strict, &capa, buf, buflen);

  if (os_strcmp(field, "auth_alg") == 0)
    return ctrl_iface_get_capability_auth_alg(wpa_s, res, strict, &capa, buf,
                                              buflen);

  if (os_strcmp(field, "modes") == 0)
    return ctrl_iface_get_capability_modes(res, strict, &capa, buf, buflen);
...
static int print_bss_info(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (mask & WPA_BSS_MASK_WIFI_DISPLAY) {
    struct wpabuf *wfd;
    ie = (const u8 *)(bss + 1);
    wfd = ieee802_11_vendor_ie_concat(ie, bss->ie_len, WFD_IE_VENDOR_TYPE);
    if (wfd) {
      ...
      pos +=
          wpa_snprintf_hex(pos, end - pos, wpabuf_head(wfd), wpabuf_len(wfd));
      wpabuf_free(wfd);
  ...
  if ((mask & WPA_BSS_MASK_INTERNETW) && bss->anqp) {
    ...
    pos = anqp_add_hex(pos, end, "anqp_capability_list", anqp->capability_list);
    pos = anqp_add_hex(pos, end, "anqp_venue_name", anqp->venue_name);
    ...
    pos = anqp_add_hex(pos, end, "anqp_nai_realm", anqp->nai_realm);
    pos = anqp_add_hex(pos, end, "anqp_3gpp", anqp->anqp_3gpp);
    pos = anqp_add_hex(pos, end, "anqp_domain_name", anqp->domain_name);
#ifdef CONFIG_HS20
    ...
    dl_list_for_each(elem, &anqp->anqp_elems, struct wpa_bss_anqp_elem, list) {
      char title[20];

      os_snprintf(title, sizeof(title), "anqp[%u]", elem->infoid);
  ...
  if (mask & WPA_BSS_MASK_EST_THROUGHPUT) {
    ret =
        os_snprintf(pos, end - pos, "est_throughput=%d\n", bss->est_throughput);
    if (os_snprintf_error(end - pos, ret))
      return 0;
    pos += ret;
  }
...
static int wpa_supplicant_ctrl_iface_bss(struct wpa_supplicant *wpa_s,
  ...
  if (os_strncmp(cmd, "RANGE=", 6) == 0) {
    if (os_strncmp(cmd + 6, "ALL", 3) == 0) {
      ...
      bsslast = dl_list_last(&wpa_s->bss_id, struct wpa_bss, list_id);
    } else { /* N1-N2 */
        ...
        if (bsslast == NULL && bss && id2 > id1) {
          ...
          for (;;) {
            ...
            if (next == &wpa_s->bss_id)
              break;
            tmp = dl_list_entry(next, struct wpa_bss, list_id);
            if (tmp->id > id2)
              break;
            bsslast = tmp;
          }
    ...
    if (bss == bsslast) {
      if ((mask & WPA_BSS_MASK_DELIM) && len &&
        ...
        if (os_snprintf_error(end - buf + 5, res)) {
          wpa_printf(MSG_DEBUG, "Could not add end delim");
        }
      }
      break;
    }
...
static int wpa_supplicant_ctrl_iface_scan_interval(struct wpa_supplicant *wpa_s,
                                                   char *cmd) {
...
static int
wpa_supplicant_ctrl_iface_bss_expire_age(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
...
static int
wpa_supplicant_ctrl_iface_bss_expire_count(struct wpa_supplicant *wpa_s,
                                           char *cmd) {
  int expire_count = atoi(cmd);
  return wpa_supplicant_set_bss_expiration_count(wpa_s, expire_count);
}

static void wpa_supplicant_ctrl_iface_bss_flush(struct wpa_supplicant *wpa_s,
                                                char *cmd) {
...
static void wpa_supplicant_ctrl_iface_drop_sa(struct wpa_supplicant *wpa_s) {
  ...
  wpa_drv_set_key(wpa_s, WPA_ALG_NONE, wpa_s->bssid, 0, 0, NULL, 0, NULL, 0);
...
static int p2ps_ctrl_parse_cpt_priority(const char *pos, u8 *cpt) {
  ...
  for (i = 0; (token = cstr_token(pos, ": \t", &last)); i++) {
    ...
    if (token_len == 3 && os_memcmp(token, "UDP", token_len) == 0) {
      cpt[i] = P2PS_FEATURE_CAPAB_UDP_TRANSPORT;
    } else if (token_len == 3 && os_memcmp(token, "MAC", token_len) == 0) {
...
static struct p2ps_provision *p2p_parse_asp_provision_cmd(const char *cmd) {
  ...
  if (pos) {
    ...
    if (info_len) {
      ...
      if (info) {
        info_len = utf8_unescape(pos, info_len, info, info_len + 1);
      } else
        info_len = 0;
    }
  }
  ...
  if (pos) {
    if (p2ps_ctrl_parse_cpt_priority(pos + 4, p2ps_prov->cpt_priority))
      goto invalid_args;
  } else {
  ...
  if (pos) {
    ...
    if (pos) {
      ...
      if (role != P2PS_SETUP_CLIENT && role != P2PS_SETUP_GROUP_OWNER)
...
static int p2p_ctrl_asp_provision_resp(struct wpa_supplicant *wpa_s,
                                       char *cmd) {
  u8 addr[ETH_ALEN];
  struct p2ps_provision *p2ps_prov;
  char *pos;
  ...
  return wpas_p2p_prov_disc(wpa_s, addr, NULL, WPAS_P2P_PD_FOR_ASP, p2ps_prov);
}
...
static int p2p_ctrl_connect(struct wpa_supplicant *wpa_s, char *cmd, char *buf,
                            size_t buflen) {
  u8 addr[ETH_ALEN];
  char *pos, *pos2;
  char *pin = NULL;
  ...
  new_pin = wpas_p2p_connect(wpa_s, addr, pin, wps_method, persistent_group,
                             automatic, join, auth, go_intent, freq, freq2,
                             persistent_id, pd, ht40, vht, max_oper_chwidth,
...
static int p2p_ctrl_serv_disc_req(struct wpa_supplicant *wpa_s, char *cmd,
  ...
  if (hwaddr_aton(cmd, dst_buf))
    return -1;
  dst = dst_buf;
  if (dst[0] == 0 && dst[1] == 0 && dst[2] == 0 && dst[3] == 0 && dst[4] == 0 &&
      dst[5] == 0)
    dst = NULL;
  pos = cmd + 17;
  if (*pos != ' ')
...
static int p2p_ctrl_group_add_persistent(struct wpa_supplicant *wpa_s, int id,
                                         int freq, int vht_center_freq2,
  ...
  return wpas_p2p_group_add_persistent(wpa_s, ssid, 0, freq, vht_center_freq2,
                                       0, ht40, vht, vht_chwidth, NULL, 0, 0);
}
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  while ((token = str_token(cmd, " ", &context))) {
    if (sscanf(token, "freq=%d", &freq) == 1 ||
      ...
      wpa_printf(MSG_DEBUG, "CTRL: Invalid P2P_GROUP_ADD parameter: '%s'",
                 token);
      return -1;
    }
  }
  ...
  if (group_id >= 0)
    return p2p_ctrl_group_add_persistent(wpa_s, group_id, freq, freq2, ht40,
                                         vht, max_oper_chwidth);
...
static int p2p_ctrl_peer(struct wpa_supplicant *wpa_s, char *cmd, char *buf,
                         size_t buflen) {
  ...
  res = os_snprintf(
      ...
      info->device_name, info->manufacturer, info->model_name,
      info->model_number, info->serial_number, info->config_methods,
      info->dev_capab, info->group_capab, info->level);
  ...
  if (info->vendor_elems) {
    ...
    pos += wpa_snprintf_hex(pos, end - pos, wpabuf_head(info->vendor_elems),
...
static int p2p_ctrl_disallow_freq(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < wpa_s->global->p2p_disallow_freq.num; i++) {
    ...
    wpa_printf(MSG_DEBUG, "P2P: Disallowed frequency range %u-%u", freq->min,
               freq->max);
  }
...
static int p2p_ctrl_set(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  *param++ = '\0';
  ...
  if (os_strcmp(cmd, "discoverability") == 0) {
    p2p_set_client_discoverability(wpa_s->global->p2p, atoi(param));
    return 0;
  }
  ...
  if (os_strcmp(cmd, "listen_channel") == 0) {
    ...
    return p2p_set_listen_channel(wpa_s->global->p2p, op_class, channel, 1);
  }
  ...
  if (os_strcmp(cmd, "disc_int") == 0) {
    ...
    *pos++ = '\0';
    ...
    return p2p_set_disc_int(wpa_s->global->p2p, min_disc_int, max_disc_int,
                            max_disc_tu);
  }
...
static int p2p_ctrl_remove_client(struct wpa_supplicant *wpa_s,
                                  const char *cmd) {
  const char *pos;
  u8 peer[ETH_ALEN];
  int iface_addr = 0;
...
static int p2p_ctrl_iface_p2p_lo_start(struct wpa_supplicant *wpa_s,
                                       char *cmd) {
  int freq = 0, period = 0, interval = 0, count = 0;

  if (sscanf(cmd, "%d %d %d %d", &freq, &period, &interval, &count) != 4) {
    wpa_printf(MSG_DEBUG, "CTRL: Invalid P2P LO Start parameter: '%s'", cmd);
    return -1;
  }
...
static int ctrl_interworking_connect(struct wpa_supplicant *wpa_s, char *dst,
  ...
  bss = wpa_bss_get_bssid(wpa_s, bssid);
  if (bss == NULL) {
    wpa_printf(MSG_DEBUG, "Could not find BSS " MACSTR, MAC2STR(bssid));
    return -1;
  }
  ...
  if (bss->ssid_len == 0) {
    ...
    wpa_printf(MSG_DEBUG,
               "Selected BSS entry for " MACSTR
               " does not have SSID information",
               MAC2STR(bssid));

    dl_list_for_each_reverse(bss, &wpa_s->bss, struct wpa_bss, list) {
      if (os_memcmp(bss->bssid, bssid, ETH_ALEN) == 0 && bss->ssid_len > 0) {
        found = 1;
        break;
      }
    }
    ...
    if (!found)
      return -1;
    wpa_printf(MSG_DEBUG, "Found another matching BSS entry with SSID");
  }
...
#define MAX_ANQP_INFO_ID 100
  ...
  return anqp_send_req(wpa_s, dst_addr, id, num_id, subtypes, get_cell_pref);
}
...
static int hs20_nai_home_realm_list(struct wpa_supplicant *wpa_s,
  ...
  ret = hs20_anqp_send_req(wpa_s, addr, BIT(HS20_STYPE_NAI_HOME_REALM_QUERY),
                           buf, len, 0);
...
static int hs20_get_nai_home_realm_list(struct wpa_supplicant *wpa_s,
  ...
  if (os_strncmp(dst + used, "realm=", 6) == 0)
    return hs20_nai_home_realm_list(wpa_s, dst_addr, dst + used + 6);
  ...
  ret = hs20_anqp_send_req(wpa_s, dst_addr,
                           BIT(HS20_STYPE_NAI_HOME_REALM_QUERY), buf, len, 0);
  os_free(buf);
...
static int get_hs20_icon(struct wpa_supplicant *wpa_s, char *cmd, char *reply,
  ...
  return hs20_get_icon(wpa_s, dst_addr, icon, atoi(poffset), atoi(psize), reply,
                       buflen);
}
...
static int wpas_ctrl_iface_wnm_sleep(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  ret = ieee802_11_send_wnmsleep_req(
      wpa_s, enter ? WNM_SLEEP_MODE_ENTER : WNM_SLEEP_MODE_EXIT, intval,
      tfs_req);
  wpabuf_free(tfs_req);
...
static int wpas_ctrl_iface_wnm_bss_query(struct wpa_supplicant *wpa_s,
                                         char *cmd) {
  ...
  if (cmd) {
    ...
    if (os_strncmp(cmd, "list", 4) == 0) {
      ...
      wpa_printf(MSG_DEBUG, "WNM Query: Invalid option %s", cmd);
      return -1;
    }
  }

  wpa_printf(MSG_DEBUG, "CTRL_IFACE: WNM_BSS_QUERY query_reason=%d%s",
...
static int wpa_supplicant_signal_poll(struct wpa_supplicant *wpa_s, char *buf,
  ...
  if (si.avg_beacon_signal) {
    ret = os_snprintf(pos, end - pos, "AVG_BEACON_RSSI=%d\n",
...
static int wpas_ctrl_iface_signal_monitor(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->bgscan && wpa_s->bgscan_priv) {
    wpa_printf(MSG_DEBUG, "Reject SIGNAL_MONITOR command - bgscan is active");
    return -1;
  }
...
static int wpas_ctrl_iface_get_pref_freq_list(struct wpa_supplicant *wpa_s,
                                              char *cmd, char *buf,
                                              size_t buflen) {
  ...
  wpa_printf(MSG_DEBUG, "CTRL_IFACE: GET_PREF_FREQ_LIST iface_type=%d (%s)",
             iface_type, buf);
  ...
  for (i = 0; i < num; i++) {
    ret = os_snprintf(pos, end - pos, "%s%u", i > 0 ? "," : "", freq_list[i]);
    if (os_snprintf_error(end - pos, ret))
      return -1;
    pos += ret;
  }
...
static int wpas_ctrl_iface_driver_flags(struct wpa_supplicant *wpa_s, char *buf,
                                        size_t buflen) {
  int ret, i;
  char *pos, *end;
...
static int wpa_supplicant_vendor_cmd(struct wpa_supplicant *wpa_s, char *cmd,
  ...
  ret = wpa_drv_vendor_cmd(wpa_s, vendor_id, subcmd, data, data_len, reply);

  if (ret == 0)
    ret =
        wpa_snprintf_hex(buf, buflen, wpabuf_head_u8(reply), wpabuf_len(reply));
...
static void wpa_supplicant_ctrl_iface_flush(struct wpa_supplicant *wpa_s) {
  ...
  struct wpa_supplicant *p2p_wpa_s =
      wpa_s->global->p2p_init_wpa_s ? wpa_s->global->p2p_init_wpa_s : wpa_s;
#endif /* CONFIG_P2P */
  ...
  if (!dl_list_empty(&wpa_s->bss)) {
    wpa_printf(MSG_DEBUG,
               "BSS table not empty after flush: %u entries, current_bss=%p "
               "bssid=" MACSTR " pending_bssid=" MACSTR,
...
static int wpas_ctrl_radio_work_show(struct wpa_supplicant *wpa_s, char *buf,
                                     size_t buflen) {
  ...
  dl_list_for_each(work, &wpa_s->radio->work, struct wpa_radio_work, list) {
    ...
    ret = os_snprintf(pos, end - pos, "%s@%s:%u:%u:%ld.%06ld\n", work->type,
                      work->wpa_s->ifname, work->freq, work->started, diff.sec,
                      diff.usec);
    if (os_snprintf_error(end - pos, ret))
      break;
    pos += ret;
...
static void wpas_ctrl_radio_work_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_dbg(work->wpa_s, MSG_DEBUG, "Timing out external radio work %u (%s)",
          ework->id, work->type);
...
static void wpas_ctrl_radio_work_cb(struct wpa_radio_work *work, int deinit) {
  ...
  if (!ework->timeout)
    ework->timeout = 10;
  eloop_register_timeout(ework->timeout, 0, wpas_ctrl_radio_work_timeout, work,
                         NULL);
}
...
static int wpas_ctrl_radio_work_done(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  dl_list_for_each(work, &wpa_s->radio->work, struct wpa_radio_work, list) {
    ...
    if (id && ework->id != id)
      continue;
    wpa_dbg(wpa_s, MSG_DEBUG, "Completed external radio work %u (%s)",
            ework->id, ework->type);
    eloop_cancel_timeout(wpas_ctrl_radio_work_timeout, work, NULL);
    wpa_s->ext_work_in_progress = 0;
    radio_work_done(work);
...
void wpas_ctrl_radio_work_flush(struct wpa_supplicant *wpa_s) {
  ...
  dl_list_for_each_safe(work, tmp, &wpa_s->radio->work, struct wpa_radio_work,
                        list) {
    struct wpa_external_work *ework;
    ...
    if (os_strncmp(work->type, "ext:", 4) != 0)
      continue;
    ework = work->ctx;
    wpa_dbg(wpa_s, MSG_DEBUG, "Flushing%s external radio work %u (%s)",
            work->started ? " started" : "", ework->id, ework->type);
    if (work->started)
      eloop_cancel_timeout(wpas_ctrl_radio_work_timeout, work, NULL);
    radio_work_done(work);
    os_free(ework);
...
static void wpas_ctrl_scan(struct wpa_supplicant *wpa_s, char *params,
  ...
  if (wpa_s->fetch_anqp_in_progress || wpa_s->network_select) {
    wpa_printf(MSG_DEBUG, "Interworking select in progress - reject new scan");
    *reply_len = os_snprintf(reply, reply_size, "FAIL-BUSY\n");
    return;
  }
#endif /* CONFIG_INTERWORKING */
  ...
  if (params) {
    ...
    pos = os_strstr(params, "freq=");
    if (pos) {
      manual_scan_freqs = freq_range_to_channel_list(wpa_s, pos + 5);
    ...
    if (pos &&
        scan_id_list_parse(wpa_s, pos + 8, &scan_id_count, scan_id) < 0) {
      *reply_len = -1;
      goto done;
    }
    ...
    while (pos && *pos != '\0') {
      if (os_strncmp(pos, "ssid ", 5) == 0) {
        ...
        if ((end - pos) & 0x01 || end - pos > 2 * SSID_MAX_LEN ||
            hexstr2bin(pos, ssid[ssid_count].ssid, (end - pos) / 2) < 0) {
          wpa_printf(MSG_DEBUG, "Invalid SSID value '%s'", pos);
          *reply_len = -1;
          goto done;
        }
        ssid[ssid_count].ssid_len = (end - pos) / 2;
        wpa_hexdump_ascii(MSG_DEBUG, "scan SSID", ssid[ssid_count].ssid,
                          ssid[ssid_count].ssid_len);
        ssid_count++;
        pos = end;
      }
  ...
  if (!wpa_s->sched_scanning && !wpa_s->scanning &&
    ...
    wpa_printf(
        MSG_DEBUG,
        "Stop ongoing sched_scan to allow requested full scan to proceed");
    wpa_supplicant_cancel_sched_scan(wpa_s);
    wpa_s->scan_req = MANUAL_SCAN_REQ;
    wpa_supplicant_req_scan(wpa_s, 0, 0);
    if (wpa_s->manual_scan_use_id) {
...
static void
wpas_ctrl_iface_mgmt_tx_cb(struct wpa_supplicant *wpa_s, unsigned int freq,
                           const u8 *dst, const u8 *src, const u8 *bssid,
                           const u8 *data, size_t data_len,
                           enum offchannel_send_action_result result) {
  wpa_msg(
      wpa_s, MSG_INFO,
      "MGMT-TX-STATUS freq=%u dst=" MACSTR " src=" MACSTR " bssid=" MACSTR
      " result=%s",
      freq, MAC2STR(dst), MAC2STR(src), MAC2STR(bssid),
      result == OFFCHANNEL_SEND_ACTION_SUCCESS
          ? "SUCCESS"
          : (result == OFFCHANNEL_SEND_ACTION_NO_ACK ? "NO_ACK" : "FAILED"));
}
...
static int wpas_ctrl_iface_mgmt_tx(struct wpa_supplicant *wpa_s, char *cmd) {
  ...
  res =
      offchannel_send_action(wpa_s, freq, da, wpa_s->own_addr, bssid, buf, len,
                             wait_time, wpas_ctrl_iface_mgmt_tx_cb, no_cck);
  os_free(buf);
  return res;
}
...
static int wpas_ctrl_iface_driver_event(struct wpa_supplicant *wpa_s,
                                        char *cmd) {
  char *pos, *param;
  union wpa_event_data event;
  ...
  if (os_strcmp(cmd, "INTERFACE_ENABLED") == 0) {
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "Testing - unknown driver event: %s", cmd);
    return -1;
  }
...
static void wpas_data_test_rx(void *ctx, const u8 *src_addr, const u8 *buf,
  ...
  if (ip.ihl != 5 || ip.version != 4 || ntohs(ip.tot_len) != HWSIM_IP_LEN)
    return;
...
static int wpas_ctrl_iface_data_test_config(struct wpa_supplicant *wpa_s,
  ...
  wpa_s->l2_test = l2_packet_init(ifname, wpa_s->own_addr, ETHERTYPE_IP,
...
static int wpas_ctrl_iface_data_test_tx(struct wpa_supplicant *wpa_s,
                                        char *cmd) {
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "test data: TX dst=" MACSTR " src=" MACSTR " tos=0x%x", MAC2STR(dst),
          MAC2STR(src), tos);
...
static int wpas_ctrl_get_alloc_fail(struct wpa_supplicant *wpa_s, char *buf,
                                    size_t buflen) {
#ifdef WPA_TRACE_BFD
...
static int wpas_ctrl_get_fail(struct wpa_supplicant *wpa_s, char *buf,
                              size_t buflen) {
#ifdef WPA_TRACE_BFD
...
static void wpas_ctrl_event_test_cb(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(MSG_DEBUG, "TEST: Send %d control interface event messages",
             count);
  for (i = 0; i < count; i++) {
    wpa_msg_ctrl(wpa_s, MSG_INFO, "TEST-EVENT-MESSAGE %d/%d", i + 1, count);
  }
}
...
static int wpas_ctrl_vendor_elem_remove(struct wpa_supplicant *wpa_s,
                                        char *cmd) {
  char *pos = cmd;
  int frame;
  size_t len;
...
#define NR_IE_MIN_LEN (ETH_ALEN + 4 + 1 + 1 + 1)
  ...
  while (len >= 2 + NR_IE_MIN_LEN) {
    ...
    if (pos[0] != WLAN_EID_NEIGHBOR_REPORT || nr_len < NR_IE_MIN_LEN) {
      wpa_printf(MSG_DEBUG,
                 "CTRL: Invalid Neighbor Report element: id=%u len=%u", data[0],
                 nr_len);
      goto out;
    }
...
static int wpas_ctrl_iface_send_neighbor_rep(struct wpa_supplicant *wpa_s,
  ...
  if (ssid_s) {
    if (ssid_parse(ssid_s + 5, &ssid)) {
      wpa_printf(MSG_ERROR, "CTRL: Send Neighbor Report: bad SSID");
      return -1;
    }
  ...
  ret = wpas_rrm_send_neighbor_rep_request(wpa_s, ssid_p, lci, civic,
                                           wpas_ctrl_neighbor_rep_cb, wpa_s);
...
static int wpas_ctrl_iface_mac_rand_scan(struct wpa_supplicant *wpa_s,
  ...
  while ((token = str_token(cmd, " ", &context))) {
    if (os_strcasecmp(token, "scan") == 0) {
      ...
      if (hwaddr_aton(token + 5, addr)) {
        wpa_printf(MSG_INFO, "CTRL: Invalid MAC address: %s", token);
        return -1;
      }
      ...
      wpa_printf(MSG_INFO, "CTRL: Invalid MAC_RAND_SCAN parameter: %s", token);
      return -1;
    }
  }
  ...
  if ((wpa_s->mac_addr_rand_supported & type) != type) {
    wpa_printf(MSG_INFO, "CTRL: MAC_RAND_SCAN types=%u != supported=%u", type,
               wpa_s->mac_addr_rand_supported);
    return -1;
  }

  if (enable > 1) {
    wpa_printf(MSG_INFO, "CTRL: MAC_RAND_SCAN enable=<0/1> not specified");
    return -1;
  }
  ...
  if ((addr && !mask) || (!addr && mask)) {
    wpa_printf(MSG_INFO, "CTRL: MAC_RAND_SCAN invalid addr/mask combination");
    return -1;
  }

  if (addr && mask && (!(mask[0] & 0x01) || (addr[0] & 0x01))) {
    wpa_printf(MSG_INFO, "CTRL: MAC_RAND_SCAN cannot allow multicast address");
    return -1;
  }

  if (type & MAC_ADDR_RAND_SCAN) {
    wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCAN, addr, mask);
  }

  if (type & MAC_ADDR_RAND_SCHED_SCAN) {
    wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCHED_SCAN, addr, mask);
  ...
  if (type & MAC_ADDR_RAND_PNO) {
    wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_PNO, addr, mask);
    if (wpa_s->pno) {
      wpas_stop_pno(wpa_s);
      wpas_start_pno(wpa_s);
    }
...
static int wpas_ctrl_iface_pmksa(struct wpa_supplicant *wpa_s, char *buf,
                                 size_t buflen) {
  size_t reply_len;
...
#ifdef CONFIG_AP
  reply_len +=
      wpas_ap_pmksa_cache_list(wpa_s, &buf[reply_len], buflen - reply_len);
#endif /* CONFIG_AP */
  return reply_len;
}
...
static int wpas_ctrl_cmd_debug_level(const char *cmd) {
  if (os_strcmp(cmd, "PING") == 0 || os_strncmp(cmd, "BSS ", 4) == 0 ||
      os_strncmp(cmd, "GET_NETWORK ", 12) == 0 ||
      os_strncmp(cmd, "STATUS", 6) == 0 || os_strncmp(cmd, "STA ", 4) == 0 ||
...
char *wpa_supplicant_ctrl_iface_process(struct wpa_supplicant *wpa_s, char *buf,
  ...
  if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ||
    ...
    if (wpa_debug_show_keys)
      ...
      wpa_dbg(wpa_s, MSG_DEBUG, "Control interface command '%s [REMOVED]'",
              os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0
                  ? WPA_CTRL_RSP
                  : "SET_NETWORK");
  } else if (os_strncmp(buf, "WPS_NFC_TAG_READ", 16) == 0 ||
             os_strncmp(buf, "NFC_REPORT_HANDOVER", 19) == 0) {
    wpa_hexdump_ascii_key(MSG_DEBUG, "RX ctrl_iface", (const u8 *)buf,
                          os_strlen(buf));
  } else {
  ...
  if (os_strcmp(buf, "PING") == 0) {
    ...
    if (reply_len >= 0) {
      reply_len += eapol_sm_get_mib(wpa_s->eapol, reply + reply_len,
                                    reply_size - reply_len);
    }
  } else if (os_strncmp(buf, "STATUS", 6) == 0) {
    reply_len =
        wpa_supplicant_ctrl_iface_status(wpa_s, buf + 6, reply, reply_size);
    ...
    reply_len = wpa_config_dump_values(wpa_s->conf, reply, reply_size);
  } else if (os_strncmp(buf, "GET ", 4) == 0) {
    reply_len =
        wpa_supplicant_ctrl_iface_get(wpa_s, buf + 4, reply, reply_size);
    ...
    reply_len =
        wpa_supplicant_ctrl_iface_wps_pin(wpa_s, buf + 8, reply, reply_size);
  } else if (os_strncmp(buf, "WPS_CHECK_PIN ", 14) == 0) {
    reply_len = wpa_supplicant_ctrl_iface_wps_check_pin(wpa_s, buf + 14, reply,
                                                        reply_size);
    ...
    reply_len = wpa_supplicant_ctrl_iface_wps_nfc_token(wpa_s, buf + 14, reply,
                                                        reply_size);
  } else if (os_strncmp(buf, "WPS_NFC_TAG_READ ", 17) == 0) {
    if (wpa_supplicant_ctrl_iface_wps_nfc_tag_read(wpa_s, buf + 17))
      reply_len = -1;
  } else if (os_strncmp(buf, "NFC_GET_HANDOVER_REQ ", 21) == 0) {
    reply_len =
        wpas_ctrl_nfc_get_handover_req(wpa_s, buf + 21, reply, reply_size);
  } else if (os_strncmp(buf, "NFC_GET_HANDOVER_SEL ", 21) == 0) {
    reply_len =
        wpas_ctrl_nfc_get_handover_sel(wpa_s, buf + 21, reply, reply_size);
    ...
    reply_len = wpa_supplicant_ctrl_iface_wps_ap_pin(wpa_s, buf + 11, reply,
                                                     reply_size);
#endif /* CONFIG_AP */
#ifdef CONFIG_WPS_ER
    ...
    if (wpa_supplicant_ctrl_iface_wps_er_set_config(wpa_s, buf + 18))
      reply_len = -1;
    ...
    reply_len = wpa_supplicant_ctrl_iface_mesh_interface_add(wpa_s, buf + 19,
                                                             reply, reply_size);
  } else if (os_strcmp(buf, "MESH_INTERFACE_ADD") == 0) {
    reply_len = wpa_supplicant_ctrl_iface_mesh_interface_add(wpa_s, "", reply,
                                                             reply_size);
    ...
    reply_len = p2p_ctrl_connect(wpa_s, buf + 12, reply, reply_size);
    ...
    reply_len = p2p_ctrl_group_member(wpa_s, buf + 17, reply, reply_size);
    ...
    reply_len = p2p_ctrl_serv_disc_req(wpa_s, buf + 18, reply, reply_size);
    ...
    reply_len =
        wifi_display_subelem_get(wpa_s->global, buf + 16, reply, reply_size);
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_INTERWORKING
    ...
    reply_len = gas_response_get(wpa_s, buf + 17, reply, reply_size);
#endif /* CONFIG_INTERWORKING */
#ifdef CONFIG_HS20
    ...
    if (wpa_supplicant_ctrl_iface_ctrl_rsp(wpa_s,
                                           buf + os_strlen(WPA_CTRL_RSP)))
      reply_len = -1;
    else {
      /*
       ...
       */
      eloop_register_timeout(0, 0, wpas_ctrl_eapol_response, wpa_s, NULL);
    }
    ...
    reply_len =
        wpa_supplicant_ctrl_iface_blacklist(wpa_s, buf + 9, reply, reply_size);
  } else if (os_strncmp(buf, "LOG_LEVEL", 9) == 0) {
    reply_len =
        wpa_supplicant_ctrl_iface_log_level(wpa_s, buf + 9, reply, reply_size);
  } else if (os_strncmp(buf, "LIST_NETWORKS ", 14) == 0) {
    reply_len = wpa_supplicant_ctrl_iface_list_networks(wpa_s, buf + 14, reply,
                                                        reply_size);
  } else if (os_strcmp(buf, "LIST_NETWORKS") == 0) {
    reply_len =
        wpa_supplicant_ctrl_iface_list_networks(wpa_s, NULL, reply, reply_size);
    ...
    reply_len =
        wpa_supplicant_ctrl_iface_scan_results(wpa_s, reply, reply_size);
    ...
    reply_len = wpa_supplicant_ctrl_iface_get_network(wpa_s, buf + 12, reply,
                                                      reply_size);
  } else if (os_strncmp(buf, "DUP_NETWORK ", 12) == 0) {
    if (wpa_supplicant_ctrl_iface_dup_network(wpa_s, buf + 12, wpa_s))
      reply_len = -1;
  } else if (os_strcmp(buf, "LIST_CREDS") == 0) {
    reply_len = wpa_supplicant_ctrl_iface_list_creds(wpa_s, reply, reply_size);
  } else if (os_strcmp(buf, "ADD_CRED") == 0) {
    reply_len = wpa_supplicant_ctrl_iface_add_cred(wpa_s, reply, reply_size);
    ...
    reply_len =
        wpa_supplicant_ctrl_iface_get_cred(wpa_s, buf + 9, reply, reply_size);
#ifndef CONFIG_NO_CONFIG_WRITE
    ...
    reply_len = wpa_supplicant_ctrl_iface_get_capability(wpa_s, buf + 15, reply,
                                                         reply_size);
    ...
    reply_len =
        wpa_supplicant_global_iface_list(wpa_s->global, reply, reply_size);
  } else if (os_strncmp(buf, "INTERFACES", 10) == 0) {
    reply_len = wpa_supplicant_global_iface_interfaces(wpa_s->global, buf + 10,
                                                       reply, reply_size);
  } else if (os_strncmp(buf, "BSS ", 4) == 0) {
    reply_len =
        wpa_supplicant_ctrl_iface_bss(wpa_s, buf + 4, reply, reply_size);
#ifdef CONFIG_AP
  } else if (os_strcmp(buf, "STA-FIRST") == 0) {
    reply_len = ap_ctrl_iface_sta_first(wpa_s, reply, reply_size);
  } else if (os_strncmp(buf, "STA ", 4) == 0) {
    reply_len = ap_ctrl_iface_sta(wpa_s, buf + 4, reply, reply_size);
    ...
    if (wpa_supplicant_ctrl_iface_tdls_chan_switch(wpa_s, buf + 17))
      reply_len = -1;
  } else if (os_strncmp(buf, "TDLS_CANCEL_CHAN_SWITCH ", 24) == 0) {
    if (wpa_supplicant_ctrl_iface_tdls_cancel_chan_switch(wpa_s, buf + 24))
      reply_len = -1;
  } else if (os_strncmp(buf, "TDLS_LINK_STATUS ", 17) == 0) {
    reply_len = wpa_supplicant_ctrl_iface_tdls_link_status(wpa_s, buf + 17,
                                                           reply, reply_size);
#endif /* CONFIG_TDLS */
    ...
    reply_len = wpa_supplicant_pktcnt_poll(wpa_s, reply, reply_size);
#ifdef CONFIG_AUTOSCAN
    ...
    reply_len = wpas_ctrl_iface_driver_flags(wpa_s, reply, reply_size);
#ifdef ANDROID
  } else if (os_strncmp(buf, "DRIVER ", 7) == 0) {
    reply_len = wpa_supplicant_driver_cmd(wpa_s, buf + 7, reply, reply_size);
#endif /* ANDROID */
  } else if (os_strncmp(buf, "VENDOR ", 7) == 0) {
    reply_len = wpa_supplicant_vendor_cmd(wpa_s, buf + 7, reply, reply_size);
    ...
    reply_len = wpas_ctrl_radio_work(wpa_s, buf + 11, reply, reply_size);
#ifdef CONFIG_TESTING_OPTIONS
    ...
    reply_len = wpas_ctrl_vendor_elem_get(wpa_s, buf + 16, reply, reply_size);
    ...
    reply_len =
        wpas_ctrl_iface_get_pref_freq_list(wpa_s, buf + 19, reply, reply_size);
  } else {
...
static int wpa_supplicant_global_iface_add(struct wpa_global *global,
  ...
  do {
    ...
    if (os_strcmp(extra, "create") == 0) {
      ...
      if (os_strcmp(pos, "sta") == 0) {
        ...
        wpa_printf(MSG_DEBUG, "INTERFACE_ADD unsupported interface type: '%s'",
                   pos);
        return -1;
      }
    } else {
      wpa_printf(MSG_DEBUG, "INTERFACE_ADD unsupported extra parameter: '%s'",
                 extra);
      return -1;
    }
  } while (0);
  ...
  if (create_iface) {
    ...
    if (!global->ifaces)
      return -1;
    if (wpa_drv_if_add(global->ifaces, type, iface.ifname, NULL, NULL, NULL,
                       mac_addr, NULL) < 0) {
    ...
    wpa_printf(MSG_DEBUG,
               "CTRL_IFACE interface '%s' created with MAC addr: " MACSTR,
...
static int wpa_supplicant_global_iface_remove(struct wpa_global *global,
  ...
  if (!ret && delete_iface) {
    wpa_printf(MSG_DEBUG, "CTRL_IFACE deleting the interface '%s'", cmd);
...
static int wpa_supplicant_global_iface_interfaces(struct wpa_global *global,
  ...
  while (wpa_s) {
    if (show_ctrl)
      res = os_snprintf(
          pos, end - pos, "%s ctrl_iface=%s\n", wpa_s->ifname,
          wpa_s->conf->ctrl_interface ? wpa_s->conf->ctrl_interface : "N/A");
    else
...
static char *wpas_global_ctrl_iface_redir_p2p(struct wpa_global *global,
...
#ifdef CONFIG_P2P
  static const char *cmd[] = {"LIST_NETWORKS",
                              "P2P_FIND",
                              "P2P_STOP_FIND",
                              "P2P_LISTEN",
                              "P2P_GROUP_ADD",
  ...
  if (found)
    return wpa_supplicant_ctrl_iface_process(global->p2p_init_wpa_s, buf,
                                             resp_len);
#endif /* CONFIG_P2P */
  return NULL;
}
...
static char *wpas_global_ctrl_iface_redir(struct wpa_global *global, char *buf,
                                          size_t *resp_len) {
  char *ret;
...
static int wpas_global_ctrl_iface_dup_network(struct wpa_global *global,
  ...
  for (i = 0; i < ARRAY_SIZE(wpa_s); i++) {
    ...
    *p = '\0';
    ...
    if (!wpa_s[i]) {
      wpa_printf(MSG_DEBUG, "CTRL_IFACE: Could not find iface=%s", cmd);
      return -1;
    }
...
static int wpas_global_ctrl_iface_save_config(struct wpa_global *global) {
  ...
  for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    if (!wpa_s->conf->update_config) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "CTRL_IFACE: SAVE_CONFIG - Not allowed to update configuration "
              "(update_config=0)");
      continue;
    }
    ...
    if (wpa_config_write(wpa_s->confname, wpa_s->conf)) {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "CTRL_IFACE: SAVE_CONFIG - Failed to update configuration");
      ret = 1;
    } else {
      wpa_dbg(wpa_s, MSG_DEBUG,
              "CTRL_IFACE: SAVE_CONFIG - Configuration updated");
      saved++;
    }
  }
...
static int wpas_global_ctrl_iface_status(struct wpa_global *global, char *buf,
                                         size_t buflen) {
  char *pos, *end;
  int ret;
  struct wpa_supplicant *wpa_s;
...
#ifdef CONFIG_WIFI_DISPLAY
  ret =
      os_snprintf(pos, end - pos, "wifi_display=%d\n", !!global->wifi_display);
  if (os_snprintf_error(end - pos, ret))
    return pos - buf;
  pos += ret;
#endif /* CONFIG_WIFI_DISPLAY */
...
static int wpas_global_ctrl_iface_fst_attach(struct wpa_global *global,
  ...
  if (!fst_parse_attach_command(cmd, ifname, sizeof(ifname), &cfg)) {
    ...
    if (wpa_s) {
      ...
      wpa_s->fst = fst_attach(ifname, wpa_s->own_addr, &iface_obj, &cfg);
      if (wpa_s->fst)
...
char *wpa_supplicant_global_ctrl_iface_process(struct wpa_global *global,
  ...
  if (os_strcmp(buf, "PING") == 0)
    level = MSG_EXCESSIVE;
  wpa_hexdump_ascii(level, "RX global ctrl_iface", (const u8 *)buf,
                    os_strlen(buf));
  ...
  if (os_strcmp(buf, "PING") == 0) {
    ...
    reply_len = wpa_supplicant_global_iface_list(global, reply, reply_size);
  } else if (os_strncmp(buf, "INTERFACES", 10) == 0) {
    reply_len = wpa_supplicant_global_iface_interfaces(global, buf + 10, reply,
                                                       reply_size);
#ifdef CONFIG_FST
  } else if (os_strncmp(buf, "FST-ATTACH ", 11) == 0) {
    reply_len =
        wpas_global_ctrl_iface_fst_attach(global, buf + 11, reply, reply_size);
  } else if (os_strncmp(buf, "FST-DETACH ", 11) == 0) {
    reply_len =
        wpas_global_ctrl_iface_fst_detach(global, buf + 11, reply, reply_size);
