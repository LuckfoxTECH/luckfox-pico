...
static int pkcs7_to_cert(struct hs20_osu_client *ctx, const u8 *pkcs7,
  ...
  if (pem_file) {
    ...
    if (out == NULL || BIO_write_filename(out, pem_file) <= 0)
      goto fail;
...
int est_load_cacerts(struct hs20_osu_client *ctx, const char *url) {
  ...
  res =
      http_download_file(ctx->http, buf, "Cert/est-cacerts.txt", ctx->ca_fname);
  http_ocsp_set(ctx->http,
                (ctx->workarounds & WORKAROUND_OCSP_OPTIONAL) ? 1 : 2);
  ctx->no_osu_cert_validation = 0;
  if (res < 0) {
    wpa_printf(MSG_INFO, "Failed to download EST cacerts from %s", buf);
    write_result(ctx, "Failed to download EST cacerts from %s", buf);
    os_free(buf);
    return -1;
  }
  os_free(buf);
  ...
  if (pkcs7 && pkcs7_len < resp_len / 2) {
    wpa_printf(MSG_INFO,
               "Too short base64 decode (%u bytes; downloaded %u bytes) - "
               "assume this was binary",
               (unsigned int)pkcs7_len, (unsigned int)resp_len);
    os_free(pkcs7);
    pkcs7 = NULL;
  }
  if (pkcs7 == NULL) {
    wpa_printf(MSG_INFO, "EST workaround - Could not decode base64, assume "
                         "this is DER encoded PKCS7");
  ...
  res = pkcs7_to_cert(ctx, pkcs7, pkcs7_len, "Cert/est-cacerts.pem", NULL);
  os_free(pkcs7);
  if (res < 0) {
    wpa_printf(MSG_INFO,
               "Could not parse CA certs from PKCS#7 cacerts response");
    write_result(ctx,
                 "Could not parse CA certs from EST PKCS#7 cacerts response");
    return -1;
  }
...
ASN1_SEQUENCE(Attribute) = {
    ...
    ASN1_SET_OF(Attribute, values, ASN1_OBJECT)} ASN1_SEQUENCE_END(Attribute);

ASN1_CHOICE(AttrOrOID) = {
    ASN1_SIMPLE(AttrOrOID, d.oid, ASN1_OBJECT),
    ASN1_SIMPLE(AttrOrOID, d.attribute, Attribute)} ASN1_CHOICE_END(AttrOrOID);

ASN1_CHOICE(CsrAttrs) = {
    ASN1_SEQUENCE_OF(CsrAttrs, attrs, AttrOrOID)} ASN1_CHOICE_END(CsrAttrs);
...
static void add_csrattrs_attr(struct hs20_osu_client *ctx, Attribute *attr,
  ...
  for (i = 0; i < num; i++) {
    ...
    wpa_printf(MSG_INFO, "Ignore unsupported cstattr::attr %s oid %s", txt,
               txt2);
  }
}
...
#ifdef OPENSSL_IS_BORINGSSL
  num = sk_num(CHECKED_CAST(_STACK *, STACK_OF(AttrOrOID) *, csrattrs->attrs));
  for (i = 0; i < num; i++) {
    AttrOrOID *ao = sk_value(
        CHECKED_CAST(_STACK *, const STACK_OF(AttrOrOID) *, csrattrs->attrs),
        i);
    switch (ao->type) {
    case 0:
...
static int generate_csr(struct hs20_osu_client *ctx, char *key_pem,
  ...
  if (old_cert) {
    ...
    if (cert == NULL)
      goto fail;
    res = X509_REQ_set_subject_name(req, X509_get_subject_name(cert));
    X509_free(cert);
    if (!res)
      goto fail;
  } else {
  ...
  ex = X509V3_EXT_nconf_nid(ctmp, NULL, NID_basic_constraints, "CA:FALSE");
  if (ex == NULL || !sk_X509_EXTENSION_push(exts, ex))
    goto fail;
...
int est_build_csr(struct hs20_osu_client *ctx, const char *url) {
  ...
  os_free(buf);
  if (res < 0) {
    wpa_printf(MSG_INFO, "Failed to download EST csrattrs - assume no extra "
    ...
    attrs = base64_decode((unsigned char *)resp, resp_len, &attrs_len);
    os_free(resp);
  ...
  if (ctx->client_cert_present) {
    os_snprintf(old_cert_buf, sizeof(old_cert_buf), "SP/%s/client-cert.pem",
                ctx->fqdn);
...
int est_simple_enroll(struct hs20_osu_client *ctx, const char *url,
  ...
  resp = http_post(ctx->http, buf, req, "application/pkcs10",
                   "Content-Transfer-Encoding: base64", ctx->ca_fname, user, pw,
                   client_cert, client_key, &resp_len);
  http_ocsp_set(ctx->http,
                (ctx->workarounds & WORKAROUND_OCSP_OPTIONAL) ? 1 : 2);
  ctx->no_osu_cert_validation = 0;
  os_free(buf);
