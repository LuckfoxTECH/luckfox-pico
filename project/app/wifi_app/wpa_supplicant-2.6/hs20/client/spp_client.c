...
static int hs20_spp_validate(struct hs20_osu_client *ctx, xml_node_t *node,
  ...
  if (strcmp(expected_name, name) != 0) {
    wpa_printf(MSG_INFO, "Unexpected SOAP method name '%s' (expected '%s')",
               name, expected_name);
    write_summary(ctx, "Unexpected SOAP method name '%s' (expected '%s')", name,
                  expected_name);
    return -1;
  }
...
static xml_node_t *build_spp_post_dev_data(struct hs20_osu_client *ctx,
  ...
  write_summary(ctx, "Building sppPostDevData requestReason='%s'", reason);
  spp_node =
      xml_node_create_root(ctx->xml, SPP_NS_URI, "spp", &ns, "sppPostDevData");
  if (spp_node == NULL)
    return NULL;
  if (ret_ns)
    *ret_ns = ns;
  ...
  xml_node_create_text(ctx->xml, spp_node, ns, "supportedSPPVersions", "1.0");
  xml_node_create_text(ctx->xml, spp_node, ns, "supportedMOList",
                       URN_HS20_PPS " " URN_OMA_DM_DEVINFO
                                    " " URN_OMA_DM_DEVDETAIL
                                    " " URN_HS20_DEVDETAIL_EXT);

  add_mo_container(ctx->xml, ns, spp_node, URN_OMA_DM_DEVINFO, "devinfo.xml");
...
static int process_update_node(struct hs20_osu_client *ctx, xml_node_t *pps,
  ...
  if (cdata && cdata_end && cdata_end > cdata &&
      cdata < strstr(str, "MgmtTree") && cdata_end > strstr(str, "/MgmtTree")) {
    char *tmp;
    ...
    if (tmp) {
      ...
      wpa_printf(MSG_DEBUG,
                 "[hs20] nodeContainer text with CDATA container removed: '%s'",
                 tmp);
  ...
  if (os_strncasecmp(pos, ctx->fqdn, fqdn_len) != 0 || pos[fqdn_len] != '/') {
    wpa_printf(MSG_INFO, "Do not allow update outside ./Wi-Fi/%s", ctx->fqdn);
    xml_node_free(ctx->xml, unode);
    xml_node_get_attr_value_free(ctx->xml, uri);
    return -1;
  }
  ...
  if (os_strncasecmp(pos, "PerProviderSubscription/", 24) != 0) {
    wpa_printf(MSG_INFO,
               "Do not allow update outside ./Wi-Fi/%s/PerProviderSubscription",
               ctx->fqdn);
    xml_node_free(ctx->xml, unode);
    xml_node_get_attr_value_free(ctx->xml, uri);
    return -1;
...
static xml_node_t *hs20_spp_upload_mo(struct hs20_osu_client *ctx,
                                      xml_node_t *cmd, const char *session_id,
                                      const char *pps_fname) {
  xml_namespace_t *ns;
  ...
  node = build_spp_post_dev_data(ctx, &ns, session_id, "MO upload");
  if (node == NULL)
    return NULL;
...
static int process_spp_user_input_response(struct hs20_osu_client *ctx,
  ...
  if (hs20_add_mo(ctx, add_mo, fname, sizeof(fname)) < 0) {
    ...
    ret = hs20_spp_update_response(ctx, session_id, "Error occurred",
...
static xml_node_t *hs20_spp_user_input_completed(struct hs20_osu_client *ctx,
  ...
  node = build_spp_post_dev_data(ctx, NULL, session_id, "User input completed");
  if (node == NULL)
    return NULL;
  ...
  if (!ret_node) {
    ...
    node =
        build_spp_post_dev_data(ctx, NULL, session_id, "User input completed");
    if (node == NULL)
      return NULL;
    ret_node = soap_send_receive(ctx->http, node);
    if (ret_node == NULL)
...
static xml_node_t *hs20_spp_get_certificate(struct hs20_osu_client *ctx,
  ...
  node = build_spp_post_dev_data(ctx, &ns, session_id,
                                 res == 0 ? "Certificate enrollment completed"
  ...
  debug_dump_node(ctx,
                  "Received response to certificate enrollment "
                  "completed",
                  ret_node);
...
static int hs20_spp_exec(struct hs20_osu_client *ctx, xml_node_t *exec,
  ...
  *ret_node = NULL;
  ...
  if (!cmd) {
    wpa_printf(MSG_INFO, "exec command element not found (cmd=%p)", cmd);
    free(id);
    return -1;
  }
  ...
  if (strcasecmp(name, "launchBrowserToURI") == 0) {
    ...
    if (res > 0) {
      wpa_printf(
          MSG_INFO,
          "User response in browser completed successfully - sessionid='%s'",
          id);
      ...
      *ret_node = hs20_spp_user_input_completed(ctx, id);
      ...
      hs20_spp_update_response(ctx, id, "Error occurred", "Other");
      free(id);
      return -1;
    }
    return 0;
  ...
  if (strcasecmp(name, "uploadMO") == 0) {
    if (pps_fname == NULL)
      return -1;
    *ret_node = hs20_spp_upload_mo(ctx, cmd, id, pps_fname);
    free(id);
    return *ret_node ? 0 : -1;
  }

  if (strcasecmp(name, "getCertificate") == 0) {
    *ret_node = hs20_spp_get_certificate(ctx, cmd, id, pps_fname);
    free(id);
    return *ret_node ? 0 : -1;
  }
...
static void process_spp_post_dev_data_response(struct hs20_osu_client *ctx,
  ...
  write_summary(ctx, "Received sppPostDevDataResponse sppStatus='%s'", status);
  ...
  wpa_printf(MSG_INFO,
             "[hs20] sppPostDevDataResponse - sppStatus: '%s'  sessionID: '%s'",
             status, session_id);
  ...
  if (use == SPP_SUBSCRIPTION_REMEDIATION &&
      strcasecmp(status, "Remediation complete, request sppUpdateResponse") ==
          0) {
    int res, ret;
    if (!update && !no_mo) {
    ...
    ret = hs20_spp_update_response(
        ctx, session_id, res < 0 ? "Error occurred" : "OK",
  ...
  if (use == SPP_SUBSCRIPTION_REMEDIATION &&
      strcasecmp(status, "Exchange complete, release TLS connection") == 0) {
    if (!no_mo) {
  ...
  if (use == SPP_POLICY_UPDATE &&
      strcasecmp(status, "Update complete, request sppUpdateResponse") == 0) {
    int res, ret;
  ...
  if (strcasecmp(status, "OK") == 0) {
    ...
    res = hs20_spp_exec(ctx, exec, session_id, pps_fname, pps, &ret);
    /* xml_node_free(ctx->xml, node); */
    node = NULL;
    if (res == 0 && ret)
      process_spp_post_dev_data_response(ctx, use, ret, pps_fname, pps);
    goto out;
  }
  ...
  if (strcasecmp(status, "Error occurred") == 0) {
    ...
    if (err)
      code = xml_node_get_attr_value(ctx->xml, err, "errorCode");
    wpa_printf(MSG_INFO, "Error occurred - errorCode=%s", code ? code : "N/A");
...
static int spp_post_dev_data(struct hs20_osu_client *ctx,
                             enum spp_post_dev_data_use use, const char *reason,
                             const char *pps_fname, xml_node_t *pps) {
  xml_node_t *payload;
  xml_node_t *ret_node;
  ...
  process_spp_post_dev_data_response(ctx, use, ret_node, pps_fname, pps);
  return 0;
}

void spp_sub_rem(struct hs20_osu_client *ctx, const char *address,
                 const char *pps_fname, const char *client_cert,
                 const char *client_key, const char *cred_username,
                 const char *cred_password, xml_node_t *pps) {
...
static int process_spp_exchange_complete(struct hs20_osu_client *ctx,
  ...
  write_summary(ctx, "Received sppExchangeComplete sppStatus='%s'", status);
  ...
  wpa_printf(MSG_INFO, "[hs20] sppStatus: '%s'  sessionID: '%s'", status,
             session_id);
  xml_node_get_attr_value_free(ctx->xml, session_id);

  if (strcasecmp(status, "Exchange complete, release TLS connection") == 0) {
    xml_node_get_attr_value_free(ctx->xml, status);
    return 0;
  }
...
static xml_node_t *build_spp_update_response(struct hs20_osu_client *ctx,
  ...
  if (error_code) {
    ...
    if (node)
      xml_node_add_attr(ctx->xml, node, NULL, "errorCode", error_code);
  }
...
static int hs20_spp_update_response(struct hs20_osu_client *ctx,
  ...
  write_summary(ctx,
                "Building sppUpdateResponse sppStatus='%s' error_code='%s'",
                spp_status, error_code);
  node = build_spp_update_response(ctx, session_id, spp_status, error_code);
  if (node == NULL)
    return -1;
  ret_node = soap_send_receive(ctx->http, node);
  if (!ret_node) {
...
void spp_pol_upd(struct hs20_osu_client *ctx, const char *address,
  ...
  if (soap_init_client(ctx->http, address, ctx->ca_fname, cred_username,
                       cred_password, client_cert, client_key) == 0) {
    spp_post_dev_data(ctx, SPP_POLICY_UPDATE, "Policy update", pps_fname, pps);
  }
}
...
int cmd_prov(struct hs20_osu_client *ctx, const char *url) {
  ...
  wpa_printf(MSG_INFO,
             "Credential provisioning requested - URL: %s ca_fname: %s", url,
             ctx->ca_fname ? ctx->ca_fname : "N/A");

  os_free(ctx->server_url);
  ctx->server_url = os_strdup(url);

  if (soap_init_client(ctx->http, url, ctx->ca_fname, NULL, NULL, NULL, NULL) <
      0)
    return -1;
