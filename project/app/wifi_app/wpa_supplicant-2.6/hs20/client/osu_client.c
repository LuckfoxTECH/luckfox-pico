...
int osu_get_certificate(struct hs20_osu_client *ctx, xml_node_t *getcert) {
  ...
  proto = xml_node_get_attr_value(ctx->xml, getcert, "enrollmentProtocol");
  if (!proto)
    return -1;
  ...
  mkdir("Cert", S_IRWXU);
  if (est_load_cacerts(ctx, url) < 0 || est_build_csr(ctx, url) < 0 ||
...
static int download_cert(struct hs20_osu_client *ctx, xml_node_t *params,
  ...
  wpa_printf(MSG_INFO,
             "Downloaded certificate into %s and validated fingerprint", fname);
...
static int cmd_dl_osu_ca(struct hs20_osu_client *ctx, const char *pps_fname,
  ...
  node = get_child_node(ctx->xml, pps, "SubscriptionUpdate/TrustRoot");
  if (node == NULL) {
    wpa_printf(MSG_INFO,
...
static int cmd_dl_polupd_ca(struct hs20_osu_client *ctx, const char *pps_fname,
  ...
  node = get_child_node(ctx->xml, pps, "Policy/PolicyUpdate/TrustRoot");
  if (node == NULL) {
    wpa_printf(MSG_INFO,
...
static int cmd_dl_aaa_ca(struct hs20_osu_client *ctx, const char *pps_fname,
  ...
  node = get_child_node(ctx->xml, pps, "AAAServerTrustRoot");
  if (node == NULL) {
...
static int server_dnsname_suffix_match(struct hs20_osu_client *ctx,
  ...
  for (i = 0; i < ctx->server_dnsname_count; i++) {
    wpa_printf(MSG_INFO, "Checking suffix match against server dNSName %s",
...
int hs20_add_pps_mo(struct hs20_osu_client *ctx, const char *uri,
  ...
  if (pos) {
    if (os_strcasecmp(pos, "/PerProviderSubscription") != 0) {
      wpa_printf(MSG_INFO,
                 "Unsupported location for addMO to add PPS MO (extra "
                 "directory): '%s'",
                 uri);
      write_result(ctx,
                   "Unsupported location for addMO to "
                   "add PPS MO (extra directory): '%s'",
                   uri);
      free(fqdn);
      return -1;
    }
  ...
  if (!server_dnsname_suffix_match(ctx, fqdn)) {
    wpa_printf(MSG_INFO,
               "FQDN '%s' for new PPS MO did not have suffix match with "
               "server's dNSName values, count: %d",
               fqdn, (int)ctx->server_dnsname_count);
    write_result(ctx,
                 "FQDN '%s' for new PPS MO did not have suffix match with "
                 "server's dNSName values",
                 fqdn);
    free(fqdn);
    return -1;
  }
  ...
  if (mkdir(fname, S_IRWXU) < 0) {
    if (errno != EEXIST) {
      int err = errno;
      wpa_printf(MSG_INFO, "mkdir(%s) failed: %s", fname, strerror(err));
      free(fqdn);
      return -1;
    }
  }
  ...
  if (os_file_exists(fname)) {
    wpa_printf(MSG_INFO, "PPS file '%s' exists - reject addMO", fname);
    write_result(ctx, "PPS file '%s' exists - reject addMO", fname);
    free(fqdn);
    return -2;
  }
  ...
  if (os_strcasecmp(name, "PerProviderSubscription") != 0) {
    wpa_printf(MSG_INFO, "[hs20] Unexpected PPS MO root node name '%s'", name);
    free(fqdn);
    return -1;
  }
...
void get_user_pw(struct hs20_osu_client *ctx, xml_node_t *pps,
  ...
  node = get_child_node(ctx->xml, pps, "Credential/UsernamePassword/Username");
  if (node)
    *user = xml_node_get_text(ctx->xml, node);

  node = get_child_node(ctx->xml, pps, "Credential/UsernamePassword/Password");
  if (node)
    *pw = xml_node_get_base64_text(ctx->xml, node, NULL);
...
static void set_pps_cred_policy_prp(struct hs20_osu_client *ctx, int id,
  ...
  if (prio == NULL)
    return;
  wpa_printf(MSG_INFO,
             "- Policy/PreferredRoamingPartnerList/<X+>/Priority = %s", prio);
  priority = atoi(prio);
  ...
  if (txt == NULL)
    goto out;
  wpa_printf(MSG_INFO,
             "- Policy/PreferredRoamingPartnerList/<X+>/FQDN_Match = %s", txt);
  pos = strrchr(txt, ',');
  if (pos == NULL)
    goto out;
  *pos++ = '\0';
...
static void set_pps_cred_policy_min_backhaul(struct hs20_osu_client *ctx,
                                             int id, xml_node_t *min_backhaul) {
  xml_node_t *node;
  ...
  if (dl == NULL && ul == NULL) {
    wpa_printf(MSG_INFO, "Ignore MinBackhaulThreshold without either "
                         "DLBandwidth or ULBandwidth nodes");
    return;
  }
  ...
  if (home) {
    if (dl && set_cred(ctx->ifname, id, "min_dl_bandwidth_home", dl) < 0)
      wpa_printf(MSG_INFO, "Failed to set cred bandwidth limit");
    if (ul && set_cred(ctx->ifname, id, "min_ul_bandwidth_home", ul) < 0)
      wpa_printf(MSG_INFO, "Failed to set cred bandwidth limit");
  } else {
    if (dl && set_cred(ctx->ifname, id, "min_dl_bandwidth_roaming", dl) < 0)
      wpa_printf(MSG_INFO, "Failed to set cred bandwidth limit");
    if (ul && set_cred(ctx->ifname, id, "min_ul_bandwidth_roaming", ul) < 0)
      wpa_printf(MSG_INFO, "Failed to set cred bandwidth limit");
  }
...
static void
set_pps_cred_policy_required_proto_ports(struct hs20_osu_client *ctx, int id,
                                         xml_node_t *node) {
  xml_node_t *child;
...
static void set_pps_cred_policy_max_bss_load(struct hs20_osu_client *ctx,
                                             int id, xml_node_t *node) {
...
static void set_pps_cred_home_sp_network_id(struct hs20_osu_client *ctx, int id,
                                            xml_node_t *node) {
  ...
  if (hessid)
    wpa_printf(MSG_INFO, "- HomeSP/NetworkID/<X+>/HESSID = %s", hessid);
...
static void set_pps_cred_home_sp_icon_url(struct hs20_osu_client *ctx, int id,
                                          xml_node_t *node) {
  char *str = xml_node_get_text(ctx->xml, node);
  if (str == NULL)
    return;
...
static void set_pps_cred_home_sp_oi(struct hs20_osu_client *ctx, int id,
  ...
  xml_node_for_each_child(ctx->xml, child, node) {
    ...
    if (strcasecmp(name, "HomeOI") == 0 && !homeoi) {
      ...
      wpa_printf(MSG_INFO, "- HomeSP/HomeOIList/<X+>/HomeOI = %s", homeoi);
      ...
      wpa_printf(MSG_INFO, "Unknown HomeOIList node '%s'", name);
  }
  ...
  wpa_printf(MSG_INFO, "- HomeSP/HomeOIList/<X+> '%s' required=%d", homeoi,
             required);

  if (required) {
    if (set_cred(ctx->ifname, id, "required_roaming_consortium", homeoi) < 0)
      wpa_printf(MSG_INFO, "Failed to set cred required_roaming_consortium");
  } else {
    if (set_cred_quoted(ctx->ifname, id, "roaming_consortium", homeoi) < 0)
...
static void set_pps_cred_home_sp_other_partner(struct hs20_osu_client *ctx,
  ...
  xml_node_for_each_child(ctx->xml, child, node) {
    ...
    if (os_strcasecmp(name, "FQDN") == 0 && !fqdn) {
      ...
      wpa_printf(MSG_INFO, "- HomeSP/OtherHomePartners/<X+>/FQDN = %s", fqdn);
    } else
      wpa_printf(MSG_INFO, "Unknown OtherHomePartners node '%s'", name);
  }
...
static void
set_pps_cred_home_sp_roaming_consortium_oi(struct hs20_osu_client *ctx, int id,
                                           xml_node_t *node) {
  char *str = xml_node_get_text(ctx->xml, node);
  if (str == NULL)
    return;
...
static void set_pps_cred_home_sp(struct hs20_osu_client *ctx, int id,
  ...
  xml_node_for_each_child(ctx->xml, child, node) {
    ...
    if (os_strcasecmp(name, "NetworkID") == 0)
      ...
      set_pps_cred_home_sp_roaming_consortium_oi(ctx, id, child);
    else
...
static void set_pps_cred_username(struct hs20_osu_client *ctx, int id,
  ...
  if (str == NULL)
    return;
  wpa_printf(MSG_INFO, "- Credential/UsernamePassword/Username = %s", str);
...
static void set_pps_cred_soft_token_app(struct hs20_osu_client *ctx, int id,
  ...
  if (str == NULL)
    return;
  wpa_printf(MSG_INFO, "- Credential/UsernamePassword/SoftTokenApp = %s", str);
  /* not used within wpa_supplicant */
  xml_node_get_text_free(ctx->xml, str);
}
...
static void set_pps_cred_able_to_share(struct hs20_osu_client *ctx, int id,
  ...
  if (str == NULL)
    return;
  wpa_printf(MSG_INFO, "- Credential/UsernamePassword/AbleToShare = %s", str);
  /* not used within wpa_supplicant */
  xml_node_get_text_free(ctx->xml, str);
}
...
static void set_pps_cred_sim(struct hs20_osu_client *ctx, int id,
  ...
  if (set_cred_quoted(ctx->ifname, id, "milenage",
                      ...
                      "cb9cccc4b9258e6dca4760379fb82581:000000000123") < 0)
...
static void set_pps_cred_credential(struct hs20_osu_client *ctx, int id,
  ...
  xml_node_for_each_child(ctx->xml, child, node) {
    ...
    if (os_strcasecmp(name, "CreationDate") == 0)
      ...
      wpa_printf(MSG_INFO, "Unknown Credential node '%s'", name);
  }
}
...
static void set_pps(struct hs20_osu_client *ctx, xml_node_t *pps,
  ...
  xml_node_for_each_child(ctx->xml, child, pps) {
    ...
    if (os_strcasecmp(name, "UpdateIdentifier") == 0) {
      ...
      if (update_identifier) {
        wpa_printf(MSG_INFO, "- UpdateIdentifier = %s", update_identifier);
        break;
      }
    }
  }
  ...
  xml_node_for_each_child(ctx->xml, child, pps) {
    ...
    if (update_identifier &&
        set_cred(ctx->ifname, id, "update_identifier", update_identifier) < 0)
      wpa_printf(MSG_INFO, "Failed to set update_identifier");
    if (set_cred_quoted(ctx->ifname, id, "provisioning_sp", fqdn) < 0)
...
void cmd_set_pps(struct hs20_osu_client *ctx, const char *pps_fname) {
  ...
  wpa_printf(MSG_INFO, "Set PPS MO info to wpa_supplicant - SP FQDN %s", fqdn);
...
static struct osu_data *parse_osu_providers(const char *fname, size_t *count) {
  ...
  while (fgets(buf, sizeof(buf), f)) {
    ...
    if (strncmp(buf, "OSU-PROVIDER ", 13) == 0) {
      ...
      snprintf(last->bssid, sizeof(last->bssid), "%s", buf + 13);
      continue;
    }
    if (!last)
      continue;
    ...
    if (strncmp(buf, "osu_ssid=", 9) == 0) {
      snprintf(last->osu_ssid, sizeof(last->osu_ssid), "%s", buf + 9);
      continue;
    }

    if (os_strncmp(buf, "osu_nai=", 8) == 0) {
      os_snprintf(last->osu_nai, sizeof(last->osu_nai), "%s", buf + 8);
      continue;
    }
    ...
    if (strncmp(buf, "icon=", 5) == 0) {
      ...
      *end = '\0';
      ...
      if (end)
        *end = '\0';
      snprintf(icon->mime_type, sizeof(icon->mime_type), "%s", pos);
      if (!pos)
        continue;
      pos = end + 1;
      ...
      if (end)
        *end = '\0';
      snprintf(icon->filename, sizeof(icon->filename), "%s", pos);
      continue;
    }
  }
...
static int osu_connect(struct hs20_osu_client *ctx, const char *bssid,
                       const char *ssid, const char *url, unsigned int methods,
                       int no_prod_assoc, const char *osu_nai) {
  int id;
  ...
  res = get_wpa_cli_event(mon, "CTRL-EVENT-CONNECTED", buf, sizeof(buf));
  ...
  if (methods & 0x02) {
    ...
    wpa_printf(MSG_DEBUG, "Calling cmd_oma_dm_prov from osu_connect");
...
static int cmd_osu_select(struct hs20_osu_client *ctx, const char *dir,
  ...
  if (osu == NULL) {
    wpa_printf(MSG_INFO, "Could not find any OSU providers from %s", fname);
  ...
  if (friendly_name) {
    for (i = 0; i < osu_count; i++) {
      ...
      for (j = 0; j < last->friendly_name_count; j++) {
        if (os_strcmp(last->friendly_name[j].text, friendly_name) == 0)
          break;
      }
    ...
    if (i == osu_count) {
      wpa_printf(MSG_INFO,
                 "Requested operator friendly name '%s' not found in the list "
                 "of available providers",
                 friendly_name);
      write_summary(ctx,
                    "Requested operator friendly name '%s' not found in the "
  ...
  for (i = 0; i < osu_count; i++) {
    ...
    for (j = 0; j < last->icon_count; j++) {
      fprintf(f, "<img src=\"osu-icon-%d.%s\">\n", last->icon[j].id,
              strcasecmp(last->icon[j].mime_type, "image/png") == 0 ? "png"
                                                                    : "icon");
    }
    fprintf(f, "<td>");
    for (j = 0; j < last->friendly_name_count; j++) {
    ...
    fprintf(f,
            ...
            "methods:%s%s<br>\n"
            "</small></p>\n",
            last->url, last->methods & 0x01 ? " OMA-DM" : "",
...
selected:
  if (ret > 0 && (size_t)ret <= osu_count) {
    ...
    for (j = 0; j < last->friendly_name_count; j++) {
      wpa_printf(MSG_INFO, "FRIENDLY_NAME: [%s]%s", last->friendly_name[j].lang,
                 last->friendly_name[j].text);
      os_strlcpy(ctx->friendly_name[j].lang, last->friendly_name[j].lang,
                 sizeof(ctx->friendly_name[j].lang));
      os_strlcpy(ctx->friendly_name[j].text, last->friendly_name[j].text,
    ...
    for (j = 0; j < last->icon_count; j++) {
      ...
      os_snprintf(
          fname, sizeof(fname), "%s/osu-icon-%d.%s", dir, last->icon[j].id,
          strcasecmp(last->icon[j].mime_type, "image/png") == 0 ? "png"
                                                                : "icon");
      wpa_printf(MSG_INFO, "ICON: %s (%s)", fname, last->icon[j].filename);
      os_strlcpy(ctx->icon_filename[j], last->icon[j].filename,
                 sizeof(ctx->icon_filename[j]));

      data = os_readfile(fname, &data_len);
      if (data) {
        sha256_vector(1, (const u8 **)&data, &data_len, ctx->icon_hash[j]);
        os_free(data);
      }
    }
    ...
    if (connect == 2) {
      if (last->methods & 0x02) {
        wpa_printf(MSG_DEBUG, "Calling cmd_prov from cmd_osu_select");
        ret = cmd_prov(ctx, last->url);
      } else if (last->methods & 0x01) {
        wpa_printf(MSG_DEBUG, "Calling cmd_oma_dm_prov from cmd_osu_select");
        ret = cmd_oma_dm_prov(ctx, last->url);
      } else {
        wpa_printf(MSG_DEBUG, "No supported OSU provisioning method");
        ret = -1;
      }
    } else if (connect)
      ret = osu_connect(ctx, last->bssid, last->osu_ssid, last->url,
                        last->methods, no_prod_assoc, last->osu_nai);
  } else
    ret = -1;
...
static int cmd_signup(struct hs20_osu_client *ctx, int no_prod_assoc,
  ...
  if (mkdir(fname, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 &&
      errno != EEXIST) {
    wpa_printf(MSG_INFO, "mkdir(%s) failed: %s", fname, strerror(errno));
    return -1;
  }
  ...
  res =
      get_wpa_cli_event(mon, "OSU provider fetch completed", buf, sizeof(buf));
...
static int cmd_sub_rem(struct hs20_osu_client *ctx, const char *address,
  ...
  if (!pps_fname) {
    ...
    if (ctx->fqdn == NULL)
      return -1;
    wpa_printf(MSG_INFO, "Home SP FQDN for current credential: %s", buf);
    os_snprintf(pps_fname_buf, sizeof(pps_fname_buf), "SP/%s/pps.xml",
                ctx->fqdn);
    pps_fname = pps_fname_buf;

    os_snprintf(ca_fname_buf, sizeof(ca_fname_buf), "SP/%s/ca.pem", ctx->fqdn);
  ...
  node = get_child_node(ctx->xml, pps, "SubscriptionUpdate/UpdateMethod");
  if (node) {
    char *tmp;
    tmp = xml_node_get_text(ctx->xml, node);
  ...
  get_user_pw(ctx, pps, "SubscriptionUpdate/UsernamePassword", &cred_username,
              &cred_password);
  if (cred_username)
  ...
  if (cred_username == NULL && cred_password == NULL &&
    ...
    os_snprintf(client_key_buf, sizeof(client_key_buf), "SP/%s/client-key.pem",
                ctx->fqdn);
  ...
  if (node) {
    ...
    if (sub_rem_uri && (!address || os_strcmp(address, sub_rem_uri) != 0)) {
  ...
  if (spp)
    spp_sub_rem(ctx, address, pps_fname, client_cert, client_key, cred_username,
                cred_password, pps);
  else
    oma_dm_sub_rem(ctx, address, pps_fname, client_cert, client_key,
...
static int cmd_pol_upd(struct hs20_osu_client *ctx, const char *address,
  ...
  if (!pps_fname) {
    ...
    os_snprintf(ca_fname_buf, sizeof(ca_fname_buf), "SP/%s/ca.pem", buf);
    ca_fname = ca_fname_buf;
  }
  ...
  node = get_child_node(ctx->xml, pps, "Policy/PolicyUpdate/UpdateMethod");
  if (node) {
    char *tmp;
    tmp = xml_node_get_text(ctx->xml, node);
    if (tmp && os_strcasecmp(tmp, "OMA-DM-ClientInitiated") == 0)
  ...
  get_user_pw(ctx, pps, "Policy/PolicyUpdate/UsernamePassword", &cred_username,
              &cred_password);
  if (cred_username)
    wpa_printf(MSG_INFO, "Using username: %s", cred_username);
  if (cred_password)
  ...
  if (spp)
    spp_pol_upd(ctx, address, pps_fname, client_cert, client_key, cred_username,
                cred_password, pps);
  else
    oma_dm_pol_upd(ctx, address, pps_fname, client_cert, client_key,
                   cred_username, cred_password, pps);
...
static int osu_cert_cb(void *_ctx, struct http_cert *cert) {
  ...
  if (host && !found) {
    wpa_printf(MSG_INFO,
               "Server name from URL (%s) did not match any dNSName - abort "
               "connection",
               host);
    write_result(ctx,
                 "Server name from URL (%s) did not match any dNSName - abort "
  ...
  for (i = 0; i < cert->num_othername; i++) {
    if (os_strcmp(cert->othername[i].oid, "1.3.6.1.4.1.40808.1.1.1") == 0) {
      wpa_hexdump_ascii(MSG_INFO, "id-wfa-hotspot-friendlyName",
                        cert->othername[i].data, cert->othername[i].len);
    }
  }

  for (j = 0; !ctx->no_osu_cert_validation && j < ctx->friendly_name_count;
       j++) {
    int found = 0;
    for (i = 0; i < cert->num_othername; i++) {
      if (os_strcmp(cert->othername[i].oid, "1.3.6.1.4.1.40808.1.1.1") != 0)
    ...
    if (!found) {
      wpa_printf(MSG_INFO, "No friendly name match found for '[%s]%s'",
                 ctx->friendly_name[j].lang, ctx->friendly_name[j].text);
  ...
  for (i = 0; i < cert->num_logo; i++) {
    ...
    wpa_printf(MSG_INFO, "logo hash alg %s uri '%s'", logo->alg_oid, logo->uri);
    wpa_hexdump_ascii(MSG_INFO, "hashValue", logo->hash, logo->hash_len);
  }
  ...
  for (j = 0; !ctx->no_osu_cert_validation && j < ctx->icon_count; j++) {
    ...
    wpa_printf(MSG_INFO, "[%i] Looking for icon file name '%s' match", j, name);
    for (i = 0; i < cert->num_logo; i++) {
      struct http_logo *logo = &cert->logo[i];
      size_t uri_len = os_strlen(logo->uri);
      char *pos;

      wpa_printf(MSG_INFO, "[%i] Comparing to '%s' uri_len=%d name_len=%d", i,
    ...
    if (!found) {
      wpa_printf(MSG_INFO, "No icon filename match found for '%s'", name);
      write_result(ctx, "No icon filename match found for '%s'", name);
      return -1;
    }
  }
  ...
  for (j = 0; !ctx->no_osu_cert_validation && j < ctx->icon_count; j++) {
    ...
    for (i = 0; i < cert->num_logo; i++) {
      ...
      if (logo->hash_len != 32) {
        wpa_printf(MSG_INFO,
                   "[%i][%i] Icon hash length invalid (should be 32): %d", j, i,
                   (int)logo->hash_len);
        continue;
      }
      ...
      wpa_printf(MSG_DEBUG, "[%u][%u] Icon hash did not match", j, i);
      wpa_hexdump_ascii(MSG_DEBUG, "logo->hash", logo->hash, 32);
      wpa_hexdump_ascii(MSG_DEBUG, "ctx->icon_hash[j]", ctx->icon_hash[j], 32);
    }
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[optind], "to_tnds") == 0) {
    ...
    cmd_to_tnds(&ctx, argv[optind + 1], argv[optind + 2],
                argc > optind + 3 ? argv[optind + 3] : NULL, 0);
    ...
    cmd_to_tnds(&ctx, argv[optind + 1], argv[optind + 2],
                argc > optind + 3 ? argv[optind + 3] : NULL, 1);
    ...
    ret = cmd_pol_upd(&ctx, argc > optind + 1 ? argv[optind + 1] : NULL,
                      argc > optind + 2 ? argv[optind + 2] : NULL,
                      argc > optind + 3 ? argv[optind + 3] : NULL);
    ...
    wpa_printf(MSG_INFO, "Launch web browser to URL %s", argv[optind + 1]);
  ...
  deinit_ctx(&ctx);
  wpa_printf(MSG_DEBUG, "===[hs20-osu-client END ]======================");
