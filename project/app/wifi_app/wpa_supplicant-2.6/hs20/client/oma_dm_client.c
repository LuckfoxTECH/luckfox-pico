...
#define DM_HS20_POLICY_UPDATE "org.wi-fi.hotspot2dot0.PolicyUpdate"
...
static xml_node_t *oma_dm_build_hdr(struct hs20_osu_client *ctx,
  ...
  syncml =
      xml_node_create_root(ctx->xml, "SYNCML:SYNCML1.2", NULL, &ns, "SyncML");
...
static xml_node_t *add_status(struct hs20_osu_client *ctx, xml_node_t *parent,
  ...
  xml_node_create_text(ctx->xml, node, NULL, "Data", int2str(data));
  if (targetref) {
    xml_node_create_text(ctx->xml, node, NULL, "TargetRef", targetref);
  }
...
static void add_item(struct hs20_osu_client *ctx, xml_node_t *parent,
  ...
  xml_node_create_text_ns(ctx->xml, node, "syncml:metinf", "Format", "Chr");
...
static void oma_dm_add_hs20_generic_alert(struct hs20_osu_client *ctx,
  ...
  xml_node_create_text_ns(ctx->xml, node, "syncml:metinf", "Format", "xml");
  xml_node_create_text(ctx->xml, item, NULL, "Data", data);
}

static xml_node_t *build_oma_dm_1(struct hs20_osu_client *ctx, const char *url,
                                  int msgid, const char *oper) {
...
static xml_node_t *build_oma_dm_1_sub_prov(struct hs20_osu_client *ctx,
  ...
  syncml = build_oma_dm_1(ctx, url, msgid, DM_HS20_SUBSCRIPTION_PROVISIONING);
  if (syncml)
...
static xml_node_t *build_oma_dm_1_sub_rem(struct hs20_osu_client *ctx,
  ...
  syncml = build_oma_dm_1(ctx, url, msgid, DM_HS20_SUBSCRIPTION_REMEDIATION);
  if (syncml)
...
static int oma_dm_exec_get_cert(struct hs20_osu_client *ctx, xml_node_t *exec) {
  ...
  if (name == NULL || os_strcasecmp(name, "getCertificate") != 0) {
    wpa_printf(MSG_INFO, "Unexpected getCertificate node name '%s'", name);
    return DM_RESP_BAD_REQUEST;
  }
...
static int oma_dm_run_add(struct hs20_osu_client *ctx, const char *locuri,
  ...
  if (os_strncasecmp(pos, ctx->fqdn, fqdn_len) != 0 || pos[fqdn_len] != '/') {
    wpa_printf(MSG_INFO, "Do not allow Add outside ./Wi-Fi/%s", ctx->fqdn);
    return DM_RESP_PERMISSION_DENIED;
  }
  pos += fqdn_len + 1;
  ...
  if (node) {
    ...
    use_tnds = node && os_strstr(type, "application/vnd.syncml.dmtnds+xml");
  }
  ...
  if (use_tnds) {
    ...
    xml_node_get_text_free(ctx->xml, data);
    if (tnds == NULL) {
      wpa_printf(MSG_INFO, "Could not parse Add/Item/Data text");
      os_free(uri);
      return DM_RESP_BAD_REQUEST;
    }
...
static int oma_dm_add(struct hs20_osu_client *ctx, xml_node_t *add,
  ...
  xml_node_get_text_free(ctx->xml, locuri);
  if (ret < 0)
    return ret == -2 ? DM_RESP_ALREADY_EXISTS : DM_RESP_COMMAND_FAILED;

  if (ctx->no_reconnect == 2) {
    os_snprintf(ctx->pps_fname, sizeof(ctx->pps_fname), "%s", fname);
    ctx->pps_cred_set = 1;
    return DM_RESP_OK;
  }
...
static int oma_dm_replace(struct hs20_osu_client *ctx, xml_node_t *replace,
  ...
  fqdn_len = os_strlen(ctx->fqdn);
  if (os_strncasecmp(pos, ctx->fqdn, fqdn_len) != 0 || pos[fqdn_len] != '/') {
    wpa_printf(MSG_INFO, "Do not allow Replace outside ./Wi-Fi/%s", ctx->fqdn);
    os_free(locuri);
    return DM_RESP_PERMISSION_DENIED;
  }
  pos += fqdn_len + 1;
  ...
  if (use_tnds) {
    ...
    xml_node_get_text_free(ctx->xml, data);
    if (tnds == NULL) {
      wpa_printf(MSG_INFO, "Could not parse Replace/Item/Data text");
      os_free(locuri);
      return DM_RESP_BAD_REQUEST;
    }
...
static int oma_dm_get(struct hs20_osu_client *ctx, xml_node_t *get,
  ...
  *value = NULL;
  ...
  fqdn_len = os_strlen(ctx->fqdn);
  if (os_strncasecmp(pos, ctx->fqdn, fqdn_len) != 0 || pos[fqdn_len] != '/') {
    wpa_printf(MSG_INFO, "Do not allow Get outside ./Wi-Fi/%s", ctx->fqdn);
    os_free(locuri);
    return DM_RESP_PERMISSION_DENIED;
  }
  pos += fqdn_len + 1;
...
static xml_node_t *oma_dm_send_recv(struct hs20_osu_client *ctx,
                                    ...
                                    const char *ext_hdr, const char *username,
                                    const char *password,
  ...
  res = http_post(ctx->http, url, str, "application/vnd.syncml.dm+xml", ext_hdr,
                  ctx->ca_fname, username, password, client_cert, client_key,
                  NULL);
  os_free(str);
  os_free(resp_uri);
  resp_uri = NULL;
...
static xml_node_t *oma_dm_process(struct hs20_osu_client *ctx, const char *url,
                                  ...
                                  char **ret_resp_uri, xml_node_t *pps,
                                  const char *pps_fname) {
  ...
  *ret_resp_uri = NULL;
  ...
  xml_node_for_each_child(ctx->xml, child, body) {
    ...
    name = xml_node_get_localname(ctx->xml, child);
    wpa_printf(MSG_INFO, "SyncBody CmdID=%d - %s", server_cmdid, name);
    if (os_strcasecmp(name, "Exec") == 0) {
      ...
      add_status(ctx, syncbody, server_msgid, server_cmdid, cmdid, name, res,
                 locuri);
      os_free(locuri);
      resp_needed = 1;
      ...
      if (res == DM_RESP_OK && value) {
        cmdid++;
        add_results(ctx, syncbody, server_msgid, server_cmdid, cmdid, locuri,
                    value);
      }
      os_free(locuri);
      ...
      locuri = oma_dm_get_target_locuri(ctx, child);
      add_status(ctx, syncbody, server_msgid, server_cmdid, cmdid, name,
                 DM_RESP_COMMAND_NOT_IMPLEMENTED, locuri);
      os_free(locuri);
      resp_needed = 1;
    }
  }
...
int cmd_oma_dm_prov(struct hs20_osu_client *ctx, const char *url) {
  ...
  while (syncml) {
    resp = oma_dm_send_recv(ctx, resp_uri ? resp_uri : url, syncml, NULL, NULL,
                            NULL, NULL, NULL);
    if (resp == NULL)
      return -1;

    msgid++;
    syncml = oma_dm_process(ctx, url, resp, msgid, &resp_uri, NULL, NULL);
...
void oma_dm_pol_upd(struct hs20_osu_client *ctx, const char *address,
  ...
  while (syncml) {
    resp =
        oma_dm_send_recv(ctx, resp_uri ? resp_uri : address, syncml, NULL,
                         cred_username, cred_password, client_cert, client_key);
    if (resp == NULL)
      return;

    msgid++;
    syncml =
        oma_dm_process(ctx, address, resp, msgid, &resp_uri, pps, pps_fname);
    xml_node_free(ctx->xml, resp);
  }
  ...
  if (ctx->pps_updated) {
    wpa_printf(MSG_INFO,
               "Update wpa_supplicant credential based on updated PPS MO");
    write_summary(ctx, "Update wpa_supplicant credential based on updated PPS "
                       "MO and request connection");
...
void oma_dm_sub_rem(struct hs20_osu_client *ctx, const char *address,
  ...
  wpa_printf(MSG_INFO, "Update wpa_supplicant credential based on updated PPS "
                       "MO and request reconnection");
  write_summary(ctx, "Update wpa_supplicant credential based on updated PPS MO "
...
void cmd_oma_dm_add(struct hs20_osu_client *ctx, const char *pps_fname,
  ...
  if (pps == NULL) {
    wpa_printf(MSG_INFO, "PPS file %s could not be parsed", pps_fname);
    return;
  }

  add = node_from_file(ctx->xml, add_fname);
  if (add == NULL) {
    wpa_printf(MSG_INFO, "Add file %s could not be parsed", add_fname);
...
void cmd_oma_dm_replace(struct hs20_osu_client *ctx, const char *pps_fname,
  ...
  if (replace == NULL) {
    wpa_printf(MSG_INFO, "Replace file %s could not be parsed", replace_fname);
    xml_node_free(ctx->xml, pps);
    return;
  }
