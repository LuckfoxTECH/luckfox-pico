...
static struct wpabuf *p2p_group_build_beacon_ie(struct p2p_group *group) {
  ...
  if (group->p2p->vendor_elem &&
      group->p2p->vendor_elem[VENDOR_ELEM_BEACON_P2P_GO])
    wpabuf_put_buf(ie, group->p2p->vendor_elem[VENDOR_ELEM_BEACON_P2P_GO]);
...
static struct wpabuf *wifi_display_build_go_ie(struct p2p_group *group) {
  ...
  if (group->p2p->wfd_assoc_bssid)
    wpabuf_put_buf(wfd_subelems, group->p2p->wfd_assoc_bssid);
  if (group->p2p->wfd_coupled_sink_info)
    wpabuf_put_buf(wfd_subelems, group->p2p->wfd_coupled_sink_info);
  ...
  if (count == 0) {
    ...
    WPA_PUT_BE16(len, (u8 *)wpabuf_put(wfd_subelems, 0) - len - 2);
    p2p_dbg(group->p2p, "WFD: WFD Session Info: %u descriptors", count);
  }
...
void p2p_buf_add_group_info(struct p2p_group *group, struct wpabuf *buf,
  ...
  WPA_PUT_LE16(group_info + 1, (u8 *)wpabuf_put(buf, 0) - group_info - 3);
}
...
void p2p_group_update_ies(struct p2p_group *group) {
  ...
  if (group->beacon_update) {
    ...
    wpa_hexdump_buf(MSG_MSGDUMP, "P2P: Update GO Beacon P2P IE", beacon_ie);
  } else
    beacon_ie = NULL;
...
static struct wpabuf *p2p_build_client_info(const u8 *addr,
  ...
  if (p2p_parse_p2p_ie(p2p_ie, &msg) || msg.capability == NULL ||
      msg.p2p_device_info == NULL)
    return NULL;
  ...
  *dev_capab = msg.capability[0];
   ...
   * Device Info
   */
  wpabuf_put_data(buf, spos + ETH_ALEN, msg.p2p_device_info_len - ETH_ALEN);
...
int p2p_group_notif_assoc(struct p2p_group *group, const u8 *addr, const u8 *ie,
  ...
  if (m->p2p_ie) {
    m->client_info =
        p2p_build_client_info(addr, m->p2p_ie, &m->dev_capab, m->dev_addr);
  }
#ifdef CONFIG_WIFI_DISPLAY
  ...
  p2p_dbg(group->p2p,
          ...
          m->client_info ? 1 : 0, group->num_members, group->cfg->max_clients);
  if (group->num_members == group->cfg->max_clients)
    group->beacon_update = 1;
  p2p_group_update_ies(group);
...
struct wpabuf *p2p_group_assoc_resp_ie(struct p2p_group *group, u8 status) {
  ...
  if (group->p2p->vendor_elem &&
      group->p2p->vendor_elem[VENDOR_ELEM_P2P_ASSOC_RESP])
    wpabuf_put_buf(resp, group->p2p->vendor_elem[VENDOR_ELEM_P2P_ASSOC_RESP]);
...
void p2p_group_notif_disassoc(struct p2p_group *group, const u8 *addr) {
  if (p2p_group_remove_member(group, addr)) {
    p2p_dbg(group->p2p,
            "Remove client " MACSTR " from group; num_members=%u/%u",
            MAC2STR(addr), group->num_members, group->cfg->max_clients);
...
static struct p2p_group_member *
p2p_group_get_client_iface(struct p2p_group *group, const u8 *interface_addr) {
  struct p2p_group_member *m;
...
int p2p_group_go_discover(struct p2p_group *group, const u8 *dev_id,
  ...
  if (m == NULL || m->client_info == NULL) {
    p2p_dbg(group->p2p, "Requested client was not in this group " MACSTR,
  ...
  p2p_dbg(group->p2p,
          "Schedule GO Discoverability Request to be sent to " MACSTR,
          MAC2STR(dev_id));
  ...
  if (p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, m->addr,
                            ...
                            group->cfg->interface_addr, wpabuf_head(req),
...
u8 p2p_group_presence_req(struct p2p_group *group,
                          const u8 *client_interface_addr, const u8 *noa,
                          size_t noa_len) {
  struct p2p_group_member *m;
  u8 curr_noa[50];
  int curr_noa_len;
  ...
  if (group->p2p->cfg->get_noa)
    curr_noa_len = group->p2p->cfg->get_noa(group->p2p->cfg->cb_ctx,
                                            group->cfg->interface_addr,
  ...
  if (curr_noa_len < 0)
    ...
    wpa_hexdump(MSG_DEBUG, "P2P: Current NoA", curr_noa, curr_noa_len);
...
int p2p_group_get_freq(struct p2p_group *group) { return group->cfg->freq; }
...

{
  ...
  p2p_channels_union(&intersect, &group->p2p->cfg->channels, &intersect);

  p2p_channels_dump(group->p2p, "Group common freqs before iterating members",
                    &intersect);
