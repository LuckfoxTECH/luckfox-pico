...
void p2p_dev_disc_req_cb(struct p2p_data *p2p, int success) {
  ...
  p2p_dbg(p2p,
          "GO acknowledged Device Discoverability Request - wait for response");
  /*
...
int p2p_send_dev_disc_req(struct p2p_data *p2p, struct p2p_device *dev) {
  ...
  if (go == NULL || dev->oper_freq <= 0) {
    p2p_dbg(p2p, "Could not find peer entry for GO and frequency to send "
  ...
  p2p_dbg(p2p,
          "Sending Device Discoverability Request to GO " MACSTR
          " for client " MACSTR,
          MAC2STR(go->info.p2p_device_addr),
          MAC2STR(dev->info.p2p_device_addr));

  p2p->pending_client_disc_go = go;
  os_memcpy(p2p->pending_client_disc_addr, dev->info.p2p_device_addr, ETH_ALEN);
...
static void p2p_send_dev_disc_resp(struct p2p_data *p2p, u8 dialog_token,
  ...
  p2p_dbg(p2p,
          "Sending Device Discoverability Response to " MACSTR
          " (status %u freq %d)",
          MAC2STR(addr), status, freq);

  p2p->pending_action_state = P2P_PENDING_DEV_DISC_RESPONSE;
  if (p2p_send_action(p2p, freq, addr, p2p->cfg->dev_addr, p2p->cfg->dev_addr,
...
void p2p_process_dev_disc_req(struct p2p_data *p2p, const u8 *sa,
  ...
  p2p_dbg(p2p,
          "Received Device Discoverability Request from " MACSTR " (freq=%d)",
          MAC2STR(sa), rx_freq);
  ...
  if (msg.dialog_token == 0) {
    p2p_dbg(p2p, "Invalid Dialog Token 0 (must be nonzero) in Device "
  ...
  if (msg.device_id == NULL) {
    p2p_dbg(
        p2p,
        "P2P Device ID attribute missing from Device Discoverability Request");
    p2p_send_dev_disc_resp(p2p, msg.dialog_token, sa, rx_freq,
                           P2P_SC_FAIL_INVALID_PARAMS);
    p2p_parse_free(&msg);
    return;
  ...
  for (g = 0; g < p2p->num_groups; g++) {
    if (p2p_group_go_discover(p2p->groups[g], msg.device_id, sa, rx_freq) ==
        0) {
      p2p_dbg(p2p,
              "Scheduled GO Discoverability Request for the target device");
      /*
  ...
  p2p_dbg(p2p, "Requested client was not found in any group or did not support "
...
void p2p_process_dev_disc_resp(struct p2p_data *p2p, const u8 *sa,
  ...
  if (go == NULL || os_memcmp(sa, go->info.p2p_device_addr, ETH_ALEN) != 0) {
  ...
  if (msg.dialog_token != go->dialog_token) {
    p2p_dbg(p2p,
            "Ignore Device Discoverability Response with unexpected dialog "
            "token %u (expected %u)",
  ...
  if (p2p->go_neg_peer == NULL ||
      ...
      os_memcmp(p2p->go_neg_peer->member_in_go_dev, go->info.p2p_device_addr,
                ETH_ALEN) != 0) {
    p2p_dbg(
        p2p,
        "No pending operation with the client discoverability peer anymore");
    return;
  }
...
void p2p_go_disc_req_cb(struct p2p_data *p2p, int success) {
  p2p_dbg(p2p, "GO Discoverability Request TX callback: success=%d", success);
  p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
  ...
  p2p_send_dev_disc_resp(p2p, p2p->pending_dev_disc_dialog_token,
                         p2p->pending_dev_disc_addr, p2p->pending_dev_disc_freq,
...
void p2p_process_go_disc_req(struct p2p_data *p2p, const u8 *da, const u8 *sa,
  ...
  tu = 100;
  if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, rx_freq, 1024 * tu / 1000, ies) <
      0) {
