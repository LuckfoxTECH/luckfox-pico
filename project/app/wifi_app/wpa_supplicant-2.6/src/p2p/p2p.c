...
void p2p_expire_peers(struct p2p_data *p2p) {
  ...
  dl_list_for_each_safe(dev, n, &p2p->devices, struct p2p_device, list) {
    ...
    if (p2p->cfg->go_connected &&
        p2p->cfg->go_connected(p2p->cfg->cb_ctx, dev->info.p2p_device_addr)) {
      /*
...
void p2p_set_state(struct p2p_data *p2p, int new_state) {
  p2p_dbg(p2p, "State %s -> %s", p2p_state_txt(p2p->state),
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc) {
  ...
  if (os_get_random((u8 *)&r, sizeof(r)) < 0)
    r = 0;
  tu = (r % ((p2p->max_disc_int - p2p->min_disc_int) + 1) + p2p->min_disc_int) *
       100;
  ...
  if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, freq, 1024 * tu / 1000, ies) <
      0) {
...
static void p2p_copy_client_info(struct p2p_device *dev,
  ...
  p2p_copy_filter_devname(dev->info.device_name, sizeof(dev->info.device_name),
...
static int p2p_add_group_clients(struct p2p_data *p2p, const u8 *go_dev_addr,
   ...
   */
  dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
    if (os_memcmp(dev->member_in_go_iface, go_interface_addr, ETH_ALEN) == 0) {
  ...
  for (c = 0; c < info.num_clients; c++) {
    ...
    if (os_memcmp(cli->p2p_device_addr, p2p->cfg->dev_addr, ETH_ALEN) == 0)
    ...
    if (dev) {
      if (dev->flags & (P2P_DEV_GROUP_CLIENT_ONLY | P2P_DEV_PROBE_REQ_ONLY)) {
        ...
        dev->info.dev_capab |=
            cli->dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
      }
    ...
    os_memcpy(dev->interface_addr, cli->p2p_interface_addr, ETH_ALEN);
    os_memcpy(&dev->last_seen, rx_time, sizeof(struct os_reltime));
    os_memcpy(dev->member_in_go_dev, go_dev_addr, ETH_ALEN);
    os_memcpy(dev->member_in_go_iface, go_interface_addr, ETH_ALEN);
...
static void p2p_copy_wps_info(struct p2p_data *p2p, struct p2p_device *dev,
  ...
  if (msg->manufacturer &&
    ...
    os_memset(dev->info.manufacturer, 0, sizeof(dev->info.manufacturer));
    os_memcpy(dev->info.manufacturer, msg->manufacturer, msg->manufacturer_len);
  }

  if (msg->model_name && msg->model_name_len < sizeof(dev->info.model_name)) {
    os_memset(dev->info.model_name, 0, sizeof(dev->info.model_name));
    os_memcpy(dev->info.model_name, msg->model_name, msg->model_name_len);
  }

  if (msg->model_number &&
      msg->model_number_len < sizeof(dev->info.model_number)) {
    os_memset(dev->info.model_number, 0, sizeof(dev->info.model_number));
    os_memcpy(dev->info.model_number, msg->model_number, msg->model_number_len);
  }

  if (msg->serial_number &&
      msg->serial_number_len < sizeof(dev->info.serial_number)) {
    os_memset(dev->info.serial_number, 0, sizeof(dev->info.serial_number));
  ...
  if (msg->wps_sec_dev_type_list) {
    os_memcpy(dev->info.wps_sec_dev_type_list, msg->wps_sec_dev_type_list,
              msg->wps_sec_dev_type_list_len);
    dev->info.wps_sec_dev_type_list_len = msg->wps_sec_dev_type_list_len;
  }
  ...
  if (msg->capability) {
    ...
    dev->info.dev_capab &= P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
    dev->info.dev_capab |=
        msg->capability[0] & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
    dev->info.group_capab = msg->capability[1];
  }

  if (msg->ext_listen_timing) {
    dev->ext_listen_period = WPA_GET_LE16(msg->ext_listen_timing);
    dev->ext_listen_interval = WPA_GET_LE16(msg->ext_listen_timing + 2);
  }

  if (!probe_req) {
    u16 new_config_methods;
    new_config_methods =
        msg->config_methods ? msg->config_methods : msg->wps_config_methods;
    if (new_config_methods && dev->info.config_methods != new_config_methods) {
      p2p_dbg(p2p, "Update peer " MACSTR " config_methods 0x%x -> 0x%x",
              MAC2STR(dev->info.p2p_device_addr), dev->info.config_methods,
              new_config_methods);
...
static void p2p_update_peer_vendor_elems(struct p2p_device *dev, const u8 *ies,
  ...
  for (pos = ies; end - pos > 1; pos += len) {
    ...
    if (len >= 4) {
      ...
      if (type == WPA_IE_VENDOR_TYPE || type == WMM_IE_VENDOR_TYPE ||
          type == WPS_IE_VENDOR_TYPE || type == P2P_IE_VENDOR_TYPE ||
...
static int p2p_compare_wfd_info(struct p2p_device *dev,
  ...
  if (dev->info.wfd_subelems && msg->wfd_subelems) {
    ...
    return os_memcmp(dev->info.wfd_subelems->buf, msg->wfd_subelems->buf,
...
int p2p_add_device(struct p2p_data *p2p, const u8 *addr, int freq,
  ...
  if (!is_zero_ether_addr(p2p->peer_filter) &&
    ...
    p2p_dbg(p2p, "Do not add peer filter for " MACSTR " due to peer filter",
   ...
   */
  if (dev->last_seen.sec > 0 && os_reltime_before(rx_time, &dev->last_seen) &&
      (!(dev->flags & P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT) ||
       os_reltime_expired(&dev->last_seen, rx_time,
                          P2P_DEV_GROUP_CLIENT_RESP_THRESHOLD))) {
    p2p_dbg(p2p,
            "Do not update peer entry based on old frame (rx_time=%u.%06u "
            "last_seen=%u.%06u flags=0x%x)",
            (unsigned int)rx_time->sec, (unsigned int)rx_time->usec,
            (unsigned int)dev->last_seen.sec, (unsigned int)dev->last_seen.usec,
            dev->flags);
    p2p_parse_free(&msg);
    return -1;
  }
  ...
  if (msg.ssid && msg.ssid[1] <= sizeof(dev->oper_ssid) &&
  ...
  if (msg.adv_service_instance && msg.adv_service_instance_len)
    dev->info.p2ps_instance = wpabuf_alloc_copy(msg.adv_service_instance,
                                                msg.adv_service_instance_len);

  if (freq >= 2412 && freq <= 2484 && msg.ds_params && *msg.ds_params >= 1 &&
      *msg.ds_params <= 14) {
    int ds_freq;
    if (*msg.ds_params == 14)
      ds_freq = 2484;
  ...
  if (dev->listen_freq && dev->listen_freq != freq && scan_res) {
    p2p_dbg(p2p,
            "Update Listen frequency based on scan results (" MACSTR
            " %d -> %d MHz (DS param %d)",
            MAC2STR(dev->info.p2p_device_addr), dev->listen_freq, freq,
  ...
  for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
    if (msg.wps_vendor_ext[i] == NULL)
      break;
    dev->info.wps_vendor_ext[i] =
        wpabuf_alloc_copy(msg.wps_vendor_ext[i], msg.wps_vendor_ext_len[i]);
  ...
  if (dev->flags & P2P_DEV_REPORTED && !wfd_changed &&
      (!msg.adv_service_instance || (dev->flags & P2P_DEV_P2PS_REPORTED)))
    return 0;

  p2p_dbg(p2p, "Peer found with Listen frequency %d MHz (rx_time=%u.%06u)",
          freq, (unsigned int)rx_time->sec, (unsigned int)rx_time->usec);
  ...
  if (dev->info.config_methods == 0 &&
     ...
     * information.
     */
    p2p_dbg(p2p, "Do not report peer " MACSTR " with unknown config methods",
            MAC2STR(addr));
    return 0;
  }
...
static void p2p_search(struct p2p_data *p2p) {
  ...
  if (p2p->drv_in_listen) {
    p2p_dbg(p2p, "Driver is still in Listen state - wait for it to end before "
                 "continuing");
    return;
  }
  ...
  res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, type, freq, p2p->num_req_dev_types,
                           p2p->req_dev_types, p2p->find_dev_id, pw_id);
  if (res < 0) {
...
static int p2p_run_after_scan(struct p2p_data *p2p) {
  ...
  if (p2p->after_scan_tx) {
    ...
    p2p->cfg->send_action(
        p2p->cfg->cb_ctx, p2p->after_scan_tx->freq, p2p->after_scan_tx->dst,
        p2p->after_scan_tx->src, p2p->after_scan_tx->bssid,
        (u8 *)(p2p->after_scan_tx + 1), p2p->after_scan_tx->len,
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout,
             ...
             unsigned int search_delay, u8 seek_count, const char **seek,
             int freq) {
  int res;
  ...
  if (seek_count == 0 || !seek) {
    ...
    for (i = 0; i < seek_count; i++) {
      ...
      p2p_dbg(p2p, "Seek service %s hash " MACSTR, seek[i], MAC2STR(buf));
      os_memcpy(&p2p->p2ps_seek_hash[count * P2PS_HASH_LEN], buf,
                P2PS_HASH_LEN);
      count++;
    }
...
static int p2p_prepare_channel_pref(struct p2p_data *p2p,
  ...
  if (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) &&
      (go ||
       !p2p_channels_includes(&p2p->cfg->cli_channels, op_class, op_channel))) {
    p2p_dbg(p2p,
...
static void p2p_prepare_channel_best(struct p2p_data *p2p) {
  ...
  if (!p2p->cfg->cfg_op_channel && p2p->best_freq_overall > 0 &&
    ...
    p2p->op_channel = op_channel;
             ...
             p2p_freq_to_channel(p2p->best_freq_24, &op_class, &op_channel) ==
                 0) {
    ...
    p2p_dbg(p2p,
            "Select possible VHT channel (op_class %u channel %u) as operating "
            "channel preference",
            p2p->op_reg_class, p2p->op_channel);
  } else if (p2p_channel_select(&p2p->cfg->channels, op_classes_ht40,
                                &p2p->op_reg_class, &p2p->op_channel) == 0) {
    p2p_dbg(p2p,
            "Select possible HT40 channel (op_class %u channel %u) as "
            "operating channel preference",
            p2p->op_reg_class, p2p->op_channel);
  } else if (p2p_channel_select(&p2p->cfg->channels, op_classes_5ghz,
                                &p2p->op_reg_class, &p2p->op_channel) == 0) {
    p2p_dbg(p2p,
            "Select possible 5 GHz channel (op_class %u channel %u) as "
            "operating channel preference",
            p2p->op_reg_class, p2p->op_channel);
  } else if (p2p_channels_includes(&p2p->cfg->channels, p2p->cfg->op_reg_class,
                                   p2p->cfg->op_channel)) {
    p2p_dbg(p2p,
            "Select pre-configured channel as operating channel preference");
    p2p->op_reg_class = p2p->cfg->op_reg_class;
    p2p->op_channel = p2p->cfg->op_channel;
  } else if (p2p_channel_random_social(&p2p->cfg->channels, &p2p->op_reg_class,
                                       &p2p->op_channel) == 0) {
    p2p_dbg(p2p,
            "Select random available social channel (op_class %u channel %u) "
            "as operating channel preference",
            p2p->op_reg_class, p2p->op_channel);
  } else {
    /* Select any random available channel from the first available
     * operating class */
    p2p_channel_select(&p2p->cfg->channels, NULL, &p2p->op_reg_class,
                       &p2p->op_channel);
    p2p_dbg(p2p,
            "Select random available channel %d from operating class %d as "
            "operating channel preference",
...
int p2p_prepare_channel(struct p2p_data *p2p, struct p2p_device *dev,
                        ...
                        int go) {
  p2p_dbg(p2p, "Prepare channel - force_freq=%u pref_freq=%u go=%d", force_freq,
          pref_freq, go);
  ...
  p2p_dbg(
      ...
      p2p->op_reg_class, p2p->op_channel, force_freq ? " (forced)" : "");
...
static void p2p_set_dev_persistent(struct p2p_device *dev,
  ...
  case 0:
    dev->flags &=
        ~(P2P_DEV_PREFER_PERSISTENT_GROUP | P2P_DEV_PREFER_PERSISTENT_RECONN);
    break;
  case 1:
...
int p2p_connect(struct p2p_data *p2p, const u8 *peer_addr,
  ...
  if (p2p_prepare_channel(p2p, dev, force_freq, pref_freq, go_intent == 15) < 0)
    return -1;

  if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
    if (!(dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
      p2p_dbg(p2p,
  ...
  p2p->ssid_set = 0;
  if (force_ssid) {
    wpa_hexdump_ascii(MSG_DEBUG, "P2P: Forced SSID", force_ssid,
                      force_ssid_len);
  ...
  if (p2p->after_scan_tx) {
     ...
     */
    p2p_dbg(p2p, "Dropped previous pending Action frame TX that was waiting "
...
int p2p_authorize(struct p2p_data *p2p, const u8 *peer_addr,
                  ...
                  const u8 *own_interface_addr, unsigned int force_freq,
                  int persistent_group, const u8 *force_ssid,
                  size_t force_ssid_len, unsigned int pref_freq,
                  u16 oob_pw_id) {
  struct p2p_device *dev;
...
void p2p_add_dev_info(struct p2p_data *p2p, const u8 *addr,
  ...
  if (msg->listen_channel) {
    ...
    if (freq < 0) {
      p2p_dbg(p2p,
              ...
              msg->listen_channel[0], msg->listen_channel[1],
              msg->listen_channel[2], msg->listen_channel[3],
              msg->listen_channel[4]);
    } else {
      p2p_dbg(p2p, "Update peer " MACSTR " Listen channel: %u -> %u MHz",
              MAC2STR(dev->info.p2p_device_addr), dev->listen_freq, freq);
      dev->listen_freq = freq;
    }
  }
  ...
  if (dev->flags & P2P_DEV_PROBE_REQ_ONLY) {
    ...
    p2p_dbg(p2p,
            "Created device entry based on GO Neg Req: " MACSTR
            " dev_capab=0x%x group_capab=0x%x name='%s' "
            "listen_freq=%d",
            MAC2STR(dev->info.p2p_device_addr), dev->info.dev_capab,
            dev->info.group_capab, dev->info.device_name, dev->listen_freq);
  }
...
void p2p_build_ssid(struct p2p_data *p2p, u8 *ssid, size_t *ssid_len) {
  ...
  os_memcpy(&ssid[P2P_WILDCARD_SSID_LEN + 2], p2p->cfg->ssid_postfix,
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer) {
  ...
  if (go) {
    ...
    p2p_channels_dump(p2p, "intersection after no-GO removal", &intersection);
  }
...
static void p2p_rx_p2p_action(struct p2p_data *p2p, const u8 *sa,
  ...
  case P2P_DEV_DISC_RESP:
    ...
    break;
  default:
    p2p_dbg(p2p, "Unsupported P2P Public Action frame type %d", data[0]);
    break;
  }
}
...
static void p2p_add_dev_from_probe_req(struct p2p_data *p2p, const u8 *addr,
  ...
  if (msg.listen_channel) {
    os_memcpy(dev->country, msg.listen_channel, 3);
    dev->listen_freq =
        p2p_channel_to_freq(msg.listen_channel[3], msg.listen_channel[4]);
  }
...
int p2p_match_dev_type(struct p2p_data *p2p, struct wpabuf *wps) {
  ...
  for (i = 0; i < p2p->cfg->num_sec_dev_types; i++) {
    if (dev_type_list_match(p2p->cfg->sec_dev_type[i], attr.req_dev_type,
                            attr.num_req_dev_type))
...
struct wpabuf *p2p_build_probe_resp_ies(struct p2p_data *p2p,
                                        const u8 *query_hash, u8 query_count) {
  struct wpabuf *buf;
  u8 *len;
  int pw_id = -1;
  size_t extra = 0;
  ...
  if (p2p->vendor_elem && p2p->vendor_elem[VENDOR_ELEM_PROBE_RESP_P2P])
    wpabuf_put_buf(buf, p2p->vendor_elem[VENDOR_ELEM_PROBE_RESP_P2P]);

  /* P2P IE */
  len = p2p_buf_add_ie_hdr(buf);
  p2p_buf_add_capability(
      buf, p2p->dev_capab & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY, 0);
...
static int p2p_build_probe_resp_buf(struct p2p_data *p2p, struct wpabuf *buf,
                                    struct wpabuf *ies, const u8 *addr,
                                    int rx_freq) {
  struct ieee80211_mgmt *resp;
  u8 channel, op_class;

  resp =
      wpabuf_put(buf, offsetof(struct ieee80211_mgmt, u.probe_resp.variable));

  resp->frame_control =
      host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_PROBE_RESP << 4));
  ...
  resp->u.probe_resp.capab_info =
      host_to_le16(WLAN_CAPABILITY_SHORT_PREAMBLE | WLAN_CAPABILITY_PRIVACY |
...
static int p2p_service_find_asp(struct p2p_data *p2p, const u8 *hash) {
  ...
  while (adv_data) {
    ...
    if (any_wfa && os_strncmp(adv_data->svc_name, P2PS_WILD_HASH_STR,
...
static enum p2p_probe_req_status p2p_reply_probe(struct p2p_data *p2p,
                                                 const u8 *addr, const u8 *dst,
                                                 const u8 *bssid, const u8 *ie,
                                                 size_t ie_len,
  ...
  if (dst && !is_broadcast_ether_addr(dst) &&
     ...
     */
    p2p_dbg(p2p, "Probe Req DA " MACSTR " not ours - ignore it", MAC2STR(dst));
  ...
  if (msg.service_hash && msg.service_hash_count) {
    ...
    p2p_dbg(p2p,
            "in_listen=%d drv_in_listen=%d when received P2PS Probe Request at "
            "%u MHz; own Listen channel %u, pending listen freq %u MHz",
            p2p->in_listen, p2p->drv_in_listen, rx_freq, p2p->cfg->channel,
            p2p->pending_listen_freq);

    if (!p2p->in_listen && !p2p->drv_in_listen && p2p->pending_listen_freq &&
        rx_freq && rx_freq != p2p->pending_listen_freq) {
      p2p_dbg(p2p,
              "Do not reply to Probe Request frame that was received on %u MHz "
              "while waiting to start Listen state on %u MHz",
    ...
    for (i = 0; i < msg.service_hash_count; i++) {
      if (p2p_service_find_asp(p2p, hash)) {
        p2p_dbg(p2p, "Service Hash match found: " MACSTR, MAC2STR(hash));
        p2ps_svc_found = 1;
        break;
      }
      hash += P2PS_HASH_LEN;
    ...
    if (!p2p->in_listen || !p2p->drv_in_listen) {
      ...
      p2p_dbg(p2p,
              "Not in Listen state (in_listen=%d drv_in_listen=%d) - ignore "
              "Probe Request",
  ...
  if (msg.wps_attributes && !p2p_match_dev_type(p2p, msg.wps_attributes)) {
   ...
   * parameters.
   */
  ies = p2p_build_probe_resp_ies(p2p, msg.service_hash, msg.service_hash_count);
  p2p_parse_free(&msg);
  if (ies == NULL)
    return P2P_PREQ_NOT_PROCESSED;
...
enum p2p_probe_req_status p2p_probe_req_rx(struct p2p_data *p2p, const u8 *addr,
  ...
  if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) &&
      p2p->go_neg_peer &&
      os_memcmp(addr, p2p->go_neg_peer->info.p2p_device_addr, ETH_ALEN) == 0 &&
  ...
  if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) &&
      ...
      os_memcmp(addr, p2p->invite_peer->info.p2p_device_addr, ETH_ALEN) == 0) {
...
static int p2p_assoc_req_ie_wlan_ap(struct p2p_data *p2p, const u8 *bssid,
  ...
  if (p2p->num_groups > 0) {
    ...
    if ((p2p->dev_capab & P2P_DEV_CAPAB_CONCURRENT_OPER) &&
        (p2p->dev_capab & P2P_DEV_CAPAB_INFRA_MANAGED) && p2p->cross_connect)
...
int p2p_assoc_req_ie(struct p2p_data *p2p, const u8 *bssid, u8 *buf, size_t len,
  ...
  if (p2p->vendor_elem && p2p->vendor_elem[VENDOR_ELEM_P2P_ASSOC_REQ])
    wpabuf_put_buf(tmp, p2p->vendor_elem[VENDOR_ELEM_P2P_ASSOC_REQ]);
...
int p2p_service_add_asp(struct p2p_data *p2p, int auto_accept, u32 adv_id,
  ...
  adv_data_len = sizeof(struct p2ps_advertisement) + adv_len + 1 + info_len + 1;
...
inserted:
  p2p_dbg(p2p,
          "Added ASP advertisement adv_id=0x%x config_methods=0x%x "
          "svc_state=0x%x adv_str='%s' cpt_mask=0x%x",
...
void p2p_wps_success_cb(struct p2p_data *p2p, const u8 *mac_addr) {
  if (p2p->go_neg_peer == NULL) {
    p2p_dbg(p2p, "No pending Group Formation - ignore WPS registration success "
                 "notification");
    return; /* No pending Group Formation */
  }

  if (os_memcmp(mac_addr, p2p->go_neg_peer->intended_addr, ETH_ALEN) != 0) {
    p2p_dbg(p2p,
            "Ignore WPS registration success notification for " MACSTR
            " (GO Negotiation peer " MACSTR ")",
            MAC2STR(mac_addr), MAC2STR(p2p->go_neg_peer->intended_addr));
...
void p2p_group_formation_failed(struct p2p_data *p2p) {
  if (p2p->go_neg_peer == NULL) {
    p2p_dbg(p2p, "No pending Group Formation - ignore group formation failure "
                 "notification");
    return; /* No pending Group Formation */
  }
...
struct p2p_data *p2p_init(const struct p2p_config *cfg) {
  ...
  if (cfg->max_peers < 1 || cfg->passphrase_len < 8 || cfg->passphrase_len > 63)
    return NULL;
  ...
  if (cfg->pref_chan) {
    p2p->cfg->pref_chan =
        os_malloc(cfg->num_pref_chan * sizeof(struct p2p_channel));
    if (p2p->cfg->pref_chan) {
      os_memcpy(p2p->cfg->pref_chan, cfg->pref_chan,
                cfg->num_pref_chan * sizeof(struct p2p_channel));
    } else
...
void p2p_flush(struct p2p_data *p2p) {
  ...
  p2p_stop_find(p2p);
  dl_list_for_each_safe(dev, prev, &p2p->devices, struct p2p_device, list) {
    dl_list_del(&dev->list);
    p2p_device_free(p2p, dev);
  }
...
static int p2p_pre_find_operation(struct p2p_data *p2p,
                                  struct p2p_device *dev) {
  int res;
  ...
  if (dev->req_config_methods && !(dev->flags & P2P_DEV_PD_FOR_JOIN)) {
    p2p_dbg(p2p,
            "Send pending Provision Discovery Request to " MACSTR
            " (config methods 0x%x)",
            MAC2STR(dev->info.p2p_device_addr), dev->req_config_methods);
...
static void p2p_sd_cb(struct p2p_data *p2p, int success) {
  p2p_dbg(p2p, "Service Discovery Query TX callback: success=%d", success);
  p2p->pending_action_state = P2P_NO_PENDING_ACTION;

  if (!success) {
    if (p2p->sd_peer) {
      if (is_zero_ether_addr(p2p->sd_query_no_ack)) {
        os_memcpy(p2p->sd_query_no_ack, p2p->sd_peer->info.p2p_device_addr,
                  ETH_ALEN);
        p2p_dbg(p2p, "First SD Query no-ACK in this search iteration: " MACSTR,
                MAC2STR(p2p->sd_query_no_ack));
      }
...
static void p2p_retry_pd(struct p2p_data *p2p) {
  ...
  dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
    if (os_memcmp(p2p->pending_pd_devaddr, dev->info.p2p_device_addr,
                  ETH_ALEN) != 0)
      continue;
    ...
    p2p_dbg(p2p,
            ...
            " (config methods 0x%x)",
            MAC2STR(dev->info.p2p_device_addr), dev->req_config_methods);
    p2p_send_prov_disc_req(p2p, dev, dev->flags & P2P_DEV_PD_FOR_JOIN,
                           p2p->pd_force_freq);
    return;
  }
}
...
static void p2p_prov_disc_cb(struct p2p_data *p2p, int success) {
  p2p_dbg(p2p, "Provision Discovery Request TX callback: success=%d", success);
  ...
  if (p2p->p2ps_prov && !p2p->p2ps_prov->pd_seeker &&
    ...
    if (p2p->cfg->p2ps_prov_complete) {
      p2p->cfg->p2ps_prov_complete(
          p2p->cfg->cb_ctx, p2p->p2ps_prov->status, p2p->p2ps_prov->adv_mac,
          p2p->p2ps_prov->adv_mac, p2p->p2ps_prov->session_mac, NULL,
          p2p->p2ps_prov->adv_id, p2p->p2ps_prov->session_id, 0, 0, NULL, 0, 0,
...
static void p2p_prov_disc_resp_cb(struct p2p_data *p2p, int success) {
  p2p_dbg(p2p, "Provision Discovery Response TX callback: success=%d", success);
  ...
  p2p_dbg(p2p, "Post-Provision Discovery operations started - do not try to "
...
int p2p_scan_res_handler(struct p2p_data *p2p, const u8 *bssid, int freq,
  ...
  if (os_reltime_before(rx_time, &p2p->find_start)) {
    ...
    p2p_dbg(p2p,
            ...
            (unsigned int)rx_time->usec, (unsigned int)p2p->find_start.sec,
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const u8 *dev_id,
  ...
  if (p2p->vendor_elem && p2p->vendor_elem[VENDOR_ELEM_PROBE_REQ_P2P])
    wpabuf_put_buf(ies, p2p->vendor_elem[VENDOR_ELEM_PROBE_REQ_P2P]);
  ...
  if (p2p->cfg->reg_class && p2p->cfg->channel)
    p2p_buf_add_listen_channel(ies, p2p->cfg->country, p2p->cfg->reg_class,
                               p2p->cfg->channel);
  if (p2p->ext_listen_interval)
...
size_t p2p_scan_ie_buf_len(struct p2p_data *p2p) {
  ...
  if (p2p && p2p->vendor_elem && p2p->vendor_elem[VENDOR_ELEM_PROBE_REQ_P2P])
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success) {
  ...
  if (!success &&
    ...
    p2p_dbg(p2p,
            "Peer " MACSTR " did not acknowledge request - try to use device "
            "discoverability through its GO",
            MAC2STR(dev->info.p2p_device_addr));
    p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
...
static void p2p_go_neg_resp_cb(struct p2p_data *p2p, int success) {
  p2p_dbg(p2p, "GO Negotiation Response TX callback: success=%d", success);
...
static void p2p_go_neg_resp_failure_cb(struct p2p_data *p2p, int success,
                                       const u8 *addr) {
  p2p_dbg(p2p, "GO Negotiation Response (failure) TX callback: success=%d",
          success);
  ...
  if (success) {
    struct p2p_device *dev;
    dev = p2p_get_device(p2p, addr);
    if (dev && dev->status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE)
...
static void p2p_go_neg_conf_cb(struct p2p_data *p2p,
  ...
  if (result == P2P_SEND_ACTION_NO_ACK) {
    ...
    if (dev && dev->go_neg_conf &&
      ...
      p2p_dbg(p2p, "GO Negotiation Confirm retry %d", dev->go_neg_conf_sent);
      p2p->pending_action_state = P2P_PENDING_GO_NEG_CONFIRM;
      if (p2p_send_action(p2p, dev->go_neg_conf_freq, dev->info.p2p_device_addr,
                          p2p->cfg->dev_addr, dev->info.p2p_device_addr,
                          wpabuf_head(dev->go_neg_conf),
                          wpabuf_len(dev->go_neg_conf), 0) >= 0) {
     ...
     * the TX status.
     */
    p2p_dbg(p2p, "Assume GO Negotiation Confirm TX was actually received by "
                 "the peer even though Ack was not reported");
  }
...
void p2p_listen_cb(struct p2p_data *p2p, unsigned int freq,
  ...
  if (freq != p2p->pending_listen_freq) {
    p2p_dbg(p2p,
            "Unexpected listen callback for freq=%u duration=%u "
            "(pending_listen_freq=%u)",
...
int p2p_listen_end(struct p2p_data *p2p, unsigned int freq) {
  ...
  if (p2p->state == P2P_CONNECT_LISTEN && p2p->go_neg_peer) {
    ...
    if (p2p->pending_listen_freq) {
       ...
       */
      p2p_dbg(p2p, "Listen operation did not seem to start - delay search "
                   "phase to avoid busy loop");
      p2p_set_timeout(p2p, 0, 100000);
      return 1;
    }
    if (p2p->search_delay) {
      p2p_dbg(p2p, "Delay search operation by %u ms", p2p->search_delay);
...
static void p2p_timeout_connect(struct p2p_data *p2p) {
  ...
  if (p2p->go_neg_peer &&
      (p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM)) {
    p2p_dbg(p2p, "Wait for GO Negotiation Confirm timed out - assume GO "
                 "Negotiation failed");
    p2p_go_neg_failed(p2p, -1);
    return;
  }
...
static void p2p_timeout_wait_peer_idle(struct p2p_data *p2p) {
  ...
  p2p_dbg(p2p, "Go to Listen state while waiting for the peer to become ready "
               "for GO Negotiation");
...
static void p2p_timeout_prov_disc_req(struct p2p_data *p2p) {
  ...
  if (p2p->pd_retries) {
    ...
    if (p2p->cfg->prov_disc_fail)
      p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, p2p->pending_pd_devaddr,
                               for_join ? P2P_PROV_DISC_TIMEOUT_JOIN
  ...
  if (p2p->invite_peer && p2p->invite_peer->invitation_reqs < 100) {
    ...
    if (p2p->invite_peer) {
      ...
      if (p2p->cfg->invitation_result)
        p2p->cfg->invitation_result(p2p->cfg->cb_ctx, -1, NULL, NULL,
...
int p2p_reject(struct p2p_data *p2p, const u8 *peer_addr) {
  ...
  dev = p2p_get_device(p2p, peer_addr);
  p2p_dbg(p2p, "Local request to reject connection attempts by peer " MACSTR,
          MAC2STR(peer_addr));
  if (dev == NULL) {
...
int p2p_get_peer_info_txt(const struct p2p_peer_info *info, char *buf,
  ...
  res = os_snprintf(
      ...
      "status=%d\n"
      "invitation_reqs=%u\n",
      (int)(now.sec - dev->last_seen.sec), dev->listen_freq,
      p2p_wps_method_text(dev->wps_method), MAC2STR(dev->interface_addr),
      MAC2STR(dev->member_in_go_dev), MAC2STR(dev->member_in_go_iface),
      dev->go_neg_req_sent, p2p_go_state_text(dev->go_state), dev->dialog_token,
      MAC2STR(dev->intended_addr), dev->country[0] ? dev->country[0] : '_',
      dev->country[1] ? dev->country[1] : '_', dev->oper_freq,
      dev->req_config_methods,
      dev->flags & P2P_DEV_PROBE_REQ_ONLY ? "[PROBE_REQ_ONLY]" : "",
      dev->flags & P2P_DEV_REPORTED ? "[REPORTED]" : "",
      dev->flags & P2P_DEV_NOT_YET_READY ? "[NOT_YET_READY]" : "",
      dev->flags & P2P_DEV_PD_PEER_DISPLAY ? "[PD_PEER_DISPLAY]" : "",
      dev->flags & P2P_DEV_PD_PEER_KEYPAD ? "[PD_PEER_KEYPAD]" : "",
      dev->flags & P2P_DEV_PD_PEER_P2PS ? "[PD_PEER_P2PS]" : "",
      dev->flags & P2P_DEV_USER_REJECTED ? "[USER_REJECTED]" : "",
      dev->flags & P2P_DEV_PEER_WAITING_RESPONSE ? "[PEER_WAITING_RESPONSE]"
                                                 : "",
      dev->flags & P2P_DEV_PREFER_PERSISTENT_GROUP ? "[PREFER_PERSISTENT_GROUP]"
                                                   : "",
      dev->flags & P2P_DEV_WAIT_GO_NEG_RESPONSE ? "[WAIT_GO_NEG_RESPONSE]" : "",
      dev->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM ? "[WAIT_GO_NEG_CONFIRM]" : "",
      dev->flags & P2P_DEV_GROUP_CLIENT_ONLY ? "[GROUP_CLIENT_ONLY]" : "",
      dev->flags & P2P_DEV_FORCE_FREQ ? "[FORCE_FREQ]" : "",
      dev->flags & P2P_DEV_PD_FOR_JOIN ? "[PD_FOR_JOIN]" : "",
      dev->flags & P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT
          ? "[LAST_SEEN_AS_GROUP_CLIENT]"
          : "",
      dev->status, dev->invitation_reqs);
  ...
  if (dev->ext_listen_period) {
    res = os_snprintf(pos, end - pos,
                      ...
                      dev->ext_listen_period, dev->ext_listen_interval);
    if (os_snprintf_error(end - pos, res))
      return pos - buf;
    pos += res;
  }
  ...
  if (dev->oper_ssid_len) {
    res = os_snprintf(pos, end - pos, "oper_ssid=%s\n",
                      wpa_ssid_txt(dev->oper_ssid, dev->oper_ssid_len));
    if (os_snprintf_error(end - pos, res))
      return pos - buf;
    pos += res;
  }
  ...
  if (dev->info.wfd_subelems) {
    ...
    pos += wpa_snprintf_hex(pos, end - pos, wpabuf_head(dev->info.wfd_subelems),
...
int p2p_presence_req(struct p2p_data *p2p, const u8 *go_interface_addr,
  ...
  p2p_dbg(p2p,
          "Send Presence Request to GO " MACSTR " (own interface " MACSTR
          ") freq=%u dur1=%u int1=%u "
          "dur2=%u int2=%u",
          MAC2STR(go_interface_addr), MAC2STR(own_interface_addr), freq,
          duration1, interval1, duration2, interval2);

  req = p2p_build_presence_req(duration1, interval1, duration2, interval2);
  if (req == NULL)
    return -1;

  p2p->pending_action_state = P2P_NO_PENDING_ACTION;
  if (p2p_send_action(p2p, freq, go_interface_addr, own_interface_addr,
                      go_interface_addr, wpabuf_head(req), wpabuf_len(req),
                      200) < 0) {
...
static void p2p_process_presence_req(struct p2p_data *p2p, const u8 *da,
  ...
  for (g = 0; g < p2p->num_groups; g++) {
    if (os_memcmp(da, p2p_group_get_interface_addr(p2p->groups[g]), ETH_ALEN) ==
        0) {
      group = p2p->groups[g];
      break;
    }
  ...
  if (group == NULL) {
    p2p_dbg(p2p, "Ignore P2P Presence Request for unknown group " MACSTR,
            MAC2STR(da));
    return;
  }
...
fail:
  if (p2p->cfg->get_noa)
    noa_len = p2p->cfg->get_noa(p2p->cfg->cb_ctx, da, noa, sizeof(noa));
  else
    noa_len = -1;
  resp = p2p_build_presence_resp(status, noa_len > 0 ? noa : NULL,
                                 noa_len > 0 ? noa_len : 0, msg.dialog_token);
  if (parsed)
    p2p_parse_free(&msg);
  if (resp == NULL)
    return;
  ...
  p2p->pending_action_state = P2P_NO_PENDING_ACTION;
  if (p2p_send_action(p2p, rx_freq, sa, da, da, wpabuf_head(resp),
                      wpabuf_len(resp), 200) < 0) {
    p2p_dbg(p2p, "Failed to send Action frame");
  }
  wpabuf_free(resp);
}
...
static void p2p_process_presence_resp(struct p2p_data *p2p, const u8 *da,
  ...
  if (p2p->cfg->presence_resp) {
    p2p->cfg->presence_resp(p2p->cfg->cb_ctx, sa, *msg.status, msg.noa,
                            msg.noa_len);
  }

  if (*msg.status) {
    p2p_dbg(p2p, "P2P Presence Request was rejected: status %u", *msg.status);
    p2p_parse_free(&msg);
    return;
  }

  p2p_dbg(p2p, "P2P Presence Request was accepted");
  wpa_hexdump(MSG_DEBUG, "P2P: P2P Presence Response - NoA", msg.noa,
              msg.noa_len);
  /* TODO: process NoA */
  p2p_parse_free(&msg);
}
...
static void p2p_ext_listen_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (p2p->state == P2P_LISTEN_ONLY && p2p->ext_listen_only) {
     ...
     */
    p2p_dbg(p2p, "Previous Extended Listen operation had not been completed - "
                 "try again");
    p2p->ext_listen_only = 0;
    p2p_set_state(p2p, P2P_IDLE);
  }
  ...
  if (p2p->state != P2P_IDLE) {
    p2p_dbg(p2p, "Skip Extended Listen timeout in active state (%s)",
            p2p_state_txt(p2p->state));
    return;
  }
...
int p2p_ext_listen(struct p2p_data *p2p, unsigned int period,
  ...
  eloop_register_timeout(p2p->ext_listen_interval_sec,
                         p2p->ext_listen_interval_usec, p2p_ext_listen_timeout,
                         p2p, NULL);
...
int p2p_set_listen_channel(struct p2p_data *p2p, u8 reg_class, u8 channel,
  ...
  if (p2p->cfg->channel_forced && forced == 0) {
    p2p_dbg(p2p, "Listen channel was previously configured - do not override "
                 "based on optimization");
    return -1;
  }

  p2p_dbg(p2p, "Set Listen channel: reg_class %u channel %u", reg_class,
          channel);
...
u8 p2p_get_listen_channel(struct p2p_data *p2p) { return p2p->cfg->channel; }
...
int p2p_set_oper_channel(struct p2p_data *p2p, u8 op_reg_class, u8 op_channel,
  ...
  p2p_dbg(p2p, "Set Operating channel: reg_class %u channel %u", op_reg_class,
          op_channel);
...
void p2p_set_peer_filter(struct p2p_data *p2p, const u8 *addr) {
  ...
  if (is_zero_ether_addr(p2p->peer_filter))
    ...
    p2p_dbg(p2p, "Enable peer filter for " MACSTR, MAC2STR(p2p->peer_filter));
}
...
void p2p_set_intra_bss_dist(struct p2p_data *p2p, int enabled) {
  p2p_dbg(p2p, "Intra BSS distribution %s", enabled ? "enabled" : "disabled");
...
void p2p_update_channel_list(struct p2p_data *p2p,
  ...
  p2p_channels_dump(p2p, "channels", &p2p->cfg->channels);
  os_memcpy(&p2p->cfg->cli_channels, cli_chan, sizeof(struct p2p_channels));
  p2p_channels_dump(p2p, "cli_channels", &p2p->cfg->cli_channels);
}
...
int p2p_send_action(struct p2p_data *p2p, unsigned int freq, const u8 *dst,
  ...
  if (p2p->p2p_scan_running) {
    ...
    p2p->after_scan_tx = os_malloc(sizeof(*p2p->after_scan_tx) + len);
  ...
  return p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, dst, src, bssid, buf,
                               len, wait_time);
}

void p2p_set_best_channels(struct p2p_data *p2p, int freq_24, int freq_5,
                           int freq_overall) {
  p2p_dbg(p2p, "Best channel: 2.4 GHz: %d,  5 GHz: %d,  overall: %d", freq_24,
          freq_5, freq_overall);
...
static struct wpabuf *p2p_build_nfc_handover(struct p2p_data *p2p,
  ...
  if (p2p->num_groups > 0) {
    ...
    if (p2p_freq_to_channel(freq, &op_class, &channel) < 0) {
      p2p_dbg(p2p, "Unknown GO operating frequency %d MHz for NFC handover",
              freq);
      wpabuf_free(buf);
      return NULL;
    }
    ...
    if (p2p_freq_to_channel(client_freq, &op_class, &channel) < 0) {
      p2p_dbg(p2p, "Unknown client operating frequency %d MHz for NFC handover",
              client_freq);
      wpabuf_free(buf);
      return NULL;
    }
  ...
  p2p_buf_add_oob_go_neg_channel(buf, p2p->cfg->country, op_class, channel,
                                 role);
  ...
  } else if (client_freq > 0 && go_dev_addr &&
             !is_zero_ether_addr(go_dev_addr) && ssid && ssid_len > 0) {
    /*
...
struct wpabuf *p2p_build_nfc_handover_req(struct p2p_data *p2p, int client_freq,
                                          ...
                                          size_t ssid_len) {
  return p2p_build_nfc_handover(p2p, client_freq, go_dev_addr, ssid, ssid_len);
}
...
int p2p_process_nfc_connection_handover(struct p2p_data *p2p,
  ...
  if (msg.oob_go_neg_channel[3] == 0 && msg.oob_go_neg_channel[4] == 0)
    freq = 0;
  else
  ...
  if (role == P2P_GO_IN_A_GROUP) {
    ...
    p2p_dbg(p2p, "Peer (client) OOB GO operating channel: %u MHz", freq);
    params->go_freq = freq;
  } else
  ...
  if (msg.group_id) {
    ...
    os_memcpy(params->go_ssid, msg.group_id + ETH_ALEN, params->go_ssid_len);
  }
...
void p2p_set_own_pref_freq_list(struct p2p_data *p2p,
  ...
  for (i = 0; i < size; i++) {
    ...
    p2p_dbg(p2p, "Own preferred frequency list[%u]=%u MHz", i,
            p2p->pref_freq_list[i]);
  }
}
...
struct wpabuf *p2p_build_probe_resp_template(struct p2p_data *p2p,
  ...
  ies = p2p_build_probe_resp_ies(p2p, NULL, 0);
  if (!ies) {
    wpa_printf(MSG_ERROR, "CTRL: Failed to build Probe Response IEs");
    return NULL;
  }
