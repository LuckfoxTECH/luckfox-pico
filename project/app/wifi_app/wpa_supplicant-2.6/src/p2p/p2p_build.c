...
void p2p_buf_add_capability(struct wpabuf *buf, u8 dev_capab, u8 group_capab) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: * Capability dev=%02x group=%02x", dev_capab,
             group_capab);
}
...
void p2p_buf_add_pref_channel_list(struct wpabuf *buf,
   ...
   */
  for (i = 0; i < size; i++) {
    if (p2p_freq_to_channel(preferred_freq_list[i], &op_class, &op_channel) ==
        0)
      count++;
  }
...
void p2p_buf_add_channel_list(struct wpabuf *buf, const char *country,
  ...
  wpa_hexdump(MSG_DEBUG, "P2P: * Channel List", len + 2,
              (u8 *)wpabuf_put(buf, 0) - len - 2);
}
...
void p2p_buf_add_device_info(struct wpabuf *buf, struct p2p_data *p2p,
  ...
  if (peer && peer->wps_method != WPS_NOT_READY) {
    ...
    methods |=
        p2p->cfg->config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_DISPLAY |
  ...
  for (i = 0; i < p2p->cfg->num_sec_dev_types; i++)
    wpabuf_put_data(buf, p2p->cfg->sec_dev_type[i], WPS_DEV_TYPE_LEN);
...
void p2p_buf_add_group_bssid(struct wpabuf *buf, const u8 *bssid) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: * P2P Group BSSID " MACSTR, MAC2STR(bssid));
}
...
void p2p_buf_add_group_id(struct wpabuf *buf, const u8 *dev_addr,
  ...
  wpa_printf(MSG_DEBUG, "P2P: * P2P Group ID " MACSTR, MAC2STR(dev_addr));
...
static void p2p_buf_add_noa_desc(struct wpabuf *buf,
                                 struct p2p_noa_desc *desc) {
  if (desc == NULL)
    return;
...
void p2p_buf_add_service_hash(struct wpabuf *buf, struct p2p_data *p2p) {
  ...
  wpa_hexdump(MSG_DEBUG, "P2P: * Service Hash", p2p->p2ps_seek_hash,
              p2p->p2ps_seek_count * P2PS_HASH_LEN);
}
...
void p2p_buf_add_connection_capability(struct wpabuf *buf, u8 connection_cap) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: * Connection Capability: 0x%x", connection_cap);
}
...
void p2p_buf_add_advertisement_id(struct wpabuf *buf, u32 id, const u8 *mac) {
  ...
  wpa_printf(MSG_DEBUG, "P2P: * Advertisement ID (%x) " MACSTR, id,
             MAC2STR(mac));
}

static int p2ps_wildcard_hash(struct p2p_data *p2p, const u8 *hash,
                              u8 hash_count) {
  u8 i;
  const u8 *test = hash;
...
static int p2p_buf_add_service_info(struct wpabuf *buf, struct p2p_data *p2p,
  ...
  info_len = sizeof(adv_id) + sizeof(config_methods) + sizeof(u8) + svc_len;

  if (info_len + *total_len > MAX_SVC_ADV_LEN) {
    p2p_dbg(p2p, "Unsufficient buffer, failed to add advertised service info");
    return -1;
  }

  if (svc_len > 255) {
    p2p_dbg(p2p,
            "Invalid service name length (%u bytes), failed to add advertised "
            "service info",
  ...
  if (*ie_len) {
    ...
    if (ie_data_len < 0 || ie_data_len > 255) {
      p2p_dbg(p2p, "Invalid IE length, failed to add advertised service info");
      return -1;
    }
  ...
  if (remaining < sizeof(u32) + sizeof(u16) + sizeof(u8)) {
    ...
    *ie_len = p2p_buf_add_ie_hdr(buf);
    ...
    remaining = 255 - 4 - (sizeof(u32) + sizeof(u16) + sizeof(u8)) - back;
  } else {
    wpabuf_put_le32(buf, adv_id);
    wpabuf_put_be16(buf, config_methods);
    wpabuf_put_u8(buf, svc_len);
    remaining -= sizeof(adv_id) + sizeof(config_methods) + sizeof(u8);
  }
...
void p2p_buf_add_service_instance(struct wpabuf *buf, struct p2p_data *p2p,
  ...
  wpa_hexdump(MSG_DEBUG, "P2PS: Probe Request service hash values", hash,
              hash_count * P2PS_HASH_LEN);
  p2ps_wildcard =
      p2ps_wildcard_hash(p2p, hash, hash_count) && p2p_wfa_service_adv(p2p);
  ...
  if (p2ps_wildcard) {
    ...
    p2p_buf_add_service_info(tmp_buf, p2p, 0, 0, P2PS_WILD_HASH_STR, &ie_len,
                             &pos, &total_len, attr_len);
  }

  /* add advertised service info of matching services */
  for (adv = adv_list; adv && total_len <= MAX_SVC_ADV_LEN; adv = adv->next) {
    const u8 *test = hash;
    u8 i;
    ...
    for (i = 0; i < hash_count; i++) {
      ...
      if (os_memcmp(test, adv->hash, P2PS_HASH_LEN) == 0 &&
          p2p_buf_add_service_info(tmp_buf, p2p, adv->id, adv->config_methods,
                                   adv->svc_name, &ie_len, &pos, &total_len,
                                   attr_len))
        break;
...
void p2p_buf_add_session_id(struct wpabuf *buf, u32 id, const u8 *mac) {
  ...
  wpabuf_put_le32(buf, id);
  wpabuf_put_data(buf, mac, ETH_ALEN);
  wpa_printf(MSG_DEBUG, "P2P: * Session ID Info (%x) " MACSTR, id,
             MAC2STR(mac));
}
...
int p2p_build_wps_ie(struct p2p_data *p2p, struct wpabuf *buf, int pw_id,
  ...
  if (all_attr) {
    ...
    if (wps_build_uuid_e(buf, p2p->cfg->uuid) < 0 ||
        p2p_add_wps_string(buf, ATTR_MANUFACTURER, p2p->cfg->manufacturer) <
            0 ||
        p2p_add_wps_string(buf, ATTR_MODEL_NAME, p2p->cfg->model_name) < 0 ||
        p2p_add_wps_string(buf, ATTR_MODEL_NUMBER, p2p->cfg->model_number) <
            0 ||
        p2p_add_wps_string(buf, ATTR_SERIAL_NUMBER, p2p->cfg->serial_number) <
            0)
      return -1;
    ...
    if (p2p_add_wps_string(buf, ATTR_DEV_NAME, p2p->cfg->dev_name) < 0)
      return -1;
  ...
  if (all_attr && p2p->cfg->num_sec_dev_types) {
    ...
    wpabuf_put_be16(buf, WPS_DEV_TYPE_LEN * p2p->cfg->num_sec_dev_types);
    wpabuf_put_data(buf, p2p->cfg->sec_dev_type,
                    WPS_DEV_TYPE_LEN * p2p->cfg->num_sec_dev_types);
  }
  ...
  for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
    if (p2p->wps_vendor_ext[i] == NULL)
      break;
    if (wpabuf_tailroom(buf) < 4 + wpabuf_len(p2p->wps_vendor_ext[i]))
      continue;
