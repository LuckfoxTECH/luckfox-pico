...
static size_t tls_server_cert_chain_der_len(struct tlsv1_server *conn) {
  ...
  while (cert) {
    ...
    if (x509_certificate_self_signed(cert))
      break;
    cert =
        x509_certificate_get_subject(conn->cred->trusted_certs, &cert->issuer);
  }

  return len;
}

static int tls_write_server_hello(struct tlsv1_server *conn, u8 **msgpos,
                                  u8 *end) {
  ...
  *pos++ = TLS_COMPRESSION_NULL;
  ...
  if (conn->session_ticket && conn->session_ticket_cb) {
    int res = conn->session_ticket_cb(
        conn->session_ticket_cb_ctx, conn->session_ticket,
        conn->session_ticket_len, conn->client_random, conn->server_random,
        conn->master_secret);
    if (res < 0) {
  ...
  if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, rhdr, end - rhdr,
                        hs_start, pos - hs_start, &rlen) < 0) {
...
static int tls_write_server_certificate(struct tlsv1_server *conn, u8 **msgpos,
                                        u8 *end) {
  ...
  while (cert) {
    if (3 + cert->cert_len > (size_t)(end - pos)) {
      wpa_printf(MSG_DEBUG,
                 ...
                 (unsigned long)cert->cert_len, (unsigned long)(end - pos));
      tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
      return -1;
    }
...
static int tls_write_server_key_exchange(struct tlsv1_server *conn, u8 **msgpos,
                                         u8 *end) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "TLSv1: DH server's secret value", conn->dh_secret,
                  conn->dh_secret_len);
  ...
  if (crypto_mod_exp(conn->cred->dh_g, conn->cred->dh_g_len, conn->dh_secret,
                     conn->dh_secret_len, dh_p, dh_p_len, dh_ys, &dh_ys_len)) {
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    os_free(dh_ys);
    return -1;
  }

  wpa_hexdump(MSG_DEBUG, "TLSv1: DH Ys (server's public value)", dh_ys,
              dh_ys_len);
  ...
  if (keyx == TLS_KEY_X_DHE_RSA) {
    ...
    if (conn->rl.tls_version >= TLS_VERSION_1_2) {
      ...
      hlen = tlsv12_key_x_server_params_hash(
          conn->rl.tls_version, TLS_HASH_ALG_SHA256, conn->client_random,
          conn->server_random, server_params, pos - server_params, hash + 19);
      ...
      *pos++ = TLS_SIGN_ALG_RSA;
      ...
      hlen = tls_key_x_server_params_hash(
          ...
          server_params, pos - server_params, hash);
    }
    ...
    wpa_hexdump(MSG_MSGDUMP, "TLS: ServerKeyExchange signed_params hash", hash,
                hlen);
#ifdef CONFIG_TESTING_OPTIONS
    if (conn->test_flags & TLS_BREAK_SRV_KEY_X_HASH) {
      tlsv1_server_log(conn,
                       "TESTING: Break ServerKeyExchange signed params hash");
      hash[hlen - 1] ^= 0x80;
    }
    ...
    clen = end - pos;
    if (conn->cred == NULL ||
        crypto_private_key_sign_pkcs1(conn->cred->key, hash, hlen, pos, &clen) <
            0) {
    ...
    if (conn->test_flags & TLS_BREAK_SRV_KEY_X_SIGNATURE) {
      tlsv1_server_log(
          conn, "TESTING: Break ServerKeyExchange signed params signature");
      pos[clen - 1] ^= 0x80;
    }
#endif /* CONFIG_TESTING_OPTIONS */
...
static int tls_write_server_hello_done(struct tlsv1_server *conn, u8 **msgpos,
                                       u8 *end) {
  u8 *pos;
  size_t rlen;
  u8 payload[4];
  ...
  *pos++ = TLS_HANDSHAKE_TYPE_SERVER_HELLO_DONE;
  ...
  if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, *msgpos,
                        end - *msgpos, payload, pos - payload, &rlen) < 0) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to generate a record");
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
...
static int tls_write_server_change_cipher_spec(struct tlsv1_server *conn,
  ...
  if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC, *msgpos,
                        end - *msgpos, payload, sizeof(payload), &rlen) < 0) {
...
static int tls_write_server_finished(struct tlsv1_server *conn, u8 **msgpos,
                                     u8 *end) {
  u8 *pos, *hs_start;
  size_t rlen, hlen;
  ...
  if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN,
              "server finished", hash, hlen, verify_data + 1 + 3,
              TLS_VERIFY_DATA_LEN)) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to generate verify_data");
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
  wpa_hexdump_key(MSG_DEBUG, "TLSv1: verify_data (server)", verify_data + 1 + 3,
                  TLS_VERIFY_DATA_LEN);
#ifdef CONFIG_TESTING_OPTIONS
  ...
  *pos++ = TLS_HANDSHAKE_TYPE_FINISHED;
  ...
  if (tlsv1_record_send(&conn->rl, TLS_CONTENT_TYPE_HANDSHAKE, *msgpos,
                        end - *msgpos, hs_start, pos - hs_start, &rlen) < 0) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to create a record");
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
...
static u8 *tls_send_server_hello(struct tlsv1_server *conn, size_t *out_len) {
  ...
  *out_len = 0;
  ...
  if (conn->status_request_multi && conn->cred->ocsp_stapling_response_multi) {
    ocsp_resp =
        os_readfile(conn->cred->ocsp_stapling_response_multi, &ocsp_resp_len);
    ocsp_multi = 1;
  } else if ((conn->status_request || conn->status_request_v2) &&
             conn->cred->ocsp_stapling_response) {
    ocsp_resp = os_readfile(conn->cred->ocsp_stapling_response, &ocsp_resp_len);
  }
  if (!ocsp_resp)
    ocsp_resp_len = 0;
...
u8 *tlsv1_server_send_alert(struct tlsv1_server *conn, u8 level, u8 description,
                            size_t *out_len) {
  u8 *alert, *pos, *length;
  ...
  *pos++ = TLS_CONTENT_TYPE_ALERT;
  ...
  WPA_PUT_BE16(pos, conn->rl.tls_version ? conn->rl.tls_version : TLS_VERSION);
  pos += 2;
