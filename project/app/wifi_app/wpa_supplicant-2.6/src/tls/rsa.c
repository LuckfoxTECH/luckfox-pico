...
struct crypto_rsa_key *crypto_rsa_import_public_key(const u8 *buf, size_t len) {
  ...
  if (pos != end) {
    wpa_hexdump(MSG_DEBUG, "RSA: Extra data in public key SEQUENCE", pos,
                end - pos);
    goto error;
  }
...
struct crypto_rsa_key *crypto_rsa_import_public_key_parts(const u8 *n,
                                                          size_t n_len,
                                                          const u8 *e,
                                                          size_t e_len) {
  struct crypto_rsa_key *key;
...
struct crypto_rsa_key *crypto_rsa_import_private_key(const u8 *buf,
  ...
  if (key->n == NULL || key->e == NULL || key->d == NULL || key->p == NULL ||
      key->q == NULL || key->dmp1 == NULL || key->dmq1 == NULL ||
      key->iqmp == NULL) {
    crypto_rsa_free(key);
    return NULL;
  }
...
int crypto_rsa_exptmod(const u8 *in, size_t inlen, u8 *out, size_t *outlen,
  ...
  *outlen = modlen;
  ...
  if (bignum_get_unsigned_bin(
          tmp, out + (modlen - bignum_get_unsigned_bin_len(tmp)), NULL) < 0)
    goto error;
