...
static int tlsv1_add_cert_der(struct x509_certificate **chain, const u8 *buf,
                              size_t len) {
  ...
  if (cert == NULL) {
    wpa_printf(MSG_INFO, "TLSv1: %s - failed to parse certificate", __func__);
    return -1;
  }
...
static int tlsv1_add_cert(struct x509_certificate **chain, const u8 *buf,
                          size_t len) {
  const u8 *pos, *end;
  unsigned char *der;
  size_t der_len;
...
static int tlsv1_set_cert_chain(struct x509_certificate **chain,
  ...
  if (cert) {
    ...
    if (buf == NULL) {
      wpa_printf(MSG_INFO, "TLSv1: Failed to read '%s'", cert);
      return -1;
    }
...
int tlsv1_set_ca_cert(struct tlsv1_credentials *cred, const char *cert,
  ...
  if (cert && os_strncmp(cert, "hash://", 7) == 0) {
    ...
    if (os_strncmp(pos, "server/sha256/", 14) != 0) {
      wpa_printf(MSG_DEBUG, "TLSv1: Unsupported ca_cert hash value '%s'", cert);
      return -1;
    }
    pos += 14;
    if (os_strlen(pos) != 32 * 2) {
      wpa_printf(MSG_DEBUG,
                 "TLSv1: Unexpected SHA256 hash length in ca_cert '%s'", cert);
      return -1;
    }
    if (hexstr2bin(pos, cred->srv_cert_hash, 32) < 0) {
      wpa_printf(MSG_DEBUG, "TLSv1: Invalid SHA256 hash value in ca_cert '%s'",
                 cert);
      return -1;
    }
    cred->server_cert_only = 1;
    cred->ca_cert_verify = 0;
    wpa_printf(MSG_DEBUG, "TLSv1: Checking only server certificate match");
    return 0;
  }
  ...
  if (tlsv1_set_cert_chain(&cred->trusted_certs, cert, cert_blob,
                           cert_blob_len) < 0)
    return -1;
...
tlsv1_set_key_enc_pem(const u8 *key, size_t len, const char *passwd) {
  const u8 *pos, *end;
  unsigned char *der;
  size_t der_len;
  struct crypto_private_key *pkey;
...
static int pkcs12_is_bagtype_oid(struct asn1_oid *oid, unsigned long type) {
  return oid->len == 9 && oid_is_rsadsi(oid) && oid->oid[4] == 1 /* pkcs */ &&
         oid->oid[5] == 12 /* pkcs-12 */ && oid->oid[6] == 10 &&
         oid->oid[7] == 1 /* bagtypes */ && oid->oid[8] == type;
}

static int is_oid_pkcs7(struct asn1_oid *oid) {
  return oid->len == 7 && oid->oid[0] == 1 /* iso */ &&
         oid->oid[1] == 2 /* member-body */ && oid->oid[2] == 840 /* us */ &&
         oid->oid[3] == 113549 /* rsadsi */ && oid->oid[4] == 1 /* pkcs */ &&
         oid->oid[5] == 7 /* pkcs-7 */;
}
...
static int is_oid_pkcs9_friendly_name(struct asn1_oid *oid) {
  return oid->len == 7 && is_oid_pkcs9(oid) && oid->oid[6] == 20;
}

static int is_oid_pkcs9_local_key_id(struct asn1_oid *oid) {
  return oid->len == 7 && is_oid_pkcs9(oid) && oid->oid[6] == 21;
}
...
static int pkcs12_keybag(struct tlsv1_credentials *cred, const u8 *buf,
                         size_t len) {
  /* TODO */
  return 0;
}

static int pkcs12_pkcs8_keybag(struct tlsv1_credentials *cred, const u8 *buf,
  ...
  wpa_printf(MSG_DEBUG, "PKCS #12: Successfully decrypted PKCS8ShroudedKeyBag");
  crypto_private_key_free(cred->key);
  cred->key = key;
...
static int pkcs12_certbag(struct tlsv1_credentials *cred, const u8 *buf,
                          size_t len) {
  struct asn1_hdr hdr;
  struct asn1_oid oid;
  char obuf[80];
  ...
  if (asn1_get_oid(pos, end - pos, &oid, &pos)) {
    wpa_printf(MSG_DEBUG, "PKCS #12: Failed to parse OID (certId)");
    return -1;
  }
  ...
  if (asn1_get_next(hdr.payload, hdr.length, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected OCTET STRING (x509Certificate) - found "
               "class %d tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }

  wpa_hexdump(MSG_DEBUG, "PKCS #12: x509Certificate", hdr.payload, hdr.length);
  if (cred->cert) {
    ...
    if (!cert) {
      wpa_printf(MSG_DEBUG, "PKCS #12: Failed to parse x509Certificate");
      return 0;
    }
...
static int pkcs12_parse_attr_friendly_name(const u8 *pos, const u8 *end) {
   ...
   */
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_BMPSTRING) {
    wpa_printf(
        MSG_DEBUG,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "PKCS #12: friendlyName", hdr.payload,
                    hdr.length);
  return 0;
}
...
static int pkcs12_parse_attr_local_key_id(const u8 *pos, const u8 *end) {
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_OCTETSTRING) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected OCTET STRING (localKeyID) - found class %d "
               "tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  wpa_hexdump_key(MSG_DEBUG, "PKCS #12: localKeyID", hdr.payload, hdr.length);
  return 0;
}
...
static int pkcs12_parse_attr(const u8 *pos, size_t len) {
  ...
  wpa_hexdump_key(MSG_MSGDUMP, "PKCS #12: attrValues", hdr.payload, hdr.length);
  pos = hdr.payload;
  end = hdr.payload + hdr.length;
...
static int pkcs12_safebag(struct tlsv1_credentials *cred, const u8 *buf,
                          size_t len, const char *passwd) {
  struct asn1_hdr hdr;
  struct asn1_oid oid;
  char obuf[80];
  ...
  if (asn1_get_oid(pos, end - pos, &oid, &pos)) {
    wpa_printf(MSG_DEBUG, "PKCS #12: Failed to parse OID (BAG-TYPE)");
    return -1;
  }
  ...
  if (pos < end) {
    ...
    wpa_hexdump_key(MSG_MSGDUMP, "PKCS #12: bagAttributes", hdr.payload,
                    hdr.length);
    ...
    while (pos < end) {
      ...
      if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
        ...
        wpa_printf(MSG_DEBUG,
                   "PKCS #12: Expected SEQUENCE (PKCS12Attribute) - found "
                   "class %d tag 0x%x",
                   hdr.class, hdr.tag);
        return -1;
      }
...
static int pkcs12_safecontents(struct tlsv1_credentials *cred, const u8 *buf,
                               size_t len, const char *passwd) {
  struct asn1_hdr hdr;
  const u8 *pos, *end;
...
static int pkcs12_parse_content_enc_data(struct tlsv1_credentials *cred,
  ...
  wpa_hexdump(MSG_MSGDUMP, "PKCS #12: EncryptedContentInfo", pos, end - pos);
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected SEQUENCE (EncryptedContentInfo) - found "
               "class %d tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  wpa_printf(MSG_DEBUG, "PKCS #12: EncryptedContentInfo::contentType %s", buf);
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC || hdr.tag != 0) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected [0] IMPLICIT (encryptedContent) - found "
               "class %d tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  data = pkcs5_decrypt(enc_alg, enc_alg_len, hdr.payload, hdr.length, passwd,
                       &data_len);
  if (data) {
    wpa_hexdump_key(MSG_MSGDUMP, "PKCS #12: Decrypted encryptedContent", data,
                    data_len);
...
static int pkcs12_parse_content(struct tlsv1_credentials *cred, const u8 *buf,
                                size_t len, const char *passwd) {
  const u8 *pos = buf;
  ...
  wpa_printf(MSG_DEBUG, "PKCS #12: Ignored unsupported contentType %s", txt);

  return 0;
}

static int pkcs12_parse(struct tlsv1_credentials *cred, const u8 *key,
                        size_t len, const char *passwd) {
  struct asn1_hdr hdr;
  const u8 *pos, *end;
  struct asn1_oid oid;
  ...
  if (asn1_get_next(key, len, &hdr) < 0 || hdr.class != ASN1_CLASS_UNIVERSAL ||
      hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected SEQUENCE (PFX) - found class %d tag 0x%x; "
               "assume PKCS #12 not used",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected SEQUENCE (authSafe) - found class %d tag "
               "0x%x; assume PKCS #12 not used",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  /* ContentType ::= OBJECT IDENTIFIER */
  if (asn1_get_oid(pos, end - pos, &oid, &pos)) {
    wpa_printf(MSG_DEBUG, "PKCS #12: Could not find OBJECT IDENTIFIER "
                          "(contentType); assume PKCS #12 not used");
    return -1;
  }
  asn1_oid_to_str(&oid, buf, sizeof(buf));
  wpa_printf(MSG_DEBUG, "PKCS #12: contentType %s", buf);
  if (!is_oid_pkcs7_data(&oid)) {
    wpa_printf(MSG_DEBUG, "PKCS #12: Unsupported contentType %s", buf);
    return -1;
  }
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC || hdr.tag != 0) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected [0] EXPLICIT (content) - found class %d tag "
               "0x%x; assume PKCS #12 not used",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_OCTETSTRING) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected OCTET STRING (Data) - found class %d tag "
               "0x%x; assume PKCS #12 not used",
               hdr.class, hdr.tag);
    return -1;
  }
   ...
   */
  wpa_hexdump(MSG_MSGDUMP, "PKCS #12: Data content", hdr.payload, hdr.length);

  if (asn1_get_next(hdr.payload, hdr.length, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #12: Expected SEQUENCE within Data content - found class "
               "%d tag 0x%x; assume PKCS #12 not used",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  while (end > pos) {
    if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
        hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
      wpa_printf(MSG_DEBUG,
                 "PKCS #12: Expected SEQUENCE (ContentInfo) - found class %d "
                 "tag 0x%x; assume PKCS #12 not used",
                 hdr.class, hdr.tag);
      return -1;
    }
    if (pkcs12_parse_content(cred, hdr.payload, hdr.length, passwd) < 0)
      return -1;
...
static int tlsv1_set_key(struct tlsv1_credentials *cred, const u8 *key,
                         size_t len, const char *passwd) {
...
int tlsv1_set_private_key(struct tlsv1_credentials *cred,
  ...
  if (private_key_blob)
    return tlsv1_set_key(cred, private_key_blob, private_key_blob_len,
                         private_key_passwd);
  ...
  if (private_key) {
    ...
    if (buf == NULL) {
      wpa_printf(MSG_INFO, "TLSv1: Failed to read '%s'", private_key);
      return -1;
    }
...
static int tlsv1_set_dhparams_der(struct tlsv1_credentials *cred, const u8 *dh,
                                  size_t len) {
  struct asn1_hdr hdr;
  const u8 *pos, *end;
...
int tlsv1_set_dhparams(struct tlsv1_credentials *cred, const char *dh_file,
  ...
  if (dh_file) {
    ...
    if (buf == NULL) {
      wpa_printf(MSG_INFO, "TLSv1: Failed to read '%s'", dh_file);
      return -1;
    }
