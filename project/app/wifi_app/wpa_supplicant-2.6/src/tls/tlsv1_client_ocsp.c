...
static unsigned int ocsp_hash_data(struct asn1_oid *alg, const u8 *data,
  ...
  wpa_printf(MSG_DEBUG, "OCSP: Could not calculate hash with alg %s", buf);
  return 0;
}
...
static int tls_process_ocsp_single_response(struct tlsv1_client *conn,
  ...
  /* CertID ::= SEQUENCE */
  if (asn1_get_next(resp, len, &hdr) < 0 || hdr.class != ASN1_CLASS_UNIVERSAL ||
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG,
               "OCSP: Expected OCTET STRING (issuerNameHash) - found class %d "
               "tag 0x%x",
               hdr.class, hdr.tag);
    return -1;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "OCSP: issuerNameHash", name_hash, name_hash_len);
  pos = hdr.payload + hdr.length;

  wpa_hexdump(MSG_DEBUG, "OCSP: Issuer subject DN", issuer->subject_dn,
              issuer->subject_dn_len);
  ...
  if (hash_len == 0 || name_hash_len != hash_len ||
    ...
    wpa_hexdump(MSG_DEBUG, "OCSP: Calculated issuerNameHash", hash, hash_len);
    return -1;
  }
  ...
  if (hash_len == 0 || key_hash_len != hash_len ||
    ...
    wpa_hexdump(MSG_DEBUG, "OCSP: Calculated issuerKeyHash", hash, hash_len);
    return -1;
  }
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
    ...
    wpa_printf(MSG_DEBUG,
               "OCSP: No INTEGER tag found for serialNumber; class=%d tag=0x%x "
               "length=%u",
  ...
  if (serial_number_len != cert->serial_number_len ||
      os_memcmp(serial_number, cert->serial_number, serial_number_len) != 0) {
  ...
  wpa_hexdump(MSG_DEBUG, "OCSP: CertStatus additional data", hdr.payload,
              hdr.length);
  pos = hdr.payload + hdr.length;
  ...
  if (pos < end) {
    ...
    if (hdr.class == ASN1_CLASS_CONTEXT_SPECIFIC && hdr.tag == 0) {
      ...
      if (asn1_get_next(hdr.payload, hdr.length, &hdr) < 0 ||
          ...
          hdr.tag != ASN1_TAG_GENERALIZEDTIME ||
          x509_parse_time(hdr.payload, hdr.length, hdr.tag, &update) < 0) {
        wpa_printf(MSG_DEBUG, "OCSP: Failed to parse nextUpdate");
        return -1;
      }
      wpa_printf(MSG_DEBUG, "OCSP: nextUpdate %lu", (unsigned long)update);
      pos = next;
      if ((unsigned long)now.sec > (unsigned long)update) {
        wpa_printf(MSG_DEBUG,
                   "OCSP: nextUpdate time in the past (response has expired)");
        return -1;
      }
    }
  }
  ...
  /* singleExtensions  [1]  EXPLICIT Extensions OPTIONAL */
  if (pos < end) {
    wpa_hexdump(MSG_MSGDUMP, "OCSP: singleExtensions", pos, end - pos);
    /* Ignore for now */
  }
...
static enum tls_ocsp_result tls_process_ocsp_responses(
    struct tlsv1_client *conn, struct x509_certificate *cert,
    struct x509_certificate *issuer, const u8 *resp, size_t len) {
  struct asn1_hdr hdr;
  const u8 *pos, *end;
  enum tls_ocsp_result res;
  ...
  while (pos < end) {
    ...
    if (tls_process_ocsp_single_response(conn, cert, issuer, hdr.payload,
                                         hdr.length, &res) == 0)
      return res;
    pos = hdr.payload + hdr.length;
  }
...
tls_process_basic_ocsp_response(struct tlsv1_client *conn,
  ...
  if (pos < end) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "OCSP: certs", hdr.payload, hdr.length);
    pos = hdr.payload;
    end = hdr.payload + hdr.length;
    while (pos < end) {
      struct x509_certificate *cert;
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 &&
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
        ...
        hdr.length != 1) {
      wpa_printf(MSG_DEBUG,
                 "OCSP: No INTEGER (len=1) tag found for version field - found "
                 "class %d tag 0x%x length %d",
                 hdr.class, hdr.tag, hdr.length);
      goto fail;
    }
    wpa_printf(MSG_DEBUG, "OCSP: ResponseData version %u", hdr.payload[0]);
    if (hdr.payload[0] != 0) {
      wpa_printf(MSG_DEBUG, "OCSP: Unsupported ResponseData version %u",
                 hdr.payload[0]);
      goto no_resp;
    }
    pos = hdr.payload + hdr.length;
  } else {
    wpa_printf(MSG_DEBUG, "OCSP: Default ResponseData version (v1)");
  }
  ...
  if (hdr.tag == 1) {
    ...
    key_hash = hdr.payload;
    wpa_hexdump(MSG_DEBUG, "OCSP: ResponderID byKey KeyHash", key_hash,
                hdr.length);
    ...
    wpa_printf(MSG_DEBUG, "OCSP: Unexpected ResponderID CHOICE %u", hdr.tag);
    goto fail;
  }
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL ||
      hdr.tag != ASN1_TAG_GENERALIZEDTIME ||
      x509_parse_time(hdr.payload, hdr.length, hdr.tag, &produced_at) < 0) {
    wpa_printf(MSG_DEBUG, "OCSP: Failed to parse producedAt");
    goto fail;
  }
  wpa_printf(MSG_DEBUG, "OCSP: producedAt %lu", (unsigned long)produced_at);
  pos = hdr.payload + hdr.length;
  ...
  if (pos < end) {
    /* responseExtensions  [1] EXPLICIT Extensions OPTIONAL */
    wpa_hexdump(MSG_MSGDUMP, "OCSP: responseExtensions", pos, end - pos);
    /* Ignore for now. */
  }
  ...
  if (srv_cert->next) {
    ...
    for (issuer = conn->cred ? conn->cred->trusted_certs : NULL; issuer;
         issuer = issuer->next) {
      if (x509_name_compare(&srv_cert->issuer, &issuer->subject) == 0)
        break;
    }
  }
  if (!issuer) {
    wpa_printf(MSG_DEBUG, "OCSP: Server certificate issuer not known - cannot "
                          "check OCSP response");
    goto no_resp;
  }
  ...
  if (ocsp_responder_id_match(issuer, &name, key_hash)) {
    ...
    for (signer = certs; signer; signer = signer->next) {
      if (!ocsp_responder_id_match(signer, &name, key_hash) ||
          x509_name_compare(&srv_cert->issuer, &issuer->subject) != 0 ||
          !(signer->ext_key_usage & X509_EXT_KEY_USAGE_OCSP) ||
          x509_certificate_check_signature(issuer, signer) < 0)
        continue;
      wpa_printf(MSG_DEBUG,
                 "OCSP: An extra certificate from the response matches "
                 "ResponderID and is trusted as an OCSP signer");
      break;
    }
    if (!signer) {
      wpa_printf(MSG_DEBUG, "OCSP: Could not find OCSP signer certificate");
      goto no_resp;
    }
  }
  ...
  res = tls_process_ocsp_responses(conn, srv_cert, issuer, responses,
                                   responses_len);
...
enum tls_ocsp_result tls_process_ocsp_response(struct tlsv1_client *conn,
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_ENUMERATED ||
      hdr.length != 1) {
    wpa_printf(MSG_DEBUG,
               "OCSP: Expected ENUMERATED (responseStatus) - found class %d "
               "tag 0x%x length %u",
               hdr.class, hdr.tag, hdr.length);
    return TLS_OCSP_INVALID;
  }
  ...
  if (asn1_get_oid(pos, end - pos, &oid, &pos)) {
    wpa_printf(MSG_DEBUG, "OCSP: Failed to parse OID (responseType)");
    return TLS_OCSP_INVALID;
  }
  ...
  while (cert) {
    if (!cert->ocsp_good && !cert->ocsp_revoked) {
      ...
      wpa_printf(MSG_DEBUG, "OCSP: Trying to find certificate status for %s",
                 sbuf);

      res =
          tls_process_basic_ocsp_response(conn, cert, hdr.payload, hdr.length);
