...
static int tls_version_disabled(struct tlsv1_client *conn, u16 ver) {
  return (
      ((conn->flags & TLS_CONN_DISABLE_TLSv1_0) && ver == TLS_VERSION_1) ||
      ((conn->flags & TLS_CONN_DISABLE_TLSv1_1) && ver == TLS_VERSION_1_1) ||
      ((conn->flags & TLS_CONN_DISABLE_TLSv1_2) && ver == TLS_VERSION_1_2));
}
...
static int tls_process_server_hello_extensions(struct tlsv1_client *conn,
  ...
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: ServerHello extensions", pos, len);
  while (pos < end) {
    u16 ext, elen;
    ...
    wpa_printf(MSG_DEBUG, "TLSv1: ServerHello ExtensionType %u", ext);
    wpa_hexdump(MSG_DEBUG, "TLSv1: ServerHello extension data", pos, elen);
...
static int tls_process_server_hello(struct tlsv1_client *conn, u8 ct,
  ...
  if (*pos != TLS_HANDSHAKE_TYPE_SERVER_HELLO) {
    wpa_printf(MSG_DEBUG,
               ...
               "message %d (expected ServerHello)",
               *pos);
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  }
  ...
  if (!tls_version_ok(tls_version) || tls_version_disabled(conn, tls_version)) {
    wpa_printf(MSG_DEBUG,
  ...
  pos += TLS_RANDOM_LEN;
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: server_random", conn->server_random,
              TLS_RANDOM_LEN);
  ...
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: session_id", conn->session_id,
              conn->session_id_len);
  ...
  if (conn->session_resumed && cipher_suite != conn->prev_cipher_suite) {
    wpa_printf(MSG_DEBUG,
               ...
               "0x%04x)",
               cipher_suite, conn->prev_cipher_suite);
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_ILLEGAL_PARAMETER);
    return -1;
  }
...
static void tls_peer_cert_event(struct tlsv1_client *conn, int depth,
  ...
  if (conn->cred->cert_probe || conn->cert_in_cb) {
    cert_buf = wpabuf_alloc_copy(cert->cert_start, cert->cert_len);
...
static int tls_process_certificate(struct tlsv1_client *conn, u8 ct,
  ...
  if (conn->cred && conn->cred->server_cert_only && chain) {
    ...
    if (sha256_vector(1, &chain->cert_start, &chain->cert_len, hash) < 0 ||
        os_memcmp(conn->cred->srv_cert_hash, hash, SHA256_MAC_LEN) != 0) {
      wpa_printf(MSG_DEBUG, "TLSv1: Server certificate hash mismatch");
      wpa_hexdump(MSG_MSGDUMP, "TLSv1: SHA256 hash", hash, SHA256_MAC_LEN);
      if (conn->event_cb) {
        union tls_event_data ev;

        os_memset(&ev, 0, sizeof(ev));
        ev.cert_fail.reason = TLS_FAIL_UNSPECIFIED;
        ev.cert_fail.reason_txt = "Server certificate mismatch";
    ...
    if (conn->event_cb) {
      ...
      ev.cert_fail.reason_txt = "Server certificate chain probe";
      if (chain) {
        x509_name_string(&chain->subject, buf, sizeof(buf));
        ev.cert_fail.subject = buf;
      }
    ...
    return -1;
             ...
             x509_certificate_chain_validate(
                 ...
                 !!(conn->flags & TLS_CONN_DISABLE_TIME_CHECKS)) < 0) {
    int tls_reason;
    wpa_printf(MSG_DEBUG,
               "TLSv1: Server certificate chain "
               "validation failed (reason=%d)",
               reason);
    switch (reason) {
    case X509_VALIDATE_BAD_CERTIFICATE:
      tls_reason = TLS_ALERT_BAD_CERTIFICATE;
      tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_BAD_CERTIFICATE,
                                   "bad certificate");
      break;
    ...
    case X509_VALIDATE_CERTIFICATE_REVOKED:
      ...
      tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_REVOKED,
                                   "certificate revoked");
      break;
    ...
    case X509_VALIDATE_UNKNOWN_CA:
      ...
      tls_cert_chain_failure_event(conn, 0, chain, TLS_FAIL_UNTRUSTED,
                                   "unknown CA");
      break;
    default:
      tls_reason = TLS_ALERT_BAD_CERTIFICATE;
...
static int tlsv1_process_diffie_hellman(struct tlsv1_client *conn,
  ...
  pos += conn->dh_ys_len;
  wpa_hexdump(MSG_DEBUG, "TLSv1: DH Ys (server's public value)", conn->dh_ys,
              conn->dh_ys_len);
  ...
  if (key_exchange == TLS_KEY_X_DHE_RSA) {
    ...
    if (conn->rl.tls_version == TLS_VERSION_1_2) {
      ...
      if (end - pos < 2)
        goto fail;
      if ((pos[0] != TLS_HASH_ALG_SHA256 && pos[0] != TLS_HASH_ALG_SHA384 &&
      ...
      hlen = tlsv12_key_x_server_params_hash(
          conn->rl.tls_version, pos[0], conn->client_random,
          conn->server_random, server_params, server_params_end - server_params,
          hash);
      pos += 2;
#else  /* CONFIG_TLSV12 */
      goto fail;
      ...
      hlen = tls_key_x_server_params_hash(
          conn->rl.tls_version, conn->client_random, conn->server_random,
          server_params, server_params_end - server_params, hash);
    }
    ...
    if (tls_verify_signature(conn->rl.tls_version, conn->server_rsa_key, hash,
...
static int tls_process_certificate_status(struct tlsv1_client *conn, u8 ct,
  ...
  if (ct != TLS_CONTENT_TYPE_HANDSHAKE) {
    wpa_printf(MSG_DEBUG,
               "TLSv1: Expected Handshake; received content type 0x%x", ct);
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  }
  ...
  if (left < 4) {
    wpa_printf(MSG_DEBUG, "TLSv1: Too short CertificateStatus (left=%lu)",
               (unsigned long)left);
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
    return -1;
  }
  ...
  if (type != TLS_HANDSHAKE_TYPE_CERTIFICATE_STATUS) {
    wpa_printf(MSG_DEBUG,
               "TLSv1: Received unexpected handshake message %d (expected "
               "CertificateStatus)",
               type);
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  ...
  status_type = *pos++;
  wpa_printf(MSG_DEBUG, "TLSv1: CertificateStatus status_type %u", status_type);

  if (status_type == 1 /* ocsp */) {
    res = tls_process_certificate_status_ocsp_response(conn, pos, end - pos);
     ...
     */
    if (end - pos < 3) {
      wpa_printf(MSG_DEBUG, "TLSv1: Truncated OCSPResponseList");
    ...
    pos += 3;
    if (end - pos < resp_len) {
      wpa_printf(MSG_DEBUG, "TLSv1: Truncated OCSPResponseList(len=%u)",
                 resp_len);
      res = TLS_OCSP_INVALID;
      goto done;
    }
    ...
    while (end - pos >= 3) {
      ...
      res = tls_process_certificate_status_ocsp_response(conn, pos - 3,
                                                         resp_len + 3);
    ...
    wpa_printf(MSG_DEBUG, "TLSv1: Ignore unsupported CertificateStatus");
    goto skip;
  }

done:
  if (res == TLS_OCSP_REVOKED) {
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_CERTIFICATE_REVOKED);
    for (cert = conn->server_cert, depth = 0; cert;
         cert = cert->next, depth++) {
      if (cert->ocsp_revoked) {
        tls_cert_chain_failure_event(conn, depth, cert, TLS_FAIL_REVOKED,
                                     "certificate revoked");
      }
    }
    return -1;
  ...
  if (conn->flags & TLS_CONN_REQUIRE_OCSP_ALL) {
    ...
    for (cert = conn->server_cert, depth = 0; cert;
      ...
      if (!cert->ocsp_good) {
        ...
        tls_cert_chain_failure_event(conn, depth, cert, TLS_FAIL_UNSPECIFIED,
  ...
  if ((conn->flags & TLS_CONN_REQUIRE_OCSP) && res != TLS_OCSP_GOOD) {
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL,
              res == TLS_OCSP_INVALID
                  ...
                  : TLS_ALERT_BAD_CERTIFICATE_STATUS_RESPONSE);
    if (conn->server_cert)
      tls_cert_chain_failure_event(conn, 0, conn->server_cert,
...
static int tls_process_server_key_exchange(struct tlsv1_client *conn, u8 ct,
  ...
  if ((conn->flags & TLS_CONN_REQUEST_OCSP) &&
    ...
    return tls_process_certificate_status(conn, ct, in_data, in_len);
  if (type == TLS_HANDSHAKE_TYPE_CERTIFICATE_REQUEST)
    return tls_process_certificate_request(conn, ct, in_data, in_len);
  if (type == TLS_HANDSHAKE_TYPE_SERVER_HELLO_DONE)
    return tls_process_server_hello_done(conn, ct, in_data, in_len);
  if (type != TLS_HANDSHAKE_TYPE_SERVER_KEY_EXCHANGE) {
    wpa_printf(MSG_DEBUG,
               ...
               "CertificateRequest/ServerHelloDone%s)",
               type,
               (conn->flags & TLS_CONN_REQUEST_OCSP) ? "/CertificateStatus"
                                                     : "");
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  }
...
static int tls_process_server_hello_done(struct tlsv1_client *conn, u8 ct,
  ...
  if ((conn->flags & TLS_CONN_REQUIRE_OCSP) && !conn->ocsp_resp_received) {
    wpa_printf(MSG_INFO, "TLSv1: No OCSP response received - reject handshake");
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL,
              TLS_ALERT_BAD_CERTIFICATE_STATUS_RESPONSE);
    return -1;
  }
...
static int tls_process_server_change_cipher_spec(struct tlsv1_client *conn,
  ...
  if (ct != TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC) {
    ...
    if (conn->use_session_ticket) {
      ...
      res = conn->session_ticket_cb(conn->session_ticket_cb_ctx, NULL, 0, NULL,
                                    NULL, NULL);
      if (res < 0) {
...
static int tls_process_server_finished(struct tlsv1_client *conn, u8 ct,
  ...
  if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN,
              "server finished", hash, hlen, verify_data,
              TLS_VERIFY_DATA_LEN)) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to derive verify_data");
    tls_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECRYPT_ERROR);
    return -1;
  }
  wpa_hexdump_key(MSG_DEBUG, "TLSv1: verify_data (server)", verify_data,
                  TLS_VERIFY_DATA_LEN);
...
static int tls_process_application_data(struct tlsv1_client *conn, u8 ct,
  ...
  wpa_hexdump(MSG_DEBUG, "TLSv1: Application Data included in Handshake", pos,
              left);
...
int tlsv1_client_process_handshake(struct tlsv1_client *conn, u8 ct,
                                   const u8 *buf, size_t *len, u8 **out_data,
                                   size_t *out_len) {
  ...
  case ACK_FINISHED:
    if (out_data &&
        tls_process_application_data(conn, ct, buf, len, out_data, out_len))
      return -1;
    break;
  default:
    wpa_printf(MSG_DEBUG,
