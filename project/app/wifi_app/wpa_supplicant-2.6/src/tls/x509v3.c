...
static int x509_whitespace(char c) { return c == ' ' || c == '\t'; }
...
static int x509_parse_public_key(const u8 *buf, size_t len,
  ...
  *next = end;
  ...
  if (x509_parse_algorithm_identifier(pos, end - pos, &cert->public_key_alg,
                                      &pos))
    return -1;
  ...
  pos = hdr.payload;
  if (*pos) {
    wpa_printf(MSG_DEBUG, "X509: BITSTRING - %d unused bits", *pos);
    /*
  ...
  wpa_hexdump(MSG_MSGDUMP, "X509: subjectPublicKey", cert->public_key,
              cert->public_key_len);
...
int x509_parse_name(const u8 *buf, size_t len, struct x509_name *name,
  ...
  while (pos < end) {
    ...
    if (oid.len == 4 && oid.oid[0] == 2 && oid.oid[1] == 5 && oid.oid[2] == 4) {
      /* id-at ::= 2.5.4 */
      switch (oid.oid[3]) {
      case 3:
        /* commonName */
    ...
    } else if (oid.len == 7 && oid.oid[0] == 1 && oid.oid[1] == 2 &&
               oid.oid[2] == 840 && oid.oid[3] == 113549 && oid.oid[4] == 1 &&
               oid.oid[5] == 9 && oid.oid[6] == 1) {
      ...
      continue;
    } else if (oid.len == 7 && oid.oid[0] == 0 && oid.oid[1] == 9 &&
               oid.oid[2] == 2342 && oid.oid[3] == 19200300 &&
               oid.oid[4] == 100 && oid.oid[5] == 1 && oid.oid[6] == 25) {
      /* 0.9.2342.19200300.100.1.25 - domainComponent */
      type = X509_NAME_ATTR_DC;
    }

    if (type == X509_NAME_ATTR_NOT_USED) {
      wpa_hexdump(MSG_DEBUG, "X509: Unrecognized OID", (u8 *)oid.oid,
                  oid.len * sizeof(oid.oid[0]));
      wpa_hexdump_ascii(MSG_MSGDUMP, "X509: Attribute Data", hdr.payload,
                        hdr.length);
      continue;
    }
...
void x509_name_string(struct x509_name *name, char *buf, size_t len) {
  ...
  if (name->email) {
    ret = os_snprintf(pos, end - pos, "/emailAddress=%s", name->email);
    if (os_snprintf_error(end - pos, ret))
      goto done;
    pos += ret;
  }
...
int x509_parse_time(const u8 *buf, size_t len, u8 asn1_tag, os_time_t *val) {
  ...
  default:
    wpa_printf(MSG_DEBUG,
               ...
               "GeneralizedTime - found tag 0x%x",
               asn1_tag);
    return -1;
  }
  ...
  if (os_mktime(year, month, day, hour, min, sec, val) < 0) {
    wpa_hexdump_ascii(MSG_DEBUG, "X509: Failed to convert Time", buf, len);
    if (year < 1970) {
      /*
...
static int x509_parse_validity(const u8 *buf, size_t len,
  ...
  *next = pos + plen;
  ...
  if (asn1_get_next(pos, plen, &hdr) < 0 || hdr.class != ASN1_CLASS_UNIVERSAL ||
      x509_parse_time(hdr.payload, hdr.length, hdr.tag, &cert->not_before) <
          0) {
    wpa_hexdump_ascii(MSG_DEBUG,
  ...
  if (asn1_get_next(pos, plen, &hdr) < 0 || hdr.class != ASN1_CLASS_UNIVERSAL ||
      x509_parse_time(hdr.payload, hdr.length, hdr.tag, &cert->not_after) < 0) {
    wpa_hexdump_ascii(MSG_DEBUG,
  ...
  wpa_printf(MSG_MSGDUMP, "X509: Validity: notBefore: %lu notAfter: %lu",
             (unsigned long)cert->not_before, (unsigned long)cert->not_after);
...
static int x509_id_ce_oid(struct asn1_oid *oid) {
  ...
  return oid->len >= 4 && oid->oid[0] == 2 /* joint-iso-ccitt */ &&
         oid->oid[1] == 5 /* ds */ && oid->oid[2] == 29 /* id-ce */;
}

static int x509_any_ext_key_usage_oid(struct asn1_oid *oid) {
  return oid->len == 6 && x509_id_ce_oid(oid) &&
...
static int x509_parse_ext_key_usage(struct x509_certificate *cert,
  ...
  if (asn1_get_next(pos, len, &hdr) < 0 || hdr.class != ASN1_CLASS_UNIVERSAL ||
      hdr.tag != ASN1_TAG_BITSTRING || hdr.length < 1) {
    wpa_printf(MSG_DEBUG,
...
static int x509_parse_alt_name_rfc8222(struct x509_name *name, const u8 *pos,
                                       size_t len) {
...
static int x509_parse_alt_name_dns(struct x509_name *name, const u8 *pos,
                                   size_t len) {
  /* dNSName IA5String */
...
static int x509_parse_alt_name_uri(struct x509_name *name, const u8 *pos,
                                   size_t len) {
  /* uniformResourceIdentifier IA5String */
  wpa_hexdump_ascii(MSG_MSGDUMP, "X509: altName - uniformResourceIdentifier",
                    pos, len);
  os_free(name->uri);
...
static int x509_parse_alt_name_ip(struct x509_name *name, const u8 *pos,
                                  size_t len) {
...
static int x509_parse_alt_name_rid(struct x509_name *name, const u8 *pos,
                                   size_t len) {
  char buf[80];
...
static int x509_parse_ext_alt_name(struct x509_name *name, const u8 *pos,
                                   size_t len) {
  struct asn1_hdr hdr;
  const u8 *p, *end;
  ...
  for (p = pos, end = pos + len; p < end; p = hdr.payload + hdr.length) {
    ...
    switch (hdr.tag) {
    case 1:
      res = x509_parse_alt_name_rfc8222(name, hdr.payload, hdr.length);
      break;
    case 2:
      res = x509_parse_alt_name_dns(name, hdr.payload, hdr.length);
      break;
    case 6:
      res = x509_parse_alt_name_uri(name, hdr.payload, hdr.length);
      break;
    case 7:
      res = x509_parse_alt_name_ip(name, hdr.payload, hdr.length);
      break;
    case 8:
      res = x509_parse_alt_name_rid(name, hdr.payload, hdr.length);
      break;
...
static int x509_parse_ext_subject_alt_name(struct x509_certificate *cert,
  ...
  return x509_parse_ext_alt_name(&cert->subject, hdr.payload, hdr.length);
}
...
static int x509_parse_ext_issuer_alt_name(struct x509_certificate *cert,
  ...
  return x509_parse_ext_alt_name(&cert->issuer, hdr.payload, hdr.length);
}

static int x509_id_pkix_oid(struct asn1_oid *oid) {
  return oid->len >= 7 && oid->oid[0] == 1 /* iso */ &&
         oid->oid[1] == 3 /* identified-organization */ &&
         oid->oid[2] == 6 /* dod */ && oid->oid[3] == 1 /* internet */ &&
         oid->oid[4] == 5 /* security */ && oid->oid[5] == 5 /* mechanisms */ &&
         oid->oid[6] == 7 /* id-pkix */;
}

static int x509_id_kp_oid(struct asn1_oid *oid) {
  /* id-kp */
  return oid->len >= 8 && x509_id_pkix_oid(oid) && oid->oid[7] == 3 /* id-kp */;
}

static int x509_id_kp_server_auth_oid(struct asn1_oid *oid) {
  /* id-kp */
  return oid->len == 9 && x509_id_kp_oid(oid) &&
...
static int x509_parse_extension(struct x509_certificate *cert, const u8 *pos,
  ...
  *next = end = pos + hdr.length;
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL ||
      (hdr.tag != ASN1_TAG_BOOLEAN && hdr.tag != ASN1_TAG_OCTETSTRING)) {
    wpa_printf(MSG_DEBUG,
               "X509: Unexpected ASN.1 header in "
  ...
  asn1_oid_to_str(&oid, buf, sizeof(buf));
  wpa_printf(MSG_DEBUG, "X509: Extension: extnID=%s critical=%d", buf,
             critical_ext);
  ...
  if (res == 1 && critical_ext) {
    wpa_printf(MSG_INFO, "X509: Unknown critical extension %s", buf);
    return -1;
  }
...
static int x509_parse_tbs_certificate(const u8 *buf, size_t len,
  ...
  if (hdr.class == ASN1_CLASS_CONTEXT_SPECIFIC) {
    ...
    cert->version = value;
    if (cert->version != X509_CERT_V1 && cert->version != X509_CERT_V2 &&
        cert->version != X509_CERT_V3) {
      wpa_printf(MSG_DEBUG, "X509: Unsupported version %d", cert->version + 1);
      return -1;
    }
  ...
  if (x509_parse_algorithm_identifier(pos, end - pos, &cert->signature, &pos))
    return -1;
  ...
  pos = hdr.payload + hdr.length;
  if (pos < end) {
    wpa_hexdump(MSG_DEBUG, "X509: Ignored extra tbsCertificate data", pos,
                end - pos);
  }
...
static int x509_rsadsi_oid(struct asn1_oid *oid) {
  return oid->len >= 4 && oid->oid[0] == 1 /* iso */ &&
         oid->oid[1] == 2 /* member-body */ && oid->oid[2] == 840 /* us */ &&
         oid->oid[3] == 113549 /* rsadsi */;
}

static int x509_pkcs_oid(struct asn1_oid *oid) {
  return oid->len >= 5 && x509_rsadsi_oid(oid) && oid->oid[4] == 1 /* pkcs */;
}

static int x509_digest_oid(struct asn1_oid *oid) {
  return oid->len >= 5 && x509_rsadsi_oid(oid) &&
         oid->oid[4] == 2 /* digestAlgorithm */;
}

int x509_sha1_oid(struct asn1_oid *oid) {
  return oid->len == 6 && oid->oid[0] == 1 /* iso */ &&
         oid->oid[1] == 3 /* identified-organization */ &&
         oid->oid[2] == 14 /* oiw */ && oid->oid[3] == 3 /* secsig */ &&
         oid->oid[4] == 2 /* algorithms */ && oid->oid[5] == 26 /* id-sha1 */;
}

static int x509_sha2_oid(struct asn1_oid *oid) {
  return oid->len == 9 && oid->oid[0] == 2 /* joint-iso-itu-t */ &&
         oid->oid[1] == 16 /* country */ && oid->oid[2] == 840 /* us */ &&
         oid->oid[3] == 1 /* organization */ && oid->oid[4] == 101 /* gov */ &&
         oid->oid[5] == 3 /* csor */ && oid->oid[6] == 4 /* nistAlgorithm */ &&
         oid->oid[7] == 2 /* hashAlgs */;
}

int x509_sha256_oid(struct asn1_oid *oid) {
  return x509_sha2_oid(oid) && oid->oid[8] == 1 /* sha256 */;
}

int x509_sha384_oid(struct asn1_oid *oid) {
  return x509_sha2_oid(oid) && oid->oid[8] == 2 /* sha384 */;
}

int x509_sha512_oid(struct asn1_oid *oid) {
  return x509_sha2_oid(oid) && oid->oid[8] == 3 /* sha512 */;
}
...
struct x509_certificate *x509_certificate_parse(const u8 *buf, size_t len) {
  ...
  if (x509_parse_algorithm_identifier(pos, end - pos, &cert->signature_alg,
                                      &pos)) {
    x509_certificate_free(cert);
    return NULL;
  }
  ...
  wpa_hexdump(MSG_MSGDUMP, "X509: signature", cert->sign_value,
              cert->sign_value_len);
...
int x509_certificate_check_signature(struct x509_certificate *issuer,
                                     struct x509_certificate *cert) {
  return x509_check_signature(issuer, &cert->signature, cert->sign_value,
                              cert->sign_value_len, cert->tbs_cert_start,
                              cert->tbs_cert_len);
}
...
int x509_check_signature(struct x509_certificate *issuer,
  ...
  if (!x509_pkcs_oid(&signature->oid) || signature->oid.len != 7 ||
  ...
  if (crypto_public_key_decrypt_pkcs1(pk, sign_value, sign_value_len, data,
                                      &data_len) < 0) {
  ...
  if (x509_sha256_oid(&oid)) {
    if (signature->oid.oid[6] != 11 /* sha2561WithRSAEncryption */) {
      wpa_printf(MSG_DEBUG,
                 "X509: digestAlgorithm SHA256 "
                 "does not match with certificate "
                 "signatureAlgorithm (%lu)",
                 signature->oid.oid[6]);
      os_free(data);
...
skip_digest_oid:
  ...
  wpa_hexdump(MSG_MSGDUMP, "X509: Decrypted Digest", hdr.payload, hdr.length);

  switch (signature->oid.oid[6]) {
  case 4: /* md5WithRSAEncryption */
    md5_vector(1, addr, len, hash);
    hash_len = 16;
    wpa_hexdump(MSG_MSGDUMP, "X509: Certificate hash (MD5)", hash, hash_len);
    break;
  case 5: /* sha-1WithRSAEncryption */
    sha1_vector(1, addr, len, hash);
    hash_len = 20;
    wpa_hexdump(MSG_MSGDUMP, "X509: Certificate hash (SHA1)", hash, hash_len);
    break;
  case 11: /* sha256WithRSAEncryption */
    sha256_vector(1, addr, len, hash);
    hash_len = 32;
    wpa_hexdump(MSG_MSGDUMP, "X509: Certificate hash (SHA256)", hash, hash_len);
    break;
  case 12: /* sha384WithRSAEncryption */
    sha384_vector(1, addr, len, hash);
    hash_len = 48;
    wpa_hexdump(MSG_MSGDUMP, "X509: Certificate hash (SHA384)", hash, hash_len);
    break;
  case 13: /* sha512WithRSAEncryption */
    sha512_vector(1, addr, len, hash);
    hash_len = 64;
    wpa_hexdump(MSG_MSGDUMP, "X509: Certificate hash (SHA512)", hash, hash_len);
    break;
  ...
  if (hdr.payload + hdr.length < data + data_len) {
    wpa_hexdump(MSG_INFO, "X509: Extra data after certificate signature hash",
                hdr.payload + hdr.length,
...
static int x509_valid_issuer(const struct x509_certificate *cert) {
  if ((cert->extensions_present & X509_EXT_BASIC_CONSTRAINTS) && !cert->ca) {
...
int x509_certificate_chain_validate(struct x509_certificate *trusted,
                                    struct x509_certificate *chain, int *reason,
  ...
  for (cert = chain, idx = 0; cert; cert = cert->next, idx++) {
    ...
    if (cert->next) {
      if (x509_name_compare(&cert->issuer, &cert->next->subject) != 0) {
        ...
        wpa_printf(MSG_DEBUG, "X509: cert issuer: %s", buf);
        x509_name_string(&cert->next->subject, buf, sizeof(buf));
        wpa_printf(MSG_DEBUG,
                   "X509: next cert "
                   "subject: %s",
                   buf);
      ...
      if ((cert->next->extensions_present & X509_EXT_PATH_LEN_CONSTRAINT) &&
      ...
      if (x509_certificate_check_signature(cert->next, cert) < 0) {
