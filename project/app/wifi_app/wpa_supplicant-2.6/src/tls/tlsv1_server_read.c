...
static int tls_process_change_cipher_spec(struct tlsv1_server *conn, u8 ct,
                                          const u8 *in_data, size_t *in_len);
...
static int tls_process_client_hello(struct tlsv1_server *conn, u8 ct,
  ...
  if (*pos != TLS_HANDSHAKE_TYPE_CLIENT_HELLO) {
    tlsv1_server_log(
        conn, "Received unexpected handshake message %d (expected ClientHello)",
        *pos);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL,
                       TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  ...
  tlsv1_server_log(conn, "Client version %d.%d", conn->client_version >> 8,
  ...
  if (conn->client_version < TLS_VERSION_1) {
    ...
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_PROTOCOL_VERSION);
    return -1;
  }
  pos += 2;
  ...
  pos += TLS_RANDOM_LEN;
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: client_random", conn->client_random,
              TLS_RANDOM_LEN);
  ...
  if (end - pos < num_suites)
    goto decode_error;
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: client cipher suites", pos, num_suites);
  if (num_suites & 1)
    goto decode_error;
  num_suites /= 2;
  ...
  if (tlsv1_record_set_cipher_suite(&conn->rl, cipher_suite) < 0) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to set CipherSuite for "
                          "record layer");
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
  ...
  if (end - pos < num_suites)
    goto decode_error;
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: client compression_methods", pos,
              num_suites);
  compr_null_found = 0;
  ...
  if (end - pos == 1) {
    tlsv1_server_log(
        conn, "Unexpected extra octet in the end of ClientHello: 0x%02x", *pos);
    goto decode_error;
  }
  ...
  if (end - pos >= 2) {
    ...
    tlsv1_server_log(conn, "%u bytes of ClientHello extensions", ext_len);
    if (end - pos != ext_len) {
      tlsv1_server_log(
    ...
    while (pos < end) {
      ...
      tlsv1_server_log(conn, "ClientHello Extension type %u", ext_type);
      ...
      if (ext_type == TLS_EXT_SESSION_TICKET) {
        ...
        if (conn->session_ticket) {
          os_memcpy(conn->session_ticket, pos, ext_len);
        ...
        tls_process_status_request_v2(conn, pos, ext_len);
      }
...
decode_error:
  ...
  tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
  return -1;
}
...
static int tls_process_certificate(struct tlsv1_server *conn, u8 ct,
  ...
  if (len > left) {
    tlsv1_server_log(
        conn, "Unexpected Certificate message length (len=%lu != left=%lu)",
  ...
  if (type == TLS_HANDSHAKE_TYPE_CLIENT_KEY_EXCHANGE) {
    ...
    return tls_process_client_key_exchange(conn, ct, in_data, in_len);
  }
  if (type != TLS_HANDSHAKE_TYPE_CERTIFICATE) {
    tlsv1_server_log(conn,
                     "Received unexpected handshake message %d (expected "
  ...
  if ((size_t)(end - pos) != list_len) {
    tlsv1_server_log(conn,
                     "Unexpected certificate_list length (len=%lu left=%lu)",
                     (unsigned long)list_len, (unsigned long)(end - pos));
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
    return -1;
  }
  ...
  while (pos < end) {
    ...
    if ((size_t)(end - pos) < cert_len) {
      tlsv1_server_log(conn, "Unexpected certificate length (len=%lu left=%lu)",
                       (unsigned long)cert_len, (unsigned long)(end - pos));
      tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
      x509_certificate_chain_free(chain);
      return -1;
    }

    tlsv1_server_log(conn, "Certificate %lu (len %lu)", (unsigned long)idx,
                     (unsigned long)cert_len);

    if (idx == 0) {
      crypto_public_key_free(conn->client_rsa_key);
      if (tls_parse_cert(pos, cert_len, &conn->client_rsa_key)) {
  ...
  if (x509_certificate_chain_validate(conn->cred->trusted_certs, chain, &reason,
                                      0) < 0) {
    int tls_reason;
    tlsv1_server_log(
        conn, "Server certificate chain validation failed (reason=%d)", reason);
    switch (reason) {
...
static int tls_process_client_key_exchange_rsa(struct tlsv1_server *conn,
  ...
  if (pos + encr_len > end) {
    tlsv1_server_log(conn,
                     "Invalid ClientKeyExchange format: encr_len=%u left=%u",
  ...
  out = os_malloc(
      outlen >= TLS_PRE_MASTER_SECRET_LEN ? outlen : TLS_PRE_MASTER_SECRET_LEN);
  if (out == NULL) {
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
  ...
  if (crypto_private_key_decrypt_pkcs1_v15(conn->cred->key, pos, encr_len, out,
                                           &outlen) < 0) {
  ...
  if (!use_random && WPA_GET_BE16(out) != conn->client_version) {
    tlsv1_server_log(conn, "Client version in ClientKeyExchange does not match "
                           "with version in ClientHello");
    use_random = 1;
  }
...
static int tls_process_client_key_exchange_dh(struct tlsv1_server *conn,
                                              const u8 *pos, const u8 *end) {
  const u8 *dh_yc;
  u16 dh_yc_len;
  u8 *shared;
  ...
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: ClientDiffieHellmanPublic", pos, end - pos);
  ...
  wpa_hexdump(MSG_DEBUG, "TLSv1: DH Yc (client's public value)", dh_yc,
              dh_yc_len);
  ...
  if (crypto_mod_exp(dh_yc, dh_yc_len, conn->dh_secret, conn->dh_secret_len,
                     dh_p, dh_p_len, shared, &shared_len)) {
    os_free(shared);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
    return -1;
  }
...
static int tls_process_client_key_exchange(struct tlsv1_server *conn, u8 ct,
  ...
  if (len > left) {
    tlsv1_server_log(
        conn, "Mismatch in ClientKeyExchange length (len=%lu != left=%lu)",
        (unsigned long)len, (unsigned long)left);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
    return -1;
  }
  ...
  if (type != TLS_HANDSHAKE_TYPE_CLIENT_KEY_EXCHANGE) {
    tlsv1_server_log(
        conn,
        "Received unexpected handshake message %d (expected ClientKeyExchange)",
        type);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL,
                       TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
...
static int tls_process_certificate_verify(struct tlsv1_server *conn, u8 ct,
  ...
  if (ct == TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC) {
    ...
    return tls_process_change_cipher_spec(conn, ct, in_data, in_len);
  }
  ...
  if (len > left) {
    tlsv1_server_log(
        conn,
        "Unexpected CertificateVerify message length (len=%lu != left=%lu)",
        (unsigned long)len, (unsigned long)left);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
    return -1;
  }
  ...
  if (type != TLS_HANDSHAKE_TYPE_CERTIFICATE_VERIFY) {
    tlsv1_server_log(
        conn,
        "Received unexpected handshake message %d (expected CertificateVerify)",
        type);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL,
                       TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  ...
  if (conn->rl.tls_version == TLS_VERSION_1_2) {
    ...
    if (conn->verify.sha256_cert == NULL ||
        crypto_hash_finish(conn->verify.sha256_cert, hpos, &hlen) < 0) {
  ...
  if (tls_verify_signature(conn->rl.tls_version, conn->client_rsa_key, hash,
                           hlen, pos, end - pos, &alert) < 0) {
...
static int tls_process_change_cipher_spec(struct tlsv1_server *conn, u8 ct,
  ...
  if (ct != TLS_CONTENT_TYPE_CHANGE_CIPHER_SPEC) {
    tlsv1_server_log(
        conn, "Expected ChangeCipherSpec; received content type 0x%x", ct);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL,
                       TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  }
...
static int tls_process_client_finished(struct tlsv1_server *conn, u8 ct,
  ...
  if ((conn->test_flags &
    ...
    tlsv1_server_log(conn, "TEST-FAILURE: Client Finished received after "
                           "invalid ServerKeyExchange");
    conn->test_failure_reported = 1;
  }

  if ((conn->test_flags & TLS_DHE_PRIME_15) && !conn->test_failure_reported) {
    tlsv1_server_log(
        conn,
        "TEST-FAILURE: Client Finished received after bogus DHE \"prime\" 15");
    conn->test_failure_reported = 1;
  }

  if ((conn->test_flags & TLS_DHE_PRIME_58B) && !conn->test_failure_reported) {
    tlsv1_server_log(conn, "TEST-FAILURE: Client Finished received after short "
                           "58-bit DHE prime in long container");
    conn->test_failure_reported = 1;
  }

  if ((conn->test_flags & TLS_DHE_PRIME_511B) && !conn->test_failure_reported) {
    tlsv1_server_log(conn, "TEST-WARNING: Client Finished received after short "
                           "511-bit DHE prime (insecure)");
    conn->test_failure_reported = 1;
  }

  if ((conn->test_flags & TLS_DHE_PRIME_767B) && !conn->test_failure_reported) {
    tlsv1_server_log(conn, "TEST-NOTE: Client Finished received after 767-bit "
                           "DHE prime (relatively insecure)");
    conn->test_failure_reported = 1;
  }

  if ((conn->test_flags & TLS_DHE_NON_PRIME) && !conn->test_failure_reported) {
    tlsv1_server_log(conn, "TEST-NOTE: Client Finished received after "
                           "non-prime claimed as DHE prime");
    conn->test_failure_reported = 1;
  }
#endif /* CONFIG_TESTING_OPTIONS */

  if (ct != TLS_CONTENT_TYPE_HANDSHAKE) {
    tlsv1_server_log(conn, "Expected Finished; received content type 0x%x", ct);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL,
                       TLS_ALERT_UNEXPECTED_MESSAGE);
    return -1;
  }
  ...
  if (len != TLS_VERIFY_DATA_LEN) {
    tlsv1_server_log(
        conn, "Unexpected verify_data length in Finished: %lu (expected %d)",
        (unsigned long)len, TLS_VERIFY_DATA_LEN);
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECODE_ERROR);
    return -1;
  }
  wpa_hexdump(MSG_MSGDUMP, "TLSv1: verify_data in Finished", pos,
              TLS_VERIFY_DATA_LEN);
  ...
  if (conn->rl.tls_version >= TLS_VERSION_1_2) {
    ...
    if (conn->verify.sha256_client == NULL ||
        crypto_hash_finish(conn->verify.sha256_client, hash, &hlen) < 0) {
      tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_INTERNAL_ERROR);
  ...
  if (tls_prf(conn->rl.tls_version, conn->master_secret, TLS_MASTER_SECRET_LEN,
              "client finished", hash, hlen, verify_data,
              TLS_VERIFY_DATA_LEN)) {
    wpa_printf(MSG_DEBUG, "TLSv1: Failed to derive verify_data");
    tlsv1_server_alert(conn, TLS_ALERT_LEVEL_FATAL, TLS_ALERT_DECRYPT_ERROR);
    return -1;
  }
  wpa_hexdump_key(MSG_DEBUG, "TLSv1: verify_data (client)", verify_data,
                  TLS_VERIFY_DATA_LEN);
...
int tlsv1_server_process_handshake(struct tlsv1_server *conn, u8 ct,
  ...
  default:
    tlsv1_server_log(conn,
                     "Unexpected state %d while processing received message",
                     conn->state);
    return -1;
  }
