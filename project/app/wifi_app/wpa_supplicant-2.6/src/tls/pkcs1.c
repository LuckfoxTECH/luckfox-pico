...
int pkcs1_encrypt(int block_type, struct crypto_rsa_key *key, int use_private,
...
int pkcs1_v15_private_key_decrypt(struct crypto_rsa_key *key, const u8 *in,
...
int pkcs1_decrypt_public_key(struct crypto_rsa_key *key, const u8 *crypt,
                             size_t crypt_len, u8 *plain, size_t *plain_len) {
  size_t len;
  u8 *pos;
...
int pkcs1_v15_sig_ver(struct crypto_public_key *pk, const u8 *s, size_t s_len,
                      const struct asn1_oid *hash_alg, const u8 *hash,
                      size_t hash_len) {
  int res;
  u8 *decrypted;
  ...
  decrypted_len = s_len;
  res =
      crypto_public_key_decrypt_pkcs1(pk, s, s_len, decrypted, &decrypted_len);
  if (res < 0) {
  ...
  if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
      hdr.class != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_SEQUENCE) {
    wpa_printf(MSG_DEBUG,
               "PKCS #1: Expected SEQUENCE (AlgorithmIdentifier) - found class "
               "%d tag 0x%x",
               hdr.class, hdr.tag);
    os_free(decrypted);
    return -1;
  ...
  if (asn1_get_oid(hdr.payload, hdr.length, &oid, &next)) {
    wpa_printf(MSG_DEBUG, "PKCS #1: Failed to parse digestAlgorithm");
    os_free(decrypted);
    return -1;
  }
  ...
  if (!asn1_oid_equal(&oid, hash_alg)) {
    ...
    wpa_printf(MSG_DEBUG, "PKCS #1: Hash alg OID mismatch: was %s, expected %s",
               txt, txt2);
    os_free(decrypted);
    return -1;
  }
  ...
  wpa_hexdump(MSG_MSGDUMP, "PKCS #1: Decrypted Digest", hdr.payload,
              hdr.length);
  ...
  if (hdr.payload + hdr.length != end) {
    wpa_printf(MSG_INFO, "PKCS #1: Extra data after signature - reject");

    wpa_hexdump(MSG_DEBUG, "PKCS #1: Extra data", hdr.payload + hdr.length,
