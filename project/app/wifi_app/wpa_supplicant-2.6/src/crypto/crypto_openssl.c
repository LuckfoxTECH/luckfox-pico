...
int rc4_skip(const u8 *key, size_t keylen, size_t skip, u8 *data,
  ...
  if (!ctx || !EVP_CIPHER_CTX_set_padding(ctx, 0) ||
...
void aes_encrypt_deinit(void *ctx) {
  ...
  if (EVP_EncryptFinal_ex(c, buf, &len) != 1) {
    wpa_printf(MSG_ERROR,
               "OpenSSL: EVP_EncryptFinal_ex failed: "
               "%s",
               ERR_error_string(ERR_get_error(), NULL));
  }
  if (len != 0) {
    wpa_printf(MSG_ERROR,
               "OpenSSL: Unexpected padding length %d "
               "in AES encrypt",
               len);
  }
...
void aes_decrypt_deinit(void *ctx) {
  ...
  if (EVP_DecryptFinal_ex(c, buf, &len) != 1) {
    wpa_printf(MSG_ERROR,
               "OpenSSL: EVP_DecryptFinal_ex failed: "
               "%s",
               ERR_error_string(ERR_get_error(), NULL));
  }
  if (len != 0) {
...
int aes_128_cbc_encrypt(const u8 *key, const u8 *iv, u8 *data,
  ...
  if (EVP_EncryptInit_ex(ctx, EVP_aes_128_cbc(), NULL, key, iv) == 1 &&
      ...
      clen == (int)data_len && EVP_EncryptFinal_ex(ctx, buf, &len) == 1 &&
      len == 0)
    res = 0;
  EVP_CIPHER_CTX_free(ctx);
...
int aes_128_cbc_decrypt(const u8 *key, const u8 *iv, u8 *data,
  ...
  if (EVP_DecryptInit_ex(ctx, EVP_aes_128_cbc(), NULL, key, iv) == 1 &&
      ...
      plen == (int)data_len && EVP_DecryptFinal_ex(ctx, buf, &len) == 1 &&
      len == 0)
    res = 0;
  EVP_CIPHER_CTX_free(ctx);
...
int crypto_cipher_encrypt(struct crypto_cipher *ctx, const u8 *plain, u8 *crypt,
                          size_t len) {
  int outl;
...
int crypto_cipher_decrypt(struct crypto_cipher *ctx, const u8 *crypt, u8 *plain,
                          size_t len) {
  int outl;
  outl = len;
...
err:
  ...
  *publ = NULL;
  ...
  g = BN_new();
  p = get_group5_prime();
  if (!g || BN_set_word(g, 2) != 1 || !p || DH_set0_pqg(dh, p, NULL, g) != 1)
    goto err;
  p = NULL;
  g = NULL;
...
int hmac_md5_vector(const u8 *key, size_t key_len, size_t num_elem,
                    const u8 *addr[], const size_t *len, u8 *mac) {
  return openssl_hmac_vector(EVP_md5(), key, key_len, num_elem, addr, len, mac,
                             16);
}
...
int pbkdf2_sha1(const char *passphrase, const u8 *ssid, size_t ssid_len,
  ...
  if (PKCS5_PBKDF2_HMAC_SHA1(passphrase, os_strlen(passphrase), ssid, ssid_len,
                             iterations, buflen, buf) != 1)
    return -1;
  return 0;
}
...
int hmac_sha1_vector(const u8 *key, size_t key_len, size_t num_elem,
                     const u8 *addr[], const size_t *len, u8 *mac) {
  return openssl_hmac_vector(EVP_sha1(), key, key_len, num_elem, addr, len, mac,
                             20);
}
...
int hmac_sha256_vector(const u8 *key, size_t key_len, size_t num_elem,
                       const u8 *addr[], const size_t *len, u8 *mac) {
  return openssl_hmac_vector(EVP_sha256(), key, key_len, num_elem, addr, len,
                             mac, 32);
}
...
int hmac_sha384_vector(const u8 *key, size_t key_len, size_t num_elem,
                       const u8 *addr[], const size_t *len, u8 *mac) {
  return openssl_hmac_vector(EVP_sha384(), key, key_len, num_elem, addr, len,
                             mac, 32);
}
...
int omac1_aes_128_vector(const u8 *key, size_t num_elem, const u8 *addr[],
...
int crypto_bignum_to_bin(const struct crypto_bignum *a, u8 *buf, size_t buflen,
                         size_t padlen) {
...
int crypto_bignum_inverse(const struct crypto_bignum *a,
  ...
  if (bnctx == NULL)
    return -1;
  res =
      BN_mod_inverse((BIGNUM *)c, (const BIGNUM *)a, (const BIGNUM *)b, bnctx);
  BN_CTX_free(bnctx);
...
int crypto_bignum_legendre(const struct crypto_bignum *a,
  ...
  if (!exp || !tmp ||
      ...
      !BN_rshift1(exp, exp) ||
      !BN_mod_exp(tmp, (const BIGNUM *)a, exp, (const BIGNUM *)p, bnctx))
    goto fail;
...
int crypto_ec_point_to_bin(struct crypto_ec *e,
  ...
  if (x_bn && y_bn &&
      EC_POINT_get_affine_coordinates_GFp(e->group, (EC_POINT *)point, x_bn,
                                          y_bn, e->bnctx)) {
    if (x) {
...
int crypto_ec_point_solve_y_coord(struct crypto_ec *e,
  ...
  if (!EC_POINT_set_compressed_coordinates_GFp(
          e->group, (EC_POINT *)p, (const BIGNUM *)x, y_bit, e->bnctx) ||
...
crypto_ec_point_compute_y_sqr(struct crypto_ec *e,
  ...
  /* y^2 = x^3 + ax + b */
  if (tmp && tmp2 && BN_mod_sqr(tmp, (const BIGNUM *)x, e->prime, e->bnctx) &&
