...
static void random_mix_pool(const void *buf, size_t len) {
  static const u32 twist[8] = {0x00000000, 0x3b6e20c8, 0x76dc4190, 0x4db26158,
...
static void random_extract(u8 *out) {
  ...
  hmac_sha1(dummy_key, sizeof(dummy_key), (const u8 *)pool, sizeof(pool), hash);
...
void random_add_randomness(const void *buf, size_t len) {
  ...
  wpa_printf(MSG_EXCESSIVE, "Add randomness: count=%u entropy=%u", count,
             entropy);
...
int random_get_bytes(void *buf, size_t len) {
  ...
  wpa_hexdump_key(MSG_EXCESSIVE, "random from os_get_random", buf, len);
  ...
  while (left) {
    ...
    u8 tmp[EXTRACT_LEN];
    random_extract(tmp);
    wpa_hexdump_key(MSG_EXCESSIVE, "random from internal pool", tmp,
                    sizeof(tmp));
  ...
  while (left) {
    ...
    wpa_hexdump_key(MSG_EXCESSIVE, "random from crypto module", tmp,
                    sizeof(tmp));
    siz = left > EXTRACT_LEN ? EXTRACT_LEN : left;
    for (i = 0; i < siz; i++)
      *bytes++ ^= tmp[i];
...
static void random_write_entropy(void) {
  ...
  if (fwrite(&opr, 1, 1, f) != 1 || fwrite(buf, RANDOM_ENTROPY_SIZE, 1, f) != 1)
    fail = 1;
  fclose(f);
  if (fail) {
    wpa_printf(MSG_ERROR,
               "random: Could not write entropy data "
               "to %s",
               random_entropy_file);
    return;
  }
