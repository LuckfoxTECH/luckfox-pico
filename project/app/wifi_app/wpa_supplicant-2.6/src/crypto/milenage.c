 ...
 * Returns: 0 on success, -1 on failure
 */
int milenage_f2345(const u8 *opc, const u8 *k, const u8 *_rand, u8 *res, u8 *ck,
                   u8 *ik, u8 *ak, u8 *akstar) {
  u8 tmp1[16], tmp2[16], tmp3[16];
  int i;
 ...
 */
void milenage_generate(const u8 *opc, const u8 *amf, const u8 *k, const u8 *sqn,
                       const u8 *_rand, u8 *autn, u8 *ik, u8 *ck, u8 *res,
                       size_t *res_len) {
  int i;
  u8 mac_a[8], ak[6];
...
int milenage_check(const u8 *opc, const u8 *k, const u8 *sqn, const u8 *_rand,
  ...
  *res_len = 8;
  ...
  if (os_memcmp_const(mac_a, autn + 8, 8) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "Milenage: Received MAC_A", autn + 8, 8);
    return -1;
  }
