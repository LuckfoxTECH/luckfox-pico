...
struct crypto_public_key *crypto_public_key_import(const u8 *key, size_t len) {
  return (struct crypto_public_key *)crypto_rsa_import_public_key(key, len);
}
...
struct crypto_public_key *crypto_public_key_import_parts(const u8 *n,
                                                         ...
                                                         const u8 *e,
                                                         size_t e_len) {
  return (struct crypto_public_key *)crypto_rsa_import_public_key_parts(
      n, n_len, e, e_len);
}
...
struct crypto_private_key *crypto_private_key_import(const u8 *key, size_t len,
  ...
  wpa_printf(MSG_DEBUG, "Trying to parse PKCS #1 encoded RSA private "
                        "key");
  return (struct crypto_private_key *)crypto_rsa_import_private_key(key, len);
}
...
int crypto_public_key_encrypt_pkcs1_v15(struct crypto_public_key *key,
                                        ...
                                        size_t *outlen) {
  return pkcs1_encrypt(2, (struct crypto_rsa_key *)key, 0, in, inlen, out,
                       outlen);
}
...
int crypto_private_key_decrypt_pkcs1_v15(struct crypto_private_key *key,
                                         const u8 *in, size_t inlen, u8 *out,
                                         size_t *outlen) {
  return pkcs1_v15_private_key_decrypt((struct crypto_rsa_key *)key, in, inlen,
                                       out, outlen);
}

int crypto_private_key_sign_pkcs1(struct crypto_private_key *key, const u8 *in,
                                  size_t inlen, u8 *out, size_t *outlen) {
  return pkcs1_encrypt(1, (struct crypto_rsa_key *)key, 1, in, inlen, out,
                       outlen);
}
...
int crypto_public_key_decrypt_pkcs1(struct crypto_public_key *key,
  ...
  return pkcs1_decrypt_public_key((struct crypto_rsa_key *)key, crypt,
                                  crypt_len, plain, plain_len);
}
