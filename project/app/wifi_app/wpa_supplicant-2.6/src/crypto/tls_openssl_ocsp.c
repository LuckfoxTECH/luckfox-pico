...
ASN1_SEQUENCE(ResponseBytes) = {
    ...
    ASN1_SIMPLE(ResponseBytes, response,
                ASN1_OCTET_STRING)} ASN1_SEQUENCE_END(ResponseBytes);

ASN1_SEQUENCE(OCSPResponse) = {
    ASN1_SIMPLE(OCSPResponse, responseStatus, ASN1_ENUMERATED),
    ASN1_EXP_OPT(OCSPResponse, responseBytes, ResponseBytes,
                 0)} ASN1_SEQUENCE_END(OCSPResponse);
...
ASN1_SEQUENCE(CertID) = {
    ...
    ASN1_SIMPLE(CertID, serialNumber, ASN1_INTEGER)} ASN1_SEQUENCE_END(CertID);

ASN1_CHOICE(ResponderID) = {ASN1_EXP(ResponderID, value.byName, X509_NAME, 1),
                            ASN1_EXP(ResponderID, value.byKey,
                                     ASN1_OCTET_STRING,
                                     2)} ASN1_CHOICE_END(ResponderID);

ASN1_SEQUENCE(RevokedInfo) = {
    ASN1_SIMPLE(RevokedInfo, revocationTime, ASN1_GENERALIZEDTIME),
    ASN1_EXP_OPT(RevokedInfo, revocationReason, ASN1_ENUMERATED,
                 0)} ASN1_SEQUENCE_END(RevokedInfo);

ASN1_CHOICE(CertStatus) = {ASN1_IMP(CertStatus, value.good, ASN1_NULL, 0),
                           ASN1_IMP(CertStatus, value.revoked, RevokedInfo, 1),
                           ASN1_IMP(CertStatus, value.unknown, ASN1_NULL,
                                    2)} ASN1_CHOICE_END(CertStatus);
...
ASN1_SEQUENCE(SingleResponse) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(SingleResponse, singleExtensions, X509_EXTENSION,
                             1)} ASN1_SEQUENCE_END(SingleResponse);
...
ASN1_SEQUENCE(ResponseData) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(ResponseData, responseExtensions, X509_EXTENSION,
                             1)} ASN1_SEQUENCE_END(ResponseData);
...
ASN1_SEQUENCE(BasicOCSPResponse) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(BasicOCSPResponse, certs, X509,
                             0)} ASN1_SEQUENCE_END(BasicOCSPResponse);
...
#define sk_SingleResponse_value(sk, i)                                         \
  ((SingleResponse *)sk_value(                                                 \
      CHECKED_CAST(_STACK *, STACK_OF(SingleResponse) *, sk), (i)))
...
static int ocsp_resp_valid(ASN1_GENERALIZEDTIME *thisupd,
  ...
  if (!ASN1_GENERALIZEDTIME_check(thisupd)) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Invalid OCSP response thisUpdate");
    return 0;
  }
  ...
  if (!ASN1_GENERALIZEDTIME_check(nextupd)) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Invalid OCSP response nextUpdate");
    return 0;
  }
...
static int issuer_match(X509 *cert, X509 *issuer, CertID *certid) {
  ...
  if (txt) {
    wpa_printf(MSG_DEBUG, "OpenSSL: calculated issuerNameHash: %s", txt);
    os_free(txt);
  }
  ...
  if (!ikey || !EVP_Digest(ikey->data, ikey->length, md, &len, dgst, NULL) ||
  ...
  txt = octet_string_str(hash);
  if (txt) {
    wpa_printf(MSG_DEBUG, "OpenSSL: calculated issuerKeyHash: %s", txt);
    os_free(txt);
  }
...
static X509 *ocsp_find_signer(STACK_OF(X509) * certs, ResponderID *rid) {
  ...
  for (i = 0; i < sk_X509_num(certs); i++) {
    ...
    if (os_memcmp(rid->value.byKey->data, hash, SHA_DIGEST_LENGTH) == 0)
      return x;
  }
...
enum ocsp_result check_ocsp_resp(SSL_CTX *ssl_ctx, SSL *ssl, X509 *cert,
  ...
  if (txt) {
    wpa_printf(MSG_DEBUG,
               "OpenSSL: Searching OCSP response for peer certificate "
               "serialNumber: %s",
               txt);
    os_free(txt);
  }
  ...
  if (status != 0) {
    wpa_printf(MSG_INFO, "OpenSSL: OCSP responder error %d", status);
    return OCSP_INVALID;
  }

  bytes = resp->responseBytes;

  if (!bytes || OBJ_obj2nid(bytes->responseType) != NID_id_pkix_OCSP_basic) {
  ...
  wpa_hexdump(MSG_DEBUG, "OpenSSL: BasicOCSPResponse", basic_data, basic_len);

  basic = d2i_BasicOCSPResponse(NULL, &basic_data, basic_len);
  if (!basic) {
    wpa_printf(MSG_INFO, "OpenSSL: Could not parse BasicOCSPResponse");
  ...
  if (basic->certs) {
    ...
    for (i = 0; i < num; i++) {
      ...
      X509_NAME_oneline(X509_get_subject_name(extra_cert), buf, sizeof(buf));
      wpa_printf(MSG_DEBUG, "OpenSSL: BasicOCSPResponse cert %s", buf);
  ...
  if (!signer)
    ...
    signer_trusted = 1;
  if (!signer) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Could not find OCSP signer certificate");
    goto fail;
  }

  skey = X509_get_pubkey(signer);
  if (!skey) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Could not get OCSP signer public key");
    goto fail;
  }
  if (ASN1_item_verify(ASN1_ITEM_rptr(ResponseData), basic->signatureAlgorithm,
                       basic->signature, basic->tbsResponseData, skey) <= 0) {
    wpa_printf(MSG_DEBUG, "OpenSSL: BasicOCSPResponse signature is invalid");
    goto fail;
  }

  X509_NAME_oneline(X509_get_subject_name(signer), buf, sizeof(buf));
  wpa_printf(MSG_DEBUG,
             "OpenSSL: Found OCSP signer certificate %s and verified "
  ...
  if (!signer_trusted) {
    ...
    if ((signer->ex_flags & EXFLAG_XKUSAGE) &&
      ...
      wpa_printf(MSG_DEBUG, "OpenSSL: OCSP signer certificate delegation OK");
    } else {
      tmp_cert = sk_X509_value(chain, sk_X509_num(chain) - 1);
      if (X509_check_trust(tmp_cert, NID_OCSP_sign, 0) != X509_TRUST_TRUSTED) {
        wpa_printf(MSG_DEBUG, "OpenSSL: OCSP signer certificate not trusted");
  ...
  if (txt) {
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP responderID: %s", txt);
    os_free(txt);
  }

  txt = generalizedtime_str(rd->producedAt);
  if (txt) {
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP producedAt: %s", txt);
    os_free(txt);
  }
  ...
  for (j = 0; j < num_resp; j++) {
    ...
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP SingleResponse %u/%u", j + 1,
               num_resp);

    txt = algor_str(sresp->certID->hashAlgorithm);
    if (txt) {
      wpa_printf(MSG_DEBUG, "OpenSSL: certID hashAlgorithm: %s", txt);
      os_free(txt);
    }

    txt = octet_string_str(sresp->certID->issuerNameHash);
    if (txt) {
      wpa_printf(MSG_DEBUG, "OpenSSL: certID issuerNameHash: %s", txt);
      os_free(txt);
    }

    txt = octet_string_str(sresp->certID->issuerKeyHash);
    if (txt) {
      wpa_printf(MSG_DEBUG, "OpenSSL: certID issuerKeyHash: %s", txt);
      os_free(txt);
    }

    txt = integer_str(sresp->certID->serialNumber);
    if (txt) {
      wpa_printf(MSG_DEBUG, "OpenSSL: certID serialNumber: %s", txt);
      os_free(txt);
    }
    ...
    if (sresp->nextUpdate) {
      ...
      if (txt) {
        wpa_printf(MSG_DEBUG, "OpenSSL: nextUpdate: %s", txt);
        os_free(txt);
      }
    }

    txt = extensions_str("singleExtensions", sresp->singleExtensions);
    if (txt) {
    ...
    cid2 = sresp->certID;
    if (j > 0 && (OBJ_cmp(cid1->hashAlgorithm->algorithm,
      ...
      wpa_printf(MSG_DEBUG,
                 "OpenSSL: Different OCSP response issuer information between "
                 "SingleResponse values within BasicOCSPResponse");
      goto fail;
    }
    ...
    if (!matching_resp && issuer &&
      ...
      wpa_printf(MSG_DEBUG, "OpenSSL: This response matches peer certificate");
      matching_resp = sresp;
    }
  }
  ...
  if (!matching_resp) {
    wpa_printf(MSG_DEBUG, "OpenSSL: Could not find OCSP response that matches "
                          "the peer certificate");
    result = OCSP_NO_RESPONSE;
    goto fail;
  }

  if (!ocsp_resp_valid(matching_resp->thisUpdate, matching_resp->nextUpdate)) {
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP response not valid at this time");
    goto fail;
  }

  if (matching_resp->certStatus->type == 1) {
    wpa_printf(MSG_DEBUG, "OpenSSL: OCSP response indicated that the peer "
