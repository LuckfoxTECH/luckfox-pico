...
struct tls_connection *tls_connection_init(void *tls_ctx) {
  ...
  if (!global->server) {
    ...
    tlsv1_client_set_cb(conn->client, global->event_cb, global->cb_ctx,
                        global->cert_in_cb);
  }
#endif /* CONFIG_TLS_INTERNAL_CLIENT */
#ifdef CONFIG_TLS_INTERNAL_SERVER
...
int tls_connection_set_params(void *tls_ctx, struct tls_connection *conn,
  ...
  if (params->flags & TLS_CONN_EXT_CERT_CHECK) {
    wpa_printf(MSG_INFO, "TLS: tls_ext_cert_check=1 not supported");
    return -1;
  }
  ...
  if (tlsv1_set_private_key(
          cred, params->private_key, params->private_key_passwd,
...
int tls_global_set_params(void *tls_ctx,
  ...
  if (params->ocsp_stapling_response)
    cred->ocsp_stapling_response = os_strdup(params->ocsp_stapling_response);
...
#ifdef CONFIG_TLS_INTERNAL_CLIENT
  if (conn->client) {
    ret = tlsv1_client_prf(conn->client, label, server_random_first, _out,
                           skip + out_len);
  }
#endif /* CONFIG_TLS_INTERNAL_CLIENT */
#ifdef CONFIG_TLS_INTERNAL_SERVER
  if (conn->server) {
    ret = tlsv1_server_prf(conn->server, label, server_random_first, _out,
                           skip + out_len);
  }
#endif /* CONFIG_TLS_INTERNAL_SERVER */
...
struct wpabuf *tls_connection_handshake2(void *tls_ctx,
  ...
  res = tlsv1_client_handshake(conn->client,
                               ...
                               in_data ? wpabuf_len(in_data) : 0, &res_len, &ad,
                               &ad_len, need_more_data);
  if (res == NULL)
    return NULL;
...
struct wpabuf *tls_connection_encrypt(void *tls_ctx,
  ...
  if (conn->client) {
    ...
    res = tlsv1_client_encrypt(conn->client, wpabuf_head(in_data),
                               wpabuf_len(in_data), wpabuf_mhead(buf),
                               wpabuf_size(buf));
    if (res < 0) {
      wpabuf_free(buf);
      return NULL;
...
int tls_get_cipher(void *tls_ctx, struct tls_connection *conn, char *buf,
                   size_t buflen) {
  if (conn == NULL)
    return -1;
#ifdef CONFIG_TLS_INTERNAL_CLIENT
