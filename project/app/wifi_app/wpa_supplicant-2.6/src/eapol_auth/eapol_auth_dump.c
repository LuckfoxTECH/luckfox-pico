...
#define _SB(b) ((b) ? "TRUE" : "FALSE")
  ret = os_snprintf(
      ...
      "portValid=%s\n"
      "reAuthenticate=%s\n",
      _SB(sm->authAbort), _SB(sm->authFail), port_state_txt(sm->authPortStatus),
      _SB(sm->authStart), _SB(sm->authTimeout), _SB(sm->authSuccess),
      _SB(sm->eap_if->eapFail), _SB(sm->eapolEap), _SB(sm->eap_if->eapSuccess),
      _SB(sm->eap_if->eapTimeout), _SB(sm->initialize),
      _SB(sm->eap_if->eapKeyAvailable), _SB(sm->keyDone), _SB(sm->keyRun),
      _SB(sm->keyTxEnabled), port_type_txt(sm->portControl),
      _SB(sm->eap_if->portEnabled), _SB(sm->portValid),
  ...
  ret = os_snprintf(pos, end - pos,
                    ...
                    _SB(sm->eap_if->eapRestart), port_type_txt(sm->portMode),
                    sm->reAuthCount, sm->quietPeriod, sm->reAuthMax,
  ...
  ret = os_snprintf(pos, end - pos,
                    ...
                    _SB(sm->eap_if->eapNoReq), _SB(sm->eap_if->eapReq),
                    _SB(sm->eap_if->eapResp), sm->serverTimeout,
                    sm->backendResponses, sm->backendAccessChallenges,
  ...
  ret = os_snprintf(pos, end - pos, "auth_key_tx_state=%s\n",
  ...
  ret = os_snprintf(pos, end - pos,
                    "key_rx_state=%s\n"
                    "rxKey=%s\n",
                    key_rx_state_txt(sm->key_rx_state), _SB(sm->rxKey));
  if (os_snprintf_error(end - pos, ret))
    return pos - buf;
  pos += ret;
