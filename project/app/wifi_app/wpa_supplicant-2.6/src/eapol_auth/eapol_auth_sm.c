...
static void eapol_auth_logger(struct eapol_authenticator *eapol, const u8 *addr,
...
static void eapol_auth_tx_canned_eap(struct eapol_state_machine *sm,
  ...
  sm->eapol->cb.eapol_send(sm->eapol->conf.ctx, sm->sta,
                           IEEE802_1X_TYPE_EAP_PACKET, (u8 *)&eap, sizeof(eap));
...
static void eapol_auth_tx_req(struct eapol_state_machine *sm) {
  if (sm->eap_if->eapReqData == NULL ||
    ...
    eapol_auth_logger(sm->eapol, sm->addr, EAPOL_LOGGER_DEBUG,
  ...
  eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_DEBUG,
                     "Sending EAP Packet (identifier %d)", sm->last_eap_id);
  sm->eapol->cb.eapol_send(
      sm->eapol->conf.ctx, sm->sta, IEEE802_1X_TYPE_EAP_PACKET,
      wpabuf_head(sm->eap_if->eapReqData), wpabuf_len(sm->eap_if->eapReqData));
  sm->dot1xAuthEapolFramesTx++;
...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (state->aWhile > 0) {
    state->aWhile--;
    if (state->aWhile == 0) {
      wpa_printf(MSG_DEBUG, "IEEE 802.1X: " MACSTR " - aWhile --> 0",
                 MAC2STR(state->addr));
    }
  }
  ...
  if (state->quietWhile > 0) {
    state->quietWhile--;
    if (state->quietWhile == 0) {
      wpa_printf(MSG_DEBUG, "IEEE 802.1X: " MACSTR " - quietWhile --> 0",
                 MAC2STR(state->addr));
    }
  }
  ...
  if (state->reAuthWhen > 0) {
    state->reAuthWhen--;
    if (state->reAuthWhen == 0) {
      wpa_printf(MSG_DEBUG, "IEEE 802.1X: " MACSTR " - reAuthWhen --> 0",
                 MAC2STR(state->addr));
    }
  }
  ...
  if (state->eap_if->retransWhile > 0) {
    ...
    if (state->eap_if->retransWhile == 0) {
      wpa_printf(MSG_DEBUG,
                 "IEEE 802.1X: " MACSTR " - (EAP) retransWhile --> 0",
                 MAC2STR(state->addr));
    }
  }
...
SM_STATE(AUTH_PAE, AUTHENTICATED) {
  ...
  eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_INFO,
                     ...
                     sm->eap_type_authsrv,
                     eap_server_get_name(0, sm->eap_type_authsrv), extra);
  ...
  if ((sm->portControl == Auto && sm->portMode != sm->portControl) ||
    ...
    case AUTH_PAE_CONNECTING:
      if (sm->eapolLogoff || sm->reAuthCount > sm->reAuthMax)
        SM_ENTER(AUTH_PAE, DISCONNECTED);
      else if ((sm->eap_if->eapReq && sm->reAuthCount <= sm->reAuthMax) ||
...
SM_STEP(AUTH_KEY_TX) {
  ...
  case AUTH_KEY_TX_NO_KEY_TRANSMIT:
    if (sm->keyTxEnabled && sm->eap_if->eapKeyAvailable && sm->keyRun &&
...
static void eapol_sm_step_run(struct eapol_state_machine *sm) {
  ...
  u8 addr[ETH_ALEN];
  unsigned int prev_auth_pae, prev_be_auth, prev_reauth_timer, prev_auth_key_tx,
...
restart:
  ...
  if (prev_auth_pae != sm->auth_pae_state ||
      ...
      prev_reauth_timer != sm->reauth_timer_state ||
      prev_auth_key_tx != sm->auth_key_tx_state ||
      prev_key_rx != sm->key_rx_state || prev_ctrl_dir != sm->ctrl_dir_state) {
    if (--max_steps > 0)
      goto restart;
  ...
  if (eapol_sm_sta_entry_alive(eapol, addr) && sm->eap) {
    ...
    if (sm->eap_if->aaaEapResp) {
      ...
      sm->eapol->cb.aaa_send(sm->eapol->conf.ctx, sm->sta,
...
static int eapol_sm_get_eap_user(void *ctx, const u8 *identity,
  ...
  ret = sm->eapol->cb.get_eap_user(sm->eapol->conf.ctx, identity, identity_len,
                                   phase2, user);
  if (user->remediation)
    sm->remediation = 1;
  return ret;
...
void eapol_auth_reauthenticate(struct eapol_state_machine *sm) {
  wpa_printf(MSG_DEBUG, "EAPOL: External reauthentication trigger for " MACSTR,
             MAC2STR(sm->addr));
  sm->reAuthenticate = TRUE;
  eapol_auth_step(sm);
}
...
int eapol_auth_set_conf(struct eapol_state_machine *sm, const char *param,
                        const char *value) {
  wpa_printf(MSG_DEBUG,
             "EAPOL: External configuration operation for " MACSTR
             " - param=%s value=%s",
