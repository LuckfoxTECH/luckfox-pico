...
static void nl80211_register_eloop_read(struct nl_handle **handle,
  ...
  if (nl_socket_set_buffer_size(*handle, 262144, 0) < 0) {
    wpa_printf(MSG_DEBUG, "nl80211: Could not set nl_socket RX buffer size: %s",
               strerror(errno));
  ...
  eloop_register_read_sock(nl_socket_get_fd(*handle), handler, eloop_data,
                           *handle);
...
static int nl80211_send_frame_cmd(struct i802_bss *bss, unsigned int freq,
                                  unsigned int wait, const u8 *buf,
                                  size_t buf_len, u64 *cookie, int no_cck,
                                  int no_ack, int offchanok,
...
int is_ap_interface(enum nl80211_iftype nlmode) {
  return nlmode == NL80211_IFTYPE_AP || nlmode == NL80211_IFTYPE_P2P_GO;
}
...
static int is_p2p_net_interface(enum nl80211_iftype nlmode) {
  return nlmode == NL80211_IFTYPE_P2P_CLIENT || nlmode == NL80211_IFTYPE_P2P_GO;
}
...
static int send_and_recv(struct nl80211_global *global,
  ...
  while (err > 0) {
    ...
    if (res < 0) {
      wpa_printf(MSG_INFO, "nl80211: %s->nl_recvmsgs failed: %d", __func__,
                 res);
    }
  }
out:
...
int send_and_recv_msgs(struct wpa_driver_nl80211_data *drv, struct nl_msg *msg,
                       int (*valid_handler)(struct nl_msg *, void *),
                       void *valid_data) {
  return send_and_recv(drv->global, drv->global->nl, msg, valid_handler,
                       valid_data);
}
...
static int family_handler(struct nl_msg *msg, void *arg) {
  ...
  nla_for_each_nested(mcgrp, tb[CTRL_ATTR_MCAST_GROUPS], i) {
    ...
    if (!tb2[CTRL_ATTR_MCAST_GRP_NAME] || !tb2[CTRL_ATTR_MCAST_GRP_ID] ||
        os_strncmp(nla_data(tb2[CTRL_ATTR_MCAST_GRP_NAME]), res->group,
...
static int nl_get_multicast_id(struct nl80211_global *global,
  ...
  if (!msg)
    return -ENOMEM;
  if (!genlmsg_put(msg, 0, 0, genl_ctrl_resolve(global->nl, "nlctrl"), 0, 0,
...
void *nl80211_cmd(struct wpa_driver_nl80211_data *drv, struct nl_msg *msg,
...
static int process_beacon_event(struct nl_msg *msg, void *arg) {
  ...
  dl_list_for_each(drv, &w->drvs, struct wpa_driver_nl80211_data, wiphy_list) {
...
nl80211_get_wiphy_data_ap(struct i802_bss *bss) {
  ...
  nl_cb_set(w->nl_cb, NL_CB_VALID, NL_CB_CUSTOM, process_beacon_event, w);
...
static void
wpa_driver_nl80211_event_newlink(struct nl80211_global *global,
                                 struct wpa_driver_nl80211_data *drv,
  ...
  if (drv)
    ...
    wpa_supplicant_event_global(global->ctx, EVENT_INTERFACE_STATUS, &event);
}

static void
wpa_driver_nl80211_event_dellink(struct nl80211_global *global,
                                 struct wpa_driver_nl80211_data *drv,
                                 int ifindex, const char *ifname) {
  union wpa_event_data event;
  ...
  if (drv && os_strcmp(drv->first_bss->ifname, ifname) == 0) {
    ...
    wpa_printf(MSG_DEBUG, "RTM_DELLINK: Interface '%s' removed", ifname);
  }
...
static int wpa_driver_nl80211_own_ifname(struct wpa_driver_nl80211_data *drv,
  ...
  while (RTA_OK(attr, attrlen)) {
    if (attr->rta_type == IFLA_IFNAME) {
      if (os_strcmp(((char *)attr) + rta_len, drv->first_bss->ifname) == 0)
        return 1;
      else
        break;
    }
...
nl80211_find_drv(struct nl80211_global *global, int idx, u8 *buf, size_t len) {
  struct wpa_driver_nl80211_data *drv;
  dl_list_for_each(drv, &global->interfaces, struct wpa_driver_nl80211_data,
                   list) {
...
static void wpa_driver_nl80211_event_rtm_newlink(void *ctx,
  ...
  wpa_printf(MSG_DEBUG,
             "RTM_NEWLINK: ifi_index=%d ifname=%s%s ifi_family=%d "
             "ifi_flags=0x%x (%s%s%s%s)",
             ifi->ifi_index, ifname, extra, ifi->ifi_family, ifi->ifi_flags,
  ...
  if (!drv->if_disabled && !(ifi->ifi_flags & IFF_UP)) {
    ...
    if (os_strcmp(drv->first_bss->ifname, ifname) != 0) {
      wpa_printf(MSG_DEBUG,
                 "nl80211: Not the main interface (%s) - do not indicate "
                 "interface down",
                 drv->first_bss->ifname);
  ...
  if (drv->if_disabled && (ifi->ifi_flags & IFF_UP)) {
    if (if_indextoname(ifi->ifi_index, namebuf) &&
      ...
      if (bss &&
          linux_get_ifhwaddr(drv->global->ioctl_sock, bss->ifname, addr) < 0) {
        wpa_printf(MSG_DEBUG, "nl80211: %s: failed to re-read MAC address",
                   bss->ifname);
      } else if (bss && os_memcmp(addr, bss->addr, ETH_ALEN) != 0) {
        wpa_printf(
            MSG_DEBUG,
            "nl80211: Own MAC address on ifindex %d (%s) changed from " MACSTR
            " to " MACSTR,
            ifi->ifi_index, bss->ifname, MAC2STR(bss->addr), MAC2STR(addr));
  ...
  if (drv->operstate == 1 &&
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set IF_OPER_UP again based on ifi_flags "
                          "and expected operstate");
    netlink_send_oper_ifla(drv->global->netlink, drv->ifindex, -1, IF_OPER_UP);
  }

event_newlink:
  if (ifname[0])
    wpa_driver_nl80211_event_newlink(global, drv, ifi->ifi_index, ifname);
  ...
  if (ifi->ifi_family == AF_BRIDGE && brid && drv) {
    ...
    if (!if_indextoname(brid, namebuf)) {
      wpa_printf(MSG_DEBUG,
                 "nl80211: Could not find bridge ifname for ifindex %u", brid);
      return;
    }
...
static void wpa_driver_nl80211_event_rtm_dellink(void *ctx,
  ...
  wpa_printf(MSG_DEBUG,
             "RTM_DELLINK: ifi_index=%d ifname=%s%s ifi_family=%d "
             "ifi_flags=0x%x (%s%s%s%s)",
             ifi->ifi_index, ifname, extra, ifi->ifi_family, ifi->ifi_flags,
             (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
             (ifi->ifi_flags & IFF_RUNNING) ? "[RUNNING]" : "",
  ...
  if (ifi->ifi_family == AF_BRIDGE && brid && drv) {
    ...
    if (!if_indextoname(brid, namebuf)) {
      ...
      wpa_printf(MSG_DEBUG, "nl80211: Remove ifindex %u for bridge %s", brid,
                 namebuf);
    }
...
unsigned int nl80211_get_assoc_freq(struct wpa_driver_nl80211_data *drv) {
  ...
  if (ret == 0) {
    unsigned int freq =
        drv->nlmode == NL80211_IFTYPE_ADHOC ? arg.ibss_freq : arg.assoc_freq;
    wpa_printf(MSG_DEBUG,
...
static int get_link_signal(struct nl_msg *msg, void *arg) {
  ...
  if (!tb[NL80211_ATTR_STA_INFO] ||
      nla_parse_nested(sinfo, NL80211_STA_INFO_MAX, tb[NL80211_ATTR_STA_INFO],
                       policy))
    return NL_SKIP;
  ...
  if (sinfo[NL80211_STA_INFO_BEACON_SIGNAL_AVG])
    sig_change->avg_beacon_signal =
        (s8)nla_get_u8(sinfo[NL80211_STA_INFO_BEACON_SIGNAL_AVG]);
  else
...
static int wpa_driver_nl80211_init_nl_global(struct nl80211_global *global) {
  ...
  nl_cb_set(global->nl_cb, NL_CB_VALID, NL_CB_CUSTOM, process_global_event,
            global);
...
static void nl80211_check_global(struct nl80211_global *global) {
  ...
  for (i = 0; groups[i]; i++) {
    ...
    if (ret < 0) {
      wpa_printf(MSG_INFO,
                 "nl80211: Could not re-add multicast membership for %s "
                 "events: %d (%s)",
...
static void wpa_driver_nl80211_handle_eapol_tx_status(int sock, void *eloop_ctx,
  ...
  for (cmsg = CMSG_FIRSTHDR(&msg); cmsg; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
    if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_WIFI_STATUS) {
      int *ack;
...
wpa_driver_nl80211_drv_init_rfkill(struct wpa_driver_nl80211_data *drv) {
  ...
  if (drv->nlmode == NL80211_IFTYPE_P2P_DEVICE) {
    ...
    dl_list_for_each(tmp1, &global->interfaces, struct wpa_driver_nl80211_data,
                     list) {
      if (drv == tmp1 || drv->wiphy_idx != tmp1->wiphy_idx || !tmp1->rfkill)
        continue;

      wpa_printf(MSG_DEBUG, "nl80211: Use (%s) to initialize P2P Device rfkill",
                 tmp1->first_bss->ifname);
      os_strlcpy(rcfg->ifname, tmp1->first_bss->ifname, sizeof(rcfg->ifname));
      break;
    }
  } else {
    os_strlcpy(rcfg->ifname, drv->first_bss->ifname, sizeof(rcfg->ifname));
  }
...
static void *wpa_driver_nl80211_drv_init(void *ctx, const char *ifname,
  ...
  if (drv->data_tx_status) {
    ...
    if (setsockopt(drv->eapol_tx_sock, SOL_SOCKET, SO_WIFI_STATUS, &enabled,
                   sizeof(enabled)) < 0) {
      ...
      eloop_register_read_sock(drv->eapol_tx_sock,
                               wpa_driver_nl80211_handle_eapol_tx_status, drv,
                               NULL);
    }
  }
...
static int nl80211_register_frame(struct i802_bss *bss,
                                  struct nl_handle *nl_handle, u16 type,
  ...
  if (ret) {
    ...
    wpa_hexdump(MSG_DEBUG, "nl80211: Register frame match", match, match_len);
  }
  return ret;
}
...
static void nl80211_mgmt_handle_register_eloop(struct i802_bss *bss) {
  nl80211_register_eloop_read(&bss->nl_mgmt, wpa_driver_nl80211_event_receive,
                              bss->nl_cb);
}

static int nl80211_register_action_frame(struct i802_bss *bss, const u8 *match,
                                         size_t match_len) {
  u16 type = (WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_ACTION << 4);
  return nl80211_register_frame(bss, bss->nl_mgmt, type, match, match_len);
}
...
static int nl80211_mgmt_subscribe_non_ap(struct i802_bss *bss) {
...
#ifdef CONFIG_P2P
  /* P2P Public Action */
  if (nl80211_register_action_frame(bss, (u8 *)"\x04\x09\x50\x6f\x9a\x09", 6) <
      0)
    ret = -1;
  /* P2P Action */
  ...
  if ((drv->capa.flags & WPA_DRIVER_FLAGS_TDLS_SUPPORT)) {
    ...
    if (nl80211_register_action_frame(bss, (u8 *)"\x04\x0e", 2) < 0)
      ret = -1;
  }
#endif /* CONFIG_TDLS */
#ifdef CONFIG_FST
...
static int nl80211_mgmt_subscribe_mesh(struct i802_bss *bss) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Subscribe to mgmt frames with mesh handle %p",
             bss->nl_mgmt);

  /* Auth frames for mesh SAE */
  if (nl80211_register_frame(
          bss, bss->nl_mgmt,
          (WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_AUTH << 4), NULL, 0) < 0)
    ret = -1;
...
static int nl80211_mgmt_subscribe_ap(struct i802_bss *bss) {
  static const int stypes[] = {
      WLAN_FC_STYPE_AUTH,        WLAN_FC_STYPE_ASSOC_REQ,
      WLAN_FC_STYPE_REASSOC_REQ, WLAN_FC_STYPE_DISASSOC,
      WLAN_FC_STYPE_DEAUTH,      WLAN_FC_STYPE_PROBE_REQ,
  ...
  for (i = 0; i < ARRAY_SIZE(stypes); i++) {
    if (nl80211_register_frame(bss, bss->nl_mgmt,
                               (WLAN_FC_TYPE_MGMT << 2) | (stypes[i] << 4),
                               NULL, 0) < 0) {
      goto out_err;
    }
  }
...
static void nl80211_del_p2pdev(struct i802_bss *bss) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Delete P2P Device %s (0x%llx): %s",
             bss->ifname, (long long unsigned int)bss->wdev_id, strerror(-ret));
}
...
static int nl80211_set_p2pdev(struct i802_bss *bss, int start) {
  ...
  msg = nl80211_cmd_msg(bss, 0,
                        start ? NL80211_CMD_START_P2P_DEVICE
                              : NL80211_CMD_STOP_P2P_DEVICE);
  ret = send_and_recv_msgs(bss->drv, msg, NULL, NULL);

  wpa_printf(MSG_DEBUG, "nl80211: %s P2P Device %s (0x%llx): %s",
             start ? "Start" : "Stop", bss->ifname,
             (long long unsigned int)bss->wdev_id, strerror(-ret));
  return ret;
}
...
static int i802_set_iface_flags(struct i802_bss *bss, int up) {
  ...
  if (nlmode != NL80211_IFTYPE_P2P_DEVICE) {
    return linux_set_iface_flags(bss->drv->global->ioctl_sock, bss->ifname, up);
  }
...
static int qca_vendor_test_cmd_handler(struct nl_msg *msg, void *arg) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: QCA vendor test command response received");
  ...
  wpa_hexdump(MSG_DEBUG, "nl80211: Received QCA vendor test command response",
...
static void qca_vendor_test(struct wpa_driver_nl80211_data *drv) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: QCA vendor test command returned %d (%s)",
             ret, strerror(-ret));
#endif /* CONFIG_TESTING_OPTIONS */
}
...
wpa_driver_nl80211_finish_drv_init(struct wpa_driver_nl80211_data *drv,
  ...
  if (first && nl80211_get_ifmode(bss) != NL80211_IFTYPE_P2P_DEVICE &&
  ...
  wpa_printf(MSG_DEBUG, "nl80211: interface %s in phy %s", bss->ifname,
             drv->phyname);

  if (set_addr &&
      (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 0) ||
       linux_set_ifhwaddr(drv->global->ioctl_sock, bss->ifname, set_addr)))
    return -1;
  ...
  if (!drv->hostapd && nlmode != NL80211_IFTYPE_P2P_DEVICE)
    netlink_send_oper_ifla(drv->global->netlink, drv->ifindex, 1,
                           IF_OPER_DORMANT);
...
static int wpa_driver_nl80211_del_beacon(struct wpa_driver_nl80211_data *drv) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Remove beacon (ifindex=%d)", drv->ifindex);
...
static void wpa_driver_nl80211_deinit(struct i802_bss *bss) {
  ...
  if (bss->added_if_into_bridge) {
    if (linux_br_del_if(drv->global->ioctl_sock, bss->brname, bss->ifname) < 0)
      wpa_printf(MSG_INFO,
  ...
  if (bss->added_bridge) {
    if (linux_set_iface_flags(drv->global->ioctl_sock, bss->brname, 0) < 0)
      wpa_printf(MSG_INFO, "nl80211: Could not set bridge %s down",
                 bss->brname);
  ...
  if (drv->addr_changed) {
    if (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 0) < 0) {
      wpa_printf(MSG_DEBUG, "nl80211: Could not set interface down to restore "
                            "permanent MAC address");
    }
    if (linux_set_ifhwaddr(drv->global->ioctl_sock, bss->ifname,
                           drv->perm_addr) < 0) {
      wpa_printf(MSG_DEBUG, "nl80211: Could not restore permanent MAC address");
    }
  }
...
static int issue_key_mgmt_set_key(struct wpa_driver_nl80211_data *drv,
  ...
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Key management set key failed: ret=%d (%s)",
               ret, strerror(-ret));
  }
...
static int wpa_driver_nl80211_set_key(const char *ifname, struct i802_bss *bss,
  ...
  if (alg == WPA_ALG_NONE) {
    ...
    if (!msg || nla_put(msg, NL80211_ATTR_KEY_DATA, key_len, key) ||
  ...
  if (addr && !is_broadcast_ether_addr(addr)) {
    ...
    if (!types || nla_put_flag(msg, NL80211_KEY_DEFAULT_TYPE_MULTICAST))
      goto fail;
  ...
  if ((ret == -ENOENT || ret == -ENOLINK) && alg == WPA_ALG_NONE)
    ret = 0;
  if (ret)
    wpa_printf(MSG_DEBUG, "nl80211: set_key failed; err=%d %s)", ret,
               strerror(-ret));
  ...
  if (!msg || nla_put_u8(msg, NL80211_ATTR_KEY_IDX, key_idx) ||
      nla_put_flag(msg,
                   (alg == WPA_ALG_IGTK || alg == WPA_ALG_BIP_GMAC_128 ||
...
static int nl_add_key(struct nl_msg *msg, enum wpa_alg alg, int key_idx,
                      int defkey, const u8 *seq, size_t seq_len, const u8 *key,
                      size_t key_len) {
  ...
  if (nla_put_u8(msg, NL80211_KEY_IDX, key_idx) ||
      ...
      (seq && seq_len && nla_put(msg, NL80211_KEY_SEQ, seq_len, seq)) ||
...
static int nl80211_set_conn_keys(struct wpa_driver_associate_params *params,
  ...
  if (params->wps == WPS_MODE_PRIVACY)
    privacy = 1;
  if (params->pairwise_suite && params->pairwise_suite != WPA_CIPHER_NONE)
    privacy = 1;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!nl_key ||
        ...
        nla_put_u32(msg, NL80211_KEY_CIPHER,
                    params->wep_key_len[i] == 5 ? WLAN_CIPHER_SUITE_WEP40
                                                : WLAN_CIPHER_SUITE_WEP104) ||
        nla_put_u8(msg, NL80211_KEY_IDX, i) ||
        (i == params->wep_tx_keyidx && nla_put_flag(msg, NL80211_KEY_DEFAULT)))
      return -ENOBUFS;
...
int wpa_driver_nl80211_mlme(struct wpa_driver_nl80211_data *drv, const u8 *addr,
                            int cmd, u16 reason_code, int local_state_change) {
  int ret;
  struct nl_msg *msg;
  ...
  if (ret) {
    wpa_dbg(drv->ctx, MSG_DEBUG,
            "nl80211: MLME command failed: reason=%u ret=%d (%s)", reason_code,
            ret, strerror(-ret));
  }
  return ret;
}
...
static int wpa_driver_nl80211_disconnect(struct wpa_driver_nl80211_data *drv,
  ...
  ret = wpa_driver_nl80211_mlme(drv, NULL, NL80211_CMD_DISCONNECT, reason_code,
                                0);
  /*
...
static int wpa_driver_nl80211_deauthenticate(struct i802_bss *bss,
  ...
  wpa_printf(MSG_DEBUG, "%s(addr=" MACSTR " reason_code=%d)", __func__,
...
static void nl80211_unmask_11b_rates(struct i802_bss *bss) {
   ...
   */
  wpa_printf(MSG_DEBUG,
             "nl80211: Interface %s mode is for non-P2P, but 11b rates were "
...
static int
wpa_driver_nl80211_authenticate(struct i802_bss *bss,
                                struct wpa_driver_auth_params *params) {
  struct wpa_driver_nl80211_data *drv = bss->drv;
  int ret = -1, i;
  struct nl_msg *msg;
  ...
  /* FIX: IBSS mode */
  nlmode = params->p2p ? NL80211_IFTYPE_P2P_CLIENT : NL80211_IFTYPE_STATION;
  if (drv->nlmode != nlmode && wpa_driver_nl80211_set_mode(bss, nlmode) < 0)
    return -1;

retry:
  wpa_printf(MSG_DEBUG, "nl80211: Authenticate (ifindex=%d)", drv->ifindex);
  ...
  for (i = 0; i < 4; i++) {
    if (!params->wep_key[i])
      continue;
    wpa_driver_nl80211_set_key(bss->ifname, bss, WPA_ALG_WEP, NULL, i,
                               i == params->wep_tx_keyidx, NULL, 0,
                               params->wep_key[i], params->wep_key_len[i]);
    if (params->wep_tx_keyidx != i)
      continue;
    if (nl_add_key(msg, WPA_ALG_WEP, i, 1, NULL, 0, params->wep_key[i],
                   params->wep_key_len[i]))
      goto fail;
  }

  if (params->bssid) {
    wpa_printf(MSG_DEBUG, "  * bssid=" MACSTR, MAC2STR(params->bssid));
  ...
  if (params->ssid) {
    wpa_hexdump_ascii(MSG_DEBUG, "  * SSID", params->ssid, params->ssid_len);
    if (nla_put(msg, NL80211_ATTR_SSID, params->ssid_len, params->ssid))
      goto fail;
  }
  wpa_hexdump(MSG_DEBUG, "  * IEs", params->ie, params->ie_len);
  if (params->ie && nla_put(msg, NL80211_ATTR_IE, params->ie_len, params->ie))
    goto fail;
  if (params->sae_data) {
  ...
  if (ret) {
    wpa_dbg(drv->ctx, MSG_DEBUG,
            "nl80211: MLME command failed (auth): ret=%d (%s)", ret,
            strerror(-ret));
    count++;
    ...
    if (ret == -ENOENT && params->freq && !is_retry) {
      ...
      wpa_printf(MSG_DEBUG, "nl80211: Authentication retry "
                            "failed");
      os_memset(&event, 0, sizeof(event));
      os_memcpy(event.timeout_event.addr, drv->auth_bssid_, ETH_ALEN);
      wpa_supplicant_event(drv->ctx, EVENT_AUTH_TIMED_OUT, &event);
    }
  } else {
    wpa_printf(MSG_DEBUG, "nl80211: Authentication request send successfully");
  }
...
static int wpa_driver_nl80211_send_frame(struct i802_bss *bss, const void *data,
                                         size_t len, int encrypt, int noack,
  ...
  if (freq == 0 && drv->nlmode == NL80211_IFTYPE_ADHOC) {
    ...
    wpa_printf(MSG_DEBUG, "nl80211: send_frame - Use assoc_freq=%u for IBSS",
               freq);
  }
  if (freq == 0) {
    wpa_printf(MSG_DEBUG, "nl80211: send_frame - Use bss->freq=%u", bss->freq);
    freq = bss->freq;
  }
  ...
  res = nl80211_send_frame_cmd(bss, freq, wait_time, data, len, &cookie, no_cck,
                               noack, offchanok, csa_offs, csa_offs_len);
  if (res == 0 && !noack) {
...
static int wpa_driver_nl80211_send_mlme(struct i802_bss *bss, const u8 *data,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             " noack=%d freq=%u no_cck=%d offchanok=%d wait_time=%u fc=0x%x "
             "(%s) nlmode=%d",
             MAC2STR(mgmt->da), noack, freq, no_cck, offchanok, wait_time, fc,
             fc2str(fc), drv->nlmode);
  ...
  if ((is_sta_interface(drv->nlmode) ||
    ...
    return nl80211_send_frame_cmd(bss, freq, 0, data, data_len, NULL, 1, noack,
                                  1, csa_offs, csa_offs_len);
  }

  if (drv->device_ap_sme && is_ap_interface(drv->nlmode)) {
    if (freq == 0) {
      wpa_printf(MSG_DEBUG, "nl80211: Use bss->freq=%d", bss->freq);
      freq = bss->freq;
    }
    return nl80211_send_frame_cmd(bss, freq,
                                  (int)freq == bss->freq ? 0 : wait_time, data,
                                  data_len, &drv->send_action_cookie, no_cck,
                                  noack, offchanok, csa_offs, csa_offs_len);
  }
  ...
  return wpa_driver_nl80211_send_frame(bss, data, data_len, encrypt, noack,
                                       freq, no_cck, offchanok, wait_time,
...
static int nl80211_set_bss(struct i802_bss *bss, int cts, int preamble,
  ...
  if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_BSS)) ||
      (cts >= 0 && nla_put_u8(msg, NL80211_ATTR_BSS_CTS_PROT, cts)) ||
      (preamble >= 0 &&
...
static int wpa_driver_nl80211_set_acl(void *priv,
  ...
  if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_SET_MAC_ACL)) ||
      nla_put_u32(msg, NL80211_ATTR_ACL_POLICY,
                  params->acl_policy
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Failed to set MAC ACL: %d (%s)", ret,
               strerror(-ret));
  }
...
static int nl80211_put_beacon_int(struct nl_msg *msg, int beacon_int) {
  if (beacon_int > 0) {
    ...
    return nla_put_u32(msg, NL80211_ATTR_BEACON_INTERVAL, beacon_int);
  }
...
static int nl80211_set_mesh_config(void *priv,
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_ERROR, "nl80211: Mesh config set failed: %d (%s)", ret,
               strerror(-ret));
    return ret;
  }
  return 0;
...
static int wpa_driver_nl80211_set_ap(void *priv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Set beacon (beacon_set=%d)", beacon_set);
  if (beacon_set)
    cmd = NL80211_CMD_SET_BEACON;

  wpa_hexdump(MSG_DEBUG, "nl80211: Beacon head", params->head,
              params->head_len);
  wpa_hexdump(MSG_DEBUG, "nl80211: Beacon tail", params->tail,
              params->tail_len);
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "nl80211: ssid", params->ssid, params->ssid_len);
  ...
  if (params->proberesp && params->proberesp_len) {
    wpa_hexdump(MSG_DEBUG, "nl80211: proberesp (offload)", params->proberesp,
  ...
  if (params->privacy && nla_put_flag(msg, NL80211_ATTR_PRIVACY))
    goto fail;
  ...
  if ((params->auth_algs & (WPA_AUTH_ALG_OPEN | WPA_AUTH_ALG_SHARED)) ==
    ...
    if (nla_put_u32(msg, NL80211_ATTR_AUTH_TYPE, NL80211_AUTHTYPE_SHARED_KEY))
      goto fail;
  } else {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: pairwise_ciphers=0x%x",
             params->pairwise_ciphers);
  num_suites = wpa_cipher_to_cipher_suites(params->pairwise_ciphers, suites,
                                           ARRAY_SIZE(suites));
  if (num_suites && nla_put(msg, NL80211_ATTR_CIPHER_SUITES_PAIRWISE,
                            num_suites * sizeof(u32), suites))
    goto fail;

  wpa_printf(MSG_DEBUG, "nl80211: group_cipher=0x%x", params->group_cipher);
  suite = wpa_cipher_to_cipher_suite(params->group_cipher);
  if (suite && nla_put_u32(msg, NL80211_ATTR_CIPHER_SUITE_GROUP, suite))
    goto fail;
  ...
  if (params->beacon_ies) {
    wpa_hexdump_buf(MSG_DEBUG, "nl80211: beacon_ies", params->beacon_ies);
    if (nla_put(msg, NL80211_ATTR_IE, wpabuf_len(params->beacon_ies),
                wpabuf_head(params->beacon_ies)))
      goto fail;
  }
  if (params->proberesp_ies) {
    wpa_hexdump_buf(MSG_DEBUG, "nl80211: proberesp_ies", params->proberesp_ies);
  ...
  if (params->assocresp_ies) {
    wpa_hexdump_buf(MSG_DEBUG, "nl80211: assocresp_ies", params->assocresp_ies);
  ...
  if (params->p2p_go_ctwindow > 0) {
    if (drv->p2p_go_ctwindow_supported) {
      ...
      if (nla_put_u8(msg, NL80211_ATTR_P2P_CTWINDOW, params->p2p_go_ctwindow))
        goto fail;
    } else {
      wpa_printf(MSG_INFO, "nl80211: Driver does not support CTWindow "
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Beacon set failed: %d (%s)", ret,
               strerror(-ret));
  } else {
    bss->beacon_set = 1;
    nl80211_set_bss(bss, params->cts_protect, params->preamble,
                    params->short_slot_time, params->ht_opmode, params->isolate,
                    params->basic_rates);
    if (beacon_set && params->freq &&
        params->freq->bandwidth != bss->bandwidth) {
      wpa_printf(MSG_DEBUG, "nl80211: Update BSS %s bandwidth: %d -> %d",
                 bss->ifname, bss->bandwidth, params->freq->bandwidth);
      ret = nl80211_set_channel(bss, params->freq, 1);
      if (ret) {
        wpa_printf(MSG_DEBUG, "nl80211: Frequency set failed: %d (%s)", ret,
                   strerror(-ret));
      } else {
        wpa_printf(MSG_DEBUG,
...
static int nl80211_put_freq_params(struct nl_msg *msg,
  ...
  if (freq->vht_enabled) {
    ...
    wpa_printf(MSG_DEBUG, "  * center_freq2=%d", freq->center_freq2);
    if (nla_put_u32(msg, NL80211_ATTR_CHANNEL_WIDTH, cw) ||
        nla_put_u32(msg, NL80211_ATTR_CENTER_FREQ1, freq->center_freq1) ||
        (freq->center_freq2 &&
         nla_put_u32(msg, NL80211_ATTR_CENTER_FREQ2, freq->center_freq2)))
      return -ENOBUFS;
    ...
    wpa_printf(MSG_DEBUG, "  * channel_type=%d", NL80211_CHAN_NO_HT);
...
static int nl80211_set_channel(struct i802_bss *bss,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             freq->freq, freq->ht_enabled, freq->vht_enabled, freq->bandwidth,
             freq->center_freq1, freq->center_freq2);

  msg = nl80211_drv_msg(
      drv, 0, set_chan ? NL80211_CMD_SET_CHANNEL : NL80211_CMD_SET_WIPHY);
...
static u32 sta_plink_state_nl80211(enum mesh_plink_state state) {
  ...
  default:
    wpa_printf(MSG_ERROR, "nl80211: Invalid mesh plink state %d", state);
  }
  return -1;
}
#endif /* CONFIG_MESH */
...
static int wpa_driver_nl80211_sta_add(void *priv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: %s STA " MACSTR, params->set ? "Set" : "Add",
             MAC2STR(params->addr));
  msg = nl80211_bss_msg(
      bss, 0, params->set ? NL80211_CMD_SET_STATION : NL80211_CMD_NEW_STATION);
  ...
  if (!params->set || (params->flags & WPA_STA_TDLS_PEER) ||
    ...
    if (nla_put(msg, NL80211_ATTR_STA_SUPPORTED_RATES, params->supp_rates_len,
                params->supp_rates) ||
    ...
    if (params->ht_capabilities) {
      ...
      if (nla_put(msg, NL80211_ATTR_HT_CAPABILITY,
                  sizeof(*params->ht_capabilities), params->ht_capabilities))
        goto fail;
    }
    ...
    if (params->vht_capabilities) {
      ...
      if (nla_put(msg, NL80211_ATTR_VHT_CAPABILITY,
                  sizeof(*params->vht_capabilities), params->vht_capabilities))
        goto fail;
    }

    if (params->ext_capab) {
      wpa_hexdump(MSG_DEBUG, "  * ext_capab", params->ext_capab,
                  params->ext_capab_len);
      if (nla_put(msg, NL80211_ATTR_STA_EXT_CAPABILITY, params->ext_capab_len,
                  params->ext_capab))
        goto fail;
    }
    ...
    if (is_ap_interface(drv->nlmode) &&
        nla_put_u8(msg, NL80211_ATTR_STA_SUPPORT_P2P_PS,
                   params->support_p2p_ps ? NL80211_P2P_PS_SUPPORTED
  ...
  if (!params->set) {
    ...
    wpa_printf(MSG_DEBUG, "  * listen_interval=%u", params->listen_interval);
  ...
  if (params->supp_channels) {
    wpa_hexdump(MSG_DEBUG, "  * supported channels", params->supp_channels,
  ...
  if (params->supp_oper_classes) {
    wpa_hexdump(MSG_DEBUG, "  * supported operating classes",
                params->supp_oper_classes, params->supp_oper_classes_len);
    if (nla_put(msg, NL80211_ATTR_STA_SUPPORTED_OPER_CLASSES,
                params->supp_oper_classes_len, params->supp_oper_classes))
      goto fail;
  }
  ...
  if (!is_mesh_interface(drv->nlmode)) {
    if (!FULL_AP_CLIENT_STATE_SUPP(drv->capa.flags)) {
      wpa_printf(MSG_DEBUG, "nl80211: Ignore ASSOC/AUTH flags since driver "
                            "doesn't support full AP client state");
  ...
  if (ret)
    wpa_printf(MSG_DEBUG,
               ...
               "result: %d (%s)",
               params->set ? "SET" : "NEW", ret, strerror(-ret));
  if (ret == -EEXIST)
    ret = 0;
fail:
  nlmsg_free(msg);
...
static void rtnl_neigh_delete_fdb_entry(struct i802_bss *bss, const u8 *addr) {
  ...
  if (err < 0) {
    wpa_printf(MSG_DEBUG,
               "nl80211: bridge FDB entry delete for " MACSTR
               " ifindex=%d failed: %s",
               MAC2STR(addr), bss->ifindex, nl_geterror(err));
  } else {
    wpa_printf(MSG_DEBUG, "nl80211: deleted bridge FDB entry for " MACSTR,
               MAC2STR(addr));
  }
...
static int wpa_driver_nl80211_sta_remove(struct i802_bss *bss, const u8 *addr,
  ...
  if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_DEL_STATION)) ||
      ...
      (deauth == 0 &&
       nla_put_u8(msg, NL80211_ATTR_MGMT_SUBTYPE, WLAN_FC_STYPE_DISASSOC)) ||
      (deauth == 1 &&
       nla_put_u8(msg, NL80211_ATTR_MGMT_SUBTYPE, WLAN_FC_STYPE_DEAUTH)) ||
      (reason_code &&
       nla_put_u16(msg, NL80211_ATTR_REASON_CODE, reason_code))) {
    nlmsg_free(msg);
    return -ENOBUFS;
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  wpa_printf(MSG_DEBUG,
             "nl80211: sta_remove -> DEL_STATION %s " MACSTR " --> %d (%s)",
...
static int nl80211_create_iface_once(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Create interface iftype %d (%s)", iftype,
             nl80211_iftype_str(iftype));

  msg = nl80211_cmd_msg(drv->first_bss, 0, NL80211_CMD_NEW_INTERFACE);
  if (!msg || nla_put_string(msg, NL80211_ATTR_IFNAME, ifname) ||
  ...
  if (iftype == NL80211_IFTYPE_MONITOR) {
    ...
    if (!flags || nla_put_flag(msg, NL80211_MNTR_FLAG_COOK_FRAMES))
      goto fail;
  ...
  fail:
    nlmsg_free(msg);
    wpa_printf(MSG_ERROR, "Failed to create interface %s: %d (%s)", ifname, ret,
               strerror(-ret));
    return ret;
  }
  ...
  wpa_printf(MSG_DEBUG, "nl80211: New interface %s created: ifindex=%d", ifname,
             ifidx);
   ...
   */
  if (drv->hostapd || iftype == NL80211_IFTYPE_AP_VLAN ||
      iftype == NL80211_IFTYPE_WDS || iftype == NL80211_IFTYPE_MONITOR) {
...
int nl80211_create_iface(struct wpa_driver_nl80211_data *drv,
  ...
  if (ret == -ENFILE && if_nametoindex(ifname)) {
    ...
    ret =
        nl80211_create_iface_once(drv, ifname, iftype, addr, wds, handler, arg);
  }
...
static int nl80211_setup_ap(struct i802_bss *bss) {
  ...
  if (drv->device_ap_sme && !drv->use_monitor)
    if (nl80211_mgmt_subscribe_ap_dev_sme(bss))
      wpa_printf(MSG_DEBUG, "nl80211: Failed to subscribe for mgmt frames from "
                            "SME driver - trying to run without it");

  if (!drv->device_ap_sme && drv->use_monitor &&
      nl80211_create_monitor_interface(drv) && !drv->device_ap_sme)
    return -1;

  if (drv->device_ap_sme && wpa_driver_nl80211_probe_req_report(bss, 1) < 0) {
...
static int nl80211_send_eapol_data(struct i802_bss *bss, const u8 *addr,
...
static int wpa_driver_nl80211_hapd_send_eapol(void *priv, const u8 *addr,
                                              const u8 *data, size_t data_len,
  ...
  if (qos) {
    hdr->frame_control |= host_to_le16(WLAN_FC_STYPE_QOS_DATA << 4);
  }
  ...
  res = wpa_driver_nl80211_send_frame(bss, (u8 *)hdr, len, encrypt, 0, 0, 0, 0,
                                      0, NULL, 0);
  if (res < 0) {
    wpa_printf(MSG_ERROR,
...
static int nl80211_leave_ibss(struct wpa_driver_nl80211_data *drv,
  ...
  if (reset_mode &&
      wpa_driver_nl80211_set_mode(drv->first_bss, NL80211_IFTYPE_STATION)) {
...
static int
nl80211_ht_vht_overrides(struct nl_msg *msg,
                         struct wpa_driver_associate_params *params) {
  ...
  if (params->htcaps && params->htcaps_mask) {
    ...
    wpa_hexdump(MSG_DEBUG, "  * htcaps_mask", params->htcaps_mask, sz);
    if (nla_put(msg, NL80211_ATTR_HT_CAPABILITY, sz, params->htcaps) ||
  ...
  if (params->vhtcaps && params->vhtcaps_mask) {
    ...
    wpa_hexdump(MSG_DEBUG, "  * vhtcaps_mask", params->vhtcaps_mask, sz);
    if (nla_put(msg, NL80211_ATTR_VHT_CAPABILITY, sz, params->vhtcaps) ||
...
retry:
  ...
  if (params->wpa_ie) {
    wpa_hexdump(MSG_DEBUG, "  * Extra IEs for Beacon/Probe Response frames",
  ...
  msg = NULL;
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Join IBSS failed: ret=%d (%s)", ret,
               strerror(-ret));
    count++;
...
static int nl80211_connect_common(struct wpa_driver_nl80211_data *drv,
  ...
  if (params->bssid_hint) {
    ...
    if (nla_put(msg, NL80211_ATTR_MAC_HINT, ETH_ALEN, params->bssid_hint))
      return -1;
  }
  ...
  if (params->bg_scan_period >= 0) {
    wpa_printf(MSG_DEBUG, "  * bg scan period=%d", params->bg_scan_period);
  ...
  if (params->group_suite == WPA_CIPHER_GTK_NOT_USED &&
     ...
     */
    wpa_printf(MSG_DEBUG,
               "  * skip group cipher configuration for GTK_NOT_USED due to "
               "missing driver support advertisement");
  ...
  if (params->rrm_used) {
    ...
    if ((!((drv_rrm_flags & WPA_DRIVER_FLAGS_DS_PARAM_SET_IE_IN_PROBES) &&
  ...
  if (params->prev_bssid) {
    ...
    if (nla_put(msg, NL80211_ATTR_PREV_BSSID, ETH_ALEN, params->prev_bssid))
      return -1;
...
static int
wpa_driver_nl80211_try_connect(struct wpa_driver_nl80211_data *drv,
                               struct wpa_driver_associate_params *params) {
  struct nl_msg *msg;
  enum nl80211_auth_type type;
  int ret;
...
static int
wpa_driver_nl80211_connect(struct wpa_driver_nl80211_data *drv,
                           struct wpa_driver_associate_params *params) {
  int ret;
  ...
  if (ret == -EALREADY) {
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Explicitly "
                          ...
                          "attempt");
    if (wpa_driver_nl80211_disconnect(drv, WLAN_REASON_PREV_AUTH_NOT_VALID))
      return -1;
    ret = wpa_driver_nl80211_try_connect(drv, params);
  }
  return ret;
...
static int
wpa_driver_nl80211_associate(void *priv,
                             struct wpa_driver_associate_params *params) {
  struct i802_bss *bss = priv;
  struct wpa_driver_nl80211_data *drv = bss->drv;
  int ret = -1;
  ...
  if (!(drv->capa.flags & WPA_DRIVER_FLAGS_SME)) {
    enum nl80211_iftype nlmode =
        params->p2p ? NL80211_IFTYPE_P2P_CLIENT : NL80211_IFTYPE_STATION;
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Associate (ifindex=%d)", drv->ifindex);
  ...
  if (ret) {
    wpa_dbg(drv->ctx, MSG_DEBUG,
            "nl80211: MLME command failed (assoc): ret=%d (%s)", ret,
            strerror(-ret));
    nl80211_dump_scan(drv);
  } else {
    wpa_printf(MSG_DEBUG, "nl80211: Association request send successfully");
  }
...
static int nl80211_set_mode(struct wpa_driver_nl80211_data *drv, int ifindex,
                            enum nl80211_iftype mode) {
  struct nl_msg *msg;
  int ret = -ENOBUFS;

  wpa_printf(MSG_DEBUG, "nl80211: Set mode ifindex %d iftype %d (%s)", ifindex,
...
fail:
  ...
  wpa_printf(MSG_DEBUG,
             ...
             " %d (%s)",
             ifindex, mode, ret, strerror(-ret));
  return ret;
}

static int wpa_driver_nl80211_set_mode_impl(
    struct i802_bss *bss, enum nl80211_iftype nlmode,
  ...
  if (!bss->in_deinit && !is_ap_interface(nlmode) &&
      !is_mesh_interface(nlmode) && nl80211_mgmt_subscribe_non_ap(bss) < 0)
...
static int wpa_driver_nl80211_set_mode_ibss(struct i802_bss *bss,
                                            struct hostapd_freq_params *freq) {
  return wpa_driver_nl80211_set_mode_impl(bss, NL80211_IFTYPE_ADHOC, freq);
}
...
static int wpa_driver_nl80211_set_operstate(void *priv, int state) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Set %s operstate %d->%d (%s)", bss->ifname,
...
static int wpa_driver_nl80211_set_supp_port(void *priv, int authorized) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Set supplicant port %sauthorized for " MACSTR,
             authorized ? "" : "un", MAC2STR(drv->bssid));
  ...
  if (!ret)
    return 0;
  wpa_printf(MSG_DEBUG, "nl80211: Failed to set STA flag: %d (%s)", ret,
             strerror(-ret));
  return ret;
}
...
static int i802_flush(void *priv) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: flush -> DEL_STATION %s (all)", bss->ifname);
...
static int get_sta_handler(struct nl_msg *msg, void *arg) {
  ...
  if (nla_parse_nested(stats, NL80211_STA_INFO_MAX, tb[NL80211_ATTR_STA_INFO],
                       stats_policy)) {
...
static int i802_set_tx_queue_params(void *priv, int queue, int aifs, int cw_min,
  ...
  if (nla_put_u16(msg, NL80211_TXQ_ATTR_TXOP, (burst_time * 100 + 16) / 32) ||
...
static int i802_set_sta_vlan(struct i802_bss *bss, const u8 *addr,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             bss->ifname, if_nametoindex(bss->ifname), MAC2STR(addr), ifname,
             if_nametoindex(ifname), vlan_id);
  if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_STATION)) ||
      nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
  ...
  if (ret < 0) {
    wpa_printf(MSG_ERROR,
               "nl80211: NL80211_ATTR_STA_VLAN (addr=" MACSTR
...
static int i802_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr,
  ...
  return wpa_driver_nl80211_send_mlme(
      bss, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.disassoc), 0, 0, 0, 0,
      0, NULL, 0);
}
...
static void dump_ifidx(struct wpa_driver_nl80211_data *drv) {
  ...
  for (i = 0; i < drv->num_if_indices; i++) {
    if (!drv->if_indices[i])
      continue;
    res = os_snprintf(pos, end - pos, " %d(%d)", drv->if_indices[i],
  ...
  *pos = '\0';
  ...
  wpa_printf(MSG_DEBUG, "nl80211: if_indices[%d]:%s", drv->num_if_indices, buf);
}
...
static void add_ifidx(struct wpa_driver_nl80211_data *drv, int ifidx,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Add own interface ifindex %d (ifidx_reason %d)", ifidx,
             ifidx_reason);
  if (have_ifidx(drv, ifidx, ifidx_reason)) {
    wpa_printf(MSG_DEBUG, "nl80211: ifindex %d already in the list", ifidx);
    return;
  }
  for (i = 0; i < drv->num_if_indices; i++) {
  ...
  drv->if_indices = os_realloc_array(old, drv->num_if_indices + 1, sizeof(int));
  drv->if_indices_reason =
      os_realloc_array(old_reason, drv->num_if_indices + 1, sizeof(int));
  if (!drv->if_indices) {
    if (!old)
      drv->if_indices = drv->default_if_indices;
    else
...
static int i802_set_wds_sta(void *priv, const u8 *addr, int aid, int val,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Set WDS STA addr=" MACSTR " aid=%d val=%d name=%s",
             MAC2STR(addr), aid, val, name);
  if (val) {
    if (!if_nametoindex(name)) {
      if (nl80211_create_iface(drv, name, NL80211_IFTYPE_AP_VLAN, bss->addr, 1,
                               NULL, NULL, 0) < 0)
        return -1;
      if (bridge_ifname &&
          linux_br_add_if(drv->global->ioctl_sock, bridge_ifname, name) < 0)
        return -1;
    }
...
static void handle_eapol(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  len =
      recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *)&lladdr, &fromlen);
  if (len < 0) {
    wpa_printf(MSG_ERROR, "nl80211: EAPOL recv failed: %s", strerror(errno));
    return;
  }
...
static int i802_check_bridge(struct wpa_driver_nl80211_data *drv,
                             struct i802_bss *bss, const char *brname,
                             const char *ifname) {
  int br_ifindex;
  char in_br[IFNAMSIZ];
  ...
  if (linux_br_get(in_br, ifname) == 0) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "bridge %s",
               ifname, in_br);
    if (linux_br_del_if(drv->global->ioctl_sock, in_br, ifname) < 0) {
      wpa_printf(MSG_ERROR,
                 "nl80211: Failed to "
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Adding interface %s into bridge %s", ifname,
             brname);
...
static void *i802_init(struct hostapd_data *hapd,
  ...
  bss = wpa_driver_nl80211_drv_init(hapd, params->ifname, params->global_priv,
                                    1, params->bssid, params->driver_params);
  if (bss == NULL)
    return NULL;
  ...
  if (linux_br_get(master_ifname, params->ifname) == 0) {
    ...
    if (linux_br_get(master_ifname, master_ifname) == 0) {
      wpa_printf(MSG_DEBUG, "nl80211: which is in bridge %s", master_ifname);
      br_ifindex = if_nametoindex(master_ifname);
      os_strlcpy(bss->brname, master_ifname, IFNAMSIZ);
    }
  } else {
  ...
  if (params->num_bridge && params->bridge[0]) {
    if (i802_check_bridge(drv, bss, params->bridge[0], params->ifname) < 0)
      goto failed;
...
wpa_driver_nl80211_if_type(enum wpa_driver_if_type type) {
  switch (type) {
  case WPA_IF_STATION:
...
static int nl80211_vif_addr(struct wpa_driver_nl80211_data *drv, u8 *new_addr) {
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Assigned new virtual interface address " MACSTR,
             MAC2STR(new_addr));
...
static int nl80211_wdev_handler(struct nl_msg *msg, void *arg) {
  ...
  if (tb[NL80211_ATTR_MAC])
    os_memcpy(wi->macaddr, nla_data(tb[NL80211_ATTR_MAC]), ETH_ALEN);
...
static int wpa_driver_nl80211_if_add(void *priv, enum wpa_driver_if_type type,
  ...
  if (nlmode == NL80211_IFTYPE_P2P_DEVICE) {
    ...
    ifidx =
        nl80211_create_iface(drv, ifname, nlmode, addr, 0, nl80211_wdev_handler,
                             &p2pdev_info, use_existing);
    if (!p2pdev_info.wdev_id_set || ifidx != 0) {
      wpa_printf(MSG_ERROR,
                 "nl80211: Failed to create a P2P Device interface %s", ifname);
      return -1;
    }
    ...
    wpa_printf(MSG_DEBUG,
               "nl80211: New P2P Device interface %s (0x%llx) created", ifname,
               (long long unsigned int)p2pdev_info.wdev_id);
  } else {
    ifidx = nl80211_create_iface(drv, ifname, nlmode, addr, 0, NULL, NULL,
                                 use_existing);
  ...
  if (!addr) {
    ...
    else if (linux_get_ifhwaddr(drv->global->ioctl_sock, ifname, if_addr) < 0) {
      if (added)
        nl80211_remove_iface(drv, ifidx);
      return -1;
    }
  ...
  if (!addr && (type == WPA_IF_P2P_CLIENT || type == WPA_IF_P2P_GROUP ||
                type == WPA_IF_P2P_GO || type == WPA_IF_MESH ||
                type == WPA_IF_STATION)) {
    /* Enforce unique address */
    u8 new_addr[ETH_ALEN];

    if (linux_get_ifhwaddr(drv->global->ioctl_sock, ifname, new_addr) < 0) {
      if (added)
        nl80211_remove_iface(drv, ifidx);
      return -1;
    }
    if (nl80211_addr_in_use(drv->global, new_addr)) {
      ...
      if (linux_set_ifhwaddr(drv->global->ioctl_sock, ifname, new_addr) < 0) {
        if (added)
          nl80211_remove_iface(drv, ifidx);
        return -1;
      }
  ...
  if (type == WPA_IF_AP_BSS && setup_ap) {
    ...
    if (bridge && i802_check_bridge(drv, new_bss, bridge, ifname) < 0) {
      wpa_printf(MSG_ERROR,
   ...
   */
  if (ifidx > 0 &&
      (drv->hostapd || nlmode == NL80211_IFTYPE_AP_VLAN ||
       nlmode == NL80211_IFTYPE_WDS || nlmode == NL80211_IFTYPE_MONITOR))
    add_ifidx(drv, ifidx, IFIDX_ANY);
...
static int nl80211_send_frame_cmd(struct i802_bss *bss, unsigned int freq,
                                  unsigned int wait, const u8 *buf,
                                  size_t buf_len, u64 *cookie_out, int no_cck,
                                  int no_ack, int offchanok,
                                  const u16 *csa_offs, size_t csa_offs_len) {
  struct wpa_driver_nl80211_data *drv = bss->drv;
  struct nl_msg *msg;
  ...
  if (ret) {
    ...
    wpa_printf(MSG_MSGDUMP,
               ...
               "cookie 0x%llx",
               no_ack ? " (no ACK)" : "", (long long unsigned int)cookie);
    ...
    if (drv->num_send_action_cookies == MAX_SEND_ACTION_COOKIES) {
      wpa_printf(MSG_DEBUG,
                 ...
                 (long long unsigned int)drv->send_action_cookies[0]);
      os_memmove(&drv->send_action_cookies[0], &drv->send_action_cookies[1],
                 (MAX_SEND_ACTION_COOKIES - 1) * sizeof(u64));
...
static int wpa_driver_nl80211_send_action(struct i802_bss *bss,
  ...
  if (is_ap_interface(drv->nlmode) &&
      (!(drv->capa.flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) ||
       (int)freq == bss->freq || drv->device_ap_sme || !drv->use_monitor))
    ret = wpa_driver_nl80211_send_mlme(bss, buf, 24 + data_len, 0, freq, no_cck,
                                       1, wait_time, NULL, 0);
  else
    ret =
        nl80211_send_frame_cmd(bss, freq, wait_time, buf, 24 + data_len,
                               &drv->send_action_cookie, no_cck, 0, 1, NULL, 0);
...
static int wpa_driver_nl80211_probe_req_report(struct i802_bss *bss,
  ...
  if (!report) {
    if (bss->nl_preq && drv->device_ap_sme && is_ap_interface(drv->nlmode) &&
  ...
  nl80211_register_eloop_read(&bss->nl_preq, wpa_driver_nl80211_event_receive,
                              bss->nl_cb);
...
static int nl80211_disable_11b_rates(struct wpa_driver_nl80211_data *drv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             ifindex,
             disabled ? "NL80211_TXRATE_LEGACY=OFDM-only"
  ...
  if (!band || (disabled && nla_put(msg, NL80211_TXRATE_LEGACY, 8,
...
static int get_channel_width(struct nl_msg *msg, void *arg) {
  ...
  if (tb[NL80211_ATTR_CHANNEL_WIDTH]) {
    sig_change->chanwidth =
        convert2width(nla_get_u32(tb[NL80211_ATTR_CHANNEL_WIDTH]));
    if (tb[NL80211_ATTR_CENTER_FREQ1])
      sig_change->center_frq1 = nla_get_u32(tb[NL80211_ATTR_CENTER_FREQ1]);
...
static int nl80211_send_frame(void *priv, const u8 *data, size_t data_len,
                              int encrypt) {
  struct i802_bss *bss = priv;
  return wpa_driver_nl80211_send_frame(bss, data, data_len, encrypt, 0, 0, 0, 0,
                                       0, NULL, 0);
}
...
static int nl80211_remove_pmkid(void *priv, const u8 *bssid, const u8 *pmkid) {
  struct i802_bss *bss = priv;
  wpa_printf(MSG_DEBUG, "nl80211: Delete PMKID for " MACSTR, MAC2STR(bssid));
...
static void add_survey(struct nlattr **sinfo, u32 ifidx,
  ...
  wpa_printf(
      MSG_DEBUG,
      "nl80211: Freq survey dump event (freq=%d MHz noise=%d channel_time=%ld "
      "busy_time=%ld tx_time=%ld rx_time=%ld filled=%04x)",
...
static void nl80211_set_rekey_info(void *priv, const u8 *kek, size_t kek_len,
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, (void *)-1);
  if (ret == -EOPNOTSUPP) {
    wpa_printf(MSG_DEBUG, "nl80211: Driver does not support rekey offload");
...
static void nl80211_send_null_frame(struct i802_bss *bss, const u8 *own_addr,
  ...
  if (qos) {
    nulldata.hdr.frame_control =
        IEEE80211_FC(WLAN_FC_TYPE_DATA, WLAN_FC_STYPE_QOS_NULL);
    size = sizeof(nulldata);
  } else {
    nulldata.hdr.frame_control =
        IEEE80211_FC(WLAN_FC_TYPE_DATA, WLAN_FC_STYPE_NULLFUNC);
...
static void nl80211_poll_client(void *priv, const u8 *own_addr, const u8 *addr,
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG,
               "nl80211: Client probe request for " MACSTR
               " failed: ret=%d (%s)",
...
static int nl80211_start_radar_detection(void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Start radar detection (CAC) %d MHz (ht_enabled=%d, "
             "vht_enabled=%d, bandwidth=%d MHz, cf1=%d MHz, cf2=%d MHz)",
             freq->freq, freq->ht_enabled, freq->vht_enabled, freq->bandwidth,
             freq->center_freq1, freq->center_freq2);
...
nl80211_tdls_enable_channel_switch(void *priv, const u8 *addr, u8 oper_class,
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Enable TDLS channel switch " MACSTR
             " oper_class=%u freq=%u",
...
static int nl80211_tdls_disable_channel_switch(void *priv, const u8 *addr) {
  struct i802_bss *bss = priv;
  struct wpa_driver_nl80211_data *drv = bss->drv;
  struct nl_msg *msg;
  ...
  if (!msg || nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr)) {
    nlmsg_free(msg);
    wpa_printf(MSG_DEBUG, "nl80211: Could not build TDLS cancel chan switch");
    return -ENOBUFS;
  }
...
static int driver_nl80211_set_key(const char *ifname, void *priv,
                                  ...
                                  const u8 *key, size_t key_len) {
  struct i802_bss *bss = priv;
  return wpa_driver_nl80211_set_key(ifname, bss, alg, addr, key_idx, set_tx,
...
static int driver_nl80211_send_mlme(void *priv, const u8 *data, size_t data_len,
                                    int noack, unsigned int freq,
                                    const u16 *csa_offs, size_t csa_offs_len) {
  struct i802_bss *bss = priv;
  return wpa_driver_nl80211_send_mlme(bss, data, data_len, noack, freq, 0, 0, 0,
                                      csa_offs, csa_offs_len);
}
...
static int driver_nl80211_send_action(void *priv, unsigned int freq,
                                      ...
                                      int no_cck) {
  struct i802_bss *bss = priv;
  return wpa_driver_nl80211_send_action(bss, freq, wait_time, dst, src, bssid,
...
static int wpa_driver_nl80211_status(void *priv, char *buf, size_t buflen) {
  ...
  res = os_snprintf(pos, end - pos,
                    ...
                    "freq=%d\n"
                    "%s%s%s%s%s",
                    bss->ifindex, bss->ifname, bss->brname, MAC2STR(bss->addr),
                    bss->freq, bss->beacon_set ? "beacon_set=1\n" : "",
  ...
  res = os_snprintf(
      ...
      drv->phyname, MAC2STR(drv->perm_addr), drv->ifindex, drv->operstate,
      scan_state_str(drv->scan_state), MAC2STR(drv->auth_bssid),
      MAC2STR(drv->auth_attempt_bssid), MAC2STR(drv->bssid),
      MAC2STR(drv->prev_bssid), drv->associated, drv->assoc_freq,
      drv->monitor_sock, drv->monitor_ifidx, drv->monitor_refcount,
      drv->last_mgmt_freq, drv->eapol_tx_sock,
      drv->ignore_if_down_event ? "ignore_if_down_event=1\n" : "",
  ...
  if (drv->has_capability) {
    res = os_snprintf(
        ...
        drv->capa.max_scan_ssids, drv->capa.max_sched_scan_ssids,
        drv->capa.sched_scan_supported, drv->capa.max_match_sets,
        drv->capa.max_remain_on_chan, drv->capa.max_stations,
        drv->capa.probe_resp_offloads, drv->capa.max_acl_mac_addrs,
        drv->capa.num_multichan_concurrent,
        drv->capa.mac_addr_rand_sched_scan_supported,
        drv->capa.mac_addr_rand_scan_supported, drv->capa.conc_capab,
        drv->capa.max_conc_chan_2_4, drv->capa.max_conc_chan_5_0,
        drv->capa.max_sched_scan_plans, drv->capa.max_sched_scan_plan_interval,
...
static int set_beacon_data(struct nl_msg *msg, struct beacon_data *settings) {
  if ((settings->head && nla_put(msg, NL80211_ATTR_BEACON_HEAD,
      ...
      (settings->tail && nla_put(msg, NL80211_ATTR_BEACON_TAIL,
                                 settings->tail_len, settings->tail)) ||
      (settings->beacon_ies &&
       nla_put(msg, NL80211_ATTR_IE, settings->beacon_ies_len,
               settings->beacon_ies)) ||
      (settings->proberesp_ies &&
       nla_put(msg, NL80211_ATTR_IE_PROBE_RESP, settings->proberesp_ies_len,
               settings->proberesp_ies)) ||
      (settings->assocresp_ies &&
       nla_put(msg, NL80211_ATTR_IE_ASSOC_RESP, settings->assocresp_ies_len,
               settings->assocresp_ies)) ||
      (settings->probe_resp &&
       nla_put(msg, NL80211_ATTR_PROBE_RESP, settings->probe_resp_len,
               settings->probe_resp)))
    return -ENOBUFS;
...
static int nl80211_switch_channel(void *priv, struct csa_settings *settings) {
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Channel switch request (cs_count=%u block_tx=%u freq=%d "
             "width=%d cf1=%d cf2=%d)",
             settings->cs_count, settings->block_tx, settings->freq_params.freq,
  ...
  if (drv->capa.max_csa_counters && csa_off_len > drv->capa.max_csa_counters) {
    wpa_printf(MSG_ERROR, "nl80211: Too many CSA counters provided");
    return -EINVAL;
  }
  ...
  for (i = 0; i < csa_off_len; i++) {
    ...
    if (settings->beacon_csa.probe_resp &&
        ((settings->beacon_csa.probe_resp_len <= csa_c_off_presp) ||
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: switch_channel failed err=%d (%s)", ret,
               strerror(-ret));
  }
  return ret;
...
static int nl80211_add_ts(void *priv, u8 tsid, const u8 *addr, u8 user_priority,
                          u16 admitted_time) {
  struct i802_bss *bss = priv;
  struct wpa_driver_nl80211_data *drv = bss->drv;
  struct nl_msg *msg;
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: add_ts request: tsid=%u admitted_time=%u up=%d", tsid,
             admitted_time, user_priority);

  if (!is_sta_interface(drv->nlmode))
    return -ENOTSUP;

  msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_ADD_TX_TS);
  if (!msg || nla_put_u8(msg, NL80211_ATTR_TSID, tsid) ||
      nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, addr) ||
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret)
    wpa_printf(MSG_DEBUG, "nl80211: add_ts failed err=%d (%s)", ret,
               strerror(-ret));
  return ret;
}
...
static int nl80211_del_ts(void *priv, u8 tsid, const u8 *addr) {
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret)
    wpa_printf(MSG_DEBUG, "nl80211: del_ts failed err=%d (%s)", ret,
               strerror(-ret));
  return ret;
}
...
static int cmd_reply_handler(struct nl_msg *msg, void *arg) {
  ...
  wpabuf_put_data(buf, genlmsg_attrdata(gnlh, 0), genlmsg_attrlen(gnlh, 0));
...
static int nl80211_vendor_cmd(void *priv, unsigned int vendor_id,
  ...
  if (vendor_id == 0xffffffff) {
    ...
    if (nlmsg_append(msg, (void *)data, data_len, NLMSG_ALIGNTO) < 0)
      goto fail;
    ret = send_and_recv_msgs(drv, msg, cmd_reply_handler, buf);
    if (ret)
      wpa_printf(MSG_DEBUG, "nl80211: command failed err=%d", ret);
    return ret;
  }
#endif /* CONFIG_TESTING_OPTIONS */
  ...
  if (ret)
    wpa_printf(MSG_DEBUG, "nl80211: vendor command failed err=%d", ret);
  return ret;
...
static int nl80211_set_qos_map(void *priv, const u8 *qos_map_set,
  ...
  wpa_hexdump(MSG_DEBUG, "nl80211: Setting QoS Map", qos_map_set,
              qos_map_set_len);
...
static int nl80211_set_wowlan(void *priv,
  ...
  if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_SET_WOWLAN)) ||
      ...
      (triggers->any && nla_put_flag(msg, NL80211_WOWLAN_TRIG_ANY)) ||
      (triggers->disconnect &&
...
static int nl80211_roaming(void *priv, int allowed, const u8 *bssid) {
  ...
  if (!drv->roaming_vendor_cmd_avail) {
    wpa_printf(MSG_DEBUG, "nl80211: Ignore roaming policy change since driver "
                          "does not provide command for setting it");
    return -1;
  }
  ...
  if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) ||
      ...
      (bssid && nla_put(msg, QCA_WLAN_VENDOR_ATTR_MAC_ADDR, ETH_ALEN, bssid))) {
    nlmsg_free(msg);
    return -1;
  }
  nla_nest_end(msg, params);
...
static int nl80211_set_mac_addr(void *priv, const u8 *addr) {
  ...
  if (linux_set_ifhwaddr(drv->global->ioctl_sock, bss->ifname, addr) < 0) {
    wpa_printf(MSG_DEBUG, "nl80211: failed to set_mac_addr for %s to " MACSTR,
  ...
  wpa_printf(MSG_DEBUG, "nl80211: set_mac_addr for %s to " MACSTR, bss->ifname,
             MAC2STR(addr));
...
static int wpa_driver_nl80211_init_mesh(void *priv) {
  if (wpa_driver_nl80211_set_mode(priv, NL80211_IFTYPE_MESH_POINT)) {
    wpa_printf(MSG_INFO, "nl80211: Failed to set interface into mesh mode");
    return -1;
  }
  return 0;
}
...
static int nl80211_put_mesh_id(struct nl_msg *msg, const u8 *mesh_id,
                               size_t mesh_id_len) {
  if (mesh_id) {
    wpa_hexdump_ascii(MSG_DEBUG, "  * Mesh ID (SSID)", mesh_id, mesh_id_len);
...
static int nl80211_join_mesh(struct i802_bss *bss,
  ...
  if (!msg || nl80211_put_freq_params(msg, &params->freq) ||
  ...
  if (params->ies) {
    ...
    if (nla_put(msg, NL80211_MESH_SETUP_IE, params->ie_len, params->ies))
      goto fail;
  }
  ...
  msg = NULL;
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: mesh join failed: ret=%d (%s)", ret,
               strerror(-ret));
    goto fail;
  }
  ret = 0;
...
static int wpa_driver_nl80211_leave_mesh(void *priv) {
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: mesh leave failed: ret=%d (%s)", ret,
               strerror(-ret));
  } else {
    wpa_printf(MSG_DEBUG, "nl80211: mesh leave request send successfully");
  }

  if (wpa_driver_nl80211_set_mode(drv->first_bss, NL80211_IFTYPE_STATION)) {
    wpa_printf(MSG_INFO, "nl80211: Failed to set interface into station mode");
  }
  return ret;
}
...
static int wpa_driver_br_add_ip_neigh(void *priv, u8 version, const u8 *ipaddr,
  ...
  if (res) {
    wpa_printf(MSG_DEBUG, "nl80211: neigh set destination addr failed");
    goto errout;
  }
  ...
  if (res) {
    wpa_printf(MSG_DEBUG, "nl80211: Adding bridge ip neigh failed: %s",
               strerror(errno));
  }
errout:
  if (nl_lladdr)
...
static int wpa_driver_br_delete_ip_neigh(void *priv, u8 version,
  ...
  if (bss->br_ifindex == 0) {
    wpa_printf(MSG_DEBUG, "nl80211: bridge must be set to delete an ip neigh");
    return -1;
  }
  ...
  if (res) {
    wpa_printf(MSG_DEBUG, "nl80211: Deleting bridge ip neigh failed: %s",
               strerror(errno));
  }
errout:
  if (nl_ipaddr)
...
static int wpa_driver_br_port_set_attr(void *priv, enum drv_br_port_attr attr,
  ...
  os_snprintf(path, sizeof(path), "/sys/class/net/%s/brport/%s", bss->ifname,
              attr_txt);
...
static int wpa_driver_br_set_net_param(void *priv, enum drv_br_net_param param,
  ...
  os_snprintf(path, sizeof(path), "/proc/sys/net/ipv%d/conf/%s/%s", ip_version,
              bss->brname, param_txt);
...
static int add_acs_freq_list(struct nl_msg *msg, const int *freq_list) {
  ...
  for (i = 0; i < len; i++)
    freqs[i] = freq_list[i];
  ret = nla_put(msg, QCA_WLAN_VENDOR_ATTR_ACS_FREQ_LIST, sizeof(u32) * len,
                freqs);
  os_free(freqs);
  return ret;
}
...
static int wpa_driver_do_acs(void *priv, struct drv_acs_params *params) {
  ...
  if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) ||
      ...
      nla_put_u16(msg, QCA_WLAN_VENDOR_ATTR_ACS_CHWIDTH, params->ch_width) ||
      (params->ch_list_len && nla_put(msg, QCA_WLAN_VENDOR_ATTR_ACS_CH_LIST,
                                      params->ch_list_len, params->ch_list)) ||
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Failed to invoke driver ACS function: %s",
               strerror(errno));
  }
  return ret;
}
...
static int nl80211_set_band(void *priv, enum set_band band) {
  ...
  ret = send_and_recv_msgs(drv, msg, NULL, NULL);
  if (ret) {
    wpa_printf(MSG_DEBUG, "nl80211: Driver setband function failed: %s",
               strerror(errno));
  }
  return ret;
}
...
static int preferred_freq_info_handler(struct nl_msg *msg, void *arg) {
  ...
  nla_parse(tb_vendor, QCA_WLAN_VENDOR_ATTR_MAX, nla_data(nl_vend),
            nla_len(nl_vend), NULL);

  attr = tb_vendor[QCA_WLAN_VENDOR_ATTR_GET_PREFERRED_FREQ_LIST_IFACE_TYPE];
  if (!attr) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Driver returned iface_type=%d", iface_type);

  attr = tb_vendor[QCA_WLAN_VENDOR_ATTR_GET_PREFERRED_FREQ_LIST];
  if (!attr) {
    wpa_printf(MSG_ERROR, "nl80211: preferred_freq_list couldn't be found");
    param->num = 0;
    return NL_SKIP;
  }
...
static int nl80211_get_pref_freq_list(void *priv,
  ...
  if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) ||
      ...
      !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
      nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_GET_PREFERRED_FREQ_LIST_IFACE_TYPE,
                  iface_type)) {
    wpa_printf(MSG_ERROR, "%s: err in adding vendor_cmd and vendor_data",
               __func__);
    nlmsg_free(msg);
    return -1;
  }
  ...
  if (ret) {
    wpa_printf(MSG_ERROR, "%s: err in send_and_recv_msgs", __func__);
    return ret;
  }
  ...
  *num = param.num;
  ...
  for (i = 0; i < *num; i++) {
    wpa_printf(MSG_DEBUG, "nl80211: preferred_channel_list[%d]=%d", i,
               freq_list[i]);
  }
...
static int nl80211_set_prob_oper_freq(void *priv, unsigned int freq) {
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Set P2P probable operating freq %u for ifindex %d", freq,
             bss->ifindex);
  ...
  if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR)) ||
      ...
      !(params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA)) ||
      nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_PROBABLE_OPER_CHANNEL_IFACE_TYPE,
                  QCA_IFACE_TYPE_P2P_CLIENT) ||
      nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_PROBABLE_OPER_CHANNEL_FREQ, freq)) {
    wpa_printf(MSG_ERROR, "%s: err in adding vendor_cmd and vendor_data",
               __func__);
    nlmsg_free(msg);
    return -1;
...
static int nl80211_p2p_lo_start(void *priv, unsigned int freq,
                                ...
                                size_t dev_types_len, const u8 *ies,
                                size_t ies_len) {
  struct i802_bss *bss = priv;
  struct wpa_driver_nl80211_data *drv = bss->drv;
  struct nl_msg *msg;
  ...
  wpa_printf(MSG_DEBUG,
             "nl80211: Start P2P Listen offload: freq=%u, period=%u, "
             "interval=%u, count=%u",
  ...
  if (nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_CHANNEL, freq) ||
      ...
      nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_INTERVAL,
                  interval) ||
      nla_put_u32(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_COUNT, count) ||
      nla_put(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_DEVICE_TYPES,
              dev_types_len, device_types) ||
      nla_put(msg, QCA_WLAN_VENDOR_ATTR_P2P_LISTEN_OFFLOAD_VENDOR_IE, ies_len,
              ies))
    goto fail;
...
static int nl80211_write_to_file(const char *name, unsigned int val) {
  ...
  fd = open(name, O_RDWR);
  if (fd < 0) {
    wpa_printf(MSG_ERROR, "nl80211: Failed to open %s: %s", name,
               strerror(errno));
    return fd;
  }
  ...
  if (len < 0)
    wpa_printf(MSG_ERROR, "nl80211: Failed to write to %s: %s", name,
               strerror(errno));
  close(fd);
...
static int nl80211_configure_data_frame_filters(void *priv, u32 filter_flags) {
  ...
  wpa_printf(MSG_DEBUG, "nl80211: Data frame filter flags=0x%x", filter_flags);
  ...
  ret = nl80211_write_to_file(
      path, !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_GTK));
  if (ret) {
    wpa_printf(MSG_ERROR,
  ...
  /* Configure filtering of unicast frame encrypted using GTK */
  os_snprintf(path, sizeof(path),
              "/proc/sys/net/ipv4/conf/%s/drop_gratuitous_arp", bss->ifname);
  ret = nl80211_write_to_file(
      path, !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_ARP));
  if (ret) {
    wpa_printf(MSG_ERROR, "nl80211: Failed set gratuitous ARP filter");
    return ret;
  }

  /* Configure filtering of IPv6 NA frames */
  os_snprintf(path, sizeof(path),
              "/proc/sys/net/ipv6/conf/%s/drop_unsolicited_na", bss->ifname);
  ret = nl80211_write_to_file(path,
                              !!(filter_flags & WPA_DATA_FRAME_FILTER_FLAG_NA));
  if (ret) {
    wpa_printf(MSG_ERROR, "nl80211: Failed to set unsolicited NA filter");
    return ret;
  }
