...
static int hostap_set_iface_flags(void *priv, int dev_up) {
  ...
  if (dev_up) {
    ...
    if (ioctl(drv->ioctl_sock, SIOCSIFMTU, &ifr) != 0) {
      wpa_printf(MSG_INFO,
                 "Setting MTU failed - trying to survive with current value: "
                 "ioctl[SIOCSIFMTU]: %s",
                 strerror(errno));
    }
  }
...
static int hostapd_ioctl(void *priv, struct prism2_hostapd_param *param,
  ...
  if (ioctl(drv->ioctl_sock, PRISM2_IOCTL_HOSTAPD, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[PRISM2_IOCTL_HOSTAPD]: %s", strerror(errno));
    return -1;
  }
...
static int wpa_driver_hostap_set_key(const char *ifname, void *priv,
  ...
  switch (alg) {
  case WPA_ALG_NONE:
    os_strlcpy((char *)param->u.crypt.alg, "NONE", HOSTAP_CRYPT_ALG_NAME_LEN);
    break;
  case WPA_ALG_WEP:
    os_strlcpy((char *)param->u.crypt.alg, "WEP", HOSTAP_CRYPT_ALG_NAME_LEN);
    break;
  case WPA_ALG_TKIP:
    os_strlcpy((char *)param->u.crypt.alg, "TKIP", HOSTAP_CRYPT_ALG_NAME_LEN);
    break;
  case WPA_ALG_CCMP:
    os_strlcpy((char *)param->u.crypt.alg, "CCMP", HOSTAP_CRYPT_ALG_NAME_LEN);
    break;
  default:
    os_free(buf);
    return -1;
...
static int hostap_set_privacy(void *priv, int enabled) {
  ...
  return hostap_ioctl_prism2param(drv, PRISM2_PARAM_PRIVACY_INVOKED, enabled);
}
...
static int hostap_read_sta_data(void *priv, struct hostap_sta_driver_data *data,
                                const u8 *addr) {
  struct hostap_driver_data *drv = priv;
  ...
  snprintf(buf, sizeof(buf), "/proc/net/hostap/%s/" MACSTR, drv->iface,
           MAC2STR(addr));
...
static int hostapd_ioctl_set_generic_elem(struct hostap_driver_data *drv) {
  ...
  if (drv->generic_ie) {
    os_memcpy(param->u.generic_elem.data, drv->generic_ie, drv->generic_ie_len);
  }
  if (drv->wps_ie) {
    os_memcpy(&param->u.generic_elem.data[drv->generic_ie_len], drv->wps_ie,
              drv->wps_ie_len);
  }
  wpa_hexdump(MSG_DEBUG, "hostap: Set generic IE", param->u.generic_elem.data,
              elem_len);
...
static int hostap_set_ap_wps_ie(void *priv, const struct wpabuf *beacon,
  ...
  if (proberesp) {
    ...
    if (drv->wps_ie == NULL)
      return -1;
    os_memcpy(drv->wps_ie, wpabuf_head(proberesp), wpabuf_len(proberesp));
...
static void hostapd_wireless_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi,
                                               u8 *buf, size_t len) {
  ...
  while (RTA_OK(attr, attrlen)) {
    if (attr->rta_type == IFLA_WIRELESS) {
      hostapd_wireless_event_wireless(drv, ((char *)attr) + rta_len,
...
static void *hostap_init(struct hostapd_data *hapd,
  ...
  if (hostap_ioctl_prism2param(drv, PRISM2_PARAM_HOSTAPD, 1)) {
    wpa_printf(MSG_ERROR, "Could not enable hostapd mode for interface %s",
               drv->iface);
...
static int hostap_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr,
  ...
  return hostap_send_mlme(drv, (u8 *)&mgmt,
                          IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0, 0, NULL,
                          0);
}
...
static int hostap_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr,
  ...
  return hostap_send_mlme(drv, (u8 *)&mgmt,
                          IEEE80211_HDRLEN + sizeof(mgmt.u.disassoc), 0, 0,
                          NULL, 0);
}

static struct hostapd_hw_modes *
hostap_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags) {
  struct hostapd_hw_modes *mode;
  int i, clen, rlen;
  const short chan2freq[14] = {2412, 2417, 2422, 2427, 2432, 2437, 2442,
