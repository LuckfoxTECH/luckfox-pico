...
int wpa_driver_wext_set_auth_param(struct wpa_driver_wext_data *drv, int idx,
                                   u32 value) {
  struct iwreq iwr;
  int ret = 0;
...
int wpa_driver_wext_get_ssid(void *priv, u8 *ssid) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCGIWESSID, &iwr) < 0) {
     ...
     * termination. */
    if (ret > 0 && ssid[ret - 1] == '\0' && drv->we_version_compiled < 21)
      ret--;
  }
...
int wpa_driver_wext_set_ssid(void *priv, const u8 *ssid, size_t ssid_len) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWESSID, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWESSID]: %s", strerror(errno));
    ret = -1;
  }
...
int wpa_driver_wext_set_freq(void *priv, int freq) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWFREQ, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWFREQ]: %s", strerror(errno));
    ret = -1;
  }
...
static void wpa_driver_wext_event_wireless_custom(void *ctx, char *custom) {
  ...
  wpa_printf(MSG_MSGDUMP, "WEXT: Custom wireless event: '%s'", custom);

  os_memset(&data, 0, sizeof(data));
  /* Host AP driver */
  if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
    data.michael_mic_failure.unicast = os_strstr(custom, " unicast ") != NULL;
    ...
    if (req_ies == NULL || hexstr2bin(spos, req_ies, bytes) < 0)
      goto done;
    ...
    if (os_strncmp(spos, " RespIEs=", 9) == 0) {
      ...
      if (resp_ies == NULL || hexstr2bin(spos, resp_ies, bytes) < 0)
        goto done;
...
static int wpa_driver_wext_event_wireless_michaelmicfailure(void *ctx,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             mic->flags, MAC2STR(mic->src_addr.sa_data));
...
static int
wpa_driver_wext_event_wireless_pmkidcand(struct wpa_driver_wext_data *drv,
                                         const char *ev, size_t len) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "flags=0x%x index=%d bssid=" MACSTR,
             cand->flags, cand->index, MAC2STR(addr));
...
static int
wpa_driver_wext_event_wireless_assocreqie(struct wpa_driver_wext_data *drv,
                                          const char *ev, int len) {
  if (len < 0)
    return -1;

  wpa_hexdump(MSG_DEBUG, "AssocReq IE wireless event", (const u8 *)ev, len);
...
static int
wpa_driver_wext_event_wireless_assocrespie(struct wpa_driver_wext_data *drv,
                                           const char *ev, int len) {
  if (len < 0)
    return -1;

  wpa_hexdump(MSG_DEBUG, "AssocResp IE wireless event", (const u8 *)ev, len);
...
static void wpa_driver_wext_event_wireless(struct wpa_driver_wext_data *drv,
  ...
  while ((size_t)(end - pos) >= IW_EV_LCP_LEN) {
    ...
    wpa_printf(MSG_DEBUG, "Wireless event: cmd=0x%x len=%d", iwe->cmd,
               iwe->len);
    ...
    if (drv->we_version_compiled > 18 &&
        (iwe->cmd == IWEVMICHAELMICFAILURE || iwe->cmd == IWEVCUSTOM ||
         iwe->cmd == IWEVASSOCREQIE || iwe->cmd == IWEVASSOCRESPIE ||
    ...
    switch (iwe->cmd) {
    case SIOCGIWAP:
      wpa_printf(MSG_DEBUG, "Wireless event: new AP: " MACSTR,
                 MAC2STR((u8 *)iwe->u.ap_addr.sa_data));
      if (is_zero_ether_addr((const u8 *)iwe->u.ap_addr.sa_data) ||
          os_memcmp(iwe->u.ap_addr.sa_data, "\x44\x44\x44\x44\x44\x44",
                    ETH_ALEN) == 0) {
        os_free(drv->assoc_req_ies);
        drv->assoc_req_ies = NULL;
        os_free(drv->assoc_resp_ies);
    ...
    case IWEVMICHAELMICFAILURE:
      ...
      wpa_driver_wext_event_wireless_michaelmicfailure(drv->ctx, custom,
                                                       iwe->u.data.length);
      break;
    case IWEVCUSTOM:
      if (iwe->u.data.length > end - custom) {
    ...
    case IWEVASSOCREQIE:
      ...
      wpa_driver_wext_event_wireless_assocreqie(drv, custom,
                                                iwe->u.data.length);
      break;
    case IWEVASSOCRESPIE:
      ...
      wpa_driver_wext_event_wireless_assocrespie(drv, custom,
                                                 iwe->u.data.length);
      break;
    case IWEVPMKIDCAND:
      if (iwe->u.data.length > end - custom) {
      ...
      wpa_driver_wext_event_wireless_pmkidcand(drv, custom, iwe->u.data.length);
      break;
    }
...
static void wpa_driver_wext_event_link(struct wpa_driver_wext_data *drv,
  ...
  event.interface_status.ievent =
      del ? EVENT_INTERFACE_REMOVED : EVENT_INTERFACE_ADDED;

  wpa_printf(MSG_DEBUG, "RTM_%sLINK, IFLA_IFNAME: Interface '%s' %s",
             del ? "DEL" : "NEW", event.interface_status.ifname,
             del ? "removed" : "added");
...
static int wpa_driver_wext_own_ifname(struct wpa_driver_wext_data *drv, u8 *buf,
                                      size_t len) {
  int attrlen, rta_len;
  struct rtattr *attr;
  ...
  while (RTA_OK(attr, attrlen)) {
    if (attr->rta_type == IFLA_IFNAME) {
      if (os_strcmp(((char *)attr) + rta_len, drv->ifname) == 0)
        return 1;
      else
        break;
    }
...
static void wpa_driver_wext_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi,
  ...
  while (RTA_OK(attr, attrlen)) {
    if (attr->rta_type == IFLA_WIRELESS) {
      wpa_driver_wext_event_wireless(drv, ((char *)attr) + rta_len,
                                     attr->rta_len - rta_len);
    } else if (attr->rta_type == IFLA_IFNAME) {
      wpa_driver_wext_event_link(drv, ((char *)attr) + rta_len,
...
static void wext_get_phy_name(struct wpa_driver_wext_data *drv) {
  ...
  f = open(buf, O_RDONLY);
  if (f < 0) {
    wpa_printf(MSG_DEBUG, "Could not open file %s: %s", buf, strerror(errno));
    return;
  }
  ...
  close(f);
  if (rv < 0) {
    wpa_printf(MSG_DEBUG, "Could not read file %s: %s", buf, strerror(errno));
    return;
  }
  ...
  if (pos)
    *pos = '\0';
  wpa_printf(MSG_DEBUG, "wext: interface %s phy: %s", drv->ifname,
             drv->phyname);
}
...
static int wext_hostap_ifname(struct wpa_driver_wext_data *drv,
  ...
  if (type == ARPHRD_IEEE80211) {
    wpa_printf(MSG_DEBUG, "WEXT: Found hostap driver wifi# interface (%s)",
               ifname);
...
static void wext_check_hostap(struct wpa_driver_wext_data *drv) {
  ...
  snprintf(buf, sizeof(buf), "/sys/class/net/%s/device/driver", drv->ifname);
  ...
  if (res > 0) {
    ...
    if (os_strncmp(pos, "hostap", 6) == 0 && wext_add_hostap(drv) == 0)
      return;
  }
...
static int wpa_driver_wext_finish_drv_init(struct wpa_driver_wext_data *drv) {
  ...
  if (send_rfkill_event) {
    eloop_register_timeout(0, 0, wpa_driver_wext_send_rfkill, drv, drv->ctx);
  }
...
int wpa_driver_wext_scan(void *priv, struct wpa_driver_scan_params *params) {
  ...
  if (ssid_len > IW_ESSID_MAX_SIZE) {
    wpa_printf(MSG_DEBUG, "%s: too long SSID (%lu)", __FUNCTION__,
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWSCAN, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWSCAN]: %s", strerror(errno));
    ret = -1;
  }
...
static u8 *wpa_driver_wext_giwscan(struct wpa_driver_wext_data *drv,
  ...
  for (;;) {
    ...
    if (errno == E2BIG && res_buf_len < 65535) {
      ...
      wpa_printf(MSG_ERROR, "ioctl[SIOCGIWSCAN]: %s", strerror(errno));
      os_free(res_buf);
      return NULL;
    }
  }
...
static void wext_get_scan_ssid(struct iw_event *iwe, struct wext_scan_data *res,
...
static void wext_get_scan_freq(struct iw_event *iwe,
  ...
  if (iwe->u.freq.e > 6) {
    wpa_printf(MSG_DEBUG,
               "Invalid freq in scan results (BSSID=" MACSTR " m=%d e=%d)",
               MAC2STR(res->res.bssid), iwe->u.freq.m, iwe->u.freq.e);
    return;
  }
...
static void wext_get_scan_qual(struct wpa_driver_wext_data *drv,
  ...
  if ((iwe->u.qual.updated & IW_QUAL_DBM) ||
      ((iwe->u.qual.level != 0) && (iwe->u.qual.level > drv->max_level))) {
...
static void wext_get_scan_rate(struct iw_event *iwe, struct wext_scan_data *res,
                               char *pos, char *end) {
  int maxrate;
...
static int wext_19_iw_point(struct wpa_driver_wext_data *drv, u16 cmd) {
  return drv->we_version_compiled > 18 &&
         (cmd == SIOCGIWESSID || cmd == SIOCGIWENCODE || cmd == IWEVGENIE ||
          cmd == IWEVCUSTOM);
}
...
struct wpa_scan_results *wpa_driver_wext_get_scan_results(void *priv) {
  ...
  while ((size_t)(end - pos) >= IW_EV_LCP_LEN) {
    ...
    case SIOCGIWAP:
      ...
      os_free(data.ie);
      os_memset(&data, 0, sizeof(data));
      os_memcpy(data.res.bssid, iwe->u.ap_addr.sa_data, ETH_ALEN);
      break;
    case SIOCGIWMODE:
...
static int wpa_driver_wext_set_psk(struct wpa_driver_wext_data *drv,
  ...
  if (ret < 0)
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWENCODEEXT] PMK: %s", strerror(errno));
  os_free(ext);
...
static int wpa_driver_wext_set_key_ext(void *priv, enum wpa_alg alg,
  ...
  if (seq_len > IW_ENCODE_SEQ_MAX_SIZE) {
    wpa_printf(MSG_DEBUG, "%s: Invalid seq_len %lu", __FUNCTION__,
  ...
  default:
    wpa_printf(MSG_DEBUG, "%s: Unknown algorithm %d", __FUNCTION__, alg);
    os_free(ext);
    return -1;
  }
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWENCODEEXT, &iwr) < 0) {
    ...
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWENCODEEXT]: %s", strerror(errno));
  }
...
int wpa_driver_wext_set_key(const char *ifname, void *priv, enum wpa_alg alg,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "key_len=%lu",
             __FUNCTION__, alg, key_idx, set_tx, (unsigned long)seq_len,
             (unsigned long)key_len);

  ret = wpa_driver_wext_set_key_ext(drv, alg, addr, key_idx, set_tx, seq,
                                    seq_len, key, key_len);
  if (ret == 0)
    return 0;

  if (ret == -2 && (alg == WPA_ALG_NONE || alg == WPA_ALG_WEP)) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWENCODE, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWENCODE]: %s", strerror(errno));
    ret = -1;
  }
  ...
  if (set_tx && alg != WPA_ALG_NONE) {
    ...
    if (ioctl(drv->ioctl_sock, SIOCSIWENCODE, &iwr) < 0) {
      wpa_printf(MSG_ERROR, "ioctl[SIOCSIWENCODE] (set_tx): %s",
                 strerror(errno));
      ret = -1;
    }
  }
...
static int wpa_driver_wext_set_countermeasures(void *priv, int enabled) {
  struct wpa_driver_wext_data *drv = priv;
  wpa_printf(MSG_DEBUG, "%s", __FUNCTION__);
  return wpa_driver_wext_set_auth_param(drv, IW_AUTH_TKIP_COUNTERMEASURES,
                                        enabled);
}
...
static int wpa_driver_wext_set_drop_unencrypted(void *priv, int enabled) {
  ...
  return wpa_driver_wext_set_auth_param(drv, IW_AUTH_DROP_UNENCRYPTED, enabled);
}
...
static int wpa_driver_wext_mlme(struct wpa_driver_wext_data *drv,
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWMLME, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWMLME]: %s", strerror(errno));
    ret = -1;
  }
...
static void wpa_driver_wext_disconnect(struct wpa_driver_wext_data *drv) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCGIWMODE, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGIWMODE]: %s", strerror(errno));
...
static int wpa_driver_wext_set_gen_ie(void *priv, const u8 *ie, size_t ie_len) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWGENIE, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWGENIE]: %s", strerror(errno));
    ret = -1;
  }
...
int wpa_driver_wext_associate(void *priv,
  ...
  if (drv->cfg80211) {
     ...
     */
    if (wpa_driver_wext_set_ssid(drv, (u8 *)"", 0) < 0) {
      wpa_printf(MSG_DEBUG, "WEXT: Failed to clear SSID to stop pending "
                            "cfg80211 association attempts (if any)");
      /* continue anyway */
    }
  }

  if (wpa_driver_wext_set_drop_unencrypted(drv, params->drop_unencrypted) < 0)
    ret = -1;
  ...
  if (!params->bssid && wpa_driver_wext_set_bssid(drv, NULL) < 0)
    ret = -1;

  /* TODO: should consider getting wpa version and cipher/key_mgmt suites
   * from configuration, not from here, where only the selected suite is
   * available */
  if (wpa_driver_wext_set_gen_ie(drv, params->wpa_ie, params->wpa_ie_len) < 0)
    ret = -1;
  ...
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_WPA_VERSION, value) < 0)
    ret = -1;
  value = wpa_driver_wext_cipher2wext(params->pairwise_suite);
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_CIPHER_PAIRWISE, value) < 0)
    ret = -1;
  value = wpa_driver_wext_cipher2wext(params->group_suite);
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_CIPHER_GROUP, value) < 0)
    ret = -1;
  ...
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_PRIVACY_INVOKED, value) < 0)
    ret = -1;
  ...
  if (wpa_driver_wext_set_psk(drv, params->psk) < 0)
    ret = -1;
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_RX_UNENCRYPTED_EAPOL,
                                     allow_unencrypted_eapol) < 0)
    ret = -1;
#ifdef CONFIG_IEEE80211W
  ...
  if (wpa_driver_wext_set_auth_param(drv, IW_AUTH_MFP, value) < 0)
    ret = -1;
#endif /* CONFIG_IEEE80211W */
  if (params->freq.freq && wpa_driver_wext_set_freq(drv, params->freq.freq) < 0)
    ret = -1;
  if (!drv->cfg80211 &&
      wpa_driver_wext_set_ssid(drv, params->ssid, params->ssid_len) < 0)
    ret = -1;
  if (params->bssid && wpa_driver_wext_set_bssid(drv, params->bssid) < 0)
    ret = -1;
  if (drv->cfg80211 &&
      wpa_driver_wext_set_ssid(drv, params->ssid, params->ssid_len) < 0)
    ret = -1;
...
static int wpa_driver_wext_set_auth_alg(void *priv, int auth_alg) {
  ...
  res = wpa_driver_wext_set_auth_param(drv, IW_AUTH_80211_AUTH_ALG, algs);
...
int wpa_driver_wext_set_mode(void *priv, int mode) {
  ...
  if (errno != EBUSY) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWMODE]: %s", strerror(errno));
    goto done;
  }
...
static int wpa_driver_wext_pmksa(struct wpa_driver_wext_data *drv, u32 cmd,
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWPMKSA, &iwr) < 0) {
    if (errno != EOPNOTSUPP)
      wpa_printf(MSG_ERROR, "ioctl[SIOCSIWPMKSA]: %s", strerror(errno));
    ret = -1;
  }
...
int wpa_driver_wext_set_operstate(void *priv, int state) {
  ...
  wpa_printf(MSG_DEBUG, "%s: operstate %d->%d (%s)", __func__, drv->operstate,
             state, state ? "UP" : "DORMANT");
  drv->operstate = state;
...
static int wpa_driver_wext_signal_poll(void *priv, struct wpa_signal_info *si) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCGIWSTATS, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "WEXT: SIOCGIWSTATS: %s", strerror(errno));
    return -1;
  }

  si->current_signal =
      stats.qual.level - ((stats.qual.updated & IW_QUAL_DBM) ? 0x100 : 0);
  si->current_noise =
      stats.qual.noise - ((stats.qual.updated & IW_QUAL_DBM) ? 0x100 : 0);
  return 0;
}
...
static int wpa_driver_wext_status(void *priv, char *buf, size_t buflen) {
  ...
  res = os_snprintf(pos, end - pos,
                    ...
                    "ifname=%s\n"
                    "addr=" MACSTR "\n",
                    drv->ifindex, drv->ifname, MAC2STR(addr));
