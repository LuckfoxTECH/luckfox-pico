...
HRESULT STDMETHODCALLTYPE call_IWbemServices_ExecQuery(
  ...
  hr = IWbemServices_ExecQuery(pSvc, bsQueryLanguage, bsQuery, lFlags, pCtx,
                               ppEnum);
...
HRESULT STDMETHODCALLTYPE call_IWbemServices_ExecNotificationQueryAsync(
  ...
  hr = IWbemServices_ExecNotificationQueryAsync(pSvc, bsQueryLanguage, bsQuery,
...
HRESULT STDMETHODCALLTYPE call_IWbemLocator_ConnectServer(
  ...
  hr = IWbemLocator_ConnectServer(pLoc, bsNetworkResource, bsUser, bsPassword,
                                  bsLocale, lSecurityFlags, bsAuthority, pCtx,
                                  ppNamespace);
...
static int ndis_events_constructor(struct ndis_events_data *events) {
  ...
  if (!CreatePipe(&events->read_pipe, &events->write_pipe, NULL, 512)) {
    wpa_printf(MSG_ERROR, "CreatePipe() failed: %d", (int)GetLastError());
    return -1;
  }
  events->event_avail = CreateEvent(NULL, TRUE, FALSE, NULL);
  if (events->event_avail == NULL) {
    wpa_printf(MSG_ERROR, "CreateEvent() failed: %d", (int)GetLastError());
...
static int ndis_events_send_event(struct ndis_events_data *events,
                                  enum event_types type, char *data,
                                  size_t data_len) {
...
static void ndis_events_media_specific(struct ndis_events_data *events,
  ...
  hr = IWbemClassObject_Get(pObj, L"NdisStatusMediaSpecificIndication", 0, &vt,
                            NULL, NULL);
  if (FAILED(hr)) {
...
ndis_events_indicate(IWbemObjectSink *this, long lObjectCount,
  ...
  for (i = 0; i < lObjectCount; i++) {
    ...
    hr = IWbemClassObject_Get(pObj, L"__CLASS", 0, &vtClass, NULL, NULL);
    if (FAILED(hr)) {
    ...
    if (wcscmp(vtClass.bstrVal, L"MSNdis_NotifyAdapterArrival") == 0) {
    ...
    if (wcscmp(vtClass.bstrVal, L"MSNdis_StatusMediaSpecificIndication") == 0) {
      ndis_events_media_specific(events, pObj);
    } else if (wcscmp(vtClass.bstrVal, L"MSNdis_StatusMediaConnect") == 0) {
      ndis_events_media_connect(events);
    } else if (wcscmp(vtClass.bstrVal, L"MSNdis_StatusMediaDisconnect") == 0) {
      ndis_events_media_disconnect(events);
    } else if (wcscmp(vtClass.bstrVal, L"MSNdis_NotifyAdapterArrival") == 0) {
      ndis_events_adapter_arrival(events);
    } else if (wcscmp(vtClass.bstrVal, L"MSNdis_NotifyAdapterRemoval") == 0) {
...
static int notification_query(IWbemObjectSink *pDestSink, IWbemServices *pSvc,
                              const char *class_name) {
  HRESULT hr;
  WCHAR query[256];

  _snwprintf(query, 256, L"SELECT * FROM %S", class_name);
  wpa_printf(MSG_DEBUG, "ndis_events: WMI: %S", query);
  hr = call_IWbemServices_ExecNotificationQueryAsync(pSvc, L"WQL", query, 0, 0,
                                                     pDestSink);
  if (FAILED(hr)) {
    wpa_printf(MSG_DEBUG,
...
static int register_async_notification(IWbemObjectSink *pDestSink,
                                       IWbemServices *pSvc) {
  int i;
  const char *class_list[] = {"MSNdis_StatusMediaConnect",
...
#define MAX_QUERY_LEN 256
  ...
  hr = call_IWbemLocator_ConnectServer(events->pLoc, L"ROOT\\CIMV2", NULL, NULL,
                                       0, 0, 0, 0, &pSvc);
  if (FAILED(hr)) {
    wpa_printf(MSG_ERROR,
               "ndis_events: Could not connect to WMI "
               "server (ROOT\\CIMV2) - error 0x%x",
               (int)hr);
  ...
  hr = call_IWbemServices_ExecQuery(pSvc, L"WQL", query,
  ...
  uReturned = 0;
  hr = IEnumWbemClassObject_Next(pEnumerator, WBEM_INFINITE, 1, &pObj,
                                 &uReturned);
  ...
  if (!SUCCEEDED(hr)) {
    wpa_printf(MSG_DEBUG,
               "ndis_events: Failed to query interface "
               "from Win32_NetworkAdapter: 0x%x",
               (int)hr);
    IWbemServices_Release(pSvc);
    return ndis_events_use_desc(events, desc);
  }
  ...
  if (!SUCCEEDED(hr)) {
    wpa_printf(MSG_DEBUG,
               "ndis_events: Failed to query interface "
               "Name from Win32_PnPEntity: 0x%x",
               (int)hr);
    IWbemServices_Release(pSvc);
    if (events->adapter_desc == NULL)
      return ndis_events_use_desc(events, desc);
  ...
  wpa_printf(MSG_DEBUG, "ndis_events: Win32_PnPEntity::Name='%S'", vt.bstrVal);
  os_free(events->adapter_desc);
  events->adapter_desc = _wcsdup(vt.bstrVal);
  VariantClear(&vt);
...
struct ndis_events_data *ndis_events_init(HANDLE *read_pipe,
  ...
  hr = call_IWbemLocator_ConnectServer(events->pLoc, L"ROOT\\WMI", NULL, NULL,
                                       0, 0, 0, 0, &events->pSvc);
  if (FAILED(hr)) {
    wpa_printf(MSG_ERROR,
