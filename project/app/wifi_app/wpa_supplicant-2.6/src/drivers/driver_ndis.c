...
static int ndis_get_oid(struct wpa_driver_ndis_data *drv, unsigned int oid,
...
#endif /* _WIN32_WCE */
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_QUERY_OID_VALUE, o,
                       sizeof(NDISUIO_QUERY_OID), o, buflen, &written, NULL)) {
    wpa_printf(MSG_DEBUG,
               "NDIS: IOCTL_NDISUIO_QUERY_OID_VALUE "
               "failed (oid=%08x): %d",
               oid, (int)GetLastError());
    os_free(o);
    return -1;
  }
  ...
  if (written < hdrlen) {
    wpa_printf(MSG_DEBUG,
               "NDIS: query oid=%08x written (%d); "
               "too short",
               oid, (unsigned int)written);
    os_free(o);
    return -1;
  }
  written -= hdrlen;
  ...
  if (!PacketRequest(drv->adapter, FALSE, o)) {
    wpa_printf(MSG_DEBUG, "%s: oid=0x%x len (%d) failed", __func__, oid, len);
    os_free(buf);
    return -1;
  }
  if (o->Length > len) {
    wpa_printf(MSG_DEBUG, "%s: oid=0x%x Length (%d) > len (%d)", __func__, oid,
               (unsigned int)o->Length, len);
    os_free(buf);
    return -1;
  }
...
static int ndis_set_oid(struct wpa_driver_ndis_data *drv, unsigned int oid,
  ...
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_SET_OID_VALUE, o, reallen,
                       NULL, 0, &written, NULL)) {
    wpa_printf(MSG_DEBUG,
               "NDIS: IOCTL_NDISUIO_SET_OID_VALUE "
               "(oid=%08x) failed: %d",
               oid, (int)GetLastError());
    os_free(o);
    return -1;
...
static int ndis_set_auth_mode(struct wpa_driver_ndis_data *drv, int mode) {
  u32 auth_mode = mode;
  if (ndis_set_oid(drv, OID_802_11_AUTHENTICATION_MODE, (char *)&auth_mode,
                   sizeof(auth_mode)) < 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to set "
...
static int ndis_get_auth_mode(struct wpa_driver_ndis_data *drv) {
  u32 auth_mode;
  int res;
  res = ndis_get_oid(drv, OID_802_11_AUTHENTICATION_MODE, (char *)&auth_mode,
                     sizeof(auth_mode));
...
static int ndis_set_encr_status(struct wpa_driver_ndis_data *drv, int encr) {
  ...
  if (ndis_set_oid(drv, OID_802_11_ENCRYPTION_STATUS, (char *)&encr_status,
                   sizeof(encr_status)) < 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to set "
               "OID_802_11_ENCRYPTION_STATUS (%d)",
               encr);
    return -1;
  }
  return 0;
...
static int ndis_get_encr_status(struct wpa_driver_ndis_data *drv) {
  u32 encr;
  int res;
  res = ndis_get_oid(drv, OID_802_11_ENCRYPTION_STATUS, (char *)&encr,
                     sizeof(encr));
  if (res != sizeof(encr)) {
    wpa_printf(MSG_DEBUG, "NDIS: Failed to get "
...
static int wpa_driver_ndis_get_bssid(void *priv, u8 *bssid) {
  ...
  return ndis_get_oid(drv, OID_802_11_BSSID, (char *)bssid, ETH_ALEN) < 0 ? -1
                                                                          : 0;
}
...
static int
wpa_driver_ndis_scan_native80211(struct wpa_driver_ndis_data *drv,
  ...
  res = ndis_set_oid(drv, OID_DOT11_SCAN_REQUEST, (char *)&req, sizeof(req));
  eloop_cancel_timeout(wpa_driver_ndis_scan_timeout, drv, drv->ctx);
  eloop_register_timeout(7, 0, wpa_driver_ndis_scan_timeout, drv, drv->ctx);
  return res;
}
...
wpa_driver_ndis_add_scan_ssid(struct wpa_scan_res *r, NDIS_802_11_SSID *ssid) {
...
static struct wpa_scan_results *wpa_driver_ndis_get_scan_results(void *priv) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (bss->IELength < sizeof(NDIS_802_11_FIXED_IEs)) {
      wpa_printf(MSG_DEBUG, "NDIS: too small IELength=%d", (int)bss->IELength);
      break;
    }
    if (((char *)bss->IEs) + bss->IELength > (char *)b + blen) {
       ...
       */
      wpa_printf(MSG_DEBUG,
                 "NDIS: skipped invalid scan "
                 "result IE (BSSID=" MACSTR ") IELength=%d",
                 MAC2STR(bss->MacAddress), (int)bss->IELength);
      break;
    }

    r = os_zalloc(sizeof(*r) + bss->IELength - sizeof(NDIS_802_11_FIXED_IEs));
    if (r == NULL)
      break;
...
static int wpa_driver_ndis_remove_key(struct wpa_driver_ndis_data *drv,
  ...
  res = ndis_set_oid(drv, OID_802_11_REMOVE_KEY, (char *)&rkey, sizeof(rkey));
  if (!pairwise) {
    index = key_idx;
    res2 =
        ndis_set_oid(drv, OID_802_11_REMOVE_WEP, (char *)&index, sizeof(index));
  } else
    res2 = 0;
...
static int wpa_driver_ndis_add_wep(struct wpa_driver_ndis_data *drv,
  ...
  wpa_hexdump_key(MSG_MSGDUMP, "NDIS: OID_802_11_ADD_WEP", (u8 *)wep, len);
...
static int wpa_driver_ndis_set_key(const char *ifname, void *priv,
  ...
  if (alg == WPA_ALG_NONE || key_len == 0) {
    return wpa_driver_ndis_remove_key(drv, key_idx, addr, bssid, pairwise);
  }

  if (alg == WPA_ALG_WEP) {
    return wpa_driver_ndis_add_wep(drv, pairwise, key_idx, set_tx, key,
                                   key_len);
  }
  ...
  wpa_hexdump_key(MSG_MSGDUMP, "NDIS: OID_802_11_ADD_KEY", (u8 *)nkey, len);
...
wpa_driver_ndis_associate(void *priv,
  ...
  if (ndis_set_oid(drv, OID_802_11_INFRASTRUCTURE_MODE, (char *)&mode,
                   sizeof(mode)) < 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to set "
  ...
  if (params->key_mgmt_suite == WPA_KEY_MGMT_NONE ||
    ...
    for (i = 0; i < 4; i++) {
      if (!params->wep_key[i])
        continue;
      wpa_printf(MSG_DEBUG,
                 "NDIS: Re-setting static WEP "
                 "key %d",
                 i);
      wpa_driver_ndis_set_key(drv->ifname, drv, WPA_ALG_WEP, bcast, i,
  ...
  if (params->wpa_ie == NULL || params->wpa_ie_len == 0) {
    ...
    if (params->wps == WPS_MODE_PRIVACY) {
      ...
      wpa_printf(MSG_DEBUG, "NDIS: Set dummy WEP key as a "
                            ...
                            "for WPS");
      wpa_driver_ndis_set_key(drv->ifname, drv, WPA_ALG_WEP, bcast, 0, 1, NULL,
                              0, dummy_key, sizeof(dummy_key));
    }
#endif /* CONFIG_WPS */
  } else {
    priv_mode = Ndis802_11PrivFilter8021xWEP;
  ...
  if (ndis_set_oid(drv, OID_802_11_PRIVACY_FILTER, (char *)&priv_mode,
                   sizeof(priv_mode)) < 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to set "
  ...
  if (params->bssid) {
    ndis_set_oid(drv, OID_802_11_BSSID, (char *)params->bssid, ETH_ALEN);
    drv->oid_bssid_set = 1;
  } else if (drv->oid_bssid_set) {
    ndis_set_oid(drv, OID_802_11_BSSID, "\xff\xff\xff\xff\xff\xff", ETH_ALEN);
    drv->oid_bssid_set = 0;
  }
...
static int wpa_driver_ndis_flush_pmkid(void *priv) {
  ...
  p.Length = 8;
  p.BSSIDInfoCount = 0;
  wpa_hexdump(MSG_MSGDUMP, "NDIS: OID_802_11_PMKID (flush)", (u8 *)&p, 8);
...
static int wpa_driver_ndis_get_associnfo(struct wpa_driver_ndis_data *drv) {
  ...
  len = ndis_get_oid(drv, OID_802_11_ASSOCIATION_INFORMATION, buf, sizeof(buf));
  if (len < 0) {
  ...
  if (len > sizeof(buf)) {
    ...
    wpa_printf(MSG_DEBUG,
               "NDIS: ignored bogus association "
               "information length %d",
               len);
    len =
        ndis_get_oid(drv, OID_802_11_ASSOCIATION_INFORMATION, buf, sizeof(buf));
    if (len < -1) {
    ...
    if (len > sizeof(buf)) {
      wpa_printf(MSG_DEBUG,
                 "NDIS: ignored bogus association"
                 " information length %d (re-read)",
                 len);
      len = sizeof(buf);
    }
  }
  wpa_hexdump(MSG_MSGDUMP, "NDIS: association information", (u8 *)buf, len);
  if (len < sizeof(*ai)) {
  ...
  wpa_printf(MSG_DEBUG,
             "NDIS: ReqFixed=0x%x RespFixed=0x%x off_req=%d "
  ...
  for (i = 0; i < b->NumberOfItems; i++) {
    ...
    if (os_memcmp(drv->bssid, bss->MacAddress, ETH_ALEN) == 0 &&
      ...
      data.assoc_info.beacon_ies =
          ((u8 *)bss->IEs) + sizeof(NDIS_802_11_FIXED_IEs);
      data.assoc_info.beacon_ies_len =
          bss->IELength - sizeof(NDIS_802_11_FIXED_IEs);
      wpa_hexdump(MSG_MSGDUMP, "NDIS: Beacon IEs", data.assoc_info.beacon_ies,
                  data.assoc_info.beacon_ies_len);
      break;
    }
    pos += bss->Length;
...
static void wpa_driver_ndis_event_auth(struct wpa_driver_ndis_data *drv,
  ...
  if (data_len < sizeof(*req)) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Too short Authentication Request "
               "Event (len=%d)",
               data_len);
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "NDIS: Authentication Request Event: "
...
static void wpa_driver_ndis_event_pmkid(struct wpa_driver_ndis_data *drv,
  ...
  if (data_len < 8) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Too short PMKID Candidate List "
               "Event (len=%d)",
               data_len);
    return;
  }
  ...
  wpa_printf(MSG_DEBUG,
             "NDIS: PMKID Candidate List Event - Version %d "
             "NumCandidates %d",
             (int)pmkid->Version, (int)pmkid->NumCandidates);

  if (pmkid->Version != 1) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Unsupported PMKID Candidate List "
               "Version %d",
               (int)pmkid->Version);
    return;
  }
  ...
  for (i = 0; i < pmkid->NumCandidates; i++) {
    ...
    wpa_printf(MSG_DEBUG, "NDIS: %d: " MACSTR " Flags 0x%x", i,
               MAC2STR(p->BSSID), (int)p->Flags);
    ...
    wpa_supplicant_event(drv->ctx, EVENT_PMKID_CANDIDATE, &event);
  }
}
...
void wpa_driver_ndis_event_media_specific(struct wpa_driver_ndis_data *drv,
  ...
  wpa_hexdump(MSG_DEBUG, "NDIS: Media Specific Indication", data, data_len);
...
wpa_driver_ndis_get_wpa_capability(struct wpa_driver_ndis_data *drv) {
  ...
  if (drv->capa.key_mgmt & WPA_DRIVER_CAPA_KEY_MGMT_WPA &&
      drv->capa.enc & (WPA_DRIVER_CAPA_ENC_TKIP | WPA_DRIVER_CAPA_ENC_CCMP)) {
  ...
  wpa_printf(MSG_DEBUG,
             "NDIS: driver capabilities: key_mgmt 0x%x "
             "enc 0x%x auth 0x%x",
...
static void wpa_driver_ndis_get_capability(struct wpa_driver_ndis_data *drv) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             (int)c->NoOfPMKIDs, (int)c->NoOfAuthEncryptPairsSupported);
  drv->has_capability = 1;
  ...
  for (i = 0; i < c->NoOfAuthEncryptPairsSupported; i++) {
    ...
    wpa_printf(MSG_MSGDUMP, "NDIS: %d - auth %d encr %d", i,
               (int)ae->AuthModeSupported, (int)ae->EncryptStatusSupported);
    ...
    case Ndis802_11AuthModeWPA2PSK:
      drv->capa.key_mgmt |= WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK;
      break;
    case Ndis802_11AuthModeWPANone:
      drv->capa.key_mgmt |= WPA_DRIVER_CAPA_KEY_MGMT_WPA_NONE;
      break;
    default:
      break;
    }
...
static void ndisuio_notification_receive(void *eloop_data, void *user_ctx) {
  ...
  if (!ReadMsgQueue(drv->event_queue, buf, NDISUIO_MSG_SIZE, &len, 0, &flags)) {
    wpa_printf(MSG_DEBUG,
               "ndisuio_notification_receive: "
               "ReadMsgQueue failed: %d",
               (int)GetLastError());
    return;
  }
  ...
  case NDISUIO_NOTIFICATION_MEDIA_SPECIFIC_NOTIFICATION:
    ...
    wpa_driver_ndis_event_media_specific(drv, hdr->pvStatusBuffer,
                                         hdr->uiStatusBufferSize);
#else
...
static void ndisuio_notification_deinit(struct wpa_driver_ndis_data *drv) {
  ...
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_REQUEST_NOTIFICATION, &req,
                       sizeof(req), NULL, 0, NULL, NULL)) {
    wpa_printf(MSG_INFO,
  ...
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_CANCEL_NOTIFICATION, NULL, 0,
                       NULL, 0, NULL, NULL)) {
    wpa_printf(MSG_INFO,
               "ndisuio_notification_deinit: "
  ...
  if (drv->event_queue) {
    eloop_unregister_event(drv->event_queue, sizeof(drv->event_queue));
...
static int wpa_driver_ndis_get_names(struct wpa_driver_ndis_data *drv) {
  ...
  for (i = 0;; i++) {
    os_memset(b, 0, blen);
    b->BindingIndex = i;
    if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b,
                         sizeof(NDISUIO_QUERY_BINDING), b, blen, &written,
                         NULL)) {
  ...
  if (!found) {
    wpa_printf(MSG_DEBUG, "NDIS: Could not find interface '%s'", drv->ifname);
    os_free(b);
    return -1;
  }
...
#define MAX_ADAPTERS 32
  ...
  wpa_printf(MSG_DEBUG, "NDIS: Packet.dll version: %s", PacketGetVersion());
  ...
  while (pos < names + len) {
    ...
    if (*pos == '\0') {
      wpa_printf(MSG_DEBUG, "NDIS: %d adapter names found", num_name);
      pos++;
      break;
    }
  }
  ...
  if (num_name != num_desc) {
    wpa_printf(MSG_DEBUG,
               "NDIS: mismatch in adapter name and "
  ...
  for (i = 0; i < num_name; i++) {
    wpa_printf(MSG_DEBUG, "NDIS: %d - %s - %s", i, name[i], desc[i]);
    if (found_name == -1 && os_strstr(name[i], drv->ifname))
      found_name = i;
    if (found_desc == -1 &&
        os_strncmp(desc[i], drv->ifname, os_strlen(drv->ifname)) == 0)
      found_desc = i;
  }

  if (found_name < 0 && found_desc >= 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Matched interface '%s' based on "
               "description '%s'",
               name[found_desc], desc[found_desc]);
    found_name = found_desc;
    os_strlcpy(drv->ifname,
               os_strncmp(name[found_desc], "\\Device\\NPF_", 12) == 0
                   ? name[found_desc] + 12
                   : name[found_desc],
               sizeof(drv->ifname));
  }
...
static int wpa_driver_ndis_rebind_adapter(struct wpa_driver_ndis_data *drv) {
  ...
  ndis = CreateFile(DD_NDIS_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                    OPEN_EXISTING, 0, NULL);
  if (ndis == INVALID_HANDLE_VALUE) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to open file to NDIS "
               "device: %d",
               (int)GetLastError());
    return -1;
  }
  ...
  if (!DeviceIoControl(ndis, IOCTL_NDIS_REBIND_ADAPTER, multi,
                       len * sizeof(TCHAR), NULL, 0, NULL, NULL)) {
    wpa_printf(MSG_DEBUG,
               "NDIS: IOCTL_NDIS_REBIND_ADAPTER "
               "failed: 0x%x",
               (int)GetLastError());
    wpa_hexdump_ascii(MSG_DEBUG, "NDIS: rebind multi_sz", (u8 *)multi,
                      len * sizeof(TCHAR));
    CloseHandle(ndis);
    return -1;
  }
...
#define WZC_DRIVER TEXT("Drivers\\BuiltIn\\ZeroConfig")
  ...
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_DEBUG,
               "NDIS: RegOpenKeyEx(DEVLOAD_ACTIVE_KEY) "
               "failed: %d %d",
  ...
  for (i = 0;; i++) {
    len = sizeof(keyname);
    ret = RegEnumKeyEx(hk, i, keyname, &len, NULL, NULL, NULL, NULL);
    if (ret != ERROR_SUCCESS) {
  ...
  len = sizeof(hnd);
  ret = RegQueryValueEx(hk2, DEVLOAD_HANDLE_VALNAME, NULL, NULL, (PUCHAR)&hnd,
                        &len);
  if (ret != ERROR_SUCCESS) {
    wpa_printf(MSG_DEBUG,
               "NDIS: RegQueryValueEx(HANDLE_VALNAME) "
               "failed: %d %d",
               (int)ret, (int)GetLastError());
    RegCloseKey(hk2);
    return -1;
  ...
  HMODULE hm;
  DWORD(WINAPI * wzc_enum_interf)(LPWSTR pSrvAddr, PINTFS_KEY_TABLE pIntfs);
  DWORD(WINAPI * wzc_query_interf)
  (LPWSTR pSrvAddr, DWORD dwInFlags, PINTF_ENTRY pIntf, LPDWORD pdwOutFlags);
  DWORD(WINAPI * wzc_set_interf)
  (LPWSTR pSrvAddr, DWORD dwInFlags, PINTF_ENTRY pIntf, LPDWORD pdwOutFlags);
  int ret = -1, j;
  DWORD res;
  ...
  if (hm == NULL) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Failed to load wzcsapi.dll (%u) "
  ...
  if (res != 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: WZCEnumInterfaces failed: %d; "
  ...
  for (i = 0; i < guids.dwNumIntfs; i++) {
    ...
    wpa_printf(MSG_DEBUG, "NDIS: intfs %d GUID '%s'", (int)i, guid);
  ...
  if (res != 0) {
    wpa_printf(MSG_DEBUG,
               "NDIS: Could not query flags for the "
  ...
  if (intf.dwCtlFlags == -1) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "NDIS: intf", (u8 *)&intf, sizeof(intf));
    goto fail;
  }
  ...
  if (enable) {
    if (!(intf.dwCtlFlags & INTFCTL_ENABLED)) {
      ...
      res = wzc_set_interf(NULL, INTFCTL_ENABLED, &intf, &flags);
      if (res != 0) {
        wpa_printf(MSG_DEBUG,
                   "NDIS: Failed to enable "
                   "WZC: %d (0x%x)",
                   (int)res, (int)res);
        wpa_printf(MSG_DEBUG, "NDIS: GetLastError: %u",
                   (unsigned int)GetLastError());
...
#define NDISUIO_DEVICE_NAME TEXT("\\\\.\\\\Ndisuio")
  DWORD written;
#endif /* _WIN32_WCE */
  drv->ndisuio = CreateFile(
      NDISUIO_DEVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
      FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, INVALID_HANDLE_VALUE);
  if (drv->ndisuio == INVALID_HANDLE_VALUE) {
    wpa_printf(MSG_ERROR,
               "NDIS: Failed to open connection to "
               "NDISUIO: %d",
               (int)GetLastError());
    return -1;
  }
...
#ifndef _WIN32_WCE
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_BIND_WAIT, NULL, 0, NULL, 0,
                       &written, NULL)) {
    wpa_printf(MSG_ERROR,
               "NDIS: IOCTL_NDISUIO_BIND_WAIT failed: "
               "%d",
               (int)GetLastError());
...
#define MAX_NDIS_DEVICE_NAME_LEN 256
  ...
  if (!DeviceIoControl(drv->ndisuio, IOCTL_NDISUIO_OPEN_DEVICE, ifname,
                       len * sizeof(WCHAR), NULL, 0, &written, NULL)) {
    wpa_printf(MSG_ERROR,
               "NDIS: IOCTL_NDISUIO_OPEN_DEVICE "
               "failed: %d",
               (int)GetLastError());
    wpa_hexdump_ascii(MSG_DEBUG, "NDIS: ifname", (const u8 *)ifname,
                      len * sizeof(WCHAR));
    CloseHandle(drv->ndisuio);
    drv->ndisuio = INVALID_HANDLE_VALUE;
    return -1;
  ...
  if (drv->adapter == NULL) {
    wpa_printf(MSG_DEBUG,
               "NDIS: PacketOpenAdapter failed for "
               "'%s'",
               ifname);
    return -1;
  }
...
static int ndis_add_multicast(struct wpa_driver_ndis_data *drv) {
  if (ndis_set_oid(drv, OID_802_3_MULTICAST_LIST, (const char *)pae_group_addr,
                   ETH_ALEN) < 0) {
...
static void *wpa_driver_ndis_init(void *ctx, const char *ifname) {
  ...
  if (ndis_get_oid(drv, OID_802_3_CURRENT_ADDRESS, (char *)drv->own_addr,
                   ETH_ALEN) < 0) {
  ...
  if (ndis_set_oid(drv, OID_802_11_INFRASTRUCTURE_MODE, (char *)&mode,
    ...
    res = ndis_get_oid(drv, OID_DOT11_CURRENT_OPERATION_MODE, buf, sizeof(buf));
    if (res > 0) {
...
#ifdef CONFIG_NDIS_EVENTS_INTEGRATED
  if (drv->events) {
    eloop_unregister_event(drv->event_avail, sizeof(drv->event_avail));
...
wpa_driver_ndis_get_interfaces(void *global_priv) {
...
#ifndef _WIN32_WCE
  if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_BIND_WAIT, NULL, 0, NULL, 0,
                       &written, NULL)) {
    wpa_printf(MSG_ERROR,
               "NDIS: IOCTL_NDISUIO_BIND_WAIT failed: "
               "%d",
  ...
  for (i = 0;; i++) {
    os_memset(b, 0, blen);
    b->BindingIndex = i;
    if (!DeviceIoControl(ndisuio, IOCTL_NDISUIO_QUERY_BINDING, b,
                         sizeof(NDISUIO_QUERY_BINDING), b, blen, &written,
                         NULL)) {
      error = (int)GetLastError();
      if (error == ERROR_NO_MORE_ITEMS)
...
void driver_ndis_init_ops(void) {
  ...
  wpa_driver_ndis_ops.get_scan_results2 = wpa_driver_ndis_get_scan_results;
