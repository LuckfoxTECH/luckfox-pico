...
static int wpa_priv_reg_cmd(struct wpa_driver_privsep_data *drv, int cmd) {
  ...
  res = sendto(drv->priv_socket, &cmd, sizeof(cmd), 0,
               (struct sockaddr *)&drv->priv_addr, sizeof(drv->priv_addr));
  if (res < 0)
...
static int wpa_priv_cmd(struct wpa_driver_privsep_data *drv, int cmd,
                        const void *data, size_t data_len, void *reply,
                        size_t *reply_len) {
  struct msghdr msg;
  struct iovec io[2];
  ...
  if (sendmsg(drv->cmd_socket, &msg, 0) < 0) {
    wpa_printf(MSG_ERROR, "sendmsg(cmd_socket): %s", strerror(errno));
    return -1;
  }
...
static int wpa_driver_privsep_scan(void *priv,
  ...
  return wpa_priv_cmd(drv, PRIVSEP_CMD_SCAN, ssid, ssid_len, NULL, NULL);
}
...
wpa_driver_privsep_get_scan_results2(void *priv) {
  ...
  if (buf == NULL)
    return NULL;
  res =
      wpa_priv_cmd(drv, PRIVSEP_CMD_GET_SCAN_RESULTS, NULL, 0, buf, &reply_len);
  if (res < 0) {
    os_free(buf);
    return NULL;
  }
...
static int wpa_driver_privsep_set_key(const char *ifname, void *priv,
  ...
  wpa_printf(MSG_DEBUG, "%s: priv=%p alg=%d key_idx=%d set_tx=%d", __func__,
             priv, alg, key_idx, set_tx);
  ...
  return wpa_priv_cmd(drv, PRIVSEP_CMD_SET_KEY, &cmd, sizeof(cmd), NULL, NULL);
}

static int
wpa_driver_privsep_authenticate(void *priv,
  ...
  for (i = 0; i < 4; i++) {
    if (params->wep_key[i])
      os_memcpy(data->wep_key[i], params->wep_key[i], params->wep_key_len[i]);
  ...
  res = wpa_priv_cmd(drv, PRIVSEP_CMD_AUTHENTICATE, data, buflen, NULL, NULL);
  os_free(data);
...
static int
wpa_driver_privsep_associate(void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             params->group_suite, params->key_mgmt_suite, params->auth_alg,
             params->mode);
  ...
  res = wpa_priv_cmd(drv, PRIVSEP_CMD_ASSOCIATE, data, buflen, NULL, NULL);
  os_free(data);
...
static int wpa_driver_privsep_get_ssid(void *priv, u8 *ssid) {
  ...
  if (ssid_len < 0 || ssid_len > SSID_MAX_LEN || sizeof(int) + ssid_len > len) {
...
static void wpa_driver_privsep_event_assoc(void *ctx, enum wpa_event_type event,
                                           u8 *buf, size_t len) {
  union wpa_event_data data;
  int inc_data = 0;
  u8 *pos, *end;
...
static void wpa_driver_privsep_event_interface_status(void *ctx, u8 *buf,
  ...
  os_memcpy(data.interface_status.ifname, buf + sizeof(int), len - sizeof(int));
  wpa_supplicant_event(ctx, EVENT_INTERFACE_STATUS, &data);
}

static void wpa_driver_privsep_event_michael_mic_failure(void *ctx, u8 *buf,
                                                         size_t len) {
  union wpa_event_data data;
...
static void wpa_driver_privsep_receive(int sock, void *eloop_ctx,
  ...
  if (buf == NULL)
    return;
  res = recvfrom(sock, buf, buflen, 0, (struct sockaddr *)&from, &fromlen);
  if (res < 0) {
    wpa_printf(MSG_ERROR, "recvfrom(priv_socket): %s", strerror(errno));
    os_free(buf);
    return;
  }
  ...
  wpa_printf(MSG_DEBUG, "privsep: Event %d received (len=%lu)", event,
  ...
  case PRIVSEP_EVENT_ASSOC:
    wpa_driver_privsep_event_assoc(drv->ctx, EVENT_ASSOC, event_buf, event_len);
    break;
  case PRIVSEP_EVENT_DISASSOC:
    wpa_supplicant_event(drv->ctx, EVENT_DISASSOC, NULL);
    break;
  case PRIVSEP_EVENT_ASSOCINFO:
    wpa_driver_privsep_event_assoc(drv->ctx, EVENT_ASSOCINFO, event_buf,
                                   event_len);
    break;
  case PRIVSEP_EVENT_MICHAEL_MIC_FAILURE:
    wpa_driver_privsep_event_michael_mic_failure(drv->ctx, event_buf,
                                                 event_len);
    break;
  case PRIVSEP_EVENT_INTERFACE_STATUS:
    wpa_driver_privsep_event_interface_status(drv->ctx, event_buf, event_len);
    break;
  case PRIVSEP_EVENT_PMKID_CANDIDATE:
    wpa_driver_privsep_event_pmkid_candidate(drv->ctx, event_buf, event_len);
    break;
  case PRIVSEP_EVENT_STKSTART:
    wpa_driver_privsep_event_stkstart(drv->ctx, event_buf, event_len);
    break;
  case PRIVSEP_EVENT_FT_RESPONSE:
    wpa_driver_privsep_event_ft_response(drv->ctx, event_buf, event_len);
    break;
  case PRIVSEP_EVENT_RX_EAPOL:
    wpa_driver_privsep_event_rx_eapol(drv->ctx, event_buf, event_len);
    break;
...
static int wpa_driver_privsep_set_param(void *priv, const char *param) {
  ...
  os_snprintf(drv->own_socket_path, len, "%s/wpa_privsep-%d-%d", own_dir,
              getpid(), counter++);
  ...
  os_snprintf(drv->own_cmd_path, len, "%s/wpa_privsep-%d-%d", own_dir, getpid(),
              counter++);

  os_free(own_dir);

  drv->priv_addr.sun_family = AF_UNIX;
  os_snprintf(drv->priv_addr.sun_path, sizeof(drv->priv_addr.sun_path), "%s/%s",
              priv_dir, drv->ifname);
  os_free(priv_dir);
  ...
  if (bind(drv->priv_socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    wpa_printf(MSG_ERROR, "privsep-set-params priv-sock: bind(PF_UNIX): %s",
               strerror(errno));
    close(drv->priv_socket);
    drv->priv_socket = -1;
    unlink(drv->own_socket_path);
  ...
  eloop_register_read_sock(drv->priv_socket, wpa_driver_privsep_receive, drv,
                           NULL);
  ...
  if (bind(drv->cmd_socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    wpa_printf(MSG_ERROR, "privsep-set-params cmd-sock: bind(PF_UNIX): %s",
               strerror(errno));
    close(drv->cmd_socket);
    drv->cmd_socket = -1;
    unlink(drv->own_cmd_path);
...
static int wpa_driver_privsep_set_country(void *priv, const char *alpha2) {
  ...
  return wpa_priv_cmd(drv, PRIVSEP_CMD_SET_COUNTRY, alpha2, os_strlen(alpha2),
                      NULL, NULL);
}
...
const struct wpa_driver_ops *const wpa_drivers[] = {&wpa_driver_privsep_ops,
                                                    NULL};
