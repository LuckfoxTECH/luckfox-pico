...
static int macsec_qca_multicast_membership(int sock, int ifindex,
  ...
  if (setsockopt(sock, SOL_PACKET,
                 add ? PACKET_ADD_MEMBERSHIP : PACKET_DROP_MEMBERSHIP, &mreq,
                 sizeof(mreq)) < 0) {
...
static int macsec_qca_get_ifstatus(const char *ifname, int *status) {
  ...
  if (ioctl(s, SIOCGIFMEDIA, (caddr_t)&ifmr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGIFMEDIA]: %s", strerror(errno));
    close(s);
    return -1;
  }
  close(s);
...
static int macsec_qca_multi(const char *ifname, const u8 *addr, int add) {
  ...
  if (ioctl(s, add ? SIOCADDMULTI : SIOCDELMULTI, (caddr_t)&ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOC{ADD/DEL}MULTI]: %s", strerror(errno));
    close(s);
    return -1;
  }
  close(s);
...
static void __macsec_drv_init(struct macsec_qca_data *drv) {
  ...
  ret = nss_macsec_secy_sc_sa_mapping_mode_set(drv->secy_id, FAL_SC_SA_MAP_1_4);
  if (ret)
    wpa_printf(MSG_ERROR, "nss_macsec_secy_sc_sa_mapping_mode_set: FAIL");
...
static void *macsec_qca_init(void *ctx, const char *ifname) {
  ...
  if (macsec_qca_get_ifflags(ifname, &flags) == 0 && !(flags & IFF_UP) &&
      macsec_qca_set_ifflags(ifname, flags | IFF_UP) == 0) {
    drv->iff_up = 1;
  }

  if (macsec_qca_multicast_membership(drv->pf_sock, if_nametoindex(drv->ifname),
                                      pae_group_addr, 1) == 0) {
    wpa_printf(MSG_DEBUG, "%s: Added multicast membership with packet socket",
               __func__);
    drv->membership = 1;
  } else if (macsec_qca_multi(ifname, pae_group_addr, 1) == 0) {
    wpa_printf(MSG_DEBUG, "%s: Added multicast membership with SIOCADDMULTI",
               __func__);
    drv->multi = 1;
  } else if (macsec_qca_get_ifflags(ifname, &flags) < 0) {
    wpa_printf(MSG_INFO, "%s: Could not get interface flags", __func__);
    os_free(drv);
    return NULL;
  } else if (flags & IFF_ALLMULTI) {
    wpa_printf(MSG_DEBUG, "%s: Interface is already configured for multicast",
               __func__);
  ...
  {
    int status;
    wpa_printf(MSG_DEBUG, "%s: waiting for link to become active", __func__);
    while (macsec_qca_get_ifstatus(ifname, &status) == 0 && status == 0)
      sleep(1);
  }
#endif /* defined(__FreeBSD__) || defined(__DragonFly__) ||                    \
          defined(FreeBSD_kernel__) */
...
static void macsec_qca_deinit(void *priv) {
  ...
  if (drv->membership &&
      macsec_qca_multicast_membership(drv->pf_sock, if_nametoindex(drv->ifname),
                                      pae_group_addr, 0) < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to remove PAE multicast group (PACKET)",
               __func__);
  }

  if (drv->multi && macsec_qca_multi(drv->ifname, pae_group_addr, 0) < 0) {
  ...
  if (drv->iff_allmulti &&
    ...
    wpa_printf(MSG_DEBUG, "%s: Failed to disable allmulti mode", __func__);
  }

  if (drv->iff_up && macsec_qca_get_ifflags(drv->ifname, &flags) == 0 &&
      (flags & IFF_UP) &&
      macsec_qca_set_ifflags(drv->ifname, flags & ~IFF_UP) < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to set the interface down", __func__);
  }
...
static int macsec_qca_macsec_init(void *priv,
  ...
  wpa_printf(MSG_DEBUG, "%s: es=%d, scb=%d, sci=%d", __func__, drv->use_es,
             drv->use_scb, drv->always_include_sci);
...
static int macsec_qca_set_replay_protect(void *priv, Boolean enabled,
  ...
  wpa_printf(MSG_DEBUG, "%s: enabled=%d, win=%u", __func__, enabled, window);
...
static int macsec_qca_set_current_cipher_suite(void *priv, u64 cs) {
  if (cs != CS_ID_GCM_AES_128) {
    wpa_printf(MSG_ERROR, "%s: NOT supported CipherSuite: %016" PRIx64,
               __func__, cs);
    return -1;
  }
...
static int macsec_qca_get_receive_lowest_pn(void *priv, u32 channel, u8 an,
  ...
  ret += nss_macsec_secy_rx_sa_next_pn_get(drv->secy_id, channel, an, &next_pn);
  ret +=
      nss_macsec_secy_rx_sc_replay_protect_get(drv->secy_id, channel, &enabled);
  ret +=
      nss_macsec_secy_rx_sc_anti_replay_window_get(drv->secy_id, channel, &win);
...
static int macsec_qca_get_transmit_next_pn(void *priv, u32 channel, u8 an,
  ...
  ret += nss_macsec_secy_tx_sa_next_pn_get(drv->secy_id, channel, an, next_pn);
...
int macsec_qca_set_transmit_next_pn(void *priv, u32 channel, u8 an,
                                    u32 next_pn) {
  struct macsec_qca_data *drv = priv;
  int ret = 0;

  ret += nss_macsec_secy_tx_sa_next_pn_set(drv->secy_id, channel, an, next_pn);
...
static int macsec_qca_get_available_receive_sc(void *priv, u32 *channel) {
  ...
  for (sc_ch = 0; sc_ch < MAXSC; sc_ch++) {
    ret = nss_macsec_secy_rx_sc_in_used_get(drv->secy_id, sc_ch, &in_use);
    if (ret)
      continue;
...
static int macsec_qca_create_receive_sc(void *priv, u32 channel,
  ...
  ret += nss_macsec_secy_rx_sc_validate_frame_set(drv->secy_id, channel, vf);
  ret += nss_macsec_secy_rx_sc_replay_protect_set(drv->secy_id, channel,
                                                  drv->replay_protect);
  ret += nss_macsec_secy_rx_sc_anti_replay_window_set(drv->secy_id, channel,
                                                      drv->replay_window);
...
static int macsec_qca_create_receive_sa(void *priv, u32 channel, u8 an,
  ...
  wpa_printf(MSG_DEBUG, "%s, channel=%d, an=%d, lpn=0x%x", __func__, channel,
             an, lowest_pn);
...
static int macsec_qca_get_available_transmit_sc(void *priv, u32 *channel) {
  ...
  for (sc_ch = 0; sc_ch < MAXSC; sc_ch++) {
    ret = nss_macsec_secy_tx_sc_in_used_get(drv->secy_id, sc_ch, &in_use);
    if (ret)
      continue;
...
static int macsec_qca_create_transmit_sc(void *priv, u32 channel,
  ...
  ret += nss_macsec_secy_tx_sc_protect_set(drv->secy_id, channel,
                                           drv->protect_frames);
  ret += nss_macsec_secy_tx_sc_confidentiality_offset_set(drv->secy_id, channel,
                                                          conf_offset);
...
static int macsec_qca_create_transmit_sa(void *priv, u32 channel, u8 an,
  ...
  ret += nss_macsec_secy_tx_sc_tci_7_2_set(drv->secy_id, channel, (tci >> 2));
