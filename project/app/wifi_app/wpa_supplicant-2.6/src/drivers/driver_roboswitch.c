...
static u16
wpa_driver_roboswitch_mdio_read(struct wpa_driver_roboswitch_data *drv,
                                u8 reg) {
  ...
  if (ioctl(drv->fd, SIOCGMIIREG, &drv->ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGMIIREG]: %s", strerror(errno));
    return 0x00;
  }
  return mii->val_out;
}
...
static void
wpa_driver_roboswitch_mdio_write(struct wpa_driver_roboswitch_data *drv, u8 reg,
                                 u16 val) {
  struct mii_ioctl_data *mii = if_mii(&drv->ifr);
  ...
  if (ioctl(drv->fd, SIOCSMIIREG, &drv->ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSMIIREG]: %s", strerror(errno));
  }
}
...
static int wpa_driver_roboswitch_reg(struct wpa_driver_roboswitch_data *drv,
  ...
  for (i = 0; i < ROBO_MII_RETRY_MAX; ++i) {
    if ((wpa_driver_roboswitch_mdio_read(drv, ROBO_MII_ADDR) & 3) == 0)
      return 0;
  }
  /* timeout */
  return -1;
...
static int wpa_driver_roboswitch_read(struct wpa_driver_roboswitch_data *drv,
  ...
  for (i = 0; i < len; ++i) {
    val[i] = wpa_driver_roboswitch_mdio_read(drv, ROBO_MII_DATA_OFFSET + i);
  }
...
static int wpa_driver_roboswitch_write(struct wpa_driver_roboswitch_data *drv,
  ...
  if (len > ROBO_MII_DATA_MAX)
    return -1;
  for (i = 0; i < len; ++i) {
    wpa_driver_roboswitch_mdio_write(drv, ROBO_MII_DATA_OFFSET + i, val[i]);
  }
  ...
  if (param == NULL || os_strstr(param, "multicast_only=1") == NULL) {
    ...
    *sep = '.';
    drv->l2 = l2_packet_init(drv->ifname, NULL, ETH_P_ALL,
                             wpa_driver_roboswitch_receive, drv, 1);
    if (drv->l2 == NULL) {
      wpa_printf(MSG_INFO, "%s: Unable to listen on %s", __func__, drv->ifname);
      return -1;
    }
    *sep = '\0';
...
static int wpa_driver_roboswitch_join(struct wpa_driver_roboswitch_data *drv,
  ...
  if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF,
                                 read1, 1) < 0)
    return -1;
  if (!(read1[0] & (1 << 4))) {
    /* multiport addresses are not yet enabled */
    read1[0] |= 1 << 4;
    wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1,
                                addr_be16, 3);
    wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1,
                                &ports, 1);
    wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2,
                                addr_be16, 3);
    wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_2,
                                &ports, 1);
    wpa_driver_roboswitch_write(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF,
                                read1, 1);
  } else {
    /* if both multiport addresses are the same we can add */
    if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1,
                                   read1, 3) ||
        wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_2,
                                   read2, 3) ||
        os_memcmp(read1, read2, 6) != 0)
      return -1;
    if (wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_VEC_1,
                                   read1, 1) ||
...
static int wpa_driver_roboswitch_leave(struct wpa_driver_roboswitch_data *drv,
  ...
  wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_CONF, &_read,
                             1);
  /* If ARL control is disabled, there is nothing to leave. */
  if (!(_read & (1 << 4)))
    return -1;

  wpa_driver_roboswitch_read(drv, ROBO_ARLCTRL_PAGE, ROBO_ARLCTRL_ADDR_1,
                             addr_read, 3);
...
static void *wpa_driver_roboswitch_init(void *ctx, const char *ifname) {
  ...
  sep = drv->ifname + os_strlcpy(drv->ifname, ifname, sizeof(drv->ifname)) - 2;
  /* find the '.' separating <interface> and <vlan> */
  while (sep > drv->ifname && *sep != '.')
    sep--;
  if (sep <= drv->ifname) {
    wpa_printf(MSG_INFO,
  ...
  *sep = '\0';
  ...
  if (ioctl(drv->fd, SIOCGMIIPHY, &drv->ifr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGMIIPHY]: %s", strerror(errno));
    os_free(drv);
    return NULL;
  }
  ...
  wpa_driver_roboswitch_write(drv, ROBO_VLAN_PAGE, ROBO_VLAN_ACCESS_5350, _read,
                              1);
  ...
  wpa_driver_roboswitch_write(
      drv, ROBO_VLAN_PAGE,
      drv->is_5350 ? ROBO_VLAN_ACCESS_5350 : ROBO_VLAN_ACCESS, &vlan, 1);
...
static void wpa_driver_roboswitch_deinit(void *priv) {
  ...
  if (wpa_driver_roboswitch_leave(drv, drv->ports, pae_group_addr) < 0) {
    wpa_printf(MSG_DEBUG, "%s: Unable to leave PAE group", __func__);
  }
