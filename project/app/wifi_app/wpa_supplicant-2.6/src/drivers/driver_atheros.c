...
#include "priv_netlink.h"
    ...
    defined(CONFIG_HS20) || defined(CONFIG_WNM) || defined(CONFIG_WPS)
...
static int set80211priv(struct atheros_driver_data *drv, int op, void *data,
                        int len) {
  struct iwreq iwr;
  ...
  if (ioctl(drv->ioctl_sock, op, &iwr) < 0) {
    wpa_printf(MSG_DEBUG,
               ...
               "(%s) len=%d failed: %d (%s)",
               __func__, drv->iface, op, athr_get_ioctl_name(op), len, errno,
               strerror(errno));
    return -1;
  }
  return 0;
...
static int set80211param(struct atheros_driver_data *drv, int op, int arg) {
  struct iwreq iwr;
  ...
  if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_SETPARAM, &iwr) < 0) {
    wpa_printf(MSG_INFO,
               "%s: %s: Failed to set parameter (op %d (%s) arg %d): "
               "ioctl[IEEE80211_IOCTL_SETPARAM]: %s",
               __func__, drv->iface, op, athr_get_param_name(op), arg,
               strerror(errno));
    return -1;
  }
  return 0;
 ...
 */
static int atheros_configure_wpa(struct atheros_driver_data *drv,
  ...
  case WPA_CIPHER_NONE:
    ...
    break;
  default:
    wpa_printf(MSG_ERROR, "Unknown group key cipher %u", params->wpa_group);
    return -1;
  }
  ...
  if (v == IEEE80211_CIPHER_WEP) {
    ...
    if (set80211param(drv, IEEE80211_PARAM_MCASTKEYLEN, v)) {
      wpa_printf(MSG_INFO, "Unable to set group key length to %u", v);
      return -1;
    }
  }
  ...
  if (set80211param(drv, IEEE80211_PARAM_UCASTCIPHERS, v)) {
    wpa_printf(MSG_INFO, "Unable to set pairwise key ciphers to 0x%x", v);
    return -1;
  }

  wpa_printf(MSG_DEBUG, "%s: key management algorithms=0x%x", __func__,
             params->wpa_key_mgmt);
  if (set80211param(drv, IEEE80211_PARAM_KEYMGTALGS, params->wpa_key_mgmt)) {
    wpa_printf(MSG_INFO, "Unable to set key management algorithms to 0x%x",
               params->wpa_key_mgmt);
    return -1;
  }
  ...
  if (set80211param(drv, IEEE80211_PARAM_RSNCAPS, v)) {
    wpa_printf(MSG_INFO, "Unable to set RSN capabilities to 0x%x", v);
    return -1;
  }
...
static int atheros_set_ieee8021x(void *priv, struct wpa_bss_params *params) {
  ...
  if (!params->enabled) {
    /* XXX restore state */
    if (set80211param(priv, IEEE80211_PARAM_AUTHMODE, IEEE80211_AUTH_AUTO) < 0)
      return -1;
...
static int atheros_set_sta_authorized(void *priv, const u8 *addr,
                                      int authorized) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_mlme mlme;
  int ret;

  wpa_printf(MSG_DEBUG, "%s: addr=%s authorized=%d", __func__,
             ether_sprintf(addr), authorized);
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to %sauthorize STA " MACSTR, __func__,
               authorized ? "" : "un", MAC2STR(addr));
  }

  return ret;
}

static int atheros_sta_set_flags(void *priv, const u8 *addr,
...
static int atheros_del_key(void *priv, const u8 *addr, int key_idx) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_del_key wk;
  int ret;

  wpa_printf(MSG_DEBUG, "%s: addr=%s key_idx=%d", __func__, ether_sprintf(addr),
             key_idx);
...
static int atheros_set_key(const char *ifname, void *priv, enum wpa_alg alg,
  ...
  wpa_printf(MSG_DEBUG, "%s: alg=%d addr=%s key_idx=%d", __func__, alg,
             ether_sprintf(addr), key_idx);
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG,
               ...
               __func__, ether_sprintf(wk.ik_macaddr), key_idx, alg,
               (unsigned long)key_len, set_tx);
  }

  return ret;
}

static int atheros_get_seqnum(const char *ifname, void *priv, const u8 *addr,
                              int idx, u8 *seq) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_key wk;

  wpa_printf(MSG_DEBUG, "%s: addr=%s idx=%d", __func__, ether_sprintf(addr),
             idx);
...
static int atheros_flush(void *priv) {
  u8 allsta[IEEE80211_ADDR_LEN];
  os_memset(allsta, 0xff, IEEE80211_ADDR_LEN);
  return atheros_sta_deauth(priv, NULL, allsta, IEEE80211_REASON_AUTH_LEAVE);
}

static int atheros_read_sta_driver_data(void *priv,
  ...
  if (set80211priv(drv, IEEE80211_IOCTL_STA_STATS, &stats, sizeof(stats))) {
    wpa_printf(MSG_DEBUG, "%s: Failed to fetch STA stats (addr " MACSTR ")",
    ...
    wpa_printf(MSG_INFO, "Failed to get station stats information element");
    return -1;
  }
...
static int atheros_sta_clear_stats(void *priv, const u8 *addr) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_mlme mlme;
  int ret;
  ...
  ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
  if (ret < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to clear STA stats (addr " MACSTR ")",
               __func__, MAC2STR(addr));
  }
...
static int atheros_set_opt_ie(void *priv, const u8 *ie, size_t ie_len) {
  struct atheros_driver_data *drv = priv;
  u8 buf[512];
  struct ieee80211req_getset_appiebuf *app_ie;

  wpa_printf(MSG_DEBUG, "%s buflen = %lu", __func__, (unsigned long)ie_len);
  ...
  if (drv->wps_beacon_ie != NULL) {
    os_memcpy(&(app_ie->app_buf[ie_len]), wpabuf_head(drv->wps_beacon_ie),
              wpabuf_len(drv->wps_beacon_ie));
    app_ie->app_buflen = ie_len + wpabuf_len(drv->wps_beacon_ie);
  }
  wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF(Beacon)", app_ie->app_buf,
              app_ie->app_buflen);
  ...
  if (drv->wps_probe_resp_ie != NULL) {
    os_memcpy(&(app_ie->app_buf[ie_len]), wpabuf_head(drv->wps_probe_resp_ie),
              wpabuf_len(drv->wps_probe_resp_ie));
    app_ie->app_buflen = ie_len + wpabuf_len(drv->wps_probe_resp_ie);
  } else
    app_ie->app_buflen = ie_len;
  wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF(ProbeResp)", app_ie->app_buf,
              app_ie->app_buflen);
  set80211priv(drv, IEEE80211_IOCTL_SET_APPIEBUF, app_ie,
               sizeof(struct ieee80211req_getset_appiebuf) +
                   app_ie->app_buflen);
...
static int atheros_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr,
  ...
  wpa_printf(MSG_DEBUG, "%s: addr=%s reason_code=%d", __func__,
             ether_sprintf(addr), reason_code);
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG,
               "%s: Failed to deauth STA (addr " MACSTR " reason %d)", __func__,
...
static int atheros_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr,
                                int reason_code) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_mlme mlme;
  int ret;
  ...
  if (ret < 0) {
    wpa_printf(MSG_DEBUG,
               "%s: Failed to disassoc STA (addr " MACSTR " reason %d)",
...
#ifdef CONFIG_WPS
static int atheros_set_wps_ie(void *priv, const u8 *ie, size_t len,
                              u32 frametype) {
  struct atheros_driver_data *drv = priv;
  u8 buf[512];
  ...
  if (((frametype == IEEE80211_APPIE_FRAME_BEACON) ||
      ...
      (drv->wpa_ie != NULL)) {
    wpa_hexdump_buf(MSG_DEBUG, "atheros: Append WPA/RSN IE", drv->wpa_ie);
    os_memcpy(&(beac_ie->app_buf[len]), wpabuf_head(drv->wpa_ie),
              wpabuf_len(drv->wpa_ie));
    beac_ie->app_buflen += wpabuf_len(drv->wpa_ie);
  }

  wpa_hexdump(MSG_DEBUG, "atheros: SET_APPIEBUF", beac_ie->app_buf,
              beac_ie->app_buflen);
...
static int atheros_set_ap_wps_ie(void *priv, const struct wpabuf *beacon,
                                 const struct wpabuf *proberesp,
                                 const struct wpabuf *assocresp) {
  struct atheros_driver_data *drv = priv;

  wpa_hexdump_buf(MSG_DEBUG, "atheros: set_ap_wps_ie - beacon", beacon);
  wpa_hexdump_buf(MSG_DEBUG, "atheros: set_ap_wps_ie - proberesp", proberesp);
  wpa_hexdump_buf(MSG_DEBUG, "atheros: set_ap_wps_ie - assocresp", assocresp);
  ...
  if (atheros_set_wps_ie(priv, beacon ? wpabuf_head(beacon) : NULL,
    ...
    return -1;
  return atheros_set_wps_ie(priv, proberesp ? wpabuf_head(proberesp) : NULL,
...
static int atheros_sta_auth(void *priv, const u8 *own_addr, const u8 *addr,
                            u16 seq, u16 status_code, const u8 *ie,
                            size_t len) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_mlme mlme;
  int ret;

  wpa_printf(MSG_DEBUG, "%s: addr=%s status_code=%d", __func__,
             ether_sprintf(addr), status_code);
  ...
  if (len) {
    if (len < IEEE80211_MAX_OPT_IE) {
      ...
      wpa_printf(MSG_DEBUG,
                 ...
                 "ie_len %d)",
                 __func__, MAC2STR(addr), status_code, (int)len);
      return -1;
    }
  }
  ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
  if (ret < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to auth STA (addr " MACSTR " reason %d)",
               __func__, MAC2STR(addr), status_code);
  }
  return ret;
}

static int atheros_sta_assoc(void *priv, const u8 *own_addr, const u8 *addr,
                             int reassoc, u16 status_code, const u8 *ie,
                             size_t len) {
  struct atheros_driver_data *drv = priv;
  struct ieee80211req_mlme mlme;
  int ret;

  wpa_printf(MSG_DEBUG, "%s: addr=%s status_code=%d reassoc %d", __func__,
             ether_sprintf(addr), status_code, reassoc);
  ...
  ret = set80211priv(drv, IEEE80211_IOCTL_SETMLME, &mlme, sizeof(mlme));
  if (ret < 0) {
    wpa_printf(MSG_DEBUG, "%s: Failed to assoc STA (addr " MACSTR " reason %d)",
               __func__, MAC2STR(addr), status_code);
  }
  return ret;
}
...
static void atheros_new_sta(struct atheros_driver_data *drv,
  ...
  if (set80211priv(drv, IEEE80211_IOCTL_GETWPAIE, &ie, sizeof(ie))) {
     ...
     */
    wpa_printf(MSG_DEBUG, "%s: Failed to get WPA/RSN IE: %s", __func__,
               strerror(errno));
    goto no_ie;
  }
  wpa_hexdump(MSG_MSGDUMP, "atheros req WPA IE", ie.wpa_ie,
              IEEE80211_MAX_OPT_IE);
  wpa_hexdump(MSG_MSGDUMP, "atheros req RSN IE", ie.rsn_ie,
              IEEE80211_MAX_OPT_IE);
#ifdef ATH_WPS_IE
  wpa_hexdump(MSG_MSGDUMP, "atheros req WPS IE", ie.wps_ie,
              IEEE80211_MAX_OPT_IE);
#endif /* ATH_WPS_IE */
  iebuf = ie.wpa_ie;
...
#define MGMT_FRAM_TAG_SIZE 30 /* hardcoded in driver */
  ...
  if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
    ...
    if (hwaddr_aton(pos, addr) == 0) {
      ...
      wpa_supplicant_event(drv->hapd, EVENT_MICHAEL_MIC_FAILURE, &data);
    } else {
      wpa_printf(MSG_DEBUG, "MLME-MICHAELMICFAILURE.indication "
    ...
    if (len < 0 || MGMT_FRAM_TAG_SIZE + len > end - custom) {
      wpa_printf(MSG_DEBUG, "Invalid Manage.assoc_req event length %d", len);
      return;
    }
    atheros_raw_receive(drv, NULL, (u8 *)custom + MGMT_FRAM_TAG_SIZE, len);
    ...
    if (len < 0 || MGMT_FRAM_TAG_SIZE + len > end - custom) {
      wpa_printf(MSG_DEBUG, "Invalid Manage.auth event length %d", len);
      return;
    }
    atheros_raw_receive(drv, NULL, (u8 *)custom + MGMT_FRAM_TAG_SIZE, len);
    ...
    if (len < 0 || MGMT_FRAM_TAG_SIZE + len > end - custom) {
      wpa_printf(MSG_DEBUG, "Invalid Manage.action event length %d", len);
      return;
    }
    atheros_raw_receive(drv, NULL, (u8 *)custom + MGMT_FRAM_TAG_SIZE, len);
#endif /* ATHEROS_USE_RAW_RECEIVE */
...
static void fetch_pending_big_events(struct atheros_driver_data *drv) {
  ...
  while (1) {
    ...
    if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_P2P_BIG_PARAM, &iwr) < 0) {
      if (errno == ENOSPC) {
        wpa_printf(MSG_DEBUG, "%s:%d exit", __func__, __LINE__);
        return;
      }
      wpa_printf(MSG_DEBUG,
    ...
    wpa_hexdump(MSG_DEBUG, "athr: P2P_FETCH_FRAME data", (u8 *)tbuf, data_len);
    if (data_len < sizeof(freq) + sizeof(frame_type) + 24) {
      wpa_printf(MSG_DEBUG, "athr: frame too short");
      continue;
    }
    os_memcpy(&freq, tbuf, sizeof(freq));
    os_memcpy(&frame_type, &tbuf[sizeof(freq)], sizeof(frame_type));
    ...
    if (frame_type == IEEE80211_EV_RX_MGMT) {
      ...
      wpa_printf(MSG_DEBUG, "athr: %s unknown type %d", __func__, frame_type);
      continue;
    }
  }
}
...
static void atheros_wireless_event_wireless(struct atheros_driver_data *drv,
  ...
  while ((size_t)(end - pos) >= IW_EV_LCP_LEN) {
    ...
    wpa_printf(MSG_MSGDUMP, "Wireless event: cmd=0x%x len=%d", iwe->cmd,
               iwe->len);
    ...
    if (drv->we_version > 18 &&
        (iwe->cmd == IWEVMICHAELMICFAILURE || iwe->cmd == IWEVASSOCREQIE ||
         iwe->cmd == IWEVCUSTOM)) {
    ...
    case IWEVCUSTOM:
      ...
      if (iwe->u.data.flags != 0) {
        atheros_wireless_event_atheros_custom(drv, (int)iwe->u.data.flags, buf,
                                              len);
      } else {
        atheros_wireless_event_wireless_custom(drv, buf,
                                               buf + iwe->u.data.length);
      }
      os_free(buf);
      break;
    }
...
static void atheros_wireless_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi,
                                               u8 *buf, size_t len) {
  struct atheros_driver_data *drv = ctx;
  int attrlen, rta_len;
  struct rtattr *attr;
  ...
  while (RTA_OK(attr, attrlen)) {
    if (attr->rta_type == IFLA_WIRELESS) {
      atheros_wireless_event_wireless(drv, ((char *)attr) + rta_len,
...
static int atheros_get_we_version(struct atheros_driver_data *drv) {
  ...
  if (ioctl(drv->ioctl_sock, SIOCGIWRANGE, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGIWRANGE]: %s", strerror(errno));
    os_free(range);
    return -1;
...
static int atheros_send_eapol(void *priv, const u8 *addr, const u8 *data,
...
static void handle_read(void *ctx, const u8 *src_addr, const u8 *buf,
                        size_t len) {
  struct atheros_driver_data *drv = ctx;
...
static void *atheros_init(struct hostapd_data *hapd,
  ...
  if (params->bridge[0]) {
    wpa_printf(MSG_DEBUG, "Configure bridge %s for EAPOL traffic.",
               params->bridge[0]);
    drv->sock_recv = l2_packet_init(params->bridge[0], NULL, ETH_P_EAPOL,
                                    handle_read, drv, 1);
    ...
    wpa_printf(MSG_DEBUG,
               ...
               brname);
    drv->sock_recv =
        l2_packet_init(brname, NULL, ETH_P_EAPOL, handle_read, drv, 1);
    if (drv->sock_recv == NULL)
      goto bad;
  } else
...
static int atheros_set_ssid(void *priv, const u8 *buf, int len) {
  struct atheros_driver_data *drv = priv;
  struct iwreq iwr;
  ...
  if (ioctl(drv->ioctl_sock, SIOCSIWESSID, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCSIWESSID,len=%d]: %s", len,
               strerror(errno));
    return -1;
  }
  return 0;
...
static int atheros_get_ssid(void *priv, u8 *buf, int len) {
  struct atheros_driver_data *drv = priv;
  struct iwreq iwr;
  int ret = 0;
  ...
  os_strlcpy(iwr.ifr_name, drv->iface, IFNAMSIZ);
  iwr.u.essid.pointer = (caddr_t)buf;
  iwr.u.essid.length = (len > IW_ESSID_MAX_SIZE) ? IW_ESSID_MAX_SIZE : len;

  if (ioctl(drv->ioctl_sock, SIOCGIWESSID, &iwr) < 0) {
    wpa_printf(MSG_ERROR, "ioctl[SIOCGIWESSID]: %s", strerror(errno));
    ret = -1;
  } else
...
static int atheros_set_countermeasures(void *priv, int enabled) {
  struct atheros_driver_data *drv = priv;
  wpa_printf(MSG_DEBUG, "%s: enabled=%d", __FUNCTION__, enabled);
  return set80211param(drv, IEEE80211_PARAM_COUNTERMEASURES, enabled);
}

static int atheros_commit(void *priv) {
  struct atheros_driver_data *drv = priv;
...
static int atheros_set_authmode(void *priv, int auth_algs) {
  ...
  if ((auth_algs & WPA_AUTH_ALG_OPEN) && (auth_algs & WPA_AUTH_ALG_SHARED))
...
static int atheros_set_ap(void *priv, struct wpa_driver_ap_params *params) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             params->key_mgmt_suites, params->auth_algs, params->wpa_version,
             params->privacy, params->interworking);
  wpa_hexdump_ascii(MSG_DEBUG, "atheros: SSID", params->ssid, params->ssid_len);
  if (params->hessid)
    wpa_printf(MSG_DEBUG, "atheros: HESSID " MACSTR, MAC2STR(params->hessid));
  wpa_hexdump_buf(MSG_DEBUG, "atheros: beacon_ies", params->beacon_ies);
  wpa_hexdump_buf(MSG_DEBUG, "atheros: proberesp_ies", params->proberesp_ies);
  wpa_hexdump_buf(MSG_DEBUG, "atheros: assocresp_ies", params->assocresp_ies);

#if defined(CONFIG_HS20) &&                                                    \
    (defined(IEEE80211_PARAM_OSEN) || defined(CONFIG_ATHEROS_OSEN))
  if (params->osen) {
...
static int set80211big(struct atheros_driver_data *drv, int op,
  ...
  iwr.u.data.length = len;
  iwr.u.data.flags = op;
  wpa_printf(MSG_DEBUG, "%s: op=0x%x=%d (%s) len=0x%x", __func__, op, op,
             athr_get_param_name(op), len);

  if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_P2P_BIG_PARAM, &iwr) < 0) {
    wpa_printf(MSG_DEBUG,
               "%s: op=0x%x (%s) subop=0x%x=%d "
               "value=0x%x,0x%x failed: %d (%s)",
               __func__, op, athr_get_ioctl_name(op), iwr.u.mode, iwr.u.mode,
               iwr.u.data.length, iwr.u.data.flags, errno, strerror(errno));
    return -1;
  }
  return 0;
}

static int atheros_send_action(void *priv, unsigned int freq, unsigned int wait,
  ...
  wpa_printf(MSG_DEBUG,
             "%s: freq=%d, wait=%u, dst=" MACSTR ", src=" MACSTR
             ", bssid=" MACSTR,
  ...
  res = set80211big(drv, IEEE80211_IOC_P2P_SEND_ACTION, act,
                    sizeof(*act) + data_len);
  os_free(act);
  return res;
}
...
static int athr_wnm_tfs(struct atheros_driver_data *drv, const u8 *peer, u8 *ie,
                        u16 *len, enum wnm_oper oper) {
#define IEEE80211_APPIE_MAX 1024 /* max appie buffer size */
  ...
  case WNM_SLEEP_TFS_REQ_IE_SET:
    if (*len >
        IEEE80211_APPIE_MAX - sizeof(struct ieee80211req_getset_appiebuf)) {
  ...
  case WNM_SLEEP_TFS_RESP_IE_ADD:
    ...
    *len = tfs_ie->app_buflen;
    ...
    wpa_printf(MSG_DEBUG, "atheros: %c len=%d", tfs_ie->app_buf[0], *len);
    break;
...
static int atheros_wnm_sleep(struct atheros_driver_data *drv, const u8 *peer,
                             enum wnm_oper oper) {
  u8 *data, *pos;
  size_t dlen;
  int ret;
  ...
  wpa_printf(MSG_DEBUG, "atheros: WNM-Sleep Oper %d, " MACSTR, oper,
             MAC2STR(peer));
...
static int atheros_wnm_oper(void *priv, enum wnm_oper oper, const u8 *peer,
  ...
  default:
    wpa_printf(MSG_DEBUG, "atheros: Unsupported WNM operation %d", oper);
    return -1;
  }
}
