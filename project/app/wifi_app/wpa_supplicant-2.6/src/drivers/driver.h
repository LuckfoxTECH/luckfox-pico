...
struct wpa_driver_ops {
   ...
   */
  struct hostapd_hw_modes *(*get_hw_feature_data)(void *priv, u16 *num_modes,
                                                  u16 *flags);
   ...
   */
  int (*get_seqnum)(const char *ifname, void *priv, const u8 *addr, int idx,
                    u8 *seq);
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*sta_set_flags)(void *priv, const u8 *addr, unsigned int total_flags,
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*if_add)(void *priv, enum wpa_driver_if_type type, const char *ifname,
                const u8 *addr, void *bss_ctx, void **drv_priv,
                char *force_ifname, u8 *if_addr, const char *bridge,
   ...
   * Returns: 0 on success, negative (<0) on failure
   */
  int (*wnm_oper)(void *priv, enum wnm_oper oper, const u8 *peer, u8 *buf,
                  u16 *buf_len);
   ...
   */
  int (*vendor_cmd)(void *priv, unsigned int vendor_id, unsigned int subcmd,
   ...
   */
  int (*sta_assoc)(void *priv, const u8 *own_addr, const u8 *addr, int reassoc,
   ...
   * to the station.
   */
  int (*sta_auth)(void *priv, const u8 *own_addr, const u8 *addr, u16 seq,
                  u16 status, const u8 *ie, size_t len);
   ...
   */
  int (*tdls_enable_channel_switch)(void *priv, const u8 *addr, u8 oper_class,
   ...
   * Returns: 0 on success, -1 on failure (or if not supported)
   */
  int (*get_receive_lowest_pn)(void *priv, u32 channel, u8 an, u32 *lowest_pn);
   ...
   * Returns: 0 on success, -1 on failure (or if not supported)
   */
  int (*get_transmit_next_pn)(void *priv, u32 channel, u8 an, u32 *next_pn);
   ...
   * Returns: 0 on success, -1 on failure (or if not supported)
   */
  int (*set_transmit_next_pn)(void *priv, u32 channel, u8 an, u32 next_pn);
   ...
   * Returns: 0 on success, -1 on failure
   */
  int (*create_receive_sa)(void *priv, u32 channel, u8 an, u32 lowest_pn,
                           const u8 *sak);
   ...
   * Returns: 0 on success or -1 on failure
   */
  int (*p2p_lo_start)(void *priv, unsigned int freq, unsigned int period,
...
enum wpa_event_type {
  ...
  /**
   * EVENT_DFS_CAC_FINISHED - Notify that channel availability check has been
   * completed
   *
  ...
  /**
   * EVENT_DFS_CAC_ABORTED - Notify that channel availability check has been
   * aborted
   *
...
union wpa_event_data {
  ...
  struct interface_status {
    ...
    char ifname[100];
    enum { EVENT_INTERFACE_ADDED, EVENT_INTERFACE_REMOVED } ievent;
  } interface_status;
  ...
  struct wnm {
    ...
    enum { WNM_SLEEP_ENTER, WNM_SLEEP_EXIT } sleep_action;
    int sleep_intval;
    u16 reason_code;
    u8 *buf;
    u16 buf_len;
  ...
  struct connect_failed_reason {
    u8 addr[ETH_ALEN];
    enum { MAX_CLIENT_REACHED, BLOCKED_CLIENT } code;
