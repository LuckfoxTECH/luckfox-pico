...
static void from_unknown_sta(struct wpa_driver_nl80211_data *drv, u8 *buf,
                             size_t len) {
  ...
  event.rx_from_unknown.wds =
      (fc & (WLAN_FC_FROMDS | WLAN_FC_TODS)) == (WLAN_FC_FROMDS | WLAN_FC_TODS);
  wpa_supplicant_event(drv->ctx, EVENT_RX_FROM_UNKNOWN, &event);
}

static void handle_frame(struct wpa_driver_nl80211_data *drv, u8 *buf,
                         size_t len, int datarate, int ssi_signal) {
  struct ieee80211_hdr *hdr;
  u16 fc;
  union wpa_event_data event;
...
static void handle_monitor_read(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  while (1) {
    ...
    case IEEE80211_RADIOTAP_TX_FLAGS:
      injected = 1;
      failed =
          le_to_host16((*(le16 *)iter.this_arg)) & IEEE80211_RADIOTAP_F_TX_FAIL;
      break;
  ...
  if (!injected)
    handle_frame(drv, buf + iter._max_length, len - iter._max_length, datarate,
                 ssi_signal);
  else
    handle_tx_callback(drv->ctx, buf + iter._max_length, len - iter._max_length,
                       !failed);
}
...
static int add_monitor_filter(int s) {
  ...
  if (setsockopt(s, SOL_SOCKET, SO_ATTACH_FILTER, &msock_filter,
                 sizeof(msock_filter))) {
...
int nl80211_create_monitor_interface(struct wpa_driver_nl80211_data *drv) {
  ...
  drv->monitor_ifidx = nl80211_create_iface(drv, buf, NL80211_IFTYPE_MONITOR,
                                            NULL, 0, NULL, NULL, 0);
  ...
  if (eloop_register_read_sock(drv->monitor_sock, handle_monitor_read, drv,
                               NULL)) {
...
int nl80211_send_monitor(struct wpa_driver_nl80211_data *drv, const void *data,
  ...
  __u8 rtap_hdr[] = {
      ...
      0xc0,
      0x00,
      0x00,                      /* bmap: flags, tx and rx flags */
      IEEE80211_RADIOTAP_F_FRAG, /* F_FRAG (fragment if required) */
      0x00,                      /* padding */
      0x00,
      0x00, /* RX and TX flags to indicate that */
      0x00,
      0x00, /* this is the injected frame directly */
  };
  struct iovec iov[2] = {{
                             .iov_base = &rtap_hdr,
