...
struct crypto_hash *eap_pwd_h_init(void) {
  ...
  return crypto_hash_init(CRYPTO_HASH_ALG_HMAC_SHA256, allzero, SHA256_MAC_LEN);
}
 ...
 */
int compute_password_element(EAP_PWD_group *grp, u16 num, const u8 *password,
                             size_t password_len, const u8 *id_server,
                             size_t id_server_len, const u8 *id_peer,
                             size_t id_peer_len, const u8 *token) {
  ...
  if (((rnd = BN_new()) == NULL) || ((cofactor = BN_new()) == NULL) ||
      ((grp->pwe = EC_POINT_new(grp->group)) == NULL) ||
      ((grp->order = BN_new()) == NULL) || ((grp->prime = BN_new()) == NULL) ||
  ...
  while (1) {
    ...
    if (eap_pwd_kdf(
            pwe_digest, SHA256_MAC_LEN, (u8 *)"EAP-pwd Hunting And Pecking",
            os_strlen("EAP-pwd Hunting And Pecking"), prfbuf, primebitlen) < 0)
      goto fail;
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-pwd: x_candidate", prfbuf, primebytelen);
     ...
     * don't have a point
     */
    if (!EC_POINT_set_compressed_coordinates_GFp(grp->group, grp->pwe,
                                                 x_candidate, is_odd, NULL))
      continue;
    /*
    ...
    if (BN_cmp(cofactor, BN_value_one())) {
      ...
      if (!EC_POINT_mul(grp->group, grp->pwe, NULL, grp->pwe, cofactor, NULL)) {
...
int compute_keys(EAP_PWD_group *grp, BN_CTX *bnctx, const BIGNUM *k,
  ...
  if (eap_pwd_kdf(mk, SHA256_MAC_LEN, session_id, SHA256_MAC_LEN + 1, msk_emsk,
                  (EAP_MSK_LEN + EAP_EMSK_LEN) * 8) < 0) {
    return -1;
  }
