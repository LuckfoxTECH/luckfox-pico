...
int eap_eke_dh_init(u8 group, u8 *ret_priv, u8 *ret_pub) {
  ...
  if (i == dh->prime_len - 1 && (ret_priv[i] == 0 || ret_priv[i] == 1))
    return -1;
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: DH private value", ret_priv,
                  dh->prime_len);

  /* y = g ^ x (mod p) */
  pub_len = dh->prime_len;
  if (crypto_mod_exp(&gen, 1, ret_priv, dh->prime_len, dh->prime, dh->prime_len,
                     ret_pub, &pub_len) < 0)
    return -1;
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: DH public value", ret_pub, dh->prime_len);
...
static int eap_eke_prf(u8 prf, const u8 *key, size_t key_len, const u8 *data,
  ...
  if (prf == EAP_EKE_PRF_HMAC_SHA2_256)
    return hmac_sha256_vector(key, key_len, num_elem, addr, len, res);
  return -1;
}
...
static int eap_eke_prf_hmac_sha1(const u8 *key, size_t key_len, const u8 *data,
  ...
  while (len > 0) {
    idx++;
    if (idx == 1)
      ret = hmac_sha1_vector(key, key_len, 2, &addr[1], &vlen[1], hash);
    else
      ret = hmac_sha1_vector(key, key_len, 3, addr, vlen, hash);
    if (ret < 0)
      return -1;
    if (len > SHA1_MAC_LEN) {
...
static int eap_eke_prf_hmac_sha256(const u8 *key, size_t key_len,
                                   const u8 *data, size_t data_len, u8 *res,
                                   size_t len) {
  ...
  while (len > 0) {
    idx++;
    if (idx == 1)
      ret = hmac_sha256_vector(key, key_len, 2, &addr[1], &vlen[1], hash);
    else
      ret = hmac_sha256_vector(key, key_len, 3, addr, vlen, hash);
    if (ret < 0)
      return -1;
...
static int eap_eke_prfplus(u8 prf, const u8 *key, size_t key_len,
                           ...
                           size_t len) {
  if (prf == EAP_EKE_PRF_HMAC_SHA1)
    return eap_eke_prf_hmac_sha1(key, key_len, data, data_len, res, len);
  if (prf == EAP_EKE_PRF_HMAC_SHA2_256)
    return eap_eke_prf_hmac_sha256(key, key_len, data, data_len, res, len);
  return -1;
}

int eap_eke_derive_key(struct eap_eke_session *sess, const u8 *password,
                       size_t password_len, const u8 *id_s, size_t id_s_len,
                       const u8 *id_p, size_t id_p_len, u8 *key) {
  ...
  if (eap_eke_prf(sess->prf, zeros, sess->prf_len, password, password_len, NULL,
                  0, temp) < 0)
    return -1;
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: temp = prf(0+, password)", temp,
                  sess->prf_len);
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-EKE: ID_S | ID_P", id, id_s_len + id_p_len);
  if (eap_eke_prfplus(sess->prf, temp, sess->prf_len, id, id_s_len + id_p_len,
                      key, key_len) < 0) {
    os_free(id);
    return -1;
  }
  os_free(id);
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: key = prf+(temp, ID_S | ID_P)", key,
                  key_len);
...
int eap_eke_dhcomp(struct eap_eke_session *sess, const u8 *key, const u8 *dhpub,
  ...
  if (random_get_bytes(iv, AES_BLOCK_SIZE))
    return -1;
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: IV for Encr(key, y)", iv, AES_BLOCK_SIZE);
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent = Encr(key, y)", ret_dhcomp,
              AES_BLOCK_SIZE + dh_len);
...
int eap_eke_shared_secret(struct eap_eke_session *sess, const u8 *key,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Decrypted peer DH pubkey", peer_pub,
                  dh->prime_len);

  /* SharedSecret = prf(0+, g ^ (x_s * x_p) (mod p)) */
  len = dh->prime_len;
  if (crypto_mod_exp(peer_pub, dh->prime_len, dhpriv, dh->prime_len, dh->prime,
  ...
  if (eap_eke_prf(sess->prf, zeros, sess->auth_len, modexp, dh->prime_len, NULL,
                  0, sess->shared_secret) < 0)
    return -1;
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: SharedSecret", sess->shared_secret,
                  sess->auth_len);

  return 0;
}

int eap_eke_derive_ke_ki(struct eap_eke_session *sess, const u8 *id_s,
                         size_t id_s_len, const u8 *id_p, size_t id_p_len) {
  ...
  if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data,
                      data_len, buf, ke_len + ki_len) < 0) {
    os_free(data);
    return -1;
  }
...
int eap_eke_derive_ka(struct eap_eke_session *sess, const u8 *id_s,
                      size_t id_s_len, const u8 *id_p, size_t id_p_len,
  ...
  if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data,
                      data_len, sess->ka, sess->prf_len) < 0) {
    os_free(data);
    return -1;
  }
  os_free(data);
...
int eap_eke_derive_msk(struct eap_eke_session *sess, const u8 *id_s,
                       size_t id_s_len, const u8 *id_p, size_t id_p_len,
                       const u8 *nonce_p, const u8 *nonce_s, u8 *msk,
                       u8 *emsk) {
  u8 *data, *pos;
  size_t data_len;
  ...
  os_memcpy(pos, nonce_s, sess->nonce_len);
  if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data,
                      data_len, buf, EAP_MSK_LEN + EAP_EMSK_LEN) < 0) {
    os_free(data);
    return -1;
  }
  os_free(data);
...
int eap_eke_prot(struct eap_eke_session *sess, const u8 *data, size_t data_len,
...
int eap_eke_decrypt_prot(struct eap_eke_session *sess, const u8 *prot,
                         size_t prot_len, u8 *data, size_t *data_len) {
  ...
  if (prot_len < 2 * block_size + icv_len || (prot_len - icv_len) % block_size)
    return -1;
  ...
  *data_len = prot_len - block_size - icv_len;
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Decrypted Prot() data", data, *data_len);
...
int eap_eke_auth(struct eap_eke_session *sess, const char *label,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Ka for Auth", sess->ka, sess->auth_len);
  wpa_hexdump_buf(MSG_MSGDUMP, "EAP-EKE: Messages for Auth", msgs);
  return eap_eke_prf(sess->prf, sess->ka, sess->auth_len, (const u8 *)label,
                     os_strlen(label), wpabuf_head(msgs), wpabuf_len(msgs),
                     auth);
}
...
int eap_eke_session_init(struct eap_eke_session *sess, u8 dhgroup, u8 encr,
  ...
  if (sess->prf_len < 0 || sess->nonce_len < 0 || sess->auth_len < 0 ||
      sess->dhcomp_len < 0 || sess->pnonce_len < 0 || sess->pnonce_ps_len < 0)
    return -1;
