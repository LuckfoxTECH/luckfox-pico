...
int eap_ikev2_derive_keymat(int prf, struct ikev2_keys *keys, const u8 *i_nonce,
                            size_t i_nonce_len, const u8 *r_nonce,
                            size_t r_nonce_len, u8 *keymat) {
  u8 *nonces;
  size_t nlen;
  ...
  if (ikev2_prf_plus(prf, keys->SK_d, keys->SK_d_len, nonces, nlen, keymat,
                     EAP_MSK_LEN + EAP_EMSK_LEN)) {
    os_free(nonces);
    return -1;
  }
  os_free(nonces);

  wpa_hexdump_key(MSG_DEBUG, "EAP-IKEV2: KEYMAT", keymat,
                  EAP_MSK_LEN + EAP_EMSK_LEN);
...
int eap_ikev2_validate_icv(int integ_alg, struct ikev2_keys *keys,
  ...
  if (ikev2_integ_hash(integ_alg, SK_a, keys->SK_integ_len, wpabuf_head(msg),
  ...
  if (os_memcmp_const(icv, end - icv_len, icv_len) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-IKEV2: Calculated ICV", icv, icv_len);
    wpa_hexdump(MSG_DEBUG, "EAP-IKEV2: Received ICV", end - icv_len, icv_len);
    return -1;
  }
