...
void eap_sim_derive_mk(const u8 *identity, size_t identity_len,
                       ...
                       const u8 *ver_list, size_t ver_list_len, int num_chal,
                       const u8 *kc, u8 *mk) {
  u8 sel_ver[2];
...
void eap_aka_derive_mk(const u8 *identity, size_t identity_len, const u8 *ik,
                       const u8 *ck, u8 *mk) {
  const u8 *addr[3];
  size_t len[3];
...
int eap_sim_derive_keys(const u8 *mk, u8 *k_encr, u8 *k_aut, u8 *msk,
                        u8 *emsk) {
  u8 buf[EAP_SIM_K_ENCR_LEN + EAP_SIM_K_AUT_LEN + EAP_SIM_KEYING_DATA_LEN +
         EAP_EMSK_LEN],
      *pos;
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: K_encr", k_encr, EAP_SIM_K_ENCR_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: K_aut", k_aut, EAP_SIM_K_AUT_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: keying material (MSK)", msk,
...
int eap_sim_derive_keys_reauth(u16 _counter, const u8 *identity,
                               size_t identity_len, const u8 *nonce_s,
                               const u8 *mk, u8 *msk, u8 *emsk) {
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-SIM: Identity", identity, identity_len);
  wpa_hexdump(MSG_DEBUG, "EAP-SIM: counter", counter, 2);
  wpa_hexdump(MSG_DEBUG, "EAP-SIM: NONCE_S", nonce_s, EAP_SIM_NONCE_S_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: MK", mk, EAP_SIM_MK_LEN);
  ...
  if (msk) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-SIM: keying material (MSK)", msk,
                EAP_SIM_KEYING_DATA_LEN);
  }
  if (emsk) {
...
int eap_sim_verify_mac(const u8 *k_aut, const struct wpabuf *req, const u8 *mac,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Verify MAC - msg", tmp, wpabuf_len(req));
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Verify MAC - extra data", extra,
              extra_len);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-SIM: Verify MAC - K_aut", k_aut,
                  EAP_SIM_K_AUT_LEN);
  hmac_sha1_vector(k_aut, EAP_SIM_K_AUT_LEN, 2, addr, len, hmac);
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Verify MAC: MAC", hmac, EAP_SIM_MAC_LEN);
  os_free(tmp);
...
void eap_sim_add_mac(const u8 *k_aut, const u8 *msg, size_t msg_len, u8 *mac,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Add MAC - extra data", extra, extra_len);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-SIM: Add MAC - K_aut", k_aut,
                  EAP_SIM_K_AUT_LEN);
  hmac_sha1_vector(k_aut, EAP_SIM_K_AUT_LEN, 2, addr, len, hmac);
  os_memcpy(mac, hmac, EAP_SIM_MAC_LEN);
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Add MAC: MAC", mac, EAP_SIM_MAC_LEN);
}

#if defined(EAP_AKA_PRIME) || defined(EAP_SERVER_AKA_PRIME)
static void prf_prime(const u8 *k, const char *seed1, const u8 *seed2,
                      size_t seed2_len, const u8 *seed3, size_t seed3_len,
...
void eap_aka_prime_derive_keys(const u8 *identity, size_t identity_len,
  ...
  prf_prime(key, "EAP-AKA'", identity, identity_len, NULL, 0, keys,
            sizeof(keys));

  pos = keys;
  os_memcpy(k_encr, pos, EAP_SIM_K_ENCR_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': K_encr", k_encr, EAP_SIM_K_ENCR_LEN);
  pos += EAP_SIM_K_ENCR_LEN;

  os_memcpy(k_aut, pos, EAP_AKA_PRIME_K_AUT_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': K_aut", k_aut, EAP_AKA_PRIME_K_AUT_LEN);
  pos += EAP_AKA_PRIME_K_AUT_LEN;

  os_memcpy(k_re, pos, EAP_AKA_PRIME_K_RE_LEN);
  wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': K_re", k_re, EAP_AKA_PRIME_K_RE_LEN);
...
int eap_aka_prime_derive_keys_reauth(const u8 *k_re, u16 counter,
  ...
  prf_prime(k_re, "EAP-AKA' re-auth", identity, identity_len, seed3,
            sizeof(seed3), keys, sizeof(keys));
...
int eap_sim_verify_mac_sha256(const u8 *k_aut, const struct wpabuf *req,
  ...
  os_memcpy(tmp, wpabuf_head(req), wpabuf_len(req));
  os_memset(tmp + (mac - wpabuf_head_u8(req)), 0, EAP_SIM_MAC_LEN);
  wpa_hexdump(MSG_MSGDUMP, "EAP-AKA': Verify MAC - msg", tmp, wpabuf_len(req));
  wpa_hexdump(MSG_MSGDUMP, "EAP-AKA': Verify MAC - extra data", extra,
              extra_len);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-AKA': Verify MAC - K_aut", k_aut,
                  EAP_AKA_PRIME_K_AUT_LEN);
  hmac_sha256_vector(k_aut, EAP_AKA_PRIME_K_AUT_LEN, 2, addr, len, hmac);
  wpa_hexdump(MSG_MSGDUMP, "EAP-AKA': Verify MAC: MAC", hmac, EAP_SIM_MAC_LEN);
  os_free(tmp);
...
void eap_sim_add_mac_sha256(const u8 *k_aut, const u8 *msg, size_t msg_len,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-AKA': Add MAC - extra data", extra, extra_len);
  wpa_hexdump_key(MSG_MSGDUMP, "EAP-AKA': Add MAC - K_aut", k_aut,
                  EAP_AKA_PRIME_K_AUT_LEN);
  hmac_sha256_vector(k_aut, EAP_AKA_PRIME_K_AUT_LEN, 2, addr, len, hmac);
  os_memcpy(mac, hmac, EAP_SIM_MAC_LEN);
  wpa_hexdump(MSG_MSGDUMP, "EAP-AKA': Add MAC: MAC", mac, EAP_SIM_MAC_LEN);
}
...
void eap_aka_prime_derive_ck_ik_prime(u8 *ck, u8 *ik, const u8 *sqn_ak,
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': Key = CK || IK", key, sizeof(key));
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': KDF output (CK' || IK')", hash,
                  sizeof(hash));
...
int eap_sim_parse_attr(const u8 *start, const u8 *end,
  ...
  while (pos < end) {
    ...
    wpa_printf(MSG_MSGDUMP, "EAP-SIM: Attribute: Type=%d Len=%d", pos[0],
               pos[1] * 4);
    if (pos + pos[1] * 4 > end) {
      wpa_printf(MSG_INFO,
    ...
    apos = pos + 2;
    alen = pos[1] * 4 - 2;
    wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Attribute data", apos, alen);
    ...
    case EAP_SIM_AT_NOTIFICATION:
      ...
      wpa_printf(MSG_DEBUG, "EAP-SIM: AT_NOTIFICATION %d", attr->notification);
      break;
    ...
    case EAP_SIM_AT_IDENTITY:
      ...
      if (plen > alen) {
        wpa_printf(MSG_INFO,
                   ...
                   "remaining length %lu)",
                   (unsigned long)plen, (unsigned long)alen);
        return -1;
      }
    ...
    case EAP_SIM_AT_VERSION_LIST:
      ...
      if (list_len < 2 || list_len > alen - 2) {
        wpa_printf(MSG_WARNING,
                   ...
                   "attr_len=%lu)",
                   (unsigned long)list_len, (unsigned long)alen);
        return -1;
      }
    ...
    case EAP_SIM_AT_COUNTER:
      ...
      wpa_printf(MSG_DEBUG, "EAP-SIM: (encr) AT_COUNTER %d", attr->counter);
      break;
    ...
    case EAP_SIM_AT_ENCR_DATA:
      ...
      if (attr->encr_data_len % 16) {
        wpa_printf(MSG_INFO,
                   ...
                   (unsigned long)attr->encr_data_len);
        return -1;
      }
      break;
    ...
    case EAP_SIM_AT_RES:
      ...
      apos += 2;
      alen -= 2;
      if (!aka || alen < EAP_AKA_MIN_RES_LEN || alen > EAP_AKA_MAX_RES_LEN) {
        wpa_printf(MSG_INFO,
...
u8 *eap_sim_parse_encr(const u8 *k_encr, const u8 *encr_data,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-SIM: Decrypted AT_ENCR_DATA", decrypted,
              encr_data_len);

  if (eap_sim_parse_attr(decrypted, decrypted + encr_data_len, attr, aka, 1)) {
...
struct wpabuf *eap_sim_msg_finish(struct eap_sim_msg *msg, int type,
  ...
  if (k_aut && msg->mac && type == EAP_TYPE_AKA_PRIME) {
    eap_sim_add_mac_sha256(
        k_aut, (u8 *)wpabuf_head(msg->buf), wpabuf_len(msg->buf),
        (u8 *)wpabuf_mhead(msg->buf) + msg->mac, extra, extra_len);
  } else
#endif /* EAP_AKA_PRIME || EAP_SERVER_AKA_PRIME */
    if (k_aut && msg->mac) {
      eap_sim_add_mac(k_aut, (u8 *)wpabuf_head(msg->buf), wpabuf_len(msg->buf),
                      (u8 *)wpabuf_mhead(msg->buf) + msg->mac, extra,
                      extra_len);
    }
...
u8 *eap_sim_msg_add_full(struct eap_sim_msg *msg, u8 attr, const u8 *data,
                         size_t len) {
  int attr_len = 2 + len;
  int pad_len;
  u8 *start;
...
u8 *eap_sim_msg_add(struct eap_sim_msg *msg, u8 attr, u16 value, const u8 *data,
                    size_t len) {
  int attr_len = 4 + len;
  int pad_len;
  u8 *start;
...
int eap_sim_msg_add_encr_start(struct eap_sim_msg *msg, u8 attr_iv,
  ...
  if (random_get_bytes(wpabuf_mhead_u8(msg->buf) + msg->iv, EAP_SIM_IV_LEN)) {
    msg->iv = 0;
    return -1;
  }
...
void eap_sim_report_notification(void *msg_ctx, int notification, int aka) {
  ...
  case EAP_SIM_GENERAL_FAILURE_AFTER_AUTH:
    wpa_printf(MSG_WARNING,
               ...
               "notification (after authentication)",
               type);
    break;
  ...
  case EAP_SIM_GENERAL_FAILURE_BEFORE_AUTH:
    wpa_printf(MSG_WARNING,
               "EAP-%s: General failure "
               "notification (before authentication)",
               type);
    break;
