...
static void eap_leap_deinit(struct eap_sm *sm, void *priv) { os_free(priv); }
...
static struct wpabuf *eap_leap_process_request(struct eap_sm *sm, void *priv,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-LEAP: Challenge from AP", challenge,
              LEAP_CHALLENGE_LEN);

  wpa_printf(MSG_DEBUG, "EAP-LEAP: Generating Challenge Response");

  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_LEAP,
                       3 + LEAP_RESPONSE_LEN + identity_len, EAP_CODE_RESPONSE,
                       eap_get_id(reqData));
  if (resp == NULL)
    return NULL;
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-LEAP: Response", rpos, LEAP_RESPONSE_LEN);
...
static struct wpabuf *eap_leap_process_success(struct eap_sm *sm, void *priv,
  ...
  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_LEAP,
                       3 + LEAP_CHALLENGE_LEN + identity_len, EAP_CODE_REQUEST,
                       eap_get_id(reqData));
  if (resp == NULL)
    return NULL;
  wpabuf_put_u8(resp, LEAP_VERSION);
...
static struct wpabuf *eap_leap_process_response(struct eap_sm *sm, void *priv,
  ...
  u8 response_len, pw_hash[16], pw_hash_hash[16], expected[LEAP_RESPONSE_LEN];
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-LEAP: Response from AP", pos, LEAP_RESPONSE_LEN);
  ...
  if (os_memcmp_const(pos, expected, LEAP_RESPONSE_LEN) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-LEAP: Expected response from AP", expected,
                LEAP_RESPONSE_LEN);
...
static u8 *eap_leap_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-LEAP: pw_hash_hash", pw_hash_hash, 16);
  wpa_hexdump(MSG_DEBUG, "EAP-LEAP: peer_challenge", data->peer_challenge,
              LEAP_CHALLENGE_LEN);
  wpa_hexdump(MSG_DEBUG, "EAP-LEAP: peer_response", data->peer_response,
              LEAP_RESPONSE_LEN);
  wpa_hexdump(MSG_DEBUG, "EAP-LEAP: ap_challenge", data->ap_challenge,
              LEAP_CHALLENGE_LEN);
  wpa_hexdump(MSG_DEBUG, "EAP-LEAP: ap_response", data->ap_response,
              LEAP_RESPONSE_LEN);
