...
static struct wpabuf *eap_gtc_process(struct eap_sm *sm, void *priv,
  ...
  if (data->prefix && (len < 10 || os_memcmp(pos, "CHALLENGE=", 10) != 0)) {
    ...
    resp =
        eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GTC, 0, EAP_CODE_RESPONSE, id);
    return resp;
  }
  ...
  resp =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GTC, plen, EAP_CODE_RESPONSE, id);
  if (resp == NULL)
    return NULL;
  if (data->prefix) {
  ...
  wpa_hexdump_ascii_key(MSG_MSGDUMP, "EAP-GTC: Response",
                        wpabuf_head_u8(resp) + sizeof(struct eap_hdr) + 1,
                        plen);
