...
static struct wpabuf *eap_gpsk_send_gpsk_2(struct eap_gpsk_data *data,
                                           u8 identifier, const u8 *csuite_list,
...
static void eap_gpsk_state(struct eap_gpsk_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-GPSK: %s -> %s", eap_gpsk_state_txt(data->state),
...
static void *eap_gpsk_init(struct eap_sm *sm) {
  ...
  if (phase1) {
    ...
    if (pos) {
      ...
      wpa_printf(MSG_DEBUG, "EAP-GPSK: Forced cipher %u", data->forced_cipher);
    }
  }
...
static const u8 *eap_gpsk_process_id_server(struct eap_gpsk_data *data,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-GPSK: ID_Server", data->id_server,
                    data->id_server_len);
  pos += alen;
...
static const u8 *eap_gpsk_process_rand_server(struct eap_gpsk_data *data,
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Server", data->rand_server,
              EAP_GPSK_RAND_LEN);
...
static int eap_gpsk_select_csuite(struct eap_sm *sm, struct eap_gpsk_data *data,
  ...
  for (i = 0; i < count; i++) {
    ...
    wpa_printf(MSG_DEBUG, "EAP-GPSK: CSuite[%d]: %d:%d", i, vendor, specifier);
  ...
  wpa_printf(MSG_DEBUG, "EAP-GPSK: Selected ciphersuite %d:%d", data->vendor,
             data->specifier);
...
static struct wpabuf *eap_gpsk_process_gpsk_1(struct eap_sm *sm,
                                              ...
                                              u8 identifier, const u8 *payload,
                                              size_t payload_len) {
  size_t csuite_list_len;
  ...
  pos = eap_gpsk_process_csuite_list(sm, data, &csuite_list, &csuite_list_len,
                                     pos, end);
  if (pos == NULL) {
  ...
  resp = eap_gpsk_send_gpsk_2(data, identifier, csuite_list, csuite_list_len);
  if (resp == NULL)
    return NULL;
...
static struct wpabuf *eap_gpsk_send_gpsk_2(struct eap_gpsk_data *data,
  ...
  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_GPSK, len, EAP_CODE_RESPONSE,
                       identifier);
  if (resp == NULL)
    return NULL;
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-GPSK: ID_Peer", data->id_peer,
                    data->id_peer_len);
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Peer", data->rand_peer,
              EAP_GPSK_RAND_LEN);
  ...
  if (eap_gpsk_derive_keys(data->psk, data->psk_len, data->vendor,
                           data->specifier, data->rand_peer, data->rand_server,
                           data->id_peer, data->id_peer_len, data->id_server,
                           data->id_server_len, data->msk, data->emsk, data->sk,
                           &data->sk_len, data->pk, &data->pk_len) < 0) {
  ...
  if (eap_gpsk_derive_session_id(
          data->psk, data->psk_len, data->vendor, data->specifier,
          data->rand_peer, data->rand_server, data->id_peer, data->id_peer_len,
          data->id_server, data->id_server_len, EAP_TYPE_GPSK, data->session_id,
          &data->id_len) < 0) {
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-GPSK: Derived Session-Id", data->session_id,
              data->id_len);
  ...
  if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
                           data->specifier, start, rpos - start, rpos) < 0) {
    eap_gpsk_state(data, FAILURE);
    wpabuf_free(resp);
    return NULL;
  }
...
static const u8 *eap_gpsk_validate_rand(struct eap_gpsk_data *data,
  ...
  if (os_memcmp(pos, data->rand_peer, EAP_GPSK_RAND_LEN) != 0) {
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Peer in GPSK-2", data->rand_peer,
                EAP_GPSK_RAND_LEN);
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Peer in GPSK-3", pos,
                EAP_GPSK_RAND_LEN);
    return NULL;
  }
  pos += EAP_GPSK_RAND_LEN;
  ...
  if (os_memcmp(pos, data->rand_server, EAP_GPSK_RAND_LEN) != 0) {
    wpa_printf(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-1 and "
                          "GPSK-3 did not match");
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-1", data->rand_server,
                EAP_GPSK_RAND_LEN);
    wpa_hexdump(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-3", pos,
                EAP_GPSK_RAND_LEN);
    return NULL;
  }
  pos += EAP_GPSK_RAND_LEN;
...
static const u8 *eap_gpsk_validate_id_server(struct eap_gpsk_data *data,
  ...
  if (len != data->id_server_len || os_memcmp(pos, data->id_server, len) != 0) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-GPSK: ID_Server in GPSK-3", pos, len);
    return NULL;
  }
...
static const u8 *eap_gpsk_validate_gpsk_3_mic(struct eap_gpsk_data *data,
  ...
  if (end - pos < (int)miclen) {
    wpa_printf(MSG_DEBUG,
               ...
               "(left=%lu miclen=%lu)",
               (unsigned long)(end - pos), (unsigned long)miclen);
    return NULL;
  }
  if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
                           data->specifier, payload, pos - payload, mic) < 0) {
...
static struct wpabuf *eap_gpsk_process(struct eap_sm *sm, void *priv,
  ...
  case EAP_GPSK_OPCODE_GPSK_3:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-GPSK: Ignoring message with unknown opcode %d",
               opcode);
    ret->ignore = TRUE;
    return NULL;
  }
