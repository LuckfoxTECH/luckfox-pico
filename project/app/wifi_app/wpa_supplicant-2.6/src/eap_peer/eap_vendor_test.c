...
static void *eap_vendor_test_init(struct eap_sm *sm) {
  ...
  data->test_pending_req =
      password && password_len == 7 && os_memcmp(password, "pending", 7) == 0;
...
static struct wpabuf *eap_vendor_test_process(struct eap_sm *sm, void *priv,
  ...
  resp = eap_msg_alloc(EAP_VENDOR_ID, EAP_VENDOR_TYPE, 1, EAP_CODE_RESPONSE,
                       eap_get_id(reqData));
  if (resp == NULL)
    return NULL;
...
int eap_peer_vendor_test_register(void) {
  ...
  eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION, EAP_VENDOR_ID,
                              EAP_VENDOR_TYPE, "VENDOR-TEST");
  if (eap == NULL)
    return -1;
