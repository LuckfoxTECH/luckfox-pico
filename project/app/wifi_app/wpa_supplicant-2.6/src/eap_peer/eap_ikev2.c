...
static struct wpabuf *eap_ikev2_build_msg(struct eap_ikev2_data *data,
  ...
  resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_IKEV2, plen, EAP_CODE_RESPONSE,
                       id);
  if (resp == NULL)
    return NULL;
  ...
  if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
    ...
    ikev2_integ_hash(data->ikev2.proposal.integ, data->ikev2.keys.SK_ar,
                     data->ikev2.keys.SK_integ_len, msg, len,
                     wpabuf_put(resp, icv_len));
  }
...
static int eap_ikev2_process_icv(struct eap_ikev2_data *data,
                                 const struct wpabuf *reqData, u8 flags,
                                 const u8 *pos, const u8 **end, int frag_ack) {
  if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
    int icv_len = eap_ikev2_validate_icv(
        data->ikev2.proposal.integ, &data->ikev2.keys, 1, reqData, pos, *end);
    if (icv_len < 0)
      return -1;
...
static struct wpabuf *eap_ikev2_process(struct eap_sm *sm, void *priv,
  ...
  if (flags & IKEV2_FLAGS_MORE_FRAGMENTS) {
    return eap_ikev2_process_fragment(data, ret, id, flags, message_length, pos,
                                      end - pos);
  }
