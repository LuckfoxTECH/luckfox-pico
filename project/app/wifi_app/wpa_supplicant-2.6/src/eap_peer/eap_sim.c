...
struct eap_sim_data {
  ...
  enum { CONTINUE, RESULT_SUCCESS, SUCCESS, FAILURE } state;
...
static void eap_sim_state(struct eap_sim_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-SIM: %s -> %s", eap_sim_state_txt(data->state),
...
static void *eap_sim_init(struct eap_sm *sm) {
  ...
  if (config && config->phase1) {
    ...
    data->result_ind = os_strstr(config->phase1, "result_ind=1") != NULL;
  }
...
static int eap_sim_ext_sim_req(struct eap_sm *sm, struct eap_sim_data *data) {
  ...
  for (i = 0; i < data->num_chal; i++) {
    ...
    pos += wpa_snprintf_hex(pos, end - pos, data->rand[i], GSM_RAND_LEN);
  }
...
static int eap_sim_ext_sim_result(struct eap_sm *sm, struct eap_sim_data *data,
  ...
  wpa_printf(MSG_DEBUG, "EAP-SIM: Use result from external SIM processing");
  ...
  pos = resp + 9;
  for (i = 0; i < data->num_chal; i++) {
    wpa_hexdump(MSG_DEBUG, "EAP-SIM: RAND", data->rand[i], GSM_RAND_LEN);

    if (hexstr2bin(pos, data->kc[i], EAP_SIM_KC_LEN) < 0)
      goto invalid;
    wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: Kc", data->kc[i], EAP_SIM_KC_LEN);
    ...
    if (hexstr2bin(pos, data->sres[i], EAP_SIM_SRES_LEN) < 0)
      goto invalid;
    wpa_hexdump_key(MSG_DEBUG, "EAP-SIM: SRES", data->sres[i],
                    EAP_SIM_SRES_LEN);
...
static int eap_sim_gsm_auth(struct eap_sm *sm, struct eap_sim_data *data) {
...
#ifdef PCSC_FUNCS
  if (conf->pcsc) {
    if (scard_gsm_auth(sm->scard_ctx, data->rand[0], data->sres[0],
                       data->kc[0]) ||
        scard_gsm_auth(sm->scard_ctx, data->rand[1], data->sres[1],
                       data->kc[1]) ||
        (data->num_chal > 2 && scard_gsm_auth(sm->scard_ctx, data->rand[2],
  ...
  if (conf->password) {
    ...
    for (i = 0; i < data->num_chal; i++) {
      if (gsm_milenage(opc, k, data->rand[i], data->sres[i], data->kc[i])) {
   ...
   * the order of pre-configured values. */
  {
    ...
    for (i = 0; i < data->num_chal; i++) {
      if (data->rand[i][0] == 0xaa) {
        os_memcpy(data->kc[i], "\xa0\xa1\xa2\xa3\xa4\xa5\xa6\xa7",
                  EAP_SIM_KC_LEN);
        os_memcpy(data->sres[i], "\xd1\xd2\xd3\xd4", EAP_SIM_SRES_LEN);
      } else if (data->rand[i][0] == 0xbb) {
        os_memcpy(data->kc[i], "\xb0\xb1\xb2\xb3\xb4\xb5\xb6\xb7",
                  EAP_SIM_KC_LEN);
        os_memcpy(data->sres[i], "\xe1\xe2\xe3\xe4", EAP_SIM_SRES_LEN);
      } else {
        os_memcpy(data->kc[i], "\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7",
                  EAP_SIM_KC_LEN);
        os_memcpy(data->sres[i], "\xf1\xf2\xf3\xf4", EAP_SIM_SRES_LEN);
      }
    }
  }
...
static int eap_sim_learn_ids(struct eap_sm *sm, struct eap_sim_data *data,
  ...
  if (attr->next_pseudonym) {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-SIM: (encr) AT_NEXT_PSEUDONYM",
                      attr->next_pseudonym, attr->next_pseudonym_len);
    os_free(data->pseudonym);
    /* Look for the realm of the permanent identity */
    identity = eap_get_config_identity(sm, &identity_len);
    if (identity) {
      for (realm = identity, realm_len = identity_len; realm_len > 0;
           realm_len--, realm++) {
        if (*realm == '@')
          break;
      }
    }
    data->pseudonym = os_malloc(attr->next_pseudonym_len + realm_len);
    ...
    os_memcpy(data->pseudonym, attr->next_pseudonym, attr->next_pseudonym_len);
    if (realm_len) {
      os_memcpy(data->pseudonym + attr->next_pseudonym_len, realm, realm_len);
    }
  ...
  if (attr->next_reauth_id) {
    ...
    os_memcpy(data->reauth_id, attr->next_reauth_id, attr->next_reauth_id_len);
    data->reauth_id_len = attr->next_reauth_id_len;
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-SIM: (encr) AT_NEXT_REAUTH_ID",
...
static struct wpabuf *eap_sim_client_error(struct eap_sim_data *data, u8 id,
  ...
  wpa_printf(MSG_DEBUG, "EAP-SIM: Send Client-Error (error code %d)", err);
...
static struct wpabuf *eap_sim_response_start(struct eap_sm *sm,
  ...
  if (id_req == ANY_ID && data->reauth_id) {
    ...
    data->reauth = 1;
  } else if ((id_req == ANY_ID || id_req == FULLAUTH_ID) && data->pseudonym) {
    ...
    identity = eap_get_config_identity(sm, &identity_len);
    if (identity) {
      eap_sim_clear_identities(sm, data, CLEAR_PSEUDONYM | CLEAR_REAUTH_ID);
    }
  }
  if (id_req != NO_ID_REQ)
  ...
  if (!data->reauth) {
    wpa_hexdump(MSG_DEBUG, "   AT_NONCE_MT", data->nonce_mt,
                EAP_SIM_NONCE_MT_LEN);
    eap_sim_msg_add(msg, EAP_SIM_AT_NONCE_MT, 0, data->nonce_mt,
                    EAP_SIM_NONCE_MT_LEN);
    wpa_printf(MSG_DEBUG, "   AT_SELECTED_VERSION %d", data->selected_version);
    eap_sim_msg_add(msg, EAP_SIM_AT_SELECTED_VERSION, data->selected_version,
                    NULL, 0);
  }

  if (identity) {
    wpa_hexdump_ascii(MSG_DEBUG, "   AT_IDENTITY", identity, identity_len);
    eap_sim_msg_add(msg, EAP_SIM_AT_IDENTITY, identity_len, identity,
                    identity_len);
  }
...
static struct wpabuf *eap_sim_response_challenge(struct eap_sim_data *data,
  ...
  return eap_sim_msg_finish(msg, EAP_TYPE_SIM, data->k_aut, (u8 *)data->sres,
                            data->num_chal * EAP_SIM_SRES_LEN);
}

static struct wpabuf *eap_sim_response_reauth(struct eap_sim_data *data, u8 id,
                                              int counter_too_small,
                                              const u8 *nonce_s) {
  struct eap_sim_msg *msg;
  unsigned int counter;

  wpa_printf(MSG_DEBUG, "Generating EAP-SIM Reauthentication (id=%d)", id);
  msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, EAP_TYPE_SIM,
...
static struct wpabuf *eap_sim_response_notification(struct eap_sim_data *data,
  ...
  if (k_aut && data->reauth) {
    ...
    eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER, data->counter, NULL, 0);
    if (eap_sim_msg_add_encr_end(msg, data->k_encr, EAP_SIM_AT_PADDING)) {
      wpa_printf(MSG_WARNING, "EAP-SIM: Failed to encrypt "
                              "AT_ENCR_DATA");
      eap_sim_msg_free(msg);
...
static struct wpabuf *eap_sim_process_start(struct eap_sm *sm,
  ...
  wpa_printf(MSG_DEBUG, "EAP-SIM: Selected Version %d", selected_version);
...
static struct wpabuf *eap_sim_process_challenge(struct eap_sm *sm,
  ...
  if (!attr->mac || !attr->rand) {
    wpa_printf(MSG_WARNING,
               ...
               "did not include%s%s",
               !attr->mac ? " AT_MAC" : "", !attr->rand ? " AT_RAND" : "");
    return eap_sim_client_error(data, id, EAP_SIM_UNABLE_TO_PROCESS_PACKET);
  }
  ...
  if (attr->num_chal < data->min_num_chal) {
    wpa_printf(MSG_INFO,
               ...
               "challenges (%lu)",
               (unsigned long)attr->num_chal);
    return eap_sim_client_error(data, id, EAP_SIM_INSUFFICIENT_NUM_OF_CHAL);
  }
  if (attr->num_chal > 3) {
    wpa_printf(MSG_INFO,
  ...
  if (os_memcmp(attr->rand, attr->rand + GSM_RAND_LEN, GSM_RAND_LEN) == 0 ||
      (attr->num_chal > 2 &&
       (os_memcmp(attr->rand, attr->rand + 2 * GSM_RAND_LEN, GSM_RAND_LEN) ==
            0 ||
        os_memcmp(attr->rand + GSM_RAND_LEN, attr->rand + 2 * GSM_RAND_LEN,
                  GSM_RAND_LEN) == 0))) {
    wpa_printf(MSG_INFO, "EAP-SIM: Same RAND used multiple times");
    return eap_sim_client_error(data, id, EAP_SIM_RAND_NOT_FRESH);
  }
  ...
  eap_sim_derive_mk(identity, identity_len, data->nonce_mt,
                    data->selected_version, data->ver_list, data->ver_list_len,
  ...
  if (data->state != FAILURE) {
    eap_sim_state(data, data->use_result_ind ? RESULT_SUCCESS : SUCCESS);
  }
...
static int eap_sim_process_notification_auth(struct eap_sim_data *data,
  ...
  if (data->reauth && eap_sim_process_notification_reauth(data, attr)) {
...
static struct wpabuf *eap_sim_process_notification(struct eap_sm *sm,
                                                   struct eap_sim_data *data,
                                                   u8 id,
                                                   const struct wpabuf *reqData,
                                                   struct eap_sim_attrs *attr) {
...
eap_sim_process_reauthentication(struct eap_sm *sm, struct eap_sim_data *data,
                                 u8 id, const struct wpabuf *reqData,
                                 struct eap_sim_attrs *attr) {
  struct eap_sim_attrs eattr;
  u8 *decrypted;
  ...
  wpa_hexdump(MSG_DEBUG, "EAP-SIM: (encr) AT_NONCE_S", data->nonce_s,
              EAP_SIM_NONCE_S_LEN);

  eap_sim_derive_keys_reauth(data->counter, data->reauth_id,
                             data->reauth_id_len, data->nonce_s, data->mk,
                             data->msk, data->emsk);
  eap_sim_clear_identities(sm, data, CLEAR_REAUTH_ID | CLEAR_EAP_ID);
  eap_sim_learn_ids(sm, data, &eattr);
...
static struct wpabuf *eap_sim_process(struct eap_sm *sm, void *priv,
  ...
  if (eap_sim_parse_attr(pos, wpabuf_head_u8(reqData) + len, &attr, 0, 0)) {
    res = eap_sim_client_error(data, id, EAP_SIM_UNABLE_TO_PROCESS_PACKET);
    goto done;
  }
  ...
  case EAP_SIM_SUBTYPE_NOTIFICATION:
    res = eap_sim_process_notification(sm, data, id, reqData, &attr);
    break;
  case EAP_SIM_SUBTYPE_REAUTHENTICATION:
    res = eap_sim_process_reauthentication(sm, data, id, reqData, &attr);
    break;
  ...
  if (data->state == FAILURE) {
    ...
    ret->decision =
        data->use_result_ind ? DECISION_UNCOND_SUCC : DECISION_COND_SUCC;
    ret->methodState = data->use_result_ind ? METHOD_DONE : METHOD_MAY_CONT;
