...
struct tnc_if_imc {
  ...
  TNC_Result (*Initialize)(TNC_IMCID imcID, TNC_Version minVersion,
                           TNC_Version maxVersion,
                           TNC_Version *pOutActualVersion);
  TNC_Result (*NotifyConnectionChange)(TNC_IMCID imcID,
                                       TNC_ConnectionID connectionID,
                                       TNC_ConnectionState newState);
  TNC_Result (*BeginHandshake)(TNC_IMCID imcID, TNC_ConnectionID connectionID);
  TNC_Result (*ReceiveMessage)(TNC_IMCID imcID, TNC_ConnectionID connectionID,
  ...
  TNC_Result (*ProvideBindFunction)(TNC_IMCID imcID,
...
static TNC_Result
TNC_TNCC_ReportMessageTypes(TNC_IMCID imcID, TNC_MessageTypeList supportedTypes,
...
static TNC_Result TNC_TNCC_SendMessage(TNC_IMCID imcID,
                                       TNC_ConnectionID connectionID,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "TNC: TNC_TNCC_SendMessage", message,
                    messageLength);
  ...
  imc->imc_send_len = os_snprintf((char *)imc->imc_send, b64len + 100,
...
static TNC_Result TNC_TNCC_RequestHandshakeRetry(TNC_IMCID imcID,
                                                 TNC_ConnectionID connectionID,
...
static TNC_Result TNC_TNCC_BindFunction(TNC_IMCID imcID, char *functionName,
                                        void **pOutfunctionPointer) {
  wpa_printf(MSG_DEBUG,
             "TNC: TNC_TNCC_BindFunction(imcID=%lu, "
             "functionName='%s')",
             (unsigned long)imcID, functionName);
...
static int tncc_imc_initialize(struct tnc_if_imc *imc) {
  ...
  wpa_printf(MSG_DEBUG, "TNC: Calling TNC_IMC_Initialize for IMC '%s'",
             imc->name);
  res = imc->Initialize(imc->imcID, TNC_IFIMC_VERSION_1, TNC_IFIMC_VERSION_1,
                        &imc_ver);
...
static int tncc_load_imc(struct tnc_if_imc *imc) {
  ...
  if (imc->dlhandle == NULL) {
    wpa_printf(MSG_ERROR, "TNC: Failed to open IMC '%s' (%s): %d", imc->name,
               imc->path, (int)GetLastError());
    return -1;
  }
#else  /* CONFIG_NATIVE_WINDOWS */
  imc->dlhandle = dlopen(imc->path, RTLD_LAZY);
  if (imc->dlhandle == NULL) {
    wpa_printf(MSG_ERROR, "TNC: Failed to open IMC '%s' (%s): %s", imc->name,
               imc->path, dlerror());
    return -1;
  }
#endif /* CONFIG_NATIVE_WINDOWS */
...
enum tncc_process_res tncc_process_if_tnccs(struct tncc_data *tncc,
  ...
  wpa_hexdump_ascii(MSG_MSGDUMP, "TNC: Received IF-TNCCS message", msg, len);
...
static struct wpabuf *tncc_build_soh(int ver) {
  ...
  if (os_get_random(correlation_id, sizeof(correlation_id)))
    return NULL;
  wpa_hexdump(MSG_DEBUG, "TNC: SoH Correlation ID", correlation_id,
