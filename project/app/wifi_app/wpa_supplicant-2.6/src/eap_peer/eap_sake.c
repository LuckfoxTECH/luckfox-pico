...
static void eap_sake_state(struct eap_sake_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-SAKE: %s -> %s", eap_sake_state_txt(data->state),
...
static void *eap_sake_init(struct eap_sm *sm) {
  ...
  os_memcpy(data->root_secret_b, password + EAP_SAKE_ROOT_SECRET_LEN,
...
static struct wpabuf *eap_sake_build_msg(struct eap_sake_data *data, int id,
                                         size_t length, u8 subtype) {
  ...
  msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_SAKE, plen, EAP_CODE_RESPONSE,
                      id);
  if (msg == NULL) {
...
static struct wpabuf *eap_sake_process_identity(struct eap_sm *sm,
  ...
  eap_sake_add_attr(resp, EAP_SAKE_AT_PEERID, data->peerid, data->peerid_len);
...
static struct wpabuf *eap_sake_process_challenge(struct eap_sm *sm,
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-SAKE: RAND_S (server rand)", data->rand_s,
              EAP_SAKE_RAND_LEN);
  ...
  wpa_hexdump(MSG_MSGDUMP, "EAP-SAKE: RAND_P (peer rand)", data->rand_p,
              EAP_SAKE_RAND_LEN);
  ...
  data->serverid_len = 0;
  if (attr.serverid) {
    wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-SAKE: SERVERID", attr.serverid,
                      attr.serverid_len);
  ...
  eap_sake_derive_keys(data->root_secret_a, data->root_secret_b, data->rand_s,
                       data->rand_p, (u8 *)&data->tek, data->msk, data->emsk);
  ...
  eap_sake_add_attr(resp, EAP_SAKE_AT_RAND_P, data->rand_p, EAP_SAKE_RAND_LEN);
  ...
  if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
                           data->serverid, data->serverid_len, data->peerid,
                           data->peerid_len, 1, wpabuf_head(resp),
                           wpabuf_len(resp), rpos, rpos)) {
...
eap_sake_process_confirm(struct eap_sm *sm, struct eap_sake_data *data,
                         struct eap_method_ret *ret, u8 id,
                         const struct wpabuf *reqData, const u8 *payload,
                         size_t payload_len) {
  struct eap_sake_parse_attr attr;
  ...
  if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p,
                           data->serverid, data->serverid_len, data->peerid,
                           data->peerid_len, 0, wpabuf_head(reqData),
                           wpabuf_len(reqData), attr.mic_s, mic_s)) {
    wpa_printf(MSG_INFO, "EAP-SAKE: Failed to compute MIC");
    ...
    return eap_sake_build_msg(data, id, 0, EAP_SAKE_SUBTYPE_AUTH_REJECT);
  }
...
static struct wpabuf *eap_sake_process(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "session_id %d",
             subtype, session_id);
  wpa_hexdump(MSG_DEBUG, "EAP-SAKE: Received attributes", pos, end - pos);

  if (data->session_id_set && data->session_id != session_id) {
    wpa_printf(MSG_INFO, "EAP-SAKE: Session ID mismatch (%d,%d)", session_id,
               data->session_id);
    ret->ignore = TRUE;
    return NULL;
  }
  ...
  case EAP_SAKE_SUBTYPE_IDENTITY:
    resp = eap_sake_process_identity(sm, data, ret, id, pos, end - pos);
    break;
  case EAP_SAKE_SUBTYPE_CHALLENGE:
    resp = eap_sake_process_challenge(sm, data, ret, id, pos, end - pos);
    break;
  case EAP_SAKE_SUBTYPE_CONFIRM:
    resp = eap_sake_process_confirm(sm, data, ret, id, reqData, pos, end - pos);
    break;
  default:
    wpa_printf(MSG_DEBUG,
