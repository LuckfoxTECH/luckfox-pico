...
u8 *hostapd_eid_vht_capabilities(struct hostapd_data *hapd, u8 *eid, u32 nsts) {
  ...
  if (mode->mode == HOSTAPD_MODE_IEEE80211G && hapd->conf->vendor_vht &&
    ...
    for (i = 0; i < hapd->iface->num_hw_features; i++) {
      if (hapd->iface->hw_features[i].mode == HOSTAPD_MODE_IEEE80211A) {
  ...
  *pos++ = sizeof(*cap);
  ...
  cap->vht_capabilities_info = host_to_le32(hapd->iface->conf->vht_capab);
  ...
  if (nsts != 0) {
    ...
    cap->vht_capabilities_info &=
        ~(host_to_le32(hapd_nsts << VHT_CAP_BEAMFORMEE_STS_OFFSET));
    cap->vht_capabilities_info |=
...
u8 *hostapd_eid_vht_operation(struct hostapd_data *hapd, u8 *eid) {
  ...
  *pos++ = sizeof(*oper);
  ...
  if (hapd->iconf->vht_oper_chwidth == 2) {
    ...
    oper->vht_op_info_chan_center_freq_seg1_idx =
        oper->vht_op_info_chan_center_freq_seg0_idx;
    if (hapd->iconf->channel < hapd->iconf->vht_oper_centr_freq_seg0_idx)
...
static int check_valid_vht_mcs(struct hostapd_hw_modes *mode,
  ...
  wpa_printf(MSG_DEBUG, "No matching VHT MCS found between AP TX and STA RX");
  return 0;
}
...
u8 *hostapd_eid_wb_chsw_wrapper(struct hostapd_data *hapd, u8 *eid) {
  ...
  if (!hapd->cs_freq_params.channel || !hapd->cs_freq_params.vht_enabled)
    return eid;
  ...
  freq1 = hapd->cs_freq_params.center_freq1 ? hapd->cs_freq_params.center_freq1
                                            : hapd->cs_freq_params.freq;
  if (ieee80211_freq_to_chan(freq1, &chan1) != HOSTAPD_MODE_IEEE80211A)
    return eid;

  if (hapd->cs_freq_params.center_freq2 &&
      ieee80211_freq_to_chan(hapd->cs_freq_params.center_freq2, &chan2) !=
...
u16 copy_sta_vht_capab(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (!vht_capab || hapd->conf->disable_11ac ||
...
u8 *hostapd_eid_vendor_vht(struct hostapd_data *hapd, u8 *eid) {
  ...
  *pos++ = (5 + /* The Vendor OUI, type and subtype */
            2 + sizeof(struct ieee80211_vht_capabilities) + 2 +
...
void hostapd_get_vht_capab(struct hostapd_data *hapd,
  ...
  if (!(own_cap & VHT_CAP_SU_BEAMFORMER_CAPABLE))
    cap &= ~(VHT_CAP_SU_BEAMFORMEE_CAPABLE | VHT_CAP_BEAMFORMEE_STS_MAX);

  if (!(own_cap & VHT_CAP_SU_BEAMFORMEE_CAPABLE))
    cap &= ~(VHT_CAP_SU_BEAMFORMER_CAPABLE | VHT_CAP_SOUNDING_DIMENSION_MAX);
