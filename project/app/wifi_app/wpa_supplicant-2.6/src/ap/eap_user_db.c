...
static void set_user_methods(struct hostapd_eap_user *user,
                             const char *methods) {
  char *buf, *start;
  int num_methods;
  ...
  while (*start) {
    ...
    if (user->methods[num_methods].vendor == EAP_VENDOR_IETF &&
      ...
      wpa_printf(MSG_INFO, "DB: Unsupported EAP type '%s'", start);
      os_free(buf);
      return;
    }
...
eap_user_sqlite_get(struct hostapd_data *hapd, const u8 *identity,
  ...
  if (identity_len >= sizeof(id_str)) {
    wpa_printf(MSG_DEBUG, "%s: identity len too big: %d >= %d", __func__,
               (int)identity_len, (int)(sizeof(id_str)));
    return NULL;
  }
  ...
  os_snprintf(cmd, sizeof(cmd),
              "SELECT * FROM users WHERE identity='%s' AND phase2=%d;", id_str,
              phase2);
  ...
  if (user == NULL && !phase2) {
    os_snprintf(cmd, sizeof(cmd), "SELECT identity,methods FROM wildcards;");
    wpa_printf(MSG_DEBUG, "DB: %s", cmd);
    if (sqlite3_exec(db, cmd, get_wildcard_cb, &hapd->tmp_eap_user, NULL) !=
        SQLITE_OK) {
      wpa_printf(MSG_DEBUG, "DB: Failed to complete SQL operation: %s  db: %s",
                 sqlite3_errmsg(db), hapd->conf->eap_user_sqlite);
    } else if (hapd->tmp_eap_user.next) {
...
const struct hostapd_eap_user *hostapd_get_eap_user(struct hostapd_data *hapd,
                                                    const u8 *identity,
                                                    size_t identity_len,
                                                    int phase2) {
  ...
  if (conf->wps_state && identity_len == WSC_ID_ENROLLEE_LEN &&
    ...
    wsc_enrollee.methods[0].method =
        eap_server_get_type("WSC", &wsc_enrollee.methods[0].vendor);
    return &wsc_enrollee;
  }
  ...
  if (conf->wps_state && identity_len == WSC_ID_REGISTRAR_LEN &&
    ...
    wsc_registrar.methods[0].method =
        eap_server_get_type("WSC", &wsc_registrar.methods[0].vendor);
    wsc_registrar.password = (u8 *)conf->ap_pin;
    wsc_registrar.password_len = conf->ap_pin ? os_strlen(conf->ap_pin) : 0;
  ...
  while (user) {
    ...
    if (user->phase2 == !!phase2 && user->wildcard_prefix &&
        ...
        os_memcmp(user->identity, identity, user->identity_len) == 0) {
      /* Wildcard prefix match */
      break;
    }

    if (user->phase2 == !!phase2 && user->identity_len == identity_len &&
  ...
  if (user == NULL && conf->eap_user_sqlite) {
    return eap_user_sqlite_get(hapd, identity, identity_len, phase2);
  }
#endif /* CONFIG_SQLITE */
