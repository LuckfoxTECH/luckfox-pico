...
static void hostapd_wpa_auth_conf(struct hostapd_bss_config *conf,
  ...
  if (conf->own_ie_override &&
    ...
    os_memcpy(wconf->own_ie_override, wpabuf_head(conf->own_ie_override),
...
static void hostapd_wpa_auth_set_eapol(void *ctx, const u8 *addr,
  ...
  case WPA_EAPOL_keyAvailable:
    if (sta->eapol_sm)
      sta->eapol_sm->eap_if->eapKeyAvailable = value ? TRUE : FALSE;
    break;
  ...
  if (sta && sta->psk && !psk) {
    ...
    for (pos = sta->psk; pos; pos = pos->next) {
      if (pos->is_passphrase) {
        pbkdf2_sha1(pos->passphrase, hapd->conf->ssid.ssid,
                    hapd->conf->ssid.ssid_len, 4096, pos->psk, PMK_LEN);
        pos->is_passphrase = 0;
      }
      if (pos->psk == prev_psk) {
...
static int hostapd_wpa_auth_set_key(void *ctx, int vlan_id, enum wpa_alg alg,
  ...
  return hostapd_drv_set_key(ifname, hapd, alg, addr, idx, 1, NULL, 0, key,
                             key_len);
}
...
static int hostapd_wpa_auth_send_eapol(void *ctx, const u8 *addr,
  ...
  if (hapd->ext_eapol_frame_io) {
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, "EAPOL-TX " MACSTR " %s", MAC2STR(addr),
            hex);
    os_free(hex);
    return 0;
  }
  ...
  return hostapd_drv_hapd_send_eapol(hapd, addr, data, data_len, encrypt,
                                     flags);
}
  ...
  for (j = 0; j < iface->num_bss; j++) {
    ...
    if (os_memcmp(hapd->own_addr, idata->dst, ETH_ALEN) == 0) {
      wpa_printf(MSG_DEBUG,
                 ...
                 "locally managed BSS " MACSTR "@%s -> " MACSTR "@%s",
                 MAC2STR(idata->src_hapd->own_addr),
                 idata->src_hapd->conf->iface, MAC2STR(hapd->own_addr),
                 hapd->conf->iface);
      wpa_ft_rrb_rx(hapd->wpa_auth, idata->src_hapd->own_addr, idata->data,
                    idata->data_len);
      return 1;
    }
  }
...
static int hostapd_wpa_auth_send_ether(void *ctx, const u8 *dst, u16 proto,
  ...
  if (hapd->ext_eapol_frame_io && proto == ETH_P_EAPOL) {
    ...
    if (hex == NULL)
      return -1;
    wpa_snprintf_hex(hex, hex_len, data, data_len);
    wpa_msg(hapd->msg_ctx, MSG_INFO, "EAPOL-TX " MACSTR " %s", MAC2STR(dst),
            hex);
    os_free(hex);
    return 0;
  }
  ...
  if (proto == ETH_P_RRB && hapd->iface->interfaces &&
    ...
    res = hapd->iface->interfaces->for_each_interface(
        hapd->iface->interfaces, hostapd_wpa_auth_ft_iter, &idata);
    if (res == 1)
      return data_len;
  }
#endif /* CONFIG_IEEE80211R */
  ...
  if (hapd->driver && hapd->driver->send_ether)
    return hapd->driver->send_ether(hapd->drv_priv, dst, hapd->own_addr, proto,
                                    data, data_len);
  if (hapd->l2 == NULL)
    return -1;
  ...
  ret =
      l2_packet_send(hapd->l2, dst, proto, (u8 *)buf, sizeof(*buf) + data_len);
  os_free(buf);
  return ret;
}
...
static int hostapd_wpa_auth_send_ft_action(void *ctx, const u8 *dst,
  ...
  m->frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT, WLAN_FC_STYPE_ACTION);
...
static struct wpa_state_machine *hostapd_wpa_auth_add_sta(void *ctx,
                                                          const u8 *sta_addr) {
  struct hostapd_data *hapd = ctx;
  struct sta_info *sta;
...
static void hostapd_rrb_receive(void *ctx, const u8 *src_addr, const u8 *buf,
  ...
  wpa_printf(MSG_DEBUG, "FT: RRB received packet " MACSTR " -> " MACSTR,
...
#ifdef CONFIG_IEEE80211R
  if (!hostapd_drv_none(hapd) && wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt)) {
    hapd->l2 = l2_packet_init(hapd->conf->bridge[0] ? hapd->conf->bridge
                                                    : hapd->conf->iface,
                              NULL, ETH_P_RRB, hostapd_rrb_receive, hapd, 1);
    if (hapd->l2 == NULL &&
...
void hostapd_deinit_wpa(struct hostapd_data *hapd) {
  ...
  if (hapd->wpa_auth) {
    ...
    if (hapd->drv_priv && hostapd_set_generic_elem(hapd, (u8 *)"", 0)) {
      wpa_printf(MSG_DEBUG,
