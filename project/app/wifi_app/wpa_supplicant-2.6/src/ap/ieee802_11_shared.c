...
void ieee802_11_send_sa_query_req(struct hostapd_data *hapd, const u8 *addr,
                                  const u8 *trans_id) {
  struct ieee80211_mgmt mgmt;
  u8 *end;

  wpa_printf(MSG_DEBUG, "IEEE 802.11: Sending SA Query Request to " MACSTR,
             MAC2STR(addr));
  wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID", trans_id,
...
static void ieee802_11_send_sa_query_resp(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG, "IEEE 802.11: Received SA Query Request from " MACSTR,
             MAC2STR(sa));
  wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID", trans_id,
              WLAN_SA_QUERY_TR_ID_LEN);
  ...
  wpa_printf(MSG_DEBUG, "IEEE 802.11: Sending SA Query Response to " MACSTR,
             MAC2STR(sa));
...
void ieee802_11_sa_query_action(struct hostapd_data *hapd, const u8 *sa,
  ...
  wpa_printf(MSG_DEBUG, "IEEE 802.11: Received SA Query Response from " MACSTR,
             MAC2STR(sa));
  wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID", trans_id,
              WLAN_SA_QUERY_TR_ID_LEN);
  ...
  for (i = 0; i < sta->sa_query_count; i++) {
    if (os_memcmp(sta->sa_query_trans_id + i * WLAN_SA_QUERY_TR_ID_LEN,
...
static void hostapd_ext_capab_byte(struct hostapd_data *hapd, u8 *pos,
                                   int idx) {
  *pos = 0x00;
...
u8 *hostapd_eid_ext_capab(struct hostapd_data *hapd, u8 *eid) {
  ...
  if (len < 7 && hapd->conf->ssid.utf8_ssid)
    len = 7;
  if (len < 9 && (hapd->conf->ftm_initiator || hapd->conf->ftm_responder))
    len = 9;
#ifdef CONFIG_WNM
  if (len < 4)
    len = 4;
...
u8 *hostapd_eid_roaming_consortium(struct hostapd_data *hapd, u8 *eid) {
  ...
  if (!hapd->conf->interworking || hapd->conf->roaming_consortium == NULL ||
...
void ap_copy_sta_supp_op_classes(struct sta_info *sta,
  ...
  os_memcpy(sta->supp_op_classes + 1, supp_op_classes, supp_op_classes_len);
}
