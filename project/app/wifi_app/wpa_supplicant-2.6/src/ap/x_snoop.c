...
int x_snoop_init(struct hostapd_data *hapd) {
  ...
  if (!conf->isolate) {
    wpa_printf(MSG_DEBUG, "x_snoop: ap_isolate must be enabled for x_snoop");
    return -1;
  }

  if (conf->bridge[0] == '\0') {
    wpa_printf(MSG_DEBUG, "x_snoop: Bridge must be configured for x_snoop");
    return -1;
  }

  if (hostapd_drv_br_port_set_attr(hapd, DRV_BR_PORT_ATTR_HAIRPIN_MODE, 1)) {
  ...
  if (hostapd_drv_br_set_net_param(hapd, DRV_BR_NET_PARAM_GARP_ACCEPT, 1)) {
    wpa_printf(
        MSG_DEBUG,
...
struct l2_packet_data *x_snoop_get_l2_packet(
  ...
  if (l2_packet_set_packet_filter(l2, type)) {
    wpa_printf(MSG_DEBUG,
               "x_snoop: Failed to set L2 packet filter for type: %d", type);
    l2_packet_deinit(l2);
    return NULL;
  }
...
void x_snoop_mcast_to_ucast_convert_send(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_EXCESSIVE,
             "x_snoop: Multicast-to-unicast conversion " MACSTR " -> " MACSTR
             " (len %u)",
  ...
  if (res < 0) {
    wpa_printf(
        MSG_DEBUG,
        "x_snoop: Failed to send mcast to ucast converted packet to " MACSTR,
        MAC2STR(sta->addr));
  }
