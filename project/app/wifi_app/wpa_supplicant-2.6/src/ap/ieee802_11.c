...
u8 *hostapd_eid_ext_supp_rates(struct hostapd_data *hapd, u8 *eid) {
  ...
  *pos++ = num;
  for (i = 0, count = 0; i < hapd->iface->num_rates && count < num + 8; i++) {
    count++;
    if (count <= 8)
...
static u16 auth_shared_key(struct hostapd_data *hapd, struct sta_info *sta,
                           ...
                           int iswep) {
  hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
...
static int send_auth_reply(struct hostapd_data *hapd, const u8 *dst,
                           const u8 *bssid, u16 auth_alg, u16 auth_transaction,
                           u16 resp, const u8 *ies, size_t ies_len) {
  ...
  reply->frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT, WLAN_FC_STYPE_AUTH);
  ...
  wpa_printf(MSG_DEBUG,
             ...
             MAC2STR(dst), auth_alg, auth_transaction, resp,
...
static void handle_auth_ft_finish(void *ctx, const u8 *dst, const u8 *bssid,
  ...
  if (sta->added_unassoc &&
      (reply_res != WLAN_STATUS_SUCCESS || status != WLAN_STATUS_SUCCESS)) {
  ...
  hostapd_logger(hapd, dst, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
...
static struct wpabuf *auth_build_sae_commit(struct hostapd_data *hapd,
  ...
  if (update && sae_prepare_commit(hapd->own_addr, sta->addr,
  ...
  sae_write_commit(sta->sae, buf,
                   sta->sae->tmp ? sta->sae->tmp->anti_clogging_token : NULL);
...
static int auth_sae_send_commit(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  reply_res =
      send_auth_reply(hapd, sta->addr, bssid, WLAN_AUTH_SAE, 1,
                      WLAN_STATUS_SUCCESS, wpabuf_head(data), wpabuf_len(data));
...
static int auth_sae_send_confirm(struct hostapd_data *hapd,
                                 struct sta_info *sta, const u8 *bssid) {
  struct wpabuf *data;
  int reply_res;
  ...
  reply_res =
      send_auth_reply(hapd, sta->addr, bssid, WLAN_AUTH_SAE, 2,
                      WLAN_STATUS_SUCCESS, wpabuf_head(data), wpabuf_len(data));
...
static int check_sae_token(struct hostapd_data *hapd, const u8 *addr,
  ...
  if (token_len != SHA256_MAC_LEN)
    return -1;
  if (hmac_sha256(hapd->sae_token_key, sizeof(hapd->sae_token_key), addr,
                  ETH_ALEN, mac) < 0 ||
...
static struct wpabuf *auth_build_token_req(struct hostapd_data *hapd, int group,
                                           const u8 *addr) {
  struct wpabuf *buf;
  u8 *token;
  ...
  if (!os_reltime_initialized(&hapd->last_sae_token_key_update) ||
    ...
    if (random_get_bytes(hapd->sae_token_key, sizeof(hapd->sae_token_key)) < 0)
      return NULL;
    wpa_hexdump(MSG_DEBUG, "SAE: Updated token key", hapd->sae_token_key,
  ...
  hmac_sha256(hapd->sae_token_key, sizeof(hapd->sae_token_key), addr, ETH_ALEN,
              token);
...
static void auth_sae_retransmit_timer(void *eloop_ctx, void *eloop_data) {
  ...
  sta->sae->sync++;
  wpa_printf(MSG_DEBUG,
             "SAE: Auth SAE retransmit timer for " MACSTR " (sync=%d state=%d)",
...
void sae_accept_sta(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  wpa_auth_pmksa_add_sae(hapd->wpa_auth, sta->addr, sta->sae->pmk,
                         sta->sae->pmkid);
}
...
static int sae_sm_step(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  case SAE_NOTHING:
    if (auth_transaction == 1) {
      ...
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_DEBUG, "SAE confirm before commit");
    }
    break;
  case SAE_COMMITTED:
  ...
  case SAE_ACCEPTED:
    if (auth_transaction == 1) {
      wpa_printf(MSG_DEBUG,
                 "SAE: remove the STA (" MACSTR ") doing reauthentication",
                 MAC2STR(sta->addr));
    ...
    break;
  default:
    wpa_printf(MSG_ERROR, "SAE: invalid state %d", sta->sae->state);
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  }
  return WLAN_STATUS_SUCCESS;
}

static void sae_pick_next_group(struct hostapd_data *hapd,
                                struct sta_info *sta) {
  ...
  if (!groups || groups[i] <= 0) {
    wpa_printf(MSG_DEBUG, "SAE: Previously selected group not found from the "
  ...
  for (;;) {
    i++;
    if (groups[i] <= 0) {
      wpa_printf(MSG_DEBUG, "SAE: No alternative group enabled");
      return;
    }
...
static void handle_auth_sae(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (sta->mesh_sae_pmksa_caching) {
    wpa_printf(MSG_DEBUG, "SAE: Cancel use of mesh PMKSA caching because peer "
                          "starts SAE authentication");
    wpa_auth_pmksa_remove(hapd->wpa_auth, sta->addr);
  ...
  if (auth_transaction == 1) {
    ...
    size_t token_len = 0;
    hostapd_logger(
        hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
        "start SAE authentication (RX commit, status=%u)", status_code);

    if ((hapd->conf->mesh & MESH_ENABLED) &&
        status_code == WLAN_STATUS_ANTI_CLOGGING_TOKEN_REQ && sta->sae->tmp) {
      pos = mgmt->u.auth.variable;
      end = ((const u8 *)mgmt) + len;
      if (pos + sizeof(le16) > end) {
        wpa_printf(MSG_ERROR, "SAE: Too short anti-clogging token request");
        resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
        goto reply;
      }
      resp = sae_group_allowed(sta->sae, hapd->conf->sae_groups,
                               WPA_GET_LE16(pos));
    ...
    if ((hapd->conf->mesh & MESH_ENABLED) &&
        ...
        sta->sae->tmp) {
      wpa_printf(MSG_DEBUG, "SAE: Peer did not accept our SAE group");
    ...
    resp = sae_parse_commit(sta->sae, mgmt->u.auth.variable,
                            ((const u8 *)mgmt) + len - mgmt->u.auth.variable,
                            &token, &token_len, hapd->conf->sae_groups);
    if (resp == SAE_SILENTLY_DISCARD) {
      wpa_printf(MSG_DEBUG,
                 "SAE: Drop commit message from " MACSTR
                 " due to reflection attack",
                 MAC2STR(sta->addr));
      goto remove_sta;
    }
    if (token && check_sae_token(hapd, sta->addr, token, token_len) < 0) {
      wpa_printf(MSG_DEBUG,
    ...
    if (!token && use_sae_anti_clogging(hapd)) {
      wpa_printf(MSG_DEBUG, "SAE: Request anti-clogging token from " MACSTR,
                 MAC2STR(sta->addr));
      data = auth_build_token_req(hapd, sta->sae->group, sta->addr);
    ...
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG,
                   "SAE authentication (RX confirm, status=%u)", status_code);
    if (status_code != WLAN_STATUS_SUCCESS)
      goto remove_sta;
    if (sta->sae->state >= SAE_CONFIRMED ||
        !(hapd->conf->mesh & MESH_ENABLED)) {
      if (sae_check_confirm(sta->sae, mgmt->u.auth.variable,
                            ((u8 *)mgmt) + len - mgmt->u.auth.variable) < 0) {
        resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
        goto reply;
      }
    }
...
reply:
  if (resp != WLAN_STATUS_SUCCESS) {
    send_auth_reply(hapd, mgmt->sa, mgmt->bssid, WLAN_AUTH_SAE,
                    auth_transaction, resp, data ? wpabuf_head(data) : (u8 *)"",
                    data ? wpabuf_len(data) : 0);
  }

remove_sta:
  if (sta->added_unassoc &&
      (resp != WLAN_STATUS_SUCCESS || status_code != WLAN_STATUS_SUCCESS)) {
    hostapd_drv_sta_remove(hapd, sta->addr);
    sta->added_unassoc = 0;
  }
  wpabuf_free(data);
...
static void handle_auth(struct hostapd_data *hapd,
  ...
  if (hapd->iconf->ignore_auth_probability > 0.0 &&
    ...
    wpa_printf(MSG_INFO, "TESTING: ignoring auth frame from " MACSTR,
               MAC2STR(mgmt->sa));
    return;
  }
  ...
  if (len >= IEEE80211_HDRLEN + sizeof(mgmt->u.auth) + 2 +
  ...
  wpa_printf(MSG_DEBUG,
             ...
             !!(fc & WLAN_FC_ISWEP), challenge ? " challenge" : "", seq_ctrl,
             (fc & WLAN_FC_RETRY) ? " retry" : "");

#ifdef CONFIG_NO_RC4
  if (auth_alg == WLAN_AUTH_SHARED_KEY) {
    wpa_printf(MSG_INFO, "Unsupported authentication algorithm (%d)", auth_alg);
  ...
  if (hapd->conf->no_auth_if_seen_on) {
    ...
    if (other) {
      ...
      wpa_printf(MSG_DEBUG,
                 "%s: Reject authentication from " MACSTR
      ...
      *pos++ = 13;
      ...
      if (other->iconf->hw_mode == HOSTAPD_MODE_IEEE80211AD)
        ...
        phytype = 7; /* ht */
      else if (other->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A)
        phytype = 4; /* ofdm */
      else if (other->iconf->hw_mode == HOSTAPD_MODE_IEEE80211G)
        phytype = 6; /* erp */
      else
        phytype = 5; /* hrdsss */
      if (ieee80211_freq_to_channel_ext(
              hostapd_hw_get_freq(other, other->iconf->channel),
              other->iconf->secondary_channel, other->iconf->ieee80211ac,
  ...
  if (sta) {
    if ((fc & WLAN_FC_RETRY) && sta->last_seq_ctrl != WLAN_INVALID_MGMT_SEQ &&
        sta->last_seq_ctrl == seq_ctrl &&
        sta->last_subtype == WLAN_FC_STYPE_AUTH) {
      hostapd_logger(
          hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
          "Drop repeated authentication frame seq_ctrl=0x%x", seq_ctrl);
      return;
    }
#ifdef CONFIG_MESH
    if ((hapd->conf->mesh & MESH_ENABLED) &&
      ...
      wpa_printf(MSG_DEBUG,
                 "Mesh peer " MACSTR " is blocked - drop Authentication frame",
                 MAC2STR(mgmt->sa));
      return;
    }
#endif /* CONFIG_MESH */
  ...
  if (vlan_id.notempty && !hostapd_vlan_valid(hapd->conf->vlan, &vlan_id)) {
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
                   "Invalid VLAN %d%s received from RADIUS server",
                   vlan_id.untagged, vlan_id.tagged[0] ? "+" : "");
    resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
    goto fail;
  }
  ...
  if (FULL_AP_CLIENT_STATE_SUPP(hapd->iface->drv_flags) &&
    ...
    if (hostapd_sta_add(hapd, sta->addr, 0, 0, NULL, 0, 0, NULL, NULL,
                        sta->flags, 0, 0, 0, 0)) {
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
  ...
  case WLAN_AUTH_OPEN:
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG, "authentication OK (open system)");
    sta->flags |= WLAN_STA_AUTH;
    wpa_auth_sm_event(sta->wpa_sm, WPA_AUTH);
  ...
  case WLAN_AUTH_SHARED_KEY:
    ...
    if (sta->challenge && auth_transaction == 1) {
      ...
      os_memcpy(resp_ies + 2, sta->challenge, WLAN_AUTH_CHALLENGE_LEN);
  ...
  case WLAN_AUTH_FT:
    ...
    wpa_ft_process_auth(sta->wpa_sm, mgmt->bssid, auth_transaction,
  ...
  case WLAN_AUTH_SAE:
#ifdef CONFIG_MESH
    if (status_code == WLAN_STATUS_SUCCESS && hapd->conf->mesh & MESH_ENABLED) {
      if (sta->wpa_sm == NULL)
        sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth, sta->addr, NULL);
      if (sta->wpa_sm == NULL) {
        wpa_printf(MSG_DEBUG, "SAE: Failed to initialize WPA state machine");
        resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
        goto fail;
      }
    }
...
fail:
  ...
  if (sta && sta->added_unassoc &&
      (resp != WLAN_STATUS_SUCCESS || reply_res != WLAN_STATUS_SUCCESS)) {
    hostapd_drv_sta_remove(hapd, sta->addr);
    sta->added_unassoc = 0;
  }
}
...
static u16 check_wmm(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (wmm_ie && hapd->conf->wmm_enabled) {
    ...
    if (!hostapd_eid_wmm_valid(hapd, wmm_ie, wmm_ie_len)) {
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_WPA, HOSTAPD_LEVEL_DEBUG,
...
static u16 copy_supp_rates(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (elems->supp_rates_len + elems->ext_supp_rates_len >
    ...
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                   ...
                   elems->supp_rates_len, elems->ext_supp_rates_len);
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  }

  sta->supported_rates_len = merge_byte_arrays(
      sta->supported_rates, sizeof(sta->supported_rates), elems->supp_rates,
      elems->supp_rates_len, elems->ext_supp_rates, elems->ext_supp_rates_len);
...
static u16 check_assoc_ies(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (hapd->conf->vendor_vht && !elems.vht_capabilities) {
    resp =
        copy_sta_vendor_vht(hapd, sta, elems.vendor_vht, elems.vendor_vht_len);
    if (resp != WLAN_STATUS_SUCCESS)
      return resp;
  }
  ...
  if (elems.p2p) {
    ...
    sta->p2p_ie = ieee802_11_vendor_ie_concat(ies, ies_len, P2P_IE_VENDOR_TYPE);
    if (sta->p2p_ie)
  ...
  if (hapd->conf->wps_state && elems.wps_ie) {
    ...
    sta->wps_ie = ieee802_11_vendor_ie_concat(ies, ies_len, WPS_IE_VENDOR_TYPE);
  ...
  if (hapd->conf->wpa && wpa_ie) {
    ...
    wpa_ie_len += 2;
    if (sta->wpa_sm == NULL)
      sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth, sta->addr, p2p_dev_addr);
    if (sta->wpa_sm == NULL) {
    ...
    if (sta->auth_alg == WLAN_AUTH_FT) {
      ...
      resp = wpa_ft_validate_reassoc(sta->wpa_sm, ies, ies_len);
      if (resp != WLAN_STATUS_SUCCESS)
        return resp;
    }
#endif /* CONFIG_IEEE80211R */
...
#ifdef CONFIG_SAE
    if (wpa_auth_uses_sae(sta->wpa_sm) && sta->auth_alg == WLAN_AUTH_OPEN) {
      struct rsn_pmksa_cache_entry *sa;
      sa = wpa_auth_sta_get_pmksa(sta->wpa_sm);
      if (!sa || sa->akmp != WPA_KEY_MGMT_SAE) {
        wpa_printf(MSG_DEBUG, "SAE: No PMKSA cache entry found for " MACSTR,
                   MAC2STR(sta->addr));
        return WLAN_STATUS_INVALID_PMKID;
      }
      wpa_printf(MSG_DEBUG, "SAE: " MACSTR " using PMKSA caching",
                 MAC2STR(sta->addr));
    ...
    if (wpa_validate_osen(hapd->wpa_auth, sta->wpa_sm, elems.osen - 2,
                          elems.osen_len + 2) < 0)
      return WLAN_STATUS_INVALID_IE;
#endif /* CONFIG_HS20 */
  } else
  ...
  if (elems.hs20 && elems.hs20_len > 4) {
    sta->hs20_ie = wpabuf_alloc_copy(elems.hs20 + 4, elems.hs20_len - 4);
  } else
    sta->hs20_ie = NULL;
#endif /* CONFIG_HS20 */
  ...
  if (hapd->conf->mbo_enabled && (hapd->conf->wpa & 2) && elems.mbo &&
      sta->cell_capa && !(sta->flags & WLAN_STA_MFP) &&
      hapd->conf->ieee80211w != NO_MGMT_FRAME_PROTECTION) {
    wpa_printf(MSG_INFO, "MBO: Reject WPA2 association without PMF");
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  }
#endif /* CONFIG_MBO */
...
static void send_deauth(struct hostapd_data *hapd, const u8 *addr,
  ...
  reply.frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT, WLAN_FC_STYPE_DEAUTH);
  ...
  if (hostapd_drv_send_mlme(hapd, &reply, send_len, 0) < 0)
    wpa_printf(MSG_INFO, "Failed to send deauth: %s", strerror(errno));
}

static int add_associated_sta(struct hostapd_data *hapd, struct sta_info *sta) {
  ...
  if (hostapd_sta_add(hapd, sta->addr, sta->aid, sta->capability,
                      ...
                      sta->vht_opmode, sta->p2p_ie ? 1 : 0, set)) {
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
...
static u16 send_assoc_resp(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  reply = (struct ieee80211_mgmt *)buf;
  reply->frame_control =
      IEEE80211_FC(WLAN_FC_TYPE_MGMT, (reassoc ? WLAN_FC_STYPE_REASSOC_RESP
  ...
  reply->u.assoc_resp.capab_info = host_to_le16(hostapd_own_capab_info(hapd));
  ...
  if (status_code == WLAN_STATUS_SUCCESS) {
    ...
    p = wpa_sm_write_assoc_resp_ies(sta->wpa_sm, p, buf + sizeof(buf) - p,
  ...
  if (hapd->iconf->ieee80211ac && !hapd->conf->disable_11ac) {
    ...
    if (hapd->conf->use_sta_nsts && sta->vht_capabilities) {
      ...
      nsts =
          (hapd->iface->conf->vht_capab >> VHT_CAP_BEAMFORMEE_STS_OFFSET) & 7;
  ...
  if (sta->p2p_ie && hapd->p2p_group) {
    ...
    if (p2p_resp_ie) {
      os_memcpy(p, wpabuf_head(p2p_resp_ie), wpabuf_len(p2p_resp_ie));
...
static void handle_assoc(struct hostapd_data *hapd,
  ...
  if (reassoc) {
    if (hapd->iconf->ignore_reassoc_probability > 0.0 &&
      ...
      wpa_printf(MSG_INFO, "TESTING: ignoring reassoc request from " MACSTR,
                 MAC2STR(mgmt->sa));
      return;
    }
  } else {
    if (hapd->iconf->ignore_assoc_probability > 0.0 &&
      ...
      wpa_printf(MSG_INFO, "TESTING: ignoring assoc request from " MACSTR,
                 MAC2STR(mgmt->sa));
      return;
    }
  }
  ...
  if (reassoc) {
    ...
    listen_interval = le_to_host16(mgmt->u.reassoc_req.listen_interval);
    wpa_printf(MSG_DEBUG,
               "reassociation request: STA=" MACSTR
               " capab_info=0x%02x listen_interval=%d current_ap=" MACSTR
               " seq_ctrl=0x%x%s",
               MAC2STR(mgmt->sa), capab_info, listen_interval,
               MAC2STR(mgmt->u.reassoc_req.current_ap), seq_ctrl,
               (fc & WLAN_FC_RETRY) ? " retry" : "");
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "seq_ctrl=0x%x%s",
               MAC2STR(mgmt->sa), capab_info, listen_interval, seq_ctrl,
               (fc & WLAN_FC_RETRY) ? " retry" : "");
  ...
  if (sta && sta->auth_alg == WLAN_AUTH_FT &&
    ...
    if (sta == NULL || (sta->flags & WLAN_STA_AUTH) == 0) {
      hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                     ...
                     "(aid=%d flags=0x%x)",
                     sta ? sta->aid : -1, sta ? sta->flags : 0);
      send_deauth(hapd, mgmt->sa, WLAN_REASON_CLASS2_FRAME_FROM_NONAUTH_STA);
      return;
    }

  if ((fc & WLAN_FC_RETRY) && sta->last_seq_ctrl != WLAN_INVALID_MGMT_SEQ &&
              sta->last_seq_ctrl == seq_ctrl && sta->last_subtype == reassoc
          ? WLAN_FC_STYPE_REASSOC_REQ
          : WLAN_FC_STYPE_ASSOC_REQ) {
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG,
                   "Drop repeated association frame seq_ctrl=0x%x", seq_ctrl);
    return;
  }
  sta->last_seq_ctrl = seq_ctrl;
  sta->last_subtype =
      reassoc ? WLAN_FC_STYPE_REASSOC_REQ : WLAN_FC_STYPE_ASSOC_REQ;
  ...
  if (listen_interval > hapd->conf->max_listen_interval) {
    hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG, "Too large Listen Interval (%d)",
                   listen_interval);
  ...
  if (hostapd_get_aid(hapd, sta) < 0) {
    hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_INFO,
...
fail:
   ...
   */
  if ((reply_res != WLAN_STATUS_SUCCESS && resp == WLAN_STATUS_SUCCESS) ||
      sta->added_unassoc) {
    hostapd_drv_sta_remove(hapd, sta->addr);
    sta->added_unassoc = 0;
  }
...
static void handle_disassoc(struct hostapd_data *hapd,
  ...
  mlme_disassociate_indication(hapd, sta,
...
static void handle_deauth(struct hostapd_data *hapd,
  ...
  wpa_msg(hapd->msg_ctx, MSG_DEBUG,
          "deauthentication: STA=" MACSTR " reason_code=%d", MAC2STR(mgmt->sa),
          le_to_host16(mgmt->u.deauth.reason_code));

  sta = ap_get_sta(hapd, mgmt->sa);
  if (sta == NULL) {
    wpa_msg(hapd->msg_ctx, MSG_DEBUG,
            "Station " MACSTR " trying "
...
static void handle_beacon(struct hostapd_data *hapd,
  ...
  (void)ieee802_11_parse_elems(
      ...
      len - (IEEE80211_HDRLEN + sizeof(mgmt->u.beacon)), &elems, 0);
...
static int hostapd_sa_query_action(struct hostapd_data *hapd,
  ...
  end = mgmt->u.action.u.sa_query_resp.trans_id + WLAN_SA_QUERY_TR_ID_LEN;
  if (((u8 *)mgmt) + len < end) {
    wpa_printf(MSG_DEBUG,
...
static int robust_action_frame(u8 category) {
  return category != WLAN_ACTION_PUBLIC && category != WLAN_ACTION_HT;
}
#endif /* CONFIG_IEEE80211W */
...
static int handle_action(struct hostapd_data *hapd,
  ...
  if (len < IEEE80211_HDRLEN + 1) {
    hostapd_logger(
        hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
        "handle_action - too short payload (len=%lu)", (unsigned long)len);
    return 0;
  }
  ...
  if (sta) {
    ...
    if ((fc & WLAN_FC_RETRY) && sta->last_seq_ctrl != WLAN_INVALID_MGMT_SEQ &&
      ...
      hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_DEBUG,
                     "Drop repeated action frame seq_ctrl=0x%x", seq_ctrl);
      return 1;
    }
...
#ifdef CONFIG_IEEE80211R
  case WLAN_ACTION_FT:
    if (!sta || wpa_ft_action_rx(sta->wpa_sm, (u8 *)&mgmt->u.action,
  ...
  case WLAN_ACTION_FST:
    if (hapd->iface->fst)
      ...
      wpa_printf(MSG_DEBUG, "FST: Ignore FST Action frame - no FST attached");
    return 1;
#endif /* CONFIG_FST */
  ...
  case WLAN_ACTION_PROTECTED_DUAL:
    ...
    if (len >= IEEE80211_HDRLEN + 2 &&
        mgmt->u.action.u.public_action.action == WLAN_PA_20_40_BSS_COEX) {
      wpa_printf(MSG_DEBUG, "HT20/40 coex mgmt frame received from STA " MACSTR,
                 MAC2STR(mgmt->sa));
      hostapd_2040_coex_action(hapd, mgmt, len);
    }
#endif /* CONFIG_IEEE80211N */
    if (hapd->public_action_cb) {
      hapd->public_action_cb(hapd->public_action_cb_ctx, (u8 *)mgmt, len,
                             hapd->iface->freq);
    }
    if (hapd->public_action_cb2) {
      hapd->public_action_cb2(hapd->public_action_cb2_ctx, (u8 *)mgmt, len,
                              hapd->iface->freq);
    }
  ...
  case WLAN_ACTION_VENDOR_SPECIFIC:
    if (hapd->vendor_action_cb) {
      if (hapd->vendor_action_cb(hapd->vendor_action_cb_ctx, (u8 *)mgmt, len,
                                 hapd->iface->freq) == 0)
        return 1;
    }
    break;
  ...
  hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                 "handle_action - unknown action category %d or invalid "
                 "frame",
                 mgmt->u.action.category);
  if (!is_multicast_ether_addr(mgmt->da) && !(mgmt->u.action.category & 0x80) &&
...
int ieee802_11_mgmt(struct hostapd_data *hapd, const u8 *buf, size_t len,
  ...
  if (!is_broadcast_ether_addr(mgmt->bssid) &&
      ...
      !((hapd->conf->p2p & P2P_GROUP_OWNER) && stype == WLAN_FC_STYPE_ACTION) &&
#endif /* CONFIG_P2P */
#ifdef CONFIG_MESH
      !(hapd->conf->mesh & MESH_ENABLED) &&
#endif /* CONFIG_MESH */
  ...
  if (os_memcmp(mgmt->da, hapd->own_addr, ETH_ALEN) != 0) {
    hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG, "MGMT: DA=" MACSTR " not our address",
                   MAC2STR(mgmt->da));
    return 0;
  }
  ...
  default:
    hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
                   HOSTAPD_LEVEL_DEBUG, "unknown mgmt frame subtype %d", stype);
    break;
  }
...
static void hostapd_set_wds_encryption(struct hostapd_data *hapd,
                                       struct sta_info *sta, char *ifname_wds) {
  int i;
  ...
  for (i = 0; i < 4; i++) {
    if (ssid->wep.key[i] &&
        hostapd_drv_set_key(ifname_wds, hapd, WPA_ALG_WEP, NULL, i,
                            i == ssid->wep.idx, NULL, 0, ssid->wep.key[i],
                            ssid->wep.len[i])) {
      wpa_printf(MSG_WARNING, "Could not set WEP keys for WDS interface; %s",
                 ifname_wds);
      break;
    }
  }
...
static void handle_assoc_cb(struct hostapd_data *hapd,
  ...
  if (sta->flags & WLAN_STA_WDS) {
    ...
    ret = hostapd_set_wds_sta(hapd, ifname_wds, sta->addr, sta->aid, 1);
    if (!ret)
  ...
  if (sta->pending_eapol_rx) {
    ...
    if (age.sec == 0 && age.usec < 200000) {
      wpa_printf(MSG_DEBUG,
                 "Process pending EAPOL frame that was received from " MACSTR
                 " just before association notification",
                 MAC2STR(sta->addr));
      ieee802_1x_receive(hapd, mgmt->da,
...
static void handle_deauth_cb(struct hostapd_data *hapd,
  ...
  sta = ap_get_sta(hapd, mgmt->da);
  if (!sta) {
    wpa_printf(MSG_DEBUG, "handle_deauth_cb: STA " MACSTR " not found",
               MAC2STR(mgmt->da));
    return;
  }
  if (ok)
    ...
    wpa_printf(MSG_DEBUG,
               "STA " MACSTR " did not acknowledge "
               "deauth",
               MAC2STR(sta->addr));
...
static void handle_disassoc_cb(struct hostapd_data *hapd,
  ...
  sta = ap_get_sta(hapd, mgmt->da);
  if (!sta) {
    wpa_printf(MSG_DEBUG, "handle_disassoc_cb: STA " MACSTR " not found",
               MAC2STR(mgmt->da));
    return;
  }
  if (ok)
...
void ieee802_11_mgmt_cb(struct hostapd_data *hapd, const u8 *buf, size_t len,
  ...
  if (hapd->ext_mgmt_frame_handling) {
    wpa_msg(hapd->msg_ctx, MSG_INFO, "MGMT-TX-STATUS stype=%u ok=%d", stype,
            ok);
    return;
  }
#endif /* CONFIG_TESTING_OPTIONS */
...
void hostapd_tx_status(struct hostapd_data *hapd, const u8 *addr, const u8 *buf,
                       size_t len, int ack) {
  struct sta_info *sta;
  ...
  if (sta->flags & WLAN_STA_PENDING_POLL) {
    wpa_printf(MSG_DEBUG,
               "STA " MACSTR " %s pending "
               "activity poll",
               MAC2STR(sta->addr), ack ? "ACKed" : "did not ACK");
    if (ack)
...
void hostapd_eapol_tx_status(struct hostapd_data *hapd, const u8 *dst,
  ...
  if (sta == NULL || !(sta->flags & WLAN_STA_ASSOC)) {
    wpa_printf(MSG_DEBUG,
               "Ignore TX status for Data frame to STA " MACSTR
...
void hostapd_client_poll_ok(struct hostapd_data *hapd, const u8 *addr) {
  ...
  if (sta == NULL)
    return;
  wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_POLL_OK MACSTR, MAC2STR(sta->addr));
  if (!(sta->flags & WLAN_STA_PENDING_POLL))
    return;

  wpa_printf(MSG_DEBUG,
             "STA " MACSTR " ACKed pending "
             "activity poll",
             MAC2STR(sta->addr));
  sta->flags &= ~WLAN_STA_PENDING_POLL;
}
...
void ieee802_11_rx_from_unknown(struct hostapd_data *hapd, const u8 *src,
  ...
  wpa_printf(MSG_DEBUG, "Data/PS-poll frame from not associated STA " MACSTR,
             MAC2STR(src));
  ...
  if (sta && (sta->flags & WLAN_STA_AUTH))
    hostapd_drv_sta_disassoc(hapd, src,
                             WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
  else
    hostapd_drv_sta_deauth(hapd, src,
                           WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
}
