...
#define AID_WORDS ((2008 + 31) / 32)
  ...
  void (*new_assoc_sta_cb)(struct hostapd_data *hapd, struct sta_info *sta,
                           int reassoc);
  ...
  void (*sta_authorized_cb)(void *ctx, const u8 *mac_addr, int authorized,
  ...
  void (*new_psk_cb)(void *ctx, const u8 *mac_addr, const u8 *p2p_dev_addr,
  ...
  int num_plinks;
  int max_plinks;
  void (*mesh_sta_free_cb)(struct hostapd_data *hapd, struct sta_info *sta);
...
void hostapd_switch_channel_fallback(
    struct hostapd_iface *iface, const struct hostapd_freq_params *freq_params);
...
int hostapd_notif_assoc(struct hostapd_data *hapd, const u8 *addr, const u8 *ie,
