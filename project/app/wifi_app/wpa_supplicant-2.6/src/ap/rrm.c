...
static void hostapd_range_rep_timeout_handler(void *eloop_data,
                                              void *user_ctx) {
  struct hostapd_data *hapd = eloop_data;
...
static void hostapd_handle_range_report(struct hostapd_data *hapd, u8 token,
  ...
  if (!hapd->range_req_active || hapd->range_req_token != token) {
    wpa_printf(MSG_DEBUG, "Unexpected range report, token %u", token);
    return;
  }
...
static void hostapd_handle_radio_msmt_report(struct hostapd_data *hapd,
  ...
  while ((ie = get_ie(pos, end - pos, WLAN_EID_MEASURE_REPORT))) {
    ...
    case MEASURE_TYPE_FTM_RANGE:
      ...
      break;
    default:
      wpa_printf(MSG_DEBUG, "Measurement report type %u is not supported",
                 ie[4]);
      break;
    }
...
static int hostapd_check_lci_age(struct hostapd_neighbor_entry *nr,
                                 u16 max_age) {
...
static void hostapd_send_nei_report_resp(struct hostapd_data *hapd,
  ...
  dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry, list) {
    ...
    if (len - 2 > 0xff) {
      wpa_printf(MSG_DEBUG, "NR entry for " MACSTR " exceeds 0xFF bytes",
                 MAC2STR(nr->bssid));
      continue;
    }
  ...
  hostapd_drv_send_action(hapd, hapd->iface->freq, 0, addr, wpabuf_head(buf),
                          wpabuf_len(buf));
  wpabuf_free(buf);
}
...
static void hostapd_handle_nei_report_req(struct hostapd_data *hapd,
  ...
  const struct ieee80211_mgmt *mgmt = (const struct ieee80211_mgmt *)buf;
  const u8 *pos, *ie, *end;
  struct wpa_ssid_value ssid = {.ssid_len = 0};
  u8 token;
  ...
  while ((ie = get_ie(pos, len, WLAN_EID_MEASURE_REQUEST))) {
    ...
    wpa_printf(MSG_DEBUG, "Neighbor report request, measure type %u", ie[4]);

    switch (ie[4]) { /* Measurement Type */
    case MEASURE_TYPE_LCI:
      lci = ie[2]; /* Measurement Token */
      lci_max_age = hostapd_parse_location_lci_req_age(ie + 2, ie[1]);
      break;
...
void hostapd_handle_radio_measurement(struct hostapd_data *hapd, const u8 *buf,
                                      size_t len) {
  const struct ieee80211_mgmt *mgmt = (const struct ieee80211_mgmt *)buf;
...
int hostapd_send_lci_req(struct hostapd_data *hapd, const u8 *addr) {
  ...
  if (!(sta->flags & WLAN_STA_AUTHORIZED)) {
    wpa_printf(MSG_INFO, "Request LCI: Destination address is not connected");
    return -1;
  }

  if (!(sta->rrm_enabled_capa[1] & WLAN_RRM_CAPS_LCI_MEASUREMENT)) {
    wpa_printf(MSG_INFO, "Request LCI: Station does not support LCI in RRM");
    return -1;
  }
...
int hostapd_send_range_req(struct hostapd_data *hapd, const u8 *addr,
                           u16 random_interval, u8 min_ap, const u8 *responders,
  ...
  wpa_printf(MSG_DEBUG,
             "Request range: dest addr " MACSTR
             " rand interval %u min AP %u n_responders %u",
             MAC2STR(addr), random_interval, min_ap, n_responders);
  ...
  if (!sta || !(sta->flags & WLAN_STA_AUTHORIZED)) {
    wpa_printf(MSG_INFO, "Request range: Destination address is not connected");
    return -1;
  }

  if (!(sta->rrm_enabled_capa[4] & WLAN_RRM_CAPS_FTM_RANGE_REPORT)) {
    wpa_printf(MSG_ERROR, "Request range: Destination station does not support "
                          "FTM range report in RRM");
    return -1;
  }
  ...
  for (i = 0; i < n_responders; i++) {
    ...
    nr = hostapd_neighbor_get(hapd, responders + ETH_ALEN * i, NULL);
    if (!nr) {
      wpa_printf(MSG_INFO, "Missing neighbor report for " MACSTR,
                 MAC2STR(responders + ETH_ALEN * i));
      wpabuf_free(buf);
      return -1;
    }
