...
gas_dialog_create(struct hostapd_data *hapd, const u8 *addr, u8 dialog_token) {
  ...
  if (!sta) {
    ...
    if (!sta) {
      wpa_printf(MSG_DEBUG, "Failed to add STA " MACSTR " for GAS query",
                 MAC2STR(addr));
      return NULL;
    }
  ...
  wpa_msg(hapd->msg_ctx, MSG_ERROR,
          "ANQP: Could not create dialog for " MACSTR
...
struct gas_dialog_info *gas_serv_dialog_find(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG,
             "ANQP: Could not find dialog for " MACSTR " dialog_token %u",
             MAC2STR(addr), dialog_token);
  return NULL;
}
...
static struct anqp_element *get_anqp_elem(struct hostapd_data *hapd,
  ...
  dl_list_for_each(elem, &hapd->conf->anqp_elem, struct anqp_element, list) {
...
static void anqp_add_elem(struct hostapd_data *hapd, struct wpabuf *buf,
  ...
  if (wpabuf_tailroom(buf) < 2 + 2 + wpabuf_len(elem->payload)) {
    wpa_printf(MSG_DEBUG, "ANQP: No room for InfoID %u payload", infoid);
    return;
  }
...
static void anqp_add_capab_list(struct hostapd_data *hapd, struct wpabuf *buf) {
  ...
  if (hapd->conf->nai_realm_data || get_anqp_elem(hapd, ANQP_NAI_REALM))
...
static int hs20_add_nai_home_realm_matches(struct hostapd_data *hapd,
  ...
  if (end - pos < 1) {
    wpa_hexdump(MSG_DEBUG, "Too short NAI Home Realm Query", home_realm,
                home_realm_len);
    return -1;
  }
  num_realms = *pos++;
  ...
  for (i = 0; i < num_realms && num_matching < 10; i++) {
    if (end - pos < 2) {
      wpa_hexdump(MSG_DEBUG, "Truncated NAI Home Realm Query", home_realm,
                  home_realm_len);
      return -1;
    }
    encoding = *pos++;
    ...
    for (j = 0; j < hapd->conf->nai_realm_count && num_matching < 10; j++) {
      ...
      while (rpos < realm_name + realm_len && num_matching < 10) {
        ...
        for (k = 0; k < MAX_NAI_REALMS && realm->realm[k] && num_matching < 10;
             k++) {
          if ((int)os_strlen(realm->realm[k]) != rend - rpos ||
              os_strncmp((char *)rpos, realm->realm[k], rend - rpos) != 0)
            continue;
...
static void anqp_add_domain_name(struct hostapd_data *hapd,
                                 struct wpabuf *buf) {
...
static void anqp_add_osu_providers_list(struct hostapd_data *hapd,
  ...
  if (hapd->conf->hs20_osu_providers_count) {
    ...
    for (i = 0; i < hapd->conf->hs20_osu_providers_count; i++) {
      anqp_add_osu_provider(buf, hapd->conf,
...
static void anqp_add_icon_binary_file(struct hostapd_data *hapd,
                                      struct wpabuf *buf, const u8 *name,
                                      size_t name_len) {
  struct hs20_icon *icon;
  size_t i;
  u8 *len;

  wpa_hexdump_ascii(MSG_DEBUG, "HS 2.0: Requested Icon Filename", name,
                    name_len);
...
static struct wpabuf *gas_serv_build_gas_resp_payload(
    struct hostapd_data *hapd, unsigned int request, const u8 *home_realm,
    size_t home_realm_len, const u8 *icon_name, size_t icon_name_len,
    const u16 *extra_req, unsigned int num_extra_req) {
  struct wpabuf *buf;
  size_t len;
  unsigned int i;
...
static void rx_anqp_query_list_id(struct hostapd_data *hapd, u16 info_id,
  ...
  case ANQP_EMERGENCY_CALL_NUMBER:
    set_anqp_req(ANQP_REQ_EMERGENCY_CALL_NUMBER, "Emergency Call Number",
  ...
  case ANQP_3GPP_CELLULAR_NETWORK:
    set_anqp_req(ANQP_REQ_3GPP_CELLULAR_NETWORK, "3GPP Cellular Network",
                 hapd->conf->anqp_3gpp_cell_net != NULL, qi);
    break;
  case ANQP_AP_GEOSPATIAL_LOCATION:
    set_anqp_req(ANQP_REQ_AP_GEOSPATIAL_LOCATION, "AP Geospatial Location",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  case ANQP_AP_CIVIC_LOCATION:
    set_anqp_req(ANQP_REQ_AP_CIVIC_LOCATION, "AP Civic Location",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  case ANQP_AP_LOCATION_PUBLIC_URI:
    set_anqp_req(ANQP_REQ_AP_LOCATION_PUBLIC_URI, "AP Location Public URI",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  ...
  case ANQP_EMERGENCY_ALERT_URI:
    set_anqp_req(ANQP_REQ_EMERGENCY_ALERT_URI, "Emergency Alert URI",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  case ANQP_TDLS_CAPABILITY:
    set_anqp_req(ANQP_REQ_TDLS_CAPABILITY, "TDLS Capability",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  case ANQP_EMERGENCY_NAI:
    set_anqp_req(ANQP_REQ_EMERGENCY_NAI, "Emergency NAI",
                 get_anqp_elem(hapd, info_id) != NULL, qi);
    break;
  default:
...
static void rx_anqp_query_list(struct hostapd_data *hapd, const u8 *pos,
                               const u8 *end, struct anqp_query_info *qi) {
...
static void rx_anqp_hs_query_list(struct hostapd_data *hapd, u8 subtype,
  ...
  case HS20_STYPE_OPERATOR_FRIENDLY_NAME:
    set_anqp_req(ANQP_REQ_OPERATOR_FRIENDLY_NAME, "Operator Friendly Name",
  ...
  case HS20_STYPE_CONNECTION_CAPABILITY:
    set_anqp_req(ANQP_REQ_CONNECTION_CAPABILITY, "Connection Capability",
                 hapd->conf->hs20_connection_capability != NULL, qi);
    break;
...
static void rx_anqp_hs_nai_home_realm(struct hostapd_data *hapd, const u8 *pos,
                                      const u8 *end,
                                      struct anqp_query_info *qi) {
...
static void rx_anqp_hs_icon_request(struct hostapd_data *hapd, const u8 *pos,
                                    const u8 *end, struct anqp_query_info *qi) {
...
static void rx_anqp_vendor_specific(struct hostapd_data *hapd, const u8 *pos,
                                    const u8 *end, struct anqp_query_info *qi) {
  u32 oui;
  u8 subtype;
...
static void gas_serv_req_local_processing(struct hostapd_data *hapd,
  ...
  buf = gas_serv_build_gas_resp_payload(
      hapd, qi->request, qi->home_realm_query, qi->home_realm_query_len,
      qi->icon_name, qi->icon_name_len, qi->extra_req, qi->num_extra_req);
  ...
  if (wpabuf_len(buf) == 0 && qi->p2p_sd) {
    wpa_printf(MSG_DEBUG,
               "ANQP: Do not send response to P2P SD from generic GAS service "
               "(P2P SD implementation will process this)");
    wpabuf_free(buf);
    return;
  }
#endif /* CONFIG_P2P */
  ...
  if (wpabuf_len(buf) > hapd->gas_frag_limit ||
    ...
    if (!di) {
      ...
      tx_buf = gas_anqp_build_initial_resp_buf(
          dialog_token, WLAN_STATUS_UNSPECIFIED_FAILURE, 0, NULL);
    } else {
      di->prot = prot;
      di->sd_resp = buf;
      di->sd_resp_pos = 0;
      tx_buf = gas_anqp_build_initial_resp_buf(
          dialog_token, WLAN_STATUS_SUCCESS, comeback_delay, NULL);
    }
  } else {
    wpa_printf(MSG_DEBUG, "ANQP: Initial response (no comeback)");
    tx_buf = gas_anqp_build_initial_resp_buf(dialog_token, WLAN_STATUS_SUCCESS,
                                             0, buf);
    wpabuf_free(buf);
  }
  if (!tx_buf)
...
static void gas_serv_rx_gas_initial_req(struct hostapd_data *hapd, const u8 *sa,
  ...
  if (*pos != ACCESS_NETWORK_QUERY_PROTOCOL) {
    struct wpabuf *buf;
    wpa_msg(hapd->msg_ctx, MSG_DEBUG,
            "GAS: Unsupported GAS advertisement protocol id %u", *pos);
    if (sa[0] & 0x01)
      return; /* Invalid source address - drop silently */
    buf = gas_build_initial_resp(
        dialog_token, WLAN_STATUS_GAS_ADV_PROTO_NOT_SUPPORTED, 0, 2 + slen + 2);
    if (buf == NULL)
      return;
    ...
    if (std_addr3)
      hostapd_drv_send_action(hapd, hapd->iface->freq, 0, sa, wpabuf_head(buf),
                              wpabuf_len(buf));
    else
      hostapd_drv_send_action_addr3_ap(hapd, hapd->iface->freq, 0, sa,
  ...
  gas_serv_req_local_processing(hapd, sa, dialog_token, &qi, prot, std_addr3);
}

static void gas_serv_rx_gas_comeback_req(struct hostapd_data *hapd,
                                         const u8 *sa, const u8 *data,
                                         size_t len, int prot, int std_addr3) {
  ...
  if (!dialog) {
    ...
    tx_buf = gas_anqp_build_comeback_resp_buf(
        dialog_token, WLAN_STATUS_NO_OUTSTANDING_GAS_REQ, 0, 0, 0, NULL);
    if (tx_buf == NULL)
      return;
    goto send_resp;
  }
  ...
  buf = wpabuf_alloc_copy(wpabuf_head_u8(dialog->sd_resp) + dialog->sd_resp_pos,
                          frag_len);
  if (buf == NULL) {
    wpa_msg(hapd->msg_ctx, MSG_DEBUG,
  ...
  tx_buf = gas_anqp_build_comeback_resp_buf(dialog_token, WLAN_STATUS_SUCCESS,
                                            dialog->sd_frag_id, more, 0, buf);
  wpabuf_free(buf);
  if (tx_buf == NULL) {
    gas_serv_dialog_clear(dialog);
    return;
  ...
  if (more) {
    wpa_msg(hapd->msg_ctx, MSG_DEBUG,
            ...
            "to be sent",
            (int)(wpabuf_len(dialog->sd_resp) - dialog->sd_resp_pos));
  } else {
    wpa_msg(hapd->msg_ctx, MSG_DEBUG,
...
static void gas_serv_rx_public_action(void *ctx, const u8 *buf, size_t len,
  ...
  case WLAN_PA_GAS_INITIAL_REQ:
    gas_serv_rx_gas_initial_req(hapd, sa, data + 1, len - 1, prot, std_addr3);
    break;
  case WLAN_PA_GAS_COMEBACK_REQ:
    gas_serv_rx_gas_comeback_req(hapd, sa, data + 1, len - 1, prot, std_addr3);
    break;
  }
}
