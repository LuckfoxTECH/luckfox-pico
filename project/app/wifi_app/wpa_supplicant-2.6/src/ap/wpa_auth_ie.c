...
static int wpa_write_wpa_ie(struct wpa_auth_config *conf, u8 *buf, size_t len) {
  ...
  if (suite == 0) {
    wpa_printf(MSG_DEBUG, "Invalid group cipher (%d).", conf->wpa_group);
    return -1;
  }
  ...
  if (num_suites == 0) {
    wpa_printf(MSG_DEBUG, "Invalid pairwise cipher (%d).", conf->wpa_pairwise);
    return -1;
  }
...
int wpa_write_rsn_ie(struct wpa_auth_config *conf, u8 *buf, size_t len,
  ...
  if (num_suites == 0) {
    wpa_printf(MSG_DEBUG, "Invalid pairwise cipher (%d).", conf->rsn_pairwise);
    return -1;
  }
  WPA_PUT_LE16(count, num_suites);
  ...
  if (conf->ieee80211w != NO_MGMT_FRAME_PROTECTION &&
    ...
    case WPA_CIPHER_BIP_CMAC_256:
      ...
      break;
    default:
      wpa_printf(MSG_DEBUG, "Invalid group management cipher (0x%x)",
...
int wpa_auth_gen_wpa_ie(struct wpa_authenticator *wpa_auth) {
  ...
  if (wpa_auth->conf.own_ie_override_len) {
    ...
    wpa_auth->wpa_ie = os_malloc(wpa_auth->conf.own_ie_override_len);
  ...
  if (wpa_auth->conf.wpa & WPA_PROTO_RSN) {
    res = wpa_write_rsn_ie(&wpa_auth->conf, pos, buf + sizeof(buf) - pos, NULL);
    if (res < 0)
      return res;
    pos += res;
  }
  ...
  if (wpa_auth->conf.wpa & WPA_PROTO_WPA) {
    res = wpa_write_wpa_ie(&wpa_auth->conf, pos, buf + sizeof(buf) - pos);
    if (res < 0)
      return res;
    pos += res;
  }
...
static int wpa_auth_okc_iter(struct wpa_authenticator *a, void *ctx) {
  ...
  data->pmksa = pmksa_cache_get_okc(a->pmksa, data->aa, data->spa, data->pmkid);
  if (data->pmksa)
    return 1;
  return 0;
}
...
int wpa_validate_wpa_ie(struct wpa_authenticator *wpa_auth,
  ...
  if (!(wpa_auth->conf.wpa & version)) {
    wpa_printf(MSG_DEBUG, "Invalid WPA proto (%d) from " MACSTR, version,
               MAC2STR(sm->addr));
  ...
  if (res) {
    wpa_printf(MSG_DEBUG, "Failed to parse WPA/RSN IE from " MACSTR " (res=%d)",
               MAC2STR(sm->addr), res);
  ...
  if (data.group_cipher != wpa_auth->conf.wpa_group) {
    wpa_printf(MSG_DEBUG, "Invalid WPA group cipher (0x%x) from " MACSTR,
  ...
  if (!key_mgmt) {
    wpa_printf(MSG_DEBUG, "Invalid WPA key mgmt (0x%x) from " MACSTR,
  ...
  if (!ciphers) {
    wpa_printf(MSG_DEBUG,
               ...
               "from " MACSTR,
               version == WPA_PROTO_RSN ? "RSN" : "WPA", data.pairwise_cipher,
               MAC2STR(sm->addr));
  ...
  for (i = 0; i < data.num_pmkid; i++) {
    wpa_hexdump(MSG_DEBUG, "RSN IE: STA PMKID", &data.pmkid[i * PMKID_LEN],
                PMKID_LEN);
  ...
  for (i = 0; sm->pmksa == NULL && wpa_auth->conf.okc && i < data.num_pmkid;
       i++) {
    ...
    if (idata.pmksa) {
      wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG, "OKC match for PMKID");
      sm->pmksa = pmksa_cache_add_okc(wpa_auth->pmksa, idata.pmksa,
                                      wpa_auth->addr, idata.pmkid);
      pmkid = idata.pmkid;
      break;
    }
  }
  if (sm->pmksa && pmkid) {
    ...
    wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG,
                     ...
                     sm->pmksa->eap_type_authsrv, vlan ? vlan->untagged : 0,
                     (vlan && vlan->tagged[0]) ? "+" : "");
...
static int wpa_parse_generic(const u8 *pos, const u8 *end,
  ...
  if (pos[1] >= 6 && RSN_SELECTOR_GET(pos + 2) == WPA_OUI_TYPE &&
      pos[2 + WPA_SELECTOR_LEN] == 1 && pos[2 + WPA_SELECTOR_LEN + 1] == 0) {
    ie->wpa_ie = pos;
  ...
  if (pos[1] >= RSN_SELECTOR_LEN + 3 * 4 &&
    ...
    wpa_hexdump(MSG_DEBUG, "WPA: IP Address Allocation in EAPOL-Key",
