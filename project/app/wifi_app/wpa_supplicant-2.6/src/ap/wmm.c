...
u8 *hostapd_eid_wmm(struct hostapd_data *hapd, u8 *eid) {
  u8 *pos = eid;
  struct wmm_parameter_element *wmm = (struct wmm_parameter_element *)(pos + 2);
  int e;
  ...
  for (e = 0; e < 4; e++) {
    ...
    struct hostapd_wmm_ac_params *acp = &hapd->iconf->wmm_ac_params[e];

    ac->aci_aifsn =
        wmm_aci_aifsn(acp->aifs, acp->admission_control_mandatory, e);
...
int hostapd_eid_wmm_valid(struct hostapd_data *hapd, const u8 *eid,
  ...
  if (len < sizeof(struct wmm_information_element)) {
    wpa_printf(MSG_DEBUG, "Too short WMM IE (len=%lu)", (unsigned long)len);
    return 0;
  }
...
static void wmm_send_action(struct hostapd_data *hapd, const u8 *addr,
  ...
  struct wmm_tspec_element *t =
      (struct wmm_tspec_element *)m->u.action.u.wmm_action.variable;
  int len;

  hostapd_logger(hapd, addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
...
int wmm_process_tspec(struct wmm_tspec_element *tspec) {
  ...
  wpa_printf(MSG_DEBUG, "WMM: TS Info: UP=%d PSB=%d Direction=%d TID=%d", up,
             psb, dir, tid);
  val = le_to_host16(tspec->nominal_msdu_size);
  wpa_printf(MSG_DEBUG, "WMM: Nominal MSDU Size: %d%s", val & 0x7fff,
             val & 0x8000 ? " (fixed)" : "");
  ...
  wpa_printf(MSG_DEBUG, "WMM: Surplus Bandwidth Allowance: %u.%04u", val >> 13,
             10000 * (val & 0x1fff) / 0x2000);
...
static void wmm_addts_req(struct hostapd_data *hapd,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "from " MACSTR,
             mgmt->u.action.u.wmm_action.dialog_token, MAC2STR(mgmt->sa));
...
void hostapd_wmm_action(struct hostapd_data *hapd,
  ...
  if (!sta || (sta->flags & (WLAN_STA_ASSOC | WLAN_STA_WMM)) !=
  ...
  case WMM_ACTION_CODE_ADDTS_REQ:
    wmm_addts_req(hapd, mgmt, (struct wmm_tspec_element *)(elems.wmm_tspec - 2),
                  len);
    return;
#if 0
  ...
  hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                 "hostapd_wmm_action - unknown action code %d", action_code);
}
