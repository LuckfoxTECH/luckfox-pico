...
static void wpa_smk_send_error(struct wpa_authenticator *wpa_auth,
  ...
  u8 kde[2 + RSN_SELECTOR_LEN + ETH_ALEN + 2 + RSN_SELECTOR_LEN +
         sizeof(struct rsn_error_kde)];
  u8 *pos;
  struct rsn_error_kde error;

  wpa_auth_logger(wpa_auth, sm->addr, LOGGER_DEBUG, "Sending SMK Error");
  ...
  pos = wpa_add_kde(pos, RSN_KEY_DATA_ERROR, (u8 *)&error, sizeof(error), NULL,
                    0);
...
void wpa_smk_m1(struct wpa_authenticator *wpa_auth,
  ...
  search.sm = NULL;
  if (wpa_auth_for_each_sta(wpa_auth, wpa_stsl_select_sta, &search) == 0 ||
      search.sm == NULL) {
    wpa_printf(MSG_DEBUG,
               "RSN: SMK handshake with " MACSTR
               " aborted - STA not associated anymore",
               MAC2STR(kde.mac_addr));
    wpa_smk_send_error(wpa_auth, sm, kde.mac_addr, STK_MUI_SMK, STK_ERR_STA_NR);
  ...
  pos = wpa_add_kde(pos, RSN_KEY_DATA_MAC_ADDR, sm->addr, ETH_ALEN, NULL, 0);
  ...
  wpa_auth_logger(wpa_auth, search.sm->addr, LOGGER_DEBUG, "Sending SMK M2");
...
static void wpa_send_smk_m4(struct wpa_authenticator *wpa_auth,
                            ...
                            struct wpa_eapol_ie_parse *kde, const u8 *smk) {
  u8 *buf, *pos;
  size_t buf_len;
  u32 lifetime;
  ...
  buf_len = 2 + RSN_SELECTOR_LEN + ETH_ALEN + 2 + RSN_SELECTOR_LEN +
            WPA_NONCE_LEN + 2 + RSN_SELECTOR_LEN + PMK_LEN + WPA_NONCE_LEN + 2 +
            RSN_SELECTOR_LEN + sizeof(lifetime);
  ...
  /* Initiator MAC Address */
  pos =
      wpa_add_kde(pos, RSN_KEY_DATA_MAC_ADDR, kde->mac_addr, ETH_ALEN, NULL, 0);

  /* Initiator Nonce */
  pos =
      wpa_add_kde(pos, RSN_KEY_DATA_NONCE, kde->nonce, WPA_NONCE_LEN, NULL, 0);

  /* SMK with PNonce */
  pos = wpa_add_kde(pos, RSN_KEY_DATA_SMK, smk, PMK_LEN, key->key_nonce,
                    WPA_NONCE_LEN);

  /* Lifetime */
  lifetime = htonl(43200); /* dot11RSNAConfigSMKLifetime */
  pos = wpa_add_kde(pos, RSN_KEY_DATA_LIFETIME, (u8 *)&lifetime,
                    sizeof(lifetime), NULL, 0);

  wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG, "Sending SMK M4");
...
static void wpa_send_smk_m5(struct wpa_authenticator *wpa_auth,
  ...
  buf_len = kde->rsn_ie_len + 2 + RSN_SELECTOR_LEN + ETH_ALEN + 2 +
            RSN_SELECTOR_LEN + WPA_NONCE_LEN + 2 + RSN_SELECTOR_LEN + PMK_LEN +
            WPA_NONCE_LEN + 2 + RSN_SELECTOR_LEN + sizeof(lifetime);
  pos = buf = os_malloc(buf_len);
  if (buf == NULL)
    return;
  ...
  /* PNonce */
  pos = wpa_add_kde(pos, RSN_KEY_DATA_NONCE, key->key_nonce, WPA_NONCE_LEN,
                    NULL, 0);

  /* SMK and INonce */
  pos = wpa_add_kde(pos, RSN_KEY_DATA_SMK, smk, PMK_LEN, kde->nonce,
                    WPA_NONCE_LEN);
  ...
  wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG, "Sending SMK M5");
...
void wpa_smk_m3(struct wpa_authenticator *wpa_auth,
  ...
  if (kde.rsn_ie == NULL || kde.mac_addr == NULL ||
      kde.mac_addr_len < ETH_ALEN || kde.nonce == NULL ||
  ...
  sha256_prf(smk, PMK_LEN, "SMK Derivation", buf, sizeof(buf), smk, PMK_LEN);
#else  /* CONFIG_IEEE80211W */
  sha1_prf(smk, PMK_LEN, "SMK Derivation", buf, sizeof(buf), smk, PMK_LEN);
...
void wpa_smk_error(struct wpa_authenticator *wpa_auth,
                   struct wpa_state_machine *sm, const u8 *key_data,
                   size_t key_data_len) {
  struct wpa_eapol_ie_parse kde;
  struct wpa_stsl_search search;
  struct rsn_error_kde error;
