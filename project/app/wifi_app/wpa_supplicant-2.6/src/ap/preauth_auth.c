...
static void rsn_preauth_receive(void *ctx, const u8 *src_addr, const u8 *buf,
  ...
  if (os_memcmp(ethhdr->h_dest, hapd->own_addr, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG, "RSN: pre-auth for foreign address " MACSTR,
...
static int rsn_preauth_iface_add(struct hostapd_data *hapd,
                                 const char *ifname) {
...
static void rsn_preauth_finished_cb(void *eloop_ctx, void *timeout_ctx) {
  ...
  wpa_printf(MSG_DEBUG,
             "RSN: Removing pre-authentication STA entry for " MACSTR,
             MAC2STR(sta->addr));
  ap_free_sta(hapd, sta);
}
...
void rsn_preauth_finished(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  const u8 *key;
  size_t len;
  hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_WPA, HOSTAPD_LEVEL_INFO,
                 "pre-authentication %s", success ? "succeeded" : "failed");

  key = ieee802_1x_get_key(sta->eapol_sm, &len);
  if (len > PMK_LEN)
    len = PMK_LEN;
  if (success && key) {
    if (wpa_auth_pmksa_add_preauth(hapd->wpa_auth, key, len, sta->addr,
...
void rsn_preauth_send(struct hostapd_data *hapd, struct sta_info *sta, u8 *buf,
                      size_t len) {
  struct rsn_preauth_interface *piface;
  struct l2_ethhdr *ethhdr;
