...
static inline const u8 *wpa_auth_get_psk(struct wpa_authenticator *wpa_auth,
  ...
  if (wpa_auth->cb.get_psk == NULL)
    return NULL;
  return wpa_auth->cb.get_psk(wpa_auth->cb.ctx, addr, p2p_dev_addr, prev_psk);
}
...
static inline int wpa_auth_set_key(struct wpa_authenticator *wpa_auth,
                                   int vlan_id, enum wpa_alg alg,
                                   const u8 *addr, int idx, u8 *key,
                                   size_t key_len) {
  if (wpa_auth->cb.set_key == NULL)
    return -1;
  return wpa_auth->cb.set_key(wpa_auth->cb.ctx, vlan_id, alg, addr, idx, key,
                              key_len);
}
...
static inline int wpa_auth_send_eapol(struct wpa_authenticator *wpa_auth,
...
static void wpa_rekey_gmk(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (random_get_bytes(wpa_auth->group->GMK, WPA_GMK_LEN)) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "GMK", wpa_auth->group->GMK, WPA_GMK_LEN);
  }

  if (wpa_auth->conf.wpa_gmk_rekey) {
    eloop_register_timeout(wpa_auth->conf.wpa_gmk_rekey, 0, wpa_rekey_gmk,
                           wpa_auth, NULL);
  }
}
...
static void wpa_rekey_gtk(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (wpa_auth->conf.wpa_group_rekey) {
    eloop_register_timeout(wpa_auth->conf.wpa_group_rekey, 0, wpa_rekey_gtk,
                           wpa_auth, NULL);
  }
}
...
static int wpa_group_init_gmk_and_counter(struct wpa_authenticator *wpa_auth,
  ...
  if (sha1_prf(rkey, sizeof(rkey), "Init Counter", buf, sizeof(buf),
    ...
    return -1;
  wpa_hexdump_key(MSG_DEBUG, "Key Counter", group->Counter, WPA_NONCE_LEN);
...
struct wpa_authenticator *wpa_init(const u8 *addr, struct wpa_auth_config *conf,
  ...
  wpa_auth->pmksa = pmksa_cache_auth_init(wpa_auth_pmksa_free_cb, wpa_auth);
  ...
  if (WPA_GET_BE32(conf->ip_addr_start)) {
    int count =
        WPA_GET_BE32(conf->ip_addr_end) - WPA_GET_BE32(conf->ip_addr_start) + 1;
    if (count > 1000)
      count = 1000;
    if (count > 0)
...
int wpa_auth_sta_associated(struct wpa_authenticator *wpa_auth,
  ...
  wpa_auth_logger(wpa_auth, sm->addr, LOGGER_DEBUG, "start authentication");
  sm->started = 1;
...
static void wpa_free_sta_sm(struct wpa_state_machine *sm) {
  ...
  if (WPA_GET_BE32(sm->ip_addr)) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               sm->ip_addr[0], sm->ip_addr[1], sm->ip_addr[2], sm->ip_addr[3],
               MAC2STR(sm->addr));
    start = WPA_GET_BE32(sm->wpa_auth->conf.ip_addr_start);
    bitfield_clear(sm->wpa_auth->ip_pool, WPA_GET_BE32(sm->ip_addr) - start);
  }
#endif /* CONFIG_P2P */
...
void wpa_auth_sta_deinit(struct wpa_state_machine *sm) {
  ...
  if (sm->wpa_auth->conf.wpa_strict_rekey && sm->has_GTK) {
    ...
    eloop_register_timeout(0, 500000, wpa_rekey_gtk, sm->wpa_auth, NULL);
  }
...
static int wpa_replay_counter_valid(struct wpa_key_replay_counter *ctr,
  ...
  for (i = 0; i < RSNA_MAX_EAPOL_RETRIES; i++) {
    if (!ctr[i].valid)
      break;
    if (os_memcmp(replay_counter, ctr[i].counter, WPA_REPLAY_COUNTER_LEN) == 0)
      return 1;
  }
  return 0;
}
...
static void wpa_replay_counter_mark_invalid(struct wpa_key_replay_counter *ctr,
  ...
  for (i = 0; i < RSNA_MAX_EAPOL_RETRIES; i++) {
    if (ctr[i].valid &&
        (replay_counter == NULL || os_memcmp(replay_counter, ctr[i].counter,
...
static int ft_check_msg_2_of_4(struct wpa_authenticator *wpa_auth,
  ...
  wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name from Supplicant", sm->sup_pmk_r1_name,
              PMKID_LEN);
  ...
  if (sm->assoc_resp_ftie && (kde->ftie[1] != sm->assoc_resp_ftie[1] ||
                              os_memcmp(kde->ftie, sm->assoc_resp_ftie,
                                        2 + sm->assoc_resp_ftie[1]) != 0)) {
    wpa_printf(MSG_DEBUG, "FT: FTIE mismatch");
    wpa_hexdump(MSG_DEBUG, "FT: FTIE in EAPOL-Key msg 2/4", kde->ftie,
                kde->ftie_len);
    wpa_hexdump(MSG_DEBUG, "FT: FTIE in (Re)AssocResp", sm->assoc_resp_ftie,
...
static int wpa_try_alt_snonce(struct wpa_state_machine *sm, u8 *data,
  ...
  for (;;) {
    if (wpa_key_mgmt_wpa_psk(sm->wpa_key_mgmt)) {
      pmk = wpa_auth_get_psk(sm->wpa_auth, sm->addr, sm->p2p_dev_addr, pmk);
      if (pmk == NULL)
        break;
      pmk_len = PMK_LEN;
    } else {
  ...
  if (!ok) {
    wpa_printf(MSG_DEBUG, "WPA: Earlier SNonce did not result in matching MIC");
    return -1;
  }
...
void wpa_receive(struct wpa_authenticator *wpa_auth,
  ...
  if (key_data_length > data_len - sizeof(*hdr) - keyhdrlen) {
    wpa_printf(MSG_INFO,
               ...
               key_data_length,
               (unsigned long)(data_len - sizeof(*hdr) - keyhdrlen));
    return;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "WPA: Received Key Nonce", key->key_nonce,
              WPA_NONCE_LEN);
  wpa_hexdump(MSG_DEBUG, "WPA: Received Replay Counter", key->replay_counter,
  ...
  if (msg == REQUEST || msg == PAIRWISE_2 || msg == PAIRWISE_4 ||
    ...
    if (sm->pairwise == WPA_CIPHER_CCMP || sm->pairwise == WPA_CIPHER_GCMP) {
      if (wpa_use_aes_cmac(sm) && sm->wpa_key_mgmt != WPA_KEY_MGMT_OSEN &&
      ...
      if (!wpa_use_aes_cmac(sm) && ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES) {
        wpa_auth_logger(wpa_auth, sm->addr, LOGGER_WARNING,
    ...
    if (wpa_key_mgmt_suite_b(sm->wpa_key_mgmt) &&
      ...
      wpa_auth_logger(wpa_auth, sm->addr, LOGGER_WARNING,
                      "did not use EAPOL-Key descriptor version 0 as required "
                      "for AKM-defined cases");
      return;
    }
  }
  ...
  if (!(key_info & WPA_KEY_INFO_REQUEST) &&
    ...
    if (msg == PAIRWISE_2 &&
        wpa_replay_counter_valid(sm->prev_key_replay, key->replay_counter) &&
    ...
    if (msg == PAIRWISE_4 && sm->alt_snonce_valid &&
      ...
      wpa_auth_vlogger(
          ...
          "Try to process received EAPOL-Key 4/4 based on old Replay Counter "
          "and SNonce from an earlier EAPOL-Key 1/4");
      goto continue_processing;
    }
    ...
    for (i = 0; i < RSNA_MAX_EAPOL_RETRIES; i++) {
      ...
      wpa_hexdump(MSG_DEBUG, "pending replay counter",
                  sm->key_replay[i].counter, WPA_REPLAY_COUNTER_LEN);
    }
    wpa_hexdump(MSG_DEBUG, "received replay counter", key->replay_counter,
                WPA_REPLAY_COUNTER_LEN);
    return;
  }
...
continue_processing:
  ...
  case PAIRWISE_2:
    ...
    if (sm->wpa_ie == NULL ||
        wpa_compare_rsn_ie(ft, sm->wpa_ie, sm->wpa_ie_len, eapol_key_ie,
                           eapol_key_ie_len)) {
      wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
                      "WPA IE from (Re)AssocReq did not "
                      "match with msg 2/4");
      if (sm->wpa_ie) {
        wpa_hexdump(MSG_DEBUG, "WPA IE in AssocReq", sm->wpa_ie,
                    sm->wpa_ie_len);
      }
      wpa_hexdump(MSG_DEBUG, "WPA IE in msg 2/4", eapol_key_ie,
                  eapol_key_ie_len);
...
#endif /* CONFIG_IEEE80211R */
#ifdef CONFIG_P2P
    if (kde.ip_addr_req && kde.ip_addr_req[0] && wpa_auth->ip_pool &&
      ...
      if (idx >= 0) {
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   sm->ip_addr[0], sm->ip_addr[1], sm->ip_addr[2],
                   sm->ip_addr[3], MAC2STR(sm->addr));
      }
    }
#endif /* CONFIG_P2P */
    break;
  case PAIRWISE_4:
    if (sm->wpa_ptk_state != WPA_PTK_PTKINITNEGOTIATING || !sm->PTK_valid) {
      wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_INFO,
  ...
  if (sm->PTK_valid && !sm->update_snonce) {
    if (wpa_verify_key_mic(sm->wpa_key_mgmt, &sm->PTK, data, data_len) &&
  ...
  if (key_info & WPA_KEY_INFO_REQUEST) {
    ...
    if (msg == SMK_ERROR) {
      ...
      if (wpa_receive_error_report(wpa_auth, sm,
      ...
      wpa_smk_m1(wpa_auth, sm, key, key_data, key_data_length);
#endif /* CONFIG_PEERKEY */
    } else if (key_data_length > 0 &&
               wpa_parse_kde_ies(key_data, key_data_length, &kde) == 0 &&
               kde.mac_addr) {
    } else {
      wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
    ...
    wpa_replay_counter_mark_invalid(sm->key_replay, key->replay_counter);
    ...
    if (msg == PAIRWISE_2) {
       ...
       */
      os_memcpy(sm->prev_key_replay, sm->key_replay, sizeof(sm->key_replay));
    } else {
      os_memset(sm->prev_key_replay, 0, sizeof(sm->prev_key_replay));
    }
...
void __wpa_send_eapol(struct wpa_authenticator *wpa_auth,
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "encr=%d)",
             version, (key_info & WPA_KEY_INFO_SECURE) ? 1 : 0,
             (key_info & WPA_KEY_INFO_MIC) ? 1 : 0,
             (key_info & WPA_KEY_INFO_ACK) ? 1 : 0,
             (key_info & WPA_KEY_INFO_INSTALL) ? 1 : 0, pairwise,
             (unsigned long)kde_len, keyidx, encr);
  ...
  key->type =
      sm->wpa == WPA_VERSION_WPA2 ? EAPOL_KEY_TYPE_RSN : EAPOL_KEY_TYPE_WPA;
  key_info |= version;
  ...
  for (i = RSNA_MAX_EAPOL_RETRIES - 1; i > 0; i--) {
    ...
    os_memcpy(sm->key_replay[i].counter, sm->key_replay[i - 1].counter,
              WPA_REPLAY_COUNTER_LEN);
  }
  ...
  if (kde && !encr) {
    ...
    if (version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
      ...
      if (mic_len == 24)
        WPA_PUT_BE16(key192->key_data_length, key_data_len);
      else
        WPA_PUT_BE16(key->key_data_length, key_data_len);
#ifndef CONFIG_NO_RC4
    } else if (sm->PTK.kek_len == 16) {
      u8 ek[32];
      os_memcpy(key->key_iv, sm->group->Counter + WPA_NONCE_LEN - 16, 16);
  ...
  if (key_info & WPA_KEY_INFO_MIC) {
    ...
    wpa_eapol_key_mic(sm->PTK.kck, sm->PTK.kck_len, sm->wpa_key_mgmt, version,
                      (u8 *)hdr, len, key_mic);
#ifdef CONFIG_TESTING_OPTIONS
    if (!pairwise && wpa_auth->conf.corrupt_gtk_rekey_mic_probability > 0.0 &&
...
static void wpa_send_eapol(struct wpa_authenticator *wpa_auth,
  ...
  __wpa_send_eapol(wpa_auth, sm, key_info, key_rsc, nonce, kde, kde_len, keyidx,
                   encr, 0);

  ctr = pairwise ? sm->TimeoutCtr : sm->GTimeoutCtr;
  if (ctr == 1 && wpa_auth->conf.tx_status)
    timeout_ms =
        pairwise ? eapol_key_timeout_first : eapol_key_timeout_first_group;
  else
...
static int wpa_verify_key_mic(int akmp, struct wpa_ptk *PTK, u8 *data,
  ...
  if (wpa_eapol_key_mic(PTK->kck, PTK->kck_len, akmp,
                        key_info & WPA_KEY_INFO_TYPE_MASK, data, data_len,
                        key192->key_mic) ||
...
SM_STATE(WPA_PTK, INITIALIZE) {
  ...
  sm->TimeoutCtr = 0;
  if (wpa_key_mgmt_wpa_psk(sm->wpa_key_mgmt)) {
    wpa_auth_set_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_authorized, 0);
  }
}
...
SM_STATE(WPA_PTK, AUTHENTICATION) {
  ...
  os_memset(&sm->PTK, 0, sizeof(sm->PTK));
  sm->PTK_valid = FALSE;
  wpa_auth_set_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_portControl_Auto, 1);
...
SM_STATE(WPA_PTK, INITPMK) {
  ...
  if (sm->pmksa) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(MSK len=%lu PMK len=%u)",
               (unsigned long)len, pmk_len);
    if (len < pmk_len) {
      wpa_printf(MSG_DEBUG, "WPA: MSK not long enough (%u) to create PMK (%u)",
...
SM_STATE(WPA_PTK, PTKSTART) {
  ...
  if (sm->wpa == WPA_VERSION_WPA2 &&
    ...
    if (sm->pmksa) {
      os_memcpy(&pmkid[2 + RSN_SELECTOR_LEN], sm->pmksa->pmkid, PMKID_LEN);
...
static int wpa_derive_ptk(struct wpa_state_machine *sm, const u8 *snonce,
  ...
  return wpa_pmk_to_ptk(pmk, pmk_len, "Pairwise key expansion",
                        sm->wpa_auth->addr, sm->addr, sm->ANonce, snonce, ptk,
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING) {
  ...
  for (;;) {
    ...
    if (wpa_verify_key_mic(sm->wpa_key_mgmt, &PTK, sm->last_rx_eapol_key,
  ...
  if (sm->wpa == WPA_VERSION_WPA2 && wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
    ...
    if (os_memcmp_const(sm->sup_pmk_r1_name, sm->pmk_r1_name,
      ...
      wpa_hexdump(MSG_DEBUG, "FT: Derived PMKR1Name", sm->pmk_r1_name,
                  WPA_PMK_NAME_LEN);
      return;
    }
  }
...
static int ieee80211w_kde_len(struct wpa_state_machine *sm) { return 0; }
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING) {
  ...
  if (sm->wpa == WPA_VERSION_WPA && (sm->wpa_auth->conf.wpa & WPA_PROTO_RSN) &&
  ...
  if (gtk) {
    ...
    hdr[0] = keyidx & 0x03;
    hdr[1] = 0;
    pos = wpa_add_kde(pos, RSN_KEY_DATA_GROUPKEY, hdr, 2, gtk, gtk_len);
  }
  ...
  if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
    ...
    if (sm->assoc_resp_ftie &&
      ...
      os_memcpy(pos, sm->assoc_resp_ftie, 2 + sm->assoc_resp_ftie[1]);
      res = 2 + sm->assoc_resp_ftie[1];
    } else {
      res = wpa_write_ftie(conf, conf->r0_key_holder, conf->r0_key_holder_len,
                           NULL, NULL, pos, kde + kde_len - pos, NULL, 0);
    }
    if (res < 0) {
  ...
  if (WPA_GET_BE32(sm->ip_addr) > 0) {
    ...
    pos = wpa_add_kde(pos, WFA_KEY_DATA_IP_ADDR_ALLOC, addr, sizeof(addr), NULL,
                      0);
  }
#endif /* CONFIG_P2P */
...
SM_STATE(WPA_PTK, PTKINITDONE) {
  ...
  if (sm->Pair) {
    ...
    if (wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, 0, sm->PTK.tk, klen)) {
      wpa_sta_disconnect(sm->wpa_auth, sm->addr);
      return;
    }
    ...
    if (sm->wpa_auth->conf.wpa_ptk_rekey) {
      eloop_cancel_timeout(wpa_rekey_ptk, sm->wpa_auth, sm);
      eloop_register_timeout(sm->wpa_auth->conf.wpa_ptk_rekey, 0, wpa_rekey_ptk,
                             sm->wpa_auth, sm);
    }
  ...
  if (0 /* IBSS == TRUE */) {
    sm->keycount++;
    if (sm->keycount == 2) {
      wpa_auth_set_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_portValid, 1);
    }
  } else {
    wpa_auth_set_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_portValid, 1);
  }
  ...
  if (sm->Init)
    ...
    case WPA_PTK_AUTHENTICATION2:
      if (wpa_key_mgmt_wpa_ieee8021x(sm->wpa_key_mgmt) &&
          wpa_auth_get_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_keyRun) > 0)
    ...
    case WPA_PTK_INITPMK:
      if (wpa_auth_get_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_keyAvailable) >
          0)
        SM_ENTER(WPA_PTK, PTKSTART);
      else {
    ...
    case WPA_PTK_INITPSK:
      if (wpa_auth_get_psk(sm->wpa_auth, sm->addr, sm->p2p_dev_addr, NULL))
        SM_ENTER(WPA_PTK, PTKSTART);
      else {
        wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_INFO,
    ...
    case WPA_PTK_PTKSTART:
      if (sm->EAPOLKeyReceived && !sm->EAPOLKeyRequest && sm->EAPOLKeyPairwise)
...
SM_STATE(WPA_PTK_GROUP, REKEYNEGOTIATING) {
  ...
  if (sm->wpa == WPA_VERSION_WPA2) {
    kde_len = 2 + RSN_SELECTOR_LEN + 2 + gsm->GTK_len + ieee80211w_kde_len(sm);
    ...
    hdr[0] = gsm->GN & 0x03;
    hdr[1] = 0;
    pos = wpa_add_kde(pos, RSN_KEY_DATA_GROUPKEY, hdr, 2, gtk, gsm->GTK_len);
    pos = ieee80211w_kde_add(sm, pos);
    kde_len = pos - kde;
  } else {
    kde = gtk;
  ...
  wpa_send_eapol(sm->wpa_auth, sm,
                 WPA_KEY_INFO_SECURE | WPA_KEY_INFO_MIC | WPA_KEY_INFO_ACK |
...
SM_STEP(WPA_PTK_GROUP) {
  if (sm->Init || sm->PtkGroupInit) {
    ...
    case WPA_PTK_GROUP_REKEYNEGOTIATING:
      ...
      else if (sm->GTimeoutCtr > (int)dot11RSNAConfigGroupUpdateCount)
...
static int wpa_gtk_update(struct wpa_authenticator *wpa_auth,
  ...
  if (wpa_gmk_to_gtk(group->GMK, "Group key expansion", wpa_auth->addr,
                     ...
                     group->GTK_len) < 0)
    ret = -1;
  wpa_hexdump_key(MSG_DEBUG, "GTK", group->GTK[group->GN - 1], group->GTK_len);
  ...
  if (wpa_auth->conf.ieee80211w != NO_MGMT_FRAME_PROTECTION) {
    ...
    os_memcpy(group->GNonce, group->Counter, WPA_NONCE_LEN);
    inc_byte_array(group->Counter, WPA_NONCE_LEN);
    if (wpa_gmk_to_gtk(group->GMK, "IGTK key expansion", wpa_auth->addr,
                       group->GNonce, group->IGTK[group->GN_igtk - 4], len) < 0)
      ret = -1;
    wpa_hexdump_key(MSG_DEBUG, "IGTK", group->IGTK[group->GN_igtk - 4], len);
  }
#endif /* CONFIG_IEEE80211W */
...
int wpa_wnmsleep_gtk_subelem(struct wpa_state_machine *sm, u8 *pos) {
  ...
  *pos++ = gsm->GTK_len;
  ...
  wpa_printf(MSG_DEBUG, "WNM: GTK Key ID %u in WNM-Sleep Mode exit", gsm->GN);
...
static int wpa_group_config_group_keys(struct wpa_authenticator *wpa_auth,
  ...
  if (wpa_auth->conf.ieee80211w != NO_MGMT_FRAME_PROTECTION) {
    ...
    if (ret == 0 && wpa_auth_set_key(wpa_auth, group->vlan_id, alg,
...
static const char *wpa_bool_txt(int val) { return val ? "TRUE" : "FALSE"; }
...
int wpa_get_mib(struct wpa_authenticator *wpa_auth, char *buf, size_t buflen) {
  ...
  wpa_snprintf_hex(pmkid_txt, sizeof(pmkid_txt), wpa_auth->dot11RSNAPMKIDUsed,
                   PMKID_LEN);
  ...
  ret = os_snprintf(
      ...
      RSN_VERSION, !!wpa_auth->conf.wpa_strict_rekey,
      dot11RSNAConfigGroupUpdateCount, dot11RSNAConfigPairwiseUpdateCount,
      wpa_cipher_key_len(wpa_auth->conf.wpa_group) * 8,
      dot11RSNAConfigPMKLifetime, dot11RSNAConfigPMKReauthThreshold,
      dot11RSNAConfigSATimeout,
      RSN_SUITE_ARG(wpa_auth->dot11RSNAAuthenticationSuiteSelected),
      RSN_SUITE_ARG(wpa_auth->dot11RSNAPairwiseCipherSelected),
      RSN_SUITE_ARG(wpa_auth->dot11RSNAGroupCipherSelected), pmkid_txt,
...
int wpa_get_mib_sta(struct wpa_state_machine *sm, char *buf, size_t buflen) {
  ...
  pairwise = wpa_cipher_to_suite(sm->wpa == WPA_VERSION_WPA2 ? WPA_PROTO_RSN
                                                             : WPA_PROTO_WPA,
                                 sm->pairwise);
  if (pairwise == 0)
    return 0;
  ...
  ret = os_snprintf(buf + len, buflen - len,
                    ...
                    MAC2STR(sm->addr), RSN_SUITE_ARG(pairwise),
...
int wpa_auth_get_pairwise(struct wpa_state_machine *sm) { return sm->pairwise; }
...
int wpa_auth_pmksa_add(struct wpa_state_machine *sm, const u8 *pmk,
  ...
  if (pmksa_cache_auth_add(sm->wpa_auth->pmksa, pmk, pmk_len, NULL, sm->PTK.kck,
                           sm->PTK.kck_len, sm->wpa_auth->addr, sm->addr,
                           session_timeout, eapol, sm->wpa_key_mgmt))
    return 0;
...
int wpa_auth_pmksa_add_preauth(struct wpa_authenticator *wpa_auth,
  ...
  if (pmksa_cache_auth_add(wpa_auth->pmksa, pmk, len, NULL, NULL, 0,
                           wpa_auth->addr, sta_addr, session_timeout, eapol,
...
int wpa_auth_pmksa_add_sae(struct wpa_authenticator *wpa_auth, const u8 *addr,
  ...
  if (pmksa_cache_auth_add(wpa_auth->pmksa, pmk, PMK_LEN, pmkid, NULL, 0,
                           wpa_auth->addr, addr, 0, NULL, WPA_KEY_MGMT_SAE))
    return 0;
...
void wpa_auth_pmksa_remove(struct wpa_authenticator *wpa_auth,
  ...
  if (pmksa) {
    wpa_printf(MSG_DEBUG,
               "WPA: Remove PMKSA cache entry for " MACSTR " based on request",
               MAC2STR(sta_addr));
...
static struct wpa_group *wpa_auth_add_group(struct wpa_authenticator *wpa_auth,
  ...
  wpa_printf(MSG_DEBUG, "WPA: Add group state machine for VLAN-ID %d", vlan_id);
...
int wpa_auth_ensure_group(struct wpa_authenticator *wpa_auth, int vlan_id) {
  ...
  wpa_printf(MSG_DEBUG,
             "WPA: Ensure group state machine running for VLAN ID %d", vlan_id);
...
int wpa_auth_release_group(struct wpa_authenticator *wpa_auth, int vlan_id) {
  ...
  wpa_printf(MSG_DEBUG, "WPA: Try stopping group state machine for VLAN ID %d",
             vlan_id);

  if (group->num_setup_iface <= 0) {
    wpa_printf(MSG_ERROR,
               "WPA: wpa_auth_release_group called more often than "
               "wpa_auth_ensure_group for VLAN ID %d, skipping.",
               vlan_id);
    return -1;
  }
  group->num_setup_iface--;
  ...
  if (group->references > 1) {
    wpa_printf(MSG_DEBUG,
               "WPA: Cannot stop group state machine for VLAN ID %d as "
               "references are still hold",
               vlan_id);
    ret = -2;
  }
...
int wpa_auth_sta_set_vlan(struct wpa_state_machine *sm, int vlan_id) {
  ...
  wpa_printf(MSG_DEBUG,
             "WPA: Moving STA " MACSTR " to use group state "
             "machine for VLAN ID %d",
...
void wpa_auth_eapol_key_tx_status(struct wpa_authenticator *wpa_auth,
  ...
  if (wpa_auth == NULL || sm == NULL)
    return;
  wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA " MACSTR " ack=%d",
             MAC2STR(sm->addr), ack);
