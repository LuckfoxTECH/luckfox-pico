...
static int prune_associations(struct hostapd_iface *iface, void *ctx) {
  ...
  for (j = 0; j < iface->num_bss; j++) {
    ...
    if (ohapd->iface->fst && data->hapd->iface->fst &&
        fst_are_ifaces_aggregated(ohapd->iface->fst, data->hapd->iface->fst))
      continue;
#endif /* CONFIG_FST */
...
void hostapd_prune_associations(struct hostapd_data *hapd, const u8 *addr) {
  ...
  if (hapd->iface->interfaces && hapd->iface->interfaces->for_each_interface)
    hapd->iface->interfaces->for_each_interface(hapd->iface->interfaces,
