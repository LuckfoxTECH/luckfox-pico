...
static void hostapd_broadcast_key_clear_iface(struct hostapd_data *hapd,
  ...
  for (i = 0; i < NUM_WEP_KEYS; i++) {
    if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE, NULL, i, 0, NULL, 0,
                            NULL, 0)) {
...
static int hostapd_broadcast_wep_set(struct hostapd_data *hapd) {
  ...
  if (ssid->wep.default_len &&
      hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP,
                          broadcast_ether_addr, idx, 1, NULL, 0,
                          ssid->wep.key[idx], ssid->wep.len[idx])) {
...
static void hostapd_free_hapd_data(struct hostapd_data *hapd) {
  ...
  if (!hapd->started) {
    wpa_printf(MSG_ERROR, "%s: Interface %s wasn't started", __func__,
               hapd->conf->iface);
    return;
  }
  hapd->started = 0;
...
#ifdef CONFIG_SQLITE
  bin_clear_free(hapd->tmp_eap_user.identity, hapd->tmp_eap_user.identity_len);
  bin_clear_free(hapd->tmp_eap_user.password, hapd->tmp_eap_user.password_len);
#endif /* CONFIG_SQLITE */
...
static void hostapd_cleanup(struct hostapd_data *hapd) {
  wpa_printf(MSG_DEBUG, "%s(hapd=%p (%s))", __func__, hapd, hapd->conf->iface);
  if (hapd->iface->interfaces && hapd->iface->interfaces->ctrl_iface_deinit)
...
static int hostapd_setup_encryption(char *iface, struct hostapd_data *hapd) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (hapd->conf->ssid.wep.key[i] && i == hapd->conf->ssid.wep.idx)
      hostapd_set_privacy(hapd, 1);
  }
...
static int hostapd_flush_old_stations(struct hostapd_data *hapd, u16 reason) {
  ...
  if (!hapd->iface->driver_ap_teardown) {
    ...
    if (hostapd_flush(hapd)) {
      wpa_msg(hapd->msg_ctx, MSG_WARNING, "Could not connect to kernel driver");
      ret = -1;
    }
  }
...
static int hostapd_validate_bssid_configuration(struct hostapd_iface *iface) {
  ...
  for (j = 0; j < iface->conf->num_bss; j++) {
    ...
    for (i = 0; i < ETH_ALEN; i++) {
      mask[i] |= iface->conf->bss[j]->bssid[i] ^ hapd->own_addr[i];
    }
  }
  ...
  if (bits > 40) {
    wpa_printf(MSG_ERROR, "Too many bits in the BSSID mask (%u)", bits);
    return -1;
  }
  ...
  for (i = 0; i < ETH_ALEN; i++) {
    if ((hapd->own_addr[i] & mask[i]) != hapd->own_addr[i]) {
      wpa_printf(MSG_ERROR,
                 "Invalid BSSID mask " MACSTR " for start address " MACSTR ".",
...
static int hostapd_das_nas_mismatch(struct hostapd_data *hapd,
  ...
  if (attr->nas_identifier &&
      (!hapd->conf->nas_identifier ||
       os_strlen(hapd->conf->nas_identifier) != attr->nas_identifier_len ||
  ...
  if (attr->nas_ip_addr &&
      (hapd->conf->own_ip_addr.af != AF_INET ||
       os_memcmp(&hapd->conf->own_ip_addr.u.v4, attr->nas_ip_addr, 4) != 0)) {
    wpa_printf(MSG_DEBUG, "RADIUS DAS: NAS-IP-Address mismatch");
    return 1;
  }

#ifdef CONFIG_IPV6
  if (attr->nas_ipv6_addr && (hapd->conf->own_ip_addr.af != AF_INET6 ||
                              os_memcmp(&hapd->conf->own_ip_addr.u.v6,
                                        attr->nas_ipv6_addr, 16) != 0)) {
...
static struct sta_info *hostapd_das_find_sta(struct hostapd_data *hapd,
  ...
  *multi = 0;
  ...
  if (attr->sta_addr) {
    ...
    if (!sta) {
      wpa_printf(MSG_DEBUG, "RADIUS DAS: No Calling-Station-Id match");
      return NULL;
    }
  ...
  if (attr->acct_session_id) {
    ...
    if (attr->acct_session_id_len != 16) {
      wpa_printf(MSG_DEBUG, "RADIUS DAS: Acct-Session-Id cannot match");
      return NULL;
    }
    count = 0;
  ...
  if (attr->acct_multi_session_id) {
    ...
    if (attr->acct_multi_session_id_len != 16) {
      wpa_printf(MSG_DEBUG, "RADIUS DAS: Acct-Multi-Session-Id cannot match");
      return NULL;
    }
    count = 0;
    ...
    for (sta = hapd->sta_list; sta; sta = sta->next) {
      if (!sta->radius_das_match)
        continue;
      if (!sta->eapol_sm || !sta->eapol_sm->acct_multi_session_id) {
        sta->radius_das_match = 0;
        continue;
      }
      os_snprintf(buf, sizeof(buf), "%016llX",
    ...
    wpa_printf(MSG_DEBUG, "RADIUS DAS: Acct-Multi-Session-Id match");
  }
  ...
  if (attr->cui) {
    ...
    for (sta = hapd->sta_list; sta; sta = sta->next) {
      ...
      if (!cui || wpabuf_len(cui) != attr->cui_len ||
          os_memcmp(wpabuf_head(cui), attr->cui, attr->cui_len) != 0)
        sta->radius_das_match = 0;
      else
        count++;
    }
    ...
    if (count == 0) {
      wpa_printf(MSG_DEBUG, "RADIUS DAS: No matches remaining after "
                            "Chargeable-User-Identity check");
      return NULL;
    }
    wpa_printf(MSG_DEBUG, "RADIUS DAS: Chargeable-User-Identity match");
  }
  ...
  if (attr->user_name) {
    ...
    for (sta = hapd->sta_list; sta; sta = sta->next) {
      ...
      if (!identity || identity_len != attr->user_name_len ||
          os_memcmp(identity, attr->user_name, identity_len) != 0)
        sta->radius_das_match = 0;
      else
        count++;
    }
    ...
    wpa_printf(MSG_DEBUG, "RADIUS DAS: User-Name match");
  }
...
hostapd_das_disconnect(void *ctx, struct radius_das_attrs *attr) {
  ...
  if (sta == NULL) {
    ...
    if (hostapd_das_disconnect_pmksa(hapd, attr) == 0) {
      wpa_printf(MSG_DEBUG, "RADIUS DAS: PMKSA cache entry matched");
  ...
  wpa_printf(MSG_DEBUG,
             "RADIUS DAS: Found a matching session " MACSTR " - disconnecting",
             MAC2STR(sta->addr));
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first) {
  ...
  wpa_printf(MSG_DEBUG, "%s(hapd=%p (%s), first=%d)", __func__, hapd,
             conf->iface, first);
  ...
  if (hapd->started) {
    wpa_printf(MSG_ERROR, "%s: Interface %s was already started", __func__,
               conf->iface);
    return -1;
  }
  hapd->started = 1;
  ...
  if (!first || first == -1) {
    ...
    if (!is_zero_ether_addr(conf->bssid)) {
      ...
      if (hostapd_mac_comp(hapd->own_addr, hapd->iface->bss[0]->own_addr) ==
          0) {
        wpa_printf(MSG_ERROR,
                   "BSS '%s' may not have "
    ...
    hapd->interface_added = 1;
    if (hostapd_if_add(hapd->iface->bss[0], WPA_IF_AP_BSS, conf->iface, addr,
                       hapd, &hapd->drv_priv, force_ifname, if_addr,
                       conf->bridge[0] ? conf->bridge : NULL, first == -1)) {
      wpa_printf(MSG_ERROR, "Failed to add BSS (BSSID=" MACSTR ")",
  ...
  if (!hostapd_drv_none(hapd)) {
    wpa_printf(MSG_ERROR,
               "Using interface %s with hwaddr " MACSTR " and ssid \"%s\"",
   ...
   * response frames) */
  if (set_ssid &&
      hostapd_set_ssid(hapd, conf->ssid.ssid, conf->ssid.ssid_len)) {
    wpa_printf(MSG_ERROR, "Could not set SSID for kernel driver");
    return -1;
  }
  ...
  if (conf->radius_das_port) {
    ...
    das_conf.shared_secret_len = conf->radius_das_shared_secret_len;
    das_conf.client_addr = &conf->radius_das_client_addr;
    das_conf.time_window = conf->radius_das_time_window;
    das_conf.require_event_timestamp = conf->radius_das_require_event_timestamp;
  ...
  if (conf->qos_map_set_len &&
      hostapd_drv_set_qos_map(hapd, conf->qos_map_set, conf->qos_map_set_len)) {
...
static void hostapd_tx_queue_params(struct hostapd_iface *iface) {
  ...
  for (i = 0; i < NUM_TX_QUEUES; i++) {
    ...
    if (hostapd_set_tx_queue_params(hapd, i, p->aifs, p->cwmin, p->cwmax,
                                    p->burst)) {
      wpa_printf(MSG_DEBUG,
...
static int hostapd_set_acl_list(struct hostapd_data *hapd,
                                struct mac_acl_entry *mac_acl, int n_entries,
                                u8 accept_acl) {
  ...
  for (i = 0; i < n_entries; i++)
    os_memcpy(acl_params->mac_acl[i].addr, mac_acl[i].addr, ETH_ALEN);
...
static void hostapd_set_acl(struct hostapd_data *hapd) {
  ...
  if (conf->bss[0]->macaddr_acl == DENY_UNLESS_ACCEPTED) {
    ...
    err = hostapd_set_acl_list(hapd, conf->bss[0]->accept_mac,
                               conf->bss[0]->num_accept_mac, accept_acl);
    if (err) {
    ...
    err = hostapd_set_acl_list(hapd, conf->bss[0]->deny_mac,
                               conf->bss[0]->num_deny_mac, accept_acl);
    if (err) {
...
static int start_ctrl_iface_bss(struct hostapd_data *hapd) {
  if (!hapd->iface->interfaces || !hapd->iface->interfaces->ctrl_iface_init)
    return 0;
...
static int setup_interface(struct hostapd_iface *iface) {
  ...
  if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
    ...
    if (os_strncmp(previous_country, country, 2) != 0) {
      ...
      eloop_register_timeout(5, 0, channel_list_update_timeout, iface, NULL);
      return 0;
    }
  }
...
static const struct wpabuf *fst_hostapd_get_mb_ie_cb(void *ctx,
                                                     const u8 *addr) {
  struct hostapd_data *hapd = ctx;
...
static void hostapd_set_own_neighbor_report(struct hostapd_data *hapd) {
  ...
  if (ht) {
    bssid_info |= NEI_REP_BSSID_INFO_HT | NEI_REP_BSSID_INFO_DELAYED_BA;
  ...
  ieee80211_freq_to_channel_ext(
      hapd->iface->freq, hapd->iconf->secondary_channel,
      hapd->iconf->vht_oper_chwidth, &op_class, &channel);
  width = hostapd_get_nr_chan_width(hapd, ht, vht);
  if (vht) {
    center_freq1 = ieee80211_chan_to_freq(
        NULL, op_class, hapd->iconf->vht_oper_centr_freq_seg0_idx);
    if (width == NR_CHAN_WIDTH_80P80)
      center_freq2 = ieee80211_chan_to_freq(
          NULL, op_class, hapd->iconf->vht_oper_centr_freq_seg1_idx);
  } else if (ht) {
    center_freq1 = hapd->iface->freq + 10 * hapd->iconf->secondary_channel;
  }
...
static int hostapd_setup_interface_complete_sync(struct hostapd_iface *iface,
  ...
  if (iface->conf->channel) {
    ...
    wpa_printf(MSG_DEBUG,
               "Mode: %s  Channel: %d  "
               "Frequency: %d MHz",
               hostapd_hw_mode_txt(iface->conf->hw_mode), iface->conf->channel,
               iface->freq);
    ...
    if (iface->mconf != NULL) {
      wpa_printf(MSG_DEBUG,
                 "%s: Mesh configuration will be applied while joining the "
                 "mesh network",
    ...
    if (!delay_apply_cfg &&
        hostapd_set_freq(
            hapd, hapd->iconf->hw_mode, iface->freq, hapd->iconf->channel,
  ...
  if (iface->current_mode) {
    if (hostapd_prepare_rates(iface, iface->current_mode)) {
      ...
      hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_WARNING, "Failed to prepare rates table.");
      goto fail;
    }
  }
  ...
  if ((iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) && !res_dfs_offload) {
    /*
...
dfs_offload:
  ...
  if (hapd->iconf->fst_cfg.group_id[0]) {
    ...
    iface->fst = fst_attach(hapd->conf->iface, hapd->own_addr, &iface_obj,
                            &hapd->iconf->fst_cfg);
    if (!iface->fst) {
...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err) {
  ...
  if (not_ready_in_sync_ifaces > 1) {
    ...
    wpa_printf(MSG_INFO, "%s: Interface waiting to sync with other interfaces",
               iface->bss[0]->conf->iface);
    return 0;
  }

  wpa_printf(MSG_INFO, "%s: Last interface to sync - starting all interfaces",
             iface->bss[0]->conf->iface);
  iface->need_to_start_in_sync = 0;
...
static void hostapd_bss_deinit(struct hostapd_data *hapd) {
  if (!hapd)
    return;
  wpa_printf(MSG_DEBUG, "%s: deinit bss %s", __func__, hapd->conf->iface);
  hostapd_bss_deinit_no_free(hapd);
  wpa_msg(hapd->msg_ctx, MSG_INFO, AP_EVENT_DISABLED);
...
fail:
  wpa_printf(MSG_ERROR, "Failed to set up interface with %s", config_file);
  if (conf)
    hostapd_config_free(conf);
  if (hapd_iface) {
    os_free(hapd_iface->config_fname);
    os_free(hapd_iface->bss);
    wpa_printf(MSG_DEBUG, "%s: free iface %p", __func__, hapd_iface);
    os_free(hapd_iface);
  }
  return NULL;
}

static int ifname_in_use(struct hapd_interfaces *interfaces,
                         const char *ifname) {
  size_t i, j;
...
hostapd_interface_init_bss(struct hapd_interfaces *interfaces, const char *phy,
  ...
  wpa_printf(MSG_INFO, "Configuration file: %s (phy %s)%s", config_fname, phy,
             iface ? "" : " --> new PHY");
  if (iface) {
    struct hostapd_config *conf;
    ...
    if (ifname[0] != '\0' && ifname_in_use(interfaces, ifname)) {
      wpa_printf(MSG_ERROR, "Interface name %s already in use", ifname);
      hostapd_config_free(conf);
      return NULL;
    }

    tmp_conf = os_realloc_array(iface->conf->bss, iface->conf->num_bss + 1,
  ...
  if (iface->conf->bss[bss_idx]->iface[0] == '\0' &&
    ...
    wpa_printf(MSG_ERROR, "Interface name not specified in %s", config_fname);
    if (new_iface)
...
void hostapd_interface_deinit_free(struct hostapd_iface *iface) {
  ...
  if (iface == NULL)
    return;
  wpa_printf(MSG_DEBUG, "%s: num_bss=%u conf->num_bss=%u", __func__,
             (unsigned int)iface->num_bss, (unsigned int)iface->conf->num_bss);
...
static void hostapd_deinit_driver(const struct wpa_driver_ops *driver,
  ...
  if (driver && driver->hapd_deinit && drv_priv) {
    ...
    for (j = 0; j < hapd_iface->num_bss; j++) {
      wpa_printf(MSG_DEBUG, "%s:bss[%d]->drv_priv=%p", __func__, (int)j,
...
int hostapd_reload_iface(struct hostapd_iface *hapd_iface) {
  ...
  wpa_printf(MSG_DEBUG, "Reload interface %s", hapd_iface->conf->bss[0]->iface);
  for (j = 0; j < hapd_iface->num_bss; j++)
    hostapd_set_security_params(hapd_iface->conf->bss[j], 1);
  if (hostapd_config_check(hapd_iface->conf, 1) < 0) {
...
int hostapd_disable_iface(struct hostapd_iface *hapd_iface) {
  ...
  hapd_iface->driver_ap_teardown =
      !!(hapd_iface->drv_flags & WPA_DRIVER_FLAGS_AP_TEARDOWN_SUPPORT);
...
hostapd_iface_alloc(struct hapd_interfaces *interfaces) {
  ...
  hapd_iface = interfaces->iface[interfaces->count] = hostapd_alloc_iface();
  if (hapd_iface == NULL) {
    wpa_printf(MSG_ERROR,
...
int hostapd_add_iface(struct hapd_interfaces *interfaces, char *buf) {
  ...
  if (os_strncmp(buf, "bss_config=", 11) == 0) {
    ...
    *pos++ = '\0';
    ...
    hapd_iface = hostapd_interface_init_bss(interfaces, phy_name, conf_file, 0);
    if (!hapd_iface)
      return -1;
    ...
    if (new_iface) {
      ...
      os_memcpy(hapd->own_addr, hapd_iface->bss[0]->own_addr, ETH_ALEN);
      ...
      if (start_ctrl_iface_bss(hapd) < 0 ||
        ...
        hapd_iface->num_bss--;
        wpa_printf(MSG_DEBUG, "%s: free hapd %p %s", __func__, hapd,
                   hapd->conf->iface);
  ...
  *ptr++ = '\0';
  ...
  for (i = 0; i < interfaces->count; i++) {
    if (!os_strcmp(interfaces->iface[i]->conf->bss[0]->iface, buf)) {
  ...
  if (conf_file && interfaces->config_read_cb) {
    ...
    if (conf && conf->bss)
      os_strlcpy(conf->bss[0]->iface, buf, sizeof(conf->bss[0]->iface));
  } else {
  ...
  wpa_printf(MSG_INFO, "Add interface '%s'", hapd_iface->conf->bss[0]->iface);
  ...
  if (hapd_iface) {
    if (hapd_iface->bss) {
      for (i = 0; i < hapd_iface->num_bss; i++) {
        ...
        if (!hapd)
          continue;
        if (hapd_iface->interfaces && hapd_iface->interfaces->ctrl_iface_deinit)
          hapd_iface->interfaces->ctrl_iface_deinit(hapd);
        wpa_printf(MSG_DEBUG, "%s: free hapd %p (%s)", __func__,
                   hapd_iface->bss[i], hapd->conf->iface);
        hostapd_cleanup(hapd);
        os_free(hapd);
        hapd_iface->bss[i] = NULL;
      }
  ...
  for (i = 0; i < interfaces->count; i++) {
    ...
    if (!os_strcmp(hapd_iface->conf->bss[0]->iface, buf)) {
      ...
      while (k < (interfaces->count - 1)) {
        interfaces->iface[k] = interfaces->iface[k + 1];
        k++;
      }
      interfaces->count--;
      return 0;
...
void hostapd_new_assoc_sta(struct hostapd_data *hapd, struct sta_info *sta,
  ...
  if (!(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_INACTIVITY_TIMER)) {
    wpa_printf(MSG_DEBUG,
               "%s: %s: reschedule ap_handle_timer timeout for " MACSTR
               " (%d seconds - ap_max_inactivity)",
               hapd->conf->iface, __func__, MAC2STR(sta->addr),
               hapd->conf->ap_max_inactivity);
    eloop_cancel_timeout(ap_handle_timer, hapd, sta);
    eloop_register_timeout(hapd->conf->ap_max_inactivity, 0, ap_handle_timer,
                           hapd, sta);
  }
}
...
void hostapd_set_state(struct hostapd_iface *iface,
...
static int hostapd_build_beacon_data(struct hostapd_data *hapd,
  ...
  ret = hostapd_build_ap_extra_ies(hapd, &beacon_extra, &proberesp_extra,
                                   &assocresp_extra);
  if (ret)
    goto free_ap_params;
  ...
  if (params.proberesp != NULL) {
    ...
    os_memcpy(beacon->probe_resp, params.proberesp, params.proberesp_len);
  ...
  if (beacon_extra) {
    ...
    os_memcpy(beacon->beacon_ies, beacon_extra->buf, wpabuf_len(beacon_extra));
    beacon->beacon_ies_len = wpabuf_len(beacon_extra);
  }

  if (proberesp_extra) {
    beacon->proberesp_ies = os_malloc(wpabuf_len(proberesp_extra));
  ...
  if (assocresp_extra) {
    beacon->assocresp_ies = os_malloc(wpabuf_len(assocresp_extra));
...
static int hostapd_change_config_freq(struct hostapd_data *hapd,
  ...
  if (old_params &&
      hostapd_set_freq_params(
          old_params, conf->hw_mode, hostapd_hw_get_freq(hapd, conf->channel),
          conf->channel, conf->ieee80211n, conf->ieee80211ac,
          conf->secondary_channel, conf->vht_oper_chwidth,
          conf->vht_oper_centr_freq_seg0_idx,
          conf->vht_oper_centr_freq_seg1_idx, conf->vht_capab))
    return -1;
...
static int hostapd_fill_csa_settings(struct hostapd_data *hapd,
  ...
  if (ieee80211_freq_to_channel_ext(settings->freq_params.freq,
                                    ...
                                    vht_bandwidth, &hapd->iface->cs_oper_class,
                                    &chan) == NUM_HOSTAPD_MODES) {
    wpa_printf(MSG_DEBUG,
               "invalid frequency for channel switch (freq=%d, "
               "sec_channel_offset=%d, vht_enabled=%d)",
  ...
  hostapd_change_config_freq(iface->bss[0], iface->conf, &old_freq, NULL);
...
void hostapd_switch_channel_fallback(
