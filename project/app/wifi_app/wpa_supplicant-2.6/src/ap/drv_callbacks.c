...
int hostapd_notif_assoc(struct hostapd_data *hapd, const u8 *addr,
  ...
  if (elems.p2p) {
    ...
    sta->p2p_ie =
        ieee802_11_vendor_ie_concat(req_ies, req_ies_len, P2P_IE_VENDOR_TYPE);
    if (sta->p2p_ie)
  ...
  if (hapd->conf->wpa) {
    ...
    if (hapd->conf->wps_state && ie[0] == 0xdd && ie[1] >= 4 &&
      ...
      wps = ieee802_11_vendor_ie_concat(ie, ielen, WPS_IE_VENDOR_TYPE);
      if (wps) {
        if (wps_is_20(wps)) {
    ...
    res = wpa_validate_wpa_ie(hapd->wpa_auth, sta->wpa_sm, ie, ielen,
                              elems.mdie, elems.mdie_len);
    if (res != WPA_IE_OK) {
      wpa_printf(MSG_DEBUG, "WPA/RSN information element rejected? (res %u)",
                 res);
    ...
    if (sta->auth_alg == WLAN_AUTH_FT) {
      status = wpa_ft_validate_reassoc(sta->wpa_sm, req_ies, req_ies_len);
    ...
    if (req_ies)
      wps =
          ieee802_11_vendor_ie_concat(req_ies, req_ies_len, WPS_IE_VENDOR_TYPE);
    else
      wps = NULL;
    ...
    if (sta->wpa_sm == NULL) {
      wpa_printf(MSG_WARNING, "Failed to initialize WPA state machine");
...
#ifdef CONFIG_IEEE80211R
  p = wpa_sm_write_assoc_resp_ies(sta->wpa_sm, buf, sizeof(buf), sta->auth_alg,
                                  req_ies, req_ies_len);
...
#ifdef CONFIG_P2P
  if (req_ies) {
    p2p_group_notif_assoc(hapd->p2p_group, sta->addr, req_ies, req_ies_len);
  }
#endif /* CONFIG_P2P */
...
void hostapd_notif_disassoc(struct hostapd_data *hapd, const u8 *addr) {
  ...
  if (addr == NULL) {
     ...
     * event here.
     */
    wpa_printf(MSG_DEBUG, "hostapd_notif_disassoc: Skip event with no address");
    return;
  }
  ...
  sta = ap_get_sta(hapd, addr);
  if (sta == NULL) {
    wpa_printf(MSG_DEBUG, "Disassociation notification for unknown STA " MACSTR,
               MAC2STR(addr));
    return;
  }
...
void hostapd_event_ch_switch(struct hostapd_data *hapd, int freq, int ht,
  ...
  hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_INFO,
                 "driver had channel switch: freq=%d, ht=%d, offset=%d, "
                 "width=%d (%s), cf1=%d, cf2=%d",
                 freq, ht, offset, width, channel_width_to_string(width), cf1,
                 cf2);
  ...
  if (hapd->csa_in_progress && freq == hapd->cs_freq_params.freq) {
    ...
    wpa_msg(hapd->msg_ctx, MSG_INFO, AP_CSA_FINISHED "freq=%d dfs=%d", freq,
            is_dfs);
...
void hostapd_event_connect_failed_reason(struct hostapd_data *hapd,
  ...
  case MAX_CLIENT_REACHED:
    wpa_msg(hapd->msg_ctx, MSG_INFO, AP_REJECTED_MAX_STA MACSTR, MAC2STR(addr));
    break;
  case BLOCKED_CLIENT:
...
void hostapd_acs_channel_selected(struct hostapd_data *hapd,
  ...
  if (!hapd->iface->current_mode) {
    ...
    if (!hapd->iface->current_mode) {
      hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211,
                     HOSTAPD_LEVEL_WARNING, "driver selected to bad hw_mode");
      err = 1;
      goto out;
    }
  }
  ...
  if (hapd->iface->conf->ieee80211ac) {
    ...
    if (acs_res->ch_width == 80) {
      hapd->iconf->vht_oper_centr_freq_seg0_idx = acs_res->vht_seg0_center_ch;
      hapd->iconf->vht_oper_chwidth = VHT_CHANWIDTH_80MHZ;
    } else if (acs_res->ch_width == 160) {
      if (acs_res->vht_seg1_center_ch == 0) {
        hapd->iconf->vht_oper_centr_freq_seg0_idx = acs_res->vht_seg0_center_ch;
        hapd->iconf->vht_oper_chwidth = VHT_CHANWIDTH_160MHZ;
      } else {
        hapd->iconf->vht_oper_centr_freq_seg0_idx = acs_res->vht_seg0_center_ch;
        hapd->iconf->vht_oper_centr_freq_seg1_idx = acs_res->vht_seg1_center_ch;
        hapd->iconf->vht_oper_chwidth = VHT_CHANWIDTH_80P80MHZ;
      }
    }
  }
...
int hostapd_probe_req_rx(struct hostapd_data *hapd, const u8 *sa, const u8 *da,
  ...
  for (i = 0; hapd->probereq_cb && i < hapd->num_probereq_cb; i++) {
    if (hapd->probereq_cb[i].cb(hapd->probereq_cb[i].ctx, sa, da, bssid, ie,
                                ie_len, ssi_signal) > 0) {
      ret = 1;
      break;
    }
...
static void hostapd_notify_auth_ft_finish(void *ctx, const u8 *dst,
                                          const u8 *bssid, u16 auth_transaction,
                                          u16 status, const u8 *ies,
                                          size_t ies_len) {
...
static void hostapd_notif_auth(struct hostapd_data *hapd,
  ...
  if (rx_auth->auth_type == WLAN_AUTH_FT && hapd->wpa_auth) {
    ...
    if (sta->wpa_sm == NULL)
      sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth, sta->addr, NULL);
    if (sta->wpa_sm == NULL) {
      wpa_printf(MSG_DEBUG, "FT: Failed to initialize WPA state machine");
      status = WLAN_STATUS_UNSPECIFIED_FAILURE;
      goto fail;
    }
    wpa_ft_process_auth(sta->wpa_sm, rx_auth->bssid, rx_auth->auth_transaction,
                        rx_auth->ies, rx_auth->ies_len,
                        hostapd_notify_auth_ft_finish, hapd);
    return;
  }
#endif /* CONFIG_IEEE80211R */
fail:
  hostapd_sta_auth(hapd, rx_auth->peer, rx_auth->auth_transaction + 1, status,
...
static int hostapd_mgmt_rx(struct hostapd_data *hapd, struct rx_mgmt *rx_mgmt) {
  ...
  if (hapd->ext_mgmt_frame_handling) {
    ...
    if (hex) {
      wpa_snprintf_hex(hex, hex_len, rx_mgmt->frame, rx_mgmt->frame_len);
  ...
  if (hapd == HAPD_BROADCAST) {
    ...
    for (i = 0; i < iface->num_bss; i++) {
      ...
      if (ieee802_11_mgmt(iface->bss[i], rx_mgmt->frame, rx_mgmt->frame_len,
                          &fi) > 0)
        ret = 1;
    }
  } else
    ret = ieee802_11_mgmt(hapd, rx_mgmt->frame, rx_mgmt->frame_len, &fi);
...
static int hostapd_event_new_sta(struct hostapd_data *hapd, const u8 *addr) {
  ...
  wpa_printf(MSG_DEBUG,
             "Data frame from unknown STA " MACSTR " - adding a new STA",
             MAC2STR(addr));
  sta = ap_sta_add(hapd, addr);
  if (sta) {
    hostapd_new_assoc_sta(hapd, sta, 0);
  } else {
    wpa_printf(MSG_DEBUG, "Failed to add STA entry for " MACSTR, MAC2STR(addr));
    return -1;
  }
...
hostapd_get_mode_channel(struct hostapd_iface *iface, unsigned int freq) {
  int i;
...
static void
hostapd_single_channel_get_survey(struct hostapd_iface *iface,
                                  struct survey_results *survey_res) {
  struct hostapd_channel_data *chan;
  struct freq_survey *survey;
  u64 divisor, dividend;

  survey = dl_list_first(&survey_res->survey_list, struct freq_survey, list);
  ...
  wpa_printf(
      ...
      survey->freq, (unsigned long int)survey->channel_time,
      (unsigned long int)survey->channel_time_busy);

  if (survey->channel_time > iface->last_channel_time &&
      survey->channel_time > survey->channel_time_busy) {
    dividend = survey->channel_time_busy - iface->last_channel_time_busy;
...
static void hostapd_event_iface_unavailable(struct hostapd_data *hapd) {
  ...
  if (hapd->csa_in_progress) {
    wpa_printf(MSG_INFO, "CSA failed (%s was stopped)", hapd->conf->iface);
...
static void hostapd_event_dfs_radar_detected(struct hostapd_data *hapd,
  ...
  hostapd_dfs_radar_detected(hapd->iface, radar->freq, radar->ht_enabled,
                             radar->chan_offset, radar->chan_width, radar->cf1,
                             radar->cf2);
}
...
void wpa_supplicant_event(void *ctx, enum wpa_event_type event,
  ...
  case EVENT_TX_STATUS:
    ...
    case WLAN_FC_TYPE_MGMT:
      hostapd_mgmt_tx_cb(hapd, data->tx_status.data, data->tx_status.data_len,
                         data->tx_status.stype, data->tx_status.ack);
      break;
    case WLAN_FC_TYPE_DATA:
      hostapd_tx_status(hapd, data->tx_status.dst, data->tx_status.data,
  ...
  case EVENT_EAPOL_TX_STATUS:
    hostapd_eapol_tx_status(
        hapd, data->eapol_tx_status.dst, data->eapol_tx_status.data,
  ...
  case EVENT_RX_PROBE_REQ:
    if (data->rx_probe_req.sa == NULL || data->rx_probe_req.ie == NULL)
      break;
    hostapd_probe_req_rx(hapd, data->rx_probe_req.sa, data->rx_probe_req.da,
  ...
  case EVENT_NEW_STA:
    ...
    break;
  case EVENT_EAPOL_RX:
    hostapd_event_eapol_rx(hapd, data->eapol_rx.src, data->eapol_rx.data,
                           data->eapol_rx.data_len);
    break;
  case EVENT_ASSOC:
    if (!data)
      return;
    hostapd_notif_assoc(hapd, data->assoc_info.addr, data->assoc_info.req_ies,
                        data->assoc_info.req_ies_len, data->assoc_info.reassoc);
    break;
  case EVENT_DISASSOC:
    if (data)
  ...
  case EVENT_CONNECT_FAILED_REASON:
    if (!data)
      break;
    hostapd_event_connect_failed_reason(hapd, data->connect_failed_reason.addr,
  ...
  case EVENT_ACS_CHANNEL_SELECTED:
    hostapd_acs_channel_selected(hapd, &data->acs_selected_channels);
    break;
#endif /* CONFIG_ACS */
  default:
...
void wpa_supplicant_event_global(void *ctx, enum wpa_event_type event,
  ...
  if (hapd && hapd->driver && hapd->driver->get_ifindex && hapd->drv_priv) {
    unsigned int ifindex;

    ifindex = hapd->driver->get_ifindex(hapd->drv_priv);
    if (ifindex != data->interface_status.ifindex) {
      wpa_dbg(hapd->msg_ctx, MSG_DEBUG,
              "interface status ifindex %d mismatch (%d)", ifindex,
