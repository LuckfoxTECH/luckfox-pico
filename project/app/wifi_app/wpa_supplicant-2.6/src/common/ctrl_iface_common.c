  ...
  case AF_UNIX: {
    ...
    if (a_len != b_len ||
        os_memcmp(u_a->sun_path, u_b->sun_path,
                  a_len - offsetof(struct sockaddr_un, sun_path)) != 0)
      return 1;
    break;
  }
...
void sockaddr_print(int level, const char *msg, struct sockaddr_storage *sock,
  ...
  case AF_INET6: {
    ...
    getnameinfo((struct sockaddr *)sock, socklen, host, sizeof(host), service,
...
int ctrl_iface_level(struct dl_list *ctrl_dst, struct sockaddr_storage *from,
  ...
  dl_list_for_each(dst, ctrl_dst, struct wpa_ctrl_dst, list) {
    if (!sockaddr_compare(from, fromlen, &dst->addr, dst->addrlen)) {
      sockaddr_print(MSG_DEBUG, "CTRL_IFACE changed monitor level", from,
                     fromlen);
