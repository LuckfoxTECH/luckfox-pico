...
int sae_set_group(struct sae_data *sae, int group) {
  ...
  if (tmp->dh) {
    ...
    tmp->prime_buf = crypto_bignum_init_set(tmp->dh->prime, tmp->prime_len);
    ...
    tmp->order_buf = crypto_bignum_init_set(tmp->dh->order, tmp->dh->order_len);
...
static struct crypto_bignum *sae_get_rand(struct sae_data *sae) {
  ...
  for (;;) {
    ...
    if (bn == NULL)
      return NULL;
    if (crypto_bignum_is_zero(bn) || crypto_bignum_is_one(bn) ||
...
static void sae_pwd_seed_key(const u8 *addr1, const u8 *addr2, u8 *key) {
  wpa_printf(MSG_DEBUG, "SAE: PWE derivation - addr1=" MACSTR " addr2=" MACSTR,
...
static struct crypto_bignum *get_rand_1_to_p_1(const u8 *prime,
                                               size_t prime_len,
                                               size_t prime_bits, int *r_odd) {
  for (;;) {
...
static int is_quadratic_residue_blind(struct sae_data *sae, const u8 *prime,
                                      size_t bits,
  ...
  if (!num || crypto_bignum_mulmod(y_sqr, r, sae->tmp->prime, num) < 0 ||
...
static int sae_test_pwd_seed_ecc(struct sae_data *sae, const u8 *pwd_seed,
  ...
  *ret_x_cand = NULL;
  ...
  wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value", pwd_value, sae->tmp->prime_len);
...
static int sae_test_pwd_seed_ffc(struct sae_data *sae, const u8 *pwd_seed,
  ...
  if (sae->tmp->dh->safe_prime) {
    ...
    exp[0] = 1;
    b = crypto_bignum_init_set(exp, sizeof(exp));
    if (b == NULL || crypto_bignum_sub(sae->tmp->prime, b, b) < 0 ||
...
static int sae_derive_pwe_ecc(struct sae_data *sae, const u8 *addr1,
  ...
  if (crypto_bignum_to_bin(sae->tmp->prime, prime, sizeof(prime), prime_len) <
      0)
    return -1;
  bits = crypto_ec_prime_len_bits(sae->tmp->ec);
   ...
   */
  if (get_random_qr_qnr(prime, prime_len, sae->tmp->prime, bits, &qr, &qnr) < 0)
    return -1;

  wpa_hexdump_ascii_key(MSG_DEBUG, "SAE: password", password, password_len);
  ...
  for (counter = 1; counter <= k || !x; counter++) {
    ...
    if (hmac_sha256_vector(addrs, sizeof(addrs), 2, addr, len, pwd_seed) < 0)
      break;
    ...
    if (res < 0)
      goto fail;
    if (res > 0 && !x) {
      wpa_printf(MSG_DEBUG, "SAE: Selected pwd-seed with counter %u", counter);
      x = x_cand;
  ...
  if (!sae->tmp->pwe_ecc)
    ...
    res = crypto_ec_point_solve_y_coord(sae->tmp->ec, sae->tmp->pwe_ecc, x,
                                        pwd_seed_odd);
...
static int sae_derive_commit_element_ecc(struct sae_data *sae,
  ...
  if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, mask,
      ...
      crypto_ec_point_invert(sae->tmp->ec, sae->tmp->own_commit_element_ecc) <
          0) {
...
static int sae_derive_commit_element_ffc(struct sae_data *sae,
  ...
  if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, mask, sae->tmp->prime,
      ...
      crypto_bignum_inverse(sae->tmp->own_commit_element_ffc, sae->tmp->prime,
                            sae->tmp->own_commit_element_ffc) < 0) {
    wpa_printf(MSG_DEBUG, "SAE: Could not compute commit-element");
    return -1;
  }
...
static int sae_derive_commit(struct sae_data *sae) {
  ...
  do {
    ...
    crypto_bignum_add(sae->tmp->sae_rand, mask, sae->tmp->own_commit_scalar);
...
int sae_prepare_commit(const u8 *addr1, const u8 *addr2, const u8 *password,
                       size_t password_len, struct sae_data *sae) {
  if (sae->tmp == NULL ||
      (sae->tmp->ec &&
       sae_derive_pwe_ecc(sae, addr1, addr2, password, password_len) < 0) ||
      (sae->tmp->dh &&
       sae_derive_pwe_ffc(sae, addr1, addr2, password, password_len) < 0) ||
...
static int sae_derive_k_ecc(struct sae_data *sae, u8 *k) {
  ...
  if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc,
      ...
      crypto_ec_point_add(sae->tmp->ec, K, sae->tmp->peer_commit_element_ecc,
                          K) < 0 ||
...
static int sae_derive_k_ffc(struct sae_data *sae, u8 *k) {
  ...
  if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, sae->peer_commit_scalar,
      ...
      crypto_bignum_to_bin(K, k, SAE_MAX_PRIME_LEN, sae->tmp->prime_len) < 0) {
    wpa_printf(MSG_DEBUG, "SAE: Failed to calculate K and k");
    goto fail;
  }
...
static int sae_derive_keys(struct sae_data *sae, const u8 *k) {
  ...
  hmac_sha256(null_key, sizeof(null_key), k, sae->tmp->prime_len, keyseed);
  wpa_hexdump_key(MSG_DEBUG, "SAE: keyseed", keyseed, sizeof(keyseed));

  crypto_bignum_add(sae->tmp->own_commit_scalar, sae->peer_commit_scalar, tmp);
  crypto_bignum_mod(tmp, sae->tmp->order, tmp);
  crypto_bignum_to_bin(tmp, val, sizeof(val), sae->tmp->prime_len);
  wpa_hexdump(MSG_DEBUG, "SAE: PMKID", val, SAE_PMKID_LEN);
  if (sha256_prf(keyseed, sizeof(keyseed), "SAE KCK and PMK", val,
                 sae->tmp->prime_len, keys, sizeof(keys)) < 0)
    goto fail;
...
int sae_process_commit(struct sae_data *sae) {
  ...
  if (sae->tmp == NULL || (sae->tmp->ec && sae_derive_k_ecc(sae, k) < 0) ||
...
void sae_write_commit(struct sae_data *sae, struct wpabuf *buf,
  ...
  if (token) {
    ...
    wpa_hexdump(MSG_DEBUG, "SAE: Anti-clogging token", wpabuf_head(token),
                wpabuf_len(token));
  }
  pos = wpabuf_put(buf, sae->tmp->prime_len);
  crypto_bignum_to_bin(sae->tmp->own_commit_scalar, pos, sae->tmp->prime_len,
                       sae->tmp->prime_len);
  wpa_hexdump(MSG_DEBUG, "SAE: own commit-scalar", pos, sae->tmp->prime_len);
  if (sae->tmp->ec) {
    pos = wpabuf_put(buf, 2 * sae->tmp->prime_len);
    crypto_ec_point_to_bin(sae->tmp->ec, sae->tmp->own_commit_element_ecc, pos,
                           pos + sae->tmp->prime_len);
    wpa_hexdump(MSG_DEBUG, "SAE: own commit-element(x)", pos,
                sae->tmp->prime_len);
    ...
    wpa_hexdump(MSG_DEBUG, "SAE: own commit-element", pos, sae->tmp->prime_len);
  }
}
...
u16 sae_group_allowed(struct sae_data *sae, int *allowed_groups, u16 group) {
  ...
  if (group != sae->group && sae_set_group(sae, group) < 0) {
    wpa_printf(MSG_DEBUG, "SAE: Unsupported Finite Cyclic Group %u", group);
    return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
  }
...
static void sae_parse_commit_token(struct sae_data *sae, const u8 **pos,
  ...
  if ((sae->tmp->ec ? 3 : 2) * sae->tmp->prime_len < end - *pos) {
    size_t tlen = end - (*pos + (sae->tmp->ec ? 3 : 2) * sae->tmp->prime_len);
...
static u16 sae_parse_commit_scalar(struct sae_data *sae, const u8 **pos,
  ...
  /* 1 < scalar < r */
  if (crypto_bignum_is_zero(peer_scalar) || crypto_bignum_is_one(peer_scalar) ||
  ...
  wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-scalar", *pos, sae->tmp->prime_len);
  *pos += sae->tmp->prime_len;
...
static u16 sae_parse_commit_element_ecc(struct sae_data *sae, const u8 *pos,
  ...
  if (os_memcmp(pos, prime, sae->tmp->prime_len) >= 0 ||
      os_memcmp(pos + sae->tmp->prime_len, prime, sae->tmp->prime_len) >= 0) {
    wpa_printf(MSG_DEBUG, "SAE: Invalid coordinates in peer "
                          "element");
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
  }

  wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-element(x)", pos,
              sae->tmp->prime_len);
...
static u16 sae_parse_commit_element_ffc(struct sae_data *sae, const u8 *pos,
  ...
  wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-element", pos, sae->tmp->prime_len);
  ...
  if (!res || !one || crypto_bignum_sub(sae->tmp->prime, one, res) ||
  ...
  if (crypto_bignum_exptmod(sae->tmp->peer_commit_element_ffc, sae->tmp->order,
                            sae->tmp->prime, res) < 0 ||
...
u16 sae_parse_commit(struct sae_data *sae, const u8 *data, size_t len,
   ...
   */
  if (!sae->tmp->own_commit_scalar ||
      crypto_bignum_cmp(sae->tmp->own_commit_scalar, sae->peer_commit_scalar) !=
          0 ||
      (sae->tmp->dh &&
      ...
      (sae->tmp->ec &&
       (!sae->tmp->own_commit_element_ecc ||
        crypto_ec_point_cmp(sae->tmp->ec, sae->tmp->own_commit_element_ecc,
...
static void sae_cn_confirm_ecc(struct sae_data *sae, const u8 *sc,
  ...
  sae_cn_confirm(sae, sc, scalar1, element_b1, 2 * sae->tmp->prime_len, scalar2,
...
static void sae_cn_confirm_ffc(struct sae_data *sae, const u8 *sc,
  ...
  sae_cn_confirm(sae, sc, scalar1, element_b1, sae->tmp->prime_len, scalar2,
...
void sae_write_confirm(struct sae_data *sae, struct wpabuf *buf) {
  ...
  if (sae->tmp->ec)
    sae_cn_confirm_ecc(
        sae, sc, sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ecc,
        sae->peer_commit_scalar, sae->tmp->peer_commit_element_ecc,
        wpabuf_put(buf, SHA256_MAC_LEN));
  else
    sae_cn_confirm_ffc(
        sae, sc, sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ffc,
        sae->peer_commit_scalar, sae->tmp->peer_commit_element_ffc,
        wpabuf_put(buf, SHA256_MAC_LEN));
}
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len) {
  ...
  if (sae->tmp->ec)
    sae_cn_confirm_ecc(sae, data, sae->peer_commit_scalar,
                       sae->tmp->peer_commit_element_ecc,
                       sae->tmp->own_commit_scalar,
                       sae->tmp->own_commit_element_ecc, verifier);
  else
    sae_cn_confirm_ffc(sae, data, sae->peer_commit_scalar,
                       sae->tmp->peer_commit_element_ffc,
                       sae->tmp->own_commit_scalar,
                       sae->tmp->own_commit_element_ffc, verifier);

  if (os_memcmp_const(verifier, data + 2, SHA256_MAC_LEN) != 0) {
    wpa_printf(MSG_DEBUG, "SAE: Confirm mismatch");
    wpa_hexdump(MSG_DEBUG, "SAE: Received confirm", data + 2, SHA256_MAC_LEN);
    wpa_hexdump(MSG_DEBUG, "SAE: Calculated verifier", verifier,
                SHA256_MAC_LEN);
    return -1;
  }
