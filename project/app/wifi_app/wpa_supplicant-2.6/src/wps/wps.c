...
struct wps_data *wps_init(const struct wps_config *cfg) {
  ...
  if (cfg->pin) {
    ...
    wpa_hexdump_key(MSG_DEBUG, "WPS: AP PIN dev_password", data->dev_password,
                    data->dev_password_len);
  }

#ifdef CONFIG_WPS_NFC
  if (cfg->pin == NULL && cfg->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)
    data->dev_pw_id = cfg->dev_pw_id;
  ...
  if (cfg->wps->ap && !cfg->registrar && cfg->wps->ap_nfc_dev_pw_id) {
    ...
    data->dev_password = os_malloc(wpabuf_len(cfg->wps->ap_nfc_dev_pw));
    if (data->dev_password == NULL) {
      os_free(data);
      return NULL;
    }
    os_memcpy(data->dev_password, wpabuf_head(cfg->wps->ap_nfc_dev_pw),
              wpabuf_len(cfg->wps->ap_nfc_dev_pw));
    data->dev_password_len = wpabuf_len(cfg->wps->ap_nfc_dev_pw);
    wpa_hexdump_key(MSG_DEBUG, "WPS: NFC dev_password", data->dev_password,
                    data->dev_password_len);
  }
  ...
  if (cfg->new_ap_settings) {
    data->new_ap_settings = os_malloc(sizeof(*data->new_ap_settings));
...
void wps_deinit(struct wps_data *data) {
  ...
  if (data->registrar && data->nfc_pw_token)
    wps_registrar_remove_nfc_pw_token(data->wps->registrar, data->nfc_pw_token);
#endif /* CONFIG_WPS_NFC */
  ...
  if (data->wps_pin_revealed) {
    wpa_printf(MSG_DEBUG, "WPS: Full PIN information revealed and "
                          "negotiation failed");
    if (data->registrar)
      wps_registrar_invalidate_pin(data->wps->registrar, data->uuid_e);
...
int wps_is_selected_pbc_registrar(const struct wpabuf *msg) {
  ...
  if (wps_parse_msg(msg, &attr) < 0 || !attr.selected_registrar ||
      *attr.selected_registrar == 0 || !attr.dev_password_id ||
      WPA_GET_BE16(attr.dev_password_id) != DEV_PW_PUSHBUTTON)
    return 0;

#ifdef CONFIG_WPS_STRICT
  if (!attr.sel_reg_config_methods ||
      !(WPA_GET_BE16(attr.sel_reg_config_methods) & WPS_CONFIG_PUSHBUTTON))
    return 0;
...
struct wpabuf *wps_build_assoc_resp_ie(void) {
  ...
  if (wps_build_version(ie) || wps_build_resp_type(ie, WPS_RESP_AP) ||
      wps_build_wfa_ext(ie, 0, NULL, 0)) {
    wpabuf_free(ie);
    return NULL;
  }
...
struct wpabuf *wps_build_probe_req_ie(u16 pw_id, struct wps_device_data *dev,
  ...
  if (wps_build_version(ie) || wps_build_req_type(ie, req_type) ||
      ...
      wps_build_uuid_e(ie, uuid) || wps_build_primary_dev_type(dev, ie) ||
      wps_build_rf_bands(dev, ie, 0) || wps_build_assoc_state(NULL, ie) ||
      wps_build_config_error(ie, WPS_CFG_NO_ERROR) ||
      wps_build_dev_password_id(ie, pw_id) || wps_build_manufacturer(dev, ie) ||
      wps_build_model_name(dev, ie) || wps_build_model_number(dev, ie) ||
...
int wps_attr_text(struct wpabuf *data, char *buf, char *end) {
  ...
  if (attr.ap_setup_locked && *attr.ap_setup_locked) {
    ret = os_snprintf(pos, end - pos, "wps_ap_setup_locked=1\n");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (attr.selected_registrar && *attr.selected_registrar) {
    ret = os_snprintf(pos, end - pos, "wps_selected_registrar=1\n");
    if (os_snprintf_error(end - pos, ret))
      return pos - buf;
    pos += ret;
  }
  ...
  if (attr.dev_password_id) {
    ret = os_snprintf(pos, end - pos, "wps_device_password_id=%u\n",
  ...
  if (attr.primary_dev_type) {
    ...
    ret = os_snprintf(
        pos, end - pos, "wps_primary_device_type=%s\n",
  ...
  if (attr.dev_name) {
    ...
    for (i = 0; i < attr.dev_name_len; i++) {
      if (attr.dev_name[i] == 0 || is_ctrl_char(attr.dev_name[i]))
        str[i] = '_';
      else
  ...
  if (attr.config_methods) {
    ret = os_snprintf(pos, end - pos, "wps_config_methods=0x%04x\n",
