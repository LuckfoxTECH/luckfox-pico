...
static int wps_set_vendor_ext_wfa_subelem(struct wps_parse_attr *attr, u8 id,
                                          u8 len, const u8 *pos) {
  wpa_printf(MSG_EXCESSIVE, "WPS: WFA subelement id=%u len=%u", id, len);
  switch (id) {
  ...
  case WFA_ELEM_REGISTRAR_CONFIGURATION_METHODS:
    if (len != 2) {
      wpa_printf(MSG_DEBUG,
                 "WPS: Invalid Registrar Configuration Methods length %u", len);
      return -1;
    }
...
static int wps_parse_vendor_ext(struct wps_parse_attr *attr, const u8 *pos,
  ...
  if (len > WPS_MAX_VENDOR_EXT_LEN) {
    wpa_printf(MSG_DEBUG, "WPS: Too long Vendor Extension (%u)", len);
    return -1;
  }
...
static int wps_set_attr(struct wps_parse_attr *attr, u16 type, const u8 *pos,
                        u16 len) {
  switch (type) {
  case ATTR_VERSION:
    if (len != 1) {
  ...
  case ATTR_OOB_DEVICE_PASSWORD:
    if (len < WPS_OOB_PUBKEY_HASH_LEN + 2 ||
        len > WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN ||
        (len < WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_MIN_LEN &&
  ...
  case ATTR_DEV_NAME:
    if (len > WPS_DEV_NAME_MAX_LEN) {
      wpa_printf(MSG_DEBUG, "WPS: Ignore too long Device Name (len=%u)", len);
      break;
    }
    attr->dev_name = pos;
  ...
  case ATTR_PUBLIC_KEY:
     ...
     */
    if (len < 190 || len > 192) {
      wpa_printf(MSG_DEBUG, "WPS: Ignore Public Key with unexpected length %u",
                 len);
      break;
    }
    attr->public_key = pos;
  ...
  case ATTR_SECONDARY_DEV_TYPE_LIST:
    if (len > WPS_SEC_DEV_TYPE_MAX_LEN || (len % WPS_DEV_TYPE_LEN) > 0) {
      wpa_printf(MSG_DEBUG,
...
int wps_parse_msg(const struct wpabuf *msg, struct wps_parse_attr *attr) {
  ...
  while (pos < end) {
    ...
    pos += 2;
    wpa_printf(MSG_EXCESSIVE, "WPS: attr type=0x%x len=%u", type, len);
    if (len > end - pos) {
       ...
       * provisioned.
       */
      if ((type & 0xff00) != 0x1000 && prev_type == ATTR_NETWORK_KEY) {
