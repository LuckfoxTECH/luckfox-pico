...
int wps_build_req_dev_type(struct wps_device_data *dev, struct wpabuf *msg,
  ...
  for (i = 0; i < num_req_dev_types; i++) {
    wpa_hexdump(MSG_DEBUG, "WPS: * Requested Device Type",
                req_dev_types + i * WPS_DEV_TYPE_LEN, WPS_DEV_TYPE_LEN);
...
int wps_build_device_attrs(struct wps_device_data *dev, struct wpabuf *msg) {
  if (wps_build_manufacturer(dev, msg) || wps_build_model_name(dev, msg) ||
      wps_build_model_number(dev, msg) || wps_build_serial_number(dev, msg) ||
      wps_build_primary_dev_type(dev, msg) || wps_build_dev_name(dev, msg))
    return -1;
  return 0;
}
...
static int wps_process_serial_number(struct wps_device_data *dev, const u8 *str,
                                     size_t str_len) {
  if (str == NULL) {
...
int wps_process_device_attrs(struct wps_device_data *dev,
  ...
  if (wps_process_manufacturer(dev, attr->manufacturer,
      ...
      wps_process_model_name(dev, attr->model_name, attr->model_name_len) ||
