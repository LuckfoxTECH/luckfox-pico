...
static void event_retry(struct wps_event_ *e, int do_next_address) {
  ...
  wpa_printf(MSG_DEBUG, "WPS UPnP: Retry event %p for subscription %p", e, s);
  event_clean(e);
  /* will set: s->current_event = NULL; */
...
static void event_addr_failure(struct wps_event_ *e) {
  ...
  e->addr->num_failures++;
  wpa_printf(MSG_DEBUG,
             "WPS UPnP: Failed to send event %p to %s "
             "(num_failures=%u)",
   ...
   */
  wpa_printf(MSG_DEBUG,
             "WPS UPnP: Deleting subscription %p "
             "address %s due to errors",
  ...
  if (dl_list_empty(&s->addr_list)) {
    ...
    wpa_printf(MSG_DEBUG,
               "WPS UPnP: Removing subscription %p "
               "with no addresses",
               s);
    dl_list_del(&s->list);
...
static void event_http_cb(void *ctx, struct http_client *c,
  ...
  wpa_printf(MSG_DEBUG,
             "WPS UPnP: HTTP client callback: e=%p c=%p "
             "event=%d",
             e, c, event);
  switch (event) {
  case HTTP_CLIENT_OK:
    wpa_printf(MSG_DEBUG, "WPS UPnP: Got event %p reply OK from %s", e,
               e->addr->domain_and_port);
...
static int event_send_start(struct subscription *s) {
   ...
   */
  if (dl_list_empty(&s->addr_list) || s->current_event ||
  ...
  itry = 0;
  dl_list_for_each(e->addr, &s->addr_list, struct subscr_addr,
                   list) if (itry++ == e->retry) break;
  if (itry < e->retry)
    return -1;
  ...
  e->http_event = http_client_addr(&e->addr->saddr, buf, 0, event_http_cb, e);
...
int event_add(struct subscription *s, const struct wpabuf *data, int probereq) {
  ...
  if (s->last_event_failed && probereq && len > 0) {
     ...
     */
    wpa_printf(MSG_DEBUG,
               "WPS UPnP: Do not queue more Probe "
  ...
  wpa_printf(MSG_DEBUG,
             "WPS UPnP: Queue event %p for subscriber %p "
             "(queue len %u)",
             e, s, len + 1);
