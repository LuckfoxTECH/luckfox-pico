...
static int wps_validate_version(const u8 *version, int mandatory) {
  ...
  if (*version != 0x10) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Version attribute "
               "value 0x%x",
               *version);
    return -1;
  }
...
static int wps_validate_version2(const u8 *version2, int mandatory) {
  ...
  if (*version2 < 0x20) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Version2 attribute "
               "value 0x%x",
               *version2);
    return -1;
  }
...
static int wps_validate_request_type(const u8 *request_type, int mandatory) {
  ...
  if (*request_type > 0x03) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Request Type "
...
static int wps_validate_response_type(const u8 *response_type, int mandatory) {
  ...
  if (*response_type > 0x03) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Response Type "
               "attribute value 0x%x",
               *response_type);
    return -1;
  }
...
static int wps_validate_config_methods(const u8 *config_methods, int wps2,
  ...
  if (!valid_config_methods(val, wps2)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Configuration "
               "Methods attribute value 0x%04x",
               val);
    return -1;
  }
  return 0;
...
static int wps_validate_assoc_state(const u8 *assoc_state, int mandatory) {
  ...
  if (val > 4) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Association State "
               "attribute value 0x%04x",
               val);
    return -1;
  }
  return 0;
...
static int wps_validate_config_error(const u8 *config_error, int mandatory) {
  ...
  if (val > 20) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Configuration Error "
               "attribute value 0x%04x",
               val);
    return -1;
  }
...
static int wps_validate_dev_password_id(const u8 *dev_password_id,
  ...
  if (val >= 0x0008 && val <= 0x000f) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Device Password ID "
               "attribute value 0x%04x",
               val);
    return -1;
  }
...
static int wps_validate_manufacturer(const u8 *manufacturer, size_t len,
  ...
  if (len > 0 && manufacturer[len - 1] == 0) {
    wpa_hexdump_ascii(MSG_INFO,
                      "WPS-STRICT: Invalid Manufacturer "
                      "attribute value",
                      manufacturer, len);
    return -1;
  }
...
static int wps_validate_model_number(const u8 *model_number, size_t len,
  ...
  if (len > 0 && model_number[len - 1] == 0) {
    wpa_hexdump_ascii(MSG_INFO,
                      "WPS-STRICT: Invalid Model Number "
                      "attribute value",
                      model_number, len);
    return -1;
  }
...
static int wps_validate_dev_name(const u8 *dev_name, size_t len,
  ...
  if (len > 0 && dev_name[len - 1] == 0) {
    wpa_hexdump_ascii(MSG_INFO,
                      "WPS-STRICT: Invalid Device Name "
                      "attribute value",
                      dev_name, len);
    return -1;
  }
...
static int wps_validate_request_to_enroll(const u8 *request_to_enroll,
  ...
  if (*request_to_enroll > 0x01) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Request to Enroll "
               "attribute value 0x%x",
               *request_to_enroll);
    return -1;
  }
...
static int wps_validate_wps_state(const u8 *wps_state, int mandatory) {
  ...
  if (*wps_state != WPS_STATE_NOT_CONFIGURED &&
    ...
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Wi-Fi Protected "
               "Setup State attribute value 0x%x",
               *wps_state);
    return -1;
  }
  return 0;
...
static int wps_validate_ap_setup_locked(const u8 *ap_setup_locked,
  ...
  if (*ap_setup_locked > 1) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid AP Setup Locked "
               "attribute value 0x%x",
               *ap_setup_locked);
    return -1;
  }
...
static int wps_validate_selected_registrar(const u8 *selected_registrar,
  ...
  if (*selected_registrar > 1) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Selected Registrar "
               "attribute value 0x%x",
               *selected_registrar);
    return -1;
  }
...
static int wps_validate_authorized_macs(const u8 *authorized_macs, size_t len,
  ...
  if (len > 30 && (len % ETH_ALEN) != 0) {
    wpa_hexdump(MSG_INFO,
                "WPS-STRICT: Invalid Authorized "
                "MACs attribute value",
                authorized_macs, len);
    return -1;
  }
  return 0;
...
static int wps_validate_msg_type(const u8 *msg_type, int mandatory) {
  ...
  if (*msg_type < WPS_Beacon || *msg_type > WPS_WSC_DONE) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Message Type "
               "attribute value 0x%x",
               *msg_type);
    return -1;
  }
...
static int wps_validate_mac_addr(const u8 *mac_addr, int mandatory) {
  ...
  if (mac_addr[0] & 0x01) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid MAC Address "
               "attribute value " MACSTR,
               MAC2STR(mac_addr));
    return -1;
  }
  return 0;
...
static int wps_validate_enrollee_nonce(const u8 *enrollee_nonce,
                                       int mandatory) {
...
static int wps_validate_auth_type_flags(const u8 *flags, int mandatory) {
  ...
  if ((val & ~WPS_AUTH_TYPES) || !(val & WPS_AUTH_WPA2PSK)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Authentication Type "
               "Flags attribute value 0x%04x",
               val);
    return -1;
  }
  return 0;
...
static int wps_validate_auth_type(const u8 *type, int mandatory) {
  ...
  if ((val & ~WPS_AUTH_TYPES) || val == 0 ||
      (num_bits_set(val) > 1 && val != (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK))) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Authentication Type "
               "attribute value 0x%04x",
               val);
...
static int wps_validate_encr_type_flags(const u8 *flags, int mandatory) {
  ...
  if ((val & ~WPS_ENCR_TYPES) || !(val & WPS_ENCR_AES)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Encryption Type "
               "Flags attribute value 0x%04x",
               val);
    return -1;
  }
...
static int wps_validate_conn_type_flags(const u8 *flags, int mandatory) {
  ...
  if ((*flags & ~(WPS_CONN_ESS | WPS_CONN_IBSS)) || !(*flags & WPS_CONN_ESS)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Connection Type "
               "Flags attribute value 0x%02x",
               *flags);
    return -1;
  }
  return 0;
...
static int wps_validate_encr_settings(const u8 *encr_settings, size_t len,
  ...
  if (len < 16) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Encrypted Settings "
               "attribute length %d",
               (int)len);
    return -1;
  }
...
static int wps_validate_network_key(const u8 *key, size_t key_len,
  ...
  if (((encr_type == NULL || WPA_GET_BE16(encr_type) != WPS_ENCR_WEP) &&
      ...
      key_len > 64) {
    wpa_hexdump_ascii_key(MSG_INFO,
                          "WPS-STRICT: Invalid Network "
                          "Key attribute value",
                          key, key_len);
    return -1;
  }
...
static int wps_validate_network_key_shareable(const u8 *val, int mandatory) {
  ...
  if (*val > 1) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Network Key "
               "Shareable attribute value 0x%x",
               *val);
    return -1;
  }
  return 0;
...
int wps_validate_beacon(const struct wpabuf *wps_ie) {
  ...
  sel_reg = attr.selected_registrar != NULL && *attr.selected_registrar != 0;
  if (wps_validate_version(attr.version, 1) ||
      ...
      wps_validate_sel_reg_config_methods(attr.sel_reg_config_methods, wps2,
                                          sel_reg) ||
...
int wps_validate_beacon_probe_resp(const struct wpabuf *wps_ie, int probe,
  ...
  if (wps_ie == NULL) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: No WPS IE in "
               "%sProbe Response frame",
               probe ? "" : "Beacon/");
    return -1;
  }
  if (wps_parse_msg(wps_ie, &attr) < 0) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Failed to parse WPS IE in "
               "%sProbe Response frame",
               probe ? "" : "Beacon/");
    return -1;
  }
  ...
  if (wps_validate_version(attr.version, 1) ||
      ...
      wps_validate_manufacturer(attr.manufacturer, attr.manufacturer_len,
                                probe) ||
      wps_validate_model_name(attr.model_name, attr.model_name_len, probe) ||
      wps_validate_model_number(attr.model_number, attr.model_number_len,
                                probe) ||
      wps_validate_serial_number(attr.serial_number, attr.serial_number_len,
                                 probe) ||
      ...
      wps_validate_authorized_macs(attr.authorized_macs,
                                   attr.authorized_macs_len, 0)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid %sProbe Response "
               "frame from " MACSTR,
               probe ? "" : "Beacon/", MAC2STR(addr));
...
int wps_validate_probe_req(const struct wpabuf *wps_ie, const u8 *addr) {
  ...
  if (wps_validate_version(attr.version, 1) ||
      ...
      wps_validate_manufacturer(attr.manufacturer, attr.manufacturer_len,
                                wps2) ||
      wps_validate_model_name(attr.model_name, attr.model_name_len, wps2) ||
      wps_validate_model_number(attr.model_number, attr.model_number_len,
                                wps2) ||
      wps_validate_dev_name(attr.dev_name, attr.dev_name_len, wps2) ||
      wps_validate_request_to_enroll(attr.request_to_enroll, 0) ||
      wps_validate_req_dev_type(attr.req_dev_type, attr.num_req_dev_type, 0)) {
    wpa_printf(MSG_INFO,
               "WPS-STRICT: Invalid Probe Request "
               "frame from " MACSTR,
               MAC2STR(addr));
    return -1;
  }
...
int wps_validate_m1(const struct wpabuf *tlvs) {
  ...
  if (wps_validate_version(attr.version, 1) ||
      ...
      wps_validate_config_methods(attr.config_methods, wps2, 1) ||
      wps_validate_wps_state(attr.wps_state, 1) ||
      wps_validate_manufacturer(attr.manufacturer, attr.manufacturer_len, 1) ||
      wps_validate_model_name(attr.model_name, attr.model_name_len, 1) ||
      wps_validate_model_number(attr.model_number, attr.model_number_len, 1) ||
      wps_validate_serial_number(attr.serial_number, attr.serial_number_len,
                                 1) ||
      wps_validate_primary_dev_type(attr.primary_dev_type, 1) ||
...
int wps_validate_m4(const struct wpabuf *tlvs) {
  ...
  if (wps_validate_version(attr.version, 1) ||
      ...
      wps_validate_encr_settings(attr.encr_settings, attr.encr_settings_len,
                                 1) ||
      wps_validate_version2(attr.version2, wps2) ||
      wps_validate_authenticator(attr.authenticator, 1)) {
...
int wps_validate_m8_encr(const struct wpabuf *tlvs, int ap, int wps2) {
  ...
  if (wps_validate_ssid(attr.ssid, attr.ssid_len, ap) ||
      ...
      wps_validate_network_key_index(attr.network_key_idx, 0) ||
      wps_validate_mac_addr(attr.mac_addr, ap) ||
      wps_validate_credential(attr.cred, attr.cred_len, attr.num_cred, !ap) ||
      wps_validate_key_wrap_auth(attr.key_wrap_auth, 1)) {
