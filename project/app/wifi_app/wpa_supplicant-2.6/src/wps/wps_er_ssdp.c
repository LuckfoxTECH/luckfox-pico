...
static void wps_er_ssdp_rx(int sd, void *eloop_ctx, void *sock_ctx) {
  ...
  nread = recvfrom(sd, buf, sizeof(buf) - 1, 0, (struct sockaddr *)&addr,
                   &addr_len);
  if (nread <= 0)
    return;
  buf[nread] = '\0';
  if (er->filter_addr.s_addr && er->filter_addr.s_addr != addr.sin_addr.s_addr)
    return;

  wpa_printf(MSG_DEBUG, "WPS ER: Received SSDP from %s",
             inet_ntoa(addr.sin_addr));
  wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Received SSDP contents", (u8 *)buf,
                    nread);
...
void wps_er_send_ssdp_msearch(struct wps_er *er) {
  ...
  wpabuf_put_str(msg, "M-SEARCH * HTTP/1.1\r\n"
...
int wps_er_ssdp_init(struct wps_er *er) {
  ...
  er->multicast_sd = ssdp_open_multicast_sock(
      er->ip_addr, er->forced_ifname ? er->ifname : NULL);
  ...
  if (eloop_register_sock(er->multicast_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er,
                          NULL) ||
      eloop_register_sock(er->ssdp_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er,
                          NULL))
    return -1;
