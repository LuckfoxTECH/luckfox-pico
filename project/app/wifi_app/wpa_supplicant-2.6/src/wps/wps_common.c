...
int wps_derive_keys(struct wps_data *wps) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: AuthKey", wps->authkey, WPS_AUTHKEY_LEN);
  wpa_hexdump_key(MSG_DEBUG, "WPS: KeyWrapKey", wps->keywrapkey,
                  WPS_KEYWRAPKEY_LEN);
...
int wps_derive_psk(struct wps_data *wps, const u8 *dev_passwd,
  ...
  if (hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN,
                  dev_passwd + (dev_passwd_len + 1) / 2, dev_passwd_len / 2,
                  hash) < 0)
    return -1;
  os_memcpy(wps->psk2, hash, WPS_PSK_LEN);

  wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Device Password", dev_passwd,
                        dev_passwd_len);
...
struct wpabuf *wps_get_oob_cred(struct wps_context *wps, int rf_band,
  ...
  if (wps->wps_state == WPS_STATE_NOT_CONFIGURED && data.new_psk && wps->ap) {
...
struct wpabuf *wps_build_nfc_pw_token(u16 dev_pw_id,
  ...
  if (wps_build_oob_dev_pw(data, dev_pw_id, pubkey, wpabuf_head(dev_pw),
                           wpabuf_len(dev_pw)) ||
...
char *wps_dev_type_bin2str(const u8 dev_type[WPS_DEV_TYPE_LEN], char *buf,
  ...
  ret = os_snprintf(buf, buf_len, "%u-%08X-%u", WPA_GET_BE16(dev_type),
                    WPA_GET_BE32(&dev_type[2]), WPA_GET_BE16(&dev_type[6]));
...
void uuid_gen_mac_addr(const u8 *mac_addr, u8 *uuid) {
  ...
  u8 nsid[16] = {0x52, 0x64, 0x80, 0xf8, 0xc9, 0x9b, 0x4b, 0xe5,
...
struct wpabuf *wps_build_wsc_nack(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_NACK) ||
      wps_build_enrollee_nonce(wps, msg) ||
      wps_build_registrar_nonce(wps, msg) ||
...
struct wpabuf *wps_build_nfc_handover_req(struct wps_context *ctx,
  ...
  if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey,
                           NULL, 0) ||
      wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
    wpabuf_free(msg);
    return NULL;
  }
...
struct wpabuf *wps_build_nfc_handover_sel(struct wps_context *ctx,
  ...
  if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey,
                           NULL, 0) ||
      wps_build_ssid(msg, ctx) || wps_build_ap_freq(msg, freq) ||
...
struct wpabuf *wps_build_nfc_handover_req_p2p(struct wps_context *ctx,
  ...
  if (wps_build_manufacturer(&ctx->dev, msg) ||
      ...
      wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey,
                           NULL, 0) ||
...
struct wpabuf *wps_build_nfc_handover_sel_p2p(struct wps_context *ctx,
  ...
  if (nfc_dh_pubkey == NULL ||
      (nfc_dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER && nfc_dev_pw == NULL)) {
    wpa_printf(MSG_DEBUG, "WPS: No NFC OOB Device Password "
                          "configured");
    return NULL;
  }
  ...
  if (wps_build_manufacturer(&ctx->dev, msg) ||
      wps_build_model_name(&ctx->dev, msg) ||
      wps_build_model_number(&ctx->dev, msg) ||
      wps_build_oob_dev_pw(msg, nfc_dev_pw_id, nfc_dh_pubkey, dev_pw,
                           dev_pw_len) ||
      wps_build_rf_bands(&ctx->dev, msg, 0) ||
      wps_build_serial_number(&ctx->dev, msg) ||
      wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
