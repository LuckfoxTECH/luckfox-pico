...
static int wps_build_wps_state(struct wps_data *wps, struct wpabuf *msg) {
  ...
  wpa_printf(MSG_DEBUG, "WPS:  * Wi-Fi Protected Setup State (%d)", state);
...
static int wps_build_e_hash(struct wps_data *wps, struct wpabuf *msg) {
  ...
  wpa_hexdump(MSG_DEBUG, "WPS: E-S2", wps->snonce + WPS_SECRET_NONCE_LEN,
...
static struct wpabuf *wps_build_m1(struct wps_data *wps) {
  ...
  if (wps->wps->ap && !wps->pbc_in_m1 &&
      (wps->dev_password_len != 0 || (config_methods & WPS_CONFIG_DISPLAY))) {
    /*
    ...
    config_methods &= ~(WPS_CONFIG_VIRT_PUSHBUTTON | WPS_CONFIG_PHY_PUSHBUTTON);
  }

  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M1) ||
      wps_build_uuid_e(msg, wps->uuid_e) ||
      wps_build_mac_addr(msg, wps->mac_addr_e) ||
      wps_build_enrollee_nonce(wps, msg) || wps_build_public_key(wps, msg) ||
...
static struct wpabuf *wps_build_m3(struct wps_data *wps) {
  ...
  if (wps->wps->ap && random_pool_ready() != 1) {
    wpa_printf(MSG_INFO, "WPS: Not enough entropy in random pool to proceed - "
                         "do not allow AP PIN to be used");
    return NULL;
  }
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M3) ||
      wps_build_registrar_nonce(wps, msg) || wps_build_e_hash(wps, msg) ||
...
static struct wpabuf *wps_build_m5(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M5) ||
      wps_build_registrar_nonce(wps, msg) || wps_build_e_snonce1(wps, plain) ||
...
static int wps_build_cred_network_key(struct wps_data *wps,
  ...
  if ((wps->wps->ap_auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) &&
    ...
    if (random_pool_ready() != 1 || random_get_bytes(psk, sizeof(psk)) < 0) {
      wpa_printf(MSG_INFO, "WPS: Could not generate random PSK");
      return -1;
    }
    wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK", psk,
                    sizeof(psk));
    ...
    if (wps->wps->registrar) {
      wps_cb_new_psk(wps->wps->registrar, wps->peer_dev.mac_addr,
...
static int wps_build_ap_settings(struct wps_data *wps, struct wpabuf *plain) {
  ...
  if (wps->wps->ap_settings) {
    ...
    wpabuf_put_data(plain, wps->wps->ap_settings, wps->wps->ap_settings_len);
    return 0;
  }
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: Plaintext AP Settings", start, end - start);
...
static struct wpabuf *wps_build_m7(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M7) ||
      wps_build_registrar_nonce(wps, msg) || wps_build_e_snonce2(wps, plain) ||
  ...
  if (wps->wps->ap && wps->wps->registrar) {
     ...
     */
    wps_device_store(wps->wps->registrar, &wps->peer_dev, wps->uuid_r);
  }
...
static struct wpabuf *wps_build_wsc_done(struct wps_data *wps) {
  ...
  if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_DONE) ||
...
static int wps_process_pubkey(struct wps_data *wps, const u8 *pk,
  ...
  if (wps->peer_pubkey_hash_set) {
    ...
    if (os_memcmp_const(hash, wps->peer_pubkey_hash, WPS_OOB_PUBKEY_HASH_LEN) !=
        0) {
      wpa_printf(MSG_ERROR, "WPS: Public Key hash mismatch");
      wpa_hexdump(MSG_DEBUG, "WPS: Received public key", pk, pk_len);
      wpa_hexdump(MSG_DEBUG,
                  "WPS: Calculated public key "
                  "hash",
                  hash, WPS_OOB_PUBKEY_HASH_LEN);
      wpa_hexdump(MSG_DEBUG, "WPS: Expected public key hash",
                  wps->peer_pubkey_hash, WPS_OOB_PUBKEY_HASH_LEN);
...
static int wps_process_r_snonce1(struct wps_data *wps, const u8 *r_snonce1) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: R-SNonce1", r_snonce1, WPS_SECRET_NONCE_LEN);
...
static int wps_process_r_snonce2(struct wps_data *wps, const u8 *r_snonce2) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPS: R-SNonce2", r_snonce2, WPS_SECRET_NONCE_LEN);
...
static int wps_process_cred_e(struct wps_data *wps, const u8 *cred,
  ...
  if (os_memcmp(wps->cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               "WPS: MAC Address in the Credential (" MACSTR
               ") does not match with own address (" MACSTR ")",
               MAC2STR(wps->cred.mac_addr), MAC2STR(wps->wps->dev.mac_addr));
    /*
  ...
  if (!(wps->cred.encr_type & (WPS_ENCR_NONE | WPS_ENCR_TKIP | WPS_ENCR_AES))) {
...
static int wps_process_ap_settings_e(struct wps_data *wps,
  ...
  if (os_memcmp(cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               "WPS: MAC Address in the AP Settings (" MACSTR
               ") does not match with own address (" MACSTR ")",
               MAC2STR(cred.mac_addr), MAC2STR(wps->wps->dev.mac_addr));
    /*
     * In theory, this could be consider fatal error, but there are
     * number of deployed implementations using other address here
     * due to unclarity in the specification. For interoperability
  ...
  if (wps2) {
    if ((cred.encr_type & (WPS_ENCR_TKIP | WPS_ENCR_AES)) == WPS_ENCR_TKIP ||
      ...
      wps->error_indication = WPS_EI_SECURITY_TKIP_ONLY_PROHIBITED;
      return -1;
    }
  }
#endif /* CONFIG_WPS_STRICT */
...
static int wps_process_dev_pw_id(struct wps_data *wps, const u8 *dev_pw_id) {
...
#ifdef CONFIG_P2P
  if ((id == DEV_PW_DEFAULT && wps->dev_pw_id == DEV_PW_REGISTRAR_SPECIFIED) ||
      (id == DEV_PW_REGISTRAR_SPECIFIED && wps->dev_pw_id == DEV_PW_DEFAULT)) {
    /*
  ...
  if (wps->dev_pw_id == DEV_PW_PUSHBUTTON && id == DEV_PW_DEFAULT) {
    wpa_printf(MSG_DEBUG, "WPS: Workaround - ignore PBC-to-PIN change");
    return 0;
  }
...
static enum wps_process_res wps_process_m2d(struct wps_data *wps,
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: Manufacturer", attr->manufacturer,
                    attr->manufacturer_len);
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: Model Name", attr->model_name,
                    attr->model_name_len);
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: Model Number", attr->model_number,
                    attr->model_number_len);
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: Serial Number", attr->serial_number,
                    attr->serial_number_len);
  wpa_hexdump_ascii(MSG_DEBUG, "WPS: Device Name", attr->dev_name,
                    attr->dev_name_len);
  ...
  if (wps->wps->event_cb) {
    ...
    if (attr->config_methods)
      m2d->config_methods = WPA_GET_BE16(attr->config_methods);
    ...
    if (attr->config_error)
      m2d->config_error = WPA_GET_BE16(attr->config_error);
    if (attr->dev_password_id)
      m2d->dev_password_id = WPA_GET_BE16(attr->dev_password_id);
...
static enum wps_process_res wps_process_m6(struct wps_data *wps,
  ...
  if (wps->wps->ap)
    wps->wps->event_cb(wps->wps->cb_ctx, WPS_EV_AP_PIN_SUCCESS, NULL);
...
static enum wps_process_res wps_process_m8(struct wps_data *wps,
  ...
  if (wps_validate_m8_encr(decrypted, wps->wps->ap, attr->version2 != NULL) <
      0) {
    wpabuf_clear_free(decrypted);
    wps->state = SEND_WSC_NACK;
    return WPS_CONTINUE;
...
static enum wps_process_res wps_process_wsc_msg(struct wps_data *wps,
  ...
  case WPS_M4:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M4, wps->config_error, wps->error_indication,
  ...
  case WPS_M6:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M6, wps->config_error, wps->error_indication,
                     wps->peer_dev.mac_addr);
    break;
  case WPS_M8:
    ...
    if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
      wps_fail_event(wps->wps, WPS_M8, wps->config_error, wps->error_indication,
                     wps->peer_dev.mac_addr);
    break;
  default:
...
static enum wps_process_res wps_process_wsc_nack(struct wps_data *wps,
  ...
  if (attr.registrar_nonce == NULL ||
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: Expected Registrar Nonce", wps->nonce_r,
                WPS_NONCE_LEN);
    return WPS_FAILURE;
  }
  ...
  if (attr.enrollee_nonce == NULL ||
      os_memcmp(wps->nonce_e, attr.enrollee_nonce, WPS_NONCE_LEN) != 0) {
    wpa_printf(MSG_DEBUG, "WPS: Mismatch in enrollee nonce");
    wpa_hexdump(MSG_DEBUG, "WPS: Received Enrollee Nonce", attr.enrollee_nonce,
                WPS_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "WPS: Expected Enrollee Nonce", wps->nonce_e,
                WPS_NONCE_LEN);
    return WPS_FAILURE;
  }
  ...
  switch (wps->state) {
  case RECV_M4:
    wps_fail_event(wps->wps, WPS_M3, config_error, wps->error_indication,
                   wps->peer_dev.mac_addr);
    break;
  case RECV_M6:
    wps_fail_event(wps->wps, WPS_M5, config_error, wps->error_indication,
                   wps->peer_dev.mac_addr);
    break;
  case RECV_M8:
    wps_fail_event(wps->wps, WPS_M7, config_error, wps->error_indication,
                   wps->peer_dev.mac_addr);
    break;
  default:
    break;
