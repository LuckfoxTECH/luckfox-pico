...
int l2_packet_send(struct l2_packet_data *l2, const u8 *dst_addr, u16 proto,
  ...
  if (l2->l2_hdr) {
    res = WriteFile(driver_ndis_get_ndisuio_handle(), buf, len, &written, o);
  } else {
    ...
    res = WriteFile(driver_ndis_get_ndisuio_handle(), eth, mlen, &written, o);
    os_free(eth);
  }
  ...
  if (!res) {
    ...
    if (err == ERROR_IO_PENDING) {
      wpa_printf(MSG_DEBUG, "L2(NDISUIO): Wait for pending "
                            "write to complete");
      res = GetOverlappedResult(driver_ndis_get_ndisuio_handle(), &overlapped,
                                &written, TRUE);
      if (!res) {
...
static void l2_packet_callback(struct l2_packet_data *l2) {
  ...
  wpa_printf(MSG_DEBUG, "L2(NDISUIO): Read %d bytes", (int)l2->rx_written);
...
#ifndef _WIN32_WCE
  if (!GetOverlappedResult(driver_ndis_get_ndisuio_handle(), &l2->rx_overlapped,
                           &l2->rx_written, FALSE)) {
    wpa_printf(MSG_DEBUG,
...
static int l2_ndisuio_set_ether_type(unsigned short protocol) {
  ...
  if (!DeviceIoControl(driver_ndis_get_ndisuio_handle(),
                       IOCTL_NDISUIO_SET_ETHER_TYPE, &proto, sizeof(proto),
                       NULL, 0, &written, NULL)) {
...
l2_packet_init(const char *ifname, const u8 *own_addr, unsigned short protocol,
  ...
  eloop_register_event(l2->rx_avail, sizeof(l2->rx_avail), l2_packet_rx_event,
                       l2, NULL);
  ...
  l2_ndisuio_global->rx_thread =
      CreateThread(NULL, 0, l2_packet_rx_thread, l2, 0, NULL);
...
void l2_packet_deinit(struct l2_packet_data *l2) {
  ...
  if (l2_ndisuio_global) {
    ...
    if (l2_ndisuio_global->refcount) {
      ...
      l2_ndisuio_set_ether_type(l2_ndisuio_global->first_proto);
      return;
    }
     ...
     */
    if (!DeviceIoControl(driver_ndis_get_ndisuio_handle(), IOCTL_CANCEL_READ,
