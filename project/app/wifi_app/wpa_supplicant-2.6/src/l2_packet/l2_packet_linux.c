...
static struct sock_filter dhcp_sock_filter_insns[] = {
    {0x80, 0, 0, 0x00000000}, {0x35, 0, 12, 0x00000116},
    {0x28, 0, 0, 0x0000000c}, {0x15, 0, 10, 0x00000800},
    {0x30, 0, 0, 0x00000017}, {0x15, 0, 8, 0x00000011},
    {0x28, 0, 0, 0x00000014}, {0x45, 6, 0, 0x00001fff},
    {0xb1, 0, 0, 0x0000000e}, {0x48, 0, 0, 0x0000000e},
    {0x15, 0, 3, 0x00000043}, {0x48, 0, 0, 0x00000010},
    {0x15, 0, 1, 0x00000044}, {0x6, 0, 0, 0x00000bb8},
    {0x6, 0, 0, 0x00000000},
};
...
static struct sock_filter ndisc_sock_filter_insns[] = {
    {0x30, 0, 0, 0x00000000}, {0x45, 0, 5, 0x00000001},
    {0x28, 0, 0, 0x0000000c}, {0x15, 0, 3, 0x000086dd},
    {0x30, 0, 0, 0x00000014}, {0x15, 0, 1, 0x0000003a},
    {0x6, 0, 0, 0x000005dc},  {0x6, 0, 0, 0x00000000},
};
...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx) {
  ...
  wpa_printf(MSG_DEBUG, "%s: src=" MACSTR " len=%d", __func__,
             MAC2STR(ll.sll_addr), (int)res);
  ...
  if (l2->fd_br_rx >= 0) {
    ...
    if (l2->num_rx_br <= 1) {
      wpa_printf(MSG_DEBUG,
                 "l2_packet_receive: Main packet socket for %s seems to have "
                 "working RX - close workaround bridge socket",
                 l2->ifname);
    ...
    if (l2->last_from_br && os_memcmp(hash, l2->last_hash, SHA1_MAC_LEN) == 0) {
...
l2_packet_init(const char *ifname, const u8 *own_addr, unsigned short protocol,
  ...
  if (l2->fd < 0) {
    wpa_printf(MSG_ERROR, "%s: socket(PF_PACKET): %s", __func__,
               strerror(errno));
    os_free(l2);
    return NULL;
  }
  ...
  if (ioctl(l2->fd, SIOCGIFINDEX, &ifr) < 0) {
    wpa_printf(MSG_ERROR, "%s: ioctl[SIOCGIFINDEX]: %s", __func__,
               strerror(errno));
    close(l2->fd);
    os_free(l2);
    return NULL;
  ...
  if (ioctl(l2->fd, SIOCGIFHWADDR, &ifr) < 0) {
    wpa_printf(MSG_ERROR, "%s: ioctl[SIOCGIFHWADDR]: %s", __func__,
               strerror(errno));
    close(l2->fd);
    os_free(l2);
    return NULL;
...
l2_packet_init_bridge(const char *br_ifname, const char *ifname,
  ...
  l2->fd_br_rx =
      socket(PF_PACKET, l2_hdr ? SOCK_RAW : SOCK_DGRAM, htons(ETH_P_ALL));
  if (l2->fd_br_rx < 0) {
    wpa_printf(MSG_DEBUG, "%s: socket(PF_PACKET-fd_br_rx): %s", __func__,
               strerror(errno));
  ...
  if (bind(l2->fd_br_rx, (struct sockaddr *)&ll, sizeof(ll)) < 0) {
    wpa_printf(MSG_DEBUG, "%s: bind[PF_PACKET-fd_br_rx]: %s", __func__,
               strerror(errno));
    /* try to continue without the workaround RX socket */
    close(l2->fd_br_rx);
    l2->fd_br_rx = -1;
...
int l2_packet_get_ip_addr(struct l2_packet_data *l2, char *buf, size_t len) {
  ...
  if (ioctl(s, SIOCGIFADDR, &ifr) < 0) {
    if (errno != EADDRNOTAVAIL)
      wpa_printf(MSG_ERROR, "%s: ioctl[SIOCGIFADDR]: %s", __func__,
                 strerror(errno));
    close(s);
    return -1;
  }
...
int l2_packet_set_packet_filter(struct l2_packet_data *l2,
  ...
  if (setsockopt(l2->fd, SOL_SOCKET, SO_ATTACH_FILTER, sock_filter,
