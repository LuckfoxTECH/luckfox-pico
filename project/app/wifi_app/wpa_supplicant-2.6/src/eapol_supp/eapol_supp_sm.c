...
SM_STATE(SUPP_PAE, CONNECTING) {
  ...
  if (send_start) {
    ...
    if (sm->conf.wps && !(sm->conf.wps & EAPOL_PEER_IS_WPS20_AP)) {
      ...
      wpa_printf(MSG_DEBUG, "EAPOL: Using shorter startWhen for WPS");
      sm->startWhen = 1;
    } else {
      sm->startWhen = 2;
    }
...
SM_STEP(SUPP_PAE) {
  if ((sm->userLogoff && !sm->logoffSent) &&
  ...
  else if (((sm->portControl == Auto) && (sm->sPortMode != sm->portControl)) ||
           sm->initialize || !sm->portEnabled)
    ...
    case SUPP_PAE_CONNECTING:
      if (sm->startWhen == 0 && sm->startCount < sm->maxStart)
        SM_ENTER(SUPP_PAE, CONNECTING);
      else if (sm->startWhen == 0 && sm->startCount >= sm->maxStart &&
               sm->portValid)
    ...
    case SUPP_PAE_AUTHENTICATING:
      if (sm->eapSuccess && !sm->portValid && sm->conf.accept_802_1x_keys &&
...
SM_STATE(KEY_RX, NO_KEY_RECEIVE) { SM_ENTRY(KEY_RX, NO_KEY_RECEIVE); }
...
static void eapol_sm_txLogoff(struct eapol_sm *sm) {
  ...
  sm->ctx->eapol_send(sm->ctx->eapol_send_ctx, IEEE802_1X_TYPE_EAPOL_LOGOFF,
                      (u8 *)"", 0);
...
static void eapol_sm_processKey(struct eapol_sm *sm) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             hdr->version, hdr->type, be_to_host16(hdr->length), key->type,
             rx_key_length, key->key_index);
  ...
  if (sm->replay_counter_valid &&
    ...
    wpa_hexdump(MSG_DEBUG, "EAPOL: last replay counter",
                sm->last_replay_counter, IEEE8021X_REPLAY_COUNTER_LEN);
  ...
  hmac_md5(keydata.sign_key, sign_key_len, sm->last_rx_key,
           sizeof(*hdr) + be_to_host16(hdr->length), key->key_signature);
  if (os_memcmp_const(orig_key_sign, key->key_signature,
    ...
    wpa_printf(MSG_DEBUG, "EAPOL: Invalid key signature in "
                          "EAPOL-Key packet");
    os_memcpy(key->key_signature, orig_key_sign, IEEE8021X_KEY_SIGN_LEN);
    return;
  }
  ...
  if (key_len == rx_key_length) {
    ...
    os_memcpy(ekey + IEEE8021X_KEY_IV_LEN, keydata.encr_key, encr_key_len);
    os_memcpy(datakey, key + 1, key_len);
    rc4_skip(ekey, IEEE8021X_KEY_IV_LEN + encr_key_len, 0, datakey, key_len);
    wpa_hexdump_key(MSG_DEBUG, "EAPOL: Decrypted(RC4) key", datakey, key_len);
#endif /* CONFIG_NO_RC4 */
  } else if (key_len == 0) {
    /*
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "len %d",
             key->key_index & IEEE8021X_KEY_INDEX_FLAG ? "unicast"
                                                       : "broadcast",
             key->key_index & IEEE8021X_KEY_INDEX_MASK, key_len);

  if (sm->ctx->set_wep_key &&
      sm->ctx->set_wep_key(
          sm->ctx->ctx, key->key_index & IEEE8021X_KEY_INDEX_FLAG,
          key->key_index & IEEE8021X_KEY_INDEX_MASK, datakey, key_len) < 0) {
...
int eapol_sm_get_status(struct eapol_sm *sm, char *buf, size_t buflen,
  ...
  if (verbose) {
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      sm->maxStart, eapol_port_control(sm->portControl),
...
#ifdef CONFIG_EAP_PROXY
  if (sm->use_eap_proxy)
    len += eap_proxy_sm_get_status(sm->eap_proxy, buf + len, buflen - len,
                                   verbose);
  else
#endif /* CONFIG_EAP_PROXY */
...
int eapol_sm_get_mib(struct eapol_sm *sm, char *buf, size_t buflen) {
  ...
  ret = os_snprintf(buf, buflen,
                    ...
                    sm->SUPP_PAE_state, sm->heldPeriod, sm->authPeriod,
                    sm->startPeriod, sm->maxStart,
                    sm->suppPortStatus == Authorized ? "Authorized"
                                                     : "Unauthorized",
                    sm->SUPP_BE_state);
  ...
  ret = os_snprintf(
      ...
      sm->dot1xSuppEapolFramesRx, sm->dot1xSuppEapolFramesTx,
      sm->dot1xSuppEapolStartFramesTx, sm->dot1xSuppEapolLogoffFramesTx,
      sm->dot1xSuppEapolRespFramesTx, sm->dot1xSuppEapolReqIdFramesRx,
      sm->dot1xSuppEapolReqFramesRx, sm->dot1xSuppInvalidEapolFramesRx,
      sm->dot1xSuppEapLengthErrorFramesRx, sm->dot1xSuppLastEapolFrameVersion,
...
int eapol_sm_rx_eapol(struct eapol_sm *sm, const u8 *src, const u8 *buf,
...
#ifdef CONFIG_WPS
  if (sm->conf.wps && sm->conf.workaround && plen < len - sizeof(*hdr) &&
...
#ifdef CONFIG_EAP_PROXY
      if (sm->use_eap_proxy) {
        eap_proxy_packet_update(sm->eap_proxy, wpabuf_mhead_u8(sm->eapReqData),
  ...
  case IEEE802_1X_TYPE_EAPOL_MKA:
    wpa_printf(MSG_EXCESSIVE, "EAPOL type %d will be handled by MKA",
               hdr->type);
    break;
#endif /* CONFIG_MACSEC */
  default:
    wpa_printf(MSG_DEBUG, "EAPOL: Received unknown EAPOL type %d", hdr->type);
    sm->dot1xSuppInvalidEapolFramesRx++;
    break;
  }
...
static void eapol_sm_notify_cert(void *ctx, int depth, const char *subject,
  ...
  struct eapol_sm *sm = ctx;
  if (sm->ctx->cert_cb)
    sm->ctx->cert_cb(sm->ctx->ctx, depth, subject, altsubject, num_altsubject,
                     cert_hash, cert);
}
...
static const struct eapol_callbacks eapol_cb = {
    eapol_sm_get_config,      eapol_sm_get_bool,
    eapol_sm_set_bool,        eapol_sm_get_int,
    eapol_sm_set_int,         eapol_sm_get_eapReqData,
    eapol_sm_set_config_blob, eapol_sm_get_config_blob,
    eapol_sm_notify_pending,  eapol_sm_eap_param_needed,
    eapol_sm_notify_cert,     eapol_sm_notify_status,
#ifdef CONFIG_EAP_PROXY
