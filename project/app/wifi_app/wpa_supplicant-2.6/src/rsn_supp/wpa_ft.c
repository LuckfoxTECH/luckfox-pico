...
int wpa_derive_ptk_ft(struct wpa_sm *sm, const unsigned char *src_addr,
  ...
  wpa_derive_pmk_r0(sm->xxkey, sm->xxkey_len, sm->ssid, sm->ssid_len,
                    sm->mobility_domain, sm->r0kh_id, sm->r0kh_id_len,
                    sm->own_addr, sm->pmk_r0, sm->pmk_r0_name);
  wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R0", sm->pmk_r0, PMK_LEN);
  wpa_hexdump(MSG_DEBUG, "FT: PMKR0Name", sm->pmk_r0_name, WPA_PMK_NAME_LEN);
  wpa_derive_pmk_r1(sm->pmk_r0, sm->pmk_r0_name, sm->r1kh_id, sm->own_addr,
...
int wpa_sm_set_ft_params(struct wpa_sm *sm, const u8 *ies, size_t ies_len) {
  ...
  if (ft.mdie) {
    wpa_hexdump(MSG_DEBUG, "FT: Mobility domain", ft.mdie,
  ...
  if (ft.r0kh_id) {
    wpa_hexdump(MSG_DEBUG, "FT: R0KH-ID", ft.r0kh_id, ft.r0kh_id_len);
  ...
  if (ft.r1kh_id) {
    wpa_hexdump(MSG_DEBUG, "FT: R1KH-ID", ft.r1kh_id, FT_R1KH_ID_LEN);
 ...
 */
static u8 *wpa_ft_gen_req_ies(struct wpa_sm *sm, size_t *len, const u8 *anonce,
                              const u8 *pmk_name, const u8 *kck, size_t kck_len,
                              const u8 *target_ap, const u8 *ric_ies,
                              size_t ric_ies_len, const u8 *ap_mdie) {
  size_t buf_len;
  ...
  buf_len = 2 + sizeof(struct rsn_mdie) + 2 + sizeof(struct rsn_ftie) + 2 +
            sm->r0kh_id_len + ric_ies_len + 100;
  ...
  if (!wpa_cipher_valid_group(sm->group_cipher)) {
    wpa_printf(MSG_WARNING, "FT: Invalid group cipher (%d)", sm->group_cipher);
    os_free(buf);
    return NULL;
  }
  RSN_SELECTOR_PUT(pos, wpa_cipher_to_suite(WPA_PROTO_RSN, sm->group_cipher));
  ...
  RSN_SELECTOR_PUT(pos,
                   wpa_cipher_to_suite(WPA_PROTO_RSN, sm->pairwise_cipher));
  pos += RSN_SELECTOR_LEN;
  ...
  *pos++ = sizeof(*mdie);
  ...
  pos += sizeof(*mdie);
  os_memcpy(mdie->mobility_domain, sm->mobility_domain, MOBILITY_DOMAIN_ID_LEN);
  mdie->ft_capab = ap_mdie && ap_mdie[1] >= 3 ? ap_mdie[4] : sm->mdie_ft_capab;
  ...
  *ftie_len = pos - ftie_len - 1;
  ...
  if (kck) {
    ...
    ftie->mic_control[1] = 3 + ieee802_11_ie_count(ric_ies, ric_ies_len);
    if (wpa_ft_mic(kck, kck_len, sm->own_addr, target_ap, 5, ((u8 *)mdie) - 2,
                   2 + sizeof(*mdie), ftie_pos, 2 + *ftie_len, (u8 *)rsnie,
                   2 + rsnie->len, ric_ies, ric_ies_len, ftie->mic) < 0) {
...
int wpa_ft_prepare_auth_request(struct wpa_sm *sm, const u8 *mdie) {
  ...
  ft_ies = wpa_ft_gen_req_ies(sm, &ft_ies_len, NULL, sm->pmk_r0_name, NULL, 0,
                              sm->bssid, NULL, 0, mdie);
  if (ft_ies) {
...
int wpa_ft_process_response(struct wpa_sm *sm, const u8 *ies, size_t ies_len,
  ...
  bssid = target_ap;
  if (wpa_pmk_r1_to_ptk(sm->pmk_r1, sm->snonce, ftie->anonce, sm->own_addr,
                        bssid, sm->pmk_r1_name, &sm->ptk, ptk_name,
                        sm->key_mgmt, sm->pairwise_cipher) < 0)
    return -1;

  ft_ies = wpa_ft_gen_req_ies(sm, &ft_ies_len, ftie->anonce, sm->pmk_r1_name,
                              sm->ptk.kck, sm->ptk.kck_len, bssid, ric_ies,
                              ric_ies_len, parse.mdie ? parse.mdie - 2 : NULL);
  if (ft_ies) {
    wpa_sm_update_ft_ies(sm, sm->mobility_domain, ft_ies, ft_ies_len);
    os_free(ft_ies);
  }
...
static int wpa_ft_process_gtk_subelem(struct wpa_sm *sm, const u8 *gtk_elem,
  ...
  wpa_hexdump_key(MSG_DEBUG, "FT: Received GTK in Reassoc Resp", gtk_elem,
                  gtk_elem_len);
  ...
  if (wpa_sm_set_key(sm, alg, broadcast_ether_addr, keyidx, 0, gtk_elem + 3,
                     rsc_len, gtk, keylen) < 0) {
...
static int wpa_ft_process_igtk_subelem(struct wpa_sm *sm, const u8 *igtk_elem,
  ...
  wpa_hexdump_key(MSG_DEBUG, "FT: Received IGTK in Reassoc Resp", igtk_elem,
                  igtk_elem_len);
  ...
  if (aes_unwrap(sm->ptk.kek, sm->ptk.kek_len, WPA_IGTK_LEN / 8, igtk_elem + 9,
                 igtk)) {
    wpa_printf(MSG_WARNING, "FT: AES unwrap failed - could not "
                            "decrypt IGTK");
    return -1;
  ...
  wpa_hexdump_key(MSG_DEBUG, "FT: IGTK from Reassoc Resp", igtk, WPA_IGTK_LEN);
...
int wpa_ft_validate_reassoc_resp(struct wpa_sm *sm, const u8 *ies,
  ...
  if (sm->ft_reassoc_completed) {
    wpa_printf(MSG_DEBUG,
               "FT: Reassociation has already been completed for this FT "
               "protocol instance - ignore unexpected retransmission");
    return 0;
  }
  ...
  if (parse.ric) {
    wpa_hexdump(MSG_MSGDUMP, "FT: RIC Response", parse.ric, parse.ric_len);
