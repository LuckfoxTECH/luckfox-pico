...
static int wpa_tdls_del_key(struct wpa_sm *sm, struct wpa_tdls_peer *peer) {
  if (wpa_sm_set_key(sm, WPA_ALG_NONE, peer->addr, 0, 0, NULL, 0, NULL, 0) <
      0) {
...
static int wpa_tdls_set_key(struct wpa_sm *sm, struct wpa_tdls_peer *peer) {
  ...
  if (peer->tk_set) {
    ...
    wpa_printf(
        MSG_INFO,
        "TDLS: TPK-TK for the peer " MACSTR
  ...
  wpa_printf(MSG_DEBUG, "TDLS: Configure pairwise key for peer " MACSTR,
             MAC2STR(peer->addr));
  if (wpa_sm_set_key(sm, alg, peer->addr, -1, 1, rsc, sizeof(rsc), peer->tpk.tk,
                     key_len) < 0) {
...
static int wpa_tdls_send_tpk_msg(struct wpa_sm *sm, const u8 *dst,
                                 ...
                                 u16 status_code, u32 peer_capab, int initiator,
                                 const u8 *buf, size_t len) {
  return wpa_sm_send_tdls_mgmt(sm, dst, action_code, dialog_token, status_code,
                               peer_capab, initiator, buf, len);
}
...
static int wpa_tdls_tpk_send(struct wpa_sm *sm, const u8 *dest, u8 action_code,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: TPK send dest=" MACSTR " action_code=%u "
             "dialog_token=%u status_code=%u peer_capab=%u initiator=%d "
             "msg_len=%u",
             MAC2STR(dest), action_code, dialog_token, status_code, peer_capab,
             initiator, (unsigned int)msg_len);

  if (wpa_tdls_send_tpk_msg(sm, dest, action_code, dialog_token, status_code,
                            peer_capab, initiator, msg, msg_len)) {
    wpa_printf(MSG_INFO,
               "TDLS: Failed to send message "
               "(action_code=%u)",
               action_code);
    return -1;
  }
  ...
  if (peer == NULL) {
    wpa_printf(MSG_INFO,
               "TDLS: No matching entry found for "
               "retry " MACSTR,
               MAC2STR(dest));
    return 0;
  }
...
static void wpa_tdls_tpk_retry_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (peer->sm_tmr.count) {
    ...
    wpa_printf(MSG_INFO,
               "TDLS: Retrying sending of message "
               "(action_code=%u)",
               peer->sm_tmr.action_code);

    if (peer->sm_tmr.buf == NULL) {
      wpa_printf(MSG_INFO,
                 "TDLS: No retry buffer available "
                 "for action_code=%u",
                 peer->sm_tmr.action_code);
      eloop_cancel_timeout(wpa_tdls_tpk_retry_timeout, sm, peer);
      return;
    ...
    if (wpa_tdls_send_tpk_msg(sm, peer->sm_tmr.dest, peer->sm_tmr.action_code,
                              peer->sm_tmr.dialog_token,
                              peer->sm_tmr.status_code, peer->sm_tmr.peer_capab,
                              peer->initiator, peer->sm_tmr.buf,
                              peer->sm_tmr.buf_len)) {
...
static void wpa_tdls_tpk_retry_timeout_cancel(struct wpa_sm *sm,
  ...
  if (action_code == peer->sm_tmr.action_code) {
    wpa_printf(MSG_DEBUG,
               "TDLS: Retry timeout cancelled for "
               "action_code=%u",
               action_code);
    ...
    wpa_printf(MSG_INFO,
               "TDLS: Error in cancelling retry timeout "
               "(Unknown action_code=%u)",
               action_code);
  }
}
...
static void wpa_tdls_generate_tpk(struct wpa_tdls_peer *peer,
  ...
  wpa_hexdump_key(MSG_DEBUG, "TDLS: TPK-Key-Input", key_input, SHA256_MAC_LEN);
  ...
  wpa_hexdump_key(MSG_DEBUG, "TDLS: TPK-KCK", peer->tpk.kck,
                  sizeof(peer->tpk.kck));
  wpa_hexdump_key(MSG_DEBUG, "TDLS: TPK-TK", peer->tpk.tk,
                  sizeof(peer->tpk.tk));
  peer->tpk_set = 1;
}
...
static int wpa_tdls_key_mic_teardown(const u8 *kck, u8 trans_seq, u16 rcode,
                                     u8 dtoken, const u8 *lnkid, const u8 *ftie,
                                     u8 *mic) {
  u8 *buf, *pos;
  struct wpa_tdls_ftie *_ftie;
  int ret;
  ...
  len = 2 + lnkid[1] + sizeof(rcode) + sizeof(dtoken) + sizeof(trans_seq) + 2 +
        ftie[1];
...
static int wpa_supplicant_verify_tdls_mic(u8 trans_seq,
  ...
  if (peer->tpk_set) {
    wpa_tdls_ftie_mic(peer->tpk.kck, trans_seq, lnkid, peer->rsnie_p, timeoutie,
                      (u8 *)ftie, mic);
    if (os_memcmp_const(mic, ftie->mic, 16) != 0) {
      wpa_printf(MSG_INFO, "TDLS: Invalid MIC in FTIE - "
                           "dropping packet");
      wpa_hexdump(MSG_DEBUG, "TDLS: Received MIC", ftie->mic, 16);
      wpa_hexdump(MSG_DEBUG, "TDLS: Calculated MIC", mic, 16);
      return -1;
    }
  } else {
...
static int wpa_supplicant_verify_tdls_mic_teardown(
  ...
  if (peer->tpk_set) {
    wpa_tdls_key_mic_teardown(peer->tpk.kck, trans_seq, rcode, dtoken, lnkid,
                              (u8 *)ftie, mic);
    if (os_memcmp_const(mic, ftie->mic, 16) != 0) {
...
static void wpa_tdls_tpk_timeout(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (peer->initiator) {
    ...
    wpa_printf(MSG_DEBUG,
               "TDLS: TPK lifetime expired for " MACSTR " - try to renew",
               MAC2STR(peer->addr));
    ...
    wpa_printf(MSG_DEBUG,
               "TDLS: TPK lifetime expired for " MACSTR " - tear down",
               MAC2STR(peer->addr));
    wpa_tdls_do_teardown(sm, peer, WLAN_REASON_TDLS_TEARDOWN_UNSPECIFIED);
  }
}
...
static void wpa_tdls_peer_remove_from_list(struct wpa_sm *sm,
  ...
  if (cur != peer) {
    wpa_printf(MSG_ERROR,
               "TDLS: Could not find peer " MACSTR
...
static int wpa_tdls_send_teardown(struct wpa_sm *sm, const u8 *addr,
  ...
  if (peer->chan_switch_enabled) {
    wpa_printf(MSG_DEBUG,
               "TDLS: First returning link with " MACSTR " to base channel",
               MAC2STR(addr));
  ...
  wpa_printf(MSG_DEBUG, "TDLS: TDLS Teardown for " MACSTR, MAC2STR(addr));
...
#endif /* CONFIG_TDLS_TESTING */
  wpa_hexdump(MSG_DEBUG, "TDLS: FTIE for TDLS Teardown handshake", (u8 *)ftie,
              pos - (u8 *)ftie);

  /* compute MIC before sending */
  wpa_tdls_linkid(sm, peer, &lnkid);
  wpa_tdls_key_mic_teardown(peer->tpk.kck, 4, reason_code, dialog_token,
                            (u8 *)&lnkid, (u8 *)ftie, ftie->mic);
...
skip_ies:
  ...
  wpa_tdls_tpk_send(sm, addr, WLAN_TDLS_TEARDOWN, 0, reason_code, 0,
                    peer->initiator, rbuf, pos - rbuf);
  os_free(rbuf);
...
int wpa_tdls_teardown_link(struct wpa_sm *sm, const u8 *addr, u16 reason_code) {
  ...
  if (peer == NULL) {
    wpa_printf(MSG_DEBUG,
               "TDLS: Could not find peer " MACSTR " for link Teardown",
               MAC2STR(addr));
    return -1;
  }
  ...
  if (!peer->tpk_success) {
    wpa_printf(MSG_DEBUG,
               "TDLS: Peer " MACSTR " not connected - cannot Teardown link",
               MAC2STR(addr));
    return -1;
  }
...
void wpa_tdls_disable_unreachable_link(struct wpa_sm *sm, const u8 *addr) {
  ...
  if (wpa_tdls_is_external_setup(sm)) {
    ...
    wpa_tdls_send_teardown(sm, addr, WLAN_REASON_TDLS_TEARDOWN_UNREACHABLE);
    wpa_tdls_peer_free(sm, peer);
  } else {
    wpa_tdls_disable_peer_link(sm, peer);
  }
...
static int wpa_tdls_recv_teardown(struct wpa_sm *sm, const u8 *src_addr,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: TDLS Teardown Request from " MACSTR " (reason code %u)",
             MAC2STR(src_addr), reason_code);
  ...
  if (wpa_supplicant_parse_ies((const u8 *)pos, ielen, &kde) < 0)
    wpa_printf(MSG_DEBUG, "TDLS: Failed to parse IEs in Teardown - ignore as "
  ...
  if (wpa_supplicant_verify_tdls_mic_teardown(4, reason_code, peer->dtoken,
                                              peer, (u8 *)lnkid, ftie) < 0) {
    wpa_printf(MSG_DEBUG,
               "TDLS: MIC failure for TDLS "
...
static int wpa_tdls_send_error(struct wpa_sm *sm, const u8 *dst, u8 tdls_action,
                               u8 dialog_token, int initiator, u16 status) {
  wpa_printf(MSG_DEBUG,
             "TDLS: Sending error to " MACSTR " (action=%u status=%u)",
             MAC2STR(dst), tdls_action, status);
  return wpa_tdls_tpk_send(sm, dst, tdls_action, dialog_token, status, 0,
                           initiator, NULL, 0);
}

static struct wpa_tdls_peer *wpa_tdls_add_peer(struct wpa_sm *sm,
                                               const u8 *addr, int *existing) {
  struct wpa_tdls_peer *peer;
  ...
  wpa_printf(MSG_INFO, "TDLS: Creating peer entry for " MACSTR, MAC2STR(addr));
...
static int wpa_tdls_send_tpk_m1(struct wpa_sm *sm, struct wpa_tdls_peer *peer) {
  size_t buf_len;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE for TPK handshake", peer->rsnie_i,
              peer->rsnie_i_len);
...
skip_rsnie:
...
#ifdef CONFIG_TDLS_TESTING
  if (wpa_tdls_get_privacy(sm) && (tdls_testing & TDLS_TESTING_LONG_FRAME))
    buf_len += 170;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: FTIE for TPK Handshake M1", (u8 *)ftie,
              sizeof(struct wpa_tdls_ftie));
...
#endif /* CONFIG_TDLS_TESTING */
  pos = wpa_add_tdls_timeoutie(pos, (u8 *)&timeoutie, sizeof(timeoutie),
                               peer->lifetime);
...
skip_ies:
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: Sending TDLS Setup Request / TPK "
             "Handshake Message 1 (peer " MACSTR ")",
             MAC2STR(peer->addr));

  status = wpa_tdls_tpk_send(sm, peer->addr, WLAN_TDLS_SETUP_REQUEST, 1, 0, 0,
                             peer->initiator, rbuf, pos - rbuf);
  os_free(rbuf);
...
static int wpa_tdls_send_tpk_m2(struct wpa_sm *sm,
  ...
  os_memcpy(ftie->Snonce, peer->inonce, WPA_NONCE_LEN);
  ftie->ie_len = sizeof(struct wpa_tdls_ftie) - 2;
  wpa_hexdump(MSG_DEBUG, "TDLS: FTIE for TPK M2", (u8 *)ftie, sizeof(*ftie));
...
skip_ies:
  status = wpa_tdls_tpk_send(sm, src_addr, WLAN_TDLS_SETUP_RESPONSE, dtoken, 0,
                             0, peer->initiator, rbuf, pos - rbuf);
  os_free(rbuf);
...
skip_ies:
  ...
  status = wpa_tdls_tpk_send(sm, src_addr, WLAN_TDLS_SETUP_CONFIRM, dtoken, 0,
                             peer_capab, peer->initiator, rbuf, pos - rbuf);
  os_free(rbuf);
...
static int wpa_tdls_send_discovery_response(struct wpa_sm *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: Sending TDLS Discovery Response "
             "(peer " MACSTR ")",
             MAC2STR(peer->addr));
  if (!wpa_tdls_get_privacy(sm))
    goto skip_rsn_ies;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE for Discovery Response", (u8 *)hdr,
              hdr->len + 2);
skip_rsn_ies:
  buf_len = 0;
  if (wpa_tdls_get_privacy(sm)) {
    /* Peer RSN IE, Lifetime */
    buf_len += peer->rsnie_i_len + sizeof(struct wpa_tdls_timeoutie);
  }
  rbuf = os_zalloc(buf_len + 1);
  if (rbuf == NULL) {
    wpa_tdls_peer_free(sm, peer);
...
static int wpa_tdls_process_discovery_request(struct wpa_sm *sm, const u8 *addr,
                                              const u8 *buf, size_t len) {
  struct wpa_eapol_ie_parse kde;
  const struct wpa_tdls_lnkid *lnkid;
  struct wpa_tdls_peer *peer;
  size_t min_req_len = sizeof(struct wpa_tdls_frame) + 1 /* dialog token */ +
                       sizeof(struct wpa_tdls_lnkid);
  u8 dialog_token;

  wpa_printf(MSG_DEBUG, "TDLS: Discovery Request from " MACSTR, MAC2STR(addr));

  if (len < min_req_len) {
    wpa_printf(MSG_DEBUG,
               "TDLS Discovery Request is too short: "
               "%d",
               (int)len);
    return -1;
  }
  ...
  if (wpa_supplicant_parse_ies(buf + sizeof(struct wpa_tdls_frame) + 1,
                               ...
                               &kde) < 0) {
    wpa_printf(MSG_DEBUG, "TDLS: Failed to parse IEs in Discovery Request - "
                          "ignore as an interop workaround");
  }
  ...
  if (os_memcmp(sm->bssid, lnkid->bssid, ETH_ALEN) != 0) {
    wpa_printf(MSG_DEBUG,
               "TDLS: Discovery Request from different "
               " BSS " MACSTR,
               MAC2STR(lnkid->bssid));
    return -1;
  }
...
int wpa_tdls_send_discovery_request(struct wpa_sm *sm, const u8 *addr) {
  ...
  wpa_printf(MSG_DEBUG, "TDLS: Sending Discovery Request to peer " MACSTR,
             MAC2STR(addr));
  return wpa_tdls_tpk_send(sm, addr, WLAN_TDLS_DISCOVERY_REQUEST, 1, 0, 0, 1,
                           NULL, 0);
}
...
static int copy_supp_rates(const struct wpa_eapol_ie_parse *kde,
  ...
  peer->supp_rates_len =
      merge_byte_arrays(peer->supp_rates, sizeof(peer->supp_rates),
...
static int copy_peer_ht_capab(const struct wpa_eapol_ie_parse *kde,
  ...
  if (!peer->ht_capabilities) {
    peer->ht_capabilities = os_zalloc(sizeof(struct ieee80211_ht_capabilities));
...
static int copy_peer_supp_channels(const struct wpa_eapol_ie_parse *kde,
  ...
  os_memcpy(peer->supp_channels, kde->supp_channels, peer->supp_channels_len);
...
static int copy_peer_supp_oper_classes(const struct wpa_eapol_ie_parse *kde,
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: Peer Supported Operating Classes",
              (u8 *)peer->supp_oper_classes, peer->supp_oper_classes_len);
  return 0;
}

static int wpa_tdls_addset_peer(struct wpa_sm *sm, struct wpa_tdls_peer *peer,
                                int add) {
  return wpa_sm_tdls_peer_addset(
      sm, peer->addr, add, peer->aid, peer->capability, peer->supp_rates,
      peer->supp_rates_len, peer->ht_capabilities, peer->vht_capabilities,
      peer->qos_info, peer->wmm_capable, peer->ext_capab, peer->ext_capab_len,
      peer->supp_channels, peer->supp_channels_len, peer->supp_oper_classes,
      peer->supp_oper_classes_len);
}
...
static int wpa_tdls_process_tpk_m1(struct wpa_sm *sm, const u8 *src_addr,
  ...
  if (existing_peer) {
    if (peer->tpk_success) {
      ...
      if (os_memcmp(sm->own_addr, src_addr, ETH_ALEN) < 0) {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Discard request "
                   "from peer with higher address " MACSTR,
                   MAC2STR(src_addr));
        return -1;
      } else {
        wpa_printf(MSG_DEBUG,
                   "TDLS: Accept request "
                   "from peer with lower address " MACSTR
  ...
  if (wpa_supplicant_parse_ies(cpos, ielen, &kde) < 0)
    wpa_printf(MSG_DEBUG, "TDLS: Failed to parse IEs in TPK M1 - ignore as an "
                          "interop workaround");
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: Link ID Received from TPK M1", kde.lnkid,
              kde.lnkid_len);
  lnkid = (struct wpa_tdls_lnkid *)kde.lnkid;
  if (os_memcmp(sm->bssid, lnkid->bssid, ETH_ALEN) != 0) {
  ...
  if (tdls_testing & TDLS_TESTING_CONCURRENT_INIT) {
    ...
    peer->initiator = 1;
    wpa_sm_tdls_peer_addset(sm, peer->addr, 1, 0, 0, NULL, 0, NULL, NULL, 0, 0,
                            NULL, 0, NULL, 0, NULL, 0);
    wpa_tdls_send_tpk_m1(sm, peer);
  }

  if ((tdls_testing & TDLS_TESTING_IGNORE_AP_PROHIBIT) && tdls_prohibited) {
  ...
  if (kde.ftie == NULL || kde.ftie_len < sizeof(*ftie) || kde.rsn_ie == NULL) {
...
skip_rsn:
  ...
  if (os_memcmp(peer->inonce, ftie->Snonce, WPA_NONCE_LEN) != 0 ||
     ...
     */
    wpa_printf(MSG_DEBUG, "TDLS: New TPK M1 INonce - generate new RNonce");
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE for TPK handshake", peer->rsnie_p,
              peer->rsnie_p_len);
...
static int wpa_tdls_enable_link(struct wpa_sm *sm, struct wpa_tdls_peer *peer) {
  ...
  if (wpa_tdls_get_privacy(sm)) {
    ...
    if (lifetime > 3 && peer->initiator)
      lifetime -= 3;
    eloop_register_timeout(lifetime, 0, wpa_tdls_tpk_timeout, sm, peer);
#ifdef CONFIG_TDLS_TESTING
...
static int wpa_tdls_process_tpk_m2(struct wpa_sm *sm, const u8 *src_addr,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: Received TDLS Setup Response / TPK M2 "
             "(Peer " MACSTR ")",
             MAC2STR(src_addr));
  for (peer = sm->tdls; peer; peer = peer->next) {
    if (os_memcmp(peer->addr, src_addr, ETH_ALEN) == 0)
  ...
  if (peer == NULL) {
    wpa_printf(MSG_INFO,
               "TDLS: No matching peer found for "
               "TPK M2: " MACSTR,
               MAC2STR(src_addr));
    return -1;
  }
  if (!peer->initiator) {
     ...
     */
    wpa_printf(MSG_INFO,
               "TDLS: We were not the initiator, so "
               "ignore TPK M2 from " MACSTR,
               MAC2STR(src_addr));
    return -1;
  }
  ...
  if (peer->tpk_success) {
    wpa_printf(MSG_INFO,
               "TDLS: Ignore incoming TPK M2 retry, from " MACSTR
               " as TPK M3 was already sent",
               MAC2STR(src_addr));
    return 0;
  }
  ...
  if (status != WLAN_STATUS_SUCCESS) {
    wpa_printf(MSG_INFO, "TDLS: Status code in TPK M2: %u", status);
    wpa_tdls_disable_peer_link(sm, peer);
    return -1;
  }
  ...
  if (wpa_supplicant_parse_ies(pos, ielen, &kde) < 0)
    wpa_printf(MSG_DEBUG, "TDLS: Failed to parse IEs in TPK M2 - ignore as an "
                          "interop workaround");
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: Link ID Received from TPK M2", kde.lnkid,
              kde.lnkid_len);
  lnkid = (struct wpa_tdls_lnkid *)kde.lnkid;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE Received from TPK M2", kde.rsn_ie,
              kde.rsn_ie_len);

  if (kde.rsn_ie_len > TDLS_MAX_IE_LEN) {
    wpa_printf(MSG_INFO, "TDLS: Too long Responder RSN IE in TPK M2");
    status = WLAN_STATUS_INVALID_RSNIE;
    goto error;
  }
  ...
  if (kde.rsn_ie_len != peer->rsnie_i_len ||
    ...
    wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE Sent in TPK M1", peer->rsnie_i,
                peer->rsnie_i_len);
    wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE Received from TPK M2", kde.rsn_ie,
                kde.rsn_ie_len);
    status = WLAN_STATUS_INVALID_RSNIE;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: FTIE Received from TPK M2", kde.ftie,
              sizeof(*ftie));
  ftie = (struct wpa_tdls_ftie *)kde.ftie;
  ...
  timeoutie = (struct wpa_tdls_timeoutie *)kde.key_lifetime;
  lifetime = WPA_GET_LE32(timeoutie->value);
  wpa_printf(MSG_DEBUG, "TDLS: TPK lifetime %u seconds in TPK M2", lifetime);
  if (lifetime != peer->lifetime) {
    wpa_printf(MSG_INFO,
               "TDLS: Unexpected TPK lifetime %u in "
               "TPK M2 (expected %u)",
               lifetime, peer->lifetime);
    status = WLAN_STATUS_UNACCEPTABLE_LIFETIME;
    goto error;
  ...
  if (wpa_supplicant_verify_tdls_mic(2, peer, (u8 *)lnkid, (u8 *)timeoutie,
                                     ftie) < 0) {
    /* Discard the frame */
...
error:
  wpa_tdls_send_error(sm, src_addr, WLAN_TDLS_SETUP_CONFIRM, dtoken, 1, status);
error_no_msg:
  wpa_tdls_disable_peer_link(sm, peer);
  return -1;
}
...
static int wpa_tdls_process_tpk_m3(struct wpa_sm *sm, const u8 *src_addr,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: Received TDLS Setup Confirm / TPK M3 "
             "(Peer " MACSTR ")",
             MAC2STR(src_addr));
  for (peer = sm->tdls; peer; peer = peer->next) {
    if (os_memcmp(peer->addr, src_addr, ETH_ALEN) == 0)
  ...
  if (status != 0) {
    wpa_printf(MSG_INFO, "TDLS: Status code in TPK M3: %u", status);
    goto error;
  }
   ...
   */
  if (wpa_supplicant_parse_ies((const u8 *)pos, ielen, &kde) < 0) {
    wpa_printf(MSG_DEBUG, "TDLS: Failed to parse KDEs in TPK M3 - ignore as an "
                          "interop workaround");
  }
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: Link ID Received from TPK M3", (u8 *)kde.lnkid,
              kde.lnkid_len);
  lnkid = (struct wpa_tdls_lnkid *)kde.lnkid;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: FTIE Received from TPK M3", kde.ftie,
              sizeof(*ftie));
  ftie = (struct wpa_tdls_ftie *)kde.ftie;
  ...
  wpa_hexdump(MSG_DEBUG, "TDLS: RSN IE Received from TPK M3", kde.rsn_ie,
              kde.rsn_ie_len);
  ...
  lifetime = WPA_GET_LE32(timeoutie->value);
  wpa_printf(MSG_DEBUG, "TDLS: TPK lifetime %u seconds in TPK M3", lifetime);
  if (lifetime != peer->lifetime) {
    wpa_printf(MSG_INFO,
               "TDLS: Unexpected TPK lifetime %u in "
               "TPK M3 (expected %u)",
  ...
  if (wpa_supplicant_verify_tdls_mic(3, peer, (u8 *)lnkid, (u8 *)timeoutie,
                                     ftie) < 0) {
    wpa_tdls_del_key(sm, peer);
    goto error;
  }
...
int wpa_tdls_start(struct wpa_sm *sm, const u8 *addr) {
  ...
  if (wpa_sm_tdls_peer_addset(sm, peer->addr, 1, 0, 0, NULL, 0, NULL, NULL, 0,
 ...
 */
static void wpa_supplicant_rx_tdls(void *ctx, const u8 *src_addr, const u8 *buf,
                                   size_t len) {
  struct wpa_sm *sm = ctx;
  struct wpa_tdls_frame *tf;

  wpa_hexdump(MSG_DEBUG, "TDLS: Received Data frame encapsulation", buf, len);
  ...
  if (tf->payloadtype != 2 /* TDLS_RFTYPE */ ||
    ...
    wpa_printf(MSG_INFO,
               "TDLS: Invalid frame - payloadtype=%u "
               "category=%u action=%u",
  ...
  default:
    ...
    wpa_printf(MSG_DEBUG, "TDLS: Ignore TDLS frame action code %u", tf->action);
    break;
  }
}
...
int wpa_tdls_init(struct wpa_sm *sm) {
  ...
  sm->l2_tdls = l2_packet_init(
      sm->bridge_ifname ? sm->bridge_ifname : sm->ifname, sm->own_addr,
      ETH_P_80211_ENCAP, wpa_supplicant_rx_tdls, sm, 0);
  if (sm->l2_tdls == NULL) {
   ...
   */
  if (wpa_sm_tdls_get_capa(sm, &sm->tdls_supported, &sm->tdls_external_setup,
  ...
  wpa_printf(MSG_DEBUG,
             "TDLS: TDLS operation%s supported by "
             "driver",
             sm->tdls_supported ? "" : " not");
...
void wpa_tdls_teardown_peers(struct wpa_sm *sm) {
  ...
  while (peer) {
    tmp = peer->next;
    wpa_printf(MSG_DEBUG, "TDLS: Tear down peer " MACSTR, MAC2STR(peer->addr));
    if (sm->tdls_external_setup)
      wpa_tdls_do_teardown(sm, peer, WLAN_REASON_DEAUTH_LEAVING);
    else
...
void wpa_tdls_ap_ies(struct wpa_sm *sm, const u8 *ies, size_t len) {
  ...
  sm->tdls_chan_switch_prohibited = wpa_tdls_chan_switch_prohibited(&elems);
...
void wpa_tdls_assoc_resp_ies(struct wpa_sm *sm, const u8 *ies, size_t len) {
  ...
  if (!sm->tdls_chan_switch_prohibited &&
    ...
    wpa_printf(MSG_DEBUG, "TDLS: TDLS channel switch prohibited based on "
                          "(Re)Association Response IEs");
...
int wpa_tdls_enable_chan_switch(struct wpa_sm *sm, const u8 *addr,
  ...
  if (sm->tdls_chan_switch_prohibited) {
    wpa_printf(MSG_DEBUG, "TDLS: Channel switching is prohibited in this BSS - "
                          "reject request to switch channel");
    return -1;
  }
  ...
  if (peer == NULL || !peer->tpk_success) {
    wpa_printf(MSG_ERROR,
               "TDLS: Peer " MACSTR " not found for channel switching",
               MAC2STR(addr));
    return -1;
  }
  ...
  if (peer->chan_switch_enabled) {
    wpa_printf(MSG_DEBUG,
               "TDLS: Peer " MACSTR " already has channel switching enabled",
               MAC2STR(addr));
    return 0;
  }

  ret = wpa_sm_tdls_enable_channel_switch(sm, peer->addr, oper_class,
                                          freq_params);
  if (!ret)
...
int wpa_tdls_disable_chan_switch(struct wpa_sm *sm, const u8 *addr) {
  ...
  if (!peer || !peer->chan_switch_enabled) {
    wpa_printf(MSG_ERROR, "TDLS: Channel switching not enabled for " MACSTR,
               MAC2STR(addr));
    return -1;
  }
