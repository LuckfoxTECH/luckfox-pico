...
static inline int wpa_sm_send_ft_action(struct wpa_sm *sm, u8 action,
  ...
  if (sm->ctx->send_ft_action)
    return sm->ctx->send_ft_action(sm->ctx->ctx, action, target_ap, ies,
                                   ies_len);
  return -1;
}
...
#ifdef CONFIG_TDLS
static inline int wpa_sm_tdls_get_capa(struct wpa_sm *sm, int *tdls_supported,
                                       int *tdls_ext_setup,
                                       int *tdls_chan_switch) {
  if (sm->ctx->tdls_get_capa)
    return sm->ctx->tdls_get_capa(sm->ctx->ctx, tdls_supported, tdls_ext_setup,
                                  tdls_chan_switch);
  return -1;
}
...
static inline int wpa_sm_send_tdls_mgmt(struct wpa_sm *sm, const u8 *dst,
  ...
  if (sm->ctx->send_tdls_mgmt)
    return sm->ctx->send_tdls_mgmt(sm->ctx->ctx, dst, action_code, dialog_token,
...
static inline int wpa_sm_tdls_peer_addset(
    struct wpa_sm *sm, const u8 *addr, int add, u16 aid, u16 capability,
  ...
  if (sm->ctx->tdls_peer_addset)
    return sm->ctx->tdls_peer_addset(
        sm->ctx->ctx, addr, add, aid, capability, supp_rates, supp_rates_len,
        ht_capab, vht_capab, qosinfo, wmm, ext_capab, ext_capab_len,
        supp_channels, supp_channels_len, supp_oper_classes,
        supp_oper_classes_len);
  return -1;
}

static inline int wpa_sm_tdls_enable_channel_switch(
    struct wpa_sm *sm, const u8 *addr, u8 oper_class,
    const struct hostapd_freq_params *freq_params) {
  if (sm->ctx->tdls_enable_channel_switch)
    return sm->ctx->tdls_enable_channel_switch(sm->ctx->ctx, addr, oper_class,
                                               freq_params);
  return -1;
}

static inline int wpa_sm_tdls_disable_channel_switch(struct wpa_sm *sm,
                                                     const u8 *addr) {
...
static inline int wpa_sm_key_mgmt_set_pmk(struct wpa_sm *sm, const u8 *pmk,
                                          size_t pmk_len) {
