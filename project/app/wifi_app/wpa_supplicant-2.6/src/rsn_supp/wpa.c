...
int wpa_eapol_key_send(struct wpa_sm *sm, const u8 *kck, size_t kck_len,
  ...
  if (is_zero_ether_addr(dest) && is_zero_ether_addr(sm->bssid)) {
    ...
    if (wpa_sm_get_bssid(sm, sm->bssid) < 0) {
      ...
      dest = sm->bssid;
      wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
              "WPA: Use BSSID (" MACSTR ") as the destination for EAPOL-Key",
              MAC2STR(dest));
    }
  }
  if (key_mic && wpa_eapol_key_mic(kck, kck_len, sm->key_mgmt, ver, msg,
                                   msg_len, key_mic)) {
...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise) {
  ...
  rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL, hdrlen, &rlen,
                            (void *)&reply);
  if (rbuf == NULL)
    return;
  reply192 = (struct wpa_eapol_key_192 *)reply;

  reply->type = (sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN)
                    ? EAPOL_KEY_TYPE_RSN
                    : EAPOL_KEY_TYPE_WPA;
  ...
  wpa_eapol_key_send(sm, sm->ptk.kck, sm->ptk.kck_len, ver, bssid, ETH_P_EAPOL,
                     rbuf, rlen, key_mic);
}
...
static void wpa_supplicant_key_mgmt_set_pmk(struct wpa_sm *sm) {
  ...
  if (sm->key_mgmt == WPA_KEY_MGMT_FT_IEEE8021X) {
    if (wpa_sm_key_mgmt_set_pmk(sm, sm->xxkey, sm->xxkey_len))
      wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
              "RSN: Cannot set low order 256 bits of MSK for key management "
              "offload");
  } else {
...
static int wpa_supplicant_get_pmk(struct wpa_sm *sm,
  ...
  if (pmkid && sm->cur_pmksa &&
    ...
    wpa_hexdump_key(MSG_DEBUG, "RSN: PMK from PMKSA cache", sm->pmk,
                    sm->pmk_len);
    ...
    if (res == 0) {
      ...
      if (sm->proto == WPA_PROTO_RSN && !wpa_key_mgmt_suite_b(sm->key_mgmt) &&
        ...
        sa = pmksa_cache_add(sm->pmksa, sm->pmk, pmk_len, NULL, NULL, 0,
      ...
      if (!sm->cur_pmksa && pmkid &&
         ...
         */
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                "RSN: PMKID mismatch - authentication server may have derived "
                "different MSK?!");
        return -1;
      }
  ...
  if (abort_cached && wpa_key_mgmt_wpa_ieee8021x(sm->key_mgmt) &&
      !wpa_key_mgmt_suite_b(sm->key_mgmt) && !wpa_key_mgmt_ft(sm->key_mgmt) &&
    ...
    buf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_START, NULL, 0, &buflen,
                             NULL);
    if (buf) {
...
int wpa_supplicant_send_2_of_4(struct wpa_sm *sm, const unsigned char *dst,
  ...
  if (wpa_key_mgmt_ft(sm->key_mgmt)) {
     ...
     */
    rsn_ie_buf =
        os_malloc(wpa_ie_len + 2 + 2 + PMKID_LEN + sm->assoc_resp_ies_len);
    if (rsn_ie_buf == NULL)
      return -1;
...
static int wpa_derive_ptk(struct wpa_sm *sm, const unsigned char *src_addr,
  ...
  return wpa_pmk_to_ptk(sm->pmk, sm->pmk_len, "Pairwise key expansion",
                        ...
                        ptk, sm->key_mgmt, sm->pairwise_cipher);
}
...
static void wpa_supplicant_key_neg_complete(struct wpa_sm *sm, const u8 *addr,
                                            int secure) {
  wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
          "WPA: Key negotiation completed with " MACSTR " [PTK=%s GTK=%s]",
          MAC2STR(addr), wpa_cipher_txt(sm->pairwise_cipher),
...
static int wpa_supplicant_install_ptk(struct wpa_sm *sm,
  ...
  if (!wpa_cipher_valid_pairwise(sm->pairwise_cipher)) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
            "WPA: Unsupported pairwise cipher %d", sm->pairwise_cipher);
    return -1;
  }
  ...
  if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, key_rsc, rsclen, sm->ptk.tk,
                     keylen) < 0) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
...
static int wpa_supplicant_check_group_cipher(struct wpa_sm *sm,
                                             int group_cipher, int keylen,
                                             int maxkeylen, int *key_rsc_len,
                                             enum wpa_alg *alg) {
  int klen;

  *alg = wpa_cipher_to_alg(group_cipher);
  if (*alg == WPA_ALG_NONE) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported Group Cipher %d",
            group_cipher);
    return -1;
  }
...
static int wpa_supplicant_install_gtk(struct wpa_sm *sm,
  ...
  if ((sm->gtk.gtk_len == (size_t)gd->gtk_len &&
      ...
      (sm->gtk_wnm_sleep.gtk_len == (size_t)gd->gtk_len &&
       os_memcmp(sm->gtk_wnm_sleep.gtk, gd->gtk, sm->gtk_wnm_sleep.gtk_len) ==
           0)) {
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
            "WPA: Not reinstalling already in-use GTK to the driver (keyidx=%d "
            "tx=%d len=%d)",
...
static int wpa_supplicant_pairwise_gtk(struct wpa_sm *sm,
  ...
  wpa_hexdump_key(MSG_DEBUG, "RSN: received GTK in pairwise handshake", gtk,
                  gtk_len);
  ...
  if (sm->group_cipher != WPA_CIPHER_GTK_NOT_USED &&
      (wpa_supplicant_check_group_cipher(sm, sm->group_cipher, gtk_len, gtk_len,
...
static int wpa_supplicant_install_igtk(struct wpa_sm *sm,
  ...
  if (keyidx > 4095) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid IGTK KeyID %d",
            keyidx);
    return -1;
  }
...
static void wpa_report_ie_mismatch(struct wpa_sm *sm, const char *reason,
                                   const u8 *src_addr, const u8 *wpa_ie,
                                   size_t wpa_ie_len, const u8 *rsn_ie,
                                   size_t rsn_ie_len) {
  wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: %s (src=" MACSTR ")", reason,
          MAC2STR(src_addr));

  if (sm->ap_wpa_ie) {
    wpa_hexdump(MSG_INFO, "WPA: WPA IE in Beacon/ProbeResp", sm->ap_wpa_ie,
                sm->ap_wpa_ie_len);
  }
  if (wpa_ie) {
    if (!sm->ap_wpa_ie) {
  ...
  if (sm->ap_rsn_ie) {
    wpa_hexdump(MSG_INFO, "WPA: RSN IE in Beacon/ProbeResp", sm->ap_rsn_ie,
                sm->ap_rsn_ie_len);
  }
  if (rsn_ie) {
    if (!sm->ap_rsn_ie) {
...
static int ft_validate_mdie(struct wpa_sm *sm, const unsigned char *src_addr,
                            struct wpa_eapol_ie_parse *ie,
  ...
  if (assoc_resp_mdie &&
    ...
    wpa_hexdump(MSG_DEBUG, "FT: MDIE in EAPOL-Key msg 3/4", ie->mdie,
                2 + ie->mdie[1]);
...
static int ft_validate_ftie(struct wpa_sm *sm, const unsigned char *src_addr,
                            struct wpa_eapol_ie_parse *ie,
  ...
  if (assoc_resp_ftie[1] != ie->ftie[1] ||
    ...
    wpa_hexdump(MSG_DEBUG, "FT: FTIE in EAPOL-Key msg 3/4", ie->ftie,
                2 + ie->ftie[1]);
...
static int ft_validate_rsnie(struct wpa_sm *sm, const unsigned char *src_addr,
                             struct wpa_eapol_ie_parse *ie) {
  ...
  if (os_memcmp_const(rsn.pmkid, sm->pmk_r1_name, WPA_PMK_NAME_LEN) != 0) {
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
            ...
            "FT 4-way handshake message 3/4");
    wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name from Authenticator", rsn.pmkid,
                WPA_PMK_NAME_LEN);
...
static int wpa_supplicant_validate_ie(struct wpa_sm *sm,
  ...
  if (ie->wpa_ie == NULL && ie->rsn_ie == NULL &&
    ...
    wpa_report_ie_mismatch(sm,
                           ...
                           src_addr, ie->wpa_ie, ie->wpa_ie_len, ie->rsn_ie,
                           ie->rsn_ie_len);
    return -1;
  }
  ...
  if ((ie->wpa_ie && sm->ap_wpa_ie &&
      ...
      (ie->rsn_ie && sm->ap_rsn_ie &&
       wpa_compare_rsn_ie(wpa_key_mgmt_ft(sm->key_mgmt), sm->ap_rsn_ie,
                          sm->ap_rsn_ie_len, ie->rsn_ie, ie->rsn_ie_len))) {
    wpa_report_ie_mismatch(sm,
                           "IE in 3/4 msg does not match "
  ...
  if (sm->proto == WPA_PROTO_WPA && ie->rsn_ie && sm->ap_rsn_ie == NULL &&
      sm->rsn_enabled) {
    wpa_report_ie_mismatch(sm,
...
static void wpa_supplicant_process_3_of_4(struct wpa_sm *sm,
  ...
  if (ie.igtk && wpa_cipher_valid_mgmt_group(sm->mgmt_group_cipher) &&
      ie.igtk_len != WPA_IGTK_KDE_PREFIX_LEN + (unsigned int)wpa_cipher_key_len(
                                                   sm->mgmt_group_cipher)) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid IGTK KDE length %lu",
  ...
  if (os_memcmp(sm->anonce, key->key_nonce, WPA_NONCE_LEN) != 0) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
            ...
            "differs from 3 of 4-Way Handshake - drop packet (src=" MACSTR ")",
            MAC2STR(sm->bssid));
    goto failed;
  }
  ...
  if (keylen != wpa_cipher_key_len(sm->pairwise_cipher)) {
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
            ...
            wpa_cipher_txt(sm->pairwise_cipher), keylen, MAC2STR(sm->bssid));
    goto failed;
  }
  ...
  if (ie.ip_addr_alloc) {
    ...
    wpa_hexdump(MSG_DEBUG, "P2P: IP address info", sm->p2p_ip_addr,
  ...
  if (sm->group_cipher == WPA_CIPHER_GTK_NOT_USED) {
    wpa_supplicant_key_neg_complete(sm, sm->bssid,
                                    key_info & WPA_KEY_INFO_SECURE);
  } else if (ie.gtk && wpa_supplicant_pairwise_gtk(sm, key, ie.gtk, ie.gtk_len,
                                                   key_info) < 0) {
  ...
  if (sm->proto == WPA_PROTO_RSN && wpa_key_mgmt_suite_b(sm->key_mgmt)) {
    ...
    sa = pmksa_cache_add(sm->pmksa, sm->pmk, sm->pmk_len, NULL, sm->ptk.kck,
                         sm->ptk.kck_len, sm->bssid, sm->own_addr,
...
static int wpa_supplicant_process_1_of_2_rsn(struct wpa_sm *sm,
  ...
  if (wpa_supplicant_check_group_cipher(sm, sm->group_cipher, gd->gtk_len,
                                        maxkeylen, &gd->key_rsc_len, &gd->alg))
    return -1;

  wpa_hexdump_key(MSG_DEBUG, "RSN: received GTK in group key handshake", ie.gtk,
                  ie.gtk_len);
  gd->keyidx = ie.gtk[0] & 0x3;
  gd->tx = wpa_supplicant_gtk_tx_bit_workaround(sm, !!(ie.gtk[0] & BIT(2)));
  if (ie.gtk_len - 2 > sizeof(gd->gtk)) {
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Too long GTK in GTK IE (len=%lu)",
...
static int wpa_supplicant_process_1_of_2_wpa(struct wpa_sm *sm,
  ...
  if (ver == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES) {
    if (maxkeylen < 8) {
      wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Too short maxkeylen (%lu)",
  ...
  if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && sm->ptk.kek_len == 16) {
    ...
    if (key_data_len > sizeof(gd->gtk)) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: RC4 key data too long (%lu)",
    ...
    if (maxkeylen % 8) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
              "WPA: Unsupported AES-WRAP len %lu", (unsigned long)maxkeylen);
      return -1;
    }
    if (maxkeylen > sizeof(gd->gtk)) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
              ...
              (unsigned long)key_data_len, (unsigned long)maxkeylen);
      return -1;
    }
    if (aes_unwrap(sm->ptk.kek, sm->ptk.kek_len, maxkeylen / 8, key_data,
                   gd->gtk)) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
    ...
    wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported key_info type %d",
            ver);
    return -1;
  }
...
static void wpa_supplicant_process_1_of_2(struct wpa_sm *sm,
  ...
  if (!sm->msg_3_of_4_ok) {
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
            "WPA: Group Key Handshake started prior to completion of 4-way "
            "handshake");
    goto failed;
  }
  ...
  if (sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN) {
    ret = wpa_supplicant_process_1_of_2_rsn(sm, key_data, key_data_len,
                                            key_info, &gd);
  } else {
    ret = wpa_supplicant_process_1_of_2_wpa(sm, key, key_data, key_data_len,
                                            key_info, ver, &gd);
  }
...
static int wpa_supplicant_verify_eapol_key_mic(struct wpa_sm *sm,
  ...
  if (sm->tptk_set) {
    ...
    wpa_eapol_key_mic(sm->tptk.kck, sm->tptk.kck_len, sm->key_mgmt, ver, buf,
                      len, key->key_mic);
  ...
  if (!ok && sm->ptk_set) {
    os_memset(key->key_mic, 0, mic_len);
    wpa_eapol_key_mic(sm->ptk.kck, sm->ptk.kck_len, sm->key_mgmt, ver, buf, len,
                      key->key_mic);
    if (os_memcmp_const(mic, key->key_mic, mic_len) != 0) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
              "WPA: Invalid EAPOL-Key MIC - "
...
static int wpa_supplicant_decrypt_key_data(struct wpa_sm *sm,
  ...
  if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && sm->ptk.kek_len == 16) {
    ...
    if (*key_data_len < 8 || *key_data_len % 8) {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported AES-WRAP len %u",
    ...
    *key_data_len -= 8; /* AES-WRAP adds 8 bytes */
    ...
    if (aes_unwrap(sm->ptk.kek, sm->ptk.kek_len, *key_data_len / 8, key_data,
                   buf)) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "WPA: decrypted EAPOL-Key key data", key_data,
                  *key_data_len);
  return 0;
}
...
static void wpa_eapol_key_dump(struct wpa_sm *sm,
  ...
  wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "  key_length=%u key_data_length=%u",
 ...
 */
int wpa_sm_rx_eapol(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf,
                    size_t len) {
  ...
  wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
          "IEEE 802.1X RX: version=%d type=%d length=%lu", hdr->version,
  ...
  if (key->type != EAPOL_KEY_TYPE_WPA && key->type != EAPOL_KEY_TYPE_RSN) {
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
            "WPA: EAPOL-Key type (%d) unknown, discarded", key->type);
    ret = 0;
    goto out;
  }
  ...
  if (ver != WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 &&
      ...
      sm->key_mgmt != WPA_KEY_MGMT_OSEN) {
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
            "WPA: Unsupported EAPOL-Key descriptor version %d", ver);
    goto out;
  }
  ...
  if (sm->key_mgmt == WPA_KEY_MGMT_OSEN &&
      ver != WPA_KEY_INFO_TYPE_AKM_DEFINED) {
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
            "OSEN: Unsupported EAPOL-Key descriptor version %d", ver);
    goto out;
  }
  ...
  if (wpa_key_mgmt_suite_b(sm->key_mgmt) &&
      ver != WPA_KEY_INFO_TYPE_AKM_DEFINED) {
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
            "RSN: Unsupported EAPOL-Key descriptor version %d (expected AKM "
            "defined = 0)",
            ver);
    goto out;
  }
  ...
  if (wpa_key_mgmt_ft(sm->key_mgmt)) {
    ...
    if (sm->pairwise_cipher == WPA_CIPHER_CCMP &&
      ...
      if (sm->group_cipher != WPA_CIPHER_CCMP &&
        ...
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                "WPA: Interoperability workaround: allow incorrect (should "
                "have been HMAC-SHA1), but stronger (is AES-128-CMAC), "
                "descriptor version to be used");
      } else
        goto out;
  ...
  if (!(key_info & WPA_KEY_INFO_SMK_MESSAGE) && peerkey) {
    if (!peerkey->initiator && peerkey->replay_counter_set &&
      ...
      if (os_memcmp(_tmp, peerkey->replay_counter, WPA_REPLAY_COUNTER_LEN) !=
          0) {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
...
#ifdef CONFIG_PEERKEY
  if ((key_info & WPA_KEY_INFO_MIC) && peerkey &&
      peerkey_verify_eapol_key_mic(sm, peerkey, key192, ver, tmp, data_len))
    goto out;
#endif /* CONFIG_PEERKEY */
  ...
  if (key_info & WPA_KEY_INFO_KEY_TYPE) {
    ...
    if (peerkey) {
      ...
      peerkey_rx_eapol_4way(sm, peerkey, key, key_info, ver, key_data,
                            key_data_len);
      ...
      /* 1/4 4-Way Handshake */
      wpa_supplicant_process_1_of_4(sm, src_addr, key, ver, key_data,
                                    key_data_len);
    }
    ...
    if (key_info & WPA_KEY_INFO_MIC) {
      ...
      wpa_supplicant_process_1_of_2(sm, src_addr, key, key_data, key_data_len,
                                    ver);
    } else {
      wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
...
static u32 wpa_key_mgmt_suite(struct wpa_sm *sm) {
  ...
  case WPA_KEY_MGMT_IEEE8021X:
    return ((sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN)
                ? RSN_AUTH_KEY_MGMT_UNSPEC_802_1X
                : WPA_AUTH_KEY_MGMT_UNSPEC_802_1X);
  case WPA_KEY_MGMT_PSK:
    return (sm->proto == WPA_PROTO_RSN ? RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X
  ...
  case WPA_KEY_MGMT_CCKM:
    return (sm->proto == WPA_PROTO_RSN ? RSN_AUTH_KEY_MGMT_CCKM
...
int wpa_sm_get_mib(struct wpa_sm *sm, char *buf, size_t buflen) {
  ...
  if (sm->cur_pmksa) {
    wpa_snprintf_hex(pmkid_txt, sizeof(pmkid_txt), sm->cur_pmksa->pmkid,
                     PMKID_LEN);
  } else
    pmkid_txt[0] = '\0';
  ...
  ret = os_snprintf(
      ...
      rsna ? "TRUE" : "FALSE", rsna ? "TRUE" : "FALSE", RSN_VERSION,
      wpa_cipher_key_len(sm->group_cipher) * 8, sm->dot11RSNAConfigPMKLifetime,
      sm->dot11RSNAConfigPMKReauthThreshold, sm->dot11RSNAConfigSATimeout);
  ...
  ret = os_snprintf(
      ...
      RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->pairwise_cipher)),
      RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->group_cipher)),
      pmkid_txt, RSN_SUITE_ARG(wpa_key_mgmt_suite(sm)),
      RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->pairwise_cipher)),
      RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->group_cipher)),
...
static void wpa_sm_pmksa_free_cb(struct rsn_pmksa_cache_entry *entry, void *ctx,
                                 enum pmksa_free_reason reason) {
  struct wpa_sm *sm = ctx;
  int deauth = 0;

  wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
          "RSN: PMKSA cache entry free_cb: " MACSTR " reason=%d",
          MAC2STR(entry->aa), reason);

  if (sm->cur_pmksa == entry) {
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: %s current PMKSA entry",
...
void wpa_sm_set_pmk(struct wpa_sm *sm, const u8 *pmk, size_t pmk_len,
  ...
  if (bssid) {
    pmksa_cache_add(sm->pmksa, pmk, pmk_len, pmkid, NULL, 0, bssid,
                    sm->own_addr, sm->network_ctx, sm->key_mgmt);
  }
}
...
#ifdef CONFIG_TESTING_OPTIONS
  if (sm->test_assoc_ie) {
    wpa_printf(MSG_DEBUG, "TESTING: Replace association WPA/RSN IE");
...
int wpa_wnmsleep_install_key(struct wpa_sm *sm, u8 subelem_id, u8 *buf) {
  ...
  if (subelem_id == WNM_SLEEP_SUBELEM_GTK) {
    ...
    gd.gtk_len = keylen;
    if (gd.gtk_len != buf[4]) {
      wpa_printf(MSG_DEBUG, "GTK len mismatch len %d vs %d", gd.gtk_len,
                 buf[4]);
      return -1;
    }
