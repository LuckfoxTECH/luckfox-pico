...
static int wpa_supplicant_send_smk_error(struct wpa_sm *sm, const u8 *dst,
                                         const u8 *peer, u16 mui,
                                         u16 error_type, int ver) {
  size_t rlen;
  ...
  rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
                            sizeof(*err) + kde_len, &rlen, (void *)&err);
  if (rbuf == NULL)
    return -1;
  ...
  if (peer) {
    wpa_printf(MSG_DEBUG,
               "RSN: Sending EAPOL-Key SMK Error (peer " MACSTR
               " mui %d error_type %d)",
  ...
  wpa_eapol_key_send(sm, sm->ptk.kck, sm->ptk.kck_len, ver, dst, ETH_P_EAPOL,
                     rbuf, rlen, err192->key_mic);
...
static int wpa_supplicant_send_smk_m3(struct wpa_sm *sm,
  ...
  kde_len = peerkey->rsnie_p_len + 2 + RSN_SELECTOR_LEN + ETH_ALEN + 2 +
            RSN_SELECTOR_LEN + WPA_NONCE_LEN;

  rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
                            sizeof(*reply) + kde_len, &rlen, (void *)&reply);
  if (rbuf == NULL)
    return -1;
  ...
  key_info =
      ver | WPA_KEY_INFO_SMK_MESSAGE | WPA_KEY_INFO_MIC | WPA_KEY_INFO_SECURE;
...
static int wpa_supplicant_process_smk_m2(struct wpa_sm *sm,
  ...
  cipher = wpa_pick_pairwise_cipher(
      ie.pairwise_cipher & sm->allowed_pairwise_cipher, 0);
  if (cipher < 0) {
    wpa_printf(MSG_INFO, "RSN: No acceptable cipher in SMK M2");
    wpa_supplicant_send_smk_error(sm, src_addr, kde.mac_addr, STK_MUI_SMK,
                                  STK_ERR_CPHR_NS, ver);
    return -1;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "WPA: RSN IE for SMK handshake", peerkey->rsnie_p,
              peerkey->rsnie_p_len);
...
static void rsn_smkid(const u8 *smk, const u8 *pnonce, const u8 *mac_p,
  ...
  char *title = "SMK Name";
  const u8 *addr[5];
  const size_t len[5] = {8, WPA_NONCE_LEN, ETH_ALEN, WPA_NONCE_LEN, ETH_ALEN};
...
static void wpa_supplicant_send_stk_1_of_4(struct wpa_sm *sm,
  ...
  mbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
                            sizeof(*msg) + kde_len, &mlen, (void *)&msg);
  if (mbuf == NULL)
    return;
  ...
  wpa_add_kde((u8 *)(msg + 1), RSN_KEY_DATA_PMKID, peerkey->smkid, PMKID_LEN);
  ...
  wpa_hexdump(MSG_DEBUG, "RSN: INonce for STK 4-Way Handshake", peerkey->inonce,
              WPA_NONCE_LEN);
  os_memcpy(msg->key_nonce, peerkey->inonce, WPA_NONCE_LEN);

  wpa_printf(MSG_DEBUG, "RSN: Sending EAPOL-Key STK 1/4 to " MACSTR,
             MAC2STR(peerkey->addr));
  wpa_eapol_key_send(sm, NULL, 0, ver, peerkey->addr, ETH_P_EAPOL, mbuf, mlen,
                     NULL);
}
...
static void wpa_supplicant_send_stk_3_of_4(struct wpa_sm *sm,
  ...
  key_info = ver | WPA_KEY_INFO_KEY_TYPE | WPA_KEY_INFO_ACK | WPA_KEY_INFO_MIC |
             WPA_KEY_INFO_SECURE;
  WPA_PUT_BE16(msg->key_info, key_info);
  ...
  wpa_add_kde(pos, RSN_KEY_DATA_LIFETIME, (u8 *)&lifetime, sizeof(lifetime));
  ...
  wpa_eapol_key_send(sm, peerkey->stk.kck, peerkey->stk.kck_len, ver,
                     peerkey->addr, ETH_P_EAPOL, mbuf, mlen, msg->key_mic);
}
...
static int wpa_supplicant_process_smk_m5(struct wpa_sm *sm,
  ...
  if (cipher < 0) {
    wpa_printf(MSG_INFO,
               "RSN: SMK Peer STA " MACSTR " selected "
               "unacceptable cipher",
               MAC2STR(kde->mac_addr));
    wpa_supplicant_send_smk_error(sm, src_addr, kde->mac_addr, STK_MUI_SMK,
                                  STK_ERR_CPHR_NS, ver);
...
static int wpa_supplicant_process_smk_m45(struct wpa_sm *sm,
  ...
  if (kde.mac_addr == NULL || kde.mac_addr_len < ETH_ALEN ||
      kde.nonce == NULL || kde.nonce_len < WPA_NONCE_LEN || kde.smk == NULL ||
      kde.smk_len < PMK_LEN + WPA_NONCE_LEN || kde.lifetime == NULL ||
      kde.lifetime_len < 4) {
  ...
  for (peerkey = sm->peerkey; peerkey; peerkey = peerkey->next) {
    if (os_memcmp(peerkey->addr, kde.mac_addr, ETH_ALEN) == 0 &&
        os_memcmp(peerkey->initiator ? peerkey->inonce : peerkey->pnonce,
  ...
  if (peerkey->initiator) {
    if (wpa_supplicant_process_smk_m5(sm, src_addr, key, ver, peerkey, &kde) <
        0)
      return -1;
  } else {
  ...
  eloop_register_timeout(lifetime, 0, wpa_supplicant_smk_timeout, sm, peerkey);

  if (peerkey->initiator) {
    rsn_smkid(peerkey->smk, peerkey->pnonce, peerkey->addr, peerkey->inonce,
              sm->own_addr, peerkey->smkid, peerkey->akmp);
    wpa_supplicant_send_stk_1_of_4(sm, peerkey);
  } else {
    rsn_smkid(peerkey->smk, peerkey->pnonce, sm->own_addr, peerkey->inonce,
              peerkey->addr, peerkey->smkid, peerkey->akmp);
  }
...
static int wpa_supplicant_process_smk_error(struct wpa_sm *sm,
  ...
  wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
          "RSN: SMK Error KDE received: MUI %d error_type %d peer " MACSTR,
          be_to_host16(error.mui), error_type, MAC2STR(peer));
...
static void wpa_supplicant_process_stk_1_of_4(struct wpa_sm *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "RSN: RX message 1 of STK 4-Way Handshake from " MACSTR
             " (ver=%d)",
             MAC2STR(peerkey->addr), ver);
  ...
  if (os_memcmp_const(ie.pmkid, peerkey->smkid, PMKID_LEN) != 0) {
    wpa_hexdump(MSG_DEBUG, "RSN: Unknown SMKID in STK 1/4", ie.pmkid,
                PMKID_LEN);
    return;
  }
  ...
  wpa_hexdump(MSG_DEBUG, "WPA: Renewed PNonce", peerkey->pnonce, WPA_NONCE_LEN);

  /* Calculate STK which will be stored as a temporary STK until it has
   * been verified when processing message 3/4. */
  stk = &peerkey->tstk;
  wpa_pmk_to_ptk(peerkey->smk, PMK_LEN, "Peer key expansion", sm->own_addr,
                 peerkey->addr, peerkey->pnonce, key->key_nonce, stk,
                 peerkey->akmp, peerkey->cipher);
  ...
  pos = wpa_add_ie(pos, peerkey->rsnie_p, peerkey->rsnie_p_len);
  lifetime = host_to_be32(peerkey->lifetime);
  pos = wpa_add_kde(pos, RSN_KEY_DATA_LIFETIME, (u8 *)&lifetime,
                    sizeof(lifetime));
  wpa_add_kde(pos, RSN_KEY_DATA_PMKID, peerkey->smkid, PMKID_LEN);

  if (wpa_supplicant_send_2_of_4(sm, peerkey->addr, key, ver, peerkey->pnonce,
...
static void wpa_supplicant_process_stk_2_of_4(struct wpa_sm *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "RSN: RX message 2 of STK 4-Way Handshake from " MACSTR
             " (ver=%d)",
             MAC2STR(peerkey->addr), ver);
  ...
  if (os_memcmp_const(kde.pmkid, peerkey->smkid, PMKID_LEN) != 0) {
    wpa_hexdump(MSG_DEBUG, "RSN: Unknown SMKID in STK 2/4", kde.pmkid,
                PMKID_LEN);
    return;
  }
  ...
  if (kde.rsn_ie_len != peerkey->rsnie_p_len ||
    ...
    wpa_hexdump(MSG_DEBUG, "RSN: Peer RSN IE in STK handshake", kde.rsn_ie,
                kde.rsn_ie_len);
    return;
  }
...
static void wpa_supplicant_process_stk_3_of_4(struct wpa_sm *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "RSN: RX message 3 of STK 4-Way Handshake from " MACSTR
             " (ver=%d)",
             MAC2STR(peerkey->addr), ver);
  ...
  if (wpa_supplicant_send_4_of_4(sm, peerkey->addr, key, ver,
                                 WPA_GET_BE16(key->key_info), &peerkey->stk))
    return;
  ...
  os_memset(rsc, 0, 6);
  if (wpa_sm_set_key(sm, peerkey->cipher, peerkey->addr, 0, 1, rsc, sizeof(rsc),
                     _key, key_len) < 0) {
...
static void wpa_supplicant_process_stk_4_of_4(struct wpa_sm *sm,
  ...
  wpa_printf(MSG_DEBUG,
             "RSN: RX message 4 of STK 4-Way Handshake from " MACSTR
             " (ver=%d)",
             MAC2STR(peerkey->addr), ver);
 ...
 */
int peerkey_verify_eapol_key_mic(struct wpa_sm *sm, struct wpa_peerkey *peerkey,
  ...
  if (peerkey->initiator && !peerkey->stk_set) {
    wpa_pmk_to_ptk(peerkey->smk, PMK_LEN, "Peer key expansion", sm->own_addr,
                   peerkey->addr, peerkey->inonce, key->key_nonce,
  ...
  if (peerkey->tstk_set) {
    ...
    wpa_eapol_key_mic(peerkey->tstk.kck, peerkey->tstk.kck_len, sm->key_mgmt,
    ...
    if (os_memcmp_const(mic, key->key_mic, mic_len) != 0) {
      ...
      peerkey->tstk_set = 0;
      peerkey->stk_set = 1;
      os_memcpy(&peerkey->stk, &peerkey->tstk, sizeof(peerkey->stk));
  ...
  if (!ok && peerkey->stk_set) {
    os_memset(key->key_mic, 0, mic_len);
    wpa_eapol_key_mic(peerkey->stk.kck, peerkey->stk.kck_len, sm->key_mgmt, ver,
                      buf, len, key->key_mic);
    if (os_memcmp_const(mic, key->key_mic, mic_len) != 0) {
      wpa_printf(MSG_WARNING, "RSN: Invalid EAPOL-Key MIC "
                              "- dropping packet");
...
int wpa_sm_stkstart(struct wpa_sm *sm, const u8 *peer) {
  ...
  rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL,
                            sizeof(*req) + kde_len, &rlen, (void *)&req);
  if (rbuf == NULL) {
    wpa_supplicant_peerkey_free(sm, peerkey);
    return -1;
  }
  ...
  key_info = WPA_KEY_INFO_SMK_MESSAGE | WPA_KEY_INFO_MIC | WPA_KEY_INFO_SECURE |
             WPA_KEY_INFO_REQUEST | ver;
  WPA_PUT_BE16(req->key_info, key_info);
  WPA_PUT_BE16(req->key_length, 0);
  os_memcpy(req->replay_counter, sm->request_counter, WPA_REPLAY_COUNTER_LEN);
  inc_byte_array(sm->request_counter, WPA_REPLAY_COUNTER_LEN);
  ...
  wpa_hexdump(MSG_DEBUG, "WPA: INonce for SMK handshake", req->key_nonce,
              WPA_NONCE_LEN);
  ...
  wpa_printf(MSG_INFO,
             "RSN: Sending EAPOL-Key SMK M1 Request (peer " MACSTR ")",
             MAC2STR(peer));
...
void peerkey_rx_eapol_4way(struct wpa_sm *sm, struct wpa_peerkey *peerkey,
  ...
  if ((key_info & (WPA_KEY_INFO_MIC | WPA_KEY_INFO_ACK)) ==
    ...
    wpa_supplicant_process_stk_3_of_4(sm, peerkey, key, ver, key_data,
                                      key_data_len);
  } else if (key_info & WPA_KEY_INFO_ACK) {
    /* 1/4 STK 4-Way Handshake */
    wpa_supplicant_process_stk_1_of_4(sm, peerkey, key, ver, key_data,
                                      key_data_len);
    ...
    wpa_supplicant_process_stk_2_of_4(sm, peerkey, key, ver, key_data,
                                      key_data_len);
  }
}
