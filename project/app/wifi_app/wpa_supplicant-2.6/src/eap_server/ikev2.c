...
static int ikev2_process_idr(struct ikev2_initiator_data *data, const u8 *idr,
                             size_t idr_len);
...
static int ikev2_derive_keys(struct ikev2_initiator_data *data) {
  ...
  shared = dh_derive_shared(data->r_dh_public, data->i_dh_private, data->dh);
  if (shared == NULL)
    return -1;
  ...
  if (ikev2_prf_hash(prf->id, buf, data->i_nonce_len + data->r_nonce_len, 2,
                     addr, len, skeyseed) < 0) {
    wpabuf_free(shared);
    os_free(buf);
    os_free(pad);
  ...
  wpa_hexdump_key(MSG_DEBUG, "IKEV2: SKEYSEED", skeyseed, prf->hash_len);
...
static int ikev2_parse_transform(struct ikev2_initiator_data *data,
  ...
  if (transform_len < (int)sizeof(*t) || transform_len > end - pos) {
    wpa_printf(MSG_INFO, "IKEV2: Invalid transform length %d", transform_len);
    return -1;
  }
  ...
  pos = (const u8 *)(t + 1);
  if (pos < tend) {
    wpa_hexdump(MSG_DEBUG, "IKEV2:     Transform Attributes", pos, tend - pos);
  }

  switch (t->transform_type) {
  case IKEV2_TRANSFORM_ENCR:
    if (ikev2_get_encr(transform_id) && transform_id == data->proposal.encr) {
  ...
  case IKEV2_TRANSFORM_PRF:
    if (ikev2_get_prf(transform_id) && transform_id == data->proposal.prf)
      prop->prf = transform_id;
    break;
  case IKEV2_TRANSFORM_INTEG:
    if (ikev2_get_integ(transform_id) && transform_id == data->proposal.integ)
      prop->integ = transform_id;
    break;
  case IKEV2_TRANSFORM_DH:
    if (dh_groups_get(transform_id) && transform_id == data->proposal.dh)
...
static int ikev2_parse_proposal(struct ikev2_initiator_data *data,
                                struct ikev2_proposal_data *prop, const u8 *pos,
                                const u8 *end) {
  const u8 *pend, *ppos;
  int proposal_len, i;
  ...
  if (proposal_len < (int)sizeof(*p) || proposal_len > end - pos) {
    wpa_printf(MSG_INFO, "IKEV2: Invalid proposal length %d", proposal_len);
    return -1;
  }
  wpa_printf(MSG_DEBUG, "IKEV2: SAi1 Proposal # %d", p->proposal_num);
  wpa_printf(MSG_DEBUG,
             "IKEV2:   Type: %d  Proposal Length: %d "
             " Protocol ID: %d",
             p->type, proposal_len, p->protocol_id);
  wpa_printf(MSG_DEBUG, "IKEV2:   SPI Size: %d  Transforms: %d", p->spi_size,
             p->num_transforms);
  ...
  if (p->spi_size) {
    wpa_hexdump(MSG_DEBUG, "IKEV2:    SPI", ppos, p->spi_size);
    ppos += p->spi_size;
  }
...
static int ikev2_process_sar1(struct ikev2_initiator_data *data, const u8 *sar1,
                              size_t sar1_len) {
  struct ikev2_proposal_data prop;
  const u8 *pos, *end;
  int found = 0;
  ...
  while (pos < end) {
    ...
    if (!found && prop.integ != -1 && prop.prf != -1 && prop.encr != -1 &&
        prop.dh != -1) {
      found = 1;
    }
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "INTEG:%d D-H:%d",
             data->proposal.proposal_num, data->proposal.encr,
             data->proposal.prf, data->proposal.integ, data->proposal.dh);

  return 0;
}

static int ikev2_process_ker(struct ikev2_initiator_data *data, const u8 *ker,
                             size_t ker_len) {
  u16 group;
...
static int ikev2_process_nr(struct ikev2_initiator_data *data, const u8 *nr,
                            size_t nr_len) {
  if (nr == NULL) {
  ...
  if (nr_len < IKEV2_NONCE_MIN_LEN || nr_len > IKEV2_NONCE_MAX_LEN) {
    wpa_printf(MSG_INFO, "IKEV2: Invalid Nr length %ld", (long)nr_len);
    return -1;
  }

  data->r_nonce_len = nr_len;
  os_memcpy(data->r_nonce, nr, nr_len);
  wpa_hexdump(MSG_MSGDUMP, "IKEV2: Nr", data->r_nonce, data->r_nonce_len);
...
static int ikev2_process_sa_init_encr(struct ikev2_initiator_data *data,
  ...
  decrypted = ikev2_decrypt_payload(data->proposal.encr, data->proposal.integ,
                                    &data->keys, 0, hdr, encrypted,
...
static int ikev2_process_sa_init(struct ikev2_initiator_data *data,
  ...
  if (pl->encrypted) {
    ...
    if (ikev2_process_sa_init_encr(data, hdr, pl->encrypted, pl->encrypted_len,
  ...
  if (data->IDr) {
    if (id_type != data->IDr_type || idr_len != data->IDr_len ||
      ...
      wpa_printf(MSG_INFO, "IKEV2: IDr differs from the one "
                           "received earlier");
      wpa_printf(MSG_DEBUG, "IKEV2: Previous IDr ID Type %d", id_type);
      wpa_hexdump_ascii(MSG_DEBUG, "Previous IKEV2: IDr", data->IDr,
                        data->IDr_len);
      return -1;
    }
    os_free(data->IDr);
...
static int ikev2_process_cert(struct ikev2_initiator_data *data, const u8 *cert,
                              size_t cert_len) {
  u8 cert_encoding;
...
static int ikev2_process_auth_cert(struct ikev2_initiator_data *data, u8 method,
...
static int ikev2_process_auth_secret(struct ikev2_initiator_data *data,
  ...
  if (ikev2_derive_auth_data(data->proposal.prf, data->r_sign_msg, data->IDr,
                             data->IDr_len, data->IDr_type, &data->keys, 0,
                             data->shared_secret, data->shared_secret_len,
                             data->i_nonce, data->i_nonce_len, data->key_pad,
                             data->key_pad_len, auth_data) < 0) {
  ...
  if (auth_len != prf->hash_len ||
    ...
    wpa_hexdump(MSG_DEBUG, "IKEV2: Received Authentication Data", auth,
                auth_len);
    wpa_hexdump(MSG_DEBUG, "IKEV2: Expected Authentication Data", auth_data,
                prf->hash_len);
    return -1;
  }
...
static int ikev2_process_auth(struct ikev2_initiator_data *data, const u8 *auth,
                              size_t auth_len) {
  u8 auth_method;
  ...
  case PEER_AUTH_CERT:
    return ikev2_process_auth_cert(data, auth_method, auth, auth_len);
  case PEER_AUTH_SECRET:
    return ikev2_process_auth_secret(data, auth_method, auth, auth_len);
  }
...
static int ikev2_process_sa_auth_decrypted(struct ikev2_initiator_data *data,
                                           u8 next_payload, u8 *payload,
                                           size_t payload_len) {
  struct ikev2_payloads pl;

  wpa_printf(MSG_DEBUG, "IKEV2: Processing decrypted payloads");

  if (ikev2_parse_payloads(&pl, next_payload, payload, payload + payload_len) <
      0) {
    wpa_printf(MSG_INFO, "IKEV2: Failed to parse decrypted "
                         "payloads");
    return -1;
...
static int ikev2_process_sa_auth(struct ikev2_initiator_data *data,
  ...
  ret = ikev2_process_sa_auth_decrypted(data, pl->encr_next_payload, decrypted,
                                        decrypted_len);
  os_free(decrypted);
...
int ikev2_initiator_process(struct ikev2_initiator_data *data,
  ...
  wpa_hexdump(MSG_DEBUG, "IKEV2:   IKE_SA Initiator's SPI", hdr->i_spi,
              IKEV2_SPI_LEN);
  wpa_hexdump(MSG_DEBUG, "IKEV2:   IKE_SA Initiator's SPI", hdr->r_spi,
              IKEV2_SPI_LEN);
  wpa_printf(MSG_DEBUG,
             "IKEV2:   Next Payload: %u  Version: 0x%x  "
             "Exchange Type: %u",
             hdr->next_payload, hdr->version, hdr->exchange_type);
  wpa_printf(MSG_DEBUG, "IKEV2:   Message ID: %u  Length: %u", message_id,
             length);
  ...
  if (length != wpabuf_len(buf)) {
    wpa_printf(MSG_INFO,
               ...
               "RX: %lu)",
               (unsigned long)length, (unsigned long)wpabuf_len(buf));
    return -1;
  }
  ...
  if ((hdr->flags & (IKEV2_HDR_INITIATOR | IKEV2_HDR_RESPONSE)) !=
      IKEV2_HDR_RESPONSE) {
    wpa_printf(MSG_INFO, "IKEV2: Unexpected Flags value 0x%x", hdr->flags);
    return -1;
  }
...
static int ikev2_build_ni(struct ikev2_initiator_data *data, struct wpabuf *msg,
                          u8 next_payload) {
  struct ikev2_payload_hdr *phdr;
  size_t plen;
...
static int ikev2_build_auth(struct ikev2_initiator_data *data,
  ...
  if (ikev2_derive_auth_data(data->proposal.prf, data->i_sign_msg, data->IDi,
                             data->IDi_len, ID_KEY_ID, &data->keys, 1,
                             data->shared_secret, data->shared_secret_len,
                             data->r_nonce, data->r_nonce_len, data->key_pad,
                             data->key_pad_len,
...
static struct wpabuf *ikev2_build_sa_init(struct ikev2_initiator_data *data) {
  ...
  if (os_get_random(data->i_spi, IKEV2_SPI_LEN))
    return NULL;
  wpa_hexdump(MSG_DEBUG, "IKEV2: IKE_SA Initiator's SPI", data->i_spi,
              IKEV2_SPI_LEN);
...
static struct wpabuf *ikev2_build_sa_auth(struct ikev2_initiator_data *data) {
  ...
  secret = data->get_shared_secret(data->cb_ctx, data->IDr, data->IDr_len,
                                   &secret_len);
  if (secret == NULL) {
  ...
  if (ikev2_build_idi(data, plain, IKEV2_PAYLOAD_AUTHENTICATION) ||
      ...
      ikev2_build_encrypted(data->proposal.encr, data->proposal.integ,
                            &data->keys, 1, msg, plain, IKEV2_PAYLOAD_IDi)) {
    wpabuf_free(plain);
    wpabuf_free(msg);
    return NULL;
  }
