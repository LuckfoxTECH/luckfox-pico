...
static void eap_ikev2_state(struct eap_ikev2_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-IKEV2: %s -> %s", eap_ikev2_state_txt(data->state),
...
static void *eap_ikev2_init(struct eap_sm *sm) {
  ...
  data->state = MSG;
  data->fragment_size =
      sm->fragment_size > 0 ? sm->fragment_size : IKEV2_FRAGMENT_SIZE;
...
static struct wpabuf *eap_ikev2_build_msg(struct eap_ikev2_data *data, u8 id) {
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_IKEV2, plen, EAP_CODE_REQUEST,
                      id);
  if (req == NULL)
    return NULL;
  ...
  if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
    ...
    ikev2_integ_hash(data->ikev2.proposal.integ, data->ikev2.keys.SK_ai,
                     data->ikev2.keys.SK_integ_len, msg, len,
                     wpabuf_put(req, icv_len));
  }
  ...
  if (data->out_used == wpabuf_len(data->out_buf)) {
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "(%lu more to send)",
               (unsigned long)send_len,
               (unsigned long)wpabuf_len(data->out_buf) - data->out_used);
...
static Boolean eap_ikev2_check(struct eap_sm *sm, void *priv,
  ...
  pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_IKEV2, respData, &len);
  if (pos == NULL) {
...
static int eap_ikev2_process_icv(struct eap_ikev2_data *data,
                                 const struct wpabuf *respData, u8 flags,
                                 const u8 *pos, const u8 **end, int frag_ack) {
  if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
    int icv_len = eap_ikev2_validate_icv(
        data->ikev2.proposal.integ, &data->ikev2.keys, 0, respData, pos, *end);
    if (icv_len < 0)
      return -1;
...
static int eap_ikev2_process_cont(struct eap_ikev2_data *data, const u8 *buf,
                                  size_t len) {
  ...
  wpa_printf(MSG_DEBUG,
             ...
             "bytes more",
             (unsigned long)len, (unsigned long)wpabuf_tailroom(data->in_buf));

  return 0;
}

static int eap_ikev2_process_fragment(struct eap_ikev2_data *data, u8 flags,
  ...
  if (data->in_buf == NULL) {
    ...
    if (message_length > 50000) {
      ...
      wpa_printf(MSG_DEBUG, "EAP-IKEV2: Ignore too long message");
      return -1;
    }
...
static int eap_ikev2_server_keymat(struct eap_ikev2_data *data) {
  if (eap_ikev2_derive_keymat(data->ikev2.proposal.prf, &data->ikev2.keys,
...
static void eap_ikev2_process(struct eap_sm *sm, void *priv,
  ...
  if (flags & IKEV2_FLAGS_MORE_FRAGMENTS) {
    if (eap_ikev2_process_fragment(data, flags, message_length, pos,
                                   end - pos) < 0)
      eap_ikev2_state(data, FAIL);
    else
...
static u8 *eap_ikev2_get_session_id(struct eap_sm *sm, void *priv,
                                    size_t *len) {
  struct eap_ikev2_data *data = priv;
  u8 *sid;
  size_t sid_len;
  ...
  if (sid) {
    ...
    offset++;
    os_memcpy(sid + offset, data->ikev2.i_nonce, data->ikev2.i_nonce_len);
    offset += data->ikev2.i_nonce_len;
    os_memcpy(sid + offset, data->ikev2.r_nonce, data->ikev2.r_nonce_len);
    *len = sid_len;
    wpa_hexdump(MSG_DEBUG, "EAP-IKEV2: Derived Session-Id", sid, sid_len);
  }
...
int eap_server_ikev2_register(void) {
  ...
  eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION,
                                EAP_VENDOR_IETF, EAP_TYPE_IKEV2, "IKEV2");
  if (eap == NULL)
    return -1;
