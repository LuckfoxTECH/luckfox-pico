...
struct eap_eke_data {
  enum { IDENTITY, COMMIT, CONFIRM, FAILURE_REPORT, SUCCESS, FAILURE } state;
  u8 msk[EAP_MSK_LEN];
...
static struct wpabuf *eap_eke_build_msg(struct eap_eke_data *data, u8 id,
  ...
  msg =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_EKE, plen, EAP_CODE_REQUEST, id);
  if (msg == NULL) {
...
static int supported_proposal(const u8 *pos) {
  if (pos[0] == EAP_EKE_DHGROUP_EKE_16 && pos[1] == EAP_EKE_ENCR_AES128_CBC &&
      pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
      pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
    return 1;

  if (pos[0] == EAP_EKE_DHGROUP_EKE_15 && pos[1] == EAP_EKE_ENCR_AES128_CBC &&
      pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
      pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
    return 1;

  if (pos[0] == EAP_EKE_DHGROUP_EKE_14 && pos[1] == EAP_EKE_ENCR_AES128_CBC &&
      pos[2] == EAP_EKE_PRF_HMAC_SHA2_256 &&
      pos[3] == EAP_EKE_MAC_HMAC_SHA2_256)
    return 1;

  if (pos[0] == EAP_EKE_DHGROUP_EKE_14 && pos[1] == EAP_EKE_ENCR_AES128_CBC &&
      pos[2] == EAP_EKE_PRF_HMAC_SHA1 && pos[3] == EAP_EKE_MAC_HMAC_SHA1)
    return 1;
...
static struct wpabuf *eap_eke_build_commit(struct eap_sm *sm,
  ...
  if (eap_eke_derive_key(&data->sess, sm->user->password,
                         sm->user->password_len, sm->server_id,
                         sm->server_id_len, data->peerid, data->peerid_len,
                         data->key) < 0) {
  ...
  if (eap_eke_dhcomp(&data->sess, data->key, pub,
                     wpabuf_put(msg, data->sess.dhcomp_len)) < 0) {
...
static struct wpabuf *eap_eke_build_confirm(struct eap_sm *sm,
  ...
  if (eap_eke_derive_ka(&data->sess, sm->server_id, sm->server_id_len,
...
static Boolean eap_eke_check(struct eap_sm *sm, void *priv,
  ...
  wpa_printf(MSG_INFO, "EAP-EKE: Unexpected EKE-Exch=%d in state=%d", eke_exch,
             data->state);
...
static void eap_eke_process_identity(struct eap_sm *sm,
  ...
  if (!supported_proposal(pos)) {
    wpa_printf(MSG_INFO, "EAP-EKE: Unexpected Proposal (%u:%u:%u:%u)", pos[0],
               pos[1], pos[2], pos[3]);
    eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
    return;
  }

  wpa_printf(MSG_DEBUG, "EAP-EKE: Selected Proposal (%u:%u:%u:%u)", pos[0],
             pos[1], pos[2], pos[3]);
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "EAP-EKE: Peer Identity", data->peerid,
                    data->peerid_len);
...
static void eap_eke_process_commit(struct eap_sm *sm, struct eap_eke_data *data,
                                   const struct wpabuf *respData,
                                   const u8 *payload, size_t payloadlen) {
  ...
  if (eap_eke_derive_ke_ki(&data->sess, sm->server_id, sm->server_id_len,
...
static void eap_eke_process_confirm(struct eap_sm *sm,
  ...
  if (eap_eke_decrypt_prot(&data->sess, payload, data->sess.pnonce_len, nonce,
                           &decrypt_len) < 0) {
  ...
  wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Received Nonce_S", nonce,
                  data->sess.nonce_len);
