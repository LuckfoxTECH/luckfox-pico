...
static void eap_tls_state(struct eap_tls_data *data, int state) {
  wpa_printf(MSG_DEBUG, "EAP-TLS: %s -> %s", eap_tls_state_txt(data->state),
...
static void eap_tls_valid_session(struct eap_sm *sm,
...
static void *eap_wfa_unauth_tls_init(struct eap_sm *sm) {
  ...
  if (eap_server_tls_ssl_init(sm, &data->ssl, 0, EAP_WFA_UNAUTH_TLS_TYPE)) {
    wpa_printf(MSG_INFO, "EAP-TLS: Failed to initialize SSL.");
    eap_tls_reset(sm, data);
    return NULL;
  }
...
static struct wpabuf *eap_tls_buildReq(struct eap_sm *sm, void *priv, u8 id) {
  ...
  case CONTINUE:
    ...
    break;
  default:
    wpa_printf(MSG_DEBUG, "EAP-TLS: %s - unexpected state %d", __func__,
               data->state);
    return NULL;
  }
...
static Boolean eap_tls_check(struct eap_sm *sm, void *priv,
  ...
  if (data->eap_type == EAP_UNAUTH_TLS_TYPE)
    ...
    pos = eap_hdr_validate(EAP_VENDOR_IETF, data->eap_type, respData, &len);
  if (pos == NULL || len < 1) {
...
static void eap_tls_process(struct eap_sm *sm, void *priv,
  ...
  if (eap_server_tls_process(sm, &data->ssl, respData, data, data->eap_type,
                             NULL, eap_tls_process_msg) < 0) {
    eap_tls_state(data, FAILURE);
    return;
  }
  ...
  wpa_printf(MSG_DEBUG, "EAP-TLS: Resuming previous session");
  eap_tls_state(data, SUCCESS);
...
static u8 *eap_tls_getKey(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  if (eapKeyData) {
    *len = EAP_TLS_KEY_LEN;
    wpa_hexdump(MSG_DEBUG, "EAP-TLS: Derived key", eapKeyData, EAP_TLS_KEY_LEN);
  } else {
...
static u8 *eap_tls_get_emsk(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  eapKeyData = eap_server_tls_derive_key(
      sm, &data->ssl, "client EAP encryption", EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
  if (eapKeyData) {
  ...
  if (emsk) {
    *len = EAP_EMSK_LEN;
    wpa_hexdump(MSG_DEBUG, "EAP-TLS: Derived EMSK", emsk, EAP_EMSK_LEN);
  } else {
...
static u8 *eap_tls_get_session_id(struct eap_sm *sm, void *priv, size_t *len) {
  ...
  return eap_server_tls_derive_session_id(sm, &data->ssl, EAP_TYPE_TLS, len);
}
...
int eap_server_wfa_unauth_tls_register(void) {
  ...
  eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION,
                                EAP_VENDOR_WFA_NEW, EAP_VENDOR_WFA_UNAUTH_TLS,
                                "WFA-UNAUTH-TLS");
  if (eap == NULL)
    return -1;
