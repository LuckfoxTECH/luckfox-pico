...
struct tnc_if_imv {
  ...
  TNC_Result (*Initialize)(TNC_IMVID imvID, TNC_Version minVersion,
                           TNC_Version maxVersion,
                           TNC_Version *pOutActualVersion);
  TNC_Result (*NotifyConnectionChange)(TNC_IMVID imvID,
                                       TNC_ConnectionID connectionID,
                                       TNC_ConnectionState newState);
  TNC_Result (*ReceiveMessage)(TNC_IMVID imvID, TNC_ConnectionID connectionID,
                               TNC_BufferReference message,
                               TNC_UInt32 messageLength,
                               TNC_MessageType messageType);
  TNC_Result (*SolicitRecommendation)(TNC_IMVID imvID,
                                      TNC_ConnectionID connectionID);
  TNC_Result (*BatchEnding)(TNC_IMVID imvID, TNC_ConnectionID connectionID);
  TNC_Result (*Terminate)(TNC_IMVID imvID);
  TNC_Result (*ProvideBindFunction)(TNC_IMVID imvID,
...
static TNC_Result
TNC_TNCS_ReportMessageTypes(TNC_IMVID imvID, TNC_MessageTypeList supportedTypes,
  ...
  for (i = 0; i < typeCount; i++) {
    wpa_printf(MSG_DEBUG, "TNC: supportedTypes[%lu] = %lu", i,
               supportedTypes[i]);
  }
...
static TNC_Result TNC_TNCS_SendMessage(TNC_IMVID imvID,
                                       TNC_ConnectionID connectionID,
                                       TNC_BufferReference message,
                                       TNC_UInt32 messageLength,
                                       TNC_MessageType messageType) {
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "TNC: TNC_TNCS_SendMessage", message,
                    messageLength);
  ...
  tncs->imv_data[imvID].imv_send_len =
      os_snprintf((char *)tncs->imv_data[imvID].imv_send, b64len + 100,
...
static TNC_Result TNC_TNCS_RequestHandshakeRetry(TNC_IMVID imvID,
                                                 TNC_ConnectionID connectionID,
...
static TNC_Result
TNC_TNCS_ProvideRecommendation(TNC_IMVID imvID, TNC_ConnectionID connectionID,
...
static TNC_Result
TNC_TNCS_GetAttribute(TNC_IMVID imvID, TNC_ConnectionID connectionID,
                      TNC_AttributeID attribureID, TNC_UInt32 bufferLength,
                      TNC_BufferReference buffer, TNC_UInt32 *pOutValueLength) {
  wpa_printf(MSG_DEBUG, "TNC: TNC_TNCS_GetAttribute");
  /* TODO */
  return TNC_RESULT_SUCCESS;
}

static TNC_Result TNC_TNCS_SetAttribute(TNC_IMVID imvID,
                                        TNC_ConnectionID connectionID,
...
static TNC_Result TNC_TNCS_BindFunction(TNC_IMVID imvID, char *functionName,
                                        void **pOutFunctionPointer) {
  wpa_printf(MSG_DEBUG,
             "TNC: TNC_TNCS_BindFunction(imcID=%lu, "
             "functionName='%s')",
             (unsigned long)imvID, functionName);
...
static int tncs_imv_resolve_funcs(struct tnc_if_imv *imv) {
  ...
  imv->SolicitRecommendation =
      tncs_get_sym(handle, "TNC_IMV_SolicitRecommendation");
...
static int tncs_imv_initialize(struct tnc_if_imv *imv) {
  ...
  wpa_printf(MSG_DEBUG, "TNC: Calling TNC_IMV_Initialize for IMV '%s'",
             imv->name);
  res = imv->Initialize(imv->imvID, TNC_IFIMV_VERSION_1, TNC_IFIMV_VERSION_1,
                        &imv_ver);
...
static int tncs_load_imv(struct tnc_if_imv *imv) {
  ...
  if (imv->dlhandle == NULL) {
    wpa_printf(MSG_ERROR, "TNC: Failed to open IMV '%s' (%s): %s", imv->name,
               imv->path, dlerror());
    return -1;
  }
...
static void tncs_send_to_imvs(struct tncs_data *tncs, unsigned int type,
  ...
  for (imv = tncs->imv; imv; imv = imv->next) {
    ...
    res = imv->ReceiveMessage(imv->imvID, tncs->connectionID,
                              (TNC_BufferReference)msg, len, type);
...
static enum tncs_process_res
tncs_derive_recommendation(struct tncs_data *tncs) {
  ...
  *end = '\0';
  ...
  while (*start) {
    ...
    *end = '\0';
    ...
    if (decoded) {
      wpa_hexdump_ascii(MSG_MSGDUMP, "TNC: TNCC-TNCS-Message Base64", decoded,
                        decoded_len);
      os_free(decoded);
    }

    if (xml) {
      wpa_hexdump_ascii(MSG_MSGDUMP, "TNC: TNCC-TNCS-Message XML",
                        (unsigned char *)xml, xmlend - xml);
    }
