...
static void eap_tnc_set_state(struct eap_tnc_data *data,
  ...
  wpa_printf(MSG_DEBUG, "EAP-TNC: %s -> %s", eap_tnc_state_txt(data->state),
...
static void *eap_tnc_init(struct eap_sm *sm) {
  ...
  data->fragment_size = sm->fragment_size > 100 ? sm->fragment_size - 98 : 1300;
...
static struct wpabuf *eap_tnc_build_start(struct eap_sm *sm,
  ...
  req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TNC, 1, EAP_CODE_REQUEST, id);
  if (req == NULL) {
...
static struct wpabuf *eap_tnc_build(struct eap_sm *sm,
  ...
  wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-TNC: Request", wpabuf_head(req),
                    wpabuf_len(req));
...
static struct wpabuf *eap_tnc_build_msg(struct eap_tnc_data *data, u8 id) {
  ...
  if (flags & EAP_TNC_FLAGS_LENGTH_INCLUDED)
    plen += 4;
  req =
      eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TNC, plen, EAP_CODE_REQUEST, id);
  if (req == NULL)
    return NULL;
...
static void tncs_process(struct eap_tnc_data *data, struct wpabuf *inbuf) {
  ...
  res =
      tncs_process_if_tnccs(data->tncs, wpabuf_head(inbuf), wpabuf_len(inbuf));
  switch (res) {
...
static int eap_tnc_process_cont(struct eap_tnc_data *data, const u8 *buf,
                                size_t len) {
...
static int eap_tnc_process_fragment(struct eap_tnc_data *data, u8 flags,
...
static void eap_tnc_process(struct eap_sm *sm, void *priv,
  ...
  if (flags & EAP_TNC_FLAGS_LENGTH_INCLUDED) {
    ...
    if (message_length < (u32)(end - pos) || message_length > 75000) {
      wpa_printf(MSG_DEBUG,
  ...
  if (flags & EAP_TNC_FLAGS_MORE_FRAGMENTS) {
    if (eap_tnc_process_fragment(data, flags, message_length, pos, end - pos) <
        0)
      eap_tnc_set_state(data, FAIL);
    else
