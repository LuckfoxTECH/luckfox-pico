...
static void fst_ctrl_iface_notify_peer_state_change(struct fst_iface *iface,
  ...
  foreach_fst_ctrl_call(on_event, EVENT_PEER_STATE_CHANGED, iface, NULL,
                        &extra);
}
...
struct fst_iface *fst_attach(const char *ifname, const u8 *own_addr,
  ...
  if (!group) {
    ...
    if (!group) {
      fst_printf(MSG_ERROR, "%s: FST group cannot be created", cfg->group_id);
      return NULL;
    }
    new_group = TRUE;
  }
  ...
  if (!iface) {
    fst_printf_group(group, MSG_ERROR, "cannot create iface for %s", ifname);
    if (new_group)
...
void fst_global_deinit(void) {
  ...
  while ((group = fst_first_group()) != NULL)
    fst_group_delete(group);
  while ((h = dl_list_first(&fst_global_ctrls_list, struct fst_ctrl_handle,
                            global_ctrls_lentry)))
...
void fst_rx_action(struct fst_iface *iface, const struct ieee80211_mgmt *mgmt,
  ...
  if (fst_iface_is_connected(iface, mgmt->sa, FALSE))
    ...
    wpa_printf(MSG_DEBUG,
               "FST: Ignore FST Action frame - no FST connection with " MACSTR,
               MAC2STR(mgmt->sa));
}
...
void fst_notify_peer_connected(struct fst_iface *iface, const u8 *addr) {
  ...
  fst_printf_iface(iface, MSG_DEBUG, MACSTR " became connected", MAC2STR(addr));
