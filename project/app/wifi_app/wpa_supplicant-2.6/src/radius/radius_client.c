...
static int radius_client_handle_send_error(struct radius_client_data *radius,
  ...
  if (_errno == ENOTCONN || _errno == EDESTADDRREQ || _errno == EINVAL ||
    ...
    hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
...
static int radius_client_retransmit(struct radius_client_data *radius,
  ...
  if (entry->msg_type == RADIUS_ACCT_INTERIM) {
    wpa_printf(MSG_DEBUG,
               "RADIUS: Failed to transmit interim accounting update to " MACSTR
               " - drop message and request a new update",
               MAC2STR(entry->addr));
  ...
  if (s < 0) {
    wpa_printf(MSG_INFO, "RADIUS: No valid socket for retransmission");
    return 1;
  }
...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx) {
  ...
  while (entry) {
    ...
    s = entry->msg_type == RADIUS_AUTH ? radius->auth_sock : radius->acct_sock;
...
static void radius_client_auth_failover(struct radius_client_data *radius) {
  ...
  hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_NOTICE,
...
static void radius_client_update_timeout(struct radius_client_data *radius) {
  ...
  hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
...
static void radius_client_list_add(struct radius_client_data *radius,
  ...
  if (radius->num_msgs >= RADIUS_CLIENT_MAX_ENTRIES) {
    wpa_printf(MSG_INFO, "RADIUS: Removing the oldest un-ACKed packet due to "
...
int radius_client_send(struct radius_client_data *radius,
  ...
  if (msg_type == RADIUS_ACCT || msg_type == RADIUS_ACCT_INTERIM) {
    ...
    if (conf->acct_server == NULL || radius->acct_sock < 0 ||
      ...
      hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS,
                     HOSTAPD_LEVEL_INFO, "No accounting server configured");
      return -1;
    }
    ...
    if (conf->auth_server == NULL || radius->auth_sock < 0 ||
      ...
      hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS,
                     HOSTAPD_LEVEL_INFO, "No authentication server configured");
      return -1;
    }
...
static int radius_change_server(struct radius_client_data *radius,
  ...
  hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
                 "%s server %s:%d", auth ? "Authentication" : "Accounting",
  ...
  if (conf->force_client_addr) {
    ...
    if (bind(sel_sock, cl_addr, claddrlen) < 0) {
      wpa_printf(MSG_INFO, "bind[radius]: %s", strerror(errno));
      return -1;
    }
  }
...
void radius_client_flush_auth(struct radius_client_data *radius,
  ...
  while (entry) {
    if (entry->msg_type == RADIUS_AUTH &&
      ...
      hostapd_logger(radius->ctx, addr, HOSTAPD_MODULE_RADIUS,
                     HOSTAPD_LEVEL_DEBUG,
...
static int radius_client_dump_auth_server(char *buf, size_t buflen,
  ...
  return os_snprintf(
      ...
      serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)), serv->port,
      serv->round_trip_time, serv->requests, serv->retransmissions,
      serv->access_accepts, serv->access_rejects, serv->access_challenges,
...
static int radius_client_dump_acct_server(char *buf, size_t buflen,
  ...
  return os_snprintf(
      ...
      serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)), serv->port,
      serv->round_trip_time, serv->requests, serv->retransmissions,
      serv->responses, serv->malformed_responses, serv->bad_authenticators,
      pending, serv->timeouts, serv->unknown_types, serv->packets_dropped);
}
