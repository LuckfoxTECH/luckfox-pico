...
static void view_cb_notify_progress(WebKitWebView *view, GParamSpec *pspec,
  ...
  wpa_printf(MSG_DEBUG, "BROWSER:%s progress=%d", __func__, ctx->progress);
...
static void view_cb_notify_load_status(WebKitWebView *view, GParamSpec *pspec,
  ...
  wpa_printf(MSG_DEBUG, "BROWSER:%s load-status=%d uri=%s", __func__, status,
...
static gboolean view_cb_mime_type_policy_decision(
    ...
    gchar *mime, WebKitWebPolicyDecision *policy, struct browser_context *ctx) {
...
static void view_cb_hovering_over_link(WebKitWebView *view, gchar *title,
                                       gchar *uri,
                                       struct browser_context *ctx) {
  wpa_printf(MSG_DEBUG, "BROWSER:%s title=%s uri=%s", __func__, title, uri);
...
int hs20_web_browser(const char *url) {
  ...
  g_object_set(G_OBJECT(s), "ssl-ca-file", "/etc/ssl/certs/ca-certificates.crt",
               NULL);
  ...
  g_signal_connect(G_OBJECT(ctx.win), "destroy", G_CALLBACK(win_cb_destroy),
                   &ctx);
