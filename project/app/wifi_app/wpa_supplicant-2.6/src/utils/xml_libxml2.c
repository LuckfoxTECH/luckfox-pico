...
int xml_validate(struct xml_node_ctx *ctx, xml_node_t *node,
  ...
  xmlSchemaSetParserErrors(pctx, (xmlSchemaValidityErrorFunc)add_str,
                           (xmlSchemaValidityWarningFunc)add_str, &errors);
...
xml_node_t *xml_node_create_root(struct xml_node_ctx *ctx, const char *ns_uri,
  ...
  if (node == NULL)
    return NULL;
  if (ns_uri) {
    ns = xmlNewNs(node, (const xmlChar *)ns_uri, (const xmlChar *)ns_prefix);
    xmlSetNs(node, ns);
  }
...
xml_node_t *xml_node_create(struct xml_node_ctx *ctx, xml_node_t *parent,
  ...
  xmlNodePtr node;
  node = xmlNewChild((xmlNodePtr)parent, (xmlNsPtr)ns, (const xmlChar *)name,
                     NULL);
  return (xml_node_t *)node;
}

xml_node_t *xml_node_create_text(struct xml_node_ctx *ctx, xml_node_t *parent,
                                 xml_namespace_t *ns, const char *name,
                                 const char *value) {
  xmlNodePtr node;
...
xml_node_t *xml_node_create_text_ns(struct xml_node_ctx *ctx,
  ...
  node = xmlNewTextChild((xmlNodePtr)parent, NULL, (const xmlChar *)name,
                         (const xmlChar *)value);
...
int xml_node_add_attr(struct xml_node_ctx *ctx, xml_node_t *node,
  ...
  if (ns) {
    attr = xmlNewNsProp((xmlNodePtr)node, (xmlNsPtr)ns, (const xmlChar *)name,
                        (const xmlChar *)value);
  } else {
...
char *xml_node_get_attr_value(struct xml_node_ctx *ctx, xml_node_t *node,
                              char *name) {
  return (char *)xmlGetNoNsProp((xmlNodePtr)node, (const xmlChar *)name);
}
...
struct xml_node_ctx *xml_node_init_ctx(void *upper_ctx, const void *env) {
