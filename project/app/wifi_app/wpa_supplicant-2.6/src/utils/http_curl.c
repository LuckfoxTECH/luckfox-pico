...
struct http_ctx {
  ...
  enum { NO_OCSP, OPTIONAL_OCSP, MANDATORY_OCSP } ocsp;
  X509 *peer_cert;
  X509 *peer_issuer;
...
static void debug_dump(struct http_ctx *ctx, const char *title, const char *buf,
                       size_t len) {
  char *txt;
  size_t i;
...
ASN1_SEQUENCE(HashAlgAndValue) = {
    ...
    ASN1_SIMPLE(HashAlgAndValue, hashValue,
                ASN1_OCTET_STRING)} ASN1_SEQUENCE_END(HashAlgAndValue);

ASN1_SEQUENCE(LogotypeReference) = {
    ASN1_SEQUENCE_OF(LogotypeReference, refStructHash, HashAlgAndValue),
    ASN1_SEQUENCE_OF(LogotypeReference, refStructURI,
                     ASN1_IA5STRING)} ASN1_SEQUENCE_END(LogotypeReference);
...
ASN1_SEQUENCE(LogotypeDetails) = {
    ...
    ASN1_SEQUENCE_OF(LogotypeDetails, logotypeURI,
                     ASN1_IA5STRING)} ASN1_SEQUENCE_END(LogotypeDetails);

ASN1_CHOICE(LogotypeImageResolution) = {
    ASN1_IMP(LogotypeImageResolution, d.numBits, ASN1_INTEGER, 1),
    ASN1_IMP(LogotypeImageResolution, d.tableSize, ASN1_INTEGER,
             2)} ASN1_CHOICE_END(LogotypeImageResolution);
...
ASN1_SEQUENCE(LogotypeImage) = {
    ...
    ASN1_OPT(LogotypeImage, imageInfo,
             LogotypeImageInfo)} ASN1_SEQUENCE_END(LogotypeImage);
...
ASN1_SEQUENCE(LogotypeAudioInfo) = {
    ...
    ASN1_IMP_OPT(LogotypeAudioInfo, language, ASN1_IA5STRING,
                 4)} ASN1_SEQUENCE_END(LogotypeAudioInfo);

ASN1_SEQUENCE(LogotypeAudio) = {
    ASN1_SIMPLE(LogotypeAudio, audioDetails, LogotypeDetails),
    ASN1_OPT(LogotypeAudio, audioInfo,
             LogotypeAudioInfo)} ASN1_SEQUENCE_END(LogotypeAudio);

ASN1_SEQUENCE(LogotypeData) = {
    ASN1_SEQUENCE_OF_OPT(LogotypeData, image, LogotypeImage),
    ASN1_IMP_SEQUENCE_OF_OPT(LogotypeData, audio, LogotypeAudio,
                             1)} ASN1_SEQUENCE_END(LogotypeData);

ASN1_CHOICE(LogotypeInfo) = {ASN1_IMP(LogotypeInfo, d.direct, LogotypeData, 0),
                             ASN1_IMP(LogotypeInfo, d.indirect,
                                      LogotypeReference,
                                      1)} ASN1_CHOICE_END(LogotypeInfo);

ASN1_SEQUENCE(OtherLogotypeInfo) = {
    ASN1_SIMPLE(OtherLogotypeInfo, logotypeType, ASN1_OBJECT),
    ASN1_SIMPLE(OtherLogotypeInfo, info,
                LogotypeInfo)} ASN1_SEQUENCE_END(OtherLogotypeInfo);
...
ASN1_SEQUENCE(LogotypeExtn) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(LogotypeExtn, otherLogos, OtherLogotypeInfo,
                             3)} ASN1_SEQUENCE_END(LogotypeExtn);
...
#define sk_LogotypeInfo_value(st, i)                                           \
  (LogotypeInfo *)sk_value(                                                    \
      CHECKED_CAST(_STACK *, const STACK_OF(LogotypeInfo) *, (st)), (i))
#define sk_LogotypeImage_num(st)                                               \
  sk_num(CHECKED_CAST(_STACK *, STACK_OF(LogotypeImage) *, (st)))
#define sk_LogotypeImage_value(st, i)                                          \
  (LogotypeImage *)sk_value(                                                   \
      CHECKED_CAST(_STACK *, const STACK_OF(LogotypeImage) *, (st)), (i))
#define sk_LogotypeAudio_num(st)                                               \
  sk_num(CHECKED_CAST(_STACK *, STACK_OF(LogotypeAudio) *, (st)))
#define sk_LogotypeAudio_value(st, i)                                          \
  (LogotypeAudio *)sk_value(                                                   \
      CHECK_CAST(_STACK *, const STACK_OF(LogotypeAudio) *, (st)), (i))
#define sk_HashAlgAndValue_num(st)                                             \
  sk_num(CHECKED_CAST(_STACK *, STACK_OF(HashAlgAndValue) *, (st)))
#define sk_HashAlgAndValue_value(st, i)                                        \
  (HashAlgAndValue *)sk_value(                                                 \
      CHECKED_CAST(_STACK *, const STACK_OF(HashAlgAndValue) *, (st)), (i))
#define sk_ASN1_IA5STRING_num(st)                                              \
  sk_num(CHECKED_CAST(_STACK *, STACK_OF(ASN1_IA5STRING) *, (st)))
#define sk_ASN1_IA5STRING_value(st, i)                                         \
  (ASN1_IA5STRING *)sk_value(                                                  \
      CHECKED_CAST(_STACK *, const STACK_OF(ASN1_IA5STRING) *, (st)), (i))
...
static void add_logo_indirect(struct http_ctx *ctx, struct http_cert *hcert,
  ...
  if (hash_num != uri_num) {
    wpa_printf(MSG_INFO,
               "Unexpected LogotypeReference array size difference %d != %d",
               hash_num, uri_num);
    return;
  }
...
static void i2r_LogotypeDetails(LogotypeDetails *details, BIO *out,
                                int indent) {
  int i, num;
  ...
  num =
      details->logotypeHash ? sk_HashAlgAndValue_num(details->logotypeHash) : 0;
  for (i = 0; i < num; i++) {
    HashAlgAndValue *hash;
...
static void i2r_LogotypeImageInfo(LogotypeImageInfo *info, BIO *out,
                                  int indent) {
  long val;
...
static void i2r_LogotypeReference(LogotypeReference *ref, const char *title,
  ...
  hash_num =
      ref->refStructHash ? sk_HashAlgAndValue_num(ref->refStructHash) : 0;
...
static void add_logotype_ext(struct http_ctx *ctx, struct http_cert *hcert,
  ...
  wpa_hexdump(MSG_DEBUG, "logotypeExtn", ASN1_STRING_data(os),
  ...
  for (i = 0; i < hcert.num_othername; i++) {
    if (os_strcmp(hcert.othername[i].oid, "1.3.6.1.4.1.40808.1.1.1") == 0) {
      ...
      wpa_hexdump_ascii(MSG_INFO, "id-wfa-hotspot-friendlyName",
                        hcert.othername[i].data, hcert.othername[i].len);
    } else {
      wpa_printf(MSG_INFO, "subjAltName[othername]: oid=%s",
                 hcert.othername[i].oid);
      wpa_hexdump_ascii(MSG_INFO, "unknown othername", hcert.othername[i].data,
...
static int curl_cb_ssl_verify(int preverify_ok, X509_STORE_CTX *x509_ctx) {
  ...
  wpa_printf(MSG_INFO,
             "Server certificate chain - depth=%d err=%d (%s) subject=%s",
  ...
  if (depth == 0 && ctx->ocsp != NO_OCSP && preverify_ok) {
    ...
    if (res == OCSP_REVOKED) {
      ...
      preverify_ok = 0;
      wpa_printf(MSG_INFO, "OCSP: bad certificate status response");
    }
  }
#endif /* OPENSSL_IS_BORINGSSL */
...
static void tls_show_errors(const char *func, const char *txt) {
  ...
  wpa_printf(MSG_DEBUG, "OpenSSL: %s - %s %s", func, txt,
...
static int ocsp_resp_cb(SSL *s, void *arg) {
  ...
  if (ctx->peer_issuer) {
    ...
    if (certs) {
      ...
      if (certs && ctx->peer_issuer_issuer) {
        ...
        if (cert && !sk_X509_push(certs, cert)) {
          tls_show_errors(__func__, "OpenSSL: Could not add issuer's issuer to "
                                    "OCSP responder trust store");
          X509_free(cert);
        }
      }
    }
  ...
  if (!OCSP_resp_find_status(basic, id, &status, &reason, &produced_at,
    ...
    wpa_printf(MSG_INFO,
               ...
               "response%s",
               (ctx->ocsp == MANDATORY_OCSP) ? "" : " (OCSP not required)");
    OCSP_CERTID_free(id);
    OCSP_BASICRESP_free(basic);
    OCSP_RESPONSE_free(rsp);
    if (ctx->ocsp == MANDATORY_OCSP)
      ...
      ctx->last_err =
          "Could not find current server certificate from OCSP response";
    return (ctx->ocsp == MANDATORY_OCSP) ? 0 : 1;
  }
  OCSP_CERTID_free(id);
...
static CURL *setup_curl_post(struct http_ctx *ctx, const char *address,
  ...
  wpa_printf(MSG_DEBUG,
             "Start HTTP client: address=%s ca_fname=%s "
             "username=%s%s",
             address, ca_fname, username, extra);
...
int soap_init_client(struct http_ctx *ctx, const char *address,
                     ...
                     const char *password, const char *client_cert,
                     const char *client_key) {
  if (post_init_client(ctx, address, ca_fname, username, password, client_cert,
                       client_key) < 0)
    return -1;

  ctx->curl_hdr =
      curl_slist_append(ctx->curl_hdr, "Content-Type: application/soap+xml");
...
int http_download_file(struct http_ctx *ctx, const char *url, const char *fname,
                       const char *ca_fname) {
  CURL *curl;
  FILE *f;
  CURLcode res;
  ...
  wpa_printf(MSG_DEBUG, "curl: Download file from %s to %s (ca=%s)", url, fname,
             ca_fname);
  curl = curl_easy_init();
  if (curl == NULL)
    return -1;
...
char *http_post(struct http_ctx *ctx, const char *url, const char *data,
                ...
                const char *ca_fname, const char *username,
                const char *password, const char *client_cert,
                const char *client_key, size_t *resp_len) {
  long http = 0;
  CURLcode res;
  char *ret;
  CURL *curl;
  ...
  curl = setup_curl_post(ctx, url, ca_fname, username, password, client_cert,
                         client_key);
  if (curl == NULL)
    return NULL;
...
const char *http_get_err(struct http_ctx *ctx) { return ctx->last_err; }
