...
static WINSCARDAPI LONG WINAPI (*dll_SCardEstablishContext)(
    IN DWORD dwScope, IN LPCVOID pvReserved1, IN LPCVOID pvReserved2,
...
static WINSCARDAPI LONG WINAPI (*dll_SCardListReadersA)(
    IN SCARDCONTEXT hContext, IN LPCSTR mszGroups, OUT LPSTR mszReaders,
    IN OUT LPDWORD pcchReaders);
#undef SCardListReaders
#define SCardListReaders dll_SCardListReadersA

static WINSCARDAPI LONG WINAPI (*dll_SCardConnectA)(
    IN SCARDCONTEXT hContext, IN LPCSTR szReader, IN DWORD dwShareMode,
    IN DWORD dwPreferredProtocols, OUT LPSCARDHANDLE phCard,
    OUT LPDWORD pdwActiveProtocol);
#undef SCardConnect
#define SCardConnect dll_SCardConnectA

static WINSCARDAPI LONG WINAPI (*dll_SCardDisconnect)(IN SCARDHANDLE hCard,
                                                      IN DWORD dwDisposition);
#define SCardDisconnect dll_SCardDisconnect

static WINSCARDAPI LONG WINAPI (*dll_SCardTransmit)(
    IN SCARDHANDLE hCard, IN LPCSCARD_IO_REQUEST pioSendPci,
    IN LPCBYTE pbSendBuffer, IN DWORD cbSendLength,
    IN OUT LPSCARD_IO_REQUEST pioRecvPci, OUT LPBYTE pbRecvBuffer,
    IN OUT LPDWORD pcbRecvLength);
#define SCardTransmit dll_SCardTransmit

static WINSCARDAPI
    LONG WINAPI (*dll_SCardBeginTransaction)(IN SCARDHANDLE hCard);
#define SCardBeginTransaction dll_SCardBeginTransaction

static WINSCARDAPI
    LONG WINAPI (*dll_SCardEndTransaction)(IN SCARDHANDLE hCard,
                                           IN DWORD dwDisposition);
...
static int scard_get_record_len(struct scard_data *scard, unsigned char recnum,
                                unsigned char mode);
static int scard_read_record(struct scard_data *scard, unsigned char *data,
                             size_t len, unsigned char recnum,
                             unsigned char mode);

static int scard_parse_fsp_templ(unsigned char *buf, size_t buf_len, int *ps_do,
                                 int *file_len) {
  ...
  pos++;
  wpa_hexdump(MSG_DEBUG, "SCARD: file header FSP template", pos, end - pos);
  ...
  while (end - pos >= 2) {
    ...
    type = pos[0];
    len = pos[1];
    wpa_printf(MSG_MSGDUMP, "SCARD: file header TLV 0x%02x len=%d", type, len);
    pos += 2;
    ...
    case USIM_TLV_FILE_DESC:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: File Descriptor TLV", pos, len);
      break;
    case USIM_TLV_FILE_ID:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: File Identifier TLV", pos, len);
      break;
    case USIM_TLV_DF_NAME:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: DF name (AID) TLV", pos, len);
      break;
    ...
    case USIM_TLV_LIFE_CYCLE_STATUS:
      wpa_hexdump(MSG_MSGDUMP,
                  "SCARD: Life Cycle Status "
                  "Integer TLV",
                  pos, len);
      break;
    case USIM_TLV_FILE_SIZE:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: File size TLV", pos, len);
      if ((len == 1 || len == 2) && file_len) {
        if (len == 1)
          *file_len = (int)pos[0];
        else
          *file_len = WPA_GET_BE16(pos);
        wpa_printf(MSG_DEBUG, "SCARD: file_size=%d", *file_len);
      }
      break;
    case USIM_TLV_TOTAL_FILE_SIZE:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: Total file size TLV", pos, len);
      break;
    case USIM_TLV_PIN_STATUS_TEMPLATE:
      wpa_hexdump(MSG_MSGDUMP,
                  "SCARD: PIN Status Template "
                  "DO TLV",
                  pos, len);
      if (len >= 2 && pos[0] == USIM_PS_DO_TAG && pos[1] >= 1 && ps_do) {
        wpa_printf(MSG_DEBUG, "SCARD: PS_DO=0x%02x", pos[2]);
        *ps_do = (int)pos[2];
      }
      break;
    ...
    case USIM_TLV_SECURITY_ATTR_AB:
      wpa_hexdump(MSG_MSGDUMP,
                  "SCARD: Security attribute "
                  "TLV",
                  pos, len);
      break;
    default:
      wpa_hexdump(MSG_MSGDUMP, "SCARD: Unrecognized TLV", pos, len);
      break;
    }
...
static int scard_pin_needed(struct scard_data *scard, unsigned char *hdr,
                            size_t hlen) {
...
static int scard_get_aid(struct scard_data *scard, unsigned char *aid,
  ...
  for (rec = 1; rec < 10; rec++) {
    rlen = scard_get_record_len(scard, rec, SIM_RECORD_MODE_ABSOLUTE);
    if (rlen < 0) {
    ...
    if (scard_read_record(scard, buf, rlen, rec, SIM_RECORD_MODE_ABSOLUTE) <
        0) {
      wpa_printf(MSG_DEBUG,
    ...
    if (aid_len < 1 || aid_len > 16) {
      wpa_printf(MSG_DEBUG, "SCARD: Invalid AID length %u", aid_len);
      continue;
    }

    wpa_hexdump(MSG_DEBUG, "SCARD: AID from EF_DIR record", aid_pos, aid_len);
...
struct scard_data *scard_init(const char *reader) {
  ...
  ret = SCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &scard->ctx);
  ...
  ret = SCardConnect(scard->ctx, &readers[pos], SCARD_SHARE_SHARED,
                     SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &scard->card,
                     &scard->protocol);
  if (ret != SCARD_S_SUCCESS) {
  ...
  if (_scard_select_file(scard, SCARD_FILE_MF, buf, &blen, SCARD_USIM, NULL,
                         0)) {
    wpa_printf(MSG_DEBUG,
               "SCARD: USIM is not supported. Trying to use GSM SIM");
  ...
  if (scard->sim_type == SCARD_GSM_SIM) {
    ...
    blen = sizeof(buf);
    if (_scard_select_file(scard, 0, buf, &blen, scard->sim_type, aid,
                           aid_len)) {
  ...
  if (pin_needed) {
    ...
    wpa_printf(MSG_DEBUG,
               "PIN1 needed for SIM access (retry "
               "counter=%d)",
               scard_get_pin_retry_counter(scard));
  }
...
static long scard_transmit(struct scard_data *scard, unsigned char *_send,
                           size_t send_len, unsigned char *_recv,
                           size_t *recv_len) {
  long ret;
#ifdef __APPLE__
  uint32_t rlen;
  ...
  wpa_hexdump_key(MSG_DEBUG, "SCARD: scard_transmit: send", _send, send_len);
  rlen = *recv_len;
  ret = SCardTransmit(scard->card,
                      scard->protocol == SCARD_PROTOCOL_T1 ? SCARD_PCI_T1
                                                           : SCARD_PCI_T0,
                      _send, (unsigned long)send_len, NULL, _recv, &rlen);
  *recv_len = rlen;
  if (ret == SCARD_S_SUCCESS) {
    wpa_hexdump(MSG_DEBUG, "SCARD: scard_transmit: recv", _recv, rlen);
  } else {
    wpa_printf(MSG_WARNING,
...
static int _scard_select_file(struct scard_data *scard, unsigned short file_id,
  ...
  if (aid) {
    wpa_hexdump(MSG_DEBUG, "SCARD: select file by AID", aid, aidlen);
  ...
  if (resp[0] != 0x6c && resp[0] != 0x9f && resp[0] != 0x61) {
    wpa_printf(MSG_WARNING,
               ...
               "(expected 0x61, 0x6c, or 0x9f)",
               resp[0]);
    return -1;
  }
  /* Normal ending of command; resp[1] bytes available */
  get_resp[4] = resp[1];
  wpa_printf(MSG_DEBUG, "SCARD: trying to get response (%d bytes)", resp[1]);
...
static int scard_select_file(struct scard_data *scard, unsigned short file_id,
                             unsigned char *buf, size_t *buf_len) {
  return _scard_select_file(scard, file_id, buf, buf_len, scard->sim_type, NULL,
                            0);
}
...
static int scard_get_record_len(struct scard_data *scard, unsigned char recnum,
  ...
  wpa_hexdump(MSG_DEBUG, "SCARD: file length determination response", buf,
              blen);
...
static int scard_read_file(struct scard_data *scard, unsigned char *data,
                           size_t len) {
...
int scard_get_pin_retry_counter(struct scard_data *scard) {
  ...
  wpa_printf(MSG_DEBUG,
             "SCARD: Unexpected PIN retry counter response "
             "value 0x%x",
             val);
  return 0;
}
...
int scard_get_imsi(struct scard_data *scard, char *imsi, size_t *len) {
  ...
  if (blen < 2 || blen > sizeof(buf)) {
    wpa_printf(MSG_DEBUG, "SCARD: invalid IMSI file length=%ld", (long)blen);
    return -3;
  }

  imsilen = (blen - 2) * 2 + 1;
  wpa_printf(MSG_DEBUG, "SCARD: IMSI file length=%ld imsilen=%ld", (long)blen,
             (long)imsilen);
...
int scard_get_mnc_len(struct scard_data *scard) {
  ...
  if (file_size < 4 || file_size > (int)sizeof(buf)) {
    wpa_printf(MSG_DEBUG, "SCARD: invalid file length=%ld", (long)file_size);
    return -4;
  }
  ...
  if (buf[3] < 2 || buf[3] > 3) {
    wpa_printf(MSG_DEBUG, "SCARD: invalid MNC length=%ld", (long)buf[3]);
    return -6;
  }
...
int scard_umts_auth(struct scard_data *scard, const unsigned char *_rand,
                    const unsigned char *autn, unsigned char *res,
                    size_t *res_len, unsigned char *ik, unsigned char *ck,
                    unsigned char *auts) {
  ...
  if (len >= 2 + AKA_AUTS_LEN && buf[0] == 0xdc && buf[1] == AKA_AUTS_LEN) {
    ...
    *res_len = *pos++;
    ...
    if (end > pos) {
      wpa_hexdump(MSG_DEBUG, "SCARD: Ignore extra data in end", pos, end - pos);
    }
