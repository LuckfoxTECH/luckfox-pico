...
static char HTTP_RESPOSE_MESSAGE[] =
...
static int initSocket(const unsigned int port) {
  ...
  ret = setsockopt(fd_socket, SOL_SOCKET, SO_REUSEADDR, (void *)&val,
                   sizeof(int));
  if (ret < 0) {
...
static void *threadAccept(void *arg) {
  ...
  while (1) {
    printf("accept...\n");
    fd_client =
        accept(fd_server, (struct sockaddr *)&addr_client, &len_addr_client);
...
int RK_softap_getState(RK_SOFTAP_STATE *pState) { return m_state; }
