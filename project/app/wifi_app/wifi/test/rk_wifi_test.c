...
/* SOFTAP CONFIG TEST */
static int rk_wifi_softap_state_callback(RK_SOFTAP_STATE state,
                                         const char *data) {
  switch (state) {
...
void rk_wifi_softap_stop(int argc, char *argv[]) { RK_softap_stop(); }
/* SOFTAP TEST END */
...
static command_t socket_cmd_table[] = {
    ...
    {"deepsleep", "rkwifi_server deepsleep [ONLY LOW POWER]",
     RK_wifi_enter_sleep},
    ...
    {"setpir", "rkwifi_server setpir <0/1> = disable/enable PIR",
     rk_wifi_set_pir},
    {"ota", "rkwifi_server ota", rk_wifi_ota},
    {"start_kp", "rkwifi_server start_kp <ip:port:expire>",
     rk_wifi_set_keepalive},
