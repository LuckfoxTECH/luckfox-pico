...
//取消配网
int atbm_wifi_cancel(void) { return atbm_wifi_recovery(); }
...
int atbm_wifi_start_keepalive(RK_WIFI_LOW_POWER_KEEPALIVE_s *cfg) {
  ...
  snprintf(buf_at_cmd, sizeof(buf_at_cmd),
           "AT+WIFI_HEART_PKT TEXT \"keepalive\" DEVID \"%s\" KEY \"%s\" "
           "PERIOD %s SERVER %s PORT %s",
           cfg->device_id, cfg->key, cfg->period, cfg->domain, cfg->port);
