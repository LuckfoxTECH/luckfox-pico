...
//注册状态回调
int RK_wifi_register_callback(RK_wifi_state_callback cb) { m_wifi_cb = cb; }
...
int RK_wifi_enable(int enable) {
  ...
  if (rk_hal_ops == NULL) {
#ifdef ATBM6441
    exec_command("cat /sys/bus/sdio/devices/*/uevent | grep \"007A\:6011\"",
                 buf, 64);
    if (buf[0]) {
...
#ifdef HI3861L
    exec_command("cat /sys/bus/sdio/devices/*/uevent | grep \"0296\:5347\"",
                 buf, 64);
    if (buf[0]) {
