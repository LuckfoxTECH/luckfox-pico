...
//由于二次唤醒后重新加载驱动，如果WiFi一直连着，则不会主动反馈连接的成功的CONNECTED
// EVENT 需要第一次主动去获取WiFi的状态，加个初始化编译，第一次主动上报状态
...
static hi_s32 sample_wlan_init_ip(hi_u8 *ip_addr, hi_u8 *netmask,
  ...
  (hi_void) memset_s(cmd, SYSTEM_CMD_SIZE, 0, SYSTEM_CMD_SIZE);
  (hi_void) snprintf_s(cmd, SYSTEM_CMD_SIZE, SYSTEM_CMD_SIZE - 1,
                       "route | grep default && route del default dev %s",
                       SYSTEM_NETDEV_NAME);
  ret = system(cmd);
  pr_info("cmd: %s\n", cmd);
  if (ret == -1) {
...
void sample_link_rec_cb(unsigned char *msg_data, int len) {
  ...
  if (NULL != item) {
    ...
    case 88: {
      ...
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.version, 10, item->valuestring,
      ...
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.mac_address, 20, item->valuestring,
                 strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "ssid");
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.ssid, 64, item->valuestring,
                 strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "bssid");
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.bssid, 20, item->valuestring,
                 strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "channel");
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.channel, 6, item->valuestring,
                 strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "status");
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.wpa_state, 20, item->valuestring,
                 strlen(item->valuestring));
      }
      ...
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.ip_address, 20, item->valuestring,
                 strlen(item->valuestring));
        memcpy_s(ipaddr, 20, item->valuestring, strlen(item->valuestring));
      ...
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.gw, 20, item->valuestring,
                 strlen(item->valuestring));
        memcpy_s(gwaddr, 20, item->valuestring, strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "dns1");
      if ((NULL != item) && (NULL != item->valuestring) &&
          (0 != strlen(item->valuestring))) {
        memcpy_s(hi_wifi_info.dns, 20, item->valuestring,
                 strlen(item->valuestring));
        memcpy_s(dnsfaddr, 20, item->valuestring, strlen(item->valuestring));
      }
      item = cJSON_GetObjectItem(root, "dns2");
      ...
      pr_info("stats old: %s, new: %s\n", g_hi_wifi_info.wpa_state,
              hi_wifi_info.wpa_state);
      if (strcmp(g_hi_wifi_info.wpa_state, hi_wifi_info.wpa_state)) {
        sample_wlan_init_ip(ipaddr, nwaddr, gwaddr, dnsfaddr, dnssaddr, 0);

        if (m_wifi_cb) {
          if (!strncmp(hi_wifi_info.wpa_state, "CONNECTED",
                       strlen("CONNECTED"))) {
