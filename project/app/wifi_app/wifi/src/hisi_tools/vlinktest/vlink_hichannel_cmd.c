...
hi_u32 vlink_hi_channel_cmd_get_info(hi_void) {
  ...
  sample_log_print(
      "vlink_hi_channel_cmd_get_info-------sendcmdbuf-[%02X]-[%02X]-[%02X]-\n",
      sendcmdbuf[0], sendcmdbuf[1], sendcmdbuf[2]);
...
hi_u32 vlink_hi_channel_cmd_sta_disconnect(hi_void) {
  ...
  sample_log_print(
      "vlink_hi_channel_cmd_sta_disconnect-------sendcmdbuf-[%02X]-\n",
      sendcmdbuf[0]);
...
hi_u32 vlink_hi_channel_cmd_ota(hi_char *path) {
  ...
  hi_u32 datalen = 0;
  hi_u32 times = 0;
  if (pthread_cond_init(&hi_ota_sync_cond, NULL) != 0 ||
      pthread_mutex_init(&hi_ota_sync_lock, NULL)) {
