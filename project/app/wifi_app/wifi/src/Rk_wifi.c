...
static RK_WIFI_encode_gbk_t *encode_gbk_insert(RK_WIFI_encode_gbk_t *head,
                                               const char *ori,
                                               const char *utf8) {
  if ((ori == NULL || strlen(ori) == 0) || (utf8 == NULL || strlen(utf8) == 0))
    return head;

  RK_WIFI_encode_gbk_t *gbk =
      (RK_WIFI_encode_gbk_t *)malloc(sizeof(RK_WIFI_encode_gbk_t));
...
static char *get_encode_gbk_ori(RK_WIFI_encode_gbk_t *head, const char *str,
                                char *dst) {
  int is_gbk = 0;
...
static char *get_encode_gbk_utf8(RK_WIFI_encode_gbk_t *head, const char *str,
                                 char *dst) {
  int is_gbk = 0;
  RK_WIFI_encode_gbk_t *gbk = head;
...
static void wifi_state_send(RK_WIFI_RUNNING_State_e state,
  ...
  if (state == RK_WIFI_State_CONNECTFAILED ||
      state == RK_WIFI_State_CONNECTING) {
    ...
    // wifi_state[state], cndinfo.ssid, cndinfo.bssid, cndinfo.reason);
    ...
    if (info) {
      ...
      // pr_info("[RKWIFI]: %s: %s, %s, %s, reason=%d\n", __func__,
      // wifi_state[state], info->ssid, info->bssid, info->reason);
    }
...
static int get_ssid_from_list_network(RK_WIFI_SAVED_INFO_s *info, int row,
                                      int columns) {
  int start = 0, end = 0;
  ...
  if (get_ssid_directly(info->state, row, columns)) {
    snprintf(cmd, sizeof(cmd),
             "cat /tmp/save_info.txt | awk '{print $2}' | sed -n %dp", row);
...
int get_bssid_from_list_network(RK_WIFI_SAVED_INFO_s *info, int row,
                                int columns) {
  ...
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_info.txt | awk '{print $%d}' | sed -n %dp",
           bssid_column, row);
  exec_command(cmd, str, 128);
  pr_info("%s: row = %d, column(%d) = %s\n", __func__, row, bssid_column, str);

  strncpy(info->bssid, str,
          ((strlen(str) - 1) > BSSID_BUF_LEN) ? BSSID_BUF_LEN
                                              : (strlen(str) - 1));
  pr_info("%s: bssid: %s\n", __func__, info->bssid);
...
int get_id_from_list_network(int row) {
  ...
  memset(str, 0, 128);
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_info.txt | awk '{print $1}' | sed -n %dp", row);
  exec_command(cmd, str, 128);
  network_id = atoi(str);
...
int get_columns_from_list_network(int row) {
  ...
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_info.txt | awk '{print NF}' | sed -n %dp", row);
  exec_command(cmd, str, 128);
  columns = atoi(str);
...
void get_flags_from_list_network(char *flags_buf, int buf_len, int row,
                                 int columns) {
  char cmd[128], str[128];
  ...
  *ap_cnt = cnt;
  ...
  for (int i = 0; i < cnt; i++) {
    pr_info("id: %d, name: %s, bssid: %s, state: %s\n", pInfo_in[i].id,
            pInfo_in[i].ssid, pInfo_in[i].bssid, pInfo_in[i].state);
  }
...
static int _get_ssid_from_list_network(RK_WIFI_SAVED_INFO_s *info, int row,
                                       int columns) {
  int start = 0, end = 0;
  char cmd[256], str[256];
  char ssid[256], sname[256], utf8[256];
  ...
  if (get_ssid_directly(info->state, row, columns)) {
    snprintf(cmd, sizeof(cmd),
             "cat /tmp/save_network.txt | awk '{print $2}' | sed -n %dp", row);
    exec_command(cmd, str, 256);
    pr_info("%s: row = %d, column(2) = %s\n", __func__, row, str);
    pr_info("%s: check len str: %d\n", __func__, strlen(str));
...
int _get_bssid_from_list_network(RK_WIFI_SAVED_INFO_s *info, int row,
                                 int columns) {
  char cmd[128], str[128];
  int bssid_column = columns;
  pr_info("%s: enter\n", __func__);
  ...
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_network.txt | awk '{print $%d}' | sed -n %dp",
           bssid_column, row);
  exec_command(cmd, str, 128);
  pr_info("%s: row = %d, column(%d) = %s\n", __func__, row, bssid_column, str);
...
int _get_id_from_list_network(int row) {
  ...
  memset(str, 0, 128);
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_network.txt | awk '{print $1}' | sed -n %dp", row);
  exec_command(cmd, str, 128);
  network_id = atoi(str);
...
int _get_columns_from_list_network(int row) {
  ...
  snprintf(cmd, sizeof(cmd),
           "cat /tmp/save_network.txt | awk '{print NF}' | sed -n %dp", row);
  exec_command(cmd, str, 128);
  columns = atoi(str);
...
void _get_flags_from_list_network(char *flags_buf, int buf_len, int row,
                                  int columns) {
  char cmd[128], str[128];
...
static int _RK_wifi_getSavedInfo(RK_WIFI_SAVED_INFO_s **pInfo, int *ap_cnt) {
  ...
  *ap_cnt = cnt;
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    _get_flags_from_list_network(pInfo_in[i].state, STATE_BUF_LEN, row,
                                 columns);
...
static int
_RK_wifi_running_getConnectionInfo(RK_WIFI_INFO_Connection_s *pInfo) {
  FILE *fp = NULL;
  char line[512];
  char *value, *orgs;
  pr_info("%s: enter\n", __func__);
  ...
  if (enable) {
    if (!is_wifi_enable()) {
      ...
      if (access("/data/cfg/wpa_supplicant.conf", F_OK) == 0) {
        exec_command_system(
            "wpa_supplicant -B -i wlan0 -c /data/cfg/wpa_supplicant.conf -d");
      } else {
        if (access("/etc/wpa_supplicant.conf", F_OK) == 0) {
          // check /data write?
          if (exec_command_system(
                  "cp /etc/wpa_supplicant.conf /data/wpa_supplicant.conf") == 0)
            exec_command_system(
          ...
          pr_info(
              "Don't found wpa_supplicant.conf so RK_wifi_enable failed!\n");
      ...
      s = pthread_create(&start_wifi_monitor_threadId, NULL,
                         RK_wifi_start_monitor, NULL);
      if (s != 0) {
        errno = s;
      ...
      if ((cols & (1 << index)) || 3 == index) {
        ...
        if (0 == index) {
          ...
          if (strlen(p_strtok) > 0) {
            ...
            // escape characters The ssid can't contains escape character while
            // do connect
            ...
            if (!is_utf8) {
              ...
              m_gbk_head =
                  encode_gbk_insert(m_gbk_head, dst_noescape, utf8_noescape);
...
char *wpa_wifi_scan_for_softap(void) { return wpa_wifi_scan_softap(0x1F); }
...
char *wpa_wifi_scan_r(void) { return wpa_wifi_scan_r_sec(0x1F); }
...
static int set_network(const int id, const char *ssid, const char *psk,
  ...
  snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d ssid %s", id,
           wifi_ssid);
  ...
  if (strlen(psk) == 0 && encryp == NONE) {
    snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d key_mgmt NONE",
             id);
    exec_command(cmd, str, 8);
    if (0 == strlen(str) || 0 != strncmp(str, "OK", 2))
      return false;
    ...
    if ((sizeof(psk) == 5) || (sizeof(psk) == 13) || (sizeof(psk) == 16))
      snprintf(cmd, sizeof(cmd),
               "wpa_cli -iwlan0 set_network %d wep_key0 \\\"%s\\\"", id, psk);
    else
      snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d wep_key0 %s",
               id, psk);
    exec_command(cmd, str, 8);
    if (0 == strlen(str) || 0 != strncmp(str, "OK", 2))
      return false;
    ...
    memset(str, 0, sizeof(str));
    memset(cmd, 0, sizeof(cmd));
    snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d auth_alg SHARED",
             id);
    exec_command(cmd, str, 8);
    if (0 == strlen(str) || 0 != strncmp(str, "OK", 2))
      return false;
    ...
    snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d psk \\\"%s\\\"",
             id, wifi_psk);
    exec_command(cmd, str, 8);
    if (0 == strlen(str) || 0 != strncmp(str, "OK", 2))
      return false;
...
static int set_hide_network(const int id) {
  ...
  memset(str, 0, sizeof(str));
  memset(cmd, 0, sizeof(cmd));
  snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d scan_ssid %d", id,
           1);
  exec_command(cmd, str, 8);
...
static int select_network(const int id) {
  ...
  snprintf(retry_connect_cmd, sizeof(retry_connect_cmd),
           "wpa_cli -iwlan0 select_network %d", id);
  exec_command(cmd, str, 8);
...
static int set_priority_network(const int id, int priority) {
  ...
  memset(str, 0, sizeof(str));
  memset(cmd, 0, sizeof(cmd));
  snprintf(cmd, sizeof(cmd), "wpa_cli -iwlan0 set_network %d priority %d", id,
           priority);
  exec_command(cmd, str, 8);
...
static void set_network_highest_priority(const int id) {
  ...
  for (int i = 0; i < cnt; i++) {
    snprintf(cmd, sizeof(cmd),
             "wpa_cli -i wlan0 list_network | awk '{print $1}' | sed -n %dp",
             i + 2);
    exec_command(cmd, str, 128);
    network_id = atoi(str);
...
static int disable_no_connected_ssid(void) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    if (network_id != info.id) {
      memset(cmd, 0, 128);
      snprintf(cmd, sizeof(cmd), "wpa_cli -i wlan0 disable_network %d",
               network_id);
...
int wpa_wifi_connect1(char *ssid, const char *psk,
                      const RK_WIFI_CONNECTION_Encryp_e encryp,
                      const int hide) {
  int id, ret;
  ...
  if (save_last_ap) {
    exec_command_system(
        "cp /data/cfg/wpa_supplicant.conf /data/cfg/wpa_supplicant.conf.bak");
    exec_command_system("wpa_cli flush");
    sleep(100000);
...
int wpa_wifi_restart_network(void) { return true; }
...
int wpa_wifi_recovery(void) {
  if (save_last_ap) {
    exec_command_system(
        "cp /data/cfg/wpa_supplicant.conf.bak /data/cfg/wpa_supplicant.conf");
  }
...
static void get_wifi_info_by_event(char *event, RK_WIFI_RUNNING_State_e state,
  ...
  case RK_WIFI_State_CONNECTFAILED_WRONG_KEY:
    ...
    if (id_tag) {
      ...
      // memset(cmd, 0, 128);
      // snprintf(cmd, sizeof(cmd), "wpa_cli -i wlan0 remove_network %d",
      // info->id); exec_command_system(cmd); exec_command_system("wpa_cli -i
      // wlan0 save_config");
    }
...
static void get_valid_connect_info(RK_WIFI_INFO_Connection_s *info) {
  ...
  while (count--) {
    _RK_wifi_running_getConnectionInfo(info);
    if (!info->id && !strlen(info->ssid) && !strlen(info->bssid) &&
        !info->freq && !strlen(info->mode) && !strlen(info->ip_address) &&
...
static int dispatch_event(char *event) {
  ...
  if (strstr(event, "CTRL-EVENT-BSS") ||
      strstr(event, "CTRL-EVENT-TERMINATING"))
    return false;
  ...
  if (is_wep && strstr(event, "CTRL-EVENT-ASSOC-REJECT") &&
      strstr(event, "status_code=13")) {
    char cmd[128];
    memset(cmd, 0, 128);
    snprintf(cmd, sizeof(cmd), "wpa_cli -i wlan0 set_network %d auth_alg OPEN",
             wep_id);
    exec_command_system(cmd);
    memset(cmd, 0, 128);
