...
//获取当前WiFi运行状态
int hisi_running_getState(RK_WIFI_RUNNING_State_e *pState) { return true; }
...
int hisi_connect1(char *ssid, const char *psk,
...
//扫描
int hisi_scan(void) { return true; }

//获取扫描结果
char *hisi_scan_r(void) { return NULL; }
...
//连接WEP加密的WIFI
int hisi_connect_wep(char *ssid, const char *psk) { return true; }
...
// AP模块设置路由信息
int hisi_set_hostname(const char *name) { return true; }

int hisi_get_hostname(char *name, int len) { return true; }
...
int hisi_ping(char *address) { return true; }

// WiFi恢复之前的配置,或重连之前的配置
int hisi_recovery(void) { return true; }

//移除连接过的WiFi
int hisi_forget_with_ssid(char *ssid) { return true; }

//取消配网
int hisi_cancel(void) { return true; }
...
//连接之前连过的WiFi
int hisi_connect_with_ssid(char *ssid) { return true; }

// WiFi恢复出厂设置
int hisi_reset(void) { return true; }

//获取AP的信号强度
int hisi_get_connected_ap_rssi(void) { return true; }

//获取COUNTRY CODE
int hisi_get_ccode(void) { return true; }

//配置COUNTRY CODE
int hisi_set_ccode(void) { return true; }

char *hisi_scan_for_softap(void) { return true; }
...
int hisi_exit_sleep(void) { return true; }
...
int hisi_set_BeaconListenInterval(int dtim) { return true; }
...
int hisi_wifi_ota(char *path) { return vlink_hi_channel_cmd_ota(path); }
