...
int RK_socket_server_setup(struct rk_socket_app *app_socket) {
  ...
  if ((bind(app_socket->server_sockfd,
            (struct sockaddr *)&app_socket->server_address,
            app_socket->server_len)) < 0) {
    log_err("fail to bind socket\n");
    perror("bind");
    return -1;
  }

  setsockopt(app_socket->server_sockfd, SOL_SOCKET, SO_RCVBUF, (void *)&buflen,
             sizeof(int));
  getsockopt(app_socket->server_sockfd, SOL_SOCKET, SO_RCVBUF, (void *)&buflen,
             &optlen);
...
int RK_socke_server_accpet(struct rk_socket_app *app_socket) {
  app_socket->client_sockfd = accept(
      app_socket->server_sockfd, (struct sockaddr *)&app_socket->server_address,
      (socklen_t *)&app_socket->client_len);
...
int RK_socket_udp_send(char *socket_path, char *msg, int len) {
  ...
  bytes = sendto(sockfd, msg, len, MSG_DONTWAIT, (struct sockaddr *)&serverAddr,
                 sizeof(serverAddr));
  if (bytes < 0)
