...
/*  A struct is used to keep track of the series list because later we need to
...
static int32_t round_fixed_point(int32_t n, int8_t shift) {
  ...
  return (decimal_part < rounding_boundary) ? (n & ~mask)
                                            : ((n >> shift) + 1) << shift;
}
...
void lv_example_chart_8(void) {
  ...
  lv_obj_add_event_cb(stacked_area_chart.obj, draw_event_cb,
  ...
  lv_chart_set_axis_tick(stacked_area_chart.obj, LV_CHART_AXIS_PRIMARY_Y, 3, 0,
                         5, 1, true, 30);
  ...
  stacked_area_chart.series_list[0] = lv_chart_add_series(
      stacked_area_chart.obj, lv_palette_main(LV_PALETTE_RED),
      LV_CHART_AXIS_PRIMARY_Y);
  stacked_area_chart.series_list[1] = lv_chart_add_series(
      stacked_area_chart.obj, lv_palette_main(LV_PALETTE_BLUE),
      LV_CHART_AXIS_PRIMARY_Y);
  stacked_area_chart.series_list[2] = lv_chart_add_series(
      stacked_area_chart.obj, lv_palette_main(LV_PALETTE_GREEN),
      LV_CHART_AXIS_PRIMARY_Y);
  ...
  for (int point = 0; point < 10; point++) {
    ...
    for (int32_t series_index = 0; series_index < 3; series_index++) {
      ...
      /*  The draw heights are equal to the percentage of the total each value
         is + the cumulative sum of the previous percentages. The accumulation
         is how the values get "stacked" */
      draw_heights[series_index] = int_sum + modifier;

      /*  Draw to the series in the reverse order to which they were
         initialised. Without this the higher values will draw on top of the
         lower ones. This is because the Z-height of a series matches the order
         it was initialised */
      lv_chart_set_next_value(
          stacked_area_chart.obj,
