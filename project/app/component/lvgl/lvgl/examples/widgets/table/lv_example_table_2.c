...
static void draw_event_cb(lv_event_t *e) {
  ...
  if (dsc->part == LV_PART_ITEMS) {
    ...
    rect_dsc.bg_color = chk ? lv_theme_get_color_primary(obj)
    ...
    sw_area.y1 =
        dsc->draw_area->y1 + lv_area_get_height(dsc->draw_area) / 2 - 10;
...
void lv_example_table_2(void) {
  ...
  lv_table_set_row_cnt(table,
                       ITEM_CNT); /*Not required but avoids a lot of memory
