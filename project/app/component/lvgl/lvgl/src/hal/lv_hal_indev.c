 ...
 * @return the next input devise or NULL if no more. Give the first input device
...
void _lv_indev_read(lv_indev_t *indev, lv_indev_data_t *data) {
  ...
  /* For touchpad sometimes users don't set the last pressed coordinate on
   * release. So be sure a coordinates are initialized to the last point */
  if (indev->driver->type == LV_INDEV_TYPE_POINTER) {
    data->point.x = indev->proc.types.pointer.last_raw_point.x;
    data->point.y = indev->proc.types.pointer.last_raw_point.y;
  }
  /*Similarly set at least the last key in case of the user doesn't set it on
     release*/
