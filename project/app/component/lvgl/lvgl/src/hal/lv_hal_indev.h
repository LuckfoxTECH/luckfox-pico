...
typedef enum {
  ...
  LV_INDEV_TYPE_BUTTON,  /**< External (hardware button) which is assigned to a
                            specific point of the screen*/
...
typedef struct {
  ...
  bool continue_reading;  /**< If set to true, the read callback is invoked
                             again*/
} lv_indev_data_t;
...
typedef struct _lv_indev_drv_t {
  ...
  /**< At least this difference should be between two points to evaluate as
   * gesture*/
...
typedef struct _lv_indev_proc_t {
  ...
  union {
    struct {
      ...
      lv_point_t scroll_sum; /*Count the dragged pixels to check
      ...
      lv_point_t gesture_sum; /*Count the gesture pixels to check
...
/** The main input device descriptor with driver, runtime data ('proc') and some
...
typedef struct _lv_indev_t {
  ...
  const lv_point_t
      *btn_points; /**< Array points assigned to the button ()screen will be
 ...
 * @param indev pointer to an input device. (return value of
...
/**
 * Remove the provided input device. Make sure not to use the provided input
 * device afterwards anymore.
 ...
 * @return the next input device or NULL if there are no more. Provide the first
 * input device when the parameter is NULL
 */
