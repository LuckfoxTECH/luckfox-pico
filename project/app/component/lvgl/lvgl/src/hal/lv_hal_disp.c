 ...
 *  STATIC PROTOTYPES
 **********************/
static lv_obj_tree_walk_res_t invalidate_layout_cb(lv_obj_t *obj,
                                                   void *user_data);
...
lv_disp_t *lv_disp_drv_register(lv_disp_drv_t *driver) {
  ...
  disp_def = disp; /*Temporarily change the default screen to create the default
                      screens on the new display*/
  ...
  if (driver->full_refresh &&
      driver->draw_buf->size < (uint32_t)driver->hor_res * driver->ver_res) {
  ...
  disp->act_scr =
      lv_obj_create(NULL); /*Create a default screen on the display*/
  ...
  if (disp_def == NULL)
    disp_def = disp; /*Initialize the default display*/

  lv_timer_ready(disp->refr_timer); /*Be sure the screen will be refreshed
...
void lv_disp_drv_update(lv_disp_t *disp, lv_disp_drv_t *new_drv) {
  ...
  if (disp->driver->full_refresh &&
      disp->driver->draw_buf->size <
          (uint32_t)disp->driver->hor_res * disp->driver->ver_res) {
  ...
  /*
   * This method is usually called upon orientation change, thus the screen is
   * now a different size. The object invalidated its previous area. That area
   * is now out of the screen area so we reset all invalidated areas and
   * invalidate the active screen's new area only.
   */
...
void lv_disp_remove(lv_disp_t *disp) {
  ...
  if (was_default)
    lv_disp_set_default(_lv_ll_get_head(&LV_GC_ROOT(_lv_disp_ll)));
}
 ...
 */
void lv_disp_set_default(lv_disp_t *disp) { disp_def = disp; }

/**
 * Get the default display
 * @return pointer to the default display
 */
lv_disp_t *lv_disp_get_default(void) { return disp_def; }
...
lv_coord_t lv_disp_get_physical_hor_res(lv_disp_t *disp) {
  ...
  if (disp == NULL) {
    ...
    case LV_DISP_ROT_90:
    case LV_DISP_ROT_270:
      return disp->driver->physical_ver_res > 0 ? disp->driver->physical_ver_res
                                                : disp->driver->ver_res;
    default:
      return disp->driver->physical_hor_res > 0 ? disp->driver->physical_hor_res
                                                : disp->driver->hor_res;
    }
  }
}
...
lv_coord_t lv_disp_get_dpi(const lv_disp_t *disp) {
  if (disp == NULL)
    disp = lv_disp_get_default();
  if (disp == NULL)
    return LV_DPI_DEF; /*Do not return 0 because it might be a divider*/
...
LV_ATTRIBUTE_FLUSH_READY void lv_disp_flush_ready(lv_disp_drv_t *disp_drv) {
  ...
  if (disp_drv->screen_transp) {
    if (disp_drv->clear_cb) {
      disp_drv->clear_cb(disp_drv, disp_drv->draw_buf->buf_act,
                         disp_drv->draw_buf->size);
    } else {
      lv_memset_00(disp_drv->draw_buf->buf_act,
                   disp_drv->draw_buf->size * sizeof(lv_color32_t));
    }
  }
#endif
