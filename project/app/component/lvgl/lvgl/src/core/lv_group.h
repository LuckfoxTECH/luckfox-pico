...
/**
 * Groups can be used to logically hold objects so that they can be individually
 * focused. They are NOT for laying out objects on a screen (try layouts for
 * that).
 */
typedef struct _lv_group_t {
  ...
  lv_group_focus_cb_t focus_cb; /**< A function to call when a new object is
                                   focused (optional)*/
#if LV_USE_USER_DATA
  void *user_data;
#endif
  ...
  uint8_t refocus_policy : 1; /**< 1: Focus prev if focused on deletion. 0:
                                 Focus next if focused on deletion.*/
...
/**
 * Set a default group. New object are added to this group if it's enabled in
...
/**
 * Set whether the next or previous item in a group is focused if the currently
 * focused obj is deleted.
 * @param group         pointer to a group
