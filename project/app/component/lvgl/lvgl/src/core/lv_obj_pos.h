 ...
 *                  LV_SIZE_CONTENT     set the size to involve all children in
 * the given direction LV_SIZE_PCT(x)     to set size in percentage of the
 ...
 *                  pixel               simple set the size accordingly
 *                  LV_SIZE_CONTENT     set the size to involve all children in
 * the given direction lv_pct(x)           to set size in percentage of the
 * parent's content area size (the size without paddings). x should be in
 * [0..1000]% range
 */
 ...
 * @param align     type of alignment (see 'lv_align_t' enum) `LV_ALIGN_OUT_...`
 * can't be used.
 */
 ...
 */
void lv_obj_align(struct _lv_obj_t *obj, lv_align_t align, lv_coord_t x_ofs,
                  lv_coord_t y_ofs);

/**
 * Align an object to an other object.
 * @param obj       pointer to an object to align
 * @param base      pointer to an other object (if NULL `obj`s parent is used).
 * 'obj' will be aligned to it.
 * @param align     type of alignment (see 'lv_align_t' enum)
 * @param x_ofs     x coordinate offset after alignment
 * @param y_ofs     y coordinate offset after alignment
 * @note            if the position or size of `base` changes `obj` needs to be
 * aligned manually again
 */
 ...
 * @param obj       pointer to an object to align
 * @note            if the parent size changes `obj` needs to be aligned
 * manually again
 */
 ...
 * @param obj       pointer to an object
 * @return          distance of `obj` from the left side of its parent plus the
 * parent's left padding
 * @note            The position of the object is recalculated only on the next
 * redraw. To force coordinate recalculation call `lv_obj_update_layout(obj)`.
 * @note            Zero return value means the object is on the left padding of
 * the parent, and not on the left edge.
 ...
 * @param obj       pointer to an object
 * @return          distance of `obj` from the right side of its parent plus the
 ...
 * @note            The position of the object is recalculated only on the next
 * redraw. To force coordinate recalculation call `lv_obj_update_layout(obj)`.
 * @note            Zero return value means the object is on the top padding of
 * the parent, and not on the top edge.
 * @note            Scrolling of the parent doesn't change the returned value.
 * @note            The returned value is always the distance from the parent
 * even if `obj` is positioned by a layout.
 */
 ...
 * @note            The position of the object is recalculated only on the next
 * redraw. To force coordinate recalculation call `lv_obj_update_layout(obj)`.
 * @note            Zero return value means the object is on the bottom padding
 * of the parent, and not on the bottom edge.
 * @note            Scrolling of the parent doesn't change the returned value.
 * @note            The returned value is always the distance from the parent
 * even if `obj` is positioned by a layout.
 */
 ...
 * @note            The position of the object is recalculated only on the next
 * redraw. To force coordinate recalculation call `lv_obj_update_layout(obj)`.
 * @return          the height which still fits into the parent without causing
 * overflow (making the parent scrollable)
 */
...
/**
 * Get the height occupied by the "parts" of the widget. E.g. the height of all
 * rows of a table.
 * @param obj       pointer to an objects
 * @return          the width of the virtually drawn content
 * @note            This size independent from the real size of the widget.
...
void lv_obj_move_children_by(struct _lv_obj_t *obj, lv_coord_t x_diff,
                             lv_coord_t y_diff, bool ignore_floating);
 ...
 * written back here.
 * @return true     visible; false not visible (hidden, out of parent, on other
 * screen, etc)
 */
...
/**
 * Clamp a width between min and max width. If the min/max width is in
 * percentage value use the ref_width
 * @param width         width to clamp
 * @param min_width     the minimal width
 * @param max_width     the maximal width
 * @param ref_width     the reference width used when min/max width is in
 * percentage
 * @return              the clamped width
 */
lv_coord_t lv_clamp_width(lv_coord_t width, lv_coord_t min_width,
                          lv_coord_t max_width, lv_coord_t ref_width);

/**
 * Clamp a height between min and max height. If the min/max height is in
 * percentage value use the ref_height
 * @param height         height to clamp
 * @param min_height     the minimal height
 * @param max_height     the maximal height
 * @param ref_height     the reference height used when min/max height is in
 * percentage
 * @return              the clamped height
 */
lv_coord_t lv_clamp_height(lv_coord_t height, lv_coord_t min_height,
                           lv_coord_t max_height, lv_coord_t ref_height);
