...
static void lv_refr_obj_and_children(lv_draw_ctx_t *draw_ctx,
                                     lv_obj_t *top_obj);
static uint32_t get_max_row(lv_disp_t *disp, lv_coord_t area_w,
                            lv_coord_t area_h);
static void draw_buf_flush(lv_disp_t *disp);
static void call_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area,
                          lv_color_t *color_p);
...
void lv_refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
  ...
  lv_color_t debug_color =
      lv_color_make(lv_rand(0, 0xFF), lv_rand(0, 0xFF), lv_rand(0, 0xFF));
  ...
  /*With overflow visible keep the previous clip area to let the children
   *visible out of this object too With not overflow visible limit the clip are
   *to the object's coordinates to clip the children*/
  bool refr_children = true;
  lv_area_t clip_coords_for_children;
  if (lv_obj_has_flag(obj, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
    clip_coords_for_children = *clip_area_ori;
  } else {
    if (!_lv_area_intersect(&clip_coords_for_children, clip_area_ori,
                            &obj->coords)) {
      refr_children = false;
    }
  }
 ...
 * invalidated areas)
 * @param disp pointer to display where the area should be invalidated (NULL can
 * be used if there is only one display)
 */
void _lv_inv_area(lv_disp_t *disp, const lv_area_t *area_p) {
  ...
  /*If there were at least 1 invalid area in full refresh mode, redraw the whole
   * screen*/
 ...
 */
lv_disp_t *_lv_refr_get_disp_refreshing(void) { return disp_refr; }
 ...
 */
void _lv_refr_set_disp_refreshing(lv_disp_t *disp) { disp_refr = disp; }
...
void _lv_disp_refr_timer(lv_timer_t *tmr) {
  ...
  if (disp_refr->inv_p != 0) {
    ...
    lv_memset_00(disp_refr->inv_area_joined,
  ...
  if (lv_tick_elaps(mem_monitor.mem_last_time) > 300) {
    ...
    lv_label_set_text_fmt(mem_label,
                          ...
                          "%d%% frag.",
                          used_kb, used_kb_tenth, mon.used_pct, mon.frag_pct);
  }
#endif
...
static void lv_refr_join_area(void) {
  ...
  for (join_in = 0; join_in < disp_refr->inv_p; join_in++) {
    ...
    for (join_from = 0; join_from < disp_refr->inv_p; join_from++) {
      ...
      _lv_area_join(&joined_area, &disp_refr->inv_areas[join_in],
...
static void lv_refr_area(const lv_area_t *area_p) {
  ...
  if (disp_refr->driver->full_refresh || disp_refr->driver->direct_mode) {
    ...
    if (disp_refr->driver->full_refresh) {
      ...
      disp_refr->driver->draw_buf->last_part =
          disp_refr->driver->draw_buf->last_area;
...
static void lv_refr_area_part(lv_draw_ctx_t *draw_ctx) {
  ...
  top_act_scr =
      lv_refr_get_top_obj(draw_ctx->buf_area, lv_disp_get_scr_act(disp_refr));
  if (disp_refr->prev_scr) {
...
static lv_obj_t *lv_refr_get_top_obj(const lv_area_t *area_p, lv_obj_t *obj) {
  ...
  if (_lv_area_is_in(area_p, &obj->coords, 0) &&
      lv_obj_has_flag(obj, LV_OBJ_FLAG_HIDDEN) == false) {
...
static uint32_t get_max_row(lv_disp_t *disp, lv_coord_t area_w,
  ...
  if (disp_refr->driver->rounder_cb) {
    ...
    do {
      ...
      /*Decrement the height of the area until it fits into `max_row` after
       * rounding*/
      h_tmp--;
    } while (h_tmp > 0);

    if (h_tmp <= 0) {
      LV_LOG_WARN("Can't set draw_buf height using the round function. (Wrong "
                  "round_cb or to "
                  "small draw_buf)");
      return 0;
    } else {
...
static void draw_buf_rotate_180(lv_disp_drv_t *drv, lv_area_t *area,
                                lv_color_t *color_p) {
...
static LV_ATTRIBUTE_FAST_MEM void
draw_buf_rotate_90(bool invert_i, lv_coord_t area_w, lv_coord_t area_h,
...
/**
 * Helper function for draw_buf_rotate_90_sqr. Given a list of four numbers,
 * rotate the entire list to the left.
 */
static inline void draw_buf_rotate4(lv_color_t *a, lv_color_t *b, lv_color_t *c,
                                    lv_color_t *d) {
  lv_color_t tmp;
  tmp = *a;
  *a = *b;
 ...
 */
static void draw_buf_rotate_90_sqr(bool is_270, lv_coord_t w,
                                   lv_color_t *color_p) {
    ...
    for (lv_coord_t j = 0; j < (w + 1) / 2; j++) {
      ...
      if (is_270) {
        draw_buf_rotate4(&color_p[i * w + j], &color_p[inv_j * w + i],
                         &color_p[inv_i * w + inv_j], &color_p[j * w + inv_i]);
      } else {
        draw_buf_rotate4(&color_p[i * w + j], &color_p[j * w + inv_i],
                         &color_p[inv_i * w + inv_j], &color_p[inv_j * w + i]);
      }
    }
  }
}
...
static void draw_buf_rotate(lv_area_t *area, lv_color_t *color_p) {
  ...
  if (drv->rotated == LV_DISP_ROT_180) {
  ...
  } else if (drv->rotated == LV_DISP_ROT_90 ||
    ...
    lv_coord_t max_row = LV_MIN(
        (lv_coord_t)((LV_DISP_ROT_MAX_BUF / sizeof(lv_color_t)) / area_w),
        area_h);
    ...
    while (row < area_h) {
      ...
      if ((row == 0) && (area_h >= area_w)) {
        ...
        height = area_w;
        draw_buf_rotate_90_sqr(drv->rotated == LV_DISP_ROT_270, area_w,
                               color_p);
        if (drv->rotated == LV_DISP_ROT_90) {
          area->x1 = init_y_off;
        ...
        draw_buf_rotate_90(drv->rotated == LV_DISP_ROT_270, area_w, height,
                           color_p, rot_buf);
      ...
      /* The original part (chunk of the current area) were split into more
       * parts here. Set the original last_part flag on the last part of
       * rotation. */
      if (row + height >= area_h && draw_buf->last_area &&
          draw_buf->last_part) {
      ...
      /*FIXME: Rotation forces legacy behavior where rendering and flushing are
       * done serially*/
      while (draw_buf->flushing) {
        if (drv->wait_cb)
          drv->wait_cb(drv);
...
static void draw_buf_flush(lv_disp_t *disp) {
  ...
  if (disp_refr->driver->draw_buf->last_area &&
      disp_refr->driver->draw_buf->last_part)
    draw_buf->flushing_last = 1;
  else
    draw_buf->flushing_last = 0;
  ...
  /*If there are 2 buffers swap them. With direct mode swap only on the last
   * area*/
  if (draw_buf->buf1 && draw_buf->buf2 &&
      (!disp->driver->direct_mode || flushing_last)) {
...
static void call_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  lv_area_t offset_area = {.x1 = area->x1 + drv->offset_x,
