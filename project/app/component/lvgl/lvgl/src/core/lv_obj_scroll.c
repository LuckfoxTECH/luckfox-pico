...
static void scroll_area_into_view(const lv_area_t *area, lv_obj_t *child,
                                  lv_point_t *scroll_value,
...
void lv_obj_set_scrollbar_mode(lv_obj_t *obj, lv_scrollbar_mode_t mode) {
  ...
  if (obj->spec_attr->scrollbar_mode == mode)
    return;
...
lv_scrollbar_mode_t lv_obj_get_scrollbar_mode(const lv_obj_t *obj) {
  if (obj->spec_attr)
    return obj->spec_attr->scrollbar_mode;
  else
...
lv_dir_t lv_obj_get_scroll_dir(const lv_obj_t *obj) {
  if (obj->spec_attr)
    return obj->spec_attr->scroll_dir;
  else
    return LV_DIR_ALL;
}
...
lv_scroll_snap_t lv_obj_get_scroll_snap_x(const lv_obj_t *obj) {
  if (obj->spec_attr)
    return obj->spec_attr->scroll_snap_x;
  else
...
lv_scroll_snap_t lv_obj_get_scroll_snap_y(const lv_obj_t *obj) {
  if (obj->spec_attr)
    return obj->spec_attr->scroll_snap_y;
  else
    return LV_SCROLL_SNAP_NONE;
}
...
lv_coord_t lv_obj_get_scroll_bottom(lv_obj_t *obj) {
  ...
  self_h = self_h -
           (lv_obj_get_height(obj) - pad_top - pad_bottom - 2 * border_width);
...
lv_coord_t lv_obj_get_scroll_left(lv_obj_t *obj) {
  ...
  /*With RTL base direction scrolling the left is normal so find the left most
   * coordinate*/
  ...
  self_w = self_w -
           (lv_obj_get_width(obj) - pad_right - pad_left - 2 * border_width);
...
lv_coord_t lv_obj_get_scroll_right(lv_obj_t *obj) {
  ...
  /*With other base direction (LTR) scrolling to the right is normal so find the
...
void lv_obj_scroll_by_bounded(lv_obj_t *obj, lv_coord_t dx, lv_coord_t dy,
                              lv_anim_enable_t anim_en) {
  if (dx == 0 && dy == 0)
    return;
  ...
  if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) != LV_BASE_DIR_RTL) {
    if (x_bounded > 0)
      x_bounded = 0;
    if (x_bounded < 0) {
      lv_coord_t scroll_max =
          lv_obj_get_scroll_left(obj) + lv_obj_get_scroll_right(obj);
      if (scroll_max < 0)
        scroll_max = 0;

      if (x_bounded < -scroll_max)
        x_bounded = -scroll_max;
    }
  } else {
    if (x_bounded < 0)
      x_bounded = 0;
...
void lv_obj_scroll_by(lv_obj_t *obj, lv_coord_t dx, lv_coord_t dy,
  ...
  if (anim_en == LV_ANIM_ON) {
    ...
    if (dx) {
      uint32_t t =
          lv_anim_speed_to_time((lv_disp_get_hor_res(d) * 2) >> 2, 0, dx);
      if (t < SCROLL_ANIM_TIME_MIN)
        t = SCROLL_ANIM_TIME_MIN;
      if (t > SCROLL_ANIM_TIME_MAX)
    ...
    if (dy) {
      uint32_t t =
          lv_anim_speed_to_time((lv_disp_get_ver_res(d) * 2) >> 2, 0, dy);
      if (t < SCROLL_ANIM_TIME_MIN)
        t = SCROLL_ANIM_TIME_MIN;
      if (t > SCROLL_ANIM_TIME_MAX)
        t = SCROLL_ANIM_TIME_MAX;
...
void lv_obj_scroll_to(lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                      lv_anim_enable_t anim_en) {
...
void lv_obj_get_scrollbar_area(lv_obj_t *obj, lv_area_t *hor_area,
                               lv_area_t *ver_area) {
  lv_area_set(hor_area, 0, 0, -1, -1);
  lv_area_set(ver_area, 0, 0, -1, -1);

  if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLLABLE) == false)
    return;
  ...
  if ((dir & LV_DIR_VER) &&
      ((sm == LV_SCROLLBAR_MODE_ON) ||
       ...
       (sm == LV_SCROLLBAR_MODE_ACTIVE &&
        lv_indev_get_scroll_dir(indev) == LV_DIR_VER))) {
    ver_draw = true;
  }
  ...
  bool rtl =
      lv_obj_get_style_base_dir(obj, LV_PART_SCROLLBAR) == LV_BASE_DIR_RTL
          ? true
          : false;
  ...
  /*Draw vertical scrollbar if the mode is ON or can be scrolled in this
   * direction*/
  ...
  if (ver_draw && content_h) {
    ...
    lv_coord_t sb_h =
        ((obj_h - top_space - bottom_space - hor_req_space) * obj_h) /
        content_h;
    sb_h = LV_MAX(sb_h, SCROLLBAR_MIN_SIZE);
    rem = (obj_h - top_space - bottom_space - hor_req_space) -
          sb_h; /*Remaining size from the scrollbar track that is not the
                   scrollbar itself*/
    lv_coord_t scroll_h =
        content_h -
        obj_h; /*The size of the content which can be really scrolled*/
    if (scroll_h <= 0) {
  ...
  /*Draw horizontal scrollbar if the mode is ON or can be scrolled in this
   * direction*/
  ...
  if (hor_draw && content_w) {
    ...
    lv_coord_t sb_w =
        ((obj_w - left_space - right_space - ver_reg_space) * obj_w) /
        content_w;
    sb_w = LV_MAX(sb_w, SCROLLBAR_MIN_SIZE);
    rem = (obj_w - left_space - right_space - ver_reg_space) -
          sb_w; /*Remaining size from the scrollbar track that is not the
                   scrollbar itself*/
    lv_coord_t scroll_w =
        content_w -
        obj_w; /*The size of the content which can be really scrolled*/
    if (scroll_w <= 0) {
      if (rtl) {
...
static void scroll_area_into_view(const lv_area_t *area, lv_obj_t *child,
  ...
  lv_coord_t ptop =
      lv_obj_get_style_pad_top(parent, LV_PART_MAIN) + border_width;
  lv_coord_t pbottom =
      lv_obj_get_style_pad_bottom(parent, LV_PART_MAIN) + border_width;
  lv_coord_t top_diff =
      parent->coords.y1 + ptop - area_tmp->y1 - scroll_value->y;
  lv_coord_t bottom_diff =
      -(parent->coords.y2 - pbottom - area_tmp->y2 - scroll_value->y);
  ...
  lv_coord_t pleft =
      lv_obj_get_style_pad_left(parent, LV_PART_MAIN) + border_width;
  lv_coord_t pright =
      lv_obj_get_style_pad_right(parent, LV_PART_MAIN) + border_width;
  lv_coord_t left_diff =
      parent->coords.x1 + pleft - area_tmp->x1 - scroll_value->x;
  lv_coord_t right_diff =
      -(parent->coords.x2 - pright - area_tmp->x2 - scroll_value->x);
  ...
  if ((scroll_dir & LV_DIR_LEFT) == 0 && x_scroll < 0)
    x_scroll = 0;
  if ((scroll_dir & LV_DIR_RIGHT) == 0 && x_scroll > 0)
    x_scroll = 0;
  if ((scroll_dir & LV_DIR_TOP) == 0 && y_scroll < 0)
    y_scroll = 0;
  if ((scroll_dir & LV_DIR_BOTTOM) == 0 && y_scroll > 0)
    y_scroll = 0;
