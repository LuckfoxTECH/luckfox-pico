...
lv_obj_t *lv_obj_class_create_obj(const lv_obj_class_t *class_p,
                                  lv_obj_t *parent) {
  LV_TRACE_OBJ_CREATE("Creating object with %p class on %p parent",
                      (void *)class_p, (void *)parent);
  ...
  if (parent == NULL) {
    ...
    if (disp->screens == NULL) {
      ...
      disp->screen_cnt++;
      disp->screens =
          lv_mem_realloc(disp->screens, sizeof(lv_obj_t *) * disp->screen_cnt);
...
void lv_obj_class_init_obj(lv_obj_t *obj) {
  ...
  if (parent) {
    /*Call the ancestor's event handler to the parent to notify it about the new
     *child. Also triggers layout update*/
...
void _lv_obj_destruct(lv_obj_t *obj) {
  ...
  if (obj->class_p->base_class) {
    /*Don't let the descendant methods run during destructing the ancestor
     * type*/
...
static void lv_obj_construct(lv_obj_t *obj) {
  ...
  if (obj->class_p->base_class) {
    /*Don't let the descendant methods run during constructing the ancestor
     * type*/
    obj->class_p = obj->class_p->base_class;
