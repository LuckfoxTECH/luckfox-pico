...
static bool get_prop_core(const lv_obj_t *obj, lv_part_t part,
                          lv_style_prop_t prop, lv_style_value_t *v);
static lv_style_value_t apply_color_filter(const lv_obj_t *obj, uint32_t part,
                                           lv_style_value_t v);
static void report_style_change_core(void *style, lv_obj_t *obj);
static void refresh_children_style(lv_obj_t *obj);
static bool trans_del(lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop,
                      trans_t *tr_limit);
...
void lv_obj_add_style(lv_obj_t *obj, lv_style_t *style,
                      lv_style_selector_t selector) {
  ...
  /*Allocate space for the new style and shift the rest of the style to the
   * end*/
  obj->style_cnt++;
  obj->styles =
      lv_mem_realloc(obj->styles, obj->style_cnt * sizeof(_lv_obj_style_t));
...
void lv_obj_remove_style(lv_obj_t *obj, lv_style_t *style,
                         lv_style_selector_t selector) {
  ...
  while (i < obj->style_cnt) {
    lv_state_t state_act =
        lv_obj_style_get_selector_state(obj->styles[i].selector);
    lv_part_t part_act =
        lv_obj_style_get_selector_part(obj->styles[i].selector);
    ...
    deleted = true;
    /*The style from the current `i` index is removed, so `i` points to the next
     *style. Therefore it doesn't needs to be incremented*/
  }
...
void lv_obj_refresh_style(lv_obj_t *obj, lv_style_selector_t selector,
                          lv_style_prop_t prop) {
  ...
  if (prop == LV_STYLE_PROP_ANY || ((prop & LV_STYLE_PROP_INHERIT) &&
                                    ((prop & LV_STYLE_PROP_EXT_DRAW) ||
                                     (prop & LV_STYLE_PROP_LAYOUT_REFR)))) {
...
void lv_obj_enable_style_refresh(bool en) { style_refr = en; }

lv_style_value_t lv_obj_get_style_prop(const lv_obj_t *obj, lv_part_t part,
                                       lv_style_prop_t prop) {
  ...
  if (!found) {
    if (part == LV_PART_MAIN &&
        (prop == LV_STYLE_WIDTH || prop == LV_STYLE_HEIGHT)) {
  ...
  if (filter)
    value_act = apply_color_filter(obj, part, value_act);
  return value_act;
}
...
lv_res_t lv_obj_get_local_style_prop(lv_obj_t *obj, lv_style_prop_t prop,
                                     lv_style_value_t *value,
                                     lv_style_selector_t selector) {
  uint32_t i;
  for (i = 0; i < obj->style_cnt; i++) {
...
void _lv_obj_style_create_transition(
    lv_obj_t *obj, lv_part_t part, lv_state_t prev_state, lv_state_t new_state,
  ...
  if (v1.ptr == v2.ptr && v1.num == v2.num && v1.color.full == v2.color.full)
    return;
  obj->state = prev_state;
  ...
  lv_style_set_prop(style_trans->style, tr_dsc->prop,
                    v1); /*Be sure `trans_style` has a valid value*/
...
_lv_style_state_cmp_t _lv_obj_style_state_compare(lv_obj_t *obj,
                                                  lv_state_t state1,
                                                  lv_state_t state2) {
  ...
  for (i = 0; i < obj->style_cnt; i++) {
    ...
    if (valid1 != valid2) {
      ...
      lv_style_value_t v;
      /*If there is layout difference on the main part, return immediately.
       * There is no more serious difference*/
      ...
      if (lv_style_get_prop(style, LV_STYLE_PAD_TOP, &v))
        ...
        layout_diff = true;
      else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_ANGLE, &v))
        layout_diff = true;
      else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_ZOOM, &v))
        layout_diff = true;
      ...
      if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_WIDTH, &v))
        ...
        res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
      else if (lv_style_get_prop(style, LV_STYLE_SHADOW_OFS_X, &v))
        res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
      else if (lv_style_get_prop(style, LV_STYLE_SHADOW_OFS_Y, &v))
        res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
      else if (lv_style_get_prop(style, LV_STYLE_SHADOW_SPREAD, &v))
        res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
      else if (lv_style_get_prop(style, LV_STYLE_LINE_WIDTH, &v))
        res = _LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
      else if (res == _LV_STYLE_STATE_CMP_SAME)
...
lv_text_align_t lv_obj_calculate_style_text_align(const struct _lv_obj_t *obj,
                                                  lv_part_t part,
                                                  const char *txt) {
 ...
 * If the local style for the part-state pair doesn't exist allocate and return
 * it.
 * @param obj pointer to an object
 * @param selector OR-ed value of parts and state for which the style should be
 * get
 * @return pointer to the local style
 */
static lv_style_t *get_local_style(lv_obj_t *obj,
                                   lv_style_selector_t selector) {
  uint32_t i;
  for (i = 0; i < obj->style_cnt; i++) {
    if (obj->styles[i].is_local && obj->styles[i].selector == selector) {
 ...
 * If the transition style for the part-state pair doesn't exist allocate and
 * return it.
 * @param obj   pointer to an object
 * @param selector OR-ed value of parts and state for which the style should be
 * get
 ...
 */
static _lv_obj_style_t *get_trans_style(lv_obj_t *obj,
                                        lv_style_selector_t selector) {
  uint32_t i;
  for (i = 0; i < obj->style_cnt; i++) {
...
static bool get_prop_core(const lv_obj_t *obj, lv_part_t part,
  ...
  for (; i < obj->style_cnt; i++) {
     ...
     *E.g. For HOVER+PRESS object state, HOVER style only is OK, but HOVER+FOCUS
     *style is not*/
 ...
 * style is found because the NULL styles are inherited from the parent
 * @param obj pointer to an object
 */
static void refresh_children_style(lv_obj_t *obj) {
  uint32_t i;
...
static bool trans_del(lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop,
  ...
  while (tr != NULL) {
    ...
    if (tr->obj == obj && (part == tr->selector || part == LV_PART_ANY) &&
        (prop == tr->prop || prop == LV_STYLE_PROP_ANY)) {
      /*Remove the transitioned property from trans. style
       *to allow changing it by normal styles*/
      uint32_t i;
      for (i = 0; i < obj->style_cnt; i++) {
        if (obj->styles[i].is_trans &&
            (part == LV_PART_ANY || obj->styles[i].selector == part)) {
  ...
  for (i = 0; i < obj->style_cnt; i++) {
    ...
    case LV_STYLE_IMG_RECOLOR:
      if (v <= 0)
        ...
        value_final.color =
            lv_color_mix(tr->end_value.color, tr->start_value.color, v);
      break;
    ...
    default:
      if (v == 0)
        ...
        value_final.num =
            tr->start_value.num +
            ((int32_t)((int32_t)(tr->end_value.num - tr->start_value.num) *
                       v) >>
             8);
      break;
    }
    ...
    if (lv_style_get_prop(obj->styles[i].style, tr->prop, &old_value)) {
      if (value_final.ptr == old_value.ptr &&
          value_final.color.full == old_value.color.full &&
...
static void trans_anim_start_cb(lv_anim_t *a) {
  ...
  /*Init prop to an invalid values to be sure `trans_del` won't delete this
   * added `tr`*/
...
static void trans_anim_ready_cb(lv_anim_t *a) {
  ...
  _LV_LL_READ(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr_i) {
    if (tr_i != tr && tr_i->obj == tr->obj && tr_i->selector == tr->selector &&
        tr_i->prop == tr->prop) {
      running = true;
      break;
    }
