...
void lv_disp_set_theme(lv_disp_t *disp, lv_theme_t *th) {
  ...
  if (disp->screen_cnt == 3 && lv_obj_get_child_cnt(disp->screens[0]) == 0 &&
...
void lv_disp_set_bg_color(lv_disp_t *disp, lv_color_t color) {
  ...
  lv_area_t a;
  lv_area_set(&a, 0, 0, lv_disp_get_hor_res(disp) - 1,
              lv_disp_get_ver_res(disp) - 1);
  _lv_inv_area(disp, &a);
}
 ...
 */
void lv_scr_load_anim(lv_obj_t *new_scr, lv_scr_load_anim_t anim_type,
...
static void set_x_anim(void *obj, int32_t v) { lv_obj_set_x(obj, v); }
