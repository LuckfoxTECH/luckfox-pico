...
typedef enum {
  ...
  LV_EVENT_REFR_EXT_DRAW_SIZE, /**< Get the required extra draw area around the
                                  ...
                                  is `lv_coord_t *` to store the size.*/
  ...
  LV_EVENT_DRAW_POST_END, /**< Finishing the post draw phase (when all children
                             are drawn)*/
  LV_EVENT_DRAW_PART_BEGIN, /**< Starting to draw a part. The event parameter is
                               `lv_obj_draw_dsc_t *`. */
  LV_EVENT_DRAW_PART_END, /**< Finishing to draw a part. The event parameter is
                             `lv_obj_draw_dsc_t *`. */
  ...
  LV_EVENT_PREPROCESS =
      0x80, /** This is a flag that can be set with an event so it's processed
...
/**
 * Used as the event parameter of ::LV_EVENT_HIT_TEST to check if an `point` can
 * click the object or not. `res` should be set like this:
 *   - If already set to `false` an other event wants that point non clickable.
 * If you want to respect it leave it as `false` or set `true` to overwrite it.
 *   - If already set `true` and `point` shouldn't be clickable set to `false`
 *   - If already set to `true` you agree that `point` can click the object
 * leave it as `true`
 */
typedef struct {
  const lv_point_t *point; /**< A point relative to screen to check if it can
                              click the object or not*/
  bool res; /**< true: `point` can click the object; false: it cannot*/
} lv_hit_test_info_t;

/**
 * Used as the event parameter of ::LV_EVENT_COVER_CHECK to check if an area is
 * covered by the object or not. In the event use `const lv_area_t * area =
 * lv_event_get_cover_area(e)` to get the area to check and
 * `lv_event_set_cover_res(e, res)` to set the result.
 */
typedef struct {
  lv_cover_res_t res;
 ...
 * @param param         arbitrary data depending on the widget type and the
 * event. (Usually `NULL`)
 * @return LV_RES_OK: `obj` was not deleted in the event; LV_RES_INV: `obj` was
 * deleted in the event_code
 */
lv_res_t lv_event_send(struct _lv_obj_t *obj, lv_event_code_t event_code,
                       void *param);

/**
 * Used by the widgets internally to call the ancestor widget types's event
 * handler
 * @param class_p   pointer to the class of the widget (NOT the ancestor class)
 * @param e         pointer to the event descriptor
 * @return          LV_RES_OK: the target object was not deleted in the event;
 * LV_RES_INV: it was deleted in the event_code
 */
lv_res_t lv_obj_event_base(const lv_obj_class_t *class_p, lv_event_t *e);

/**
 * Get the object originally targeted by the event. It's the same even if the
 * event is bubbled.
 * @param e     pointer to the event descriptor
...
/**
 * Nested events can be called and one of them might belong to an object that is
 * being deleted. Mark this object's `event_temp_data` deleted to know that its
 * `lv_event_send` should return `LV_RES_INV`
 ...
 * @param obj       pointer to an object
 * @param filter    and event code (e.g. `LV_EVENT_CLICKED`) on which the event
 * should be called. `LV_EVENT_ALL` can be sued the receive all the events.
 * @param event_cb  the new event function
 * @param           user_data custom data data will be available in `event_cb`
 * @return          a pointer the event descriptor. Can be used in
 ...
 * @param obj       pointer to an object
 * @param event_cb  the event function to remove, or `NULL` to remove the
 * firstly added event callback
 ...
 * @param obj               pointer to an object
 * @param event_cb          the event function to remove, or `NULL` only
 * `user_data` matters.
 * @param event_user_data   the user_data specified in ::lv_obj_add_event_cb
 * @return                  true if any event handlers were removed
 */
bool lv_obj_remove_event_cb_with_user_data(struct _lv_obj_t *obj,
                                           lv_event_cb_t event_cb,
                                           const void *event_user_data);

/**
 * DEPRECATED because doesn't work if multiple event handlers are added to an
 * object. Remove an event handler function for an object.
 * @param obj       pointer to an object
 * @param event_dsc pointer to an event descriptor to remove (returned by
 * ::lv_obj_add_event_cb)
 * @return          true if any event handlers were removed
 */
bool lv_obj_remove_event_dsc(struct _lv_obj_t *obj,
                             struct _lv_event_dsc_t *event_dsc);

/**
 * The user data of an event object event callback. Always the first match with
 * `event_cb` will be returned.
 * @param obj               pointer to an object
 ...
 * @return      the indev that triggered the event or NULL if called on a not
 * indev related event
 */
 ...
 * @param e     pointer to an event
 * @return      the part draw descriptor to hook the drawing or NULL if called
 * on an unrelated event
 */
lv_obj_draw_part_dsc_t *lv_event_get_draw_part_dsc(lv_event_t *e);

/**
 * Get the draw context which should be the first parameter of the draw
 * functions. Namely: `LV_EVENT_DRAW_MAIN/POST`,
 * `LV_EVENT_DRAW_MAIN/POST_BEGIN`, `LV_EVENT_DRAW_MAIN/POST_END`
 * @param e     pointer to an event
 * @return      pointer to a draw context or NULL if called on an unrelated
 * event
 */
 ...
 * @param e     pointer to an event
 * @return      the old absolute area of the object or NULL if called on an
 * unrelated event
 */
 ...
 * @param e     pointer to an event
 * @return      the animation that will scroll the object. (can be modified as
 * required)
 */
...
/**
 * Get a pointer to an `lv_point_t` variable in which the self size should be
 * saved (width in `point->x` and height `point->y`). Can be used in
...
/**
 * Get a pointer to an `lv_hit_test_info_t` variable in which the hit test
 * result should be saved. Can be used in `LV_EVENT_HIT_TEST`
 * @param e     pointer to an event
 * @return      pointer to `lv_hit_test_info_t` or NULL if called on an
 * unrelated event
 */
lv_hit_test_info_t *lv_event_get_hit_test_info(lv_event_t *e);

/**
 * Get a pointer to an area which should be examined whether the object fully
 * covers it or not. Can be used in `LV_EVENT_HIT_TEST`
 * @param e     pointer to an event
