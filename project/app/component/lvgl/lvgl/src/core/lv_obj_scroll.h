...
enum {
  ...
  LV_SCROLLBAR_MODE_ACTIVE, /**< Show scroll bars when object is being
                               scrolled*/
  LV_SCROLLBAR_MODE_AUTO, /**< Show scroll bars when the content is large enough
                             to be scrolled*/
};
 ...
 * @param obj       pointer to an object
 * @param dir       the allow scroll directions. An element or OR-ed values of
 * `lv_dir_t`
 */
...
/**
 * Get the X and Y coordinates where the scrolling will end for this object if a
 * scrolling animation is in progress. If no scrolling animation, give the
 * current `x` or `y` scroll position.
 * @param obj       pointer to an object
 ...
 * @param anim_en   LV_ANIM_ON: scroll with animation; LV_ANIM_OFF: scroll
 * immediately
 * @note            > 0 value means scroll right/bottom (show the more content
 * on the right/bottom)
 * @note            e.g. dy = -20 means scroll down 20 px
 */
void lv_obj_scroll_by(struct _lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                      lv_anim_enable_t anim_en);

/**
 * Scroll by a given amount of pixels.
 * `dx` and `dy` will be limited internally to allow scrolling only on the
 * content area.
 * @param obj       pointer to an object to scroll
 * @param dx        pixels to scroll horizontally
 * @param dy        pixels to scroll vertically
 ...
 * @note            e.g. dy = -20 means scroll down 20 px
 */
void lv_obj_scroll_by_bounded(struct _lv_obj_t *obj, lv_coord_t dx,
                              lv_coord_t dy, lv_anim_enable_t anim_en);

/**
 * Scroll to a given coordinate on an object.
 * `x` and `y` will be limited internally to allow scrolling only on the content
 * area.
 * @param obj       pointer to an object to scroll
 ...
 * immediately
 */
void lv_obj_scroll_to(struct _lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                      lv_anim_enable_t anim_en);
 ...
 * immediately
 */
void lv_obj_scroll_to_x(struct _lv_obj_t *obj, lv_coord_t x,
                        lv_anim_enable_t anim_en);
 ...
 * immediately
 */
void lv_obj_scroll_to_y(struct _lv_obj_t *obj, lv_coord_t y,
                        lv_anim_enable_t anim_en);
 ...
 * Therefore the object will be scrolled into view even it has nested scrollable
 * parents
 * @param obj       pointer to an object to scroll into view
 * @param anim_en   LV_ANIM_ON: scroll with animation; LV_ANIM_OFF: scroll
 * immediately
 */
void lv_obj_scroll_to_view_recursive(struct _lv_obj_t *obj,
                                     lv_anim_enable_t anim_en);
...
/**
 * Check the children of `obj` and scroll `obj` to fulfill the scroll_snap
 * settings
 ...
 */
void lv_obj_get_scrollbar_area(struct _lv_obj_t *obj, lv_area_t *hor,
                               lv_area_t *ver);
