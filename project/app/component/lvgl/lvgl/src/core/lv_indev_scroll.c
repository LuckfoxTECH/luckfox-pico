 ...
 *      DEFINES
 *********************/
#define ELASTIC_SLOWNESS_FACTOR                                                \
  4 /*Scrolling on elastic parts are slower by this factor*/
...
static lv_coord_t find_snap_point_x(const lv_obj_t *obj, lv_coord_t min,
                                    lv_coord_t max, lv_coord_t ofs);
static lv_coord_t find_snap_point_y(const lv_obj_t *obj, lv_coord_t min,
                                    lv_coord_t max, lv_coord_t ofs);
static void scroll_limit_diff(_lv_indev_proc_t *proc, lv_coord_t *diff_x,
                              lv_coord_t *diff_y);
...
static lv_coord_t elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff,
                               lv_coord_t scroll_start, lv_coord_t scroll_end,
                               lv_dir_t dir);
...
void _lv_indev_scroll_throw_handler(_lv_indev_proc_t *proc) {
  ...
  if (proc->types.pointer.scroll_dir == LV_DIR_VER) {
    ...
    if (align_y == LV_SCROLL_SNAP_NONE) {
      ...
      proc->types.pointer.scroll_throw_vect.y =
          elastic_diff(scroll_obj, proc->types.pointer.scroll_throw_vect.y, st,
                       sb, LV_DIR_VER);

      lv_obj_scroll_by(scroll_obj, 0, proc->types.pointer.scroll_throw_vect.y,
                       LV_ANIM_OFF);
    }
    /*With snapping find the nearest snap point and scroll there*/
      ...
      lv_coord_t y =
          find_snap_point_y(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, diff_y);
    ...
    if (align_x == LV_SCROLL_SNAP_NONE) {
      ...
      proc->types.pointer.scroll_throw_vect.x =
          elastic_diff(scroll_obj, proc->types.pointer.scroll_throw_vect.x, sl,
                       sr, LV_DIR_HOR);

      lv_obj_scroll_by(scroll_obj, proc->types.pointer.scroll_throw_vect.x, 0,
                       LV_ANIM_OFF);
    }
    /*With snapping find the nearest snap point and scroll there*/
    else {
      ...
      lv_coord_t x =
          find_snap_point_x(scroll_obj, LV_COORD_MIN, LV_COORD_MAX, diff_x);
      lv_obj_scroll_by(scroll_obj, x + diff_x, 0, LV_ANIM_ON);
    }
  }

  /*Check if the scroll has finished*/
  if (proc->types.pointer.scroll_throw_vect.x == 0 &&
      proc->types.pointer.scroll_throw_vect.y == 0) {
 ...
 * @return the difference compared to the current position when the throw would
 * be finished
 */
...
static lv_obj_t *find_scroll_obj(_lv_indev_proc_t *proc) {
   ...
   *More precisely:
   * 1. Check the pressed object and all of its ancestors and try to find an
   *object which is scrollable
   * 2. Scrollable means it has some content out of its area
   * 3. If an object can be scrolled into the current direction then use it
   *("real match"")
   * 4. If can be scrolled on the current axis (hor/ver) save it as candidate
   *(at least show an elastic scroll effect)
   * 5. Use the last candidate. Always the "deepest" parent or the object from
   *point 3*/
  ...
  bool hor_en = false;
  bool ver_en = false;
  if (LV_ABS(proc->types.pointer.scroll_sum.x) >
      LV_ABS(proc->types.pointer.scroll_sum.y)) {
    hor_en = true;
  } else {
    ver_en = true;
  }
  ...
  while (obj_act) {
    if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLLABLE) == false) {
      /*If this object don't want to chain the scroll to the parent stop
       * searching*/
      if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_HOR) == false &&
          hor_en)
        break;
      if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_VER) == false &&
          ver_en)
        break;
    ...
    /*Consider both up-down or left/right scrollable according to the current
     * direction*/
    bool up_en = ver_en;
    bool down_en = ver_en;
    bool left_en = hor_en;
    ...
    /*The object is scrollable to a direction if its content overflow in that
     * direction.*/
    ...
    /*If this object is scrollable into the current scroll direction then save
     *it as a candidate. It's important only to be scrollable on the current
     *axis (hor/ver) because if the scroll is propagated to this object it can
     *show at least elastic scroll effect. But if not hor/ver scrollable do not
     *scroll it at all (so it's not a good candidate)*/
    if ((st > 0 || sb > 0) &&
    ...
    /*If the object really can be scrolled into the current direction the use
     * it.*/
...
static void init_scroll_limits(_lv_indev_proc_t *proc) {
  ...
  if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLL_ONE) == false) {
    lv_area_set(&proc->types.pointer.scroll_area, LV_COORD_MIN, LV_COORD_MIN,
  ...
  /*With STOP limit the scrolling to the perv and next snap point*/
    ...
    case LV_SCROLL_SNAP_START:
      proc->types.pointer.scroll_area.y1 =
          find_snap_point_y(obj, obj->coords.y1 + 1, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.y2 =
          find_snap_point_y(obj, LV_COORD_MIN, obj->coords.y1 - 1, 0);
      break;
    case LV_SCROLL_SNAP_END:
      proc->types.pointer.scroll_area.y1 =
          find_snap_point_y(obj, obj->coords.y2, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.y2 =
          find_snap_point_y(obj, LV_COORD_MIN, obj->coords.y2, 0);
      break;
    case LV_SCROLL_SNAP_CENTER: {
      lv_coord_t y_mid = obj->coords.y1 + lv_area_get_height(&obj->coords) / 2;
      proc->types.pointer.scroll_area.y1 =
          find_snap_point_y(obj, y_mid + 1, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.y2 =
          find_snap_point_y(obj, LV_COORD_MIN, y_mid - 1, 0);
      break;
    }
    default:
    ...
    case LV_SCROLL_SNAP_START:
      proc->types.pointer.scroll_area.x1 =
          find_snap_point_x(obj, obj->coords.x1, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.x2 =
          find_snap_point_x(obj, LV_COORD_MIN, obj->coords.x1, 0);
      break;
    case LV_SCROLL_SNAP_END:
      proc->types.pointer.scroll_area.x1 =
          find_snap_point_x(obj, obj->coords.x2, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.x2 =
          find_snap_point_x(obj, LV_COORD_MIN, obj->coords.x2, 0);
      break;
    case LV_SCROLL_SNAP_CENTER: {
      lv_coord_t x_mid = obj->coords.x1 + lv_area_get_width(&obj->coords) / 2;
      proc->types.pointer.scroll_area.x1 =
          find_snap_point_x(obj, x_mid + 1, LV_COORD_MAX, 0);
      proc->types.pointer.scroll_area.x2 =
          find_snap_point_x(obj, LV_COORD_MIN, x_mid - 1, 0);
      break;
    }
    default:
  ...
  /*Allow scrolling on the edges. It will be reverted to the edge due to
   * snapping anyway*/
 ...
 * @param ofs offset to snap points. Useful the get a snap point in an imagined
 * case what if children are already moved by this value
  ...
  for (i = 0; i < child_cnt; i++) {
    ...
    if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
      ...
      case LV_SCROLL_SNAP_CENTER:
        ...
        x_parent = obj->coords.x1 + pad_left +
                   (lv_area_get_width(&obj->coords) - pad_left - pad_right) / 2;
        break;
      default:
        continue;
      }
 ...
 * @param min ignore snap points smaller than this. (Absolute coordinate)
 * @param max ignore snap points greater than this. (Absolute coordinate)
 * @param ofs offset to snap points. Useful to get a snap point in an imagined
 * case what if children are already moved by this value
 * @return the distance of the snap point.
 */
static lv_coord_t find_snap_point_y(const lv_obj_t *obj, lv_coord_t min,
  ...
  for (i = 0; i < child_cnt; i++) {
    ...
    if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
      ...
      case LV_SCROLL_SNAP_CENTER:
        ...
        y_parent =
            obj->coords.y1 + pad_top +
            (lv_area_get_height(&obj->coords) - pad_top - pad_bottom) / 2;
        break;
      default:
        continue;
      }
...
static void scroll_limit_diff(_lv_indev_proc_t *proc, lv_coord_t *diff_x,
                              lv_coord_t *diff_y) {
  if (diff_y) {
    if (proc->types.pointer.scroll_sum.y + *diff_y <
        proc->types.pointer.scroll_area.y1) {
      *diff_y =
          proc->types.pointer.scroll_area.y1 - proc->types.pointer.scroll_sum.y;
    }

    if (proc->types.pointer.scroll_sum.y + *diff_y >
        proc->types.pointer.scroll_area.y2) {
      *diff_y =
          proc->types.pointer.scroll_area.y2 - proc->types.pointer.scroll_sum.y;
    }
  }

  if (diff_x) {
    if (proc->types.pointer.scroll_sum.x + *diff_x <
        proc->types.pointer.scroll_area.x1) {
      *diff_x =
          proc->types.pointer.scroll_area.x1 - proc->types.pointer.scroll_sum.x;
    }

    if (proc->types.pointer.scroll_sum.x + *diff_x >
        proc->types.pointer.scroll_area.x2) {
      *diff_x =
          proc->types.pointer.scroll_area.x2 - proc->types.pointer.scroll_sum.x;
    }
  }
}
...
static lv_coord_t elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff,
  ...
  if (lv_obj_has_flag(scroll_obj, LV_OBJ_FLAG_SCROLL_ELASTIC)) {
     ...
     *because it's natural that the first and last items are scrolled (snapped)
     *in.*/
    lv_scroll_snap_t snap;
    snap = dir == LV_DIR_HOR ? lv_obj_get_scroll_snap_x(scroll_obj)
                             : lv_obj_get_scroll_snap_y(scroll_obj);
    ...
    if (dir == LV_DIR_HOR) {
      ...
      lv_coord_t pad_right =
          lv_obj_get_style_pad_right(scroll_obj, LV_PART_MAIN);

      switch (snap) {
      case LV_SCROLL_SNAP_CENTER:
        snap_point =
            pad_left +
            (lv_area_get_width(&scroll_obj->coords) - pad_left - pad_right) /
                2 +
            scroll_obj->coords.x1;
        act_obj_point =
            lv_area_get_width(&act_obj->coords) / 2 + act_obj->coords.x1;
        break;
      case LV_SCROLL_SNAP_START:
      ...
      lv_coord_t pad_bottom =
          lv_obj_get_style_pad_bottom(scroll_obj, LV_PART_MAIN);

      switch (snap) {
      case LV_SCROLL_SNAP_CENTER:
        snap_point =
            pad_top +
            (lv_area_get_height(&scroll_obj->coords) - pad_top - pad_bottom) /
                2 +
            scroll_obj->coords.y1;
        act_obj_point =
            lv_area_get_height(&act_obj->coords) / 2 + act_obj->coords.y1;
        break;
      case LV_SCROLL_SNAP_START:
