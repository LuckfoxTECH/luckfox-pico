...
lv_indev_t *lv_indev_get_act(void) { return indev_act; }
...
void lv_indev_reset(lv_indev_t *indev, lv_obj_t *obj) {
  if (indev) {
    ...
    if (indev->driver->type == LV_INDEV_TYPE_POINTER ||
        indev->driver->type == LV_INDEV_TYPE_KEYPAD) {
    ...
    while (i) {
      ...
      if (i->driver->type == LV_INDEV_TYPE_POINTER ||
          i->driver->type == LV_INDEV_TYPE_KEYPAD) {
...
void lv_indev_get_point(const lv_indev_t *indev, lv_point_t *point) {
  ...
  if (indev->driver->type != LV_INDEV_TYPE_POINTER &&
      indev->driver->type != LV_INDEV_TYPE_BUTTON) {
    point->x = -1;
    point->y = -1;
  } else {
...
lv_obj_t *lv_indev_get_obj_act(void) { return indev_obj_act; }
...
lv_obj_t *lv_indev_search_obj(lv_obj_t *obj, lv_point_t *point) {
  ...
  /*If the point is on this object or has overflow visible check its children
   * too*/
  ...
  /*If not return earlier for a clicked child and this obj's hittest was ok use
   *it else return NULL*/
  if (hit_test_ok)
    return obj;
  else
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  if (i->proc.wait_until_release) {
    ...
    i->proc.types.keypad.last_state =
        LV_INDEV_STATE_RELEASED; /*To skip the processing of release*/
  }
  ...
  /*Save the last key.
   *It must be done here else `lv_indev_get_key` will return the last key in
   *events*/
  i->proc.types.keypad.last_key = data->key;

  /*Save the previous state so we can detect state changes below and also set
   *the last state now so if any event handler on the way returns `LV_RES_INV`
   *the last state is remembered for the next time*/
  uint32_t prev_state = i->proc.types.keypad.last_state;
  i->proc.types.keypad.last_state = data->state;

  /*Key press happened*/
  if (data->state == LV_INDEV_STATE_PRESSED &&
      prev_state == LV_INDEV_STATE_RELEASED) {
  ...
  /*Pressing*/
           ...
           prev_state == LV_INDEV_STATE_PRESSED) {
    ...
    /*Long press repeated time has elapsed?*/
             ...
             lv_tick_elaps(i->proc.longpr_rep_timestamp) >
                 i->driver->long_press_repeat_time) {
  ...
  /*Release happened*/
    ...
    /*The user might clear the key when it was released. Always release the
     * pressed key*/
    data->key = prev_key;
    if (data->key == LV_KEY_ENTER) {
...
static void indev_encoder_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  if (data->state == LV_INDEV_STATE_PRESSED &&
    ...
    if (data->key == LV_KEY_ENTER) {
      bool editable_or_scrollable =
          lv_obj_is_editable(indev_obj_act) ||
    ...
    if (i->proc.long_pr_sent == 0 &&
      ...
      if (data->key == LV_KEY_ENTER) {
        ...
        if (editable_or_scrollable) {
          /*Don't leave edit mode if there is only one object (nowhere to
           * navigate)*/
          if (lv_group_get_obj_count(g) > 1) {
            LV_LOG_INFO("toggling edit mode");
            lv_group_set_editing(g,
                                 lv_group_get_editing(g)
                                     ? false
                                     : true); /*Toggle edit mode on long press*/
            lv_obj_clear_state(
                indev_obj_act,
                LV_STATE_PRESSED); /*Remove the pressed state manually*/
          }
        }
        /*If not editable then just send a long press Call the ancestor's event
           handler*/
        else {
          lv_event_send(indev_obj_act, LV_EVENT_LONG_PRESSED, indev_act);
          if (indev_reset_check(&i->proc))
  ...
  /*Release happened*/
    ...
    if (data->key == LV_KEY_ENTER) {
      ...
      /*If the focused object is editable and now in navigate mode then on enter
         switch edit mode*/
 ...
 */
static void indev_proc_press(_lv_indev_proc_t *proc) {
  LV_LOG_INFO("pressed at x:%d y:%d", proc->types.pointer.act_point.x,
              proc->types.pointer.act_point.y);
  ...
  if (indev_obj_act == NULL) {
    indev_obj_act = lv_indev_search_obj(lv_disp_get_layer_sys(disp),
                                        &proc->types.pointer.act_point);
    if (indev_obj_act == NULL)
      indev_obj_act = lv_indev_search_obj(lv_disp_get_layer_top(disp),
                                          &proc->types.pointer.act_point);
    if (indev_obj_act == NULL)
  ...
  /*If there is last object but it is not scrolled and not protected also
     search*/
  ...
  /*If a new object was found reset some variables and send a pressed Call the
   * ancestor's event handler*/
  if (indev_obj_act != proc->types.pointer.act_obj) {
    proc->types.pointer.last_point.x = proc->types.pointer.act_point.x;
    proc->types.pointer.last_point.y = proc->types.pointer.act_point.y;

    /*If a new object found the previous was lost, so send a Call the ancestor's
     * event handler*/
    if (proc->types.pointer.act_obj != NULL) {
      /*Save the obj because in special cases `act_obj` can change in the Call
       * the ancestor's event handler function*/
  ...
  /*Calculate the vector and apply a low pass filter: new value = 0.5 *
   * old_value + 0.5 * new_value*/
  proc->types.pointer.vect.x =
      proc->types.pointer.act_point.x - proc->types.pointer.last_point.x;
  proc->types.pointer.vect.y =
      proc->types.pointer.act_point.y - proc->types.pointer.last_point.y;

  proc->types.pointer.scroll_throw_vect.x =
      (proc->types.pointer.scroll_throw_vect.x * 4) >> 3;
  proc->types.pointer.scroll_throw_vect.y =
      (proc->types.pointer.scroll_throw_vect.y * 4) >> 3;

  proc->types.pointer.scroll_throw_vect.x +=
      (proc->types.pointer.vect.x * 4) >> 3;
  proc->types.pointer.scroll_throw_vect.y +=
      (proc->types.pointer.vect.y * 4) >> 3;

  proc->types.pointer.scroll_throw_vect_ori =
      proc->types.pointer.scroll_throw_vect;
  ...
  if (indev_obj_act) {
    ...
    if (proc->types.pointer.scroll_obj == NULL && proc->long_pr_sent == 0) {
      /*Call the ancestor's event handler about the long press if enough time
       * elapsed*/
      if (lv_tick_elaps(proc->pr_timestamp) >
        ...
        /*Mark the Call the ancestor's event handler sending to do not send it
         * again*/
        proc->long_pr_sent = 1;
    ...
    if (proc->types.pointer.scroll_obj == NULL && proc->long_pr_sent == 1) {
      /*Call the ancestor's event handler about the long press repeat if enough
       * time elapsed*/
      if (lv_tick_elaps(proc->longpr_rep_timestamp) >
          indev_act->driver->long_press_repeat_time) {
        lv_event_send(indev_obj_act, LV_EVENT_LONG_PRESSED_REPEAT, indev_act);
        if (indev_reset_check(proc))
          return;
        proc->longpr_rep_timestamp = lv_tick_get();
...
static void indev_click_focus(_lv_indev_proc_t *proc) {
  ...
  lv_group_t *g_prev = proc->types.pointer.last_pressed
                           ? lv_obj_get_group(proc->types.pointer.last_pressed)
                           : NULL;
  ...
  if (g_act == g_prev) {
    ...
    /*The object are not in group*/
      ...
      if (proc->types.pointer.last_pressed) {
        lv_event_send(proc->types.pointer.last_pressed, LV_EVENT_DEFOCUSED,
                      indev_act);
        if (indev_reset_check(proc))
          return;
      }
  ...
  /*The object are not in the same group (in different groups or one has no
     group)*/
  else {
    ...
    /*Focus on a non-group object*/
      ...
      if (proc->types.pointer.last_pressed) {
        ...
        /*If the prev. object also was in a group at least "LEAVE" it instead of
           defocus*/
        else {
          lv_event_send(proc->types.pointer.last_pressed, LV_EVENT_LEAVE,
                        indev_act);
          if (indev_reset_check(proc))
            return;
        }
...
void indev_gesture(_lv_indev_proc_t *proc) {
  ...
  while (gesture_obj &&
         lv_obj_has_flag(gesture_obj, LV_OBJ_FLAG_GESTURE_BUBBLE)) {
  ...
  if ((LV_ABS(proc->types.pointer.vect.x) <
       indev_act->driver->gesture_min_velocity) &&
      (LV_ABS(proc->types.pointer.vect.y) <
       indev_act->driver->gesture_min_velocity)) {
    proc->types.pointer.gesture_sum.x = 0;
    proc->types.pointer.gesture_sum.y = 0;
  }
  ...
  if ((LV_ABS(proc->types.pointer.gesture_sum.x) >
       indev_act->driver->gesture_limit) ||
      (LV_ABS(proc->types.pointer.gesture_sum.y) >
       indev_act->driver->gesture_limit)) {
...
/**
 * Checks if the reset_query flag has been set. If so, perform necessary global
 * indev cleanup actions
 * @param proc pointer to an input device 'proc'
