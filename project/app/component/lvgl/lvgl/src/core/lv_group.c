...
void lv_group_set_default(lv_group_t *group) { default_group = group; }

lv_group_t *lv_group_get_default(void) { return default_group; }
...
void lv_group_add_obj(lv_group_t *group, lv_obj_t *obj) {
  ...
  if (group_cur) {
    if (obj->spec_attr->group_p &&
        *(obj->spec_attr->group_p->obj_focus) == obj) {
  ...
  *next = obj;
   ...
   *linked list. In this case automatically activate it*/
...
void lv_group_remove_obj(lv_obj_t *obj) {
  ...
  if (g->obj_focus && *g->obj_focus == obj) {
    ...
    if (_lv_ll_get_head(&g->obj_ll) == g->obj_focus &&
        _lv_ll_get_tail(&g->obj_ll) == g->obj_focus) {
  ...
  /*If the focuses object is still the same then it was the only object in the
   *group but it will be deleted. Set the `obj_focus` to NULL to get back to the
   *initial state of the group with zero objects*/
  if (g->obj_focus && *g->obj_focus == obj) {
    g->obj_focus = NULL;
  }
...
void lv_group_focus_obj(lv_obj_t *obj) {
  ...
  _LV_LL_READ(&g->obj_ll, i) {
    if (*i == obj) {
      if (g->obj_focus != NULL &&
          obj != *g->obj_focus) { /*Do not defocus if the same object needs to
                                     be focused again*/
        lv_res_t res =
            lv_event_send(*g->obj_focus, LV_EVENT_DEFOCUSED, get_indev(g));
        if (res != LV_RES_OK)
      ...
      if (g->obj_focus != NULL) {
        if (g->focus_cb)
          g->focus_cb(g);
        lv_res_t res =
            lv_event_send(*g->obj_focus, LV_EVENT_FOCUSED, get_indev(g));
        if (res != LV_RES_OK)
          return;
        lv_obj_invalidate(*g->obj_focus);
      }
...
void lv_group_set_editing(lv_group_t *group, bool edit) {
  ...
  if (en_val == group->editing)
    return; /*Do not set the same mode again*/
  ...
  if (focused) {
    lv_res_t res =
        lv_event_send(*group->obj_focus, LV_EVENT_FOCUSED, get_indev(group));
    if (res != LV_RES_OK)
      return;
...
void lv_group_set_wrap(lv_group_t *group, bool en) { group->wrap = en ? 1 : 0; }
  ...
  for (;;) {
    if (obj_next == NULL) {
      if (group->wrap || obj_sentinel == NULL) {
        ...
        /*Currently focused object is the last/first in the group, keep it that
         * way*/
        return;
      }
    }
    ...
    if (can_move) {
      ...
      /*Give up if we walked the entire list and haven't found another visible
       * object*/
...
/**
 * Find an indev preferably with KEYPAD or ENCOEDR type that uses the given
 * group. In other words, find an indev, that is related to the given group. In
 * the worst case simply return the latest indev
...
static lv_indev_t *get_indev(const lv_group_t *g) {
  ...
  /*In lack of a better option use the first input device. (It can be NULL if
