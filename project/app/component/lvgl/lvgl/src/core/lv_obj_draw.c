...
void lv_obj_init_draw_rect_dsc(lv_obj_t *obj, uint32_t part,
  ...
  lv_opa_t main_opa =
      part != LV_PART_MAIN ? lv_obj_get_style_opa(obj, part) : LV_OPA_COVER;
    ...
    if (draw_dsc->bg_opa > LV_OPA_MIN) {
      ...
      if (grad && grad->dir != LV_GRAD_DIR_NONE) {
        ...
        if (draw_dsc->bg_grad.dir != LV_GRAD_DIR_NONE) {
          draw_dsc->bg_grad.stops[0].color =
              lv_obj_get_style_bg_color_filtered(obj, part);
          draw_dsc->bg_grad.stops[1].color =
              lv_obj_get_style_bg_grad_color_filtered(obj, part);
          draw_dsc->bg_grad.stops[0].frac =
              lv_obj_get_style_bg_main_stop(obj, part);
          draw_dsc->bg_grad.stops[1].frac =
              lv_obj_get_style_bg_grad_stop(obj, part);
        }
  ...
  if (draw_dsc->border_width) {
    if (draw_dsc->border_opa != LV_OPA_TRANSP) {
      ...
      if (draw_dsc->border_opa > LV_OPA_MIN) {
        ...
        draw_dsc->border_color =
            lv_obj_get_style_border_color_filtered(obj, part);
      }
    }
  }
  ...
  if (draw_dsc->outline_width) {
    if (draw_dsc->outline_opa != LV_OPA_TRANSP) {
      ...
      if (draw_dsc->outline_opa > LV_OPA_MIN) {
        ...
        draw_dsc->outline_color =
            lv_obj_get_style_outline_color_filtered(obj, part);
      }
    }
  }
    ...
    if (draw_dsc->bg_img_src) {
      ...
      if (draw_dsc->bg_img_opa > LV_OPA_MIN) {
        if (lv_img_src_get_type(draw_dsc->bg_img_src) == LV_IMG_SRC_SYMBOL) {
          ...
          draw_dsc->bg_img_recolor =
              lv_obj_get_style_text_color_filtered(obj, part);
        } else {
          draw_dsc->bg_img_recolor =
              lv_obj_get_style_bg_img_recolor_filtered(obj, part);
          draw_dsc->bg_img_recolor_opa =
              lv_obj_get_style_bg_img_recolor_opa(obj, part);
    ...
    if (draw_dsc->shadow_width) {
      if (draw_dsc->shadow_opa > LV_OPA_MIN) {
        ...
        if (draw_dsc->shadow_opa > LV_OPA_MIN) {
          ...
          draw_dsc->shadow_color =
              lv_obj_get_style_shadow_color_filtered(obj, part);
        }
      }
    }
  }
  ...
  if (opa < LV_OPA_MAX) {
    ...
    draw_dsc->bg_img_opa =
        (uint16_t)((uint16_t)draw_dsc->bg_img_opa * opa) >> 8;
    draw_dsc->border_opa =
        (uint16_t)((uint16_t)draw_dsc->border_opa * opa) >> 8;
    draw_dsc->shadow_opa =
        (uint16_t)((uint16_t)draw_dsc->shadow_opa * opa) >> 8;
    draw_dsc->outline_opa =
        (uint16_t)((uint16_t)draw_dsc->outline_opa * opa) >> 8;
  }
...
void lv_obj_init_draw_label_dsc(lv_obj_t *obj, uint32_t part,
...
void lv_obj_init_draw_img_dsc(lv_obj_t *obj, uint32_t part,
...
void lv_obj_init_draw_line_dsc(lv_obj_t *obj, uint32_t part,
...
void lv_obj_init_draw_arc_dsc(lv_obj_t *obj, uint32_t part,
...
void lv_obj_draw_dsc_init(lv_obj_draw_part_dsc_t *dsc,
...
bool lv_obj_draw_part_check_type(lv_obj_draw_part_dsc_t *dsc,
                                 const lv_obj_class_t *class_p, uint32_t type) {
