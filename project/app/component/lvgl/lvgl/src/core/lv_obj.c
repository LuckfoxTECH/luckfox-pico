...
bool lv_is_initialized(void) { return lv_initialized; }
...
void lv_init(void) {
  ...
  if (txt_u8[0] != 0xc3 || txt_u8[1] != 0x81 || txt_u8[2] != 0x00) {
    LV_LOG_WARN("The strings have no UTF-8 encoding. Non-ASCII characters "
                "won't be displayed.");
  }
  ...
  if (big_endian) {
    LV_ASSERT_MSG(LV_BIG_ENDIAN_SYSTEM == 1,
                  "It's a big endian system but LV_BIG_ENDIAN_SYSTEM is not "
                  "enabled in lv_conf.h");
  } else {
    LV_ASSERT_MSG(LV_BIG_ENDIAN_SYSTEM == 0,
                  "It's a little endian system but LV_BIG_ENDIAN_SYSTEM is "
                  "enabled in lv_conf.h");
  }

#if LV_USE_ASSERT_MEM_INTEGRITY
  LV_LOG_WARN("Memory integrity checks are enabled via "
              "LV_USE_ASSERT_MEM_INTEGRITY which makes LVGL much slower");
#endif

#if LV_USE_ASSERT_OBJ
  LV_LOG_WARN("Object sanity checks are enabled via LV_USE_ASSERT_OBJ which "
              "makes LVGL much slower");
#endif
...
static void lv_obj_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
  ...
  if (parent) {
    ...
    obj->coords.y1 =
        parent->coords.y1 + lv_obj_get_style_pad_top(parent, LV_PART_MAIN) - st;
    obj->coords.y2 = obj->coords.y1 - 1;
    obj->coords.x1 = parent->coords.x1 +
                     lv_obj_get_style_pad_left(parent, LV_PART_MAIN) - sl;
...
static void lv_obj_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
  ...
  /*Remove all style*/
  lv_obj_enable_style_refresh(false); /*No need to refresh the style because the
  ...
  if (code == LV_EVENT_COVER_CHECK) {
...
#if LV_DRAW_COMPLEX
    if (lv_obj_get_style_blend_mode(obj, LV_PART_MAIN) !=
    ...
    if (clip_corner) {
      lv_draw_mask_radius_param_t *mp =
          lv_mem_buf_get(sizeof(lv_draw_mask_radius_param_t));
      lv_draw_mask_radius_init(mp, &obj->coords, draw_dsc.radius, false);
      /*Add the mask and use `obj+8` as custom id. Don't use `obj` directly
       * because it might be used by the user*/
 ...
 * @return LV_RES_OK: the scrollbar is visible; LV_RES_INV: the scrollbar is not
 * visible
 */
static lv_res_t scrollbar_init_draw_dsc(lv_obj_t *obj,
  ...
  if (dsc->shadow_opa > LV_OPA_MIN) {
    ...
    if (dsc->shadow_width > 0) {
      dsc->shadow_spread =
          lv_obj_get_style_shadow_spread(obj, LV_PART_SCROLLBAR);
...
static void lv_obj_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_PRESSED) {
    ...
    if (lv_indev_get_scroll_obj(param) == NULL &&
    ...
    if (lv_obj_has_flag(obj, LV_OBJ_FLAG_CHECKABLE)) {
    ...
    } else if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLLABLE |
      ...
      if (c == LV_KEY_DOWN) {
        ...
        lv_obj_scroll_to_y(
            obj, lv_obj_get_scroll_y(obj) + lv_obj_get_height(obj) / 4,
            anim_enable);
      } else if (c == LV_KEY_UP) {
        lv_obj_scroll_to_y(
            obj, lv_obj_get_scroll_y(obj) - lv_obj_get_height(obj) / 4,
        ...
        /*If the object can't be scrolled horizontally then scroll it
         * vertically*/
        if (!((lv_obj_get_scroll_dir(obj) & LV_DIR_HOR) && (sl > 0 || sr > 0)))
          ...
          lv_obj_scroll_to_x(
              obj, lv_obj_get_scroll_x(obj) + lv_obj_get_width(obj) / 4,
              anim_enable);
 ...
 * If specified in the styles, transition animations will be started from the
 * previous state to the current.
 * @param obj       pointer to an object
...
static void lv_obj_set_state(lv_obj_t *obj, lv_state_t new_state) {
  ...
  _lv_style_state_cmp_t cmp_res =
      _lv_obj_style_state_compare(obj, prev_state, new_state);
  /*If there is no difference in styles there is nothing else to do*/
  if (cmp_res == _LV_STYLE_STATE_CMP_SAME)
    return;

  _lv_obj_style_transition_dsc_t *ts = lv_mem_buf_get(
      sizeof(_lv_obj_style_transition_dsc_t) * STYLE_TRANSITION_MAX);
  lv_memset_00(ts,
               sizeof(_lv_obj_style_transition_dsc_t) * STYLE_TRANSITION_MAX);
  uint32_t tsi = 0;
  uint32_t i;
  for (i = 0; i < obj->style_cnt && tsi < STYLE_TRANSITION_MAX; i++) {
    ...
    lv_style_value_t v;
    if (lv_style_get_prop_inlined(obj_style->style, LV_STYLE_TRANSITION, &v) ==
        false)
      continue;
    const lv_style_transition_dsc_t *tr = v.ptr;

    /*Add the props to the set if not added yet or added but with smaller
     * weight*/
    uint32_t j;
    for (j = 0; tr->props[j] != 0 && tsi < STYLE_TRANSITION_MAX; j++) {
      ...
      for (t = 0; t < tsi; t++) {
        ...
        if (ts[t].prop == tr->props[j] && part_ts == part_act &&
            state_ts >= state_act)
          break;
      }
  ...
  for (i = 0; i < tsi; i++) {
    ...
    _lv_obj_style_create_transition(obj, part_act, prev_state, new_state,
                                    &ts[i]);
  }
