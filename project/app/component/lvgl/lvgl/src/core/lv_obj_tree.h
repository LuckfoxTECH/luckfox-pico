...
typedef lv_obj_tree_walk_res_t (*lv_obj_tree_walk_cb_t)(struct _lv_obj_t *,
                                                        void *);
...
/**
 * A function to be easily used in animation ready callback to delete an object
 ...
 * @note to move forward (up): lv_obj_move_to_index(obj, lv_obj_get_index(obj) -
 * 1)
 */
 ...
 * @param user_data     pointer to any user related data (will be passed to
 * `cb`)
 */
void lv_obj_tree_walk(struct _lv_obj_t *start_obj, lv_obj_tree_walk_cb_t cb,
                      void *user_data);
