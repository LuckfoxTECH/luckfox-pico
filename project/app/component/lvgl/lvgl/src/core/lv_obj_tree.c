...
static lv_obj_tree_walk_res_t walk_core(lv_obj_t *obj, lv_obj_tree_walk_cb_t cb,
                                        void *user_data);
...
void lv_obj_del(lv_obj_t *obj) {
  ...
  /*Call the ancestor's event handler to the parent to notify it about the child
   * delete*/
  if (par) {
...
void lv_obj_del_anim_ready_cb(lv_anim_t *a) { lv_obj_del(a->var); }
...
void lv_obj_set_parent(lv_obj_t *obj, lv_obj_t *parent) {
  ...
  for (i = lv_obj_get_index(obj);
       i <= (int32_t)lv_obj_get_child_cnt(old_parent) - 2; i++) {
  ...
  parent->spec_attr->children =
      lv_mem_realloc(parent->spec_attr->children,
...
void lv_obj_tree_walk(lv_obj_t *start_obj, lv_obj_tree_walk_cb_t cb,
                      void *user_data) {
...
static void obj_del_core(lv_obj_t *obj) {
  ...
  if (obj->parent == NULL) {
    ...
    disp->screen_cnt--;
    disp->screens =
        lv_mem_realloc(disp->screens, disp->screen_cnt * sizeof(lv_obj_t *));
  }
  /*Remove the object from the child list of its parent*/
    ...
    for (i = id; i < obj->parent->spec_attr->child_cnt - 1; i++) {
      obj->parent->spec_attr->children[i] =
          obj->parent->spec_attr->children[i + 1];
    }
...
static lv_obj_tree_walk_res_t walk_core(lv_obj_t *obj, lv_obj_tree_walk_cb_t cb,
  ...
  if (obj == NULL) {
    ...
    return LV_OBJ_TREE_WALK_END; /*The value doesn't matter as it wasn't called
                                    recursively*/
  }
