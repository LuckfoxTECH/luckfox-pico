...
bool lv_obj_refr_size(lv_obj_t *obj) {
  ...
  if (obj->w_layout) {
    ...
    if (w_is_content) {
      ...
      if (parent->w_layout == 0 &&
          lv_obj_get_style_width(parent, 0) == LV_SIZE_CONTENT) {
...
bool lv_obj_is_layout_positioned(const lv_obj_t *obj) {
  if (lv_obj_has_flag_any(obj, LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_IGNORE_LAYOUT |
...
void lv_obj_mark_layout_as_dirty(lv_obj_t *obj) {
  ...
  /*Mark the screen as dirty too to mark that there is something to do on this
   * screen*/
...
uint32_t lv_layout_register(lv_layout_update_cb_t cb, void *user_data) {
  layout_cnt++;
  LV_GC_ROOT(_lv_layout_list) = lv_mem_realloc(
      LV_GC_ROOT(_lv_layout_list), layout_cnt * sizeof(lv_layout_dsc_t));
...
void lv_obj_align(lv_obj_t *obj, lv_align_t align, lv_coord_t x_ofs,
                  lv_coord_t y_ofs) {
  lv_obj_set_style_align(obj, align, 0);
...
void lv_obj_align_to(lv_obj_t *obj, const lv_obj_t *base, lv_align_t align,
  ...
  if (lv_obj_get_style_base_dir(parent, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
    x += x_ofs + base->coords.x1 - parent->coords.x1 +
         lv_obj_get_scroll_right(parent) - pleft;
  } else {
    x += x_ofs + base->coords.x1 - parent->coords.x1 +
         lv_obj_get_scroll_left(parent) - pleft;
  }
  y += y_ofs + base->coords.y1 - parent->coords.y1 +
       lv_obj_get_scroll_top(parent) - ptop;
...
void lv_obj_move_to(lv_obj_t *obj, lv_coord_t x, lv_coord_t y) {
  ...
  if (parent) {
    ...
    lv_coord_t border_width =
        lv_obj_get_style_border_width(parent, LV_PART_MAIN);
    x += border_width;
    y += border_width;
  }
...
void lv_obj_move_children_by(lv_obj_t *obj, lv_coord_t x_diff,
...
bool lv_obj_area_is_visible(const lv_obj_t *obj, lv_area_t *area) {
  ...
  /*Invalidate the object only if it belongs to the current or previous or one
   * of the layers'*/
  ...
  if (!lv_obj_has_flag(obj, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
    ...
    if (!_lv_area_intersect(area, area, &obj_coords))
      return false;
  }
  ...
  while (par != NULL) {
    ...
    if (!lv_obj_has_flag(par, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
      if (!_lv_area_intersect(area, area, &par->coords))
        return false;
    }
  ...
  if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
    for (i = 0; i < child_cnt; i++) {
      ...
      if (!lv_obj_is_layout_positioned(child)) {
        ...
        case LV_ALIGN_BOTTOM_RIGHT:
        case LV_ALIGN_RIGHT_MID:
          /*Normal right aligns. Other are ignored due to possible circular
           * dependencies*/
        ...
        default:
          ...
          if (lv_obj_get_style_x(child, 0) == 0) {
            child_res = LV_MAX(child_res,
                               lv_area_get_width(&child->coords) + pad_right);
          }
        }
      } else {
        child_res = LV_MAX(child_res, obj->coords.x2 - child->coords.x1 + 1);
      ...
      if (!lv_obj_is_layout_positioned(child)) {
        ...
        default:
          ...
          if (lv_obj_get_style_y(child, 0) == 0) {
            child_res =
                LV_MAX(child_res, lv_area_get_width(&child->coords) + pad_left);
          }
        }
      } else {
        child_res = LV_MAX(child_res, child->coords.x2 - obj->coords.x1 + 1);
  ...
  for (i = 0; i < child_cnt; i++) {
    ...
    if (!lv_obj_is_layout_positioned(child)) {
      ...
      default:
        ...
        if (lv_obj_get_style_y(child, 0) == 0) {
          child_res =
              LV_MAX(child_res, lv_area_get_height(&child->coords) + pad_top);
        }
        break;
      }
    } else {
