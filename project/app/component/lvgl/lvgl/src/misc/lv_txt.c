...
uint32_t (*_lv_txt_encoded_get_byte_id)(const char *,
                                        uint32_t) = lv_txt_utf8_get_byte_id;
uint32_t (*_lv_txt_encoded_get_char_id)(const char *,
                                        uint32_t) = lv_txt_utf8_get_char_id;
...
uint32_t (*_lv_txt_encoded_next)(const char *,
                                 uint32_t *) = lv_txt_iso8859_1_next;
uint32_t (*_lv_txt_encoded_prev)(const char *,
                                 uint32_t *) = lv_txt_iso8859_1_prev;
...
void lv_txt_get_size(lv_point_t *size_res, const char *text,
  ...
  if (flag & LV_TEXT_FLAG_EXPAND)
    max_width = LV_COORD_MAX;
  ...
  while (text[line_start] != '\0') {
    new_line_start += _lv_txt_get_next_line(
        &text[line_start], font, letter_space, max_width, NULL, flag);

    if ((unsigned long)size_res->y + (unsigned long)letter_height +
            (unsigned long)line_space >
        LV_MAX_OF(lv_coord_t)) {
      LV_LOG_WARN(
          "lv_txt_get_size: integer overflow while calculating text height");
      return;
    } else {
    ...
    lv_coord_t act_line_length =
        lv_txt_get_width(&text[line_start], new_line_start - line_start, font,
                         letter_space, flag);
  ...
  /*Correction with the last line space or set the height manually if the text
   * is empty*/
  if (size_res->y == 0)
    size_res->y = letter_height;
  else
 ...
 *
 * If the word cannot fit in the max_width space, obey LV_TXT_LINE_BREAK_LONG_*
 * rules.
 *
...
static uint32_t lv_txt_get_next_word(const char *txt, const lv_font_t *font,
  ...
  uint32_t break_letter_count =
      0; /*Number of characters up to the long word break point*/
  ...
  /*Word is a "long", but letters may need to be better distributed*/
  {
    i = break_index;
    int32_t n_move =
        LV_TXT_LINE_BREAK_LONG_POST_MIN_LEN - (word_len - break_letter_count);
...
uint32_t _lv_txt_get_next_line(const char *txt, const lv_font_t *font,
  ...
  while (txt[i] != '\0' && max_width > 0) {
    uint32_t word_w = 0;
    uint32_t advance =
        lv_txt_get_next_word(&txt[i], font, letter_space, max_width, flag,
                             &word_w, &cmd_state, i == 0);
    max_width -= word_w;
    line_w += word_w;
...
lv_coord_t lv_txt_get_width(const char *txt, uint32_t length,
                            const lv_font_t *font, lv_coord_t letter_space,
                            lv_text_flag_t flag) {
  if (txt == NULL)
    return 0;
  ...
  if (length != 0) {
    ...
    if (width > 0) {
      width -= letter_space; /*Trim the last letter space. Important if the text
                                is center aligned*/
    }
  }
...
bool _lv_txt_is_cmd(lv_text_cmd_state_t *state, uint32_t c) {
  ...
  if (*state == LV_TEXT_CMD_STATE_PAR) {
    if (c == ' ') {
      *state = LV_TEXT_CMD_STATE_IN; /*After the parameter the text is in the
                                        command*/
    }
    ret = true;
  }
...
void _lv_txt_ins(char *txt_buf, uint32_t pos, const char *ins_txt) {
  ...
  pos = _lv_txt_encoded_get_byte_id(
      txt_buf, pos); /*Convert to byte index instead of letter index*/
...
void _lv_txt_cut(char *txt, uint32_t pos, uint32_t len) {
  ...
  pos = _lv_txt_encoded_get_byte_id(
      txt, pos); /*Convert to byte index instead of letter index*/
...
void _lv_txt_encoded_letter_next_2(const char *txt, uint32_t *letter,
                                   uint32_t *letter_next, uint32_t *ofs) {
 ...
 * @return UTF-8 coded character in Little Endian to be compatible with C chars
 * (e.g. 'Á', 'Ű')
 */
static uint32_t lv_txt_unicode_to_utf8(uint32_t letter_uni) {
  if (letter_uni < 128)
  ...
  if ((c & 0x80) != 0) {
    ...
    for (i = 0; i < 4; i++) {
      if ((swapped & 0xFF) == 0)
        swapped =
            (swapped >>
             8); /*Ignore leading zeros (they were in the end originally)*/
    }
    c = swapped;
  }
#endif
...
static uint32_t lv_txt_utf8_next(const char *txt, uint32_t *i) {
  ...
  /*Real UTF-8 decode*/
    ...
    if (LV_IS_2BYTES_UTF8_CODE(txt[*i])) {
      ...
      (*i)++;
      if (LV_IS_INVALID_UTF8_CODE(txt[*i]))
        return 0;
      result += (txt[*i] & 0x3F);
      (*i)++;
    }
...
static uint32_t lv_txt_utf8_prev(const char *txt, uint32_t *i) {
  ...
  do {
    if (cnt >= 4)
      return 0; /*No UTF-8 char found before the initial*/
  ...
  uint32_t i_tmp = *i;
  uint32_t letter =
      _lv_txt_encoded_next(txt, &i_tmp); /*Character found, get it*/
 ...
 * E.g. in "AÁRT" index of 'R' is 2th char but start at byte 3 because 'Á' is 2
 * bytes long
...
static uint32_t lv_txt_utf8_get_char_id(const char *txt, uint32_t byte_id) {
  ...
  while (i < byte_id) {
    _lv_txt_encoded_next(
        txt, &i); /*'i' points to the next letter so use the prev. value*/
    char_cnt++;
  }
 ...
 * UTF-8 if enabled. E.g.: "ÁBC" is 3 characters (but 4 bytes)
 ...
 * (e.g. 'A') are ASCII compatible by default. So this function does nothing
 * just returns with `c`.
 * @param c a character, e.g. 'A'
 * @return same as `c`
 */
static uint32_t lv_txt_iso8859_1_conv_wc(uint32_t c) { return c; }
...
static uint32_t lv_txt_iso8859_1_prev(const char *txt, uint32_t *i) {
  if (i == NULL)
    return *(txt - 1); /*Get the prev. char*/
 ...
 * @return byte index of the 'utf8_id'th letter
 */
static uint32_t lv_txt_iso8859_1_get_byte_id(const char *txt,
                                             uint32_t utf8_id) {
 ...
 * @return character index of the letter at 'byte_id'th position
 */
static uint32_t lv_txt_iso8859_1_get_char_id(const char *txt,
                                             uint32_t byte_id) {
  LV_UNUSED(txt); /*Unused*/
