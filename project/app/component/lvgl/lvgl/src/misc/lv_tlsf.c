...
#ifdef TLSF_64BIT
#define TLSF_FLS(n)                                                            \
  ((n)&0xffffffff00000000ull ? 32 + TLSF_FLS32((size_t)(n) >> 32)              \
                             : TLSF_FLS32(n))
#else
...
#define TLSF_FLS32(n)                                                          \
  ((n)&0xffff0000 ? 16 + TLSF_FLS16((n) >> 16) : TLSF_FLS16(n))
...
tlsf_decl int tlsf_fls(unsigned int word) { return TLSF_FLS32(word) - 1; }
...
*/
size_t lv_tlsf_size(void) { return sizeof(control_t); }

size_t lv_tlsf_align_size(void) { return ALIGN_SIZE; }

size_t lv_tlsf_block_size_min(void) { return block_size_min; }

size_t lv_tlsf_block_size_max(void) { return block_size_max; }
...
** sentinel block.
*/
size_t lv_tlsf_pool_overhead(void) { return 2 * block_header_overhead; }

size_t lv_tlsf_alloc_overhead(void) { return block_header_overhead; }
...
#if defined(TLSF_64BIT)
    printf("lv_tlsf_add_pool: Memory size must be between 0x%x and 0x%x00 "
           "bytes.\n",
