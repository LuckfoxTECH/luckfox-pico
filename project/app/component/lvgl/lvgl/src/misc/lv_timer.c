...
LV_ATTRIBUTE_TIMER_HANDLER uint32_t lv_timer_handler(void) {
  ...
  do {
    ...
    while (LV_GC_ROOT(_lv_timer_act)) {
      ...
      next =
          _lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll), LV_GC_ROOT(_lv_timer_act));

      if (lv_timer_exec(LV_GC_ROOT(_lv_timer_act))) {
        /*If a timer was created or deleted then this or the next item might be
         * corrupted*/
        if (timer_created || timer_deleted) {
          TIMER_TRACE("Start from the first timer again because a timer was "
                      "created or deleted");
          break;
        }
      }
  ...
  while (next) {
    ...
    next = _lv_ll_get_next(&LV_GC_ROOT(_lv_timer_ll),
                           next); /*Find the next timer*/
  }
  ...
  if (idle_period_time >= IDLE_MEAS_PERIOD) {
    idle_last =
        (busy_time * 100) / idle_period_time; /*Calculate the busy percentage*/
 ...
 * @param timer_xcb a callback which is the timer itself. It will be called
 * periodically. (the 'x' in the argument name indicates that it's not a fully
 ...
 */
lv_timer_t *lv_timer_create(lv_timer_cb_t timer_xcb, uint32_t period,
                            void *user_data) {
 ...
 * @param timer pointer to a lv_timer.
 */
void lv_timer_reset(lv_timer_t *timer) { timer->last_run = lv_tick_get(); }

/**
 * Enable or disable the whole lv_timer handling
 * @param en true: lv_timer handling is running, false: lv_timer handling is
 * suspended
 */
void lv_timer_enable(bool en) { lv_timer_run = en; }

/**
 * Get idle percentage
 * @return the lv_timer idle in percentage
 */
uint8_t lv_timer_get_idle(void) { return idle_last; }

/**
 * Iterate through the timers
 * @param timer NULL to start iteration or the previous return value to get the
 * next timer
...
static bool lv_timer_exec(lv_timer_t *timer) {
  ...
  if (lv_timer_time_remaining(timer) == 0) {
     ...
     * If any timer is deleted `if(timer->repeat_count == 0)` is not executed
     * below but at least the repeat count is zero and the timer can be deleted
     * in the next round*/
  ...
  if (timer_deleted == false) { /*The timer might be deleted by itself as well*/
    if (timer->repeat_count ==
        0) { /*The repeat count is over, delete the timer*/
      TIMER_TRACE(
          "deleting timer with %p callback because the repeat count is over",
          *((void **)&timer->timer_cb));
      lv_timer_del(timer);
    }
  }
