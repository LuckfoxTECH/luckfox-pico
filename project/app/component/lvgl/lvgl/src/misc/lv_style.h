...
#define LV_STYLE_PROP_EXT_DRAW                                                 \
  (1 << 11) /*Requires ext. draw size update when changed*/
#define LV_STYLE_PROP_LAYOUT_REFR                                              \
  (1 << 12) /*Requires layout update when changed*/
#define LV_STYLE_PROP_PARENT_LAYOUT_REFR                                       \
  (1 << 13) /*Requires layout update on parent when changed*/
...
#define LV_STYLE_CONST_INIT(var_name, prop_array)                              \
  const lv_style_t var_name = {.sentinel = LV_STYLE_SENTINEL_VALUE,            \
...
#define LV_STYLE_CONST_INIT(var_name, prop_array)                              \
  const lv_style_t var_name = {                                                \
      .v_p = {.const_props = prop_array}, .has_group = 0xFF, .is_const = 1}
#endif
 ...
 */
enum {
  LV_DITHER_NONE,     /**< No dithering, colors are just quantized to the output
                         resolution*/
  LV_DITHER_ORDERED,  /**< Ordered dithering. Faster to compute and use less
                         memory but lower quality*/
  LV_DITHER_ERR_DIFF, /**< Error diffusion mode. Slower to compute and use more
                         memory but give highest dither quality*/
};
...
typedef enum {
  ...
  LV_STYLE_TRANSLATE_X =
      12 | LV_STYLE_PROP_LAYOUT_REFR | LV_STYLE_PROP_PARENT_LAYOUT_REFR,
  LV_STYLE_TRANSLATE_Y =
      13 | LV_STYLE_PROP_LAYOUT_REFR | LV_STYLE_PROP_PARENT_LAYOUT_REFR,
  ...
  LV_STYLE_TEXT_COLOR_FILTERED =
      87 | LV_STYLE_PROP_INHERIT | LV_STYLE_PROP_FILTER,
  ...
  LV_STYLE_TEXT_LETTER_SPACE =
      90 | LV_STYLE_PROP_INHERIT | LV_STYLE_PROP_LAYOUT_REFR,
  LV_STYLE_TEXT_LINE_SPACE =
      91 | LV_STYLE_PROP_INHERIT | LV_STYLE_PROP_LAYOUT_REFR,
...
#if LV_USE_USER_DATA
  void *user_data; /**< A custom user data that will be passed to the
                      animation's user_data */
#endif
 ...
 *       because this function won't free the used memory, just sets a default
 * state for the style. In other words be sure to initialize styles only once!
 */
 ...
 * { lv_style_value_t v = {.color = value}; lv_style_set_prop(style, MY_PROP,
 * v); }
 *
 * ...
 ...
 * @return true: the property was found and removed; false: the property wasn't
 * found
 */
 ...
 * @param value `lv_style_value_t` variable in which a field is set according to
 * the type of `prop`
 */
void lv_style_set_prop(lv_style_t *style, lv_style_prop_t prop,
                       lv_style_value_t value);
 ...
 * @return LV_RES_INV: the property wasn't found in the style (`value` is
 * unchanged) LV_RES_OK: the property was fond, and `value` is set accordingly
 * @note For performance reasons there are no sanity check on `style`
 */
lv_res_t lv_style_get_prop(const lv_style_t *style, lv_style_prop_t prop,
                           lv_style_value_t *value);
 ...
 * unchanged) LV_RES_OK: the property was fond, and `value` is set accordingly
 * @note For performance reasons there are no sanity check on `style`
 * @note This function is the same as ::lv_style_get_prop but inlined. Use it
 * only on performance critical places
 */
static inline lv_res_t lv_style_get_prop_inlined(const lv_style_t *style,
                                                 lv_style_prop_t prop,
                                                 lv_style_value_t *value) {
  if (style->is_const) {
    const lv_style_const_prop_t *const_prop;
    for (const_prop = style->v_p.const_props;
         const_prop->prop != LV_STYLE_PROP_INV; const_prop++) {
  ...
  if (style->prop_cnt > 1) {
    ...
    for (i = 0; i < style->prop_cnt; i++) {
      if (props[i] == prop) {
        lv_style_value_t *values =
            (lv_style_value_t *)style->v_p.values_and_props;
        *value = values[i];
        return LV_RES_OK;
      }
    }
 ...
 * @param props     an array with the properties to transition. The last element
 * must be zero.
 * @param path_cb   an animation path (ease) callback. If `NULL` liner path will
 * be used.
 * @param time      duration of the transition in [ms]
 * @param delay     delay before the transition in [ms]
 * @param user_data any custom data that will be saved in the transition
 * animation and will be available when `path_cb` is called
 * @example
 * const static lv_style_prop_t trans_props[] = { LV_STYLE_BG_OPA,
 * LV_STYLE_BG_COLOR, 0 }; static lv_style_transition_dsc_t trans1;
...
/**
 * Tell the group of a property. If the a property from a group is set in a
 * style the (1 << group) bit of style->has_group is set. It allows early
 * skipping the style if the property is not exists in the style at all.
...
#define LV_ASSERT_STYLE(style_p)                                               \
  do {                                                                         \
    ...
    LV_ASSERT_MSG(style_p->sentinel == LV_STYLE_SENTINEL_VALUE,                \
                  "Style is not initialized or corrupted");                    \
  } while (0)
#else
