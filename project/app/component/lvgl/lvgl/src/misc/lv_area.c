...
uint32_t lv_area_get_size(const lv_area_t *area_p) {
  ...
  size =
      (uint32_t)(area_p->x2 - area_p->x1 + 1) * (area_p->y2 - area_p->y1 + 1);
 ...
 */
bool _lv_area_intersect(lv_area_t *res_p, const lv_area_t *a1_p,
 ...
 * @param a2_p pointer to the second area
 */
void _lv_area_join(lv_area_t *a_res_p, const lv_area_t *a1_p,
                   const lv_area_t *a2_p) {
 ...
 */
bool _lv_area_is_point_on(const lv_area_t *a_p, const lv_point_t *p_p,
                          lv_coord_t radius) {
...
bool _lv_area_is_on(const lv_area_t *a1_p, const lv_area_t *a2_p) {
  if ((a1_p->x1 <= a2_p->x2) && (a1_p->x2 >= a2_p->x1) &&
      (a1_p->y1 <= a2_p->y2) && (a1_p->y2 >= a2_p->y1)) {
    return true;
  } else {
    return false;
  }
 ...
 */
bool _lv_area_is_in(const lv_area_t *ain_p, const lv_area_t *aholder_p,
                    lv_coord_t radius) {
  bool is_in = false;

  if (ain_p->x1 >= aholder_p->x1 && ain_p->y1 >= aholder_p->y1 &&
      ain_p->x2 <= aholder_p->x2 && ain_p->y2 <= aholder_p->y2) {
    is_in = true;
  }
  ...
  p.x = ain_p->x1;
  p.y = ain_p->y1;
  if (_lv_area_is_point_on(aholder_p, &p, radius) == false)
    return false;
 ...
 */
bool _lv_area_is_out(const lv_area_t *aout_p, const lv_area_t *aholder_p,
                     lv_coord_t radius) {
  if (aout_p->x2 < aholder_p->x1 || aout_p->y2 < aholder_p->y1 ||
      aout_p->x1 > aholder_p->x2 || aout_p->y1 > aholder_p->y2) {
    return true;
  }
  ...
  p.x = aout_p->x1;
  p.y = aout_p->y1;
  if (_lv_area_is_point_on(aholder_p, &p, radius))
    return false;
 ...
 */
void lv_area_align(const lv_area_t *base, lv_area_t *to_align, lv_align_t align,
                   lv_coord_t ofs_x, lv_coord_t ofs_y) {
