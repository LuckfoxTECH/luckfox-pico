...
static lv_fs_res_t lv_fs_read_cached(lv_fs_file_t *file_p, char *buf,
  ...
  if (start <= file_position && file_position < end) {
    ...
    if (btr <= buffer_remaining_length) {
      /*Data is in cache buffer, and buffer end not reached, no need to read
       * from FS*/
      lv_memcpy(buf, buffer + buffer_offset, btr);
    } else {
      /*First part of data is in cache buffer, but we need to read rest of data
       * from FS*/
      lv_memcpy(buf, buffer + buffer_offset, buffer_remaining_length);

      if (btr > buffer_size) {
        /*If remaining data chuck is bigger than buffer size, then do not use
         * cache, instead read it directly from FS*/
        res = file_p->drv->read_cb(file_p->drv, file_p->file_d,
                                   (void *)(buf + buffer_remaining_length),
                                   btr - buffer_remaining_length, br);
      } else {
        /*If remaining data chunk is smaller than buffer size, then read into
         * cache buffer*/
        uint32_t bytes_read_to_buffer = 0;

        /*Read into cache buffer:*/
        res = file_p->drv->read_cb(file_p->drv, file_p->file_d, (void *)buffer,
                                   buffer_size, &bytes_read_to_buffer);
    ...
    if (btr > buffer_size) {
      /*If bigger data is requested, then do not use cache, instead read it
       * directly*/
      res = file_p->drv->read_cb(file_p->drv, file_p->file_d, (void *)buf, btr,
                                 br);
    } else {
...
lv_fs_res_t lv_fs_write(lv_fs_file_t *file_p, const void *buf, uint32_t btw,
  ...
  uint32_t bw_tmp = 0;
  lv_fs_res_t res =
      file_p->drv->write_cb(file_p->drv, file_p->file_d, buf, btw, &bw_tmp);
  if (bw != NULL)
    *bw = bw_tmp;
...
lv_fs_res_t lv_fs_seek(lv_fs_file_t *file_p, uint32_t pos,
  ...
  if (file_p->drv->cache_size) {
    ...
    case LV_FS_SEEK_SET: {
      ...
      if (file_p->cache->file_position < file_p->cache->start ||
          file_p->cache->file_position > file_p->cache->end) {
        res =
            file_p->drv->seek_cb(file_p->drv, file_p->file_d,
                                 file_p->cache->file_position, LV_FS_SEEK_SET);
      }
    ...
    case LV_FS_SEEK_END: {
      /*Because we don't know the file size, we do a little trick: do a FS seek,
       * then get new file position from FS*/
