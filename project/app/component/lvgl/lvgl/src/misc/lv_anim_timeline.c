...
lv_anim_timeline_t *lv_anim_timeline_create(void) {
  lv_anim_timeline_t *at =
      (lv_anim_timeline_t *)lv_mem_alloc(sizeof(lv_anim_timeline_t));

  LV_ASSERT_MALLOC(at);

  if (at)
    lv_memset_00(at, sizeof(lv_anim_timeline_t));
...
void lv_anim_timeline_add(lv_anim_timeline_t *at, uint32_t start_time,
                          lv_anim_t *a) {
  LV_ASSERT_NULL(at);

  at->anim_dsc_cnt++;
  at->anim_dsc = lv_mem_realloc(
      at->anim_dsc, at->anim_dsc_cnt * sizeof(lv_anim_timeline_dsc_t));
  ...
  if (a->var == NULL && a->exec_cb == NULL) {
    ...
    at->anim_dsc[at->anim_dsc_cnt - 1].anim.exec_cb =
        lv_anim_timeline_virtual_exec_cb;
  }
}
