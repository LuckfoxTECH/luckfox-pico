...
/**
 * MSVC compiler's definition of the __cplusplus indicating 199711L regardless
 * to C++ standard version see
...
#define _LV_COLOR_ZERO_INITIALIZER1                                            \
  { 0x00 }
#define LV_COLOR_MAKE1(r8, g8, b8)                                             \
  { (uint8_t)((b8 >> 7) | (g8 >> 7) | (r8 >> 7)) }
...
#define _LV_COLOR_ZERO_INITIALIZER8                                            \
  {                                                                            \
    { 0x00, 0x00, 0x00 }                                                       \
  }
#define LV_COLOR_MAKE8(r8, g8, b8)                                             \
  {                                                                            \
    {                                                                          \
      (uint8_t)((b8 >> 6) & 0x3U), (uint8_t)((g8 >> 5) & 0x7U),                \
          (uint8_t)((r8 >> 5) & 0x7U)                                          \
    }                                                                          \
  }
...
#else
#define LV_COLOR_SET_G16(c, v)                                                 \
  {                                                                            \
    (c).ch.green_h = (uint8_t)(((v) >> 3) & 0x7);                              \
    (c).ch.green_l = (uint8_t)((v)&0x7);                                       \
  }
#endif
...
#if LV_COLOR_16_SWAP == 0
#define _LV_COLOR_ZERO_INITIALIZER16                                           \
  {                                                                            \
    { 0x00, 0x00, 0x00 }                                                       \
  }
#define LV_COLOR_MAKE16(r8, g8, b8)                                            \
  {                                                                            \
    {                                                                          \
      (uint8_t)((b8 >> 3) & 0x1FU), (uint8_t)((g8 >> 2) & 0x3FU),              \
          (uint8_t)((r8 >> 3) & 0x1FU)                                         \
    }                                                                          \
  }
#else
...
#define LV_COLOR_MAKE16(r8, g8, b8)                                            \
  {                                                                            \
    {                                                                          \
      (uint8_t)((g8 >> 5) & 0x7U), (uint8_t)((r8 >> 3) & 0x1FU),               \
          (uint8_t)((b8 >> 3) & 0x1FU), (uint8_t)((g8 >> 2) & 0x7U)            \
    }                                                                          \
  }
#endif
...
#define _LV_COLOR_ZERO_INITIALIZER32                                           \
  {                                                                            \
    { 0x00, 0x00, 0x00, 0x00 }                                                 \
  }
#define LV_COLOR_MAKE32(r8, g8, b8)                                            \
  {                                                                            \
    { b8, g8, r8, 0xff }                                                       \
  } /*Fix 0xff alpha*/
...
#define _LV_COLOR_ZERO_INITIALIZER                                             \
  LV_CONCAT(_LV_COLOR_ZERO_INITIALIZER, LV_COLOR_DEPTH)
#define LV_COLOR_MAKE(r8, g8, b8)                                              \
  LV_CONCAT(LV_COLOR_MAKE, LV_COLOR_DEPTH)(r8, g8, b8)
...
typedef union {
  uint8_t full; /*must be declared first to set all bits of byte via initializer
                   list*/
  union {
    uint8_t blue : 1;
    uint8_t green : 1;
...
typedef lv_color_t (*lv_color_filter_cb_t)(
    const struct _lv_color_filter_dsc_t *, lv_color_t, lv_opa_t);
...
#elif LV_COLOR_DEPTH == 8
  lv_color16_t ret;
  LV_COLOR_SET_R16(ret, LV_COLOR_GET_R(color) *
                            4); /*(2^5 - 1)/(2^3 - 1) = 31/7 = 4*/
  LV_COLOR_SET_G16(ret, LV_COLOR_GET_G(color) *
                            9); /*(2^6 - 1)/(2^3 - 1) = 63/7 = 9*/
  LV_COLOR_SET_B16(ret, LV_COLOR_GET_B(color) *
                            10); /*(2^5 - 1)/(2^2 - 1) = 31/3 = 10*/
  return ret.full;
#elif LV_COLOR_DEPTH == 16
  return color.full;
#elif LV_COLOR_DEPTH == 32
...
static inline uint32_t lv_color_to32(lv_color_t color) {
   ...
   *   multiplier = FLOOR( ( (2^bitsto - 1) << divisor ) / (float)(2^bitsfrom -
   * 1) )
   *
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t
lv_color_mix(lv_color_t c1, lv_color_t c2, uint8_t mix) {
  lv_color_t ret;
  ...
  uint32_t fg =
      (uint32_t)((uint32_t)c1.full | ((uint32_t)c1.full << 16)) & 0x7E0F81F;
  uint32_t result = ((((fg - bg) * mix) >> 5) + bg) & 0x7E0F81F;
  ret.full = (uint16_t)((result >> 16) | result);
#elif LV_COLOR_DEPTH != 1
  /*LV_COLOR_DEPTH == 8, 16 or 32*/
  LV_COLOR_SET_R(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_R(c1) * mix +
                                 LV_COLOR_GET_R(c2) * (255 - mix) +
                                 LV_COLOR_MIX_ROUND_OFS));
  LV_COLOR_SET_G(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_G(c1) * mix +
                                 LV_COLOR_GET_G(c2) * (255 - mix) +
                                 LV_COLOR_MIX_ROUND_OFS));
  LV_COLOR_SET_B(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_B(c1) * mix +
                                 LV_COLOR_GET_B(c2) * (255 - mix) +
                                 LV_COLOR_MIX_ROUND_OFS));
...
LV_ATTRIBUTE_FAST_MEM static inline void
lv_color_premult(lv_color_t c, uint8_t mix, uint16_t *out) {
#if LV_COLOR_DEPTH != 1
...
/**
 * Mix two colors with a given ratio. It runs faster then `lv_color_mix` but
 * requires some pre computation.
 * @param premult_c1 The first color. Should be preprocessed with
 * `lv_color_premult(c1)`
 * @param c2 The second color. As it is no pre computation required on it
 * @param mix The ratio of the colors. 0: full `c1`, 255: full `c2`, 127: half
 * `c1` and half `c2`. Should be modified like mix = `255 - mix`
 * @return the mixed color
 * @note 255 won't give clearly `c1`.
 */
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t
lv_color_mix_premult(uint16_t *premult_c1, lv_color_t c2, uint8_t mix) {
  lv_color_t ret;
#if LV_COLOR_DEPTH != 1
  /*LV_COLOR_DEPTH == 8 or 32*/
  LV_COLOR_SET_R(ret, LV_UDIV255(premult_c1[0] + LV_COLOR_GET_R(c2) * mix +
                                 LV_COLOR_MIX_ROUND_OFS));
  LV_COLOR_SET_G(ret, LV_UDIV255(premult_c1[1] + LV_COLOR_GET_G(c2) * mix +
                                 LV_COLOR_MIX_ROUND_OFS));
  LV_COLOR_SET_B(ret, LV_UDIV255(premult_c1[2] + LV_COLOR_GET_B(c2) * mix +
                                 LV_COLOR_MIX_ROUND_OFS));
  LV_COLOR_SET_A(ret, 0xFF);
#else
  /*LV_COLOR_DEPTH == 1*/
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline void
lv_color_mix_with_alpha(lv_color_t bg_color, lv_opa_t bg_opa,
  ...
  /*Both colors have alpha. Expensive calculation need to be applied*/
    ...
    /*Save the parameters and the result. If they will be asked again don't
     * compute again*/
    ...
    if (fg_opa != fg_opa_save || bg_opa != bg_opa_save ||
      ...
      res_opa_saved =
          255 - ((uint16_t)((uint16_t)(255 - fg_opa) * (255 - bg_opa)) >> 8);
...
static inline uint8_t lv_color_brightness(lv_color_t color) {
  ...
  uint16_t bright =
      (uint16_t)(3u * LV_COLOR_GET_R32(c32) + LV_COLOR_GET_B32(c32) +
                 4u * LV_COLOR_GET_G32(c32));
...
static inline lv_color_t lv_color_hex(uint32_t c) {
  ...
  r.full = (uint16_t)(((c & 0xF80000) >> 8) | ((c & 0xFC00) >> 5) |
                      ((c & 0xFF) >> 3));
#else
  /* We want: rrrr rrrr GGGg gggg bbbb bbbb => gggb bbbb rrrr rGGG */
  r.full = (uint16_t)(((c & 0xF80000) >> 16) | ((c & 0xFC00) >> 13) |
                      ((c & 0x1C00) << 3) | ((c & 0xF8) << 5));
#endif
  return r;
#elif LV_COLOR_DEPTH == 32
...
static inline void lv_color_filter_dsc_init(lv_color_filter_dsc_t *dsc,
...
/**
 * Just a wrapper around LV_COLOR_CHROMA_KEY because it might be more convenient
 * to use a function is some cases
