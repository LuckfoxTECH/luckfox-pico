...
void lv_style_reset(lv_style_t *style) {
  ...
  if (style->prop_cnt > 1)
    lv_mem_free(style->v_p.values_and_props);
  lv_memset_00(style, sizeof(lv_style_t));
#if LV_USE_ASSERT_STYLE
  style->sentinel = LV_STYLE_SENTINEL_VALUE;
#endif
...
bool lv_style_remove_prop(lv_style_t *style, lv_style_prop_t prop) {
  ...
  uint8_t *tmp =
      style->v_p.values_and_props + style->prop_cnt * sizeof(lv_style_value_t);
  uint16_t *old_props = (uint16_t *)tmp;
  uint32_t i;
  for (i = 0; i < style->prop_cnt; i++) {
    if (old_props[i] == prop) {
      lv_style_value_t *old_values =
      ...
      if (style->prop_cnt == 2) {
        ...
        size_t size = (style->prop_cnt - 1) *
                      (sizeof(lv_style_value_t) + sizeof(uint16_t));
        ...
        for (i = j = 0; j <= style->prop_cnt;
             j++) { /*<=: because prop_cnt already reduced but all the old
                       props. needs to be checked.*/
          if (old_props[j] != prop) {
...
void lv_style_set_prop(lv_style_t *style, lv_style_prop_t prop,
  ...
  if (style->prop_cnt > 1) {
    ...
    size_t size =
        (style->prop_cnt + 1) * (sizeof(lv_style_value_t) + sizeof(uint16_t));
    uint8_t *values_and_props =
        lv_mem_realloc(style->v_p.values_and_props, size);
    ...
    uint8_t *tmp =
        values_and_props + style->prop_cnt * sizeof(lv_style_value_t);
    uint16_t *props = (uint16_t *)tmp;
    lv_style_value_t *values = (lv_style_value_t *)values_and_props;
    props[0] = style->prop1;
    props[1] = prop;
...
uint8_t _lv_style_get_prop_group(lv_style_prop_t prop) {
  ...
  if (group > 7)
    group = 7; /*The MSB marks all the custom properties*/
