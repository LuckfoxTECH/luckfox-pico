...
typedef enum {
  LV_FS_SEEK_SET =
      0x00, /**< Set the position from absolutely (from the start of file)*/
...
typedef struct _lv_fs_drv_t {
  ...
  void *(*open_cb)(struct _lv_fs_drv_t *drv, const char *path,
                   lv_fs_mode_t mode);
  lv_fs_res_t (*close_cb)(struct _lv_fs_drv_t *drv, void *file_p);
  lv_fs_res_t (*read_cb)(struct _lv_fs_drv_t *drv, void *file_p, void *buf,
                         uint32_t btr, uint32_t *br);
  lv_fs_res_t (*write_cb)(struct _lv_fs_drv_t *drv, void *file_p,
                          const void *buf, uint32_t btw, uint32_t *bw);
  lv_fs_res_t (*seek_cb)(struct _lv_fs_drv_t *drv, void *file_p, uint32_t pos,
                         lv_fs_whence_t whence);
  lv_fs_res_t (*tell_cb)(struct _lv_fs_drv_t *drv, void *file_p,
                         uint32_t *pos_p);
 ...
 *                  corresponding function pointers. Only pointer is saved, so
 * the driver should be static or dynamically allocated.
 */
...
/**
 * Test if a drive is ready or not. If the `ready` function was not initialized
 ...
 * @param buf       buffer to store the letters ('\0' added after the last
 * letter)
 * @return          the buffer
 */
 ...
 * @return          pointer to the beginning extension or empty string if no
 * extension
 */
