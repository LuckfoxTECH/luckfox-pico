...
void *_lv_ll_ins_tail(lv_ll_t *ll_p) {
  ...
  if (n_new != NULL) {
    ...
    node_set_prev(ll_p, n_new,
                  ll_p->tail); /*The prev. before new is the old tail*/
    if (ll_p->tail !=
...
void _lv_ll_chg_list(lv_ll_t *ll_ori_p, lv_ll_t *ll_new_p, void *node,
  ...
  if (head) {
    ...
    if (ll_new_p->tail !=
        NULL) { /*If there is old tail then after it goes the new*/
      node_set_next(ll_new_p, ll_new_p->tail, node);
    }

    ll_new_p->tail = node; /*Set the new tail in the dsc.*/
    if (ll_new_p->head ==
        NULL) { /*If there is no head (first node) set the head too*/
      ll_new_p->head = node;
    }
  }
}
...
uint32_t _lv_ll_get_len(const lv_ll_t *ll_p) {
  ...
  for (node = _lv_ll_get_head(ll_p); node != NULL;
       node = _lv_ll_get_next(ll_p, node)) {
    len++;
  }
...
void _lv_ll_move_before(lv_ll_t *ll_p, void *n_act, void *n_after) {
  ...
  if (n_after != NULL)
    ...
    n_before = _lv_ll_get_tail(
        ll_p); /*if `n_after` is NULL `n_act` should be the new tail*/
 ...
 */
static void node_set_prev(lv_ll_t *ll_p, lv_ll_node_t *act,
                          lv_ll_node_t *prev) {
  if (act == NULL)
 ...
 */
static void node_set_next(lv_ll_t *ll_p, lv_ll_node_t *act,
                          lv_ll_node_t *next) {
  if (act == NULL)
