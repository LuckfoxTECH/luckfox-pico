 ...
 *      DEFINES
 *********************/
/*memset the allocated memories to 0xaa and freed memories to 0xbb (just for
 * testing purposes)*/
...
static uint32_t zero_mem =
    ZERO_MEM_SENTINEL; /*Give the address of this variable if 0 byte should be
                          allocated*/
...
void lv_mem_init(void) {
  ...
  tlsf = lv_tlsf_create_with_pool((void *)LV_MEM_POOL_ALLOC(LV_MEM_SIZE),
                                  LV_MEM_SIZE);
#else
  /*Allocate a large array to store the dynamically allocated data*/
  static LV_ATTRIBUTE_LARGE_RAM_ARRAY MEM_UNIT
      work_mem_int[LV_MEM_SIZE / sizeof(MEM_UNIT)];
...
void *lv_mem_buf_get(uint32_t size) {
  ...
  for (uint8_t i = 0; i < LV_MEM_BUF_MAX_NUM; i++) {
    if (LV_GC_ROOT(lv_mem_buf[i]).used == 0 &&
      ...
      else if (LV_GC_ROOT(lv_mem_buf[i]).size <
               LV_GC_ROOT(lv_mem_buf[i_guess]).size) {
        i_guess = i;
      }
    }
  }
  ...
  if (i_guess >= 0) {
    ...
    MEM_TRACE("returning already allocated buffer (buffer id: %d, address: %p)",
              i_guess, LV_GC_ROOT(lv_mem_buf[i_guess]).p);
  ...
  for (uint8_t i = 0; i < LV_MEM_BUF_MAX_NUM; i++) {
    if (LV_GC_ROOT(lv_mem_buf[i]).used == 0) {
      ...
      LV_ASSERT_MSG(buf != NULL, "Out of memory, can't allocate a new buffer "
                                 "(increase your LV_MEM_SIZE/heap size)");
      if (buf == NULL)
        return NULL;
      ...
      MEM_TRACE("allocated (buffer id: %d, address: %p)", i,
                LV_GC_ROOT(lv_mem_buf[i]).p);
      return LV_GC_ROOT(lv_mem_buf[i]).p;
    }
  }
...
LV_ATTRIBUTE_FAST_MEM void lv_memset(void *dst, uint8_t v, size_t len) {
  ...
  uint32_t v32 = (uint32_t)v + ((uint32_t)v << 8) + ((uint32_t)v << 16) +
                 ((uint32_t)v << 24);
