...
// Highest bit of the 16-bit pos_conv value specifies whether this pos is RTL or
// not
...
static lv_base_dir_t get_next_run(const char *txt, lv_base_dir_t base_dir,
                                  uint32_t max_len, uint32_t *len,
                                  uint16_t *pos_conv_len);
static void rtl_reverse(char *dest, const char *src, uint32_t len,
                        uint16_t *pos_conv_out, uint16_t pos_conv_rd_base,
                        uint16_t pos_conv_len);
static uint32_t char_change_to_pair(uint32_t letter);
static lv_base_dir_t bracket_process(const char *txt, uint32_t next_pos,
                                     uint32_t len, uint32_t letter,
 ...
 */
void _lv_bidi_process(const char *str_in, char *str_out,
                      lv_base_dir_t base_dir) {
  ...
  while (str_in[par_start] != '\0') {
    ...
    _lv_bidi_process_paragraph(&str_in[par_start], &str_out[par_start], par_len,
                               base_dir, NULL, 0);
    par_start += par_len;
...
lv_base_dir_t _lv_bidi_detect_base_dir(const char *txt) {
  ...
  while (txt[i] != '\0') {
    ...
    if (dir == LV_BASE_DIR_RTL || dir == LV_BASE_DIR_LTR)
      return dir;
  }
 ...
 * @param bidi_txt internally the text is bidi processed which buffer can be get
 * here. If not required anymore has to freed with `lv_mem_free()` Can be `NULL`
 * is unused
 * @param len length of the line in character count
 * @param base_dir base direction of the text: `LV_BASE_DIR_LTR` or
 * `LV_BASE_DIR_RTL`
 * @param visual_pos the visual character position which logical position should
 * be get
 * @param is_rtl tell the char at `visual_pos` is RTL or LTR context
 * @return the logical character position
 */
uint16_t _lv_bidi_get_logical_pos(const char *str_in, char **bidi_txt,
                                  uint32_t len, lv_base_dir_t base_dir,
  ...
  _lv_bidi_process_paragraph(str_in, bidi_txt ? *bidi_txt : NULL, len, base_dir,
                             pos_conv_buf, pos_conv_len);
 ...
 * @param base_dir base direction of the text: `LV_BASE_DIR_LTR` or
 * `LV_BASE_DIR_RTL`
 * @param logical_pos the logical character position which visual position
 * should be get
 * @param is_rtl tell the char at `logical_pos` is RTL or LTR context
 * @return the visual character position
 */
uint16_t _lv_bidi_get_visual_pos(const char *str_in, char **bidi_txt,
                                 uint16_t len, lv_base_dir_t base_dir,
 ...
 * @param pos_conv_out an `uint16_t` array to store the related logical position
 * of the character. Can be `NULL` is unused
...
void _lv_bidi_process_paragraph(const char *str_in, char *str_out, uint32_t len,
  ...
  while (rd < len) {
    ...
    pos_conv_rd++;
    dir = lv_bidi_get_letter_dir(letter);
    if (dir == LV_BASE_DIR_NEUTRAL)
      dir = bracket_process(str_in, rd, len, letter, base_dir);
    if (dir != LV_BASE_DIR_NEUTRAL && dir != LV_BASE_DIR_WEAK)
      break;
  }
  ...
  if (rd) {
    if (base_dir == LV_BASE_DIR_LTR) {
      ...
      wr -= rd;
      pos_conv_wr -= pos_conv_rd;
      rtl_reverse(str_out ? &str_out[wr] : NULL, str_in, rd,
                  pos_conv_out ? &pos_conv_out[pos_conv_wr] : NULL, 0,
                  pos_conv_rd);
    }
  }
  ...
  while (rd < len && str_in[rd]) {
    ...
    if (base_dir == LV_BASE_DIR_LTR) {
      if (run_dir == LV_BASE_DIR_LTR) {
        if (str_out)
          lv_memcpy(&str_out[wr], &str_in[rd], run_len);
        if (pos_conv_out)
          fill_pos_conv(&pos_conv_out[pos_conv_wr], pos_conv_run_len,
                        pos_conv_rd);
      } else
...
void lv_bidi_calculate_align(lv_text_align_t *align, lv_base_dir_t *base_dir,
                             const char *txt) {
  if (*base_dir == LV_BASE_DIR_AUTO)
...
static bool lv_bidi_letter_is_rtl(uint32_t letter) {
  ...
  /*Check for Persian and Arabic characters
...
static lv_base_dir_t get_next_run(const char *txt, lv_base_dir_t base_dir,
  ...
  while (i_prev < max_len && txt[i] != '\0' && txt[i] != '\n' &&
    ...
    /*New dir found?*/
    if ((next_dir == LV_BASE_DIR_RTL || next_dir == LV_BASE_DIR_LTR) &&
        next_dir != run_dir) {
  ...
  while (i) {
    ...
    if (lv_bidi_letter_is_weak(letter)) {
      ...
      while (i) {
        ...
        /*No need to call `char_change_to_pair` because there not such chars
         * here*/
      ...
      if (dest)
        lv_memcpy(&dest[wr], &src[first_weak], last_weak - first_weak + 1);
      if (pos_conv_out)
        fill_pos_conv(&pos_conv_out[pos_conv_wr],
                      pos_conv_last_weak - pos_conv_first_weak + 1,
    ...
    /*Simply store in reversed order*/
      ...
      /*Just store the letter*/
        ...
        if (dest)
          lv_memcpy(&dest[wr], &src[i], letter_size);
        if (pos_conv_out)
          pos_conv_out[pos_conv_wr] =
...
static lv_base_dir_t bracket_process(const char *txt, uint32_t next_pos,
  ...
  for (i = 0; bracket_left[i] != '\0'; i++) {
    if (bracket_left[i] == letter) {
       ...
       *If a char with base dir. direction is found then the brackets will have
       *`base_dir` direction*/
      ...
      /*There where a strong char with base dir in the bracket so the dir is
       * found.*/
      if (bracket_dir != LV_BASE_DIR_NEUTRAL && bracket_dir != LV_BASE_DIR_WEAK)
        break;

      /*If there were no matching strong chars in the brackets then check the
       * previous chars*/
      txt_i = next_pos;
      if (txt_i)
      ...
      if (bracket_dir != LV_BASE_DIR_NEUTRAL)
        break;
  ...
  if (bracket_left[i] != '\0') {
    ...
    if (bracket_dir == LV_BASE_DIR_NEUTRAL ||
        br_stack_p == LV_BIDI_BRACKLET_DEPTH)
      return LV_BASE_DIR_NEUTRAL;
