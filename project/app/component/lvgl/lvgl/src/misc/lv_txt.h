...
enum {
  ...
  LV_TEXT_FLAG_EXPAND =
      0x02, /**< Ignore max-width to avoid automatic word wrapping*/
  LV_TEXT_FLAG_FIT = 0x04, /**< Max-width is already equal to the longest line.
                              (Used to skip some calculation)*/
};
 ...
 * character but 2 bytes in UTF-8)
 * @param font pointer to a font
 * @param letter_space letter space
 ...
 * @param txt_buf the original text (must be big enough for the result text and
 * NULL terminated)
 ...
 * @param txt string to modify, must be '\0' terminated and should point to a
 * heap or stack frame, not read-only memory.
 * @param pos position where to start the deleting (0: before the first char, 1:
 ...
 * @param letter_next the second decoded Unicode character or 0 on invalid data
 * code
 * @param ofs start index in 'txt' where to start.
 *                After the call it will point to the next encoded char in
 * 'txt'. NULL to use txt[0] as index
 */
 ...
 * @param txt pointer to '\0' terminated string
 * @param i_start index in 'txt' where to start. After the call it will point to
 * the previous encoded char in 'txt'.
