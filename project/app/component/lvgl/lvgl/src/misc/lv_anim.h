 ...
 * parameter. It's more consistent but less convenient. Might be used by binding
 * generator functions.*/
...
/** Callback used when the animation values are relative to get the current
 * value*/
...
typedef struct _lv_anim_t {
  ...
  lv_anim_start_cb_t start_cb; /**< Call it when the animation is starts
                                  (considering `delay`)*/
  ...
  uint8_t early_apply : 1; /**< 1: Apply start value immediately even is there
                              is `delay`*/

  /*Animation system use these - user shouldn't set*/
  uint8_t playback_now : 1; /**< Play back is in progress*/
  uint8_t run_round : 1;    /**< Indicates the animation has run in this round*/
  uint8_t start_cb_called : 1; /**< Indicates that the `start_cb` was already
                                  called*/
} lv_anim_t;
 ...
 */
static inline void lv_anim_set_var(lv_anim_t *a, void *var) { a->var = var; }
 ...
 *                  E.g. lv_obj_set_x
 */
static inline void lv_anim_set_exec_cb(lv_anim_t *a,
 ...
 */
static inline void lv_anim_set_values(lv_anim_t *a, int32_t start,
                                      int32_t end) {
  a->start_value = start;
 ...
 */
static inline void
lv_anim_set_custom_exec_cb(lv_anim_t *a, lv_anim_custom_exec_cb_t exec_cb) {
  a->var = a;
 ...
 */
static inline void lv_anim_set_path_cb(lv_anim_t *a,
 ...
 */
static inline void lv_anim_set_start_cb(lv_anim_t *a,
                                        lv_anim_start_cb_t start_cb) {
  a->start_cb = start_cb;
}

/**
 * Set a function to use the current value of the variable and make start and
 * end value relative to the returned current value.
 * @param a             pointer to an initialized `lv_anim_t` variable
 * @param get_value_cb  a function call when the animation starts
 */
static inline void
lv_anim_set_get_value_cb(lv_anim_t *a, lv_anim_get_value_cb_t get_value_cb) {
 ...
 */
static inline void lv_anim_set_ready_cb(lv_anim_t *a,
 ...
 * @param a         pointer to an initialized `lv_anim_t` variable
 * @param time      the duration of the playback animation in milliseconds. 0:
 * disable playback
 */
 ...
 * Make the animation to play back to when the forward direction is ready
 * @param a         pointer to an initialized `lv_anim_t` variable
 * @param delay     delay in milliseconds before starting the playback
 * animation.
 */
 ...
 * @param a         pointer to an initialized `lv_anim_t` variable
 * @param cnt       repeat count or `LV_ANIM_REPEAT_INFINITE` for infinite
 * repetition. 0: to disable repetition.
 */
...
/**
 * Set a whether the animation's should be applied immediately or only when the
 * delay expired.
 * @param a         pointer to an initialized `lv_anim_t` variable
 * @param en        true: apply the start value immediately in `lv_anim_start`;
 *                  false: apply the start value only when `delay` ms is elapsed
 * and the animations really starts
 */
 ...
 * @return          pointer to the created animation (different from the `a`
 * parameter)
 */
 ...
 */
static inline uint32_t lv_anim_get_delay(lv_anim_t *a) { return -a->act_time; }
 ...
 * @return          true: at least 1 animation is deleted, false: no animation
 * is deleted
 */
 ...
 * @param var       pointer to variable
 * @param exec_cb   a function pointer which is animating 'var', or NULL to
 * return first matching 'var'
 ...
 * This function exists because it's logical that all anim. functions receives
 * an `lv_anim_t` as their first parameter. It's not practical in C but might
 * make the API more consequent and makes easier to generate bindings.
 * @param a         pointer to an animation.
 * @param exec_cb   a function pointer which is animating 'var',
 *                  or NULL to ignore it and delete all the animations of 'var
 * @return          true: at least 1 animation is deleted, false: no animation
 * is deleted
 */
static inline bool lv_anim_custom_del(lv_anim_t *a,
 ...
 * @return          pointer to the animation.
 */
static inline lv_anim_t *lv_anim_custom_get(lv_anim_t *a,
                                            lv_anim_custom_exec_cb_t exec_cb) {
 ...
 * @return          the required time [ms] for the animation with the given
 * parameters
 */
