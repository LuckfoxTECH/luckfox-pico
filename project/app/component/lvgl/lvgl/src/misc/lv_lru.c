 ...
 */
static uint32_t lv_lru_hash(lv_lru_t *cache, const void *key,
                            uint32_t key_length);

/** compare a key against an existing item's key */
static int lv_lru_cmp_keys(lv_lru_item_t *item, const void *key,
                           uint32_t key_length);

/** remove an item and push it to the free items queue */
static void lv_lru_remove_item(lv_lru_t *cache, lv_lru_item_t *prev,
                               lv_lru_item_t *item, uint32_t hash_index);

/**
 * remove the least recently used item
 *
 * @todo we can optimise this by finding the n lru items, where n =
 * required_space / average_length
 */
...
#define test_for_missing_value()                                               \
  error_for(!value || value_length == 0, LV_LRU_MISSING_VALUE)
#define test_for_value_too_large()                                             \
  error_for(value_length > cache->total_memory, LV_LRU_VALUE_TOO_LARGE)

/**********************
 *   GLOBAL FUNCTIONS
 **********************/

lv_lru_t *lv_lru_create(size_t cache_size, size_t average_length,
                        lv_lru_free_t *value_free, lv_lru_free_t *key_free) {
  // create the cache
  ...
  // size the hash table to a guestimate of the number of slots required
  // (assuming a perfect hash)
  cache->items = (lv_lru_item_t **)lv_mem_alloc(sizeof(lv_lru_item_t *) *
...
lv_lru_res_t lv_lru_set(lv_lru_t *cache, const void *key, size_t key_length,
...
lv_lru_res_t lv_lru_get(lv_lru_t *cache, const void *key, size_t key_size,
                        void **value) {
  test_for_missing_cache();
  test_for_missing_key();
