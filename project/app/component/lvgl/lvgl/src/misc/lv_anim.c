...
lv_anim_t *lv_anim_start(const lv_anim_t *a) {
  ...
  if (a->exec_cb != NULL)
    lv_anim_del(
        a->var,
        a->exec_cb); /*exec_cb == NULL would delete all animations of var*/

  /*If the list is empty the anim timer was suspended and it's last run measure
   * is invalid*/
...
uint32_t lv_anim_get_playtime(lv_anim_t *a) {
  ...
  playtime +=
      (a->repeat_delay + a->time + a->playback_delay + a->playback_time) *
      (a->repeat_cnt - 1);
...
bool lv_anim_del(void *var, lv_anim_exec_xcb_t exec_cb) {
  ...
  while (a != NULL) {
    ...
    if ((a->var == var || var == NULL) &&
        (a->exec_cb == exec_cb || exec_cb == NULL)) {
      _lv_ll_remove(&LV_GC_ROOT(_lv_anim_ll), a);
      lv_mem_free(a);
      anim_mark_list_change(); /*Read by `anim_timer`. It need to know if a
...
void lv_anim_refr_now(void) { anim_timer(NULL); }
...
static void anim_timer(lv_timer_t *param) {
  ...
  while (a != NULL) {
    ...
    if (a->run_round != anim_run_round) {
      a->run_round = anim_run_round; /*The list readying might be reset so need
                                        to know which anim has run already*/
...
static void anim_ready_handler(lv_anim_t *a) {
  ...
  if (a->playback_now == 0 && a->repeat_cnt > 0 &&
  ...
  if (a->repeat_cnt == 0 && (a->playback_time == 0 || a->playback_now == 1)) {
     ...
     * This way the `ready_cb` will see the animations like it's animation is
     * ready deleted*/
    _lv_ll_remove(&LV_GC_ROOT(_lv_anim_ll), a);
