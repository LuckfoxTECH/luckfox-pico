...
#define CIRCLE_CACHE_AGING(life, r)                                            \
  life = LV_MIN(life + (r < 16 ? 1 : (r >> 4)), 1000)
...
lv_draw_mask_line(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                  lv_coord_t len, lv_draw_mask_line_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_radius(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                    lv_coord_t len, lv_draw_mask_radius_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_angle(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                   lv_coord_t len, lv_draw_mask_angle_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_fade(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                  lv_coord_t len, lv_draw_mask_fade_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_map(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                 lv_coord_t len, lv_draw_mask_map_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_polygon(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                     lv_coord_t len, lv_draw_mask_polygon_param_t *param);

LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
line_mask_flat(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
               lv_coord_t len, lv_draw_mask_line_param_t *p);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
line_mask_steep(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                lv_coord_t len, lv_draw_mask_line_param_t *p);
...
static void circ_calc_aa4(_lv_draw_mask_radius_circle_dsc_t *c,
                          lv_coord_t radius);
static lv_opa_t *get_next_line(_lv_draw_mask_radius_circle_dsc_t *c,
                               lv_coord_t y, lv_coord_t *len,
                               lv_coord_t *x_start);
LV_ATTRIBUTE_FAST_MEM static inline lv_opa_t mask_mix(lv_opa_t mask_act,
                                                      lv_opa_t mask_new);
...
/**
 * Add a draw mask. Everything drawn after it (until removing the mask) will be
 * affected by the mask.
 * @param param an initialized mask parameter. Only the pointer is saved.
 * @param custom_id a custom pointer to identify the mask. Used in
 * `lv_draw_mask_remove_custom`.
 * @return the an integer, the ID of the mask. Can be used in
...
int16_t lv_draw_mask_add(void *param, void *custom_id) {
  ...
  for (i = 0; i < _LV_MASK_MAX_NUM; i++) {
    if (LV_GC_ROOT(_lv_draw_mask_list[i]).param == NULL)
      break;
  }
...
/**
 * Apply the added buffers on a line. Used internally by the library's drawing
 * routines.
 * @param mask_buf store the result mask here. Has to be `len` byte long. Should
 * be initialized with `0xFF`.
 ...
 */
LV_ATTRIBUTE_FAST_MEM lv_draw_mask_res_t lv_draw_mask_apply(lv_opa_t *mask_buf,
                                                            lv_coord_t abs_x,
                                                            lv_coord_t abs_y,
                                                            lv_coord_t len) {
  bool changed = false;
  ...
  while (m->param) {
    ...
    else if (res == LV_DRAW_MASK_RES_CHANGED)
      changed = true;
...
/**
 * Apply the specified buffers on a line. Used internally by the library's
 * drawing routines.
 * @param mask_buf store the result mask here. Has to be `len` byte long. Should
 * be initialized with `0xFF`.
 * @param abs_x absolute X coordinate where the line to calculate start
 ...
 * opacity of each pixel in the given line
 */
LV_ATTRIBUTE_FAST_MEM lv_draw_mask_res_t
lv_draw_mask_apply_ids(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
 ...
 * If more masks have `custom_id` ID then the last mask's parameter will be
 * returned
 */
 ...
 * Needs to be called only in special cases when the mask is not added by
 * `lv_draw_mask_add` and not removed by `lv_draw_mask_remove_id` or
 * `lv_draw_mask_remove_custom`
...
void _lv_draw_mask_cleanup(void) {
  ...
  for (i = 0; i < LV_CIRCLE_CACHE_SIZE; i++) {
    ...
    lv_memset_00(&LV_GC_ROOT(_lv_circle_cache[i]),
                 sizeof(LV_GC_ROOT(_lv_circle_cache[i])));
  }
}
...
LV_ATTRIBUTE_FAST_MEM uint8_t lv_draw_mask_get_cnt(void) {
  ...
  uint8_t i;
  for (i = 0; i < _LV_MASK_MAX_NUM; i++) {
    if (LV_GC_ROOT(_lv_draw_mask_list[i]).param)
      cnt++;
  }
  return cnt;
}
...
bool lv_draw_mask_is_any(const lv_area_t *a) {
  ...
  uint8_t i;
  for (i = 0; i < _LV_MASK_MAX_NUM; i++) {
    _lv_draw_mask_common_dsc_t *comm_param =
        LV_GC_ROOT(_lv_draw_mask_list[i]).param;
    if (comm_param == NULL)
      continue;
    if (comm_param->type == LV_DRAW_MASK_TYPE_RADIUS) {
      ...
      if (radius_param->cfg.outer) {
        if (!_lv_area_is_out(a, &radius_param->cfg.rect,
                             radius_param->cfg.radius))
          return true;
      } else {
        if (!_lv_area_is_in(a, &radius_param->cfg.rect,
                            radius_param->cfg.radius))
          return true;
      }
    } else {
 ...
 * @param side and element of `lv_draw_mask_line_side_t` to describe which side
 *to keep. With `LV_DRAW_MASK_LINE_SIDE_LEFT/RIGHT` and horizontal line all
 *pixels are kept With `LV_DRAW_MASK_LINE_SIDE_TOP/BOTTOM` and vertical line all
 *pixels are kept
 */
void lv_draw_mask_line_points_init(lv_draw_mask_line_param_t *param,
                                   lv_coord_t p1x, lv_coord_t p1y,
                                   lv_coord_t p2x, lv_coord_t p2y,
  ...
  if (param->cfg.side == LV_DRAW_MASK_LINE_SIDE_LEFT)
    param->inv = 0;
  else if (param->cfg.side == LV_DRAW_MASK_LINE_SIDE_RIGHT)
    param->inv = 1;
...
void lv_draw_mask_line_angle_init(lv_draw_mask_line_param_t *param,
   ...
   *Theoretically a line with `angle` or `angle+180` is the same only the points
   *are swapped Find the degree which keeps the origo in place*/
  if (angle > 180)
    angle -= 180; /*> 180 will swap the origo*/
 ...
 * @param start_angle start angle in degrees. 0 deg on the right, 90 deg, on the
 * bottom
 * @param end_angle end angle
 */
void lv_draw_mask_angle_init(lv_draw_mask_angle_param_t *param,
                             lv_coord_t vertex_x, lv_coord_t vertex_y,
  ...
  LV_ASSERT_MSG(start_angle >= 0 && start_angle <= 360,
                "Unexpected start angle");
  ...
  lv_draw_mask_line_angle_init(&param->start_line, vertex_x, vertex_y,
                               start_angle, start_side);
  lv_draw_mask_line_angle_init(&param->end_line, vertex_x, vertex_y, end_angle,
                               end_side);
}
 ...
 * @param inv true: keep the pixels inside the rectangle; keep the pixels
 * outside of the rectangle
 */
void lv_draw_mask_radius_init(lv_draw_mask_radius_param_t *param,
  ...
  for (i = 0; i < LV_CIRCLE_CACHE_SIZE; i++) {
    if (LV_GC_ROOT(_lv_circle_cache[i]).used_cnt == 0) {
      if (!entry)
        entry = &LV_GC_ROOT(_lv_circle_cache[i]);
      else if (LV_GC_ROOT(_lv_circle_cache[i]).life < entry->life)
        entry = &LV_GC_ROOT(_lv_circle_cache[i]);
    }
  }
 ...
 */
void lv_draw_mask_fade_init(lv_draw_mask_fade_param_t *param,
                            const lv_area_t *coords, lv_opa_t opa_top,
                            lv_coord_t y_top, lv_opa_t opa_bottom,
                            lv_coord_t y_bottom) {
 ...
 */
void lv_draw_mask_map_init(lv_draw_mask_map_param_t *param,
                           const lv_area_t *coords, const lv_opa_t *map) {
  lv_area_copy(&param->cfg.coords, coords);
  param->cfg.map = map;
...
void lv_draw_mask_polygon_init(lv_draw_mask_polygon_param_t *param,
  ...
  if (points[0].x != points[point_cnt - 1].x ||
...
line_mask_flat(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
  ...
  int32_t xe;
  if (p->yx_steep > 0)
    xe = ((abs_y * 256) * p->xy_steep) >> 10;
  else
    xe = (((abs_y + 1) * 256) * p->xy_steep) >> 10;
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_angle(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                   lv_coord_t len, lv_draw_mask_angle_param_t *p) {
  ...
  if (p->cfg.start_angle < 180 && p->cfg.end_angle < 180 &&
    ...
    /*Do not let the line end cross the vertex else it will affect the opposite
     * part*/
    if (p->cfg.start_angle > 270 && p->cfg.start_angle <= 359 &&
        start_angle_last < 0)
      start_angle_last = 0;
    else if (p->cfg.start_angle > 0 && p->cfg.start_angle <= 90 &&
             start_angle_last < 0)
      start_angle_last = 0;
    else if (p->cfg.start_angle > 90 && p->cfg.start_angle < 270 &&
             start_angle_last > 0)
      start_angle_last = 0;

    if (p->cfg.end_angle > 270 && p->cfg.end_angle <= 359 &&
        start_angle_last < 0)
      start_angle_last = 0;
    else if (p->cfg.end_angle > 0 && p->cfg.end_angle <= 90 &&
             start_angle_last < 0)
      start_angle_last = 0;
    else if (p->cfg.end_angle > 90 && p->cfg.end_angle < 270 &&
             start_angle_last > 0)
      start_angle_last = 0;
    ...
    if (tmp < 0)
      tmp = 0;
    res2 = lv_draw_mask_line(&mask_buf[tmp], abs_x + tmp, abs_y, len - tmp,
                             &p->end_line);
    ...
    if (res1 == res2)
      ...
      return LV_DRAW_MASK_RES_CHANGED;
  } else if (p->cfg.start_angle > 180 && p->cfg.end_angle > 180 &&
             p->cfg.start_angle > p->cfg.end_angle) {
    ...
    if (tmp > len)
      tmp = len;
    if (tmp > 0) {
      res1 = lv_draw_mask_line(&mask_buf[0], abs_x, abs_y, tmp,
                               (lv_draw_mask_line_param_t *)&p->end_line);
      if (res1 == LV_DRAW_MASK_RES_TRANSP) {
        lv_memset_00(&mask_buf[0], tmp);
      }
    }
    ...
    if (tmp < 0)
      tmp = 0;
    res2 = lv_draw_mask_line(&mask_buf[tmp], abs_x + tmp, abs_y, len - tmp,
                             (lv_draw_mask_line_param_t *)&p->start_line);
    if (res2 == LV_DRAW_MASK_RES_TRANSP) {
      lv_memset_00(&mask_buf[tmp], len - tmp);
    }
    if (res1 == res2)
    ...
    if (p->cfg.start_angle == 180) {
      if (abs_y < p->cfg.vertex_p.y)
        res1 = LV_DRAW_MASK_RES_FULL_COVER;
      else
    ...
    if (p->cfg.end_angle == 180) {
      if (abs_y < p->cfg.vertex_p.y)
        ...
        res2 = LV_DRAW_MASK_RES_FULL_COVER;
    ...
    if (res1 == LV_DRAW_MASK_RES_TRANSP || res2 == LV_DRAW_MASK_RES_TRANSP)
      return LV_DRAW_MASK_RES_TRANSP;
    else if (res1 == LV_DRAW_MASK_RES_UNKNOWN &&
             res2 == LV_DRAW_MASK_RES_UNKNOWN)
      return LV_DRAW_MASK_RES_TRANSP;
    else if (res1 == LV_DRAW_MASK_RES_FULL_COVER &&
             res2 == LV_DRAW_MASK_RES_FULL_COVER)
      return LV_DRAW_MASK_RES_FULL_COVER;
    else
      return LV_DRAW_MASK_RES_CHANGED;
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_radius(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                    lv_coord_t len, lv_draw_mask_radius_param_t *p) {
  ...
  //    abs_y, p->cfg.radius, p->cfg.outer ? "inv" : "norm");

  //    if( abs_x == 276 && abs_x + len - 1 == 479 && abs_y == 63 &&
  //    p->cfg.radius == 5 && p->cfg.outer == 1) {
  //        char x = 0;
  //    }
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_fade(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                  lv_coord_t len, lv_draw_mask_fade_param_t *p) {
  if (abs_y < p->cfg.coords.y1)
    return LV_DRAW_MASK_RES_FULL_COVER;
  if (abs_y > p->cfg.coords.y2)
    return LV_DRAW_MASK_RES_FULL_COVER;
  if (abs_x + len < p->cfg.coords.x1)
    return LV_DRAW_MASK_RES_FULL_COVER;
  if (abs_x > p->cfg.coords.x2)
    return LV_DRAW_MASK_RES_FULL_COVER;

  if (abs_x + len > p->cfg.coords.x2)
    len -= abs_x + len - p->cfg.coords.x2 - 1;
  ...
  if (abs_y <= p->cfg.y_top) {
    ...
    lv_opa_t opa_act =
        (int32_t)((int32_t)(abs_y - p->cfg.y_top) * opa_diff) / y_diff;
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t
lv_draw_mask_map(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
                 lv_coord_t len, lv_draw_mask_map_param_t *p) {
...
lv_draw_mask_polygon(lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y,
  ...
  for (i = 0; i < param->cfg.point_cnt; i++) {
    ...
    if (psign_prev && psign_prev == psign)
      continue;
    psign_prev = psign;
    line_cnt++;
    if (line_cnt == 2)
  ...
  lv_draw_mask_line_points_init(&line_p, lines[0].p1.x, lines[0].p1.y,
  ...
  if (line_p.steep == 0 && line_p.flat) {
    ...
    for (i = 0; i < len; i++) {
      mask_buf[i] =
          mask_mix(mask_buf[i], (abs_x + i >= x1 && abs_x + i <= x2) * 0xFF);
    }
    lv_draw_mask_free_param(&line_p);
    return LV_DRAW_MASK_RES_CHANGED;
  }
  lv_draw_mask_res_t res1 =
      lv_draw_mask_line(mask_buf, abs_x, abs_y, len, &line_p);
  lv_draw_mask_free_param(&line_p);
  if (res1 == LV_DRAW_MASK_RES_TRANSP) {
    return LV_DRAW_MASK_RES_TRANSP;
  }
  lv_draw_mask_line_points_init(&line_p, lines[1].p1.x, lines[1].p1.y,
  ...
  lv_draw_mask_res_t res2 =
      lv_draw_mask_line(mask_buf, abs_x, abs_y, len, &line_p);
  lv_draw_mask_free_param(&line_p);
  if (res2 == LV_DRAW_MASK_RES_TRANSP) {
    return LV_DRAW_MASK_RES_TRANSP;
  }
  if (res1 == LV_DRAW_MASK_RES_CHANGED || res2 == LV_DRAW_MASK_RES_CHANGED)
    return LV_DRAW_MASK_RES_CHANGED;
  return res1;
}
/**
 ...
 */
static bool circ_cont(lv_point_t *c) { return c->y <= c->x ? true : false; }
...
static void circ_calc_aa4(_lv_draw_mask_radius_circle_dsc_t *c,
  ...
  c->buf = lv_mem_alloc(radius * 6 +
                        6); /*Use uint16_t for opa_start_on_y and x_start_on_y*/
