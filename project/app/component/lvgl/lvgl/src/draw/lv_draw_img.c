 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static lv_res_t
decode_and_draw(lv_draw_ctx_t *draw_ctx, const lv_draw_img_dsc_t *draw_dsc,
                const lv_area_t *coords, const void *src);

static void show_error(lv_draw_ctx_t *draw_ctx, const lv_area_t *coords,
                       const char *msg);
 ...
 */
void lv_draw_img(lv_draw_ctx_t *draw_ctx, const lv_draw_img_dsc_t *dsc,
                 const lv_area_t *coords, const void *src) {
  if (src == NULL) {
...
lv_img_src_t lv_img_src_get_type(const void *src) {
  ...
  if (u8_p[0] >= 0x20 && u8_p[0] <= 0x7F) {
    img_src_type =
        LV_IMG_SRC_FILE; /*If it's an ASCII character then it's file name*/
  } else if (u8_p[0] >= 0x80) {
    img_src_type = LV_IMG_SRC_SYMBOL; /*Symbols begins after 0x7F*/
  } else {
    img_src_type =
        LV_IMG_SRC_VARIABLE; /*`lv_img_dsc_t` is draw to the first byte < 0x20*/
  }
...
decode_and_draw(lv_draw_ctx_t *draw_ctx, const lv_draw_img_dsc_t *draw_dsc,
  ...
  _lv_img_cache_entry_t *cdsc =
      _lv_img_cache_open(src, draw_dsc->recolor, draw_dsc->frame_id);
   ...
   *Just draw it!*/
    ...
    if (draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE) {
      ...
      _lv_img_buf_get_transformed_area(&map_area_rot, w, h, draw_dsc->angle,
    ...
    bool union_ok;
    union_ok =
        _lv_area_intersect(&clip_com, draw_ctx->clip_area, &map_area_rot);
    /*Out of mask. There is nothing to draw so the image is drawn
     * successfully.*/
    if (union_ok == false) {
      draw_cleanup(cdsc);
      return LV_RES_OK;
