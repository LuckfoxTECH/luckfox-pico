 ...
 * (Use `lv_img_src_get_type` to determine the type)
 * @param header store the info here
 * @return LV_RES_OK: info written correctly; LV_RES_INV: failed
 */
typedef lv_res_t (*lv_img_decoder_info_f_t)(struct _lv_img_decoder_t *decoder,
                                            const void *src,
 ...
 * initialized in it.
 */
typedef lv_res_t (*lv_img_decoder_open_f_t)(struct _lv_img_decoder_t *decoder,
 ...
 */
typedef lv_res_t (*lv_img_decoder_read_line_f_t)(
 ...
 * @param dsc pointer to decoder descriptor
 */
typedef void (*lv_img_decoder_close_f_t)(struct _lv_img_decoder_t *decoder,
                                         struct _lv_img_decoder_dsc_t *dsc);
...
typedef struct _lv_img_decoder_dsc_t {
  ...
  /**Info about the opened image: color format, size, etc. MUST be set in `open`
   * function*/
  lv_img_header_t header;

  /** Pointer to a buffer where the image's data (pixels) are stored in a
   * decoded, plain format. MUST be set in `open` function*/
 ...
 *  1) File name: E.g. "S:folder/img1.png" (The drivers needs to registered via
 * `lv_fs_add_drv()`) 2) Variable: Pointer to an `lv_img_dsc_t` variable 3)
...
/**
 * Open an image.
 * Try the created image decoder one by one. Once one is able to open the image
 * that decoder is save in `dsc`
 ...
 * @param frame_id the index of the frame. Used only with animated images, set 0
 * for normal images
 * @return LV_RES_OK: opened the image. `dsc->img_data` and `dsc->header` are
 * set. LV_RES_INV: none of the registered image decoders were able to open the
 * image.
 */
