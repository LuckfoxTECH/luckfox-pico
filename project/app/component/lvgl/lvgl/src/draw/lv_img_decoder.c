 ...
 **********************/
static lv_res_t
lv_img_decoder_built_in_line_true_color(lv_img_decoder_dsc_t *dsc, lv_coord_t x,
...
static lv_res_t lv_img_decoder_built_in_line_alpha(lv_img_decoder_dsc_t *dsc,
                                                   ...
                                                   lv_coord_t len,
                                                   uint8_t *buf);
static lv_res_t lv_img_decoder_built_in_line_indexed(lv_img_decoder_dsc_t *dsc,
                                                     lv_coord_t x, lv_coord_t y,
                                                     lv_coord_t len,
                                                     uint8_t *buf);
...
lv_res_t lv_img_decoder_open(lv_img_decoder_dsc_t *dsc, const void *src,
                             lv_color_t color, int32_t frame_id) {
 ...
 */
void lv_img_decoder_set_read_line_cb(
    lv_img_decoder_t *decoder, lv_img_decoder_read_line_f_t read_line_cb) {
...
lv_res_t lv_img_decoder_built_in_info(lv_img_decoder_t *decoder,
  ...
  if (src_type == LV_IMG_SRC_VARIABLE) {
    ...
    header->w = 1;
    header->h = 1;
    /*Symbols always have transparent parts. Important because of cover check in
     *the draw function. The actual value doesn't matter because lv_draw_label
     *will draw it*/
 ...
 * @param decoder the decoder where this function belongs
 * @param dsc pointer to decoder descriptor. `src`, `color` are already
 * initialized in it.
 * @return LV_RES_OK: the info is successfully stored in `header`; LV_RES_INV:
 * unknown format or other error.
 */
    ...
    if (dsc->src_type == LV_IMG_SRC_FILE) {
      ...
      for (i = 0; i < palette_size; i++) {
        ...
        user_data->palette[i] = lv_color_make(
            cur_color.ch.red, cur_color.ch.green, cur_color.ch.blue);
        user_data->opa[i] = cur_color.ch.alpha;
      }
    } else {
      /*The palette begins in the beginning of the image data. Just point to
       * it.*/
      lv_color32_t *palette_p =
          (lv_color32_t *)((lv_img_dsc_t *)dsc->src)->data;

      uint32_t i;
      for (i = 0; i < palette_size; i++) {
        user_data->palette[i] = lv_color_make(
            palette_p[i].ch.red, palette_p[i].ch.green, palette_p[i].ch.blue);
 ...
 * them in `buf`. Required only if the "open" function can't return with the
 * whole decoded pixel array.
...
static lv_res_t lv_img_decoder_built_in_line_alpha(lv_img_decoder_dsc_t *dsc,
  ...
  const lv_opa_t alpha2_opa_table[4] = {0, 85, 170,
                                        255}; /*Opacity mapping with bpp = 2*/
  const lv_opa_t alpha4_opa_table[16] = {
      0,  17, 34,  51, /*Opacity mapping with bpp = 4*/
      68, 85, 102, 119, 136, 153, 170, 187, 204, 221, 238, 255};

  /*Simply fill the buffer with the color. Later only the alpha value will be
   * modified.*/
  ...
  for (i = 0; i < len; i++) {
    ...
    /*Because of Alpha byte 16 bit color can start on odd address which can
     * cause crash*/
...
#error "Invalid LV_COLOR_DEPTH. Check it in lv_conf.h"
  ...
  if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
    ...
    lv_fs_seek(&user_data->f, ofs + 4,
               LV_FS_SEEK_SET); /*+4 to skip the header*/
