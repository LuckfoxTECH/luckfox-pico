 ...
 *      DEFINES
 *********************/
#define SPLIT_RADIUS_LIMIT                                                     \
  10 /*With radius greater than this the arc will drawn in quarters. A quarter \
        is drawn only if there is arc in it*/
#define SPLIT_ANGLE_GAP_LIMIT                                                  \
  60 /*With small gaps in the arc don't bother with splitting because there is \
        nothing to skip.*/
...
static void get_rounded_area(int16_t angle, lv_coord_t radius,
                             uint8_t thickness, lv_area_t *res_area);
...
void lv_draw_sw_arc(lv_draw_ctx_t *draw_ctx, const lv_draw_arc_dsc_t *dsc,
  ...
  area_out.x2 =
      center->x + radius -
      1; /*-1 because the center already belongs to the left/bottom part*/
  ...
  lv_draw_mask_angle_init(&mask_angle_param, center->x, center->y, start_angle,
                          end_angle);
  ...
  if (dsc->rounded) {
    ...
    if (_lv_area_intersect(&clip_area2, clip_area_ori, &round_area)) {
      lv_draw_mask_radius_init(&mask_end_param, &round_area, LV_RADIUS_CIRCLE,
                               false);
...
static void draw_quarter_0(quarter_draw_dsc_t *q) {
  ...
  if (q->start_quarter == 0 && q->end_quarter == 0 &&
      q->start_angle < q->end_angle) {
    /*Small arc here*/
    quarter_area.y1 =
        q->center->y +
        ((lv_trigo_sin(q->start_angle) * (q->radius - q->width)) >>
         LV_TRIGO_SHIFT);
    quarter_area.x2 =
        q->center->x +
        ((lv_trigo_sin(q->start_angle + 90) * (q->radius)) >> LV_TRIGO_SHIFT);

    quarter_area.y2 =
        q->center->y +
        ((lv_trigo_sin(q->end_angle) * q->radius) >> LV_TRIGO_SHIFT);
    quarter_area.x1 =
        q->center->x +
        ((lv_trigo_sin(q->end_angle + 90) * (q->radius - q->width)) >>
         LV_TRIGO_SHIFT);
  ...
  } else if ((q->start_quarter == q->end_quarter && q->start_quarter != 0 &&
              q->end_angle < q->start_angle) ||
...
static void draw_quarter_1(quarter_draw_dsc_t *q) {
  ...
  if (q->start_quarter == 1 && q->end_quarter == 1 &&
      q->start_angle < q->end_angle) {
    /*Small arc here*/
    quarter_area.y2 =
        q->center->y +
        ((lv_trigo_sin(q->start_angle) * (q->radius)) >> LV_TRIGO_SHIFT);
    quarter_area.x2 =
        q->center->x +
        ((lv_trigo_sin(q->start_angle + 90) * (q->radius - q->width)) >>
         LV_TRIGO_SHIFT);

    quarter_area.y1 =
        q->center->y + ((lv_trigo_sin(q->end_angle) * (q->radius - q->width)) >>
                        LV_TRIGO_SHIFT);
    quarter_area.x1 =
        q->center->x +
        ((lv_trigo_sin(q->end_angle + 90) * (q->radius)) >> LV_TRIGO_SHIFT);
    ...
    if (q->end_quarter == 1) {
      ...
      quarter_area.y1 =
          q->center->y +
          ((lv_trigo_sin(q->end_angle) * (q->radius - q->width)) >>
           LV_TRIGO_SHIFT);
      quarter_area.x1 =
          q->center->x +
          ((lv_trigo_sin(q->end_angle + 90) * (q->radius)) >> LV_TRIGO_SHIFT);
  ...
  } else if ((q->start_quarter == q->end_quarter && q->start_quarter != 1 &&
              q->end_angle < q->start_angle) ||
...
static void draw_quarter_2(quarter_draw_dsc_t *q) {
  ...
  if (q->start_quarter == 2 && q->end_quarter == 2 &&
      q->start_angle < q->end_angle) {
    /*Small arc here*/
    quarter_area.x1 =
        q->center->x +
  ...
  } else if ((q->start_quarter == q->end_quarter && q->start_quarter != 2 &&
              q->end_angle < q->start_angle) ||
             (q->start_quarter == 0 && q->end_quarter == 3) ||
...
static void draw_quarter_3(quarter_draw_dsc_t *q) {
  ...
  if (q->start_quarter == 3 && q->end_quarter == 3 &&
      q->start_angle < q->end_angle) {
    /*Small arc here*/
    quarter_area.x1 =
        q->center->x +
  ...
  } else if ((q->start_quarter == q->end_quarter && q->start_quarter != 3 &&
              q->end_angle < q->start_angle) ||
...
static void get_rounded_area(int16_t angle, lv_coord_t radius,
  ...
  cir_x = ((radius - thick_half) * lv_trigo_sin(90 - angle)) >>
          (LV_TRIGO_SHIFT - ps);
  cir_y =
      ((radius - thick_half) * lv_trigo_sin(angle)) >> (LV_TRIGO_SHIFT - ps);

  /*Actually the center of the pixel need to be calculated so apply 1/2 px
   * offset*/
  if (cir_x > 0) {
    cir_x = (cir_x - pa) >> ps;
