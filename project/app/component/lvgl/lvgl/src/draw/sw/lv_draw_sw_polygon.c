 ...
 */
void lv_draw_sw_polygon(lv_draw_ctx_t *draw_ctx,
  ...
  lv_area_t poly_coords = {.x1 = LV_COORD_MAX,
                           .y1 = LV_COORD_MAX,
                           .x2 = LV_COORD_MIN,
                           .y2 = LV_COORD_MIN};
  ...
  lv_draw_mask_line_param_t *mp =
      lv_mem_buf_get(sizeof(lv_draw_mask_line_param_t) * point_cnt);
  ...
  if (i_next_left < 0)
    i_next_left = point_cnt + i_next_left;

  i_next_right = y_min_i + 1;
  if (i_next_right > point_cnt - 1)
    i_next_right = 0;
  ...
  do {
    if (!inv) {
      ...
      i_next_left = i_prev_left + 1;
      if (i_next_left > point_cnt - 1)
        i_next_left = 0;

      i_next_right = i_prev_right - 1;
      if (i_next_right < 0)
        i_next_right = point_cnt + i_next_right;
    }
    ...
    if (p[i_next_left].y >= p[i_prev_left].y) {
      if (p[i_next_left].y != p[i_prev_left].y &&
        ...
        lv_draw_mask_line_points_init(
            mp_next, p[i_prev_left].x, p[i_prev_left].y, p[i_next_left].x,
            p[i_next_left].y, LV_DRAW_MASK_LINE_SIDE_RIGHT);
    ...
    if (p[i_next_right].y >= p[i_prev_right].y) {
      if (p[i_next_right].y != p[i_prev_right].y &&
        ...
        lv_draw_mask_line_points_init(
            mp_next, p[i_prev_right].x, p[i_prev_right].y, p[i_next_right].x,
            p[i_next_right].y, LV_DRAW_MASK_LINE_SIDE_LEFT);
        lv_draw_mask_add(mp_next, mp);
        mp_next++;
      }
      mask_cnt++;
