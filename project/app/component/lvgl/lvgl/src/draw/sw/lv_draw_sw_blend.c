...
static void fill_set_px(lv_color_t *dest_buf, const lv_area_t *blend_area,
                        lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa,
                        const lv_opa_t *mask, lv_coord_t mask_stide);
LV_ATTRIBUTE_FAST_MEM static void
fill_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
            lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa,
...
static void map_set_px(lv_color_t *dest_buf, const lv_area_t *dest_area,
                       lv_coord_t dest_stride, const lv_color_t *src_buf,
                       lv_coord_t src_stride, lv_opa_t opa,
                       const lv_opa_t *mask, lv_coord_t mask_stride);

LV_ATTRIBUTE_FAST_MEM static void
map_normal(lv_color_t *dest_buf, const lv_area_t *dest_area,
           lv_coord_t dest_stride, const lv_color_t *src_buf,
           lv_coord_t src_stride, lv_opa_t opa, const lv_opa_t *mask,
           lv_coord_t mask_stride);

#if LV_DRAW_COMPLEX
static void map_blended(lv_color_t *dest_buf, const lv_area_t *dest_area,
                        lv_coord_t dest_stride, const lv_color_t *src_buf,
                        lv_coord_t src_stride, lv_opa_t opa,
                        const lv_opa_t *mask, lv_coord_t mask_stride,
                        lv_blend_mode_t blend_mode);
...
color_blend_true_color_additive(lv_color_t fg, lv_color_t bg, lv_opa_t opa);
static inline lv_color_t
color_blend_true_color_subtractive(lv_color_t fg, lv_color_t bg, lv_opa_t opa);
static inline lv_color_t
color_blend_true_color_multiply(lv_color_t fg, lv_color_t bg, lv_opa_t opa);
#endif /*LV_DRAW_COMPLEX*/
...
#define FILL_NORMAL_MASK_PX(color)                                             \
  if (*mask == LV_OPA_COVER)                                                   \
    ...
    *dest_buf = lv_color_mix(color, *dest_buf, *mask);                         \
  mask++;                                                                      \
  dest_buf++;
...
#define FILL_NORMAL_MASK_PX(color)                                             \
  if (*mask == LV_OPA_COVER)                                                   \
    *dest_buf = color;                                                         \
    ...
    lv_color_mix_with_alpha(*dest_buf, dest_buf->ch.alpha, color, *mask,       \
                            dest_buf, &dest_buf->ch.alpha);                    \
  else                                                                         \
    *dest_buf = lv_color_mix(color, *dest_buf, *mask);                         \
  mask++;                                                                      \
  dest_buf++;
...
#define MAP_NORMAL_MASK_PX(x)                                                  \
  if (*mask_tmp_x) {                                                           \
    if (*mask_tmp_x == LV_OPA_COVER)                                           \
      ...
      dest_buf[x] = lv_color_mix(src_buf[x], dest_buf[x], *mask_tmp_x);        \
  }                                                                            \
  mask_tmp_x++;
...
#define MAP_NORMAL_MASK_PX_SCR_TRANSP(x)                                       \
  if (*mask_tmp_x) {                                                           \
    if (*mask_tmp_x == LV_OPA_COVER)                                           \
      ...
      lv_color_mix_with_alpha(dest_buf[x], dest_buf[x].ch.alpha, src_buf[x],   \
                              *mask_tmp_x, &dest_buf[x],                       \
                              &dest_buf[x].ch.alpha);                          \
    else                                                                       \
      dest_buf[x] = lv_color_mix(src_buf[x], dest_buf[x], *mask_tmp_x);        \
  }                                                                            \
...
void lv_draw_sw_blend(lv_draw_ctx_t *draw_ctx,
...
LV_ATTRIBUTE_FAST_MEM void
lv_draw_sw_blend_basic(lv_draw_ctx_t *draw_ctx,
                       const lv_draw_sw_blend_dsc_t *dsc) {
  const lv_opa_t *mask;
  ...
  if (mask) {
    ...
    mask += mask_stride * (dsc->mask_area->y1 - blend_area.y1) +
            (dsc->mask_area->x1 - blend_area.x1);
  } else {
    mask_stride = 0;
  }
  ...
  if (disp->driver->set_px_cb) {
    if (dsc->src_buf == NULL) {
      fill_set_px(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa,
                  mask, mask_stride);
    } else {
      map_set_px(dest_buf, &blend_area, dest_stride, src_buf, src_stride,
                 dsc->opa, mask, mask_stride);
    }
  } else if (dsc->src_buf == NULL) {
    if (dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
      fill_normal(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa,
                  mask, mask_stride);
    }
#if LV_DRAW_COMPLEX
    else {
      fill_blended(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa,
                   mask, mask_stride, dsc->blend_mode);
    }
#endif
  } else {
    if (dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
      map_normal(dest_buf, &blend_area, dest_stride, src_buf, src_stride,
                 dsc->opa, mask, mask_stride);
    }
#if LV_DRAW_COMPLEX
    else {
      map_blended(dest_buf, &blend_area, dest_stride, src_buf, src_stride,
                  dsc->opa, mask, mask_stride, dsc->blend_mode);
    }
#endif
  }
}
  ...
  if (mask == NULL) {
    for (y = blend_area->y1; y <= blend_area->y2; y++) {
      for (x = blend_area->x1; x <= blend_area->x2; x++) {
        disp->driver->set_px_cb(disp->driver, (void *)dest_buf, dest_stride, x,
                                y, color, opa);
      }
    }
  } else {
    ...
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++) {
        if (mask[x]) {
          disp->driver->set_px_cb(disp->driver, (void *)dest_buf, dest_stride,
                                  blend_area->x1 + x, blend_area->y1 + y, color,
        ...
        for (x = 0; x < w; x++) {
          if (last_dest_color.full != dest_buf[x].full) {
            ...
            if (disp->driver->screen_transp) {
              lv_color_mix_with_alpha(dest_buf[x], dest_buf[x].ch.alpha, color,
                                      opa, &last_res_color,
            ...
            {
              last_res_color =
                  lv_color_mix_premult(color_premult, dest_buf[x], opa_inv);
            }
          }
...
#if LV_COLOR_SCREEN_TRANSP
              if (disp->driver->screen_transp) {
                lv_color_mix_with_alpha(dest_buf[x], dest_buf[x].ch.alpha,
                                        color, opa_tmp, &last_res_color,
                                        &last_res_color.ch.alpha);
              } else
#endif
              {
                if (opa_tmp == LV_OPA_COVER)
                  ...
                  last_res_color = lv_color_mix(color, dest_buf[x], opa_tmp);
              }
              last_mask = *mask;
              last_dest_color.full = dest_buf[x].full;
            }
...
static void fill_blended(lv_color_t *dest_buf, const lv_area_t *dest_area,
  ...
  /*Masked*/
    ...
    lv_opa_t opa_tmp =
        mask[0] >= LV_OPA_MAX ? opa : (uint32_t)((uint32_t)mask[0] * opa) >> 8;
    ...
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++) {
        ...
        if (mask[x] != last_mask || last_dest_color.full != dest_buf[x].full) {
          opa_tmp = mask[x] >= LV_OPA_MAX
                        ? opa
                        : (uint32_t)((uint32_t)mask[x] * opa) >> 8;
    ...
    /*Handle opa and mask values too*/
      ...
      for (y = 0; y < h; y++) {
        for (x = 0; x < w; x++) {
          if (mask[x]) {
            lv_opa_t opa_tmp =
                mask[x] >= LV_OPA_MAX ? opa : ((opa * mask[x]) >> 8);
#if LV_COLOR_SCREEN_TRANSP
            if (disp->driver->screen_transp) {
              lv_color_mix_with_alpha(dest_buf[x], dest_buf[x].ch.alpha,
                                      src_buf[x], opa_tmp, &dest_buf[x],
                                      &dest_buf[x].ch.alpha);
            } else
#endif
            {
...
#else
  tmp = (bg.ch.green_h << 3) + bg.ch.green_l + (fg.ch.green_h << 3) +
        fg.ch.green_l;
  tmp = LV_MIN(tmp, 63);
  fg.ch.green_h = tmp >> 3;
