...
draw_letter_normal(lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc,
                   const lv_point_t *pos, lv_font_glyph_dsc_t *g,
                   const uint8_t *map_p);

#if LV_DRAW_COMPLEX && LV_USE_FONT_SUBPX
static void draw_letter_subpx(lv_draw_ctx_t *draw_ctx,
...
const uint8_t _lv_bpp1_opa_table[2] = {
    0, 255}; /*Opacity mapping with bpp = 1 (Just for compatibility)*/
const uint8_t _lv_bpp2_opa_table[4] = {0, 85, 170,
                                       255}; /*Opacity mapping with bpp = 2*/

const uint8_t _lv_bpp3_opa_table[8] = {
    0,   36,  73,  109, /*Opacity mapping with bpp = 3*/
    146, 182, 219, 255};

const uint8_t _lv_bpp4_opa_table[16] = {
    0,  17, 34,  51, /*Opacity mapping with bpp = 4*/
    68, 85, 102, 119, 136, 153, 170, 187, 204, 221, 238, 255};

const uint8_t _lv_bpp8_opa_table[256] = {
 ...
 * @param mask_p the letter will be drawn only on this area  (truncated to
 * draw_buf area)
 ...
 */
void lv_draw_sw_letter(lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc,
  ...
  if (g_ret == false) {
    ...
    if (letter >= 0x20 && letter != 0xf8ff && /*LV_SYMBOL_DUMMY*/
      ...
      LV_LOG_WARN("lv_draw_letter: glyph dsc. not found for U+%X",
                  (unsigned int)letter);
    }
    return;
  }
  ...
  gpos.y = pos_p->y + (dsc->font->line_height - dsc->font->base_line) -
           g.box_h - g.ofs_y;
...
#else
    LV_LOG_WARN("Can't draw sub-pixel rendered letter because "
                "LV_USE_FONT_SUBPX == 0 in lv_conf.h");
#endif
  } else {
...
draw_letter_normal(lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc,
  ...
  if (opa < LV_OPA_MAX) {
    if (prev_opa != opa || prev_bpp != bpp) {
      uint32_t i;
      for (i = 0; i < shades; i++) {
        opa_table[i] = bpp_opa_table_p[i] == LV_OPA_COVER
                           ? opa
                           : ((bpp_opa_table_p[i] * opa) >> 8);
      }
    }
  ...
  int32_t col_start =
      pos->x >= draw_ctx->clip_area->x1 ? 0 : draw_ctx->clip_area->x1 - pos->x;
  int32_t col_end = pos->x + box_w <= draw_ctx->clip_area->x2
                        ? box_w
                        : draw_ctx->clip_area->x2 - pos->x + 1;
  int32_t row_start =
      pos->y >= draw_ctx->clip_area->y1 ? 0 : draw_ctx->clip_area->y1 - pos->y;
  int32_t row_end = pos->y + box_h <= draw_ctx->clip_area->y2
                        ? box_h
                        : draw_ctx->clip_area->y2 - pos->y + 1;
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    /*Apply masks if any*/
    if (mask_any) {
      blend_dsc.mask_res = lv_draw_mask_apply(
          mask_buf + mask_p_start, fill_area.x1, fill_area.y2, fill_w);
...
static void draw_letter_subpx(lv_draw_ctx_t *draw_ctx,
  ...
  /*Calculate the col/row start/end on the map*/
  int32_t col_start = pos->x >= draw_ctx->clip_area->x1
                          ? 0
                          : (draw_ctx->clip_area->x1 - pos->x) * 3;
  int32_t col_end = pos->x + box_w / 3 <= draw_ctx->clip_area->x2
                        ? box_w
                        : (draw_ctx->clip_area->x2 - pos->x + 1) * 3;
  int32_t row_start =
      pos->y >= draw_ctx->clip_area->y1 ? 0 : draw_ctx->clip_area->y1 - pos->y;
  int32_t row_end = pos->y + box_h <= draw_ctx->clip_area->y2
                        ? box_h
  ...
  lv_coord_t hor_res = lv_disp_get_hor_res(_lv_refr_get_disp_refreshing());
  int32_t mask_buf_size =
      box_w * box_h > hor_res ? hor_res : g->box_w * g->box_h;
  lv_opa_t *mask_buf = lv_mem_buf_get(mask_buf_size);
  int32_t mask_p = 0;
  ...
  dest_buf_tmp += ((pos->y - draw_ctx->buf_area->y1) * dest_buf_stride) +
                  pos->x - draw_ctx->buf_area->x1;
...
#else
  uint8_t txt_rgb[3] = {
      color.ch.red, (color.ch.green_h << 3) + color.ch.green_l, color.ch.blue};
#endif
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    for (col = col_start; col < col_end; col++) {
      ...
      if (letter_px != 0) {
        if (opa >= LV_OPA_MAX) {
          ...
          px_opa =
              bpp == 8
                  ? (uint32_t)((uint32_t)letter_px * opa) >> 8
...
#if LV_COLOR_16_SWAP == 0
        uint8_t bg_rgb[3] = {dest_buf_tmp->ch.red, dest_buf_tmp->ch.green,
                             dest_buf_tmp->ch.blue};
#else
        uint8_t bg_rgb[3] = {dest_buf_tmp->ch.red,
                             (dest_buf_tmp->ch.green_h << 3) +
                                 dest_buf_tmp->ch.green_l,
                             dest_buf_tmp->ch.blue};
#endif

#if LV_FONT_SUBPX_BGR
        res_color.ch.blue = (uint32_t)((uint32_t)txt_rgb[0] * font_rgb[0] +
                                       (bg_rgb[0] * (255 - font_rgb[0]))) >>
                            8;
        res_color.ch.red = (uint32_t)((uint32_t)txt_rgb[2] * font_rgb[2] +
                                      (bg_rgb[2] * (255 - font_rgb[2]))) >>
                           8;
#else
        res_color.ch.red = (uint32_t)((uint16_t)txt_rgb[0] * font_rgb[0] +
                                      (bg_rgb[0] * (255 - font_rgb[0]))) >>
                           8;
        res_color.ch.blue = (uint32_t)((uint16_t)txt_rgb[2] * font_rgb[2] +
                                       (bg_rgb[2] * (255 - font_rgb[2]))) >>
                            8;
#endif

#if LV_COLOR_16_SWAP == 0
        res_color.ch.green = (uint32_t)((uint32_t)txt_rgb[1] * font_rgb[1] +
                                        (bg_rgb[1] * (255 - font_rgb[1]))) >>
                             8;
#else
        uint8_t green = (uint32_t)((uint32_t)txt_rgb[1] * font_rgb[1] +
                                   (bg_rgb[1] * (255 - font_rgb[1]))) >>
                        8;
    ...
    if (mask_any) {
      blend_dsc.mask_res =
          lv_draw_mask_apply(mask_buf + mask_p_start, map_area.x1, map_area.y2,
