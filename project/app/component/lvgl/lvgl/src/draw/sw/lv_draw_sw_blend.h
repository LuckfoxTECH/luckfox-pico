...
typedef struct {
  const lv_area_t *blend_area; /**< The area with absolute coordinates to draw
                                * on `draw_ctx->buf`            will be clipped
                                * to `draw_ctx->clip_area` */
  ...
  const lv_area_t
      *mask_area; /**< The area of `mask_buf` with absolute coordinates*/
 ...
 */
void lv_draw_sw_blend(struct _lv_draw_ctx_t *draw_ctx,
...
lv_draw_sw_blend_basic(struct _lv_draw_ctx_t *draw_ctx,
                       const lv_draw_sw_blend_dsc_t *dsc);
