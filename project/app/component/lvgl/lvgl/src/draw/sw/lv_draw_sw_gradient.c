...
#define MAX_WIN_RES                                                            \
  1024 /**TODO: Find a way to get this information: max(horz_res, vert_res)*/

#if _DITHER_GRADIENT
#if LV_DITHER_ERROR_DIFFUSION == 1
#define LV_DEFAULT_GRAD_CACHE_SIZE                                             \
  sizeof(lv_gradient_cache_t) + MAX_WIN_RES * sizeof(lv_grad_color_t) +        \
      MAX_WIN_RES * sizeof(lv_color_t) + MAX_WIN_RES * sizeof(lv_scolor24_t)
#else
#define LV_DEFAULT_GRAD_CACHE_SIZE                                             \
  sizeof(lv_gradient_cache_t) + MAX_WIN_RES * sizeof(lv_grad_color_t) +        \
      MAX_WIN_RES * sizeof(lv_color_t)
...
static lv_grad_t *allocate_item(const lv_grad_dsc_t *g, lv_coord_t w,
                                lv_coord_t h);
...
static uint32_t compute_key(const lv_grad_dsc_t *g, lv_coord_t size,
                            lv_coord_t w) {
  union void_cast v;
  v.ptr = g;
  return (v.value ^ size ^ (w >> 1)); /*Yes, this is correct, it's like a hash
                                         that changes if the width changes*/
}
...
static lv_grad_t *next_in_cache(lv_grad_t *item) {
  ...
  if ((uint8_t *)item + s >= grad_cache_end)
    return NULL;
  else
    return (lv_grad_t *)((uint8_t *)item + s);
}
...
static lv_grad_t *allocate_item(const lv_grad_dsc_t *g, lv_coord_t w,
  ...
  lv_coord_t map_size =
      LV_MAX(w, h); /* The map is being used horizontally (width) unless
                       no dithering is selected where it's used vertically */

  size_t req_size =
      ALIGN(sizeof(lv_grad_t)) + ALIGN(map_size * sizeof(lv_color_t));
#if _DITHER_GRADIENT
  ...
  if (req_size + act_size < grad_cache_size) {
    ...
    /*Need to evict items from cache until we find enough space to allocate this
     * one */
...
#if _DITHER_GRADIENT
    item->hmap = (lv_color32_t *)(p + ALIGN(sizeof(*item)) +
                                  ALIGN(map_size * sizeof(lv_color_t)));
#if LV_DITHER_ERROR_DIFFUSION == 1
    item->error_acc = (lv_scolor24_t *)(p + ALIGN(sizeof(*item)) +
                                        ALIGN(size * sizeof(lv_grad_color_t)) +
                                        ALIGN(map_size * sizeof(lv_color_t)));
    item->w = w;
#endif
#endif
...
#if _DITHER_GRADIENT
    item->hmap = (lv_color32_t *)(grad_cache_end + ALIGN(sizeof(*item)) +
                                  ALIGN(map_size * sizeof(lv_color_t)));
#if LV_DITHER_ERROR_DIFFUSION == 1
    item->error_acc = (lv_scolor24_t *)(grad_cache_end + ALIGN(sizeof(*item)) +
                                        ALIGN(size * sizeof(lv_grad_color_t)) +
                                        ALIGN(map_size * sizeof(lv_color_t)));
    item->w = w;
#endif
...
LV_ATTRIBUTE_FAST_MEM lv_grad_color_t lv_gradient_calculate(
    const lv_grad_dsc_t *dsc, lv_coord_t range, lv_coord_t frac) {
  lv_grad_color_t tmp;
  ...
  lv_grad_color_t r =
      GRAD_CM(LV_UDIV255(two.ch.red * mix + one.ch.red * imix),
