...
#if LV_DRAW_COMPLEX
LV_ATTRIBUTE_FAST_MEM static void draw_shadow(lv_draw_ctx_t *draw_ctx,
                                              const lv_draw_rect_dsc_t *dsc,
                                              const lv_area_t *coords);
LV_ATTRIBUTE_FAST_MEM static void
shadow_draw_corner_buf(const lv_area_t *coords, uint16_t *sh_buf, lv_coord_t s,
                       lv_coord_t r);
LV_ATTRIBUTE_FAST_MEM static void
shadow_blur_corner(lv_coord_t size, lv_coord_t sw, uint16_t *sh_ups_buf);
#endif

void draw_border_generic(lv_draw_ctx_t *draw_ctx, const lv_area_t *outer_area,
                         const lv_area_t *inner_area, lv_coord_t rout,
                         lv_coord_t rin, lv_color_t color, lv_opa_t opa,
                         lv_blend_mode_t blend_mode);

static void draw_border_simple(lv_draw_ctx_t *draw_ctx,
                               const lv_area_t *outer_area,
                               const lv_area_t *inner_area, lv_color_t color,
                               lv_opa_t opa);
...
void lv_draw_sw_bg(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
                   const lv_area_t *coords) {
...
static void draw_bg(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
  ...
  /*If the border fully covers make the bg area 1px smaller to avoid artifacts
   * on the corners*/
  if (dsc->border_width > 1 && dsc->border_opa >= LV_OPA_MAX &&
      dsc->radius != 0) {
  ...
  lv_color_t bg_color = grad_dir == LV_GRAD_DIR_NONE
                            ? dsc->bg_color
                            : dsc->bg_grad.stops[0].color;
  ...
  if (grad_dir == LV_GRAD_DIR_VER && dither_mode != LV_DITHER_NONE) {
    /* When dithering, we are still using a map that's changing from line to
     * line*/
  ...
  if (mask_any) {
    for (h = clipped_coords.y1; h <= clipped_coords.y2; h++) {
      ...
      blend_dsc.mask_res =
          lv_draw_mask_apply(mask_buf, clipped_coords.x1, h, clipped_w);
  ...
  for (h = 0; h < rout; h++) {
    ...
    lv_memset(mask_buf, opa, clipped_w);
    blend_dsc.mask_res =
        lv_draw_mask_apply(mask_buf, blend_area.x1, top_y, clipped_w);
    if (blend_dsc.mask_res == LV_DRAW_MASK_RES_FULL_COVER)
      blend_dsc.mask_res = LV_DRAW_MASK_RES_CHANGED;
  ...
  /*With gradient and/or mask draw line by line*/
    ...
    for (h = bg_coords.y1 + rout; h <= h_end; h++) {
      /*If there is no other mask do not apply mask as in the center there is no
       * radius to mask*/
      if (mask_any_center) {
        lv_memset(mask_buf, opa, clipped_w);
        blend_dsc.mask_res =
...
static void draw_border(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
  ...
  draw_border_generic(draw_ctx, coords, &area_inner, rout, rin,
...
LV_ATTRIBUTE_FAST_MEM static void draw_shadow(lv_draw_ctx_t *draw_ctx,
  ...
  /*Calculate the rectangle which is blurred to get the shadow in
   * `shadow_area`*/
  lv_area_t core_area;
  ...
  lv_coord_t short_side =
      LV_MIN(lv_area_get_width(&bg_area), lv_area_get_height(&bg_area));
  ...
  short_side =
      LV_MIN(lv_area_get_width(&core_area), lv_area_get_height(&core_area));
  ...
  if (sh_cache_size == corner_size && sh_cache_r == r_sh) {
    ...
    shadow_draw_corner_buf(&core_area, (uint16_t *)sh_buf, dsc->shadow_width,
                           r_sh);
  ...
  /*Skip a lot of masking if the background will cover the shadow that would be
   * masked out*/
  ...
  /*Draw the corners if they are on the current clip area and not fully covered
   * by the bg*/
    ...
    if (w > 0) {
      ...
      for (y = clip_area_sub.y1; y <= clip_area_sub.y2; y++) {
        ...
        if (!simple_sub) {
          ...
          blend_dsc.mask_res =
              lv_draw_mask_apply(mask_buf, clip_area_sub.x1, y, w);
          if (blend_dsc.mask_res == LV_DRAW_MASK_RES_FULL_COVER)
            blend_dsc.mask_res = LV_DRAW_MASK_RES_CHANGED;
        } else {
    ...
    if (w > 0) {
      ...
      for (y = clip_area_sub.y1; y <= clip_area_sub.y2; y++) {
        ...
        if (!simple_sub) {
          ...
          blend_dsc.opa = opa == LV_OPA_COVER
                              ? sh_buf_tmp[0]
  ...
  /*Bottom left corner.
   *Almost the same as bottom right just read the lines of `sh_buf` from then
   *end*/
  blend_area.x1 = shadow_area.x1;
  blend_area.x2 = shadow_area.x1 + corner_size - 1;
  blend_area.y1 = shadow_area.y2 - corner_size + 1;
 ...
 * @param sh_buf a buffer to store the result. Its size should be `(sw + r)^2 *
 * 2`
 * @param sw shadow width
 * @param r radius
 */
LV_ATTRIBUTE_FAST_MEM static void
shadow_draw_corner_buf(const lv_area_t *coords, uint16_t *sh_buf, lv_coord_t sw,
                       lv_coord_t r) {
  int32_t sw_ori = sw;
  ...
  for (y = 0; y < size; y++) {
    ...
    lv_draw_mask_res_t mask_res =
        mask_param.dsc.cb(mask_line, 0, y, size, &mask_param);
...
static void draw_outline(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
  ...
  draw_border_generic(draw_ctx, &area_outer, &area_inner, rout, rin,
...
void draw_border_generic(lv_draw_ctx_t *draw_ctx, const lv_area_t *outer_area,
  ...
  if (!split_hor) {
    ...
    for (h = 0; h < max_h; h++) {
      ...
      lv_memset_ff(blend_dsc.mask_buf, draw_area_w);
      blend_dsc.mask_res = lv_draw_mask_apply(blend_dsc.mask_buf, blend_area.x1,
                                              top_y, draw_area_w);
