...
lv_color_t lv_img_buf_get_px_color(lv_img_dsc_t *dsc, lv_coord_t x,
  ...
  if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR ||
...
#if LV_COLOR_SIZE == 32
    p_color.ch.alpha =
        0xFF; /*Only the color should be get so use a default alpha value*/
#endif
     ...
     *aligned so the possible real width are 8, 16, 24 ...*/
     ...
     *aligned so the possible real width are 4, 8, 12 ...*/
     ...
     *aligned so the possible real width are 2, 4, 6 ...*/
 ...
 */
lv_opa_t lv_img_buf_get_px_alpha(lv_img_dsc_t *dsc, lv_coord_t x,
                                 lv_coord_t y) {
  uint8_t *buf_u8 = (uint8_t *)dsc->data;

  if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
    uint32_t px = dsc->header.w * y * LV_IMG_PX_SIZE_ALPHA_BYTE +
                  x * LV_IMG_PX_SIZE_ALPHA_BYTE;
    ...
    const uint8_t opa_table[4] = {0, 85, 170,
...
void lv_img_buf_set_px_color(lv_img_dsc_t *dsc, lv_coord_t x, lv_coord_t y,
  ...
  if (dsc->header.cf == LV_IMG_CF_TRUE_COLOR ||
    ...
    uint8_t px_size = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
    uint32_t px = dsc->header.w * y * px_size + x * px_size;
    lv_memcpy_small(&buf_u8[px], &c,
                    px_size - 1); /*-1 to not overwrite the alpha value*/
  } else if (dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
...
void _lv_img_buf_transform_init(lv_img_transform_dsc_t *dsc) {
  ...
  dsc->tmp.sinma =
      dsc->tmp.sinma >> (LV_TRIGO_SHIFT - _LV_TRANSFORM_TRIGO_SHIFT);
  dsc->tmp.cosma =
      dsc->tmp.cosma >> (LV_TRIGO_SHIFT - _LV_TRANSFORM_TRIGO_SHIFT);

  dsc->tmp.chroma_keyed = lv_img_cf_is_chroma_keyed(dsc->cfg.cf) ? 1 : 0;
  dsc->tmp.has_alpha = lv_img_cf_has_alpha(dsc->cfg.cf) ? 1 : 0;
  if (dsc->cfg.cf == LV_IMG_CF_TRUE_COLOR ||
      dsc->cfg.cf == LV_IMG_CF_TRUE_COLOR_ALPHA ||
  ...
  dsc->tmp.zoom_inv =
      (((256 * 256) << _LV_ZOOM_INV_UPSCALE) + dsc->cfg.zoom / 2) /
      dsc->cfg.zoom;
...
bool _lv_img_buf_transform(lv_img_transform_dsc_t *dsc, lv_coord_t x,
  ...
  if (dsc->cfg.zoom == LV_IMG_ZOOM_NONE) {
    ...
    xs = ((dsc->tmp.cosma * xt - dsc->tmp.sinma * yt) >>
          (_LV_TRANSFORM_TRIGO_SHIFT - 8)) +
         dsc->tmp.pivot_x_256;
    ys = ((dsc->tmp.sinma * xt + dsc->tmp.cosma * yt) >>
          (_LV_TRANSFORM_TRIGO_SHIFT - 8)) +
         dsc->tmp.pivot_y_256;
  ...
  if (xs_int >= dsc->cfg.src_w)
    return false;
  else if (xs_int < 0)
    return false;

  if (ys_int >= dsc->cfg.src_h)
    return false;
  else if (ys_int < 0)
    return false;
  ...
  if (dsc->tmp.native_color) {
    ...
    pxi = 0;     /*unused*/
    px_size = 0; /*unused*/
    dsc->res.color = lv_img_buf_get_px_color(&dsc->tmp.img_dsc, xs_int, ys_int,
                                             dsc->cfg.color);
  ...
  if (dsc->tmp.chroma_keyed) {
    ...
    if (dsc->res.color.full == ct.full)
      return false;
  }

  if (dsc->cfg.antialias == false)
    return true;
...
bool _lv_img_buf_transform_anti_alias(lv_img_transform_dsc_t *dsc) {
  ...
  if (xs_fract < 0x70) {
    xn = -1;
    if (dsc->tmp.xs_int + xn < 0)
      xn = 0;
    xr = xs_fract + 0x80;
  } else if (xs_fract > 0x90) {
    xn = 1;
    if (dsc->tmp.xs_int + xn >= dsc->cfg.src_w)
      xn = 0;
  ...
  if (ys_fract < 0x70) {
    yn = -1;
    if (dsc->tmp.ys_int + yn < 0)
      yn = 0;

    yr = ys_fract + 0x80;
  } else if (ys_fract > 0x90) {
    yn = 1;
    if (dsc->tmp.ys_int + yn >= dsc->cfg.src_h)
      yn = 0;
  ...
  if (dsc->tmp.native_color) {
    lv_memcpy_small(&c01, &src_u8[dsc->tmp.pxi + dsc->tmp.px_size * xn],
                    sizeof(lv_color_t));
    lv_memcpy_small(
        &c10, &src_u8[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn],
        sizeof(lv_color_t));
    lv_memcpy_small(
        &c11,
        &src_u8[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn +
                dsc->tmp.px_size * xn],
        sizeof(lv_color_t));
    if (dsc->tmp.has_alpha) {
      a10 = src_u8[dsc->tmp.pxi + dsc->tmp.px_size * xn + dsc->tmp.px_size - 1];
      a01 = src_u8[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn +
                   dsc->tmp.px_size - 1];
      a11 = src_u8[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn +
                   dsc->tmp.px_size * xn + dsc->tmp.px_size - 1];
    }
  } else {
    c01 = lv_img_buf_get_px_color(&dsc->tmp.img_dsc, dsc->tmp.xs_int + xn,
                                  dsc->tmp.ys_int, dsc->cfg.color);
    c10 = lv_img_buf_get_px_color(&dsc->tmp.img_dsc, dsc->tmp.xs_int,
                                  dsc->tmp.ys_int + yn, dsc->cfg.color);
    c11 = lv_img_buf_get_px_color(&dsc->tmp.img_dsc, dsc->tmp.xs_int + xn,
                                  dsc->tmp.ys_int + yn, dsc->cfg.color);

    if (dsc->tmp.has_alpha) {
      a10 = lv_img_buf_get_px_alpha(&dsc->tmp.img_dsc, dsc->tmp.xs_int + xn,
                                    dsc->tmp.ys_int);
      a01 = lv_img_buf_get_px_alpha(&dsc->tmp.img_dsc, dsc->tmp.xs_int,
                                    dsc->tmp.ys_int + yn);
      a11 = lv_img_buf_get_px_alpha(&dsc->tmp.img_dsc, dsc->tmp.xs_int + xn,
                                    dsc->tmp.ys_int + yn);
    }
  }
  ...
  if (dsc->tmp.has_alpha) {
    ...
    if (a0 <= LV_OPA_MIN && a1 <= LV_OPA_MIN)
      return false;
    if (a0 <= LV_OPA_MIN)
      yr = LV_OPA_TRANSP;
    if (a1 <= LV_OPA_MIN)
  ...
  lv_color_t c0;
  if (xr0 == LV_OPA_TRANSP)
    c0 = c01;
  else if (xr0 == LV_OPA_COVER)
    c0 = c00;
  else
    c0 = lv_color_mix(c00, c01, xr0);

  lv_color_t c1;
  if (xr1 == LV_OPA_TRANSP)
    c1 = c11;
  else if (xr1 == LV_OPA_COVER)
    c1 = c10;
  else
    c1 = lv_color_mix(c10, c11, xr1);

  if (yr == LV_OPA_TRANSP)
    dsc->res.color = c1;
  else if (yr == LV_OPA_COVER)
    dsc->res.color = c0;
  else
