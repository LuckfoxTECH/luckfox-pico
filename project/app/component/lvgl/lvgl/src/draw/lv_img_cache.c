...
/*Don't let life to be greater than this limit because it would require a lot of
 * time to "die" from very high values*/
 ...
 * allocated memory then it will remain. The image is closed if a new image is
 * opened and the new image takes its place in the cache.
 * @param src source of the image. Path to file or pointer to an `lv_img_dsc_t`
 * variable
...
_lv_img_cache_entry_t *_lv_img_cache_open(const void *src, lv_color_t color,
  ...
  for (i = 0; i < entry_cnt; i++) {
    if (color.full == cache[i].dec_dsc.color.full &&
       ...
       *Image difficult to open should live longer to keep avoid frequent their
       *recaching. Therefore increase `life` with `time_to_open`*/
      cached_src = &cache[i];
      cached_src->life +=
          cached_src->dec_dsc.time_to_open * LV_IMG_CACHE_LIFE_GAIN;
  ...
  lv_res_t open_res =
      lv_img_decoder_open(&cached_src->dec_dsc, src, color, frame_id);
  if (open_res == LV_RES_INV) {
    LV_LOG_WARN("Image draw cannot open the image resource");
    lv_memset_00(cached_src, sizeof(_lv_img_cache_entry_t));
    cached_src->life =
        INT32_MIN; /*Make the empty entry very "weak" to force its us*/
    return NULL;
  }
 ...
 * More cached images mean more opened image at same time which might mean more
 * memory usage. E.g. if 20 PNG or JPG images are open in the RAM they consume
...
void lv_img_cache_set_size(uint16_t new_entry_cnt) {
  ...
  LV_LOG_WARN("Can't change cache size because it's disabled by "
  ...
  LV_GC_ROOT(_lv_img_cache_array) =
      lv_mem_alloc(sizeof(_lv_img_cache_entry_t) * new_entry_cnt);
  ...
  /*Clean the cache*/
  lv_memset_00(LV_GC_ROOT(_lv_img_cache_array),
               entry_cnt * sizeof(_lv_img_cache_entry_t));
#endif
}
