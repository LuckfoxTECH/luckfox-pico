...
static void draw_shadow(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
                        const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc);
...
static lv_draw_rect_shadow_key_t
rect_shadow_key_create(lv_coord_t radius, lv_coord_t size, lv_coord_t blur);

static lv_draw_rect_border_key_t
rect_border_key_create(lv_coord_t rout, lv_coord_t rin,
                       const lv_area_t *outer_area,
                       const lv_area_t *inner_area);
 ...
 *      MACROS
 **********************/
#define SKIP_BORDER(dsc)                                                       \
  ((dsc)->border_opa <= LV_OPA_MIN || (dsc)->border_width == 0 ||              \
   (dsc)->border_side == LV_BORDER_SIDE_NONE || (dsc)->border_post)
#define SKIP_SHADOW(dsc)                                                       \
  ((dsc)->shadow_width == 0 || (dsc)->shadow_opa <= LV_OPA_MIN ||              \
   ((dsc)->shadow_width == 1 && (dsc)->shadow_spread <= 0 &&                   \
    (dsc)->shadow_ofs_x == 0 && (dsc)->shadow_ofs_y == 0))
#define SKIP_IMAGE(dsc)                                                        \
  ((dsc)->bg_img_src == NULL || (dsc)->bg_img_opa <= LV_OPA_MIN)
#define SKIP_OUTLINE(dsc)                                                      \
...
void lv_draw_sdl_draw_rect(lv_draw_ctx_t *draw_ctx,
  ...
  if (!SKIP_SHADOW(dsc)) {
    lv_coord_t ext =
        (lv_coord_t)(dsc->shadow_spread - dsc->shadow_width / 2 + 1);
  ...
  lv_draw_sdl_composite_begin(ctx, coords, clip, &extension, dsc->blend_mode,
...
SDL_Texture *lv_draw_sdl_rect_bg_frag_obtain(lv_draw_sdl_ctx_t *ctx,
  ...
  if (texture == NULL) {
    ...
    texture =
        lv_draw_sdl_mask_dump_texture(ctx->renderer, &coords_frag, &mask_id, 1);
...
void lv_draw_sdl_rect_bg_frag_draw_corners(lv_draw_sdl_ctx_t *ctx,
  ...
  if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
    ...
    if (full) {
      ...
      SDL_Rect src_rect = {corner_area.x2 - dst_area.x2,
                           dst_area.y1 - corner_area.y1, dw, dh};
  ...
  if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
    ...
    if (full) {
      ...
      SDL_Rect src_rect = {corner_area.x2 - dst_area.x2,
                           corner_area.y2 - dst_area.y2, dw, dh};
      SDL_RenderCopyEx(ctx->renderer, frag, &src_rect, &dst_rect, 0, NULL,
  ...
  if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
    ...
    if (full) {
      ...
      SDL_Rect src_rect = {dst_area.x1 - corner_area.x1,
                           corner_area.y2 - dst_area.y2, dw, dh};
      SDL_RenderCopyEx(ctx->renderer, frag, &src_rect, &dst_rect, 0, NULL,
                       SDL_FLIP_VERTICAL);
    }
...
static void draw_bg_color(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
  ...
  lv_coord_t bg_w = lv_area_get_width(coords),
             bg_h = lv_area_get_height(coords);
...
static void draw_bg_img(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
  ...
  if (src_type == LV_IMG_SRC_SYMBOL) {
    ...
    lv_draw_label((lv_draw_ctx_t *)ctx, &label_draw_dsc, &a, dsc->bg_img_src,
                  NULL);
  } else {
    lv_img_header_t header;
    size_t key_size;
    lv_draw_sdl_cache_key_head_img_t *key =
        lv_draw_sdl_texture_img_key_create(dsc->bg_img_src, 0, &key_size);
    bool key_found;
    lv_img_header_t *cache_header = NULL;
    SDL_Texture *texture = lv_draw_sdl_texture_cache_get_with_userdata(
        ctx, key, key_size, &key_found, (void **)&cache_header);
    SDL_free(key);
    if (texture) {
      header = *cache_header;
    } else if (key_found ||
               lv_img_decoder_get_info(dsc->bg_img_src, &header) != LV_RES_OK) {
      /* When cache hit but with negative result, use default decoder. If still
       * fail, return.*/
    ...
    if (dsc->bg_img_tiled == false) {
      ...
      for (; area.y1 <= coords->y2; area.y1 += header.h, area.y2 += header.h) {
        ...
        area.x1 = coords->x1;
        area.x2 = area.x1 + header.w - 1;
        for (; area.x1 <= coords->x2;
             area.x1 += header.w, area.x2 += header.w) {
          lv_draw_img((lv_draw_ctx_t *)ctx, &img_dsc, &area, dsc->bg_img_src);
        }
      }
    }
...
static void draw_shadow(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
  ...
  lv_coord_t frag_size =
      LV_MIN3(lv_area_get_width(&core_area) / 2,
              lv_area_get_height(&core_area) / 2, LV_MAX(sw / 2, radius));
  ...
  SDL_Texture *texture =
      lv_draw_sdl_texture_cache_get(ctx, &key, sizeof(key), NULL);
  if (texture == NULL) {
    lv_area_t mask_area = {blur_growth, blur_growth},
              mask_area_blurred = {0, 0};
    ...
    int16_t mask_id = lv_draw_mask_add(&mask_rout_param, NULL);
    lv_opa_t *mask_buf =
        lv_draw_sdl_mask_dump_opa(&mask_area_blurred, &mask_id, 1);
  ...
  SDL_SetTextureColorMod(texture, shadow_color.r, shadow_color.g,
                         shadow_color.b);

  lv_draw_sdl_rect_bg_frag_draw_corners(ctx, texture, blur_frag_size,
                                        &shadow_area, clip, false);
  frag_render_borders(ctx->renderer, texture, blur_frag_size, &shadow_area,
                      clip, false);
...
static void draw_border(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
  ...
  lv_area_copy(&area_inner,
               coords); //        lv_area_increase(&area_inner, 1, 1);
  area_inner.x1 +=
      ((dsc->border_side & LV_BORDER_SIDE_LEFT) ? dsc->border_width
                                                : -(dsc->border_width + rout));
  area_inner.x2 -=
      ((dsc->border_side & LV_BORDER_SIDE_RIGHT) ? dsc->border_width
                                                 : -(dsc->border_width + rout));
  area_inner.y1 +=
      ((dsc->border_side & LV_BORDER_SIDE_TOP) ? dsc->border_width
                                               : -(dsc->border_width + rout));
...
static void draw_outline(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords,
  ...
  /*Bring the outline closer to make sure there is no color bleeding with
   * pad=0*/
...
static void draw_border_generic(lv_draw_sdl_ctx_t *ctx,
  ...
  lv_draw_rect_border_key_t key =
      rect_border_key_create(rout, rin, outer_area, inner_area);
  lv_coord_t radius = LV_MIN3(rout, lv_area_get_width(outer_area) / 2,
                              lv_area_get_height(outer_area) / 2);
  lv_coord_t max_side =
      LV_MAX4(key.offsets.x1, key.offsets.y1, -key.offsets.x2, -key.offsets.y2);
  ...
  if (texture == NULL) {
    ...
    if (rin < 0)
      rin = 0;
    const lv_area_t frag_inner_area = {
        frag_area.x1 + key.offsets.x1, frag_area.y1 + key.offsets.y1,
  ...
  lv_draw_sdl_rect_bg_frag_draw_corners(ctx, texture, frag_size, outer_area,
                                        clip, true);
...
rect_border_key_create(lv_coord_t rout, lv_coord_t rin,
                       ...
                       const lv_area_t *inner_area) {
  lv_draw_rect_border_key_t key;
  /* VERY IMPORTANT! Padding between members is uninitialized, so we have to
   * wipe them manually */
  SDL_memset(&key, 0, sizeof(key));
