...
static lv_draw_line_key_t line_key_create(const lv_draw_line_dsc_t *dsc,
                                          lv_coord_t length);

static SDL_Texture *line_texture_create(lv_draw_sdl_ctx_t *sdl_ctx,
...
void lv_draw_sdl_draw_line(lv_draw_ctx_t *draw_ctx,
  ...
  SDL_Texture *texture =
      lv_draw_sdl_texture_cache_get(sdl_ctx, &key, sizeof(key), NULL);
  if (!texture) {
  ...
  lv_area_t extension = {dsc->width / 2, dsc->width / 2, dsc->width / 2,
                         dsc->width / 2};
  lv_draw_sdl_composite_begin(sdl_ctx, &coords, clip, &extension,
                              dsc->blend_mode, &t_coords, &t_clip, &apply_area);
  ...
  SDL_Rect srcrect = {0, 0, (int)length + dsc->width + 2, dsc->width + 2},
           dstrect = {t_coords.x1 - 1 - dsc->width / 2, t_coords.y1 - 1,
                      srcrect.w, srcrect.h};
...
static lv_draw_line_key_t line_key_create(const lv_draw_line_dsc_t *dsc,
  ...
  key.width = dsc->width;
  key.round =
      (dsc->round_start ? ROUND_START : 0) | (dsc->round_end ? ROUND_END : 0);
  return key;
}
...
static SDL_Texture *line_texture_create(lv_draw_sdl_ctx_t *sdl_ctx,
  ...
  if (dsc->round_start || dsc->round_end) {
    ...
    SDL_Texture *round_texture = lv_draw_sdl_mask_dump_texture(
        sdl_ctx->renderer, &round_area, &mask_id, 1);
    lv_draw_mask_remove_id(mask_id);

    SDL_Rect round_src = {0, 0, dsc->width, dsc->width};
    SDL_Rect round_dst = {line_rect.x - dsc->width / 2, 1, dsc->width,
                          dsc->width};
