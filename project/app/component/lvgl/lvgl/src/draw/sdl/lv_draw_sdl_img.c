...
static SDL_Texture *upload_img_texture(SDL_Renderer *renderer,
                                       lv_img_decoder_dsc_t *dsc);

static SDL_Texture *upload_img_texture_fallback(SDL_Renderer *renderer,
                                                lv_img_decoder_dsc_t *dsc);

static bool check_mask_simple_radius(const lv_area_t *coords,
                                     lv_coord_t *radius);

static void draw_img_simple(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
                            const lv_draw_sdl_img_header_t *header,
                            const lv_draw_img_dsc_t *draw_dsc,
                            const lv_area_t *coords, const lv_area_t *clip);

static void draw_img_rounded(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
                             const lv_draw_sdl_img_header_t *header,
                             const lv_draw_img_dsc_t *draw_dsc,
                             const lv_area_t *coords, const lv_area_t *clip,
                             lv_coord_t radius);
...
static SDL_Texture *
img_rounded_frag_obtain(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
                        const lv_draw_sdl_img_header_t *header, int w, int h,
                        lv_coord_t radius);

static lv_draw_img_rounded_key_t rounded_key_create(const SDL_Texture *texture,
                                                    lv_coord_t w, lv_coord_t h,
                                                    lv_coord_t radius);

static void calc_draw_part(SDL_Texture *texture,
                           const lv_draw_sdl_img_header_t *header,
                           const lv_area_t *coords, const lv_area_t *clip,
                           SDL_Rect *clipped_src, SDL_Rect *clipped_dst);
/**********************
 *  STATIC VARIABLES
 **********************/
...
static void apply_recolor_opa(SDL_Texture *texture,
                              const lv_draw_img_dsc_t *draw_dsc);
...
lv_res_t lv_draw_sdl_img_core(lv_draw_ctx_t *draw_ctx,
                              const lv_draw_img_dsc_t *draw_dsc,
  ...
  lv_draw_sdl_cache_key_head_img_t *key =
      lv_draw_sdl_texture_img_key_create(src, draw_dsc->frame_id, &key_size);
  bool texture_found = false;
  lv_draw_sdl_img_header_t *header = NULL;
  SDL_Texture *texture = lv_draw_sdl_texture_cache_get_with_userdata(
      ctx, key, key_size, &texture_found, (void **)&header);
  if (!texture_found) {
    lv_draw_sdl_img_load_texture(ctx, key, key_size, src, draw_dsc->frame_id,
                                 &texture, &header);
  }
  SDL_free(key);
  if (!texture) {
  ...
  _lv_img_buf_get_transformed_area(&zoomed_cords, lv_area_get_width(coords),
                                   lv_area_get_height(coords), 0,
  ...
  if (!check_mask_simple_radius(&t_coords, &radius)) {
    lv_draw_sdl_composite_begin(ctx, &zoomed_cords, clip, NULL,
                                draw_dsc->blend_mode, &t_coords, &t_clip,
                                &apply_area);
  }
  ...
  if (radius > 0) {
    draw_img_rounded(ctx, texture, header, draw_dsc, &t_coords, &t_clip,
                     radius);
  } else {
...
bool lv_draw_sdl_img_load_texture(lv_draw_sdl_ctx_t *ctx,
  ...
  _lv_img_cache_entry_t *cdsc =
      _lv_img_cache_open(src, lv_color_white(), frame_id);
  ...
  if (cdsc) {
    ...
    if (dsc->user_data &&
        SDL_memcmp(dsc->user_data, LV_DRAW_SDL_DEC_DSC_TEXTURE_HEAD, 8) == 0) {
      lv_draw_sdl_dec_dsc_userdata_t *ptr =
          (lv_draw_sdl_dec_dsc_userdata_t *)dsc->user_data;
      *texture = ptr->texture;
      rect = ptr->rect;
  ...
  if (texture && cdsc) {
    *header = SDL_malloc(sizeof(lv_draw_sdl_img_header_t));
    SDL_memcpy(&(*header)->base, &cdsc->dec_dsc.header,
               sizeof(lv_img_header_t));
    (*header)->rect = rect;
    lv_draw_sdl_texture_cache_put_advanced(ctx, key, key_size, *texture,
                                           *header, SDL_free, tex_flags);
  } else {
...
static SDL_Texture *upload_img_texture(SDL_Renderer *renderer,
  ...
  bool chroma_keyed =
      dsc->header.cf == (uint32_t)LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED;
  ...
  SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(data, w, h, LV_COLOR_DEPTH,
                                                  w * LV_COLOR_DEPTH / 8, rmask,
                                                  gmask, bmask, amask);
...
static bool check_mask_simple_radius(const lv_area_t *coords,
  ...
  for (uint8_t i = 0; i < _LV_MASK_MAX_NUM; i++) {
    ...
    if (param->type == LV_DRAW_MASK_TYPE_RADIUS) {
      lv_draw_mask_radius_param_t *rparam =
          (lv_draw_mask_radius_param_t *)param;
      if (rparam->cfg.outer)
        return false;
      if (!_lv_area_is_equal(&rparam->cfg.rect, coords))
        return false;
...
static void draw_img_simple(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
  ...
  SDL_RenderCopyEx(ctx->renderer, texture, &src_rect, &dst_rect,
                   draw_dsc->angle, &pivot, SDL_FLIP_NONE);
  if (draw_dsc->angle != 0) {
...
static void draw_img_rounded(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
  ...
  SDL_Texture *frag =
      img_rounded_frag_obtain(ctx, texture, header, w, h, real_radius);
  apply_recolor_opa(frag, draw_dsc);
  lv_draw_sdl_rect_bg_frag_draw_corners(ctx, frag, real_radius, coords, clip,
                                        true);
  ...
  for (int i = w > h ? ROUNDED_IMG_PART_LEFT : ROUNDED_IMG_PART_TOP, j = i + 3;
       i <= j; i++) {
    switch (i) {
    case ROUNDED_IMG_PART_LEFT:
      lv_area_set(&part, coords->x1, coords->y1 + radius,
                  coords->x1 + radius - 1, coords->y2 - radius);
      break;
    case ROUNDED_IMG_PART_HCENTER:
      lv_area_set(&part, coords->x1 + radius, coords->y1, coords->x2 - radius,
                  coords->y2);
      break;
    case ROUNDED_IMG_PART_RIGHT:
      lv_area_set(&part, coords->x2 - radius + 1, coords->y1 + radius,
                  coords->x2, coords->y2 - radius);
      break;
    ...
    case ROUNDED_IMG_PART_VCENTER:
      lv_area_set(&part, coords->x1 + radius, coords->y2 - radius + 1,
                  coords->x2 - radius, coords->y2);
      break;
    case ROUNDED_IMG_PART_BOTTOM:
      lv_area_set(&part, coords->x1, coords->y1 + radius, coords->x2,
                  coords->y2 - radius);
      break;
    default:
      break;
    }
    if (!_lv_area_intersect(&clip_tmp, &part, clip))
      continue;
    SDL_Rect clip_rect;
...
static void apply_recolor_opa(SDL_Texture *texture,
  ...
  if (draw_dsc->recolor_opa > LV_OPA_TRANSP) {
    ...
    lv_color_t recolor = lv_color_mix(draw_dsc->recolor, lv_color_white(),
                                      draw_dsc->recolor_opa);
    SDL_SetTextureColorMod(texture, recolor.ch.red, recolor.ch.green,
                           recolor.ch.blue);
  } else {
...
img_rounded_frag_obtain(lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture,
  ...
  SDL_Texture *img_frag =
      lv_draw_sdl_texture_cache_get(ctx, &key, sizeof(key), NULL);
  if (img_frag == NULL) {
    const lv_coord_t full_frag_size = radius * 2 + 3;
    img_frag = SDL_CreateTexture(ctx->renderer, LV_DRAW_SDL_TEXTURE_FORMAT,
                                 SDL_TEXTUREACCESS_TARGET, full_frag_size,
                                 full_frag_size);
    ...
    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, mask_frag, radius, &frag_coords,
                                          NULL, false);
