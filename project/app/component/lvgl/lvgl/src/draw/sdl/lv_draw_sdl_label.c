...
static lv_font_glyph_key_t font_key_glyph_create(const lv_font_t *font_p,
                                                 uint32_t letter);
...
void lv_draw_sdl_draw_letter(lv_draw_ctx_t *draw_ctx,
  ...
  int32_t pos_y =
      pos_p->y + (font_p->line_height - font_p->base_line) - g.box_h - g.ofs_y;

  const lv_area_t letter_area = {pos_x, pos_y, pos_x + g.box_w - 1,
                                 pos_y + g.box_h - 1};
  lv_area_t draw_area;
  ...
  if (!glyph_found) {
    ...
    SDL_Surface *mask =
        lv_sdl_create_opa_surface(buf, g.box_w, g.box_h, g.box_w);
  ...
  bool has_mask = lv_draw_sdl_composite_begin(ctx, &letter_area, clip_area,
                                              NULL, dsc->blend_mode, &t_letter,
                                              &t_clip, &apply_area);
