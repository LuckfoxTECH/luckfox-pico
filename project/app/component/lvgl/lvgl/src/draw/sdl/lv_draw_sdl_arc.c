...
static void get_cap_area(int16_t angle, lv_coord_t thickness, uint16_t radius,
                         const lv_point_t *center, lv_area_t *out);

/**********************
 *   GLOBAL FUNCTIONS
 **********************/
void lv_draw_sdl_draw_arc(lv_draw_ctx_t *draw_ctx, const lv_draw_arc_dsc_t *dsc,
  ...
  if ((start_angle - end_angle) % 360) {
    lv_draw_mask_angle_init(&mask_angle_param, center->x, center->y,
  ...
  if (mask_ids_count == 3 && dsc->rounded) {
    ...
    lv_draw_mask_radius_init(&cap_start_param, &start_area, dsc->width / 2,
                             false);
  ...
  SDL_Texture *texture = lv_draw_sdl_composite_texture_obtain(
      ctx, LV_DRAW_SDL_COMPOSITE_TEXTURE_ID_STREAM1, w, h);
  SDL_SetTextureBlendMode(texture, SDL_BLENDMODE_BLEND);
  dump_masks(texture, &draw_area, mask_ids, mask_ids_count,
             cap_ids[0] != LV_MASK_ID_INV ? cap_ids : NULL);
...
static void get_cap_area(int16_t angle, lv_coord_t thickness, uint16_t radius,
  ...
  cir_x = ((radius - thick_half) * lv_trigo_sin((int16_t)(90 - angle))) >>
          (LV_TRIGO_SHIFT - ps);
  cir_y =
