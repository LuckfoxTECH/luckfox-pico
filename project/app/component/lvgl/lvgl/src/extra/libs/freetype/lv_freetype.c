...
bool lv_freetype_init(uint16_t max_faces, uint16_t max_sizes,
                      uint32_t max_bytes) {
  ...
  error = FTC_Manager_New(library, max_faces, max_sizes, max_bytes,
                          font_face_requester, NULL, &cache_manager);
  if (error) {
...
static bool get_bold_glyph(const lv_font_t *font, FT_Face face,
  ...
  dsc_out->ofs_y =
      ...
      face->glyph->bitmap
          .rows;    /*Y offset of the bitmap measured from the as line*/
...
static bool get_glyph_dsc_cb_cache(const lv_font_t *font,
  ...
  FT_UInt glyph_index =
      FTC_CMapCache_Lookup(cmap_cache, face_id, charmap_index, unicode_letter);
...
#if LV_FREETYPE_SBIT_CACHE
  FT_Error error =
      FTC_SBitCache_Lookup(sbit_cache, &desc_type, glyph_index, &sbit, NULL);
  if (error) {
  ...
  dsc_out->ofs_y =
      sbit->top -
      sbit->height; /*Y offset of the bitmap measured from the as line*/
  dsc_out->bpp = 8; /*Bit per pixel: 1/2/4/8*/
#else
  FT_Error error = FTC_ImageCache_Lookup(image_cache, &desc_type, glyph_index,
                                         &image_glyph, NULL);
  if (error) {
...
static bool lv_ft_font_init_cache(lv_ft_info_t *info) {
  ...
  int8_t thickness =
      FT_MulFix(scale, face_size->face->underline_thickness) >> 6;
  font->underline_position =
      FT_MulFix(scale, face_size->face->underline_position) >> 6;
...
static const uint8_t *get_glyph_bitmap_cb_nocache(const lv_font_t *font,
                                                  uint32_t unicode_letter) {
  LV_UNUSED(unicode_letter);
  lv_font_fmt_ft_dsc_t *dsc = (lv_font_fmt_ft_dsc_t *)(font->dsc);
  FT_Face face = dsc->size->face;
...
static bool lv_ft_font_init_nocache(lv_ft_info_t *info) {
  ...
  if (face == NULL) {
    FT_Error error;
    if (dsc->mem) {
      error = FT_New_Memory_Face(library, dsc->mem, (FT_Long)dsc->mem_size, 0,
                                 &face);
    } else {
