...
Supprted stdint.h. / Jul 01, 2020 R0.01d Fixed wrong integer type usage. / May
08, 2021 R0.02  Supprted grayscale image. Separated configuration options. / Jun
11, 2021 R0.02a Some performance improvement. / Jul 01, 2021 R0.03  Added
JD_FASTDECODE option. /                     Some performance improvement.
/----------------------------------------------------------------------------*/
...
alloc_pool(/* Pointer to allocated memory block (NULL:no memory available) */
  ...
  return (void *)
      rp; /* Return allocated memory block (NULL:no memory to allocate) */
}
  ...
  while (ndata) { /* Process all tables in the segment */
    ...
    for (i = 0; i < 64; i++) { /* Load the table */
      ...
      pb[zi] = (int32_t)((uint32_t)*data++ *
                         Ipsf[zi]); /* Apply scale factor of Arai algorithm to
                                       the de-quantizers */
    }
  }
...
create_huffman_tbl(          /* 0:OK, !0:Failed */
                   JDEC *jd, /* Pointer to the decompressor object */
                   const uint8_t
                       *data,   /* Pointer to the packed huffman tables */
                   size_t ndata /* Size of input data */
) {
  ...
  while (ndata) { /* Process all tables in the segment */
    ...
    ph = alloc_pool(jd, np * sizeof(uint16_t)); /* Allocate a memory block for
                                                   the code word table */
    if (!ph)
      return JDR_MEM1; /* Err: not enough memory */
    ...
    for (i = 0; i < np;
         i++) { /* Load decoded data corresponds to each code word */
      d = *data++;
      if (!cls && d > 11)
        return JDR_FMT1;
      pd[i] = d;
    ...
    { /* Create fast huffman decode table */
      ...
      if (cls) {
        tbl_ac = alloc_pool(jd, HUFF_LEN *
                                    sizeof(uint16_t)); /* LUT for AC elements */
        if (!tbl_ac)
          return JDR_MEM1; /* Err: not enough memory */
        ...
        tbl_dc = alloc_pool(jd, HUFF_LEN *
                                    sizeof(uint8_t)); /* LUT for AC elements */
        if (!tbl_dc)
          return JDR_MEM1; /* Err: not enough memory */
        ...
        memset(
            tbl_dc, 0xFF,
            HUFF_LEN *
                sizeof(uint8_t)); /* Default value (0xFF: may be long code) */
      }
      for (i = b = 0; b < HUFF_BIT; b++) { /* Create LUT */
        for (j = pb[b]; j; j--) {
          ...
          if (cls) {
            td = pd[i++] | ((b + 1) << 8); /* b15..b8: code length, b7..b0: zero
                                              run and data length */
            ...
            td = pd[i++] |
                 ((b + 1) << 4); /* b7..b4: code length, b3..b0: data length */
            for (span = 1 << (HUFF_BIT - 1 - b); span;
  ...
  do {
    if (!bm) {          /* Next byte? */
      if (!dc) {        /* No input data is available, re-fill input buffer */
        ...
        if (!dc)
          return 0 -
                 (int)JDR_INP; /* Err: read error or wrong stream termination */
      } else {
        dp++; /* Next data ptr */
      }
      ...
      if (flg) { /* In flag sequence? */
        ...
        if (*dp != 0)
          return 0 - (int)JDR_FMT1; /* Err: unexpected flag is detected (may be
                                       collapted data) */
...
static JRESULT restart(JDEC *jd,     /* Pointer to the decompressor object */
  ...
  if ((marker & 0xFFD8) != 0xFFD0 || (marker & 7) != (rstn & 7)) {
    return JDR_FMT1; /* Err: expected RSTn marker was not detected (may be
                        collapted data) */
  }
...
static void block_idct(int32_t *src, /* Input block data (de-quantized and
                       ...
                       jd_yuv_t *dst /* Pointer to the destination to store the
                                        block as byte array */
) {
...
static JRESULT mcu_load(JDEC *jd /* Pointer to the decompressor object */
  ...
  int32_t *tmp =
      (int32_t *)
          jd->workbuf; /* Block working buffer for de-quantize and IDCT */
  int d, e;
  ...
  for (blk = 0; blk < nby + 2; blk++) { /* Get nby Y blocks and two C blocks */
    ...
    if (cmp &&
        jd->ncomp != 3) { /* Clear C blocks if not exist (monochrome image) */
      ...
      dqf = jd->qttbl[jd->qtid[cmp]]; /* De-quantizer table ID for this
                                         component */
      ...
      do {
        ...
        if (bc &= 0x0F) {     /* Bit length? */
          ...
          tmp[i] = d * dqf[i] >> 8; /* De-quantize, apply scale factor of Arai
                                       algorithm and descale 8 bits */
        }
      ...
      if (JD_FORMAT != 2 ||
        ...
        if (z == 1 ||
            (JD_USE_SCALE &&
             jd->scale ==
                 3)) { /* If no AC element or scale ratio is 1/8, IDCT can be
...
static JRESULT mcu_output(JDEC *jd, /* Pointer to the decompressor object */
                          int (*outfunc)(JDEC *, void *,
  ...
  rx = (img_x + mx <= jd->width)
           ? mx
           : jd->width - img_x; /* Output rectangular size (it may be clipped at
                                   right/bottom end of image) */
  ...
  if (!JD_USE_SCALE || jd->scale != 3) { /* Not for 1/8 scaling */
    ...
    if (JD_FORMAT != 2) { /* RGB output (build an RGB MCU from Y/C component) */
      for (iy = 0; iy < my; iy++) {
        ...
        for (ix = 0; ix < mx; ix++) {
          ...
          *pix++ = /*G*/ BYTECLIP(
              yy -
              ((int)(0.344 * CVACC) * cb + (int)(0.714 * CVACC) * cr) / CVACC);
    ...
    if (JD_USE_SCALE && jd->scale) {
      ...
      a = (mx - w) * (JD_FORMAT != 2
                          ? 3
                          : 1); /* Bytes to skip for next line in the square */
      op = (uint8_t *)jd->workbuf;
      for (iy = 0; iy < my; iy += w) {
        for (ix = 0; ix < mx; ix += w) {
          pix = (uint8_t *)jd->workbuf +
                (iy * mx + ix) * (JD_FORMAT != 2 ? 3 : 1);
          r = g = b = 0;
...
JRESULT jd_prepare(
  ...
  memset(jd, 0,
         sizeof(JDEC));  /* Clear decompression object (this might be a problem
                            if machine's null pointer is not all bits zero) */
  ...
  for (;;) { /* Parse JPEG segments */
    ...
    case 0xC0: /* SOF0 (baseline JPEG) */
      ...
      for (i = 0; i < jd->ncomp; i++) {
        ...
        jd->qtid[i] =
            seg[8 + 3 * i]; /* Get dequantizer table ID for this component */
        if (jd->qtid[i] > 3)
    ...
    case 0xDA: /* SOS - Start of Scan */
      ...
      if (!jd->workbuf)
        return JDR_MEM1; /* Err: not enough memory */
      jd->mcubuf = alloc_pool(
          jd,
          (n + 2) * 64 * sizeof(jd_yuv_t)); /* Allocate MCU working buffer */
      if (!jd->mcubuf)
        return JDR_MEM1; /* Err: not enough memory */
    ...
    default: /* Unknown segment (comment, exif or etc..) */
      /* Skip segment data (null pointer specifies to remove data from the
       * stream) */
...
JRESULT jd_decomp(JDEC *jd, /* Initialized decompression object */
  ...
  for (y = 0; y < jd->height; y += my) {  /* Vertical loop of MCUs */
    for (x = 0; x < jd->width; x += mx) { /* Horizontal loop of MCUs */
      ...
      rc = mcu_load(jd); /* Load an MCU (decompress huffman coded stream,
                            dequantize and apply IDCT) */
      if (rc != JDR_OK)
        return rc;
      rc =
          mcu_output(jd, outfunc, x,
