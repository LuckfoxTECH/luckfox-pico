...
static void *fs_open(lv_fs_drv_t *drv, const char *path, lv_fs_mode_t mode) {
  ...
  return (void *)CreateFileA(buf, desired_access, FILE_SHARE_READ, NULL,
...
static lv_fs_res_t fs_close(lv_fs_drv_t *drv, void *file_p) {
  LV_UNUSED(drv);
  return CloseHandle((HANDLE)file_p) ? LV_FS_RES_OK
...
static lv_fs_res_t fs_tell(lv_fs_drv_t *drv, void *file_p, uint32_t *pos_p) {
  ...
  if (SetFilePointerEx((HANDLE)file_p, distance_to_move, &file_pointer,
                       FILE_CURRENT)) {
...
static lv_fs_res_t fs_dir_close(lv_fs_drv_t *drv, void *dir_p) {
  LV_UNUSED(drv);
  return FindClose((HANDLE)dir_p) ? LV_FS_RES_OK
                                  : fs_error_from_win32(GetLastError());
}
