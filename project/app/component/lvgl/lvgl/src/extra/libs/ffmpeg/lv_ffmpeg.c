...
lv_res_t lv_ffmpeg_player_set_src(lv_obj_t *obj, const char *path) {
  ...
  if (period > 0) {
    LV_LOG_INFO("frame refresh period = %d ms, rate = %d fps", period,
                1000 / period);
...
static int ffmpeg_output_video_frame(struct ffmpeg_context_s *ffmpeg_ctx) {
  ...
  if (frame->width != width || frame->height != height ||
    ...
    LV_LOG_ERROR(
        ...
        width, height, av_get_pix_fmt_name(ffmpeg_ctx->video_dec_ctx->pix_fmt),
  ...
  if (ffmpeg_ctx->sws_ctx == NULL) {
    ...
    ffmpeg_ctx->sws_ctx = sws_getContext(
        width, height, ffmpeg_ctx->video_dec_ctx->pix_fmt, width, height,
        ffmpeg_ctx->video_dst_pix_fmt, swsFlags, NULL, NULL, NULL);
  }
  ...
  if (!ffmpeg_ctx->has_alpha) {
    ...
    if (dst_linesize != lv_linesize) {
      LV_LOG_WARN("ffmpeg linesize = %d, but lvgl image require %d",
                  dst_linesize, lv_linesize);
      ffmpeg_ctx->video_dst_linesize[0] = lv_linesize;
    }
  }

  ret = sws_scale(ffmpeg_ctx->sws_ctx,
                  (const uint8_t *const *)(ffmpeg_ctx->video_src_data),
                  ffmpeg_ctx->video_src_linesize, 0, height,
                  ffmpeg_ctx->video_dst_data, ffmpeg_ctx->video_dst_linesize);
...
static int ffmpeg_open_codec_context(int *stream_idx, AVCodecContext **dec_ctx,
                                     AVFormatContext *fmt_ctx,
  ...
  if (ret < 0) {
    ...
    if (dec == NULL) {
      LV_LOG_ERROR("Failed to find %s codec", av_get_media_type_string(type));
    ...
    *dec_ctx = avcodec_alloc_context3(dec);
    ...
    if ((ret = avcodec_parameters_to_context(*dec_ctx, st->codecpar)) < 0) {
      LV_LOG_ERROR("Failed to copy %s codec parameters to decoder context",
                   av_get_media_type_string(type));
      return ret;
    }
    ...
    if ((ret = avcodec_open2(*dec_ctx, dec, &opts)) < 0) {
      LV_LOG_ERROR("Failed to open %s codec", av_get_media_type_string(type));
      return ret;
    }
...
static int ffmpeg_get_img_header(const char *filepath,
  ...
  if (ffmpeg_open_codec_context(&video_stream_idx, &video_dec_ctx, fmt_ctx,
...
struct ffmpeg_context_s *ffmpeg_open_file(const char *path) {
  ...
  if (ffmpeg_open_codec_context(&(ffmpeg_ctx->video_stream_idx),
                                &(ffmpeg_ctx->video_dec_ctx),
                                ffmpeg_ctx->fmt_ctx, AVMEDIA_TYPE_VIDEO) >= 0) {
...
static int ffmpeg_image_allocate(struct ffmpeg_context_s *ffmpeg_ctx) {
  ...
  /* allocate image where the decoded image will be put */
  ret = av_image_alloc(
      ffmpeg_ctx->video_src_data, ffmpeg_ctx->video_src_linesize,
      ffmpeg_ctx->video_dec_ctx->width, ffmpeg_ctx->video_dec_ctx->height,
      ffmpeg_ctx->video_dec_ctx->pix_fmt, 4);
  ...
  ret = av_image_alloc(
      ffmpeg_ctx->video_dst_data, ffmpeg_ctx->video_dst_linesize,
      ffmpeg_ctx->video_dec_ctx->width, ffmpeg_ctx->video_dec_ctx->height,
      ffmpeg_ctx->video_dst_pix_fmt, 4);
