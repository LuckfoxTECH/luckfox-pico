...
/    ----------
/    SJPEG is a custom created modified JPEG file format for small embedded
platforms. /    It will contain multiple JPEG fragments all embedded into a
single file with a custom header. /    This makes JPEG decoding easier using any
JPEG library. Overall file size will be almost /    similar to the parent jpeg
file. We can generate sjpeg from any jpeg using a python script /    provided
along with this project. / (by vinodstanur | 2020 ) /    SJPEG FILE STRUCTURE /
--------------------------------------------------------------------------------------------------------------------------------
/    Bytes                       |   Value | /
--------------------------------------------------------------------------------------------------------------------------------
/
...
/
/    8 - 13                      |   "V1.00" followed by '\0'       [VERSION OF
SJPG FILE for future compatibiliby]
/
/    14 - 15                     |   X_RESOLUTION (width)            [little
endian]
/
/    16 - 17                     |   Y_RESOLUTION (height)           [little
endian]
/
/    18 - 19                     |   TOTAL_FRAMES inside sjpeg       [little
endian]
/
/    20 - 21                     |   JPEG BLOCK WIDTH (16 normally)  [little
endian]
/
/    22 - [(TOTAL_FRAMES*2 )]    |   SIZE OF EACH JPEG SPLIT FRAGMENTS
(FRAME_INFO_ARRAY)
/
/   SJPEG data                   |   Each JPEG frame can be extracted from SJPEG
data by parsing the FRAME_INFO_ARRAY one time.
/
/----------------------------------------------------------------------------------------------------------------------------------
/                   JPEG DECODER
/                   ------------
/   We are using TJpgDec - Tiny JPEG Decompressor library from ELM-CHAN for
decoding each split-jpeg fragments. /   The tjpgd.c and tjpgd.h is not modified
and those are used as it is. So if any update comes for the tiny-jpeg, /   just
replace those files with updated files.
/---------------------------------------------------------------------------------------------------------------------------------*/
...
static lv_res_t decoder_info(lv_img_decoder_t *decoder, const void *src,
  ...
  if (src_type == LV_IMG_SRC_VARIABLE) {
    ...
    if (!strncmp((char *)raw_sjpeg_data, "_SJPG__", strlen("_SJPG__"))) {
      ...
      JRESULT rc = jd_prepare(&jd_tmp, input_func, workb_temp,
                              (size_t)TJPGD_WORKBUFF_SIZE, &io_source_temp);
      if (rc == JDR_OK) {
...
static int img_data_cb(JDEC *jd, void *data, JRECT *rect) {
  ...
  for (int y = rect->top; y <= rect->bottom; y++) {
    int row_offset =
        y * xres * INPUT_PIXEL_SIZE + rect->left * INPUT_PIXEL_SIZE;
...
static size_t input_func(JDEC *jd, uint8_t *buff, size_t ndata) {
  ...
  if (io->type == SJPEG_IO_SOURCE_C_ARRAY) {
    const uint32_t bytes_left =
        io->raw_sjpg_data_size - io->raw_sjpg_data_next_read_pos;
    const uint32_t to_read = ndata <= bytes_left ? (uint32_t)ndata : bytes_left;
    if (to_read == 0)
      return 0;
    if (buff) {
      memcpy(buff, io->raw_sjpg_data + io->raw_sjpg_data_next_read_pos,
             to_read);
    }
...
static lv_res_t decoder_open(lv_img_decoder_t *decoder,
  ...
  if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
    ...
    if (!strncmp((char *)sjpeg->sjpeg_data, "_SJPG__", strlen("_SJPG__"))) {
      ...
      sjpeg->frame_base_array =
          lv_mem_alloc(sizeof(uint8_t *) * sjpeg->sjpeg_total_frames);
      ...
      sjpeg->frame_cache = (void *)lv_mem_alloc(
          sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3 /*2*/);
      ...
      if (rc == JDR_OK) {
        ...
        sjpeg->sjpeg_cache_frame_index = -1;
        sjpeg->frame_cache = (void *)lv_mem_alloc(
            sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3);
        if (!sjpeg->frame_cache) {
          lv_sjpg_cleanup(sjpeg);
          sjpeg = NULL;
          return LV_RES_INV;
    ...
    if (!strcmp(&fn[strlen(fn) - 5], ".sjpg")) {
      ...
      if (strcmp((char *)buff, "_SJPG__") == 0) {
        ...
        sjpeg->frame_base_array = NULL; // lv_mem_alloc( sizeof(uint8_t *) *
                                        // sjpeg->sjpeg_total_frames );
        sjpeg->frame_base_offset =
            lv_mem_alloc(sizeof(int) * sjpeg->sjpeg_total_frames);
        if (!sjpeg->frame_base_offset) {
          lv_fs_close(&lv_file);
          lv_sjpg_cleanup(sjpeg);
          return LV_RES_INV;
        }
        int img_frame_start_offset =
            (SJPEG_FRAME_INFO_ARRAY_OFFSET + sjpeg->sjpeg_total_frames * 2);
        sjpeg->frame_base_offset[0] =
            img_frame_start_offset; // pointer used to save integer for now...
        ...
        for (int i = 1; i < sjpeg->sjpeg_total_frames; i++) {
          ...
          int offset = *data++;
          offset |= *data++ << 8;
          sjpeg->frame_base_offset[i] =
              sjpeg->frame_base_offset[i - 1] + offset;
        }

        sjpeg->sjpeg_cache_frame_index =
            -1; // INVALID AT BEGINNING for a forced compare mismatch at first
                // time.
        sjpeg->frame_cache = (void *)lv_mem_alloc(
            sjpeg->sjpeg_x_res * sjpeg->sjpeg_single_frame_height * 3);
        if (!sjpeg->frame_cache) {
 ...
 * them in `buf`. Required only if the "open" function can't open the whole
 * decoded pixel array. (dsc->img_data == NULL)
...
static lv_res_t decoder_read_line(lv_img_decoder_t *decoder,
  ...
  if (dsc->src_type == LV_IMG_SRC_VARIABLE) {
    ...
    if (sjpeg_req_frame_index != sjpeg->sjpeg_cache_frame_index) {
      ...
      if (sjpeg_req_frame_index == (sjpeg->sjpeg_total_frames - 1)) {
        ...
        const uint32_t frame_offset =
            (uint32_t)(sjpeg->io.raw_sjpg_data - sjpeg->sjpeg_data);
        sjpeg->io.raw_sjpg_data_size = sjpeg->sjpeg_data_size - frame_offset;
      } else {
        sjpeg->io.raw_sjpg_data_size =
            (uint32_t)(sjpeg->frame_base_array[sjpeg_req_frame_index + 1] -
                       sjpeg->io.raw_sjpg_data);
      }
      sjpeg->io.raw_sjpg_data_next_read_pos = 0;
      rc = jd_prepare(sjpeg->tjpeg_jd, input_func, sjpeg->workb,
                      (size_t)TJPGD_WORKBUFF_SIZE, &(sjpeg->io));
      if (rc != JDR_OK)
        return LV_RES_INV;
    ...
    int offset = 0;
    uint8_t *cache =
        (uint8_t *)sjpeg->frame_cache + x * 3 +
        (y % sjpeg->sjpeg_single_frame_height) * sjpeg->sjpeg_x_res * 3;
...
#error Unsupported LV_COLOR_DEPTH
    ...
    /*If line not from cache, refresh cache */
    if (sjpeg_req_frame_index != sjpeg->sjpeg_cache_frame_index) {
      sjpeg->io.raw_sjpg_data_next_read_pos =
          (int)(sjpeg->frame_base_offset[sjpeg_req_frame_index]);
...
static int is_jpg(const uint8_t *raw_data) {
  const uint8_t jpg_signature[] = {0xFF, 0xD8, 0xFF, 0xE0, 0x00,
