...
static lv_res_t decoder_info(struct _lv_img_decoder_t *decoder, const void *src,
  ...
  if (src_type == LV_IMG_SRC_FILE) {
    ...
    if (!strcmp(&fn[strlen(fn) - 3], "png")) { /*Check the extension*/
      ...
      /*The width and height are stored in Big endian format so convert them to
       * little endian*/
      header->w = (lv_coord_t)((size[0] & 0xff000000) >> 24) +
                  ((size[0] & 0x00ff0000) >> 8);
      header->h = (lv_coord_t)((size[1] & 0xff000000) >> 24) +
                  ((size[1] & 0x00ff0000) >> 8);
...
static lv_res_t decoder_open(lv_img_decoder_t *decoder,
  ...
  if (dsc->src_type == LV_IMG_SRC_FILE) {
    ...
    if (!strcmp(&fn[strlen(fn) - 3], "png")) { /*Check the extension*/
      ...
      unsigned char *png_data; /*Pointer to the loaded data. Same as the
                                  original file just loaded into the RAM*/
      size_t png_data_size;    /*Size of `png_data` in bytes*/

      error =
          lodepng_load_file(&png_data, &png_data_size, fn); /*Load the file*/
      if (error) {
      ...
      error = lodepng_decode32(&img_data, &png_width, &png_height, png_data,
                               png_data_size);
  ...
  /*If it's a PNG file in a  C array...*/
    ...
    error = lodepng_decode32(&img_data, &png_width, &png_height, img_dsc->data,
                             img_dsc->data_size);
...
/**
 * If the display is not in 32 bit format (ARGB888) then covert the image to the
 * current color depth
...
static void convert_color_depth(uint8_t *img, uint32_t px_cnt) {
  ...
  uint32_t i;
  for (i = 0; i < px_cnt; i++) {
    c = lv_color_make(img_argb[i].ch.red, img_argb[i].ch.green,
                      img_argb[i].ch.blue);
  ...
  uint32_t i;
  for (i = 0; i < px_cnt; i++) {
    c = lv_color_make(img_argb[i].ch.blue, img_argb[i].ch.green,
                      img_argb[i].ch.red);
