...
static lv_obj_t *find_chid(lv_obj_t *obj, lv_obj_t *start_child,
                           find_mode_t mode);
...
static void gridnav_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_KEY) {
    ...
    if (key == LV_KEY_RIGHT) {
      if ((dsc->ctrl & LV_GRIDNAV_CTRL_SCROLL_FIRST) &&
          lv_obj_has_flag(dsc->focused_obj, LV_OBJ_FLAG_SCROLLABLE) &&
        ...
        if (guess == NULL) {
          if (dsc->ctrl & LV_GRIDNAV_CTRL_ROLLOVER) {
            ...
            if (guess == NULL)
              guess = find_first_focusable(obj);
          } else {
    ...
    if (guess && guess != dsc->focused_obj) {
      lv_obj_clear_state(dsc->focused_obj,
...
static lv_obj_t *find_chid(lv_obj_t *obj, lv_obj_t *start_child,
  ...
  lv_coord_t h_max = lv_obj_get_height(obj) + lv_obj_get_scroll_top(obj) +
  ...
  for (i = 0; i < child_cnt; i++) {
    ...
    case FIND_LEFT:
      ...
      if (x_err >= 0)
        continue; /*It's on the right*/
      if (LV_ABS(y_err) > h_half)
        continue; /*Too far*/
      break;
    case FIND_RIGHT:
      x_err = get_x_center(child) - x_start;
    ...
    case FIND_TOP:
      ...
      y_err = get_y_center(child) - y_start;
      if (y_err >= 0)
        continue; /*It's on the bottom*/
      break;
    case FIND_BOTTOM:
      x_err = get_x_center(child) - x_start;
      y_err = get_y_center(child) - y_start;
      if (y_err <= 0)
        continue; /*It's on the top*/
      break;
    ...
    if (guess == NULL ||
        (y_err * y_err + x_err * x_err <
         y_err_guess * y_err_guess + x_err_guess * x_err_guess)) {
      guess = child;
      x_err_guess = x_err;
      y_err_guess = y_err;
    }
...
static bool obj_is_focuable(lv_obj_t *obj) {
  if (lv_obj_has_flag(obj, LV_OBJ_FLAG_HIDDEN))
    return false;
  if (lv_obj_has_flag(obj, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_CLICK_FOCUSABLE))
    return true;
  else
    return false;
}
