...
typedef enum {
  ...
  /**
   * If an arrow is pressed and the focused object can be scrolled in that
   * direction then it will be scrolled instead of going to the next/previous
   * object. If there is no more room for scrolling the next/previous object
   * will be focused normally */
...
/**
 * Add grid navigation feature to an object. It expects the children to be
 * arranged into a grid-like layout. Although it's not required to have pixel
 * perfect alignment. This feature makes possible to use keys to navigate among
 * the children and focus them. The keys other than arrows and press/release
 * related events are forwarded to the focused child.
