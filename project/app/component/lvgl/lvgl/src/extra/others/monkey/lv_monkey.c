...
static const lv_key_t lv_key_map[] = {
    LV_KEY_UP,   LV_KEY_DOWN, LV_KEY_RIGHT,     LV_KEY_LEFT,
    LV_KEY_ESC,  LV_KEY_DEL,  LV_KEY_BACKSPACE, LV_KEY_ENTER,
...
lv_monkey_t *lv_monkey_create(const lv_monkey_config_t *config) {
  ...
  monkey->timer = lv_timer_create(lv_monkey_timer_cb,
                                  monkey->config.period_range.min, monkey);
...
static void lv_monkey_read_cb(lv_indev_drv_t *indev_drv,
...
static void lv_monkey_timer_cb(lv_timer_t *timer) {
  ...
  case LV_INDEV_TYPE_ENCODER:
    data->enc_diff = (int16_t)lv_monkey_random(monkey->config.input_range.min,
                                               monkey->config.input_range.max);
    break;
  case LV_INDEV_TYPE_BUTTON:
    data->btn_id = (uint32_t)lv_monkey_random(monkey->config.input_range.min,
                                              monkey->config.input_range.max);
    break;
  case LV_INDEV_TYPE_KEYPAD: {
    int32_t index =
        lv_monkey_random(0, sizeof(lv_key_map) / sizeof(lv_key_map[0]) - 1);
  ...
  data->state = lv_monkey_random(0, 100) < 50 ? LV_INDEV_STATE_RELEASED
                                              : LV_INDEV_STATE_PRESSED;

  lv_timer_set_period(monkey->timer,
                      lv_monkey_random(monkey->config.period_range.min,
                                       monkey->config.period_range.max));
}
