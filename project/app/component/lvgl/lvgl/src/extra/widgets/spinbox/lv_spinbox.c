...
static void lv_spinbox_constructor(const lv_obj_class_t *class_p,
                                   lv_obj_t *obj);
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_spinbox_class = {.constructor_cb =
...
void lv_spinbox_set_value(lv_obj_t *obj, int32_t i) {
  ...
  if (i > spinbox->range_max)
    i = spinbox->range_max;
  if (i < spinbox->range_min)
    i = spinbox->range_min;
...
void lv_spinbox_set_digit_format(lv_obj_t *obj, uint8_t digit_count,
  ...
  if (separator_position >= digit_count)
...
void lv_spinbox_set_pos(lv_obj_t *obj, uint8_t pos) {
  ...
  int32_t step_limit;
  step_limit =
      LV_MAX(spinbox->range_max, (spinbox->range_min < 0 ? (-spinbox->range_min)
                                                         : spinbox->range_min));
...
/**
 * Set direction of digit step when clicking an encoder button while in editing
 * mode
 * @param spinbox pointer to spinbox
...
/**
 * Get the spinbox numeral value (user has to convert to float according to its
 * digit format)
 * @param obj pointer to spinbox
...
/**
 * Get the spinbox step value (user has to convert to float according to its
 * digit format)
 * @param obj pointer to spinbox
...
void lv_spinbox_increment(lv_obj_t *obj) {
  ...
  if (spinbox->value + spinbox->step <= spinbox->range_max) {
    ...
    if ((spinbox->value + spinbox->step) > 0 && spinbox->value < 0)
...
static void lv_spinbox_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_RELEASED) {
     ...
     * Set `step` accordingly*/
      ...
      if (spinbox->ta.cursor.pos > spinbox->dec_point_pos &&
          spinbox->dec_point_pos != 0)
        cp--;
      uint32_t pos = len - cp;
...
static void lv_spinbox_updatevalue(lv_obj_t *obj) {
  ...
  /*Convert the numbers to string (the sign is already handled so always covert
   * positive number)*/
  ...
  int32_t intDigits;
  intDigits = (spinbox->dec_point_pos == 0) ? spinbox->digit_count
                                            : spinbox->dec_point_pos;
