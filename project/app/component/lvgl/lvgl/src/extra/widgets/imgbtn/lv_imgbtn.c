...
static lv_imgbtn_state_t suggest_state(lv_obj_t *imgbtn,
 ...
 * @param src_left pointer to an image source for the left side of the button (a
 * C array or path to a file)
 * @param src_mid pointer to an image source for the middle of the button
 * (ideally 1px wide) (a C array or path to a file)
 * @param src_right pointer to an image source for the right side of the button
 * (a C array or path to a file)
 */
void lv_imgbtn_set_src(lv_obj_t *obj, lv_imgbtn_state_t state,
...
void lv_imgbtn_set_state(lv_obj_t *obj, lv_imgbtn_state_t state) {
  ...
  if (state == LV_IMGBTN_STATE_PRESSED ||
      state == LV_IMGBTN_STATE_CHECKED_PRESSED)
    obj_state |= LV_STATE_PRESSED;
  if (state == LV_IMGBTN_STATE_DISABLED ||
      state == LV_IMGBTN_STATE_CHECKED_DISABLED)
    obj_state |= LV_STATE_DISABLED;
  if (state == LV_IMGBTN_STATE_CHECKED_DISABLED ||
      state == LV_IMGBTN_STATE_CHECKED_PRESSED ||
  ...
  lv_obj_clear_state(obj,
                     LV_STATE_CHECKED | LV_STATE_PRESSED | LV_STATE_DISABLED);
...
static void lv_imgbtn_constructor(const lv_obj_class_t *class_p,
                                  lv_obj_t *obj) {
  LV_UNUSED(class_p);
  lv_imgbtn_t *imgbtn = (lv_imgbtn_t *)obj;
...
static void draw_main(lv_event_t *e) {
  ...
  if (src) {
    ...
    bool comm_res;
    comm_res = _lv_area_intersect(&clip_area_center, &clip_area_center,
                                  draw_ctx->clip_area);
    if (comm_res) {
      lv_coord_t i;
      lv_img_decoder_get_info(src, &header);
      ...
      for (i = coords_part.x1;
           i < (lv_coord_t)(clip_area_center.x2 + header.w - 1);
           i += header.w) {
        lv_draw_img(draw_ctx, &img_dsc, &coords_part, src);
...
/**
 * If `src` is not defined for the current state try to get a state which is
 * related to the current but has `src`. E.g. if the PRESSED src is not set but
 * the RELEASED does, use the RELEASED.
