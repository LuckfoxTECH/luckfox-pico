...
static bool lv_txt_get_snippet(const char *txt, const lv_font_t *font,
                               ...
                               lv_text_flag_t flag, lv_coord_t *use_width,
                               uint32_t *end_ofs);
...
lv_span_t *lv_spangroup_get_child(const lv_obj_t *obj, int32_t id) {
  ...
  /*If using a negative index, start from the tail and use cur -1 to indicate
   * the end*/
...
lv_coord_t lv_spangroup_get_expand_height(lv_obj_t *obj, lv_coord_t width) {
  ...
  while (cur_span) {
    ...
    /* the loop control to find a line and push the relevant span info into
     * stack  */
    while (1) {
      ...
      if (cur_txt_ofs == 0) {
        ...
        snippet.letter_space =
            lv_span_get_style_text_letter_space(obj, cur_span);
      ...
      bool isfill = lv_txt_get_snippet(&cur_txt[cur_txt_ofs], snippet.font,
                                       snippet.letter_space, max_w, txt_flag,
                                       &use_width, &next_ofs);
      ...
      if (isfill && next_ofs > 0 && snippet_cnt > 0) {
        ...
        if (!(letter == '\0' || letter == '\n' || letter == '\r' ||
          ...
          tmp_ofs = 0;
          letter =
              _lv_txt_encoded_next(&cur_txt[cur_txt_ofs + next_ofs], &tmp_ofs);
          if (!(letter == '\0' || letter == '\n' || letter == '\r' ||
                _lv_txt_is_break_char(letter))) {
            break;
          }
...
static void lv_snippet_push(lv_snippet_t *item) {
  if (snippet_stack.index < LV_SPAN_SNIPPET_STACK_SIZE) {
    memcpy(&snippet_stack.stack[snippet_stack.index], item,
           sizeof(lv_snippet_t));
    snippet_stack.index++;
  } else {
    LV_LOG_ERROR("span draw stack overflow, please set "
                 "LV_SPAN_SNIPPET_STACK_SIZE too larger");
  }
}

static uint16_t lv_get_snippet_cnt(void) { return snippet_stack.index; }

static lv_snippet_t *lv_get_snippet(uint16_t index) {
  return &snippet_stack.stack[index];
}

static void lv_snippet_clear(void) { snippet_stack.index = 0; }
...
static void lv_draw_span(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  while (cur_span) {
      ...
      if (isfill) {
        ...
        if (cur_txt[cur_txt_ofs + next_ofs] == '\0') {
          ...
          if (next_span) { /* have the next line */
            next_line_h = lv_font_get_line_height(
                              lv_span_get_style_text_font(obj, next_span)) +
                          line_space;
          }
        }
        lv_coord_t cur_line_h =
            max_line_h < snippet.line_h ? snippet.line_h : max_line_h;
        if (txt_pos.y + cur_line_h + next_line_h - line_space >
            coords.y2 + 1) { /* for overflow if is end line. */
          if (cur_txt[cur_txt_ofs + next_ofs] != '\0') {
            next_ofs = strlen(&cur_txt[cur_txt_ofs]);
            use_width =
                lv_txt_get_width(&cur_txt[cur_txt_ofs], next_ofs, snippet.font,
                                 snippet.letter_space, txt_flag);
            ellipsis_valid =
                spans->overflow == LV_SPAN_OVERFLOW_ELLIPSIS ? true : false;
            is_end_line = true;
          }
        } else if (next_ofs > 0 && lv_get_snippet_cnt() > 0) {
          /* To prevent infinite loops, the _lv_txt_get_next_line() may return
           * incomplete words, */
          ...
          uint32_t tmp_ofs = next_ofs;
          uint32_t letter =
              _lv_txt_encoded_prev(&cur_txt[cur_txt_ofs], &tmp_ofs);
          if (!(letter == '\0' || letter == '\n' || letter == '\r' ||
                _lv_txt_is_break_char(letter))) {
            tmp_ofs = 0;
            letter = _lv_txt_encoded_next(&cur_txt[cur_txt_ofs + next_ofs],
                                          &tmp_ofs);
            if (!(letter == '\0' || letter == '\n' || letter == '\r' ||
                  _lv_txt_is_break_char(letter))) {
              break;
    ...
    for (i = 0; i < item_cnt; i++) {
      ...
      label_draw_dsc.blend_mode =
          lv_span_get_style_text_blend_mode(obj, pinfo->span);
      if (obj_opa < LV_OPA_MAX) {
        label_draw_dsc.opa =
            (uint16_t)((uint16_t)label_draw_dsc.opa * obj_opa) >> 8;
      }
      ...
      while (j < txt_bytes) {
        ...
        int32_t letter_w =
            lv_font_get_glyph_width(pinfo->font, letter, letter_next);
        ...
        if (ellipsis_valid &&
            pos.x + letter_w + pinfo->letter_space > ellipsis_width) {
      ...
      if (decor != LV_TEXT_DECOR_NONE) {
        ...
        line_dsc.width = label_draw_dsc.font->underline_thickness
                             ? pinfo->font->underline_thickness
                             : 1;
        ...
        if (decor & LV_TEXT_DECOR_STRIKETHROUGH) {
          ...
          lv_point_t p2;
          p1.x = txt_pos.x;
          p1.y = pos.y + ((pinfo->line_h - line_space) >> 1) +
                 (line_dsc.width >> 1);
          p2.x = pos.x;
          p2.y = p1.y;
