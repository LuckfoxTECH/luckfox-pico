 ...
 *      DEFINES
 *********************/
#define LV_MSGBOX_FLAG_AUTO_PARENT                                             \
  LV_OBJ_FLAG_WIDGET_1 /*Mark that the parent was automatically created*/
...
const lv_obj_class_t lv_msgbox_content_class = {.base_class = &lv_obj_class,
                                                .width_def = LV_PCT(100),
                                                .height_def = LV_SIZE_CONTENT,
                                                .instance_size =
                                                    sizeof(lv_obj_t)};

const lv_obj_class_t lv_msgbox_backdrop_class = {.base_class = &lv_obj_class,
                                                 .width_def = LV_PCT(100),
...
lv_obj_t *lv_msgbox_create(lv_obj_t *parent, const char *title, const char *txt,
  ...
  if (add_close_btn) {
    ...
    lv_obj_add_event_cb(mbox->close_btn, msgbox_close_click_event_cb,
  ...
  if (btn_txts) {
    ...
    lv_btnmatrix_set_btn_ctrl_all(mbox->btns, LV_BTNMATRIX_CTRL_CLICK_TRIG |
    ...
    const lv_font_t *font =
        lv_obj_get_style_text_font(mbox->btns, LV_PART_ITEMS);
    ...
    lv_obj_add_flag(mbox->btns,
                    LV_OBJ_FLAG_EVENT_BUBBLE); /*To see the event directly on
                                                  the message box*/
  }
