...
/**
 * The OUTER_MASK_WIDTH define is required to assist with the placing of a mask
 * over the outer ring of the widget as when the multicoloured radial lines are
 * calculated for the outer ring of the widget their lengths are jittering
 * because of the integer based arithmetic. From tests the maximum delta was
 * found to be 2 so the current value is set to 3 to achieve appropriate
 * masking.
 */
 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_colorwheel_constructor(const lv_obj_class_t *class_p,
                                      lv_obj_t *obj);
...
bool lv_colorwheel_set_hsv(lv_obj_t *obj, lv_color_hsv_t hsv) {
  ...
  if (colorwheel->hsv.h == hsv.h && colorwheel->hsv.s == hsv.s &&
...
bool lv_colorwheel_set_rgb(lv_obj_t *obj, lv_color_t color) {
  ...
  return lv_colorwheel_set_hsv(
      obj, lv_color_rgb_to_hsv(c32.ch.red, c32.ch.green, c32.ch.blue));
}
...
lv_color_t lv_colorwheel_get_rgb(lv_obj_t *obj) {
  ...
  return lv_color_hsv_to_rgb(colorwheel->hsv.h, colorwheel->hsv.s,
                             colorwheel->hsv.v);
}
...
static void draw_disc_grad(lv_event_t *e) {
...
#if LV_DRAW_COMPLEX
  /*Mask outer and inner ring of widget to tidy up ragged edges of lines while
   * drawing outer ring*/
  lv_draw_mask_radius_param_t mask_out_param;
  lv_draw_mask_radius_init(&mask_out_param, &obj->coords, LV_RADIUS_CIRCLE,
                           false);
   ...
   *So make lines a little bit longer because the masking makes a more even
   *result*/
  ...
  for (i = 0; i <= 256; i += LV_CPICKER_DEF_QF, a += 360 * LV_CPICKER_DEF_QF) {
    ...
    uint16_t angle_trigo =
        (uint16_t)(a >> 8); /*i * 360 / 256 is the scale to apply, but we can
                               skip multiplication here*/

    lv_point_t p[2];
    p[0].x =
        cx + ((r + cir_w_extra) * lv_trigo_sin(angle_trigo) >> LV_TRIGO_SHIFT);
    p[0].y =
        cy + ((r + cir_w_extra) * lv_trigo_cos(angle_trigo) >> LV_TRIGO_SHIFT);
    p[1].x = cx + ((r - cir_w - cir_w_extra) * lv_trigo_sin(angle_trigo) >>
                   LV_TRIGO_SHIFT);
    p[1].y = cy + ((r - cir_w - cir_w_extra) * lv_trigo_cos(angle_trigo) >>
                   LV_TRIGO_SHIFT);
...
static void lv_colorwheel_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
    ...
    *s = LV_MAX(*s, knob_pad);
    ...
    uint32_t c =
        *((uint32_t *)lv_event_get_param(e)); /*uint32_t because can be UTF-8*/
    ...
    /*If the inner area is being pressed, go to the next color mode on long
     * press*/
    uint32_t diff = lv_tick_elaps(colorwheel->last_change_time);
    if (!on_ring && diff > indev->driver->long_press_time &&
        !colorwheel->mode_fixed) {
      next_color_mode(obj);
    ...
    info->res =
        _lv_area_is_point_on(&obj->coords, info->point, LV_RADIUS_CIRCLE);
...
static void refr_knob_pos(lv_obj_t *obj) {
  ...
  colorwheel->knob.pos.x =
      (((int32_t)r * lv_trigo_sin(angle)) >> LV_TRIGO_SHIFT);
  colorwheel->knob.pos.y =
      (((int32_t)r * lv_trigo_cos(angle)) >> LV_TRIGO_SHIFT);
...
static lv_res_t double_click_reset(lv_obj_t *obj) {
  ...
  if (lv_tick_elaps(colorwheel->last_click_time) <
 ...
 * Here we want to compute an approximate RGB value from a HSV input color
 * space. We don't want to be accurate (for that, there's lv_color_hsv_to_rgb),
 * but we want to be fast.
 *
 * Few tricks are used here: Hue is in range [0; 6 * 256] (so that the sextant
 * is in the high byte and the fractional part is in the low byte) both s and v
 * are in [0; 255] range (very convenient to avoid divisions).
 *
 * We fold all symmetry by swapping the R, G, B pointers so that the code is the
 * same for all sextants. We replace division by 255 by a division by 256, a.k.a
 * a shift right by 8 bits. This is wrong, but since this is only used to
 * compute the pixels on the screen and not the final color, it's ok.
 */
static void fast_hsv2rgb(uint16_t h, uint8_t s, uint8_t v, uint8_t *r,
                         uint8_t *g, uint8_t *b);
static void fast_hsv2rgb(uint16_t h, uint8_t s, uint8_t v, uint8_t *r,
                         uint8_t *g, uint8_t *b) {
  if (!s) {
  ...
  uint8_t sextant = h >> 8;
  HSV_PTR_SWAP(sextant, r, g,
               b); /*Swap pointers so the conversion code is the same*/
  ...
  *b = ww >> 8;
  ...
  if (!(sextant & 1)) {
    /*Up slope*/
    ww = !h_frac ? ((uint16_t)s << 8)
                 : (s * (uint8_t)(-h_frac)); /*Skip multiply if not required*/
  } else {
    /*Down slope*/
    ww = s * h_frac;
  }
...
static lv_color_t angle_to_mode_color_fast(lv_obj_t *obj, uint16_t angle) {
  ...
  case LV_COLORWHEEL_MODE_HUE:
    ...
    fast_hsv2rgb(angle * 6, s, v, &r, &g,
                 &b); /*A smart compiler will replace x * 6 by (x << 2) + (x <<
