...
static void new_points_alloc(lv_obj_t *obj, lv_chart_series_t *ser,
...
void lv_chart_set_range(lv_obj_t *obj, lv_chart_axis_t axis, lv_coord_t min,
                        lv_coord_t max) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_chart_set_update_mode(lv_obj_t *obj,
...
uint16_t lv_chart_get_x_start_point(const lv_obj_t *obj,
                                    lv_chart_series_t *ser) {
  LV_UNUSED(obj);
  LV_ASSERT_NULL(ser);
...
void lv_chart_get_point_pos_by_id(lv_obj_t *obj, lv_chart_series_t *ser,
  ...
  int32_t temp_y = 0;
  temp_y =
      (int32_t)((int32_t)ser->y_points[id] - chart->ymin[ser->y_axis_sec]) * h;
  temp_y =
      temp_y / (chart->ymax[ser->y_axis_sec] - chart->ymin[ser->y_axis_sec]);
  p_out->y = h - temp_y;
...
void lv_chart_hide_series(lv_obj_t *chart, lv_chart_series_t *series,
                          bool hide) {
...
void lv_chart_set_series_color(lv_obj_t *chart, lv_chart_series_t *series,
                               lv_color_t color) {
  LV_ASSERT_OBJ(chart, MY_CLASS);
  LV_ASSERT_NULL(series);
...
void lv_chart_set_x_start_point(lv_obj_t *obj, lv_chart_series_t *ser,
                                uint16_t id) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(ser);
...
lv_chart_series_t *lv_chart_get_series_next(const lv_obj_t *obj,
 ...
 */
lv_chart_cursor_t *lv_chart_add_cursor(lv_obj_t *obj, lv_color_t color,
                                       lv_dir_t dir) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
 ...
 */
void lv_chart_set_cursor_pos(lv_obj_t *chart, lv_chart_cursor_t *cursor,
                             lv_point_t *pos) {
 ...
 */
lv_point_t lv_chart_get_cursor_point(lv_obj_t *chart,
...
void lv_chart_set_all_value(lv_obj_t *obj, lv_chart_series_t *ser,
                            lv_coord_t value) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(ser);
...
void lv_chart_set_ext_y_array(lv_obj_t *obj, lv_chart_series_t *ser,
                              lv_coord_t array[]) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(ser);
...
void lv_chart_set_ext_x_array(lv_obj_t *obj, lv_chart_series_t *ser,
                              lv_coord_t array[]) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(ser);
...
static void lv_chart_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_PRESSED) {
    ...
    lv_event_set_ext_draw_size(
        e, LV_MAX4(chart->tick[0].draw_size, chart->tick[1].draw_size,
...
static void draw_div_lines(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  bool mask_ret =
      _lv_area_intersect(&series_clip_area, &obj->coords, draw_ctx->clip_area);
  if (mask_ret == false)
    return;
...
static void draw_series_line(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  _LV_LL_READ_BACK(&chart->series_ll, ser) {
    ...
    lv_coord_t start_point =
        chart->update_mode == LV_CHART_UPDATE_MODE_SHIFT ? ser->start_point : 0;
    ...
    int32_t y_tmp = (int32_t)((int32_t)ser->y_points[p_prev] -
                    ...
                    h;
    y_tmp =
        y_tmp / (chart->ymax[ser->y_axis_sec] - chart->ymin[ser->y_axis_sec]);
    p2.y = h - y_tmp + y_ofs;
    ...
    for (i = 0; i < chart->point_cnt; i++) {
      ...
      /*Don't draw the first point. A second point is also required to draw the
       * line*/
      if (i != 0) {
        if (crowded_mode) {
          if (ser->y_points[p_prev] != LV_CHART_POINT_NONE &&
              ser->y_points[p_act] != LV_CHART_POINT_NONE) {
            /*Draw only one vertical line between the min and max y-values on
             * the same x-value*/
            ...
            if (p1.x != p2.x) {
              ...
              p2.y = y_max;
              if (p1.y == p2.y)
                p2.y++; /*If they are the same no line will be drawn*/
              lv_draw_line(draw_ctx, &line_dsc_default, &p1, &p2);
              p2.x++;        /*Compensate the previous x--*/
              y_min = y_cur; /*Start the line of the next x from the current
                                last y*/
              y_max = y_cur;
            }
          }
          ...
          part_draw_dsc.p1 =
              ser->y_points[p_prev] != LV_CHART_POINT_NONE ? &p1 : NULL;
          part_draw_dsc.p2 =
              ser->y_points[p_act] != LV_CHART_POINT_NONE ? &p2 : NULL;
          ...
          if (point_w && point_h &&
              ser->y_points[p_prev] != LV_CHART_POINT_NONE) {
...
static void draw_series_scatter(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  lv_coord_t h = ((int32_t)lv_obj_get_content_height(obj) * chart->zoom_y) >> 8;
  lv_coord_t x_ofs =
      obj->coords.x1 + pad_left + border_width - lv_obj_get_scroll_left(obj);
  lv_coord_t y_ofs =
...
static void draw_series_bar(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  for (i = 0; i < chart->point_cnt; i++) {
    ...
    _LV_LL_READ_BACK(&chart->series_ll, ser) {
      if (ser->hidden)
        continue;
      lv_coord_t start_point = chart->update_mode == LV_CHART_UPDATE_MODE_SHIFT
                                   ? ser->start_point
                                   : 0;
...
static void draw_cursors(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
  ...
  lv_area_t clip_area;
  if (!_lv_area_intersect(&clip_area, draw_ctx->clip_area, &obj->coords))
    return;
  ...
  _LV_LL_READ_BACK(&chart->cursor_ll, cursor) {
    ...
    if (cursor->pos_set) {
      ...
      if (cursor->point_id == LV_CHART_POINT_NONE)
        continue;
      lv_point_t p;
...
static void draw_y_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
  ...
  for (i = 0; i <= total_tick_num; i++) {
    ...
    p2.y = p1.y =
        y_ofs + (int32_t)((int32_t)(h - line_dsc.width) * i) / total_tick_num;
    ...
    int32_t tick_value = lv_map(total_tick_num - i, 0, total_tick_num,
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
  ...
  for (i = 0; i <= total_tick_num;
    ...
    p2.x = p1.x =
        x_ofs + (int32_t)((int32_t)(w - line_dsc.width) * i) / total_tick_num;
...
static void invalidate_point(lv_obj_t *obj, uint16_t i) {
  ...
  if (chart->type == LV_CHART_TYPE_LINE) {
    ...
    if (i < chart->point_cnt - 1) {
      coords.x1 =
          ((w * i) / (chart->point_cnt - 1)) + x_ofs - line_width - point_w;
  ...
  if (ser->start_point != 0) {
    ...
    if (cnt >= point_cnt_old) {
      for (i = 0; i < point_cnt_old; i++) {
        new_points[i] = (*a)[(i + ser->start_point) %
                             point_cnt_old]; /*Copy old contents to new array*/
      }
      for (i = point_cnt_old; i < cnt; i++) {
        new_points[i] =
            LV_CHART_POINT_NONE; /*Fill up the rest with default value*/
      }
    } else {
      for (i = 0; i < cnt; i++) {
        new_points[i] = (*a)[(i + ser->start_point) %
