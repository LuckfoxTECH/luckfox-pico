...
static void lv_menu_page_constructor(const lv_obj_class_t *class_p,
                                     lv_obj_t *obj);
static void lv_menu_page_destructor(const lv_obj_class_t *class_p,
                                    lv_obj_t *obj);
static void lv_menu_cont_constructor(const lv_obj_class_t *class_p,
                                     lv_obj_t *obj);
static void lv_menu_section_constructor(const lv_obj_class_t *class_p,
                                        lv_obj_t *obj);

const lv_obj_class_t lv_menu_class = {.constructor_cb = lv_menu_constructor,
...
const lv_obj_class_t lv_menu_cont_class = {.constructor_cb =
                                               lv_menu_cont_constructor,
                                           .base_class = &lv_obj_class,
                                           .width_def = LV_PCT(100),
                                           .height_def = LV_SIZE_CONTENT};

const lv_obj_class_t lv_menu_section_class = {.constructor_cb =
                                                  lv_menu_section_constructor,
                                              .base_class = &lv_obj_class,
                                              .width_def = LV_PCT(100),
                                              .height_def = LV_SIZE_CONTENT};

const lv_obj_class_t lv_menu_separator_class = {.base_class = &lv_obj_class,
                                                .width_def = LV_SIZE_CONTENT,
                                                .height_def = LV_SIZE_CONTENT};

const lv_obj_class_t lv_menu_sidebar_cont_class = {.base_class = &lv_obj_class};

const lv_obj_class_t lv_menu_main_cont_class = {.base_class = &lv_obj_class};

const lv_obj_class_t lv_menu_main_header_cont_class = {.base_class =
                                                           &lv_obj_class};
...
void lv_menu_set_page(lv_obj_t *obj, lv_obj_t *page) {
  ...
  if (menu->sidebar_page != NULL) {
    ...
    if (menu->cur_depth >= 2 ||
        menu->mode_root_back_btn == LV_MENU_ROOT_BACK_BTN_ENABLED) {
      lv_obj_clear_flag(menu->main_header_back_btn, LV_OBJ_FLAG_HIDDEN);
      lv_obj_add_flag(menu->main_header_back_btn, LV_OBJ_FLAG_CLICKABLE);
    } else {
      lv_obj_add_flag(menu->main_header_back_btn, LV_OBJ_FLAG_HIDDEN);
  ...
  if (page != NULL) {
    ...
    if (!menu->sidebar_generated) {
      ...
      lv_obj_t *sidebar_cont =
          lv_obj_class_create_obj(&lv_menu_sidebar_cont_class, obj);
      ...
      lv_obj_t *sidebar_header = lv_obj_class_create_obj(
          &lv_menu_sidebar_header_cont_class, sidebar_cont);
      ...
      lv_obj_set_flex_align(sidebar_header, LV_FLEX_ALIGN_START,
      ...
      lv_obj_add_event_cb(sidebar_header_back_btn, lv_menu_back_event_cb,
      ...
      lv_obj_t *sidebar_header_back_icon =
          lv_img_create(menu->sidebar_header_back_btn);
...
void lv_menu_set_mode_root_back_btn(
    lv_obj_t *obj, lv_menu_mode_root_back_btn_t mode_root_back_btn) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_menu_set_load_page_event(lv_obj_t *menu, lv_obj_t *obj,
                                 lv_obj_t *page) {
  ...
  lv_menu_load_page_event_data_t *event_data =
      lv_mem_alloc(sizeof(lv_menu_load_page_event_data_t));
  event_data->menu = menu;
  event_data->page = page;

  lv_obj_add_event_cb(obj, lv_menu_load_page_event_cb, LV_EVENT_CLICKED,
                      event_data);
  lv_obj_add_event_cb(obj, lv_menu_obj_del_event_cb, LV_EVENT_DELETE,
                      event_data);
}
...
bool lv_menu_back_btn_is_root(lv_obj_t *menu, lv_obj_t *obj) {
  ...
  if (obj == ((lv_menu_t *)menu)->main_header_back_btn &&
      ((lv_menu_t *)menu)->prev_depth <= 1) {
    return true;
  }
...
static void lv_menu_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
  ...
  lv_obj_t *main_header =
      lv_obj_class_create_obj(&lv_menu_main_header_cont_class, main_cont);
  ...
  lv_obj_set_flex_align(main_header, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
  ...
  lv_obj_add_event_cb(main_header_back_btn, lv_menu_back_event_cb,
                      LV_EVENT_CLICKED, menu);
  ...
  lv_obj_add_event_cb(obj, lv_menu_value_changed_event_cb,
...
static void lv_menu_back_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    if (!(obj == menu->main_header_back_btn ||
          obj == menu->sidebar_header_back_btn))
      return;

    menu->prev_depth =
        menu->cur_depth; /* Save the previous value for user event handler */
    ...
    if (prev_hist != NULL) {
       ...
       *  Remove it from the history because `lv_menu_set_page` will add it
       * again */
...
static void lv_menu_value_changed_event_cb(lv_event_t *e) {
  ...
  lv_menu_page_t *sidebar_page =
      (lv_menu_page_t *)lv_menu_get_cur_sidebar_page(obj);
