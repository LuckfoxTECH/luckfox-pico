...
#error                                                                         \
    "lv_meter: Complex drawing is required. Enable it in lv_conf.h (LV_DRAW_COMPLEX 1)"
#endif
 ...
 * @param obj           pointer to a meter object
 * @param scale         pointer to scale (added to `meter`)
 * @param nth           make every Nth normal tick major tick. (start from the
 * first on the left)
 ...
 * @param rotation      the angular offset from the 3 o'clock position
 * (clock-wise)
 */
 ...
 * @param r_mod         the radius modifier (added to the scale's radius) to get
 * the lines length
 ...
 * @param obj           pointer to a meter object
 * @param scale         pointer to scale (added to `meter`)
 * @param src           the image source of the indicator. path or pointer to
 * ::lv_img_dsc_t
 * @param pivot_x       the X pivot point of the needle
 * @param pivot_y       the Y pivot point of the needle
 * @return              the new indicator
 * @note                the needle image should point to the right, like
 * -O----->
 */
lv_meter_indicator_t *
 ...
 * @param local         tell how to map start and end color. true: the
 * indicator's start and end_value; false: the scale's min max value
...
/**
 * Set the value of the indicator. It will set start and and value to the same
 * value
 * @param obj           pointer to a meter object
