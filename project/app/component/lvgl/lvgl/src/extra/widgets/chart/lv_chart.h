...
typedef enum {
  ...
  LV_CHART_DRAW_PART_DIV_LINE_HOR,  /**< Used for each horizontal division
                                       lines*/
  LV_CHART_DRAW_PART_DIV_LINE_VER,  /**< Used for each vertical division lines*/
  LV_CHART_DRAW_PART_LINE_AND_POINT, /**< Used on line and scatter charts for
                                        lines and points*/
 ...
 * @param obj       pointer to a chart object
 * @param zoom_x    zoom in x direction. LV_ZOOM_NONE or 256 for no zoom, 512
 * double zoom
 */
 ...
 * @param obj       pointer to a chart object
 * @param zoom_y    zoom in y direction. LV_ZOOM_NONE or 256 for no zoom, 512
 * double zoom
 */
 ...
 *                      (start with 20 px and increase if the ticks/labels are
 * clipped)
 */
 ...
 * @param axis      the y axis to which the series should be attached
 ...
 * This point will be considers the first (left) point and the other points will
 * be drawn after it.
 * @param obj       pointer to a chart object
 * @param ser       pointer to a data series on 'chart'
 ...
 */
lv_chart_series_t *lv_chart_get_series_next(const lv_obj_t *chart,
 ...
 * @param point_id  the point's index or  `LV_CHART_POINT_NONE` to not assign to
 * any points.
 */
 ...
 * @param obj       pointer to chart object
 * @param ser       pointer to a data series on 'chart'
 * @param value     the new value for all points. `LV_CHART_POINT_NONE` can be
 * used to hide the points.
 */
...
/**
 * Set an individual point's y value of a chart's series directly based on its
 * index
 * @param obj     pointer to a chart object
 * @param ser     pointer to a data series on 'chart'
...
/**
 * Set an individual point's x and y value of a chart's series directly based on
 * its index Can be used only with `LV_CHART_TYPE_SCATTER`.
 * @param obj       pointer to chart object
 * @param ser       pointer to a data series on 'chart'
 * @param id        the index of the x point in the array
 * @param x_value   the new X value of the next data
 ...
 * NOTE: It is the users responsibility to make sure the `point_cnt` matches the
 * external array size.
 * @param obj       pointer to a chart object
 * @param ser       pointer to a data series on 'chart'
 ...
 * @param obj       pointer to a chart object
 * @return          the index of the point [0 .. point count] or
