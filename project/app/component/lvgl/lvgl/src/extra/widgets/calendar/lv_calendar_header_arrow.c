...
lv_obj_t *lv_calendar_header_arrow_create(lv_obj_t *parent) {
  lv_obj_t *obj =
      lv_obj_class_create_obj(&lv_calendar_header_arrow_class, parent);
...
static void my_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
  ...
  lv_obj_add_event_cb(obj, value_changed_event_cb, LV_EVENT_VALUE_CHANGED,
                      NULL);
...
static void month_event_cb(lv_event_t *e) {
  ...
  lv_label_set_text_fmt(label, "%d %s", newd.year,
                        month_names_def[newd.month - 1]);
}
...
static void value_changed_event_cb(lv_event_t *e) {
  ...
  lv_obj_t *label = lv_obj_get_child(header, 1);
  lv_label_set_text_fmt(label, "%d %s", cur_date->year,
                        month_names_def[cur_date->month - 1]);
}
