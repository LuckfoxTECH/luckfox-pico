...
#error                                                                         \
    "lv_spinbox: lv_ta is required. Enable it in lv_conf.h (LV_USE_TEXTAREA  1) "
#endif
...
typedef struct {
  ...
  uint16_t digit_step_dir : 2; // the direction the digit will step on encoder
                               // button press when editing
} lv_spinbox_t;
 ...
 * Set spinbox step
 * @param obj pointer to spinbox
 * @param step steps on increment/decrement. Can be 1, 10, 100, 1000, etc the
 * digit that will change.
 */
