 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_calendar_constructor(const lv_obj_class_t *class_p,
                                    lv_obj_t *obj);
...
void lv_calendar_set_showed_date(lv_obj_t *obj, uint32_t year, uint32_t month) {
  ...
  for (i = 0, c = prev_mo_len - day_first + 1; i < day_first; i++, c++) {
    lv_snprintf(calendar->nums[i], sizeof(calendar->nums[0]), "%d", c);
    lv_btnmatrix_set_btn_ctrl(calendar->btnm, i + 7,
...
lv_res_t lv_calendar_get_pressed_date(const lv_obj_t *obj,
  ...
  const char *txt = lv_btnmatrix_get_btn_text(
      calendar->btnm, lv_btnmatrix_get_selected_btn(calendar->btnm));
...
static void lv_calendar_constructor(const lv_obj_class_t *class_p,
  ...
  lv_btnmatrix_set_btn_ctrl_all(calendar->btnm,
                                LV_BTNMATRIX_CTRL_CLICK_TRIG |
                                    LV_BTNMATRIX_CTRL_NO_REPEAT);
  lv_obj_add_event_cb(calendar->btnm, draw_part_begin_event_cb,
  ...
  lv_calendar_set_showed_date(obj, calendar->showed_date.year,
                              calendar->showed_date.month);
  lv_calendar_set_today_date(obj, calendar->today.year, calendar->today.month,
                             calendar->today.day);
...
static void draw_part_begin_event_cb(lv_event_t *e) {
  ...
  if (dsc->part == LV_PART_ITEMS) {
    ...
    } else if (lv_btnmatrix_has_btn_ctrl(obj, dsc->id,
                                         LV_BTNMATRIX_CTRL_DISABLED)) {
      dsc->rect_dsc->bg_opa = LV_OPA_TRANSP;
      dsc->rect_dsc->border_opa = LV_OPA_TRANSP;
 ...
 * @param year a year
 * @param month a month. The range is basically [1..12] but [-11..0] or [13..24]
 * is also supported to handle next/prev. year
 * @return [28..31]
 */
static uint8_t get_month_length(int32_t year, int32_t month) {
  month--;
  if (month < 0) {
    year--; /*Already in the previous year (won't be less then -12 to skip a
               whole year)*/
 ...
 * @return [0..6] which means [Sun..Sat] or [Mon..Sun] depending on
...
static uint8_t get_day_of_week(uint32_t year, uint32_t month, uint32_t day) {
  ...
  uint32_t day_of_week = (day + (31 * (month - 2 + 12 * a) / 12) + b + (b / 4) -
                          (b / 100) + (b / 400) - 1) %
                         7;
#else
  uint32_t day_of_week = (day + (31 * (month - 2 + 12 * a) / 12) + b + (b / 4) -
...
static void highlight_update(lv_obj_t *obj) {
  ...
  lv_btnmatrix_clear_btn_ctrl_all(
      calendar->btnm, LV_CALENDAR_CTRL_TODAY | LV_CALENDAR_CTRL_HIGHLIGHT);
  ...
  if (calendar->highlighted_dates) {
    for (i = 0; i < calendar->highlighted_dates_num; i++) {
      if (calendar->highlighted_dates[i].year == calendar->showed_date.year &&
        ...
        lv_btnmatrix_set_btn_ctrl(calendar->btnm,
                                  calendar->highlighted_dates[i].day - 1 +
                                      day_first + 7,
