...
static void item_repos(lv_obj_t *item, _lv_grid_calc_t *c,
                       item_repos_hint_t *hint);
static lv_coord_t grid_align(lv_coord_t cont_size, bool auto_size,
                             uint8_t align, lv_coord_t gap, uint32_t track_num,
                             lv_coord_t *size_array, lv_coord_t *pos_array,
                             bool reverse);
...
void lv_obj_set_grid_dsc_array(lv_obj_t *obj, const lv_coord_t col_dsc[],
...
void lv_obj_set_grid_cell(lv_obj_t *obj, lv_grid_align_t x_align,
                          uint8_t col_pos, uint8_t col_span,
                          lv_grid_align_t y_align, uint8_t row_pos,
                          uint8_t row_span)
...
void lv_style_set_grid_row_dsc_array(lv_style_t *style,
                                     const lv_coord_t value[]) {
  lv_style_value_t v = {.ptr = (const void *)value};
  lv_style_set_prop(style, LV_STYLE_GRID_ROW_DSC_ARRAY, v);
}

void lv_style_set_grid_column_dsc_array(lv_style_t *style,
                                        const lv_coord_t value[]) {
  lv_style_value_t v = {.ptr = (const void *)value};
...
void lv_style_set_grid_row_align(lv_style_t *style, lv_grid_align_t value) {
  lv_style_value_t v = {.num = (lv_grid_align_t)value};
...
void lv_obj_set_style_grid_row_dsc_array(lv_obj_t *obj,
                                         const lv_coord_t value[],
...
void lv_obj_set_style_grid_column_dsc_array(lv_obj_t *obj,
                                            const lv_coord_t value[],
                                            lv_style_selector_t selector) {
  lv_style_value_t v = {.ptr = (const void *)value};
...
void lv_obj_set_style_grid_row_align(lv_obj_t *obj, lv_grid_align_t value,
                                     lv_style_selector_t selector) {
  lv_style_value_t v = {.num = (int32_t)value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_ROW_ALIGN, v, selector);
}

void lv_obj_set_style_grid_column_align(lv_obj_t *obj, lv_grid_align_t value,
                                        lv_style_selector_t selector) {
  lv_style_value_t v = {.num = (int32_t)value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_COLUMN_ALIGN, v, selector);
}

void lv_obj_set_style_grid_cell_column_pos(lv_obj_t *obj, lv_coord_t value,
                                           lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_CELL_COLUMN_POS, v, selector);
}

void lv_obj_set_style_grid_cell_column_span(lv_obj_t *obj, lv_coord_t value,
                                            lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_CELL_COLUMN_SPAN, v, selector);
}

void lv_obj_set_style_grid_cell_row_pos(lv_obj_t *obj, lv_coord_t value,
                                        lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_CELL_ROW_POS, v, selector);
}

void lv_obj_set_style_grid_cell_row_span(lv_obj_t *obj, lv_coord_t value,
                                         lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_CELL_ROW_SPAN, v, selector);
}

void lv_obj_set_style_grid_cell_x_align(lv_obj_t *obj, lv_coord_t value,
                                        lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
  lv_obj_set_local_style_prop(obj, LV_STYLE_GRID_CELL_X_ALIGN, v, selector);
}

void lv_obj_set_style_grid_cell_y_align(lv_obj_t *obj, lv_coord_t value,
                                        lv_style_selector_t selector) {
  lv_style_value_t v = {.num = value};
...
static void grid_update(lv_obj_t *cont, void *user_data) {
  ...
  if (col_templ == NULL || row_templ == NULL)
    return;
   ...
   *It will be used as helper during item repositioning to avoid calculating
   *this value for every children*/
  lv_coord_t border_widt = lv_obj_get_style_border_width(cont, LV_PART_MAIN);
  lv_coord_t pad_left =
      lv_obj_get_style_pad_left(cont, LV_PART_MAIN) + border_widt;
  lv_coord_t pad_top =
      lv_obj_get_style_pad_top(cont, LV_PART_MAIN) + border_widt;
 ...
 * @note `_lv_grid_calc_free(calc_out)` needs to be called when `calc_out` is
 * not needed anymore
 */
static void calc(lv_obj_t *cont, _lv_grid_calc_t *calc_out) {
  ...
  calc_out->grid_w =
      grid_align(cont_w, auto_w, get_grid_col_align(cont), col_gap,
                 calc_out->col_num, calc_out->w, calc_out->x, rev);

  bool auto_h = (h_set == LV_SIZE_CONTENT && !cont->h_layout) ? true : false;
  lv_coord_t cont_h = lv_obj_get_content_height(cont);
  calc_out->grid_h =
      grid_align(cont_h, auto_h, get_grid_row_align(cont), row_gap,
                 calc_out->row_num, calc_out->h, calc_out->y, false);
...
static void calc_cols(lv_obj_t *cont, _lv_grid_calc_t *c) {
  ...
  if (last_fr_i >= 0) {
    c->w[last_fr_i] =
        free_w - ((free_w * (col_fr_cnt - last_fr_x)) / col_fr_cnt);
  }
}
...
static void calc_rows(lv_obj_t *cont, _lv_grid_calc_t *c) {
  ...
  lv_coord_t row_gap = lv_obj_get_style_pad_row(cont, LV_PART_MAIN);
  lv_coord_t cont_h =
      lv_obj_get_content_height(cont) - row_gap * (c->row_num - 1);
  ...
  if (last_fr_i >= 0) {
    c->h[last_fr_i] =
        free_h - ((free_h * (row_fr_cnt - last_fr_x)) / row_fr_cnt);
  }
}
 ...
 * @param child_id_ext helper value if the ID of the child is know (order from
 * the oldest) else -1
 * @param grid_abs helper value, the absolute position of the grid, NULL if
 * unknown
 */
static void item_repos(lv_obj_t *item, _lv_grid_calc_t *c,
                       item_repos_hint_t *hint) {
  if (lv_obj_has_flag_any(item, LV_OBJ_FLAG_IGNORE_LAYOUT | LV_OBJ_FLAG_HIDDEN |
                                    LV_OBJ_FLAG_FLOATING))
    return;
  uint32_t col_span = get_col_span(item);
  uint32_t row_span = get_row_span(item);
  ...
  lv_coord_t col_x2 =
      c->x[col_pos + col_span - 1] + c->w[col_pos + col_span - 1];
  lv_coord_t col_w = col_x2 - col_x1;

  lv_coord_t row_y1 = c->y[row_pos];
  lv_coord_t row_y2 =
      c->y[row_pos + row_span - 1] + c->h[row_pos + row_span - 1];
  ...
  if (lv_obj_get_style_base_dir(item, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
    if (col_align == LV_GRID_ALIGN_START)
      col_align = LV_GRID_ALIGN_END;
    else if (col_align == LV_GRID_ALIGN_END)
      col_align = LV_GRID_ALIGN_START;
  }
...
/**
 * Place the grid track according to align methods. It keeps the track sizes but
 * sets their position. It can process both columns or rows according to the
 * passed parameters.
...
static lv_coord_t grid_align(lv_coord_t cont_size, bool auto_size,
  ...
  if (auto_size) {
    ...
    if (align == LV_GRID_ALIGN_SPACE_AROUND ||
        align == LV_GRID_ALIGN_SPACE_BETWEEN ||
        align == LV_GRID_ALIGN_SPACE_EVENLY) {
      gap = 0;
      if (track_num == 1)
        align = LV_GRID_ALIGN_CENTER;
    }
  ...
  /*Set the position of all tracks from the start position, gaps and track
   * sizes*/
