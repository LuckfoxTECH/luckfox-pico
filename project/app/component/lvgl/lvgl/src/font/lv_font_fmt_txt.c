...
static void decompress(const uint8_t *in, uint8_t *out, lv_coord_t w,
                       lv_coord_t h, uint8_t bpp, bool prefilter);
static inline void decompress_line(uint8_t *out, lv_coord_t w);
static inline uint8_t get_bits(const uint8_t *in, uint32_t bit_pos,
                               uint8_t len);
static inline void bits_write(uint8_t *out, uint32_t bit_pos, uint8_t val,
                              uint8_t len);
...
/**
 * Used as `get_glyph_bitmap` callback in LittelvGL's native font format if the
...
const uint8_t *lv_font_get_bitmap_fmt_txt(const lv_font_t *font,
  ...
  /*Handle compressed bitmap*/
    ...
    bool prefilter =
        fdsc->bitmap_format == LV_FONT_FMT_TXT_COMPRESSED ? true : false;
    decompress(&fdsc->glyph_bitmap[gdsc->bitmap_index],
               LV_GC_ROOT(_lv_font_decompr_buf), gdsc->box_w, gdsc->box_h,
               (uint8_t)fdsc->bpp, prefilter);
    return LV_GC_ROOT(_lv_font_decompr_buf);
#else /*!LV_USE_FONT_COMPRESSED*/
    LV_LOG_WARN("Compressed fonts is used but LV_USE_FONT_COMPRESSED is not "
  ...
  for (i = 0; i < fdsc->cmap_num; i++) {
    ...
    if (fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY) {
      ...
      uint16_t *p = _lv_utils_bsearch(
          &key, fdsc->cmaps[i].unicode_list, fdsc->cmaps[i].list_length,
...
static int8_t get_kern_value(const lv_font_t *font, uint32_t gid_left,
  ...
  if (fdsc->kern_classes == 0) {
    ...
    if (kdsc->glyph_ids_size == 0) {
      ...
      uint16_t g_id_both =
          (gid_right << 8) + gid_left; /*Create one number from the ids*/
      uint16_t *kid_p = _lv_utils_bsearch(&g_id_both, g_ids, kdsc->pair_cnt, 2,
                                          kern_pair_8_compare);
      ...
      uint32_t g_id_both =
          (gid_right << 16) + gid_left; /*Create one number from the ids*/
      uint32_t *kid_p = _lv_utils_bsearch(&g_id_both, g_ids, kdsc->pair_cnt, 4,
                                          kern_pair_16_compare);
    ...
    if (left_class > 0 && right_class > 0) {
      value = kdsc->class_pair_values[(left_class - 1) * kdsc->right_class_cnt +
                                      (right_class - 1)];
    }
  }
  return value;
...
/**
 * Write `val` data to `bit_pos` position of `out`. The write can NOT cross byte
 * boundary.
