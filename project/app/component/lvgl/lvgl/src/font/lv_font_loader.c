...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
...
static bool load_cmaps_tables(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
                              uint32_t cmaps_start,
                              cmap_table_bin_t *cmap_table) {
  if (lv_fs_read(fp, cmap_table, font_dsc->cmap_num * sizeof(cmap_table_bin_t),
                 NULL) != LV_FS_RES_OK) {
    return false;
  }
  ...
  for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
    lv_fs_res_t res =
        lv_fs_seek(fp, cmaps_start + cmap_table[i].data_offset, LV_FS_SEEK_SET);
    if (res != LV_FS_RES_OK) {
      return false;
    }

    lv_font_fmt_txt_cmap_t *cmap =
        (lv_font_fmt_txt_cmap_t *)&(font_dsc->cmaps[i]);
...
static int32_t load_cmaps(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
                          uint32_t cmaps_start) {
  ...
  if (lv_fs_read(fp, &cmaps_subtables_count, sizeof(uint32_t), NULL) !=
      LV_FS_RES_OK) {
    return -1;
  }
  ...
  cmap_table_bin_t *cmaps_tables =
      lv_mem_alloc(sizeof(cmap_table_bin_t) * font_dsc->cmap_num);
...
static int32_t load_glyph(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
                          uint32_t start, uint32_t *glyph_offset,
  ...
  lv_font_fmt_txt_glyph_dsc_t *glyph_dsc =
      (lv_font_fmt_txt_glyph_dsc_t *)lv_mem_alloc(
  ...
  for (unsigned int i = 0; i < loca_count; ++i) {
    ...
    int nbits =
        header->advance_width_bits + 2 * header->xy_bits + 2 * header->wh_bits;
    int next_offset =
        (i < loca_count - 1) ? glyph_offset[i + 1] : (uint32_t)glyph_length;
  ...
  for (unsigned int i = 1; i < loca_count; ++i) {
    ...
    if (nbits % 8 == 0) { /*Fast path*/
      if (lv_fs_read(fp, &glyph_bmp[cur_bmp_size], bmp_size, NULL) !=
          LV_FS_RES_OK) {
        return -1;
      }
    } else {
      ...
      glyph_bmp[cur_bmp_size + bmp_size - 1] =
          read_bits(&bit_it, 8 - nbits % 8, &res);
      if (res != LV_FS_RES_OK) {
        return -1;
      }

      /*The last fragment should be on the MSB but read_bits() will place it to
       * the LSB*/
      glyph_bmp[cur_bmp_size + bmp_size - 1] =
          glyph_bmp[cur_bmp_size + bmp_size - 1] << (nbits % 8);
    }
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font) {
  lv_font_fmt_txt_dsc_t *font_dsc =
      (lv_font_fmt_txt_dsc_t *)lv_mem_alloc(sizeof(lv_font_fmt_txt_dsc_t));
  ...
  if (lv_fs_read(fp, &font_header, sizeof(font_header_bin_t), NULL) !=
      LV_FS_RES_OK) {
    return false;
  }
  ...
  if (font_header.index_to_loc_format == 0) {
    ...
    if (lv_fs_read(fp, glyph_offset, loca_count * sizeof(uint32_t), NULL) !=
        LV_FS_RES_OK) {
      failed = true;
    }
  } else {
    LV_LOG_WARN("Unknown index_to_loc_format: %d.",
  ...
  int32_t glyph_length = load_glyph(fp, font_dsc, glyph_start, glyph_offset,
                                    loca_count, &font_header);
  ...
  int32_t kern_length =
      load_kern(fp, font_dsc, font_header.glyph_id_format, kern_start);
...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
  ...
  int32_t padding;
  if (lv_fs_read(fp, &kern_format_type, sizeof(uint8_t), NULL) !=
          LV_FS_RES_OK ||
  ...
  if (0 == kern_format_type) { /*sorted pairs*/
    lv_font_fmt_txt_kern_pair_t *kern_pair =
        lv_mem_alloc(sizeof(lv_font_fmt_txt_kern_pair_t));
    ...
    if (lv_fs_read(fp, &glyph_entries, sizeof(uint32_t), NULL) !=
        LV_FS_RES_OK) {
      return -1;
    }
    ...
    if (lv_fs_read(fp, &kern_class_mapping_length, sizeof(uint16_t), NULL) !=
            LV_FS_RES_OK ||
        lv_fs_read(fp, &kern_table_rows, sizeof(uint8_t), NULL) !=
            LV_FS_RES_OK ||
        lv_fs_read(fp, &kern_table_cols, sizeof(uint8_t), NULL) !=
            LV_FS_RES_OK) {
      return -1;
    }
    ...
    if (lv_fs_read(fp, kern_left, kern_class_mapping_length, NULL) !=
            LV_FS_RES_OK ||
        lv_fs_read(fp, kern_right, kern_class_mapping_length, NULL) !=
            LV_FS_RES_OK ||
