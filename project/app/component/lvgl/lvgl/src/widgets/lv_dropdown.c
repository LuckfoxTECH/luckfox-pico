...
static void lv_dropdown_constructor(const lv_obj_class_t *class_p,
                                    lv_obj_t *obj);
static void lv_dropdown_destructor(const lv_obj_class_t *class_p,
                                   lv_obj_t *obj);
static void lv_dropdown_event(const lv_obj_class_t *class_p, lv_event_t *e);
static void draw_main(lv_event_t *e);

static void lv_dropdownlist_constructor(const lv_obj_class_t *class_p,
                                        lv_obj_t *obj);
static void lv_dropdownlist_destructor(const lv_obj_class_t *class_p,
                                       lv_obj_t *list_obj);
static void lv_dropdown_list_event(const lv_obj_class_t *class_p,
                                   lv_event_t *e);
static void draw_list(lv_event_t *e);

static void draw_box(lv_obj_t *dropdown_obj, lv_draw_ctx_t *draw_ctx,
                     uint16_t id, lv_state_t state);
static void draw_box_label(lv_obj_t *dropdown_obj, lv_draw_ctx_t *draw_ctx,
                           uint16_t id, lv_state_t state);
...
void lv_dropdown_add_option(lv_obj_t *obj, const char *option, uint32_t pos) {
  ...
  if ((insert_pos > 0) && (pos >= dropdown->option_cnt))
    _lv_txt_ins(dropdown->options,
                _lv_txt_encoded_get_char_id(dropdown->options, insert_pos++),
                "\n");

  /*Insert the new option, adding \n if necessary*/
  char *ins_buf =
      lv_mem_buf_get(ins_len + 2); /*+ 2 for terminating NULL and possible \n*/
...
void lv_dropdown_set_selected(lv_obj_t *obj, uint16_t sel_opt) {
  ...
  dropdown->sel_opt_id =
      sel_opt < dropdown->option_cnt ? sel_opt : dropdown->option_cnt - 1;
...
void lv_dropdown_get_selected_str(const lv_obj_t *obj, char *buf,
                                  uint32_t buf_size) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_dropdown_open(lv_obj_t *dropdown_obj) {
  ...
  lv_coord_t border_width =
      lv_obj_get_style_border_width(dropdown->list, LV_PART_MAIN);
  lv_coord_t top =
      lv_obj_get_style_pad_top(dropdown->list, LV_PART_MAIN) + border_width;
  lv_coord_t bottom =
      lv_obj_get_style_pad_bottom(dropdown->list, LV_PART_MAIN) + border_width;
  ...
  if (dir == LV_DIR_BOTTOM)
    lv_obj_align_to(dropdown->list, dropdown_obj, LV_ALIGN_OUT_BOTTOM_LEFT, 0,
                    0);
  ...
  lv_text_align_t align =
      lv_obj_calculate_style_text_align(label, LV_PART_MAIN, dropdown->options);
...
static void lv_dropdown_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_FOCUSED) {
    ...
    if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
       ...
       * Do no process it if ENTER is sent by the dropdown because it's handled
       * in LV_EVENT_RELEASED */
...
static void draw_main(lv_event_t *e) {
  ...
  lv_coord_t right =
      lv_obj_get_style_pad_right(obj, LV_PART_MAIN) + border_width;
  ...
  if (dropdown->symbol) {
    ...
    if (symbol_type == LV_IMG_SRC_SYMBOL) {
      lv_point_t size;
      lv_txt_get_size(&size, dropdown->symbol, symbol_dsc.font,
                      symbol_dsc.letter_space, symbol_dsc.line_space,
    ...
    if (symbol_type == LV_IMG_SRC_SYMBOL) {
      ...
      lv_draw_label(draw_ctx, &symbol_dsc, &symbol_area, dropdown->symbol,
                    NULL);
    } else {
  ...
  lv_point_t size;
  lv_txt_get_size(&size, opt_txt, label_dsc.font, label_dsc.letter_space,
...
static void draw_list(lv_event_t *e) {
  ...
  bool has_common;
  has_common = _lv_area_intersect(&clip_area_core, draw_ctx->clip_area,
                                  &dropdown->list->coords);
  if (has_common) {
    const lv_area_t *clip_area_ori = draw_ctx->clip_area;
    draw_ctx->clip_area = &clip_area_core;
    if (dropdown->selected_highlight) {
      if (dropdown->pr_opt_id == dropdown->sel_opt_id) {
        draw_box(dropdown_obj, draw_ctx, dropdown->pr_opt_id,
                 LV_STATE_CHECKED | LV_STATE_PRESSED);
        draw_box_label(dropdown_obj, draw_ctx, dropdown->pr_opt_id,
                       LV_STATE_CHECKED | LV_STATE_PRESSED);
      } else {
        draw_box(dropdown_obj, draw_ctx, dropdown->pr_opt_id, LV_STATE_PRESSED);
        draw_box_label(dropdown_obj, draw_ctx, dropdown->pr_opt_id,
                       LV_STATE_PRESSED);
        draw_box(dropdown_obj, draw_ctx, dropdown->sel_opt_id,
                 LV_STATE_CHECKED);
        draw_box_label(dropdown_obj, draw_ctx, dropdown->sel_opt_id,
                       LV_STATE_CHECKED);
      }
    } else {
      draw_box(dropdown_obj, draw_ctx, dropdown->pr_opt_id, LV_STATE_PRESSED);
...
static void draw_box(lv_obj_t *dropdown_obj, lv_draw_ctx_t *draw_ctx,
  ...
  const lv_font_t *font =
      lv_obj_get_style_text_font(list_obj, LV_PART_SELECTED);
  lv_coord_t line_space =
      lv_obj_get_style_text_line_space(list_obj, LV_PART_SELECTED);
...
static void draw_box_label(lv_obj_t *dropdown_obj, lv_draw_ctx_t *draw_ctx,
  ...
  label_dsc.line_space = lv_obj_get_style_text_line_space(
      list_obj, LV_PART_SELECTED); /*Line space should come from the list*/
  ...
  if (area_ok) {
    ...
    lv_draw_label(draw_ctx, &label_dsc, &label->coords,
                  lv_label_get_text(label), NULL);
    draw_ctx->clip_area = clip_area_ori;
  }
  ...
  if (lv_indev_get_scroll_obj(indev) == NULL) {
    if (lv_dropdown_is_open(obj)) {
      ...
      if (dropdown->sel_opt_id_orig != dropdown->sel_opt_id) {
        ...
        lv_res_t res;
        uint32_t id =
            dropdown->sel_opt_id; /*Just to use uint32_t in event data*/
 ...
 * @return LV_RES_INV if the list is not being deleted in the user callback.
 * Else LV_RES_OK
 */
static lv_res_t list_release_handler(lv_obj_t *list_obj) {
  lv_dropdown_list_t *list = (lv_dropdown_list_t *)list_obj;
  ...
  /*Search the clicked option (For KEYPAD and ENCODER the new value should be
   * already set)*/
