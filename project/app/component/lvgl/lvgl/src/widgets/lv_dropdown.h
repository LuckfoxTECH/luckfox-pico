...
#if LV_USE_LABEL == 0
#error                                                                         \
    "lv_dropdown: lv_label is required. Enable it in lv_conf.h (LV_USE_LABEL 1)"
#endif
...
typedef struct {
  ...
  uint8_t selected_highlight : 1; /**< 1: Make the selected option highlighted
                                     in the list*/
} lv_dropdown_t;
 ...
 * If set to a specific text then that text will be shown regardless of the
 * selected option.
 ...
 * The options will be copied and saved in the object so the `options` can be
...
/**
 * Set the options in a drop-down list from a static string (global, static or
 * dynamically allocated). Only the pointer of the option string will be saved.
 * @param obj       pointer to drop-down list object
 * @param options   a static string with '\n' separated options. E.g.
 * "One\nTwo\nThree"
 */
void lv_dropdown_set_options_static(lv_obj_t *obj, const char *options);

/**
 * Add an options to a drop-down list from a string.  Only works for non-static
 * options.
 * @param obj       pointer to drop-down list object
 * @param option    a string without '\n'. E.g. "Four"
 * @param pos       the insert position, indexed from 0, LV_DROPDOWN_POS_LAST =
 * end of string
 */
void lv_dropdown_add_option(lv_obj_t *obj, const char *option, uint32_t pos);

/**
 * Clear all options in a drop-down list.  Works with both static and dynamic
 * options.
 * @param obj       pointer to drop-down list object
 */
...
/**
 * Set an arrow or other symbol to display when on drop-down list's button.
 * Typically a down caret or arrow.
 * @param obj       pointer to drop-down list object
 * @param symbol    a text like `LV_SYMBOL_DOWN`, an image (pointer or path) or
 * NULL to not draw symbol icon
