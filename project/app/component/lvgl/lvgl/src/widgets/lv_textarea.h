 ...
 *      DEFINES
 *********************/
#define LV_TEXTAREA_CURSOR_LAST                                                \
  (0x7FFF) /*Put the cursor after the last character*/
...
typedef struct {
  ...
  char *placeholder_txt; /*Place holder label. only visible if text is an empty
                            string*/
  char *pwd_tmp;         /*Used to store the original text in password mode*/
  const char *accepted_chars; /*Only these characters will be accepted. NULL:
                                 accept all*/
  uint32_t max_length;        /*The max. number of characters. 0: no limit*/
  uint16_t pwd_show_time;     /*Time to show characters in password mode before
                                 change them to '*'*/
  struct {
    ...
    uint8_t click_pos : 1; /*1: Enable positioning the cursor by clicking the
                              text area*/
  } cursor;
 ...
 * @param parent    pointer to an object, it will be the parent of the new text
 * area
...
/**
 * Enable/Disable the positioning of the cursor by clicking the text on the text
 * area.
 * @param obj       pointer to a text area object
...
/**
 * Set a list of characters. Only these characters will be accepted by the text
 * area
 * @param obj       pointer to a text area object
 ...
 * @param obj       pointer to a text area object
 * @param num       the maximal number of characters can be added
 * (`lv_textarea_set_text` ignores it)
 */
void lv_textarea_set_max_length(lv_obj_t *obj, uint32_t num);

/**
 * In `LV_EVENT_INSERT` the text which planned to be inserted can be replaced by
 * an other text. It can be used to add automatic formatting to the text area.
 * @param obj       pointer to a text area object
 * @param txt       pointer to a new string to insert. If `""` no text will be
 * added. The variable must be live after the `event_cb` exists. (Should be
 * `global` or `static`)
 */
 ...
 * It sets where the label is aligned (in one line mode it can be smaller than
 * the text area) and how the lines of the area align in case of multiline text
 * area
 * @param obj       pointer to a text area object
