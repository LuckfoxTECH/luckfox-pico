 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_line_class = {.constructor_cb = lv_line_constructor,
...
void lv_line_set_points(lv_obj_t *obj, const lv_point_t points[],
                        uint16_t point_num) {
...
static void lv_line_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
    ...
    if (line->point_num == 0 || line->point_array == NULL)
      return;
    ...
    for (i = 0; i < line->point_num - 1; i++) {
      ...
      line_dsc.round_start =
          0; /*Draw the rounding only on the end points after the first line*/
    }
  }
}
#endif
