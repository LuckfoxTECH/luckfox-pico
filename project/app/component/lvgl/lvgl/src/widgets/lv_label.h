...
enum {
  LV_LABEL_LONG_WRAP, /**< Keep the object width, wrap the too long lines and
...
typedef struct {
  ...
  char *text;
  union {
    char *tmp_ptr; /*Pointer to the allocated memory containing the character
                      replaced by dots*/
    char tmp[LV_LABEL_DOT_NUM +
             1]; /*Directly store the characters if <=4 characters*/
  } dot;
  ...
  lv_label_long_mode_t
      long_mode : 3;         /*Determine what to do with the long texts*/
  uint8_t static_txt : 1;    /*Flag to indicate the text is static*/
  uint8_t recolor : 1;       /*Enable in-line letter re-coloring*/
  uint8_t expand : 1;        /*Ignore real width (used by the library with
                                LV_LABEL_LONG_SCROLL)*/
  uint8_t dot_tmp_alloc : 1; /*1: dot is allocated, 0: dot directly holds up to
                                4 chars*/
} lv_label_t;
 ...
 * if the text if aligned to the left)
 */
 ...
 * @return          The index of the letter on the 'pos_p' point (E.g. on 0;0 is
 * the 0. letter if aligned to the left) Expressed in character index and not
 ...
 * @param obj       pointer to a label object
 * @param pos       character index to insert. Expressed in character index and
 * not byte index. 0: before first char. LV_LABEL_POS_LAST: after last char.
