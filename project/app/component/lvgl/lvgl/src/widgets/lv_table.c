...
static lv_coord_t get_row_height(lv_obj_t *obj, uint16_t row_id,
                                 const lv_font_t *font, lv_coord_t letter_space,
                                 lv_coord_t line_space, lv_coord_t cell_left,
                                 lv_coord_t cell_right, lv_coord_t cell_top,
                                 lv_coord_t cell_bottom);
...
void lv_table_set_cell_value(lv_obj_t *obj, uint16_t row, uint16_t col,
                             const char *txt) {
  ...
  table->cell_data[cell] =
      lv_mem_realloc(table->cell_data[cell],
                     strlen(txt) + 2); /*+1: trailing '\0; +1: format byte*/
  LV_ASSERT_MALLOC(table->cell_data[cell]);
...
void lv_table_set_cell_value_fmt(lv_obj_t *obj, uint16_t row, uint16_t col,
                                 const char *fmt, ...) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(fmt);
  ...
  /*Allocate space for the new text by using trick from C99 standard
   * section 7.19.6.12*/
...
#else
  table->cell_data[cell] = lv_mem_realloc(
      table->cell_data[cell], len + 2); /*+1: trailing '\0; +1: format byte*/
  LV_ASSERT_MALLOC(table->cell_data[cell]);
  if (table->cell_data[cell] == NULL) {
    va_end(ap2);
    return;
  ...
  lv_coord_t letter_space =
      lv_obj_get_style_text_letter_space(obj, LV_PART_ITEMS);
...
void lv_table_set_row_cnt(lv_obj_t *obj, uint16_t row_cnt) {
  ...
  table->row_h =
      lv_mem_realloc(table->row_h, table->row_cnt * sizeof(table->row_h[0]));
  ...
  table->cell_data = lv_mem_realloc(
      table->cell_data, table->row_cnt * table->col_cnt * sizeof(char *));
  ...
  if (old_row_cnt < row_cnt) {
    ...
    uint32_t new_cell_cnt = table->col_cnt * table->row_cnt;
    lv_memset_00(&table->cell_data[old_cell_cnt],
                 (new_cell_cnt - old_cell_cnt) * sizeof(table->cell_data[0]));
  }
...
void lv_table_set_col_cnt(lv_obj_t *obj, uint16_t col_cnt) {
  ...
  table->col_w =
      lv_mem_realloc(table->col_w, col_cnt * sizeof(table->row_h[0]));
  ...
  char **new_cell_data =
      lv_mem_alloc(table->row_cnt * table->col_cnt * sizeof(char *));
  ...
  for (row = 0; row < table->row_cnt; row++) {
    ...
    lv_memcpy_small(&new_cell_data[new_col_start],
                    &table->cell_data[old_col_start],
...
void lv_table_set_col_width(lv_obj_t *obj, uint16_t col_id, lv_coord_t w) {
  ...
  if (col_id >= table->col_cnt)
    lv_table_set_col_cnt(obj, col_id + 1);

  table->col_w[col_id] = w;
  refr_size(obj, 0);
}

void lv_table_add_cell_ctrl(lv_obj_t *obj, uint16_t row, uint16_t col,
  ...
  if (table->cell_data[cell] == NULL) {
    table->cell_data[cell] =
        lv_mem_alloc(2); /*+1: trailing '\0; +1: format byte*/
    LV_ASSERT_MALLOC(table->cell_data[cell]);
    if (table->cell_data[cell] == NULL)
      return;
...
void lv_table_clear_cell_ctrl(lv_obj_t *obj, uint16_t row, uint16_t col,
                              lv_table_cell_ctrl_t ctrl) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
bool lv_table_has_cell_ctrl(lv_obj_t *obj, uint16_t row, uint16_t col,
                            lv_table_cell_ctrl_t ctrl) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  ...
  if (table->cell_data[cell] == NULL)
    ...
    return (table->cell_data[cell][0] & ctrl) == ctrl ? true : false;
}
...
static void lv_table_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_STYLE_CHANGED) {
    ...
    if (pr_res == LV_RES_OK &&
        (table->col_act != col || table->row_act != row)) {
      table->col_act = col;
      table->row_act = row;
      lv_obj_invalidate(obj);
    }
    ...
    if (indev_type == LV_INDEV_TYPE_POINTER ||
        indev_type == LV_INDEV_TYPE_BUTTON) {
...
static void draw_main(lv_event_t *e) {
  ...
  cell_area.y2 =
      obj->coords.y1 + bg_top - 1 - lv_obj_get_scroll_y(obj) + border_width;
  ...
  for (row = 0; row < table->row_cnt; row++) {
    ...
    if (cell_area.y1 > clip_area.y2)
      break;

    if (rtl)
      cell_area.x1 = obj->coords.x2 - bg_right - 1 - scroll_x - border_width;
    else
      cell_area.x2 = obj->coords.x1 + bg_left - 1 - scroll_x + border_width;
    ...
    for (col = 0; col < table->col_cnt; col++) {
      ...
      for (col_merge = 0; col_merge + col < table->col_cnt - 1; col_merge++) {
        if (table->cell_data[cell + col_merge]) {
          ...
          if (next_cell_data)
            ctrl = next_cell_data[0];
          if (ctrl & LV_TABLE_CELL_CTRL_MERGE_RIGHT)
            if (rtl)
              cell_area.x1 -= table->col_w[col + col_merge + 1];
            else
              cell_area.x2 += table->col_w[col + col_merge + 1];
          else {
            break;
          }
        } else {
      ...
      /*Expand the cell area with a half border to avoid drawing 2 borders next
       * to each other*/
      lv_area_t cell_area_border;
      lv_area_copy(&cell_area_border, &cell_area);
      if ((rect_dsc_def.border_side & LV_BORDER_SIDE_LEFT) &&
          cell_area_border.x1 > obj->coords.x1 + bg_left) {
        cell_area_border.x1 -= rect_dsc_def.border_width / 2;
      }
      if ((rect_dsc_def.border_side & LV_BORDER_SIDE_TOP) &&
          cell_area_border.y1 > obj->coords.y1 + bg_top) {
        cell_area_border.y1 -= rect_dsc_def.border_width / 2;
      }
      if ((rect_dsc_def.border_side & LV_BORDER_SIDE_RIGHT) &&
          cell_area_border.x2 < obj->coords.x2 - bg_right - 1) {
        cell_area_border.x2 +=
            rect_dsc_def.border_width / 2 + (rect_dsc_def.border_width & 0x1);
      }
      ...
      if (row == table->row_act && col == table->col_act) {
        if (!(obj->state & LV_STATE_SCROLLED) &&
            (obj->state & LV_STATE_PRESSED))
          cell_state |= LV_STATE_PRESSED;
        if (obj->state & LV_STATE_FOCUSED)
          cell_state |= LV_STATE_FOCUSED;
        if (obj->state & LV_STATE_FOCUS_KEY)
          cell_state |= LV_STATE_FOCUS_KEY;
        if (obj->state & LV_STATE_EDITED)
          cell_state |= LV_STATE_EDITED;
      }
      ...
      if (table->cell_data[cell]) {
        ...
        if (crop)
          ...
          txt_flags = LV_TEXT_FLAG_NONE;

        lv_txt_get_size(&txt_size, table->cell_data[cell] + 1,
                        label_dsc_def.font, label_dsc_act.letter_space,
                        label_dsc_act.line_space, lv_area_get_width(&txt_area),
                        txt_flags);
        ...
        bool label_mask_ok;
        label_mask_ok =
            _lv_area_intersect(&label_clip_area, &clip_area, &cell_area);
        if (label_mask_ok) {
          draw_ctx->clip_area = &label_clip_area;
          lv_draw_label(draw_ctx, &label_dsc_act, &txt_area,
                        table->cell_data[cell] + 1, NULL);
          draw_ctx->clip_area = &clip_area;
        }
      }
...
static void refr_size(lv_obj_t *obj, uint32_t strat_row) {
  ...
  for (i = strat_row; i < table->row_cnt; i++) {
    table->row_h[i] =
        get_row_height(obj, i, font, letter_space, line_space, cell_left,
                       cell_right, cell_top, cell_bottom);
...
static lv_coord_t get_row_height(lv_obj_t *obj, uint16_t row_id,
  ...
  for (cell = row_start, col = 0; cell < row_start + table->col_cnt;
       cell++, col++) {
    if (table->cell_data[cell] != NULL) {
