...
static void inv_arc_area(lv_obj_t *arc, uint16_t start_angle,
                         uint16_t end_angle, lv_part_t part);
static void inv_knob_area(lv_obj_t *obj);
static void get_center(lv_obj_t *obj, lv_point_t *center, lv_coord_t *arc_r);
static void get_knob_area(lv_obj_t *arc, const lv_point_t *center, lv_coord_t r,
                          lv_area_t *knob_area);
  ...
  if (code == LV_EVENT_PRESSING) {
    ...
    if (arc->dragging == false) {
      lv_coord_t indic_width =
          lv_obj_get_style_arc_width(obj, LV_PART_INDICATOR);
      r -= indic_width;
    ...
    /*No angle can be determined if exactly the middle of the arc is being
     * pressed*/
    if (p.x == 0 && p.y == 0)
      return;
     ...
     *It's mainly to avoid jumping to the opposite end if the "dead" range
     *between min. and max. is crossed. Check which end was closer on the last
     *valid press (arc->min_close) and prefer that end*/
    ...
    int32_t round = ((bg_end - arc->bg_angle_start) * 8) /
                    (arc->max_value - arc->min_value);
    round = (round + 4) >> 4;
    angle += round;
    ...
    int16_t new_value = lv_map(angle, arc->bg_angle_start, bg_end,
                               arc->min_value, arc->max_value);
    if (new_value != lv_arc_get_value(obj)) {
      arc->last_tick = lv_tick_get(); /*Cache timestamp for the next iteration*/
      lv_arc_set_value(
          obj,
          new_value); /*set_value caches the last_angle for the next iteration*/
    ...
    lv_coord_t knob_pad =
        LV_MAX4(knob_left, knob_right, knob_top, knob_bottom) + 2;
...
static void lv_arc_draw(lv_event_t *e) {
  ...
  if (arc_r > 0) {
    ...
    lv_draw_arc(draw_ctx, &arc_dsc, &center, part_draw_dsc.radius,
                arc->bg_angle_start + arc->rotation,
  ...
  if (indic_r > 0) {
    ...
    if (arc_dsc.width > part_draw_dsc.radius)
      arc_dsc.width = part_draw_dsc.radius;
    lv_draw_arc(draw_ctx, &arc_dsc, &center, part_draw_dsc.radius,
                arc->indic_angle_start + arc->rotation,
...
static void inv_arc_area(lv_obj_t *obj, uint16_t start_angle,
                         uint16_t end_angle, lv_part_t part) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  ...
  lv_area_t inv_area;
  lv_draw_arc_get_area(c.x, c.y, r, start_angle, end_angle, w, rounded,
                       &inv_area);
...
static void get_knob_area(lv_obj_t *obj, const lv_point_t *center, lv_coord_t r,
                          lv_area_t *knob_area) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  lv_arc_t *arc = (lv_arc_t *)obj;
...
static void value_update(lv_obj_t *obj) {
  ...
  case LV_ARC_MODE_SYMMETRICAL:
    ...
    if (arc->value < range_midpoint) {
      angle = lv_map(arc->value, arc->min_value, range_midpoint,
                     arc->bg_angle_start, bg_midpoint);
      lv_arc_set_start_angle(obj, angle);
      lv_arc_set_end_angle(obj, bg_midpoint);
    } else {
      angle = lv_map(arc->value, range_midpoint, arc->max_value, bg_midpoint,
                     bg_end);
      lv_arc_set_start_angle(obj, bg_midpoint);
