...
#define LV_LABEL_HINT_HEIGHT_LIMIT                                             \
  1024 /*Enable "hint" to buffer info about labels larger than this. (Speed up \
          drawing)*/
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_label_class = {.constructor_cb = lv_label_constructor,
...
void lv_label_set_long_mode(lv_obj_t *obj, lv_label_long_mode_t long_mode) {
  ...
  if (long_mode == LV_LABEL_LONG_SCROLL ||
      long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR ||
      long_mode == LV_LABEL_LONG_CLIP)
    label->expand = 1;
  else
    label->expand = 0;

  /*Restore the character under the dots*/
  if (label->long_mode == LV_LABEL_LONG_DOT &&
...
void lv_label_set_recolor(lv_obj_t *obj, bool en) {
  ...
  /*Refresh the text because the potential color codes in text needs to be
   * hidden or revealed*/
  lv_label_refr_text(obj);
}
...
void lv_label_get_letter_pos(const lv_obj_t *obj, uint32_t char_id,
                             lv_point_t *pos) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
  LV_ASSERT_NULL(pos);
  ...
  lv_text_align_t align =
      lv_obj_calculate_style_text_align(obj, LV_PART_MAIN, txt);
  ...
  while (txt[new_line_start] != '\0') {
    new_line_start += _lv_txt_get_next_line(&txt[line_start], font,
                                            letter_space, max_w, NULL, flag);
  ...
  if (base_dir == LV_BASE_DIR_AUTO)
    base_dir = _lv_bidi_detect_base_dir(txt);
  ...
  if (new_line_start == byte_id) {
    ...
    uint32_t line_char_id =
        _lv_txt_encoded_get_char_id(&txt[line_start], byte_id - line_start);

    bool is_rtl;
    uint32_t visual_char_pos = _lv_bidi_get_visual_pos(
        &txt[line_start], &mutable_bidi_txt, new_line_start - line_start,
  ...
  lv_coord_t x =
      lv_txt_get_width(bidi_txt, visual_byte_pos, font, letter_space, flag);
  if (char_id != line_start)
    x += letter_space;
...
#if LV_USE_BIDI
  if (mutable_bidi_txt)
    lv_mem_buf_release(mutable_bidi_txt);
#endif
}
...
uint32_t lv_label_get_letter_on(const lv_obj_t *obj, lv_point_t *pos_in) {
  ...
  lv_text_align_t align =
      lv_obj_calculate_style_text_align(obj, LV_PART_MAIN, label->text);
  ...
  if (new_line_start > 0 && txt[new_line_start - 1] == '\0' && txt_len > 0)
    txt_len--;
  _lv_bidi_process_paragraph(txt + line_start, bidi_txt, txt_len,
                             lv_obj_get_style_base_dir(obj, LV_PART_MAIN), NULL,
                             0);
#else
  ...
  if (new_line_start > 0) {
    while (i + line_start < new_line_start) {
      ...
      if (pos.x < x + gw || i + line_start == new_line_start ||
          txt[i_act + line_start] == '\0') {
        i = i_act;
        break;
      }
      x += gw;
  ...
  if (txt[line_start + i] == '\0') {
    ...
    logical_pos = _lv_bidi_get_logical_pos(
        &txt[line_start], NULL, txt_len,
        lv_obj_get_style_base_dir(obj, LV_PART_MAIN), cid, &is_rtl);
    if (is_rtl)
      logical_pos++;
  }
...
bool lv_label_is_char_under_pos(const lv_obj_t *obj, lv_point_t *pos) {
  ...
  int32_t max_diff =
      lv_font_get_glyph_width(font, letter, letter_next) + letter_space + 1;
...
static void lv_label_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_STYLE_CHANGED) {
    ...
    lv_txt_get_size(&size, label->text, font, letter_space, line_space, w,
                    flag);
...
static void draw_main(lv_event_t *e) {
  ...
  lv_bidi_calculate_align(&label_draw_dsc.align, &label_draw_dsc.bidi_dir,
                          label->text);

  label_draw_dsc.sel_start = lv_label_get_text_selection_start(obj);
  label_draw_dsc.sel_end = lv_label_get_text_selection_end(obj);
  if (label_draw_dsc.sel_start != LV_DRAW_LABEL_NO_TXT_SEL &&
      label_draw_dsc.sel_end != LV_DRAW_LABEL_NO_TXT_SEL) {
    label_draw_dsc.sel_color =
        lv_obj_get_style_text_color_filtered(obj, LV_PART_SELECTED);
    label_draw_dsc.sel_bg_color =
        lv_obj_get_style_bg_color(obj, LV_PART_SELECTED);
  }

  /* In SCROLL and SCROLL_CIRCULAR mode the CENTER and RIGHT are pointless, so
   * remove them. (In addition, they will create misalignment in this
   * situation)*/
  if ((label->long_mode == LV_LABEL_LONG_SCROLL ||
       label->long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR) &&
  ...
  lv_area_t txt_clip;
  bool is_common =
      _lv_area_intersect(&txt_clip, &txt_coords, draw_ctx->clip_area);
  if (!is_common)
    return;
  ...
  if (label->long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR) {
    ...
    if (size.x > lv_area_get_width(&txt_coords)) {
      label_draw_dsc.ofs_x =
          label->offset.x + size.x +
          lv_font_get_glyph_width(label_draw_dsc.font, ' ', ' ') *
    ...
    if (size.y > lv_area_get_height(&txt_coords)) {
      label_draw_dsc.ofs_x = label->offset.x;
      label_draw_dsc.ofs_y = label->offset.y + size.y +
                             lv_font_get_line_height(label_draw_dsc.font);
...
static void lv_label_refr_text(lv_obj_t *obj) {
  ...
  lv_txt_get_size(&size, label->text, font, letter_space, line_space, max_w,
                  flag);
  ...
  if (label->long_mode == LV_LABEL_LONG_SCROLL) {
    ...
    if (anim_speed == 0)
      anim_speed = LV_LABEL_DEF_SCROLL_SPEED;
    lv_anim_t a;
    lv_anim_init(&a);
    ...
    if (size.x > lv_area_get_width(&txt_coords)) {
      ...
      lv_anim_set_time(
          &a, lv_anim_speed_to_time(anim_speed, a.start_value, a.end_value));
    ...
    if (size.y > lv_area_get_height(&txt_coords) && hor_anim == false) {
      lv_anim_set_values(&a, 0,
                         lv_area_get_height(&txt_coords) - size.y -
  ...
  /*In roll inf. mode keep the size but start offset animations*/
    ...
    if (size.x > lv_area_get_width(&txt_coords)) {
      ...
      if (base_dir == LV_BASE_DIR_RTL) {
        start = -size.x - lv_font_get_glyph_width(font, ' ', ' ') *
                              LV_LABEL_WAIT_CHAR_COUNT;
        end = 0;
      } else {
        start = 0;
        end = -size.x - lv_font_get_glyph_width(font, ' ', ' ') *
                            LV_LABEL_WAIT_CHAR_COUNT;
      }
...
#else
      lv_anim_set_values(&a, 0,
                         -size.x - lv_font_get_glyph_width(font, ' ', ' ') *
                                       LV_LABEL_WAIT_CHAR_COUNT);
#endif
      lv_anim_set_exec_cb(&a, set_ofs_x_anim);
      lv_anim_set_time(
    ...
    if (size.y <=
        lv_area_get_height(
            &txt_coords)) { /*No dots are required, the text is short enough*/
      label->dot_end = LV_LABEL_DOT_END_INV;
    } else if (size.y <=
               lv_font_get_line_height(
                   font)) { /*No dots are required for one-line texts*/
      label->dot_end = LV_LABEL_DOT_END_INV;
    } else if (_lv_txt_get_encoded_length(label->text) <=
      ...
      y_overed = p.y % (lv_font_get_line_height(font) +
                        line_space); /*Round down to the last line*/
...
static void lv_label_revert_dots(lv_obj_t *obj) {
  ...
  if (label->long_mode != LV_LABEL_LONG_DOT)
    return;
  if (label->dot_end == LV_LABEL_DOT_END_INV)
    return;
