 ...
 */
void lv_canvas_set_px_color(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
                            lv_color_t c);

/**
 * DEPRECATED: added only for backward compatibility
 */
static inline void lv_canvas_set_px(lv_obj_t *canvas, lv_coord_t x,
                                    lv_coord_t y, lv_color_t c) {
 ...
 */
void lv_canvas_set_px_opa(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
                          lv_opa_t opa);
 ...
 * @param canvas pointer to a canvas object to store the result of the
 * transformation.
 ...
 * @param offset_x offset X to tell where to put the result data on destination
 * canvas
 * @param offset_y offset X to tell where to put the result data on destination
 * canvas
 ...
 * @param antialias apply anti-aliasing during the transformation. Looks better
 * but slower.
 */
 ...
 * @param src      image source. Can be a pointer an `lv_img_dsc_t` variable or
 * a path an image.
