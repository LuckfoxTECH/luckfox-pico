 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_textarea_constructor(const lv_obj_class_t *class_p,
                                    lv_obj_t *obj);
static void lv_textarea_destructor(const lv_obj_class_t *class_p,
                                   lv_obj_t *obj);
...
void lv_textarea_add_char(lv_obj_t *obj, uint32_t c) {
  ...
  if (char_is_accepted(obj, c_uni) == false) {
    LV_LOG_INFO("Character is not accepted by the text area (too long text or "
  ...
  lv_label_ins_text(ta->label, ta->cursor.pos,
                    letter_buf);    /*Insert the character*/
  lv_textarea_clear_selection(obj); /*Clear selection*/

  if (ta->pwd_mode != 0) {
    ta->pwd_tmp =
        lv_mem_realloc(ta->pwd_tmp, strlen(ta->pwd_tmp) + strlen(letter_buf) +
                                        1); /*+2: the new char + \0*/
...
void lv_textarea_add_text(lv_obj_t *obj, const char *txt) {
  ...
  /*Add the character one-by-one if not all characters are accepted or there is
   * character limit.*/
  ...
  if (ta->pwd_mode != 0) {
    ta->pwd_tmp =
        lv_mem_realloc(ta->pwd_tmp, strlen(ta->pwd_tmp) + strlen(txt) + 1);
    LV_ASSERT_MALLOC(ta->pwd_tmp);
    if (ta->pwd_tmp == NULL)
      return;
  ...
  lv_textarea_set_cursor_pos(obj, lv_textarea_get_cursor_pos(obj) +
                                      _lv_txt_get_encoded_length(txt));
...
void lv_textarea_set_placeholder_text(lv_obj_t *obj, const char *txt) {
  ...
  if (txt_len == 0) {
    ...
    if (ta->placeholder_txt == NULL) {
      LV_LOG_ERROR("lv_textarea_set_placeholder_text: couldn't allocate memory "
                   "for placeholder");
      return;
    }
...
void lv_textarea_set_cursor_pos(lv_obj_t *obj, int32_t pos) {
  ...
  /*The text area needs to have it's final size to see if the cursor is out of
   * the area or not*/
...
bool lv_textarea_text_is_selected(const lv_obj_t *obj) {
  ...
  if ((lv_label_get_text_selection_start(ta->label) !=
           LV_DRAW_LABEL_NO_TXT_SEL ||
       lv_label_get_text_selection_end(ta->label) !=
           LV_DRAW_LABEL_NO_TXT_SEL)) {
    return true;
  } else {
    return false;
...
void lv_textarea_cursor_down(lv_obj_t *obj) {
  ...
  if (pos.y < lv_obj_get_height(ta->label)) {
    ...
    lv_coord_t cur_valid_x_tmp =
        ta->cursor
            .valid_x; /*Cursor position set overwrites the valid position*/
...
void lv_textarea_cursor_up(lv_obj_t *obj) {
  ...
  uint32_t new_cur_pos = lv_label_get_letter_on(ta->label, &pos);
  lv_coord_t cur_valid_x_tmp =
      ta->cursor.valid_x; /*Cursor position set overwrites the valid position*/
  lv_textarea_set_cursor_pos(obj, new_cur_pos);
  ta->cursor.valid_x = cur_valid_x_tmp;
}
...
static void lv_textarea_constructor(const lv_obj_class_t *class_p,
  ...
  ta->max_length = 0;
  ta->cursor.show = 1;
  /*It will be set to zero later (with zero value
   * lv_textarea_set_cursor_pos(obj, 0); wouldn't do anything as there is no
   * difference)*/
  ta->cursor.pos = 1;
  ...
  if (code == LV_EVENT_FOCUSED) {
    ...
    if (c == LV_KEY_RIGHT)
      ...
      lv_textarea_add_char(obj, c);
             ...
             code == LV_EVENT_PRESS_LOST || code == LV_EVENT_RELEASED) {
...
static void pwd_char_hider(lv_obj_t *obj) {
  ...
  if (ta->pwd_mode != 0) {
    ...
    bool has_bullet;
    has_bullet =
        lv_font_get_glyph_dsc(font, &g, LV_TEXTAREA_PWD_BULLET_UNICODE, 0);
    const char *bullet;
    if (has_bullet)
...
static bool char_is_accepted(lv_obj_t *obj, uint32_t c) {
  ...
  /*Too many characters?*/
  if (ta->max_length > 0 &&
      _lv_txt_get_encoded_length(lv_textarea_get_text(obj)) >= ta->max_length) {
    return false;
  }
  ...
  if (ta->accepted_chars) {
    ...
    return true; /*If the accepted char list in not specified the accept the
                    character*/
  }
}
...
static void refr_cursor_area(lv_obj_t *obj) {
  ...
  lv_text_align_t align = lv_obj_calculate_style_text_align(
      ta->label, LV_PART_MAIN, lv_label_get_text(ta->label));
  ...
  lv_coord_t bottom =
      lv_obj_get_style_pad_bottom(obj, LV_PART_CURSOR) + border_width;
  lv_coord_t left =
      lv_obj_get_style_pad_left(obj, LV_PART_CURSOR) + border_width;
  lv_coord_t right =
      lv_obj_get_style_pad_right(obj, LV_PART_CURSOR) + border_width;
...
static void update_cursor_position_on_click(lv_event_t *e) {
  ...
  if (ta->text_sel_en) {
    if (!ta->text_sel_in_prog && !click_outside_label &&
  ...
  if (ta->text_sel_in_prog) {
    ...
    if (ta->sel_start > ta->sel_end) {
      if (label_data->sel_start != ta->sel_end ||
      ...
      if (label_data->sel_start != ta->sel_start ||
      ...
      if (label_data->sel_start != LV_DRAW_LABEL_NO_TXT_SEL ||
...
static lv_res_t insert_handler(lv_obj_t *obj, const char *txt) {
  ...
  if (ta_insert_replace) {
    if (ta_insert_replace[0] == '\0')
      return LV_RES_INV; /*Drop this text*/
...
static void draw_cursor(lv_event_t *e) {
  ...
  lv_coord_t top = lv_obj_get_style_pad_top(obj, LV_PART_CURSOR) + border_width;
  char letter_buf[8] = {0};
  lv_memcpy(letter_buf, &txt[ta->cursor.txt_byte_pos],
            _lv_txt_encoded_size(&txt[ta->cursor.txt_byte_pos]));

  cur_area.x1 += left;
  cur_area.y1 += top;

  /*Draw the letter over the cursor only if
   *the cursor has background or the letter has different color than the
   *original. Else the original letter is drawn twice which makes it look
   *bolder*/
  ...
  if (cur_dsc.bg_opa > LV_OPA_MIN ||
      cur_label_dsc.color.full != label_color.full) {
