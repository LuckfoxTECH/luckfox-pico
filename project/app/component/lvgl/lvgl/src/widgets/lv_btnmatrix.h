...
enum {
  ...
  LV_BTNMATRIX_CTRL_CLICK_TRIG =
      0x0100, /**< 1: Send LV_EVENT_VALUE_CHANGE on CLICK, 0: Send
                 LV_EVENT_VALUE_CHANGE on PRESS*/
  LV_BTNMATRIX_CTRL_POPOVER =
      0x0200, /**< Show a popover when pressing this key*/
  LV_BTNMATRIX_CTRL_RECOLOR =
      0x1000, /**< Enable text recoloring with `#color`*/
...
typedef bool (*lv_btnmatrix_btn_draw_cb_t)(lv_obj_t *btnm, uint32_t btn_id,
...
typedef struct {
  ...
  uint16_t btn_id_sel; /*Index of the active button (being pressed/released etc)
                          or LV_BTNMATRIX_BTN_NONE*/
 ...
 * excludes newline entries). An element of the map should look like e.g.:
 *                 `ctrl_map[0] = width | LV_BTNMATRIX_CTRL_NO_REPEAT |
 ...
 * @param btn_id         0 based index of the button to modify. (Not counting
 * new lines)
 */
 ...
 * lines)
 * @param ctrl      OR-ed attributs. E.g. `LV_BTNMATRIX_CTRL_NO_REPEAT |
 ...
 * @param obj       pointer to a button matrix object
 * @param ctrl      attribute(s) to set from `lv_btnmatrix_ctrl_t`. Values can
 * be ORed.
 */
 ...
 * @param width     relative width compared to the buttons in the same row.
 * [1..7]
 */
void lv_btnmatrix_set_btn_width(lv_obj_t *obj, uint16_t btn_id, uint8_t width);

/**
 * Make the button matrix like a selector widget (only one button may be checked
 * at a time). `LV_BTNMATRIX_CTRL_CHECKABLE` must be enabled on the buttons to
 * be selected using `lv_btnmatrix_set_ctrl()` or
 ...
 * released, focused etc) Useful in the `event_cb` to get the text of the
 * button, check if hidden etc.
 * @param obj       pointer to button matrix object
 * @return          index of the last released button (LV_BTNMATRIX_BTN_NONE: if
 * unset)
 */
...
/**
 * Get the whether a control value is enabled or disabled for button of a button
 * matrix
 * @param obj       pointer to a button matrix object
