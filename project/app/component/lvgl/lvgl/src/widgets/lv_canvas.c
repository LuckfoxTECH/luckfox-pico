...
static void init_fake_disp(lv_obj_t *canvas, lv_disp_t *disp,
                           lv_disp_drv_t *drv, lv_area_t *clip_area);
static void deinit_fake_disp(lv_obj_t *canvas, lv_disp_t *disp);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_canvas_class = {.constructor_cb = lv_canvas_constructor,
...
void lv_canvas_set_px_color(lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                            lv_color_t c) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_canvas_set_px_opa(lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                          lv_opa_t opa) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_canvas_copy_buf(lv_obj_t *obj, const void *to_copy, lv_coord_t x,
  ...
  if (x + w - 1 >= (lv_coord_t)canvas->dsc.header.w ||
      y + h - 1 >= (lv_coord_t)canvas->dsc.header.h) {
...
void lv_canvas_transform(lv_obj_t *obj, lv_img_dsc_t *img, int16_t angle,
                         uint16_t zoom, lv_coord_t offset_x,
  ...
  for (y = -offset_y; y < dest_height - offset_y; y++) {
    for (x = -offset_x; x < dest_width - offset_x; x++) {
      ...
      if (x + offset_x >= 0 && x + offset_x < dest_width && y + offset_y >= 0 &&
        ...
        /*If the image has no alpha channel just simple set the result color on
         * the canvas*/
        if (lv_img_cf_has_alpha(img->header.cf) == false) {
          lv_img_buf_set_px_color(&canvas->dsc, x + offset_x, y + offset_y,
                                  dsc.res.color);
        } else {
          lv_color_t bg_color = lv_img_buf_get_px_color(
              &canvas->dsc, x + offset_x, y + offset_y, dsc.cfg.color);

          /*If the canvas has no alpha but the image has mix the image's color
           * with canvas*/
          ...
          /*Both the image and canvas has alpha channel. Some extra calculation
             is required*/
          else {
            lv_opa_t bg_opa = lv_img_buf_get_px_alpha(
                &canvas->dsc, x + offset_x, y + offset_y);
            /*Pick the foreground if it's fully opaque or the Background is
             *fully transparent*/
            if (dsc.res.opa >= LV_OPA_MAX || bg_opa <= LV_OPA_MIN) {
              lv_img_buf_set_px_color(&canvas->dsc, x + offset_x, y + offset_y,
                                      dsc.res.color);
              lv_img_buf_set_px_alpha(&canvas->dsc, x + offset_x, y + offset_y,
                                      dsc.res.opa);
            }
            ...
            /*Both colors have alpha. Expensive calculation need to be applied*/
              ...
              lv_opa_t opa_res_2 =
                  255 -
                  ((uint16_t)((uint16_t)(255 - dsc.res.opa) * (255 - bg_opa)) >>
                   8);
              if (opa_res_2 == 0) {
                opa_res_2 = 1; /*never happens, just to be sure*/
              }
              lv_opa_t ratio =
                  (uint16_t)((uint16_t)dsc.res.opa * 255) / opa_res_2;
...
void lv_canvas_blur_hor(lv_obj_t *obj, const lv_area_t *area, uint16_t r) {
  ...
  lv_coord_t line_w =
      lv_img_buf_get_img_size(canvas->dsc.header.w, 1, canvas->dsc.header.cf);
...
void lv_canvas_blur_ver(lv_obj_t *obj, const lv_area_t *area, uint16_t r) {
  ...
  bool has_alpha = lv_img_cf_has_alpha(canvas->dsc.header.cf);
  lv_coord_t col_w =
      lv_img_buf_get_img_size(1, canvas->dsc.header.h, canvas->dsc.header.cf);
  ...
  for (x = a.x1; x <= a.x2; x++) {
    ...
    for (y = a.y1 - r_back; y <= a.y1 + r_front; y++) {
      y_safe = y < 0 ? 0 : y;
      y_safe =
          y_safe > canvas->dsc.header.h - 1 ? canvas->dsc.header.h - 1 : y_safe;
...
void lv_canvas_fill_bg(lv_obj_t *canvas, lv_color_t color, lv_opa_t opa) {
  ...
  if (dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
    ...
    /*+8 skip the palette*/
    lv_memset((uint8_t *)dsc->data + 8, color.full ? 0xff : 0x00,
              row_byte_cnt * dsc->header.h);
  } else if (dsc->header.cf == LV_IMG_CF_ALPHA_1BIT) {
    uint32_t row_byte_cnt = (dsc->header.w + 7) >> 3;
    lv_memset((uint8_t *)dsc->data, opa > LV_OPA_50 ? 0xff : 0x00,
              row_byte_cnt * dsc->header.h);
  } else {
    uint32_t x;
    uint32_t y;
...
void lv_canvas_draw_text(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
                         lv_coord_t max_w, lv_draw_label_dsc_t *draw_dsc,
                         const char *txt) {
  LV_ASSERT_OBJ(canvas, MY_CLASS);
...
void lv_canvas_draw_polygon(lv_obj_t *canvas, const lv_point_t points[],
  ...
  if (dsc->header.cf >= LV_IMG_CF_INDEXED_1BIT &&
      dsc->header.cf <= LV_IMG_CF_INDEXED_8BIT) {
    LV_LOG_WARN(
        "lv_canvas_draw_polygon: can't draw to LV_IMG_CF_INDEXED canvas");
    return;
  }
...
void lv_canvas_draw_arc(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
                        lv_coord_t r, int32_t start_angle, int32_t end_angle,
...
static void lv_canvas_constructor(const lv_obj_class_t *class_p,
                                  lv_obj_t *obj) {
  LV_UNUSED(class_p);
...
static void init_fake_disp(lv_obj_t *canvas, lv_disp_t *disp,
  ...
  /*Allocate the fake driver on the stack as the entire display doesn't outlive
   * this function*/
