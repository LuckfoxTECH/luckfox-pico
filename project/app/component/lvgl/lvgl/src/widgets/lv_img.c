 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_img_class = {.constructor_cb = lv_img_constructor,
...
void lv_img_set_src(lv_obj_t *obj, const void *src) {
  ...
  if (src_type == LV_IMG_SRC_VARIABLE) {
    ...
    if (img->src_type == LV_IMG_SRC_FILE ||
        img->src_type == LV_IMG_SRC_SYMBOL) {
      lv_mem_free((void *)img->src);
    }
    img->src = src;
    ...
    if (img->src != src) {
      ...
      /*If memory was allocated because of the previous `src_type` then save its
       *pointer and free after allocation. It's important to allocate first to
       *be sure the new data will be on a new address. Else `img_cache` wouldn't
       *see the change in source.*/
      if (img->src_type == LV_IMG_SRC_FILE ||
          img->src_type == LV_IMG_SRC_SYMBOL) {
        old_src = img->src;
      }
  ...
  if (src_type == LV_IMG_SRC_SYMBOL) {
    /*`lv_img_dsc_get_info` couldn't set the with and height of a font so set it
     * here*/
    ...
    lv_point_t size;
    lv_txt_get_size(&size, src, font, letter_space, line_space, LV_COORD_MAX,
                    LV_TEXT_FLAG_NONE);
    header.w = size.x;
    header.h = size.y;
  }
...
void lv_img_set_angle(lv_obj_t *obj, int16_t angle) {
  ...
  lv_area_t a;
  _lv_img_buf_get_transformed_area(&a, w, h, transf_angle + img->angle,
                                   transf_zoom, &img->pivot);
  a.x1 += obj->coords.x1;
  a.y1 += obj->coords.y1;
  a.x2 += obj->coords.x1;
  a.y2 += obj->coords.y1;
...
void lv_img_set_pivot(lv_obj_t *obj, lv_coord_t x, lv_coord_t y) {
  ...
  lv_coord_t h = lv_obj_get_height(obj);
  lv_area_t a;
  _lv_img_buf_get_transformed_area(&a, w, h, transf_angle, transf_zoom,
                                   &img->pivot);
  a.x1 += obj->coords.x1;
  a.y1 += obj->coords.y1;
  a.x2 += obj->coords.x1;
...
void lv_img_set_zoom(lv_obj_t *obj, uint16_t zoom) {
  ...
  lv_coord_t h = lv_obj_get_height(obj);
  lv_area_t a;
  _lv_img_buf_get_transformed_area(&a, w, h, transf_angle,
                                   ((int32_t)transf_zoom * img->zoom) >> 8,
                                   &img->pivot);
...
static lv_point_t lv_img_get_transformed_size(lv_obj_t *obj) {
  ...
  _lv_img_buf_get_transformed_area(&area_transform, img->w, img->h, angle_final,
                                   zoom_final, &img->pivot);

  return (lv_point_t){lv_area_get_width(&area_transform),
                      lv_area_get_height(&area_transform)};
}
...
static void lv_img_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_STYLE_CHANGED) {
    ...
    /*If the object is exactly image sized (not cropped, not mosaic) and
     *transformed perform hit test on its transformed area*/
    if (img->w == lv_obj_get_width(obj) && img->h == lv_obj_get_height(obj) &&
        (zoom != LV_IMG_ZOOM_NONE || angle != 0 || img->pivot.x != img->w / 2 ||
         img->pivot.y != img->h / 2)) {
...
static void draw_img(lv_event_t *e) {
  ...
  if (code == LV_EVENT_COVER_CHECK) {
    ...
    if (info->res == LV_COVER_RES_MASKED)
      return;
    if (img->src_type == LV_IMG_SRC_UNKNOWN ||
        img->src_type == LV_IMG_SRC_SYMBOL) {
    ...
    if (zoom_final == LV_IMG_ZOOM_NONE) {
      ...
      lv_area_t a;
      _lv_img_buf_get_transformed_area(&a, lv_obj_get_width(obj),
                                       lv_obj_get_height(obj), 0, zoom_final,
                                       &img->pivot);
      a.x1 += obj->coords.x1;
      a.y1 += obj->coords.y1;
      a.x2 += obj->coords.x1;
    ...
    lv_coord_t pbottom =
        lv_obj_get_style_pad_bottom(obj, LV_PART_MAIN) + border_width;
    ...
    if (img->obj_size_mode == LV_IMG_SIZE_MODE_REAL) {
      ...
      _lv_img_buf_get_transformed_area(&bg_coords, obj_w, obj_h, angle_final,
                                       zoom_final, &bg_pivot);

      /*Modify the coordinates to draw the background for the rotated and scaled
       * coordinates*/
    ...
    if (code == LV_EVENT_DRAW_MAIN) {
      ...
      if (img->src_type == LV_IMG_SRC_FILE ||
          img->src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if (!_lv_area_intersect(&img_clip_area, draw_ctx->clip_area,
                                &img_clip_area))
          return;
        ...
        for (; coords_tmp.y1 < img_max_area.y2;
          ...
          for (; coords_tmp.x1 < img_max_area.x2;
