...
#define LV_SLIDER_KNOB_COORD(hor, is_rtl, area)                                \
  (hor ? (is_rtl ? area.x1 : area.x2) : (is_rtl ? area.y1 : area.y2))
...
static void position_knob(lv_obj_t *obj, lv_area_t *knob_area,
                          lv_coord_t knob_size, bool hor);
static void draw_knob(lv_event_t *e);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_slider_class = {.constructor_cb = lv_slider_constructor,
...
static void lv_slider_constructor(const lv_obj_class_t *class_p,
                                  lv_obj_t *obj) {
  LV_UNUSED(class_p);
  lv_slider_t *slider = (lv_slider_t *)obj;
...
static void lv_slider_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_HIT_TEST) {
    ...
    lv_coord_t ext_click_area =
        obj->spec_attr ? obj->spec_attr->ext_click_pad : 0;
    ...
    if (type == LV_SLIDER_MODE_NORMAL || type == LV_SLIDER_MODE_SYMMETRICAL) {
      ...
      if (hor) {
        if ((base_dir != LV_BASE_DIR_RTL && p.x > slider->right_knob_area.x2) ||
            (base_dir == LV_BASE_DIR_RTL && p.x < slider->right_knob_area.x1)) {
          slider->value_to_set = &slider->bar.cur_value;
        } else if ((base_dir != LV_BASE_DIR_RTL &&
                    p.x < slider->left_knob_area.x1) ||
                   (base_dir == LV_BASE_DIR_RTL &&
                    p.x > slider->left_knob_area.x2)) {
          slider->value_to_set = &slider->bar.start_value;
        } else {
          /*Calculate the distance from each knob*/
          dist_left = LV_ABS(
              (slider->left_knob_area.x1 +
               (slider->left_knob_area.x2 - slider->left_knob_area.x1) / 2) -
              p.x);
          dist_right = LV_ABS(
              (slider->right_knob_area.x1 +
               (slider->right_knob_area.x2 - slider->right_knob_area.x1) / 2) -
              p.x);
        ...
        if (p.y < slider->right_knob_area.y1) {
          ...
          /*Calculate the distance from each knob*/
          dist_left = LV_ABS(
              (slider->left_knob_area.y1 +
               (slider->left_knob_area.y2 - slider->left_knob_area.y1) / 2) -
              p.y);
          dist_right = LV_ABS(
              (slider->right_knob_area.y1 +
               (slider->right_knob_area.y2 - slider->right_knob_area.y1) / 2) -
              p.y);
    ...
    if (w >= h) {
      ...
      if (base_dir == LV_BASE_DIR_RTL) {
        new_value = (obj->coords.x2 - bg_right) -
                    p.x; /*Make the point relative to the indicator*/
      } else {
        new_value =
            p.x - (obj->coords.x1 +
                   bg_left); /*Make the point relative to the indicator*/
      }
      ...
      new_value =
          p.y - (obj->coords.y2 +
                 bg_bottom); /*Make the point relative to the indicator*/
    ...
    lv_coord_t knob_size = LV_MIN(lv_obj_get_width(obj) + 2 * trans_w,
                                  lv_obj_get_height(obj) + 2 * trans_h) >>
                           1;
    knob_size = (knob_size * zoom) >> 8;
    knob_size +=
        LV_MAX(LV_MAX(knob_left, knob_right), LV_MAX(knob_bottom, knob_top));
    ...
    *s = LV_MAX(*s, knob_size);
    ...
    if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
      if (!slider->left_knob_focus)
        ...
        lv_slider_set_left_value(obj, lv_slider_get_left_value(obj) + 1,
                                 LV_ANIM_ON);
...
static void draw_knob(lv_event_t *e) {
  ...
  bool sym = false;
  if (slider->bar.mode == LV_BAR_MODE_SYMMETRICAL &&
      slider->bar.min_value < 0 && slider->bar.max_value > 0)
    sym = true;
  ...
  if (hor) {
    if (!sym) {
      knob_area.x1 = LV_SLIDER_KNOB_COORD(hor, base_dir == LV_BASE_DIR_RTL,
                                          slider->bar.indic_area);
    } else {
