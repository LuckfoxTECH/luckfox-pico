...
#define LV_SWITCH_IS_ANIMATING(sw)                                             \
  (((sw)->anim_state) != LV_SWITCH_ANIM_STATE_INV)

/** Switch animation start value. (Not the real value of the switch just
 * indicates process animation)*/
#define LV_SWITCH_ANIM_STATE_START 0

/** Switch animation end value.  (Not the real value of the switch just
 * indicates process animation)*/
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_switch_class = {.constructor_cb = lv_switch_constructor,
...
static void lv_switch_constructor(const lv_obj_class_t *class_p,
                                  lv_obj_t *obj) {
  LV_UNUSED(class_p);
...
static void lv_switch_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
    ...
    lv_coord_t knob_size =
        LV_MAX4(knob_left, knob_right, knob_bottom, knob_top);
...
/**
 * Starts an animation for the switch knob. if the anim_time style property is
 * greater than 0
...
static void lv_switch_trigger_anim(lv_obj_t *obj) {
  ...
  if (anim_dur_full > 0) {
    ...
    uint32_t anim_dur = (anim_dur_full * LV_ABS(anim_start - anim_end)) /
                        LV_SWITCH_ANIM_STATE_END;
