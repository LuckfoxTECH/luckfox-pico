 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_roller_class = {.constructor_cb = lv_roller_constructor,
...
const lv_obj_class_t lv_roller_label_class = {.event_cb = lv_roller_label_event,
                                              .instance_size =
                                                  sizeof(lv_label_t),
 ...
 */
void lv_roller_set_options(lv_obj_t *obj, const char *options,
  ...
  /*If the selected text has larger font the label needs some extra draw padding
   * to draw it.*/
 ...
 */
void lv_roller_set_selected(lv_obj_t *obj, uint16_t sel_opt,
                            lv_anim_enable_t anim) {
  LV_ASSERT_OBJ(obj, MY_CLASS);

  /*Set the value even if it's the same as the current value because
   *if moving to the next option with an animation which was just deleted in the
   *PRESS Call the ancestor's event handler nothing will continue the
   *animation.*/

  lv_roller_t *roller = (lv_roller_t *)obj;

  /*In infinite mode interpret the new ID relative to the currently visible
   * "page"*/
  ...
  roller->sel_opt_id =
      sel_opt < roller->option_cnt ? sel_opt : roller->option_cnt - 1;
  roller->sel_opt_id_ori = roller->sel_opt_id;
...
void lv_roller_set_visible_row_count(lv_obj_t *obj, uint8_t row_cnt) {
  ...
  lv_obj_set_height(obj,
                    (lv_font_get_line_height(font) + line_space) * row_cnt +
                        2 * border_width);
}
 ...
 */
void lv_roller_get_selected_str(const lv_obj_t *obj, char *buf,
                                uint32_t buf_size) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
static void lv_roller_constructor(const lv_obj_class_t *class_p,
                                  lv_obj_t *obj) {
  LV_UNUSED(class_p);
  lv_roller_t *roller = (lv_roller_t *)obj;
...
static void lv_roller_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_GET_SELF_SIZE) {
    ...
    if (label)
      lv_event_send(label, LV_EVENT_STYLE_CHANGED, NULL);
    ...
    if (indev_type == LV_INDEV_TYPE_ENCODER) {
      ...
      roller->sel_opt_id_ori =
          roller->sel_opt_id; /*Save the current value. Used to revert this
    ...
    if (c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
      if (roller->sel_opt_id + 1 < roller->option_cnt) {
        uint16_t ori_id =
            roller
                ->sel_opt_id_ori; /*lv_roller_set_selected will overwrite this*/
...
static void lv_roller_label_event(const lv_obj_class_t *class_p,
                                  lv_event_t *e) {
  LV_UNUSED(class_p);
  ...
  if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
    /*If the selected text has a larger font it needs some extra space to draw
     * it*/
...
static void draw_main(lv_event_t *e) {
  ...
  /*Post draw when the children are drawn*/
    ...
    if (area_ok) {
      ...
      lv_point_t res_p;
      lv_txt_get_size(&res_p, lv_label_get_text(label), label_dsc.font,
      ...
      label_y_prop =
          (label_y_prop * 16384) /
          lv_obj_get_height(label); /*Proportional position from the middle line
                                       (upscaled by << 14)*/

      /*Apply a correction with different line heights*/
      const lv_font_t *normal_label_font =
          lv_obj_get_style_text_font(obj, LV_PART_MAIN);
      lv_coord_t corr =
          (label_dsc.font->line_height - normal_label_font->line_height) / 2;
      ...
      lv_draw_label(draw_ctx, &label_dsc, &label_sel_area,
...
static void draw_label(lv_event_t *e) {
  ...
  if (!_lv_area_intersect(&roller_clip_area, draw_ctx->clip_area,
                          &roller->coords))
    return;
  ...
  if (_lv_area_intersect(&clip2, draw_ctx->clip_area, &clip2)) {
    ...
    lv_draw_label(draw_ctx, &label_draw_dsc, &label_obj->coords,
                  lv_label_get_text(label_obj), NULL);
 ...
 * roller->ddlist.selected_option_id
 * @param roller pointer to a roller object
...
static void refr_position(lv_obj_t *obj, lv_anim_enable_t anim_en) {
  ...
  lv_text_align_t align = lv_obj_calculate_style_text_align(
      label, LV_PART_MAIN, lv_label_get_text(label));

  switch (align) {
  case LV_TEXT_ALIGN_CENTER:
    lv_obj_set_x(label,
                 (lv_obj_get_content_width(obj) - lv_obj_get_width(label)) / 2);
    break;
  case LV_TEXT_ALIGN_RIGHT:
    lv_obj_set_x(label,
                 lv_obj_get_content_width(obj) - lv_obj_get_width(label));
    break;
  ...
  /*Normally the animation's `end_cb` sets correct position of the roller if
   *infinite. But without animations do it manually*/
...
static lv_res_t release_handler(lv_obj_t *obj) {
  ...
  if (lv_indev_get_type(indev) == LV_INDEV_TYPE_POINTER ||
    ...
    if (roller->moved == 0) {
      ...
      lv_coord_t label_y1 =
          label->coords.y1 + lv_indev_scroll_throw_predict(indev, LV_DIR_VER);
      int32_t id = (mid - label_y1) / label_unit;

      if (id < 0)
        id = 0;
      if (id >= roller->option_cnt)
...
static lv_coord_t get_selected_label_width(const lv_obj_t *obj) {
  ...
  lv_coord_t letter_space =
      lv_obj_get_style_text_letter_space(obj, LV_PART_SELECTED);
  const char *txt = lv_label_get_text(label);
  lv_point_t size;
  lv_txt_get_size(&size, txt, font, letter_space, 0, LV_COORD_MAX,
                  LV_TEXT_FLAG_NONE);
  return size.x;
}
...
static void set_y_anim(void *obj, int32_t v) { lv_obj_set_y(obj, v); }
