 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_btnmatrix_constructor(const lv_obj_class_t *class_p,
                                     lv_obj_t *obj);
static void lv_btnmatrix_destructor(const lv_obj_class_t *class_p,
                                    lv_obj_t *obj);
...
static void allocate_btn_areas_and_controls(const lv_obj_t *obj,
                                            const char **map);
...
void lv_btnmatrix_set_map(lv_obj_t *obj, const char *map[]) {
  ...
  for (row = 0; row < btnm->row_cnt; row++) {
    ...
    lv_coord_t row_y1 =
        ptop + (max_h_no_gap * row) / btnm->row_cnt + row * prow;
    lv_coord_t row_y2 =
        ptop + (max_h_no_gap * (row + 1)) / btnm->row_cnt + row * prow - 1;
    ...
    for (btn = 0; btn < btn_cnt; btn++, btn_tot_i++, txt_tot_i++) {
      ...
      lv_coord_t btn_x2 =
          (max_w_no_gap * (row_unit_cnt + btn_u)) / unit_cnt + btn * pcol - 1;
      ...
      lv_area_set(&btnm->button_areas[btn_tot_i], btn_x1, row_y1, btn_x2,
                  row_y2);
  ...
  /*Popovers in the top row will draw outside the widget and the extended draw
   *size depends on the row height which may have changed when setting the new
   *map*/
...
void lv_btnmatrix_set_ctrl_map(lv_obj_t *obj,
  ...
  lv_btnmatrix_t *btnm = (lv_btnmatrix_t *)obj;
  lv_memcpy(btnm->ctrl_bits, ctrl_map,
            sizeof(lv_btnmatrix_ctrl_t) * btnm->btn_cnt);
...
void lv_btnmatrix_set_selected_btn(lv_obj_t *obj, uint16_t btn_id) {
  ...
  if (btn_id >= btnm->btn_cnt && btn_id != LV_BTNMATRIX_BTN_NONE)
    return;
...
void lv_btnmatrix_set_btn_ctrl(lv_obj_t *obj, uint16_t btn_id,
...
void lv_btnmatrix_clear_btn_ctrl(lv_obj_t *obj, uint16_t btn_id,
                                 lv_btnmatrix_ctrl_t ctrl) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
bool lv_btnmatrix_has_btn_ctrl(lv_obj_t *obj, uint16_t btn_id,
                               lv_btnmatrix_ctrl_t ctrl) {
  LV_ASSERT_OBJ(obj, MY_CLASS);
...
static void lv_btnmatrix_event(const lv_obj_class_t *class_p, lv_event_t *e) {
  ...
  if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
    ...
    if (has_popovers_in_top_row(obj)) {
      /*reserve one row worth of extra space to account for popovers in the top
       * row*/
      *s = btnm->row_cnt > 0 ? lv_obj_get_content_height(obj) / btnm->row_cnt
                             : 0;
    } else {
      *s = 0;
    }
  ...
  if (code == LV_EVENT_STYLE_CHANGED) {
    ...
    if (indev_type == LV_INDEV_TYPE_POINTER ||
      ...
      if (btn_pr != LV_BTNMATRIX_BTN_NONE) {
        if (button_is_inactive(btnm->ctrl_bits[btn_pr]) == false &&
          ...
          invalidate_button_area(obj,
                                 btnm->btn_id_sel); /*Invalidate the new area*/
        }
      }
    }
    ...
    if (btn_pr != btnm->btn_id_sel) {
      ...
      lv_indev_reset_long_press(
          param); /*Start the log press time again on the new button*/
      if (btn_pr != LV_BTNMATRIX_BTN_NONE &&
    ...
    if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE) {
      ...
      if (button_is_checkable(btnm->ctrl_bits[btnm->btn_id_sel]) &&
        ...
        if (button_get_checked(btnm->ctrl_bits[btnm->btn_id_sel]) &&
            !btnm->one_check) {
    ...
    if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE) {
      if (button_is_repeat_disabled(btnm->ctrl_bits[btnm->btn_id_sel]) ==
              false &&
          button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel]) == false &&
          button_is_hidden(btnm->ctrl_bits[btnm->btn_id_sel]) == false) {
        uint32_t b = btnm->btn_id_sel;
    ...
    if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
      if (indev_type == LV_INDEV_TYPE_KEYPAD ||
          (indev_type == LV_INDEV_TYPE_ENCODER && editing)) {
        uint32_t b = 0;
        if (btnm->one_check) {
          while (button_is_hidden(btnm->ctrl_bits[b]) ||
    ...
    if (c == LV_KEY_RIGHT) {
      ...
      while (button_is_hidden(btnm->ctrl_bits[btnm->btn_id_sel]) ||
             button_is_inactive(btnm->ctrl_bits[btnm->btn_id_sel])) {
        btnm->btn_id_sel++;
        if (btnm->btn_id_sel >= btnm->btn_cnt)
          btnm->btn_id_sel = 0;
      ...
      if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
        ...
        lv_coord_t pr_center =
            btnm->button_areas[btnm->btn_id_sel].x1 +
            (lv_area_get_width(&btnm->button_areas[btnm->btn_id_sel]) >> 1);

        for (area_below = btnm->btn_id_sel; area_below < btnm->btn_cnt;
             area_below++) {
          if (btnm->button_areas[area_below].y1 >
      ...
      if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
        ...
        for (area_above = btnm->btn_id_sel; area_above >= 0; area_above--) {
          if (btnm->button_areas[area_above].y1 <
                  btnm->button_areas[btnm->btn_id_sel].y1 &&
...
static void draw_main(lv_event_t *e) {
  ...
  for (btn_i = 0; btn_i < btnm->btn_cnt; btn_i++, txt_i++) {
    ...
    if (btn_state == LV_STATE_DEFAULT) {
      lv_memcpy(&draw_rect_dsc_act, &draw_rect_dsc_def,
                sizeof(lv_draw_rect_dsc_t));
      lv_memcpy(&draw_label_dsc_act, &draw_label_dsc_def,
                sizeof(lv_draw_label_dsc_t));
    }
...
#endif
    lv_point_t txt_size;
    lv_txt_get_size(&txt_size, txt, font, letter_space, line_space,
                    lv_area_get_width(&area_obj), draw_label_dsc_act.flag);
...
static uint16_t get_button_from_point(lv_obj_t *obj, lv_point_t *p) {
  ...
  /*Get the half gap. Button look larger with this value. (+1 for rounding
   * error)*/
  ...
  for (i = 0; i < btnm->btn_cnt; i++) {
    ...
    if (btn_area.x2 >= w - pright - 2)
      btn_area.x2 +=
          obj_cords.x1 +
          LV_MIN(pright, BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
    else
      btn_area.x2 += obj_cords.x1 + pcol;

    if (btn_area.y2 >= h - pbottom - 2)
      btn_area.y2 +=
          obj_cords.y1 +
          LV_MIN(pbottom, BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
    else
...
static void invalidate_button_area(const lv_obj_t *obj, uint16_t btn_idx) {
  ...
  /*The buttons might have outline and shadow so make the invalidation larger
   *with the gaps between the buttons. It assumes that the outline or shadow is
  ...
  if ((btn_idx == btnm->btn_id_sel) &&
      (btnm->ctrl_bits[btn_idx] & LV_BTNMATRIX_CTRL_POPOVER)) {
    /*Push up the upper boundary of the btn area to also invalidate the
     * popover*/
...
static void make_one_button_checked(lv_obj_t *obj, uint16_t btn_idx) {
  ...
  bool was_toggled =
      lv_btnmatrix_has_btn_ctrl(obj, btn_idx, LV_BTNMATRIX_CTRL_CHECKED);

  lv_btnmatrix_clear_btn_ctrl_all(obj, LV_BTNMATRIX_CTRL_CHECKED);

  if (was_toggled)
    lv_btnmatrix_set_btn_ctrl(obj, btn_idx, LV_BTNMATRIX_CTRL_CHECKED);
}

/**
 * Check if any of the buttons in the first row has the
 * LV_BTNMATRIX_CTRL_POPOVER control flag set.
 * @param obj Button matrix object
