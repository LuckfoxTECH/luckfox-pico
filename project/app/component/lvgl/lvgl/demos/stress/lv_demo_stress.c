...
static void obj_test_task_cb(lv_timer_t *tmr) {
  ...
  case 4:
    ...
    lv_obj_del_async(
        obj); /*Delete on next call of `lv_task_handler` (so not now)*/
  ...
  case 7:
    ...
    lv_slider_set_value(obj, 5000,
                        LV_ANIM_ON); /*Animate to out of range value*/
  ...
  case 9:
    lv_textarea_set_text(
        ta,
        "A very very long text which will should make the text area scrollable"
        "Here area some dummy sentences to be sure the text area will be "
        "really scrollable.");
    break;
  case 10:
  ...
  case 13:
    ...
    lv_meter_add_needle_line(obj, scale, 3, lv_palette_main(LV_PALETTE_RED),
                             -10);
  ...
  case 14:
    obj = lv_msgbox_create(NULL, "Title",
                           "Some text on the message box with average length",
                           mbox_btns, true);

    lv_timer_t *msgbox_tmr =
        lv_timer_create(msgbox_del, LV_DEMO_STRESS_TIME_STEP * 5 + 30, obj);
  ...
  case 18:
    ...
    {
      lv_obj_t *b;
      b = lv_list_add_btn(obj, LV_SYMBOL_OK,
                          "1. Some very long text to scroll");
      auto_del(b, 10);
      ...
      b = lv_list_add_btn(obj, LV_SYMBOL_OK,
                          "4. Some very long text to scroll");
      auto_del(b, LV_DEMO_STRESS_TIME_STEP);
      b = lv_list_add_btn(obj, LV_SYMBOL_OK,
                          "5. Some very long text to scroll");
      auto_del(b, LV_DEMO_STRESS_TIME_STEP + 90);
      b = lv_list_add_btn(obj, LV_SYMBOL_OK,
                          "6. Some very long text to scroll");
  ...
  case 23:
    ...
    {
      lv_chart_series_t *s1 = lv_chart_add_series(
...
static void msgbox_del(lv_timer_t *tmr) { lv_msgbox_close(tmr->user_data); }

static void set_y_anim(void *obj, int32_t v) { lv_obj_set_y(obj, v); }

static void set_width_anim(void *obj, int32_t v) { lv_obj_set_width(obj, v); }
