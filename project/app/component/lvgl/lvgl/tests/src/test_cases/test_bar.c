 ...
 * - base direction: LTR
 */
void test_bar_should_update_indicator_right_coordinate_based_on_bar_value(
    void) {
 ...
 * - max value: 100
 */
void test_bar_rtl_should_update_indicator_left_coordinate_based_on_bar_value(
    void) {
  lv_bar_t *bar_ptr = (lv_bar_t *)bar;
...
void test_bar_indicator_area_should_get_smaller_when_padding_is_increased(
    void) {
  lv_bar_t *bar_ptr = (lv_bar_t *)bar;
...
void test_bar_start_value_should_be_smaller_than_current_value_in_range_mode(
    void) {
  /* Set bar in RANGE mode so we can edit the start value */
  lv_bar_set_mode(bar, LV_BAR_MODE_RANGE);
  lv_bar_set_value(bar, 50, LV_ANIM_OFF);
...
void test_bar_current_value_should_be_truncated_to_max_value_when_exceeds_it(
    void) {
 ...
 * - base direction: LTR
 */
void test_bar_indicator_should_be_drawn_towards_the_min_range_side_after_setting_a_more_negative_value(
    void) {
  lv_bar_t *bar_ptr = (lv_bar_t *)bar;
