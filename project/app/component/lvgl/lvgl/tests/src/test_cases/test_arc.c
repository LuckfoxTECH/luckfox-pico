...
void test_arc_should_update_angles_when_changing_to_symmetrical_mode_value_more_than_middle_range(
    void);
...
void setUp(void) { active_screen = lv_scr_act(); }
...
void test_arc_should_update_value_after_updating_range(void) {
  ...
  TEST_ASSERT_EQUAL_INT16(value_after_updating_max_range,
                          lv_arc_get_value(arc));

  lv_arc_set_value(arc, 10);
  lv_arc_set_range(arc, 30, 50);

  TEST_ASSERT_EQUAL_INT16(value_after_updating_min_range,
                          lv_arc_get_value(arc));
}
...
void test_arc_angles_when_reversed(void) {
  ...
  TEST_ASSERT_EQUAL_UINT16(expected_start_angle,
                           lv_arc_get_angle_start(arcBlack));
