...
/* hfp client event handler */
static void bt_av_hdl_hfp_client_evt(uint16_t event, void *p_param);
...
//hfp
static rk_hf_client_audio_state_t s_hfp_client_audio_state = RK_HF_CLIENT_AUDIO_STATE_DISCONNECTED;
static const char *s_hfp_client_conn_state_str[] = {"Disconnected", "Connecting", "Connected", "Disconnecting"};
//a2dp
static rk_a2d_audio_state_t s_a2dp_audio_state = RK_A2D_AUDIO_STATE_STOPPED;
...
/* callback for hfp client */
void bt_app_hfp_client_cb(rk_hf_client_cb_event_t event, rk_hf_client_cb_param_t *param)
{
    switch (event)
    {
    case RK_HF_CLIENT_CONNECTION_STATE_EVT:
    case RK_HF_CLIENT_AUDIO_STATE_EVT:
    case RK_HF_CLIENT_BVRA_EVT:
    case RK_HF_CLIENT_CIND_CALL_EVT:
    case RK_HF_CLIENT_CIND_CALL_SETUP_EVT:
    case RK_HF_CLIENT_CIND_CALL_HELD_EVT:
    case RK_HF_CLIENT_CCWA_EVT:
    {
        bt_app_work_dispatch(bt_av_hdl_hfp_client_evt, event, param, sizeof(rk_hf_client_cb_param_t), NULL);
        break;
    }
    default:
        RK_LOGE(BT_AV_TAG, "Invalid hfp client event: %d", event);
        break;
    }
}
...
void bt_app_hfp_client_incoming_data_cb_t(const uint8_t *buf, uint32_t len)
{
    if (++in_cnt % 100 == 0)
    {
        RK_LOGI(BT_AV_TAG, "[ %s ] packet count %u", __func__, in_cnt);
    }
...
uint32_t bt_app_hfp_client_outgoing_data_cb_t(uint8_t *buf, uint32_t len)
{
    if (++out_cnt % 100 == 0)
    {
        RK_LOGI(BT_AV_TAG, "[ %s ] packet count %u", __func__, out_cnt);
    }
    return 0;
}
...
static void bt_av_hdl_hfp_client_evt(uint16_t event, void *p_param)
{
    ...
    rk_hf_client_cb_param_t *hfp = NULL;
    switch (event)
    {
    case RK_HF_CLIENT_CONNECTION_STATE_EVT:
    {
        hfp = (rk_hf_client_cb_param_t *)(p_param);
        uint8_t *bda = hfp->conn_stat.remote_bda;
        RK_LOGI(BT_AV_TAG, "hfp client connection state: %s, [%02x:%02x:%02x:%02x:%02x:%02x]",
                s_hfp_client_conn_state_str[hfp->conn_stat.state], bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        if (hfp->conn_stat.state == RK_HF_CLIENT_CONNECTION_STATE_DISCONNECTED)
        {
            rk_bt_gap_set_scan_mode(RK_BT_CONNECTABLE, RK_BT_GENERAL_DISCOVERABLE);
        }
        else if (hfp->conn_stat.state == RK_HF_CLIENT_CONNECTION_STATE_CONNECTING)
        {
    ...
    {
        hfp = (rk_hf_client_cb_param_t *)(p_param);
        RK_LOGI(BT_AV_TAG, "HFP Client audio state: %s", s_hfp_client_conn_state_str[hfp->audio_stat.state]);
        s_hfp_client_audio_state = hfp->audio_stat.state;
        if (RK_HF_CLIENT_AUDIO_STATE_CONNECTING == hfp->audio_stat.state)
        {
            s_pkt_cnt = 0;
        }
        break;
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case RK_A2D_AUDIO_STATE_EVT:
    {
        ...
        s_a2dp_audio_state = a2d->audio_stat.state;
