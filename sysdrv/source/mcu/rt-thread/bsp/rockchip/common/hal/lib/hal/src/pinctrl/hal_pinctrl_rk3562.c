...
#if defined(HAL_PINCTRL_MODULE_ENABLED) && defined(SOC_RK3562)
...
#define SMT_2_BIT_PER_PIN                         (2)
#define SMT_8_PIN_PER_REG                         (8)
#define SMT_0(__B, __G)                           (GPIO##__B##_IOC->GPIO##__B##__G##_IE)
#define SET_SMT_0(_B, _G, _GP, _V, _W)            _PINCTRL_WRITE(SMT_0(_B, _G), RK_GEN_VAL(_GP, _V, _W))
#define RK_SET_SMT_0(B, G, BP, V)                 SET_SMT_0(B, G, BP % SMT_8_PIN_PER_REG, V, SMT_2_BIT_PER_PIN)
#define SET_SMT_2_BIT(BANK, GROUP, BANK_PIN, VAL) RK_SET_SMT_0(BANK, GROUP, BANK_PIN, VAL)

#define PINCTRL_SET_SMT_2_BIT(bank, bank_pin, val) \
{                                                  \
    if (bank_pin < 8) {                            \
        SET_SMT_2_BIT(bank, A, bank_pin, val);     \
    } else if (bank_pin < 16) {                    \
        SET_SMT_2_BIT(bank, B, bank_pin, val);     \
    } else if (bank_pin < 24) {                    \
        SET_SMT_2_BIT(bank, C, bank_pin, val);     \
    } else if (bank_pin < 32) {                    \
        SET_SMT_2_BIT(bank, D, bank_pin, val);     \
    } else {                                       \
        HAL_DBG("unknown pin %d\n", bank_pin);     \
    }                                              \
}
...
static HAL_Status PINCTRL_SetIOMUX(eGPIO_bankId bank, uint8_t pin, uint32_t val)
{
    ...
    case 0:
        if (pin < 8) {
            ...
            SET_IOMUX_4_BIT(0, B, pin, val);
        } else if (pin < 24) {
            SET_IOMUX_4_BIT(0, C, pin, val);
        } else {
            RK_SET_IOMUX_L(0, D, pin, val);
        }
        break;
    case 1:
...
static HAL_Status PINCTRL_SetPULL(eGPIO_bankId bank, uint8_t pin, uint32_t val)
{
    switch (bank) {
    case 0:
        PINCTRL_SET_PULL_2_BIT(0, pin, val);
        break;
    case 1:
...
static HAL_Status PINCTRL_SetDRV(eGPIO_bankId bank, uint8_t pin, uint32_t val)
{
    ...
    case 0:
        if (pin < 8) {
            ...
            SET_DRV_8_BIT_4(0, B, pin, val);
        } else if (pin < 24) {
            SET_DRV_8_BIT_4(0, C, pin, val);
        } else {
            SET_DRV_8_BIT_1(0, D, pin, val);
        }
        break;
    case 1:
        PINCTRL_SET_DRV_8_BIT(1, pin, val);
        break;
    case 2:
        if (pin < 8) {
            SET_DRV_8_BIT_1(2, A, pin, val);
        }
        break;
    case 3:
        PINCTRL_SET_DRV_8_BIT(3, pin, val);
        break;
    case 4:
        if (pin < 8) {
            SET_DRV_8_BIT_4(4, A, pin, val);
        } else if (pin < 16) {
            SET_DRV_8_BIT_4(4, B, pin, val);
        }
        break;
    default:
        HAL_DBG("unknown gpio%d\n", bank);
...
static HAL_Status PINCTRL_SetSR(eGPIO_bankId bank, uint8_t pin, uint32_t val)
{
    switch (bank) {
    case 0:
        PINCTRL_SET_SR_2_BIT(0, pin, val);
        break;
    case 1:
    ...
    case 2:
        ...
        break;
    case 3:
        PINCTRL_SET_SR_2_BIT(3, pin, val);
        break;
    case 4:
        if (pin < 8) {
...
static HAL_Status PINCTRL_SetSMT(eGPIO_bankId bank, uint8_t pin, uint32_t val)
{
    switch (bank) {
    case 0:
        PINCTRL_SET_SMT_2_BIT(0, pin, val);
        break;
    case 1:
        PINCTRL_SET_SMT_2_BIT(1, pin, val);
        break;
    case 2:
        if (pin < 8) {
            SET_SMT_2_BIT(2, A, pin, val);
        }
        break;
    case 3:
        PINCTRL_SET_SMT_2_BIT(3, pin, val);
        break;
    case 4:
        if (pin < 8) {
            SET_SMT_2_BIT(4, A, pin, val);
        } else if (pin < 16) {
            SET_SMT_2_BIT(4, B, pin, val);
        }
        break;
    default:
        HAL_DBG("unknown gpio%d\n", bank);
