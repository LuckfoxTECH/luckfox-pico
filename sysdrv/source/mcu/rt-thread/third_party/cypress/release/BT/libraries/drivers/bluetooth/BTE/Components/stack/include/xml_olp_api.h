 ...
 **
 **
 **  Description:   This module contains xml parser of object list object
 **
 **  Copyright (c) 2014, Broadcom Corporation, All Rights Reserved.
 **  Broadcom Bluetooth Core. Proprietary and confidential.
 ******************************************************************************/

#ifndef XML_OLP_API_H
#define XML_OLP_API_H
...
#ifndef XML_OL_CARRY_OVER_LEN
#define XML_OL_CARRY_OVER_LEN 512 /* number of bytes we can store in case we did not yet find the > */
#endif
...
typedef struct
{
    UINT16          obj_handle_len;
    UINT16          update_len;
    UINT16          cal_type_len;
    UINT16          summary_len;
    UINT16          starttime_len;
    UINT16          endtime_len;
    UINT16          ori_name_len;
    UINT16          ori_addr_len;
    UINT16          size_len;
    UINT16          priority_len;
    UINT16          pstatus_len;
    UINT16          alarmstatus_len;
    UINT16          sendstatus_len;
    UINT16          recurrent_len;
    UINT16          att_size_len;
    UINT16          att_type_len;
    UINT16          att_name_len;
    UINT16          att_id_len;
...
enum
{
    XML_OL_OK, /* parsing is ok, operation is ok */
    XML_OL_PENDING, /* parsing is ok but not enough data */
    XML_OL_END_LIST, /* found </MAP-msg-listing> */
    XML_OL_OUT_FULL, /* output buffer full  /MAP-msg-listing not reached! data is dumped */
    XML_OL_ERROR,     /* some parsing error occured */
    XML_OL_NO_RES,    /* ran out of resources (memory) */
    XML_OL_DST_NO_RES /* ran out of destination data buffer */
};
typedef UINT8    tXML_OL_RES;


typedef struct
{
    tXML_OL_ENTRY     *p_entry;   /* point to  parser_cb->mlist.evt_rpt */
...
{
    tXML_MUL_STATE    xml;
    tXML_OL_STATE     xml_user_data;
} tXML_OL_PARSER;

/* only this value is of significance, if not ok frame is dropped by parser */
#define XML_OL_ENTRY_OK    0

typedef UINT8 tXML_OL_STATUS;
...
typedef struct
{
    ...
    tXML_OL_STATUS   status;    /* Fields are valid when status is BTA_FTC_OK */
} tXML_OL_LIST; /* clone of tBTA_FTC_LIST */
...
** Returns          void
**************************************************************************************/
BT_API void XML_OlInit(tXML_OL_PARSER  *p_xml_state,
                       tXML_OL_ENTRY   *p_entry,
                       const UINT16    max_entry);


/**************************************************************************************
** Function         XML_OLParse
**
...
**************************************************************************************/
BT_API extern tXML_OL_RES XML_OlParse(tXML_OL_PARSER   *p_xml_state,
