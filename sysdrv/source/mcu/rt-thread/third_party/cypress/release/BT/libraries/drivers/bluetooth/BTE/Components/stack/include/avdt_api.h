...
typedef UINT8 tAVDT_API_RESULT;
...
typedef struct
{
    UINT16      avdt_mtu;   /* L2CAP MTU for AVDT PSM */
...
typedef struct
{
    tAVDT_API_RESULT    err_code;           /* Zero if operation succeeded; nonzero if operation failed */
...
typedef struct
{
    ...
    UINT8           num_seps_found;     /* Actual number of stream endpoints found */
} tAVDT_DISCOVER;
...
** Function         AVDT_Init
**
** Description      initialize AVDTP data structures
**                  (Only required for lite-stack version of avdt)
**
** Returns          void
**
*******************************************************************************/
AVDT_API extern void AVDT_Init(void);
#endif
...
**
*******************************************************************************/
AVDT_API extern UINT16 AVDT_Register(tAVDT_REG *p_reg, tAVDT_CTRL_CBACK *p_cback);
...
**
** Parameter        sep_type:  AVDT_TSEP_SRC or AVDT_TSEP_SNK
**                  available: True Set all SEPs to available
**                             FALSE Set all SEPs to unavailable
**
** Returns          void
**
*******************************************************************************/
AVDT_API extern UINT16 AVDT_UpdateStream(UINT8 sep_type, BOOLEAN available);
...
/*******************************************************************************
**
** Function         AVDT_SecuritySetScms
**
** Description      Set the SCMS Content Protection. This function must be called
...
**
*******************************************************************************/
AVDT_API extern UINT16 AVDT_SecuritySetScms(UINT8 handle, BOOLEAN enable,
        UINT8 scms_hdr);
...
/*******************************************************************************
**
** Function         AVDT_StartRsp
**
** Description      Send A2DP start response
**
**
** Returns          AVDT_SUCCESS if successful, otherwise error.
**
*******************************************************************************/
AVDT_API extern UINT16 AVDT_StartRsp(UINT8 handle, UINT8 label, UINT8 status);
...
/*******************************************************************************
**
** Function         AVDT_WriteMultiAv
**
** Description      Send a media packet to the connected peer devices.  The
**                  streams must be started and added to L2CAP before this
**                  function is called.  Also, this function can only be called
**                  if the stream is a SRC.
**
**                  When AVDTP has sent the media packet and is ready for the
**                  next packet, an AVDT_WRITE_CFM_EVT is sent to the
**                  application via the control callback.  The application must
**                  wait for the AVDT_WRITE_CFM_EVT before it makes the next
**                  call to AVDT_WriteMultiAv().  If the applications calls
**                  AVDT_WriteMultiAv() before it receives the event the packet
**                  will not be sent.
**
**                  The application passes the packet using the BT_HDR structure.
**                  This structure is described in section 2.1.  The offset
**                  field must be equal to or greater than (AVDT_MEDIA_HDR_SIZE
**                  + (L2CAP_MULTI_AV_L2C_HDR_LEN * number of A2DP streams)).
**                  This allows enough space in the buffer for the L2CAP and
**                  AVDTP headers.
**
**                  The memory pointed to by p_pkt must be a GKI buffer
...
**
*******************************************************************************/
AVDT_API extern void AVDT_WriteMultiAv(BT_HDR *p_data, UINT32 time_stamp, UINT8 m_pt);
