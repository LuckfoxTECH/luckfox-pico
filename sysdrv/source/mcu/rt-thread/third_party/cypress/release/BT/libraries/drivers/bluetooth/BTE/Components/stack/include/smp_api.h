...
#define SMP_FAIL                    (SMP_MAX_FAIL_RSN_PER_SPEC + 0x09) /* 0x17   unspecified failed reason */
#define SMP_CONN_TOUT               (SMP_MAX_FAIL_RSN_PER_SPEC + 0x0A) /* 0x18   unspecified failed reason */
...
{
    UINT32                      passkey;
    tBTM_IO_CAPS_REQ_BLE        io_req;     /* IO request */
    tSMP_CMPL                   cmplt;
#if BLE_INCLUDED == TRUE && SMP_INCLUDED == TRUE && SMP_LE_SC_INCLUDED == TRUE
    tSMP_SC_KEY_TYPE            peer_keypr_notif;   /* SMP_PEER_KEYPR_NOT_EVT */
#if SMP_LE_SC_OOB_INCLUDED == TRUE
...
** events occur.
*/
typedef tSMP_STATUS(tSMP_CALLBACK)(tSMP_EVT event, BD_ADDR bd_addr, tSMP_EVT_DATA *p_data);
