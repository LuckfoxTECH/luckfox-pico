...
#define AVDT_PSC                (AVDT_PSC_TRANS | AVDT_PSC_MUX | AVDT_PSC_REPORT | AVDT_PSC_DELAY_RPT | AVDT_PSC_CP)
#define AVDT_LEG_PSC            (AVDT_PSC_TRANS | AVDT_PSC_MUX | AVDT_PSC_REPORT)
#else /* AVDT_MULTIPLEXING && AVDT_REPORTING  */

#if (AVDT_MULTIPLEXING == TRUE)
#define AVDT_PSC                (AVDT_PSC_TRANS | AVDT_PSC_MUX | AVDT_PSC_DELAY_RPT | AVDT_PSC_CP)
#define AVDT_LEG_PSC            (AVDT_PSC_TRANS | AVDT_PSC_MUX)
#else /* AVDT_MULTIPLEXING */

#if (AVDT_REPORTING == TRUE)
#define AVDT_PSC                (AVDT_PSC_TRANS | AVDT_PSC_REPORT | AVDT_PSC_DELAY_RPT | AVDT_PSC_CP)
#define AVDT_LEG_PSC            (AVDT_PSC_TRANS | AVDT_PSC_REPORT)
#else /* AVDT_REPORTING  */
#define AVDT_PSC                (AVDT_PSC_TRANS | AVDT_PSC_DELAY_RPT | AVDT_PSC_CP)
...
enum
{
    ...
    // Fix Me : Add the below states from 719B1 and modify the application
    //AVDT_SCB_SND_START_RSP,
    //AVDT_SCB_SND_START_REJ,
...
enum
{
    ...
    AVDT_SCB_API_SET_SCMS_EVT,
// Fix Me : Add the below states from 719B1 and modify the application
//  AVDT_SCB_API_START_RSP_EVT,
//  AVDT_SCB_API_START_REJ_EVT,
...
#define AVDT_AD_ST_IDLE         0       /* No connection */
#define AVDT_AD_ST_WAITING      1       /* Waiting to accept a connection */
#define AVDT_AD_ST_CONNECTING   2       /* Waiting for outbound connection to complete */
#define AVDT_AD_ST_OPEN         3       /* Channel opened */
#define AVDT_AD_ST_CLOSING      4       /* Channel closing */
...
typedef struct
{
    ...
    tAVDT_L2C_INFO      signal_chan;    /* For signalling */
    tAVDT_L2C_INFO      media_chan;     /* For media      */
#if AVDT_REPORTING == TRUE
    tAVDT_L2C_INFO      reporting_chan;     /* For reporting capability      */
#endif
    UINT8   scb_hdl;                    /* stream control block associated with this connection */
} tAVDT_CCB;
...
/* Content Protection SCMS data */
typedef struct
{
    BOOLEAN enable;                 /* Is CP SCMS enabled on this channel */
    UINT8   scms_hdr;               /* CP SCMS Header value */
} tAVDT_CP_SCMS;
...
#endif
    tAVDT_CP_SCMS   cp_scms;        /* Content Protection SCMS */
} tAVDT_SCB;
...
extern tAVDT_CCB *avdt_ccb_by_cid(UINT16 lcid);
...
extern void avdt_ccb_chk_snd_msg(tAVDT_CCB *p_ccb);
...
// Fix Me : Add the below states from 719B1 and modify the application
//extern void avdt_scb_snd_start_rsp( tAVDT_SCB *p_scb, tAVDT_SCB_EVT *p_data );
//extern void avdt_scb_snd_start_rej( tAVDT_SCB *p_scb, tAVDT_SCB_EVT *p_data );
...
extern void avdt_scb_set_scms(tAVDT_SCB *p_scb, tAVDT_SCB_EVT *p_data);
extern void avdt_scb_sync_scms(tAVDT_SCB *p_scb, tAVDT_SCB_EVT *p_data);
...
extern void avdt_ad_tc_close_ind(tAVDT_CCB *p_ccb, UINT8 type);
extern void avdt_ad_tc_open_ind(tAVDT_CCB *p_ccb, UINT8 type);
extern void avdt_ad_tc_cong_ind(tAVDT_CCB *p_ccb, UINT8 type, BOOLEAN is_congested);
extern void avdt_ad_tc_data_ind(tAVDT_CCB *p_ccb, UINT8 type, BT_HDR *p_buf);
...
/* dual stack support functions */
extern BOOLEAN avdt_sync_notify(UINT8 event);
extern void avdt_sync_request(tBTM_SYNC_INFO *p_btm_sync_info, tBTM_SYNC_CPLT_CBACK p_sync_cplt_cback);
extern void avdt_sync_reset(void);
extern void avdt_sync_complete(tBTM_STATUS status);
extern void avdt_send_sync_request(UINT8 op_code, UINT8 avdt_handle[]);
#endif      /* (BTU_DUAL_STACK_MM_INCLUDED == TRUE) || (BTU_DUAL_STACK_BTC_INCLUDED == TRUE) */

#if (BTU_STACK_LITE_ENABLED == TRUE)
extern void avdt_rcv_sync_request(tAVDT_SYNC_MSG *p_sync_msg);
extern void avdt_send_sync_reply(UINT8 op_code, UINT8 status);
tAVDT_SCB *avdt_find_scb_by_lcid(UINT16 lcid);
#endif

#if (BTU_DUAL_STACK_MM_INCLUDED == TRUE) || (BTU_DUAL_STACK_BTC_INCLUDED == TRUE) || (BTU_STACK_LITE_ENABLED == TRUE)
extern void avdt_send_sync_info(tAVDT_SYNC_MSG *p_sync_msg);
...
extern tL2CAP_APPL_INFO avdt_l2c_appl;
