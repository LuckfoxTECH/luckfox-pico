...
/* wifi Service UUID */
#define UUID_WIFI_CONFIG_SERVICE    0x180A
#define UUID_WIFI_CHAR_SERVICE  0x9999

const uint8_t wifi_config_gatt_database[] =
{
    /* Declare mandatory GATT service */
    PRIMARY_SERVICE_UUID16(HANDLE_HSENS_GATT_SERVICE, UUID_SERVCLASS_GATT_SERVER),
...
#if 1
    /* Declare proprietary Hello Service with 128 byte UUID */
    PRIMARY_SERVICE_UUID16(HANDLE_WIFI_CONFIG_SERVICE, UUID_WIFI_CONFIG_SERVICE),

    /* Declare characteristic used to notify/indicate change */
    CHARACTERISTIC_UUID16_WRITABLE(HANDLE_WIFI_CONFIG_SERVICE_CHAR, HANDLE_WIFI_CONFIG_SERVICE_CHAR_VAL,
                                   UUID_WIFI_CHAR_SERVICE,
     ...
     * for bonded devices.  Setting value to 1 tells this application to send notification
     * when value of the characteristic changes.  Value 2 is to allow indications. */
    CHAR_DESCRIPTOR_UUID16_WRITABLE(HANDLE_WIFI_CONFIG_CHAR_CFG_DESC, GATT_UUID_CHAR_CLIENT_CONFIG,
                                    LEGATTDB_PERM_READABLE | LEGATTDB_PERM_WRITE_REQ),
#endif
};
...
static void hello_sensor_application_init(void)
{
    ...
    gatt_status =  wiced_bt_gatt_db_init(wifi_config_gatt_database, sizeof(wifi_config_gatt_database));
...
void hello_sensor_set_advertisement_data(void)
{
    ...
    uint8_t num_elem                            = 0;
    uint8_t hello_service_uuid[LEN_UUID_128]    = { UUID_ADV_SERVICE };
...
#include <wlan_mgnt.h>
#define UNIT_SIZE (512 * 3)
#define UNIT_SEND (134)

//static const char *MSG_BLE_WIFI_LIST_FORMAT = "{\"cmd\":\"wifilists\", \"ret\":[{\"rssi\":\"-55\",\"ssid\":\"ZainAP\"}]}";
static const char *MSG_BLE_WIFI_LIST_FORMAT = "{\"cmd\":\"wifilists\", \"ret\":%s}";
static char rk_wifi_list_buf[2048];

static int get_wifi_list(char *scan_results)
{
    struct rt_wlan_scan_result *scan_result = RT_NULL;
    scan_result = rt_wlan_scan_sync();
    if (scan_result)
    {
        int index, num, size = 0;
        char *security = NULL;
        char *scan_r;
        char *col;

        col = (char *)rt_malloc(128 * sizeof(char));
        if (col == RT_NULL)
            printf("rt_malloc fail! \n");

        size += UNIT_SIZE;

        scan_r = (char *)rt_malloc(size * sizeof(char));
        if (scan_r == RT_NULL)
            printf("rt_malloc fail! \n");
        ...
        for (index = 0; index < num; index ++)
        {
            ...
            memset(col, 0, 128);
            if (scan_result->info[index].ssid.val[0] == '\0')
                continue;
            snprintf(col, 128, "{\"rssi\":%d,\"ssid\":\"%s\"},", scan_result->info[index].rssi, &scan_result->info[index].ssid.val[0]);
            //sprintf(col, "{\"rssi\":%d,\"ssid\":\"%s\"},", scan_result->info[index].rssi, scan_result->info[index].ssid);
            strcat(scan_r, col);
        ...
        strcat(scan_r, "]");
        printf("scan_r len [%d]\n", strlen(scan_r));
        printf("[RK_RTOS_BLE] scan_results: %p\n", scan_results);
        memcpy(scan_results, scan_r, strlen(scan_r));
        rt_free(scan_r);
        rt_free(col);
...
{
    int scanr_len;
    int llen, scanr_len_use = 0;
    uint8_t *ldata;
    char *wifilist;
    int ret = -1, ret_data = 0;

    RockChipBleData *ble_data = (RockChipBleData *)data;
    printf("[RK_RTOS_BLE] ble_data.cmd: %s\n", ble_data->cmd);

    if (strncmp(ble_data->cmd, "wifilists", 9) == 0)
    {
        wifilist = (char *)malloc(2048 * sizeof(char));
        memset(wifilist, 0, 2048);
        printf("[RK_RTOS_BLE] wifilist: %p\n", wifilist);
        get_wifi_list(wifilist);
        printf("[RK_RTOS_BLE] wifilist: %s\n", wifilist);
        snprintf(rk_wifi_list_buf, sizeof(rk_wifi_list_buf), MSG_BLE_WIFI_LIST_FORMAT, wifilist);
        free(wifilist);

        printf("rk_wifi_list_buf: %s [%d]\n", rk_wifi_list_buf, strlen(rk_wifi_list_buf));
        scanr_len = strlen(rk_wifi_list_buf);

        while (scanr_len)
        {
            llen = (scanr_len > UNIT_SEND) ? UNIT_SEND : scanr_len;
            ldata = rk_wifi_list_buf + scanr_len_use;
            rt_thread_mdelay(20);
            //btc_blufi_send_notify_for_rk(ldata, llen);
            wiced_bt_gatt_send_notification(hello_sensor_state.conn_id,
                                            HANDLE_WIFI_CONFIG_SERVICE_CHAR_VAL,
                                            llen, ldata);
            scanr_len -= llen;
            scanr_len_use += llen;
        }
    ...
    {
        printf("[RK_RTOS_BLE] wifi ssid is %s\n", ble_data->ssid);
        printf("[RK_RTOS_BLE] wifi psk is %s\n", ble_data->psk);

        ret = rt_wlan_connect(ble_data->ssid, ble_data->psk);
        if (ret < 0)
            ret_data = 2;
        else
            ret_data = 1;
...
static wiced_bt_gatt_status_t hello_sensor_gatt_server_write_request_handler(uint16_t conn_id, wiced_bt_gatt_write_t *p_data)
{
    ...
    case HANDLE_HSENS_SERVICE_CHAR_BLINK_VAL:
        ...
        break;
    case HANDLE_WIFI_CONFIG_SERVICE_CHAR_VAL:
        btc_blufi_recv_handler(p_data->p_val, p_data->val_len);
        break;
    default:
...
static void ble_speed(int argc, char **argv)
{
    char data[MAX_MTU];
    uint8 i = 0, ret = 0;
    uint32 tick1 = 0, tick2 = 0, total = 0;
    tick1 = HAL_GetTick();
    uint32 send_mtu =  atoi(argv[1]);
    uint32 delay_ms =  atoi(argv[2]);
    uint32 speed_avag = 0, cnt = 6;

    rt_kprintf("send_mtu: %d delay_ms: %d\n", send_mtu, delay_ms);

    while (1)
    {
        memset(data, i, send_mtu);
        i++;
retry:
        ret = wiced_bt_gatt_send_notification(hello_sensor_state.conn_id,
                                              HANDLE_WIFI_CONFIG_SERVICE_CHAR_VAL,
                                              send_mtu, data);
        if (ret != WICED_BT_GATT_SUCCESS)
        {
            //rt_kprintf("error: send_notification 0x%2x\n", ret);
    ...
    //rt_kprintf("speed: %d KB/s tick %d index: 0x%x\n", total/(tick2 - tick1), tick2 - tick1, --i);
}
