...
/* Pending tx_complete from L2CAP (used to for PORT_EV_TXCHAR notification) */
typedef struct
{
    UINT8       lcid;           /* local_cid associated with tx_complete */
    UINT8       num_bufs;       /* Number of rfcomm data buffers associated with tx_commplete (0 for commands and partial segments) */
} tPORT_TX_PENDING;
...
typedef struct
{
    ...
    tPORT        *port;            /* Port info pool */
    tRFC_MCB     *rfc_mcb;    /* RFCOMM bd_connections pool */
#else
    tPORT        port[MAX_RFC_PORTS];            /* Port info pool */
    tRFC_MCB     rfc_mcb[MAX_BD_CONNECTIONS];    /* RFCOMM bd_connections pool */
#endif

    tPORT_TX_PENDING *p_tx_pending;  /* Table of pending tx_complete notifications from l2cap */
    UINT16      tx_pending_max;      /* Maximun number of entries in tx_pending table */
    UINT16      tx_pending_num;      /* Current buffers waiting for tx_complete from l2cap */
} tPORT_CB;
...
extern tPORT    *port_find_port_cb(UINT16 handle);
extern int      port_find_verify_port_cb(UINT16 handle, tPORT **pp_port);
