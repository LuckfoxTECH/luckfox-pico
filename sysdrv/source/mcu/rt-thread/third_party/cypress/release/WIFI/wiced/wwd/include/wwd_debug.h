...
#define WPRINT_MACRO(args) do {if (WPRINT_PLATFORM_PERMISSION_FUNC()) WWD_LOG(args)} while(0==1)
#else
...
#ifdef WPRINT_ENABLE_NETWORK_MEM
#define WPRINT_NETWORK_MEM(args) WPRINT_MACRO(args)
#else
#define WPRINT_NETWORK_MEM(args)
#endif
...
#ifdef WPRINT_ENABLE_WWD_SDIO
#define WPRINT_WWD_SDIO(args) WPRINT_MACRO(args)
#else
#define WPRINT_WWD_SDIO(args)
#endif
...
typedef struct
{
    ...
    wiced_bool_t roll_over;
    wiced_bool_t over_write;
} wwd_logging_t;

extern wwd_logging_t logbuf;
#else
#define WWD_LOG( x )
