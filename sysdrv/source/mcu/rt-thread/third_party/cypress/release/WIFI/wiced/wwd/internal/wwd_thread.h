...
typedef void (*wwd_thread_filter_tx_packet_t)(wiced_buffer_t buffer, void *userdata);
 ...
 * IMPORTANT: This functionality must be used with extreme care. The callback will be called in the
 * context of the WWD thread which normally operates with a very small stack. And delays in processing
 * will significantly impact network performance.
 *
 * @param tx_packet_callback : Callback routine to invoke when packet is sent.
 * @param userdata           : Opaque data pointer to pass back to application in callback.
 */
extern void wwd_thread_set_tx_packet_filter(wwd_thread_filter_tx_packet_t tx_packet_callback, void *userdata);
