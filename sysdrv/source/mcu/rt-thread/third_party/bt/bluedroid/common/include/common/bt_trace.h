...
// Mapping between RK_LOG_LEVEL and BT_TRACE_LEVEL
...
#define BT_PRINT_E(tag, format, ...)   {rt_kprintf("(E) " tag ": " format "\n", ##__VA_ARGS__); }
#define BT_PRINT_W(tag, format, ...)   {rt_kprintf("(W) " tag ": " format "\n", ##__VA_ARGS__); }
#define BT_PRINT_I(tag, format, ...)   {rt_kprintf("(I) " tag ": " format "\n", ##__VA_ARGS__); }
#define BT_PRINT_D(tag, format, ...)   {rt_kprintf("(D) " tag ": " format "\n", ##__VA_ARGS__); }
#define BT_PRINT_V(tag, format, ...)   {rt_kprintf("(V) " tag ": " format "\n", ##__VA_ARGS__); }

#ifndef assert
    #define assert(x)   do { if (!(x)) BT_PRINT_E(TAG, "bt host error %s %u\n", __FILE__, __LINE__); } while (0)
#endif

osi_inline void trc_dump_buffer(const char *prefix, uint8_t *data, uint16_t len)
{
    uint16_t i;
...
#ifndef CONFIG_BT_STACK_NO_LOG
    #define LOG_ERROR(format, ... )             {if (LOG_LOCAL_LEVEL >= RK_LOG_ERROR)    rt_kprintf("(E) BT_LOG: " format, ##__VA_ARGS__); }
    #define LOG_WARN(format, ... )              {if (LOG_LOCAL_LEVEL >= RK_LOG_WARN)     rt_kprintf("(W) BT_LOG: " format, ##__VA_ARGS__); }
    #define LOG_INFO(format, ... )              {if (LOG_LOCAL_LEVEL >= RK_LOG_INFO)     rt_kprintf("(I) BT_LOG: " format, ##__VA_ARGS__); }
    #define LOG_DEBUG(format, ... )             {if (LOG_LOCAL_LEVEL >= RK_LOG_DEBUG)    rt_kprintf("(D) BT_LOG: " format, ##__VA_ARGS__); }
    #define LOG_VERBOSE(format, ... )           {if (LOG_LOCAL_LEVEL >= RK_LOG_VERBOSE)  rt_kprintf("(V) BT_LOG: " format, ##__VA_ARGS__); }
