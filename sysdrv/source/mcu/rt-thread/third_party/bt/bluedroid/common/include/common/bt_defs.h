...
/* relate to RK_BT_STATUS_xxx in rk_bt_defs.h */
typedef enum
{
    BT_STATUS_SUCCESS = 0,
    BT_STATUS_FAIL,
...
osi_inline uint16_t swap_byte_16(uint16_t x)
{
    return (((x & 0x00ffU) << 8) |
            ((x & 0xff00U) >> 8));
}

osi_inline uint32_t swap_byte_32(uint32_t x)
{
...
#ifndef ntohs
osi_inline uint16_t ntohs(uint16_t x)
{
...
#ifndef htons
osi_inline uint16_t htons(uint16_t x)
{
#ifdef CPU_LITTLE_ENDIAN
    return swap_byte_16(x);
#else
...
#ifndef ntohl
osi_inline uint32_t ntohl(uint32_t x)
{
#ifdef CPU_LITTLE_ENDIAN
...
#ifndef htonl
osi_inline uint32_t htonl(uint32_t x)
{
#ifdef CPU_LITTLE_ENDIAN
    return swap_byte_32(x);
#else
