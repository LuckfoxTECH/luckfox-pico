...
#define DBG_SECTION_NAME    "PENKEY"
...
static int key_value[] =
{
    ...
    KEY_FUNC5,      // FUN_4    press:0x04 long press: 0x0B
...
static int recpen_key_device_init()
{
    rt_err_t result;
...
static int recpen_key_device_deinit()
{
    rt_free(read_buffer);
...
static void recpen_key_task(void *arg)
{
...
#ifdef RT_USING_KEYCTRL
    recpen_key_device_deinit();
#endif
}

void recpen_key_init(void)
{
    rt_thread_t tid;

    tid = rt_thread_create("rp_key",
                           recpen_key_task,
                           RT_NULL,
                           RECPEN_KEY_TASK_STACK_SIZE,
                           RECPEN_KEY_TASK_PRIORITY,
                           10);

    if (tid != RT_NULL)
    {
        LOG_D("Create recpen_key_task OK\n");
        rt_thread_startup(tid);
        return;
    }

    LOG_E("Create recpen_key_task failed\n");
    RT_ASSERT(0);
}

static void recpen_key_send_usage(char *arg)
{
    rt_kprintf("usage:\n\t%s <value>\n", arg);
    rt_kprintf("value:\n\tKEY_RECORD: 0x%08x\n", KEY_RECORD);
...
void recpen_key_send(int argc, char *argv[])
{
    struct recpen_app_msg send_msg;
...
#include <finsh.h>
MSH_CMD_EXPORT(recpen_key_send, send key to recpen thread);
#endif
