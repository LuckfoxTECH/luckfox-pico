...
/* display win layers */
#define ANIMATION_RGB565_WIN    2

#define ANI_MAX_XRES        240
#define ANI_MAX_YRES        320

/* display region define */
#define ANI_REGION_X        0
#define ANI_REGION_Y        0
#define ANI_FB_W            240         /* aligned 2 (32bit) */
#define ANI_FB_H            240         /* aligned 2 */
#define ANI_FB_PIXELS       16
...
static void app_animation_timer(void *parameter)
{
    ...
    if (++ani_data->resID >= ANI_RES_FILE_NUM)
    {
        ani_data->resID = 0;
    }
...
static rt_err_t app_animation_init(struct global_ani_data *ani_data)
{
    ...
    ani_data->fb     = (rt_uint8_t *)rt_malloc_large(ani_data->fblen);
...
static void app_animation_deinit(struct global_ani_data *ani_data)
{
    ...
    rt_free_large((void *)ani_data->fb);
}
                                      ...
                                      struct rt_display_config *wincfg)
{
    ...
    wincfg->winId = ANIMATION_RGB565_WIN;
    ...
    if (1)
    {
        ...
        snprintf(fname, sizeof(fname), "/animation_%03d.dta", (int)ani_data->resID);
