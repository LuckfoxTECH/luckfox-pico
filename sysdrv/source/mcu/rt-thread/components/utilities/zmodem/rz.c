...
static rt_err_t zget_file_info(char *name, struct zfile *zf)
{
	...
	if ((zf->fd=open(full_path, O_DIRECTORY,0)) < 0)	 
	{
	    zsend_can();
	    rt_kprintf("\b\b\bcan not open file:%s, path=%s\r\n",
	        zf->fname, full_path);
		close(zf->fd);
		zf->fd = -1;
		rt_free(full_path);
	...
	sscanf((const char *)p, "%ld%lo%lo", &zf->bytes_total,&zf->ctime,&zf->mode);
#if defined(RT_USING_DFS) && defined(DFS_USING_WORKDIR)
	...
	if ((zf->fd = open(zf->fname, O_CREAT|O_WRONLY,0)) < 0)	 /* create or replace exist file */
	{
	    zsend_can();
