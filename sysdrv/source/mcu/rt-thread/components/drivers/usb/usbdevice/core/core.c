...
static rt_err_t _data_notify(udevice_t device, struct ep_msg* ep_msg)
{
...
#if defined(RT_USB_DEVICE_UVC)
    maxpacket = RT_USB_DEVICE_UVC_BUFLEN_MAX;
#elif defined(RT_USB_DEVICE_WINUSB)
    maxpacket = RT_USB_WINUSB_TRANSLEN;
#else
    ...
    else
    {
        ...
        {
            dcd_ep_read_prepare(device->dcd, EP_ADDRESS(ep), ep->request.buffer, ep->request.remain_size > maxpacket ? maxpacket : ep->request.remain_size);
        }
    }
...
#elif defined(RT_USB_DEVICE_UVC)
    maxpacket = RT_USB_DEVICE_UVC_BUFLEN_MAX;
#elif defined(RT_USB_DEVICE_WINUSB)
    maxpacket = RT_USB_WINUSB_TRANSLEN;
#else
...
uep_t rt_usbd_find_endpoint(udevice_t device, ufunction_t* pfunc, rt_uint8_t ep_addr)
{
    ...
    for (i=device->curr_cfg->func_list.next; i!=&device->curr_cfg->func_list; i=i->next)
    {
        ...
        {
            intf = (uintf_t)rt_list_entry(j, struct uinterface, list);
            for(l=intf->setting_list.next; l!=&intf->setting_list; l=l->next)
            {
                    ualtsetting_t setting =(ualtsetting_t)rt_list_entry(l, struct ualtsetting, list);
    ...
    for (i=cfg->func_list.next; i!=&cfg->func_list; i=i->next)
    {
        ...
        for(j=func->intf_list.next; j!=&func->intf_list; j=j->next)
        {
            ...
            for(l=intf->setting_list.next; l!=&intf->setting_list; l=l->next)
            {
                ...
                for(k=setting->ep_list.next;
                        k!=&setting->ep_list; k=k->next)
                {
                    ep = (uep_t)rt_list_entry(k, struct uendpoint, list);
                ...
                rt_memcpy((void*)&cfg->cfg_desc.data[cfg->cfg_desc.wTotalLength - USB_DESC_LENGTH_CONFIG],
                        (void*)setting->desc,
                        setting->desc_size);
                cfg->cfg_desc.wTotalLength += setting->desc_size;
            }
        }
    }
...
#if defined(RT_USB_DEVICE_UAC1) || defined(RT_USB_DEVICE_UVC)
    udevice_t device;
