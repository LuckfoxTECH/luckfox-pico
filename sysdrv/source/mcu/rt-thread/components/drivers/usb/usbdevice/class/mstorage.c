...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    data->ep_in->buffer = (rt_uint8_t*)rt_dma_malloc(RT_USB_MSTORAGE_BUFLEN);
    if(data->ep_in->buffer == RT_NULL)
    {
        rt_kprintf("no memory\n");
        return -RT_ENOMEM;
    }
    data->ep_out->buffer = (rt_uint8_t*)rt_dma_malloc(RT_USB_MSTORAGE_BUFLEN);
    if(data->ep_out->buffer == RT_NULL)
    {
        rt_dma_free(data->ep_in->buffer);
        rt_kprintf("no memory\n");
        return -RT_ENOMEM;
    }    
