...
ALIGN(4) static struct ucdc_line_coding line_coding;
...
{
    ALIGN(4) struct ucdc_line_coding data;
    rt_uint16_t size;
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    data = (struct vcom*)func->user_data;
    data->ep_out->buffer = rt_dma_malloc(CDC_RX_BUFSIZE);
...
static void vcom_tx_thread_entry(void* parameter)
{
    ...
    ALIGN(CACHE_LINE_SIZE) rt_uint8_t ch[CDC_BULKIN_MAXSIZE];
