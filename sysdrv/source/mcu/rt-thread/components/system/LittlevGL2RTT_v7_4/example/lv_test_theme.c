...
void lv_test_theme(lv_theme_t *th)
{
    ...
    lv_scr_load(scr);
    lv_cont_set_style(scr, 0, th->style.bg);
...
static void create_tab1(lv_theme_t * th, lv_obj_t *parent)
{
    ...
    lv_cont_set_fit(h, true);
    lv_cont_set_layout(h, LV_LAYOUT_COL_M);

    lv_obj_t *btn = lv_btn_create(h, NULL);
    lv_btn_set_style(btn, LV_BTN_STYLE_REL, th->style.btn.rel);
    lv_btn_set_style(btn, LV_BTN_STYLE_PR, th->style.btn.pr);
    lv_btn_set_style(btn, LV_BTN_STYLE_TGL_REL, th->style.btn.tgl_rel);
    lv_btn_set_style(btn, LV_BTN_STYLE_TGL_PR, th->style.btn.tgl_pr);
    lv_btn_set_style(btn, LV_BTN_STYLE_INA, th->style.btn.ina);
    lv_btn_set_fit(btn, true);
...
//    lv_btnm_set_toggle(btnm, true, 3);
    ...
    lv_cont_set_style(sw_h, 0, &lv_style_transp);
    lv_cont_set_fit(sw_h, false);
    ...
    lv_line_set_style(line, 0, th->style.line.decor);

    lv_obj_t *ta = lv_ta_create(h, NULL);
    lv_obj_set_style(ta, th->style.ta.oneline);
    ...
    lv_btn_set_toggle(lv_list_add_btn(list, LV_SYMBOL_WIFI, "WiFi"), true);
...
static void create_tab3(lv_theme_t * th, lv_obj_t *parent)
{
    ...
    lv_page_set_scrl_fit(page, true);
