...
int host_ota_test(const char * file_path)
{
    ...
    struct custom_msg_ctrl_ota_req *ota_req = kmalloc(1048, GFP_KERNEL);
    ...
    #endif
    rwnx_tx_msg((u8 *)ota_req, sizeof(struct custom_msg_ctrl_ota_req));
    ...
    while((ret=kernel_read(filp, filp->f_pos, ota_req->ota_msg.ota_pkg_info.pkg_buf, read_size))) {
        filp->f_pos += ret;
    #endif
        ota_req->ota_msg.ota_pkg_info.flash_addr = UPGRADE_IMAGE_ADDR + read_size*pkg_index;
        if(ret == read_size) {
