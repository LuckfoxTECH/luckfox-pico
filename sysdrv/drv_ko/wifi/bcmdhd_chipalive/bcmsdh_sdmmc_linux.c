...
static int sdioh_probe(struct sdio_func *func)
{
	...
	DHD_PRINT("bus num (host idx)=%d, slot num (rca)=%d\n", host_idx, rca);
	adapter = dhd_wifi_platform_get_adapter(SDIO_BUS, host_idx, rca);
	if (adapter != NULL) {
		DHD_PRINT("found adapter info '%s'\n", adapter->name);
                              ...
                              const struct sdio_device_id *id)
{
	...
	if ((func->num == 2) || (func->num == 1 && func->device == 0x4)) {
		...
		if (!dhd_chip_alive)
			dhd_chip_alive = id->alive;
		printf("%s dhd_chip_alive=%d, id->alive=%d\n", __FUNCTION__,
			dhd_chip_alive, id->alive);
#endif
