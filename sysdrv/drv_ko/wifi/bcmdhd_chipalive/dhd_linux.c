...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		{
			DHD_PRINT("%s: net device is NOT registered yet. drop packet\n",
			__FUNCTION__);
			PKTCFREE(dhdp->osh, pktbuf, FALSE);
			continue;
		}
...
#endif /* PROP_TXSTATUS_VSDB */
			{
				DHD_PRINT("%s: net device is NOT registered. drop event packet\n",
				__FUNCTION__);
#ifdef DHD_USE_STATIC_CTRLBUF
				PKTFREE_STATIC(dhdp->osh, pktbuf, FALSE);
#else
...
dhd_stop(struct net_device *net)
{
	...
	DHD_PRINT("%s: ######### dhd_stop called for ifidx=%d #########\n", __FUNCTION__, ifidx);
...
#if (defined(WL_EXT_IAPSTA) && defined(ISAM_PREINIT)) || defined(DHD_LOAD_CHIPALIVE)
	int bytes_written = 0;
	int retry = 0;
#endif
...
fail_retry:
			...
			if (ret != 0) {
				...
				retry++;
				if (dhd_chip_alive) {
					DHD_ERROR(("Turn off dhd_chip_alive, retry=%d\n", retry));
					dhd_chip_alive = 0;
				}
				if (retry < 3)
					goto fail_retry;
...
#ifdef DHD_LOAD_CHIPALIVE
		if (dhd_chip_alive) {
			wl_android_ext_priv_cmd(net, "setsuspendmode 0", 0, &bytes_written);
		} else {
#endif
...
#define VENDOR_ETC_FIRMWARE "/oem/"
...
//	}
	...
	sprintf(firmware, "%s%s", VENDOR_ETC_FIRMWARE, "fw_bcmdhd.bin");
...
//	setbit(eventmask, WLC_E_SCAN_COMPLETE); // terence 20150628: remove redundant event
	...
	DHD_PRINT("%s: not define PROP_TXSTATUS\n", __FUNCTION__);
...
#ifdef DHD_LOAD_CHIPALIVE
void
dhd_alive_preinit(dhd_pub_t *dhd)
{
	/* We should set bus_rxglom first, so DON'T set/get any fw cmd in this function,  */

	DHD_TRACE(("Enter %s\n", __FUNCTION__));

#ifdef WL_EXT_IAPSTA
	if (op_mode == 0) {
		wl_ext_iapsta_alive_preinit(dhd->info->iflist[0]->net);
	}
#endif

	dhd_txglom_enable_host(dhd, dhd->conf->bus_rxglom);
}

void
dhd_alive_postinit(dhd_pub_t *dhd)
{
	int ret = 0;
	char buf[WLC_IOCTL_SMLEN];
	char *ptr;
	...
	/* Don't change any fw setting here, only get fw seettings to sync for host side */

	DHD_TRACE(("Enter %s\n", __FUNCTION__));

	if (op_mode == 0) {
#ifdef WL_EXT_IAPSTA
		op_mode = wl_ext_iapsta_alive_postinit(dhd->info->iflist[0]->net);
#else
		op_mode = DHD_FLAG_STA_MODE;
#endif
	}
	DHD_PRINT("%s: set op_mode %d\n", __FUNCTION__, op_mode);
...
#ifdef ARP_OFFLOAD_SUPPORT
	dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "arpoe", (char *)&arpoe, sizeof(arpoe));
	if (arpoe) {
		ret = dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "arp_version",
	...
	/* Get the default device MAC address directly from firmware */
	dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "cur_etheraddr",
		(char *)dhd->mac.octet, ETHER_ADDR_LEN);
...
#ifdef WLTDLS
	if (FW_SUPPORTED(dhd, tdls)) {
		ret= dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "tdls_enable", (char *)&tdls,
			sizeof(tdls));
		if (ret)
			dhd->tdls_enable = FALSE;
		else
			dhd->tdls_enable = TRUE;
	}
#endif /* WLTDLS */
...
#ifdef PROP_TXSTATUS
	/* 1. In AP mode, we have to issue "dhd -i wlan0 proptx 0" before host shut down
	  *    to avoid MAC_OPEN/MAC_CLOSE from fw when there is client in the "wl assoclist".
	  *    So we have to enable proptxstatus again when we back from chip alive.
	  * 2. Otherwise, call dhd_chipalive_wlfc_init to check fw have init wlfc already or not,
	  *    if initialized, issue proptx_credit_map to trigger WLC_E_FIFO_CREDIT_MAP
	  *    and WLC_E_BCMC_CREDIT_SUPPORT to sync for dhd driver.
	  */
	if (op_mode & DHD_FLAG_HOSTAP_MODE && dhd->conf->disable_proptx!=1) {
		dhd_wlfc_init(dhd);
	} else {
		ret = dhd_chipalive_wlfc_init(dhd);
		if (!ret) {
			dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "proptx_credit_map",
				(char *)buf, sizeof(buf));
		}
	}
#endif
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
	...
	DHD_PRINT("Register interface [%s]  MAC: "MACDBG"\n\n", net->name,
