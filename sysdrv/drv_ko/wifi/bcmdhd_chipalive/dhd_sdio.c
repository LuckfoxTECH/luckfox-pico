...
static int
dhdsdio_clk_kso_disable(dhd_bus_t *bus)
{
	uint8 wr_val = 0, rd_val, cmp_val, bmask;
	int err = 0;
	int try_cnt = 0;
...
static int
dhdsdio_clk_devsleep1(dhd_bus_t *bus)
{
	int err = 0;
	uint8 val;
		...
		err = dhdsdio_clk_kso_disable(bus);
		if (OOB_WAKEUP_ENAB(bus))
		{
			err = bcmsdh_gpioout(bus->sdh, GPIO_DEV_WAKEUP, FALSE);  /* GPIO_1 is off */
		}
	...
	if (err == 0)
		...
		DHD_ERROR(("%s: Sleep request failed: kso:%d err:%d\n",
			__FUNCTION__, bus->kso, err));
	}
...
static int
dhdsdio_bussleep1(dhd_bus_t *bus, bool sleep)
{
	int err = 0;
	bcmsdh_info_t *sdh = bus->sdh;
	sdpcmd_regs_t *regs = bus->regs;
	...
	DHD_INFO(("dhdsdio_bussleep1: request %s (currently %s)\n",
	         (sleep ? "SLEEP" : "WAKE"),
	          (bus->sleeping ? "SLEEP" : "WAKE")));
	...
	devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
	if (err) {
		DHD_ERROR(("%s: fail to get SBSDIO_DEVICE_CTL %d devctl %d", __func__, err, devctl));
		return err;
	}
	...
	if (sleep) {
		/* Don't sleep if something is pending */
#ifdef DHD_USE_IDLECOUNT
		if (bus->dpc_sched || bus->rxskip || pktq_n_pkts_tot(&bus->txq) || bus->readframes ||
			bus->ctrl_frame_stat)
#else
		if (bus->dpc_sched || bus->rxskip || pktq_n_pkts_tot(&bus->txq))
#endif /* DHD_USE_IDLECOUNT */
		...
		if (!SLPAUTO_ENAB(bus)) {
			...
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL,
				devctl, NULL);
			bcmsdh_set_sdmmc_sleep(sdh, TRUE);
		} else {
			/* Tell device to start using OOB wakeup */
			DHD_ERROR(("SR send mailbox\n"));
			W_SDREG(SMB_USE_OOB, &regs->tosbmailbox, retries);
			if (retries > retry_limit)
				DHD_ERROR(("CANNOT SIGNAL CHIP, WILL NOT WAKE UP!!\n"));

#if defined(SDIO_ISO_EXT)
			devctl |= SBSDIO_DEVCTL_PADS_ISO;
			DHD_ERROR(("DO SDIO bus isolation. %d\n", devctl));
			/* Isolate the bus */
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL,
				devctl, NULL);
#endif /* SDIO_ISO_EXT */
		...
		if (!SLPAUTO_ENAB(bus)) {
			if (bcmsdh_get_sdmmc_sleep(sdh)) {
				printf("%s: start to clear isolation\n", __FUNCTION__);
				bcmsdh_set_sdmmc_sleep(sdh, FALSE);
			}
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, 0, &err);

			/* Force pad isolation off if possible (in case power never toggled) */
			devctl &= ~SBSDIO_DEVCTL_PADS_ISO;
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, NULL);
...
#if defined(SDIO_ISO_EXT)
				devctl &= ~SBSDIO_DEVCTL_PADS_ISO;
				/* clear SDIO Fun1 ISOLATION */
				DHD_ERROR(("clear SDIO bus isolation. %d\n", devctl));
				/* Force pad isolation off if possible (in case power never toggled) */
				bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, &err);
				if (err)
					printf("%s: clear SDIO isolation failed\n", __FUNCTION__);
#endif /* SDIO_ISO_EXT */
			} else {
				err = dhdsdio_clk_devsleep_iovar(bus, FALSE /* wake */);
			}
...
const bcm_iovar_t dhdsdio_iovars[] = {
	...
	{"sleep",	IOV_SLEEP,	0, 0,	IOVT_UINT32,	0 },
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	if (vi->varid == IOV_SLEEP) {
		if (IOV_ISSET(actionid)) {
			...
			if (bool_val)
				BUS_WAKE(bus);
			bcmerror = dhdsdio_bussleep1(bus, bool_val);
#else
			bcmerror = dhdsdio_bussleep(bus, bool_val);
#endif /* DHD_LOAD_CHIPALIVE */
		} else {
...
#ifdef DHD_LOAD_CHIPALIVE
void
dhd_txglom_enable_host(dhd_pub_t *dhdp, bool enable)
{
	dhd_bus_t *bus = dhdp->bus;

	bus->txglom_enable = enable;
	printf("%s: enable %d\n",  __FUNCTION__, bus->txglom_enable);
	dhd_conf_set_txglom_params(bus->dhd, bus->txglom_enable);
	bcmsdh_set_mode(bus->sdh, bus->dhd->conf->txglom_mode);
}
...
#ifdef DHD_LOAD_CHIPALIVE
	if (bus->intr && dhdp->busstate == DHD_BUS_DATA && dhd_chip_alive) {
		/* Register interrupt callback, but mask it (not operational yet). */
		DHD_INTR(("%s: disable SDIO interrupts (not interested yet)\n", __FUNCTION__));
		bcmsdh_intr_disable(bus->sdh);
		if ((ret = bcmsdh_intr_reg(bus->sdh, dhdsdio_isr, bus)) != 0) {
...
#if defined(DHD_LOAD_CHIPALIVE) && defined(SDIO_ISO_EXT)
	/* clear SDIO isolation if chip is alive */
	if (dhd_chip_alive) {
		uint32 devctl = 0;
		devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
		DHD_INFO(("Read SDIO SBSDIO_DEVICE_CTL %d\n", devctl));
		if (err) {
			DHD_ERROR(("%s: fail to get SBSDIO_DEVICE_CTL %d\n", __func__, err));
		}
		/* clear SDIO Fun1 ISOLATION */
		devctl &= ~SBSDIO_DEVCTL_PADS_ISO;
		DHD_INFO(("Clear SDIO bus isolation. %d\n", devctl));
		bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, &err);
		if (err)
			printf("%s: clear SDIO isolation failed\n", __FUNCTION__);
	}
...
dhd_set_bus_params(struct dhd_bus *bus)
{
	...
	if (bus->dhd->conf->dhd_poll >= 0) {
		...
		if (!bus->pollrate)
			bus->pollrate = 1;
		DHD_PRINT("%s: set polling mode %d\n", __FUNCTION__, bus->dhd->conf->dhd_poll);
	}
...
dhd_bus_sleep(dhd_pub_t *dhdp, bool sleep, uint32 *intstatus)
{
	...
	if (bus) {
		...
		if (sleep) {
			if (SLPAUTO_ENAB(bus)) {
#ifdef DHD_LOAD_CHIPALIVE
				ret = dhdsdio_bussleep1(bus, sleep);
#else
