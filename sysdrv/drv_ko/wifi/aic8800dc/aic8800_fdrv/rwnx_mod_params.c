	...
	COMMON_PARAM(ht_on, true, true) COMMON_PARAM(vht_on, true, true) COMMON_PARAM(
		...
		true) COMMON_PARAM(mcs_map, IEEE80211_VHT_MCS_SUPPORT_0_9,
				   IEEE80211_VHT_MCS_SUPPORT_0_9) COMMON_PARAM(he_mcs_map,
									       ...
									       IEEE80211_HE_MCS_SUPPORT_0_11)
		COMMON_PARAM(he_ul_on, false, false) COMMON_PARAM(
			ldpc_on, true,
			true) COMMON_PARAM(stbc_on, true,
					   true) COMMON_PARAM(gf_rx_on, false,
							      false) COMMON_PARAM(phy_cfg,
										  2,
										  2)
			COMMON_PARAM(uapsd_timeout, 300, 300) COMMON_PARAM(
				ap_uapsd_on, true,
				true) COMMON_PARAM(sgi, true,
						   true) COMMON_PARAM(sgi80,
								      false,
								      false)
				COMMON_PARAM(use_2040, 1, 1) COMMON_PARAM(nss, 2, 2) COMMON_PARAM(
					amsdu_rx_max,
					1, 1) COMMON_PARAM(bfmee,
							   true, true) COMMON_PARAM(bfmer, false, false)
					COMMON_PARAM(mesh, true, true) COMMON_PARAM(
						murx, true,
						true) COMMON_PARAM(mutx,
								   true, true)
						COMMON_PARAM(mutx_on, true, true) COMMON_PARAM(
							use_80,
							false,
							false) COMMON_PARAM(custregd,
									    true,
									    true)
							COMMON_PARAM(custchan, false, false) COMMON_PARAM(
								roc_dur_max,
								500,
								500) COMMON_PARAM(listen_itv,
										  0,
										  0)
								COMMON_PARAM(listen_bcmc, true, true) COMMON_PARAM(
									lp_clk_ppm,
									20,
...
static void rwnx_set_he_capa(struct rwnx_hw *rwnx_hw, struct wiphy *wiphy)
{
	...
	else if (rwnx_hw->sdiodev->chipid == PRODUCT_ID_AIC8801 ||
...
int rwnx_handle_dynparams(struct rwnx_hw *rwnx_hw, struct wiphy *wiphy)
{
	...
	if (rwnx_hw->sdiodev->chipid != PRODUCT_ID_AIC8800D80 &&
		...
		AICWFDBG(LOGINFO, "%s unsupport mcs11 change to mcs9",
			 __func__);
