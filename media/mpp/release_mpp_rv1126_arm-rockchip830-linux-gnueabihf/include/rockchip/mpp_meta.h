...
#define FOURCC_META(a, b, c, d)                                                \
  ((RK_U32)(a) << 24 | ((RK_U32)(b) << 16) | ((RK_U32)(c) << 8) |              \
   ((RK_U32)(d) << 0))
 ...
 *    This metadata contains information of input / output data flow. For
 * example A. decoder input side task the input packet must be defined and
 * output frame may not be defined. Then decoder will try malloc or use
 * committed buffer to complete decoding. B. decoder output side task
 *
 *
...
typedef enum MppMetaKey_e {
  ...
  KEY_INPUT_IDR_REQ =
      FOURCC_META('i', 'i', 'd', 'r'), /* input idr frame request flag */
  KEY_OUTPUT_INTRA =
      FOURCC_META('o', 'i', 'd', 'r'), /* output intra frame indicator */
   ...
   *
   * H.265 - 16x16 block qp is reorder to 64x64/32x32 ctu order then 64x64 /
   * 32x32 ctu raster order 64x64 ctu 00 01 02 03 04 05 06 07 -> 00 01 02 03 10
   * 11 12 13 20 21 22 23 30 31 32 33 04 05 06 07 14 15 16 17 24 25 26 27 34 35
   * 36 37 10 11 12 13 14 15 16 17 20 21 22 23 24 25 26 27 30 31 32 33 34 35 36
   * 37 32x32 ctu 00 01 02 03 04 05 06 07 -> 00 01 10 11 02 03 12 13 04 05 14 15
   * 06 07 16 17 10 11 12 13 14 15 16 17    20 21 30 31 22 23 32 33 24 25 34 35
   * 26 27 36 37 20 21 22 23 24 25 26 27 30 31 32 33 34 35 36 37
   */
...
MPP_RET mpp_meta_get_with_tag(MppMeta *meta, const char *tag,
                              const char *caller);
...
MPP_RET mpp_meta_get_s32_d(MppMeta meta, MppMetaKey key, RK_S32 *val,
                           RK_S32 def);
MPP_RET mpp_meta_get_s64_d(MppMeta meta, MppMetaKey key, RK_S64 *val,
                           RK_S64 def);
MPP_RET mpp_meta_get_ptr_d(MppMeta meta, MppMetaKey key, void **val, void *def);
MPP_RET mpp_meta_get_frame_d(MppMeta meta, MppMetaKey key, MppFrame *frame,
                             MppFrame def);
MPP_RET mpp_meta_get_packet_d(MppMeta meta, MppMetaKey key, MppPacket *packet,
                              MppPacket def);
MPP_RET mpp_meta_get_buffer_d(MppMeta meta, MppMetaKey key, MppBuffer *buffer,
                              MppBuffer def);
