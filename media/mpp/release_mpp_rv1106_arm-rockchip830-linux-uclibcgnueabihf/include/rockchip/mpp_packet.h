 ...
 * pos    : ( R/W ) current access position of the whole packet memory, used for
 * buffer read/write length : ( R/W ) the rest length from current position to
 * end of buffer NOTE: normally length is updated only by set_pos, so set length
 * must be used carefully for special usage
 */
