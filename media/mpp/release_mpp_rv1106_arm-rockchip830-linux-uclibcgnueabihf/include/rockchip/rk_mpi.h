 ...
 *
 * encode_get_packet: get encoded video packet from encoder only, async
 * interface
 *
 ...
 *
 * control  : similiar to ioctl in kernel driver, setup or get mpp internal
 * parameter
 *
...
typedef struct MppApi_t {
   ...
   * @param[out] packet The output compressed data, its usage can refer
   * mpp_packet.h.
   * @return 0 and positive for success, negative for failure. The return
   *         value is an error code. For details, please refer mpp_err.h.
   */
   ...
   * @param[in] ctx The context of mpp, created by mpp_create() and initiated
   *                by mpp_init().
   * @param[in] type input port or output port which are both for data
   * transaction
   ...
   * @param[in] type input port or output port which are both for data
   * transaction
   * @param[out] task MppTask popped from mpp task queue, its usage can refer
   * mpp_task.h.
   * @return 0 and positive for success, negative for failure. The return
   *         value is an error code. For details, please refer mpp_err.h.
   */
   ...
   * @param[in] type input port or output port which are both for data
   * transaction
   * @param[in] task MppTask which is sent to mpp for process, its usage can
   * refer mpp_task.h.
   * @return 0 and positive for success, negative for failure. The return
   *         value is an error code. For details, please refer mpp_err.h.
   */
 ...
 * supported by MPP on standard output.
 */
