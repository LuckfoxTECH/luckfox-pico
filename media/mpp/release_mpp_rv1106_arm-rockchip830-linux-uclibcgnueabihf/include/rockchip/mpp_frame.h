...
typedef enum {
  ...
  MPP_FRAME_PRI_BT470M =
      4, ///< also FCC Title 47 Code of Federal Regulations 73.682 (a)(20)

  MPP_FRAME_PRI_BT470BG = 5, ///< also ITU-R BT601-6 625 / ITU-R BT1358 625 /
                             ///< ITU-R BT1700 625 PAL & SECAM
  MPP_FRAME_PRI_SMPTE170M =
...
typedef enum {
  ...
  MPP_FRAME_TRC_GAMMA22 =
      4, ///< also ITU-R BT470M / ITU-R BT1700 625 PAL & SECAM
  MPP_FRAME_TRC_GAMMA28 = 5,   ///< also ITU-R BT470BG
  MPP_FRAME_TRC_SMPTE170M = 6, ///< also ITU-R BT601-6 525 or 625 / ITU-R BT1358
...
typedef enum {
  ...
  MPP_FRAME_SPC_CHROMA_DERIVED_NCL =
      12, ///< Chromaticity-derived non-constant luminance system
  MPP_FRAME_SPC_CHROMA_DERIVED_CL =
 ...
 * Payload offset = MPP_ALIGN(MPP_ALIGN(width, 16) * MPP_ALIGN(height, 16) / 16,
 * SZ_4K)
 */
...
#define MPP_FRAME_FMT_IS_YUV(fmt)                                              \
  (((fmt & MPP_FRAME_FMT_COLOR_MASK) == MPP_FRAME_FMT_YUV) &&                  \
   ((fmt & MPP_FRAME_FMT_MASK) < MPP_FMT_YUV_BUTT))
#define MPP_FRAME_FMT_IS_RGB(fmt)                                              \
  (((fmt & MPP_FRAME_FMT_COLOR_MASK) == MPP_FRAME_FMT_RGB) &&                  \
...
#define MPP_FRAME_FMT_IS_LE(fmt)                                               \
  ((fmt & MPP_FRAME_FMT_LE_MASK) == MPP_FRAME_FMT_LE_MASK)
...
void mpp_frame_set_color_primaries(MppFrame frame,
                                   MppFrameColorPrimaries color_primaries);
MppFrameColorTransferCharacteristic
mpp_frame_get_color_trc(const MppFrame frame);
void mpp_frame_set_color_trc(MppFrame frame,
...
void mpp_frame_set_chroma_location(MppFrame frame,
...
MppFrameMasteringDisplayMetadata
mpp_frame_get_mastering_display(const MppFrame frame);
void mpp_frame_set_mastering_display(
    MppFrame frame, MppFrameMasteringDisplayMetadata mastering_display);
MppFrameContentLightMetadata mpp_frame_get_content_light(const MppFrame frame);
void mpp_frame_set_content_light(MppFrame frame,
