...
#define MPP_ENC_QUERY_ALL                                                      \
  (MPP_ENC_QUERY_STATUS | MPP_ENC_QUERY_WAIT | MPP_ENC_QUERY_FPS |             \
   MPP_ENC_QUERY_BPS | MPP_ENC_QUERY_ENC_IN_FRM | MPP_ENC_QUERY_ENC_WORK |     \
...
typedef enum MppEncRcCfgChange_e {
  ...
  MPP_ENC_RC_CFG_CHANGE_FPS_IN =
      (1 << 5), /* change on fps in  flex / numerator / denorminator */
  MPP_ENC_RC_CFG_CHANGE_FPS_OUT =
      (1 << 6), /* change on fps out flex / numerator / denorminator */
...
typedef struct MppEncHwCfg_t {
   ...
   * If the value is smaller then encoder will be more likely to encode
   * corresponding block mode.
   */
  RK_S32 mode_bias[8];
   ...
   *                If the value is smaller then encoder will be more likely to
   * encode skip block.
   */
  RK_S32 skip_bias_en;
  RK_S32 skip_sad;
...
typedef enum MppEncPrepCfgChange_e {
  ...
  MPP_ENC_PREP_CFG_CHANGE_COLOR_PRIME =
      (1 << 18),                                 /* change on color primaries */
...
typedef enum MppEncSeiMode_e {
  ...
  MPP_ENC_SEI_MODE_ONE_FRAME /* one frame may have one SEI, if SEI info has
                                changed */
} MppEncSeiMode;
...
typedef struct MppEncH264Cfg_t {
   ...
   *
   * do NOT setup the three option below unless you are familiar with encoder
   * detail poc_type             - picture order count type 0 ~ 2
   ...
   * Mpp encoder constraint_set uses type RK_U32 to store force_flag and
   * constraint_force as followed. | 00 | force_flag | 00 | constraint_force |
   * As for force_flag and constraint_force, only low 6 bits are valid,
   * corresponding to constraint_setx_flag from 5 to 0.
   * If force_flag bit is enabled, constraint_setx_flag will be set
   * correspondingly. Otherwise, constraint_setx_flag will use default value.
   */
 ...
 * @note  x, y, w, h are calculated in pixels, which had better be 16-pixel
 * aligned. These parameters MUST retain in memory when encoder is running. Both
 * absolute qp and relative qp are supported in vepu541. Only absolute qp is
 * supported in rv1108
 */
...
typedef enum MppEncOSDPltCfgChange_e {
  ...
  MPP_ENC_OSD_PLT_CFG_CHANGE_PLT_VAL =
      (1 << 1), /* change osd plt table value */
