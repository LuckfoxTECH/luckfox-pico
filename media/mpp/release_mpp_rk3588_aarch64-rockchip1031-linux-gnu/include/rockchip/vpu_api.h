...
typedef enum OMX_RK_VIDEO_CODINGTYPE {
  ...
  OMX_RK_VIDEO_CodingVC1 =
      0x01000000,           /**< Windows Media Video (WMV1,WMV2,WMV3)*/
  ...
  OMX_RK_VIDEO_CodingKhronosExtensions =
      0x6F000000, /**< Reserved region for introducing Khronos Standard
                     Extensions */
  OMX_RK_VIDEO_CodingVendorStartUnused =
...
typedef struct VpuCodecContext {
   ...
   */
  RK_S32(*init)
  (struct VpuCodecContext *ctx, RK_U8 *extraData, RK_U32 extra_size);
  /**
   ...
   */
  RK_S32(*decode)
  (struct VpuCodecContext *ctx, VideoPacket_t *pkt, DecoderOut_t *aDecOut);
  /**
   ...
   */
  RK_S32(*encode)
  (struct VpuCodecContext *ctx, EncInputStream_t *aEncInStrm,
  ...
  RK_S32(*control)
  (struct VpuCodecContext *ctx, VPU_API_CMD cmdType, void *param);
  /**
   ...
   * @return 0 for success, others for failure.
   */
  RK_S32(*encoder_getstream)
  (struct VpuCodecContext *ctx, EncoderOut_t *aEncOut);
} VpuCodecContext_t;
...
int create_vpu_memory_pool_allocator(vpu_display_mem_pool **ipool, int num,
                                     int size);
