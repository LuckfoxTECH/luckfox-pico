...
static uint8_t iv[12] = {
    0x10, 0x44, 0x80, 0xb3, 0x88, 0x5f, 0x02, 0x03, 0x05, 0x21, 0x07, 0xc9,
};
static uint8_t aad_in[32] = {
    0x10, 0x44, 0x80, 0xb3, 0x88, 0x5f, 0x02, 0x03, 0xac, 0x13, 0xfb,
    0x23, 0x93, 0x4a, 0x66, 0xe4, 0x05, 0x21, 0x07, 0xc9, 0x44, 0x00,
    0x1b, 0x80, 0xeb, 0xe7, 0xde, 0x12, 0x8d, 0x77, 0xf4, 0xe8,
};
static uint8_t input[32] = {
    0xc9, 0x07, 0x21, 0x05, 0x80, 0x1b, 0x00, 0x44, 0xac, 0x13, 0xfb,
    0x23, 0x93, 0x4a, 0x66, 0xe4, 0xc9, 0x07, 0x21, 0x05, 0x80, 0x1b,
    0x00, 0x44, 0xac, 0x13, 0xfb, 0x23, 0x93, 0x4a, 0x66, 0xe4,
};
static uint8_t expected_enc[32] = {
    0x92, 0x8e, 0xc6, 0x87, 0x7f, 0xca, 0xc7, 0x96, 0xbf, 0xa7, 0x86,
    0xb4, 0xfe, 0xda, 0xf3, 0xec, 0xdc, 0x24, 0x4d, 0x45, 0x78, 0x3a,
    0xe3, 0x3b, 0x55, 0xbf, 0x9b, 0x9f, 0xbd, 0x4c, 0xf7, 0xe7,
};
...
static RK_RES set_ae_config(rk_ae_config *config, uint32_t algo, uint32_t mode,
                            uint32_t operation, uint8_t *key, uint32_t key_len,
                            uint8_t *iv, uint32_t iv_len, uint32_t tag_len,
                            uint32_t aad_len) {
  if (!config || !key || !iv)
...
RK_RES demo_ae(void) {
  ...
  res =
      set_ae_config(&config, algo, mode, RK_OP_CIPHER_ENC, key_0, sizeof(key_0),
                    iv, sizeof(iv), sizeof(expected_tag), aad_len);
  if (res) {
...
RK_RES demo_ae_virt(void) {
  ...
  res =
      set_ae_config(&config, algo, mode, RK_OP_CIPHER_ENC, key_0, sizeof(key_0),
                    iv, sizeof(iv), sizeof(expected_tag), sizeof(aad_in));
  if (res) {
    printf("set_ae_config error! res: 0x%08x\n", res);
    goto exit;
  }
