...
static RK_RES test_cipher_item_virt(const struct test_cipher_item *item,
  ...
  if (posix_memalign((void *)&cipher_soft, page_size, data_len) ||
      !cipher_soft) {
    E_TRACE("cipher_soft malloc %uByte error!\n", data_len);
    goto exit;
  }

  if (posix_memalign((void *)&cipher_hard, page_size, data_len) ||
      !cipher_hard) {
    ...
    for (j = 0; j < ARRAY_SIZE(item->key_lens); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(ops); k++) {
        ...
        while (loop_nbytes) {
          ...
          tmp_len =
              loop_nbytes > MULTI_BLOCKSIZE ? MULTI_BLOCKSIZE : loop_nbytes;

          res = rk_cipher_crypt_virt(cipher_hdl, tmp_data_in, tmp_data_out,
                                     tmp_len);
          if (res) {
        ...
        res = soft_cipher(algo, mode, operation, cipher_cfg.key,
                          cipher_cfg.key_len, iv_tmp, plain, data_len,
                          cipher_soft);
        if (res) {
  ...
  for (i = 0; i < ARRAY_SIZE(item->modes); i++) {
    ...
    for (j = 0; j < ARRAY_SIZE(item->key_lens); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(ops); k++) {
        ...
        res = rk_cipher_crypt(cipher_hdl, plain->dma_fd, cipher_hard->dma_fd,
                              data_len);
        if (res) {
          rk_cipher_final(cipher_hdl);
        ...
        res = soft_cipher(algo, mode, operation, cipher_cfg.key,
                          cipher_cfg.key_len, iv_tmp, plain->vaddr, data_len,
                          cipher_soft->vaddr);
        if (res) {
          E_TRACE("soft_cipher error[%x]\n", res);
          goto exit;
