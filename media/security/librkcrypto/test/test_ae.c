      ...
      for (k = 0; k < ARRAY_SIZE(item->key_lens); k++) {
        ...
        for (l = 0; l < ARRAY_SIZE(item->data_lens); l++) {
          ...
          for (n = 0; n < ARRAY_SIZE(item->aad_lens); n++) {
            ...
            res = soft_ae(algo, mode, operation, ae_cfg.key, ae_cfg.key_len,
                          iv_tmp, iv_len, aad, ae_cfg.aad_len, ae_cfg.tag_len,
            ...
            res =
                rk_ae_crypt_virt(handle, ae_hard, ae_hard, data_len, tag_hard);
            if (res) {
              rk_ae_final(handle);
              printf("rk_ae_crypt_virt error[%x]\n", res);
              goto exit;
      ...
      for (k = 0; k < ARRAY_SIZE(item->key_lens); k++) {
        ...
        for (l = 0; l < ARRAY_SIZE(item->data_lens); l++) {
          ...
          for (n = 0; n < ARRAY_SIZE(item->aad_lens); n++) {
            ...
            res = rk_ae_crypt(handle, plain->dma_fd, ae_hard->dma_fd, data_len,
                              tag_hard);
            if (res) {
              rk_ae_final(handle);
            ...
            res = soft_ae(algo, mode, operation, ae_cfg.key, ae_cfg.key_len,
                          iv_tmp, iv_len, aad->vaddr, aad_len, ae_cfg.tag_len,
            ...
            res = rk_ae_crypt(handle, ae_hard->dma_fd, ae_hard->dma_fd,
                              data_len, tag_hard);
            if (res) {
              rk_ae_final(handle);
              printf("rk_ae_crypt error[%x]\n", res);
