...
static RK_RES test_hash_item_virt(const struct test_hash_item *item,
  ...
  if (is_hmac)
    res =
        soft_hmac(algo, key, key_len, buffer, buffer_len, hash_soft, &out_len);
  else
...
static RK_RES test_hash_item_fd(const struct test_hash_item *item,
  ...
  if (is_hmac)
    res = soft_hmac(algo, key, key_len, buffer->vaddr, buffer->size, hash_soft,
                    &out_len);
  else
