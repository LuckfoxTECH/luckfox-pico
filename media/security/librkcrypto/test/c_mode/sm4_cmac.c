...
static void rk_array_xor(int len, unsigned char *a1, unsigned char *a2,
                         unsigned char *des) {
  int i;
  for (i = 0; i < len; i++) {
    des[i] = a1[i] ^ a2[i];
...
static void rk_derive_mac_key(sm4_context *key, unsigned char *k1,
                              unsigned char *k2) {
  unsigned char plain[SM4_BLOCK_SIZE] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00};
  unsigned char Rb[SM4_BLOCK_SIZE] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x87};
...
int rk_sm4_genarate_cmac(unsigned char *key, unsigned int key_len,
  ...
  if (msg_len % SM4_BLOCK_SIZE == 0 && msg_len != 0) {
    ...
    if (msg_len == 0) {
      ...
      for (i = 0; i < block - 1; i++) {
        rk_array_xor(SM4_BLOCK_SIZE, &msg[i * SM4_BLOCK_SIZE], IVtemp,
                     Blocktemp);
        rk_sm4_crypt_ecb(&ctx, Blocktemp, IVtemp);
      }
...
int rk_sm4_verify_cmac(unsigned char *key, unsigned int key_len,
                       unsigned char *msg, unsigned int msg_len,
                       unsigned char *macvalue) {
  int i, block;
  int result = -1;
  ...
  if (msg_len % SM4_BLOCK_SIZE == 0 && msg_len != 0) {
    ...
    rk_array_xor(SM4_BLOCK_SIZE, &msg[(block - 1) * SM4_BLOCK_SIZE], IVtemp,
                 Blocktemp);
