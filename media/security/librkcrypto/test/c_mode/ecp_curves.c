...
#define BYTES_TO_T_UINT_4(a, b, c, d) BYTES_TO_T_UINT_8(a, b, c, d, 0, 0, 0, 0)

#define BYTES_TO_T_UINT_2(a, b) BYTES_TO_T_UINT_8(a, b, 0, 0, 0, 0, 0, 0)
 ...
 */
static int ecp_group_load(mbedtls_ecp_group *grp, const mbedtls_mpi_uint *p,
                          size_t plen, const mbedtls_mpi_uint *a, size_t alen,
...
#define LOAD_GROUP_A(G)                                                        \
  ecp_group_load(grp, G##_p, sizeof(G##_p), G##_a, sizeof(G##_a), G##_b,       \
                 sizeof(G##_b), G##_gx, sizeof(G##_gx), G##_gy,                \
                 sizeof(G##_gy), G##_n, sizeof(G##_n))

#define LOAD_GROUP(G)                                                          \
  ecp_group_load(grp, G##_p, sizeof(G##_p), NULL, 0, G##_b, sizeof(G##_b),     \
                 G##_gx, sizeof(G##_gx), G##_gy, sizeof(G##_gy), G##_n,        \
                 sizeof(G##_n))
...
static inline void add64(mbedtls_mpi_uint *dst, mbedtls_mpi_uint *src,
 ...
 */
static inline int fix_negative(mbedtls_mpi *N, signed char c, mbedtls_mpi *C,
                               size_t bits) {
  int ret;
...
static inline int ecp_mod_koblitz(mbedtls_mpi *N, mbedtls_mpi_uint *Rp,
                                  size_t p_limbs, size_t adjust, size_t shift,
