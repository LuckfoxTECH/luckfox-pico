...
static void rk_array_xor(int len, const unsigned char *a1,
                         const unsigned char *a2, unsigned char *des) {
  int i;
  for (i = 0; i < len; i++) {
    des[i] = a1[i] ^ a2[i];
  }
...
static void rk_derive_mac_key(RK_AES_KEY *key, unsigned char *k1,
                              unsigned char *k2) {
  unsigned char plain[AES_BLOCK_SIZE] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00, 0x00, 0x00, 0x00};
  unsigned char Rb[AES_BLOCK_SIZE] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x87};
...
int rk_aes_genarate_cmac(const unsigned char *key, unsigned int key_len,
  ...
  if (msg_len % AES_BLOCK_SIZE == 0 && msg_len != 0) {
    ...
    if (msg_len == 0) {
      ...
      for (i = 0; i < block - 1; i++) {
        rk_array_xor(AES_BLOCK_SIZE, &msg[i * AES_BLOCK_SIZE], IVtemp,
                     Blocktemp);
        rk_aes_encrypt(Blocktemp, IVtemp, &aes_key);
      }
...
int rk_aes_verify_cmac(const unsigned char *key, unsigned int key_len,
  ...
  if (msg_len % AES_BLOCK_SIZE == 0 && msg_len != 0) {
    ...
    rk_array_xor(AES_BLOCK_SIZE, &msg[(block - 1) * AES_BLOCK_SIZE], IVtemp,
                 Blocktemp);
