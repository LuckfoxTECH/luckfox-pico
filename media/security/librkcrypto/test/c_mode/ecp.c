...
static int ecp_safe_invert_jac(const mbedtls_ecp_group *grp,
                               mbedtls_ecp_point *Q, unsigned char inv) {
  int ret;
 ...
 */
static void ecp_comb_fixed(unsigned char x[], size_t d, unsigned char w,
                           const mbedtls_mpi *m) {
  size_t i, j;
...
static int ecp_precompute_comb(const mbedtls_ecp_group *grp,
                               mbedtls_ecp_point T[],
                               const mbedtls_ecp_point *P, unsigned char w,
                               size_t d) {
  int ret;
  unsigned char i, k;
  size_t j;
...
static int ecp_mul_mxz(mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
  ...
  i = mbedtls_mpi_bitlen(
      m); /* one past the (zero-based) most significant bit */
  while (i-- > 0) {
...
int mbedtls_ecp_check_privkey(const mbedtls_ecp_group *grp,
  ...
  if (ecp_get_type(grp) == ECP_TYPE_MONTGOMERY) {
    ...
    if (mbedtls_mpi_get_bit(d, 0) != 0 || mbedtls_mpi_get_bit(d, 1) != 0 ||
        ...
        mbedtls_mpi_bitlen(d) - 1 !=
            grp->nbits) /* mbedtls_mpi_bitlen is one-based! */
      return (MBEDTLS_ERR_ECP_INVALID_KEY);
    else
      return (0);
  }
