...
void mbedtls_des_init(rk_des_context *ctx) {
  memset(ctx, 0, sizeof(rk_des_context));
}

void mbedtls_des_free(rk_des_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls_zeroize(ctx, sizeof(rk_des_context));
}

void mbedtls_des3_init(rk_des3_context *ctx) {
  memset(ctx, 0, sizeof(rk_des3_context));
}

void mbedtls_des3_free(rk_des3_context *ctx) {
  if (ctx == NULL)
    return;

  mbedtls_zeroize(ctx, sizeof(rk_des3_context));
}
...
void mbedtls_des_key_set_parity(unsigned char key[DES_BLOCK_SIZE]) {
  int i;
 ...
 */
int mbedtls_des_key_check_key_parity(const unsigned char key[DES_BLOCK_SIZE]) {
  int i;
...
static const unsigned char weak_key_table[WEAK_KEY_COUNT][DES_BLOCK_SIZE] = {
...
int mbedtls_des_key_check_weak(const unsigned char key[DES_BLOCK_SIZE]) {
  int i;

  for (i = 0; i < WEAK_KEY_COUNT; i++)
    if (memcmp(weak_key_table[i], key, DES_BLOCK_SIZE) == 0)
      return (1);
 ...
 */
int rk_des_setkey_enc(rk_des_context *ctx,
                      const unsigned char key[DES_BLOCK_SIZE]) {
 ...
 */
int rk_des_setkey_dec(rk_des_context *ctx,
                      const unsigned char key[DES_BLOCK_SIZE]) {
  int i;
 ...
 */
int rk_des_crypt_ecb(const unsigned char input[8], unsigned char output[8],
                     void *ctx) {
  int i;
  uint32_t X, Y, T, *SK;

  SK = ((rk_des_context *)ctx)->sk;
 ...
 */
int rk_des3_crypt_ecb(const unsigned char input[8], unsigned char output[8],
                      void *ctx) {
  int i;
  uint32_t X, Y, T, *SK;

  SK = ((rk_des3_context *)ctx)->sk;
