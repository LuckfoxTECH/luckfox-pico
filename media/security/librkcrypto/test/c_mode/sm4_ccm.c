...
int rk_crypto_ccm128_encrypt(CCM128_CONTEXT *ctx, const unsigned char *inp,
...
static size_t rk_crypto_ccm128_tag(CCM128_CONTEXT *ctx, unsigned char *tag,
                                   size_t len) {
...
int rk_sm4_ccm_op(struct sm4_ae_in *in, struct sm4_ae_out *out, const int enc) {
  ...
  if (enc == 0) {
    if ((ret = rk_crypto_ccm128_decrypt(&ctx, out->dest, in->src,
                                        out->dest_len)) != 0)
      printf("=========rk_crypto_ccm128_decrypt ret = %d===\n", ret);
  }
