 ...
 *
 */
static void sm4_one_round(unsigned long sk[32], const unsigned char input[16],
...
int rk_sm4_crypt_ecb(void *ctx, const unsigned char *input,
                     unsigned char *output) {
  sm4_one_round(((sm4_context *)ctx)->sk, input, output);
  return 0;
}

int rk_rk_sm4_crypt_ecb(const unsigned char *input, unsigned char *output,
                        void *ctx) {
  return rk_sm4_crypt_ecb(ctx, input, output);
}
