...
void mbedtls_md5_init( RK_MD5_CTX *ctx )
{
    memset( ctx, 0, sizeof( RK_MD5_CTX ) );
}
...
void mbedtls_md5_clone( RK_MD5_CTX *dst,
                        const RK_MD5_CTX *src )
{
    *dst = *src;
}
 ...
 * MD5 process buffer
 */
void mbedtls_md5_update( RK_MD5_CTX *ctx, const unsigned char *input, size_t ilen )
{
    size_t fill;
    uint32_t left;
