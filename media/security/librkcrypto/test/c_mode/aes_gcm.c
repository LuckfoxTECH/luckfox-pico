...
static void CRYPTO_gcm128_init(GCM128_CONTEXT *ctx,void *key,block128_f block)
{
	const union { long one; char little; } is_endian = {1};
...
static void CRYPTO_gcm128_setiv(GCM128_CONTEXT *ctx,const unsigned char *iv,size_t len)
{
	const union { long one; char little; } is_endian = {1};
	unsigned int ctr;
#ifdef GCM_FUNCREF_4BIT
...
static int CRYPTO_gcm128_aad(GCM128_CONTEXT *ctx,const unsigned char *aad,size_t len)
{
	size_t i;
	unsigned int n;
...
static int CRYPTO_gcm128_encrypt(GCM128_CONTEXT *ctx,
...
static int CRYPTO_gcm128_decrypt(GCM128_CONTEXT *ctx,
		const unsigned char *in, unsigned char *out,
		size_t len)
{
	const union { long one; char little; } is_endian = {1};
...
static void CRYPTO_gcm128_tag(GCM128_CONTEXT *ctx, unsigned char *tag, size_t len)
{
...
int rk_aes_gcm_encrypt(struct aes_ae_in *in, struct aes_ae_out *out, const int enc)
{
	...
	CRYPTO_gcm128_init(&ctx,&ks,(block128_f)rk_aes_encrypt);
	CRYPTO_gcm128_setiv(&ctx,in->iv,in->iv_len);
	if (in->aad_len) CRYPTO_gcm128_aad(&ctx,in->aad,in->aad_len);
	if(enc){
		if (in->src_len) CRYPTO_gcm128_encrypt(&ctx,in->src,out->dest,in->src_len);
		CRYPTO_gcm128_tag(&ctx, out->tag, in->tag_size);
		return 0;
	}else{
	   	if (in->src_len) CRYPTO_gcm128_decrypt(&ctx,in->src,out->dest,in->src_len);
	   	CRYPTO_gcm128_tag(&ctx, tmp, in->tag_size);
		return compare_string(tmp, out->tag, in->tag_size);
	}
}
