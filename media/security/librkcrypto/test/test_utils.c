...
bool is_no_multi_blocksize(uint32_t mode) {
  if (mode == RK_CIPHER_MODE_CTS || mode == RK_CIPHER_MODE_CTR ||
      mode == RK_CIPHER_MODE_CFB || mode == RK_CIPHER_MODE_OFB)
    return true;
  else
    return false;
}
...
static const struct test_name_map mode_map_tbl[] = {
    {RK_CIPHER_MODE_ECB, "ECB"}, {RK_CIPHER_MODE_CBC, "CBC"},
    {RK_CIPHER_MODE_CTS, "CTS"}, {RK_CIPHER_MODE_CTR, "CTR"},
    {RK_CIPHER_MODE_CFB, "CFB"}, {RK_CIPHER_MODE_OFB, "OFB"},
    {RK_CIPHER_MODE_XTS, "XTS"}, {RK_CIPHER_MODE_CCM, "CCM"},
...
static const char *get_name_from_map(uint32_t id,
                                     const struct test_name_map *map,
                                     uint32_t map_cnt) {
  uint32_t i;
