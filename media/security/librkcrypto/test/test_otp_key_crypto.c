...
uint8_t otp_key0[32] = {
    0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x08, 0x07, 0x06,
    0x05, 0x04, 0x03, 0x02, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
    0x07, 0x08, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
};

uint8_t otp_key1[32] = {
    0xdf, 0x20, 0x5a, 0xb3, 0x88, 0x50, 0x9e, 0x4f, 0x01, 0x21, 0xe7,
    0xc9, 0x24, 0x00, 0x1b, 0x84, 0x2a, 0xfb, 0x83, 0xac, 0xe4, 0x61,
    0x4a, 0x94, 0x1f, 0xf4, 0x84, 0xc3, 0x1f, 0xe5, 0x52, 0xc7,
};

uint8_t otp_key2[32] = {
    0xd5, 0x20, 0xaa, 0xb3, 0x88, 0x5f, 0x9e, 0x41, 0x05, 0x21, 0x07,
    0xc9, 0x44, 0x00, 0x1b, 0x80, 0x23, 0xfb, 0x13, 0xac, 0xe4, 0x66,
    0x4a, 0x93, 0x13, 0xf4, 0x04, 0xc3, 0x3f, 0xe7, 0x52, 0xc0,
};

uint8_t otp_key3[32] = {
    0x10, 0x44, 0x80, 0xb3, 0x88, 0x5f, 0x02, 0x03, 0x05, 0x21, 0x07,
    0xc9, 0x44, 0x00, 0x1b, 0x80, 0x5f, 0x9e, 0x41, 0xac, 0xe4, 0x64,
    0x43, 0xa3, 0x13, 0x06, 0x07, 0x08, 0x3f, 0xe7, 0x05, 0x06,
};
...
void test_write_otp_key(void) {
  ...
  res = rk_write_oem_otp_key(RK_OEM_OTP_KEY0, otp_key0, sizeof(otp_key0));
  printf("write otp key 0. res:%d\n", res);

  res = rk_write_oem_otp_key(RK_OEM_OTP_KEY1, otp_key1, sizeof(otp_key1));
  printf("write otp key 1. res:%d\n", res);

  res = rk_write_oem_otp_key(RK_OEM_OTP_KEY2, otp_key2, sizeof(otp_key2));
  printf("write otp key 2. res:%d\n", res);

  res = rk_write_oem_otp_key(RK_OEM_OTP_KEY3, otp_key3, sizeof(otp_key3));
  printf("write otp key 3. res:%d\n", res);

  return;
}

static int test_otp_key_item_virt(uint32_t key_id,
                                  const struct test_otp_key_item *item) {
  int res = 0;
  RK_RES rk_res;
  uint32_t i, j, k;
  uint8_t *key = NULL;
  ...
  for (i = 0; i < ARRAY_SIZE(item->modes); i++) {
    ...
    for (j = 0; j < ARRAY_SIZE(item->key_lens); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(item->operations); k++) {
        ...
        rk_res = rk_oem_otp_key_cipher_virt(key_id, &cipher_cfg, plain,
                                            cipher_hard, data_len);
        ...
        rk_res = soft_cipher(algo, mode, operation, key, key_len, cipher_cfg.iv,
...
static int test_otp_key_item_fd(uint32_t key_id,
                                const struct test_otp_key_item *item) {
  int res = 0;
  RK_RES rk_res;
  uint32_t i, j, k;
  ...
  for (i = 0; i < ARRAY_SIZE(item->modes); i++) {
    ...
    for (j = 0; j < ARRAY_SIZE(item->key_lens); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(item->operations); k++) {
        ...
        rk_res = soft_cipher(algo, mode, operation, key, key_len, cipher_cfg.iv,
                             plain->vaddr, data_len, cipher_soft);
        if (res) {
          printf("soft_cipher fail! 0x%08x\n", res);
          res = -1;
          goto exit;
