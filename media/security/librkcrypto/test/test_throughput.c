...
static int test_otp_key_item_tp(bool is_virt, uint32_t key_id, uint32_t key_len,
                                ...
                                uint32_t operation, rk_crypto_mem *fd,
                                uint32_t data_len) {
  uint32_t res = 0;
  ...
  while (total_nsec < nsec) {
    ...
    if (is_virt)
      res =
          rk_oem_otp_key_cipher_virt(key_id, &config, in_out, in_out, data_len);
    else
      res = rk_oem_otp_key_cipher(key_id, &config, fd->dma_fd, fd->dma_fd,
                                  data_len);

    if (res == RK_CRYPTO_ERR_NOT_SUPPORTED) {
      if (is_virt)
        printf("virt:\totpkey\t[%s-%u]\t%s\t%s\tN/A.\n", test_algo_name(algo),
               key_len * 8, test_mode_name(mode), test_op_name(operation));
      else
  ...
  for (h = 0; h < ARRAY_SIZE(algo_tab); h++) {
    for (j = 0; j < ARRAY_SIZE(mode_tab); j++) {
      for (k = 0; k < ARRAY_SIZE(op_tab); k++) {
        ...
        if (test_otp_key_item_tp(true, key_id, key_len, algo, mode, operation,
                                 NULL, len))
          goto error;
      }
    }
  ...
  for (h = 0; h < ARRAY_SIZE(algo_tab); h++) {
    for (j = 0; j < ARRAY_SIZE(mode_tab); j++) {
      for (k = 0; k < ARRAY_SIZE(op_tab); k++) {
        ...
        if (test_otp_key_item_tp(false, key_id, key_len, algo, mode, operation,
                                 in_out, len)) {
...
static int test_cipher_item_tp(bool is_virt, uint32_t key_len, uint32_t algo,
                               uint32_t mode, uint32_t operation, void *in_out,
                               uint32_t data_len) {
  uint32_t res = 0;
  rk_handle handle = 0;
  rk_cipher_config config;
  ...
  while (total_nsec < nsec) {
    ...
    if (is_virt)
      ...
      res = rk_cipher_crypt(handle, ((rk_crypto_mem *)in_out)->dma_fd,
...
static int test_cipher_tp(void) {
  ...
  if (posix_memalign((void *)&in_out_virt, page_size, TEST_BLOCK_SIZE) ||
      !in_out_virt) {
  ...
  for (h = 0; h < ARRAY_SIZE(test_item_tbl); h++) {
    for (j = 0; j < ARRAY_SIZE(test_item_tbl[h].modes); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(test_item_tbl[h].op); k++) {
        ...
        if (test_cipher_item_tp(false, key_len, algo, mode, operation,
                                in_out_fd, len)) {
  ...
  for (h = 0; h < ARRAY_SIZE(test_item_tbl); h++) {
    for (j = 0; j < ARRAY_SIZE(test_item_tbl[h].modes); j++) {
      ...
      for (k = 0; k < ARRAY_SIZE(test_item_tbl[h].op); k++) {
        ...
        if (test_cipher_item_tp(true, key_len, algo, mode, operation,
                                in_out_virt, len)) {
...
static int test_ae_item_tp(bool is_virt, uint32_t key_len, uint32_t algo,
                           uint32_t mode, void *in, void *out,
                           uint32_t data_len, void *aad, uint32_t aad_len,
                           void *tag) {
  uint32_t res = 0;
  rk_handle handle = 0;
  rk_ae_config config;
  ...
  while (total_nsec < nsec) {
    ...
    if (is_virt) {
      ...
      res = rk_ae_crypt(handle, ((rk_crypto_mem *)in)->dma_fd,
                        ((rk_crypto_mem *)out)->dma_fd, data_len, tag);
    }
  ...
  if (is_virt)
    printf("virt:\t[%s-%u]\t%s\t%s\t%dMB/s.\n", test_algo_name(algo),
           key_len * 8, test_mode_name(mode), test_op_name(config.operation),
           (data_len * rounds / (1024 * 1024)));
  else
    printf("dma_fd:\t[%s-%u]\t%s\t%s\t%dMB/s.\n", test_algo_name(algo),
           key_len * 8, test_mode_name(mode), test_op_name(config.operation),
  ...
  while (total_nsec < nsec) {
    ...
    if (is_virt) {
      ...
      res = rk_ae_crypt(handle, ((rk_crypto_mem *)out)->dma_fd,
                        ((rk_crypto_mem *)in)->dma_fd, data_len, tag);
    }
...
static int test_ae_tp(void) {
  ...
  if (posix_memalign((void *)&in_virt, page_size, TEST_BLOCK_SIZE) ||
      !in_virt) {
    printf("malloc %uByte error!\n", TEST_BLOCK_SIZE);
    res = -1;
    goto out;
  ...
  if (posix_memalign((void *)&out_virt, page_size, TEST_BLOCK_SIZE) ||
      !out_virt) {
    printf("malloc %uByte error!\n", TEST_BLOCK_SIZE);
    res = -1;
    goto out;
  ...
  for (h = 0; h < ARRAY_SIZE(test_item_tbl); h++) {
    for (j = 0; j < ARRAY_SIZE(test_item_tbl[h].modes); j++) {
      ...
      if (test_ae_item_tp(false, key_len, algo, mode, in_fd, out_fd, len,
                          aad_fd, aad_len, tag)) {
  ...
  for (h = 0; h < ARRAY_SIZE(test_item_tbl); h++) {
    for (j = 0; j < ARRAY_SIZE(test_item_tbl[h].modes); j++) {
      ...
      if (test_ae_item_tp(true, key_len, algo, mode, in_virt, out_virt, len,
...
static int test_hash_item_tp(bool is_virt, bool is_hmac, uint32_t algo,
  ...
  while (total_nsec < nsec) {
    ...
    if (is_virt) {
      ...
      res = rk_hash_update(hash_hdl, ((rk_crypto_mem *)input)->dma_fd,
                           data_block);
      if (res) {
        rk_hash_final(hash_hdl, NULL);
...
static int test_hash_tp(void) {
  ...
  if (posix_memalign((void *)&input_virt, page_size, TEST_BLOCK_SIZE) ||
      !input_virt) {
    printf("malloc %uByte error!\n", TEST_BLOCK_SIZE);
    res = -1;
    goto out;
...
static int test_rsa_item_tp(uint32_t nbits) {
  ...
  res = rk_rsa_priv_encrypt(&priv_key, RK_RSA_CRYPT_PADDING_NONE, data_plain,
                            nbytes, data_enc, &out_len);
  if (res) {
    if (res != RK_CRYPTO_ERR_NOT_SUPPORTED)
  ...
  res = rk_rsa_pub_decrypt(&pub_key, RK_RSA_CRYPT_PADDING_NONE, data_enc,
                           out_len, data_dec, &out_len);
  if (res) {
  ...
  priv_cost = (t2.tv_sec - t1.tv_sec) * 1000000000 + (t2.tv_nsec - t1.tv_nsec);

  pub_cost = (t3.tv_sec - t2.tv_sec) * 1000000000 + (t3.tv_nsec - t2.tv_nsec);

  printf("virt:\t[RSA-%u]\tPRIV\tENCRYPT\t%lums.\n", nbits,
         (unsigned long)(priv_cost / 1000000));

  printf("virt:\t[RSA-%u]\tPUB\tDECRYPT\t%lums.\n", nbits,
         (unsigned long)(pub_cost / 1000000));
