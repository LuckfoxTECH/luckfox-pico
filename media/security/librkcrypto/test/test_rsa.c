...
#define TEST_END_PASS(nbits, padding_name)                                     \
  do {                                                                         \
    if (verbose)                                                               \
...
#define TEST_END_FAIL(nbits, padding_name)                                     \
  do {                                                                         \
    if (verbose)                                                               \
...
#define TEST_END_NA(nbits, padding_name)                                       \
  do {                                                                         \
    if (verbose)                                                               \
...
#define TEST_RSA_CRYPT(test, name, size)                                       \
  { test, RK_RSA_CRYPT_PADDING_##name, #name, size }
#define TEST_RSA_SIGN(test, name, size)                                        \
  { test, RK_RSA_SIGN_PADDING_##name, #name, size }
#define TEST_RSA_EMPTY()                                                       \
  { NULL, 0, NULL, 0 }
...
static RK_RES rk2ssl_padding(uint32_t rk_padding, int *ssl_padding,
...
static RK_RES openssl_encrypt(const uint8_t *in, uint32_t in_len, uint8_t *out,
                              uint32_t *out_len, int padding,
...
static RK_RES openssl_decrypt(const uint8_t *in, uint32_t in_len, uint8_t *out,
                              uint32_t *out_len, int padding,
                              const EVP_MD *digest_algorithm,
                              rk_rsa_priv_key_pack *priv) {
  RK_RES res = RK_CRYPTO_ERR_GENERIC;
...
static RK_RES openssl_sign(const uint8_t *in, uint32_t in_len, uint8_t *out,
                           uint32_t *out_len, int padding,
                           const EVP_MD *digest_algorithm,
                           rk_rsa_priv_key_pack *priv) {
  RK_RES res = RK_CRYPTO_ERR_GENERIC;
...
static RK_RES openssl_verify(const uint8_t *in, uint32_t in_len, uint8_t *sign,
                             uint32_t sign_len, int padding,
                             const EVP_MD *digest_algorithm,
                             rk_rsa_priv_key_pack *priv) {
  RK_RES res = RK_CRYPTO_ERR_GENERIC;
  EVP_PKEY_CTX *pkey_ctx;
...
static RK_RES calc_padding_digest(uint32_t padding, const uint8_t *data,
...
static RK_RES test_rsa_pub_enc(uint32_t padding, const char *padding_name,
  ...
  res =
      rk_rsa_pub_encrypt(&pub_key, padding, data, data_len, enc_buf, &out_len);
  if (res) {
  ...
  if (rk2ssl_padding(padding, &ssl_padding, &digest_md) == RK_CRYPTO_SUCCESS) {
    res = openssl_decrypt(enc_buf, out_len, dec_buf, &out_len, ssl_padding,
                          digest_md, &priv_key);
    if (res) {
    ...
    res = openssl_encrypt(data, data_len, enc_buf, &out_len, ssl_padding,
                          digest_md, &priv_key);
    if (res) {
...
static RK_RES test_rsa_priv_enc(uint32_t padding, const char *padding_name,
                                uint32_t nbits, uint32_t data_len,
                                int verbose) {
  RK_RES res = RK_CRYPTO_SUCCESS;
  uint8_t *data = NULL;
  uint8_t *enc_buf = NULL;
  ...
  res = rk_rsa_priv_encrypt(&priv_key, padding, data, data_len, enc_buf,
                            &out_len);
  if (res) {
    if (res != RK_CRYPTO_ERR_NOT_SUPPORTED &&
        res != RK_CRYPTO_ERR_PADDING_OVERFLOW)
...
static RK_RES test_rsa_sign_common(uint32_t padding, const char *padding_name,
                                   uint32_t nbits, const uint8_t *in,
                                   uint32_t in_len, const uint8_t *hash,
                                   int verbose) {
  RK_RES res = RK_CRYPTO_SUCCESS;
  uint8_t *sign = NULL;
  ...
  if (rk2ssl_padding(padding, &ssl_padding, &digest_md) == RK_CRYPTO_SUCCESS) {
    res = openssl_verify(in, in_len, sign, sign_len, ssl_padding, digest_md,
                         &priv_key);
    if (res) {
    ...
    res = openssl_sign(in, in_len, sign, &sign_len, ssl_padding, digest_md,
                       &priv_key);
    if (res) {
...
static RK_RES test_rsa_sign(uint32_t padding, const char *padding_name,
  ...
  res = test_rsa_sign_common(padding, padding_name, nbits, data, in_len, NULL,
                             verbose);
  ...
  res = test_rsa_sign_common(padding, padding_name, nbits, data, in_len, digest,
                             verbose);
  ...
  for (i = 0; i < ARRAY_SIZE(rsa_key_nbits); i++) {
    ...
    for (j = 0; j < ARRAY_SIZE(test_rsa_tbl); j++) {
      ...
      res = test_rsa_tbl[j].do_test(test_rsa_tbl[j].padding,
                                    ...
                                    rsa_key_nbits[i], data_size, verbose);
      if (res)
        goto exit;
    }
