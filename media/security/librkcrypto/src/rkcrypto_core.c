...
static RK_RES xioctl(int fd, unsigned long int request, void *arg) {
  return ioctl(fd, request, arg) ? kernel_to_crypto_code(-errno)
                                 : RK_CRYPTO_SUCCESS;
}

static RK_RES rk_get_crypto_id(uint32_t algo, uint32_t mode,
                               uint32_t *crypto_id) {
  uint32_t i;
...
static RK_RES rk_add_sess_node(uint32_t sess_id, uint32_t config_type,
                               const void *config, void *priv) {
...
static RK_RES rk_update_user_iv(const void *cfg) {
  ...
  if (node->priv) {
    ...
    case RK_CONFIG_TYPE_CIPHER:
      memcpy(node->priv, node->config.cipher.iv,
             sizeof(node->config.cipher.iv));
      break;
    case RK_CONFIG_TYPE_AE:
      ae_priv = node->priv;
...
RK_RES rk_cipher_init(const rk_cipher_config *config, rk_handle *handle) {
  ...
  return rk_create_session(&sess, RK_CONFIG_TYPE_CIPHER, config,
                           (void *)config->iv, handle);
}
...
RK_RES rk_ae_init(const rk_ae_config *config, rk_handle *handle) {
  ...
  return rk_create_session(&sess, RK_CONFIG_TYPE_AE, config, (void *)ae_priv,
                           handle);
}
...
RK_RES rk_ae_crypt(rk_handle handle, int in_fd, int out_fd, uint32_t len,
  ...
  V_TRACE("in and out are %s address.",
          (in_fd == out_fd) ? "the same" : "different");
  V_TRACE("cipher data len = %d, aad_len = %d, tag_len = %d", len,
          ae_cfg->aad_len, ae_cfg->tag_len);
...
static RK_RES rk_rsa_crypt_common(void *key, uint16_t flag, uint16_t op,
  ...
  if ((flag & COP_FLAG_RSA_PRIV) == COP_FLAG_RSA_PRIV)
    res = rk_rsa_privkey_encode((rk_rsa_priv_key_pack *)key, asn1_key,
                                &asn1_key_len, &key_bits);
  else
    res = rk_rsa_pubkey_encode((rk_rsa_pub_key_pack *)key, asn1_key,
                               &asn1_key_len, &key_bits);
...
RK_RES rk_rsa_pub_encrypt(const rk_rsa_pub_key_pack *pub,
  ...
  /* deal with padding */
  res = rk_rsa_crypt_do_padding(padding, pub->key.n_len, false, in, in_len,
                                data_pad, &data_pad_len);
  if (res) {
...
RK_RES rk_rsa_priv_decrypt(const rk_rsa_priv_key_pack *priv,
  ...
  res = rk_rsa_crypt_common((void *)priv, COP_FLAG_RSA_PRIV, AOP_DECRYPT, in,
                            in_len, data_pad, &data_pad_len);
  if (res) {
    D_TRACE("rk_rsa_crypt_common error! %x", res);
    return res;
  }

  return rk_rsa_crypt_undo_padding(padding, priv->key.n_len, true, data_pad,
                                   data_pad_len, out, out_len);
}
...
RK_RES rk_rsa_priv_encrypt(const rk_rsa_priv_key_pack *priv,
  ...
  /* deal with padding */
  res = rk_rsa_crypt_do_padding(padding, priv->key.n_len, true, in, in_len,
                                data_pad, &data_pad_len);
  if (res) {
    D_TRACE("rsa padding %d error!", padding);
    return res;
...
RK_RES rk_rsa_pub_decrypt(const rk_rsa_pub_key_pack *pub,
  ...
  res = rk_rsa_crypt_common((void *)pub, COP_FLAG_RSA_PUB, AOP_ENCRYPT, in,
                            in_len, data_pad, &data_pad_len);
  if (res) {
    D_TRACE("rk_rsa_crypt_common error! %x", res);
    return res;
  ...
  return rk_rsa_crypt_undo_padding(padding, pub->key.n_len, false, data_pad,
                                   data_pad_len, out, out_len);
}
...
RK_RES rk_rsa_sign(const rk_rsa_priv_key_pack *priv,
  ...
  res = rk_rsa_sign_do_padding(padding, priv->key.n_len,
                               rk_mpi_bitlen(priv->key.n, priv->key.n_len), in,
                               in_len, hash, data_pad, &data_pad_len);
  if (res) {
    D_TRACE("rsa padding %d error!", padding);
    return res;
  }
...
RK_RES rk_rsa_verify(const rk_rsa_pub_key_pack *pub,
  ...
  res = rk_rsa_crypt_common((void *)pub, COP_FLAG_RSA_PUB, AOP_ENCRYPT, sign,
                            sign_len, dec_pad, &dec_pad_len);
  if (res) {
  ...
  res = rk_rsa_sign_undo_padding(padding, pub->key.n_len,
                                 rk_mpi_bitlen(pub->key.n, pub->key.n_len), in,
                                 in_len, hash, dec_pad);
  if (res) {
    D_TRACE("rsa padding %d error!", padding);
    goto exit;
