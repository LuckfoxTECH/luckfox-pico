...
static struct mem_pool_node *crypto_alloc_node_drm(int dma_node_fd,
                                                   uint32_t size) {
  int ret = -1;
  size_t min_size;
...
static void crypto_free_node_drm(int dma_node_fd, struct mem_pool_node *node) {
  ...
  if (node->mem.vaddr)
    munmap(node->mem.vaddr,
           node->mem.size < min_size ? min_size : node->mem.size);
...
static struct mem_pool_node *crypto_alloc_node_dma_heap(int dma_node_fd,
                                                        uint32_t size) {
  int ret = -1;
  size_t min_size;
  struct mem_pool_node *node = NULL;
...
#ifdef __ANDROID__
  node->mem.vaddr =
      mmap64(0, req.len, PROT_READ | PROT_WRITE, MAP_SHARED, req.fd, 0);
#else
  node->mem.vaddr =
      mmap(0, req.len, PROT_READ | PROT_WRITE, MAP_SHARED, req.fd, 0);
#endif
  if (node->mem.vaddr == MAP_FAILED) {
...
static void crypto_free_node_dma_heap(int dma_node_fd,
