...
static const uint8_t sha224_oid[] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                     0x03, 0x04, 0x02, 0x04};
static const uint8_t sha256_oid[] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                     0x03, 0x04, 0x02, 0x01};
static const uint8_t sha384_oid[] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                     0x03, 0x04, 0x02, 0x02};
static const uint8_t sha512_oid[] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                     0x03, 0x04, 0x02, 0x03};
...
static RK_RES get_oid_by_md(uint32_t hash_algo, const uint8_t **oid,
                            uint32_t *old_len) {
  uint32_t i;
...
static RK_RES asn1_compose_len(uint32_t len, uint8_t *field,
                               uint32_t *field_len) {
...
static RK_RES asn1_set_object(const uint8_t *in, uint32_t in_len, uint8_t tag,
                              uint8_t need_plus, uint8_t *out, uint32_t out_max,
                              uint32_t *out_len) {
  RK_RES res;
  uint8_t *pout = out;
  uint32_t field_len;
...
static RK_RES get_hash_algo_from_padding(uint32_t padding, uint32_t *hlen,
                                         uint32_t *hash_algo) {
...
static RK_RES calc_padding_digest(uint32_t algo, const uint8_t *data,
                                  uint32_t data_len, uint8_t *digest) {
  RK_RES res;
 ...
 */
static RK_RES rsa_padding_add_oaep_type(enum RK_RSA_CRYPT_PADDING padding,
                                        uint16_t key_len, const uint8_t *label,
                                        uint32_t label_len, const uint8_t *in,
  ...
  *p++ = 1;
  ...
  res =
      mgf_mask(out + hlen + 1, olen - hlen - 1, out + 1, hlen, hash_algo, hlen);
  if (res)
    goto error;

  /* maskedSeed: Apply seedMask to seed */
  res =
      mgf_mask(out + 1, hlen, out + hlen + 1, olen - hlen - 1, hash_algo, hlen);
  if (res)
    goto error;
 ...
 */
RK_RES rsa_padding_check_oaep_type(enum RK_RSA_CRYPT_PADDING padding,
                                   uint16_t key_len, const uint8_t *label,
                                   size_t label_len, const uint8_t *in,
  ...
  res =
      mgf_mask(buf + 1, hlen, buf + hlen + 1, ilen - hlen - 1, hash_algo, hlen);
  if (res)
    goto exit;

  /* DB: Apply dbMask to maskedDB */
  res =
      mgf_mask(buf + hlen + 1, ilen - hlen - 1, buf + 1, hlen, hash_algo, hlen);
  if (res)
    goto exit;
 ...
 */
static int rsa_padding_add_pkcs15_sign_type(uint32_t hash_algo,
                                            uint16_t key_len,
                                            const uint8_t *hash,
                                            uint8_t hash_len, uint8_t *out) {
  RK_RES res;
  uint32_t oid_size = 0;
   ...
   */
  if (8 + hash_len + oid_size >= 0x80 || 10 + hash_len < hash_len ||
...
RK_RES rsa_padding_add_pss_type(uint16_t key_len, uint16_t n_bits,
  ...
  *p++ = 0x01;
  ...
  /* maskedDB: Apply dbMask to DB */
  res = mgf_mask(out + offset, olen - hlen - 1 - offset, p, hlen, hash_algo,
                 hash_len);
  if (res)
    goto exit;
...
RK_RES rk_rsa_padding_check_pss_type(uint16_t key_len, uint16_t n_bits,
  ...
  res = mgf_mask(p, siglen - hlen - 1, p + siglen - hlen - 1, hlen, hash_algo,
                 hash_len);
  if (res)
    goto error;
...
RK_RES rk_rsa_pubkey_encode(rk_rsa_pub_key_pack *pub, uint8_t *asn1_key,
                            uint16_t *asn1_key_len, uint16_t *key_bits) {
  RK_RES res;
  ...
  RK_CRYPTO_CHECK_PARAM(
      rsa_key->n_len != 1024 / 8 && rsa_key->n_len != 2048 / 8 &&
      rsa_key->n_len != 3072 / 8 && rsa_key->n_len != 4096 / 8);
...
RK_RES rk_rsa_privkey_encode(rk_rsa_priv_key_pack *priv, uint8_t *asn1_key,
                             uint16_t *asn1_key_len, uint16_t *key_bits) {
  RK_RES res;
  ...
  for (i = 0; i < ARRAY_SIZE(object_list); i++) {
    ...
    res = asn1_set_object(data, data_len, object_list[i].tag,
                          object_list[i].need_plus, asn1_key + total_len,
                          out_max, &tmp_len);
    if (res != RK_CRYPTO_SUCCESS) {
...
RK_RES rk_rsa_crypt_do_padding(enum RK_RSA_CRYPT_PADDING padding,
  ...
  RK_CRYPTO_CHECK_PARAM(
      key_len * 8 != RSA_BITS_1024 && key_len * 8 != RSA_BITS_2048 &&
      key_len * 8 != RSA_BITS_3072 && key_len * 8 != RSA_BITS_4096);
  ...
  case RK_RSA_CRYPT_PADDING_OAEP_SHA384:
  case RK_RSA_CRYPT_PADDING_OAEP_SHA512:
    res = rsa_padding_add_oaep_type(padding, key_len, NULL, 0, data, data_len,
                                    pad);
    break;
  case RK_RSA_CRYPT_PADDING_PKCS1_V1_5:
    res =
        rsa_padding_add_pkcs15_type(key_len, is_priv_key, data, data_len, pad);
    break;
  default:
...
RK_RES rk_rsa_crypt_undo_padding(enum RK_RSA_CRYPT_PADDING padding,
  ...
  case RK_RSA_CRYPT_PADDING_OAEP_SHA384:
  case RK_RSA_CRYPT_PADDING_OAEP_SHA512:
    res = rsa_padding_check_oaep_type(padding, key_len, NULL, 0, pad, data,
                                      data_len);
    break;
  case RK_RSA_CRYPT_PADDING_PKCS1_V1_5:
    res = rsa_padding_check_pkcs15_type(key_len, is_priv_key, pad, data,
                                        data_len);
    break;
  default:
    D_TRACE("unknown padding %d", padding);
...
RK_RES rk_rsa_sign_do_padding(enum RK_RSA_SIGN_PADDING padding,
                              uint16_t key_len, uint16_t n_bits,
                              const uint8_t *data, uint32_t data_len,
                              const uint8_t *hash, uint8_t *pad,
                              uint32_t *pad_len) {
  RK_RES res = RK_CRYPTO_SUCCESS;
  ...
  case RK_RSA_SIGN_PADDING_PKCS1_V15_SHA384:
  case RK_RSA_SIGN_PADDING_PKCS1_V15_SHA512:
    res = rsa_padding_add_pkcs15_sign_type(hash_algo, key_len, tmp_hash,
                                           hash_len, pad);
    break;
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA1:
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA224:
  ...
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA384:
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA512:
    res = rsa_padding_add_pss_type(key_len, n_bits, hash_algo, tmp_hash,
                                   hash_len, pad);
    break;
  default:
    D_TRACE("unknown padding %d", padding);
...
RK_RES rk_rsa_sign_undo_padding(enum RK_RSA_SIGN_PADDING padding,
                                uint16_t key_len, uint16_t n_bits,
                                const uint8_t *data, uint32_t data_len,
                                const uint8_t *hash, const uint8_t *dec) {
  RK_RES res = RK_CRYPTO_SUCCESS;
  ...
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA384:
  case RK_RSA_SIGN_PADDING_PKCS1_PSS_SHA512:
    res = rk_rsa_padding_check_pss_type(key_len, n_bits, hash_algo, hash_len,
                                        tmp_hash, dec);
    break;
  default:
    D_TRACE("unknown padding %d", padding);
