...
RK_RES rk_write_oem_otp_key(enum RK_OEM_OTP_KEYID key_id, uint8_t *key,
  ...
  RK_CRYPTO_CHECK_PARAM(
      key_id != RK_OEM_OTP_KEY0 && key_id != RK_OEM_OTP_KEY1 &&
      key_id != RK_OEM_OTP_KEY2 && key_id != RK_OEM_OTP_KEY3 &&
      key_id != RK_OEM_OTP_KEY_FW);
  RK_CRYPTO_CHECK_PARAM(!key);
  RK_CRYPTO_CHECK_PARAM(key_len != 16 && key_len != 24 && key_len != 32);
  RK_CRYPTO_CHECK_PARAM(key_id == RK_OEM_OTP_KEY_FW && key_len != 16);
  ...
  res = TEEC_InvokeCommand(&session, STORAGE_CMD_WRITE_OEM_OTP_KEY, &operation,
                           &error_origin);
  if (res != TEEC_SUCCESS) {
...
RK_RES rk_oem_otp_key_cipher_virt(enum RK_OEM_OTP_KEYID key_id,
  ...
  RK_CRYPTO_CHECK_PARAM(config->key_len != 16 && config->key_len != 24 &&
                        config->key_len != 32);
  RK_CRYPTO_CHECK_PARAM(key_id == RK_OEM_OTP_KEY_FW && config->key_len != 16);
  RK_CRYPTO_CHECK_PARAM(len > RK_CRYPTO_MAX_DATA_LEN || len == 0);
  RK_CRYPTO_CHECK_PARAM(config->mode == RK_CIPHER_MODE_CTS && len <= 16);
