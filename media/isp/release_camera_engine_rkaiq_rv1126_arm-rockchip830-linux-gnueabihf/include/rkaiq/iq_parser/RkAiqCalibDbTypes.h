...
#define CALIBDB_AINR_LUMA_LEN 6
...
#define AEC_HDCIRIS_DOT_MAX_NUM (1024)
...
typedef struct CalibDb_Sensor_Para_s {
    ...
    Cam1x5UShortMatrix_t    CISHdrTimeRegMaxStep;
...
typedef struct CalibDb_AINR_Setting_s {
    float iso[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float gain[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float sigma[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float shade[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float sharp[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float min_luma[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float sat_scale[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float dark_contrast[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float ai_ratio[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float mot_thresh[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float static_thresh[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float mot_nr_stren[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float luma_curve[CALIBDB_AINR_LUMA_LEN];
    float sigma_curve[CALIBDB_NR_SHARP_MAX_ISO_LEVEL][CALIBDB_AINR_LUMA_LEN];
} CalibDb_AINR_Setting_t;

typedef struct CalibDb_AINR_ModeCell_s {
    char name[CALIBDB_MAX_MODE_NAME_LENGTH];
    CalibDb_AINR_Setting_t setting;
} CalibDb_AINR_ModeCell_t;

typedef struct CalibDb_AINR_s {
    int enable;
    char version[64];
    int gain_tab_len;
    int gain_max;
    int tuning_visual_flag;
    float dynamicSw[2];
    CalibDb_AINR_ModeCell_t* mode_cell;
    int mode_num;
} CalibDb_AINR_t;

typedef struct CalibDb_AINR_tool_s {
    int enable;
    char version[64];
    int gain_tab_len;
    int gain_max;
    int tuning_visual_flag;
    float dynamicSw[2];
    CalibDb_AINR_ModeCell_t mode_cell[13];
    int mode_num;
} CalibDb_AINR_tool_t;
...
typedef struct CalibDb_MFNR_Motion_V2_s {
    int enable;
    int dynamicEnable;
    float dynamicSw[2];
    float useAiMdSw[2];
    ...
    int gainUpdFlag;
    int  medEn;
    float mdSigmaScale[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float yuvnrGainScale3[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float yuvnrGainGlb[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float yuvnrGainTH[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float mfnrGainScale[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float mfnrDnStr[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float thresh01[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float thresh23[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float md_thresh[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float md_motion_ratio[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
} CalibDb_MFNR_Motion_V2_t;

typedef struct CalibDb_MFNR_Motion_Manual_s {
    int enable;
    int dynamicEnable;
    float useAiMdSw[2];
    float dynamicSw[2];
...
} CalibDb_MFNR_Motion_Manual_t;

typedef struct CalibDb_MFNR_Motion_V1_s {
    int enable;
    int dynamicEnable;
    float dynamicSw[2];
    float useAiMdSw[2];
    ...
    float frame_limit_y[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
    float md_thresh[CALIBDB_NR_SHARP_MAX_ISO_LEVEL];
} CalibDb_MFNR_Motion_V1_t;


typedef struct CalibDb_MFNR_ModeCell_V2_s {
    char name[CALIBDB_MAX_MODE_NAME_LENGTH];
    CalibDb_MFNR_Setting_t setting[CALIBDB_NR_SHARP_SETTING_LEVEL];
    CalibDb_MFNR_Dynamic_t dynamic;
    CalibDb_MFNR_Motion_V2_t  motion;
} CalibDb_MFNR_ModeCell_V2_t;

typedef struct CalibDb_MFNR_ModeCell_V1_s {
    char name[CALIBDB_MAX_MODE_NAME_LENGTH];
    CalibDb_MFNR_Setting_t setting[CALIBDB_NR_SHARP_SETTING_LEVEL];
    CalibDb_MFNR_Dynamic_t dynamic;
    CalibDb_MFNR_Motion_V1_t  motion;
} CalibDb_MFNR_ModeCell_V1_t;
...
typedef struct CalibDb_MFNR_s {
    ...
    CalibDb_MFNR_ModeCell_V1_t mode_cell[CALIBDB_MAX_MODE_NUM];
} CalibDb_MFNR_t;

typedef struct CalibDb_MFNR_V2_s {
    int enable;
    char version[64];
    unsigned char local_gain_en;
    unsigned char motion_detect_en;
    ...
    unsigned char back_ref_num;
    struct CalibDb_awb_uv_ratio_s uv_ratio[4];
    CalibDb_MFNR_ModeCell_V2_t mode_cell[CALIBDB_MAX_MODE_NUM];
} CalibDb_MFNR_V2_t;
...
typedef struct CalibDb_MFNR_2_s {
    ...
    unsigned char max_level_uv;
    unsigned char back_ref_num;
    int motion_detect_ds_size[2];
    struct CalibDb_awb_uv_ratio_s uv_ratio[4];
    CalibDb_MFNR_ModeCell_V2_t *mode_cell;
    int mode_num;
} CalibDb_MFNR_2_t;
...
typedef enum fec_working_mode_e {
    WORKING_ALGO_FEC,
    WORKING_ALGO_AIS,
} fec_working_mode_t;
...
typedef enum  CalibDb_ALut3dParamMode_e {
    ALut3d_PARAM_MODE_NORMAL = 0,
    ALut3d_PARAM_MODE_HDR,
    ALut3d_PARAM_MODE_MAX
} CalibDb_ALut3dParamMode_t;

typedef struct CalibDb_Lut3d_ModeCell_s {
    char name[CALIBDB_MAX_MODE_NAME_LENGTH];
    unsigned short look_up_table_r[729];
    unsigned short look_up_table_g[729];
    unsigned short look_up_table_b[729];
} CalibDb_Lut3d_ModeCell_t;

typedef struct {
    bool enable;
    CalibDb_Lut3d_ModeCell_t mode_cell[ALut3d_PARAM_MODE_MAX];
