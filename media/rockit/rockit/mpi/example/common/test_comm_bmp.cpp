...
inline RK_U32 OSD_MAKECOLOR_ARGB(RK_U8 r, RK_U8 g, RK_U8 b, OSD_COMP_INFO compinfo) {
    RK_U8 r1, g1, b1;
    RK_U32 pixel = 0;
...
inline RK_U32 OSD_MAKECOLOR_BGRA(RK_U8 r, RK_U8 g, RK_U8 b, OSD_COMP_INFO compinfo) {
    RK_U8 r1, g1, b1;
    RK_U32 pixel = 0;
...
RK_S32 load_bmp_ex(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt) {
    ...
    if (bmpInfo.bmiHeader.biCompression != 0) {
        printf("not support compressed bitmap file! biCompression = %d\n",
    ...
    pVideoLogo->height = (RK_U16)((bmpInfo.bmiHeader.biHeight > 0) ?
    ...
    /* RGB8888 or RGB1555 */
    pOrigBMPBuf = reinterpret_cast<RK_U8 *>(malloc(h * stride));
    if (NULL == pOrigBMPBuf) {
