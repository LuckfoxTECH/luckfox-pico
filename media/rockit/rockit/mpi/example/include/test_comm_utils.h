...
#define CHECK_CHN_RET(express, chn, name)                                      \
  do {                                                                         \
    ...
    if (RK_SUCCESS != s32Ret) {                                                \
      RK_LOGE("\033[0;31m%s chn %d failed with %#x!\033[0;39m", name, chn,     \
              s32Ret);                                                         \
      return s32Ret;                                                           \
    }                                                                          \
  } while (0)
...
RK_S32 TEST_COMM_OpenFileUris(const char *pCfgFileUri, char **pFileUris,
                              RK_U32 u32UriCount);
void TEST_COMM_CloseFileUris(char **pFileUris, RK_U32 u32UriCount);
void TEST_COMM_MkDirs(const char *muldir, RK_S32 mode);
RK_S32 TEST_COMM_CheckFileSizeInRange(const char *pFilePath, RK_S32 minSize,
                                      RK_S32 maxSize);

RK_S32 TEST_COMM_FileReadOneFrame(const char *pFileName,
                                  VIDEO_FRAME_INFO_S *pstVideoFrame);
RK_S32 TEST_COMM_FileWriteOneFrame(const char *pFileName,
                                   VIDEO_FRAME_INFO_S *pstVideoFrame);
