...
public:
  ...
  RT_RET
  prepareForRun(RtMetaData *options,
  ...
  void setQueueSizeCallbacks(
      ...
      std::function<void(RTInputStreamManager *, bool *)>
          becomesNotFullCallback,
  ...
  RT_RET initialize(INT32 nodeId, RTTaskNodeInfo *nodeInfo,
  ...
  RT_RET addStreamSource(INT32 streamId,
                         RTOutputStreamManager *lastOutputManager);
  ...
  RTSchedulerQueue *getSchedulerQueue() const { return mSchedulerQueue; }

  // Sets the scheduler queue the node is assigned to.
  void setSchedulerQueue(RTSchedulerQueue *queue) { mSchedulerQueue = queue; }
...
public:
  ...
  virtual RT_RET queryStatInternal(RTTaskNodeStat *nodeStat) {
