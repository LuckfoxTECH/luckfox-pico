...
public:
  ...
  RTCBHandle
  observeOutputStream(const std::string streamName,
...
public:
  // deprecated api
  RT_RET
  observeOutputStream(const std::string &streamName, INT32 streamId,
                      std::function<RT_RET(RTMediaBuffer *)> streamCallback);
...
public:
  ...
  RTLinkHandle addDownGraph(RTTaskGraph *graph, std::string srcName,
                            std::string dstName);
...
protected:
  RT_RET addPacketToInputStream(std::string streamName, INT64 timeoutUs,
                                RTMediaBuffer *packet);
  ...
  virtual RT_RET
  onReceiveOutputStreams(RTGraphOutputStream *graphOutputStream) {
    return RT_OK;
  }
...
private:
  ...
  RT_RET linkMultiNode(T src, T dst, Args... rest) {
    ...
    if (ret != RT_OK) {
      RT_LOGE("src node %d dst node %d link failed", src->getID(),
              dst->getID());
    } else {
  ...
  template <class T> RT_RET linkMultiNode(T end) { return RT_OK; }
  ...
  template <class T> RT_RET selectMultiMode(T end) {
...
private:
  ...
  void updateThrottledNodesNoLock(RTInputStreamManager *stream,
                                  bool *streamWasFull);
