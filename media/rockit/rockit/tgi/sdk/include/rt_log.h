...
//! super macro.
#define RT_LOGD_IF(condition, format, ...)                                     \
  if (condition > 0)                                                           \
  LOG_TRACE(RT_LOG_DEBUG, format, ##__VA_ARGS__)
#define RT_LOGE_IF(condition, format, ...)                                     \
  if (condition > 0)                                                           \
  LOG_ERROR(RT_LOG_ERR, format, ##__VA_ARGS__)
...
// construct/destruct
#define RT_LOG_CONSTRUCT_IN(thiz)                                              \
  RT_LOGD_IF(LOG_FLAG, "%s(%p) construct", __FUNCTION__, thiz)
#define RT_LOG_CONSTRUCT_DONE(thiz)                                            \
  RT_LOGD_IF(LOG_FLAG, "%s(%p) construct ok", __FUNCTION__, thiz)
#define RT_LOG_DESTRUCT_IN(thiz)                                               \
  RT_LOGD_IF(LOG_FLAG, "%s(%p) destructor", __FUNCTION__, thiz)
#define RT_LOG_DESTRUCT_DONE(thiz)                                             \
  RT_LOGD_IF(LOG_FLAG, "%s(%p) destructor ok", __FUNCTION__, thiz)
