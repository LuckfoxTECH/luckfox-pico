...
INT32 util_vsnprintf(char *buffer, size_t max_len, const char *format,
                     va_list args);
...
constexpr UINT64 func_hash_bkdr_constexpr(const char *str,
                                          UINT64 last_value = 0) {
  return *str ? func_hash_bkdr_constexpr(str + 1,
                                         last_value * RT_HASH_SEED + *str)
              : last_value;
}
