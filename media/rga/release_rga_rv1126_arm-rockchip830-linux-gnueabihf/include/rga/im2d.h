...
#define wrapbuffer_virtualaddr(vir_addr, width, height, format, ...)           \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_buffer =                                                        \
          wrapbuffer_virtualaddr_t(vir_addr, width, height, im2d_api_args[0],  \
                                   im2d_api_args[1], format);                  \
    } else {                                                                   \
...
#define wrapbuffer_physicaladdr(phy_addr, width, height, format, ...)          \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_buffer =                                                        \
          wrapbuffer_physicaladdr_t(phy_addr, width, height, im2d_api_args[0], \
                                    im2d_api_args[1], format);                 \
    } else {                                                                   \
      printf("invalid parameter\n");                                           \
    }                                                                          \
...
#define wrapbuffer_fd(fd, width, height, format, ...)                          \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_buffer = wrapbuffer_fd_t(fd, width, height, im2d_api_args[0],   \
                                        im2d_api_args[1], format);             \
    } else {                                                                   \
      printf("invalid parameter\n");                                           \
    }                                                                          \
  ...
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      rga_check_perpare((rga_buffer_t *)(&src), (rga_buffer_t *)(&dst),        \
                        ...
                        (im_rect *)(&im2d_api_pat_rect), 0);                   \
      im2d_api_ret = imcheck_t(src, dst, im2d_api_pat, src_rect, dst_rect,     \
                               im2d_api_pat_rect, 0);                          \
    } else if (im2d_api_argc == 1) {                                           \
      rga_check_perpare((rga_buffer_t *)(&src), (rga_buffer_t *)(&dst),        \
                        (rga_buffer_t *)(&im2d_api_pat),                       \
                        (im_rect *)(&src_rect), (im_rect *)(&dst_rect),        \
                        (im_rect *)(&im2d_api_pat_rect), im2d_api_args[0]);    \
      im2d_api_ret = imcheck_t(src, dst, im2d_api_pat, src_rect, dst_rect,     \
                               im2d_api_pat_rect, im2d_api_args[0]);           \
    } else {                                                                   \
...
#define imcheck_composite(src, dst, pat, src_rect, dst_rect, pat_rect, ...)    \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_ret =                                                           \
          imcheck_t(src, dst, pat, src_rect, dst_rect, pat_rect, 0);           \
    } else if (im2d_api_argc == 1) {                                           \
      rga_check_perpare((rga_buffer_t *)(&src), (rga_buffer_t *)(&dst),        \
                        (rga_buffer_t *)(&pat), (im_rect *)(&src_rect),        \
                        (im_rect *)(&dst_rect), (im_rect *)(&pat_rect),        \
                        im2d_api_args[0]);                                     \
      im2d_api_ret = imcheck_t(src, dst, pat, src_rect, dst_rect, pat_rect,    \
                               im2d_api_args[0]);                              \
    } else {                                                                   \
      im2d_api_ret = IM_STATUS_FAILED;                                         \
      printf("check failed\n");                                                \
...
#define imresize(src, dst, ...)                                                \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_ret = imresize_t(src, dst, im2d_api_args[0], im2d_api_args[1],  \
                                INTER_LINEAR, 1);                              \
    } else if (im2d_api_argc == 3) {                                           \
      im2d_api_ret = imresize_t(src, dst, im2d_api_args[0], im2d_api_args[1],  \
                                (int)im2d_api_args[2], 1);                     \
    } else if (im2d_api_argc == 4) {                                           \
      im2d_api_ret = imresize_t(src, dst, im2d_api_args[0], im2d_api_args[1],  \
                                (int)im2d_api_args[2], (int)im2d_api_args[3]); \
    } else {                                                                   \
...
#define imblend(srcA, dst, ...)                                                \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      ...
      im2d_api_ret =                                                           \
          imblend_t(srcA, srcB, dst, im2d_api_args[0], im2d_api_args[1]);      \
    } else {                                                                   \
      im2d_api_ret = IM_STATUS_INVALID_PARAM;                                  \
      printf("invalid parameter\n");                                           \
    }                                                                          \
...
#define imcolorkey(src, dst, range, ...)                                       \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      im2d_api_ret =                                                           \
          imcolorkey_t(src, dst, range, IM_ALPHA_COLORKEY_NORMAL, 1);          \
    } else if (im2d_api_argc == 1) {                                           \
      im2d_api_ret = imcolorkey_t(src, dst, range, im2d_api_args[0], 1);       \
    } else if (im2d_api_argc == 2) {                                           \
      im2d_api_ret =                                                           \
          imcolorkey_t(src, dst, range, im2d_api_args[0], im2d_api_args[1]);   \
    } else {                                                                   \
      im2d_api_ret = IM_STATUS_INVALID_PARAM;                                  \
      printf("invalid parameter\n");                                           \
    }                                                                          \
...
#define imcvtcolor(src, dst, sfmt, dfmt, ...)                                  \
  ({                                                                           \
    ...
    if (im2d_api_argc == 0) {                                                  \
      im2d_api_ret =                                                           \
          imcvtcolor_t(src, dst, sfmt, dfmt, IM_COLOR_SPACE_DEFAULT, 1);       \
    } else if (im2d_api_argc == 1) {                                           \
      im2d_api_ret = imcvtcolor_t(src, dst, sfmt, dfmt, im2d_api_args[0], 1);  \
    } else if (im2d_api_argc == 2) {                                           \
      im2d_api_ret = imcvtcolor_t(src, dst, sfmt, dfmt, im2d_api_args[0],      \
                                  im2d_api_args[1]);                           \
    } else {                                                                   \
      im2d_api_ret = IM_STATUS_INVALID_PARAM;                                  \
      printf("invalid parameter\n");                                           \
