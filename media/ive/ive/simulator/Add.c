...
RK_VOID AddSample(const RK_CHAR *moduleName, RK_CHAR *fileName1, RK_CHAR *fileName2,
    ...
    memset(&stCtrl, 0, sizeof(IVE_ADD_CTRL_S));
    ...
    stCtrl.u0q16X = 10000;
    stCtrl.u0q16Y = RK_MAX_U16 + 1 - stCtrl.u0q16X;

    dTime = (RK_DOUBLE)RK_GetTickCount();
    s32Result =
        RK_MPI_IVE_Add(&handle, &stSrc1, &stSrc2, &stDst, &stCtrl, RK_TRUE);
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    ...
    if (outDir == NULL) {
        ...
        RK_SPRINTF(outFile, RK_MAX_FILE_LEN, "%s/rve_%s_add_out.yuv", outDir, moduleName);
        RK_WriteFile(outFile, (void *)stDst.au64VirAddr[0],
                     stDst.u32Width * stDst.u32Height, &fo);
    }

 #if BUILD_SIMULATOR
    gfx_show_img("add", (void *)stDst.au64VirAddr[0],
                 stDst.u32Width, stDst.u32Height,
