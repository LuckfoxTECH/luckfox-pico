...
RK_VOID SADSample(const RK_CHAR *moduleName, RK_CHAR *fileName1, RK_CHAR *fileName2,
    ...
    memset(&stCtrl, 0, sizeof(IVE_SAD_CTRL_S));
    ...
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    memset(stSrc2.au64VirAddr[0], 0, u32Width * u32Height);

    s32Result =
        RK_CreateIveImage(&stDst, IVE_IMAGE_TYPE_U8C1, u32Width/8, u32Height/8);
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    ...
    stCtrl.enMode = IVE_SAD_MODE_MB_8X8;
    stCtrl.enOutMode = IVE_SAD_OUT_MODE_SAD;
    stCtrl.enOutBits = IVE_SAD_OUT_8_BITS;
    stCtrl.u16Thr = 128;
    stCtrl.u8MinVal = 16;
    stCtrl.u8MaxVal = 236;
    ...
    s32Result =
        RK_MPI_IVE_SAD(&handle, &stSrc1, &stSrc2, &stDst, NULL, &stCtrl, RK_TRUE);
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    dTime = (RK_DOUBLE)RK_GetTickCount() - dTime;

    printf("SAD,time = %g ms\n",
