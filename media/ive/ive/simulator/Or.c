...
RK_VOID OrSample(const RK_CHAR *moduleName, RK_CHAR *fileName1, RK_CHAR *fileName2,
    ...
    s32Result = RK_MPI_IVE_Or(&handle, &stSrc1, &stSrc2, &stDst, RK_TRUE);
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    ...
    if (outDir == NULL) {
        ...
        RK_SPRINTF(outFile, RK_MAX_FILE_LEN, "%s/rve_%s_or_out.yuv", outDir, moduleName);
        RK_WriteFile(outFile, (void *)stDst.au64VirAddr[0],
                     stDst.u32Width * stDst.u32Height, &fo);
    }
#if BUILD_SIMULATOR
    gfx_show_img("or", (void *)stDst.au64VirAddr[0], stDst.u32Width,
