...
RK_VOID CastSample(const RK_CHAR *moduleName, RK_CHAR *fileName, RK_CHAR *outDir,
    ...
    IVE_8BIT_TO_8BIT_CTRL_S stCtrl;
    RK_S32 s32Result;
    ...
    stCtrl.enMode = IVE_8BIT_TO_8BIT_MODE_U8_TO_U8;
    stCtrl.u8Denominator = 255;
    stCtrl.u8Numerator = 128;
    stCtrl.s8Bias = 0;

    dTime = (RK_DOUBLE)RK_GetTickCount();
    s32Result =
        RK_MPI_IVE_8BitTo8Bit(&handle, &stSrc, &stDst, &stCtrl, RK_TRUE);
    RK_CHECK_ET_GOTO(s32Result, RK_FAILURE, FAILURE);
    dTime = (RK_DOUBLE)RK_GetTickCount() - dTime;

    printf("cast ,time = %g ms\n",
 ...
 #if BUILD_SIMULATOR
    gfx_show_img("cast", (void *)stDst.au64VirAddr[0], stDst.u32Width,
