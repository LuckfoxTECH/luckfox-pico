...
static RK_S32 test_venc_init(int chnId, int width, int height, int buf_width, int buf_height, RK_CODEC_ID_E enType) {
	printf("========%s========\n", __func__);
	...
	if (enType == RK_VIDEO_ID_AVC) {
		...
		stAttr.stRcAttr.stH264Cbr.u32SrcFrameRateNum = 25;
		...
		stAttr.stRcAttr.stH265Cbr.u32SrcFrameRateNum = 25;
		...
		stAttr.stRcAttr.stMjpegCbr.u32SrcFrameRateNum = 25;
		stAttr.stRcAttr.stMjpegCbr.u32SrcFrameRateDen = 1;
		stAttr.stRcAttr.stMjpegCbr.fr32DstFrameRateNum = 30;
		stAttr.stRcAttr.stMjpegCbr.fr32DstFrameRateDen = 1;
	}
	...
	stAttr.stVencAttr.u32VirWidth = buf_width;
	stAttr.stVencAttr.u32VirHeight = buf_height;
...
int vi_dev_init(int buf_width, int buf_height) {
	printf("%s\n", __func__);
	int ret = 0;
	int devId = 0;
	int pipeId = devId;
	...
	stBindPipe.bDataOffline = 1;
	stBindPipe.bUserStartPipe[0] = 1;
	// 0. get dev config status
	ret = RK_MPI_VI_GetDevAttr(devId, &stDevAttr);
	if (ret == RK_ERR_VI_NOT_CONFIG) {
		// 0-1.config dev
		stDevAttr.stMaxSize.u32Width = buf_width;
		stDevAttr.stMaxSize.u32Height = buf_height;
		stDevAttr.enBufType = VI_V4L2_MEMORY_TYPE_DMABUF;
		stDevAttr.u32BufCount = 2;
	...
	if (ret != RK_SUCCESS) {
		...
		RK_MPI_VI_CreatePipe(0, &stPipeAttr);
...
int vi_chn_init(int channelId, int width, int height, int buf_width, int buf_height) {
	int ret;
	int buf_cnt = 2;
	// VI init
	...
	vi_chn_attr.stIspOpt.stWindow.u32Width = buf_width;
	vi_chn_attr.stIspOpt.stWindow.u32Height = buf_height;
	...
	vi_chn_attr.stShareBufChn.s32ChnId = -1;
...
static RK_CHAR optstr[] = "?::a::w:h:c:I:e:b:W:H:";
static void print_usage(const RK_CHAR *name) {
	printf("Usage example:\n");
	printf("\t%s -I 0 -w 1920 -h 1080 (rtsp://ip/live/0)\n", name);
	printf("\t-w | --width: VI width, Default:2304\n");
	printf("\t-h | --heght: VI height, Default:1296\n");
	printf("\t-W | --buffer_width: buffer width, Default: 2304\n");
	printf("\t-H | --buffer_heght: buffer height, Default: 1296\n");
...
int main(int argc, char *argv[]) {
	...
	RK_U32 u32Width = 2304;
	RK_U32 u32Height = 1296;
	RK_U32 u32BufWidth = 2304;
	RK_U32 u32BufHeight = 1296;
	...
	if (iq_dir) {
		...
		rk_aiq_uapi2_sysctl_preInit_rkrawstream_info(
			aiq_static_info.sensor_info.sensor_name, &info);
	...
	vi_dev_init(u32BufWidth, u32BufHeight);
	vi_chn_init(s32chnlId, u32Width, u32Height, u32BufWidth, u32BufHeight);

	// venc  init
	test_venc_init(0, u32Width, u32Height, u32BufWidth, u32BufHeight,
